...
typedef enum ChiBufferFormat
{
    ...
    ChiFormatYUV422NV16      = 5,       ///< YUV 422 format as described by the NV16 fourcc
    ...
    ChiFormatRawMIPI8        = 19,      ///< 8BPP: Each pixel occupies one bytes, starting at LSB.
                                        ///  Output width of image has no restrictions.
    ChiFormatFDY8            = 20,      ///< Luma only, 8 bits per pixel for FD.
    ChiFormatFDYUV420NV12    = 21,      ///< YUV 420 format as described by the NV12 fourcc FD.
    ...
    ChiFormatARGB            = 26,      ///< ARGB format, each pixel occupies 4 or 8 bytes.
                                        ///< ARGB8888 and ARGB2101010 are 4 bytes, ARGB16161616 are 8 bytes
    ChiFormatYUV422YUYV      = 27,      ///< YUV 422 format as described by the YUYV pattern
} CHIBUFFERFORMAT;
...
struct CHIBufferManagerCreateData
{
    ...
    const ChiStream*    pChiStream;             ///< Stream info needed for image format parameters initialization
};
...
typedef struct ChiPrivateData
{
    ...
    CHIBUFFERINFO          inputBuffers[BufferQueueDepth];    ///< Input buffer handle pointers for this request
    CHIBUFFERMANAGERHANDLE bufferManagers[BufferQueueDepth];  ///< The buffer managers which the input buffers belong to
} CHIPRIVDATA;
...
/// @brief AF sink Raw format
struct LCRRawformatPorts
{
    UINT inputPortId;                       ///< AF input port id
    UINT format;                           ///< RAW format
    UINT sinkPortId;                        ///< IFE sink port ID
};
...
typedef enum ChiSensorStreamType
{
    ...
    StreamMetaROIHist        = 5                ///< Sensor ROI Histogram Stats stream
} CHISENSORSTREAMTYPE;
...
typedef struct ChiRectEXT
{
    UINT32      left;                           ///< x coordinate for top-left point
    UINT32      top;                            ///< y coordinate for top-left point
    UINT32      right;                          ///< x coordinate for bottom-right point
    UINT32      bottom;                         ///< y coordinate for bottom-right point
} CHIRECTEXT;
