...
class ElementInfo:
    def __init__(self, element, child_ids):
...
class SegmentInfo:
    def __init__(self, has_changes=False, header_str=""):
        logging.info("Creating SegmentInfo")
    ...
    def merge_info_into_pipeline(self, input_filename, output_filename):
        ...
        self.included_segments = {}
        logging.info("Including segments into pipelines")
...
def node_include(root, cwd, depth, segment_info=None, target_info=None):
    ...
    for child_element in root:
        ...
        if INCLUDE_TAG == child_element.tag:
            ...
            logging.info(fmt_str % (type, location))
...
def merge_elements(root_element, elements, assert_equal=False):
    ...
    if len(elements) > 1:
        ...
        logging.info("No elements to merge")
    else:
        logging.info("Elements was empty!")


def match_and_combine(search_root, element_type, match_types, combine_types, remove_existing=True):
    """
    match_and_combine
    ...
    Arguments:
        search_root   {[type]} -- [description]
        element_type  {[type]} -- [description]
        match_types   {[type]} -- [description]
        combine_types {[type]} -- [description]
    ...
    for child_element in list(search_root):
        if child_element.tag == element_type:
            logging.info("Match found: %s", child_element)
            ...
            if match_id not in unique_elements:
                ...
                unique_elements[match_id] = ElementInfo(child_element, elements_to_combine.keys())
            else:
                logging.debug("Found exisiting UID: %s", match_id)
                existing = unique_elements[match_id]
                for combine_element_id in elements_to_combine.keys():
                    if combine_element_id not in list(existing.child_ids):
                        # Update existing element
                        logging.info("Merging %s into %s [%s]", child_element, existing.element, match_id)
                        existing.element.append(elements_to_combine[combine_element_id])
                        existing.child_ids.append(combine_element_id)
                # Remove duplicate
                if remove_existing:
...
def merge_pipeline_elements(pipeline_root):
    try:
        ...
        if main_linkage is not None:
            match_and_combine(main_linkage, "Link", ["SrcPort"], ["DstPort", "BufferProperties", "LinkProperties"])
