    ...
    for segment_type in segment_files:
        ...
        if True == use_composition:
            ...
            if len(segment_file_map.keys()) == 1:
                print("%s: Using [%s] implementation" % (segment_type, COMMON_TARGET))
                files = [segment_file_map[COMMON_TARGET]]
            else:
                print("%s: Overriding [%s] with [%s] implementation" % (segment_type, COMMON_TARGET, target))
