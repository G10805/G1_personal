...
sub GetData
{
    my $xml  = XMLin($_[0], forcearray => [ 'Usecase', 'Pipeline', 'Target', 'Node', 'NodeProperty', 'BufferFlags', 'LinkFlags' , 'TargetFormat' , 'DstPort', 'BypassPortSrcId', 'Link']);
    #print Dumper($xml)."\n";
    #exit;
    my $usecases = $_[1];
...
foreach my $usecase (sort {scalar(@{$a->{targets}}) <=> scalar(@{$b->{targets}})} sort {$a->{Name} cmp $a->{Name}} @{$selfUsecases})
{
    ...
    foreach my $pipeline (sort {$a->{sinkCount} <=> $b->{sinkCount}} sort {$a->{Name} cmp $a->{Name}} @{$usecase->{pipelines}})
    {
        ...
        foreach my $link (@{$pipeline->{links}})
        {
            ...
            foreach my $port (@{$link->{ports}})
            {
                ...
                if ($targetName && (($isSinkPort != 1) && $port->{NodeId} != 4))
                {
                    ...
                    if ($isSource)
                    {
                        ...
                        push(@targetNodeListarray, " {$nodeId, $nodeIn, $nodePort, 0 } ");
                    }
                }
            }
            ...
            foreach my $port (@{$link->{ports}})
            {
                ...
                if ($targetName && (($isSinkPort != 1) && $port->{NodeId} != 4))
                {
                    ...
                    if ($isSink && $isSrcPort)
                    {
                        $sinkTargetList .= "    {\"$targetName\", \&$usecasename\_$targetName\_target, 1, $usecasename\_$pipelinename\_sink_Link_$linkcount"."NodeDescriptors}, // $targetName\n";
                        $numSinkTargets++;
                        print $FILE "static ChiLinkNodeDescriptor $name\_sink_Link_$linkcount"."NodeDescriptors[] =\n{\n";
                        print $FILE "\t{$nodeId, $nodeIn, $nodePort, 0 }, \n";
                        print $FILE "};\n";
                    }
                }
        ...
        my $pipelineString = "    {\"$pipelinename\", { 0, $numNodes, $name"."Nodes, $linkNum, $name"."Links, $isRT,0,0,0,NULL}, $sinkTargets, $sourceTargets},  // $pipeline->{Name}\n";
