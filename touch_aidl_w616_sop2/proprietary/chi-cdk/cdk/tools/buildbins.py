...
def getParserPath(camx_cdk_path):
    ...
    return os.path.join(camx_cdk_path, 'tools', progDir, exeName)
...
class CBuildCodeAndBinary(object):
    ...
    def __init__(self, chicdkroot, args):
        ...
        self.camx_cdk_path          = os.path.join(self._ChiCdkRoot, "cdk")
        ...
        self.camx_parser = os.path.join(self.camx_cdk_path, 'tools', \
                                        progDir, exeName)
        ...
        self.camx_fd_xsd_dir = os.path.join(self.camx_cdk_path, "fd")
        self.camx_sensor_xsd_dir = os.path.join(self.camx_cdk_path, "sensor")
        self.camx_chromatix_xsd_dir = os.path.join(self.camx_cdk_path, \
                                                   "chromatix","XSD")
        ...
        if self._GenCodeDir:
            ...
            self.camx_generated_path = os.path.join(self.camx_cdk_path, "generated")
        ...
        self.camx_vendor_path     = os.path.join(self._ChiCdkRoot, "vendor")
        self.camx_sample_tuning_proj = os.path.join(self.camx_vendor_path,
                                                    "tuning",
                                                    self._DefaultTuning)
        self.camx_default_tuning_proj = os.path.join(self.camx_vendor_path,
                                                     "tuning",
                                                     "default_c7project")
        ...
        yamlPath = os.path.join(self._ChiCdkRoot, 'cdk', 'tools', self._YamlFileName)
        ...
        for binType, bins in self.bin_conf.items():
            for items in bins:
                for name, files in items.items():
                    ...
                    files = [os.path.join(self._ChiCdkRoot, "vendor", f) for f in files]
...
if __name__ == '__main__':
    ...
    yaml_file = "buildbins.yaml"
    if "TARGET_BOARD_PLATFORM" in os.environ:
        product_yaml_file = "buildbins_" + os.environ["TARGET_BOARD_PLATFORM"] + ".yaml"
        if os.path.exists(os.path.join(chiCdkRoot, 'cdk', 'tools', product_yaml_file)):
            yaml_file = product_yaml_file
    ...
    parser.add_argument('--default-tuning-project', help='A tuning poject folder, which will be used to create default tuning binary',
                        action='store', default='imx318_c7project')
    ...
    parser.add_argument('--yaml-file-name', help='yaml file name update for multiple yaml files usage',
                        default=yaml_file, action='store')
