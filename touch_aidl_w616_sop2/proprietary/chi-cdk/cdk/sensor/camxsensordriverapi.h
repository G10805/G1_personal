...
static const UINT8  MaxImageFaceROIs     = 10;   ///< Maximum index for Face ROIs
...
typedef struct RegSetting
{
    ...
    UINT32                slaveAddrExists;  ///< flag that specifies if slave address exists
    UINT16                slaveAddr;        ///< slave address
#endif
} RegSetting;
...
/* TODO: WE SHOULD TRY TO AVOID DUPLICATE DEFINITIONS OF THESE STRUCTURES */

/// @brief Structure describing Rectangle
typedef struct
{
...
typedef struct
{
    ...
    FDRectangle            faceRect;     ///< Detected Face rectangle
} SensorFaceROIData;

/// @brief Structure describing Face ROI info
typedef struct
{
    UINT64         requestId;                         ///< Request ID corresponding to this results
    UINT32         ROICount;                          ///< Number of ROIs
    SensorFaceROIData    unstabilizedROI[MaxImageFaceROIs]; ///< Unstabilized Face ROI data
    SensorFaceROIData    stabilizedROI[MaxImageFaceROIs];   ///< Stabilized Face ROI data
} SensorFaceROIInformation;
...
typedef struct SensorLibraryAPI
{
    ...
    /// FillFDSettings
    ///
    /// @brief  Fill the FD settings
    ///
    /// @param  pRegSettingsInfo    pointer to sensor register settings to be filled
    /// @param  pFDData             pointer to FD data to fill exposure reg settings
    ///
    /// @return TRUE if successful else FALSE
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    BOOL (*pFillFDSettings)(
        RegSettingsInfo*          pRegSettingsInfo,
        SensorFaceROIInformation* pFDData);         ///< pointer to FillFDSettings function
