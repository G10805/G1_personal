...
static uint32_t GetBufferSize(uint32_t width, uint32_t height, uint32_t stride, uint32_t format)
{
    uint32_t size  = 0;
    uint32_t slice = ALIGN(height, 64u);

    switch (format)
    {
        case HAL_PIXEL_FORMAT_YCbCr_420_888:
            size = stride * slice * 1.5f;
            break;
    ...
    uint32_t           index)
{
    ...
    buffer_handle_t hnl      = *pAllocatedBuffer;
    uint32_t        buffSize = GetBufferSize(width, height, *pStride, format);
    mBufferinfo[index].vaddr = mmap(NULL, buffSize, PROT_READ  | PROT_WRITE, MAP_SHARED, hnl->data[0], 0);
    mBufferinfo[index].size  = buffSize;
    ALOGI("vaddr: %p fd: %d, size: %u WxH: %dx%d, stride %d fromat %d",
          mBufferinfo[index].vaddr, hnl->data[0], buffSize, width, height, *pStride, format);
