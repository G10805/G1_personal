...
void DumpFrame(CameraStream *stream, buffer_handle_t *buf,int frameNum,int dumpType)
{
    char fname[256];
    if (dumpType)
        snprintf(fname, sizeof(fname), "%s/camera_video_%d.%s",CAMERA_STORAGE_DIR,frameNum, "yuv");
    else
        snprintf(fname, sizeof(fname), "%s/camera_preview_%d.%s",CAMERA_STORAGE_DIR,frameNum, "yuv");
    ...
    camera3_capture_result *result)
{
    ...
    for (uint32_t i = 0;i < result->num_output_buffers ;i++) {
        ...
        if (stream->streamType == CAMERA3_TEMPLATE_STILL_CAPTURE) {
            snprintf(fname, sizeof(fname), "%s/camera_capture_%d.%s",CAMERA_STORAGE_DIR, result->frame_number,
                    (HAL_PIXEL_FORMAT_RAW16 == mConfig->mSnapshotStream.format) ? "raw" : "jpg");
            FILE* fd = fopen(fname, "wb");
