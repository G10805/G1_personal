    ...
    for (uint32_t i = 0;i < result->num_output_buffers ;i++) {
        ...
        if (stream->streamType == CAMERA3_TEMPLATE_STILL_CAPTURE) {
            ...
            if (!(!mConfig->mZslEnabled && result->frame_number < NONZSL_STABLE_FRAME_COUNT)
                ...
                t = localtime(&timer);
                snprintf(fname, sizeof(fname), "%s/camera_capture_%d_%4d_%02d_%02d_%02d_%02d_%02d.%s",
                        CAMERA_STORAGE_DIR, result->frame_number,
        ...
        if (stream->streamType == CAMERA3_TEMPLATE_PREVIEW && testsnap->mDumpPreviewNum > 0) {
            ...
            snprintf(fname, sizeof(fname), "%s/camera_preview_%d_%4d_%02d_%02d_%02d_%02d_%02d.%s",
