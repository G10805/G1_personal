...
VOID UsecaseQuadCFA::Destroy(BOOL isForced)
{
    ...
    {
        ChxUtils::AndroidMetadata::FreeMetaData(static_cast<camera_metadata_t*>(m_pOfflineInputMeta));
    ...
    camera3_stream_configuration_t* pStreamConfig) ///< Stream configuration
{
    ...
    createData.pChiStream                   = m_pQCFARawStream;

    m_pRDIBufferManager = CHIBufferManager::Create("RDIBufferManager", &createData);
    ...
    if (NULL != m_pRDIBufferManager)
    {
        for (UINT queueIndex = 0; queueIndex < BufferQueueDepth; queueIndex++)
        {
            ...
            if (NULL != m_QCFARawQueue[queueIndex].pRdiOutputBuffer)
            {
                m_QCFARawQueue[queueIndex].pRdiOutputBuffer->size                = sizeof(CHISTREAMBUFFER);
                m_QCFARawQueue[queueIndex].pRdiOutputBuffer->acquireFence.valid  = FALSE;
                m_QCFARawQueue[queueIndex].pRdiOutputBuffer->bufferInfo.phBuffer = NULL;
            }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// UsecaseQuadCFA::ResetMetadataStatus
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID UsecaseQuadCFA::ResetMetadataStatus(
    ...
    camera3_capture_request_t* pRequest)
{
    ...
    if (ChiStreamFormatBlob == pRequest->output_buffers[0].stream->format)
    {
        ...
        if (NULL != m_pCachedMeta)
        {
            ...
            if (TRUE == ChxUtils::AndroidMetadata::IsVendorTagPresent(reinterpret_cast<const VOID*>(m_pOfflineInputMeta),
                                                                      DebugDataTag))
            {
                CHAR* pData = NULL;
                ChxUtils::AndroidMetadata::GetVendorTagValue((VOID*)(m_pOfflineInputMeta), DebugDataTag, (VOID**)&pData);
                ...
                if ((NULL != pDebug) && (NULL != pDebug->pData) && (0 < pDebug->size))
                {
                    ...
                    if (NULL != m_debugDataOffline.pData)
                    {
                        ...
                        result = ChxUtils::AndroidMetadata::SetVendorTagValue(m_pOfflineInputMeta,
                                                             DebugDataTag,
                                                             sizeof(DebugData),
                                                             &m_debugDataOffline);
                        if (CDKResultSuccess != result)
        ...
        m_pSnapshotInputMeta = ChxUtils::AndroidMetadata::AllocateCopyMetaData(pRequest->settings);
        ...
        CHX_LOG("after rdi ready, sending buffer:type %d handle %p to reprocess",
                m_QCFARawQueue[0].pRdiOutputBuffer->bufferInfo.bufferType,
                m_QCFARawQueue[0].pRdiOutputBuffer->bufferInfo.phBuffer);
    ...
    {
        if (FALSE == m_pSession[m_previewIdx]->IsPipelineActive())
        {
            CHX_LOG_INFO("activate preview pipeline");
            result = ExtensionModule::GetInstance()->ActivatePipeline(m_pSession[m_previewIdx]->GetSessionHandle(),
                m_pSession[m_previewIdx]->GetPipelineHandle());
            if (CDKResultSuccess == result)
            {
                m_pSession[m_previewIdx]->SetPipelineActivateFlag();
            }
        }

        // for preview request
        CHICAPTUREREQUEST chiRequest    = { 0 };
        CHISTREAMBUFFER*  pOutputBuffer = static_cast<CHISTREAMBUFFER*>(
        ...
        if (CamxResultSuccess == result)
        {
            ...
            {
                ChxUtils::PopulateHALToChiStreamBuffer(&pRequest->output_buffers[i], &pOutputBuffer[i]);
            }
            ...
            chiRequest.pOutputBuffers    = pOutputBuffer;
            chiRequest.pMetadata         = pRequest->settings;

            UINT32 sensorModeIndex = m_pSession[m_previewIdx]->GetSensorModeInfo(0)->modeIndex;
            ChxUtils::AndroidMetadata::FillTuningModeData(const_cast<VOID*>(chiRequest.pMetadata),
                                         pRequest,
                                         sensorModeIndex,
                                         &m_effectModeValue,
                                         &m_sceneModeValue,
            ...
            ChxUtils::AndroidMetadata::FillCameraId(const_cast<camera_metadata_t*>(pRequest->settings), m_cameraId);
            ...
            MetadataCaptureRequestUpdate(chiRequest, m_previewIdx, true);
    ...
    camera3_capture_request_t* pRequest)
{
    ...
    inputBuffer.bufferInfo          = m_QCFARawQueue[0].pRdiOutputBuffer->bufferInfo;
    inputBuffer.pStream             = m_pQCFARawStream;

    ChxUtils::PopulateHALToChiStreamBuffer(pRequest->output_buffers, &outputBuffer);
    ...
    {
        INT status = 0;
        status = ChxUtils::AndroidMetadata::MergeMetadata(m_QCFARawQueue[0].pMetadata,
    ...
    ChxUtils::AndroidMetadata::FillTuningModeData(const_cast<VOID*>(chiRequest.pMetadata),
                                ...
                                &m_tuningFeature1Value,
                                &m_tuningFeature2Value);
    ChxUtils::AndroidMetadata::FillCameraId(const_cast<VOID*>(chiRequest.pMetadata), m_cameraId);
    CHX_LOG("sensorModeIndex:%d, camera id:%d", sensorModeIndex, m_cameraId);

    ChiTuningModeParameter* pTunningModeData = NULL;
    ChxUtils::AndroidMetadata::GetVendorTagValue(const_cast<VOID*>(chiRequest.pMetadata),
    ...
    MetadataCaptureRequestUpdate(chiRequest, m_offlineIdx, false);
    result = SubmitRequest(&submitRequest);
    ...
    if (NULL != m_pSnapshotInputMeta)
    {
        ChxUtils::AndroidMetadata::FreeMetaData(static_cast<camera_metadata_t*>(m_pSnapshotInputMeta));
        m_pSnapshotInputMeta = NULL;
    }
    ...
    camera3_capture_request_t* pRequest)
{
    ...
    {
        INT status = 0;
        status = ChxUtils::AndroidMetadata::MergeMetadata(reinterpret_cast<camera_metadata_t*>(m_pSnapshotInputMeta),
    ...
    MetadataCaptureRequestUpdate(chiRequest, m_OfflineYuvIdx, false);
    result = SubmitRequest(&submitRequest);
    ...
    camera3_capture_request_t* pRequest)
{
    ...
    if (FALSE == m_pSession[m_QCFARawIdx]->IsPipelineActive())
    {
        CHX_LOG_INFO("activate full size raw pipeline");
        result = ExtensionModule::GetInstance()->ActivatePipeline(m_pSession[m_QCFARawIdx]->GetSessionHandle(),
            m_pSession[m_QCFARawIdx]->GetPipelineHandle());
        if (CDKResultSuccess == result)
        {
            m_pSession[m_QCFARawIdx]->SetPipelineActivateFlag();
        }
    }

    outputBuffer.size               = sizeof(CHISTREAMBUFFER);
    outputBuffer.acquireFence.valid = FALSE;
    outputBuffer.bufferInfo         = m_pRDIBufferManager->GetImageBufferInfo();
    outputBuffer.pStream            = m_pQCFARawStream;
    ...
    MetadataCaptureRequestUpdate(chiRequest, m_QCFARawIdx, false);
    result = SubmitRequest(&submitRequest);
    ...
    VOID*             pPrivateCallbackData)
{
    ...
    if(FlushStatus::NotFlushing != pUsecaseQuadCfa->GetFlushStatus())
    {
        //ignore IR result when in destory and dont process flushed result
        return ;
    }

    pUsecaseQuadCfa->MetadataCaptureResultGet(*pResult, pSessionPrivateData->sessionId);
    ...
    pUsecaseQuadCfa->MetadataCaptureResultRelease(pResult->pResultMetadata);
}
    ...
    ChiCaptureResult* pResult)
{
    ...
    camera3_capture_result_t captureResult = { 0 };
    ChxUtils::Memcpy(&captureResult, pResult, sizeof(camera3_capture_result_t));
    camera3_stream_buffer_t outputBuffers = { 0 };
    ChxUtils::PopulateChiToHALStreamBuffer(&pResult->pOutputBuffers[0], &outputBuffers);
    captureResult.output_buffers = &outputBuffers;
    ...
    ChiCaptureResult* pResult)
{
    ...
    if (pResult->pResultMetadata != NULL)
    {
                                                                  ...
                                                                  DebugDataTag))
        {
            ...
            if ((NULL != pDebug) && (NULL != pDebug->pData) && (0 < pDebug->size))
            {
                result = ChxUtils::AndroidMetadata::SetVendorTagValue((VOID*)m_QCFARawQueue[0].pMetadata, DebugDataTag,
                                                                      sizeof(DebugData), pDebug);
                if (CDKResultSuccess != result)
                {
        ...
        camera3_capture_result_t captureResult = { 0 };
        ChxUtils::Memcpy(&captureResult, pResult, sizeof(camera3_capture_result));
        captureResult.num_output_buffers = 0;
        captureResult.output_buffers     = NULL;
    ...
    for (UINT i = 0 ; i < pResult->numOutputBuffers; i++)
    {
        if (pResult->pOutputBuffers[i].pStream == m_pQCFARawStream)
        {
            ...
            m_QCFARawQueue[0].pRdiOutputBuffer->acquireFence.valid  = FALSE;
            m_QCFARawQueue[0].pRdiOutputBuffer->size                = sizeof(CHISTREAMBUFFER);
            m_QCFARawQueue[0].frameNumber                           = pResult->frameworkFrameNum;
    ...
    {
        CHX_LOG("notify RDI buffer is ready, buffer type %d handler:%p!",
                m_QCFARawQueue[0].pRdiOutputBuffer->bufferInfo.bufferType,
                m_QCFARawQueue[0].pRdiOutputBuffer->bufferInfo.phBuffer);
    ...
    ChiCaptureResult* pResult)
{
    ...
    // Release reference to the input RDI buffer
    m_pRDIBufferManager->ReleaseReference(&(m_QCFARawQueue[0].pRdiOutputBuffer->bufferInfo));

    for (UINT i = 0 ; i < pResult->numOutputBuffers; i++)
    {
        if (pResult->pOutputBuffers[i].pStream == m_pSnapshotStream)
        {
            CHX_LOG("SNAPSHOT buffer received, buffer type %d handler:%p!",
            ...
            ChxUtils::Memcpy(&captureResult, pResult, sizeof(camera3_capture_result));
            camera3_stream_buffer_t outputBuffers = { 0 };
            ChxUtils::PopulateChiToHALStreamBuffer(&pResult->pOutputBuffers[i], &outputBuffers);
            captureResult.output_buffers = &outputBuffers;
    ...
    ChiCaptureResult* pResult)
{
    ...
    for (UINT i = 0 ; i < pResult->numOutputBuffers; i++)
    {
        ...
        {
            CHX_LOG("Yuv SNAPSHOT buffer received, buffer type %d handler:%p!",
                    pResult->pOutputBuffers[i].bufferInfo.bufferType,
                    pResult->pOutputBuffers[i].bufferInfo.phBuffer);
    ...
    UINT32 pipelineIdx)
{
    ...
    if (NULL != m_pSession[pipelineIdx])
    {
        m_pSession[pipelineIdx]->SetPipelineDeactivate();
