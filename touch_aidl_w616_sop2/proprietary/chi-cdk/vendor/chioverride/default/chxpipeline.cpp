...
CDKResult Pipeline::CreateDescriptor()
{
    ...
    CHX_LOG_CONFIG("Pipeline[%s] pipeline pointer %p numInputs=%d, numOutputs=%d stream wxh=%dx%d",
...
VOID Pipeline::SetTuningUsecase()
{
    ...
    ChxUtils::SetVendorTagValue(m_pPipelineDescriptorMetadata, TuningMode, sizeof(ChiTuningModeParameter), &chiTuningModeParameter);
}
