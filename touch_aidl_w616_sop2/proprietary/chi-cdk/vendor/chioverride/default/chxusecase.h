...
union UsecaseRequestFlags
{
    struct
    {
        ...
        BIT reserved                    : 25; ///< Reserved for future use
    };
    UINT value;
};
...
public:
    ...
    CDKResult FlushAllSessions(Session* const * ppSession);
    CDKResult FlushAllSessionsInParallel(Session* const * ppSession);
        ...
        UINT32        frameNumber)
    {
        ...
        {
            if (((TRUE == pPipelineData->pPipeline->IsRealTime()) &&
                (numPCRsBeforeStreamOn <= frameNumber)) ||
                (FALSE  == pPipelineData->pPipeline->IsRealTime()))
            {
                result = ExtensionModule::GetInstance()->ActivatePipeline(
                    pSession->GetSessionHandle(), pSession->GetPipelineHandle());
...
private:
    ...
    UINT32                      m_numBuffersInErrorState[MaxOutstandingRequests];       ///< Number of buffers returned with error
};
