            ...
            for (UINT32 idx = 0; idx < m_numLinkedSessions; idx++)
            {
                 if (defaultCamIdx == idx)
                 {
                     CHX_LOG("Default Camera id %d, index %d", m_linkedCameraInfo[idx].cameraId, idx);
                      ...
                      translatedZoom->zoomPreview.totalZoom[idx] = *userZoom;
                 }
                 else
                 {
        ...
        CHIRECTEXT& dstRect)
{
    ...
    if (NULL == m_libHandle)
    {
        ...
        UINT32 xMin = srcRect.left;
        UINT32 yMin = srcRect.top;
        UINT32 xMax = srcRect.right;
        UINT32 yMax = srcRect.bottom;
        ...
        else
        {
            ...
            xMin += alignmentWidth;
            xMax += alignmentWidth;
            yMin += alignmentHeight;
            yMax += alignmentHeight;
