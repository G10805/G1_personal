...
const static UINT32 MaxRealTimePipelines   = 4;
...
public:
        ...
        CHIHANDLE phSession) const
    {
        ...
        {
            // Avoid NULL pointer dereference in QCFA Snapshot flush case
            if ((NULL != m_sessions[sessionId].pSession) &&
                (phSession == m_sessions[sessionId].pSession->GetSessionHandle()))
            {
                result = sessionId;
                break;
            }
    ...
    {
        return ((1 < ExtensionModule::GetInstance()->GetNumBatchedFrames()) ? TRUE : FALSE);
    }
...
protected:
    ...
    static const UINT32 MaxNumOfTargets = 60;
...
private:
    ...
    CDKResult UpdateMetadataWithSnapshotSettings(
        ChiMetadata& rMetadata);

    /// Replace the capture result metadata with input request metadata
    CDKResult UpdateMetadataWithInputSettings(
...
public:
    ...
    */
    ...
    CHX_INLINE BOOL IsIHDRSnapshotNeeded()
    {
        return m_isIHDRSnapshotNeeded;
    }
...
protected:
    ...
    BOOL                m_isIHDRSnapshotNeeded;            ///< Flag to indicate IHDR snapshot
