...
static const FLOAT  AECGAIN_THRESHOLD            = 3.0f;
static const CHAR*  ZSL_USECASE_NAME             = "UsecaseZSL";

static inline BOOL IsThisClonedStream(ChiStream** ppClonedStream, ChiStream* pActualStream, UINT *pIndex)
{
...
static ChiUsecase* pAdvancedUsecase  = NULL;
static ChiUsecase* pZslTuningUsecase = g_pUsecaseZSLTuning;

static const INT32 AdvancedPipelineIdMapping[] =
{
...
static const INT32 AdvancedPipelineIdTuningMapping[] =
{
    ...
    for (UINT32 descIndex = 0; descIndex < m_pChiUsecase->numPipelines; descIndex++)
    {
        ...
        for (UINT32 targetIndex = 0; targetIndex < pDesc->sinkTarget.numTargets; targetIndex++)
        {
            ...
            if (NULL != pTarget)
            {
                if (TRUE == StreamIsInternal(pTarget->pChiStream))
                {
                    CHX_LOG_CONFIG("Pipeline[%s], %s,Create CHI buffers for targets [%d/%d], "
                        "chistream:%p, format:%d, %dx%d",
                        pDesc->pPipelineName,
                        pDesc->sinkTarget.pTargetPortDesc[targetIndex].pTargetName,
                        targetIndex,
                    ...
                    createData.consumerFlags        = ChiGralloc1ConsumerUsageCamera;
                    ...
                    if (TRUE == IsFdStream(pTarget->pChiStream))
                    {
                        ...
                        createData.producerFlags |= ChiGralloc1ProducerUsageCpuWrite;
                        createData.consumerFlags |= ChiGralloc1ProducerUsageCpuRead;
    ...
    UINT32 bufferID)
{
    if (MaxChiStreams > bufferID)
    {
        ...
        for (UINT32 i = 0; i< BufferQueueDepth; i++)
        {
    ...
    CHIBUFFERHANDLE phBuffer)
{
    ...
    UINT32          queueIndex      = frameNumber % BufferQueueDepth;
    ...
    BOOL                isBufferReady)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            UINT32 releasedBufferIdx = (frameNumber - pTargetBuffer->validBufferLength) % BufferQueueDepth;
    ...
    if (INVALIDSEQUENCEID != snapshotRequestID)
    {
        ...
        if ((NULL != pTargetRDIBuffer) && (NULL != pTargetFDBuffer))
        {
            ...
            for (UINT32 i = 0; i < pInputInfo->numOfBuffers; i++)
            {
                ...
                bufferIndex      = inputFrameNumber % BufferQueueDepth;
                ...
                if (CDKResultSuccess == result)
                {
                    ...
                    if ((NULL != pSnapshotInputMeta) && (NULL != pInputInfo->pAppMetadata))
                    {
                        ...
                        UpdateMetadataWithInputSettings(*pInputInfo->pAppMetadata, *pSnapshotInputMeta);
            ...
            if (CDKResultSuccess == result)
            {
                if (TRUE == bUpdateValidLength)
                {
                    ...
                    if (pInputInfo->numOfBuffers <= pTargetRDIBuffer->validBufferLength)
                    {
                        ...
                        if ((NULL != pRequestInfo->pFeature) && (FeatureType::MFNR == pRequestInfo->pFeature->GetFeatureType()))
                        {
    ...
    INT32  delta)
{
    ...
    if ((resultVal < 0) || ((UINT32)resultVal > BufferQueueDepth))
    {
    ...
    if (NULL != ppDstMetadata)
    {
        ...
        if (NULL != pDstMetadata)
        {
            if (NULL != pSrcMetadata)
            {
                ...
                UpdateMetadataWithInputSettings(*pSrcMetadata, *pDstMetadata);
            }

            UpdateMetadataWithSnapshotSettings(*pDstMetadata);
    ...
    ChiMetadata&  metadata)
{
    ...
    UINT32 tagId = ExtensionModule::GetInstance()->GetVendorTagId(IsFlashRequiredTag);
    ...
    for (UINT session = 0; session < MaxSessions; session++)
    {
        ...
        for (UINT pipeline = 0; pipeline < m_sessions[session].numPipelines; pipeline++)
        {
            ...
            if (0 < outputCount)
            {
                ...
                // preactivate the pipeline
                CheckAndActivatePipeline(m_sessions[session].pSession,
                                         &m_sessions[session].pipelines[pipeline],
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CDKResult CameraUsecaseBase::UpdateMetadataWithSnapshotSettings(
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// CameraUsecaseBase::UpdateMetadataWithInputSettings
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CDKResult CameraUsecaseBase::UpdateMetadataWithInputSettings(
   ChiMetadata& rInputMetadata,
   ChiMetadata& rOutputMetadata)
{
    CDKResult result = CDKResultSuccess;
    ...
    ChxUtils::GetVendorTagValue(&rInputMetadata, TuningMode, (VOID**)&pData);
    if (NULL != pData)
    {
        ChxUtils::SetVendorTagValue(&rOutputMetadata, TuningMode, sizeof(ChiTuningModeParameter), pData);
    }
    ...
    const ChiMessageDescriptor* pMessageDescriptor)
{
    ...
    switch (pMessageDescriptor->message.errorMessage.errorMessageCode)
    {
        ...
        case MessageCodeBuffer:
            CHX_LOG("Frame %" PRIu64 ": Buffer error for stream %p",
    ...
    const CHAR*     sProcessingStage)
{
    ...
    if (TRUE == ChxUtils::IsVendorTagPresent(pMetadata, DebugDataTag))
    {
        CHAR* pData = NULL;
        ChxUtils::GetVendorTagValue(pMetadata, DebugDataTag, (VOID**)&pData);
        if (NULL != pData)
        {
...
CDKResult AdvancedCameraUsecase::ExecuteFlush()
{
    ...
    for (UINT32 index = 0; index < m_numOfPhysicalDevices; index++)
    {
        ...
        {
            CHX_LOG_INFO("AdvancedCameraUsecase FLUSH started FEATURE TYPE: %d",
...
VOID AdvancedCameraUsecase::Destroy(BOOL isForced)
{
    ...
    if (TRUE == m_isUsecaseCloned)
    {
        UsecaseSelector::DestroyUsecase(m_pChiUsecase);
    }
    ...
    UINT pipelineIndex)      ///< Index of the pipeline created (within the context of the session)
{
    ...
    {
        m_pRawJPEG->PipelineCreated(sessionId, pipelineIndex);
    }
}
    ...
    camera3_capture_request_t* pRequest)
{
    ...
    if (TRUE ==
        ChxUtils::AndroidMetadata::IsVendorTagPresent(reinterpret_cast<const VOID*>(pRequest->settings), VideoHDR10Mode))
    {
        VOID* pData = NULL;
    ...
    if (StreamConfigModeFastShutter == ExtensionModule::GetInstance()->GetOpMode(m_cameraId) && NULL != pRequest->settings)
    {
        ...
        UINT32   FSModeTagId       = ExtensionModule::GetInstance()->GetVendorTagId(FastShutterMode);
    ...
    ChiMetadata* pResultMetadata)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            CHX_LOG_VERBOSE("ParseResultMetadata: isSeamlessIHDRSupported=%u, IHDRTrigger=%u",
            ...
            {
                m_isIHDRSnapshotNeeded = pAECFrameControl->isInsensorHDRSnapshot;
            }
        }
    ...
    VOID*                       pPrivateCallbackData)
{
    if (TRUE == AdvancedFeatureEnabled())
    {
        ...
        Feature* pFeature = FindFeatureToProcessResult(pResult->pPrivData,
                                                       pResult->frameworkFrameNum,
                                                       pPrivateCallbackData);
        if (NULL != pFeature)
        {
            CHX_LOG_CONFIG("pFeature->GetFeatureType() = %d", pFeature->GetFeatureType());
    ...
    VOID*                    pPrivateCallbackData)
{
    ...
    {
        m_pRawJPEG->ProcessDriverPartialCaptureResult(pResult, pPrivateCallbackData);
    }
    else
    {
...
VOID AdvancedCameraUsecase::SelectFeatures(camera3_stream_configuration_t* pStreamConfig)
{
    ...
    if (StreamConfigModeFastShutter == ExtensionModule::GetInstance()->GetOpMode(m_cameraId))
    {
        enabledAdvanceFeatures = AdvanceFeatureSWMF;
    }
    ...
    for (UINT32 physicalCameraIndex = 0 ; physicalCameraIndex < m_numOfPhysicalDevices ; physicalCameraIndex++)
    {
        ...
        {
            m_enabledFeatures[physicalCameraIndex][index] = FeatureYuvCB::Create(this);
            index++;
        }
        else
        {
            if (AdvanceFeatureZSL == (enabledAdvanceFeatures & AdvanceFeatureZSL))
            {
                if (FALSE == IsMultiCameraUsecase())
                {
                    ...
                    {
                        m_enabledFeatures[physicalCameraIndex][index] = FeatureZSL::Create(this);
                        index++;
                    }
                    else
                    {
        ...
        {
            CHX_LOG_INFO("VideoLiveShot only enable featureZSL");
        }
        else
        {
            if ((UsecaseId::PreviewZSL  == m_usecaseId)  ||
                ...
                (UsecaseId::QuadCFA     == m_usecaseId))
            {
                ...
                {
                    m_enabledFeatures[physicalCameraIndex][index] = FeatureHDR::Create(this, physicalCameraIndex);
                    index++;
                }
                ...
                if (AdvanceFeatureMFNR == (enabledAdvanceFeatures & AdvanceFeatureMFNR))
                {
                    ...
                    m_enabledFeatures[physicalCameraIndex][index]  = FeatureMFNR::Create(this, physicalCameraIndex);
                    index++;
                ...
                {
                    m_enabledFeatures[physicalCameraIndex][index] = FeatureMultiframe::Create(this, physicalCameraIndex);
                    index++;
                }

                /* Disable multiframe features
                if (AdvanceFeatureMFSR == (enabledAdvanceFeatures & AdvanceFeatureMFSR))
                {
                    m_enabledFeatures[index]  = FeatureMFSR::Create(this);
                    index++;
                    m_isFDstreamBuffersNeeded = TRUE;
                }
                */
            ...
            if ((UsecaseId::QuadCFA == m_usecaseId) &&
                (AdvanceFeatureQCFA == (enabledAdvanceFeatures & AdvanceFeatureQCFA)))
            {
                // Make sure FeatureZSL is enabled
                // Here FeatureZSL is mandatory for Quad CFA for upscaling mode snapshot

                CHX_LOG_INFO("Creating FeatureQuadCFA");
                m_enabledFeatures[physicalCameraIndex][index] = FeatureQuadCFA::Create(this);

                // For quadra cfa remosaic snapshot, session will be destroyed and recreated,
                // It won't work with early PCR right now.
                // So choose FeatureQuadCFA for preview, which will disable early PCR.
                m_pActiveFeature = m_enabledFeatures[physicalCameraIndex][index];
    ...
    UINT32                     physicalCameraIndex)
{
    ...
    entry.tag = ExtensionModule::GetInstance()->GetVendorTagId(BurstFps);
    ...
    entry.tag = ExtensionModule::GetInstance()->GetVendorTagId(CustomNoiseReduction);
    ...
    else
    {
        for (UINT32 i = 0; i < m_enabledFeaturesCount[physicalCameraIndex]; i++)
        {
            if ((0 == burstShotFps) && (NULL == pRequest->input_buffer)
    ...
    if ((NULL == pFeature) && (NULL != m_enabledFeatures[physicalCameraIndex][0]))
    {
            pFeature = m_enabledFeatures[physicalCameraIndex][0];
    ...
    camera3_stream_configuration_t* pStreamConfig)
{
    ...
    if (UsecaseZSLTuningId == static_cast<UsecaseId3Target>(ExtensionModule::GetInstance()->OverrideUseCase()))
    {
        m_pChiUsecase  = UsecaseSelector::CloneUsecase(pZslTuningUsecase, totalPipelineCount, pipelineIDMap);
    }
    else
    {
        m_pChiUsecase  = UsecaseSelector::CloneUsecase(pAdvancedUsecase, totalPipelineCount, pipelineIDMap);
    }
    ...
    for (UINT32 physicalCameraIndex = 0 ; physicalCameraIndex < m_numOfPhysicalDevices; physicalCameraIndex++)
    {
        ...
        for(UINT32 i = 0 ; i < pipelineCounts[physicalCameraIndex]; i++)
        {
            ...
            {
                if ( (NULL != m_pChiUsecase)  &&
                     (NULL != (m_pChiUsecase->pPipelineTargetCreateDesc)) )
                {
                    SetupSharedPipeline(&m_pChiUsecase->pPipelineTargetCreateDesc[descIndex], pipelines[physicalCameraIndex][i]);
                }
                else
                {
                    CHX_LOG_ERROR("Error: Pipeline TargetCreateDescriptor is NULL");
                }
            }
            else
            {
...
const CHISENSORMODEPICKHINT* AdvancedCameraUsecase::GetSensorModePickHint(UINT pipelineIndex)
{
    ...
    else
    {
        ...
        pSensorModePickHint = &m_defaultSensorModePickHint;
        if (TRUE == selectIHDRUsecase)
        {
            m_defaultSensorModePickHint.sensorModeCaps.value  = 0;
            m_defaultSensorModePickHint.sensorModeCaps.u.IHDR = TRUE;
        }
    }

    // Overwrtie the default sensor mode to HFR if batch mode is enabled.
    if (TRUE == IsBatchModeEnabled())
    {
        m_defaultSensorModePickHint.sensorModeCaps.value = 2;
        m_defaultSensorModePickHint.sensorModeCaps.u.HFR = TRUE;
    }
