...
using  GraphicsRect              = aidl::android::hardware::graphics::common::Rect;
using BufferDescriptorInfo_v4    =
    android::hardware::graphics::mapper::V4_0::IMapper::BufferDescriptorInfo;
using BufferDescriptor_v4        = android::hardware::graphics::mapper::V4_0::BufferDescriptor;
using MetadataType_v4            = ::android::hardware::graphics::mapper::V4_0::IMapper::MetadataType;
using MetadataTypeDescription_v4 =
...
using IQtiAllocator_v4          = ::vendor::qti::hardware::display::allocator::V4_0::IQtiAllocator;
using IQtiMapper_v4             = ::vendor::qti::hardware::display::mapper::V4_0::IQtiMapper;
using IQtiMapperExtensions_v1   = ::vendor::qti::hardware::display::mapperextensions::V1_1::IQtiMapperExtensions;
using MetadataType              = ::android::hardware::graphics::mapper::V4_0::IMapper::MetadataType;

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// @brief Structure to store Mapper and Allocator info
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
struct ChiGrallocInterface
{
    sp<IQtiMapperExtensions>    qtiMapperExtn;      ///< QtiMapper Extension#v1_0
    sp<IQtiAllocator_v4>        qtiAllocatorV4;     ///< QtiAllocator version#4
    sp<IQtiMapper_v4>           qtiMapperV4;        ///< QtiMapper version#4
};
    ...
    const camera3_stream_configuration_t* pStreamConfig)
{
    ...
    ///  - stream[1]: snapshot, format: implemenation_defined, usage: GrallocUsageHwImageEncoder
    if (((TRUE == IsPreviewStream(pStreamConfig->streams[0]))       &&
         (TRUE == IsHEIFStream(pStreamConfig->streams[1]))) ||
        ((TRUE == IsPreviewStream(pStreamConfig->streams[1]))       &&
         (TRUE == IsHEIFStream(pStreamConfig->streams[0]))))
    {
        CHX_LOG_CONFIG("HEIF snapshot case");
        isMatch = TRUE;
    }
    ...
    if (pStreamConfig->num_streams <= ChiMaxNumTargets)
    {
        ...
        else if ((TRUE == secureMode) && (NULL == pSelectedUsecase))
        {
            ...
            case 1:
                ...
                else if (TRUE == IsMatchingUsecase(pStreamConfig, &Usecases1Target[UsecaseDepth1Id]))
                {
                    pSelectedUsecase = &Usecases1Target[UsecaseDepth1Id];
                }
                break;
            case 2:
                if (TRUE == IsMatchingUsecase(pStreamConfig, &Usecases2Target[UsecaseDepth2Id]))
                {
        ...
        else if (NULL == pSelectedUsecase)
        {
            ...
            if (TRUE == IsVideoEISV3Enabled(pStreamConfig))
            {
                ...
                if (TRUE == IsMatchingUsecase(pStreamConfig, &Usecases3Target[usecaseEIS3Id]))
                {
    ...
    camera3_stream_configuration_t* pStreamConfig)          ///< Stream config
{
    ...
    switch (usecaseId)
    {
        ...
        case UsecaseId::Torch:
            ...
            break;
#if defined(__ANDROID__) // SuperSlowMotion not supported in LE
...
Gralloc::~Gralloc()
{
    if (NULL != m_pGrallocIntf)
    {
        ChiGrallocInterface* pGrallocIntf = static_cast<ChiGrallocInterface*>(m_pGrallocIntf);

        pGrallocIntf->qtiMapperExtn.clear();
        pGrallocIntf->qtiAllocatorV4.clear();
        pGrallocIntf->qtiMapperV4.clear();

        pGrallocIntf->qtiMapperExtn   = NULL;
        pGrallocIntf->qtiAllocatorV4  = NULL;
        pGrallocIntf->qtiMapperV4     = NULL;

        CHX_DELETE pGrallocIntf;
    }
}
...
// Gralloc::Initialize
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CDKResult Gralloc::Initialize()
{
    CDKResult         result        = CDKResultEFailed;
    ChiGrallocInterface* pGrallocIntf  = CHX_NEW ChiGrallocInterface;

    // VOID pointer
    m_pGrallocIntf = pGrallocIntf;

    if (NULL != m_pGrallocIntf)
    {
        result = InitializeMapper4();
    }
    else
    {
        CHX_LOG_ERROR("m_pGrallocIntf Memory allocation failed");
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Gralloc::InitializeMapper4
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CDKResult Gralloc::InitializeMapper4()
{
    CDKResult           result       = CDKResultEFailed;
    const CHAR*         pReason      = NULL;
    ChiGrallocInterface*   pGrallocIntf = static_cast<ChiGrallocInterface*>(m_pGrallocIntf);

    pGrallocIntf->qtiAllocatorV4 = IQtiAllocator_v4::getService();

    if (pGrallocIntf->qtiAllocatorV4 == NULL)
    {
        pReason = "qtiAllocator Version 4 getService failed";
    }

    if (NULL == pReason)
    {
        pGrallocIntf->qtiMapperV4 = IQtiMapper_v4::castFrom(IMapper_v4::getService());

        if (pGrallocIntf->qtiMapperV4 == NULL)
        {
            pReason = "qtiMapper Version 4 getService failed";
        }
    }
    if (NULL == pReason)
    {
        sp<IQtiMapperExtensions> qtiMapperExtn = NULL;
        Error_v4                 status;

        auto hidlCb = [&](const auto& tmpError, const auto& _extensions)
        {
            status          = tmpError;
            qtiMapperExtn   = _extensions;
        };

        Return<void> ret = pGrallocIntf->qtiMapperV4->getMapperExtensions(hidlCb);

        if (FALSE == ret.isOk())
        {
            pReason = "Unable to send response for Mapper version 4";
            CHX_LOG_ERROR("Unable to send response for Mapper version 4, Exception : %s",
                           ret.description().c_str());
        }
        else if (Error_v4::NONE == status)
        {
            pGrallocIntf->qtiMapperExtn = qtiMapperExtn;
            result                      = CDKResultSuccess;
        }
        else
        {
            pReason = "getMapperExtensions failed";
        }
    }

    if (NULL != pReason)
    {
        CHX_LOG_ERROR("Gralloc Init failed for Version 4, reason %s", pReason);
        pGrallocIntf->qtiAllocatorV4.clear();
        pGrallocIntf->qtiMapperV4.clear();
        pGrallocIntf->qtiMapperExtn.clear();
    }
...
Gralloc* Gralloc::GetInstance()
{
    ...
    if (NULL == pGralloc)
    {
        pGralloc = CHX_NEW Gralloc;

        if (NULL != pGralloc)
        {
            CDKResult result = pGralloc->Initialize();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// Gralloc::SetGrallocMetaData
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CDKResult Gralloc::SetGrallocMetaData(
    buffer_handle_t     hNativehandle,
    UINT32              grallocEnumType,
    UINT32              metadataSize,
    VOID*               pMetadata)
{
    CDKResult result   = CDKResultEFailed;
    ...
    if (NULL != pGralloc)
    {
        hidl_vec<UINT8>      metaDataV4;
        MetadataType         metaDataType   = {qtigralloc::VENDOR_QTI, grallocEnumType};
        VOID*                phNativeHandle = const_cast<VOID*>(
                                            (reinterpret_cast<const VOID*>(hNativehandle)));
        ChiGrallocInterface* pGrallocIntf   =
            static_cast<ChiGrallocInterface*>(pGralloc->GetGrallocInterface());

        switch (grallocEnumType)
        {
            case COLOR_METADATA:
            {
                ColorMetaData* pGralocMetadata = static_cast<ColorMetaData*>(pMetadata);
                qtigralloc::encodeColorMetadata(*pGralocMetadata, &metaDataV4);
                break;
            }

            case SET_CVP_METADATA:
            {
                CVPMetadata* pGralocMetadata = static_cast<CVPMetadata*>(pMetadata);
                qtigralloc::encodeCVPMetadata(*pGralocMetadata, &metaDataV4);
                break;
            }

            case SET_UBWC_CR_STATS_INFO:
            {
                UBWCStats* pGralocMetadata = static_cast<UBWCStats*>(pMetadata);
                qtigralloc::encodeUBWCStats(pGralocMetadata, &metaDataV4);
                break;
            }

            case UPDATE_BUFFER_GEOMETRY:
            {
                GraphicsRect bufDimension  = {};
                BufferDim_t* pBufDimension = static_cast<BufferDim_t*>(pMetadata);

                bufDimension.left   = 0;
                bufDimension.top    = 0;
                bufDimension.right  = pBufDimension->sliceWidth;
                bufDimension.bottom = pBufDimension->sliceHeight;

                // medataType is different for Crop
                metaDataType = ::android::gralloc4::MetadataType_Crop;

                std::vector<GraphicsRect> input;
                input.push_back(bufDimension);
                android::gralloc4::encodeCrop(input, &metaDataV4);
                break;
            }

            default:
            {
                // update gralloc type value
                metaDataType.value = grallocEnumType;

                metaDataV4.resize(metadataSize);
                memcpy(metaDataV4.data(), pMetadata, metadataSize);
                break;
            }
        }

        Return<Error_v4> ret = pGrallocIntf->qtiMapperV4->set(phNativeHandle, metaDataType, metaDataV4);

        if (false == ret.isOk())
        {
            CHX_LOG_ERROR("QtiMapper Unable to send response. Exception : %s",
                          ret.description().c_str());
        }
        else if (Error_v4::NONE != ret)
        {
            CHX_LOG_ERROR("setMetaData() failed, metadata Type %d, Size %d",
                          grallocEnumType, metadataSize);
        }
        else
        {
            result = CDKResultSuccess;
    ...
    {
        CHX_LOG_ERROR("setMetaData() failed pGralloc is NULL, metadata Type %d, Size %d",
                      grallocEnumType, metadataSize);
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Gralloc::AllocateGrallocBufferWithMapper4
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CDKResult Gralloc::AllocateGrallocBufferWithMapper4(
    UINT32              width,
    UINT32              height,
    UINT32              format,
    UINT64              producerFlags,
    UINT64              consumerFlags,
    buffer_handle_t&    rhGrallocBuffer,
    UINT32*             pStride)
{
    CDKResult              result       = CDKResultEFailed;
    UINT32                 stride       = 0;
    ChiGrallocInterface*   pGrallocIntf = static_cast<ChiGrallocInterface*>(m_pGrallocIntf);
    Error_v4               hidlErr      = Error_v4::NONE;

    IMapper_v4::BufferDescriptorInfo info =
    {
        .width        = width,
        .height       = height,
        .layerCount   = 1,
        .format       = static_cast<PixelFormat>(format),
        .usage        = (producerFlags | consumerFlags),
    };

    BufferDescriptor_v4 descriptor = BufferDescriptor_v4();

    if (NULL != pGrallocIntf)
    {
        auto hidlCb = [&](const auto &_error, const auto &_descriptor)
        {
            hidlErr     = _error;
            descriptor  = _descriptor;
        };

        pGrallocIntf->qtiMapperV4->createDescriptor(info, hidlCb);

        if (Error_v4::NONE != hidlErr)
        {
            CHX_LOG_ERROR("createDescriptor failed");
            result = CDKResultEFailed;
        }
        else
        {
    ...
    {
        CHX_LOG_ERROR("pGrallocIntf is NULL");
        result = CDKResultENoMemory;
    }

    if (CDKResultSuccess == result)
    {
        auto importCb = [&](const auto &_error, const auto &_buffer)
        {
            hidlErr          = _error;
            rhGrallocBuffer  = static_cast<const native_handle_t *>(_buffer);
        };
        auto allocCb = [&](const auto &_error, const auto &_stride, const auto &_buffers)
        {
            hidlErr = _error;
            stride  = _stride;
            ...
            if (Error_v4::NONE == hidlErr)
            {
                pGrallocIntf->qtiMapperV4->importBuffer(_buffers[0], importCb);
            }
        };
        pGrallocIntf->qtiAllocatorV4->allocate(descriptor, 1, allocCb);

        if ((Error_v4::NONE != hidlErr) || (NULL == rhGrallocBuffer))
        {
            CHX_LOG_ERROR("Failed to allocate Gralloc buffer version 4");
            result = CDKResultENoMemory;
        }
    }
    ...
    CHX_LOG_VERBOSE("Gralloc Bufer#4, WxH %dx%d, format %d rhGrallocBuffer %p flags 0x%" PRIx64,
                    width, height, format,
                    rhGrallocBuffer, (producerFlags | consumerFlags));

    return result;
}

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Gralloc::FreeGrallocBuffer
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CDKResult Gralloc::FreeGrallocBuffer(
    buffer_handle_t hGrallocBuffer)
{
    ...
    if ((NULL != pGrallocIntf) && (NULL != hGrallocBuffer))
    {
        native_handle_t* phNativeHandle = const_cast<native_handle_t*>(hGrallocBuffer);

        if (pGrallocIntf->qtiMapperV4 != NULL)
        {
            pGrallocIntf->qtiMapperV4->freeBuffer(phNativeHandle);
        }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Gralloc::GetBufferSize
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
UINT32 Gralloc::GetBufferSize(
    buffer_handle_t hNativehandle)
{
    UINT32 size = 0;

#if !defined(LE_CAMERA)
    if (NULL != hNativehandle)
    {
        ChiGrallocInterface* pGrallocIntf   = static_cast<ChiGrallocInterface*>(m_pGrallocIntf);
        native_handle_t*     phNativeHandle = const_cast<native_handle_t*>
                                        (reinterpret_cast<const native_handle_t*>(hNativehandle));
        ...
        Return<void> ret = pGrallocIntf->qtiMapperExtn->getSize(static_cast<VOID*>(phNativeHandle), hidl_cb);

        if (false == ret.isOk())
        {
            CHX_LOG_ERROR("Gralloc QtiMapper Unable to send response. Exception : %s",
                          ret.description().c_str());
        }
        else if (ExtensionError::NONE != error)
        {
            CHX_LOG_ERROR("Gralloc getSize returned error %d", error);
        }
    }
#endif // LE_CAMERA
    ...
    buffer_handle_t hNativehandle)
{
    ...
    if (NULL != hNativehandle)
    {
        ...
        ExtensionError error   = ExtensionError::NONE;
        auto           hidl_cb = [&](const auto tmpError, const auto bufferFD)
        {
            if (ExtensionError::NONE != tmpError)
            {
                error = tmpError;
        ...
        Return<void> ret = pGrallocIntf->qtiMapperExtn->getFd(static_cast<VOID*>(phNativeHandle), hidl_cb);
        ...
        {
            CHX_LOG_ERROR("Gralloc getFd returned error %d", error);
        }
    }
#endif // LE_CAMERA
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Gralloc::GetBufferOffset
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
UINT64 Gralloc::GetBufferOffset(
    buffer_handle_t hNativehandle)
{
    UINT64 offset = 0;
    ...
    if (NULL != hNativehandle)
    {
        ...
        ExtensionError error   = ExtensionError::NONE;
        auto           hidl_cb = [&](const auto tmpError, const auto bufferOffset)
        {
            if (ExtensionError::NONE != tmpError)
            {
                error = tmpError;
        ...
        Return<void> ret = pGrallocIntf->qtiMapperExtn->getOffset(static_cast<VOID*>(phNativeHandle), hidl_cb);
        ...
        {
            CHX_LOG_ERROR("Gralloc getOffset returned error %d", error);
        }
    }
#endif // LE_CAMERA
    ...
    UINT32*             pStride)
{
    ...
    else
    {
        ...
        if (NULL != pImageBuffer)
        {
            result = pImageBuffer->AllocateGrallocBuffer(width,
                                                         height,
                                                         format,
                                                         producerUsageFlags,
                                                         consumerUsageFlags,
...
VOID ImageBuffer::Destroy()
{
    ...
    if (NULL != pGrallocBuffer)
    {
        ...
        if (NULL != pGralloc)
        {
            pGralloc->FreeGrallocBuffer(pGrallocBuffer);
        }
        else
        {
            CHX_LOG_ERROR("pGralloc is NULL");
        }
    ...
    UINT32*             pStride)
{
    ...
    if (NULL != pGralloc)
    {
        result = pGralloc->AllocateGrallocBufferWithMapper4(width, height, grallocFormat,
                                                            producerFlags, consumerFlags,
                                                            pGrallocBuffer,
                                                            pStride);
    }
    else
...
#else
                CHX_LOG_ERROR("[%s] Gralloc GetInstance failed!", m_pBufferManagerName);
#endif //CAMX_USE_GRALLOC1
            }
        }
    }
...
#if defined(__ANDROID__) && !defined(ANDROID_Q_AOSP)
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
UINT32 CHIBufferManager::GetBufferSize(const CHIBUFFERINFO* pBufferInfo)
{
    ...
    if (NULL != pBufferInfo)
    {
        ...
        if ((NULL != pGralloc) && (NULL != phBufferHandle))
        {
            size = pGralloc->GetBufferSize(*phBufferHandle);
        }
        else
        {
            CHX_LOG_WARN(" Gralloc size failed pGralloc %p, phBufHandle %p", pGralloc, phBufferHandle);
        }
    ...
    CHIBUFFERINFO*          pDstBufferInfo)
{
    ...
    {
        pGralloc = Gralloc::GetInstance();
    }

    if ((CDKResultSuccess == result) && (NULL != pGralloc))
    {
        ...
        {
            fd[SRC] = pGralloc->GetBufferFD(const_cast<native_handle_t*> (reinterpret_cast<const native_handle_t*>(hSrc)));
            fd[DST] = pGralloc->GetBufferFD(const_cast<native_handle_t*> (reinterpret_cast<const native_handle_t*>(hDst)));
            buf_size[SRC] = pGralloc->GetBufferSize(const_cast<native_handle_t*> (reinterpret_cast<const native_handle_t*>(hSrc)));
            buf_size[DST] = pGralloc->GetBufferSize(const_cast<native_handle_t*> (reinterpret_cast<const native_handle_t*>(hDst)));
            offset[SRC] = pGralloc->GetBufferOffset(const_cast<native_handle_t*> (reinterpret_cast<const native_handle_t*>(hSrc)));
            offset[DST] = pGralloc->GetBufferOffset(const_cast<native_handle_t*> (reinterpret_cast<const native_handle_t*>(hDst)));
        }
    }

    if (CDKResultSuccess == result)
    {
        void*   pSrc    = ChxUtils::MemMap(fd[SRC], buf_size[SRC], offset[SRC]);
        void*   pDst    = ChxUtils::MemMap(fd[DST], buf_size[DST], offset[DST]);
        ...
        if (CDKResultSuccess == result)
        {
            int size = (buf_size[SRC] < buf_size[DST]) ? buf_size[SRC] : buf_size[DST];
            ChxUtils::Memcpy(pDst, pSrc, size);
        }

        if (NULL != pSrc)
        {
            ChxUtils::MemUnmap(pSrc, buf_size[SRC]);
        }

        if (NULL != pDst)
        {
            ChxUtils::MemUnmap(pDst, buf_size[DST]);
        }
    }
    ...
    CHIBUFFERINFO* pBufferInfo)
{
    ...
    if (NULL != pBufferInfo)
    {
        ...
        {
#if defined(__ANDROID__)
            Gralloc::SetGrallocMetaData(*phBufferHandle, SET_VIDEO_PERF_MODE, sizeof(UINT32), &perfMode);
#endif // ANDROID
        }
    }
}

#else //if defined(ANDROID_Q_AOSP)
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// CHIBufferManager::GetBufferSize
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
UINT32 CHIBufferManager::GetBufferSize(const CHIBUFFERINFO* pBufferInfo)
