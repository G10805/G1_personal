    ...
    const  DesiredSensorMode* pDesiredSensorMode)
{
    ...
    {
        if (pAllModes[i].frameRate >= static_cast<UINT32>(pDesiredSensorMode->frameRate) &&
            ChxSensorModeSelect::IsMatchingWidth(&pAllModes[i], pDesiredSensorMode) &&
            ChxSensorModeSelect::IsMatchingHeight(&pAllModes[i], pDesiredSensorMode) &&
    ...
    /// Didn't find suitable sensor mode at steps 2, then we need to upscale a closest low size sensor mode
    ...
    if (FALSE == found)
    {
        ...
        minFpsDelta              = static_cast<UINT32>(-1);
        for (i = 0; i < modeCount; i++)
        {
            if (pAllModes[i].frameRate >= static_cast<UINT32>(pDesiredSensorMode->frameRate) &&
                ChxSensorModeSelect::IsMatchingCapability(&pAllModes[i], pDesiredSensorMode) &&
                ...
                (pAllModes[i].frameDimension.height <= pDesiredSensorMode->optimalHeight))
            {
                 ...
                 else if ((closestWidthDiff == pDesiredSensorMode->optimalWidth - pAllModes[i].frameDimension.width) &&
                     (closestHeightDiff == pDesiredSensorMode->optimalHeight - pAllModes[i].frameDimension.height))
                 {
                     // If width & height matches then check the closest fps
                     if (minFpsDelta > (pAllModes[i].frameRate - static_cast<UINT32>(pDesiredSensorMode->frameRate)))
                     {
                         found             = TRUE;
                         closestModeIndex  = i;
                         closestWidthDiff  = pDesiredSensorMode->optimalWidth - pAllModes[i].frameDimension.width;
    ...
    const DesiredSensorMode* pDesiredSensorMode)
{
    ...
    else if ((TRUE == isSensorSupportIHDR) && (TRUE == pDesiredSensorMode->sensorModeCaps.u.IHDR))
    {
        usecase = UsecaseIHDR;
    }
    ...
    const DesiredSensorMode* pDesiredSensorMode)
{
    ...
    else
    {
        ...
        {
            CHX_LOG_ERROR("Desired Sensor Mode is NULL pointer");
