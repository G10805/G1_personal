    ...
    BOOL isForced)
{
    ...
    else if (TRUE == m_useParallelFlush)
    {
        ...
        FlushAllSessionsInParallel(pFakeSessions);
    }
    ...
    camera3_capture_request_t* pRequest)
{
    ...
    m_numBuffersInErrorState[resultFrameIndexChi] = 0;
    ...
    const VOID* pMetadata)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (0 == find_camera_metadata_entry(m_pReplacedMetadata, ANDROID_SCALER_CROP_REGION, &entry))
        {
            ...
            ChxUtils::AndroidMetadata::SetVendorTagValue(m_pReplacedMetadata, CropRegions,
    ...
    BOOL                       shouldReturnInputBuffer)
{
    ...
    for (UINT32 lastErrorRequest = pRequest->frame_number; lastErrorRequest < m_lastAppRequestFrame; lastErrorRequest++)
    {
        ...
        if (m_lastResultMetadataFrameNum == static_cast<INT64>(lastErrorRequest - 1))
        {
    ...
    UINT32                          cameraID)
{
    ...
    if (NULL != pLogicalCameraInfo)
    {
        ...
        if (TRUE == pChiCameraInfo->lensCaps.isFixedFocus)
        {
            ...
            findResult = find_camera_metadata_entry(
                const_cast<camera_metadata_t*>(pOverrideResult->result), ANDROID_LENS_STATE, &metadata_entry);

            INT setLensState[] = { 0 };
            if (0 != findResult)
            {
                add_camera_metadata_entry(const_cast<camera_metadata_t*>(pOverrideResult->result),
                    ANDROID_LENS_STATE, setLensState, 1);
            }
        }
    }
    ...
    if ((TRUE == metadataAvailable) && (TRUE == metadataErrorSent))
    {
        canBeSent = FALSE;
    }

    if ((FALSE == m_requestFlags[resultFrameIndexChi].isInErrorState) && (canBeSent == TRUE))
    {
        ...
        if (MessageCodeBuffer == errorCode)
        {
            ...
            for (UINT i = 0; i < pRequest->num_output_buffers; i++)
            {
                if (pRequest->output_buffers[i].stream == pErrorStream)
                {
                    ...
                    m_numBuffersInErrorState[frameIndex]++;
                    break;
                }
            }
        ...
        UINT32 cameraID)
{
    for (INT64 i = m_nextAppMessageFrame; i <= m_lastAppMessageFrameReceived; i++)
    {
        ...
        if((TRUE == pRequestFlags->isInErrorState) ||
            ((TRUE == m_requestFlags[frameIndex].isMetadataErrorSent) &&
             (m_numBuffersInErrorState[frameIndex] > 0 &&
              m_numBuffersInErrorState[frameIndex] == m_pendingPCRs[frameIndex].num_output_buffers)))
        {
            inErrorState = TRUE;
        }
        CHX_LOG("frameIndex = %d messageAvailable = %d isInErrorState = %d inErrorState %d",
                frameIndex, m_requestFlags[frameIndex].isMessageAvailable, pRequestFlags->isInErrorState, inErrorState);
        if (((TRUE == pRequestFlags->isMessageAvailable) && (TRUE == pRequestFlags->isMessagePending)) ||
            (TRUE == inErrorState))
        {
            if (FALSE == inErrorState)
            {
        ...
        else // Wait until the nextAppMessageFrame is ready
        {
            break;
        }
    }
    ...
    camera_metadata_t* pMetaData)
{
    ...
    if ((NULL != pMetaData))
    {
        if (FALSE == ChxUtils::AndroidMetadata::IsVendorTagPresent(reinterpret_cast<const VOID*>(pMetaData), SensorModeIndex))
        {
            result = ChxUtils::AndroidMetadata::SetVendorTagValue(pMetaData, SensorModeIndex, 1, &m_selectedSensorModeIndex);
        }
        if (CDKResultSuccess != result)
        {
    ...
    camera_metadata_t* pMetaData)
{
    ...
    if ((NULL != pMetaData))
    {
        if (FALSE == ChxUtils::AndroidMetadata::IsVendorTagPresent(reinterpret_cast<const VOID*>(pMetaData), Feature1Mode))
        {
            ChiModeFeature1SubModeType featureMode = ChiModeFeature1SubModeType::None;
            result = ChxUtils::AndroidMetadata::SetVendorTagValue(pMetaData, Feature1Mode, 1, &featureMode);
        }
        ...
        if (FALSE == ChxUtils::AndroidMetadata::IsVendorTagPresent(reinterpret_cast<const VOID*>(pMetaData), Feature2Mode))
        {
            ChiModeFeature2SubModeType featureMode = ChiModeFeature2SubModeType::None;
            result = ChxUtils::AndroidMetadata::SetVendorTagValue(pMetaData, Feature2Mode, 1, &featureMode);
        }
        if (CDKResultSuccess != result)
        {
            CHX_LOG_ERROR("Cant set Feature1Mode mode index vendor tag");
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CDKResult Usecase::FlushAllSessions(Session* const * ppSession)
{
    CDKResult result            = CDKResultSuccess;
    if (m_useParallelFlush)
    {
        result = FlushAllSessionsInParallel(ppSession);
    }
    else
    {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CDKResult Usecase::FlushAllSessionsInParallel(Session* const * ppSession)
{
    CHX_LOG_INFO("[E]");
...
CDKResult Usecase::Flush()
{
    ...
    if (TRUE == executeFlush)
    {
        ...
        // metadata manager flush
        if (NULL != m_pMetadataManager)
        {
            m_pMetadataManager->Flush();
        }
    ...
    {
        CHX_LOG_ERROR("[%s] Cannot Flush because a flush has already occured!", m_pChiUsecase->pUsecaseName);
    }
    else
    {
        CHX_LOG("[%s] Flush Already in progress!", m_pChiUsecase->pUsecaseName);
    ...
    UINT32                    cameraID)
{
    ...
    if (NULL != pOverrideResult->result)
    {
        ...
        case MetaDataResultCount::TwoMetaDataCount:
        {
            ...
            if (PartialMetaSupport::SeperatePartialMeta == ExtensionModule::GetInstance()->EnableCHIPartialData())
            {
                ...
                else if (PartialResultCount::SecondPartialResult == partialResultCount)
                {
                    ...
                    {
                        // Since CHI Partial result has not been sent, we inject a frame and mark it as sent
                        CHX_LOG("CHI Partial Result not yet sent and thereby injecting for frame %d",
                            pOverrideResult->frame_number);
                        InjectFrameworkResult(pOverrideResult->frame_number,
                            PartialResultCount::FirstPartialResult,
                            cameraID);
                        SetMetaDataHasBeenSent(PartialResultSender::CHIPartialData, resultFrameIndexChi, TRUE);
                    }
                    SetMetaDataHasBeenSent(PartialResultSender::DriverMetaData, resultFrameIndexChi, TRUE);
            ...
            else
            {
                if (PartialResultCount::FirstPartialResult == partialResultCount)
                {
                    ...
                    {
                        // This is handling of 3) Driver and CHI Partial Data is being sent as merged
                        CHX_LOG("CHI Partial Result is being sent as combined with Driver partial Data"
                                " for framenumber=%d", frame_Number);
                        SetMetaDataHasBeenSent(PartialResultSender::CHIPartialData, resultFrameIndexChi, TRUE);
                    }
                ...
                {
                    // Check if Driver Partial Result has been sent else not inject
                    if (FALSE == CheckIfMetaDataHasBeenSent(PartialResultSender::DriverPartialData, resultFrameIndexChi))
                    {
                        ...
                        CHX_LOG("Driver Partial Result not yet sent and thereby injecting for frame %d",
                            pOverrideResult->frame_number);
                        InjectFrameworkResult(pOverrideResult->frame_number,
                            PartialResultCount::FirstPartialResult,
                            cameraID);
        ...
        case MetaDataResultCount::ThreeMetaDataCount:
        {
            ...
            {
                // Step1: Check if Driver Partial Result has been sent else not inject
                if (FALSE == CheckIfMetaDataHasBeenSent(PartialResultSender::DriverPartialData, resultFrameIndexChi))
                {
                    CHX_LOG("Driver Partial Result not yet sent and thereby injecting for frame %d",
                        pOverrideResult->frame_number);
                    InjectFrameworkResult(pOverrideResult->frame_number,
                        ChxUtils::GetPartialResultCount(PartialResultSender::DriverPartialData),
                        cameraID);
                    SetMetaDataHasBeenSent(PartialResultSender::DriverPartialData, resultFrameIndexChi, TRUE);
                }
                // Step2: Check if CHIPartial Result Needs to be sent else not inject
                if (FALSE == CheckIfMetaDataHasBeenSent(PartialResultSender::CHIPartialData, resultFrameIndexChi))
                {
                    CHX_LOG("CHI Partial Result not yet sent and thereby injecting for frame %d",
                        pOverrideResult->frame_number);
                    InjectFrameworkResult(pOverrideResult->frame_number,
                        ChxUtils::GetPartialResultCount(PartialResultSender::CHIPartialData),
                        cameraID);
                    SetMetaDataHasBeenSent(PartialResultSender::CHIPartialData, resultFrameIndexChi, TRUE);
                }
...
VOID Usecase::DeleteAllPendingResults()
{
    ...
    for (UINT i = 0; i < MaxOutstandingRequests; i++)
    {
        ...
        {
            CHX_LOG_ERROR("Deleting pending result - ChiOverrideFrameNum: %d", pRequest->frame_number);
            HandleProcessRequestError(pRequest);
        }
    }
}
