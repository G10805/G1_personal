    ...
    VOID*                       pPrivateCallbackData)
{
    ...
    if (ChiMessageTypeShutter == pMessageDescriptor->messageType)
    {
        ...
        if (pendingResult.logicalPipelineId == pMessageDescriptor->pPrivData->requestIndex)
        {
    ...
    VOID*             pPrivateCallbackData)
{
    ...
    {
        pipelineIndex = pResult->pPrivData->requestIndex;
    }
    else
    {
    ...
    if (pResult->pOutputMetadata == NULL) {
        ...
        camera3_stream_buffer_t *pAppResultBuffer         =
         ...
         {
            CHX_LOG_ERROR("Error: Failed to allocate memory");
            return;
         }
    } else {
    ...
    camera3_capture_request_t* pRequest)
{
    ...
    if (cnt_outbuffer0 > 0) {
        ...
        request->pPrivData               = &privRealTime1[pRequest->frame_number % MaxOutstandingRequests];
        request->pPrivData->requestIndex = REALTIMEPIPELINEMAIN;
    ...
    if (cnt_outbuffer1 > 0) {
        ...
        request->pPrivData               = &privRealTime2[pRequest->frame_number % MaxOutstandingRequests];
        request->pPrivData->requestIndex = REALTIMEPIPELINEAUX;
...
CDKResult UsecaseDualCamera::UpdateVendorTags(ChiMetadata *settings, UINT pipelineId)
{
   ...
   if (m_IsFirstRequest[pipelineId])
   {
        ...
        inputMetadata.currentCameraRole = (pipelineId == REALTIMEPIPELINEMAIN) ?
            CameraRoleTypeTele : CameraRoleTypeWide;
        inputMetadata.logicalCameraId = m_cameraId;
        inputMetadata.masterCameraRole = CameraRoleTypeTele;
        CDKResult result = settings->SetTag(MultiCamControllerManager::m_vendorTagMultiCameraRole,
        ...
        UsecaseDualCamera* usecase, const ChiCaptureResult& captureResult, std::string physicalId)
{
    ...
    UINT32 pipelineIndex = captureResult.pPrivData->requestIndex;
