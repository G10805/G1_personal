    ...
    ChiNodeInterface* pChiNodeInterface)
{
    ...
    {
        CHICDK_LOG_ERROR("Error: %u during GetVendorTagBase for: %s %s Interface: %p",
                  getResult,
    ...
    ChiVendorTagInfo*  pNodeVendorTagInfo)
{
    ...
    {
        result = CDKResultEInvalidPointer;
        CHICDK_LOG_ERROR("Invalide argument: pNodeVendorTagInfo is NULL");
    }
    ...
    CHINODEQUERYBUFFERINFO* pQueryBufferInfo)
{
    ...
    if (NULL != pQueryBufferInfo)
    {
        for (UINT outputIndex = 0; outputIndex < pQueryBufferInfo->numOutputPorts; outputIndex++)
        {
            ...
            CHICDK_LOG_VERBOSE("Input:  (w x h) optimal %d x %d min %d x %d max %d x %d", optimalInputWidth,
                optimalInputHeight, minInputWidth, minInputHeight, maxInputWidth, maxInputHeight);
            CHICDK_LOG_VERBOSE("Output: (w x h) optimal %d x %d min %d x %d max %d x %d", perOutputPortOptimalWidth,
    ...
    {
        CHICDK_LOG_ERROR("Invalide argument: pQueryBufferInfo is NULL");
    }
}
...
#error Unsupported target defined
    ...
    if (NULL == hLibrary)
    {
        CHICDK_LOG_ERROR("Error Loading Library: %s", libFilename);
#if defined (_LINUX) || defined (__QNXNTO__)
        if (dlerror())
        {
            CHICDK_LOG_ERROR("dlopen error %s", dlerror());
        }
#endif // _LINUX
    }
    return hLibrary;
...
#error Unsupported target defined
    ...
    if (CDKResultSuccess != result)
    {
        CHICDK_LOG_ERROR("Failed to close CHI node Library %d", result);
    }
...
#else
        CHICDK_LOG_ERROR("GetDateTime not implemented");
...
#elif defined (_LINUX)
            CamX::OsUtils::StrLCat(pDst, pTokenString, sizeDst);
#elif defined (_WINDOWS)
    ...
    INT*         pScanlineAlignment)
{
    ...
    default:
        useDefault = TRUE;
        CHICDK_LOG_WARN("Unsupported format for media color format: %d, using default alignments", format);
        break;
    }
    ...
    if (FALSE == useDefault)
    {
        ...
        if (0 == planeId)
        {
            ...
            tempAlignedW  = VENUS_Y_STRIDE(mediaColorFmt, alignedWidth + 1);
            tempAlignedH  = VENUS_Y_SCANLINES(mediaColorFmt, alignedHeight + 1);

            strideAlignment   = tempAlignedW - alignedWidth;
            scanlineAlignment = tempAlignedH - alignedHeight;
        }
        else
        {
            alignedWidth  = VENUS_UV_STRIDE(mediaColorFmt, width);
            alignedHeight = VENUS_UV_SCANLINES(mediaColorFmt, height);
            tempAlignedW  = VENUS_UV_STRIDE(mediaColorFmt, alignedWidth + 1);
            tempAlignedH  = VENUS_UV_SCANLINES(mediaColorFmt, (alignedHeight << 1) + 1);
    ...
    CHICDK_LOG_VERBOSE("Format = %d, mediaColorFmt = %d, planeId = %d, strideAlignment = %d, scanlineAlignment = %d",
