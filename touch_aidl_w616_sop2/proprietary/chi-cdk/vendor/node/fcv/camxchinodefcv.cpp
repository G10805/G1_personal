...
#error Unsupported target defined
    ...
    if (NULL == hLibrary)
    {
        CHICDK_LOG_ERROR("Error Loading Library: %s", pLibraryName);
    }
...
#error Unsupported target defined
    ...
    {
        CHICDK_LOG_ERROR(CamxLogGroupCHI, "Failed to close FCV Node Library");
    }
    return result;
}
    ...
    CHINODECREATEINFO* pCreateInfo)
{
    ...
    {
        CHICDK_LOG_ERROR("Calling fcvSetOperationModeExt");
    ...
    m_processedFrame = 0;
    CHICDK_LOG_INFO(" caps %x", m_nodeCaps);
    return result;
}
    ...
    CHINODEQUERYBUFFERINFO* pQueryBufferInfo)
{
    ...
    if (CDKResultSuccess == result)
    {
        for (UINT32 i = 0; i < pQueryBufferInfo->numOutputPorts; i++)
        {
            ...
            CHICDK_LOG_INFO("Input min W %d H %d, max W %d h %d,"
            ...
            CHICDK_LOG_INFO("Output min W %d H %d, max W %d h %d,"
...
CDKResult ChiFCVNode::ProcessRequest(
    CHINODEPROCESSREQUESTINFO* pProcessRequestInfo)
{
    CHICDK_LOG_INFO("FCV ProcessRequest");
    for (UINT32 i = 0; i < pProcessRequestInfo->inputNum; i++)
    {
        UpdateOrientation(pProcessRequestInfo->frameNum);
        CHICDK_LOG_INFO(" UV: 0/p plane 0 dim: %dx%d stride %d scanline %d",
        ...
        CHICDK_LOG_INFO(" UV: 0/p plane 1 dim: %dx%d stride %d scanline %d",
        ...
        CHICDK_LOG_INFO(" UV: i/p plane 0 dim: %dx%d stride %d scanline %d",
        ...
        CHICDK_LOG_INFO(" UV: i/p plane 1 dim: %dx%d stride %d scanline %d",
    ...
    RotationAngle       targetRotation)
{
    ...
    for (UINT i = 0; i < hOutput->imageCount; i++)
    {
        if ((NULL != m_pfnRemoteRegisterBuf) && (RotationAngle::Rotate0Degrees != targetRotation))
        {
            ...
            CHICDK_LOG_INFO("Y rotation status %d", status);
            ...
            CHICDK_LOG_VERBOSE("UV: i/p dim: %dx%d", hInput->format.formatParams.yuvFormat[0].width, hInput->format.formatParams.yuvFormat[1].height);
            status = m_pfnfcvRotateImageInterleavedu8(hInput->pImageList[i].pAddr[1], src_width, src_height,
                                                      ...
                                                      dst_stride, (fcvRotateDegree)targetRotation);
            CHICDK_LOG_INFO("UV rotation status %d", status);
        ...
        {
            CHICDK_LOG_INFO("Start Copy for 0 degree or rotation handle null");
    ...
    UINT64 requestId)
{
    ...
    default:
        m_currentRotation = Rotate0Degrees;
        CHICDK_LOG_ERROR(" Proper Orientation is not set so seeting to default %d",
                  m_currentRotation);
        break;
    }
    CHICDK_LOG_INFO("Orientation %d", m_currentRotation);
}
