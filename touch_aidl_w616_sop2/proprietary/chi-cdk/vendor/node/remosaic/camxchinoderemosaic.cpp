    ...
    CHINODERESPONSEINFO* pInfo)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            CHICDK_LOG_ERROR("CHINODERESPONSEINFO is smaller than expected");
            result = CDKResultEFailed;
        }

        CHICDK_LOG_INFO("flush response time: %d", static_cast<INT32>(pInfo->responseTimeInMillisec));
    }
...
{
    CHICDK_LOG_INFO("%s, [INPUT], format:%d, wxh:%dx%d, stridexscanline:%dx%d, plane[0] size:%zu",
        __func__,
    ...
    CHICDK_LOG_INFO("%s, [OUTPUT], format:%d, wxh:%dx%d, stridexscanline:%dx%d, plane[0] size:%zu",
        __func__,
    ...
    for (UINT i = 0; i < hOutput->imageCount; i++)
    {
        ...
        {
            CHICDK_LOG_INFO("Quad CFA dummy unpacking (mipi10 --> plain16)");
...
CDKResult ChiRemosaicNode::QueryVendorTagIds()
{
    ...
    {
        CHICDK_LOG_ERROR("Unable to get AECFrameControlTagId Vendor Tag Id");
    }
    ...
    {
        CHICDK_LOG_ERROR("Unable to get AWBFrameControl Vendor Tag Id");
    }
    ...
    BOOL                       useInputPool)
{
    ...
    if (NULL != aecControl)
    {
        CHICDK_LOG_VERBOSE("aec output info: luxIndex:%f, ET:%" PRIu64 ", gain:%f",
            aecControl->luxIndex,
    ...
    {
        CHICDK_LOG_ERROR("fail to get aec info from metadata");
        result = CDKResultEFailed;
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (NULL != awbControl)
        {
            CHICDK_LOG_VERBOSE("awb output info: cct:%d, r_gain:%f, g_gain:%f, b_gain:%f",
        ...
        {
            CHICDK_LOG_ERROR("fail to get awb info from metadata");
            result = CDKResultEFailed;
        }
    }
