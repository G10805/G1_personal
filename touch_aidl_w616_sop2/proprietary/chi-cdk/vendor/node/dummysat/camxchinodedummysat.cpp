    ...
    CHINODEMETADATALIST* pMetadataPublishlist)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            CHICDK_LOG_ERROR("CHINODEMETADATALIST is smaller than expected");
            result = CDKResultEFailed;
        }
    }
...
CDKResult ChiDummySATNode::ConfigureInputBufferMapping()
{
    ...
    if (NULL != pData)
    {
        ...
        for (UINT32 cfgIdx = 0; cfgIdx < pCameraInputConfig->numConfigurations; cfgIdx++)
        {
            ...
            if ((pNodeDescriptor->nodeId == m_nodeId) && (pNodeDescriptor->nodeInstanceId == m_nodeInstanceId))
            {
                ...
                {
                    CHICDK_LOG_ERROR("No Camera Index found for camera id: %u", cameraId);
                    result = CDKResultEFailed;
                }
                else
                {
                    ...
                    CHICDK_LOG_VERBOSE("Camera id %d, Full Input buffer Idx %d",
    ...
    {
        CHICDK_LOG_ERROR("PhysicalCameraInputConfig metadata is NULL");
        result = CDKResultEFailed;
    }
    return result;
}
...
CDKResult ChiDummySATNode::ConfigureCameraInfo()
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            CHICDK_LOG_ERROR("Number of linked cameras %d, exceeds max supported linked cameras %d ",
                      m_numOfLinkedCameras, MaxLinkedCameras);
            result = CDKResultEFailed;
        }
        else
        {
            CHICDK_LOG_INFO("Number of linked cameras %d, primary camera id %d ",
    ...
    CHINODECREATEINFO* pCreateInfo)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        m_resultDataOZ.masterCamera = m_cameraInfo[m_primaryCameraIndex].pCameraConfig->cameraRole;
        m_resultDataOZ.masterCameraId = m_cameraInfo[m_primaryCameraIndex].pCameraConfig->cameraId;
        m_resultDataOZ.recommendedMasterCamera = m_resultDataOZ.masterCamera;
        ...
        for (UINT32 i = 0; i < m_numOfLinkedCameras; i++)
        {
            ...
            m_resultDataOZ.lowPowerMode[i].cameraRole = m_cameraInfo[i].pCameraConfig->cameraRole;
        ...
        else
        {
            ...
            {
                CHICDK_LOG_ERROR("Fatal! Cannot enable %d cameras at a time. Enable LPM mode",
    ...
    CHINODEMETADATALIST* pMetadataPublishlist)
{
    ...
    CHICDK_LOG_INFO("Vendor Tag published by SAT node %x", outputMetadataVendorTagBase);

    pMetadataPublishlist->tagCount        = 1;
    pMetadataPublishlist->partialTagCount = 0;
    pMetadataPublishlist->tagArray[0]     = outputMetadataVendorTagBase;
    ...
    CHINODEPROCESSREQUESTINFO* pProcessRequestInfo)
{
    ...
    {
        CHICDK_LOG_ERROR("pInputMetaOZ is null");
        result = CDKResultEInvalidPointer;
    }
    else if (0 == pInputMetaOZ->numInputs)
    {
        CHICDK_LOG_ERROR("Invalid number of inputs");
        result = CDKResultEFailed;
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            CHICDK_LOG_VERBOSE("SAT Num of Input %d, Camera Id %d: leftxtop %d x %d,"
                        "lpm mode - (%d - %d) , Master: %d Current CameraRole - (%d)",
                        ...
                        pInputMetaOZ->cameraMetadata[i].masterCameraRole,
                        pInputMetaOZ->cameraMetadata[i].cameraRole);
        }

        LPMMode        localLPMMode     = m_lpmMode;
        CameraRoleType currentMaster    = pInputMetaOZ->cameraMetadata[0].masterCameraRole;
        UINT32         currentMasterId  = pInputMetaOZ->cameraMetadata[0].masterCameraId;
        UINT32         currentMasterIdx = GetCameraIndexFromRole(pInputMetaOZ->cameraMetadata[0].masterCameraRole);
        //Decide LPM
        if (TRUE == m_cameraInfo[currentMasterIdx].pCameraConfig->enableSmoothTransition)
        {
            if (TRUE == m_LPMEnable)
            {
                     ...
                     (pInputMetaOZ->cameraMetadata[0].userCropRegion.left > TRIPLE_TRANSITION_LOW))))
                {
                    ...
                    CHICDK_LOG_INFO("Dummy SAT node - Moving to OVERLAP zone");
                }
                ...
                else if (OVERLAP == m_lpmMode)
                {
                    ...
                    for (UINT32 i = 0; i < pInputMetaOZ->numInputs; i++)
                    {
                        if (((m_numOfLinkedCameras == 2) &&
                            (CameraRoleTypeWide == pInputMetaOZ->cameraMetadata[i].masterCameraRole) &&
                            ...
                            (pInputMetaOZ->cameraMetadata[i].userCropRegion.left <= TRIPLE_TRANSITION_LOW)))
                        {
                            ...
                            CHICDK_LOG_INFO("Dummy SAT node - Moving to WIDEONLY zone");
                        }
                        // Switch to Tele Zone only if master is TELE and Zoom > TRANSITION_HIGH
                        else if (((m_numOfLinkedCameras == 2) &&
                            (CameraRoleTypeTele == pInputMetaOZ->cameraMetadata[i].masterCameraRole) &&
                            ...
                            (pInputMetaOZ->cameraMetadata[i].userCropRegion.left > TRIPLE_TRANSITION_HIGH)))
                        {
                            ...
                            CHICDK_LOG_INFO("Dummy SAT node - Moving to TELEONLY zone");
                        }
                    }
                }
                ...
                {
                    CHICDK_LOG_INFO("Current Master:%d, masterId:%d, result masterRole %d, result masterId %d",
                             currentMaster, currentMasterId, m_resultDataOZ.masterCamera, m_resultDataOZ.masterCameraId);
                    ...
                    else if (OVERLAP == localLPMMode)
                    {
                        ...
                        for (UINT32 i = 0; i < m_numOfLinkedCameras; i++)
                        {
                            cameraIdx = GetCameraIndexFromRole(m_resultDataOZ.lowPowerMode[i].cameraRole);
                    ...
                    CHICDK_LOG_INFO("Current Master:%d, masterId:%d, result masterRole:%d, result masterId:%d",
                             currentMaster, currentMasterId, m_resultDataOZ.masterCamera, m_resultDataOZ.masterCameraId);
                    CHICDK_LOG_ERROR("SAT: Trigger LPM %d => %d",
            ...
            if (OVERLAP == m_lpmMode)
            {
                ...
                if (0 == m_preRoleSwitchWaitFrames)
                {
                    ...
                    {
                        currentMaster = CameraRoleTypeTele;
                    }
                    else
                    {
                        currentMaster = CameraRoleTypeWide;
                    }
                    currentMasterId = m_cameraInfo[GetCameraIndexFromRole(currentMaster)].pCameraConfig->cameraId;
                    CHICDK_LOG_INFO("Current Master:%d, masterId:%d, result masterRole:%d, result masterId:%d",
                             currentMaster, currentMasterId, m_resultDataOZ.masterCamera, m_resultDataOZ.masterCameraId);
                }
                else
                {
                    CHICDK_LOG_INFO("wait for role switch m_preRoleSwitchWaitFrames is %d ",
                             m_preRoleSwitchWaitFrames);
                }

                if (pInputMetaOZ->cameraMetadata[0].masterCameraRole != currentMaster)
                {
                    CHICDK_LOG_ERROR("SAT: Switch Master: %d => %d",
                              pInputMetaOZ->cameraMetadata[0].masterCameraRole, currentMaster);
                }
            }
            else
            {
            ...
            m_resultDataOZ.masterCamera              = currentMaster;
            m_resultDataOZ.recommendedMasterCamera   = currentMaster;
            ...
            CHICDK_LOG_INFO("Current Master:%d, masterId:%d, result masterRole:%d, result masterId:%d"
                     ...
                     currentMaster,
                     currentMasterId,
                     m_resultDataOZ.masterCamera,
                     m_resultDataOZ.masterCameraId,
                     pInputMetaOZ->numInputs);
        }
        else
        {
            m_resultDataOZ.masterCamera              = pInputMetaOZ->cameraMetadata[0].masterCameraRole;
            m_resultDataOZ.recommendedMasterCamera   = pInputMetaOZ->cameraMetadata[0].masterCameraRole;
            ...
            CHICDK_LOG_INFO("Else Current Master:%d, masterId:%d, result masterRole:%d, result masterId:%d",
                     currentMaster, currentMasterId, m_resultDataOZ.masterCamera, m_resultDataOZ.masterCameraId);
        }
        ...
        {
            currentMaster   = m_resultDataOZ.masterCamera;
            currentMasterId = m_cameraInfo[GetCameraIndexFromRole(currentMaster)].pCameraConfig->cameraId;

            CHICDK_LOG_INFO("SAT Result Switching Dynamically masterRole = %d, MasterRole=%d",
                    currentMaster, currentMasterId);
        }
        else
        {
            currentMaster               = pInputMetaOZ->cameraMetadata[0].masterCameraRole;
        ...
        UINT32 masterCameraIdx      = GetCameraIndexFromRole(currentMaster);
        ...
        for (UINT32 i = 0; i < pInputMetaOZ->numInputs; i++)
        {
            if (pProcessRequestInfo->phInputBuffer[i]->portId == m_cameraInfo[masterCameraIdx].inputPortId)
            {
                ...
                CHICDK_LOG_INFO("Dummy SAT node - Master is %d, Master Port Id %d, Input Port Id %d",
        ...
        CHICDK_LOG_INFO("Request: %" PRIi64 " %sing input port %u @ idx %u to output port 0 ",
    ...
    for (UINT32 i = 0; i < m_numOfLinkedCameras; i++)
    {
        CHICDK_LOG_INFO("SAT Result: cameraid %d, cameraRole %d, lpm enable %d, Master: %d,"
                 " Master Camera Id: %u recommended master role %d",
                 m_resultDataOZ.lowPowerMode[i].cameraId,
                 m_resultDataOZ.lowPowerMode[i].cameraRole,
                 m_resultDataOZ.lowPowerMode[i].isEnabled,
                 m_resultDataOZ.masterCamera,
                 m_resultDataOZ.masterCameraId,
                 m_resultDataOZ.recommendedMasterCamera);
    }
    ...
    UINT64 requestId)
{
    ...
    const UINT32           tagSize      = sizeof(g_VendorTagSectionDummySAT) / sizeof(g_VendorTagSectionDummySAT[0]) + 3;
    ...
    g_ChiNodeInterface.pGetVendorTagBase(&vendorTagBase);
    CHICDK_LOG_VERBOSE("Vendor Tags value shall be same %x %x",
    ...
    if (NULL != pData)
    {
        CHICDK_LOG_VERBOSE("supported feature %d supported feature %d",
    ...
    if (NULL != pData)
    {
        CHICDK_LOG_VERBOSE("current mode %d current mode %d",
    ...
    if (NULL != pData)
    {
        CHICDK_LOG_VERBOSE("processed frameNum %" PRIu64 " processed frame %" PRIu64,
    ...
    if (NULL != pData)
    {
        ...
        CHICDK_LOG_VERBOSE("frame dimension %d %d frame dimenstion %d %d",
    ...
    if (NULL != pData)
    {
        ...
        CHICDK_LOG_VERBOSE("timestamp for current frame %" PRIi64, timestamp);
    }
    ...
    if (NULL != pData)
    {
        ...
        CHICDK_LOG_VERBOSE("ae banding modes %d %d %d %d", aeModes[0], aeModes[1], aeModes[2], aeModes[3]);
    }
#endif
}

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// ChiDummySATNode::GetCameraIndexFromRole
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
UINT32 ChiDummySATNode::GetCameraIndexFromRole(
    CameraRoleType camRole)
{
    ...
    for (UINT32 i = 0; i < m_numOfLinkedCameras; i++)
    {
        if (m_cameraInfo[i].pCameraConfig->cameraRole == camRole)
        {
            camIndex = i;
            break;
        }
