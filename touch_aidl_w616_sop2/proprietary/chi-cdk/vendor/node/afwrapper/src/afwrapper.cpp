...
static CHIAFALGORITHMCALLBACKS g_AFLibCallbacks;
static CHIAFAlgorithm*         g_AFLibAlgorithmIntf;
...
CAMX_VISIBILITY_PUBLIC VOID ChiAFAlgorithmEntry(CHIAFALGORITHMCALLBACKS* pCallBacks)
{
    ...
    else
    {
        ...
        (void)CamX::OsUtils::SNPrintF(libFilename, FILENAME_MAX, "%s%s%s.%s",
                CamX::VendorCompPath, CamX::PathSeparator, pAFLibName, CamX::SharedLibraryExtension);
        ...
        if (NULL != handle)
        {
            ...
            {
                STATS_LOG_ERROR("AF lib entrt function is NULL");
            }
        }
        else
        {
            STATS_LOG_ERROR("Failed load AF librart -%s", libFilename);
        }
