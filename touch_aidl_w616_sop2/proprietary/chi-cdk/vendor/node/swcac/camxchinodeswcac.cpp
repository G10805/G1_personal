...
static const CHAR*  pCac3AlgoLibName             = "libmmcamera_cac3";    /// Cac algo library name.
...
CDKResult ChiSwCacNode::InitializeFuncPtrs()
{
    ...
    if (NULL == m_hCac3Lib)
    {
        CHICDK_LOG_ERROR("Unable to open %s library", libFilePath);
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            CHICDK_LOG_ERROR("Unable to find cac3_process");
            result = CDKResultEUnableToLoad;
        }
        ...
        if (CDKResultSuccess == result)
        {
            ...
            if (NULL == m_hCac3Init)
            {
                CHICDK_LOG_ERROR("Unable to find cac3_init");
                result = CDKResultEUnableToLoad;
            }
            ...
            if (CDKResultSuccess == result)
            {
                ...
                {
                    CHICDK_LOG_ERROR("Unable to find cac3_deinit");
                    result = CDKResultEUnableToLoad;
                }
            }
        }
    ...
    CHINODECREATEINFO* pCreateInfo)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (CDKResultSuccess != result)
        {
            CHICDK_LOG_ERROR("cac3 library not loaded");
            result = CDKResultEInvalidPointer;
        }
    }
    ...
    CHINODEBUFFERHANDLE* phBuffer)
{
    ...
    {
        CHICDK_LOG_ERROR("CAC doesn't support %d CamX image format",
    ...
    CHINODEBUFFERHANDLE* phBuffer)
{
    ...
    if (FALSE == m_bIsCacInitDone)
    {
        ...
        {
            CHICDK_LOG_ERROR("cac3_init failed");
            result = CDKResultEFailed;
        }
        else
        {
            CHICDK_LOG_VERBOSE("cac3_init done");
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (CDKResultSuccess == result)
        {
            ...
            {
                CHICDK_LOG_ERROR("cac3_process failed");
                result = CDKResultEFailed;
            }
            else
            {
                CHICDK_LOG_VERBOSE("Cac3 algo applied");
            }
        }
    }
    ...
    for (UINT32 i = 0; i < pProcessRequestInfo->outputNum; i++)
    {
        if (TRUE == IsBypassableNode())
        {
            ...
            if (Cac3State || m_enableSwCacAlways)
            {
                ...
                {
                    CHICDK_LOG_ERROR("ApplyCacAlgo failed");
                    result = CDKResultEFailed;
                }
                else
                {
                    CHICDK_LOG_INFO("Cac algo applied");
                }
            }
            else
            {
                CHICDK_LOG_INFO("Cac feature is disabled ");
            }
        ...
        {
            CHICDK_LOG_ERROR("Node is not bypassable");
            result = CDKResultEFailed;
        }
    }
