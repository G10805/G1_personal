...
static CHIAECALGORITHMCALLBACKS g_AECLibCallbacks;
static CHIAECAlgorithm*         g_AECLibAlgorithmIntf;
...
CAMX_VISIBILITY_PUBLIC VOID ChiAECAlgorithmEntry(CHIAECALGORITHMCALLBACKS* pCallBacks)
{
    ...
    (void)CamX::OsUtils::SNPrintF(libFilename, FILENAME_MAX, "%s%s%s.%s",
            CamX::VendorCompPath, CamX::PathSeparator, pAECLibName, CamX::SharedLibraryExtension);
    ...
    if (NULL != handle)
    {
        ...
        {
            STATS_LOG_ERROR("AEC lib entrt function is NULL");
        }
    }
    else
    {
        STATS_LOG_ERROR("Failed load AEC librart -%s", libFilename);
    }
}
