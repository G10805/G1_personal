    ...
    const AWBAlgoDestroyParamList*  pDestroyParams)
{
    ...
    STATS_LOG_ERROR("StaticAWB enter");
        ...
        switch (pParamList->destroyParamType)
        {
            case AWBDestroyParamTypeCameraCloseIndicator:
            {
                ...
                if (TRUE == isCameraClosing)
                {
                    ...
                    {
                        STATS_LOG_ERROR("StaticAWB pAWBInternalData in destroy is %p", pAWBInternalData);
                ...
                {
                    STATS_LOG_ERROR("overrideCameraClose in  AWBDestroy %d", overrideCameraClose);
                }
                break;
            }
            default:
    ...
    AWBAlgoOutputList*      pOutputs)
{
    ...
    {
        STATS_LOG_ERROR("Invalid args pAWBAlgorithm:%p pInputs:%p pOutputs:%p");
    ...
    CHIAWBAlgorithm**               ppAWBAlgorithm)
{
    ...
    {
        STATS_LOG_ERROR("Invalid Args pCreateParams:%p ppAWBAlgorithm:%p", pCreateParams, ppAWBAlgorithm);
        result = CDKResultEInvalidArg;
    }
    ...
    if (result == CDKResultSuccess)
    {
        for (UINT32 j = 0; j < pCreateParams->createParamsCount; j++)
        {
            ...
            {
                STATS_LOG_INFO("StaticAWB algo sizeOfParam for CHISTATSHANDLE is %d", pParamList->sizeOfCreateParam);
    ...
    if (NULL == pAWBInternalData)
    {
        STATS_LOG_INFO("pAWBInternalData before %p", pAWBInternalData);
            ...
            if (CDKResultSuccess == result)
            {
                ...
                if (CDKResultSuccess == result)
                {
                    ...
                    *ppAWBAlgorithm = reinterpret_cast<CHIAWBAlgorithm*>(pAWBInternalData);
                    STATS_LOG_INFO("pAWBInternalData first time %p", pAWBInternalData);
                }
                else
                {
                    CAMX_DELETE pAWBInternalData;
    ...
    {
        STATS_LOG_INFO("pAWBInternalData after %p", pAWBInternalData);
        *ppAWBAlgorithm = reinterpret_cast<CHIAWBAlgorithm*>(pAWBInternalData);
        result = CDKResultSuccess;
    }
...
CDKResult AWBGetCapabilities(CHIALGORITHMCAPSINFO* pCapsInfo)
{
    ...
    STATS_LOG_HIGH("Get supported Capabilities size:%d, algo mask: %d", pCapsInfo->size, pCapsInfo->algorithmCapsMask);
