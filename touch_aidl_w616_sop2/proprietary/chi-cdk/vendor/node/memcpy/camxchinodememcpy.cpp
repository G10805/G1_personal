    ...
    CHINODECAPSINFO* pCapsInfo)
{
    ...
    if ((CDKResultSuccess != result) && (NULL != pCapsInfo))
    {
    ...
    CHINODEFLUSHREQUESTINFO* pInfo)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            CHICDK_LOG_ERROR("CHINODEFLUSHREQUESTINFO is smaller than expected");
            result = CDKResultEFailed;
        }
    }
    ...
    CHINODEPROCESSREQUESTINFO* pProcessRequestInfo)
{
    ...
    if (TRUE == satisfied)
    {
        for (UINT32 i = 0; i < pProcessRequestInfo->outputNum; i++)
        {
            ...
            {
                CHICDK_LOG_INFO("Flush is in Progress for the session, aborting");
                break;
            }
    ...
    CHINODEBUFFERHANDLE hInput)
{
    ...
    if (hOutput->numberOfPlanes < hInput->numberOfPlanes )
    {
        ...
        CHICDK_LOG_ERROR("Invalid argument: output numberOfPlanes is smaller than input");
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        for (UINT i = 0; i < hOutput->imageCount; i++)
        {
            if (((outPlaneStride != inPlaneStride) || (outSliceHeight != inSliceHeight)) &&
                ...
                (hOutput->format.format == FDYUV420NV12) || (hOutput->format.format == FDY8)       ||
                (hOutput->format.format == Y8)))
            {
                ...
                if ((hOutput->format.format != FDY8) && (hOutput->format.format != Y8))
                {
    ...
    UINT64 requestId)
{
    ...
    else if (requestId != lastFlushedRequestId)
    {
        ...
        {
            CHICDK_LOG_VERBOSE("Last Request Id flushed %" PRIu64, lastFlushedRequestId);
        }
#if PRINT_METADATA
    ...
    CHINODEFLUSHREQUESTINFO* pFlushRequestInfo)
{
    ...
    CHICDK_LOG_INFO("Flush request Id %" PRIu64 " from node", pFlushRequestInfo->frameNum);
