...
#define MAX_SHDR_BG_STATS_SIZE 117
...
public:
   ...
   VOID Autoltm2_Linear_CDF_Gen(
    ...
    /// WaitAEC
    ///
    /// @brief  Check if property is set to wait for AEC dependency
    ///
    /// @return 1 or 0
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    UINT32 WaitAEC() const
    {
        return m_aecWait;
    }
...
#define STATS_BIT_DEPTH_BIN_IOT (12)
#define MAX_SHDR_VHIST1_STATS_NONUNIFROM_SIZE \
	  (0x1 << STATS_BIT_DEPTH_BIN_MOBILE)
#define MAX_SHDR_VHIST2_STATS_NONUNIFROM_SIZE (0x1 << STATS_BIT_DEPTH_BIN_IOT)
    ...
    typedef struct ShdrStatsInfo
    {
        ...
        float r_avg[MAX_SHDR_BG_STATS_SIZE];
        float b_avg[MAX_SHDR_BG_STATS_SIZE];
        uint16_t bin[MAX_SHDR_BHIST_STATS_SIZE];
        uint16_t bin_vhist1[MAX_SHDR_VHIST1_STATS_NONUNIFROM_SIZE];  // non-uniform hist,
        uint16_t bin_vhist2[MAX_SHDR_VHIST2_STATS_NONUNIFROM_SIZE];  // non-uniform
    } SHDRSTATSINFO;
...
private:
    ...
    UINT32              m_aecWait;      ///< wait for AEC dependency
    ...
    /*** shdr tm tuning table   ***/
    typedef struct TmTuningTable {
        float target_ratio;
        float gtm;
        float ltm_max_gain;
    ...
    typedef struct ShdrInitConfig
    {
        ...
        uint32_t src_stride;
        uint32_t dst_stride;
        TMTUNINGTABLE *tm_tuning_table;
        uint32_t tm_tuning_table_size;
        SENSORCOMPANDING companding;
    } SHDRINITCONFIG;
    ...
    **/
    ...
    float                   **GTM_GAMMA;
    TMTUNINGTABLE           *m_tm_tuning_table;
    int                     num_gamma_range;
    int                     gamma_entries;
