...
//#define SHDR_UNITY_AWB // using unity gain for AWB instead of meta tags value.
...
static const CHAR* pLibName = "libshdr";
...
static CHIVENDORTAGDATA g_VendorTagSectionDummyShdr[] =
{
    ...
    { "SHDRBGBEconfigStats",    TYPE_BYTE, sizeof(BGBEConfig) }
};
    ...
    CHIQUERYVENDORTAG* pQueryVendorTag)
{
    ...
    {
        result = CDKResultEInvalidPointer;
        CHICDK_LOG_ERROR("Invalid argument: pQueryVendorTag is NULL");
    }
    ...
    CHINODECREATEINFO* pCreateInfo)
{
    ...
    {
        result = CDKResultEInvalidPointer;
        CHICDK_LOG_ERROR("Invalid argument: pCreateInfo is NULL");
        return result;
    }
    ...
    CHINODECREATEINFO* pCreateInfo)
{
    ...
    {
        CHICDK_LOG_ERROR("Failed to get Vendor tag bases");
    }

    if (CDKResultSuccess == result)
    {
        UINT index;
        // Check node properties to see if AEC is enabled.
        for(index = 0;index < pCreateInfo->nodePropertyCount;index++)
        {
          ChiNodeProperty* pNodeProperties = &pCreateInfo->pNodeProperties[index];
          if(pNodeProperties->id == SHDR_AEC_ENABLE_PROPERTY)
          {
              m_aecWait = (UINT32)strtol((const char *)pNodeProperties->pValue, NULL, 10);
          }
        }
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (CDKResultSuccess == result)
        {
            ...
            memset(m_bgbeconfig, 0,sizeof(BGBEConfig));
            m_p_shdr_ctx->input_param.p_tuning   = &m_p_shdr_ctx->tuning;
            m_p_shdr_ctx->input_param.stats_info = &m_p_shdr_ctx->stats_info;

            m_p_shdr_ctx->init_config.sensor_layout = SHDR_FUSED_FRAME;
        }
    }

    else
    {
        CHICDK_LOG_ERROR("Load SHDR algo lib failed");
    }
    ...
    SHDROUTPUTPARAM *buf_3a, void *user_data)
{
    ...
    if (!p_cback) {
        CHICDK_LOG_ERROR("invalid input arguments p_cback: NULL");
        return;
    }
    if (!p_cback->user_data_frame || !p_cback->user_data_Node)
    {
        CHICDK_LOG_ERROR("invalid input arguments ");
        free(p_cback);
        return;
    }
    ...
    CHICDK_LOG_VERBOSE("Got Callback ShdrCompletionCallback Framenum : %d", framenum);

    if (pShdr->m_isDestroyInProgress == FALSE)
    {
    ...
    CHINODESETBUFFERPROPERTIESINFO* pSetBufferInfo)
{
    ...
    CHICDK_LOG_VERBOSE("SetBufferInfo Width=%d, Height=%d",
    ...
    {
        CHICDK_LOG_ERROR("Can not get capabilites %d", lib_ret);
        result = CDKResultEFailed;
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        CHICDK_LOG_VERBOSE("lib ver %.2f api ver %.2f w %d h %d row pitch %d buf padding %d, sensor layout %d",
        ...
        CHICDK_LOG_VERBOSE("lef offset %d sef offset %d lib version %.2f",
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (NULL != pTuningManager)
        {
           ...
           CAMX_ASSERT(NULL != pChromatix);
            ...
            {
                result = CDKResultEFailed;
                CHICDK_LOG_ERROR("GTM_GAMMA is null");
                return result;
            }
            ...
            if (NULL != pChromatix)
            {
                ...
                CHICDK_LOG_VERBOSE("TuningData ADC_BIT_DEPTH=%d, TM out bit depth =%d"
                ...
                CHICDK_LOG_VERBOSE("TuningData hdr_dark_n2_minus_n1_normalization_factor=%d "
                ...
                CHICDK_LOG_VERBOSE("TuningData perf_hint=%d num_gpu_passes=%d gpu_ltm_en=%d",
                ...
                CHICDK_LOG_VERBOSE("black level %u", pChromatix->BayerProcTuningData.black_level);
                ...
                m_p_shdr_ctx->init_config.companding                   = static_cast<SENSORCOMPANDING>(pChromatix->BayerProcTuningData.companding);

                m_p_shdr_ctx->init_config.bayer_pattern                = static_cast<SHDRBAYERPATTERN>(pChromatix->BayerProcTuningData.bayerPattern);
                m_p_shdr_ctx->init_config.input_bayer_format           = static_cast<SHDRINPUTBAYERFORMAT>(pChromatix->BayerProcTuningData.inputBayerFormat);
                m_p_shdr_ctx->init_config.output_bayer_format          = static_cast<SHDROUTPUTBAYERFORMAT>(pChromatix->BayerProcTuningData.outputBayerFormat);
                m_p_shdr_ctx->init_config.tuning.BLACK_LEVEL           = pChromatix->BayerProcTuningData.black_level;
            }
            else
            {
                m_p_shdr_ctx->init_config.tuning.TM_OUT_BIT_DEPTH  = 14;
                CHICDK_LOG_VERBOSE("TuningData pChromatix is NULL, loading default values");

                m_p_shdr_ctx->init_config.tuning.ADC_BIT_DEPTH                             = 20;
                m_p_shdr_ctx->init_config.tuning.TM_OUT_BIT_DEPTH                          = 14;
                m_p_shdr_ctx->init_config.tuning.HDR_MAX_WEIGHT                            = 256;
                m_p_shdr_ctx->init_config.tuning.TM_GAIN                                   = 1.0f;

                m_p_shdr_ctx->init_config.perf_hint                                        = SHDR_PERF_HINT_NORMAL;
                m_p_shdr_ctx->init_config.num_gpu_passes                                   = 1;
                m_p_shdr_ctx->init_config.gpu_ltm_en                                       = 1;
                m_p_shdr_ctx->init_config.companding                                       = NO_COMPANDING;

                m_p_shdr_ctx->init_config.bayer_pattern                                    = SHDR_BAYER_GRBG;
                ...
                m_p_shdr_ctx->init_config.tuning.BLACK_LEVEL                               = 64.0f;
            }
            ...
            CHICDK_LOG_VERBOSE("bitsperpixel =%d", m_bitsperpixel);

            if (NULL != pChromatix && NULL != pChromatix->TMTable.TMTableEntry)
            {
                num_tm_table_entries       = pChromatix->TMTable.TMTableEntryCount;
            }
            else
            {
                CHICDK_LOG_ERROR("TuningData pChromatix is NULL, loading default values");
                num_tm_table_entries       = 10;
            }

            m_tm_tuning_table             = (TMTUNINGTABLE *) malloc(num_tm_table_entries * sizeof(TMTUNINGTABLE));

            if (NULL == m_tm_tuning_table)
            {
                m_p_shdr_ctx->init_config.tm_tuning_table = NULL;
                m_p_shdr_ctx->init_config.tm_tuning_table_size = 0;
                CHICDK_LOG_ERROR("Not enough space to allocate TM tuning table.");
                result = CDKResultENoMemory;
            }
            else
            {
                if (NULL != pChromatix && NULL != pChromatix->TMTable.TMTableEntry)
                {
                    for (INT i = 0; i < num_tm_table_entries; i++) {
                        m_tm_tuning_table[i].target_ratio = pChromatix->TMTable.TMTableEntry[i].target_ratio;
                        m_tm_tuning_table[i].gtm = pChromatix->TMTable.TMTableEntry[i].gtm;
                        m_tm_tuning_table[i].ltm_max_gain = pChromatix->TMTable.TMTableEntry[i].ltm_max_gain;
                        m_tm_tuning_table[i].ltm_comp_target = pChromatix->TMTable.TMTableEntry[i].ltm_comp_target;
                        m_tm_tuning_table[i].ltm_inverse = pChromatix->TMTable.TMTableEntry[i].ltm_inverse;
                    }
                }
                else
                {
                    m_tm_tuning_table[0] = ((TMTUNINGTABLE){0.1f, 0.4f, 2.0f, 0.3f, 1.0f});
                    m_tm_tuning_table[1] = ((TMTUNINGTABLE){0.2f, 0.4f, 2.0f, 0.3f, 1.0f});
                    m_tm_tuning_table[2] = ((TMTUNINGTABLE){0.3f, 0.4f, 2.0f, 0.3f, 1.0f});
                    m_tm_tuning_table[3] = ((TMTUNINGTABLE){0.4f, 0.4f, 2.0f, 0.3f, 1.0f});
                    m_tm_tuning_table[4] = ((TMTUNINGTABLE){0.5f, 0.4f, 2.0f, 0.3f, 1.0f});
                    m_tm_tuning_table[5] = ((TMTUNINGTABLE){0.6f, 0.4f, 2.0f, 0.3f, 1.0f});
                    m_tm_tuning_table[6] = ((TMTUNINGTABLE){0.7f, 0.4f, 2.0f, 0.3f, 1.0f});
                    m_tm_tuning_table[7] = ((TMTUNINGTABLE){0.8f, 0.4f, 2.0f, 0.3f, 1.0f});
                    m_tm_tuning_table[8] = ((TMTUNINGTABLE){0.9f, 0.4f, 2.0f, 0.3f, 1.0f});
                    m_tm_tuning_table[9] = ((TMTUNINGTABLE){1.0f, 0.4f, 2.0f, 0.3f, 1.0f});
                }

                m_p_shdr_ctx->init_config.tm_tuning_table = m_tm_tuning_table;
                m_p_shdr_ctx->init_config.tm_tuning_table_size = num_tm_table_entries;
            }

            m_p_shdr_ctx->input_param.pregain_gr                    = 1.85f;
            m_p_shdr_ctx->input_param.pregain_gb                    = 1.54f;
            if (NULL != pChromatix )
                m_p_shdr_ctx->init_config.tuning.EXPOSURE_RATIO_REAL[0] = pChromatix->BayerProcTuningData.hdr_ratio;
            else
                m_p_shdr_ctx->init_config.tuning.EXPOSURE_RATIO_REAL[0] = 1;
            m_p_shdr_ctx->init_config.num_exposure                  = 1;
            m_p_shdr_ctx->init_config.enable_deinterlace            = 0;

        }
        else
        {
            CHICDK_LOG_VERBOSE("SHDR TuningData TuningManager == NULL");
        }
        ...
        CHICDK_LOG_VERBOSE("SetBufferInfo dst_stride=%d Output: buf_size=%d wxh=%dx%d",
        ...
        m_p_shdr_ctx->init_config.src_stride = m_inputWidth*2;
        ...
        m_p_shdr_ctx->init_config.input_buf[0].buf_size = m_inputWidth*m_inputHeight*(m_bitsperpixel/8);
        m_p_shdr_ctx->init_config.input_buf[0].buf = NULL;

        m_p_shdr_ctx->init_config.input_buf[0].fd = -1;

        CHICDK_LOG_VERBOSE("SetBufferInfo src_stride=%d Input: buf_size=%d wxh=%dx%d",
        ...
        m_p_shdr_ctx->input_param.offset[0] = 0;//m_inputWidth*(m_bitsperpixel/8))*2+m_bitsperpixel;
        m_p_shdr_ctx->input_param.offset[1] = 0;//(m_inputWidth*(m_bitsperpixel/8))*2+3880*2;

        m_p_shdr_ctx->init_config.offset[0] = 0;//shdr_ctx->input_param.offset[0];
        m_p_shdr_ctx->init_config.offset[1] = 0;//shdr_ctx->input_param.offset[1];

        CHICDK_LOG_VERBOSE("SetBufferInfo LEF offset=%d, SEF offset=%d", m_p_shdr_ctx->init_config.offset[0],
                                                                                 m_p_shdr_ctx->init_config.offset[1]);

        if (NULL == &(m_p_shdr_ctx->handle) || NULL == &(m_p_shdr_ctx->init_config))
        {
            CHICDK_LOG_ERROR("SHDR handle or init_config is NULL");
            result = CDKResultEFailed;
        }
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            CHICDK_LOG_ERROR(" Can not init library %d", ret_lib);
            result = CDKResultEFailed;
        }
        else
        {
            CHICDK_LOG_VERBOSE("SHDR Library Init Succesful");
        }
    }
...
CDKResult ChiDummyShdrNode::ProcessRequest(
    CHINODEPROCESSREQUESTINFO* pProcessRequestInfo)
{
    CHICDK_LOG_VERBOSE("Enter");
    CHIVENDORTAGBASEINFO vendorTagBaseInfo = { 0 };
    CDKResult result = CDKResultSuccess;
    SHDRCALLBACK *p_cback = new SHDRCALLBACK;
    if (!p_cback)
    {
        CHICDK_LOG_ERROR("failed: no memory");
        result = CDKResultEFailed;
    }
    BOOL satisfied;
#ifdef __CAMX_AUTO_CHANGES__
    if (WaitAEC())
    {
    ...
    {
        // No AEC so don't wait for dependency.
        satisfied = TRUE;
    }
#endif
    ...
    if (TRUE == satisfied)
    {
        if ((CDKResultSuccess == result) )
        {
            ...
            {
                CHICDK_LOG_ERROR("SHDR Handle is NULL");
                result = CDKResultEFailed;
            }
            ...
            else
            {
                ...
                CHICDK_LOG_VERBOSE("ReqId : %d ProcessRequest input buf w=%d, h=%d, st=%d, buf_size=%d, off=%d,buf=%p,fd=%d",
                ...
                {
                    CHICDK_LOG_ERROR("Can not map input buffer %d", lib_ret);
                    result = CDKResultEFailed;
                }
            }
        }
        ...
        if(pAFDMetadata != NULL) {
            ...
            if (bandingMode == ANDROID_STATISTICS_SCENE_FLICKER_50HZ) {
                ...
                CHICDK_LOG_VERBOSE(" banding mode %d", bandingMode);
            }
        ...
        FLOAT  pAWBBGainData  = 1.0;
            ...
            if (NULL != pMetaData)
            {
                ...
                CHICDK_LOG_VERBOSE("AEC ReqId : %d and expRatio:%f ", (int)pProcessRequestInfo->frameNum, expRatio);
                CHICDK_LOG_VERBOSE("Lexposure : %lld and Sexposure:%lld",
            ...
            {
                CHICDK_LOG_ERROR("AEC Meta is NULL req %d",(int)pProcessRequestInfo->frameNum);
            }
        if (expRatio <=1.0f)
        {
        ...
        if(expRatio <= 0.0 || isnan(expRatio))
            expRatio =63.9;

        float gtm_gamma = Calc_Gtm_Gamma(expRatio);
        CHICDK_LOG_VERBOSE("expRatio:%f and gtm_gamma :%f", expRatio, gtm_gamma);

        if(gtm_gamma >1.0 || gtm_gamma <= 0.0 )
            gtm_gamma = 0.23;

#ifndef SHDR_UNITY_AWB
        ...
        if (NULL != pAWBRGainMetaData)
        {
            ...
            CHICDK_LOG_VERBOSE("AWB ReqId : %d and rgain:%f", (int)pProcessRequestInfo->frameNum, pAWBRGainData);
        }
        else
        {
                CHICDK_LOG_VERBOSE("AWB R Gain Meta is NULL using default ");
        }
        ...
        if (NULL != pAWBGGainMetaData)
        {
            ...
            CHICDK_LOG_VERBOSE("AWB ReqId : %d and ggain:%f", (int)pProcessRequestInfo->frameNum, pAWBGGainData);
        }
        else
        {
                CHICDK_LOG_ERROR("AWB G Gain Meta is NULL");
        }
        ...
        if (NULL != pAWBBGainMetaData)
        {
            ...
            CHICDK_LOG_VERBOSE("AWB ReqId : %d and bgain:%f", (int)pProcessRequestInfo->frameNum, pAWBBGainData);
        }
        else
        {
                CHICDK_LOG_ERROR("AWB B Gain Meta is NULL");
        }
#endif
        ...
        if(CDKResultSuccess == result)
        {
            ...
            CHICDK_LOG_VERBOSE("ProcessRequest output buf w=%d, h=%d, st=%d, buf_size=%d, off=%d, buf=%p,fd=%d",
            ...
            if (SHDR_SUCCESS != lib_ret && SHDR_ALREADY_MAPPED != lib_ret)
            {
                CHICDK_LOG_ERROR("Can not map output buffer %d", lib_ret);
                result = CDKResultEFailed;
            }
        }
        ...
        if(CDKResultSuccess == result)
        {
...
#ifdef SHDR_UNITY_AWB
            m_p_shdr_ctx->input_param.pregain_gr = 1.0;
            m_p_shdr_ctx->input_param.pregain_gb = 1.0;
#else
...
#endif
            CHICDK_LOG_VERBOSE("PerFrame[] Stats HDR_MAX_WEIGHT:%d"
            " TM_GAIN:%f, ADC_BIT_DEPTH:%d, EXPOSURE_RATIO_REAL:%f, BLACK_LEVEL:%f"
            "TM_OUT_BIT_DEPTH:%d, pregain_gr:%f, pregain_gb:%f ",
            ...
            ChiNodeUtils::GetVendorTagBase("org.quic.camera.tuningdata","SHDRDumpRaw",&g_ChiNodeInterface,&vendorTagBaseInfo);
            VOID*  DumpRaw = NULL;
            UINT8* DRD = NULL;
            DumpRaw = ChiNodeUtils::GetMetaData(pProcessRequestInfo->frameNum,
                                                vendorTagBaseInfo.vendorTagBase|InputMetadataSectionMask,
                                                ChiMetadataDynamic,
                                                &g_ChiNodeInterface,
                                                m_hChiSession);
            m_p_shdr_ctx->input_param.Dump_Frame = 0;
            if (DumpRaw!=NULL)
            {
                DRD = reinterpret_cast<UINT8*>(DumpRaw);
                m_p_shdr_ctx->input_param.Dump_Frame = DRD[0];
            }
            ...
            CHICDK_LOG_VERBOSE("Sync shdr_lib_process return =%d", ret_lib);

        }
    }
    else
    {
        CHICDK_LOG_ERROR("SHDR waiting for dependency");
    }
...

{
    ...
    {
        CHICDK_LOG_ERROR("Unable to get AEC Sensor mode");
    }
#ifndef SHDR_UNITY_AWB
    if (CDKResultSuccess == result)
    {
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            CHICDK_LOG_ERROR("Unable to get AWBFrameControlBGain tag");
        }
    }
#endif
    return result;
    ...
    VOID * user_data)
{
    ...
    if (NULL == p_cback || NULL == stats) {
        CHICDK_LOG_ERROR("AECStatsReadyCallback p_cback or stats is NULL");
        result = CDKResultEFailed;
    }
    ...
    if(CDKResultSuccess == result)
    {
        ...
        if (NULL == pNode)
        {
            CHICDK_LOG_ERROR("AECStatsReadyCallback pNode == NULL");
        }
        else
        {
            ...
            CHICDK_LOG_VERBOSE("y_sum %f, numberOfBEStatsROIs is %d, num_grid_w %d"
            ...
            if (CDKResultSuccess != result)
            {
                CHICDK_LOG_ERROR("pSetMetadata is NULL");
            } else {
                CHICDK_LOG_VERBOSE("published stats %x %x %x req#=%u",
                    tagList[0], tagList[1], tagList[2], (unsigned int)tagData[index].requestId);
            }
        }
#endif
    }
...
CDKResult ChiDummyShdrNode::LoadLib()
{
    ...
    m_hShdrLib = ChiNodeUtils::LibMapFullName(libFilePath);
    if (NULL == m_hShdrLib)
    {
        CHICDK_LOG_ERROR("Error loading lib %s", libFilePath);
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            CHICDK_LOG_ERROR("Error Initializing one or more function pointers in Library");
...
VOID ChiDummyShdrNode::Autoltm2_Histogram_Matching(
  ...
  *b8bit = b;
  ...
  CHICDK_LOG_VERBOSE("autoltm2_histogram_matching -X");
}
...
VOID ChiDummyShdrNode::Shdr_Autoltm2_Unity_Gain(
  ...
  if (cdf == NULL || gain_lut == NULL) {
      CHICDK_LOG_ERROR("cdf or gain_lut is null ");
      return;
  }
  ...
  *k2_8bit = (l_weight) * k2_8bit_pre + (1.0 - l_weight) * k2_8bit_cur;
  ...
  CHICDK_LOG_VERBOSE("shdr_autoltm2_unity_gain-X");
