    ...
    CHINODECREATEINFO* pCreateInfo)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        m_resultDataRTB.masterCamera              = m_cameraInfo[m_primaryCameraIndex].pCameraConfig->cameraRole;
        m_resultDataRTB.masterCameraId            = m_cameraInfo[m_primaryCameraIndex].pCameraConfig->cameraId;
        m_resultDataRTB.recommendedMasterCamera   = m_resultDataRTB.masterCamera;
    ...
    CHINODEPROCESSREQUESTINFO* pProcessRequestInfo)
{
    ...
    CHICDK_LOG_ERROR("input number:%dd",pProcessRequestInfo->inputNum);
    if (TRUE == satisfied)
    {
        ...
        {
            CHICDK_LOG_ERROR("pInputMetaBK is null");
            result = CDKResultEInvalidPointer;
        }
        ...
        if (CDKResultSuccess == result)
        {
            for (UINT32 i = 0; i < pProcessRequestInfo->inputNum; i++)
            {
                ...
                if ( pInputMetaBokeh->cameraMetadata[i].masterCameraId == pInputMetaBokeh->cameraMetadata[i].cameraId)
                {
            ...
            if (DualCamCount == m_numOfLinkedCameras)
            {
                ...
                {
                    m_resultDataRTB.recommendedMasterCamera   = pInputMetaBokeh->cameraMetadata[1].cameraRole;
                    m_resultDataRTB.recommendedMasterCameraId = pInputMetaBokeh->cameraMetadata[1].cameraId;
                }
                else
                {
                    m_resultDataRTB.recommendedMasterCamera   = pInputMetaBokeh->cameraMetadata[0].cameraRole;
            ...
            else if (TripleCamCount == m_numOfLinkedCameras)
            {
                ...
                if (0 != pInputMetaBokeh->cameraMetadata[m_primaryCameraIndex].userCropRegion.width)
                {
                    ...
                    CHICDK_LOG_VERBOSE("zoom: %f", zoom);
                }
                else
                {
                    CHICDK_LOG_ERROR("primary camera: %d, usercropregion is 0", m_primaryCameraId);
                }
               ...
               // Fov sort
                ...
                if (zoom < 2)
                {
                    ...
                    m_resultDataRTB.recommendedMasterCamera     = m_cameraInfo[FovOderIndexArray[1]].pCameraConfig->cameraRole;
                ...
                else
                {
                    ...
                    m_resultDataRTB.recommendedMasterCamera     = m_cameraInfo[FovOderIndexArray[2]].pCameraConfig->cameraRole;
                    isactive[FovOderIndexArray[1]]              = TRUE;
                    isactive[FovOderIndexArray[2]]              = TRUE;
                }
            }

            m_resultDataRTB.masterCamera   = pInputMetaBokeh->cameraMetadata[0].masterCameraRole;
            ...
            for (UINT32 i = 0; i < m_numOfLinkedCameras; i++)
            {
                CHICDK_LOG_VERBOSE("Active cameras camera id[%d], index [%d] Active[%d]",
                    m_cameraInfo[i].pCameraConfig->cameraId, i, isactive[i]);
            }
        }
    }
    CHICDK_LOG_VERBOSE("recommend master id:%d, role:%d current master id:%d, role:%d",
        m_resultDataRTB.recommendedMasterCameraId,
        m_resultDataRTB.recommendedMasterCamera,
        m_resultDataRTB.masterCameraId,
        m_resultDataRTB.masterCamera);
