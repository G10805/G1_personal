...
static const CHAR*       pDefaultAWBLibName = "com.qti.stats.awb";
static const UINT32      MaxCameras         = 8;                            ///< Define max simultaneous cameras supported
static CHIAWBALGORITHMCALLBACKS g_AWBLibCallbacks;
static CHIAWBAlgorithm*         g_AWBLibAlgorithmIntf[MaxCameras];
static CHIALGORITHMINTERFACE    g_ChiAlgoInterface;                         ///< The instance save the CAMX Chi interface
...
static BOOL IsValidAWBGain(AWBAlgoGains* pAWBGains)
{
    if (NULL != pAWBGains &&
...
CDKResult AWBGetCapabilities(CHIALGORITHMCAPSINFO* pCapsInfo)
{
    ...
    STATS_LOG_LOW("Get supported Capabilities size:%d, algo mask: %d", pCapsInfo->size, pCapsInfo->algorithmCapsMask);
...
{
    STATS_LOG_HIGH("ChiAWBAlgorithmEntry");
    ...
    (void)CamX::OsUtils::SNPrintF(libFilename, FILENAME_MAX, "%s%s%s.%s",
            CamX::VendorCompPath, CamX::PathSeparator, pAWBLibName, CamX::SharedLibraryExtension);
    ...
    if (NULL != handle)
    {
        ...
        {
            STATS_LOG_ERROR("AWB lib entry function is NULL");
        }
    }
    else
    {
        STATS_LOG_ERROR("Failed loaf AWB library - %s", libFilename);
    }
}
