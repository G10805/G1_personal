...
static const CHAR* pDefaultOpenCLLibraryName = "libOpenCL";

#elif defined (_WIN32)

static const CHAR* pDefaultOpenCLLibraryName = "OpenCL";

#else
#error Unsupported target defined
#endif // defined(_LINUX) || defined(_WIN32)

static const CHAR* pOpenCLProgramBinName = "ChiNodeGPU.cl.bin";
...
CDKResult GPUOpenCL::InitializeFuncPtrs()
{
    ...
    {
        CHICDK_LOG_ERROR("Error Initializing one or more function pointers in Library: %s",
                  libFilePath);
...
CDKResult GPUOpenCL::Initialize()
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (CL_SUCCESS != error)
        {
            CHICDK_LOG_ERROR("Error getting the platform ID: %d", error);
        ...
        if (CDKResultSuccess == result)
        {
            ...
            if (CL_SUCCESS != error)
            {
                CHICDK_LOG_ERROR("Error getting the requested Device: %d", error);
                result = CDKResultEFailed;
            }
        }
        ...
        if (CDKResultSuccess == result)
        {
            ...
            {
                CHICDK_LOG_ERROR("Error creating an OpenCL context: %d", error);
        ...
        if (CDKResultSuccess == result)
        {
            ...
            {
                CHICDK_LOG_ERROR("Error creating the OpenCL Command Queue: %d", error);
                result = CDKResultEUnsupported;
            }
        }
    }
...
CDKResult GPUOpenCL::InitializeResources()
{
    ...
    if (CL_SUCCESS != error)
    {
        CHICDK_LOG_ERROR("Failed to create ds4 weights image with error %d", error);
        result = CDKResultEFailed;
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (CL_SUCCESS != error)
        {
            CHICDK_LOG_ERROR("Failed to create ds4 sampler with error %d", error);
            result = CDKResultEFailed;
        }
    }
...
CDKResult GPUOpenCL::CreateProgram()
{
    ...
    if (NULL == m_program)
    {
        ...
        CHICDK_LOG_VERBOSE("Fail to load program binary, create Program from source.");
        m_program = m_pfnCLCreateProgramWithSource(m_context, 1, reinterpret_cast<const CHAR**>(&m_pProgramSource), NULL, &err);

        if (CL_SUCCESS != err)
        {
            CHICDK_LOG_ERROR("clCreateProgramWithSource failed: error: %d", err);
            result = CDKResultEFailed;
        }
        ...
        if (CDKResultSuccess == result)
        {
            ...
            if (CL_SUCCESS != err)
            {
                ...
                CHICDK_LOG_ERROR("clBuildProgram failed: error: %d, Log: %s", err, log);
                result = CDKResultEFailed;
            }
        }
        ...
        if (CDKResultSuccess == result)
        {
            ...
            {
                // Not a fatal error
                CHICDK_LOG_WARN("Failed to write program binary");
            }
            else
            {
                CHICDK_LOG_VERBOSE("Save program binary successful %s", binFilePath);
            }
        }
    }
    else
    {
        CHICDK_LOG_VERBOSE("Load program binary %s successful!", binFilePath);
    }
    ...
    const CHAR*  pFileName)
{
    ...
    {
        CHICDK_LOG_ERROR("Error querying for number of devices. errNum = %d", errNum);
        result = CDKResultEFailed;
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        else
        {
            ...
            if (errNum != CL_SUCCESS)
            {
                CHICDK_LOG_ERROR("Error querying for devices. errNum = %d", errNum);
                result = CDKResultEFailed;
            }
        }
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        else
        {
            ...
            if (errNum != CL_SUCCESS)
            {
                CHICDK_LOG_ERROR("Error querying for program binary sizes. errNum = %d", errNum);
                result = CDKResultEFailed;
            }
            else
            {
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (errNum != CL_SUCCESS)
        {
            CHICDK_LOG_ERROR("Error querying for program binaries. errNum = %d", errNum);
            result = CDKResultEFailed;
        }
    }
        ...
        for (cl_uint i = 0; i < numDevices; i++)
        {
            ...
            if (pDevices[i] == device)
            {
                ...
                if (NULL != hProgBin)
                {
                    ...
                    {
                        CHICDK_LOG_VERBOSE("FWrite failed to write %" PRIu64 ,
                ...
                {
                    result = CDKResultEFailed;
                    CHICDK_LOG_VERBOSE("Fail to open file %s for device id = %d", pFileName, i);
                }
                break;
            }
        }
    ...
    const CHAR*  pFileName)
{
    ...
    if (NULL != hprogramBinary)
    {
        ...
        if (NULL != pProgramBinary)
        {
            ...
            {
                CHICDK_LOG_ERROR("clCreateProgramWithBinary failed: error: %d, status: %d", err, binaryStatus);
                program = NULL;
            }
            else
            {
        ...
        else
        {
            ...
            CHICDK_LOG_ERROR("clCreateProgramWithBinary failed: error: %d", err);
        }
...
CDKResult GPUOpenCL::InitializeKernel()
{
    ...
    if (CL_SUCCESS != err)
    {
        ...
        CHICDK_LOG_ERROR("clCreateKernel for the CopyImage Kernel failed: error: %d", err);
        result = CDKResultEFailed;
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (CL_SUCCESS != err)
        {
            ...
            CHICDK_LOG_ERROR("clCreateKernel for the RotateImage Kernel failed: error: %d", err);
            result = CDKResultEFailed;
        }
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (CL_SUCCESS != err)
        {
            ...
            CHICDK_LOG_ERROR("clCreateKernel for the FlipImage Kernel failed: error: %d", err);
            result = CDKResultEFailed;
        }
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (CL_SUCCESS != err)
        {
            ...
            CHICDK_LOG_ERROR("clCreateKernel for the DS4SinglePlane Kernel failed: error: %d", err);
            result = CDKResultEFailed;
        }
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (CL_SUCCESS != err)
        {
            ...
            CHICDK_LOG_ERROR("clCreateKernel for the BoxFilterSinglePlane Kernel failed: error: %d", err);
            result = CDKResultEFailed;
        }
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (CL_SUCCESS != err)
        {
            ...
            CHICDK_LOG_ERROR("clCreateKernel for the p010_to_pd10 Kernel failed: error: %d", err);
            result = CDKResultEFailed;
        }
    }
    ...
    cl_mem*  pUVImage)
{
    ...
    if (it != m_mapList.end())
    {
        ...
        if (foundMapList.pAddr == pAddr)
        {
            ...
            *pUVImage     = foundMapList.UVImage;
            bIsEntryFound = TRUE;
            CHICDK_LOG_VERBOSE("fd %d already maaped YUVImage=%p YImage=%p UVImage=%p pAddr=%p",
                                         fd, *pYUVImage, *pYImage, *pUVImage, pAddr);
        }
        else
        {
            CHICDK_LOG_VERBOSE("fd %d already maaped but pAddr %p mismatched", fd, pAddr);
    ...
    else
    {
        ...
        CHICDK_LOG_VERBOSE("Entry %d not found", fd);
    }
    ...
    cl_mem*  pUVImage)
{
    ...
    CHICDK_LOG_VERBOSE("Mapped fd %d YUVImage=%p YImage=%p UVImage=%p pAddr=%p",
                              fd, *pYUVImage, *pYImage, *pUVImage, pAddr);
}
    ...
    cl_mem*             pUVImage)
{
    ...
    CHICDK_LOG_INFO(" Create Separate Image input params"
    ...
    if (bMapEntryFound == FALSE)
    {
        ...
        ionMem.ext_host_ptr.allocation_type = (IsCompressedFormat(format)) ?
            (CL_MEM_ION_HOST_PTR_COMPRESSED_YUV_QCOM) :
        ...
        *pYUVImage = m_pfnCLCreateImage(m_context,
        ...
        {
            CHICDK_LOG_ERROR("clCreateImage failed: error: %d", err);
            result = CDKResultEFailed;
        }
...
VOID GPUOpenCL::ReleaseYUVImagePlanes()
{
    ...
    while (it != m_mapList.end())
    {
        ...
        CHICDK_LOG_INFO("Deleting fd %d YUVImage=%p YImage=%p UVImage=%p pAddr=%p",
            deleteMapEntery.fd,
    ...
    ChiFormat chiFormat)
{
    ...
    switch (chiFormat)
    {
        ...
        case ChiFormat::UBWCTP10:
            ...
            break;
        default:
            CHICDK_LOG_ERROR("Unsupported Chi Format for OpenCL: %d, falling back to NV12", chiFormat);
            break;
    }
    ...
    CLImageFormats format)
{
    ...
    switch (format)
    {
        ...
        case CLImageFormats::CLP010Linear:
            ...
            break;
        default:
            CHICDK_LOG_ERROR("Unknown format type: %d", format);
            break;
    }
    ...
    UINT32  height)
{
    ...
    if (CL_SUCCESS != err)
    {
        CHICDK_LOG_ERROR("ExecuteCopyImage failed: error: %d", err);
        result = CDKResultEFailed;
    }
    ...
    RotationAngle   rotation)
{
    ...
    case RotationAngle::Rotate270Degrees:
        ...
        break;
    default:
        CHICDK_LOG_ERROR("Unknown rotation angle %d", rotation);
    ...
    if (CL_SUCCESS != err)
    {
        CHICDK_LOG_ERROR("ExecuteRotateImage failed: error: %d", err);
        result = CDKResultEFailed;
    }
    ...
    FlipDirection direction)
{
    ...
    if (CL_SUCCESS != err)
    {
        CHICDK_LOG_ERROR("ExecuteFlipImage failed: error: %d", err);
        result = CDKResultEFailed;
    }
    ...
    UINT32        outStride)
{
    ...
    if (CL_SUCCESS != err)
    {
        CHICDK_LOG_ERROR("ExecuteConvertP010ImageToPD10 failed: error: %d", err);
        result = CDKResultEFailed;
    }
    ...
    CHINODEBUFFERHANDLE hInput)
{
    ...
    CHICDK_LOG_WARN("GPUOpenCL::CopyBuffer : NOT TESTED");
    ...
    CHINODEBUFFERHANDLE hInput)
{
    ...
    if (CDKResultSuccess != result)
    {
        CHICDK_LOG_ERROR("GPUOpenCL::CopyImage failed: error: %d", result);
    }
    ...
    FlipDirection       direction)
{
    ...
    for (UINT i = 0; i < hOutput->imageCount; i++)
    {
        ...
        CHICDK_LOG_ERROR("Input width: %d, inStride is %d, output stride is %d ", width, inStride, outStride);
        ...
        if (CL_SUCCESS != err)
        {
            CHICDK_LOG_ERROR("GPUOpenCL::FlipImage failed to create clBuffer dstY: error: %d", err);
        }
        ...
        if (CL_SUCCESS != err)
        {
            CHICDK_LOG_ERROR("GPUOpenCL::FlipImage failed to create clBuffer dstUV: error: %d", err);
        }
        ...
        if (CL_SUCCESS != err)
        {
            CHICDK_LOG_ERROR("GPUOpenCL::FlipImage failed to create clBuffer srcY: error: %d", err);
        }
        ...
        if (CL_SUCCESS != err)
        {
            CHICDK_LOG_ERROR("GPUOpenCL::FlipImage failed to create clBuffer srcUV: error: %d", err);
            result  = CDKResultEFailed;
        }
    ...
    if (CDKResultSuccess != result)
    {
        CHICDK_LOG_ERROR("GPUOpenCL::FlipImage failed: error: %d", result);
    }
    ...
    CHINODEBUFFERHANDLE hInput)
{
    ...
    if (CDKResultSuccess != result)
    {
        CHICDK_LOG_ERROR("GPUOpenCL::DownscaleBy4Image failed: result: %d, error: %d", result, err);
    }
    ...
    CHINODEBUFFERHANDLE hInput)
{
    ...
    if (2 == hInput->numberOfPlanes)
    {
        for (UINT i = 0; i < hOutput->imageCount; i++)
        {
            ...
            if ((CDKResultSuccess == result) && (NULL != dstYImage) && (NULL != srcYImage))
            {
                ...
                if (CDKResultSuccess != result)
                {
                    CHICDK_LOG_ERROR("Failed to downscale the first plane");
                }
            }
            ...
            if ((CDKResultSuccess == result) && (NULL != dstUVImage) && (NULL != srcUVImage))
            {
                ...
                if (CDKResultSuccess != result)
                {
                    CHICDK_LOG_ERROR("Failed to downscale the second plane");
                }
            }
    ...
    else
    {
        ...
        CHICDK_LOG_ERROR("Unable to downscale image with current format");
    }

    if (CDKResultSuccess != result)
    {
        CHICDK_LOG_ERROR("GPUOpenCL::DownscaleImage failed: result: %d, error: %d", result, err);
    }
    ...
    FLOAT           srcYMax)
{
    ...
    if (CL_SUCCESS != err)
    {
        CHICDK_LOG_ERROR("ExecuteDownscaleBy4SinglePlane failed setting arg %u with err %d", argIdx, err);
    }
    ...
    if (CL_SUCCESS != err)
    {
        CHICDK_LOG_ERROR("ExecuteDownscaleBy4SinglePlane failed: error: %d", err);
        result = CDKResultEFailed;
    }
    ...
    FLOAT           srcYMax)
{
    ...
    if (CL_SUCCESS != err)
    {
        CHICDK_LOG_ERROR("ExecuteBoxFilterSinglePlane failed setting arg %u with err %d", argIdx, err);
    }
    ...
    if (CL_SUCCESS != err)
    {
        CHICDK_LOG_ERROR("ExecuteBoxFilterSinglePlane failed: error: %d", err);
        result = CDKResultEFailed;
    }
    ...
    CHINODEBUFFERHANDLE hInput)
{
    ...
    for (UINT i = 0; i < hOutput->imageCount; i++)
    {
        ...
        if (CL_SUCCESS != err)
        {
            CHICDK_LOG_ERROR("GPUOpenCL::ConvertP010ImageToPD10 failed to create clBuffer dst: error: %d", err);
        }
        ...
        if (CL_SUCCESS != err)
        {
            CHICDK_LOG_ERROR("GPUOpenCL::ConvertP010ImageToPD10 failed to create clBuffer src: error: %d", err);
        }
    ...
    if (CDKResultSuccess != result)
    {
        CHICDK_LOG_ERROR("GPUOpenCL::ConvertP010ImageToPD10 failed: error: %d", result);
    }
...
CDKResult GPUOpenCL::Uninitialize()
{
    ...
    if (NULL != m_queue)
    {
        ...
        CHICDK_LOG_VERBOSE("GPUOpenCL ReleaseCommandQueue error -0 means success-: %d", err);
        m_queue = NULL;
    }
    ...
    if (NULL != m_context)
    {
        ...
        CHICDK_LOG_VERBOSE("GPUOpenCL ReleaseContext error -0 means success-: %d", err);
        m_context = NULL;
    }
    ...
    if (CDKResultSuccess != result)
    {
        CHICDK_LOG_ERROR("GPUOpenCL::Uninitialize Failed to unmap lib: %d", result);
    }
...
static UINT32           g_bypassCameraIdTagBase  = 0;   ///< Chi assigned runtime vendor tag base for the node
    ...
    CHIQUERYVENDORTAG* pQueryVendorTag)
{
    ...
    {
        result = CDKResultEInvalidPointer;
        CHICDK_LOG_ERROR("Invalide argument: pQueryVendorTag is NULL");
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            CHICDK_LOG_ERROR("pQueryVendorTag is smaller than expected");
            result = CDKResultEFailed;
        }
    }
    ...
    CHINODEDESTROYINFO* pDestroyInfo)
{
    ...
    {
        result = CDKResultEInvalidPointer;
        CHICDK_LOG_ERROR("Invalid argument: pDestroyInfo->hNodeSession is NULL");
    }
    ...
    CHINODEQUERYBUFFERINFO* pQueryBufferInfo)
{
    ...
    {
        result = CDKResultEInvalidPointer;
        CHICDK_LOG_ERROR("Invalid pQueryBufferInfo argument");
    }

    if ((CDKResultSuccess == result) && (NULL == pQueryBufferInfo->hNodeSession))
    {
        result = CDKResultEInvalidPointer;
        CHICDK_LOG_ERROR("Invalid pQueryBufferInfo->hNodeSessionn argument");
    }
    ...
    CHINODESETBUFFERPROPERTIESINFO* pSetBufferInfo)
{
    ...
    {
        result = CDKResultEInvalidPointer;
        CHICDK_LOG_ERROR("Invalid pSetBufferInfo argument");
    }

    if ((CDKResultSuccess == result) && (NULL == pSetBufferInfo->hNodeSession))
    {
        result = CDKResultEInvalidPointer;
        CHICDK_LOG_ERROR("Invalid pSetBufferInfo->hNodeSession argument");
    }
    ...
    CHINODEPROCESSREQUESTINFO* pProcessRequestInfo)
{
    ...
    {
        result = CDKResultEInvalidPointer;
        CHICDK_LOG_ERROR("Invalid pProcessRequestInfo argument");
    }

    if ((CDKResultSuccess == result) && (NULL == pProcessRequestInfo->hNodeSession))
    {
        result = CDKResultEInvalidPointer;
        CHICDK_LOG_ERROR("Invalid pProcessRequestInfo->hNodeSession argument");
    }
    ...
    VOID* pNodeSession)
{
    ...
    {
        CHICDK_LOG_ERROR("Null GpuNodePostPipelineCreate pNodeSession");
    }
    ...
    ChiNodeInterface* pNodeInterface)
{
    ...
    {
        result = CDKResultEInvalidPointer;
        CHICDK_LOG_ERROR("Invalid argument: pNodeInterface is NULL");
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            CHICDK_LOG_ERROR("CHINODEPROCREQUESTINFO is smaller than expected");
            result = CDKResultEFailed;
        }
    }
    ...
    CHINODEFLUSHREQUESTINFO* pInfo)
{
    ...
    {
        result = CDKResultEInvalidPointer;
        CHICDK_LOG_ERROR("Invalid argument: pInfo = %p", pInfo);
    }

    if ((CDKResultSuccess == result) && (NULL == pInfo->hNodeSession))
    {
        result = CDKResultEInvalidPointer;
        CHICDK_LOG_ERROR("Invalid argument: session handle = %p", pInfo->hNodeSession);
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            CHICDK_LOG_ERROR("CHINODEFLUSHREQUESTINFO is smaller than expected: pInfo->size = %d, sizeof(CHINODEFLUSHREQUESTINFO) = %d",
    ...
    CHINODERESPONSEINFO* pInfo)
{
    ...
    {
        result = CDKResultEInvalidPointer;
        CHICDK_LOG_ERROR("Invalid argument: session handle = %p", pInfo->hChiSession);
    }
    ...
    if (CDKResultSuccess == result)
    {
        ...
        {
            CHICDK_LOG_ERROR("CHINODERESPONSEINFO is smaller than expected: pInfo->size = %d, sizeof(CHINODEFLUSHREQUESTINFO) = %d",
                pInfo->size, static_cast<INT>(sizeof(CHINODEFLUSHREQUESTINFO)));
...
VOID ChiGPUNode::InitializeDownscaleBypassMap()
{
    ...
    {
        CHICDK_LOG_INFO("[%p] - InstanceId: %u ReqId: %u pConfig: %p - numConfigurations: %u",
        ...
        for (UINT i = 0; i < pCameraConfig->numConfigurations; i++)
        {
            ...
            switch(pInputConfig->bufferDownscaleType)
            {
                ...
                case BUFFER_DS16:
                    ...
                    break;
                default:
                    CHICDK_LOG_ERROR("Cannot map: %u", pInputConfig->bufferDownscaleType);
                    break;
            }
            ...
            CHICDK_LOG_VERBOSE("Physical Camera: %u -> Node: %u:%u @ InputPort: %u | Type: %u",
        ...
        {
            CHICDK_LOG_ERROR("Mismatching number of DS4(%u) <-> DS16(%u) inputs",
                      numDS4Mapped, numDS16Mapped);
        }
    }
    else
    {
        CHICDK_LOG_WARN("pCameraConfig: NULL");
    }
}
    ...
    CHINODECREATEINFO* pCreateInfo)
{
    ...
    {
        CHICDK_LOG_INFO("Before m_nodeFlags 0x%x", m_nodeCaps);
        ...
        CHICDK_LOG_INFO("After m_nodeFlags 0x%x m_bIsSkipProcessing %d", m_nodeCaps, m_bIsSkipProcessing);
    }
    if (m_nodeCaps & ChiNodeCapsGPUEnableMapping)
    {
        ...
        CHICDK_LOG_INFO("After m_nodeFlags 0x%x bIsBufferMappingEnabled %d", m_nodeCaps, bIsBufferMappingEnabled);
    }

    pCreateInfo->nodeFlags.canSetInputBufferDependency = TRUE;

    CHICDK_LOG_INFO("bypass set %d", m_nodeFlags.isBypassable);
    ...
    CHINODEQUERYBUFFERINFO* pQueryBufferInfo)
{
    ...
    CHICDK_LOG_INFO("numOutputPorts = %d, numInputPorts = %d",
    ...
    for (UINT32 i = 0; i < pQueryBufferInfo->numOutputPorts; i++)
    {
        ...
        CHICDK_LOG_VERBOSE("output port id = %d, format = %d",
    ...
    for (UINT32 i = 0; i < pQueryBufferInfo->numInputPorts; i++)
    {
        INT  strideAlignment    = 0;
        INT  scanlineAlignment  = 0;
        UINT outIdx             = (ChiNodeCapsGPUDownscale == m_nodeCaps) ? 0 : i; // During DS, we may have inputs > outputs

        CHINODEBUFFERREQUIREMENT* pInputOptions      = &pQueryBufferInfo->pInputOptions[i].inputBufferOption;
        CHINODEBUFFERREQUIREMENT* pOutputRequirement = &pQueryBufferInfo->pOutputPortQueryInfo[outIdx].pBufferRequirement[0];
        ChiFormat                 format             = pQueryBufferInfo->pInputOptions[i].inputBufferOption.format;

        CHICDK_LOG_INFO("Input port id = %d, Format = %d - outIdx: %u m_nodeCaps: %u",
    ...
    UINT previewHeight)
{
    ...
    {
        CHICDK_LOG_INFO("[E] - Preview Dimensions: (%u x %u)", previewWidth, previewHeight);
                                       ...
                                       CHINODEBUFFERHANDLE& hBuffer) -> CHIBUFFERMANAGERHANDLE
    {
        ...
        {
            CHICDK_LOG_ERROR("Could not create: %s", pName);
        }
        else
        {
            ...
            if (NULL == hBuffer)
            {
                CHICDK_LOG_ERROR("Could not obtain buffer from: %s", pName);
            }
        }
    ...
    CHINODESETBUFFERPROPERTIESINFO* pSetBufferInfo)
{
    ...
    CHICDK_LOG_INFO("OutputPortId: %u - (%u x %u)",
    ...
    CHINODEPROCESSREQUESTINFO* pProcessRequestInfo)
{
    ...
    for (UINT32 i = 0; i < pProcessRequestInfo->outputNum; i++)
    {
        ...
        CHICDK_LOG_VERBOSE("Request: %" PRIu64 " Bypass: %u Output Port: %u Fmt: %u - (%u x %u)",
                              ...
                              CHINODEBUFFERHANDLE hTemp) -> VOID
    {
        if (INVALID_INDEX != idxOutput)
        {
            ...
            if (TRUE == formatError)
            {
                CHICDK_LOG_ERROR("Unsupported downscale format from Input (Port: %u, Format: %u) to Output (Port: %u, Format: %u)",
    ...
    if (TRUE == shouldBypass)
    {
        ...
        CHICDK_LOG_VERBOSE("Master Camera Id: %u ds4Id: %u ds16Id: %u",
    ...
    CHINODEPROCESSREQUESTINFO* pProcessRequestInfo)
{
    ...
    CHICDK_LOG_INFO("ProcessRequest Entry m_nodeCaps %u Frame Num %" PRIu64 " Seq number %d numInputs: %u",
    ...
    if (m_vendorTagPreviewStreamPresent != 0)
    {
        ...
        if (NULL != pData)
        {
            ...
            CHICDK_LOG_VERBOSE("bIsPreviewStreamPresent %d, request %llu ", bIsPreviewStreamPresent,
                (long long int)pProcessRequestInfo->frameNum);
        }
        else
        {
            CHICDK_LOG_VERBOSE("Unable to read endorTag IsPreviewStreamPrasent request %llu",
                (long long int)pProcessRequestInfo->frameNum);
        }

        if ((m_bIsSkipProcessing == TRUE) && (FALSE == bIsPreviewStreamPresent))
        {
            CHICDK_LOG_VERBOSE("This is time to skip request %llu", (long long int)pProcessRequestInfo->frameNum);
        ...
        case ChiNodeCapsGPURotate:
            for (UINT32 i = 0; (i < pProcessRequestInfo->inputNum) && (CDKResultSuccess == result); i++)
            {
                ...
                if (TRUE == IsBypassableNode())
                {
                    ...
                    {
                        CHICDK_LOG_INFO("[BYPASS] ProcessRequest GPU Rotation as rotation = %d",
                            m_currentRotation);
                    ...
                    {
                        CHICDK_LOG_INFO("[NoBYPASS] ProcessRequest GPU Rotation rotation = %d",
                            m_currentRotation);
                        pProcessRequestInfo->pBypassData[i].isBypassNeeded = FALSE;
        ...
        case ChiNodeCapsGPUFlip:
            ...
            for (UINT32 i = 0; (i < pProcessRequestInfo->inputNum) && (CDKResultSuccess == result); i++)
            {
                if (TRUE == IsBypassableNode())
                {
                    ...
                    {
                        CHICDK_LOG_ERROR("[BYPASS] ProcessRequest GPU NOFLIP  %d , rotation = %d",
                            m_currentFlip,
                            m_currentRotation);
                        pProcessRequestInfo->pBypassData[i].isBypassNeeded         = TRUE;
                        pProcessRequestInfo->pBypassData[i].selectedInputPortIndex = i;
                    ...
                    {
                        CHICDK_LOG_ERROR("[NoBYPASS] ProcessRequest GPU Flip = %d , rotation = %d",
                            m_currentFlip,
                            m_currentRotation);
                        pProcessRequestInfo->pBypassData[i].isBypassNeeded = FALSE;
    ...
    if (CDKResultSuccess != result)
    {
        CHICDK_LOG_ERROR("ProcessRequest failed for request = %" PRIu64 "Seq number %d",
    ...
    CHINODEFLUSHREQUESTINFO* pFlushRequestInfo)
{
    ...
    CHICDK_LOG_VERBOSE("Flush request Id %" PRIu64 " from node", pFlushRequestInfo->frameNum);
