...
void WWANDBReceiverServiceDied(void* cookie) {
    ...
    auto thiz = static_cast<ILocAidlWWANDBReceiverCallback*>(cookie);
    sCallback = nullptr;
    if (nullptr != thiz) {
        thiz = nullptr;
    }
...
::ndk::ScopedAStatus LocAidlWWANDBReceiver::pushBSWWANDB(
    ...
    bsSpecialInfoListSize = std::min(bsLocationDataListSize, (int32_t)bsSpecialInfoList.size());
...
::ndk::ScopedAStatus LocAidlWWANDBReceiver::init(
    ...
    if (mCallbackIface != nullptr && mDeathRecipient != nullptr) {
        mDeathRecipient->unregisterFromPeer(mCallbackIface->asBinder().get(), mCallbackIface.get());
    }
    mCallbackIface = callback;
    if (mCallbackIface != nullptr && mDeathRecipient != nullptr) {
        mDeathRecipient->registerToPeer(mCallbackIface->asBinder().get(), mCallbackIface.get());
    } else {
