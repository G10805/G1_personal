...
void LBSAdapter::sendMapData(const string maString)
{
    struct sendMapDataMsg : public LocMsg {
        LBSAdapter* mLBSAdapter;
        LBSApiBase* mLBSApi;
        string mMaString;
        inline sendMapDataMsg(LBSAdapter* lbsAdapter, LBSApiBase* lbsApi,
                              const string& maString) :
                LocMsg(), mLBSAdapter(lbsAdapter), mLBSApi(lbsApi),
                mMaString(maString) {}
        inline virtual void proc() const {
            if (nullptr == mLBSApi) {
                LOC_LOGd("Null LBSApi pointer");
                return;
            }
            if (mLBSAdapter->isFeatureSupported(LOC_SUPPORTED_FEATURE_MEASUREMENTS_CORRECTION)) {
                LOC_LOGd("mDataSize = %d", mMaString.length());
                mLBSApi->sendMapData(mMaString);
            } else {
    ...
    sendMsg(new sendMapDataMsg(this, mLBSApi, maString));
}
