/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  Copyright (c) 2015 - 2016, 2021-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
void IzatPassiveLocationListener::stopClientPassiveLocationListening(const std::string clientName,
        IIzatManager *izatManager) {
    auto it = sObjects.find(clientName);
    if (it != sObjects.end()) {
        if (0 != izatManager->unsubscribeListener(it->second)) {
            LOC_LOGe("Failed to unsubcribe location listener for %s", it->first.c_str());
        }
        it->second = nullptr;
    }
}
...
)
{
    ...
    if ((NULL != izatLoc) && (NULL != ulpLoc)) {
        ...
        if (izatLoc->mHasVertUnc) {
            ...
            ulpLoc->gpsLocation.vertUncertainity = izatLoc->mVertUnc;
        }
