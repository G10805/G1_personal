...
void GtpWifiProxy::requestWifiLocationMsg::proc() const
{
    ...
    do {
        ...
        if (LocationRequestAction_Start == mLocationRequest.action) {
           ...
           BREAK_IF_NON_ZERO(5, card->addString("TO", "NLP-API"));
           BREAK_IF_NON_ZERO(6, card->addString("FROM", mWifiProxy->mProviderTag.c_str()));
           BREAK_IF_NON_ZERO(7, card->addString("REQ", "POSITION"));
           BREAK_IF_NON_ZERO(8, card->addBool("EMERGENCY-REQUEST",
                   mLocationRequest.emergencyRequest));
           BREAK_IF_NON_ZERO(9, card->finalize());
           BREAK_IF_NON_ZERO(10,
                   mWifiProxy->mIzatContext->mIPCMessagingProxyObj->sendMsg(card, "NLP-API"));
           LOC_LOGD("fireGTPFixSession: Send position request status to NLP-API %d",
                   mWifiProxy->mTxId);

           card = OutPostcard::createInstance();
           BREAK_IF_ZERO(11, card);
           BREAK_IF_NON_ZERO(12, card->init());
           BREAK_IF_NON_ZERO(13, card->addString("TO", "XTWiFi-PE"));
           BREAK_IF_NON_ZERO(14, card->addString("FROM", mWifiProxy->mProviderTag.c_str()));
           BREAK_IF_NON_ZERO(15, card->addString("REQ", "POSITION"));
           BREAK_IF_NON_ZERO(16, card->addInt32("TX-ID", ++mWifiProxy->mTxId));
           BREAK_IF_NON_ZERO(17, card->addBool("SERVER-ACCESS-ALLOWED", !(mNeedFixFromCachedScan)));
           BREAK_IF_NON_ZERO(18, card->addBool("EMERGENCY-REQUEST",
                   mLocationRequest.emergencyRequest));
           BREAK_IF_NON_ZERO(19, card->finalize());
           BREAK_IF_NON_ZERO(20,
                   mWifiProxy->mIzatContext->mIPCMessagingProxyObj->sendMsg(card, "XTWiFi-PE"));
           LOC_LOGD("fireGTPFixSession: Fire a position request to XTWiFi-PE %d",
                   mWifiProxy->mTxId);
