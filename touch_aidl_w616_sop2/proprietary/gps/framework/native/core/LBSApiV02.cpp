/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  Copyright (c) 2013-2014, 2016-2018, 2021-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
void LBSApiV02::injectFdclData(const FdclData& fdclData)
{
    ...
    for (uint32_t i = 0; i < fdclData.bsLocationDataList.size() &&
            i < total_size; ++i) {
    ...
    for (uint32_t i = 0, index = fdclData.bsLocationDataList.size() + i;
            i < fdclData.bsSpecialInfoList.size() && index < total_size;
                ++i, index = fdclData.bsLocationDataList.size() + i) {
...
void LBSApiV02::sendMapData(const string maString) {

    sendMsg(new LocApiMsg([this, maString]() {
        LocationError err = LOCATION_ERROR_SUCCESS;

        int32_t dataSize = maString.length();
        ...
        qmiLocInjectRawDataReqMsgT_v02 mapDataMsg = {};

        int32_t remainingBytesToSend = (int32_t)dataSize;
        mapDataMsg.injectionType = eQMI_LOC_MAP_AIDING_DATA_V02;
        mapDataMsg.totalSize = dataSize;
        mapDataMsg.totalParts = (dataSize + QMI_LOC_MAX_RAW_DATA_PART_LEN_V02 - 1) /
                QMI_LOC_MAX_RAW_DATA_PART_LEN_V02;
        mapDataMsg.partNum = 0;
        while (remainingBytesToSend > 0) {
            mapDataMsg.partNum++;
            mapDataMsg.partData_len =
                    std::min(remainingBytesToSend, QMI_LOC_MAX_RAW_DATA_PART_LEN_V02);
            maString.copy((char*)mapDataMsg.partData,
                          mapDataMsg.partData_len,
                          dataSize - remainingBytesToSend);

            req_union.pLocInjectRawDataReq = &mapDataMsg;
            LOC_LOGd("dataSize=%d partNum=%d partData_len=%d remainingBytesToSend=%d",
                     dataSize, mapDataMsg.partNum, mapDataMsg.partData_len, remainingBytesToSend);

            bytesToSend = (int)mapDataMsg.partData_len;
            k = 0;
            while (bytesToSend > 0) {
                howMany = std::min(chunk, bytesToSend);
                sstream.str("");
                for (int j = 0; j < howMany; j++) {
                    sstream << std::uppercase <<
                        std::hex << std::setw(2) << std::setfill('0')
                                << (int)mapDataMsg.partData[k + j];
                }
                sstream << std::endl;
                outStr = sstream.str();
                LOC_LOGv("SERIALIZEDMAPDATA = %s", outStr.c_str());
                bytesToSend -= howMany;
                k += howMany;
            }

            status = locClientSendReq(QMI_LOC_INJECT_RAW_DATA_REQ_V02, req_union);
            if (status != eLOC_CLIENT_SUCCESS) {
                LOC_LOGe("failed! status %d", status);
                err = LOCATION_ERROR_GENERAL_FAILURE;
                break;
            }
            remainingBytesToSend -= (int32_t)mapDataMsg.partData_len;
        }
