...
#define XTRA_INFO_MASK_XTRA_DISABLED (0x00000080) /**<  Xtra feature disabled  */
#define XTRA_REQ_MASK_INJECT_REQUEST (0x00000001) /**<  Request for injection of predicted orbits */
#define XTRA_REQ_MASK_SERVER_UPDATE (0x00000002) /**<  Update server list */
#define XTRA_REQ_MASK_REFRESH_UPDATE_RATE (0x00000004) /**<  Refresh periodicity of injection  */
#define XTRA_RATE_UPDATE_INJECTED_ORBIT (1) /**<  Injected orbit rate update */
#define XTRA_RATE_UPDATE_INTEGRITY (2) /**<  Integrity rate update  */
...
class IzatApiBase {
    ...
    LocRilSessionManager* mLocRilSessMgr;
protected:
...
public:
    ...
    virtual void injectEedGpsData(const char* data, uint32_t len, bool legacyRetry,
                                LocApiResponse* adapterResponse, uint32_t type = 0,
                                uint32_t interval = 0);
    virtual void injectEedGloData(const char* data, uint32_t len, bool legacyRetry,
                                LocApiResponse* adapterResponse, uint32_t type = 0,
                                uint32_t interval = 0);
    ...
    // XT Adapter
#ifdef LOC_QCRIL_SUPPORT_ENABLED
    virtual void getRilSessMgr();
#endif
