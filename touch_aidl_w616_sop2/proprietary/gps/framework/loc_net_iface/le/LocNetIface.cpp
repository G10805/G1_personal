...
#ifdef OPENWRT_BUILD
    // required to inform all call related information to openWRT Netifd
    // QCMAPLanClient is used to inform all call related information to openWRT Netifd
    // In order to integrate with openWRT Netifd (Network Interface Daemon)
    // NetIfd will do all processes like install routes, NAT etc
    mQcmapLanClient = new QCMAP_LAN_Client();
    if (NULL == mQcmapLanClient) {
        LOC_LOGe("Failed to allocate QCMAP_LAN_Client instance !");
        return;
    }
    LOC_LOGd("Created QCMAP_LAN_Client instance %p", mQcmapLanClient);
#endif
...
#ifdef OPENWRT_BUILD
    // required to inform all call related information to openWRT Netifd
    delete mQcmapLanClient;
    mQcmapLanClient = NULL;
#endif
...
void LocNetIface::qcmapClientCallbackInternal(
    ...
    case QMI_QCMAP_MSGR_WLAN_STATUS_IND_V01: {
        ...
        handleQcmapCallback(wlanStatusIndData);
        break;
    }
    ...
    case QMI_QCMAP_MSGR_BRING_UP_WWAN_IND_V01: {
        ...
        handleQcmapCallback(bringUpWwanIndData);
        break;
    }
    ...
    case QMI_QCMAP_MSGR_TEAR_DOWN_WWAN_IND_V01: {
        ...
        handleQcmapCallback(teardownWwanIndData);
        break;
    }
    ...
    case QMI_QCMAP_MSGR_BACKHAUL_STATUS_IND_V01:
    {
        ...
        handleQcmapCallback(backhaulStatusData);
        break;
     }
    ...
    case QMI_QCMAP_MSGR_WWAN_ROAMING_STATUS_IND_V01:
    {
        ...
        handleQcmapCallback(roamingStatusData);
        break;
    }
    ...
    case QMI_QCMAP_MSGR_WWAN_STATUS_IND_V01: {
        ...
        handleQcmapCallbackSetEnableNetIFd(wwanStatusIndData);
        break;
    }
#endif
...
#ifdef OPENWRT_BUILD
void LocNetIface::handleQcmapCallbackSetEnableNetIFd(
        qcmap_msgr_wwan_status_ind_msg_v01 &wwanStatusIndData) {
    ENTRY_LOG();
    ...
    if ((QCMAP_MSGR_WWAN_STATUS_CONNECTED_V01 == wwanStatusIndData.wwan_status) ||
        ...
        // Changes to integrate NetIfD - Pass wwanStatusIndData to LAN side client library
        ...
        // initialize the structure from QCMAP LAN Client
        qcmap_wwan_backhaul_info wwan_backhaul_info;
        memset(&wwan_backhaul_info, 0, sizeof(qcmap_wwan_backhaul_info));

        // Initialize enum for backhaul type
        qcmap_backhaul_type pdn_type = isIpv6 ? BACKHAUL_V6 : BACKHAUL_V4;

        LOC_LOGd("WWAN Status (Connected_v4,v6=3,9): %d, isIpv6: %d, isIpv4: %d, pdn_type: %d",
                wwanStatusIndData.wwan_status, isIpv6, isIpv4, (int)pdn_type);

        wwan_backhaul_info.profile_handle = profileHdl;
        /* condition to check if wwan_info_ex structure is valid */
        if (wwanStatusIndData.wwan_info_ex_valid) {
            // populate interface name
            memcpy(&(wwan_backhaul_info.iface_name), wwanStatusIndData.wwan_info_ex.iface_name,
                    sizeof(wwan_backhaul_info.iface_name));
            if (isIpv6) {
                // Fill IPv6 information
                // Public IPV6 address
                memcpy(&(wwan_backhaul_info.v6_addr), wwanStatusIndData.wwan_info_ex.v6_addr,
                        sizeof(wwan_backhaul_info.v6_addr));
                // IPV6 Gateway address
                memcpy(&(wwan_backhaul_info.v6_gw_addr), wwanStatusIndData.wwan_info_ex.v6_gw_addr,
                        sizeof(wwan_backhaul_info.v6_gw_addr));
                // IPV6 Primary DNS Address
                memcpy(&(wwan_backhaul_info.v6_pri_dns_addr),
                        wwanStatusIndData.wwan_info_ex.v6_pri_dns_addr,
                        sizeof(wwan_backhaul_info.v6_pri_dns_addr));
                // IPV6 Secondary DNS address
                memcpy(&(wwan_backhaul_info.v6_sec_dns_addr),
                        wwanStatusIndData.wwan_info_ex.v6_sec_dns_addr,
                        sizeof(wwan_backhaul_info.v6_sec_dns_addr));
                // IPV6 Prefix length
                wwan_backhaul_info.v6_addr_prefix_len = wwanStatusIndData.v6_prefix_len;
                LOC_LOGd("IPv6 iface_name: %s, v6_addr_prefix_len: %d",
                        wwan_backhaul_info.iface_name, wwan_backhaul_info.v6_addr_prefix_len);
            } else {
                // Fill IPv4 information
                // Public IPV4 address
                wwan_backhaul_info.v4_addr = wwanStatusIndData.wwan_info_ex.v4_addr;
                // IPV4 Gateway address
                wwan_backhaul_info.v4_gw_addr = wwanStatusIndData.wwan_info_ex.v4_gw_addr;
                // IPV4 Primary DNS Address
                wwan_backhaul_info.v4_pri_dns_addr =
                        wwanStatusIndData.wwan_info_ex.v4_pri_dns_addr;
                // IPV4 Secondary DNS address
                wwan_backhaul_info.v4_sec_dns_addr =
                        wwanStatusIndData.wwan_info_ex.v4_sec_dns_addr;
                // Populate IPV4 subnet mask
                wwan_backhaul_info.v4_addr_subnet_mask = wwanStatusIndData.v4_subnet_mask;
                LOC_LOGd("IPv4 iface_name: %s, v4_addr: 0x%x, gw_addr: 0x%x, subnet_mask: 0x%x",
                       wwan_backhaul_info.iface_name, wwan_backhaul_info.v4_addr,
                        wwan_backhaul_info.v4_gw_addr, wwan_backhaul_info.v4_addr_subnet_mask);
            }
            // send information to LAN CLient Library
            bool retval = mQcmapLanClient->EnableNetIFd(&wwan_backhaul_info, pdn_type);
            if (retval) {
                LOC_LOGd("Send succeeded to LAN client library");
                if (!isIpv6) {
                    // NAT required for V4 only, not required for V6 as clients can communicate
                    // with outside world with client IPV6 IP only
                    mQcmapLanClient->EnableNatType(wwan_backhaul_info.profile_handle);
                }
            }
            else {
              LOC_LOGe("Send failed to LAN client library");
            }
        }
    } else {
    ...
    // Notify backhaul status for WWAN. Currently Backhaul status CB is not
    // available for Openwrt. Till Backhaul changes are in, let us manually
    // notify WWAN backhaul information using this method.
    setCurrentBackHaulStatus(QCMAP_MSGR_WWAN_BACKHAUL_V01, isIpv4, isIpv6);
    notifyCurrentNetworkInfo(false);
}
#endif
...
void LocNetIface::handleQcmapCallback (
    ...
    if (bringUpWwanIndData.conn_status == QCMAP_MSGR_WWAN_STATUS_CONNECTED_V01 ||
        ...
        // Clear disconnect queue recevied before connection status update.
        ...
        if (numBackHaulClients > 0) {
            ...
            for (auto clientContext : mBackHaulDisconnReqCache) {
                ...
                disconnectBackhaul(clientContext.second.clientName, true,
...
void LocNetIface::handleQcmapCallback(
    ...
    if (teardownWwanIndData.conn_status == QCMAP_MSGR_WWAN_STATUS_DISCONNECTED_V01 ||
        ...
        if (mWwanCallStatusCb != NULL) {
            // Send wwan status callback to HAL for SUPL calls.
            LOC_LOGd("LOC_NET_WWAN_CALL_EVT_CLOSE_SUCCESS");
            // If we get success for IPv4v6 for first callback, ignore the second CB.
            if (LOC_APN_IP_IPV4V6 == getSuplIpType()) {
        ...
        if (mWwanCallStatusCb != NULL) {
            LOC_LOGd("LOC_NET_WWAN_CALL_EVT_CLOSE_FAILED");
            if (LOC_APN_IP_IPV4V6 == getSuplIpType()) {
                LOC_LOGd("IPV4V6 Current Disconnect Cb State: %d", mWwanSuplDiscnctCbState);
                if (LOC_NET_WWAN_SUPL_CB_STATE_DEFAULT == mWwanSuplDiscnctCbState) {
            ...
            // Send wwan status callback to HAL for SUPL calls.
            mWwanCallStatusCb(LOC_NET_WWAN_CALL_EVT_CLOSE_FAILED, LOC_NET_CONN_TYPE_WWAN_SUPL,
                    getSuplApnName(), getSuplIpType());
        }
    } else {
...
void LocNetIface::notifyObserverForWlanStatus(bool isWlanEnabled) {
    ...
    LocNetIfaceBase::sNotifyCb(LocNetIfaceBase::sNotifyCbUserDataPtr, {&wifiStateDataItem});
}
...
void LocNetIface::notifyObserverForNetworkInfo(
    ...
    int32 niConnType = (isConnected) ? (int32)connType:LOC_NET_CONN_TYPE_INVALID;
    // Create a network data item
    NetworkInfoDataItem networkInfoDataItem(niConnType, "", "", isConnected, isConnected,
            mIsRoaming, NETWORK_HANDLE_UNKNOWN, "");
    /* Notify back to client */
    LocNetIfaceBase::sNotifyCb(LocNetIfaceBase::sNotifyCbUserDataPtr, {&networkInfoDataItem});
}
...
bool LocNetIface::isAnyBackHaulConnected() {
...
#else
    // Currently Backhaul status CB is not available for Openwrt. Till
    // Backhaul changes are in, let us use GetWWANStatus for WWAN
    // status notification
    backhaulStatus.backhaul_type = QCMAP_MSGR_WWAN_BACKHAUL_V01;
    ...
    if (mQcmapClientPtr->GetWWANStatus(
        ...
        if (v4_status == QCMAP_MSGR_WWAN_STATUS_CONNECTED_V01) {
            backhaulStatus.backhaul_v4_available = true;
        } else if (v6_status == QCMAP_MSGR_WWAN_STATUS_IPV6_CONNECTED_V01) {
            backhaulStatus.backhaul_v6_available = true;
        }
    }
#endif
    ...
    */
    ...
    do {
        ...
        if (apn.empty() && (qcmapSubId != QCMAP_MSGR_PRIMARY_SUBS_V01) && !isSuplEs) {
            LOC_LOGw("This is an invalid configuration."
                "If subId is not LOC_PRIMARY_SUB then apn cannot be empty"
                "Trying to establish a connection on LOC_PRIMARY_SUB");
        ...
        if (!(apn.empty()) || (qcmapSubId != QCMAP_MSGR_PRIMARY_SUBS_V01) || isSuplEs) {
            ...
            if (!bProfileIsInTheList) {
                ...
                if (isSuplEs) {
                    ...
                    strlcpy(net_policy.apn_name, getSuplApnName(), QCMAP_MAX_NUM_BACKHAULS_V01);
                } else {
                    net_policy.profile_id_3gpp = (uint8_t) mCidDefaultProfile;
                    strlcpy(net_policy.apn_name, apnstr, QCMAP_MAX_NUM_BACKHAULS_V01);
                }
                ...
                if (mQcmapClientPtr->CreateWWANPolicyEx(net_policy,
...
#ifdef OPENWRT_BUILD
                    LOC_LOGd("Invoke CreateWWANPolicy for QCMAP LAN Client");
                    mQcmapLanClient->CreateWWANPolicy(profile_handle);
#endif
                    if (isSuplEs) {
                    ...
                    if (QMI_ERR_NO_EFFECT_V01 == qmi_err_num) {
                        // Not an error
                        LOC_LOGd("CreateWWANPolicyEx is already set for same profile_handle=%d",
                                profile_handle);
                        retValue = true;
...
bool LocNetIface::connectBackhaulInternal(const string& clientName,
    ...
    BackhaulContext ctx = { clientName, (uint16_t) subId, apn, (uint16_t) ipType };
    ...
    if (mapIter != mProfileHdlToBackhaulCtxt.end()) {
        LOC_LOGd("Connection already requested for profile handle:%d with Sub=%d, APN=%s",
                profile_handle, subId, apn.c_str());
        // Add this new client to ProfileHdl2Client map, backhaulCtxSet
        mapIter->second.emplace(ctx);
        sameProfileFound = true;
    } else {
        LOC_LOGd("New profile handle:%d with Sub=%d, APN=%s", profile_handle, subId, apn.c_str());
