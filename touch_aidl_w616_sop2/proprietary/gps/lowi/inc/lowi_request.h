...
Copyright (c) 2012-2013, 2015-2022 Qualcomm Technologies, Inc.
All Rights Reserved.
...
public:
  ...
  enum eRequestType
  {
    ...
    /** Set 11az Configuration */
    SET_11AZ_CONFIGURATION
  };
...
public:
  ...
  enum e6GChannelType
  {
    USE_NONE_6G_CHANNEL = 0x0,
    USE_ALL_6G_CHANNEL = 0x1,
    USE_PSC_6G_CHANNEL = 0x2,
  };

  /** Defines the bands bit for which the Discovery Scan Request can be made */
  enum eBandBit
  {
    BAND_INVALID_BIT = -1,
    BAND_2P4G_BIT    = 0,
    BAND_5G_BIT      = 1,
    BAND_6G_BIT      = 2,
    BAND_LAST_BIT    = BAND_6G_BIT + 1,
    BAND_MAX_BIT     = BAND_LAST_BIT,
  };

  /** Define the band mask for which discovery scan request can be made */
  enum eBandMask
  {
    INVALID_MASK = 0,
    /** 2.4 GHz only*/
    BAND_2P4_GHZ = 1 << BAND_2P4G_BIT,
    /** 5 GHz only */
    BAND_5_GHZ = 1 << BAND_5G_BIT,
    /** 6 GHz only */
    BAND_6_GHZ = 1 << BAND_6G_BIT,
  };
   ...
   */
  vector <LOWIChannelInfo> & getChannelInfoVec ();
  /**
  ...
  /**
   * Returns the band mask
   * @return RF band to scan.
   */
  uint32 getBandMask() const;

  void setBandMask(uint32 bmask);
...
/** Defines supported TMR types for 11AZ Ranging scan request */
enum eTmrType
{
  TMR_LEGACY = 0, /* Legacy/EDCA based TMR Ranging */
  TMR_NTB    = 1, /* Non triggered based Ranging */
  TMR_TB     = 2, /* Triggered based Ranging */
};

/** Defines supported Security types for 11AZ Ranging scan request */
enum eSecType
{
  SEC_NONE    = 0, /* No security while using 11AZ */
  SEC_FTM     = 1, /* Mac level security while using 11AZ */
  SEC_FTM_LTF = 2, /* Mac + Phy level security  while using 11AZ */
};
...
struct LOWINodeInfo
{
  ...
  /** 11AZ TMR releated parameters */
  /***************************************************************************
   *Bits 1:0:  TMR subtype (TB or NTB) check eTmrType for detail
   *Bits 3:2   Security mode (sec FTM, sec LTF, none), see eSecType for detail
   *Bits 5:4   TB Ranging priority
   *Bits 6:    I2R feedback policy
   *Bits 31:7: Reserved
   ***************************************************************************/
  ...
  #define TB_PRI_S          4
  #define TB_PRI_M          0x3
  #define I2R_FB_POLICY     6

  #define TMR_SET_TYPE(x, y) (x = ((x & ~(TYPE_M << TYPE_S)) | (y << TYPE_S)))
  #define TMR_GET_TYPE(x)   ((x & (TYPE_M << TYPE_S)) >> TYPE_S)
  #define TMR_SET_SEC(x, y)  (x = ((x & ~(SEC_M << SEC_S)) | (y << SEC_S)))
  #define TMR_GET_SEC(x)    ((x & (SEC_M << SEC_S)) >> SEC_S)
  #define TMR_SET_TB_PRI(x, y)  (x = ((x & ~(TB_PRI_M << TB_PRI_S)) | (y << TB_PRI_S)))
  #define TMR_GET_TB_PRI(x) ((x & (TB_PRI_M << TB_PRI_S)) >> TB_PRI_S)
  #define TMR_SET_I2R_FB_POLICY(x) (x = (x | (1 << I2R_FB_POLICY)))
  #define TMR_CLEAR_I2R_FB_POLICY(x) (x = (x & ~(1 << I2R_FB_POLICY)))
  #define TMR_GET_I2R_FB_POLICY(x) ((x & (1 << I2R_FB_POLICY)) >> I2R_FB_POLICY)

  uint32 tmrParams;

  /***************************************************************************
   *Bits 22:0:   Min time between measurements
   *Bits 31:23:  Reserved
   ***************************************************************************/
  #define MIN_DEL_S      0
  #define MIN_DEL_M      0x003FFFFF

  #define TMR_SET_MIN_DELTA(x, y) (x = ((x & ~(MIN_DEL_M << MIN_DEL_S)) | (y << MIN_DEL_S)))
  #define TMR_GET_MIN_DELTA(x)   ((x & (MIN_DEL_M << MIN_DEL_S)) >> MIN_DEL_S)

  uint32 tmrMinDelta;

  /***************************************************************************
   *Bits 19:0:   Max time between measurements
   *Bits 31:20:  Reserved
   ***************************************************************************/
  #define MAX_DEL_S    0
  #define MAX_DEL_M    0x0007FFFF

  #define TMR_SET_MAX_DELTA(x, y) (x = ((x & ~(MAX_DEL_M << MAX_DEL_S)) | (y << MAX_DEL_S)))
  #define TMR_GET_MAX_DELTA(x)  ((x & (MAX_DEL_M << MAX_DEL_S)) >> MAX_DEL_S)
...
#define RTT_REPORT_PER_FRAME_WITH_CFR_CIR 4
  ...
  /** Is rtt bg node */
  bool isRttBgNode;
  /** TB measurement periodicity in units of 10 TUs */
  uint32 tbPeriodicity;
  /** TB measurement duration in units of 100 Msec */
  uint32 tbDuration;
  /** TB max session expiry, Max delay between continous measurements */
  uint8 tbMaxSessionExpiry;
public:
  /**
   * Constructor
   */
...
enum eI2rLmrPolicy
{
  LOWI_I2R_LMR_USE_DEFAULT_POLICY       = 0,  /* used default policy */
  LOWI_I2R_LMR_FORCE_FTM_POLICY         = 1,  /* Force use FTM i2r lmr policy */
  LOWI_I2R_LMR_ABORT_ON_POLICY_MISMATCH = 2,  /* Abort ranging on i2r lmr policy mismatch */
};

/** LOWI 11az Config to be set to FW */
struct LOWI11azConfig
{
  int num_valid_configs;
  eI2rLmrPolicy i2r_lmr_policy;
  bool i2r_lmr_policy_valid;
  int chan_switch_dur;
  bool chan_switch_dur_valid;
  int tb_ista_aw;
  bool tb_ista_aw_valid;

  /** Constructor */
  LOWI11azConfig();
};

/** Set LOWI 11az Configuration */
class LOWISet11azConfig : public LOWIRequest
{
private:
  LOWI11azConfig m11azConfig;
public:

  /**
   * Constructor
   * lowi11azconfig: lowi 11az config to set
   */
  LOWISet11azConfig(uint32 requestID, LOWI11azConfig &lowi11azconfig);

  /** Destructor */
  ~LOWISet11azConfig();

  /** Returns 11az Config injected into device */
  const LOWI11azConfig& get11azConfig() const;
