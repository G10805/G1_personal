...
public:
    ...
    virtual void updateNetworkAvailability(uint64_t netHandle, bool isConnected,
                                           uint64_t caps, const char* apn) = 0;
    virtual void gnssNiResponse(uint32_t id, GnssNiResponse response) = 0;
    virtual int gnssInjectBestLocation(Location location) = 0;
    virtual void configSvStatus(bool enable) {}
    virtual void configNmea(bool enable) {}
    ...
    /******************************Android specific LocAIDL API********************************/
    //update ILocAidlCallback
    ...
    virtual void setEsStatusReceiverCbs(GnssCbBase* callbacks) {}
    //Izat subscription
    virtual void dataItemsUpdate (::std::unordered_set<loc_core::IDataItemCore *> & dset) {}
    virtual void dataItemUpdate (loc_core::IDataItemCore* dataItem) {}
