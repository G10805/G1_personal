...
=============================================================================*/
#define LOG_TAG "GARDEN_GnssLocationAPIClient"
...
GnssLocationAPIClient::~GnssLocationAPIClient() {
    lock_guard<std::mutex> guard(mGnssLock);
    if (mControlClient) {
...
int GnssLocationAPIClient::setCallbacks(GnssCbBase* callbacks) {
    ...
    if (callbacks != nullptr) {
        ...
        locationCallbacks.gnssDataCb = [this](const GnssDataNotification& gnssDataNotification) {
...
void GnssLocationAPIClient::onTrackingCb(const Location& location) {
...
void GnssLocationAPIClient::onGnssNiCb(uint32_t id, const GnssNiNotification& gnssNiNotification) {
...
void GnssLocationAPIClient::onGnssSvCb(const GnssSvNotification& gnssSvNotification) {
...
void GnssLocationAPIClient::onGnssDataCb(const GnssDataNotification& gnssDataNotification) {
...
void GnssLocationAPIClient::onGnssLocationInfoCb(
        const GnssLocationInfoNotification& gnssLocInfoNotif) {
    LOC_LOGD("(flags: %" PRIx64")", gnssLocInfoNotif.flags);
    lock_guard<std::mutex> guard(mGnssLock);
    if (mGnssCallbacks != nullptr) {
...
void GnssLocationAPIClient::onCapabilitiesCb(LocationCapabilitiesMask capabilitiesMask) {
    LOC_LOGd("(capabilities: %" PRIx64")", capabilitiesMask);
