...
GnssConfig GmccUtils::getUserInputGnssConfig(std::string prompt)
{
    ...
    while(!exit_loop) {
        ...
        case 4:
        case 5:
            getUserInputLppeControlPlaneMask(config);
            config.flags |= GNSS_CONFIG_FLAGS_LPPE_CONTROL_PLANE_VALID_BIT;
            getUserInputEnterToContinue();
            break;
        case 6:
            getUserInputLppeUserPlaneMask(config);
            config.flags |= GNSS_CONFIG_FLAGS_LPPE_USER_PLANE_VALID_BIT;
            getUserInputEnterToContinue();
            break;
        case 7:
        case 8:
...
GmccUtils::getUserInputGnssConfigBlacklistSvId(GnssConfig& config)
{
    ...
    if (maskInput) {
        ...
        uint64_t navicMask = getUserInputMask64Bit("Enter NAVIC Blacklist Mask (SV Range 401-414): ");
        ...
        convertGnssSvIdMaskToList(navicMask, config.blacklistedSvIds,
...
GmccUtils::getUserInputGnssSvTypesMask(std::string prompt)
{
    ...
    if (valueAsMask) {
        ...
        PRINTLN("GNSS_SV_TYPES_MASK_NAVIC_BIT  = (1<<4) = 16");

        mask = getUserInputMask64Bit("Enter hex mask value: ");

    } else {

        PRINTLN("Constellation values: gps / glo / gal / bds / qzss/ navic");
...
GmccUtils::printGnssConfigBlacklistSvId(const GnssConfig& config)
{
    ...
    PRINTLN("config.blacklistedSvIds.size() = %u", config.blacklistedSvIds.size());
    ...
    std::string navicBlacklistedSvIds = "NAVIC: ";
    for (GnssSvIdSource source: config.blacklistedSvIds) {
        ...
        case GNSS_SV_TYPE_NAVIC:
            navicBlacklistedSvIds.append(std::to_string(source.svId));
            navicBlacklistedSvIds.append(" ");
            break;
        default:
    ...
    PRINTLN("%s", navicBlacklistedSvIds.c_str());
}
...
void
GmccUtils::getUserInputLppeControlPlaneMask(GnssConfig& config)
{
    PRINTLN("Here are the bit values for the LPPe Control Plane mask:\n");

    PRINTLN("GNSS_CONFIG_LPPE_CONTROL_PLANE_DBH_BIT                      = (1<<0)  = 1  =   0x1");
    PRINTLN("GNSS_CONFIG_LPPE_CONTROL_PLANE_WLAN_AP_MEASUREMENTS_BIT     = (1<<1)  = 2  =   0x2");
    PRINTLN("GNSS_CONFIG_LPPE_CONTROL_PLANE_SENSOR_BARO_MEASUREMENTS_BIT = (1<<3)  = 8  =   0x8");
    PRINTLN("GNSS_CONFIG_LPPE_CONTROL_PLANE_NON_E911_BIT                 = (1<<4)  = 16 =  0x10");
    PRINTLN("GNSS_CONFIG_LPPE_CONTROL_PLANE_CIV_ADDRESS_BIT              = (1<<5)  = 32 =  0x20");

    config.lppeControlPlaneMask =
        (GnssConfigLppeControlPlaneMask)getUserInputMask64Bit("\nEnter LPPe Control Plane mask: ");

    // print the selected mask
    PRINTLN("\nLPPe Control Plane Mask Bits you've selected:\n");
    if (config.lppeControlPlaneMask & GNSS_CONFIG_LPPE_CONTROL_PLANE_DBH_BIT) {
        PRINTLN("GNSS_CONFIG_LPPE_CONTROL_PLANE_DBH_BIT");
    }
    if (config.lppeControlPlaneMask & GNSS_CONFIG_LPPE_CONTROL_PLANE_WLAN_AP_MEASUREMENTS_BIT) {
        PRINTLN("GNSS_CONFIG_LPPE_CONTROL_PLANE_WLAN_AP_MEASUREMENTS_BIT");
    }
    if (config.lppeControlPlaneMask & GNSS_CONFIG_LPPE_CONTROL_PLANE_SENSOR_BARO_MEASUREMENTS_BIT) {
        PRINTLN("GNSS_CONFIG_LPPE_CONTROL_PLANE_SENSOR_BARO_MEASUREMENTS_BIT");
    }
    if (config.lppeControlPlaneMask & GNSS_CONFIG_LPPE_CONTROL_PLANE_NON_E911_BIT) {
        PRINTLN("GNSS_CONFIG_LPPE_CONTROL_PLANE_NON_E911_BIT");
    }
    if (config.lppeControlPlaneMask & GNSS_CONFIG_LPPE_CONTROL_PLANE_CIV_ADDRESS_BIT) {
        PRINTLN("GNSS_CONFIG_LPPE_CONTROL_PLANE_CIV_ADDRESS_BIT");
    }
}

void
GmccUtils::getUserInputLppeUserPlaneMask(GnssConfig& config)
{
    PRINTLN("Here are the bit values for the LPPe User Plane mask:\n");

    PRINTLN("GNSS_CONFIG_LPPE_USER_PLANE_DBH_BIT                      = (1<<0)  = 1  =  0x1");
    PRINTLN("GNSS_CONFIG_LPPE_USER_PLANE_WLAN_AP_MEASUREMENTS_BIT     = (1<<1)  = 2  =  0x2");
    PRINTLN("GNSS_CONFIG_LPPE_USER_PLANE_SENSOR_BARO_MEASUREMENTS_BIT = (1<<3)  = 8  =  0x8");
    PRINTLN("GNSS_CONFIG_LPPE_USER_PLANE_NON_E911_BIT                 = (1<<4)  = 16 =  0x10");
    PRINTLN("GNSS_CONFIG_LPPE_USER_PLANE_CIV_ADDRESS_BIT              = (1<<5)  = 32 =  0x20");

    config.lppeUserPlaneMask =
        (GnssConfigLppeUserPlaneMask)getUserInputMask64Bit("\nEnter LPPe User Plane mask: ");

    // print the selected mask
    PRINTLN("\nLPPe Control Plane Mask Bits you've selected:\n");
    if (config.lppeUserPlaneMask & GNSS_CONFIG_LPPE_USER_PLANE_DBH_BIT) {
        PRINTLN("GNSS_CONFIG_LPPE_USER_PLANE_DBH_BIT");
    }
    if (config.lppeUserPlaneMask & GNSS_CONFIG_LPPE_USER_PLANE_WLAN_AP_MEASUREMENTS_BIT) {
        PRINTLN("GNSS_CONFIG_LPPE_USER_PLANE_WLAN_AP_MEASUREMENTS_BIT");
    }
    if (config.lppeUserPlaneMask & GNSS_CONFIG_LPPE_USER_PLANE_SENSOR_BARO_MEASUREMENTS_BIT) {
        PRINTLN("GNSS_CONFIG_LPPE_USER_PLANE_SENSOR_BARO_MEASUREMENTS_BIT");
    }
    if (config.lppeUserPlaneMask & GNSS_CONFIG_LPPE_USER_PLANE_NON_E911_BIT) {
        PRINTLN("GNSS_CONFIG_LPPE_USER_PLANE_NON_E911_BIT");
    }
    if (config.lppeUserPlaneMask & GNSS_CONFIG_LPPE_USER_PLANE_CIV_ADDRESS_BIT) {
        PRINTLN("GNSS_CONFIG_LPPE_USER_PLANE_CIV_ADDRESS_BIT");
    }
}
