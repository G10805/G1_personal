...
=============================================================================*/
#ifndef GFW_AIDLAPI_H
#define GFW_AIDLAPI_H
...
class GfwAidlApi : public IGnssAPI {
public:
    GfwAidlApi();
    virtual ~GfwAidlApi();
    ...
    void setEsStatusReceiverCbs(GnssCbBase* callbacks) override;
    ...
    void updateNetworkAvailability(uint64_t netHandle, bool isConnected,
                                   uint64_t caps, const char* apn) override;

    void gnssNiResponse(uint32_t id, GnssNiResponse response) {};
    int gnssInjectBestLocation(Location location) override;

    void configSvStatus(bool enable) override;
    void configNmea(bool enable) override;
    ...
    bool agnssRilUpdateNetworkAvailability(bool available, const std::string& apn) { return true; }
    ...
    //LOC HIDL APIs
    //Izat subscription
    void dataItemsUpdate (std::unordered_set<::loc_core::IDataItemCore *> & dset) override;
    void dataItemUpdate (::loc_core::IDataItemCore* dataItem) override;
