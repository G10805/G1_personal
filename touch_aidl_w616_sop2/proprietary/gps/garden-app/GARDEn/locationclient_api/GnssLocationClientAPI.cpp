...
=============================================================================*/
#define LOG_TAG "GARDEN_GnssLocationClientAPI"
...
static void gnssEnergyConsumedInfoCb(
        const location_client::GnssEnergyConsumedInfo& gnssEneryConsumed) {
    gardenPrint("<<< onGnssEnergyConsumedInfoCb energy: (valid=%d, value=%" PRIu64"",
            (gnssEneryConsumed.flags & location_client::ENERGY_CONSUMED_SINCE_FIRST_BOOT_BIT) != 0,
...
static void gnssMeasurementsCb(const location_client::GnssMeasurements& gnssMeasurements) {
    ...
    if (mPlocationClient->getCbOutput()) {
        ...
        gardenPrint(" time_ns: %" PRId64 " full_bias_ns: %" PRId64 ""
            ...
            " hw_clock_discontinuity_count: %d, flags: 0x%04x, isNHz: %d",
            ...
            gnssMeasurements.clock.flags, gnssMeasurements.isNhz);
        for (int i = 0;
...
void GnssLocationClientAPI::updateNetworkAvailability(uint64_t netHandle, bool isConnected,
                                                      uint64_t caps, const char* apn) {
    mPClient->updateNetworkAvailability(isConnected ? 1 : 0);
}
...
int GnssLocationClientAPI::flpStartSession(uint32_t mode, LocationOptions options, uint32_t id) {
    ...
    if (mode == SESSION_MODE_ON_FIX) {
        uint32_t option = 0xfff;
        uint32_t intervalInMs = options.minInterval;
        uint32_t distanceInMeters = options.minDistance;
        ::LocReqEngineTypeMask engMask = ::LOC_REQ_ENGINE_FUSED_BIT;
        int gnssReportOptions = option & 1<<0;
        gnssStartOptions(GNSS_REPORT, intervalInMs, distanceInMeters, engMask, gnssReportOptions);
        mSessionIdMap[id] = mode;
        return gnssStart();
    } else if (mode == SESSION_MODE_ON_FULL){
        numBatchingCb = 0;
        mSessionIdMap[id] = mode;
        return mPClient->startRoutineBatchingSession(
                options.minInterval, options.minDistance, batchingCb, onResponseCb);
    } else {
        LOC_LOGd("unsuported flp Mode!");
    }
}

int GnssLocationClientAPI::flpStopSession(uint32_t id) {
    auto iter = mSessionIdMap.find(id);
    if (iter != mSessionIdMap.end()) {
        if (iter->second == SESSION_MODE_ON_FIX) {
            mPClient->stopPositionSession();
        } else if (iter->second == SESSION_MODE_ON_FULL) {
...
int GnssLocationClientAPI::flpUpdateSession(uint32_t sessionMode,
        LocationOptions options, uint32_t id) {
