/*
Copyright (c) 2011-2012, 2014-2015, 2017, 2020, 2022 Qualcomm Technologies, Inc.
All Rights Reserved.
...
const uint32_t  NDKProvider::s_qNdkRebindTimeoutMs = 5000;
/** @brief Timeout in ms between link to DeathRecipient attempts */
const uint32_t  NDKProvider::s_qDeathRecipientReLinkTimeoutMs = 2000;
...
/** @brief max number of retry attempts for link to DeathRecipient */
const uint32_t  NDKProvider::s_qMaxReLinkAttempts = 5;
...
bool isDeathRecipientLinked = false;
bool isLinkAttemptExceed = false;
...
)
{
    ...
    IF_SLIM_LOGD
    {
        ...
        const uint64_t tTimeElapsedMs = slim_TimeToMilliseconds(
                    m_zSensorData.samples[m_zSensorData.samples_len - 1].sampleTimeOffsetMs);
...
)
{
    ...
    if (NULL == m_pzSensor){
        SLIM_LOGE("Asensor manager instance not available. service not enabled.");
...
)
{
    ...
    zSample.sampleTimeOffsetMs = calcMsTimeDiff(m_zSensorData.timeBase, tTimestampMs);
    zSample.sample[0] = zData[0];
    zSample.sample[1] = zData[1];
    zSample.sample[2] = zData[2];
    m_tLastSampleDelay = ((int64_t)(readCoreTimeUs() / 1000)) - tTimestampMs;
    ...
    SLIM_LOGV(
            ...
            m_zSensorData.samples_len,
            tTimestampMs,
            m_zSensorData.samples[m_zSensorData.samples_len].sampleTimeOffsetMs,
            m_tLastSampleDelay,
...
*/
uint16_t NDKProvider::calcMsTimeDiff(uint64_t tTs1Ms, uint64_t tTs2Ms)
{
    uint64_t tDiffMs = 0;
    if (tTs1Ms <= tTs2Ms)
    {
    ...
    else
    {
        ...
        SLIM_LOGE("Max limit of uint64_t used in offset calc\n");
    }
    SLIM_LOGD("Ts1Ms: %" PRIu64 " Ts2Ms: %" PRIu64 " tDiffMs: %" PRIu64,
             tTs1Ms, tTs2Ms, tDiffMs);
    return (uint16_t)tDiffMs;
}
...
bool NDKProvider::threadLoop()
{
    ...
    m_pzDeathRecipientTimer = slim_TimerCreate(eIPC_TIMER_ID_RELINK, THREAD_ID_NDK);
    ...
    m_zLooper->addFd(
            ...
            pzThreadSelf);
    /* signal handler is responsible to receive timercb */
    /* timercb is received in threadloop and this is redundant */
    m_zLooper->addFd(
    ...
    /* Handle BindNDK failure using timers */
    while (false == isBindSuccess)
    {
      SLIM_LOGE("NDK bind failed - Retry upon timeout.");
      slim_IpcMsgT* pz_Msg = NULL;
      while (NULL == (pz_Msg = slim_IpcReceive()));
      SLIM_LOGD("NDK IPC message received. q_MsgId:%" PRIu32 ", q_SrcThreadId:%" PRIu32,
        pz_Msg->q_MsgId,
      ...
      switch (pz_Msg->q_MsgId)
      {
        case eIPC_MSG_TIMER:
        {
          ...
          slim_Memscpy(&z_TimerMsg, sizeof(z_TimerMsg), pz_Msg->p_Data,
                        pz_Msg->q_Size);
          if (eIPC_TIMER_ID_REBIND == z_TimerMsg.q_TimerId)
          {
            SLIM_LOGD("q_TimerId:%d Retrying NDK Bind\n", z_TimerMsg.q_TimerId);
            bindNDKSensors(z_TimerMsg.q_Param);
          }
          break;
        }
        default:
          SLIM_LOGE("Unknown IPC received.");
          break;
      }
    }

    SLIM_LOGD("bind operation pass");

    linkDeathRecipient(0);
    while (false == isDeathRecipientLinked && false == isLinkAttemptExceed) {
      SLIM_LOGE("Link to Death Recipient Fail - Retry upon timeout.");
      slim_IpcMsgT* pz_Msg = NULL;
      while (NULL == (pz_Msg = slim_IpcReceive()));
      SLIM_LOGD("NDK IPC message received. q_MsgId:%" PRIu32 ", q_SrcThreadId:%" PRIu32,
        pz_Msg->q_MsgId,
      ...
      switch (pz_Msg->q_MsgId)
      {
        case eIPC_MSG_TIMER:
        {
          ...
          slim_Memscpy(&z_TimerMsg, sizeof(z_TimerMsg), pz_Msg->p_Data,
                        pz_Msg->q_Size);
          if (eIPC_TIMER_ID_RELINK == z_TimerMsg.q_TimerId)
          {
            SLIM_LOGD("q_TimerId:%d Re Linking Death Recipient\n", z_TimerMsg.q_TimerId);
            linkDeathRecipient(z_TimerMsg.q_Param);
          }
          break;
        }
        default:
    ...
    m_pzBinderTimer = 0;
    slim_TimerDestroy(m_pzDeathRecipientTimer);
    m_pzDeathRecipientTimer = 0;
...
)
{
    ...
    case eIPC_TIMER_ID_REBIND:
        ...
        break;
    case eIPC_TIMER_ID_RELINK:
        {
        // Attempt connect to ISensorManager death recipient
        linkDeathRecipient(qParam);
        }
        break;
...
)
{
    ...
    case eIPC_MSG_ID_REBIND:
        // Attempt connect to ISensorManager and query available sensors
        bindNDKSensors(0);
        // Rebind is attempted when sensor clean up is performed
        // Retry link to dead recipient
        linkDeathRecipient(0);
        break;
    default:
...
void NDKProvider::cleanupSensors()
{
    ...
    if (!slim_TimerStop(m_pzDeathRecipientTimer))
    {
        SLIM_LOGE("Failed to stop death recipient timer");
    }
    ...
    /* Reset the flags */
    isBindSuccess = false;
    isDeathRecipientLinked = false;
    isLinkAttemptExceed = false;
...
void NDKProvider::linkDeathRecipient
(
    uint32_t qAttemptNo
)
{
    ...
    SLIM_LOGD("Connecting Death Recipient; attempt=%" PRIu32, qAttemptNo);
    // Register death recipient
    m_zSensorMgr = ISensorManager::getService();
    if (NULL != m_zSensorMgr)
    {
        Return<bool> linked = m_zSensorMgr->linkToDeath(s_pzInstance, /*cookie*/ 0);
        if (!linked.isOk()) {
            SLIM_LOGE("Transaction error in linking to sensor service death");
            vSuccess = false;
        } else if (!linked) {
            SLIM_LOGE("Unable to link to sensor service death notifications");
            vSuccess = false;
        } else {
            SLIM_LOGI("Link to death recipient success!!");
            vSuccess = true;
        }
    }
    ...
    if (!vSuccess)
    {
        if (qAttemptNo == s_qMaxReLinkAttempts) {
            isLinkAttemptExceed = true;
            isDeathRecipientLinked = vSuccess;
            SLIM_LOGE("Link to Death recipient failed. Retry Attempts Exceeded");
        }
        else if (slim_TimerStart(m_pzDeathRecipientTimer,
                                 s_qDeathRecipientReLinkTimeoutMs, qAttemptNo + 1))
        {
            SLIM_LOGI("Delaying link to death recipient by %" PRIu32 "ms.",
                                                  s_qDeathRecipientReLinkTimeoutMs);
        }
        else
        {
            SLIM_LOGE("Retry for Link to Death recipient operation failed.");
        }
    }
    isDeathRecipientLinked = vSuccess;
    return;
