...
void SensorPlayBackProvider::processGPS2APSSClkLog(unsigned char *pRaw)
{
  ...
  SLIM_LOGD("0x1C5e received, tunc- %u", apTimeStampUncertaintyMs);
  // once tunc is less than 10ms, the validity becomes true throughtout run unless
  // week is no longer valid.
  if ( 1 != pLog->msrReport.msrInfo.gpsSystemTime.validityMask.isSystemWeekMsecValid ||
       1 != pLog->msrReport.msrInfo.gpsSystemTime.validityMask.isSystemWeekValid ||
     ( !s_bValidGpsTimeRxd && (10 < apTimeStampUncertaintyMs) ) )
  {
  ...
  else
  {
    ...
    uint32 qGPS2APSSTimerOffset = nGPSMs;
    uint64 deltaMs = 0;
    // uint32 qGPS2APSSTimerOffset =  236179993 - nSysRtcTime;
    deltaMs = slim_TimeTickGetMilliseconds();
    qGPS2APSSTimerOffset -= deltaMs;
