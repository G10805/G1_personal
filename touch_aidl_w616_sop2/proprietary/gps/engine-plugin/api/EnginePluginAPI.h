...
/** Major File Version */
#define EP_INTERFACE_FILE_MAJOR_VERSION  7
/** Minor File Version */
#define EP_INTERFACE_FILE_MINOR_VERSION  2
...
/** Delete DR Sensor calibration */
#define EP_DELETE_DR_SENSOR_CALIBRATION ((epAidingdataMask)0x10000000)
...
/** Triggers Feature activation */
#define EP_FEATURE_ACTIVATE_CONNECTION ((epConnectionStatus)0x00000008)
...
/** SBAS L1 RF Band - Data*/
#define EP_GNSS_SIGNAL_SBAS_L1_CA     ((epGnssSignalTypeMask)0x00020000)
/** BEIDOU B2A_Q RF Band - Pilot*/
#define EP_GNSS_SIGNAL_BEIDOU_B2A_Q   ((epGnssSignalTypeMask)0x00040000)
/**BEIDOU B2B_I RF Band - Data*/
#define EP_GNSS_SIGNAL_BEIDOU_B2B_I   ((epGnssSignalTypeMask)0x00080000)
/**BEIDOU B2B_Q RF Band - Pilot*/
#define EP_GNSS_SIGNAL_BEIDOU_B2B_Q   ((epGnssSignalTypeMask)0x00100000)
...
/**  Position Report uses SBAS corrected SVs */
#define EP_POSITION_FLAG_SBAS_CORRECTED_SV_USED ((epPositionFlags)0x00001000)
...
*/
/** Set when vehicular data is used */
#define EP_POSITION_FLAG_VEH   ((epPositionFlags)0x00001000)
/** Set when visual data is used */
#define EP_POSITION_FLAG_VIS   ((epPositionFlags)0x00002000)
...
#define EP_POSITION_SOURCE_VPE     ((epPVTSourceType)0x04)
...
typedef enum {
    ...
    */
    ...
    /** Set Integrity Risk in the engine
        (1) Engine Plugin may send COMMAND_SET_INTEGRITY_RISK anytime.
        (2) The PPE shall support only one “integrity risk” at a time.
            Upon receiving a new “integrity risk”, the newly received
            “integrity risk” replaces any prior “integrity risk” as soon as possible.
        (3) When the “integrity risk” is in the range of [1, 4e9-1], the PPE
            shall compute the protection levels and outputs them in epPVTReport.
            When an integrity risk command is received by the PPE with an
            “integrity risk” value that is NOT in the range of [1, 4e9-1],
            the PPE shall disable/invalidate the protection levels in the PVT report.
       (4)  When the PPE is initialized on boot, the default shall be to disable/invalidate
            the protections levels in the PVT report.
    */
    EP_COMMAND_SET_INTEGRITY_RISK,
    /** Request the position engine to be put into pause state.
        Currently, only DRE engine will receive the pause/resume request.
        (1) When the engine is placed in paused state, the engine shall stop.
        (2) If there is an on-going session, engine shall stop producing fixes.
        (3) No Data will be fed to the engines in pause state.
        (4) Calling API to delete aiding data from the paused engine shall not have effect.
    */
    EP_COMMAND_PAUSE,
    /** Request the position engine to be put into resume state.
     *  Currently, only DRE engine will receive the pause/resume
     *  request. */
    EP_COMMAND_RESUME,
...
typedef struct {
    ...
    uint16_t isIntraSystemTimeBias3AndUncValid : 1;
    uint16_t : 7;
...
typedef struct {
   ...
   /** validity bit of the Time Group Delay for BDS B1C Band. */
   uint32_t isTgdB1CValid : 1;
   /** validity bit of the Inter Signal Correction for BDS B1C Band. */
   uint32_t isIscB1CValid : 1;
   /** validity bit of the Time Group Delay for BDS B2B Band. */
   uint32_t isTgdB2BiValid : 1;
   /** validity bit of the Inter Signal Correction for BDS B2B Band. */
   uint32_t isIscB2BiValid : 1;
   uint32_t : 24;
} epBdsTgdValidity;
...
typedef struct {
    ...
    union {
      ...
      epSetIntegrityRiskStructType integrityRisk;
    } cmdReqParameter;
    uint8_t reserve[32];
} epCommandRequest;
...
typedef struct {
   ...
   */
    ...
    /**  For GPS: reserved
         For BDS: BDS B1I to B2B_I time bias (B1I-B2B_I)
         For GAL: reserved
         For QZSS: reserved
         Unit: millisecond  */
    float intraSystemTimeBias3;
    /** Uncertainty of intraSystemTimeBias3
        Unit: millisecond  */
...
typedef struct {
    ...
    /** North standard deviation.
        Standard deviation is defined with 68% confidence level
        Unit: meter */
    ...
    /** Yaw rate is in body frame
        ...
        The positive value is clockwise and
        negative value is anti-clockwise
        Unit: radian/second */
    float yawRate;
    /** Body pitch
        Unit: radian */
    ...
    */
    ...
    /** Heading Rate is in NED frame.
        This field is obsoleted and is not used anymore.
        The engine can set this field validity bit to zero
        and doesn’t need to populate this field.
        YawRate will be used to propagate Heading.
        Range: +/- 180.
        The positive value is clockwise and
        negative value is anti-clockwise.
        Unit: degree/second */
    ...
    /*** Upper triangle elements of full matrix of position
        ...
        Unit: px,x, px,y, px,z, py,y, py,z, pz,z is in meter
...
/** Represents GPS Navigation model info
    ...
    Hardware doesn’t decode CNAV ephemeris on L5.
    However, Hardware decodes the satellite health information
    from GPS L5 signal. EnginePlugin will send GNSS measurement
    only when the signal of satellite is healthy.
*/
typedef struct {
    /**  GNSS SV ID.
         For GPS:      1 to 32
...
typedef struct {
   ...
   /** Inter Signal Correction - BDS B1C
       Unit: milliseconds */
   float iscB1c;
   /** Time of group delay - BDS B2B_I
       Unit: millisecond  */
   float tgdB2bi;
   /** Intersignal correction - BDS B2B_I
       Unit: millisecond  */
   float iscB2bi;
   uint8_t reserve[64];
} epPolyBdsTgd;
