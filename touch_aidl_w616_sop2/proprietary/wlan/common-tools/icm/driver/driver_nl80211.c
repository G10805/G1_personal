...
static int
convert_6g_power_mode_nl_to_internal(uint8_t nl_pwr_mode,
                                     ICM_AFC_POWER_MODE_T *pwr_mode)
{
    switch (nl_pwr_mode) {
        case QCA_WLAN_VENDOR_EXTERNAL_ACS_CHAN_LOW_POWER:
            *pwr_mode = ICM_LOW_POWER_MODE;
             break;
        case QCA_WLAN_VENDOR_EXTERNAL_ACS_CHAN_STANDARD_POWER:
            *pwr_mode = ICM_STANDARD_POWER_MODE;
             break;
        case QCA_WLAN_VENDOR_EXTERNAL_ACS_CHAN_VERY_LOW_POWER:
            *pwr_mode = ICM_VERY_LOW_POWER_MODE;
             break;
        default:
             icm_printf("nl80211: Invalid 6GHz power mode %u", nl_pwr_mode);
             return -1;
    }
    return 0;
}
...
static int qca_nl80211_start_acs_event(ICM_INFO_T *picm, u8 *data, size_t len)
{
    ...
    ICM_CAPABILITY_INFO_T *acs_data = NULL;
    ICM_AFC_POWER_MODE_T pwr_mode = 0;
    struct nlattr *nl, *nl_power, *nl_pcl[5], *cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_INFO_ATTR_MAX + 1];
    struct nlattr *power_info[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_POWER_INFO_ATTR_MAX + 1];
    int rem, rem_pwr, i = 0;
    u32 *freq_list = NULL;
    ...
    acs_data->afc_capable = !!tb[QCA_WLAN_VENDOR_ATTR_EXTERNAL_ACS_EVENT_AFC_CAPABILITY];
    ...
    if (nl != NULL) {
        acs_data->freq_list_len = nla_len(nl)/sizeof(u32);
        freq_list = nla_data(nl);

        ICM_ASSERT(sizeof(acs_data->freq_list[0]) == sizeof(u32));

        acs_data->freq_list = zalloc(sizeof(acs_data->freq_list[0]) *
                                acs_data->freq_list_len);
        if (acs_data->freq_list == NULL) {
            icm_printf("nl80211: Failed to allocate memory\n");
            ret = NL_SKIP;
            goto fail;
        }

        memcpy(acs_data->freq_list, freq_list,
                sizeof(u32) * acs_data->freq_list_len);
    }
    ...
    if (nl != NULL) {
        ...
        nla_for_each_nested(nl, tb[QCA_WLAN_VENDOR_ATTR_EXTERNAL_ACS_EVENT_PCL], rem) {
            ...
            /* <frequency (u32), weight (u8)> */
            if (nl_pcl[QCA_WLAN_VENDOR_ATTR_PCL_FREQ])
                acs_data->pcl.list[i] =
                    nla_get_u32(nl_pcl[QCA_WLAN_VENDOR_ATTR_PCL_FREQ]);
            if (nl_pcl[QCA_WLAN_VENDOR_ATTR_PCL_WEIGHT])
                acs_data->pcl.weight[i] =
                    nla_get_u8(nl_pcl[QCA_WLAN_VENDOR_ATTR_PCL_WEIGHT]);
            i++;
        }
    }
    ...
    nla_for_each_nested(nl, tb[QCA_WLAN_VENDOR_ATTR_EXTERNAL_ACS_EVENT_CHAN_INFO], rem) {
        ...
        if (cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_INFO_ATTR_SUPP_POWER_MODES])
            acs_data->afc_info[i].supp_power_modes = nla_get_u16(cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_INFO_ATTR_SUPP_POWER_MODES]);
        ...
        if (cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_INFO_ATTR_FREQ_SEG_0])
            acs_data->chan_info[i].ic_vhtop_ch_num_seg1 = icm_convert_mhz2channel(
                                                          nla_get_u32(cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_INFO_ATTR_FREQ_SEG_0]));

        if (cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_INFO_ATTR_FREQ_SEG_1])
            acs_data->chan_info[i].ic_vhtop_ch_num_seg2 = icm_convert_mhz2channel(
                                                          nla_get_u32(cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_INFO_ATTR_FREQ_SEG_1]));
        ...
        if (!cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_POWER_INFO_ATTR]) {
            icm_printf("nl80211: AFC Channel information missing\n");
            ret = NL_SKIP;
            goto fail;
        }

        nla_for_each_nested(nl_power, cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_POWER_INFO_ATTR], rem_pwr) {
            if (nla_parse(power_info, QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_POWER_INFO_ATTR_MAX,
                    nla_data(nl_power), nla_len(nl_power), NULL)) {
                icm_printf("nl80211: failed to parse AFC channel info\n");
                ret = NL_SKIP;
                goto fail;
            }

            if (power_info[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_POWER_INFO_ATTR_POWER_MODE]) {
                if (convert_6g_power_mode_nl_to_internal(nla_get_u8(power_info
                    [QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_POWER_INFO_ATTR_POWER_MODE]), &pwr_mode) == -1) {
                    icm_printf("nl80211: Unable to convert 6GHz power mode to ICM internal value");
                    ret = NL_SKIP;
                    goto fail;
                }
            }

            if (power_info[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_POWER_INFO_ATTR_POWER_VALUE])
                acs_data->afc_info[i].tx_power[pwr_mode]
                           = nla_get_u32(power_info[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_POWER_INFO_ATTR_POWER_VALUE]);
        }
        if (cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_INFO_ATTR_FREQ_OVERLAP_SEG_1]) {
            acs_data->chan_info[i+1] = acs_data->chan_info[i];
            acs_data->chan_info[i+1].ic_vhtop_ch_num_seg2 =
                                   icm_convert_mhz2channel(nla_get_u32(cinfo[QCA_WLAN_VENDOR_EXTERNAL_ACS_EVENT_CHAN_INFO_ATTR_FREQ_OVERLAP_SEG_1]));
            i++;
        }
        i++;
    }
    ...
    /* Save the interface name received in the first ACS event */
    if (!picm->acs_in_progress)
    {
        ifidx = if_nametoindex(picm->dev_ifname_global_event);
        if (!ifidx) {
        ...
        os_strlcpy(picm->dev_ifname, picm->dev_ifname_global_event, IFNAMSIZ);
    }
#endif /* ICM_RTR_DRIVER */

    picm->acs_in_progress = true;
    // Send Event to ICM application here
    process_nl80211_events(picm, EVENT_START_ACS, acs_data);

fail:
    if (acs_data) {
        if (acs_data->freq_list)
            free(acs_data->freq_list);
...
/*
 * Function     : icm_is_intf_radiochild
 * Description  : Determine if a VAP with a given interface index is the child
 *                of the radio for the given icm info instance.
 * Input params : Pointer to icm info, interface index for the VAP
...
static int icm_is_intf_radiochild(ICM_INFO_T *picm, int ifidx) {
    char vap_ifname[IF_NAMESIZE] = {0};
    u_int8_t zero_addr[sizeof(picm->radio_addr)] = {0};

    /* If the MAC address of the radio is uninitialized, return false */
    if (memcmp(picm->radio_addr, zero_addr, sizeof(picm->radio_addr)) == 0) {
        return FALSE;
    }

    if (if_indextoname(ifidx, vap_ifname) == NULL) {
            icm_printf("Error converting ifindex %d to interface name. Error: %s)",
                    ifidx, strerror(errno));
            return FALSE;
    }

    return icm_is_vap_radiochild(vap_ifname, picm->radio_ifname);
}
    ...
    for (i = 0; i < pdev->conf.num_radios; i++) {
        ...
        if ((ifidx  == -1 && !wdev_id_set) ||
            ...
            if (ifidx != -1) {
                ...
                picm->dev_ifname_global_event = dev_ifname;
            } else {
...
static int driver_nl80211_init_nl_global(struct nl80211_global *global)
{
    ...
    ret = nl_socket_set_msg_buf_size(global->nl_event,
            ICM_NL80211_EVENT_DESIRED_MSG_SIZE);
    if (ret < 0) {
        icm_printf("nl80211: Could not set message buffer size: %d (%s)\n",
                ret, strerror(-ret));
        goto err;
    }
...
int driver_nl80211_vendor_scan(void *data, struct nl80211_scan_config *params)
{
    ...
    if (params->freqs) {
        ...
        /* WAR for kernel 5.4, libnl version < 3.5: Add NLA_F_NESTED
         * with QCA_WLAN_VENDOR_ATTR_SCAN_FREQUENCIES. This is needed as missing
         * NLA_F_NESTED flag from nested attributes would cause rejection of these
         * NL commands with new kernel versions and libnl versions older than 3.5.
         * A work around for this has been added in nla_nest_start(), but that is
         * not included in commonly used libnl releases (Added from 3.5).
         */
        freqs = nla_nest_start(msg,
                QCA_WLAN_VENDOR_ATTR_SCAN_FREQUENCIES | NLA_F_NESTED);
        if (freqs == NULL)
            goto fail;
...
#ifdef ICM_RTR_DRIVER
    if (nla_put_u64(msg, QCA_WLAN_VENDOR_ATTR_SCAN_DWELL_TIME,
                ICM_VENDOR_SCAN_COMMON_DWELL_TIME_MS * MILLISEC_TO_MICROSEC))
        goto fail;
#endif /* ICM_RTR_DRIVER */
...
int driver_nl80211_set_channel(void *ctx, struct nl80211_channel_config *chan_config)
{
    ...
    if (!chan_config) {
        icm_printf("nl80211: chan_config is null\n");
        return ret;
    }

    ifidx = if_nametoindex(chan_config->ifname);
    if (!ifidx) {
        icm_printf("nl80211: if_nametoindex() failed\n");
        return ret;
    }

    icm_printf("nl80211: Send Set channel request on %s\n", chan_config->ifname);
    ...
    for (i = 0; i < chan_config->num_channel; i++) {
        ...
        if (nla_put_u32(msg,
                    ...
                    chan_config->channel_list[i].primary_freq) ||
                nla_put_u32(msg,
                    QCA_WLAN_VENDOR_ATTR_EXTERNAL_ACS_FREQUENCY_SECONDARY,
                    chan_config->channel_list[i].secondary_freq) ||
                nla_put_u8(msg,
                    QCA_WLAN_VENDOR_ATTR_EXTERNAL_ACS_CHANNEL_WIDTH,
                    chan_config->channel_list[i].channel_width) ||
                nla_put_u32(msg,
                    QCA_WLAN_VENDOR_ATTR_EXTERNAL_ACS_FREQUENCY_CENTER_SEG0,
                    chan_config->channel_list[i].seg0_center_freq) ||
                nla_put_u32(msg,
                    QCA_WLAN_VENDOR_ATTR_EXTERNAL_ACS_FREQUENCY_CENTER_SEG1,
                    chan_config->channel_list[i].seg1_center_freq) ||
                nla_put_u16(msg,
                    QCA_WLAN_VENDOR_ATTR_EXTERNAL_ACS_PUNCTURE_BITMAP,
                    chan_config->channel_list[i].puncture_bitmap))
            goto fail;
    ...
    if (tb[NL80211_ATTR_VENDOR_DATA]) {
        ...
        if (nl != NULL) {
            ...
            nla_for_each_nested(nl,
                    tb_vendor[QCA_WLAN_VENDOR_ATTR_RROP_INFO_RTPL],
                    rem) {
                if (nla_parse(nl_rtplinst, QCA_WLAN_VENDOR_ATTR_RTPLINST_MAX,
                            nla_data(nl), nla_len(nl), NULL)) {
...
static int wifi_offchancac_status_handler(struct nl_msg *msg,
                                          void *arg)
{
    struct nlattr *tb[NL80211_ATTR_MAX + 1];
    struct genlmsghdr *gnlh = NULL;
    struct nlattr *gnld = NULL;
    struct nl80211_channel_status *val = NULL;
    ...
    if (tb[NL80211_ATTR_VENDOR_DATA]) {
        ...
        if (msg_len > 0) {
            num_entries_in_msg = msg_len/
                sizeof(struct nl80211_ocac_linst);

            if (num_entries_in_msg == 0) {
                icm_printf("nl80211: Number of entries in message determined "
                           "to be 0. Message length=%u, size of OCAC channel "
                           "status list instance structure=%zu. Investigate."
                           "\n",
                           msg_len,
                           sizeof(struct nl80211_ocac_linst));
                return NL_SKIP;
            ...
            if ((num_entries_in_msg *
                ...
                icm_printf("nl80211: Message length=%u is not an exact "
                           "multiple of size of OCAC channel status list "
                           "instance structure=%zu. Investigate.\n",
                           msg_len,
                           sizeof(struct nl80211_ocac_linst));
                return NL_SKIP;
            }
            ...
            if (tb_vendor[QCA_WLAN_VENDOR_ATTR_PARAM_DATA]) {
                ...
                val->num_ocaclinst = num_entries_in_msg;
                val->ocacl = NULL;

                if (val->num_ocaclinst) {
                    val->ocacl = zalloc(num_entries_in_msg *
                                        sizeof(struct nl80211_ocac_linst));
                    if (!val->ocacl) {
                        icm_printf("nl80211: Could not allocate memory for ocac "
                                   "list instance. Investigate.\n");
                        return NL_SKIP;
                    }

                    memcpy((void *)val->ocacl, temp_msg_ptr,
                           num_entries_in_msg *
                               sizeof(struct nl80211_ocac_linst));
                }
            }
...
int driver_get_wifi_offchancac_status(void *ctx,
                                 const char *ifname,
                                 struct nl80211_channel_status *val)
{
    int ret = -1;
    int status = -1;
    struct nl_msg *msg = NULL;
    ...
    if (nla_put_u32(msg,
                ...
                QCA_NL80211_VENDOR_SUBCMD_GET_ACS_PRECAC_SUPPORT)) {
    ...
    status = send_and_recv_msgs(drv->global, msg,
                    wifi_offchancac_status_handler,
                    val);
    msg = NULL;

    if (status) {
        icm_printf("nl80211: get command for off-channel CAC channel list "
...
static int wifi_pri20_blockchanlist_handler(struct nl_msg *msg, void *arg)
{
    ...
    uint16_t *temp_msg_ptr = NULL;
    uint32_t msg_len = 0;
    uint8_t num_freq_to_copy = 0;
    uint8_t i = 0;
    ...
    if (tb[NL80211_ATTR_VENDOR_DATA]) {
        ...
        /*
         * The vendor specific data expected here is an array of zero or more
         * uint16_t primary 20 MHz center frequency values.
         */
        if (tb_vendor[QCA_WLAN_VENDOR_ATTR_PARAM_LENGTH]) {
            msg_len =
                nla_get_u32(tb_vendor[QCA_WLAN_VENDOR_ATTR_PARAM_LENGTH]);
        ...
        if (msg_len > 0) {
            if (tb_vendor[QCA_WLAN_VENDOR_ATTR_PARAM_DATA]) {
                ...
                num_freq_to_copy = MIN_INT((msg_len/sizeof(uint16_t)),
                                        ARRAY_LEN(val->freq));
                for (i = 0; i < num_freq_to_copy; i++) {
                    val->freq[i] = temp_msg_ptr[i];
                }
                val->num_freq = num_freq_to_copy;
            }
        }
    }
