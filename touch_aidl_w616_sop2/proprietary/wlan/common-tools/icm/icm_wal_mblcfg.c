...
int icm_wal_mblcfg_set_width_and_channel(ICM_INFO_T *picm, char *dev_ifname)
{
    ...
    if (channel_width > ICM_CH_BW_40) {
        /* figure out cfreq1_channel value from ic_ch_freq_seg1 of supported channel list. */
        cfreq1_channel = picm->chlist.ch[ch_index].ic_ch_freq_seg1;
    }
    ...
    if (channel_width == ICM_CH_BW_40MINUS) {
        ...
        sec_channel = channel + 4; // Secondary for 40+ is (primary + 20 Mhz)
    } else if (channel_width == ICM_CH_BW_80 || channel_width == ICM_CH_BW_160 || channel_width == ICM_CH_BW_320) {
...
send:
    ...
    for (i = 0; i < chan_config.num_channel; i++) {
        ...
        chan_list[i].puncture_bitmap = ICM_GET_CHANNEL_PUNCTURE_BITMAP(picm, band, channel);
    }
