...
const char *icm_seldbg_rejcode_str[] = {
    ...
    "Primary channel is OCAC incomplete ",     /* ICM_SELDBG_REJCODE_PRI_OCAC     */
    "Bonding channel is OCAC incomplete ",     /* ICM_SELDBG_REJCODE_BOND_OCAC    */
...
const char *icm_seldbg_rejcode_shortstr[] = {
    ...
    "PriOCAC",                             /* ICM_SELDBG_REJCODE_PRI_OCAC     */
    "BndOCAC",                             /* ICM_SELDBG_REJCODE_BOND_OCAC    */
...
const char *icm_phy_spec_str[] = {
    ...
    "11BEG",                               /* ICM_PHY_SPEC_11BEG             */
    "11BEA",                               /* ICM_PHY_SPEC_11BEA             */
...
static int icm_init_radio_iface(ICM_INFO_T* picm, const char *radio_ifname);
    ...
    for (i = 0; i < MAX_DEV_NUM; i++) {
        if (IS_DEV_ACTIVE(pdev, i)) {
            ...
            ICM_DPRINTF(pdev,
                        ...
                        "in progress on this radio...\n");
            if (icm_clear_radio_ext_acs_request_in_progress(picm) != SUCCESS)
                exit_code = EXIT_FAILURE;
#endif /* ICM_RTR_DRIVER */
...
#ifdef WLAN_SPECTRAL_ENABLE
    if (pthread_mutex_destroy(&pdev->spectral_nlsockop_ref_lock) != 0) {
...
static int icm_radio_init(ICM_DEV_INFO_T* pdev, const char *radio_ifname, const uint8_t index)
{
    ICM_INFO_T* picm = NULL;
    ...
    ICM_ASSERT(radio_ifname != NULL);

    if (index >= MAX_DEV_NUM) {
        icm_printf("Invalid radio index %d, name = %s\n", index, radio_ifname);
        return FAILURE;
    }
    picm = &pdev->icm[index];
    ...
    if (picm->init_complete)
            return SUCCESS;

    /* init device and radio interface names */
    picm->dev_index = index;
    picm->phy_spec = conf->dev_config[index].phy_spec;

    if (strlen(conf->dev_config[index].dev_ifname))
        os_strlcpy(picm->dev_ifname, conf->dev_config[index].dev_ifname, IFNAMSIZ);
    else
        os_strlcpy(picm->dev_ifname, DEFAULT_DEV_IFNAME, IFNAMSIZ);

    picm->radio_ifname =  strlen(conf->dev_config[index].radio_ifname) ? conf->dev_config[index].radio_ifname : DEFAULT_RADIO_IFNAME;
    picm->def_channel = conf->dev_config[index].def_channel;
    picm->channel_width = conf->dev_config[index].channel_width;
    picm->best_channel = 0;
    ...
    picm->usage_factor_80p80_bw80 = ICM_DEFAULT_USAGE_FACTOR_80p80_BW80;
...
#ifdef ICM_RTR_DRIVER
        if (icm_init_radio_iface(picm, radio_ifname) != SUCCESS) {
            ICM_DPRINTF(pdev, ICM_PRCTRL_FLAG_NONE, ICM_DEBUG_LEVEL_MAJOR, ICM_MODULE_ID_MAIN, "Could not initialize the radio %d interface\n", index);
            return FAILURE;
        }

        if(picm->numdevs == 0 && conf->walflag != ICM_WAL_CFG) {
            return SUCCESS;
        }
#endif /* ICM_RTR_DRIVER */
...
#ifdef WLAN_SPECTRAL_ENABLE
        if (conf->walflag != ICM_WAL_CFG && icm_init_spectral(picm) != SUCCESS) {
            status = FAILURE;
        }
#endif /* WLAN_SPECTRAL_ENABLE */
        ...
        if (icm_init_scanner_thread(picm) == FAILURE) {
            ICM_DPRINTF(pdev, ICM_PRCTRL_FLAG_NONE, ICM_DEBUG_LEVEL_MAJOR, ICM_MODULE_ID_MAIN, "Could not initialize scanner thread\n", index);
            return FAILURE;
        }

        /* Init scan job thread */
        if (icm_init_chanset_thread(picm) == FAILURE) {
            ICM_DPRINTF(pdev, ICM_PRCTRL_FLAG_NONE, ICM_DEBUG_LEVEL_MAJOR, ICM_MODULE_ID_MAIN, "Could not initialize channel set thread\n", index);
            return FAILURE;
        }
#endif /* ICM_RTR_DRIVER */
...
#ifdef WLAN_SPECTRAL_ENABLE
        if (conf->spectral_enab) {
            picm->scan_config.scan_type |= ICM_SCAN_TYPE_SPECTRAL_SCAN;
        }
#endif /* WLAN_SPECTRAL_ENABLE */

        if (status) {
            SET_DEV_ACTIVE(pdev, index);
        } else {
            CLR_DEV_ACTIVE(pdev, index);
        }

    picm->init_complete = true;
...
int icm_init(ICM_DEV_INFO_T* pdev)
{
    ...
    if (pthread_mutex_init(&pdev->spectral_nlsockop_ref_lock, NULL) != 0)
    {
        perror("pthread_mutex_init");
        exit(EXIT_FAILURE);
    }

    pdev->spectral_nlsockop_ref = 0;
#endif /* WLAN_SPECTRAL_ENABLE */
    ...
    for (i = 0; i < conf->num_radios; i++) {
        if (icm_radio_init(pdev, radio_listing.ifnames[i], i) != SUCCESS) {
            icm_printf("ICM radio(index %d, name %s) init failed\n", i, radio_listing.ifnames[i]);
            icm_sys_ifnames_deinit(&radio_listing);
            exit(EXIT_FAILURE);
        }
    }
...
int icm_vap_init_with_cap_info(ICM_DEV_INFO_T* pdev, ICM_CAPABILITY_INFO_T *cap_info, int dev_index)
{
    ...
    icm_printf("Dev Index %d, phy_mode: %s, iface_name: %s, addr: "MACSTR"\n"
            "chan_width: %s, spectral_capable: %d, add_chan_stat_supported: %d afc_capable: %d\n",
            dev_index, icm_phy_mode_to_string(cap_info->phy_mode),
            picm->dev_ifname, MAC2STR(picm->radio_addr), icm_chan_width_to_string(cap_info->chan_width),
            cap_info->spectral_capable, cap_info->add_chan_stat_supported, cap_info->afc_capable);
    ...
    picm->afc_capable = cap_info->afc_capable;
    picm->puncture_legacy_weightage = ICM_DEFAULT_LEGACY_CLIENT_WEIGHTAGE;
    picm->puncture_ofdma_weightage = ICM_DEFAULT_OFDMA_CLIENT_WEIGHTAGE;
    picm->puncture_nofdma_weightage = ICM_DEFAULT_NONOFDMA_CLIENT_WEIGHTAGE;
    ...
    picm->pcl->list = zalloc(sizeof(picm->pcl->list[0]) * picm->pcl->len);
    picm->pcl->weight = zalloc(sizeof(picm->pcl->weight[0]) * picm->pcl->len);
    ...
    icm_printf("Frequency list length: %zu, Channel Information: \n",
            cap_info->freq_list_len);
    picm->chlist.count = cap_info->freq_list_len;
    for (i = 0; i < (int)cap_info->freq_list_len; i++) {
        ICM_BAND_T band = ICM_BAND_INVALID;

        for (j = 0; j < ARRAY_SIZE(cap_info->chan_info); j++)
            if (cap_info->chan_info[j].ic_freq == cap_info->freq_list[i])
                break;

        if (j == ARRAY_SIZE(cap_info->chan_info)) {
            icm_printf("No channel info found for %u MHz\n",
                    cap_info->freq_list[i]);
            continue;
        }

        icm_printf("\tfreq: %u, flags: 0x%llx, flagext: 0x%x, maxregpower: %d, maxpower: %d, minpower: %d, regclassId: %u, antennamax: %u, seg1: %u, seg2: %u\n",
                ...
                cap_info->chan_info[j].ic_vhtop_ch_num_seg1,
                cap_info->chan_info[j].ic_vhtop_ch_num_seg2);

        if (picm->afc_capable) {
            icm_printf("\tSupported power modes: %d, SP maxregpower: %d, LPI maxregpower: %d, VLP maxregpower: %d\n",
                       cap_info->afc_info[j].supp_power_modes,
                       cap_info->afc_info[j].tx_power[ICM_STANDARD_POWER_MODE],
                       cap_info->afc_info[j].tx_power[ICM_LOW_POWER_MODE],
                       cap_info->afc_info[j].tx_power[ICM_VERY_LOW_POWER_MODE]);
        }

        band = icm_get_band_from_freq(cap_info->chan_info[j].ic_freq);
        if  (ICM_BAND_INVALID == band) {
            icm_printf("Unable to map frequency %u to recognized band. Skipping channel.\n",
                    cap_info->chan_info[j].ic_freq);
            continue;
        }

        icm_printf("\tAdding band %s as a candidate band.\n",
                icm_band_to_string(band));

        ICM_SET_AS_CANDIDATE_BAND(picm, band);
        ...
        picm->chlist.ch[i].ic_ieee =
            icm_convert_mhz2channel(cap_info->chan_info[j].ic_freq);
        picm->chlist.ch[i].channel = picm->chlist.ch[i].ic_ieee;
        ...
        picm->chlist.ch[i].supp_power_modes = cap_info->afc_info[j].supp_power_modes;
        for (pwr_idx = ICM_LOW_POWER_MODE; pwr_idx < ICM_MAX_POWER_MODE; pwr_idx++)
            picm->chlist.ch[i].afc_tx_power[pwr_idx] = cap_info->afc_info[j].tx_power[pwr_idx];
        ...
        picm->chlist.ch[i].puncture_bitmap = ICM_PUNCTURE_NONE;

        if ((ICM_IEEE80211_IS_CHAN_11AC_VHT80(&cap_info->chan_info[j]) ||
                ICM_IEEE80211_IS_CHAN_11AC_VHT80_80(&cap_info->chan_info[j]) ||
                ICM_IEEE80211_IS_CHAN_11AXA_HE80(&cap_info->chan_info[j]) ||
                ICM_IEEE80211_IS_CHAN_11AXA_HE80_80(&cap_info->chan_info[j]) ||
                ICM_IEEE80211_IS_CHAN_11BEA_EHT80(&cap_info->chan_info[j])) &&
                !picm->chlist.ch[i].ic_ch_freq_seg1) {
            picm->chlist.ch[i].ic_ch_freq_seg1 = cap_info->chan_info[j].ic_vhtop_ch_num_seg1;
        }

        if ((ICM_IEEE80211_IS_CHAN_11AC_VHT80_80(&cap_info->chan_info[j]) ||
                ICM_IEEE80211_IS_CHAN_11AXA_HE80_80(&cap_info->chan_info[j])) &&
                !picm->chlist.ch[i].ic_ch_freq_seg2) {
            picm->chlist.ch[i].ic_ch_freq_seg2 = cap_info->chan_info[j].ic_vhtop_ch_num_seg2;
        }
        ...
        if (ICM_IEEE80211_IS_CHAN_11AC_VHT160(&cap_info->chan_info[j]) ||
                 ICM_IEEE80211_IS_CHAN_11AXA_HE160(&cap_info->chan_info[j]) ||
                 ICM_IEEE80211_IS_CHAN_11BEA_EHT160(&cap_info->chan_info[j])) {
            picm->chlist.ch[i].ic_ch_freq_seg1_160 = cap_info->chan_info[j].ic_vhtop_ch_num_seg2;
        }

        if (ICM_IEEE80211_IS_CHAN_11BEA_EHT320(&cap_info->chan_info[j])) {
            picm->chlist.ch[i].ic_ch_freq_seg1_320[0] = cap_info->chan_info[j].ic_vhtop_ch_num_seg2;
            if ((j + 1) == ARRAY_SIZE(cap_info->chan_info)) {
                icm_printf("No overlapping segment 2 found for %u MHz\n",
                           cap_info->freq_list[i]);
                continue;
            }
            icm_printf("\tOverlapping 320 MHz seg2: %u\n", cap_info->chan_info[j + 1].ic_vhtop_ch_num_seg2);
            picm->chlist.ch[i].ic_ch_freq_seg1_320[1] = cap_info->chan_info[j + 1].ic_vhtop_ch_num_seg2;

            if (ICM_IEEE80211_IS_CHAN_11AC_VHT160(&cap_info->chan_info[j]) ||
                ICM_IEEE80211_IS_CHAN_11AXA_HE160(&cap_info->chan_info[j]) ||
                ICM_IEEE80211_IS_CHAN_11BEA_EHT160(&cap_info->chan_info[j])) {
                if (picm->chlist.ch[i].ic_ch_freq_seg1 < picm->chlist.ch[i].ic_ch_freq_seg1_320[0])
                    picm->chlist.ch[i].ic_ch_freq_seg1_160 =
                              cap_info->chan_info[j].ic_vhtop_ch_num_seg2 - CHANNEL_OFFSET_160MHZ_IN_320MHZ;
                else
                    picm->chlist.ch[i].ic_ch_freq_seg1_160 =
                              cap_info->chan_info[j].ic_vhtop_ch_num_seg2 + CHANNEL_OFFSET_160MHZ_IN_320MHZ;
            }
    ...
    picm->usage_factor_6g_320_bw40 = ICM_DEFAULT_USAGE_FACTOR_6GHZ_320_BW40;
    picm->usage_factor_6g_320_bw80 = ICM_DEFAULT_USAGE_FACTOR_6GHZ_320_BW80;
    picm->usage_factor_6g_320_bw160 = ICM_DEFAULT_USAGE_FACTOR_6GHZ_320_BW160;
    picm->usage_factor_6g_320_bw320 = ICM_DEFAULT_USAGE_FACTOR_6GHZ_320_BW320;

    picm->usage_factor_5g_320_bw40 = ICM_DEFAULT_USAGE_FACTOR_5GHZ_320_BW40;
    picm->usage_factor_5g_320_bw80 = ICM_DEFAULT_USAGE_FACTOR_5GHZ_320_BW80;
    picm->usage_factor_5g_320_bw160 = ICM_DEFAULT_USAGE_FACTOR_5GHZ_320_BW160;
    picm->usage_factor_5g_320_bw320 = ICM_DEFAULT_USAGE_FACTOR_5GHZ_320_BW320;

    picm->usage_factor_80p80_bw40 = ICM_DEFAULT_USAGE_FACTOR_80p80_BW40;
    picm->usage_factor_80p80_bw80 = ICM_DEFAULT_USAGE_FACTOR_80p80_BW80;

    picm->mb_scaling_factor_info.multiband_scaling_factor_bw20 =
        ICM_DEFAULT_MULTIBAND_SCALING_FACTOR_BW20;
    picm->mb_scaling_factor_info.multiband_scaling_factor_bw40 =
        ICM_DEFAULT_MULTIBAND_SCALING_FACTOR_BW40;
    picm->mb_scaling_factor_info.multiband_scaling_factor_bw80 =
        ICM_DEFAULT_MULTIBAND_SCALING_FACTOR_BW80;
    picm->mb_scaling_factor_info.multiband_scaling_factor_bw160 =
        ICM_DEFAULT_MULTIBAND_SCALING_FACTOR_BW160;
    picm->mb_scaling_factor_info.multiband_scaling_factor_bw320 =
        ICM_DEFAULT_MULTIBAND_SCALING_FACTOR_BW320;
    picm->mb_scaling_factor_info.multiband_scaling_factor_bw80p80 =
        ICM_DEFAULT_MULTIBAND_SCALING_FACTOR_BW80P80;
    picm->mb_scaling_factor_info.multiband_scaling_factor_max =
        ICM_DEFAULT_MULTIBAND_SCALING_FACTOR_MAX;
    picm->mb_scaling_factor_info.multiband_scaled_usability_margin_maxregpwr =
        ICM_DEFAULT_MULTIBAND_SCALED_USABILITY_MARGIN_MAXREGPWR;
...
ICM_SCAN_SELECT_STATUS_T icm_cfg_select_channel(ICM_INFO_T* picm, bool setchannel)
{
    ...
    int ret = -1;
    ICM_SELECTION_RESULTS_INFO_T selresults = {0};
    ICM_BAND_T band = ICM_BAND_INVALID;
...
#ifndef ICM_RTR_DRIVER
    /* In case both 2.4 GHz and 5 GHz are candidate bands, update chanlist for
     * 5 GHz and 2.4 GHz
     */
    if (ICM_IS_2_4GHZ_CANDIDATE_BAND(picm) &&
            ICM_IS_5GHZ_CANDIDATE_BAND(picm)) {
        ICM_DPRINTF(pdev, ICM_PRCTRL_FLAG_NONE, ICM_DEBUG_LEVEL_MAJOR, ICM_MODULE_ID_SELECTOR,
                "Refresh channel list for 5 GHz and 2.4 GHz in case of dual band.");
    ...
    ret = icm_select_home_channel_multiband(picm, picm->channel_width,
                        &selresults);

   if (!ret) {
       picm->best_band = selresults.selected_best_band;
       picm->best_channel = selresults.selected_best_chan_idx;
       if (ICM_CH_BW_80_PLUS_80 == selresults.selected_width) {
           picm->best_cfreq2_channel = selresults.selected_best_cfreq2_chan_idx;
       }
       picm->selected_channel_width = selresults.selected_width;
   } else {
       if ((NULL != picm->pcl) && (picm->pcl->len > 0) &&
                (picm->pcl->policy ==
                    QCA_WLAN_VENDOR_ATTR_EXTERNAL_ACS_POLICY_PCL_MANDATORY)) {
            icm_printf("Failed to find best channel. Selecting from PCL now\n");

            selresults.selected_best_chan_idx = -1;
            for (i = 0; i < picm->pcl->len; i++) {
               if (picm->pcl->weight[i] > tmp_pcl_weight) {
                   ...
                   selresults.selected_best_band = band;
                   selresults.selected_best_chan_idx =
                       icm_convert_mhz2channel(picm->pcl->list[i]);
               }
            }

            if (selresults.selected_best_chan_idx != -1) {
                icm_printf("Selecting best channel %d based on PCL weightage: %d\n",
                        selresults.selected_best_chan_idx, tmp_pcl_weight);
                picm->best_band = selresults.selected_best_band;
                picm->best_channel = selresults.selected_best_chan_idx;
                picm->selected_channel_width = ICM_CH_BW_20;
            } else {
                picm->best_band = ICM_BAND_INVALID;
                picm->best_channel = 0;
                picm->selected_channel_width = ICM_CH_BW_INVALID;
...
static void icm_config_defaults(ICM_CONFIG_T* pconfig)
{
    ...
    for (i = 0; i < ARRAY_LEN(pconfig->dev_config); i++) {
        os_strlcpy(pconfig->dev_config[i].radio_ifname, DEFAULT_RADIO_IFNAME,
                SIZEOF_IFNAME);
        os_strlcpy(pconfig->dev_config[i].dev_ifname, DEFAULT_DEV_IFNAME,
                SIZEOF_IFNAME);
        pconfig->dev_config[i].band = ICM_BAND_INVALID;
        pconfig->dev_config[i].channel_width = ICM_CH_BW_INVALID;
        pconfig->dev_config[i].phy_spec = ICM_PHY_SPEC_INVALID;
        pconfig->dev_config[i].def_channel = INVALID_CHANNEL;
    }
 ...
 * Description  : Determine if a VAP with a given name is the child of a radio
 *                with a given name. To determine this, we check the parent name
 *                in the sysfs entry for the VAP.
 * Input params : VAP name, radio name
 *                It is the caller's responsibility to ensure that the names are
 *                correct.
 * Return       : TRUE if the VAP is the child of the radio, FALSE if the VAP is
 *                not the child of the radio or if an error occurred when trying
 *                to determine the status.
 */
bool icm_is_vap_radiochild(const char *vapname, const char *radioname)
{
    char path[PATH_SYSNET_PARENT_SZ] = {0};
    ...
    if (vapname == NULL) {
        ICM_DPRINTF(pdev, ICM_PRCTRL_FLAG_NONE, ICM_DEBUG_LEVEL_MAJOR,
            ICM_MODULE_ID_MAIN, "Error: vapname is NULL\n");
        return FALSE;
    }

    if (radioname == NULL) {
        ICM_DPRINTF(pdev, ICM_PRCTRL_FLAG_NONE, ICM_DEBUG_LEVEL_MAJOR,
            ICM_MODULE_ID_MAIN, "Error: radioname is NULL\n");
        return FALSE;
    }

    if ((strlcpy(path, PATH_SYSNET, bufsize) >= bufsize)
            || (strlcat(path, vapname, bufsize) >= bufsize)
            || (strlcat(path, "/parent", bufsize) >= bufsize)) {
        ICM_DPRINTF(pdev, ICM_PRCTRL_FLAG_NONE, ICM_DEBUG_LEVEL_MAJOR,
            ICM_MODULE_ID_MAIN, "Error forming path name for VAP parent\n");
        return FALSE;
    }
    ...
    if (fp == NULL) {
        ICM_DPRINTF(pdev, ICM_PRCTRL_FLAG_NONE, ICM_DEBUG_LEVEL_MAJOR,
            ICM_MODULE_ID_MAIN, "Error opening file for VAP parent\n");
        return FALSE;
    }

    readstr = fgets(parent, sizeof(parent), fp);
    fclose(fp);

    if (readstr == NULL) {
        ICM_DPRINTF(pdev, ICM_PRCTRL_FLAG_NONE, ICM_DEBUG_LEVEL_MAJOR,
            ICM_MODULE_ID_MAIN, "Error reading name for VAP parent\n");
        return FALSE;
    }

    minstrlen = MIN(strlen(parent), strlen(radioname));
    if (minstrlen > IFNAMSIZ) {
        minstrlen = IFNAMSIZ;
    }

    if (strncmp(parent, radioname, minstrlen) == 0)
        return TRUE;
...
static int icm_build_vap_listing(sys_ifnames *vap_listing)
{
    ...
    dir = opendir(PATH_SYSCLASS_NET);

    if (NULL == dir) {
        perror("icm : " PATH_SYSCLASS_NET);
        return FAILURE;
    }
    ...
    while ((dentry = readdir(dir)) != NULL)
    {
        ...
        if (0 > snprintf(temp_name, IFNAMSIZ, "%s", dentry->d_name))
            continue;
        ...
        bool *ispresent)
{
    ...
    *ispresent = FALSE;
    ...
    if (icm_is_vap_radiochild(dev_ifname, picm->radio_ifname)) {
        *ispresent = TRUE;
    }
    ...
    while((optc = getopt(argc, argv, "ab:c:efg:hinq:r:s:S:tu:vx:")) != -1) {
        switch (optc) {
            ...
            case 'v':
                ...
                break;
            case 'x':
                /* option to enable/disable spectral scan in 2.4 GHz */
                if (!icm_is_numericalbool_valid(optarg)) {
                    fprintf(stderr, "Invalid value %s for enable/disable "
                                    "use of spectral scan in 2.4 GHz\n",
                            optarg);
                    print_usage();
                    exit(EXIT_FAILURE);
                }
    ...
    while(1) {
        ...
        if (ret == 0) {
            ...
            while ((chanset = icm_chanset_ll_del(picm, &picm->main_list))
                ...
                if (!icm_is_vap_radiochild(chanset->dev_ifname,
                            picm->radio_ifname)) {
...
static int icm_init_radio_iface(ICM_INFO_T* picm, const char *radio_ifname)
{
    ...
    for (i = 0; i < vap_listing.curr_size; i++ )
    {
        if (icm_is_vap_radiochild(vap_listing.ifnames[i], radio_ifname)) {
    ...
    if(picm->numdevs > 0) {
        if (pconf->walflag != ICM_WAL_CFG) {
            ...
            band = icm_get_radio_band(phymode);
            if (ICM_BAND_INVALID == band) {
                err("Could not get radio's band\n");
                goto bad;
            }
            ICM_SET_SINGLE_CANDIDATE_BAND(picm, band);
...
/*
 * Function     : icm_get_picm_index
 * Description  : get the index to the picm data structure corresponding to
 *                the radio interface
 * Input params : pointer pointing to ICM_DEV_INFO_T, radio interface from radio_listing
 * Return       : picm index in the pdev data structure
 *
 */
int icm_get_picm_index(ICM_DEV_INFO_T* pdev, const char *radio_listing_ifname)
{
   ...
   picm = &pdev->icm[idx];
   while (idx < conf->num_radios && picm->init_complete) {
        if (os_strcmp(picm->radio_ifname, radio_listing_ifname) == 0)
            return idx;
        picm = &pdev->icm[++idx];
   }
   return idx;
...
int icm_configure_radio_iface(ICM_DEV_INFO_T* pdev)
{
    ...
    for (i = 0; i < conf->num_radios; i++) {
        picm_idx = icm_get_picm_index(pdev, radio_listing.ifnames[i]);
        picm = &pdev->icm[picm_idx];
        ...
        if (!picm->init_complete) {
            if (icm_radio_init(pdev, radio_listing.ifnames[i], picm_idx) != SUCCESS) {
                icm_printf("ICM radio(index %d, name %s) init failed\n", i, radio_listing.ifnames[i]);
                status = FAILURE;
            }
        } else {
            if (icm_init_radio_iface(picm, radio_listing.ifnames[i]) != SUCCESS) {
                icm_printf("Could not initialize the radio(index %d, name %s) interface\n", i, radio_listing.ifnames[i]);
                status = FAILURE;
            }
        }
    }
...
/*
 * Function     : icm_clear_radio_ext_acs_request_in_progress
 * Description  : Helper function to clear the indication of external
 *                ACS being in progress, on a given radio.
 * Input params : pointer to icm info structure
 * Return       : SUCCESS/FAILURE
 */
int icm_clear_radio_ext_acs_request_in_progress(ICM_INFO_T *picm)
{
    return icm_wal_clear_radio_ext_acs_request_in_progress(picm);
}
#endif /* ICM_RTR_DRIVER */
