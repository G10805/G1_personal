...
void handleGetCTLTPCFTMDataStandaloneRSP(void *parms)
{
	int len = 0, pos = 0;
	CMD_GETCTLTPCFTMDATASTANDALONERSP_PARMS *pParms =
			(CMD_GETCTLTPCFTMDATASTANDALONERSP_PARMS *)parms;
	TLV2_CTLTPCFTMDATASTANDALONE_RSP_PARMS *rspStruct;
	...
	ALOGE("\n**********GETCTLTPCFTMDATASTANDALONERSP INFO**************\n");
	ALOGE("Status: %d numRows:%d\n", pParms->status, pParms->numRows);
	ALOGE("eachRowSize:%d\n", pParms->eachRowSize);

	if (pParms->eachRowSize != sizeof(TLV2_CTLTPCFTMDATASTANDALONE_RSP_PARMS)) {
		ALOGE("eachRowsize:%d is invalid", pParms->eachRowSize);
		return;
	}

	for (int i = 0; i < pParms->numRows; i++) {
		ALOGE("Row: %d Data Start\n", i);
		rspStruct = (TLV2_CTLTPCFTMDATASTANDALONE_RSP_PARMS *)
					&pParms->ctlRowData[pos];
		if (!rspStruct) {
			ALOGE("CTL response is missing for row:%d\n", i);
			return;
		}
		ALOGE("freq:%u rateBitIndex:%u, ctlRd:%u ctlGroup:%u"
		      " ctlNonOfdmaType:%u ctlChainPower:%d ctlFinalTotalPower:%d"
		      " r2pPower:%d sarPower:%d ctlExceptionValue:%d"
		      ...
		      " beamForming:%u numChain:%u nss:%u ctlEbw:%u",
		      rspStruct->freq, rspStruct->rateBitIndex, rspStruct->ctlRd,
		      rspStruct->ctlGroup, rspStruct->ctlNonOfdmaType,
		      rspStruct->ctlChainPower, rspStruct->ctlFinalTotalPower,
		      rspStruct->r2pPower, rspStruct->sarPower,
		      rspStruct->ctlExceptionValue, rspStruct->ctlExceptionValue2,
		      rspStruct->ctlExceptionValueApplied, rspStruct->ctlExceptionValueAdjust,
		      rspStruct->hcOffset, rspStruct->heOffset, rspStruct->regulatoryPowerLimit,
		      rspStruct->regulatoryPSDLimit, rspStruct->ctlAntElementGain,
		      rspStruct->ctlAddedMargin, rspStruct->ctlArrayGain,
		      rspStruct->beamForming, rspStruct->numChain, rspStruct->nss,
		      rspStruct->ctlEbw);
		pos += pParms->eachRowSize;
		ALOGE("\nRow: %d Data End\n", i);
	}
}
...
{
	uint8_t ctlRd, ctlGroup, ctlNonOfdmaType, beamForming,
		numChain, ctlVerbose;
	uint16_t freq, rateBitIndex, ctlEbw;
	uint32_t nss;
	...
	if (token == NULL) {
		ALOGE("%s: Unable to read ctlRd param", __func__);
		return -1;
	}
	ctlRd = atoi(token);
	...
	if (token == NULL) {
		ALOGE("%s: Unable to read ctlGroup param", __func__);
		return -1;
	}
	ctlGroup = atoi(token);
	...
	token = strtok_r(NULL, ",", &context);
	if (token == NULL) {
		ALOGE("%s: Unable to read ctlEbw param", __func__);
		return -1;
	}
	ctlEbw = atoi(token);
	...
	token = strtok_r(NULL, ",", &context);
	if (token == NULL) {
		ALOGE("%s: Unable to read ctlNonOfdmaType param", __func__);
		return -1;
	}
	ctlNonOfdmaType = atoi(token);

	token = strtok_r(NULL, ",", &context);
	if (token == NULL) {
		ALOGE("%s: Unable to read freq param", __func__);
		return -1;
	}
	freq = atoi(token);
	...
	token = strtok_r(NULL, ",", &context);
	if (token == NULL) {
		ALOGE("%s: Unable to read rateBitIndex param", __func__);
		return -1;
	}
	rateBitIndex = atoi(token);

	token = strtok_r(NULL, ",", &context);
	if (token == NULL) {
		ALOGE("%s: Unable to read nss param", __func__);
		return -1;
	}
	nss = atoi(token);
	...
	token = strtok_r(NULL, ",", &context);
	if (token == NULL) {
		ALOGE("%s: Unable to read beamForming param", __func__);
		return -1;
	}
	beamForming = atoi(token);

	token = strtok_r(NULL, ",", &context);
	if (token == NULL) {
		ALOGE("%s: Unable to read numChain param", __func__);
		return -1;
	}
	numChain = atoi(token);
	...
	token = strtok_r(NULL, ",", &context);
	if (token == NULL) {
		ALOGE("%s: Unable to read ctlExceptionValue param", __func__);
		return -1;
	}
	ctlExceptionValue = atoi(token);

	token = strtok_r(NULL, ",", &context);
	if (token == NULL) {
		ALOGE("%s: Unable to read ctlVerbose param", __func__);
		return -1;
	}
	ctlVerbose = atoi(token);

	/*Bind to TLV2 parser*/
	addTLV2p0BinCmdParser();
	addTLV2p0Encoder();
	registerGETCTLTPCFTMDATASTANDALONERSPHandler(handleGetCTLTPCFTMDataStandaloneRSP);

	tlv2CreateCmdHeader(CMD_GETCTLTPCFTMDATASTANDALONE);

	tlv2AddParms(2, PARM_CTLRD, ctlRd);
	tlv2AddParms(2, PARM_CTLGROUP, ctlGroup);
	tlv2AddParms(2, PARM_CTLEBW, ctlEbw);
	tlv2AddParms(2, PARM_CTLNONOFDMATYPE, ctlNonOfdmaType);
	tlv2AddParms(2, PARM_FREQ, freq);
	tlv2AddParms(2, PARM_RATEBITINDEX, rateBitIndex);
	...
	ALOGE("\n %s() TLV2 CMD_GETCTLTPCFTMDATASTANDALONE"
	      " ctlRd:%u ctlGroup:%u ctlEbw:%u ctlNonOfdmaType:%u"
	      " freq:%u rateBitIndex:%u nss:%u beamForming:%u numChain:%u"
	      " ctlExceptionValue:%u ctlVerbose:%u\n",
	      __func__, ctlRd, ctlGroup, ctlEbw, ctlNonOfdmaType, freq,
	      rateBitIndex, nss, beamForming, numChain, ctlExceptionValue,
	      ctlVerbose);
	/*Complete TLV2 stream by adding length, CRC, etc */
	pCmdStream = (TESTFLOW_CMD_STREAM_V2 *) tlv2CompleteCmdRsp();
	if (!pCmdStream) {
