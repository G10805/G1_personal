/*
 * Copyright (c) 2017,2019,2022,2023 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
static int
wlan_service_read_sys_param(const char *path, char *str, size_t max_size)
{
        size_t ret_len;
        FILE *fp = fopen(path, "r");

        if (fp == NULL) {
                wsvc_printf_err("Failed to open %s", path);
                return -1;
        }
        ...
        if (ret_len == 0 || ret_len == max_size) {
                wsvc_printf_err("Faild to read %s, ret_len = %zu", path, ret_len);
                return -1;
        }

        wsvc_printf_dbg("%s: %s %s", __FUNCTION__,  path, str);
        return 0;
}
...
int wlan_dp_service_start()
{
	...
	if (wlan_service_read_sys_param("/proc/sys/net/ipv4/tcp_adv_win_scale",
				    def_tcp_adv_win_scale,
				    SIZE_TCP_PARAM) == 0)
		def_tcp_adv_win_scale_valid = true;
...
static int wlan_service_set_tuning_parameter(const char* param, const char *value)
{
	UNUSED(param);
	UNUSED(value);
	return 0;
	...
	case WLAN_SVC_TP_NONE:
		if (rx_tp_flags & TCP_ADV_WIN_SCL)
			if (def_tcp_adv_win_scale_valid)
				wlan_service_set_adv_win_scale(
							def_tcp_adv_win_scale);
		break;
...
int wlan_service_rps_enable(void *data, int len)
{
	...
	if (fp != NULL) {
		...
		wsvc_printf_err("%s: unable to open /sys/devices/soc0/soc_id", __func__);
		/* set defalut SoC ID */
		soc_id = 0;
	}
...
{
	int rc = 0;
	int resources[8] = {0, 0, 0, 0, 0, 0, 0, 0};
	...
	if (data->coremask == 0x00f0) {
		...
		resources[1] = data->freq;
	} else if (data->coremask == 0xff) {
		wsvc_printf_info("%s: Always allow to acquire lock", __func__);
		/* If not set, then if display is off lock will not be acquired */
		resources[0] = 0x42C28000;
		resources[1] = 1;
		wsvc_printf_info("%s: BIG CL", __func__);
		/* big cluster */
		resources[2] = 0x40800000;
		resources[3] = 0xfff;
		wsvc_printf_info("%s: LITTLE CL", __func__);
		/* little cluster */
		resources[4] = 0x40800100;
		resources[5] = 0xfff;
		wsvc_printf_info("%s: PRIME CORE", __func__);
		/* prime core */
		resources[6] = 0x40800200;
		resources[7] = 0xfff;
	} else {
