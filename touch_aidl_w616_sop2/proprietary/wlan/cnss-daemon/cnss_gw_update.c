...
#ifdef CNSS_DAEMON_GW_NUD_GET_IFTYPE
...
static int clear_nud_stats(struct cnss_gw_data *gw_data,char* iface_name)
{
	struct nl_msg *nlmsg;
	int ret = -1;
	...
	nlmsg = alloc_nlmsg(QCA_NL80211_VENDOR_SUBCMD_NUD_STATS_SET, (char *)iface_name);
	if (!nlmsg)
		return ret;
			     ...
			     char* iface_name)
{
	...
	nlmsg = alloc_nlmsg(QCA_NL80211_VENDOR_SUBCMD_NUD_STATS_GET, (char *)iface_name);
	if (!nlmsg)
		return ret;
...
#ifdef CNSS_DAEMON_GW_NUD_GET_IFTYPE
uint32_t get_interface_activemode(struct cnss_gw_data *gw_data,
...
static int cnss_gw_init_wifi_socket(struct cnss_gw_data *gw_data)
{
	...
	ret = nl_connect(gw_update_info.wifi_sockfd, NETLINK_GENERIC);
	if (ret) {
		nl_socket_free(gw_update_info.wifi_sockfd);
		wsvc_printf_err("wifi nl connect failed ret %d\n", ret);
		return -1;
	}
...
int cnss_get_gw_update_sockfd(void)
{
