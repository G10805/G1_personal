...
int cnss_hang_event_handler(char *data, int len)
{
	struct nlattr *tb[QCA_WLAN_VENDOR_ATTR_HANG_MAX + 1];
	uint32_t hang_reason = 0;
	uint8_t *vendata = NULL;
	int datalen;
	...
	if (tb[QCA_WLAN_VENDOR_ATTR_HANG_REASON]) {
	        ...
	        wsvc_printf_info(" hang reason : %u", hang_reason);
	}

	if (tb[QCA_WLAN_VENDOR_ATTR_HANG_REASON_DATA]) {
		vendata = ((uint8_t*) nla_data(tb[QCA_WLAN_VENDOR_ATTR_HANG_REASON_DATA]));
		datalen = nla_len(tb[QCA_WLAN_VENDOR_ATTR_HANG_REASON_DATA]);
		memset(dump_buffer, 0, sizeof(dump_buffer));
		if (datalen > 0) {
			wsvc_printf_info("hang event hex dump start len : %d", datalen);
			for (i = 0; i < datalen; i++) {
				...
				if (!((i + 1) % 16) && (i != 0)) {
					total_dump_len += 16;
					wsvc_printf_info("i: %d %s", i, dump_buffer);
					memset(dump_buffer, 0, sizeof(dump_buffer));
					j = 0;
				}
			}
			if (total_dump_len != datalen)
				wsvc_printf_info("%s", dump_buffer);
			wsvc_printf_info("hang event hex dump end ");
		}
	}
...
int cnss_vendor_cmd_event_init(void)
{
	int ret;

	ret = cnss_nl80211_register_handler(cnss_hang_event_handler,
	...
	wsvc_printf_info(" cnss hang event handler init successfully\n");
	return 0;
...
int cnss_vendor_cmd_event_deinit(void)
{
	cnss_nl80211_unregister_handler(
				QCA_NL80211_VENDOR_SUBCMD_HANG);
	return 0;
