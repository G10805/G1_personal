/*
 * Copyright (c) 2011-2012, 2019, 2022 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
static struct nl_sock *nl80211_handle_alloc(void)
{
	...
	if (ret) {
		...
		wsvc_printf_err("netlink connect failed ret %d\n", ret);
		return NULL;
	}
...
{
	return nl_socket_get_fd(global_nlcmn.event_sockfd);
}
