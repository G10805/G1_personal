...
class QC2Codec {
 public:
    ...
    struct NotifyKey {
        ...
        static constexpr const char * kConfigUpdate = "config-update"; ///< key for ConfigUpdateInfo
    };
    ...
    enum NotifyEventId : uint32_t {
        ...
        CODEC_NOTIFY_ERROR = 0x104,             ///< notify error\n
                                                /// kErrorCode : {int32_t} (QC2Status) \n
                                                /// kErrorMsg  : {string} (optional msg)
        CODEC_NOTIFY_CONFIG_UPDATE = 0x105,     ///< notify output format change\n
                                                /// kConfigUpdate :  ConfigUpdateInfo
    };
    ...
    /**
     * @brief Payload for signaling listener with updated codec config (CODEC_NOTIFY_UPDATE_CONFIG)
     *
     * Added with key = NotifyEventInfoKey::kConfigUpdate \n
     * When codec updates the static parameters, it notifies the handler and
     * provides the list of params that have been updated. This could happen
     * as a part of CSD update received in a frame. \n
     * The implementer of listener (component) is expected to invoke configInput(...)
     * to notify the interface of updated configuration.
     */
    struct ConfigUpdateInfo : Bundle::Storable {
        std::list<std::shared_ptr<C2Param>> mTargetConfig;   ///< updated config for codec
    };
     ...
     *
     * This is a blocking call.
     * Codec releases all references to its resources
     *
     * @return QC2_OK
     */
    ...
    /**
     * @brief Send multiple parameters to codec
     *
     * Set the supplied set of parameters
     * @note This method is used to set configs out-of-band. The frame-accurate ones will be queued
     * to the codec along with the input
     * @note The parameter values provided is not interpreted as flexible. Codec shall apply the
     * actual values as-is or may reject if any value is not supported.
     *
     * @param[in] targetConfig updated set of configuration
     * @return QC2_OK parameters are set successfully
     * @return QC2_BAD_INDEX parameters are not supported
     * @return QC2_BAD_VALUE parameter values are not supported
     */
    virtual QC2Status configure(std::vector<std::unique_ptr<C2Param>> targetConfig) = 0;
    ...
    /**
     * @brief Suspend processing in the codec
     *
     * Suspend processing the input and output buffers in the codec
     *
     * @return QC2_OK is suspend was successful
     * @return QC2_ERROR encountered an unknown error
     */
    virtual QC2Status suspend() = 0;

    /**
     * @brief Resume processing for the suspended codec
     *
     * Signal codec to resume processing input and output buffers
     *
     * @return QC2_OK is resume was sucessful
     * @return QC2_ERROR encountered an unknown error
     */
    virtual QC2Status resume() = 0;
