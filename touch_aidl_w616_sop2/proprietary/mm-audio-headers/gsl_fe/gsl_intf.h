 ...
 * \param[out] payload: pointer to a buffer containing custom calibration
...
int32_t gsl_set_cal_data_to_acdb(
	struct gsl_key_vector *graph_key_vect,
	struct gsl_key_vector *cal_key_vect, uint8_t *payload,
 ...
 * \param[out] payload: pointer to a buffer containing custom tag data
 * \param[in] payload_size: holds the size of the client buffer passed in for
 *		payload
 *
 * \return EOK on success, error code otherwise
...
int32_t gsl_set_tag_data_to_acdb(
	struct gsl_key_vector *graph_key_vect, uint32_t tag_id,
	struct gsl_key_vector *tag_key_vect, uint8_t *payload,
	uint32_t payload_size);
