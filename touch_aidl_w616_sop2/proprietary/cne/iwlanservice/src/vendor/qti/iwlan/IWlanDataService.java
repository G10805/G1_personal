...
public class IWlanDataService extends DataService {
    ...
    private class IWlanDataServiceProvider extends DataService.DataServiceProvider {
        ...
        private List<DataCallResponse> convertDataCallList(List<android.hardware.radio.V1_6.SetupDataCallResult> dcList) {
            List<DataCallResponse> dcResponseList = new ArrayList<>();
            for (android.hardware.radio.V1_6.SetupDataCallResult dcResult : dcList) {
        ...
        public void setupDataCall(int accessNetworkType, DataProfile dataProfile,
            ...
            proxy.setupDataCall_1_1(dataProfile, isRoaming, allowRoaming, reason, linkProperties,
                                    trafficDescriptor, matchAllRuleAllowed, message);
        }
    ...
    public DataCallResponse convertDataCallResult(SetupDataCallResult dcResult) {
        ...
        List<LinkAddress> laList = dcResult.addresses.stream().map(la ->
                createLinkAddressFromString(la.address, la.properties, la.deprecationTime,
        ...
        List<TrafficDescriptor> trafficDescriptors = dcResult.trafficDescriptors.stream().map(td ->
