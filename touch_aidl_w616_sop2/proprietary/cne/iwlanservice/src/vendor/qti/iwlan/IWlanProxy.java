...
import vendor.qti.hardware.data.iwlan.V1_1.IIWlan;
...
public class IWlanProxy {
    ...
    private IIWlan getService() {
        ...
        if (mService != null) {
            try {
                ...
                iwlanLog.e(TAG, "setResponseFunctions failure");
            }
        } else {
    ...
    private String convertDpiV1_5ToString(android.hardware.radio.V1_5.DataProfileInfo dpi) {
        StringBuilder b = new StringBuilder();
        b.append("Profile ID = " + dpi.profileId);
        b.append(", APN = " + dpi.apn);
        b.append(", Protocol = " + dpi.protocol);
        ...
        b.append(", Supported Apn Types bitmap = " + dpi.supportedApnTypesBitmap);
        b.append(", BearerBitmap = " + dpi.bearerBitmap);
        b.append(", MTU V4 = " + dpi.mtuV4);
        b.append(", MTU V6 = " + dpi.mtuV6);
        return b.toString();
    }
    ...
    public void setupDataCall_1_1(DataProfile dataProfile, boolean isRoaming,
                                  boolean allowRoaming, int reason, LinkProperties linkProperties,
        ...
        if (service != null) {
            ...
            // Convert to HAL data profile
            android.hardware.radio.V1_5.DataProfileInfo dpi = convertToHalDataProfileV1_5(dataProfile);
            ArrayList<android.hardware.radio.V1_5.LinkAddress> linkAddresses = new ArrayList<>();
            ArrayList<String> dnses = new ArrayList<>();
            if (linkProperties != null) {
                for (LinkAddress address : linkProperties.getLinkAddresses()) {
                    android.hardware.radio.V1_5.LinkAddress la = new android.hardware.radio.V1_5.LinkAddress();
            ...
            iwlanLog.v(IWlanDataService.TAG, req.mSerial + " > " + requestToString(req.mRequest) + ", reason = " + reason +
                       ", dpi: " + convertDpiV1_5ToString(dpi) + ", addresses = [" + TextUtils.join(",", linkAddresses) + "]" +
                       ", dnses = [" + TextUtils.join(",", dnses) + "], traffic descriptor = " + trafficDescriptor.toString());

            try {
                service.setupDataCall_1_1(req.mSerial, dpi, allowRoaming, reason, linkAddresses, dnses,
                        convertToHalTrafficDescriptor(trafficDescriptor), matchAllRuleAllowed);
            } catch (RemoteException | RuntimeException e) {
                iwlanLog.e(IWlanDataService.TAG, req.mSerial + " > " + requestToString(req.mRequest) + " error : " + e);
            }
        }
    ...
    private static android.hardware.radio.V1_5.DataProfileInfo convertToHalDataProfileV1_5(DataProfile dp) {
        android.hardware.radio.V1_5.DataProfileInfo dpi = new android.hardware.radio.V1_5.DataProfileInfo();
