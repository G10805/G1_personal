/*========================================================================================
 ...
 Copyright (c) 2013-2015, 2019, 2021 Qualcomm Technologies, Inc.  All Rights Reserved.
...
struct req_client {
	...
	int  *fd;
	int  type;
	int  clnt_fd[MAX_CAMERA_BW_CLIENT];
	int  def_val;
	int  total_clients;
};
...
static struct req_client req_clients[] = {
	...
	},
	{
		...
		.type = BANDWIDTH_REQ_TYPE,
		.def_val = 0,
		.clnt_fd = { [ 0 ... MAX_CAMERA_BW_CLIENT-1 ] = -1 },
		.total_clients = MAX_CAMERA_BW_CLIENT-1,
	},
	{
		.name = "display_bw",
		.fd   = &sockfd_server_recv_passive,
...
static int thermal_recv_data_from_client(int client_fd, int socket_fd)
{
	...
	for (i = 0; i < ARRAY_SIZE(req_clients); i++) {
		if(!req_clients[i].total_clients) {
			if (0 == strncmp(req_clients[i].name, client_msg.client_name,
				CLIENT_NAME_MAX) &&
				*(req_clients[i].fd) == socket_fd)
				break;
		} else {
			if (is_valid_camera_client(client_msg.client_name, req_clients[i].name,
				req_clients[i].total_clients) &&
				*(req_clients[i].fd) == socket_fd)
				break;
		}
	}
	...
	if (req_clients[i].type == BANDWIDTH_REQ_TYPE) {
		...
		if (ret < 0)
			return ret;
		if(!req_clients[i].total_clients) {
			if (req_clients[i].clnt_fd[0] < 0) {
				req_clients[i].clnt_fd[0] = client_fd;
			} else if (req_clients[i].clnt_fd[0] != client_fd) {
				info("BW clnt:%s is already connected with fd:%d, disconnecting this one\n",
					client_msg.client_name, req_clients[i].clnt_fd[0]);
				return ret;
			}
		}
		else {
			int client_no = -1;
			sscanf(client_msg.client_name, "camera_bw_%d", &client_no);
			if (req_clients[i].clnt_fd[client_no] < 0) {
				req_clients[i].clnt_fd[client_no] = client_fd;
			} else if (req_clients[i].clnt_fd[client_no] != client_fd) {
				info("BW clnt:%s is already connected with fd:%d, disconnecting this one\n",
				client_msg.client_name, req_clients[i].clnt_fd[client_no]);
				return ret;
			}
		}
		ret = 1;
...
static int check_for_bw_client_update(int fd, uint8_t is_close)
{
	...
	for (i = 0; i < ARRAY_SIZE(req_clients); i++) {
		if (req_clients[i].type == BANDWIDTH_REQ_TYPE) {
			for (int j = 0; j < req_clients[i].total_clients + 1; j++) {
				if (fd == req_clients[i].clnt_fd[j]) {
					if (is_close) {
						req_clients[i].clnt_fd[j] = -1;
						if(!req_clients[i].total_clients) {
							thermal_recv_bw_data(
								req_clients[i].name,
								req_clients[i].def_val);
						} else {
							char camera_client_name[CAMERA_BW_CLIENT_NAME_MAX + 1];
							snprintf(camera_client_name,
								CAMERA_BW_CLIENT_NAME_MAX + 1,
								req_clients[i].name, j);
							thermal_recv_bw_data(
								camera_client_name,
								req_clients[i].def_val);
						}
...
int thermal_server_register_client_req_handler(char *client_name, int (*callback)(int, void *, void *), void *data)
{
	...
	for (i = 0; i < ARRAY_SIZE(req_clients); i++) {
		if (!req_clients[i].total_clients) {
			...
			if(is_valid_camera_client(client_name, req_clients[i].name,
				req_clients[i].total_clients))
				break;
		}
	}
