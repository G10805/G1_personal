...
#define CPU_CLUSTER_NUM_NODE "/topology/physical_package_id"
#define CPU_CLUSTER_NUM_NODE1 "/topology/cluster_id"
...
static device_clnt_handle cluster_cpu_clnt[MAX_CLUSTERS];
static device_clnt_handle hotplug_dev_clnt[MAX_CPUS];
static device_clnt_handle cdsp_cdev_hdl;
...
static void init_cooling_device( struct devices_manager_dev *dev)
{
	...
	if (!strncmp(dev->dev_info.name, LCD_DEV_NAME,
		...
		dev->dev_info.cooling_dev_id = get_cdevn(cdev_name);
				...
				strlen(dev->dev_info.name))) {
		snprintf(cdev_name, sizeof(cdev_name), GPU_COOLING_DEV_NAME1);
		dev->dev_info.cooling_dev_id = get_cdevn(cdev_name);
		if (dev->dev_info.cooling_dev_id < 0) {
			snprintf(cdev_name, sizeof(cdev_name),
		...
		if (is_cdev_dup) {
			/* We expect a CPU Pause cooling device if the flag is set.
			 * So if there is no match the hotplug feature for that
			 * cpu might be disabled.*/
			return;
		}
...
static int tmd_add_cpufreq_dev_data(int cluster_idx, int cpu_idx)
{
	int ret_val = 0, cdev_id = 0;
	uint32_t idx = 0, num_freqs = 0, boost_freqs = 0;
	struct devices_manager_dev *dev_mgr = NULL;
	char path[DEVICES_MAX_PATH] = {0};
	...
	uint32_t arr_idx;
	static bool is_cdev = false, is_dup_cdev = false;

	snprintf(cdev_name, DEVICES_MAX_NAME_LEN, CLUSTER_CPU_COOLING_DEV_NAME,
			cluster_idx);
	cdev_id = get_cdevn(cdev_name);
	if (!is_dup_cdev && cdev_id < 0) {
		snprintf(cdev_name, DEVICES_MAX_NAME_LEN,
				CPU_COOLING_DEV_NAME, cluster_idx);
		cdev_id = get_cdevn(cdev_name);
	} else
		is_dup_cdev = true;
		/* This will be set when a duplicate CPU cooling device is
		 * present and we shouldnt look for the upstream CPU cooling
		 * device.
		 * */
	if (cdev_id != -1) {
		is_cdev = true;
	...
	snprintf(dev_mgr->dev_info.name, DEVICES_MAX_NAME_LEN, CLUSTER_CPU_COOLING_DEV_NAME,
		 cluster_idx);
...
int tmd_init_cpu_devs(void)
{
	int num_cpus = 0;
	int ret_val = 0;
	int idx = 0, idy = 0, cluster_number = 0, cluster_max = -1;
	char name[DEVICES_MAX_NAME_LEN];
	char path[DEVICES_MAX_PATH] = {0};
	char buf[10];
	int clusters[MAX_CPUS] = {-1};
	int cpus[MAX_CPUS] = {-1};
	...
	for (idx = 0; idx < num_cpus; idx++) {
		snprintf(path, sizeof(path), CPU_SYSFS(CPU_CLUSTER_NUM_NODE1), idx);
		if (read_line_from_file(path, buf, sizeof(buf)) <= 0) {
			/*check for other sysfs path*/
			snprintf(path, sizeof(path), CPU_SYSFS(CPU_CLUSTER_NUM_NODE), idx);
			if (read_line_from_file(path, buf, sizeof(buf)) <= 0) {
				info("%s: Cluster info node not found%s\n",
				__func__, CPU_CLUSTER_NUM_NODE);
				return -1;
			}
		}
		sscanf(buf, "%d", &cluster_number);
		clusters[idx] = cluster_number;
		cpus[idx] = idx;
		if(cluster_max < cluster_number)
			cluster_max = cluster_number;
	};

	/* Add individual CPU cluster TMD's */
	for (idx = 0; idx <= cluster_max; idx++) {
		for(idy = 0; idy < num_cpus; idy++) {
			if (clusters[idy] == idx)
				break;
		}
		if (idy >= num_cpus)
			continue;
		snprintf(name, DEVICES_MAX_NAME_LEN, CLUSTER_CPU_COOLING_DEV_NAME, idx);

		ret_val = tmd_add_cpufreq_dev_data(idx, cpus[idy]);
		if (ret_val != 0) {
			if (ret_val == -(ENODEV))
				continue;
		...
		cluster_cpu_clnt[idx] = devices_manager_reg_clnt(name);
		if (cluster_cpu_clnt[idx] == NULL) {
...
int tmd_init_gpu_devs(void)
{
	...
	if (num_freqs < 1) {
		...
		if (num_freqs < 1 || num_freqs !=
				(uint32_t)(get_gpu_cooling_device_max_state() + 1)) {
			msg("%s: Invalid number for freqs\n", __func__);
			ret_val = -(EFAULT);
			goto handle_error;
		}
...
static int hvx_sw_action(struct devices_manager_dev *dev_mgr)
{
	...
	if (cdsp_cdev_hdl) {
		req.value = 0;
		dbgmsg("ACTION: CDSP %d\n", dev_mgr->active_req.value);
		thermalmsg(LOG_LVL_DBG,
			(LOG_LOGCAT | LOG_LOCAL_SOCKET | LOG_TRACE),
			"%s: CDSP %d\n", MITIGATION,
			dev_mgr->active_req.value);
		if (dev_mgr->active_req.value)
			device_clnt_request(cdsp_cdev_hdl, &req);
		else
			device_clnt_cancel_request(cdsp_cdev_hdl);
	}
