...
Copyright (c) 2007-2011, 2014-2015, 2016, 2020-2023 Qualcomm Technologies, Inc.
All Rights Reserved.
...
#define LOG_WRITE_WAIT_US	1
#define LOG_WRITE_TIMEOUT_US	1000
...
diag_log_mask_t enabled_log_mask_arr[MAX_EQUIP_ID];
...
SIDE EFFECTS
  None
===========================================================================*/
void diag_cb_func_type_wrap(unsigned char *data_ptr, unsigned short data_len, unsigned char *rsp_ptr, unsigned short *rsp_len_ptr) {}

void diag_lsm_log_wrapper_init(void) {
	diag_cb_func_type address;
	address = &diag_cb_func_type_wrap;
}

static int log_write(const void *buf, unsigned int length)
{
	int ret = 0;
	...
	while (elapsed_time_us < LOG_WRITE_TIMEOUT_US) {
		wait_tv.tv_sec = 0;
		wait_tv.tv_usec = LOG_WRITE_WAIT_US;
		ret = diag_lsm_comm_write(diag_fd, (unsigned char*)buf, length, MSG_DONTWAIT);
...
void log_update_mask(unsigned char* ptr, int len)
{
	...
	for (i = 0; i < MAX_EQUIP_ID && read_len < len; i++, dest++) {
		...
		enabled_log_mask_arr[i].equip_id = dest->equip_id;
		enabled_log_mask_arr[i].num_items = dest->num_items;
		memcpy(enabled_log_mask_arr[i].mask, dest->mask, sizeof(dest->mask));
...
)
{
  ...
  {
     DIAG_LOGE("diag: Alloc invalid length: %u", length);
  }
...
void log_commit (void *ptr)
{
  if (ptr)
  {
	...
	NumberOfBytesWritten = log_write((const void *)ptr, DIAG_REST_OF_DATA_POS + LOG_DIAGPKT_OFFSET + phdr_ptr->len);
	...
	if (length > sizeof(log_header_type))
	{
		if ((enabled_mask = log_status_mask(code)) != 0) {
			...
			if (pdiag_data != NULL) {
                ...
                int NumberOfBytesWritten = 0;
				NumberOfBytesWritten = log_write((const void *)pdiag_data, DIAG_REST_OF_DATA_POS +
								 FPOS(diag_log_rsp_type, log) + length);
				if (NumberOfBytesWritten) {
					if (errno != EWOULDBLOCK)
