...
Copyright (c) 2007-2011, 2013-2015, 2018, 2020, 2022-2023 Qualcomm Technologies, Inc.
All Rights Reserved.
...
#define MSG_WRITE_WAIT_US	1
#define MSG_WRITE_TIMEOUT_US	1000
...
static unsigned int msg_commit_to_cs_fail;
static unsigned char msg_mask[MSG_MASK_SIZE];
diag_msg_mask_t enabled_msg_mask_arr[MSG_MASK_TBL_CNT];
...
static int msg_write(const void *buf, unsigned int length)
{
	int ret = 0;
	...
	while (elapsed_time_us < MSG_WRITE_TIMEOUT_US) {
		wait_tv.tv_sec = 0;
		wait_tv.tv_usec = MSG_WRITE_WAIT_US;
...
void msg_sprintf(const msg_const_type *const_blk, ...)
{
	 ...
	 */
	write_len = msg_write((const void *)pMsg, (uint32)(str - (char *)pMsg));
	if (write_len) {
		if (errno != EWOULDBLOCK)
			DIAG_LOGE("Diag_LSM_Msg: Write failed in %s, bytes written: %d, error: %d\n",
				  __func__, write_len, errno);
...
void msg_update_mask(unsigned char *ptr, int len)
{
	...
	for (i = 0; i < MSG_MASK_TBL_CNT && (read_len + sizeof(diag_msg_mask_update_t) < len); i++, mask_ptr++) {
		...
		enabled_msg_mask_arr[i].ssid_first = mask_ptr->ssid_first;
		enabled_msg_mask_arr[i].ssid_last = mask_ptr->ssid_last;
		memcpy(enabled_msg_mask_arr[i].ptr, mask_ptr->ptr, range * sizeof(uint32));
