...
private:  // CanWrapper is a SINGLETON
    ...
    CanWrapper(int mode, uint8_t timeStampSrc);
...
public:
     ...
     */
    static CanWrapper * getInstance(int mode, uint8_t timeStampSrc);
     ...
     * @param  pointer to the CwNodeSet object which points to list of CwFrames object
     *      CwFrame objects are deleted before returning.
     * @param ifNo = the interface number
     ...
     *              ** the pFrameList and its CwFrames are still owned by caller in this condition
     * @desc it bears repeating:
     *    >> ownership of pFrameList object is passed to CanWrapper except when error <<
     */
    int sendMessages(CwNodeSet *pFrameList, int ifNo);
