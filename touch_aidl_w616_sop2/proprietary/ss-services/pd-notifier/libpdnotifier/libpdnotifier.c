...
pd_rcode pd_notifier_restart_pd(PD_Notifier_Handle *handle)
{
	int retVal = 0;
	qmi_servreg_notif_restart_pd_req_msg_v01 req = {0};
	qmi_servreg_notif_restart_pd_resp_msg_v01 resp = {0};
	qmi_idl_service_object_type serviceObject;
	qmi_cci_os_signal_type osParams;
	bool release = false;
	pd_rcode rc = PD_NOTIFIER_SUCCESS;
	...
	strlcpy(req.process_spc_name,
		handle->serviceName,
		sizeof(req.process_spc_name));

	retVal = qmi_client_send_msg_sync(handle->pd_notifier_client,
			QMI_SERVREG_NOTIF_RESTART_PD_REQ_V01,
			&req,
			sizeof(req),
			&resp,
			sizeof(resp),
			5000);
	if (retVal != QMI_NO_ERR) {
		ALOGE("Send restart PD reg msg failed for %s :instance:%d rval:%d",
			handle->serviceName,
			handle->instance_id,
			retVal);
	...
	if (resp.resp.result == QMI_RESULT_FAILURE_V01) {
		switch(resp.resp.error) {
			case QMI_ERR_DISABLED_V01:
				ALOGE("PD %s restart PD disabled\n",
				      req.process_spc_name);
				break;
			case QMI_ERR_INVALID_HANDLE_V01:
				ALOGE("PD %s restart PD inavlid handle\n",
					req.process_spc_name);
				break;
			default:
				break;
