...
def CalculateTextSize(vdso_end_addr, text_start_addr, text_size):
	vdso_end_addr = int(vdso_end_addr, 16)
        text_start_addr = int(text_start_addr, 16)
        text_size = int(text_size, 16)
        new_text_size = vdso_end_addr - text_start_addr
        new_text_size = str(hex(new_text_size)).strip("L")
        return new_text_size

def get_mpdb(catalog, kernel, config, start_ro_after_init = False, exclude_vdso = False):
	mpdb = MPDB(catalog)
	a = iter(catalog)
	headTextAddr = None
	for x in a:
		try:
			...
			if x.mp_type is mpgen_mpdb.RD_ONLY_AFTER_INIT :
				...
				section = kernel.get_section(x.name)
				if (symbol is None) : continue
				if start_ro_after_init:
					if symbol.name == ".rodata":
						ro_init_symbol = kernel.get_symbol('__start_ro_after_init')
						symbol.size = str(hex(int(ro_init_symbol.value, 16) - int(symbol.value, 16))).strip("L")
                                """if symbol.name == ".text":
                                    ro_init_symbol = kernel.get_symbol('__entry_text_end')
                                    symbol.size = str(hex(int(symbol.size, 16) - (int(ro_init_symbol.value, 16) - int(symbol.value, 16)))).strip("L")
                                    symbol.value = ro_init_symbol.value"""
				if (exclude_vdso == True and section is not None and symbol.name == ".rodata") : symbol.value, symbol.size = CalculateAddressAndSize(headTextAddr, symbol.value, section.file_off, symbol.size)
				sha256 = kernel.get_symbal_sha256(x.name)
