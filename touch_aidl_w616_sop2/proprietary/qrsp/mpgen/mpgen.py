...
try:
    ...
    def main():
        ...
        parser.add_argument('--symdump', type = str, help = 'file name of the `objdump` utility for symbols', default = '../llvm-arm-toolchain-ship/12.0/bin/llvm-readelf')
	...
	parser.add_argument('--start_ro_after_init', action = 'store_true')
        parser.add_argument('--exclude_vdso', action = 'store_true')
        ...
        if args.dts_payload or args.dts:
            ...
            #*/
            print("POPULATE")
            mpdb = mpgen_populate.get_mpdb(catalog, kernel, config, args.start_ro_after_init, args.exclude_vdso)
            ...
            if cborgen_found:
                with open("qrks_mp.bin", "w") as f_cborfile:
                    ...
                    hex_data = binascii.hexlify(cbor_content).upper()
		    hex_array = [hex_data[i:i+2] for i in range(0, len(hex_data), 2)]
		    for byte in range(len(hex_array)):
			    hex_array[byte] = "0x" + hex_array[byte]
		    hex_data_edited = ', '.join(hex_array)
		    with open("qrks_mp_hex_edited.txt", "w") as f_hexfile:
			    f_hexfile.write(hex_data_edited)
    ...
    print("Mpgen Fatal error will not stop execution")
    exit(0)
