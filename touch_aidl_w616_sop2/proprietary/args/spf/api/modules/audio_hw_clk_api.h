...
struct audio_hw_clk_cfg_v2_t
{
   ...
   /**< Root clock source.
        @values
        - #_CLOCK_ROOT_SRC_DEFAULT
        - #_CLOCK_ROOT_SRC_RCO
        - #_CLOCK_ROOT_SRC_EXTERNAL */

   uint32_t mux_index;
   /**< Root Clock source to use.
        This Value should match the MUX input on which the external source is connected. */

   uint32_t divider_2x;
   /**< Integer divider to use (2x the desired divider). */

   uint32_t counter_m_value;
   /**< The M value for any M/N counter (0 to bypass).
        Not all clock domains have M/N counters;
        these values will be ignored in such cases. */

   uint32_t counter_n_value;
   /**< The N value for M/N counter. */

   uint32_t d_value;
   /**< Twice the D value for any M/N counter.
        The raw value is doubled to allow for half-integer duty cycles. */
}
