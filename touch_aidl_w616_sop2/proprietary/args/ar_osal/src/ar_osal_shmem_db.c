...
/*
 *\brief ar_shmem_check_mdf
 *        internal function to validate if MDF usecase .
 * \return
 * true -- MDF usecase
 * false -- non-MDF usecase
 */
static bool ar_shmem_check_mdf(_In_ ar_shmem_info *info)
{
    return info->num_sys_id > 1;
}
 ...
 * false -- non-MDF usecase
 */
static uint64_t ar_shmem_ss_masks(_In_ ar_shmem_info *info)
{
    uint32_t ss_masks = 0;
...
int32_t ar_shmem_init(void)
{
    ...
    /*Check if dma_buf heap initialize the dmabuf_handle to invalid handle */
    pdata->dmabuf_fd_uncached = open(DMABUF_SYS_HEAP_PATH_UNCACHED, O_RDONLY | O_CLOEXEC);
    if (pdata->dmabuf_fd_uncached < 0) {
      AR_LOG_ERR(AR_OSAL_SHMEM_LOG_TAG,"%s:dmabuf dev open failed fd:%d errno=%d %s\n", __func__, pdata->dmabuf_fd_uncached, errno, DMABUF_SYS_HEAP_PATH_UNCACHED);
      status = AR_ENOTEXIST;
    ...
    if (pdata->armem_fd < 0) {
      ...
      close(pdata->dmabuf_fd_uncached);
      close(pdata->dmabuf_fd);
      goto end;
    }
    ...
    if(pdata->dmabuf_cma_mem_enabled) {
        ...
        if (pdata->armem_fd_cma <= 0) {
          status = AR_ENOTEXIST;
          AR_LOG_ERR(AR_OSAL_SHMEM_LOG_TAG,"%s armem fd cma open failed %s status %d\n",
...
int32_t ar_shmem_alloc(_Inout_ ar_shmem_info *info)
{
    ...
    is_mdf = ar_shmem_check_mdf(info);
    ...
    if (!is_mdf && !((info->flags) & (1 << AR_SHMEM_SHIFT_HW_ACCELERATOR_FLAG))) {
        fd = (info->cache_type == AR_SHMEM_CACHED) ? pdata->dmabuf_fd : pdata->dmabuf_fd_uncached;
        status = ioctl(fd, DMA_HEAP_IOCTL_ALLOC, &heap_data);
        if (status) {
        ...
        AR_LOG_ERR(AR_OSAL_SHMEM_LOG_TAG,"%s: no DMA heap for config: flag %d, mdf %d, cma enabled %d\n",
             __func__, info->flags, is_mdf, pdata->dmabuf_cma_mem_enabled);
        status = AR_ENOTEXIST;
        goto end;
    }
    ...
    AR_LOG_VERBOSE(AR_OSAL_SHMEM_LOG_TAG," SHMEM: cache_type(0x%x) |memory_type(0x%x)|index_type(0x%x) buf_size(0x%x)|vaddr(0x%pK)| ipa_lsw(0x%x)| status(0x%x) ",
		   (info->cache_type == AR_SHMEM_CACHED) ? 1 : 0, info->mem_type, info->index_type, info->buf_size, info->vaddr, info->ipa_lsw, status);
    if (is_mdf || ((info->flags) & (1 << AR_SHMEM_SHIFT_HW_ACCELERATOR_FLAG))) {
    ...
    if (is_mdf) {
        ...
        mdf_data.ss_masks = ar_shmem_ss_masks(info);
        if ((mdf_data.ss_masks & 0x1) && (mdf_data.ss_masks & 0x2)) {
            /* TZ only support assignment to adsp, mdsp & sdsp*/
            mdf_data.ss_masks |= 0x8;
        }
        if (pdata->armem_fd_cma) {
            status = ioctl(pdata->armem_fd_cma, IOCTL_MAP_HYP_ASSIGN_V2, &mdf_data);
            if (status) {
                AR_LOG_ERR(AR_OSAL_SHMEM_LOG_TAG,"%s:HYP ASSIGN failed for MDF status %d\n",
                            __func__, status);
                goto err2;
            }
        } else {
            status = AR_ENOTEXIST;
            AR_LOG_ERR(AR_OSAL_SHMEM_LOG_TAG,"%s:CMA memory does not exist for MDF %d\n",
                        __func__, status);
        }
    }
...
int32_t ar_shmem_free(_In_ ar_shmem_info *info)
{
    ...
    if (is_mdf) {
        ...
        if (pdata->armem_fd_cma) {
            AR_LOG_INFO(AR_OSAL_SHMEM_LOG_TAG,"%s: reclaim MDF memory before any memory op\n",
                        __func__);
            status = ioctl(pdata->armem_fd_cma, IOCTL_UNMAP_HYP_ASSIGN_V2, &mdf_data);
        } else {
            status = AR_ENOTEXIST;
            AR_LOG_ERR(AR_OSAL_SHMEM_LOG_TAG,"%s:DMA_BUF CMA does not exist %d\n",
                        __func__, status);
        }
    }
