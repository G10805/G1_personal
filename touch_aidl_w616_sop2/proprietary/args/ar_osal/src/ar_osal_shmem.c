 ...
 *        nternal function to validate if MDF usecase .
 * \return
 * true -- MDF usecase
 * false -- non-MDF usecase
 */
bool ar_shmem_check_mdf(uint8_t num_sys_id, uint8_t *sys_id)
{
    bool is_mdf = false;
    if (0 == num_sys_id || NULL == sys_id)
    {
...
int32_t ar_shmem_alloc(_Inout_ ar_shmem_info *info)
{
    ...
    is_mdf = ar_shmem_check_mdf(info->num_sys_id, info->sys_id);
...
#ifdef TARGET_USES_ION_CMA_MEMORY
    heap_mask = (is_mdf || ((info->flags) & (1 << AR_SHMEM_SHIFT_HW_ACCELERATOR_FLAG)))?
                HEAP_MASK_CMA : HEAP_MASK;
#else
    heap_mask = HEAP_MASK;
#endif
    ...
    if (is_mdf) {
        if (pdata->ion_fd_cma) {
            status = ioctl(pdata->ion_fd_cma, IOCTL_MAP_HYP_ASSIGN_V2, info->ipa_lsw);
        } else {
            status = AR_ENOTEXIST;
            AR_LOG_ERR(AR_OSAL_SHMEM_LOG_TAG,"%s:ION CMA memory does not exist for MDF %d\n",
                    __func__, status);
        }
    }
    if (status) {
...
int32_t ar_shmem_free(_In_ ar_shmem_info *info)
{
    ...
    if (is_mdf) {
        if (pdata->ion_fd_cma) {
            AR_LOG_INFO(AR_OSAL_SHMEM_LOG_TAG,"%s: reclaim MDF shared memory before any memory op\n",
                        __func__);
            status = ioctl(pdata->ion_fd_cma, IOCTL_UNMAP_HYP_ASSIGN_V2, shmem_handle->ion_mem_fd);
        } else {
            status = AR_ENOTEXIST;
            AR_LOG_ERR(AR_OSAL_SHMEM_LOG_TAG,"%s:ION CMA does not exist %d\n",
                        __func__, status);
        }
    }
