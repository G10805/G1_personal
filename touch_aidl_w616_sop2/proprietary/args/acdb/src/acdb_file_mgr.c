...
static AcdbFileManPathInfo gacdb_path;
...
int32_t AcdbFileManGetDirectoryFromFilePath(const char* fpath, uint32_t fpath_len, AcdbFileManPathInfo* path_info)
{
...
int32_t AcdbFileManUpdateFileInfo(AcdbCmdFileInfo *cmdInfo)
{
    ...
    ACDB_MEM_CPY_SAFE(
        &gDataFileInfo.fInfo[idx].chFileName[0], MAX_FILENAME_LENGTH,
        &cmdInfo->chFileName[0], MAX_FILENAME_LENGTH);
    ...
    result = AcdbFileManGetDirectoryFromFilePath(&cmdInfo->chFileName[0],
        cmdInfo->filename_len, &gacdb_path);
...
int32_t AcdbFileManUpdateTempPathInfo(AcdbFileManPathInfo *cmdInfo)
{
    ...
    ACDB_MEM_CPY_SAFE(&gtemp_file_path.path[0], MAX_FILENAME_LENGTH,
        &cmdInfo->path[0], MAX_FILENAME_LENGTH);
...
int32_t AcdbFileManGetFileName(uint32_t *findex, AcdbFileManPathInfo *file_name_info)
{
    ...
    {
        if (src_str_size >= MAX_FILENAME_LENGTH)
        {
...
int32_t AcdbFileManCheckQwspFileIsLoaded(void)
{
    int32_t status = AR_EOK;
    uint32_t idx = 0;
    ...
    uint32_t sz_rsp)
{
    ...
    case ACDB_FILE_MAN_GET_AVAILABLE_FILE_SLOTS:
    {
        ...
        ACDB_MEM_CPY_SAFE(rsp, sizeof(int32_t),
            &no_of_avail_slots, sizeof(int32_t));
        status = AR_EOK;
        break;
    }
    ...
    case ACDB_FILE_MAN_GET_TEMP_PATH_INFO:
    {
        ...
        ACDB_MEM_CPY_SAFE(
            &((AcdbFileManPathInfo*)rsp)->path[0], gtemp_file_path.path_len,
...
int32_t AcdbGetChunkInfo(uint32_t chkID, uint32_t* chkBuf, uint32_t* chkLen)
{
    ...
    if (res != ACDB_PARSE_SUCCESS) {
        ACDB_DBG("Failed to get information for Chunk[%08X]", chkID);
		return AR_ENOTEXIST;
    }

    if (*chkLen <= 0)
    {
        ACDB_DBG("The requested Chunk[%08X] is empty");
        return AR_ENOTEXIST;
    }
...
int32_t FileManGetFilePointer1(void** file_ptr, size_t data_size, uint32_t *offset)
{
    ...
    if (*offset + data_size > gDataFileInfo.fInfo[gACDBFileIndex].file_size)
    {
        return AR_EBADPARAM;
    }
...
int32_t FileManGetFilePointer2(void** file_ptr, uint32_t offset)
{
    ...
    addr_start = gDataFileInfo
        .fInfo[gACDBFileIndex]
        .file_buffer;
    addr_end = (uint8_t*)gDataFileInfo
        .fInfo[gACDBFileIndex]
        .file_buffer
        + gDataFileInfo
        .fInfo[gACDBFileIndex]
