...
#define ACDB_SOFTWARE_VERSION_MINOR 0x00000019
...
int32_t AcdbCmdGetCalData(AcdbGetCalDataReq* req, AcdbBlob* rsp)
{
    ...
    * not applicable here */
    ...
    for (uint32_t i = 0; i < req->num_module_instance; i++)
    {
        ...
        status = AcdbGetCalDataForSubgraph(&info, &blob_offset, rsp);
        if (status == AR_ENOTEXIST)
        {
            status = AR_EOK;
            continue;
    ...
    AcdbUintList* subgraph_list, AcdbGetGraphRsp* graph)
{
    ...
    status = AcdbGetSubgraphList(sg_list_offset, subgraph_list, graph);
    if (AR_FAILED(status))
    {
...
int32_t AcdbCmdGetSubgraphConn(AcdbSubGraphList *pInput, AcdbBlob *pOutput, uint32_t rsp_struct_size)
{
	...
	for (i = 0;i < pInput->num_subgraphs;i++)
	{
		...
		ACDB_MEM_CPY_SAFE(&src_ID, sizeof(src_ID),
            pInput->subgraphs + input_offset, sizeof(src_ID));

		input_offset += sizeof(src_ID);
		ACDB_MEM_CPY_SAFE(&num_dst_sgid, sizeof(num_dst_sgid),
            pInput->subgraphs + input_offset, sizeof(num_dst_sgid));
		...
		for (j = 0; j < num_dst_sgid; j++)
		{
			...
			ACDB_MEM_CPY_SAFE(&dest_ID, sizeof(dest_ID),
                pInput->subgraphs + input_offset, sizeof(dest_ID));
			...
			for (uint32_t k = 0; k < num_def_entries; k++)
			{
				...
				if (pOutput->buf != NULL)
				{
					...
					ACDB_MEM_CPY_SAFE(pOutput->buf + data_offset,
                        sizeof(mid), &mid, sizeof(mid));
					data_offset += sizeof(mid);
					ACDB_MEM_CPY_SAFE(pOutput->buf + data_offset,
                        sizeof(pid), &pid, sizeof(pid));
					data_offset += sizeof(pid);
					ACDB_MEM_CPY_SAFE(pOutput->buf + data_offset,
                        sizeof(payload_size), &payload_size, sizeof(payload_size));
					data_offset += sizeof(payload_size);
					ACDB_MEM_CPY_SAFE(pOutput->buf + data_offset,
                        sizeof(errcode), &errcode, sizeof(errcode));
					...
					ACDB_MEM_CPY_SAFE(pOutput->buf + data_offset,
                        payload_size, &glb_buf_1, payload_size);
...
int32_t AcdbCmdGetAmdbRegData(AcdbAmdbProcID *req, AcdbBlob *rsp, uint32_t rsp_size)
{
    ...
    for (uint32_t p = 0; p < num_processors; p++)
    {
        ...
        {
            offset += glb_buf_1[0] - sz_amdb_obj_header + sizeof(uint32_t);
        }
    }
...
int32_t AcdbCmdGetAmdbBootupLoadModules(AcdbAmdbProcID *req, AcdbBlob *rsp, uint32_t rsp_size)
{
	...
	} while (offset < boot_load_chk_offset + boot_load_chk_size && !found);

	if (!found)
	{
		ACDB_INFO("Error[%d]: Processor ID = %d does not exist",
            AR_ENOTEXIST, req->proc_id);
		return AR_ENOTEXIST;
	}
    ...
    AcdbCmdGetTagsFromGkvRsp* rsp)
{
    ...
    lut_end_offset =
        lut_chunk->chunk_offset + lut_chunk->chunk_size - sizeof(uint32_t);
