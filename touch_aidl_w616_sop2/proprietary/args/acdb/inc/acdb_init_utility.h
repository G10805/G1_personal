...
int32_t AcdbInitUtilDeleteDeltaFileData(
    const char* acdb_file_path, uint32_t acdb_file_path_length, AcdbFile* delta_path);
...
/**
*	\brief
*		Retrieves basic delta file properties such as file size, file name,
*       file name length.
*       Callee must specify info->file_access
*       Callee is responsible for freeing memory associated with info->path
*
*	\param[in] acdb_file_path: The null terminated *.acdb file path string
*	\param[in] acdb_file_path_length: The length of the acdb file path
...
*
*   \sa
*   AcdbInitUtilGetDeltaInfo
*/
int32_t AcdbInitUtilGetDeltaInfo2(
    const char* acdb_file_path, uint32_t acdb_file_path_length,
    AcdbFile* delta_path, AcdbInitUtilDeltaInfo *info);
