...
#define GSL_SPF_READ_WRITE_TIMEOUT_MS  (1000) /* 1 sec */
#endif

#define GSL_GET_SPF_SS_MASK(OSAL_SYS_ID) (1<<(OSAL_SYS_ID - 1))
#define GSL_TEST_SPF_SS_BIT(SPF_SS_MASK, OSAL_SYS_ID) \
((SPF_SS_MASK & GSL_GET_SPF_SS_MASK(OSAL_SYS_ID))>>(OSAL_SYS_ID - 1))
...
static inline void gsl_print_sg_conn_info(uint32_t *sg_conn_info, uint32_t num_sgs)
{
	uint32_t i = 0, num_children = 0;
	uint32_t *p = sg_conn_info;

	GSL_INFO("num_sg_conn = %d", num_sgs);
	for (i = 0; i < num_sgs; ++i) {
		GSL_INFO("sg %x has %d children:", *(p),	num_children = *(p + 1));
		p += 2;
		while (num_children) {
			GSL_INFO("%x, ", *(p++));
			--num_children;
...
inline void gsl_print_sg_conn_info(uint32_t *sg_conn_info, uint32_t num_sgs)
