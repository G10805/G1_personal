...
enum gsl_graph_sig {
	 ...
	 * client. Group 1 includes: OPEN, CLOSE, PREPARE, START and STOP,
...
struct gsl_graph_rtc_cache {
	 ...
	 */
	struct gsl_sgid_list pruned_plus_reopen_sgids;
...
/**
 * \brief Set custom graph configuration on SPF. Used exclusively for the OOB usecase
 *
 * \param[in] graph: pointer to graph's memory
 * \param[in] tag: tag ID
 * \param[in] payload: pointer to config payload
