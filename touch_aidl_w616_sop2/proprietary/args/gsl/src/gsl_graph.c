	...
	uint32_t dst_port, struct gpr_packet_t **p_rsp_pkt)
{
	...
	if (opcode == APM_CMD_SET_CFG) {
		if (graph->rtc_conn_active) {
			...
			GSL_INFO("set custom config oob: caching buff %x to QACT",
				info.buffer);
	...
	struct gsl_sgobj_list *sg_objs,	const struct gsl_key_vector *new_ckv)
{
	...
	for (i = 0; i < sg_objs->len; ++i) {
		...
		if (!sg) {
			 ...
			 * especially with CMA
			 */
			GSL_INFO("Skipping persist cal for SG_ID %d: already started",
				sg->sg_id);
			continue;
		}
...
int32_t gsl_graph_deinit(struct gsl_graph *graph)
{
	...
	ar_osal_mutex_destroy(graph->get_set_cfg_lock);
	ar_osal_mutex_destroy(graph->graph_lock);
	graph->get_set_cfg_lock = NULL;
	graph->graph_lock = NULL;

	ar_list_clear(&graph->gkv_list);
	ar_osal_mutex_destroy(graph->gkv_list_lock);
	graph->gkv_list_lock = NULL;
...
int32_t gsl_graph_set_tagged_custom_config_v2(struct gsl_graph *graph,
	uint32_t tag, const uint8_t *payload, const size_t payload_size,
	ar_osal_mutex_t lock)
{
	int32_t rc = AR_EOK;
	struct apm_cmd_header_t *cmd_header;
	struct gsl_shmem_alloc_data *share_memory;
	...
	if (!is_shmem_supported) {
		...
		GSL_ERR("__gpr_cmd_is_shared_mem_supported failed %d", rc)
		goto exit;
	}
	...
	for (i = 0; i < num_sg_objs; ++i) {
		...
		if (sg_objs[i]->user_persist_cfg_data.handle &&
		    ...
		    GSL_ERR("gsl_graph_set_tagged_custom_config_v2 need to degregister sg index: %d", i);
			rc = gsl_msg_alloc(APM_CMD_DEREGISTER_CFG, graph->src_port,
				module_info->module_entry[0].module_iid, sizeof(*cmd_header), 0,
				graph->proc_id, 0, true, &gsl_msg);
			if (rc) {
				GSL_ERR("Failed to allocate GPR packet %d", rc);
				goto free_mem;
			}

			share_memory = (struct gsl_shmem_alloc_data *)payload;
			cmd_header = GPR_PKT_GET_PAYLOAD(struct apm_cmd_header_t,
				gsl_msg.gpr_packet);
			if(cmd_header == NULL) {
				rc = AR_EBADPARAM;
			...
			cmd_header->mem_map_handle = share_memory->spf_mmap_handle;
			cmd_header->payload_address_lsw = (uint32_t)share_memory->spf_addr;
			cmd_header->payload_address_msw = (uint32_t)(share_memory->spf_addr >> 32);
			cmd_header->payload_size = (uint32_t)GSL_ALIGN_8BYTE(payload_size);
			GSL_ERR("share memory addr  %p", share_memory->v_addr);
		...
		break; // rc == AR_EOK - module tag info found, break from the loop
	}
	if (rc == AR_ENOTEXIST || module_info == NULL) {
		GSL_ERR("module info is not found");
		goto cleanup;
	}

	for (i = 0; i < module_info->num_modules; ++i) {
		rc = gsl_msg_alloc(APM_CMD_REGISTER_CFG, graph->src_port,
			module_info->module_entry[i].module_iid, sizeof(*cmd_header), 0,
			graph->proc_id, 0, true, &gsl_msg);
		if (rc) {
			GSL_ERR("Failed to allocate GPR packet %d", rc);
		...
		/* clients don't know MIID. Set it here */
		*(uint32_t*)share_memory->v_addr = module_info->module_entry[i].module_iid;
	...
	struct gsl_sgid_list sgids, AcdbSubgraph *sg_conn, uint32_t num_sg_conn)
{
	...
	GSL_INFO("num_sgid= %d", sgids.len);
	GSL_INFO("sg list:");
	for (i = 0; i < sgids.len; ++i)
		GSL_INFO("%x", sgids.sg_ids[i]);
	...
	struct gsl_cmd_properties *props)
{
	...
	while (i < gkv_node->num_of_subgraphs) {
		...
		if (is_matching_sg_property(gkv_node->sg_array[i], props)) {
			 ...
			 */
			gsl_memcpy(sg_conn_entry, remaining_bytes,
				next_sg_conn_entry, remaining_bytes);
	...
	struct gsl_cmd_properties *props, bool_t preserve_gkv_node)
{
	...
	(void)gsl_sg_pool_prune_sg_list(gkv_node->sg_array,
		gkv_node->num_of_subgraphs, props, &pruned_sg_ids);
	...
	struct gsl_graph_sg_conn_data *pruned_sg_connections)
{
	...
	num_sg_conn = 0;
	while (i < gkv_node->num_of_subgraphs) {
		rc = gsl_subgraph_add_children(gkv_node->sg_array[i], sg_conn, p);
		if (p->num_dst_sgids) {
			p = (AcdbSubgraph *)((uint32_t *)p->dst_sg_ids +
	...
	struct gsl_key_vector *gkv, struct gsl_key_vector *ckv)
{
	...
	if (sgids->len) {
		...
		if (rc == AR_EUNSUPPORTED || rc == AR_ENOTEXIST) {
			 ...
			 * original error
			 */
			GSL_ERR("gsl_graph_set_sg_cal failed,closing all opened SGs:%d", rc);
	...
	const struct gsl_key_vector *tkv, ar_osal_mutex_t lock)
{
	...
	if (graph->rtc_conn_active) {
		...
		GSL_INFO("set_config: caching buffer %x to QACT", info.buffer);
		ar_data_log_submit(&info);
	}
...
int32_t gsl_graph_config_data_path_extern_push_pull(struct gsl_graph *graph,
	enum gsl_data_dir dir, struct gsl_configure_read_write_params_extern_push_pull *cfg)
{
	int32_t rc = AR_EOK;
	struct gsl_data_path_info *dp_info;
	...
	rc = gsl_dp_config_data_path_extern_push_pull(dp_info, cfg, graph->src_port,
		&graph->graph_signal[GRAPH_CTRL_GRP2_CMD_SIG], dir, NULL, graph->proc_id);
	if (rc)
		GSL_ERR("configure data path failed %d", rc);
	...
	struct gsl_cmd_graph_select *cg, ar_osal_mutex_t lock)
{
	...
	struct gsl_sgid_list sgids = { 0, NULL }, pruned_sgids = { 0, NULL };
	AcdbGetGraphRsp sg_conn_info;
	AcdbSubgraph *p;
	struct gsl_graph_sg_conn_data pruned_sg_conn = {0,};
	uint32_t i = 0;
	...
	while (!ar_list_is_empty(&graph->gkv_list)) {
		...
		rc = gsl_graph_close_single_gkv(graph, temp_node, 0, NULL, NULL, NULL, 0);
		if (rc) {
	...
	ar_osal_mutex_t lock)
{
	...
	GSL_INFO("old SGs & conns");
	//GSL_INFO("num_sgid= %d", old_node->num_of_subgraphs);
	//GSL_INFO("sg list:");
	//for (i = 0; i < old_node->num_of_subgraphs; ++i)
		//GSL_INFO("%x", old_node->sg_array[i]->sg_id);
	uint32_t num_children = 0, num_sg_conn = old_node->sg_conn_data.num_sgs;
	uint32_t *q = (uint32_t *)old_node->sg_conn_data.subgraphs;

	GSL_INFO("num_sg_conn = %d", num_sg_conn);
	for (i = 0; i < num_sg_conn; ++i) {
		GSL_INFO("sg %x has %d children:", *(q), num_children = *(q + 1));
		q += 2;
		while (num_children) {
			GSL_INFO("%x, ", *(q++));
			--num_children;
	...
	GSL_INFO("new SGs & conns");
	//GSL_INFO("num_sgid= %d", params->num_sgs);
	//GSL_INFO("sg list:");
	//for (i = 0; i < params->num_sgs; ++i)
		//GSL_INFO("%x", params->sgs[i]);
	...
	GSL_INFO("Reopen SGs & conns");
	//GSL_INFO("num_sgid= %d", params->num_sgs_to_reopen);
	//GSL_INFO("sg list:");
	//for (i = 0; i < params->num_sgs_to_reopen; ++i)
		//GSL_INFO("%x", params->sgs_to_reopen[i]);
	...
	if (pruned_plus_reopen_sg_conn->size) {
		...
		if (!pruned_plus_reopen_sg_conn  || !pruned_plus_reopen_sg_conn->subgraphs) {
			rc = AR_ENOMEMORY;
	...
	*/
	...
	for (i = 0; i < params->num_sgs; ++i) {
		GSL_INFO("new sg: 0x%x", params->sgs[i]);
