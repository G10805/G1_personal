 ...
 *
 *  Copyright (c) 2018-2024 by Qualcomm Technologies, Inc.
 *  All Rights Reserved.
...
#define WAIT_SS_READY() do {usleep(10*1000);} while(0)
...
static bool_t gsl_main_start_client_op(struct gsl_ctxt_ *ctxt)
{
	...
	if (ctxt->rtgm_state_info.num_rtgm_in_prog) {
...
static int32_t gsl_main_start_client_op_blocking(struct gsl_ctxt_ *ctxt)
{
	...
	while (ctxt->rtgm_state_info.num_rtgm_in_prog) {
		GSL_MUTEX_UNLOCK(ctxt->graph_hdl_lock);
...
static int32_t gsl_rtc_callback(enum gsl_rtc_request_type req, void *cb_data)
{
	...
	static uint32_t restart_list_position = 0, restart_list_size = 0;
	static uint32_t *restart_handle_list = NULL;
	struct gsl_graph *graph = NULL;
	...
	case GSL_RTC_PREPARE_CHANGE_GRAPH:
		...
		if (gsl_graph_get_state(graph) == GRAPH_STARTED) {
			if (!restart_handle_list) {
				GSL_INFO("allocating restart list");
			...
			GSL_INFO("stopping graph 0x%x", prep_change_graph_params->graph_handle);
	...
	case GSL_RTC_CHANGE_GRAPH:
		...
		if (restart_handle_list && change_graph_params->graph_handle
			...
			GSL_INFO("starting graph 0x%x", change_graph_params->graph_handle);

			/* clear restart list if no more to be restarted */
			if (++restart_list_position == restart_list_size) {
				GSL_INFO("freeing restart list");
	...
	void *data_payload, uint32_t *data_payload_size)
{
	...
	rc = acdb_ioctl(ACDB_CMD_GET_GRAPH_TAG_KVS, graph_key_vect,
		sizeof(*graph_key_vect), acdb_rsp.buf, acdb_rsp.buf_size);
	if (rc != AR_EOK)
...
int32_t gsl_init(struct gsl_init_data *init_data)
{
	...
	/*
	 * use graph_hdl_lock here as no sessions will be active at init time and after SSR
	 * so this lock is available for use to use.
	 */
	rc = gsl_signal_create(&gsl_ctxt.rsp_signal, &gsl_ctxt.graph_hdl_lock);
	if (rc) {
	...
	for (i = 0; i < num_master_procs; i++) {
		...
		gsl_spf_ss_state_set(master_procs[i], GSL_GET_SPF_SS_MASK(AR_GP_DSP0) | GSL_GET_SPF_SS_MASK(AR_GP_DSP1) | 0x3F,
	...
	const struct gsl_key_vector *cal_key_vect, gsl_handle_t *graph_handle)
{
	...
	*graph_handle = hdl;
	...
	if (gsl_ctxt.rtc_conn_active)
        ...
        if (ss_retry_count == 0 && rc == AR_ESUBSYSRESET) {
            release_graph_handle(hdl);
        }
...
int32_t gsl_set_tagged_custom_config_v2(gsl_handle_t graph_handle, uint32_t tag,
	const uint8_t *payload, const size_t payload_size)
{
	struct gsl_graph *graph;
	int32_t rc = AR_EOK;
	...
	rc = gsl_graph_set_tagged_custom_config_v2(graph, tag, payload,
		payload_size, NULL);
	if (rc)
		GSL_ERR("graph set tagged custom config failed: %d", rc);
	...
	enum gsl_cmd_id cmd_id, void *cmd_payload, size_t cmd_payload_sz)
{
	...
	case GSL_CMD_CONFIGURE_READ_PARAMS:
		if (((struct gsl_cmd_configure_read_write_params*)cmd_payload)->attributes == GSL_DATA_MODE_EXTERN_PUSH_PULL) {
			GSL_ERR("entering gsl_graph_config_data_path_push_pull");
			rc = gsl_graph_config_data_path_extern_push_pull(graph, GSL_DATA_DIR_READ, (struct gsl_configure_read_write_params_extern_push_pull *) cmd_payload);
		} else {
			if (cmd_payload_sz !=
	...
	case GSL_CMD_CONFIGURE_WRITE_PARAMS:
		if (((struct gsl_cmd_configure_read_write_params*)cmd_payload)->attributes == GSL_DATA_MODE_EXTERN_PUSH_PULL) {
			rc = gsl_graph_config_data_path_extern_push_pull(graph, GSL_DATA_DIR_WRITE, (struct gsl_configure_read_write_params_extern_push_pull *) cmd_payload);
		} else {
			if (cmd_payload_sz !=
				sizeof(struct gsl_cmd_configure_read_write_params)) {
				rc = AR_EBADPARAM;
