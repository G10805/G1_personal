	...
	struct gsl_shmem_page **new_page)
{
	...
	page->shmem_info.cache_type = (flags & GSL_SHMEM_MAP_CACHED) ? AR_SHMEM_CACHED : AR_SHMEM_UNCACHED;
	...
	if (ext_mem_hdl == GSL_EXT_MEM_HDL_NOT_ALLOCD) {
		...
		GSL_VERBOSE("map ext mem page. ext_mem_hdl: 0x%x pa_msw 0x%x, pa_lsw 0x%x",
...
int32_t gsl_shmem_map_extern_mem_push_pull(uint64_t ext_mem_hdl, uint32_t size_bytes, uint32_t master_proc_id,
	struct gsl_shmem_alloc_data *alloc_data, uint32_t flags)
{
	int32_t rc = AR_EOK;
	uint32_t size_page_aligned = 0;
	struct gsl_shmem_page *page = NULL;
	...
	GSL_INFO("ext_mem_hdl=0x%lx, size_bytes=%d, size_page_aligned=%d", ext_mem_hdl, size_bytes, size_page_aligned);

	GSL_MUTEX_LOCK(ctxt[master_proc_id]->mutex);

	// won't actually allocate when we send an ext_mem_handle, it will only map
	rc = allocate_page(size_page_aligned, GSL_SHMEM_MGR_BIN_IDX_DEDICATED,
			GSL_GET_SPF_SS_MASK(master_proc_id), flags, 0, ext_mem_hdl, master_proc_id,
			&page); //pt_map_flag: 1,  use pt to do ar_shmem_map, rather than truly allocate
	if (rc) {
		GSL_ERR("failed to allocate page %d", rc);
		goto exit;
	}
	if (alloc_data && page) {
		...
		GSL_DBG("phandle: 0x%llx, v_addr: 0x%llx, spf_addr: 0x%llx",
			alloc_data->handle, alloc_data->v_addr, alloc_data->spf_addr);
	}
#ifdef GSL_SHMEM_MGR_STATS_ENABLE
	GSL_LOG_PKT("mem_stat", GSL_SHMEM_SRC_PORT, &stats,
		sizeof(struct gsl_shmem_stats), NULL, 0);
...
int32_t gsl_shmem_unmap_extern_mem_push_pull(struct gsl_shmem_alloc_data alloc_data)
{
	int32_t rc = AR_EOK;
	struct gsl_shmem_page *page = alloc_data.handle;
	uint32_t master_proc_id = 0;
