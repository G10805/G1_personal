...
static void ext_mem_cache_clean(uint64_t dp_handle)
{
	int32_t rc = AR_EOK;
	uint32_t i;
	...
	for (i = 0; i < GSL_MAX_CACHE_SIZE; ++i) {
		if(ext_mem_cache.entries[i].dp_handle == dp_handle) {

			GSL_MUTEX_LOCK(ext_mem_cache.entries[i].lock);
			if(ext_mem_cache.entries[i].dp_handle != dp_handle) {
				GSL_DBG("entry[%d] for dp_handle 0x%u is already evicted", i, dp_handle);
				return;
			}
			if(ext_mem_cache.entries[i].num_bufs_in_flight != 0) {
				/* this indicates bad state in audio */
				GSL_ERR("Fatal error: dp_handle 0x%u: alloc_handle 0x%u still in use",
				    dp_handle, ext_mem_cache.entries[i].alloc_handle);
				//todo: should skip unmap and double check unique dp_handle when use cache entry
			}

			/* force unmap to avoid system level memory corruption */
			GSL_DBG("unmap extern mem 0x%u, dp_handle is 0x%u",
			    ext_mem_cache.entries[i].alloc_handle, dp_handle);
			ext_mem_cache.entries[i].alloc_handle = GSL_EXT_MEM_HDL_NOT_ALLOCD;
			ext_mem_cache.entries[i].dp_handle = 0xFFFFFFFF;
			gsl_memcpy (&to_unmap, sizeof(struct gsl_shmem_alloc_data),
			    &ext_mem_cache.entries[i].shmem_data, sizeof(struct gsl_shmem_alloc_data));
			GSL_MUTEX_UNLOCK(ext_mem_cache.entries[i].lock);

			/* unmap out of global lock */
			rc = gsl_shmem_unmap_extern_mem(to_unmap);
			if (rc != AR_EOK)
				GSL_ERR("unmap extern mem failed, rc=%d", rc);

			return;
		}
	}
	GSL_DBG("nothing to unmap for dp_handle 0x%u", dp_handle);
}
	...
	while (!entry_locked) {
		...
		if (ext_mem_cache.entries[lru_idx].num_bufs_in_flight == 0)	{
			...
			ext_mem_cache.entries[lru_idx].alloc_handle
				= GSL_EXT_MEM_HDL_NOT_ALLOCD;
			ext_mem_cache.entries[lru_idx].dp_handle
				= 0xFFFFFFFF;
...
static uint32_t ext_mem_cache_get_entry(uint32_t proc_id,
	...
	struct gsl_shmem_alloc_data *alloc_data, uint32_t *idx, uint64_t dp_handle)
{
	int32_t rc = AR_EOK;
	...
	for (i = 0; i < GSL_MAX_CACHE_SIZE; ++i) {
		...
		if (ext_mem_cache.entries[i].num_bufs_in_flight == 0 &&
			...
			if (ext_mem_cache.entries[i].num_bufs_in_flight == 0 &&
				...
				ext_mem_cache.entries[i].alloc_handle
					= GSL_EXT_MEM_HDL_NOT_ALLOCD;
				ext_mem_cache.entries[i].dp_handle
					= 0xFFFFFFFF;
	...
	ext_mem_cache.entries[i].dp_handle = dp_handle;
	GSL_DBG("mapped for alloc_handle 0x%u, associated with dp_handle 0x%u",
	    ext_mem_cache.entries[i].alloc_handle, ext_mem_cache.entries[i].dp_handle);
	GSL_MUTEX_UNLOCK(ext_mem_cache.entries[i].lock);
	 ...
	 */
	if (to_unmap.handle) {
		GSL_DBG("Unmapping because evicted, handle 0x%x", ext_mem_cache.entries[i].alloc_handle);
		rc = gsl_shmem_unmap_extern_mem(to_unmap);
		if (rc != AR_EOK) {
	...
	struct gsl_data_path_info *dp_info, struct gsl_signal *signal)
{
	...
	GSL_DBG("Data Buffer - mem_map_handle: 0x%lx, buf_addr_lsw: 0x%lx, buf_addr_msw: 0x%lx, buf_size: 0x%lx",
		pull_push_cfg->circ_buf_mem_map_handle, pull_push_cfg->shared_circ_buf_addr_lsw, pull_push_cfg->shared_circ_buf_addr_msw, pull_push_cfg->shared_circ_buf_size);
	GSL_DBG("Pos Buffer - mem_map_handle=0x%lx, buf_addr_lsw=0x%lx, buf_addr_msw=0x%lx",
		   pull_push_cfg->pos_buf_mem_map_handle, pull_push_cfg->shared_pos_buf_addr_lsw, pull_push_cfg->shared_pos_buf_addr_msw);
	...
	struct gsl_buff *client_buff)
{
	...
	rc = gsl_allocate_gpr_packet(opcode,
	...
	if (client_buff->flags & GSL_BUFF_FLAG_MEDIA_FORMAT) {
		...
		write_cmd =
			GPR_PKT_GET_PAYLOAD(data_cmd_wr_sh_mem_ep_data_buffer_v2_t, send_pkt);
	...
	struct gsl_buff *buff)
{
	...
	rc = ext_mem_cache_get_entry(dp_info->master_proc_id, buff->alloc_info,
				     &internal_buf.gsl_msg.shmem, &cache_idx, (uint64_t)dp_info);
	if (rc != AR_EOK)
		return rc;
...
void gsl_data_path_deinit(struct gsl_data_path_info *dp_info)
{
	...
	if (GSL_DP_DATA_MODE(dp_info) == GSL_DATA_MODE_EXTERN_MEM) {
		/* memory is freed after datapath destroy, cleanup cached map entry associated with dp to avoid memory corroption */
		ext_mem_cache_clean((uint64_t)dp_info);
		ext_mem_cache_deinit();
	}

	if (GSL_DP_DATA_MODE(dp_info) == GSL_DATA_MODE_EXTERN_PUSH_PULL) {
		for (i = 0; i < PUSH_PULL_BUFFS; i++) {
			rc = gsl_shmem_unmap_extern_mem_push_pull(dp_info->buff_list[i].gsl_msg.shmem);
			if (rc)
				GSL_ERR("gsl_shmem_unmap_extern_mem_push_pull failed for buff list idx: %u, rc=%d", i , rc);

			if (dp_info->buff_list[i].gsl_msg.gpr_packet)
				__gpr_cmd_free(dp_info->buff_list[i].gsl_msg.gpr_packet);
		}
	} else {
	...
	ar_osal_mutex_t lock, uint32_t proc_id)
{
	 ...
	 */
	gpr_pkt_info.bytes_per_min_size_packet = 0;
...
int32_t gsl_dp_config_data_path_extern_push_pull(struct gsl_data_path_info *dp_info,
	struct gsl_configure_read_write_params_extern_push_pull *cfg, uint32_t src_port,
	struct gsl_signal *push_pull_cfg_signal, enum gsl_data_dir dir,
	ar_osal_mutex_t lock, uint32_t proc_id)
{
	int32_t rc = AR_EOK;
	...
	dp_info->config.buff_size = cfg->buff_size;
	dp_info->config.num_buffs = PUSH_PULL_BUFFS; //cfg->num_buffs;
	dp_info->config.start_threshold = cfg->start_threshold;
	dp_info->config.attributes = cfg->attributes;
	dp_info->config.shmem_ep_tag = cfg->shmem_ep_tag;
	dp_info->config.platform_info = cfg->platform_info;
	...
	rc = gsl_shmem_map_extern_mem_push_pull((uint64_t)cfg->push_pull_cfg[0].phandle, dp_info->config.buff_size, proc_id, &dp_info->buff_list[0].gsl_msg.shmem, flags);
	if (rc) {
		GSL_ERR("failed to map push pull info buffers, %d for proc id %d", rc, proc_id);
		goto exit;
	}

	rc = gsl_shmem_map_extern_mem_push_pull((uint64_t)cfg->push_pull_cfg[1].phandle, sizeof(struct sh_mem_pull_push_mode_position_buffer_t), proc_id, &dp_info->buff_list[1].gsl_msg.shmem, GSL_SHMEM_DEDICATED_PAGE | GSL_SHMEM_MAP_UNCACHED);

	if (rc) {
		GSL_ERR("failed to map push pull position buffers, %d for proc id %d", rc, proc_id);
		goto error_pos_buf;
	}
	...
	if (GSL_DATAPATH_SETUP_MODE(cfg) == GSL_DATAPATH_SETUP_SPF_PROVISION_ONLY
		 ...
		 * send now-allocated push pull buffers
		 */
		rc = gsl_dp_configure_pull_push_on_spf(dp_info, push_pull_cfg_signal);
		if (rc) {
			  GSL_ERR("push_pull_dbg: failed to send push pull buffers to spf %d", rc);
			  goto cleanup;
		} else {
			  GSL_INFO("push_pull_dbg: map to spf succesfully");
		}
	}
...
cleanup:
	/** free pos buf **/
	gsl_shmem_free(&dp_info->buff_list[1].gsl_msg.shmem);
	...
	gpr_packet_t *packet, gsl_cb_func_ptr cb, void *client_data)
{
	...
	if (!dp_info->is_shmem_supported)
		return gsl_handle_buff_done_nonshmem(dp_info, packet,
						     GSL_EVENT_ID_WRITE_DONE,
						     cb, client_data, status,
						     buff_idx);
...
int32_t gsl_dp_queue_read_buffers_to_spf(struct gsl_data_path_info *dp_info)
{
	...
	if (GSL_DP_DATA_MODE(dp_info) != GSL_DATA_MODE_SHMEM &&
		GSL_DP_DATA_MODE(dp_info) != GSL_DATA_MODE_PUSH_PULL &&
		GSL_DP_DATA_MODE(dp_info) != GSL_DATA_MODE_EXTERN_PUSH_PULL) {
