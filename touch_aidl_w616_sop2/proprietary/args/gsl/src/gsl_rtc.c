...
static uint32_t gsl_rtc_gpr_callback(gpr_packet_t *packet, void *cb_data)
{
	...
	case GPR_IBASIC_RSP_RESULT:
		...
		case APM_CMD_DEREGISTER_CFG:
			...
			break;
		default:
			GSL_ERR("unknown opcode %x freeing GPR rsp packet",
				basic_rsp->opcode);
			break;
		}
		break;
	case APM_CMD_RSP_GET_CFG:
		...
		break;
	default:
		GSL_ERR("unknown opcode %x freeing GPR rsp packet", packet->opcode);
		break;
	}
	if(rc) {
		GSL_ERR("signal failed, freeing GPR rsp packet");
	}
	...
	uint32_t total_size, uint8_t *sg_cal_data)
{
	...
	apm_module_param_data_t *param_data = (apm_module_param_data_t *)sg_cal_data;
	...
	rc = gsl_send_spf_cmd(&gsl_msg.gpr_packet,&rtc_internal_ctxt.sig, &rsp);

	if (rc !=  AR_EOK && IS_ONE_PARAM(param_data->param_size,cmd_header->payload_size)) {
