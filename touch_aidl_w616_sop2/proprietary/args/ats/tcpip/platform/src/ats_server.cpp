...
void* ats_server_transmit_routine(void* client_socket_ptr)
{
    ...
    */
	while (true)
	{
        ...
        ATS_DBG("Command[%s-%d]: Request data length is %d bytes", svc_id_str, ATS_GET_COMMAND_ID(svc_cmd_id), msgBufCheck);
		...
		ATS_DBG("Command[%s-%d]: Sending response of %d bytes", svc_id_str, ATS_GET_COMMAND_ID(svc_cmd_id), outlen);
...
int32_t ats_server_deinit()
{
    ...
    if (AR_FAILED(status))
    {
        ATS_ERR("Error[%d]:An error occured when waiting to close the listener thread.", status);
    }

    status = ar_osal_mutex_destroy(g_connected_mutex);
    if (AR_FAILED(status))
    {
        ATS_ERR("Error[%d]:An error occured while destroying the mutex.", status);
    }
    return status;
}
