...
gpr_proc_info_t gpr_proc_info[GPR_PL_NUM_TOTAL_DOMAINS_V] = { { GPR_IDS_DOMAIN_ID_MODEM_V, "modem" },
...
GPR_INTERNAL uint32_t gpr_drv_deinit(void)
{
   ...
   for (uint32_t idx = 0; idx < gpr_ctxt_struct_t.num_static_packet_pools; idx++)
   {
      ...
      ar_heap_info heap_info;
      ar_mem_set((void *)&heap_info, 0, sizeof(ar_heap_info));
      gpr_populate_ar_heap_info(gpr_ctxt_struct_t.static_pool_arr[idx].heap_index,
                                         AR_HEAP_ALIGN_8_BYTES,
                                         &heap_info);
