...
// clang-format off
/*
$Header: //components/rel/gpr.common/1.0/core/src/gpr_drv_island.c#1 $
*/
// clang-format on
...
uint32_t __gpr_cmd_alloc_v2(uint32_t alloc_size, gpr_heap_index_t heap_index, gpr_packet_t **ret_packet)
{
   ...
   if (NULL == new_packet)
   {
      ...
      {
         if ((alloc_size <= gpr_ctxt_struct_t.dyn_pool_arr[idx].buf_size) &&
             (gpr_ctxt_struct_t.dyn_pool_arr[idx].curr_num_packets < gpr_ctxt_struct_t.dyn_pool_arr[idx].max_num_packets) &&
             (heap_index == gpr_ctxt_struct_t.dyn_pool_arr[idx].heap_index))
         {
            found_packet_pool = TRUE;

            gpr_allocate_dynamic_packet(&new_packet, alloc_size);
            if (new_packet == NULL)
            {
...
uint32_t __gpr_cmd_end_command(gpr_packet_t *packet, uint32_t status)
{
         ...
         ((opcode_type & AR_GUID_TYPE_DATA_EVENT) == AR_GUID_TYPE_DATA_EVENT)))
   {
      ...
      if (rc)
      {
         AR_MSG(DBG_ERROR_PRIO, "gpr_unable to send");
         goto __bailout;
      }
   }
   else
