...
int32_t ServiceStateUpate(void *context)
{
    ...
    ar_osal_servreg_entry_type dmn_name[AR_SUB_SYS_ID_LAST] = { 0 };
...
int32_t clientServiceListener(void *context)
{
    ...
    CreateTestThread(ActiveThreads++, (ar_osal_thread_start_routine)ServiceStateUpate);
...
void ar_test_servreg_main(void)
{
    int32_t status = AR_EOK;
    ActiveThreads = 0;
    ...
    CreateTestThread(ActiveThreads++, (ar_osal_thread_start_routine)clientServiceListener);
