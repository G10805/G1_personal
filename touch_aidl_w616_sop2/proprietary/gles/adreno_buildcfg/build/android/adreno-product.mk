...
MM_GRAPHICS += libVkLayer_ADRENO_qprofiler
...
MM_GRAPHICS += libadreno_compiler_core
...
MM_GRAPHICS += sequence_manifest.bin
MM_GRAPHICS += unified_ksqs.bin
MM_GRAPHICS += unified_kbcs_64.bin
MM_GRAPHICS += unified_kbcs_32.bin

ADRENO_BUILDCFG_DIR := $(dir $(realpath $(lastword $(MAKEFILE_LIST))))
include $(ADRENO_BUILDCFG_DIR)android_version.mk
...
ifneq ($(filter $(TARGET_BOARD_PLATFORM), $(VULKAN_DISABLED_TARGETS_LIST)),$(TARGET_BOARD_PLATFORM))
...
ifeq ($(call is-board-platform-in-list,$(VULKAN_API_VERSION_1_3_TARGETS_LIST)), true)
        PRODUCT_COPY_FILES += \
...
ifeq ($(call is-board-platform-in-list,$(VULKAN_FEATURE_LEVEL_0_TARGETS_LIST)), true)
    ...
    ifeq ($(call CHECK_VERSION_GE, $(ADRENO_PLATFORM_SDK_VERSION), $(PLATFORM_SDK_UPDK)),true)
        PRODUCT_COPY_FILES += \
            frameworks/native/data/etc/android.software.vulkan.deqp.level-2023-03-01.xml:$(TARGET_COPY_OUT_VENDOR)/etc/permissions/android.software.vulkan.deqp.level.xml
    else ifeq ($(call CHECK_VERSION_GE, $(ADRENO_PLATFORM_SDK_VERSION), $(PLATFORM_SDK_TPDK)),true)
        PRODUCT_COPY_FILES += \
...
# android.software.opengles.deqp.level*.xml is a new file added in android S and above defining dEQP version supported for device.
ifeq ($(call CHECK_VERSION_GE, $(ADRENO_PLATFORM_SDK_VERSION), $(PLATFORM_SDK_UPDK)),true)
    PRODUCT_COPY_FILES += \
...
else ifeq ($(call CHECK_VERSION_GE, $(ADRENO_PLATFORM_SDK_VERSION), $(PLATFORM_SDK_SPDK)),true)
    PRODUCT_COPY_FILES += \
...
include $(ADRENO_BUILDCFG_DIR)driverpackagename.mk
...
ifneq ($(filter $(TARGET_BOARD_PLATFORM), $(EXTENSION_PACK_FEATURE_DISABLED_TARGETS_LIST)),$(TARGET_BOARD_PLATFORM))
...
ifeq ($(filter $(TARGET_BOARD_PLATFORM), sdm845 msm8996 msm8998),$(TARGET_BOARD_PLATFORM))
PRODUCT_COPY_FILES += \
    frameworks/native/data/etc/android.hardware.vr.high_performance.xml:$(TARGET_COPY_OUT_VENDOR)/etc/permissions/android.hardware.vr.high_performance.xml
endif

# GSL mem pool def file. If this is specified for any application,
# defualt mempool sizes will be overridden
GSL_MEMPOOL_DEF_PATH = vendor/qcom/proprietary/gles/adreno200/gsl_rpc_mempool.def
ifneq ("$(wildcard $(TOPDIR)$(GSL_MEMPOOL_DEF_PATH))","")
PRODUCT_COPY_FILES += \
    $(GSL_MEMPOOL_DEF_PATH):$(TARGET_COPY_OUT_VENDOR)/etc/gsl_rpc_mempool.def
$(warning "Adreno: Using gsl_rpc_mempool.def")
else
$(warning "Adreno: Not using gsl_rpc_mempool.def")
endif
...
# System prop for GPU Profiler support
# Support for profiler is not present in driver yet
