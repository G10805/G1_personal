...
static void PopulatePowerModes(drmModePropertyRes *prop) {
  for (auto i = 0; i < prop->count_enums; i++) {
    ...
    if (enum_name == "ON") {
      ON = static_cast<uint8_t>(prop->enums[i].value);
    } else if (enum_name == "LP1") {
      DOZE = static_cast<uint8_t>(prop->enums[i].value);
    } else if (enum_name == "LP2") {
      DOZE_SUSPEND = static_cast<uint8_t>(prop->enums[i].value);
    } else if (enum_name == "OFF") {
      OFF = static_cast<uint8_t>(prop->enums[i].value);
    }
  }
}
...
static DRMTopology GetTopologyEnum(const string &topology) {
  if (topology == "sde_singlepipe")
    return DRMTopology::SINGLE_LM;
  if (topology == "sde_singlepipe_dsc")
    return DRMTopology::SINGLE_LM_DSC;
  if (topology == "sde_dualpipe")
    return DRMTopology::DUAL_LM;
  if (topology == "sde_dualpipe_dsc")
    return DRMTopology::DUAL_LM_DSC;
  if (topology == "sde_dualpipemerge")
    return DRMTopology::DUAL_LM_MERGE;
  if (topology == "sde_dualpipemerge_dsc")
    return DRMTopology::DUAL_LM_MERGE_DSC;
  if (topology == "sde_dualpipe_dscmerge")
    return DRMTopology::DUAL_LM_DSCMERGE;
  if (topology == "sde_triplepipe")
    return DRMTopology::TRIPLE_LM;
  if (topology == "sde_triplepipe_dsc")
    return DRMTopology::TRIPLE_LM_DSC;
  if (topology == "sde_quadpipemerge")
    return DRMTopology::QUAD_LM_MERGE;
  if (topology == "sde_quadpipe_dscmerge")
    return DRMTopology::QUAD_LM_DSCMERGE;
  if (topology == "sde_quadpipe_3dmerge_dsc")
    return DRMTopology::QUAD_LM_MERGE_DSC;
  if (topology == "sde_sixpipemerge")
    return DRMTopology::SIX_LM_MERGE;
  if (topology == "sde_sixpipe_dscmerge")
    return DRMTopology::SIX_LM_DSCMERGE;
  if (topology == "sde_ppsplit")
    return DRMTopology::PPSPLIT;
...
static void PopulateQsyncModes(drmModePropertyRes *prop) {
  for (auto i = 0; i < prop->count_enums; i++) {
    string enum_name(prop->enums[i].name);
    if (enum_name == "none") {
      QSYNC_MODE_NONE = static_cast<uint8_t>(prop->enums[i].value);
    } else if (enum_name == "continuous") {
      QSYNC_MODE_CONTINUOUS = static_cast<uint8_t>(prop->enums[i].value);
    } else if (enum_name == "one_shot") {
      QSYNC_MODE_ONESHOT = static_cast<uint8_t>(prop->enums[i].value);
    }
  }
}
...
void DRMConnector::ParseCapabilities(uint64_t blob_id, DRMConnectorInfo *info) {
  ...
  const string max_blendstages = "max_blendstages=";
  ...
  while (std::getline(stream, line)) {
    if (line.find(pixel_formats) != string::npos) {
      ...
      info->max_linewidth = static_cast<uint32_t>(std::stoi(string(line, max_linewidth.length())));
    ...
    } else if (line.find(max_blendstages) != string::npos) {
      info->max_blendstages = static_cast<uint32_t>(std::stoi(string(line, max_blendstages.length())));
    }
  }
...
void DRMConnector::ParseModeProperties(uint64_t blob_id, DRMConnectorInfo *info) {
  ...
  while (std::getline(stream, line)) {
    if (line.find(mode_name) != string::npos) {
      ...
      mode_item->bit_clk_rate = static_cast<uint64_t>(std::stoi(string(line, bit_clk_rate.length())));
    } else if (line.find(mdp_transfer_time_us) != string::npos) {
      mode_item->transfer_time_us = static_cast<uint32_t>(std::stoi(string(line, mdp_transfer_time_us.length())));
    }
  }
...
void DRMConnector::ParseCapabilities(uint64_t blob_id, drm_msm_ext_hdr_properties *hdr_info) {
  ...
  if (hdr_cdata) {
    ...
    DRM_LOGI(
        "hdr_supported=%d , hdr_eotf= %d , hdr_metadata_type_one= %d,"
...
void DRMConnector::ParseCapabilities(uint64_t blob_id, std::vector<uint8_t> *edid) {
  ...
  uint8_t *edid_blob = reinterpret_cast<uint8_t *>(blob->data);
...
int DRMConnector::GetInfo(DRMConnectorInfo *info) {
  ...
  if (prop_mgr_.IsPropertyAvailable(DRMProperty::HDR_PROPERTIES)) {
    index = static_cast<uint32_t>(std::distance(props->props,
  ...
  if (prop_mgr_.IsPropertyAvailable(DRMProperty::CAPABILITIES)) {
    index =
        static_cast<uint32_t>(std::distance(props->props, std::find(props->props, props->props + props->count_props,
  ...
  if (prop_mgr_.IsPropertyAvailable(DRMProperty::TOPOLOGY_CONTROL)) {
    ...
    info->topology_control = static_cast<uint32_t>(props->prop_values[index]);
  }
  if (prop_mgr_.IsPropertyAvailable(DRMProperty::EDID)) {
    index = static_cast<uint32_t>(std::distance(props->props, std::find(props->props, props->props + props->count_props,
...
void DRMConnector::Perform(DRMOps code, drmModeAtomicReq *req, va_list args) {
  ...
  switch (code) {
    ...
    case DRMOps::CONNECTOR_SET_OUTPUT_RECT: {
      ...
      drmModeAtomicAddProperty(req, obj_id, prop_mgr_.GetPropertyId(DRMProperty::DST_X), rect.left);
      drmModeAtomicAddProperty(req, obj_id, prop_mgr_.GetPropertyId(DRMProperty::DST_Y), rect.top);
      ...
      DRM_LOGD("Connector %d: Setting dst [x,y,w,h][%d,%d,%d,%d]", obj_id, rect.left, rect.top,
    ...
    case DRMOps::CONNECTOR_SET_POWER_MODE: {
      ...
      switch (drm_power_mode) {
        case static_cast<int>(DRMPowerMode::ON):
          power_mode = ON;
          break;
        case static_cast<int>(DRMPowerMode::DOZE):
          power_mode = DOZE;
          break;
        case static_cast<int>(DRMPowerMode::DOZE_SUSPEND):
          power_mode = DOZE_SUSPEND;
          break;
        case static_cast<int>(DRMPowerMode::OFF):
          power_mode = OFF;
          break;
        default:
    ...
    case DRMOps::CONNECTOR_SET_FB_SECURE_MODE: {
      ...
      uint32_t fb_secure_mode = (secure_mode == static_cast<int>(DRMSecureMode::SECURE)) ? SECURE
              : NON_SECURE;
      drmModeAtomicAddProperty(
          req, obj_id, prop_mgr_.GetPropertyId(DRMProperty::FB_TRANSLATION_MODE), fb_secure_mode);
    ...
    case DRMOps::CONNECTOR_SET_EPT: {
      ...
      DRM_LOGD("Connector %d: Setting ePT = %", obj_id, expected_present_time);
    } break;
...
void DRMConnector::SetROI(drmModeAtomicReq *req, uint32_t obj_id, uint32_t num_roi,
  ...
  for (uint32_t i = 0; i < num_roi; i++) {
    roi_v1.roi[i].x1 = static_cast<unsigned short>(conn_rois[i].left);
    roi_v1.roi[i].x2 = static_cast<unsigned short>(conn_rois[i].right);
    roi_v1.roi[i].y1 = static_cast<unsigned short>(conn_rois[i].top);
    roi_v1.roi[i].y2 = static_cast<unsigned short>(conn_rois[i].bottom);
    DRM_LOGD("Conn %d, ROI[l,t,b,r][%d %d %d %d]", obj_id, roi_v1.roi[i].x1, roi_v1.roi[i].y1,
...
int DRMConnector::GetPossibleEncoders(set<uint32_t> *possible_encoders) {
  ...
  uint32_t count_enc = static_cast<uint32_t>(drm_connector_->count_encoders);
  if (count_enc == 0) {
