...
void ParseFormats(const string &line, vector<pair<uint32_t, uint64_t>> *formats) {
  ...
  while (std::regex_search(tmp_line, str_match, exp_base)) {  // clang_sa_ignore[core.CallAndMessage] //NOLINT
    ...
    if (std::regex_match(matched_sub_str, exp_modifier)) {  // clang_sa_ignore[core.CallAndMessage]
      ...
      uint64_t vendor_code = static_cast<uint64_t>(std::stoi(vendor_sub_str));

      // Match modifier
      uint64_t fmt_modifier = static_cast<uint64_t>(std::stoi(vendor_sub_str.substr(vendor_sub_str.find("/") + 1)));
