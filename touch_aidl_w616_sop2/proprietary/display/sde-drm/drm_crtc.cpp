  ...
  if (!security_levels_populated) {
    for (auto i = 0; i < prop->count_enums; i++) {
      ...
      if (enum_name == "sec_and_non_sec") {
        SECURE_NON_SECURE = static_cast<uint8_t>(prop->enums[i].value);
      } else if (enum_name == "sec_only") {
        SECURE_ONLY = static_cast<uint8_t>(prop->enums[i].value);
      }
    }
  ...
  if (!capture_modes_populated) {
    for (auto i = 0; i < prop->count_enums; i++) {
      ...
      if (enum_name == "capture_mixer_out") {
        CAPTURE_MIXER_OUT = static_cast<uint8_t>(prop->enums[i].value);
      } else if (enum_name == "capture_pp_out") {
        CAPTURE_DSPP_OUT = static_cast<uint8_t>(prop->enums[i].value);
      }
    }
  ...
  if (!idle_pc_state_populated) {
    for (auto i = 0; i < prop->count_enums; i++) {
      ...
      if (enum_name == "idle_pc_none") {
        IDLE_PC_STATE_NONE = static_cast<uint8_t>(prop->enums[i].value);
      } else if (enum_name == "idle_pc_enable") {
        IDLE_PC_STATE_ENABLE = static_cast<uint8_t>(prop->enums[i].value);
      } else if (enum_name == "idle_pc_disable") {
        IDLE_PC_STATE_DISABLE = static_cast<uint8_t>(prop->enums[i].value);
      }
    }
...
void DRMCrtcManager::Init(drmModeRes *resource) {
  for (int i = 0; i < resource->count_crtcs; i++) {
    unique_ptr<DRMCrtc> crtc(new DRMCrtc(fd_, static_cast<uint32_t>(i)));
...
void DRMCrtcManager::Perform(DRMOps code, uint32_t obj_id, drmModeAtomicReq *req, va_list args) {
...
int DRMCrtcManager::Reserve(const std::set<uint32_t> &possible_crtc_indices,
  ...
  for (auto &item : crtc_pool_) {
    if (item.second->GetStatus() == DRMStatus::FREE) {
      if (possible_crtc_indices.find(static_cast<const unsigned int>(item.second->GetIndex())) != possible_crtc_indices.end()) {
        item.second->Lock();
        token->crtc_id = item.first;
        token->crtc_index = static_cast<uint32_t>(item.second->GetIndex());
        return 0;
      }
    }
  }
...
int DRMCrtcManager::GetCrtcIndex(uint32_t crtc_id) {
  auto iter = crtc_pool_.find(crtc_id);
  if (iter == crtc_pool_.end()) {
    DRM_LOGE("Invalid crtc id %d", crtc_id);
    return -ENODEV;
...
void DRMCrtc::ParseProperties() {
  ...
  for (uint32_t j = 0; j < props->count_props; j++) {
    ...
    if (prop_enum < DRMProperty::MAX) {
      prop_mgr_.SetPropertyId(prop_enum, info->prop_id);
    } else {
      DRM_LOGW("prop enum index %d is greater than size %d", prop_enum,
               DRMProperty::MAX);
    }
...
void DRMCrtc::ParseCapabilities(uint64_t blob_id) {
  drmModePropertyBlobRes *blob = drmModeGetPropertyBlob(fd_, static_cast<uint32_t>(blob_id));
  if (!blob) {
    return;
  }
  ...
  if(!fmt_str) {
    DRM_LOGE("Failed to allocate memory");
    return;
  }
  ...
  string vbif_cmd_ff = "vbif_cmd_ff=";
  ...
  while (std::getline(stream, line)) {
    if (line.find(max_blendstages) != string::npos) {
      crtc_info_.max_blend_stages = static_cast<uint32_t>(std::stoll(string(line, max_blendstages.length())));
      ...
      crtc_info_.has_src_split = std::stoll(string(line, has_src_split.length()));
      ...
      crtc_info_.dest_scale_prefill_lines =
          static_cast<uint32_t>(std::stoll(string(line, dest_scale_prefill_lines.length())));
    } else if (line.find(undersized_prefill_lines) != string::npos) {
      crtc_info_.undersized_prefill_lines =
          static_cast<uint32_t>(std::stoll(string(line, undersized_prefill_lines.length())));
    } else if (line.find(macrotile_prefill_lines) != string::npos) {
      crtc_info_.macrotile_prefill_lines =
          static_cast<uint32_t>(std::stoll(string(line, macrotile_prefill_lines.length())));
    } else if (line.find(yuv_nv12_prefill_lines) != string::npos) {
      crtc_info_.nv12_prefill_lines = static_cast<uint32_t>(std::stoll(string(line, yuv_nv12_prefill_lines.length())));
    } else if (line.find(linear_prefill_lines) != string::npos) {
      crtc_info_.linear_prefill_lines = static_cast<uint32_t>(std::stoll(string(line, linear_prefill_lines.length())));
    } else if (line.find(downscaling_prefill_lines) != string::npos) {
      crtc_info_.downscale_prefill_lines =
          static_cast<uint32_t>(std::stoll(string(line, downscaling_prefill_lines.length())));
    } else if (line.find(xtra_prefill_lines) != string::npos) {
      crtc_info_.extra_prefill_lines = static_cast<uint32_t>(std::stoll(string(line, xtra_prefill_lines.length())));
    } else if (line.find(amortizable_threshold) != string::npos) {
      crtc_info_.amortized_threshold = static_cast<uint32_t>(std::stoll(string(line, amortizable_threshold.length())));
      ...
      crtc_info_.hw_version = static_cast<uint32_t>(std::stoul(string(line,hw_version.length())));
    } else if (line.find(solidfill_stages) != string::npos) {
      crtc_info_.max_solidfill_stages = static_cast<uint32_t>(std::stoll(string(line, solidfill_stages.length())));
    } else if (line.find(dest_scaler_count) != string::npos) {
      crtc_info_.dest_scaler_count = static_cast<uint32_t>(std::stoll(string(line, dest_scaler_count.length())));
    } else if (line.find(max_dest_scale_up) != string::npos) {
      crtc_info_.max_dest_scale_up = static_cast<uint32_t>(std::stoll(string(line, max_dest_scale_up.length())));
    } else if (line.find(max_dest_scaler_input_width) != string::npos) {
      crtc_info_.max_dest_scaler_input_width =
           static_cast<uint32_t>(std::stoll(string(line, max_dest_scaler_input_width.length())));
    } else if (line.find(max_dest_scaler_output_width) != string::npos) {
      crtc_info_.max_dest_scaler_output_width =
           static_cast<uint32_t>(std::stoll(string(line, max_dest_scaler_output_width.length())));
    } else if (line.find(has_hdr) != string::npos) {
      crtc_info_.has_hdr = std::stoll(string(line, has_hdr.length()));
    } else if (line.find(min_prefill_lines) != string::npos) {
      crtc_info_.min_prefill_lines =  static_cast<uint32_t>(std::stoll(string(line, min_prefill_lines.length())));
    } else if (line.find(sec_ui_blendstage) != string::npos) {
      crtc_info_.secure_disp_blend_stage =  static_cast<int>(std::stoll(string(line, (sec_ui_blendstage).length())));
    } else if (line.find(num_mnocports) != string::npos) {
      crtc_info_.num_mnocports =  static_cast<uint32_t>(std::stoll(string(line, num_mnocports.length())));
    } else if (line.find(mnoc_bus_width) != string::npos) {
      crtc_info_.mnoc_bus_width =  static_cast<uint32_t>(std::stoll(string(line, mnoc_bus_width.length())));
    } else if (line.find(linewidth_constraints) != string::npos) {
      crtc_info_.line_width_constraints_count =
           static_cast<uint32_t>(std::stoll(string(line, (linewidth_constraints).length())));
    } else if (line.find(vig) != string::npos) {
      crtc_info_.vig_limit_index =  static_cast<uint32_t>(std::stoll(string(line, (vig).length())));
    } else if (line.find(dma) != string::npos) {
      crtc_info_.dma_limit_index =  static_cast<uint32_t>(std::stoll(string(line, (dma).length())));
    } else if (line.find(scaling) != string::npos) {
      crtc_info_.scaling_limit_index =  static_cast<uint32_t>(std::stoll(string(line, (scaling).length())));
    } else if (line.find(rotation) != string::npos) {
      crtc_info_.rotation_limit_index =  static_cast<uint32_t>(std::stoll(string(line, (rotation).length())));
    } else if (line.find(linewidth_values) != string::npos) {
      uint32_t num_linewidth_values =  static_cast<uint32_t>(std::stoll(string(line, (linewidth_values).length())));
      vector< pair <uint32_t, uint32_t> > constraints_vector;
      for (uint32_t i = 0; i < num_linewidth_values; i++) {
        uint32_t constraint = 0;
        uint32_t value  = 0;
        std::getline(stream, line);
        if (line.find(limit_constraint) != string::npos) {
          constraint =  static_cast<uint32_t>(std::stoll(string(line, (limit_constraint).length())));
        }
        std::getline(stream, line);
        if (line.find(limit_value) != string::npos) {
          value =  static_cast<uint32_t>(std::stoll(string(line, (limit_value).length())));
        }
        if (value) {
          constraints_vector.push_back(std::make_pair(constraint, value));
        }
      }
      crtc_info_.line_width_limits = std::move(constraints_vector);
    } else if (line.find(use_baselayer_for_stage) != string::npos) {
      crtc_info_.use_baselayer_for_stage =
          std::stoll(string(line, use_baselayer_for_stage.length()));
    } else if (line.find(vbif_cmd_ff) != string::npos) {
      crtc_info_.vbif_cmd_ff = std::stof(string(line, vbif_cmd_ff.length()));
    }
  }
...
void DRMCrtc::ParseCompRatio(string line, bool real_time) {
  ...
  for (uint32_t i = 0; i < format_cr_list.size(); i++) {
    ...
    uint64_t vendor_code =  static_cast<uint64_t>(stoi(format_cr.at(1)));
    uint64_t fmt_modifier = static_cast<uint64_t>(stoi(format_cr.at(2)));
...
void DRMCrtc::Unlock() {
  if (mode_blob_id_) {
    drmModeDestroyPropertyBlob(fd_, static_cast<uint32_t>(mode_blob_id_));
    mode_blob_id_ = 0;
  }
...
void DRMCrtc::Perform(DRMOps code, drmModeAtomicReq *req, va_list args) {
  ...
  switch (code) {
    ...
    case DRMOps::CRTC_SET_OUTPUT_FENCE_OFFSET: {
      ...
      AddProperty(req, obj_id, prop_mgr_.GetPropertyId(DRMProperty::OUTPUT_FENCE_OFFSET), offset,
                  true /* cache */, &tmp_prop_val_map_);
    }; break;

    case DRMOps::CRTC_SET_CORE_CLK: {
      uint32_t core_clk = va_arg(args, uint32_t);
      AddProperty(req, obj_id, prop_mgr_.GetPropertyId(DRMProperty::CORE_CLK), core_clk,
                  true /* cache */, &tmp_prop_val_map_);
    }; break;

    case DRMOps::CRTC_SET_CORE_AB: {
      uint64_t core_ab = va_arg(args, uint64_t);
      AddProperty(req, obj_id, prop_mgr_.GetPropertyId(DRMProperty::CORE_AB), core_ab,
                  true /* cache */, &tmp_prop_val_map_);
    }; break;

    case DRMOps::CRTC_SET_CORE_IB: {
      uint64_t core_ib = va_arg(args, uint64_t);
      AddProperty(req, obj_id, prop_mgr_.GetPropertyId(DRMProperty::CORE_IB), core_ib,
                  true /* cache */, &tmp_prop_val_map_);
    }; break;

    case DRMOps::CRTC_SET_LLCC_AB: {
      uint64_t llcc_ab = va_arg(args, uint64_t);
      AddProperty(req, obj_id, prop_mgr_.GetPropertyId(DRMProperty::LLCC_AB), llcc_ab,
                  true /* cache */, &tmp_prop_val_map_);
    }; break;

    case DRMOps::CRTC_SET_LLCC_IB: {
      uint64_t llcc_ib = va_arg(args, uint64_t);
      AddProperty(req, obj_id, prop_mgr_.GetPropertyId(DRMProperty::LLCC_IB), llcc_ib,
                  true /* cache */, &tmp_prop_val_map_);
    }; break;

    case DRMOps::CRTC_SET_DRAM_AB: {
      uint64_t dram_ab = va_arg(args, uint64_t);
      AddProperty(req, obj_id, prop_mgr_.GetPropertyId(DRMProperty::DRAM_AB), dram_ab,
                  true /* cache */, &tmp_prop_val_map_);
    }; break;

    case DRMOps::CRTC_SET_DRAM_IB: {
      uint64_t dram_ib = va_arg(args, uint64_t);
      AddProperty(req, obj_id, prop_mgr_.GetPropertyId(DRMProperty::DRAM_IB), dram_ib,
                  true /* cache */, &tmp_prop_val_map_);
    }; break;

    case DRMOps::CRTC_SET_ROT_PREFILL_BW: {
      uint64_t rot_bw = va_arg(args, uint64_t);
      drmModeAtomicAddProperty(req, obj_id, prop_mgr_.GetPropertyId(DRMProperty::ROT_PREFILL_BW),
                               rot_bw);
    }; break;

    case DRMOps::CRTC_SET_ROT_CLK: {
      uint32_t rot_clk = va_arg(args, uint32_t);
      AddProperty(req, obj_id, prop_mgr_.GetPropertyId(DRMProperty::ROT_CLK), rot_clk,
                  true /* cache */, &tmp_prop_val_map_);
    }; break;
    ...
    case DRMOps::CRTC_SET_SECURITY_LEVEL: {
      ...
      if (security_level == static_cast<int>(DRMSecurityLevel::SECURE_ONLY)) {
    ...
    case DRMOps::CRTC_SET_IDLE_TIMEOUT: {
      ...
      AddProperty(req, obj_id, prop_mgr_.GetPropertyId(DRMProperty::IDLE_TIME), timeout_ms,
                  true /* cache */, &tmp_prop_val_map_);
    } break;
    ...
    case DRMOps::CRTC_SET_DEST_SCALER_CONFIG: {
      ...
      sde_drm_dest_scaler_data *ds_data = reinterpret_cast<sde_drm_dest_scaler_data *>(dest_scaler);
      dest_scale_copy = *ds_data;
      AddProperty(req, obj_id, prop_id, reinterpret_cast<uint64_t>(&dest_scale_copy),
    ...
    case DRMOps::CRTC_SET_CAPTURE_MODE: {
      ...
      if (capture_mode == static_cast<int>(DRMCWbCaptureMode::DSPP_OUT)) {
...
void DRMCrtc::SetROI(drmModeAtomicReq *req, uint32_t obj_id, uint32_t num_roi, DRMRect *crtc_rois) {
  ...
  if (!num_roi || num_roi > SDE_MAX_ROI_V1 || !prop_mgr_.IsPropertyAvailable(DRMProperty::ROI_V1)) {
    return;
  }
  ...
  for (uint32_t i = 0; i < num_roi; i++) {
    roi_v1.roi[i].x1 = static_cast<unsigned short>(crtc_rois[i].left);
    roi_v1.roi[i].x2 = static_cast<unsigned short>(crtc_rois[i].right);
    roi_v1.roi[i].y1 = static_cast<unsigned short>(crtc_rois[i].top);
    roi_v1.roi[i].y2 = static_cast<unsigned short>(crtc_rois[i].bottom);
    DRM_LOGD("CRTC %d, ROI[l,t,b,r][%d %d %d %d]", obj_id, roi_v1.roi[i].x1, roi_v1.roi[i].y1,
             roi_v1.roi[i].x2, roi_v1.roi[i].y2);
  }
...
void DRMCrtc::SetSolidfillStages(drmModeAtomicReq *req, uint32_t obj_id,
  ...
  drm_dim_layer_v1.num_layers = static_cast<unsigned int>(solid_fills->size());
  ...
  AddProperty(req, obj_id, prop_mgr_.GetPropertyId(DRMProperty::DIM_STAGES_V1),
              reinterpret_cast<uint64_t>(&drm_dim_layer_v1), false /* cache */, &tmp_prop_val_map_);
#endif
}
...
bool DRMCrtc::ConfigureScalerLUT(drmModeAtomicReq *req, uint32_t dir_lut_blob_id,
  ...
  if (dir_lut_blob_id) {
    AddProperty(req, drm_crtc_->crtc_id, prop_mgr_.GetPropertyId(DRMProperty::DS_LUT_ED),
                dir_lut_blob_id, false /* cache */, &tmp_prop_val_map_);
  }
  if (cir_lut_blob_id) {
    AddProperty(req, drm_crtc_->crtc_id, prop_mgr_.GetPropertyId(DRMProperty::DS_LUT_CIR),
                cir_lut_blob_id, false /* cache */, &tmp_prop_val_map_);
  }
  if (sep_lut_blob_id) {
    AddProperty(req, drm_crtc_->crtc_id, prop_mgr_.GetPropertyId(DRMProperty::DS_LUT_SEP),
