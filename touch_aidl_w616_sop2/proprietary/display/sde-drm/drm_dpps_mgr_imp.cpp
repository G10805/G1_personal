...
DRMDppsManagerIntf *GetDppsManagerIntf() {
  ...
  return new DRMDppsManagerImp;
#else
  return new DRMDppsManagerDummyImp;
#endif
}
...
int DRMDppsManagerImp::GetDrmResources(drmModeRes *res) {
  ...
  for (auto i = 0; i < res->count_connectors; i++) {
    ...
    if (conn && conn->connector_type == DRM_MODE_CONNECTOR_DisplayPort)
      return 0;
  }
  for (auto i = 0; i < res->count_connectors; i++) {
    conn = drmModeGetConnector(drm_fd_, res->connectors[i]);
    if (conn && conn->connector_type == DRM_MODE_CONNECTOR_DSI && conn->count_modes &&
      ...
      conn_id_ = static_cast<int>(conn->connector_id);
      break;
    }
  ...
  for (auto i = 0; i < conn->count_encoders; i++) {
    ...
    if (enc && enc->encoder_type == DRM_MODE_ENCODER_DSI) {
      ...
      enc_id = static_cast<int>(enc->encoder_id);
      break;
    }
  ...
  for (auto i = 0; i < res->count_crtcs; i++) {
    if (enc->possible_crtcs & (1 << i)) {
      ...
      if (crtc) {
        ...
        crtc_id_ = static_cast<int>(crtc->crtc_id);
        break;
      }
...
int DRMDppsManagerImp::InitCrtcProps() {
  ...
  drmModeObjectProperties *props =
      drmModeObjectGetProperties(drm_fd_, static_cast<uint32_t>(crtc_id_), DRM_MODE_OBJECT_CRTC);
...
int DRMDppsManagerImp::InitConnProps() {
  ...
  drmModeObjectProperties *props =
      drmModeObjectGetProperties(drm_fd_,  static_cast<uint32_t>(conn_id_), DRM_MODE_OBJECT_CONNECTOR);
  if (!props || !props->props || !props->prop_values) {
    drmModeFreeObjectProperties(props);
    return -EINVAL;
  }
...
void DRMDppsManagerImp::Init(int fd, drmModeRes *res) {
  ...
  dpps_feature_[kFeatureAd4Mode] = DRMDppsPropInfo{
      ...
      false /* is_event */};
  dpps_feature_[kFeatureAd4Init] = DRMDppsPropInfo{
      4 /* version */, DRMProperty::SDE_DSPP_AD4_INIT,
      prop_mgr_.GetPropertyId(DRMProperty::SDE_DSPP_AD4_INIT), false /* is_event */};
  dpps_feature_[kFeatureAd4Cfg] =
      DRMDppsPropInfo{4 /* version */, DRMProperty::SDE_DSPP_AD4_CFG,
                      prop_mgr_.GetPropertyId(DRMProperty::SDE_DSPP_AD4_CFG), false /* is_event */};
  dpps_feature_[kFeatureAd4Input] = DRMDppsPropInfo{
      4 /* version */, DRMProperty::SDE_DSPP_AD4_INPUT,
      prop_mgr_.GetPropertyId(DRMProperty::SDE_DSPP_AD4_INPUT), false /* is_event */};
  dpps_feature_[kFeatureAd4Backlight] = DRMDppsPropInfo{
      4 /* version */, DRMProperty::SDE_DSPP_AD4_BACKLIGHT,
      prop_mgr_.GetPropertyId(DRMProperty::SDE_DSPP_AD4_BACKLIGHT), false /* is_event */};
  dpps_feature_[kFeatureAd4Assertiveness] = DRMDppsPropInfo{
      4 /* version */, DRMProperty::SDE_DSPP_AD4_ASSERTIVENESS,
      prop_mgr_.GetPropertyId(DRMProperty::SDE_DSPP_AD4_ASSERTIVENESS), false /* is_event */};
  dpps_feature_[kFeatureAd4Roi] =
      DRMDppsPropInfo{4 /* version */, DRMProperty::SDE_DSPP_AD4_ROI,
                      prop_mgr_.GetPropertyId(DRMProperty::SDE_DSPP_AD4_ROI), false /* is_event */};
  dpps_feature_[kFeatureAd4ManualStrength] = DRMDppsPropInfo{
      4 /* version */, DRMProperty::SDE_DSPP_AD4_STRENGTH,
      prop_mgr_.GetPropertyId(DRMProperty::SDE_DSPP_AD4_STRENGTH), false /* is_event */};
  dpps_feature_[kFeatureAbaHistCtrl] = DRMDppsPropInfo{
      1 /* version */, DRMProperty::SDE_DSPP_ABA_HIST_CTRL,
      prop_mgr_.GetPropertyId(DRMProperty::SDE_DSPP_ABA_HIST_CTRL), false /* is_event */};
  dpps_feature_[kFeatureAbaHistIRQ] = DRMDppsPropInfo{
      1 /* version */, DRMProperty::SDE_DSPP_ABA_HIST_IRQ,
      prop_mgr_.GetPropertyId(DRMProperty::SDE_DSPP_ABA_HIST_IRQ), false /* is_event */};
  dpps_feature_[kFeatureAbaLut] =
      DRMDppsPropInfo{1 /* version */, DRMProperty::SDE_DSPP_ABA_LUT,
                      prop_mgr_.GetPropertyId(DRMProperty::SDE_DSPP_ABA_LUT), false /* is_event */};
  dpps_feature_[kFeatureAd4BlScale] = DRMDppsPropInfo{
      1 /* version */, DRMProperty::SDE_DSPP_AD4_BL_SCALE,
      prop_mgr_.GetPropertyId(DRMProperty::SDE_DSPP_AD4_BL_SCALE), false /* is_event */};
  dpps_feature_[kFeatureBacklightScale] = DRMDppsPropInfo{
      1 /* version */, DRMProperty::SDE_DSPP_BL_SCALE,
      prop_mgr_.GetPropertyId(DRMProperty::SDE_DSPP_BL_SCALE), false /* is_event */};

  dpps_feature_[kFeaturePowerEvent] =
      DRMDppsPropInfo{1, DRMProperty::INVALID, 0, true /* is_event */};
  dpps_feature_[kFeatureAbaHistEvent] =
      DRMDppsPropInfo{1, DRMProperty::INVALID, 0, true /* is_event */};
  dpps_feature_[kFeatureBackLightEvent] =
      DRMDppsPropInfo{1, DRMProperty::INVALID, 0, true /* is_event */};
  dpps_feature_[kFeatureAdAttBlEvent] =
      DRMDppsPropInfo{1, DRMProperty::INVALID, 0, true /* is_event */};
}
