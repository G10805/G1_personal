...
void DRMPPManager::Init(const DRMPropertyManager &pm, uint32_t object_type) {
  ...
  for (uint32_t i = (uint32_t)DRMProperty::INVALID + 1; i < (uint32_t)DRMProperty::MAX; i++) {
    ...
    if (i >= (uint32_t)DRMProperty::SDE_DSPP_GAMUT_V3 &&
        i <= (uint32_t)DRMProperty::SDE_DSPP_GAMUT_V5) {
    ...
    } else if (i >= (uint32_t)DRMProperty::SDE_DSPP_GC_V1 &&
               i <= (uint32_t)DRMProperty::SDE_DSPP_GC_V2) {
      ...
      pp_prop_map_[kFeaturePASixZone].version =
          i - (uint32_t)DRMProperty::SDE_DSPP_PA_SIXZONE_V1 + 1;
      ...
      pp_prop_map_[kFeaturePAMemColSkin].version =
          i - (uint32_t)DRMProperty::SDE_DSPP_PA_MEMCOL_SKIN_V1 + 1;
      ...
      pp_prop_map_[kFeaturePAMemColSky].version =
          i - (uint32_t)DRMProperty::SDE_DSPP_PA_MEMCOL_SKY_V1 + 1;
      ...
      pp_prop_map_[kFeaturePAMemColFoliage].version =
          i - (uint32_t)DRMProperty::SDE_DSPP_PA_MEMCOL_FOLIAGE_V1 + 1;
      DRM_LOGI("MemColor foliage version %d, prop_id %d",
               pp_prop_map_[kFeaturePAMemColFoliage].version,
      ...
      pp_prop_map_[kFeaturePAMemColProt].version =
          i - (uint32_t)DRMProperty::SDE_DSPP_PA_MEMCOL_PROT_V1 + 1;
      ...
      pp_prop_map_[kFeatureVigGamut].version =
          i - (uint32_t)DRMProperty::SDE_VIG_3D_LUT_GAMUT_V5 + 5;
...
void DRMPPManager::SetPPFeature(drmModeAtomicReq *req, uint32_t obj_id, const DRMPPFeatureInfo
                                &feature) {
  if (!req) {
...
int DRMPPManager::SetPPBlobProperty(drmModeAtomicReq *req, uint32_t obj_id,
                                    struct DRMPPPropInfo *prop_info, const DRMPPFeatureInfo
                                    &feature) {
