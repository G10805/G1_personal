...
#ifndef __ABA_CABL_H__
#define __ABA_CABL_H__
...
typedef uint32_t bool32; /* Boolean value type. */
...
typedef enum { USER_QL_MODE, AUTO_QL_MODE } DEFAULT_QL_MODE;
...
typedef struct {
  ...
  uint32_t uLimitedBacklightPowerSaveRatio;   // Filtered backlight ration limited by
                                              // uBacklightScaleRatioUpperLimit
  uint32_t uBacklightPowerSaveRatioMin;  // Minimum ration to apply to the input backlight without
...
typedef struct {
  ...
  uint32_t uBacklightStepSizeHighCorrelation;  // Backlight change delta allowed when b2b frames are
                                               // similar
  uint32_t
      uSceneCorrelationThreshold;  // Correlation value beyond which frames are considered similar
  uint32_t uSceneChangeThreshold;  // Correlation value below which the scene is considered to have
                                   // changed
} CablQualityParametersType;
