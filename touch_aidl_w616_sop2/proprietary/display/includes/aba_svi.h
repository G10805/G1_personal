...
#ifndef __ABA_SVI_H__
#define __ABA_SVI_H__
...
#define ABA_SVI_VERSION \
  (ABA_SVI_VERSION_MAJOR << 24 | ABA_SVI_VERSION_MINOR << 16 | ABA_SVI_VERSION_REVISION)
...
typedef struct {
  uint32_t
      uContrastStrengthFactor;     // 0 - 255, histogram segmentation (content based contrast gain)
  ...
  uint32_t
      *pSensorMappingTableReference;    // Lux Mapping table for sensor calibration - reference axis
  ...
  uint32_t uBacklightReductionRatio;  // Value 0-1024 indicating the reduction ratio relative to the
                                      // original backlight
...
Use for dynamic configuration of SVI parameters. Most parameters are extracted from the static
config structure.
*/
typedef struct {
