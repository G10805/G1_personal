 ...
 **************************************************************************************************
*/
#ifndef _QC2_BUFFER_H_
#define _QC2_BUFFER_H_
...
class QC2Buffer : public std::enable_shared_from_this<QC2Buffer> {
 public:
    ...
    /// unique identifier for the backing buffer
    uint64_t uid() const {
        return mId;
    }
 ...
 private:
    ...
    explicit QC2Buffer(std::shared_ptr<C2GraphicBlock> graphicBlock, uint64_t usage);
...
class QC2Buffer::Graphic {
 ...
 private:
    ...
    Graphic(QC2Buffer *buf, std::shared_ptr<C2GraphicBlock> gb, uint64_t usage = 0);
...
class QC2MemTracker {
 public:
    static std::unique_ptr<QC2MemTracker> Create(const char* name);

    virtual ~QC2MemTracker() = default;

    virtual QC2Status track(uint64_t bufId, size_t sizeInBytes) = 0;

    virtual QC2Status unTrack(uint64_t bufId) = 0;

    virtual void clear() = 0;

    virtual const char * name() const = 0;

    virtual QC2Status usage(size_t *numBuffers, uint64_t *totalSizeBytes,
            size_t *peakNumBuffers, uint64_t *peakSizeBytes) const = 0;
};
...
class QC2LinearBufferPool {
 public:
    ...
    QC2Status setMemTracker(std::shared_ptr<QC2MemTracker> tracker) {
        mMemTracker = tracker;
        return QC2_OK;
    }
 ...
 private:
    ...
    std::shared_ptr<QC2MemTracker> mMemTracker = nullptr;
    uint64_t mUsage;
    uint32_t mBufSize;
    uint32_t mAlignment;
};
...
class QC2GraphicBufferPool {
 public:
    ...
    /// set memory tracker
    virtual QC2Status setMemTracker(std::shared_ptr<QC2MemTracker> tracker) = 0;
    /// set resolution
    ...
    /// allocate a buffer
    virtual QC2Status allocate(std::shared_ptr<QC2Buffer> *buf, bool isC2D = false) = 0;
...
class QC2GraphicBufferPoolImpl : public QC2GraphicBufferPool {
 public:
    ...
    QC2GraphicBufferPoolImpl(std::shared_ptr<C2BlockPool> pool, bool isSecure,
            std::shared_ptr<RefTracker> refTracker = nullptr);
    ...
    /// set memory tracker
    virtual QC2Status setMemTracker(std::shared_ptr<QC2MemTracker> tracker) {
        mMemTracker = tracker;
        return QC2_OK;
    }
    /// set resolution
    ...
    QC2Status setUsage(uint64_t usage);
    /// allocate a buffer
    virtual QC2Status allocate(std::shared_ptr<QC2Buffer> *buf, bool isC2D = false);
    /// keep reference to a previously allocated buffer (specifically, to its Block).
    /// This will avoid re-allocating a handle that is already being referenced by the allocatee
 ...
 private:
    ...
    std::shared_ptr<C2BlockPool> mPool;
    std::shared_ptr<QC2MemTracker> mMemTracker = nullptr;
    std::shared_ptr<RefTracker> mRefTracker;
