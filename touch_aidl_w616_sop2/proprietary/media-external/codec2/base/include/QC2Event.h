 ...
 **************************************************************************************************
*/
#ifndef _QC2_UTILS_EVENT_H_
#define _QC2_UTILS_EVENT_H_
...
class Event {
 public:
    ...
    enum class Flags : uint32_t {
        DEDUP = 0x1,    ///< keep a single instance of event by removing duplicates
    };
    ...
    /**
     * @brief create an event with special flags
     */
    explicit Event(uint32_t id, Flags f) : mId(id), mPriority(Priority::NORMAL), mRetired(false),
            mFlags(static_cast<uint32_t>(f)) {
    }
 ...
 private:
    ...
    uint32_t mFlags;            ///< special flags (immutable, once set)
