//*************************************************************************************************
// Copyright (c) 2009-2021, 2022 Qualcomm Technologies, Inc.
// All Rights Reserved.
...
typedef enum {
    ...
    C2D_FORMAT_BT601_LIMITEDRANGE    = (1 << 24), /* YUV format is limited range following BT 601 */
    C2D_FORMAT_BT709_FULLRANGE       = (1 << 25), /* YUV format is full range following BT 709 */
    C2D_FORMAT_BT709_LIMITEDRANGE    = (1 << 26), /* YUV format is limited range following BT 709 */
} C2D_FORMAT_MODE;
...
typedef enum {
    ...
    C2D_SURFACE_WITH_PHYS_DUMMY = (1<<4), /* physical address allready mapped */
                                        /* this bit is valid with HOST types */
    C2D_SURFACE_PROTECTED       = (1<<5), /* Protected memory surface */
} C2D_SURFACE_TYPE;
...
/* Configuration bits, driver capabilities used by 2Dapplications */
typedef enum {
    C2D_DRIVER_CONFIG_SECURE_CONTEXT              = (1 << 0), /* enable secure context blit */
} C2D_DRIVER_CONFIG;
...
/* allows user to map a protected memory region to the gpu. only supported on linux
 * mem_fd is the fd of the memory region, hostptr is the host pointer to the region,
 * len and offset are the size and offset of the memory.
 * flags is one of the memory types supported by gsl
 * gpuaddr is passed by refernce back to the user
 */
C2D_API C2D_STATUS c2dMapAddrProtected ( int mem_fd, void * hostptr, uint32 len, uint32 offset, uint32 flags, void ** gpuaddr);
