...
OMX_ERRORTYPE XmlParser::ResolveMasterXmlPath(OMX_STRING inputstr, OMX_STRING pMasterXmlLocation)
{
    ...
    if (stat(inputstr, &s) == 0) {
        if (s.st_mode & S_IFDIR) {
            ...
            m_pMasterXmlFullPath = (OMX_STRING)malloc(strlen(inputstr) + strlen("MasterConfig.xml") + 5);
            if (m_pMasterXmlFullPath) {
                SNPRINTF(m_pMasterXmlFullPath, strlen(inputstr) + strlen("MasterConfig.xml") + 5,
                        "%s/MasterConfig.xml", inputstr);
            }
        }
        else if (s.st_mode & S_IFREG) {
            char *cp = NULL;
            VTEST_MSG_LOW("%s", inputstr);
            STRLCPY(sTemp, inputstr, MAX_STR_LEN);
...
OMX_ERRORTYPE XmlParser::ProcessMasterConfig(VideoStaticProperties* pGlobalVideoProp)
{
    ...
    if (!pMasterDoc) {
        VTEST_MSG_ERROR("Unable to open %s or Invalid MasterConfig.xml", m_pMasterXmlFullPath);
        return status;
    }
