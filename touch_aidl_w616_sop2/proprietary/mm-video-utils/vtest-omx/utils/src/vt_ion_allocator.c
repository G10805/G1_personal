...
int vt_ion_allocate(int secure,
    ...
    *client_ion_buffer = NULL;
    ...
    ion_buffer->dev_fd = -1;
#ifdef USEDMABUFHEAP
    BufferAllocator *m_bufferAllocator = CreateDmabufHeapBufferAllocator();
    if (m_bufferAllocator == NULL) {
        VTEST_MSG_ERROR("failed to create dmabufheap buffer allocator");
        return -ENOMEM;
    }
    if (flags & ION_FLAG_SECURE) {
       ion_buffer->data_fd =
           DmabufHeapAlloc(m_bufferAllocator, "qcom,secure-pixel", size, 0, 0);
    } else {
       ion_buffer->data_fd =
           DmabufHeapAlloc(m_bufferAllocator, "qcom,system-uncached", size, 0, 0);
    }
    if (ion_buffer->data_fd < 0) {
        VTEST_MSG_ERROR("failed to allocate buffer from DmabufHeap");
        return -ENOMEM;
    }

    if (m_bufferAllocator != NULL) {
        FreeDmabufHeapBufferAllocator(m_bufferAllocator);
