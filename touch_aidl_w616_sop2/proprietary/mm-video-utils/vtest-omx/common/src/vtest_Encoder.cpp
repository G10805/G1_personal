...
OMX_ERRORTYPE Encoder::DeliverInput(OMX_BUFFERHEADERTYPE *pBuffer) {
    ...
    while (m_sConfig.pDynamicProperties != NULL &&
        ...
        ProcessDynamicConfiguration(m_sConfig.pDynamicProperties, m_nDynamicIndexCount, pBuffer->nTimeStamp);
...
OMX_ERRORTYPE Encoder::ProcessDynamicConfiguration(
        DynamicConfig *dynamic_config, int index, OMX_TICKS nTimeStamp) {
    ...
    result = SetDynamicConfig((OMX_INDEXTYPE)dynamic_config[index].config_param, &dynamic_config[index].config_data, nTimeStamp);
    if (result == OMX_ErrorNone) {
        VTEST_MSG_MEDIUM(
...
/////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
OMX_ERRORTYPE Encoder::SetDynamicConfig(OMX_INDEXTYPE config_param, DynamicConfigData* config_data, OMX_TICKS nTimeStamp) {
    ...
    switch(config_param_qcom_ext) {
        ...
        case OMX_QTIIndexConfigVideoRoiRectRegionInfo:
            OMX_QTI_VIDEO_CONFIG_ROI_RECT_REGION_INFO roi_info;

            OMX_INIT_STRUCT(&roi_info, OMX_QTI_VIDEO_CONFIG_ROI_RECT_REGION_INFO);

            VTEST_MSG_ERROR("OMX_QTIIndexConfigVideoRoiRectRegionInfo calling ");
            roi_info.nPortIndex = (OMX_U32)config_data->roi_rect.nPortIndex;
            roi_info.nTimeStamp = nTimeStamp;
            roi_info.nRegionNum = config_data->roi_rect.nRegionNum;
            if (roi_info.nRegionNum > MAX_RECT_ROI_NUM)
                return OMX_ErrorBadParameter;

            memset(&roi_info.nRegions, 0, sizeof(roi_info.nRegions));
            memcpy(roi_info.nRegions, &config_data->roi_rect.nRegions,
            sizeof(QOMX_VIDEO_RECT_REGION_DELTAQP) * MAX_RECT_ROI_NUM);

            result = OMX_SetConfig(m_hEncoder,
                                   (OMX_INDEXTYPE) config_param_qcom_ext, (OMX_PTR) &roi_info);
            if (result != OMX_ErrorNone) {
                VTEST_MSG_ERROR("Failed: Setting ROI failed with error: %d", result);
            }
            break;
...
OMX_ERRORTYPE Encoder::ProcessHDR10plusMetadata(OMX_BUFFERHEADERTYPE *pBuffer){
    ...
    if(mMetadataFile != NULL && m_bMetaMode)
    {
...
OMX_ERRORTYPE Encoder::ProcessCvpMeta(OMX_BUFFERHEADERTYPE *pBuffer) {
    ...
    if (mCVPMetaFile != NULL && m_bMetaMode)
    {
...
OMX_ERRORTYPE Encoder::SetColorInfo(CodecConfigType *pConfig, OMX_BUFFERHEADERTYPE *pBuffer) {
    ...
    ColorMetaData colorData;
    if (!m_bMetaMode) {
      VTEST_MSG_HIGH("Don't support setting color info for non-meta-mode");
      return result;
    }
    MetaBuffer *pMetaBuffer = (MetaBuffer *)pBuffer->pBuffer;
    private_handle_t *pMetaHandle = (private_handle_t *)pMetaBuffer->meta_handle;
