...
FASTCV += fastcv_hetero_compute_test
...
#CV COMMON
CV_COMMON := libcv_common
CV_COMMON += libcv_common.so
...
#CVPCPU_REV
CVPCPU_REV += libcvpcpuRev_skel
CVPCPU_REV += libcvpcpuRev_skel.so
...
CVP_INTERNAL += eva3Test

EVA_INTERNAL := libevadspTest_skel
EVA_INTERNAL += libevadspTest_skel.so
EVA_INTERNAL += libJsonCpp
EVA_INTERNAL += libJsonCpp.so
EVA_INTERNAL += libevadspTestRev_skel
EVA_INTERNAL += libevadspTestRev_skel.so
...
#OPENVX_TEST_BINARIES
OPENVX_INTERNAL := vx_test_conformance
...
ifeq ($(ENABLE_EVA), true)
ifeq ($(call is-board-platform-in-list,lahaina kona msmnile lito),true)
PRODUCT_PACKAGES += $(CVP_COMMON)
else
PRODUCT_PACKAGES += $(CV_COMMON)
endif

# REV SKEL Compilation for fillmore SPF of taro
ifeq ($(call is-board-platform-in-list,taro),true)
   PRODUCT_PACKAGES += $(CVPCPU_REV)
endif
ifeq ($(call is-board-platform-in-list,taro),true)
   PRODUCT_PACKAGES += $(EVA)
   PRODUCT_PACKAGES += $(EVA_INTERNAL)
   PRODUCT_PACKAGES += $(CVP2_1)
endif
ifeq ($(call is-board-platform-in-list,cape),true)
   PRODUCT_PACKAGES += $(CVPCPU_REV)
endif
ifeq ($(call is-board-platform-in-list,cape),true)
   PRODUCT_PACKAGES += $(EVA)
...
PRODUCT_PACKAGES += $(OPENVX_INTERNAL)
endif
