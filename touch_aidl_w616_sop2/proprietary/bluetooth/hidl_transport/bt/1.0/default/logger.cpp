...
void Logger::GetUartLogFilename(char* path) {
  snprintf(path, UART_LOG_PATH_BUF_SIZE, UART_LOGS_DEST_PATH, time_year, time_month, time_day, time_hour, time_min, time_sec);
  dump_files.push_back(std::string(path));
}
...
int Logger :: ReadSsrLevel() {
  char value[PROPERTY_VALUE_MAX] = {'\0'};
  /*
   * Set default value for SubSystem Restart level
   * 1/2: Crash whole device(kernel panic) after collecting crash dump
   * 3  : Restart BT host after collecting crash dump
   */
