...
int PatchDLManager::SocInit(uint8_t *bdaddr, bool is_emb_wp_mode)
{
  ...
  if (data_handler) {
    soc_type_ = data_handler->GetSocTypeInt();
  }
...
int PatchDLManager::ReadNewHciEventWithTimer(unsigned char* buf, int size)
{
  ...
  /* timer value */
  timeout.tv_sec = 1;  /* in second */
  timeout.tv_usec = 0; /* in usecond */

  do {
    retval = select(fd_transport_ + 1, &set, NULL, NULL, &timeout);
    if(retval <= 0) {
      ALOGE("%s timer is expired(1)", __func__);
      return -1;
    }
  ...
  retval = select(fd_transport_ + 1, &set, NULL, NULL, &timeout);
  if(retval <= 0) {
    ALOGE("%s timer is expired(2)", __func__);
    return -1;
  }
  ...
  retval = select(fd_transport_ + 1, &set, NULL, NULL, &timeout);
  if(retval <= 0) {
    ALOGE("%s timer is expired(3)",__FUNCTION__);
    return -1;
  }
...
void PatchDLManager::SetBtProperty()
{
  switch (chipset_ver_) {
    case ROME_VER_3_2:
      SetBtSocProperty("rome");
      SetTokenKeyProperty(false);
      break;
    case HASTINGS_VER_2_0:
      SetBtSocProperty("hastings");
      SetTokenKeyProperty(true);
      break;
    case GENOA_VER_1_0:
    case GENOA_VER_2_0:
    case GENOA_VER_2_0_0_2:
      SetBtSocProperty("genoa");
      SetTokenKeyProperty(true);
      break;
    case HSP_VER_1_0:
    case HSP_VER_1_1:
...
void PatchDLManager::SetBtSocProperty(const char* soc)
{
  ALOGI("%s: %s", __func__, soc);
  property_set("persist.vendor.qcom.bluetooth.soc", soc);
}

void PatchDLManager::SetTokenKeyProperty(bool supported)
{
  ALOGI("%s: %s", __func__, supported ? "supported" : "not supported");
  property_set("vendor.bt.a2dp.token_key", supported ? "true" : "false");
}
...
void PatchDLManager::LoadPatchMaptable() {
  ...
  PatchPathInfoMap_.insert(std::make_pair<uint64_t, PatchPathManager*>(ROME_VER_3_4,
                          new PatchPathManager("ROME3_4", "btfw34.tlv", "btnv34.bin")));
