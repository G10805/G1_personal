...
const char *convertMSMToStr(soc_info_v0_1 soc_info) {
  switch (soc_info.msm_cpu){
    case MSM_CPU_KALAMA:
      return "MSM_CPU_KALAMA";
    case MSM_CPU_PINEAPPLE:
      return "MSM_CPU_PINEAPPLE";
    default:
                             ...
                             const uint8_t *data, size_t length)
{
  ...
  if (is_xpan_supported) {
      ...
      bool status = QHci::Get()->IsQhciTxPkt(data, length);
      if (status) {
        QHci::Get()->ProcessTxPktCmd(data, length);
      } else {
...
void DataHandler::InternalOnPacketReady(ProtocolType ptype, HciPacketType type,
  ...
  if (cb_data != nullptr && controller_ != nullptr) {
    if (!cb_data->is_pending_init_cb ) {
      if (((UartController *)controller_)->IsRxthreadTimeoutEnabled() &&
                                ...
                                const hidl_vec<uint8_t>*hidl_data, bool from_soc)
{
...
#ifdef XPAN_SUPPORTED
  if (is_xpan_supported) {
    if ((ptype == TYPE_BT) && (type == HCI_PACKET_TYPE_EVENT) && from_soc) {
      if(data_handler->GetQHciState()) {
  ...
  if (from_soc) {
    ...
    if (cmd_opcode != 0x00 && ptype == TYPE_BT && len >= MIN_OPCODE_LEN) {
      if (len >= BTTPI_EVENT_LEN &&
        isBtTpiEvent(type, data)) {
        if (bttpi_event_cb) {
                       ...
                       DataReadCallback data_read_cb)
{
      ...
      if (!data_handler->CheckSignalCaughtStatus()) {
        ...
        if (strcmp(value, "1") == 0) {
          ALOGE("%s: Do kernel panic immediately as property \"trigger_crash\" set to %s",
                 __func__, value);
          if (controller_ && ((UartController *)controller_)->bt_kernel_panic() == 0) {
            return;
          } else {
  ...
  property_get("persist.vendor.service.bt.xpan_supported", value_prop,
                "false");
...
msm_cpu_id DataHandler :: get_soc_info_internally() {
  FILE *fp;
  msm_cpu_id msm_id = MSM_CPU_UNKNOWN_INT;

  fp = fopen(MSM_ID_SYSFS_NODE, "r");
  if (fp == nullptr) {
    ALOGE("%s: failed to open :%s", __func__, MSM_ID_SYSFS_NODE);
    return MSM_CPU_UNKNOWN_INT;
  }
...
void DataHandler :: SetHostAddOnFeatures(uint64_t chip_ver) {
  soc_info_v0_1 soc_info = {MSM_CPU_UNKNOWN, MSM_CPU_UNKNOWN};
  msm_cpu_id msm_info = MSM_CPU_UNKNOWN_INT;
  ...
  if (soc_info.msm_cpu == MSM_CPU_UNKNOWN) {
    ALOGI("%s: Trying to get the cpu info from sysfs", __func__);
    msm_info = get_soc_info_internally();

    ALOGI("%s: msm_info is %d", __func__, msm_info);
  }
  ...
  if (soc_info.msm_cpu <= MSM_CPU_UNKNOWN &&  msm_info == MSM_CPU_UNKNOWN_INT) {
    ...
    if (soc_info.msm_cpu == MSM_CPU_KALAMA ||
        soc_info.msm_cpu == MSM_CPU_PINEAPPLE ||
        msm_info == MSM_CPU_ANORAK_INT) {
      if (chip_ver == HSP_VER_2_0 ||
          chip_ver == HSP_VER_2_0_G ||
          chip_ver == HSP_VER_2_1 ||
          chip_ver == HSP_VER_2_1_G ||
          chip_ver == HAMILTON_VER_1_0 ||
          chip_ver == HAMILTON_VER_1_0_1 ||
          chip_ver == HAMILTON_VER_2_0) {
