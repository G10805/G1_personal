...
#define HCI_CRASH_SOC_TIMEOUT_VALUE (2)
...
SocAddOnFeatures_t add_on_features = {0, 0, 0, {0}};
  ...
  hci_packetizer_([this](hidl_vec<uint8_t> *data) { OnPacketReady(data); })
{
  ...
  logger_->RegisterHWEventCallback([this]() {
     ...
     */
   if ((is_nmi_interrupt_triggered == true || is_warm_reset_triggered == true)  &&
...
int WriteSpecialBufferTask(HciUartTransport *uart_transport,
    unsigned char buf[], size_t len)
{
  ALOGW("%s", __func__);
  return uart_transport->UartWrite(buf, len);
}
...
void UartController::SendSpecialBuffer(PrimaryReasonCode reason)
{
  ...
  if (reason != BT_HOST_REASON_SSR_INVALID_BYTES_RCVD) {
    auto future = std::async(std::launch::async, WriteSpecialBufferTask,
                             uart_transport, buf, LEN_PACKET_TO_SOC);

    int crashSOCTimeout= property_get_int32("persist.vendor.bluetooth.crashsoc.timeout",
            HCI_CRASH_SOC_TIMEOUT_VALUE);
    if (future.wait_for(std::chrono::seconds(crashSOCTimeout)) ==
            std::future_status::timeout) {
        ALOGE("%s: failed to send special buffer, report SOC failure directly", __func__);
...
void UartController::InitRxthreadTimer()
{
  char value[PROPERTY_VALUE_MAX];
  property_get("persist.vendor.bt.rxthread.timeout", value, DEFAULT_HCI_TIMEOUT);
  if (strcmp(value, "false") == 0)
    is_Rxthread_timeout_enabled_ = false;
  else {
    is_Rxthread_timeout_enabled_ = true;
    rxthread_timeout_ms = atoi(value);
  }
  ALOGD("%s: HCI timer enable: %d, timeout value is: %d ms", __func__,
          is_Rxthread_timeout_enabled_, rxthread_timeout_ms);
}

bool UartController::IsRxthreadTimeoutEnabled()
{
  return is_Rxthread_timeout_enabled_;
}
...
void UartController::StartRxThreadTimer()
{
  ...
  if ((GetRxThreadTimerState() == TIMER_CREATED) ||
    ...
    rx_timer_state_machine_.timeout_ms = rxthread_timeout_ms;
