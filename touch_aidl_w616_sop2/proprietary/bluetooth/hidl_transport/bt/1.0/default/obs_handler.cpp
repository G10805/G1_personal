...
int ObsHandler::Obs_data_set_sighandler(void)
{
  struct sigaction Obs_sig_act;
  int number = 0;

  ALOGV("%s: Entry", __func__);
  memset(&Obs_sig_act, 0, sizeof(Obs_sig_act));
  Obs_sig_act.sa_sigaction = Obs_data_sighandler;
  sigemptyset(&Obs_sig_act.sa_mask);
  Obs_sig_act.sa_flags = (SA_SIGINFO | SA_RESTART);
  sigaction(OBS_HOSTWAKE_SIG, &Obs_sig_act, NULL);

  ALOGV("%s: Opening Driver", __func__);
  instance_->obs_fd_ = open("/dev/btpower", O_RDWR | O_NONBLOCK);
  if (instance_->obs_fd_ < 0) {
    ALOGE("Cannot open device file...\n");
    return -1;
  }

  ALOGI("%s: Registering application", __func__);
  /* register this task with kernel for signal */
  if (ioctl(instance_->obs_fd_, BT_CMD_OBS_SIGNAL_TASK,(int32_t*) &number)) {
    ALOGE("%s: Failed to ioctl OBS_CURRENT_TASK", __func__);
    close(instance_->obs_fd_);
    instance_->obs_fd_ = -1;
    return -1;
  }
