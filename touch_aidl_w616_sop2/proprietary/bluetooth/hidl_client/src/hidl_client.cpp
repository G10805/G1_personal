...
using IBluetoothHciCallbacks = ::aidl::android::hardware::bluetooth::BnBluetoothHciCallbacks;
#else
...
class BluetoothHciCallbacks : public IBluetoothHciCallbacks {
    ...
    ::ndk::ScopedAStatus initializationComplete(AidlStatus status) {
        if (status == AidlStatus::SUCCESS) {
            ...
            ALOGE("Error in AIDL initialization: %x", status);
        }
        sem_post(&s_cond);
    ...
    ::ndk::ScopedAStatus hciEventReceived(const std::vector<uint8_t>& event) override {
        ...
        if (safe_write(server_fd, &val, 1) == -1) {
            ...
            return ndk::ScopedAStatus::fromServiceSpecificError(static_cast<int32_t>(AidlStatus::UNKNOWN));
        }
...
bool hidl_client_initialize(int mode, int *tool_fd) {
    ...
    switch (mode) {
        case MODE_BT:
            ALOGI("%s: Initialize the BT HAL with Mode BT", __func__);
            // If android.hardware.bluetooth* is not found, Bluetooth can not continue.
#ifdef BT_AIDL_HAL
            ALOGI("%s: Starting AIDL", __func__);
            ABinderProcess_startThreadPool();
            btHci = IBluetoothHci::fromBinder(
                SpAIBinder(AServiceManager_waitForService(kBluetoothAidlHalServiceName)));
#else
            ALOGI("%s: Starting HIDL", __func__);
            btHci = IBluetoothHci::getService();
#endif
            if (btHci != nullptr) {
#ifdef BT_AIDL_HAL
                auto on_binder_death = [](void* /*cookie*/) { };
                bluetooth_hci_death_recipient =
                    AIBinder_DeathRecipient_new(on_binder_death);
                AIBinder_linkToDeath(btHci->asBinder().get(),
                ...
                ALOGE("Unable to initialize the BT HAL");
                return false;
            }
            {
#ifdef BT_AIDL_HAL
                std::shared_ptr<IBluetoothHciCallbacks> callbacks =
                    ::ndk::SharedRefBase::make<BluetoothHciCallbacks>();
#else
