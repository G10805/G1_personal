...
//Definitions being used for UEFI test mode
#define BOOTING_MODE_PROPERTY "persist.vendor.mdm_helper.booting_mode"
char booting_mode_property_string[PROPERTY_VALUE_MAX];
...
int mdm9k_sahara(struct mdm_device *dev){
	...
	property_get(BOOTING_MODE_PROPERTY, booting_mode_property_string, "");
	//bypass efs sync if uefi test mode
	if (strncmp(booting_mode_property_string, "uefi", 4)) {
		ALOGI("%s: booting to non UEFI mode.", dev->mdm_name);
		if (setup_efs_sync(dev) != RET_SUCCESS)
			goto error;
	} else {
		ALOGI("%s: booting to UEFI mode.", dev->mdm_name);
	}
