...
static char const * const ssr_root = "/sys/bus/msm_subsys/devices";
static char const * const rproc_root = "/sys/class/remoteproc";
static char const * const rproc_modem_name = "remoteproc-mss";
...
int get_soc_powerup_node(const char *esoc_dev_name, char *powerup_node, size_t size)
{
    struct dirent *de;
    DIR *dir_esoc = NULL;
    ...
    if (!powerup_node || size == 0)
        return -1;

    powerup_node[0] = '\0';

    /* Try opening rproc node */
    snprintf(soc_path, sizeof(soc_path), "%s/%s/remoteproc", ESOC_ROOT_DIR, esoc_dev_name);
    if (!(dir_esoc = opendir(soc_path))) {
        snprintf(powerup_node, size, "/dev/subsys_%s", esoc_dev_name);
        return 0;
    }
    ...
    while((de = readdir(dir_esoc))) {
        if(de->d_name[0] == '.')
            continue;
        /* The esoc remoteproc node is the only entry in this directory */
        snprintf(powerup_node, size, "/dev/%s", de->d_name);
        break;
    }
    closedir(dir_esoc);
...
int get_esoc_details(struct mdm_info *dev, char *esoc_dev_name)
{
    ...
    temp = NULL;
    ret = get_soc_powerup_node(esoc_dev_name, dev->powerup_node, sizeof(dev->powerup_node));
    if (ret) {
        ALOGE("Failed to get powerup path for modem");
        goto error;
    }
...
int get_system_info(struct dev_info *dev)
{
    ...
    char subsys_name[MAX_NAME_LEN];
    int rcode = 0;
    const char *subsys_root;
    struct mdm_info *device;
    ...
    /* Try opening ssr dir and then rproc */
    if ((dir_ssr_bus = opendir(ssr_root))) {
        subsys_root = ssr_root;
    } else if ((dir_ssr_bus = opendir(rproc_root))) {
        subsys_root = rproc_root;
    } else {
        ALOGE("Failed to open subsys root dir");
        goto error;
    }
    ALOGI("Using %s root directory\n", (subsys_root == rproc_root) ? "rproc" : "ssr");
    ...
    while((de = readdir(dir_ssr_bus))) {
        ...
        //remoteproc subsystem names are completely different from that
        //in PIL/SSR. This replaces the name of the modem remoteproc
        //with that which is expected by the mdm helper
        if(strstr(subsys_name, rproc_modem_name))
            snprintf(subsys_name, MAX_NAME_LEN, "modem");
        ...
        if(!strncmp(subsys_name, "modem", 5)) {
            ...
            device = &(dev->mdm_list[dev->num_modems]);
            if( get_subsystem_info(device, subsys_name) != RET_SUCCESS) {
            ...
            device = &(dev->subsystem_list[dev->num_additional_subsystems]);
            rcode = get_subsystem_info(device, subsys_name);
            if (rcode == RET_FAILED){
        ...
        if (subsys_root == ssr_root)
            snprintf(device->powerup_node, sizeof(device->powerup_node),
                     "/dev/subsys_%s", subsys_name);
        else
            snprintf(device->powerup_node, sizeof(device->powerup_node),
                     "/dev/%s", de->d_name);
    }
...
int get_peripheral_state(char *name)
{
    ...
    /* Try opening ssr dir and then rproc */
    if ((dir_ssr = opendir(ssr_root))) {
        subsys_root = ssr_root;
    } else if ((dir_ssr = opendir(rproc_root))) {
        subsys_root = rproc_root;
    } else {
        ALOGE("Failed to open subsys bus directory");
        goto error;
    }
    ALOGI("Using %s root directory\n", (subsys_root == rproc_root) ? "rproc" : "ssr");
