/*
* Copyright (c) 2013, 2017-2019, 2021-2023 Qualcomm Technologies, Inc.
	      ...
	      fd = open(VIRTUAL_FLASH_PARTITION, O_RDWR);
              if (fd < 0) {
                  ...
                  if (access(FTM_CFG_FILE_PATH, F_OK) == 0) {
                      /* Get the file offset and slot size to write from ftm_cfg
                       * if incoming art slot id is matched with ftm_cfg slot ID
                       */
                      if (!ftm_cfg.total_num_slots) {
                          DPRINTF(FTM_DBG_TRACE, "%s file entry is not present.\n",
                                  FTM_CFG_FILE_PATH);
                          close(fd);
                          return rsp;
                      }
                      for (i = 0; i <= art_slot_id; i++) {
                          if (art_slot_id == ftm_cfg.slot_id[i]) {
