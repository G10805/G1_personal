...
/*UCI Responses from the UWB SoC*/
#define LOG_UWB_UCI_RX_CTRL_C               0x1E29
/*UCI Notifications from the UWB SoC*/
#define LOG_UWB_UCI_RX_NTF_C                0x1E2A
/*UCI TX Data*/
#define LOG_UWB_UCI_TX_DATA_C               0x1E2B
/*UCI RX Data*/
#define LOG_UWB_UCI_RX_DATA_C               0x1E2C
/*UWB Enhanced Logs*/
#define LOG_UWB_UCI_TX_LOG_C                0x1E2D
...
/* UWB Command buffer */
static uint8 uwb_ftm_buffer[256];
...
uint8 uwb_event_read_buff[256]={0};
...
boolean uwb_qinsert_cmd(ftm_uwb_pkt_type *ftm_uwb_pkt)
{
  ...
  if(newitem->cmd_len != 0)
  {
     ...
     {
       DPRINTF(FTM_DBG_TRACE,"UWB FTM:  %02X\n", value[i]);
     }
   }
...
boolean ftm_uwb_hal_nwrite (uint8 *buf, int size)
{
  ...
  do
  {
    buf_len[0] = size - tx_bytes;

    nwrite_len = write(uwb_fd_transport, buf_len, 1);
    DPRINTF(FTM_DBG_TRACE,"UWB FTM: %s:nwrite_len:%d\n", __func__,nwrite_len);
  ...
  DPRINTF(FTM_DBG_TRACE,"UWB FTM: [%s] %s: CMD:\n", get_current_time(), __func__);
...
boolean ftm_uwb_send_cmd(uint8 * cmd_buf, uint16 cmd_len)
{
  ...
  {
#ifdef FTM_DEBUG
    DPRINTF(FTM_DBG_TRACE,"UWB FTM: %s: cmd is\n", __func__);
    for (int i =0; i<cmd_len;i++)
    {
      DPRINTF(FTM_DBG_TRACE,"UWB FTM: %X\n",cmd_buf[i]);
    }
#endif
...
void ftm_uwb_dispatch(void *ftm_uwb_pkt ,int cmd_len )
{
  int ret;
  DPRINTF(FTM_DBG_TRACE,"UWB FTM: %s:enter\n", __func__);
  memcpy(uwb_ftm_buffer, (void*)ftm_uwb_pkt, cmd_len);
  ...
  ret = ftm_uwb_send_cmd((uint8 *) uwb_ftm_buffer, cmd_len);
  if (ret != TRUE)
  {
    ftm_send_uwb_rsp_back_to_diag(&uwb_event_buf_user_cmd_unknown_err[0],uwblogsize);
...
=========================================================================================*/
uwb_request_status ftm_uwb_hal_nread(int size)
{
...
boolean ftm_uwb_read_event (void)
{
  ...
  int size =0;
  DPRINTF(FTM_DBG_TRACE,"UWB FTM: %s:enter\n", __func__);
  rx_status = ftm_uwb_hal_nread(1);
  size = uwb_event_read_buff[0];
    DPRINTF(FTM_DBG_TRACE,"UWB FTM: %s:size to read =%d\n", __func__,size);
  rx_status = ftm_uwb_hal_nread(size);
