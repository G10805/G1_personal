...
/******************************************************************************
  ...
  ---------------------------------------------------------------------------
  Copyright (c) 2008, 2010-2011, 2013-2024 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
#define DS_ESOC_MDM_NAME_SDXLEMUR       "SDXLEMUR"
...
ds_image_type_t global_imtype = -1;
...
static const ds_target_str_map_t ds_target_str_map[] =
{
    ...
    { DS_TARGET_LITO,             "lito"            },
    { DS_TARGET_LAHAINA,          "lahaina"         },
    { DS_TARGET_WAIPIO,           "waipio"          },
    { DS_TARGET_WAIPIO_VT,        "waipio_vt"       },
    { DS_TARGET_FUSION_WAIPIO_SDXLEMUR, "fusion_waipio_sdxlemur" },
    { DS_TARGET_KAILUA, "kailua" },
    { DS_TARGET_MONACO, "monaco" },
    { DS_TARGET_GEN4,   "gen4"  },
};
...
/*!
@brief
  Returns duplicated string or NULL if malloc call fails

@return
  char * buffer to be returned
...
/*!
@brief
  Returns duplicated string of maxiumum size n_bytes. If original string
  exceeds n_bytes in length, then the resulting string will be truncated.
...
*/
/*=========================================================================*/
char *ds_strndup(const char *str, size_t n_bytes)
{
  char *copy;
  size_t len;

  len = strnlen(str, n_bytes);
  copy = malloc(len + 1);
  if (copy != NULL) {
    ds_memscpy(copy, len, str, len);
    copy[len] = '\0';
  }
...
static ds_target_t ds_get_target_from_mdmdetect()
{
  ...
  /* devinfo.num_modems is 1 for SDXLEMUR */
  else if ((!(std_stricmp(devinfo.mdm_list[0].mdm_name, DS_ESOC_MDM_NAME_SDXLEMUR))) &&
           devinfo.num_modems == 1)
  {
    /* Assuming AP is WAIPIO */
    target = DS_TARGET_FUSION_WAIPIO_SDXLEMUR;
  }
...
ds_target_soc_id_t ds_get_soc_id()
{
  ...
  else if (0 == fscanf(fp,"%d", &soc_id))
  {
    ...
    soc_id = DS_TARGET_SOC_ID_INVALID;
  }
    ...
    do
    {
      ...
      else
      {
        ...
        switch((int)soc_id)
        {
          ...
          case DS_TARGET_SOC_ID_QTANG:
          case DS_TARGET_SOC_ID_KHAJE:
          case DS_TARGET_SOC_ID_KHAJE_IOT:
          ...
          case DS_TARGET_SOC_ID_SDA865:
            ...
            break;
          case DS_TARGET_SOC_ID_SM_LAHAINA:
          case DS_TARGET_SOC_ID_SMP_LAHAINA:
          case DS_TARGET_SOC_ID_HOLI:
          case DS_TARGET_SOC_ID_HOLI_H:
          case DS_TARGET_SOC_ID_SHIMA:
          case DS_TARGET_SOC_ID_YUPIK:
          case DS_TARGET_SOC_ID_YUPIK_IOT:
            target = DS_TARGET_LAHAINA;
            break;
          case DS_TARGET_SOC_ID_SM_WAIPIO:
          case DS_TARGET_SOC_ID_SMP_WAIPIO:
            target = DS_TARGET_WAIPIO;
            break;
          case DS_TARGET_SOC_ID_DIWALI:
          case DS_TARGET_SOC_ID_CAPE:
          case DS_TARGET_SOC_ID_CAPEV2:
          case DS_TARGET_SOC_ID_ALAKAI:
            target = DS_TARGET_WAIPIO_VT;
            break;
          case DS_TARGET_SOC_ID_KAILUA:
          case DS_TARGET_SOC_ID_SMP_KAILUA:
          case DS_TARGET_SOC_ID_SG_CPU_KALAMA:
          case DS_TARGET_SOC_ID_SGP_CPU_KALAMA:
          case DS_TARGET_SOC_ID_QCM_KALAMA:
          case DS_TARGET_SOC_ID_CROW:
          case DS_TARGET_SOC_ID_SHAW:
            target = DS_TARGET_KAILUA;
            break;
          case DS_TARGET_SOC_ID_MONACO:
            target = DS_TARGET_MONACO;
            break;
...
)
{
  ...
  int rc, read_rc, select_rc, cmd_rc, commandLen = 0;
  ...
  if(command[buflen-1] != 0)
    commandLen = buflen;
  else
    commandLen = strlen(command);
  rcmd_len = commandLen + DS_EXEC_NEW_BUFFSIZE;
  ...
  else
  {
    ...
    if (opts->log_options & DS_EXEC_LOG_EXECTIME)
    {
      ...
      {
        ds_log_low("%s(): [cpid:%d] Exec completed in %lu.%06lu seconds Image Type: %s\n",
                   __func__, pid, ts_elapsed.tv_sec, (ts_elapsed.tv_nsec/1000),
                   (global_imtype == DS_IMAGE_TYPE_PERF) ? "perf": "debug");
...
wait_status:
    if ((wait_pid = waitpid(pid, &cmd_rc, 0) == pid) ) {
      if(WIFEXITED(cmd_rc))
      {
      ...
      else if(WIFSIGNALED(cmd_rc))
      {
          cmd_rc = WEXITSTATUS(cmd_rc);
          ds_log_med("%s(): [cpid:%d] Process exited with a terminating signal with rc=%d\n",
                     __func__, pid, cmd_rc);
          if (out_file)
          {
            fprintf(out_file, "%s(): [cpid:%d] Process exited with a terminating signal with rc=%d\n",
                    __func__, pid, cmd_rc);
          }
          if (cmd_rc != 0)
          {
...
)
{
  ...
  rc = ds_exec_internal(command, opts, cmd_loc, commandLen);
...
)
{
  ...
  result = ds_exec(command, &opts, sizeof(command));
...
)
{
  ...
  result = ds_exec(cmd_string, &opts, sizeof(cmd_string));
  if (result != DS_EXEC_OK)
    result = -1;
...
)
{
  ...
  if (global_imtype >= 0)
  {
    return global_imtype;
  }
  if (target == DS_TARGET_LAHAINA ||
      target == DS_TARGET_WAIPIO  ||
      target == DS_TARGET_WAIPIO_VT ||
      target == DS_TARGET_KAILUA)
  {
    ...
    ds_log_high("%s() Uname %s %s %s %s %s\n", __func__, n.sysname, n.nodename,
                n.release, n.version, n.machine);

    /* Under the assumption uname doesn't change during runtime */
    if (strstr(n.release, "debug") != NULL ||
        strstr(n.release, "consolidate") != NULL)
    {
      global_imtype = DS_IMAGE_TYPE_DEBUG;
      return DS_IMAGE_TYPE_DEBUG;
    }
    global_imtype = DS_IMAGE_TYPE_PERF;
