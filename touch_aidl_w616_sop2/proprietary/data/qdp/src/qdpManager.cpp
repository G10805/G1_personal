...
)
{
  ...
  *techType = QDP_3GPP;
  ...
  if (p_params.hasApnName()) {
    std::string apnName = p_params.getApnName();
    strlcpy(apn, apnName.c_str(), sizeof(apn));
  }
  if(p_params.hasHomeIPType()) {
      *pdn = convert_to_qdp_pdn_type(p_params.getHomeIPType());
  }
  ...
  if (!foundProfiles.empty()) {
    ...
    if(param_strings[QDP_RIL_APN] != NULL) {
      if(!params->hasApnName() || (strncasecmp(param_strings[QDP_RIL_APN], params->getApnName().c_str(), (params->getApnName().size())+1))) {
        ...
        strlcpy(modify_params->apn_name, param_strings[QDP_RIL_APN], sizeof(modify_params->apn_name));
        *updateNeeded = TRUE;
      }
    ...
    if(param_strings[QDP_RIL_NAI] != NULL) {
      if(!params->hasUserName()  || (strncasecmp(param_strings[QDP_RIL_NAI], params->getUserName().c_str(), (params->getUserName().size())+1))) {
        ...
        strlcpy(modify_params->username, param_strings[QDP_RIL_NAI], sizeof(modify_params->username));
        *updateNeeded = TRUE;
      }
    ...
    if(param_strings[QDP_RIL_PASSWORD] != NULL) {
      if(!params->hasPassword() || (strncasecmp(param_strings[QDP_RIL_PASSWORD], params->getPassword().c_str(), (params->getPassword().size())+1))) {
        ...
        strlcpy(modify_params->password, param_strings[QDP_RIL_PASSWORD], sizeof(modify_params->password));
      }
...
)
{
  ...
  if(param_strings[QDP_RIL_APN] != NULL) {
    ...
    strlcpy(modify_params->apn_string, param_strings[QDP_RIL_APN], sizeof(modify_params->apn_string));
  }

  if(param_strings[QDP_RIL_NAI] != NULL) {
    modify_params->user_id_valid  = TRUE;
    strlcpy(modify_params->user_id, param_strings[QDP_RIL_NAI], sizeof(modify_params->user_id));
  }
  ...
  if(param_strings[QDP_RIL_PASSWORD] != NULL) {
    ...
    strlcpy(modify_params->auth_password, param_strings[QDP_RIL_PASSWORD], sizeof(modify_params->auth_password));
  }
