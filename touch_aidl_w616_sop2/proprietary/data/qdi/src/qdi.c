...
/*===========================================================================
  ...
  Copyright (c) 2011-2016, 2018, 2021-2023 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
typedef struct
{
  ...
  boolean v4_bad_handle;
  boolean v6_bad_handle;
} qdi_call_info_t;
...
)
{
  ...
  QDI_LOG_MED("dev=%s qdi_handle=%d rsp_data={pkt_data_handle=%"PRIu32""
  ...
  else if (user_handle == info->handle_ipv6)
  {
    ...
    txn->cb_rsp_data.flags |= QDI_RSP_DATA_V6;
    if (call_info->mode == QDI_MODE_V4V6 && call_info->v4_bad_handle == TRUE)
    {
      txn->cb_rsp_data.flags |= QDI_RSP_DATA_V4;
      /* V4 handle is bad - use the v6 errors for v4 */
      txn->cb_rsp_data.rsp_data_v4.sys_err = sys_err_code;
      txn->cb_rsp_data.rsp_data_v4.qmi_err = qmi_err_code;
    }
    call_info->v4_bad_handle = FALSE;
  ...
  if(rsp_id == QMI_WDS_START_NETWORK_INTERFACE_RESP_V01 || (txn->v4_cb_pend == FALSE && txn->v6_cb_pend == FALSE))
  {
      QDI_LOG_HIGH("Calling client callback function");
      QDI_LOG_LOW("qdi_cb_hdlr: user_data=0x%lx flags %d",
                  (unsigned long)txn->user_data, txn->cb_rsp_data.flags);
      ...
      if(txn->v4_cb_pend == FALSE)
        txn->txn_v4 = QDI_INVALID_TXN_HNDL;
      if(txn->v6_cb_pend == FALSE)
...
/*===========================================================================
  FUNCTION  qdi_get_modem_link_info_ex
===========================================================================*/
/*!
@brief
  Function to query Modem WDS information for specific link on the specified
  PDU.
...
*/
/*=========================================================================*/
static int qdi_get_modem_link_info_ex
(
...
{
  /* Query on the Primary PDU. This always has ID 0 */
  return qdi_get_modem_link_info_ex(client_hndl, ip_family, addr, 0);
}
...
)
{
  ...
  qdi_call_info_t  *call_info = (qdi_call_info_t *)call_hndl;
  qdi_handle_info_t* info = NULL;
  wds_bind_subscription_enum_v01 existing_subs_id = WDS_DONT_CARE_SUBS_V01;
  boolean bad_handle = FALSE;
  ...
  *qmi_err_code = QMI_INTERNAL_ERR;
  ...
  if (QDI_SUCCESS != qdi_qmi_wds_get_bind_subscription(info->handle_ipv4,
                                                       &existing_subs_id))
  {
    QDI_LOG_ERROR("%s(): failed to get subscription for IPv4 client [%p]",
                  __func__, info->handle_ipv4);
  }

  if (QDI_SUCCESS != qdi_qmi_wds_bind_subscription(info->handle_ipv4,
                                                   subs_id))
  {
    QDI_LOG_ERROR("%s(): subs_id %d binding failed for IPv4 client [%p] with current subs_id %d",
                  __func__, subs_id, info->handle_ipv4, existing_subs_id);
  }
  else
  {
    QDI_LOG_LOW("%s(): Successfully bound IPv4 handle [%p] to subs_id %d with previous subs_id %d",
                __func__, info->handle_ipv4, subs_id, existing_subs_id);
  }
  ...
  existing_subs_id = WDS_DONT_CARE_SUBS_V01;

  if (QDI_SUCCESS != qdi_qmi_wds_get_bind_subscription(info->handle_ipv6,
                                                       &existing_subs_id))
  {
    QDI_LOG_ERROR("%s(): failed to get subscription for IPv6 client [%p]",
                  __func__, info->handle_ipv6);
  }

  if (QDI_SUCCESS != qdi_qmi_wds_bind_subscription(info->handle_ipv6,
                                                   subs_id))
  {
    QDI_LOG_ERROR("%s(): subs_id %d binding failed for IPv6 client [%p] with current subs_id %d",
                  __func__, subs_id, info->handle_ipv6, existing_subs_id);
  }
  else
  {
    QDI_LOG_LOW("%s(): Successfully bound IPv6 handle [%p] to subs_id %d with previous subs_id %d",
                 __func__, info->handle_ipv6, subs_id, existing_subs_id);
  }
...
)
{
  ...
  {
    QDI_LOG_MED("Dual-IP call with invalid TXN... Attempting to stop the V4 call if packet handle is not bad");
    ret_v4 = qdi_qmi_wds_stop_nw_if(info->handle_ipv4,
                                    FALSE,
                                    NULL,
                                    qmi_err_code,
    ...
    if (ret_v4 < 0)
    {
        QDI_LOG_ERROR("Failed to stop network interface using IPv4 "
                    "bound handle");
        if(call_info->v4_bad_handle == TRUE)
        {
            /* Since our handle is bad, we update the ret value to NO_ERR since it has already been aborted.*/
            ret_v4 = QMI_NO_ERR;
        }
    }
  ...
  {
    QDI_LOG_MED("Dual-IP call with invalid TXN... Attempting to stop the V6 call if pkt handle is not bad");
    ...
    if (ret_v6 < 0)
    {
      QDI_LOG_ERROR("Failed to stop network interface using IPv6 "
                    "bound handle");
      if(call_info->v6_bad_handle == TRUE)
      {
        /* Since we are having a bad_handle this isn't really an error, so update the ret values */
        ret_v6 = QMI_NO_ERR;
      }
    }
  }
...
/*===========================================================================
  FUNCTION  qdi_get_addr_info_ex
===========================================================================*/
/*!
@brief
  This function returns the Gateway and DNS IP addresses of the given family
...
@param
  ...
  ifname      - IP family of the address to return
  ipfamily    - AF_INET or AF_INET6
  pdu_id      - PDU ID on which to query for information
...
)
{
  ...
  /* Obtain the Gateway and DNS information from the modem */
  if (QDI_SUCCESS != qdi_get_modem_link_info_ex(handle, ip_family, addr_info, pdu_id))
  {
    QDI_LOG_ERROR("qdi_get_addr_info: failed to get info from modem "
                  "for ifname=%s trying ip_family=%s on pdu=%d\n",
                  ifname,
                  DSI_GET_IP_FAMILY(ip_family),
                  pdu_id);
    goto bail;
