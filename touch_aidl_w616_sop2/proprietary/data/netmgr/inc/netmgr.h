...
/*===========================================================================
  ...
  Copyright (c) 2010-2015, 2017-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
typedef enum netmgr_nl_events_e {
  ...
  NET_PLATFORM_OEMPRXY_IFACE_UP,   /* OEMPrxy interface entered UP state   */
  NET_PLATFORM_OEMPRXY_IFACE_DOWN, /* OEMPrxy interface entered DOWN state */
  NET_PLATFORM_PDU_AVAILABLE_EV,
  NET_PLATFORM_PDU_PROMOTION_EV,
  NET_PLATFORM_PDU_RELEASE_EV,
  NET_PLATFORM_PDU_RECONFIG_EV,
  NET_PLATFORM_OP_FAILED_EV,       /* Indicate NETMGR failures             */
...
#define NETMGR_USER_CMD_QUERY_LINK_MTU          8
#define NETMGR_USER_CMD_QUERY_LINK_IFACE_MTU_ONLY 9
#define NETMGR_USER_CMD_QUERY_OEMPROXY_DATAPATH_IFACE 10
#define NETMGR_USER_CMD_QUERY_MATCHING_CALL_IFACE 11
#define NETMGR_USER_CMD_QUERY_LINK_MTU_PDU_ID   12
...
#define NETMGR_EVT_PARAM_PDU_ID       (0x0400)
#define NETMGR_EVT_PARAM_ADDR_TYPE    (0x0800)
#define NETMGR_EVT_PARAM_MASK         ( NETMGR_EVT_PARAM_LINK      | \
                                        ...
                                        NETMGR_EVT_PARAM_PDU_ID    | \
                                        NETMGR_EVT_PARAM_ADDR_TYPE )
...
#define NETMGR_EVT_PARAM_OP_STATUS    (0x4000)
...
typedef enum netmgr_addr_type_e {
  NETMGR_ADDR_TYPE_DEFAULT,
  NETMGR_ADDR_TYPE_CANDIDATE,
  NETMGR_ADDR_TYPE_MULTIHOME,
  NETMGR_ADDR_TYPE_MAX,
} netmgr_addr_type;
...
typedef enum netmgr_client_listen_proto
{
  NETMGR_CLIENT_NETLINK_LISTEN = 0,
  NETMGR_CLIENT_TIPC_LISTEN = 1,
  NETMGR_CLIENT_MAX_LISTEN
} netmgr_client_listen_proto;
...
typedef enum netmgr_post_restart_state
{
  NETMGR_POST_RESTART_STATE_INVALID = -1,
  NETMGR_POST_RESTART_STATE_INIT_START = 0,
  NETMGR_POST_RESTART_STATE_INIT_ONGOING = 1,
  NETMGR_POST_RESTART_STATE_INIT_DONE = 2,
  NETMGR_POST_RESTART_STATE_MAX
} netmgr_restart_state_t;
...
typedef struct
{
  char device_name[NETMGR_IF_NAME_MAX_LEN + 1];
  int  device_name_valid;
} netmgr_qmi_matching_call_check_resp_t;

typedef struct netmgr_v4_v6_mtu_s
{
  unsigned int v4_mtu;
  unsigned int v6_mtu;
} netmgr_v4_v6_mtu_t;
...
} netmgr_oemprxy_req_params_t;

typedef struct
{
  int oemprxy_iface_id;
} netmgr_oemprxy_resp_params_t;
...
typedef struct netmgr_user_cmd_data_s {
  ...
  union{
    ...
    netmgr_v4_v6_mtu_t v4_v6_mtu;
    netmgr_oemprxy_resp_params_t netmgr_oemprxy_resp_params;
  }data;
  ...
  int                                           modem_subs_id;
  uint8_t                                       ignore_subs;
  netmgr_restart_state_t                        netmgr_restart_state;
  netmgr_oemprxy_req_params_t                   netmgr_oemprxy_req_params;
  netmgr_qmi_matching_call_check_resp_t         matching_call_check_resp;
...
typedef enum
{
  NETMGR_OP_STATUS_CODE_INVALID = -1,
  NETMGR_OP_STATUS_CODE_SUCCESS,  /* Default success, most likely will not be seen by clients */
  NETMGR_OP_STATUS_CODE_FAIL,
  NETMGR_OP_STATUS_CODE_MAX
} netmgr_op_status_code_t;

typedef enum
{
  NETMGR_OP_STATUS_IP_FAMILY_IGNORE,
  NETMGR_OP_STATUS_IP_FAMILY_V4,
  NETMGR_OP_STATUS_IP_FAMILY_V6,
  NETMGR_OP_STATUS_IP_FAMILY_MAX
} netmgr_op_status_ip_family_t;

typedef struct
{
  netmgr_op_status_ip_family_t op_ip_family;
  netmgr_op_status_code_t op_code;
} netmgr_op_status_info_t;
