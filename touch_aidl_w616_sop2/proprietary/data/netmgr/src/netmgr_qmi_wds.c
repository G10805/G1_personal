...
/*===========================================================================
  ...
  Copyright (c) 2015-2016, 2018-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
pthread_cond_t netmgr_qmi_wds_fragment_ind_cond = PTHREAD_COND_INITIALIZER;
pthread_mutex_t netmgr_qmi_wds_fragment_ind_lock = PTHREAD_MUTEX_INITIALIZER;
...
)
{
  ...
  case QMI_WDS_IPSEC_STATIC_SA_CONFIG_EX_INFO_IND_V01:
    /* Block here to ensure synchronization with netmgr_kif_install_nl_sa_rules / netmgr_kif_install_sa_rules
     * and to get a transaction id so we can reassemble the fragment.
     * Strict ordering is needed - request -> response -> indication
     * We cannot signal the sync function as we have not decoded the message yet
     * Since kif does not release the lock till the condition variable is setup (on static sa ex response),
     * we will automatically get blocked here (hence we have this weird immediate lock / unlock)
     * Ideally this would also need to have a (timed) condition variable here
     * Who would signal this multiple times though
     */
    pthread_mutex_lock(&netmgr_qmi_wds_fragment_ind_lock);
    pthread_mutex_unlock(&netmgr_qmi_wds_fragment_ind_lock);
    if (!netmgr_qmi_wds_ipsec_config_ex_frags.num_reassembled)
    {
      dest = &netmgr_qmi_wds_ipsec_config_ex_frags.first_frag;
    }
    else
    {
      /* num_reassembled already incremented once for first frag */
      dest = &netmgr_qmi_wds_ipsec_config_ex_frags.other_frags[netmgr_qmi_wds_ipsec_config_ex_frags.num_reassembled - 1];
    }
    dest_len = sizeof(wds_ipsec_static_sa_config_ex_info_ind_msg_v01);
    break;
  ...
  if (QMI_NO_ERR != qmi_err) {
    netmgr_log_err("%s(): error decoding indication=0x%x on link=%d\n", __func__, msg_id, link);
    goto bail;
  }

#ifdef FEATURE_DATA_IWLAN
  if (msg_id == QMI_WDS_IPSEC_STATIC_SA_CONFIG_EX_INFO_IND_V01)
  {
    if (!netmgr_qmi_wds_ipsec_config_ex_frags.num_reassembled)
    {
      netmgr_log_low("%s() transaction [%u:%u] num_reassembled [%u] num_segments [%u] segment_index [%u]", __func__,
                     netmgr_qmi_wds_ipsec_config_ex_frags.transaction_id,
                     netmgr_qmi_wds_ipsec_config_ex_frags.first_frag.tx_info.transaction_id,
                     netmgr_qmi_wds_ipsec_config_ex_frags.num_reassembled + 1,
                     netmgr_qmi_wds_ipsec_config_ex_frags.first_frag.tx_info.num_segments,
                     netmgr_qmi_wds_ipsec_config_ex_frags.first_frag.tx_info.segment_index);

      if (netmgr_qmi_wds_ipsec_config_ex_frags.transaction_id ==
          netmgr_qmi_wds_ipsec_config_ex_frags.first_frag.tx_info.transaction_id)
      {
        netmgr_qmi_wds_ipsec_config_ex_frags.num_reassembled++;
      }
      else
      {
        netmgr_log_err("%s() Invalid transaction id", __func__);
        goto bail;
      }
      ...
      {
        pthread_mutex_lock(&netmgr_qmi_wds_fragment_ind_lock);
        pthread_cond_signal(&netmgr_qmi_wds_fragment_ind_cond);
        pthread_mutex_unlock(&netmgr_qmi_wds_fragment_ind_lock);
        netmgr_log_low("%s() Reassembly complete for transaction id %d",
                       __func__, netmgr_qmi_wds_ipsec_config_ex_frags.transaction_id);
      }

      netmgr_qmi_wds_ipsec_config_ex_frags.other_frags = calloc(netmgr_qmi_wds_ipsec_config_ex_frags.first_frag.tx_info.num_segments - 1,
    ...
    else
    {
      ...
      netmgr_log_low("%s() transaction [%u:%u] num_reassembled [%u] num_segments [%u] segment_index [%u]", __func__,
                     netmgr_qmi_wds_ipsec_config_ex_frags.transaction_id,
                     netmgr_qmi_wds_ipsec_config_ex_frags.other_frags[netmgr_qmi_wds_ipsec_config_ex_frags.num_reassembled - 1].tx_info.transaction_id,
                     netmgr_qmi_wds_ipsec_config_ex_frags.num_reassembled + 1,
                     netmgr_qmi_wds_ipsec_config_ex_frags.other_frags[netmgr_qmi_wds_ipsec_config_ex_frags.num_reassembled - 1].tx_info.num_segments,
                     netmgr_qmi_wds_ipsec_config_ex_frags.other_frags[netmgr_qmi_wds_ipsec_config_ex_frags.num_reassembled - 1].tx_info.segment_index);
      ...
      /* num_reassembled already incremented once for first frag and then once above here so need to decrement twice */
      if (netmgr_qmi_wds_ipsec_config_ex_frags.num_reassembled ==
          netmgr_qmi_wds_ipsec_config_ex_frags.other_frags[netmgr_qmi_wds_ipsec_config_ex_frags.num_reassembled - 2].tx_info.num_segments)
      {
        pthread_mutex_lock(&netmgr_qmi_wds_fragment_ind_lock);
        pthread_cond_signal(&netmgr_qmi_wds_fragment_ind_cond);
        pthread_mutex_unlock(&netmgr_qmi_wds_fragment_ind_lock);
...
)
{
  ...
  *clnt_hndl = NETMGR_QMI_CLIENT_INVALID;
  ...
  rc = netmgr_qmi_client_wds_init_instance(dev_str,
...
bail:
  if (NETMGR_QMI_CLIENT_INVALID != *clnt_hndl) {
    netmgr_qmi_client_release(*clnt_hndl);
  }
...
)
{
  ...
  rc = netmgr_qmi_client_release(clnt_hndl);
...
/*===========================================================================
  FUNCTION  netmgr_qmi_wds_get_ipsec_static_sa_config_ex
===========================================================================*/
/*!
@brief
  Query the IPSec ex config from the Modem
...
*/
/*=========================================================================*/
int netmgr_qmi_wds_get_ipsec_static_sa_config_ex
(
  int                                             link,
  qmi_client_type                                 clnt_hndl,
  wds_get_ipsec_static_sa_config_ex_resp_msg_v01  *result
)
{
  int rc = QMI_INTERNAL_ERR;
  int ret = NETMGR_FAILURE;
  wds_get_ipsec_static_sa_config_ex_req_msg_v01 req;
  ...
  rc = qmi_client_send_msg_sync(clnt_hndl,
                                QMI_WDS_GET_IPSEC_STATIC_SA_CONFIG_EX_REQ_V01,
                                (void *)&req,
                                sizeof(req),
                                (void *)result,
                                sizeof(*result),
...
/*===========================================================================
  FUNCTION  netmgr_qmi_wds_get_ipsec_static_sa_config_ex_sync
===========================================================================*/
/*!
@brief
  Wait for the IPSec ex config ind from the Modem
...
*/
/*=========================================================================*/
int netmgr_qmi_wds_get_ipsec_static_sa_config_ex_sync
(
  void
)
{
  struct timespec ts;
  int err = NETMGR_SUCCESS;
  long tv_nsec;

  /* lock is already acquired at this point in netmgr_kif */

  /* pthread_cond_timewait works with CLOCK_REALTIME only.
   * if CLOCK_REALTIME needs to be changed to CLOCK_MONOTONIC,
   * use a pthread_condattr_t, set pthread_condattr_setclock to
   * CLOCK_MONOTONIC and then initialize the condition variable
   */
  clock_gettime(CLOCK_REALTIME, &ts);

  if (ts.tv_nsec >= 960000000)
  {
    ts.tv_nsec = ts.tv_nsec - 960000000;
    ts.tv_sec += 1;
  }
  else
  {
  ...
  err = pthread_cond_timedwait(&netmgr_qmi_wds_fragment_ind_cond, &netmgr_qmi_wds_fragment_ind_lock, &ts);
  if (err == ETIMEDOUT)
  {
    netmgr_log_err("%s() wds_ipsec_static_sa_config_ex_info_ind_msg_v01 exceeded 40ms threshold",
                   __func__);
    err = NETMGR_FAILURE;
  }
...
/*===========================================================================
  FUNCTION  netmgr_qmi_wds_block_ipsec_static_sa_config_ind
===========================================================================*/
/*!
@brief
  Pause the QMI_WDS_IPSEC_STATIC_SA_CONFIG_EX_INFO_IND_V01 callback
...
*/
/*=========================================================================*/
void netmgr_qmi_wds_block_ipsec_static_sa_config_ind
(
  void
)
{
...
/*===========================================================================
  FUNCTION  netmgr_qmi_wds_unblock_ipsec_static_sa_config_ind
===========================================================================*/
/*!
@brief
  Continue with the QMI_WDS_IPSEC_STATIC_SA_CONFIG_EX_INFO_IND_V01 callback
...
*/
/*=========================================================================*/
void netmgr_qmi_wds_unblock_ipsec_static_sa_config_ind
(
  void
)
{
