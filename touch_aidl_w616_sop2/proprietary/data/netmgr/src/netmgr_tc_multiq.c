...
out:
  netmgr_log_high("Assigned multiq num %u for bearer %u flow 0x%x ref %u",
...
)
{
  ...
  {
    netmgr_tc_cfg.links[link].mq_info[i].ref--;
    netmgr_log_high("Released multiq num %u for bearer %u flow 0x%x ref %u",
...
/*===========================================================================
  FUNCTION  netmgr_tc_match_2nd_pdu_flows
===========================================================================*/
/*!
@brief
  Compare flow objects to determine if they are the 2nd PDU flows
...
)
{
  ...
  if (list_data && NETMGR_IS_SEC_PDU_FLOW(list_data->qos_flow.flow_id))
  {
    return 0;
  }
  return 1;
...
)
{
  ...
  netmgr_log_high("Freeing flow_id=0x%x ip=%d",
...
)
{
  ...
  netmgr_log_high("Rebinding flow 0x%x from bearer %d to bearer %d",
...
)
{
  ...
  else
  {
    ...
    {
      netmgr_log_high( "Cannot find flow object[0x%x], creating\n", qos_flow->flow_id );
      result = netmgr_tc_flow_create_multiq( link, qos_flow );
    }
  }
...
)
{
  ...
  else if (NETMGR_IS_SEC_PDU_DEFAULT_FLOW(qos_flow->flow_id))
  {
    netmgr_log_med("Empty the 2nd PDU flow list");
    ...
    do
    {
      ...
      if (flow_info)
      {
        netmgr_log_med("Found and delete 2nd PDU flow 0x%x",
                       flow_info->qos_flow.flow_id);
        netmgr_tc_flow_delete_multiq(link, flow_info);
      }
      ds_dll_free(node);
  ...
  {
    netmgr_log_med("Deleting flow 0x%x", qos_flow->flow_id);
    /* find the tail of the flow list */
    node = ds_dll_next(netmgr_tc_cfg.links[link].flow_list, &dummy);
    while (NULL != node)
    {
