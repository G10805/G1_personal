...
)
{
  ...
  /* Alter the flow id of the sec PDU */
  if (qmi_ind.sec_pdu_id_valid)
  {
    flow_info->qos_flow_state.qos_identifier = NETMGR_SEC_PDU_FLOW_ID(
                                 flow_info->qos_flow_state.qos_identifier);
  }
...
)
{
  ...
  if (user_handle == link_info->qos_info.clnt_hdl[NETMGR_QMI_CLIENT_IPV6])
  {
...
)
{
  ...
  qos_flow.state = NETMGR_TC_FLOW_ACTIVATE;
  ...
  return netmgr_qmi_hold_flow_info(link, &qos_flow);
}
