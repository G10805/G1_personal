...
)
{
  ...
  int i, rc;
  const char *iifname = NM_MDMPRXY_IIFACE;
  ...
  if ((iifname = netmgr_kif_get_wlan_ifname_by_link(slot)) == NULL)
  {
    netmgr_log_err("%s(): invalid wlan info ip family [%d] slot [%d]",
                    __func__, af, slot);
    return NETMGR_FAILURE;
  }
  ...
  for (i = start; i >= 0; i--)
  {
    memcpy(rule->outrules[slot].iif, iifname, NM_MDMPRXY_MAXDEVSTR);
...
)
{
  ...
  if ((iifname = netmgr_kif_get_wlan_ifname_by_link(slot)) == NULL)
  {
    netmgr_log_err("%s() failed to get wlan info for link:%d", __func__, slot);
    goto error;
  }
#endif /* FEATURE_DATA_IWLAN */
  ...
  for (i = 0; i < route_prov_cmd_tbl->per_iface_forward_rule_list_len; i++)
  {
    ...
    ds_memscpy(rule->outrules[slot].iif, NM_MDMPRXY_MAXDEVSTR, iifname, strlen(iifname));
    rc = nm_mdmprxy_process_rule(&route_prov_cmd_tbl->per_iface_forward_rule_list[i],
                                 NM_MDMPRXY_RUN_RULE, af, &rule->outrules[slot]);
    if (rc != NETMGR_SUCCESS)
    {
...
)
{
  ...
  {
    case AF_INET:
      snprintf(buff, bufflen, formatstr, IPTABLES, nfo->iif, nfo->oif);
      break;

    case AF_INET6:
      snprintf(buff, bufflen, formatstr, IP6TABLES, nfo->iif, nfo->oif);
      break;
...
)
{
  ...
  {
  case AF_INET:
      snprintf(buff, bufflen, formatstr, IPTABLES, nfo->oif, nfo->iif);
      break;

  case AF_INET6:
      snprintf(buff, bufflen, formatstr, IP6TABLES, nfo->oif, nfo->iif);
      break;
...
/*===========================================================================
  FUNCTION nm_mdmprxy_static_rule_notrack_formatter
===========================================================================*/
/*!
@brief
  Formatter for masquerade jump rules
...
)
{
  ...
  {
	case AF_INET:
		snprintf(buff, bufflen, formatstr, IPTABLES, "icmp");
		break;

	case AF_INET6:
		snprintf(buff, bufflen, formatstr, IP6TABLES, "ipv6-icmp");
		break;
