...
static void nm_mdmprxy_socket_mark_set_reserved_ports(void)
{
  ...
  fp = netmgr_fopen(FILEPATH_RESERVED_PORT_BIND, "we");
  if (!fp)
  {
  ...
  fp = netmgr_fopen(FILEPATH_IP_LOCAL_RESERVED_PORTS, "we");
  if (!fp)
  {
...
{
  struct ds_sockaddr_storage addr;
    ...
    case NM_MDMPRXY_SOCKET_STATE_LISTENING:
      {
        if (NM_MDMPRXY_SOCKET_STATE_SHUTDOWN == remote_sockets[i].sock_state)
        {
          ...
          if (rc < 0)
          {
            ...
            netmgr_close(remote_sockets[i].fd);
...
static void nm_mdmprxy_socket_mark_close_socket(unsigned int handle)
{
  ...
  netmgr_close(remote_sockets[handle].fd);
...
(
  int type,
  struct ds_sockaddr_storage *source,
  ssize_t slen,
  uint32_t mark
)
{
  ...
  if (source->ss_family == AF_UNSPEC)
    fd = netmgr_socket(AF_INET6, type | SOCK_CLOEXEC, 0);
  else
    fd = netmgr_socket(source->ss_family, type | SOCK_CLOEXEC, 0);
...
@brief
  ...
  reserve TCP V4 ports for the modem. Formats a ds_sockaddr_storage
  ...
  case DFS_PORT_PROP_ADJACENT_START_EVEN_V01:
    do
    {
      ...
      idx_even = _nm_mdmprxy_socket_mark_socket_bind(SOCK_STREAM,
                                                     (struct ds_sockaddr_storage *)&sockaddr_v4,
                                                     sizeof(sockaddr_v4),
                                                     port_params->mark);
...
@brief
  Convenience wrapper around _nm_mdmprxy_socket_mark_socket_bind to help
  reserve UDP V4 ports for the modem. Formats a ds_sockaddr_storage
  based on the arguments then proceeds to call
...
@brief
  Convenience wrapper around _nm_mdmprxy_socket_mark_socket_bind to help
  reserve TCP & UDP V4 ports for the modem. Formats a ds_sockaddr_storage
  based on the arguments then proceeds to call
  _nm_mdmprxy_socket_mark_socket_bind. Port preferences provided by modem
  are enforced if valid.
...
@brief
  Convenience wrapper around _nm_mdmprxy_socket_mark_socket_bind to help
  reserve TCP V6 ports for the modem. Formats a ds_sockaddr_storage
  based on the arguments then proceeds to call
  _nm_mdmprxy_socket_mark_socket_bind. Port preferences provided by modem
  are enforced if valid.
...
)
{
  ...
  case DFS_PORT_PROP_ADJACENT_START_EVEN_V01:
    do
    {
      ...
      idx_even = _nm_mdmprxy_socket_mark_socket_bind(SOCK_STREAM,
                                                     (struct ds_sockaddr_storage *)&sockaddr_v6,
                                                     sizeof(sockaddr_v6),
                                                     port_params->mark);
...
@brief
  Convenience wrapper around _nm_mdmprxy_socket_mark_socket_bind to help
  reserve UDP V6 ports for the modem. Formats a ds_sockaddr_storage
  based on the arguments then proceeds to call
  _nm_mdmprxy_socket_mark_socket_bind. Port preferences provided by modem
  are enforced if valid.
...
@brief
  Convenience wrapper around _nm_mdmprxy_socket_mark_socket_bind to help
  reserve TCP & UDP V6 ports for the modem. Formats a ds_sockaddr_storage
  based on the arguments then proceeds to call
  _nm_mdmprxy_socket_mark_socket_bind. Port preferences provided by modem
  are enforced if valid.
...
static void nm_mdmprxy_socket_mark_release_qmi_clients(void)
{
  ...
  netmgr_qmi_client_release(pr_qmi_dfs_clnt_hndl);
...
static void nm_mdmprxy_socket_mark_init_qmi_clients(void)
{
  ...
  rc = netmgr_qmi_client_init_instance(dfs_get_service_object_v01(),
