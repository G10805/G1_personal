...
/*===========================================================================
  ...
  Copyright (c) 2015-2016, 2018-2020, 2023 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
#ifdef FEATURE_DATA_IWLAN
  if ((wlan_ifname = netmgr_kif_get_wlan_ifname_by_link(ifinfo->link)) == NULL)
  {
    netmgr_log_err("%s(): Invalid wlan info for iface %d !",
...
static void* nm_mdmprxy_install_wlan_rules(void *arg)
{
  ...
  netmgr_log_low("%s(): WLAN interface %s is UP, installing routing rules", __func__, ifinfo->wlan_ifname);
...
static void* nm_mdmprxy_remove_wlan_rules(void *arg)
{
  ...
  {
    /* If not the same wlan on which the call is going on, ignore */
    if ((wlan_ifname = netmgr_kif_get_wlan_ifname_by_link(ifinfo->link)) == NULL)
    {
      netmgr_log_err("%s(): Invalid wlan info for link %d, return w/o removing",
                        __func__, ifinfo->link);
      goto out;
    }
    else if (strcmp(wlan_ifname, ifinfo->wlan_ifname))
    {
      netmgr_log_low("%s() %s is going down, call is on %s.Ignoring wlan down ",
                     __func__, ifinfo->wlan_ifname, wlan_ifname);
      goto out;
    }
  }
#endif /* FEATURE_DATA_IWLAN */
...
*/
/*=========================================================================*/
int nm_mdmprxy_register_plugin(void)
{
