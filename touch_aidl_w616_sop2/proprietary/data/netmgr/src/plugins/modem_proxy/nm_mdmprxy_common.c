...
static const struct nm_mdmprxy_cmd notrack_static_rule_list[] =
{
  /*Enable connection tracking for icmp/icmpv6 only for pre-tunnel ping support*/
  { "%s" NM_MDMPRXY_FLAGS " -t raw -A nm_mdmprxy_raw_pre ! -p %s -i r_rmnet_data+ -j NOTRACK",
    "%s" NM_MDMPRXY_FLAGS " -t raw -D nm_mdmprxy_raw_pre ! -p %s -i r_rmnet_data+ -j NOTRACK",
    nm_mdmprxy_static_rule_notrack_formatter }
};
...
static const struct nm_mdmprxy_cmd static_jump_rule_list[] =
{
  ...
  { "%s" NM_MDMPRXY_FLAGS " -t nat -I POSTROUTING 1 -j nm_mdmprxy_masquerade_skip",
    "%s" NM_MDMPRXY_FLAGS " -t nat -D POSTROUTING -j nm_mdmprxy_masquerade_skip",
    nm_mdmprxy_static_rule_masquerade_formatter
  }
};
...
/*===========================================================================
  FUNCTION  nm_mdmprxy_common_uninstall_static_notrack_rules
===========================================================================*/
/*!
@brief
  Uninstall static iptables rules
...
*/
/*=========================================================================*/
static int nm_mdmprxy_common_uninstall_static_notrack_rules(sa_family_t af, int idx)
{
  int i = 0;
  int rc = NETMGR_SUCCESS;
  int rules_len = nm_mdmprxy_rules_len(notrack_static_rule_list);
  struct nm_mdmprxy_notrack_rule_info nfo;
  size_t len = 0;
  ...
  for (i = idx; i >= 0; i--)
  {
    rc |= nm_mdmprxy_process_rule(&notrack_static_rule_list[i], NM_MDMPRXY_RUN_ANTIRULE, af, &nfo);
...
bail:
  netmgr_log_low("%s(): static notrack rule uninstalled for ip_family [%d]", __func__, af);
  return rc;
}
/*===========================================================================
...
/*===========================================================================
  FUNCTION  nm_mdmprxy_common_install_static_notrack_rules
===========================================================================*/
/*!
@brief
  Install static iptables rules
...
*/
/*=========================================================================*/
static int nm_mdmprxy_common_install_static_notrack_rules(sa_family_t af)
{
  int i = 0;
  int rc = NETMGR_FAILURE;
  ...
  for (i = 0; i < (int) nm_mdmprxy_rules_len(notrack_static_rule_list); i++)
  {
    if (!notrack_static_rule_list[i].rule)
    {
      continue;
    }

    rc = nm_mdmprxy_process_rule(&notrack_static_rule_list[i], NM_MDMPRXY_RUN_RULE, af, &nfo);
    if (NETMGR_SUCCESS != rc)
    {
      netmgr_log_err("%s(): Failed to apply rule [%d]", __func__, i);
      goto bail;
  ...
  netmgr_log_low("%s(): NOTRACK static rules installation complete for ip_family [%d]", __func__, af);

  rc = NETMGR_SUCCESS;
  return rc;

bail:
  nm_mdmprxy_common_uninstall_static_notrack_rules(af, i);
  return rc;
}
...
int nm_mdmprxy_common_init(void* arg)
{
  ...
  rc |= nm_mdmprxy_common_install_static_notrack_rules(AF_INET);
  rc |= nm_mdmprxy_common_install_static_notrack_rules(AF_INET6);
