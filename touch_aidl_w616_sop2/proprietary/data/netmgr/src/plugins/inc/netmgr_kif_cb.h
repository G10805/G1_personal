...
#define NETMGR_KIF_CB_EXEC_CMD  "EXEC_CMD"

#define NETMGR_KIF_MAXCMDSTR_LEN 256

#define NETMGR_KIF_CB_GET_WLAN_IFNAME_BY_LINK "GET_WLAN_IFNAME_BY_LINK"
#define NETMGR_KIF_CB_GET_WWAN_IFNAME_BY_LINK "GET_WWAN_IFNAME_BY_LINK"
...
#define NETMGR_KIF_CB_IF_TECH_TYPE_EPDG_PROXY   3
#define NETMGR_KIF_CB_IF_TECH_TYPE_C_WLAN       4
...
struct netmgr_kif_cb_ifinfo {
  ...
  char wlan_ifname[NETMGR_IF_NAME_MAX_LEN];
};
...
typedef struct
{
  char **cmd_list;
  int  cmd_list_len;
} netmgr_kif_cb_exec_cmd_req_t;

typedef struct
{
  int idx;      /* index in the cmd_list array of request to indicate exact point of failure*/
  int ret_code; /* Return code from command execution */
} netmgr_kif_cb_exec_cmd_resp_t;

typedef struct
{
  netmgr_kif_cb_exec_cmd_req_t  req;
  netmgr_kif_cb_exec_cmd_resp_t *resp;
} netmgr_kif_cb_exec_cmd_info_t;

typedef struct
{
  int link;
  char iface[NETMGR_KIF_CB_IFNAMELEN];
  int iface_valid;
} netmgr_kif_cb_get_wlan_ifname_by_link_exec_info;
