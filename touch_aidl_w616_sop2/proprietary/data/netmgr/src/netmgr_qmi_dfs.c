...
/*===========================================================================
  ...
  Copyright (c) 2014-2016,2018-2019, 2021-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
)
{
  ...
  for (i = 0; i < num_rules; ++i)
  {
        ...
        (QMI_DFS_IPSEC_FILTER_MASK_SPI_V01 & rules[i].xport_info.esp_info.valid_params))
    {
      ...
      ifinfo.spi = rules[i].xport_info.esp_info.spi;
      ifinfo.filter_op = filter_op;

      if (addr_info->valid_mask & NETMGR_ADDRSET_MASK_IPV4 &&
          addr_info->valid_mask & NETMGR_ADDRSET_MASK_IPV6)
      {
        ...
        ifinfo.addr._in4.addr = addr_set->if_addr.addr.v4;
        ifinfo.addr._in4.mask = addr_set->if_mask;

        addr_set = netmgr_qmi_get_addr_set(addr_info, NETMGR_IPV6_ADDR,
                                           NETMGR_ADDR_TYPE_DEFAULT);
        memcpy(&ifinfo.addr._in6.addr, &addr_set->if_addr.addr,
               sizeof(ifinfo.addr._in6.addr));
        ifinfo.addr._in6.mask = addr_set->if_mask;

        ip_family = AF_UNSPEC;
      }
      else if (addr_info->valid_mask & NETMGR_ADDRSET_MASK_IPV4)
      {
        netmgr_address_set_t *addr_set;
...
)
{
  ...
  {
    rc = netmgr_qmi_client_release(qmi_dfs_clnt_hndl);
...
bail:
  if (rc != QMI_NO_ERR || QMI_RESULT_SUCCESS_V01 != resp.resp.result)
  {
    if (NULL != qmi_hndl)
    {
      netmgr_qmi_client_release(qmi_hndl);
    }
