...
/*===========================================================================
  ...
  Copyright (c) 2010, 2013-2014, 2016, 2019-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
#define NETMGR_RTATYPE_PDU_ID     (NETMGR_RTATYPE_BASE+11)/* PDU ID             */
#define NETMGR_RTATYPE_ADDR_TYPE  (NETMGR_RTATYPE_BASE+12)/* IPv6 Address class */
#define NETMGR_RTATYPE_OP_STATUS  (NETMGR_RTATYPE_BASE+13)/* Op status code   */
...
#define NETMGR_RTASIZE_DNSPADDR   RTA_SPACE( sizeof(struct ds_sockaddr_storage) )
#define NETMGR_RTASIZE_DNSSADDR   RTA_SPACE( sizeof(struct ds_sockaddr_storage) )
#define NETMGR_RTASIZE_DEVNAME    RTA_SPACE( NETMGR_IF_NAME_MAX_LEN )
#define NETMGR_RTASIZE_MTU        RTA_SPACE( sizeof(unsigned int)*NUM_MTU_VALS )
#define NETMGR_RTASIZE_PDU_ID     RTA_SPACE( sizeof(unsigned int) )
#define NETMGR_RTASIZE_USER_CMD   RTA_SPACE( sizeof(unsigned int))
#define NETMGR_RTASIZE_ADDR_TYPE  RTA_SPACE( sizeof(netmgr_addr_type) )
#define NETMGR_RTASIZE_CMD_DATA   RTA_SPACE( sizeof(netmgr_user_cmd_data_t))
#define NETMGR_RTASIZE_OP_STATUS  RTA_SPACE( sizeof(netmgr_op_status_info_t))
...
#define NETMGR_ROUTE_PARAM_NONE      (0x0000)
#define NETMGR_ROUTE_PARAM_DST       (0x0001)
#define NETMGR_ROUTE_PARAM_SRC       (0x0002)
#define NETMGR_ROUTE_PARAM_OIF       (0x0004)
#define NETMGR_ROUTE_PARAM_IIF       (0x0008)
#define NETMGR_ROUTE_PARAM_GATEWAY   (0x0010)
#define NETMGR_ROUTE_PARAM_METRICS   (0x0020)
#define NETMGR_ROUTE_PARAM_TABLE     (0x0040)
#define NETMGR_ROUTE_PARAM_MTU       (0x0080)
#define NETMGR_ROUTE_PARAM_CACHEINFO (0x0100)

#define NETMGR_DEFAULT_ROUTE_ATTR_MASK ( NETMGR_ROUTE_PARAM_OIF | \
                                         NETMGR_ROUTE_PARAM_GATEWAY | \
                                         NETMGR_ROUTE_PARAM_TABLE )
...
typedef struct netmgr_nl_addr_info_s {
  ...
  struct {                                      /* attributes  */
    ...
    struct ds_sockaddr_storage       prefix_addr;
    struct ds_sockaddr_storage       local_addr;
    struct ds_sockaddr_storage       bcast_addr;
    struct ds_sockaddr_storage       acast_addr;
    struct ds_sockaddr_storage       mcast_addr;
...
typedef struct netmgr_nl_route_info_s {
  struct rtmsg           metainfo;
  struct {
    unsigned int               param_mask;
    struct ds_sockaddr_storage dst_addr;
    struct ds_sockaddr_storage src_addr;
    struct ds_sockaddr_storage gateway_addr;
    struct rta_cacheinfo       cacheinfo;
    unsigned int               oif;
    unsigned int               iif;
    unsigned int               tableid;
    unsigned int               rtax_mtu;
  } attr_info;
} netmgr_nl_route_info_t;
...
typedef struct {
  ...
  union {
    ...
    struct sockaddr_tipc tipc_addr_loc;
  };
  union {
    struct sockaddr_nl  sk_addr_rem; /* remote endpoint's address */
    struct sockaddr_tipc tipc_addr_rem;
  };
