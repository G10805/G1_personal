...
#define CAL_TEARDOWN_IFACE_VAL(x,y) !((x || (y-1)) && !(x && (y-1)))

#define GET_TEARDOWN_IFACE_VAL(x,y,z)     \
    if((y != NETMGR_ADDRSET_MASK_IPV4V6) && (y != NETMGR_ADDRSET_MASK_INVALID)) \
    z = CAL_TEARDOWN_IFACE_VAL(x,y);  \
    else                                \
    z = 0;                            \
...
#ifdef FEATURE_DATA_IWLAN
struct netmgr_qmi_wds_ipsec_config_ex_fragments netmgr_qmi_wds_ipsec_config_ex_frags;
#endif /* FEATURE_DATA_IWLAN */
/*---------------------------------------------------------------------------
...
netmgr_wds_qmi_drv_info_t *__netmgr_qmi_get_link_wds_info_pdu(int link, int pdu_cnf)
{
  return &netmgr_qmi_cfg.links[link].pdu_wds_info[pdu_cnf];
}
...
netmgr_qmi_reset_link_info (int link)
{
  ...
  netmgr_link_info_t *link_info = netmgr_qmi_get_link_info(link);
  ...
  ifindex = netmgr_kif_get_link_ifindex(link);
  if (ifindex)
  {
    netmgr_sscm3_link_info_reset(link, ifindex);
  }

  /* Reset per-PDU information */
  for (pdu_cnf = 0; pdu_cnf < MAX_PDU; pdu_cnf++)
  {
    netmgr_wds_qmi_drv_info_t *wds_info;

    wds_info = __netmgr_qmi_get_link_wds_info_pdu(link, pdu_cnf);
    wds_info->clnt_hdl[NETMGR_QMI_CLIENT_IPV4] = NETMGR_QMI_CLIENT_INVALID;
    wds_info->clnt_hdl[NETMGR_QMI_CLIENT_IPV6] = NETMGR_QMI_CLIENT_INVALID;
    wds_info->iface_mtu = NETMGR_MTU_INVALID;
    wds_info->mtu_ipv4 = NETMGR_MTU_INVALID;
    wds_info->mtu_ipv6 = NETMGR_MTU_INVALID;
    wds_info->apn_type_mask = WDS_APN_TYPE_MASK_UNSPECIFIED_V01;
    memset(&wds_info->addr_info, 0, sizeof(wds_info->addr_info));
    wds_info->first_conn_clnt = NETMGR_QMI_CLIENT_MAX;
    wds_info->v4_pkt_conn = FALSE;
    wds_info->v6_pkt_conn = FALSE;
    wds_info->in_use = FALSE;
    wds_info->xlat_capable = FALSE;

    ds_dll_init(&link_info->qos_info.cached_flow_list[pdu_cnf]);

#ifdef FEATURE_DATA_IWLAN
    netmgr_util_circ_list_destroy(&wds_info->rev_ip_txns[NETMGR_QMI_CLIENT_IPV4]);
    netmgr_util_circ_list_destroy(&wds_info->rev_ip_txns[NETMGR_QMI_CLIENT_IPV6]);
#endif /* FEATURE_DATA_IWLAN */
  }

  link_info->qos_info.clnt_hdl[NETMGR_QMI_CLIENT_IPV4] = NETMGR_QMI_CLIENT_INVALID;
  link_info->qos_info.clnt_hdl[NETMGR_QMI_CLIENT_IPV6] = NETMGR_QMI_CLIENT_INVALID;

#ifdef FEATURE_DATA_IWLAN
  link_info->dfs_info.clnt_hdl[NETMGR_QMI_CLIENT_IPV4] = NETMGR_QMI_CLIENT_INVALID;
  link_info->dfs_info.clnt_hdl[NETMGR_QMI_CLIENT_IPV6] = NETMGR_QMI_CLIENT_INVALID;
  link_info->assoc_link = NETMGR_LINK_MAX;
#endif /* FEATURE_DATA_IWLAN */

  link_info->modem_subs_id = NETMGR_DATA_DEFAULT_SUBS;
...
)
{
  ...
  if (NETMGR_QMI_CLIENT_INVALID != link_info->dfs_info.clnt_hdl[NETMGR_QMI_CLIENT_IPV4])
  {
    ...
    netmgr_qmi_client_release( link_info->dfs_info.clnt_hdl[NETMGR_QMI_CLIENT_IPV4] );
  }
  ...
  if (NETMGR_QMI_CLIENT_INVALID != link_info->dfs_info.clnt_hdl[NETMGR_QMI_CLIENT_IPV6])
  {
    ...
    netmgr_qmi_client_release( link_info->dfs_info.clnt_hdl[NETMGR_QMI_CLIENT_IPV6] );
  }
#endif /* FEATURE_DATA_IWLAN */
...
)
{
  ...
  if (netmgr_main_cfg.ep_config_ext_ioctl_v2) {
    free((void *) netmgr_main_cfg.eg_fmt.egress_param_ptr);
    free((void *) netmgr_main_cfg.ing_fmt.ingress_param_ptr);
  }
...
)
{
  ...
  int pdu_index = (pdu_id) ? SECONDARY_PDU : PRIMARY_PDU;
  netmgr_link_info_t *link_info;
  netmgr_wds_qmi_drv_info_t *wds_info;
  ...
  wds_info = __netmgr_qmi_get_link_wds_info_pdu(link, pdu_index);
  clnt = netmgr_qmi_convert_ip_pref_to_client_type(evt_ip_family);
  client_hndl = netmgr_qmi_wds_get_clnt_id(link, clnt);

  /*TODO GET CORRECT QMI defined for lifetime request*/
  if (NETMGR_SUCCESS != netmgr_qmi_wds_get_runtime_settings(link,
                                                            ...
                                                            (QMI_WDS_MASK_REQ_SETTINGS_DNS_ADDR_V01  |
                                                             ...
                                                             QMI_WDS_MASK_REQ_SETTINGS_APN_TYPE_MASK_V01 |
  ...
  {
    addr_info_ptr = netmgr_qmi_get_addr_set(&wds_info->addr_info, NETMGR_IPV4_ADDR,
                                            NETMGR_ADDR_TYPE_DEFAULT);
    mtu_ptr = &wds_info->mtu_ipv4;
  }
  else if (WDS_IP_FAMILY_IPV6_V01 == evt_ip_family)
  {
    addr_info_ptr = netmgr_qmi_get_addr_set(&wds_info->addr_info, NETMGR_IPV6_ADDR,
                                            (pdu_index == SECONDARY_PDU) ?
                                            NETMGR_ADDR_TYPE_CANDIDATE :
                                            NETMGR_ADDR_TYPE_DEFAULT);
    mtu_ptr = &wds_info->mtu_ipv6;
  }
  else
  {
  ...
  if (call_info.ip_family_valid)
  {
    ...
    addr_info_ptr->modem_addr.type = call_info.ip_family;
  ...
  if (call_info.ipv4_address_preference_valid)
  {
    ...
    if (FALSE == addr_info_ptr->is_addr_purge_pend)
    {
      ...
      addr_info_ptr->modem_addr.addr.v4 = htonl((uint32_t)call_info.ipv4_address_preference);
      NETMGR_LOG_IPV4_ADDR(med, "Modem iface",
                           addr_info_ptr->modem_addr.addr.v4);
    }
  }
  else
  {
    ...
    if (FALSE == addr_info_ptr->is_addr_purge_pend)
    {
      if (call_info.ipv6_addr_valid)
      {
        ...
        memcpy(addr_info_ptr->modem_addr.addr.v6_addr8,
               call_info.ipv6_addr.ipv6_addr,
               sizeof(addr_info_ptr->modem_addr.addr.v6_addr8));
        NETMGR_LOG_IPV6_ADDR(med, "Modem iface",
                             addr_info_ptr->modem_addr.addr.v6_addr64);
      }
      else
      {
  ...
  /* Process ssc3 lifetime*/
  if (call_info.ssc3_pdu_lifetime_valid)
  {
    /* Lifetime from modem is in milliseconds */
    addr_info_ptr->ssc3_pdu_lifetime = (unsigned int)call_info.ssc3_pdu_lifetime / 1000;
    if (0 != addr_info_ptr->ssc3_pdu_lifetime)
    {
      /* Add a fudge factor to prevent premature address deletion if there is delay in
       * receiving the PROMOTE event from modem.
       */
      addr_info_ptr->ssc3_pdu_lifetime += 3;
    }

    addr_info_ptr->ssc3_pdu_lifetime_valid = (uint32_t)call_info.ssc3_pdu_lifetime_valid;
    netmgr_log_med("Ssc3_pdu_lifetime = %d\n", addr_info_ptr->ssc3_pdu_lifetime );
  }
  ...
  if (call_info.apn_type_mask_valid)
  {
    netmgr_log_med("Link %d IP type 0x%x APN Type Mask received: 0x%x\n",
                   link, evt_ip_family, call_info.apn_type_mask);
    /* Update mask to add this (accounts for dual-ip) */
    wds_info->apn_type_mask |= call_info.apn_type_mask;
    netmgr_log_med("Link %d APN Type Mask: 0x%x\n",
                   link, wds_info->apn_type_mask);
  }
  else
  {
    netmgr_log_med("Link %d APN Type Mask TLV absent\n", link);
  }

  /* Store PDU ID the information belons to */
  addr_info_ptr->pdu_id = pdu_id;
...
/*===========================================================================
  FUNCTION  netmgr_qmi_get_modem_link_info_mtu
===========================================================================*/
/*!
@brief
...
*/
/*=========================================================================*/
int netmgr_qmi_get_modem_link_info_mtu
(
  uint8                   link,
  wds_ip_family_enum_v01  evt_ip_family,
  unsigned int pdu_id
...
)
{
  ...
  /* If either of these values are 2, the per interface coalescing configuration
   * will not be managed here.
   */
  if (rsc == 2 || rsb == 2)
  {
    tcp_coal = 1;
...
{
  int i = 0;
  int phy_transport_status = NETMGR_FAILURE;
  ...
  case NETMGR_QMI_SYS_EVENT_MODEM_OUT_OF_SERVICE_IND:
    ...
    {
      netmgr_qmi_client_release(netmgr_route_look_up_hndl);
    ...
    if (netmgr_main_get_rmnet_data_enabled())
    {
      ...
      else
      {
                      ...
                      NETMGR_GET_MODEM_SSR_STATE() != NETMGR_MODEM_OOS_STATE; i++)
        {
          ...
          {
            phy_transport_status = NETMGR_FAILURE;
            sleep(1);
          }
          else
          {
            phy_transport_status = NETMGR_SUCCESS;
            break;
          }
        }

	netmgr_log_high("%s(): Physical transport %s init status %d retry count %d\n",
                        __func__, netmgr_main_cfg.phys_net_dev,
			phy_transport_status, i);
      }
    }
...
LOCAL void
netmgr_wds_legacy_pkt_srvc_connected
(
  int link,
...
{
  netmgr_wds_qmi_drv_info_t *wds_info;
  netmgr_address_set_t *cached_addr;
  ...
  if (NETMGR_QMI_CLIENT_MAX == wds_info->first_conn_clnt ||
      (wds_info->v4_pkt_conn == FALSE && wds_info->v6_pkt_conn == FALSE))
  {
    wds_info->first_conn_clnt = clnt;
  }

  if (clnt == NETMGR_QMI_CLIENT_IPV6)
  {
    new_conn = !wds_info->v6_pkt_conn;
    wds_info->v6_pkt_conn = TRUE;
  }
  else if (clnt == NETMGR_QMI_CLIENT_IPV4)
  {
    new_conn = !wds_info->v4_pkt_conn;
    wds_info->v4_pkt_conn = TRUE;

    if (ind_data->xlat_capable_valid == TRUE &&
        ind_data->xlat_capable == TRUE)
    {
      wds_info->xlat_capable = TRUE;
      netmgr_log_low("Call on link %d is xlat capable", link);
    }
  }

  netmgr_log_low("V4 and V6 pkt_srvc_counts are : [%d] [%d]",
                 wds_info->v4_pkt_conn, wds_info->v6_pkt_conn);

  /*-------------------------------------------------------------------------
    For reconfiguraiton, capture the current address before its updated.
    Secondary PDU events may also need this information, so cache it if the
    indication says secondary events are valid.
  -------------------------------------------------------------------------*/
  if (ind_data->status.reconfiguration_required || ind_data->secondary_pdu_status_valid)
  {
    if (NETMGR_ADDRSET_MASK_INVALID != wds_info->addr_info.valid_mask)
    {
      /* Copy current address into buffer */
      cached_addr = netmgr_qmi_get_addr_set(&wds_info->addr_info,
                                            (WDS_IP_FAMILY_IPV4_V01 == ip_family) ?
                                            NETMGR_IPV4_ADDR : NETMGR_IPV6_ADDR,
                                            NETMGR_ADDR_TYPE_DEFAULT);
      memcpy(address_set_ptr, cached_addr, sizeof(*cached_addr));
    }
    else
    {
      netmgr_log_err("no previously cached address for link[%d]\n", link);
    }
    address_set_ptr->is_reconfig = ind_data->status.reconfiguration_required;
  }
  ...
  {
    wds_info->xlat_capable = ind_data->xlat_capable;
  }
  ...
  if (
      ...
      (NETMGR_SUCCESS != netmgr_qmi_get_modem_link_info( link, ip_family, PRIMARY_PDU ))
     )
  {
  ...
  if (!ind_data->status.reconfiguration_required &&
      /* Don't overwrite cache. Secondary PDU might need the older addresses */
      !ind_data->secondary_pdu_status_valid)
  {
    if (ip_family == WDS_IP_FAMILY_IPV4_V01)
    {
      netmgr_log_high("setting valid_mask for ip_family %d for link[%d]",ip_family,link);
      wds_info->addr_info.valid_mask |= NETMGR_ADDRSET_MASK_IPV4;
      cached_addr = netmgr_qmi_get_addr_set(&wds_info->addr_info, NETMGR_IPV4_ADDR,
                                            NETMGR_ADDR_TYPE_DEFAULT);
      cached_addr->if_addr.type = NETMGR_IPV4_ADDR;
    ...
    else if (ip_family == WDS_IP_FAMILY_IPV6_V01)
    {
      ...
      wds_info->addr_info.valid_mask |= NETMGR_ADDRSET_MASK_IPV6;
      cached_addr = netmgr_qmi_get_addr_set(&wds_info->addr_info, NETMGR_IPV6_ADDR,
                                            NETMGR_ADDR_TYPE_DEFAULT);
      cached_addr->if_addr.type = NETMGR_IPV6_ADDR;
...
#ifdef NETMGR_QOS_ENABLED
  if (new_conn && netmgr_main_get_qos_enabled())
  {
...
)
{
  ...
  netmgr_address_set_t * sec_address_set_ptr = NULL;
  netmgr_qmi_client_type_t clnt;
  qmi_client_type client_hndl;
  netmgr_nl_event_info_t * event_info = NULL;
  netmgr_wds_qmi_drv_info_t *wds_info;
  int ind_pdu_id = 0;
  int pdu_index = PRIMARY_PDU;
  ...
  /* SSC will use primary PDU's clnt hdl*/
  if (user_handle != client_hndl)
  {
  ...
  /* Ignore the indication if SSCM3 is not enabled and for Sec PDU */
  if (!netmgr_main_cfg.sscm3 && ind_data->secondary_pdu_status_valid) {
     netmgr_log_low("ignoring IP[%d] type pkt_srvc_ind on WDS handle[%p] for link[%d]\n",
                   ip_family,
                   user_handle,
                   link);
  ...
  if (ind_data->secondary_pdu_status_valid && netmgr_main_cfg.sscm3)
  {
    ind_pdu_id = ind_data->secondary_pdu_status.secondary_pdu_id;
    pdu_index = (!ind_pdu_id)? PRIMARY_PDU : SECONDARY_PDU;
  }
  ...
  switch (ind_data->status.connection_status)
  {
    case WDS_CONNECTION_STATUS_CONNECTED_V01:
    {
      ...
      netmgr_wds_legacy_pkt_srvc_connected(link, ip_family, clnt, address_set_ptr, ind_data);

      if (ind_data->secondary_pdu_status_valid && netmgr_main_cfg.sscm3)
      {
        sec_address_set_ptr = ds_malloc( sizeof(netmgr_address_set_t) );
        if( NULL == sec_address_set_ptr )
        {
          netmgr_log_err("failed to allocate address_set\n");
          return;
        }
        memset(sec_address_set_ptr, 0, sizeof(netmgr_address_set_t));
        netmgr_sscm3_wds_pkt_srvc_connected(link, ip_family, clnt,
                                            &netmgr_qmi_cfg.links[link],
                                            address_set_ptr,
                                            sec_address_set_ptr,
                                            ind_data);
      }
      ...
      /* This will either set it to NULL or to valid memory */
      address_set_ptr->sec_addr_info_ptr = sec_address_set_ptr;
      cmd->data.info.connect_msg.addr_info_ptr = address_set_ptr;

      /* If for Non-Primary PDU put it in the command */
      cmd->data.info.connect_msg.non_primary = ind_pdu_id;
      if (ind_pdu_id) {
        cmd->data.info.connect_msg.pdu_status = ind_data->secondary_pdu_status.secondary_pdu_status;
      }

      wds_info = __netmgr_qmi_get_link_wds_info_pdu(link, pdu_index);
      if (ind_data->tech_name_valid)
      {
        wds_info->tech_name = ind_data->tech_name;
      }
      else
      {
        wds_info->tech_name = WDS_TECHNOLOGY_NAME_ENUM_MAX_ENUM_VAL_V01;
      }
      ...
      event_info = netmgr_sscm3_event_notify(link, link_name, ip_family,
                                             netmgr_qmi_cfg.links[link].modem_subs_id,
                                             ind_data);
      if (event_info)
      {
        if (NETMGR_SUCCESS != netmgr_kif_send_event_msg(event_info))
        {
          netmgr_log_err("failed on kif_send_event PDU status\n");
        }
    ...
    case WDS_CONNECTION_STATUS_DISCONNECTED_V01:
    {
      ...
      wds_info = __netmgr_qmi_get_link_wds_info_pdu(link, PRIMARY_PDU);
      v4_addr = netmgr_qmi_get_addr_set(&wds_info->addr_info, NETMGR_IPV4_ADDR,
                                        NETMGR_ADDR_TYPE_DEFAULT);
      v6_addr = netmgr_qmi_get_addr_set(&wds_info->addr_info, NETMGR_IPV6_ADDR,
                                        NETMGR_ADDR_TYPE_DEFAULT);
      ...
      cmd->data.info.disconnect_msg.teardown_iface       =
        (NETMGR_ADDRSET_MASK_IPV4V6 != wds_info->addr_info.valid_mask);

      cmd->data.info.disconnect_msg.addr_info_ptr   =
        (WDS_IP_FAMILY_IPV6_V01 == ip_family) ?
        v6_addr : v4_addr;

      netmgr_sscm3_wds_pkt_srvc_disconnected_cmd(link, ip_family,
                                                 __netmgr_qmi_get_link_wds_info_pdu(link, SECONDARY_PDU),
                                                 cmd);
      /* Clear the address purge pending flags */
      v4_addr->is_addr_purge_pend = FALSE;
      v6_addr->is_addr_purge_pend = FALSE;

      v4_addr->if_addr.type = NETMGR_IPV4_ADDR;
      v6_addr->if_addr.type = NETMGR_IPV6_ADDR;
      ...
      if (FALSE == cmd->data.info.disconnect_msg.teardown_iface)
      {
        ...
        {
          v4_addr->is_addr_purge_pend = TRUE;
        }
        else
        {
          v6_addr->is_addr_purge_pend = TRUE;
        }
      }
      ...
      netmgr_log_low("V4 and V6 pkt_srvc_counts are : [%d] [%d]",
                     wds_info->v4_pkt_conn,
                     wds_info->v6_pkt_conn);
...
(
  uint8  link, unsigned int pdu_id
)
{
  qmi_client_type  client_hndl;
  wds_get_runtime_settings_resp_msg_v01  call_info;
  ...
  client_hndl = netmgr_qmi_wds_get_clnt_id(link, NETMGR_QMI_CLIENT_IPV4);
  wds_info = __netmgr_qmi_get_link_wds_info_pdu(link, pdu_id);
  mtu_ptr = &wds_info->mtu_ipv4;
           ...
           call_info.resp.error == QMI_ERR_OUT_OF_CALL_V01)
  {
    ...
    client_hndl = netmgr_qmi_wds_get_clnt_id(link, NETMGR_QMI_CLIENT_IPV6);
    mtu_ptr = &wds_info->mtu_ipv6;
...
)
{
  ...
  if (TRUE == ind_data->bearer_tech_ex_valid)
  {
    ...
    prev_bt = &wds_info->bearer_tech_ex.bt;
#endif /* FEATURE_DATA_IWLAN */

    netmgr_log_high("bearer_tech_ex_ind: link=%d technology=0x%x rat_mask=0x%08x so_mask=0x%016llx addr_mask 0x%02x\n",
                    link,
    ...
    if (NETMGR_QMI_IS_BEARER_TECH_EX_IWLAN(curr_bt) &&
        (wds_info->addr_info.valid_mask & NETMGR_ADDRSET_MASK_IPV6))
    {
      int rc = netmgr_kif_send_icmpv6_router_solicitation(link);

      if (rc == NETMGR_SUCCESS)
      {
        netmgr_log_low("%s(): sent router_solicitation on link %d", __func__, link);
      }
      else
      {
        netmgr_log_err("%s(): failed to send router_solicitation on link %d", __func__, link);
      }
    }
#endif
    ...
    if (TRUE != wds_info->bearer_tech_ex.is_valid)
    {
    ...
    else
    {
      ...
      {
  //TODO sscm3 CHECK MTU
         if (NETMGR_SUCCESS == netmgr_qmi_query_modem_mtu((uint8)link, 0 ))
         {
    ...
    *prev_bt = ind_data->bearer_tech_ex;
    wds_info->bearer_tech_ex.is_valid = TRUE;
#endif /* FEATURE_DATA_IWLAN */
  }
  ...
  else if (TRUE == ind_data->current_bearer_tech_valid)
  {
    ...
    if( NETMGR_SUCCESS !=
        netmgr_qmi_get_modem_link_info_mtu((uint8)link, WDS_IP_FAMILY_IPV4_V01, 0))
    {
...
)
{
  ...
  if( ind_data->changed_ip_config_valid &&
    ...
    else
    {
      ...
      /* sscm3 check for PDU info, No need to check cfg.sscm3 since this isnt sent by modem otherwise */
      if (ind_data->sec_pdu_id_valid)
      {
        event_info->pdu_id = ind_data->sec_pdu_id;
      }
      else
      {
        event_info->pdu_id = 0; /* primary PDU ID is always 0 */
      }
      event_info->param_mask |= NETMGR_EVT_PARAM_PDU_ID;

      event_info->addr_info.addr.ip_addr.ss_family = AF_INET6;
      /* set this to make dsi ignore the address itself */
      event_info->addr_info.ext_ip = 1;
...
/*Perform handling of MTU updates*/
     ...
     ind_data->changed_ip_config & QMI_WDS_MASK_REQ_SETTINGS_MTU_V01)
  {
    ...
    if (ind_data->sec_pdu_id_valid)
      pdu_id = ind_data->sec_pdu_id;

    if (user_handle == netmgr_qmi_wds_get_clnt_id(link, NETMGR_QMI_CLIENT_IPV4))
    {
      netmgr_qmi_get_modem_link_info_mtu((uint8)link, WDS_IP_FAMILY_IPV4_V01, pdu_id);
      netmgr_log_low("Get runtime setting for ipv4 clients");
    }
    else if (user_handle == netmgr_qmi_wds_get_clnt_id(link, NETMGR_QMI_CLIENT_IPV6))
    {
      netmgr_qmi_get_modem_link_info_mtu((uint8)link, WDS_IP_FAMILY_IPV6_V01, pdu_id);
...
)
{
  ...
  netmgr_address_set_t  *tmp_addr = NULL;
  ...
  if (netmgr_main_cfg.oemprxy_enabled)
  {
    if (WDS_TECHNOLOGY_NAME_IWLAN_S2B_PROXY_V01 != rev_ip_ind->tech_name)
    {
      netmgr_log_err("netmgr_wds_handle_rev_ip_connect_ind: S2B proxy tech type not received,"
                     " unexpected technology=0x%x", rev_ip_ind->tech_name);
      goto bail;
    }
  }
  else
  {
    if (WDS_TECHNOLOGY_NAME_WLAN_LOCAL_BRKOUT_V01 != rev_ip_ind->tech_name &&
        WDS_TECHNOLOGY_NAME_IWLAN_S2B_V01 != rev_ip_ind->tech_name &&
        WDS_TECHNOLOGY_NAME_CIWLAN_V01 != rev_ip_ind->tech_name)
    {
  ...
  wds_info_ptr = netmgr_qmi_get_link_wds_info(link);
  if (!wds_info_ptr)
  {
    netmgr_log_err("invalid link %d\n", link);
    goto bail;
  ...
  if (rev_ip_ind->ipv4_addr_valid)
  {
    wds_info_ptr->v4_pkt_conn = TRUE;
    addr_info_ptr->valid_mask |= NETMGR_ADDRSET_MASK_IPV4;
    tmp_addr = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV4_ADDR,
                                       NETMGR_ADDR_TYPE_DEFAULT);
    tmp_addr->if_addr.type = NETMGR_IPV4_ADDR;
    tmp_addr->if_addr.addr.v4 = htonl((uint32_t)rev_ip_ind->ipv4_addr);
    tmp_addr->sa.is_sa_valid = FALSE;
    tmp_addr->sa.is_sa_shared = FALSE;
    tmp_addr->sa.ts_mode = NETMGR_TS_MODE_DISABLED;

    if (rev_ip_ind->is_shared_valid)
    {
      tmp_addr->sa.is_sa_shared = rev_ip_ind->is_shared;
    }

    NETMGR_LOG_IPV4_ADDR( med, "Modem iface", tmp_addr->if_addr.addr.v4 );

    memcpy(addr_set_ptr, tmp_addr, sizeof(*tmp_addr));
  }
  else if (rev_ip_ind->ipv6_addr_valid)
  {
    wds_info_ptr->v6_pkt_conn = TRUE;
    addr_info_ptr->valid_mask |= NETMGR_ADDRSET_MASK_IPV6;
    tmp_addr = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV6_ADDR,
                                       NETMGR_ADDR_TYPE_DEFAULT);
    tmp_addr->if_addr.type = NETMGR_IPV6_ADDR;
    tmp_addr->if_mask = (unsigned int)rev_ip_ind->ipv6_addr.ipv6_prefix_length;
    memcpy(tmp_addr->if_addr.addr.v6_addr8,
           rev_ip_ind->ipv6_addr.ipv6_addr,
           sizeof(tmp_addr->if_addr.addr.v6_addr8));
    ...
    NETMGR_LOG_IPV6_ADDR( med, "Modem iface",
                          tmp_addr->if_addr.addr.v6_addr64 );

    memcpy(addr_set_ptr, tmp_addr, sizeof(*tmp_addr));
  }

  if (NETMGR_MAIN_CFG_IS_IWLAN_CONCURRENCY_SET() &&
      WDS_TECHNOLOGY_NAME_WLAN_LOCAL_BRKOUT_V01 == rev_ip_ind->tech_name)
  {
    struct in_addr addr;
    struct in6_addr addr6;
    char   ipaddr[NETMGR_MAX_STR_LENGTH];
    ...
    if ( rev_ip_ind->ipv4_addr_valid )
    {
      ...
      memcpy( (char*)&addr.s_addr,
                    (char*)&tmp_addr->if_addr.addr.v4,
                    sizeof(addr.s_addr) );
      if( NULL == inet_ntop( AF_INET, &addr, ipaddr, INET_ADDRSTRLEN ) ) {
        netmgr_log_sys_err("error on inet_ntop():\n");
      }
      netmgr_kif_save_wlan_ifname_to_link(link, ipaddr, AF_INET);
    }
    else if ( rev_ip_ind->ipv6_addr_valid )
    {
      /* Convert address to string notation */
      ...
      tmp_addr = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV6_ADDR,
                                         NETMGR_ADDR_TYPE_DEFAULT);
      memcpy( (char*)&addr6.s6_addr,
                    (char *)&tmp_addr->if_addr.addr.v6_addr8,
                    sizeof(struct in6_addr) );
      if( NULL == inet_ntop( AF_INET6, &addr6, ipaddr, INET6_ADDRSTRLEN ) ) {
        netmgr_log_sys_err("error on inet_ntop():\n");
      }
      netmgr_kif_save_wlan_ifname_to_link(link, ipaddr, AF_INET6);
    }
  }

  netmgr_log_low("V4 and V6 pkt_srvc_counts are : [%d] [%d]",
                 wds_info_ptr->v4_pkt_conn,
                 wds_info_ptr->v6_pkt_conn);
  ...
  cmd->data.info.connect_msg.txn_id = rev_ip_ind->transaction_id;
...
)
{
   ...
   * indication will then teardown interface as normal.
   */
  GET_TEARDOWN_IFACE_VAL(clnt, wds_info->addr_info.valid_mask, tiface);
  cmd->data.info.disconnect_msg.teardown_iface = tiface;
  cmd->data.info.disconnect_msg.addr_info_ptr   =
    (NETMGR_QMI_CLIENT_IPV6 == clnt)?
    v6_addr :
    v4_addr;
  cmd->data.info.disconnect_msg.txn_id = rev_ip_ind->transaction_id;

  /*Explicitly set IP family as modem doesnt give it*/
  v4_addr->if_addr.type = NETMGR_IPV4_ADDR;
  v6_addr->if_addr.type = NETMGR_IPV6_ADDR;
  ...
  /* Clear disconnected address from cached valid mask if specified */
  wds_info->addr_info.valid_mask &= (NETMGR_QMI_CLIENT_IPV4 == clnt) ?
...
)
{
  ...
  cmd->data.link = link;
  cmd->data.info.qos_flow.flow_id =
            (pdu == PRIMARY_PDU ? 0 : NETMGR_SEC_PDU_DEFAULT_FLOW_ID);
  cmd->data.info.qos_flow.ip_ver  = ip_ver;
  cmd->data.info.qos_flow.bearer_id = bearer_id;
  cmd->data.info.qos_flow.state = NETMGR_TC_FLOW_REBIND;
...
)
{
  ...
  case QMI_WDS_REBIND_DEFAULT_FLOW_INFO_IND_V01:
#ifdef NETMGR_QOS_ENABLED
    {
      ...
      int pdu = PRIMARY_PDU;
      if (netmgr_qmi_wds_ind->ind_data.rebind_ind.sec_pdu_id_valid)
      {
        pdu = SECONDARY_PDU;
      }
...
)
{
  ...
  cmd->data.info.qos_flow.state = NETMGR_TC_FLOW_ACTIVATE;
...
)
{
  ...
  cmd->data.info.qos_flow.priority = priority;
  cmd->data.info.qos_flow.datarate = datarate;
  cmd->data.info.qos_flow.state = NETMGR_TC_FLOW_MODIFY;
...
)
{
  ...
  cmd->data.info.qos_flow.flow_type = (uint8)((NETMGR_QOS_EVENT_REPORT_FLOW_INFO_FLOW_TYPE_PARAM &
                                       flow_ptr->param_mask)?
                                      flow_ptr->flow_type : NETMGR_FLOW_TYPE_INVALID);
  cmd->data.info.qos_flow.state = NETMGR_TC_FLOW_DELETE;
...
)
{
  ...
  cmd->data.info.qos_flow.priority  = priority;
  cmd->data.info.qos_flow.datarate  = datarate;
  cmd->data.info.qos_flow.state = NETMGR_TC_FLOW_SUSPENDED;
...
)
{
  ...
  clnt_hndl = netmgr_qmi_wds_get_clnt_id(0, NETMGR_QMI_CLIENT_IPV4);
...
/* TODO: Implement qmi_wds_indication_register on off-target then remove this check */
  ...
  if (
      ...
      /* enable if any variant of iwlan is configured from xml */
      ((netmgr_main_get_iwlan_enabled() == IWLAN_ENABLE ||
        netmgr_main_get_iwlan_enabled() == IWLAN_V2_ENABLE) ||
       netmgr_main_cfg.xlat_txcsum_disabled == TRUE)
#else
      netmgr_main_cfg.xlat_txcsum_disabled == TRUE
#endif /* FEATURE_DATA_IWLAN */
...
)
{
    ...
    (request->dl_data_aggregation_protocol == response->dl_data_aggregation_protocol))
  {
    ...
    {
      /* QMAP format and demuxing are enabled by default */
    }
    else
    {
    ...
    (request->ul_data_aggregation_protocol == response->ul_data_aggregation_protocol))
  {
    ...
    {
      rmnet_flags |= NETMGR_RMNET_EGRESS_MAP_CKSUMV3_FLAG;
    }
    else if (request->ul_data_aggregation_protocol == WDA_UL_DATA_AGG_QMAP_ENABLED_V01)
    {
      /* QMAP format and muxing are enabled by default */
    }
    else
    {
  ...
  if (netmgr_kif_set_data_format((const char *)netmgr_main_get_phys_net_dev(), rmnet_flags)
    != NETMGR_SUCCESS)
  {
...
netmgr_qmi_service_init( int link, netmgr_data_format_t data_format )
{
  ...
  if (NETMGR_QMI_CLIENT_INVALID != netmgr_qmi_wds_get_clnt_id(link, NETMGR_QMI_CLIENT_IPV4))
  {
    ...
    netmgr_log_high("The wds client ids [%p/%p]\n",
                    netmgr_qmi_wds_get_clnt_id(link, NETMGR_QMI_CLIENT_IPV4),
                    netmgr_qmi_wds_get_clnt_id(link, NETMGR_QMI_CLIENT_IPV6));
  }
...
error:
  ...
  if (NETMGR_QMI_CLIENT_INVALID != netmgr_qmi_wds_get_clnt_id(link, NETMGR_QMI_CLIENT_IPV6))
  {
    netmgr_qmi_wds_release_client( link,
                                   netmgr_qmi_wds_get_clnt_id(link, NETMGR_QMI_CLIENT_IPV6) );
...
/*===========================================================================
  FUNCTION  netmgr_qmi_get_port_config
===========================================================================*/
netmgr_ctl_port_config_type *netmgr_qmi_get_port_config(int link)
{
  if (NETMGR_SUCCESS != netmgr_qmi_verify_link(link))
  {
    return NULL;
  ...
  return &netmgr_qmi_cfg.link_array[link];
}

netmgr_link_info_t *__netmgr_qmi_get_link_info(int link)
{
  return &netmgr_qmi_cfg.links[link];
}

/*===========================================================================
  FUNCTION  netmgr_qmi_get_link_info
===========================================================================*/
netmgr_link_info_t *netmgr_qmi_get_link_info(int link)
{
  if (NETMGR_SUCCESS != netmgr_qmi_verify_link(link))
  {
    return NULL;
  ...
  return __netmgr_qmi_get_link_info(link);
}

/*===========================================================================
  FUNCTION netmgr_qmi_get_link_wds_info
===========================================================================*/
netmgr_wds_qmi_drv_info_t *netmgr_qmi_get_link_wds_info(int link)
{
  return netmgr_qmi_get_link_wds_info_pdu(link, PRIMARY_PDU);
}

/*===========================================================================
  FUNCTION  netmgr_qmi_get_link_wds_info_pdu
===========================================================================*/
netmgr_wds_qmi_drv_info_t *netmgr_qmi_get_link_wds_info_pdu(int link, int pdu_cnf)
{
  if (NETMGR_SUCCESS != netmgr_qmi_verify_link(link) ||
      pdu_cnf < PRIMARY_PDU || pdu_cnf >= MAX_PDU)
  {
    return NULL;
  }

  return __netmgr_qmi_get_link_wds_info_pdu(link, pdu_cnf);
}

/*===========================================================================
  FUNCTION  netmgr_qmi_get_link_mtu_by_family
===========================================================================*/
/*!
@brief
  Retrieves the MTU of the provided ip family for a given link based off of
  the global netmgr_qmi_cfg struct. Ip_fam parameter
  must be either NETMGR_IPV4_ADDR or NETMGR_IPV6_ADDR.

@return
  unsigned int - ip family specific mtu value for given link
...
*/
/*=========================================================================*/
unsigned int netmgr_qmi_get_link_mtu_by_family(int link, int ip_fam)
{
  netmgr_wds_qmi_drv_info_t *wds_info;
  unsigned int mtu;

  if (link < NETMGR_MAX_LINKS_PER_MODEM_EXT) {
    wds_info = __netmgr_qmi_get_link_wds_info_pdu(link, PRIMARY_PDU);
    if (ip_fam == NETMGR_IPV4_ADDR)
      mtu = wds_info->mtu_ipv4;
    else if (ip_fam == NETMGR_IPV6_ADDR)
      mtu = wds_info->mtu_ipv6;
    else {
...
netmgr_ip_addr_t netmgr_qmi_get_ip_addr_type_first_conn_clnt( int link )
{
  ...
  if( NETMGR_SUCCESS == netmgr_qmi_verify_link( link ) )
  {
    ...
    wds_info = __netmgr_qmi_get_link_wds_info_pdu(link, PRIMARY_PDU);
    switch (wds_info->first_conn_clnt)
    {
...
/*===========================================================================
  FUNCTION  netmgr_qmi_get_first_conn_clnt_from_ip_family_type
===========================================================================*/
/*!
@brief
  Retrives clnt type for IP Address

@return
  netmgr_qmi_client_type_t - QMI Client type(NETMGR_QMI_CLIENT_IPV4/NETMGR_QMI_CLIENT_IPV6),
                             NETMGR_QMI_CLIENT_MAX if undefined
...
*/
/*=========================================================================*/
netmgr_qmi_client_type_t netmgr_qmi_get_first_conn_clnt_from_ip_family_type( netmgr_ip_addr_t family )
{
  netmgr_qmi_client_type_t rc = NETMGR_QMI_CLIENT_MAX;

  switch( family )
  {
    case NETMGR_IPV4_ADDR:
      rc = NETMGR_QMI_CLIENT_IPV4 ;
      break;
    case NETMGR_IPV6_ADDR:
      rc = NETMGR_QMI_CLIENT_IPV6;
      break;
    default:
      netmgr_log_err( "%s(): unsupported value[%d]\n", __func__, family);
      break;
  }
  return rc;
}
...
/*===========================================================================
  FUNCTION  netmgr_qmi_update_first_conn_clnt
===========================================================================*/
/*!
@brief
  Update first_conn_clnt field in QMI WDS info.

@return
  NETMGR_SUCCESS - Upon success,
  NETGMR_FAILURE - Upon failure
...
*/
/*=========================================================================*/
int netmgr_qmi_update_first_conn_clnt( int link,  netmgr_qmi_client_type_t clnt)
{
  netmgr_wds_qmi_drv_info_t *wds_info_ptr = NULL;
  int rc = NETMGR_FAILURE;

  wds_info_ptr = netmgr_qmi_get_link_wds_info(link);
  if(NULL != wds_info_ptr)
  {
    wds_info_ptr->first_conn_clnt = clnt;
    netmgr_log_high("%s(): Updated first_conn_clnt to [%d] link [%d]\n",
                     __func__,
                     clnt,
                     link);
    rc = NETMGR_SUCCESS;
  ...
  {
    netmgr_log_err("%s(): Unable to fetch wds_info for link [%d]\n",
                    __func__,
                    link);
  rc = NETMGR_FAILURE;
  }
...
*/
/*=========================================================================*/
netmgr_address_info_t * netmgr_qmi_get_addr_info( int link, int pdu_cnf )
{
  netmgr_wds_qmi_drv_info_t *wds_info;
  ...
  return &wds_info->addr_info;
}

/*===========================================================================
  FUNCTION  netmgr_qmi_get_addr_set
===========================================================================*/
/*!
@brief
  Retrives the network address from the given address info of the specified
  family and type

@return
  netmgr_address_set_t * - IP address struct pointer,
                            NULL if undefined
...
*/
/*=========================================================================*/
netmgr_address_set_t *netmgr_qmi_get_addr_set
(
...
{
  if (NETMGR_IPV4_ADDR == addr_family)
  {
    return &addr_info->ipv4;
  }

  if (NETMGR_IPV6_ADDR == addr_family)
  {
    if (addr_type < NETMGR_ADDR_TYPE_MAX)
    {
      return &addr_info->ipv6[addr_type];
    }
  }
...
)
{
  ...
  struct timespec ts_current, ts_diff;
  ...
  {
    link_info = __netmgr_qmi_get_link_info(i);
    wds_info = __netmgr_qmi_get_link_wds_info_pdu(i, PRIMARY_PDU);
    netmgr_log_err("%s() State %d Link %d in_use %d", __func__,
    (int)stm_get_state(&NETMGR_SM[i]), i, wds_info->in_use);

    clock_gettime(CLOCK_MONOTONIC, &ts_current);
    ts_diff = ds_time_diff(ts_current, link_info->last_cleared_ts);
    netmgr_log_low("%s(): time since interface was cleared %u.%06u",
                   __func__, (uint32_t)ts_diff.tv_sec, (uint32_t)(ts_diff.tv_nsec / 1000));
        ...
        wds_info->in_use == FALSE)
    {
      if((netmgr_qmi_port_in_call(i, modem_subs_id) == NETMGR_SUCCESS) &&
         ((last_cleared_iface < netmgr_main_cfg.max_links_per_modem) &&
          (last_cleared_iface != (int)i) && (ts_diff.tv_sec > 4)))
        break;
      else
        continue;
    }
  ...
  {
    link_info = __netmgr_qmi_get_link_info(last_cleared_iface);
    wds_info = __netmgr_qmi_get_link_wds_info_pdu(last_cleared_iface,
                                                  PRIMARY_PDU);
       ...
       ( wds_info->in_use == FALSE ))
    {
      clock_gettime(CLOCK_MONOTONIC, &ts_current);
      ts_diff = ds_time_diff(ts_current, link_info->last_cleared_ts);
      netmgr_log_low("%s(): time since last interface was cleared %u.%06u",
                     __func__, (uint32_t)ts_diff.tv_sec, (uint32_t)(ts_diff.tv_nsec / 1000));

      if (ts_diff.tv_sec > 4)
      {
        selected_iface = last_cleared_iface;
        wds_info->in_use = TRUE;
        netmgr_log_med("selecting only available interface, cleared last: %d\n",last_cleared_iface);
      }
      else
      {
        netmgr_log_err("%s(): No available interface released within last 4 seconds", __func__);
      }
    }
    else
    {
  ...
  {
    link_info = __netmgr_qmi_get_link_info(selected_iface);
    link_info->modem_subs_id = modem_subs_id;
  }
...
)
{
  ...
  for (i = iface_start; i < iface_end; ++i)
  {
    ...
    clock_gettime(CLOCK_MONOTONIC, &ts_current);
    ts_diff = ds_time_diff(ts_current, link_info->last_cleared_ts);
    netmgr_log_low("%s(): time since interface %d was cleared %u.%06u",
                   __func__, i, (uint32_t)ts_diff.tv_sec, (uint32_t)(ts_diff.tv_nsec / 1000));
  ...
  else
  {
    ...
    wds_info = __netmgr_qmi_get_link_wds_info_pdu(selected_iface, PRIMARY_PDU);
    wds_info->in_use = TRUE;
  }
...
*/
/*=========================================================================*/
int netmgr_qmi_find_available_rev_iface(void)
{
  unsigned int i;
  int selected_iface = NETMGR_INVALID_IFACE;
  int dynamic_iface = NETMGR_INVALID_IFACE;
  ...
  for (i = iface_start; i < iface_end; ++i)
  {
    ...
    if (NETMGR_STATE_INITED == (int)stm_get_state(&NETMGR_SM[i]) ||
        ...
        NETMGR_STATE_DOWN == (int)stm_get_state(&NETMGR_SM[i]))
    {
      if (available_iface == NETMGR_INVALID_IFACE) {
        available_iface = i;
        netmgr_log_med("%s() Available iface set to %d", __func__, available_iface);
...
)
{
 ...
 if(NETMGR_SUCCESS != netmgr_qmi_wds_set_bind_subscription(netmgr_route_look_up_hndl,
  ...
  {
    if (rsp_data.qmi_inst > netmgr_main_cfg.num_data_ports)
    {
      dsi_rsp_data->is_qmi_inst_valid = FALSE;
      dsi_rsp_data->qmi_inst = NETMGR_INVALID_IFACE;
      dsi_rsp_data->is_companion_ip = FALSE;
      netmgr_log_err("%s() Valid instance outside expected range found in Route look up %d",
                     __func__, dsi_rsp_data->qmi_inst);
    }
    else
    {
...
/*===========================================================================
  FUNCTION  netmgr_qmi_check_matching_call
===========================================================================*/
/*!
@brief
  Check for active calls matching the provided params
...
*/
/*=========================================================================*/
int netmgr_qmi_check_matching_call
(
  netmgr_qmi_wds_route_look_up_params_type   *params,
  netmgr_qmi_matching_call_check_resp_t      *matching_call_resp,
  int                                        modem_subs_id
)
{
  int ret = NETMGR_FAILURE;
  ...
  if (WDS_IP_FAMILY_PREF_IPV4_V01 == params->ip_family_pref
       || WDS_IP_FAMILY_PREF_IPV6_V01 == params->ip_family_pref)
  {
    rc = netmgr_qmi_wds_route_look_up(netmgr_route_look_up_hndl, params, &rsp_data);
    if (NETMGR_SUCCESS != rc)
    {
      netmgr_log_err("%s() Route look up failed ", __func__);
      goto bail;
    ...
    /* Valid QMI instance which means there is a matching call on up on modem */
    if (rsp_data.qmi_inst_is_valid)
    {
      netmgr_log_med("%s() Valid instance found in Route look up %d", __func__, rsp_data.qmi_inst);

      /* Check if interface is actually UP in NETMGR */
      if (NETMGR_STATE_UP != (int) stm_get_state(&NETMGR_SM[rsp_data.qmi_inst]))
      {
        netmgr_log_err("%s(): interface not in UP state, matching call does not exist yet",
                       __func__);
        goto bail;
      }

      matching_call_resp->device_name_valid = TRUE;
      (void) strlcpy(matching_call_resp->device_name,
                     netmgr_kif_get_name(rsp_data.qmi_inst),
                     (NETMGR_IF_NAME_MAX_LEN + 1));
    }
  }
  else if (WDS_IP_FAMILY_PREF_UNSPEC_V01 == params->ip_family_pref)
  {
    /* Need to do route lookup on both families. If both return valid instances then we can
       return the matching iface. Otherwise we return failure */

    /* Update pparams IP pref to IPv4 */
    params->ip_family_pref = WDS_IP_FAMILY_PREF_IPV4_V01;
    rc = netmgr_qmi_wds_route_look_up(netmgr_route_look_up_hndl, params, &rsp_data);
    if (NETMGR_SUCCESS != rc)
    {
      netmgr_log_err("%s() Route look up failed for IPv4", __func__);
      goto bail;
    }
    ...
    if (rsp_data.qmi_inst_is_valid)
    {
      netmgr_log_med("%s() Valid instance found in Route look up %d for IPv4, checking for IPv6",
                     __func__, rsp_data.qmi_inst);

      /* Update pparams IP pref to IPv6 */
      params->ip_family_pref = WDS_IP_FAMILY_PREF_IPV6_V01;
      rc = netmgr_qmi_wds_route_look_up(netmgr_route_look_up_hndl, params, &rsp_data);
      if (NETMGR_SUCCESS != rc)
      {
        netmgr_log_err("%s() Route look up failed for IPv6", __func__);
        goto bail;
      }

      if (rsp_data.qmi_inst_is_valid)
      {
        netmgr_log_med("%s() Valid instance found in Route look up %d for IPv6",
                       __func__, rsp_data.qmi_inst);
...
*/
/*=========================================================================*/
unsigned int netmgr_qmi_get_mtu( int link, unsigned int type , unsigned int pdu_id)
{
  netmgr_wds_qmi_drv_info_t *wds_info;

  wds_info = netmgr_qmi_get_link_wds_info_pdu(link, pdu_id);
  if (wds_info)
  {
    switch (type) {
      case QMI_IP_FAMILY_PREF_IPV4:
        return (unsigned int)wds_info->mtu_ipv4;

      case QMI_IP_FAMILY_PREF_IPV6:
        return (unsigned int)wds_info->mtu_ipv6;
...
)
{
  ...
  if (wds_info)
  {
    ...
    if (wds_info->apn_type_mask & WDS_APN_TYPE_MASK_DEFAULT_V01)
    {
      clock_gettime(CLOCK_MONOTONIC, &link_info->last_cleared_ts);
      netmgr_log_low("%s(): setting last_cleared_iface_ts link %d APN Type Mask: 0x%x",
                     __func__, link, wds_info->apn_type_mask);
    }
    ...
    memset(&wds_info->rev_ip_config_status, 0, sizeof(wds_info->rev_ip_config_status));
    memset(&wds_info->bearer_tech_ex, 0, sizeof(wds_info->bearer_tech_ex));
#endif /* FEATURE_DATA_IWLAN */
  }
}

/*===========================================================================
  FUNCTION  netmgr_qmi_reset_link_wds_data_by_family
===========================================================================*/
/*!
@brief
  Reset the QMI WDS state information associated with the given link for a
  given family
...
/*=========================================================================*/
void
netmgr_qmi_reset_link_wds_data_by_family
(
  int link,
  unsigned pdu_cnf,
  int ip_family
)
{
  ...
  wds_info = netmgr_qmi_get_link_wds_info_pdu(link, pdu_cnf);
  if (!wds_info ||
      (NETMGR_IPV4_ADDR != ip_family && NETMGR_IPV6_ADDR != ip_family))
  {
    netmgr_log_err("%s(): invalid parameters\n", __func__);
    return;
  }

  /* This only matters if both address families are up. Otherwise, just wipe
   * out everything.
   */
  if (!wds_info->v4_pkt_conn || !wds_info->v6_pkt_conn)
  {
    netmgr_qmi_reset_link_wds_data(link, pdu_cnf);
    return;
  }

  if (NETMGR_IPV4_ADDR == ip_family)
  {
    addr_set = netmgr_qmi_get_addr_set(&wds_info->addr_info, NETMGR_IPV4_ADDR,
                                       NETMGR_ADDR_TYPE_DEFAULT);
    memset(addr_set, 0, sizeof(*addr_set));
    wds_info->mtu_ipv4 = NETMGR_MTU_INVALID;
    wds_info->first_conn_clnt = NETMGR_QMI_CLIENT_IPV6;
    wds_info->v4_pkt_conn = FALSE;
#ifdef FEATURE_DATA_IWLAN
    netmgr_util_circ_list_destroy(&wds_info->rev_ip_txns[NETMGR_QMI_CLIENT_IPV4]);
#endif
  ...
  {
    addr_set = netmgr_qmi_get_addr_set(&wds_info->addr_info, NETMGR_IPV6_ADDR,
                                       NETMGR_ADDR_TYPE_DEFAULT);
    memset(addr_set, 0, sizeof(*addr_set));
    wds_info->mtu_ipv6 = NETMGR_MTU_INVALID;
    wds_info->first_conn_clnt = NETMGR_QMI_CLIENT_IPV4;
    wds_info->v6_pkt_conn = FALSE;
#ifdef FEATURE_DATA_IWLAN
    netmgr_util_circ_list_destroy(&wds_info->rev_ip_txns[NETMGR_QMI_CLIENT_IPV6]);
#endif
...
)
{
  ...
  if (!(clnt_hndl = netmgr_qmi_wds_get_clnt_id(link, clnt)))
  {
  ...
  if (NETMGR_SUCCESS != netmgr_qmi_wds_util_deq_txn(clnt, link, &txn_id))
  {
    ...
    /*Check whether caller sent a specific txn_id to be used*/
    if(ctxn_id != NETMGR_INVALID_TXN_ID)
    {
      netmgr_log_high("%s():Using txn_id sent from caller [%d]\n", __func__, ctxn_id);
      txn_id = ctxn_id;
    }
    else
    {
      netmgr_log_err("%s(): No valid txn id available either in list nor passed by caller\n",__func__);
      goto bail;
    }
  }
  ...
  /* If the result is not NETMGR_WDS_REV_IP_CONFIG_COMPLETE_FAILURE, then
   * rules/routes are installed/cleared from netmgr, based on the mode.
   * set/clear the wlan iface information irrespective of qmi message status.
   */
  if (res != NETMGR_WDS_REV_IP_CONFIG_COMPLETE_FAILURE )
  {
      (NETMGR_QMI_IWLAN_CALL_CLEANUP == mode ) ?
      netmgr_kif_clear_wlan_ifname_on_link(link, ip_family) :
      netmgr_kif_set_wlan_ifname_addrmask(link, ip_family);
  }
...
)
{
  ...
  if (NETMGR_LINK_MAX != (cur_assoc = netmgr_qmi_iwlan_get_link_assoc(link)))
  {
        ...
        NETMGR_STATE_GOING_DOWN == stm_get_state(&NETMGR_SM[cur_assoc]))
    {
      netmgr_log_med("netmgr_qmi_iwlan_update_link_assoc: updating link=%d, bringing down prior associated rev_link=%d\n",
                     link,
                     cur_assoc);
      netmgr_kif_iface_close((uint8)cur_assoc, NULL, TRUE, FALSE);
    }
  }
  ...
  v4_addr = netmgr_qmi_get_addr_set(addr_info, NETMGR_IPV4_ADDR,
                                    NETMGR_ADDR_TYPE_DEFAULT);
  v6_addr = netmgr_qmi_get_addr_set(addr_info, NETMGR_IPV6_ADDR,
                                    NETMGR_ADDR_TYPE_DEFAULT);
  netmgr_log_med("netmgr_qmi_iwlan_update_link_assoc: finding assoc for link=%d [v4=0x%x, v6=0x%llx], type=%d, modem=%d in [%d, %d]\n",
                 link,
                 (addr_info->valid_mask & NETMGR_ADDRSET_MASK_IPV4) ? v4_addr->if_addr.addr.v4 : 0u,
                 (addr_info->valid_mask & NETMGR_ADDRSET_MASK_IPV6) ? v6_addr->if_addr.addr.v6_addr64[0] : 0llu,
                 type,
                 i,
                 start,
                 end);
  ...
  for (j = start; j <= end; ++j)
  {
    /* Iwlan is always on primary */
    tmp_addr_info = netmgr_qmi_get_addr_info(j, PRIMARY_PDU);
    ...
    {
      tmp_addr = netmgr_qmi_get_addr_set(tmp_addr_info, NETMGR_IPV4_ADDR,
                                         NETMGR_ADDR_TYPE_DEFAULT);
      netmgr_log_med("netmgr_qmi_iwlan_update_link_assoc: valid V4 addr link=%d [0x%x], link=%d [0x%x]\n",
                     link,
                     v4_addr->if_addr.addr.v4,
                     j,
                     tmp_addr->if_addr.addr.v4);


      /* If a matching IPv4 address is found */
      if (v4_addr->if_addr.addr.v4 == tmp_addr->if_addr.addr.v4)
      {
        break;
      }
    }
    ...
    {
      tmp_addr = netmgr_qmi_get_addr_set(tmp_addr_info, NETMGR_IPV6_ADDR,
                                         NETMGR_ADDR_TYPE_DEFAULT);
      netmgr_log_med("netmgr_qmi_iwlan_update_link_assoc: valid V6 addr link=%d [0x%llx], link=%d [0x%llx]\n",
                     link,
                     (unsigned long long)v6_addr->if_addr.addr.v6_addr64[0],
                     j,
                     (unsigned long long)tmp_addr->if_addr.addr.v6_addr64[0]);

      /* If a matching IPv6 prefix is found */
      if (v6_addr->if_addr.addr.v6_addr64[0] == tmp_addr->if_addr.addr.v6_addr64[0])
      {
        break;
      }
    }
...
)
{
  ...
  {
    link_info = __netmgr_qmi_get_link_info(fwd_link);
    link_info->assoc_link = NETMGR_LINK_MAX;
  ...
  {
    link_info = __netmgr_qmi_get_link_info(rev_link);
    link_info->assoc_link = NETMGR_LINK_MAX;
...
)
{
  ...
  link_info = __netmgr_qmi_get_link_info(link);
  ret = link_info->assoc_link;
...
)
{
  ...
  wds_info = __netmgr_qmi_get_link_wds_info_pdu(link, PRIMARY_PDU);
  wds_info->rev_ip_config_status.ip_family = ip_family;
  wds_info->rev_ip_config_status.status = status;

  /* Set the state info as valid */
  wds_info->rev_ip_config_status.is_valid = TRUE;
...
)
{
  ...
  wds_info = __netmgr_qmi_get_link_wds_info_pdu(link, PRIMARY_PDU);
  if (FALSE == wds_info->rev_ip_config_status.is_valid)
  {
    netmgr_log_med("netmgr_qmi_retrieve_rev_ip_config_status: invalid state info\n");
    goto bail;
  }

  *ip_family = wds_info->rev_ip_config_status.ip_family;
  *status = wds_info->rev_ip_config_status.status;

  /* Invalidate the status info upon retrieval */
  wds_info->rev_ip_config_status.is_valid = FALSE;
...
static void *netmgr_qmi_rev_ip_ind_reg_all(void *arg)
{
  ...
  if (netmgr_main_cfg.oemprxy_enabled)
  {
    if (TRUE != netmgr_main_is_oemprxy_plugin_enabled())
    {
      /* We need to register to reverse IP inidications if one of oemprxy or modem proxy
         is initialized */
      netmgr_log_err("%s(): oemprxy plugin not initialized" , __func__);
      return 0;
    }
  }
  else if (((netmgr_main_cfg.iwlan_enabled == IWLAN_ENABLE ||
             netmgr_main_cfg.iwlan_enabled == NSWO_ONLY) &&
             !nm_mdmprxy_is_initialized()) ||
             ((netmgr_main_cfg.iwlan_enabled == IWLAN_V2_ENABLE ||
               netmgr_main_cfg.iwlan_enabled == NSWO_V2_ONLY) &&
               !netmgr_modem_proxy_is_initialized()))
  {
...
/*===========================================================================
  FUNCTION  netmgr_qmi_query_ipsec_sa_config_ex
===========================================================================*/
/*!
@brief
  Query the extended Security Associations (SA) for the given family and link
...
/*=========================================================================*/
int
netmgr_qmi_query_ipsec_sa_config_ex
(
  int                                             ip_family,
  int                                             link,
...
)
{
  ...
  if (AF_INET != ip_family && AF_INET6 != ip_family)
  {
    netmgr_log_err("%s(): invalid ip_family=%d\n", __func__, ip_family);
    goto bail;
  }
  ...
  if (!(clnt_hndl = netmgr_qmi_wds_get_clnt_id(link, clnt)))
  {
    netmgr_log_err("%s(): invalid clnt_hndl=%p, link=%d, ip_family=%d\n",
                   __func__, clnt_hndl, link, clnt);
    goto bail;
  }

  if (NETMGR_SUCCESS !=
      (rc = netmgr_qmi_wds_get_ipsec_static_sa_config_ex(link, clnt_hndl, sa_config_ex_resp)))
  {
    netmgr_log_err("%s(): SA query failed rc=%d\n", __func__, rc);
    goto bail;
  }
...
)
{
  ...
  if (!wds_info)
  {
    netmgr_log_med("%s() Link %d has no WDS info. Skipping", __func__, link);
    return;
  }

  if (wds_info->v4_pkt_conn == TRUE)
  {
    netmgr_qmi_configure_data_coalescing(link, WDS_IP_FAMILY_IPV4_V01, coal, coal);
    goto set_gro_hw;
  }

  if (wds_info->v6_pkt_conn == TRUE)
  {
...
/*===========================================================================
  FUNCTION  netmgr_qmi_hold_flow_info
===========================================================================*/
/*!
@brief
  Hold the QMI QOS flow info.
...
)
{
  ...
  return netmgr_sscm3_hold_flow_info(__netmgr_qmi_get_link_info(link), flow_info);
}

/*===========================================================================
  FUNCTION  netmgr_qmi_maybe_hold_flow_info
===========================================================================*/
/*!
@brief
  Maybe Hold the QMI QOS flow info.

@return
  TRUE if flow is held
  FALSE otherwise
*/
/*=========================================================================*/
boolean netmgr_qmi_maybe_hold_flow_info
(
  int                          link,
  netmgr_qmi_qos_flow_info_t * flow_info
)
{
  ...
  return netmgr_sscm3_maybe_hold_flow_info(__netmgr_qmi_get_link_info(link),
                                           flow_info);
}

/*===========================================================================
  FUNCTION netmgr_qmi_clear_flow_cache
===========================================================================*/
/*!
@brief
  clean the QMI QOS flow Cache

@return
  None
*/
/*=========================================================================*/
void netmgr_qmi_clear_flow_cache
(
  int link,
  int pdu
)
{
  ...
  return netmgr_sscm3_clear_flow_cache(__netmgr_qmi_get_link_info(link), pdu);
}

/*===========================================================================
  FUNCTION netmgr_qmi_submit_flow_info
===========================================================================*/
/*!
@brief
  Submit one cached qos flow for processing

@return
  None
*/
/*=========================================================================*/
void netmgr_qmi_submit_flow_info
(
  int link,
  netmgr_qmi_qos_flow_info_t * flow_info
)
{
  ...
  netmgr_nl_events_t nl_event = NET_PLATFORM_INVALID_EV;
  const char *dev_name;

  NETMGR_ASSERT(flow_info);

  netmgr_log_high("Submitting flow 0x%x for processing in state %u",
                  flow_info->flow_id, flow_info->state);

  switch (flow_info->state)
  {
  case NETMGR_TC_FLOW_ACTIVATE:
    netmgr_tc_flow_activate(link, flow_info);
    nl_event = NET_PLATFORM_FLOW_ACTIVATED_EV;
    break;

  case NETMGR_TC_FLOW_MODIFY:
    netmgr_tc_flow_modify(link, flow_info);
    nl_event = NET_PLATFORM_FLOW_MODIFIED_EV;
    break;

  case NETMGR_TC_FLOW_DELETE:
    netmgr_tc_flow_delete(link, flow_info);
    nl_event = NET_PLATFORM_FLOW_DELETED_EV;
    break;

  case NETMGR_TC_FLOW_SUSPENDED:
    netmgr_tc_flow_suspend(link, flow_info);
    nl_event = NET_PLATFORM_FLOW_SUSPENDED_EV;
    break;

  case NETMGR_TC_FLOW_ACTIVE:
    netmgr_tc_flow_control(link, flow_info, flow_info->state);
    nl_event = NET_PLATFORM_FLOW_ENABLED_EV;
    break;

  case NETMGR_TC_FLOW_DISABLED:
    netmgr_tc_flow_control(link, flow_info, flow_info->state);
    nl_event = NET_PLATFORM_FLOW_DISABLED_EV;
    break;

  case NETMGR_TC_FLOW_REBIND:
    netmgr_tc_flow_rebind(link, flow_info);
    break;

  default:
    netmgr_log_err("Unknown flow state %u", flow_info->state);
    break;
  }

  if (nl_event == NET_PLATFORM_INVALID_EV)
    return;

  /* We don't send sec PDU flow for now */
  if (NETMGR_IS_DEFAULT_FLOW(flow_info->flow_id) ||
      NETMGR_IS_SEC_PDU_FLOW(flow_info->flow_id))
    return;
  ...
  event_info->event = nl_event;
  event_info->link = link;
  event_info->flow_info.flow_id = flow_info->flow_id;
  event_info->flow_info.flow_type = flow_info->flow_type;
  event_info->param_mask |= (NETMGR_EVT_PARAM_LINK | NETMGR_EVT_PARAM_FLOWINFO);
  strlcpy(event_info->dev_name, dev_name, sizeof(event_info->dev_name));
  event_info->param_mask |= NETMGR_EVT_PARAM_DEVNAME;

  if (NETMGR_SUCCESS != netmgr_kif_send_event_msg(event_info))
  {
      netmgr_log_err("failed on kif_send_event %u\n", nl_event);
  }
...
/*===========================================================================
  FUNCTION netmgr_qmi_try_submit_cached_flows
===========================================================================*/
/*!
@brief
  Try to submit cached qos flows for processing

@return
  None
*/
/*=========================================================================*/
void netmgr_qmi_try_submit_cached_flows
(
  int link
)
{
  ...
  netmgr_sscm3_try_submit_cached_flows(link, __netmgr_qmi_get_link_info(link));
}
