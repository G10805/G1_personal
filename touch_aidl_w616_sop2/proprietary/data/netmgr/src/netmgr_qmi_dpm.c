...
/*===========================================================================
  ...
  Copyright (c) 2013-2016, 2018-2019, 2021-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
)
{
  ...
  case NETMGR_QMI_DPM_OOS_EV:
    ...
    (void) netmgr_qmi_client_release(netmgr_qmi_dpm_state.dpm_clid);
...
static int netmgr_qmi_dpm_init_client(void)
{
  ...
  rc = netmgr_qmi_client_init_instance(dpm_svc_obj,
...
static int netmgr_qmi_dpm_port_open(void)
{
  ...
  memset(&open_resp, 0, sizeof(dpm_open_port_resp_msg_v01));
  ...
  {
    open_req->hardware_data_port_list_len = 2;
    open_req->hardware_data_port_list[1].ep_id.ep_type =  netmgr_main_cfg.ep_type;
    open_req->hardware_data_port_list[1].ep_id.iface_id = netmgr_main_cfg.llc.epid;
    open_req->hardware_data_port_list[1].hardware_ep_pair.consumer_pipe_num = netmgr_main_cfg.llc.consumer_pipe;
    open_req->hardware_data_port_list[1].hardware_ep_pair.producer_pipe_num = netmgr_main_cfg.llc.producer_pipe;
  }
  ...
  if(open_resp.resp.result != QMI_RESULT_SUCCESS_V01)
  {
    netmgr_log_err("Failure to open DPM PORT, err %d", open_resp.resp.result);
    goto bail;
  }
