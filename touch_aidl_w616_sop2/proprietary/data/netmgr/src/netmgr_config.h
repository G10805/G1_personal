...
/* LLC configuration */
struct netmgr_llc_config
{
  boolean enabled;
  uint32 epid;
  uint32 consumer_pipe;
  uint32 producer_pipe;
  uint32 ul_agg_cnt;
  uint32 ul_agg_size;
  uint32 ul_agg_time;
  uint32 ul_agg_features;
  uint32 dl_agg_cnt;
  uint32 dl_agg_size;
  int pnd_rps_mask;
};
...
struct netmgr_main_cfg_s {
  ...
  int ep_config_ext_ioctl_v2; /* Endpoint configuration extended ioctl v2 */
  struct egress_format_v2 eg_fmt;    /* Egress format V2 */
  struct ingress_format_v2 ing_fmt;   /* Ingress format V2 */
  ...
  int rmnet_offload;    /* load/unload rmnet_offload module */
  boolean rmnet_shs;        /* load/unload rmnet_shs module */
  boolean xlat_txcsum_disabled; /* disable txcsum for xlat */
  int rsc;      /* Receive side coalescing */
  int rsb;      /* Receive side boxing */
  ...
  int netmgr_listen_ev_proto;   /* netmgr using TIPC as IPC with clients */
  boolean netmgr_recovery_enabled; /* Netmgr Recovery Feature Enabled */
  int debug_netdev_max_backlog; /* System-wide netdev max backlog */
  int debug_netdev_budget;
  boolean iwlan_concurrency; /* IWLAN concurrency with tethering */
  boolean nl_xfrm; /* Updated XFRM handling */
  int phy_gso_max_size; /* gso_max_size to be set on physical device */
  struct netmgr_llc_config llc; /* LLC configuration */
  boolean oemprxy_enabled;
  boolean dfc_ps_ext;           /* DFC powersave ext */
  int sscm3_config;
  int sscm3;
  int ipv6_multihoming_config;
  int ipv6_multihoming;
};
...
#define NETMGR_MAIN_CFG_IS_IWLAN_CONCURRENCY_SET() \
        (netmgr_main_cfg.iwlan_concurrency)
...
int netmgr_config_check_new_rmnet_driver_availability();
