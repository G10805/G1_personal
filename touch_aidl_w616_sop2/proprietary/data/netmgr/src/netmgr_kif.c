...
#include "netmgr_xfrm.h"
#include "netmgr_xfrm_utils.h"
#endif
...
#include "netmgr_modem_proxy.h"
#include "netmgr_sscm3.h"
...
/*---------------------------------------------------------------------------
   Type representing address information of wlan interfaces
---------------------------------------------------------------------------*/
typedef struct {
    char     ifname[NETMGR_IF_NAME_MAX_LEN];
    uint32   ifa_index;
    char     v4addr[NETMGR_MAX_STR_LENGTH];
    char     v6addr[NETMGR_MAX_STR_LENGTH];
    boolean  v4_valid;
    boolean  v6_valid;
    boolean  infovalid;
} netmgr_wlanifinfo_t;
...
---------------------------------------------------------------------------*/
int netmgr_kif_remove_nl_sa_rules(int link, int ip_family);
...
LOCAL void
netmgr_kif_delete_all_route_timers(int link);
...
{                                                                          \
  ...
  if (netmgr_main_cfg.nl_xfrm)                                             \
  {                                                                        \
    if (NETMGR_FAILURE == netmgr_xfrm_add_policy_ex(ipsec_dir))            \
    {                                                                      \
      netmgr_log_err("failed to install v6 special policies\n");           \
      break;                                                               \
    }                                                                      \
  }                                                                        \
  else                                                                     \
  {                                                                        \
    ...
    cmd_length = snprintf( xfrm_policy_cmd,                                \
                           ...
                           "ip xfrm policy add dir %s priority 10"         \
                            " dst fe80::/64 proto 58",                     \
                           (XFRM_POLICY_OUT == ipsec_dir) ? "out" : "in"); \
                                                                           \
...
{                                                                    \
  ...
  if (netmgr_main_cfg.nl_xfrm)                                       \
  {                                                                  \
    if (NETMGR_FAILURE == netmgr_xfrm_remove_policy_ex(ipsec_dir))   \
    {                                                                \
      netmgr_log_err("Failed to remove v6 special policies\n");      \
      break;                                                         \
    }                                                                \
  }                                                                  \
  else                                                               \
  {                                                                  \
    ...
    cmd_length = snprintf( xfrm_policy_cmd,                          \
                           sizeof(xfrm_policy_cmd),                  \
                           "ip xfrm policy delete dir %s"            \
                            " dst fe80::/64 proto 58",               \
                           (XFRM_POLICY_OUT == ipsec_dir) ? "out" :  \
                                                            "in");   \
                                                                     \
    if (DS_EXEC_OK != netmgr_ds_exec(xfrm_policy_cmd, &opts))        \
    {                                                                \
      netmgr_log_err("Failed to remove v6 special policies, err: %s",\
                     err_str);                                       \
      break;                                                         \
    }                                                                \
  }                                                                  \
...
)
{
  ...
  if (NETMGR_KIF_IS_REV_RMNET_LINK(link))
  {
    ...
    else if(WDS_TECHNOLOGY_NAME_CIWLAN_V01 == tech_name)
    {
      ifinfo->tech_type = NETMGR_KIF_CB_IF_TECH_TYPE_C_WLAN;
    }
    else
    {
  ...
  else if (WDS_TECHNOLOGY_NAME_IWLAN_S2B_PROXY_V01 == tech_name)
  {
    ifinfo->tech_type = NETMGR_KIF_CB_IF_TECH_TYPE_EPDG_PROXY;
  }
  else
  {
...
)
{
  ...
  /*Iwlan will use this struct and it uses only primary */

  addr_info = netmgr_qmi_get_addr_info(link, PRIMARY_PDU);
  if (addr_info)
  {
    if ( addr_info->valid_mask & NETMGR_ADDRSET_MASK_IPV4)
    {
      addr_set = netmgr_qmi_get_addr_set(addr_info, NETMGR_IPV4_ADDR,
                                         NETMGR_ADDR_TYPE_DEFAULT);
      ssr_ifinfo->addr_mask |= NETMGR_KIF_IPF_MASK_IPV4;
      ssr_ifinfo->ipv4.type = NETMGR_KIF_CB_ADDR_IPV4;
      ssr_ifinfo->ipv4._in4.mask = addr_set->if_mask;
      ssr_ifinfo->ipv4._in4.addr = addr_set->if_addr.addr.v4;
    }

    if ( addr_info->valid_mask & NETMGR_ADDRSET_MASK_IPV6)
    {
      addr_set = netmgr_qmi_get_addr_set(addr_info, NETMGR_IPV6_ADDR,
                                         NETMGR_ADDR_TYPE_DEFAULT);
      ssr_ifinfo->addr_mask |= NETMGR_KIF_IPF_MASK_IPV6;
      ssr_ifinfo->ipv6.type = NETMGR_KIF_CB_ADDR_IPV6;
      ssr_ifinfo->ipv6._in6.mask = addr_set->if_mask;
      memcpy(&ssr_ifinfo->ipv6._in6.addr, &addr_set->if_addr.addr,
...
netmgr_kif_ifioctl_get_flags (const char * dev, short * p_flags)
{
  ...
  if ((fd = netmgr_socket(AF_INET, SOCK_DGRAM | SOCK_CLOEXEC, 0)) < 0) {
...
/*=========================================================================*/
unsigned int
netmgr_kif_get_mtu(int link, int pdu_id)
{
#ifdef FEATURE_DS_LINUX_ANDROID
  unsigned int iface_mtu;
  unsigned int pdu_cnf = PRIMARY_PDU;
  netmgr_wds_qmi_drv_info_t *wds_info;

  if (netmgr_main_cfg.sscm3_config && pdu_id)
  {
    pdu_cnf = SECONDARY_PDU;
  }

  wds_info = netmgr_qmi_get_link_wds_info_pdu(link, pdu_cnf);
  if (wds_info)
  {
    iface_mtu = wds_info->iface_mtu;
    netmgr_log_high("netmgr_kif_get_mtu: mtu=[%u] pdu=[%d]", iface_mtu, pdu_id);
    return iface_mtu;
  }
  else
  {
    netmgr_log_err("netmgr_kif_get_mtu: invalid link [%d] pdu=[%d]", link, pdu_id);
  ...
  fd = netmgr_socket(AF_INET, SOCK_STREAM | SOCK_CLOEXEC, 0);
  if ( fd < 0 ) {
...
)
{
  ...
  /* Get current if flags for the device */
  if (ioctl(fd, ext_ioctl_version, &ifr) < 0) {
...
static void
netmgr_kif_ioctl_set_rmnet_mtu(int link, unsigned int mtu4, unsigned int mtu6)
{
  char* phys_dev = netmgr_main_get_phys_net_dev();
  const char *ifname = netmgr_kif_get_name(link);
  struct rmnet_ioctl_extended_s ext_ioctl;
  int rc = NETMGR_FAILURE;
  ...
  if (ifname == NULL)
  {
    netmgr_log_err("netmgr_kif_ioctl_set_rmnet_mtu: invalid link [%d]", link);
    return;
  }

#ifdef RMNET_IOCTL_SET_MTU
  ext_ioctl.extended_ioctl = RMNET_IOCTL_SET_MTU;

  strlcpy(ext_ioctl.u.mtu_params.if_name, ifname, IFNAMSIZ);
  ext_ioctl.u.mtu_params.if_name[IFNAMSIZ - 1] = '\0';

  ext_ioctl.u.mtu_params.mtu_v4 = mtu4;
  ext_ioctl.u.mtu_params.mtu_v6 = mtu6;

  rc = netmgr_kif_extended_ioctl(phys_dev, &ext_ioctl, RMNET_IOCTL_EXTENDED);

  if (NETMGR_FAILURE == rc)
  {
    netmgr_log_err("%s: RMNET_IOCTL_SET_MTU failed on phys_dev[%s]",
                   __func__, phys_dev);
  }
  else
  {
    netmgr_log_med("%s: RMNET_IOCTL_SET_MTU set mtu IPv4 %d IPv6 %d for link %s on phys_dev[%s]",
                   __func__, mtu4, mtu6, ifname, phys_dev);
  }
#endif
}
...
netmgr_kif_set_mtu(int link, boolean post_nl_ev)
{
  ...
  unsigned int sec_mtu4, sec_mtu6 = NETMGR_MTU_MAX;

  unsigned int max_mtu = NETMGR_MTU_MAX;
  netmgr_wds_qmi_drv_info_t *wds_info;

  /* Get Modem MTU for link and assign it to network interface */
  mtu4 = netmgr_qmi_get_mtu(link, QMI_IP_FAMILY_PREF_IPV4, PRIMARY_PDU);
  mtu6 = netmgr_qmi_get_mtu(link, QMI_IP_FAMILY_PREF_IPV6, PRIMARY_PDU);

  /* Sec should be invalid or valid here */
  if(netmgr_main_cfg.sscm3_config){
    sec_mtu4 = netmgr_qmi_get_mtu(link, QMI_IP_FAMILY_PREF_IPV4, SECONDARY_PDU);
    sec_mtu6 = netmgr_qmi_get_mtu(link, QMI_IP_FAMILY_PREF_IPV6, SECONDARY_PDU);
  }
  else
  {
   sec_mtu4 = NETMGR_MTU_INVALID;
   sec_mtu6 = NETMGR_MTU_INVALID;
  }
  netmgr_log_low("%s() Got MTUs for Link #%d: IPV4 =  %u, IPV6 = %u sec IPV4 =  %u, sec IPV6 = %u\n",
                 __func__, link, mtu4, mtu6, sec_mtu4, sec_mtu6);
  ...
  if ( (NETMGR_MTU_INVALID != mtu4) && (NETMGR_MTU_INVALID != mtu6) )
  {
    ...
    sec_mtu4 = (sec_mtu4 == NETMGR_MTU_INVALID)? NETMGR_MTU_MAX : MAX(sec_mtu4, NETMGR_KIF_MIN_V6_MTU);
    sec_mtu6 = (sec_mtu6 == NETMGR_MTU_INVALID)? NETMGR_MTU_MAX : MAX(sec_mtu6, NETMGR_KIF_MIN_V6_MTU);
    /* Invalid sec mtu's will cause no-op here, otherwise they will be min */
    mtu = MIN(MIN(mtu4, mtu6), MIN( sec_mtu4, sec_mtu6));
  }
  else if(NETMGR_MTU_INVALID != mtu4)
  {
    ...
    mtu = MIN(MAX(mtu4, NETMGR_KIF_MIN_V6_MTU), MIN( sec_mtu4, sec_mtu6));
  ...
  else if(NETMGR_MTU_INVALID != mtu6)
  {
    ...
    mtu = MIN(MAX(mtu6, NETMGR_KIF_MIN_V6_MTU), MIN( sec_mtu4, sec_mtu6));
  }
  ...
  /* dual IP MTU applies to forward links only */
  if (!NETMGR_KIF_IS_REV_RMNET_LINK(link))
  {
    netmgr_kif_ioctl_set_rmnet_mtu(link, netmgr_qmi_get_mtu(link, QMI_IP_FAMILY_PREF_IPV4, PRIMARY_PDU),
                                   netmgr_qmi_get_mtu(link, QMI_IP_FAMILY_PREF_IPV6, PRIMARY_PDU));
  }
...
#endif /* FEATURE_DS_LINUX_ANDROID */
  wds_info = netmgr_qmi_get_link_wds_info(link);
  if (wds_info)
  {
    wds_info->iface_mtu = mtu;
  }
#ifndef FEATURE_DS_LINUX_ANDROID
  /* In android case, the android framework will handle setting MTU, so we
   * only want to set iface mtu through ioctl if we are on a non-android target
   */
  if( NETMGR_FAILURE ==
...
#ifdef FEATURE_DS_LINUX_ANDROID
  /*For Link-local, we need to set MTU as *only* client(ims) uses it for proprietary communication to modem*/
  if(WDS_TECHNOLOGY_NAME_MODEM_LINK_LOCAL_V01 == netmgr_qmi_wds_get_tech_name(link))
  {
      if(NETMGR_SUCCESS == netmgr_kif_ifioctl_set_mtu( netmgr_kif_get_name(link), mtu ))
      {
        netmgr_log_high("%s() assigned MTU %d on %s\n",
                     __func__, mtu, netmgr_kif_get_name(link) ? : "NULL");
      }
      ...
      {
        netmgr_log_err( "%s() failed to set MTU for %s\n", __func__, netmgr_kif_get_name(link) ? : "NULL");
      }
  }
...
/*=========================================================================*/
LOCAL int
netmgr_kif_ifioctl_get_epid(const char *dev, uint32 *epid, bool llc)
{
  int fd;
  int rval = NETMGR_FAILURE;
  struct ifreq ifr;
  ...
  memset(ext_ioctl_arg, 0, sizeof(struct rmnet_ioctl_extended_s));
  if (llc)
    ext_ioctl_arg->extended_ioctl = RMNET_IOCTL_GET_EPID_LL;
  else
...
/*=========================================================================*/
LOCAL int
netmgr_kif_ifioctl_get_ep_pair
(
  const char *dev,
...
)
{
  ...
  memset(ext_ioctl_arg, 0, sizeof(struct rmnet_ioctl_extended_s));
  if (llc)
    ext_ioctl_arg->extended_ioctl = RMNET_IOCTL_GET_EP_PAIR_LL;
  else
...
netmgr_kif_ipa_ioctl_set_dev_name(const char * dev)
{
  ...
  ipa_fd = netmgr_open(NETMGR_KIF_IPA_DEV_NAME, O_RDONLY|O_CLOEXEC);
...
#define NETMGR_DEV_SECONDARY_ADDR_PROMOTE_ENABLED 1
#define NETMGR_DEV_SECONDARY_ADDR_PROMOTE_DISABLED 0
LOCAL inline int
netmgr_kif_set_secondary_addr_promote
(
  const char *dev_name,
  int state
)
{
  ...
  {
    netmgr_log_err("%s(): failed to set promote_secondaries on device [%s]\n",
                   __func__, dev_name);
  }
...
)
{
  ...
  return netmgr_kif_extended_ioctl(dev, &ext_ioctl, RMNET_IOCTL_EXTENDED);
}
...
)
{
  ...
  ext_ioctl.u.data = 0;
  if(netmgr_kif_extended_ioctl(dev, &ext_ioctl, RMNET_IOCTL_EXTENDED) != NETMGR_SUCCESS)
  {
...
)
{
  ...
  if (netmgr_kif_ifioctl_get_ep_pair(dev_name, &ep_pair, false) != NETMGR_SUCCESS)
  {
  ...
  if (netmgr_kif_ifioctl_get_epid(dev_name, &epid, false) != NETMGR_SUCCESS)
  {
  ...
  // Retrieve LLC EP params
  if (netmgr_main_cfg.llc.enabled)
  {
    memset(&ep_pair, 0, sizeof(ep_pair));
    if (netmgr_kif_ifioctl_get_ep_pair(dev_name, &ep_pair, true) != NETMGR_SUCCESS ||
        netmgr_kif_ifioctl_get_epid(dev_name, &epid, true) != NETMGR_SUCCESS)
    {
      netmgr_log_err("Failed to retrieve LLC EP params\n");
      netmgr_main_cfg.llc.enabled = FALSE;
    }
    else
    {
      netmgr_main_cfg.llc.epid = epid;
      netmgr_main_cfg.llc.consumer_pipe = ep_pair.consumer_pipe_num;
      netmgr_main_cfg.llc.producer_pipe = ep_pair.producer_pipe_num;
      netmgr_log_low("LLC epid %u cpipe %u ppipe %u\n",
                     epid, ep_pair.consumer_pipe_num, ep_pair.producer_pipe_num);
    }
  }
...
)
{
  ...
  if (netmgr_main_cfg.pnd_rps_mask > 0)
  {
    ...
    if (netmgr_kif_set_rps_mask(dev_name, 0, netmgr_main_cfg.pnd_rps_mask) != NETMGR_SUCCESS)
    {
  ...
  if (netmgr_main_cfg.llc.enabled && netmgr_main_cfg.llc.pnd_rps_mask)
  {
    netmgr_log_low("%s(): Setting LLC RPS mask on %s to %d\n",
                   __func__, dev_name, netmgr_main_cfg.llc.pnd_rps_mask);

    if (netmgr_kif_set_rps_mask(dev_name, 1, netmgr_main_cfg.llc.pnd_rps_mask) != NETMGR_SUCCESS)
    {
      netmgr_log_med("%s(): Failed to LLC RPS mask on transport (%s).\n",
                      __func__, dev_name);
    }
  }
...
#ifdef RMNET_IOCTL_EXTENDED_V2
/*===========================================================================
  FUNCTION  netmgr_kif_ifioctl_v2_configure_egress_eps
 ===========================================================================*/
/*!
@brief
  Calls the optional extended v2 IOCTLs to set egress EP configuration

@arg phys_dev device to call IOCTL on
@arg rmnet_ioctl_extended_s_v2 passed in to be sent via ioctl
...
/*=========================================================================*/
LOCAL int
netmgr_kif_ifioctl_v2_configure_egress_eps
(
  const char *phys_dev,
  struct rmnet_ioctl_extended_s_v2 *ext_ioctl_v2
)
{
  int rc = NETMGR_FAILURE;
  ...
  ext_ioctl_v2->ioctl_ptr = (__u64)(uintptr_t) &netmgr_main_cfg.eg_fmt;
  ext_ioctl_v2->ioctl_data_size = sizeof(struct egress_format_v2);
  ext_ioctl_v2->extended_v2_ioctl_type = RMNET_IOCTL_SET_EGRESS_DATA_FORMAT_V2;

  rc = netmgr_kif_extended_ioctl(phys_dev, ext_ioctl_v2, RMNET_IOCTL_EXTENDED_V2);

  netmgr_log_med("%s(): Configuring egress EP configuration, rc = %d\n", __func__, rc);

  return rc;
}

/*===========================================================================
  FUNCTION  netmgr_kif_ifioctl_v2_configure_ingress_eps
 ===========================================================================*/
/*!
@brief
  Calls the optional extended v2 IOCTLs to set ingress EP configuration
...
/*=========================================================================*/
LOCAL int
netmgr_kif_ifioctl_v2_configure_ingress_eps
(
  const char *phys_dev,
  struct rmnet_ioctl_extended_s_v2 *ext_ioctl_v2
)
{
  ...
  ext_ioctl_v2->ioctl_ptr = (__u64)(uintptr_t) &netmgr_main_cfg.ing_fmt;
  ext_ioctl_v2->ioctl_data_size = sizeof(struct ingress_format_v2);
  ext_ioctl_v2->extended_v2_ioctl_type = RMNET_IOCTL_SET_INGRESS_DATA_FORMAT_V2;

  rc = netmgr_kif_extended_ioctl(phys_dev, ext_ioctl_v2, RMNET_IOCTL_EXTENDED_V2);

  netmgr_log_med("%s(): Configuring ingress EP configuration, rc = %d\n", __func__, rc);

  return rc;
}

static int netmgr_kif_set_dev_ulso_flags
(
  const char *dev
)
{
  ...
  rval |= netmgr_kif_ethtool_config_feature(dev, "tx-udp-segmentation", 1);
  rval |= netmgr_kif_ethtool_config_feature(dev, "tx-tcp-segmentation", 1);
  rval |= netmgr_kif_ethtool_config_feature(dev, "tx-tcp6-segmentation", 1);
  rval |= netmgr_kif_ethtool_config_feature(dev, "tx-tcp-mangleid-segmentation", 1);
  rval |= netmgr_kif_ethtool_config_feature(dev, "tx-tcp-ecn-segmentation", 1);

  if (!strncmp(netmgr_main_cfg.phys_net_dev, dev, IFNAMSIZ) &&
      netmgr_main_cfg.phy_gso_max_size)
  {
    rval = netmgr_rtnetlink_gso_max_size(dev, netmgr_main_cfg.phy_gso_max_size);
  }

  if (rval)
  {
    netmgr_log_err("%s(): Failed to configure a ULSO required HW feature, rval = %d\n", __func__, rval);
  }
  else
  {
    netmgr_log_low("%s(): Configured ULSO required HW features, rval = %d\n", __func__, rval);
  }

  return rval;
}

static int netmgr_kif_set_dev_csum_flags
(
  const char *dev
)
{
  ...
  rval |= netmgr_kif_ethtool_config_feature(dev, "tx-checksum-ipv4", 1);
  rval |= netmgr_kif_ethtool_config_feature(dev, "tx-checksum-ipv6", 1);

  if (rval)
  {
    netmgr_log_err("%s(): Failed to configure a CSUM required HW feature, rval = %d\n", __func__, rval);
  }
  else
  {
    netmgr_log_low("%s(): Configured CSUM required HW features, rval = %d\n", __func__, rval);
  }

  return rval;
}

static int netmgr_kif_set_dev_hw_feature_flags
(
  const char *dev
)
{
  int num_pipes, i, rval, flags = 0;
  struct egress_format_v2 *eg_fmt;
  struct rmnet_egress_param *eg_param;
  struct ingress_format_v2 *ing_fmt;
  struct rmnet_ingress_param *ing_param;

  netmgr_kif_ethtool_config_feature(dev, "tx-scatter-gather", 1);
  if (netmgr_main_cfg.ep_config_ext_ioctl_v2)
  {
    /* loop through egress pipes and if any have ULSO turned to true, then for
     * this device we need to enable tso/tx-udp-segmentation on the device.
     */
    eg_fmt = &netmgr_main_cfg.eg_fmt;
    num_pipes = eg_fmt->number_of_eps;
    for (i = 0; i < num_pipes; i++)
    {
      eg_param = (struct rmnet_egress_param *) (eg_fmt->egress_param_ptr +
                 (__u64) (i*sizeof(struct rmnet_egress_param)));
      if (eg_param->ulso_en)
      {
        flags |= NETMGR_RMNET_HW_FEATURE_ULSO;
      }
      if (eg_param->cs_offload_en)
      {
        flags |= NETMGR_RMNET_HW_FEATURE_CSUM_OFFLOAD;
      }
    }

    ing_fmt = &netmgr_main_cfg.ing_fmt;
    num_pipes = ing_fmt->number_of_eps;
    for (i = 0; i < num_pipes; i++)
    {
      ing_param = (struct rmnet_ingress_param *) (ing_fmt->ingress_param_ptr +
                 (__u64) (i*sizeof(struct rmnet_ingress_param)));
      if (ing_param->cs_offload_en)
      {
        flags |= NETMGR_RMNET_HW_FEATURE_CSUM_OFFLOAD;
      }
    }

    if (flags & NETMGR_RMNET_HW_FEATURE_CSUM_OFFLOAD)
    {
      rval = netmgr_kif_set_dev_csum_flags(dev);
      if (rval)
      {
        if (flags & NETMGR_RMNET_HW_FEATURE_ULSO)
        {
          netmgr_log_err("%s(): Due to CSUM offload failing to be configured, ULSO cannot be enabled", __func__);
        }
      }
      else
      {
        if (flags & NETMGR_RMNET_HW_FEATURE_ULSO)
        {
          netmgr_kif_set_dev_ulso_flags(dev);
        }
      }
    }
    else
    {
      if (flags & NETMGR_RMNET_HW_FEATURE_ULSO)
      {
        netmgr_log_err("%s(): ULSO cannot be enabled unless CSUM offload is, not enabling ULSO", __func__);
      }
    }
  }
...
(
  const char *phys_dev,
  uint32_t rmnet_flags
)
{
  uint32_t ingress_flags = 0;
  uint32_t egress_flags = 0;
  uint32_t features = 0;
  int rc = 0;
...
#ifdef RMNET_IOCTL_EXTENDED_V2
  netmgr_kif_set_dev_hw_feature_flags(phys_dev);
#endif /* RMNET_IOCTL_EXTENDED_V2 */
  ...
  {
#ifndef RMNET_IOCTL_EXTENDED_V2
    egress_flags = netmgr_rmnet_convert_rmnet_edf_to_ioctl_edf(rmnet_flags);
    ...
    if (netmgr_main_cfg.ep_config_ext_ioctl_v2)
    {
      int i;
      struct rmnet_ioctl_extended_s_v2 ext_ioctl_v2;
      struct rmnet_egress_param *eg_ptr_temp;
      int rc = NETMGR_FAILURE;

      rc = netmgr_kif_ifioctl_v2_configure_egress_eps(phys_dev, &ext_ioctl_v2);
      if (rc)
      {
        netmgr_log_err("%s(): failed to set egress EP configurations", __func__);
        return NETMGR_FAILURE;
      }

      for (i = 0; i < netmgr_main_cfg.eg_fmt.number_of_eps; i++) {
        eg_ptr_temp = ((struct rmnet_egress_param *) netmgr_main_cfg.eg_fmt.egress_param_ptr) + i;
        netmgr_log_low("%s(): egress ep config received back from HW for ep%d: %u, %u, %u, %u, %u, %u, %u, %u", __func__, i, eg_ptr_temp->egress_ep_type, eg_ptr_temp->pipe_setup_status,
                                                                                              eg_ptr_temp->cs_offload_en, eg_ptr_temp->aggr_en,
                                                                                              eg_ptr_temp->ulso_en, eg_ptr_temp->ipid_min_max_idx,
                                                                                              eg_ptr_temp->int_modt, eg_ptr_temp->int_modc);
        if (eg_ptr_temp->pipe_setup_status == IPA_PIPE_SETUP_FAILURE)
        {
          netmgr_log_err("%s(): Egress EP configuration response indicates failure with status = %u", __func__, eg_ptr_temp->pipe_setup_status);
          return NETMGR_FAILURE;
        }
        else
        {
          netmgr_log_low("%s(): Egress EP configuration response status = %u", __func__, eg_ptr_temp->pipe_setup_status);
        }
      }
    }
    else
  ...
  {
#ifndef RMNET_IOCTL_EXTENDED_V2
    ingress_flags = netmgr_rmnet_convert_rmnet_idf_to_ioctl_idf(rmnet_flags);
    ...
    if (netmgr_main_cfg.ep_config_ext_ioctl_v2)
    {
      int i;
      struct rmnet_ioctl_extended_s_v2 ext_ioctl_v2;
      struct rmnet_ingress_param *ing_ptr_temp;
      int rc = NETMGR_FAILURE;

      rc = netmgr_kif_ifioctl_v2_configure_ingress_eps(phys_dev, &ext_ioctl_v2);
      if (rc)
      {
        netmgr_log_err("%s(): failed to set ingress EP configurations", __func__);
        return NETMGR_FAILURE;
      }

      for (i = 0; i < netmgr_main_cfg.ing_fmt.number_of_eps; i++) {
        ing_ptr_temp = ((struct rmnet_ingress_param *) netmgr_main_cfg.ing_fmt.ingress_param_ptr) + i;
        netmgr_log_low("%s(): ingress ep config received back from ipa for ep%d: %u, %u, %u, %u, %u, %u, %u, %u, %u", __func__, i, ing_ptr_temp->ingress_ep_type, ing_ptr_temp->pipe_setup_status,
                                                                                              ing_ptr_temp->cs_offload_en, ing_ptr_temp->buff_size,
                                                                                              ing_ptr_temp->agg_byte_limit, ing_ptr_temp->agg_time_limit,
                                                                                              ing_ptr_temp->agg_pkt_limit, ing_ptr_temp->int_modt,
                                                                                              ing_ptr_temp->int_modc);
        if (ing_ptr_temp->pipe_setup_status == IPA_PIPE_SETUP_FAILURE)
        {
          netmgr_log_err("%s(): Ingress EP configuration response indicates failure with status = %u", __func__, ing_ptr_temp->pipe_setup_status);
          return NETMGR_FAILURE;
        }
        else
        {
          netmgr_log_low("%s(): Ingress EP configuration response status = %u", __func__, ing_ptr_temp->pipe_setup_status);
        }
      }
    }
    else
...
netmgr_kif_purge_address( int link, netmgr_ip_address_t * addr_ptr, unsigned int mask )
{
  ...
  do {
    ...
    if( NETMGR_IPV4_ADDR == addr_ptr->type )
    {
      ...
      /* Mask is the actual netmask for ipv4 */
      prefix_len = (unsigned int)ds_get_num_bits_set_count(mask);
      /* Remove address from network interface */
      netmgr_log_med("deleting address [%s/%u] from iface[%s]\n", buff, prefix_len, netmgr_kif_info[ link ].name );
      rc = ifc_del_address( netmgr_kif_info[ link ].name, buff, prefix_len );
      if( 0 > rc )
      {
        netmgr_log_err("error on ifc_del_address(): %d, %s\n", rc, strerror(-rc));
        break;
      }
...
netmgr_kif_clear_iface( int link )
{
  ...
  {
    netmgr_log_low("skip clearing kernel interface for reverse link [%d]\n", link );
  }
...
/*=========================================================================*/
LOCAL int
netmgr_kif_nl_post_addr_msg
(
  int link,
  const netmgr_nl_msg_t *nlmsg_info_ptr,
  int pdu_id,
...
)
{
  ...
  if( NETMGR_NLA_PARAM_PREFIXADDR & nlmsg_info_ptr->addr_info.attr_info.param_mask ) {
    ...
    event_info.addr_info.addr.mask = nlmsg_info_ptr->addr_info.metainfo.ifa_prefixlen;
    ...
    event_info.addr_info.address_type = addr_type;
    event_info.param_mask |= NETMGR_EVT_PARAM_ADDR_TYPE;

    event_info.mtu = netmgr_kif_get_mtu(link, pdu_id);
    event_info.mtu4 = netmgr_qmi_get_link_mtu_by_family(event_info.link,
                                                          NETMGR_IPV4_ADDR);
    event_info.mtu6 = netmgr_qmi_get_link_mtu_by_family(event_info.link,
                                                          NETMGR_IPV6_ADDR);
    event_info.param_mask |= NETMGR_EVT_PARAM_MTU;
    event_info.pdu_id = pdu_id;
    event_info.param_mask |= NETMGR_EVT_PARAM_PDU_ID;
...
netmgr_kif_open_req( int link, netmgr_address_set_t * addr_info_ptr )
{
  ...
  /*Update first_conn_clnt here*/
  clnt = netmgr_qmi_get_first_conn_clnt_from_ip_family_type(addr_info_ptr->if_addr.type);
  if((clnt != NETMGR_QMI_CLIENT_MAX) &&
     (NETMGR_SUCCESS != netmgr_qmi_update_first_conn_clnt(link, clnt)))
  {
    netmgr_log_err("%s(): Failed to update first_conn_clnt for link [%d] \n",
                    __func__,
                    link);
  }
  ...
  else
  {
    ...
    else if (netmgr_main_cfg.vnd_rps_mask > 0)
    {
      netmgr_log_med("%s(): Setting RPS mask on kif open for link %s mask 0x%x",__func__, link_name, netmgr_main_cfg.vnd_rps_mask);
      if (netmgr_kif_set_rps_mask(link_name, 0, netmgr_main_cfg.vnd_rps_mask))
      {
...
{
  /* IWLAN uses Primary PDU only*/
  netmgr_address_info_t  *addr_info_ptr = netmgr_qmi_get_addr_info(link, PRIMARY_PDU);
  ...
  if (WDS_TECHNOLOGY_NAME_WLAN_LOCAL_BRKOUT_V01 == netmgr_qmi_wds_get_tech_name(link))
  {
    ...
    {
      netmgr_address_set_t *v4_addr;

      v4_addr = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV4_ADDR,
                                        NETMGR_ADDR_TYPE_DEFAULT);
      if (NULL !=
            inet_ntop(AF_INET, &(v4_addr->if_addr.addr.v4),
  ...
  if (NETMGR_ADDRSET_MASK_IPV4 & addr_info_ptr->valid_mask)
  {
    cmd_buf->data.info.connect_msg.addr_info_ptr =
      netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV4_ADDR,
                               NETMGR_ADDR_TYPE_DEFAULT);
  }
  else
  {
    cmd_buf->data.info.connect_msg.addr_info_ptr =
      netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV6_ADDR,
                              NETMGR_ADDR_TYPE_DEFAULT);
  }
...
*/
/*=========================================================================*/
static int netmgr_kif_cfg_req(int link, netmgr_ip_addr_t addr_type, boolean is_reconfig, int pdu_id, netmgr_kif_lft_change_t lft_chng)
{
  ...
  addr_info_ptr = netmgr_qmi_get_addr_info( link , pdu_id);
  if(!addr_info_ptr)
  {
  ...
  ifinfo.link = link;
  if (NETMGR_KIF_IS_REV_RMNET_LINK(link) && pdu_id == PRIMARY_PDU)
  {
    ...
    if (NETMGR_SUCCESS == result &&
        (NETMGR_KIF_CB_IF_TECH_TYPE_EPDG == ifinfo.tech_type || NETMGR_KIF_CB_IF_TECH_TYPE_C_WLAN == ifinfo.tech_type)
...
#ifdef FEATURE_DATA_IWLAN
    /* Update the wlan ifname in the link, for Non-LBO the tunnel endpoint
       local addr will be the wlan interface address */
    if (NETMGR_SUCCESS == result && NETMGR_KIF_CB_IF_TECH_TYPE_EPDG == ifinfo.tech_type) {
      netmgr_kif_save_wlan_ifname_to_link(link, local_addr, tunnel_family);
    }
    else if (NETMGR_SUCCESS == result && NETMGR_KIF_CB_IF_TECH_TYPE_EPDG_PROXY == ifinfo.tech_type
               && netmgr_kif_info[link].oemprxy_info.wlan_iface_name_valid == TRUE) {
      (void) strlcpy(ifinfo.wlan_ifname,
                     netmgr_kif_info[link].oemprxy_info.wlan_iface_name,
                     NETMGR_IF_NAME_MAX_LEN);
    }
    else if (NETMGR_SUCCESS == result && NETMGR_KIF_CB_IF_TECH_TYPE_C_WLAN == ifinfo.tech_type) {
      netmgr_kif_save_wwan_ifname_to_link(link, local_addr, tunnel_family);
    }
#endif /* FEATURE_DATA_IWLAN */
  }
  else if (WDS_TECHNOLOGY_NAME_MODEM_LINK_LOCAL_V01 == tech_name)
  ...
  switch(addr_type)
  {
    case NETMGR_IPV4_ADDR:
      addr_ptr = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV4_ADDR,
                                         NETMGR_ADDR_TYPE_DEFAULT);
      ifinfo.addr.type = NETMGR_KIF_CB_ADDR_IPV4;
      ifinfo.addr._in4.addr = addr_ptr->if_addr.addr.v4;
      ifinfo.addr._in4.mask = addr_ptr->if_mask;
      break;
    case NETMGR_IPV6_ADDR:
      addr_ptr = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV6_ADDR,
                                         NETMGR_ADDR_TYPE_DEFAULT);
      ifinfo.addr.type = NETMGR_KIF_CB_ADDR_IPV6;
      memcpy(&ifinfo.addr._in6.addr,
             &addr_ptr->if_addr.addr,
             sizeof(ifinfo.addr._in6.addr));
      ifinfo.addr._in6.mask = addr_ptr->if_mask;
      break;
    default:
  ...
  if (NETMGR_KIF_CB_IFTYPE_REVERSE == ifinfo.type)
  {
    ...
    /* Cleanup XFRM state and polcies on failure */
    if (NETMGR_FAILURE == result)
    {
      netmgr_kif_remove_nl_sa_rules(link, ip_family);
    }
  ...
  if(NETMGR_IPV4_ADDR == addr_type)
  {
    ...
    unsigned int prefix_len;
    unsigned int lft = 0xFFFFFFFF; /* "forever" */

    /*configure new IP address*/
    memset(v4_addr, 0, INET_ADDRSTRLEN);
    memset(v4_netmask, 0, INET_ADDRSTRLEN);

    if (!inet_ntop(AF_INET, &(addr_ptr->if_addr.addr.v4), v4_addr, INET_ADDRSTRLEN))
    {
    ...
    if (addr_ptr->if_addr.addr.v4 == 0)
    {
      /* This condition can happen only if there is an issue with the netmgrd processing and
       * the addr_info_ptr is cleared incorrect.
       * For this particular issue, this was observed for a IPv4v6 call with the following
       * sequence of events for an IPv4v6 call-
       * i. IPv6 call connect
       * ii. IPv6 call disconnect
       * iii. IPv4 call connect received in NETMGR_GOING_DOWN state
       *
       * The WDS_CONNECT is received and get_runtime_settings
       * is invoked, however the IPv6 call disconnect causes the address pointer
       * to be cleared resulting in the IPv4 address to be configured as 0.0.0.0
       * which doesn't trigger RTM_NEWADDR causing the KIF state maching to be stuck
       * in NETMGR_KIF_OPENING.
       * Only way to recover here is to reset the NETMGR_KIF state.
       * The main state machine resets to NETMGR_STATE_INITED since the subsequent
       * WDS_DISCONNECT processing will fail resetting the main netmgr state machine.
       */

      netmgr_log_err("%s(): Invalid address %s, resetting KIF state to NETMGR_KIF_CLOSED",
                     __func__, v4_addr);
      netmgr_kif_set_state(link, NETMGR_KIF_CLOSED);
      return NETMGR_FAILURE;
    }

    if (!inet_ntop(AF_INET, &(addr_ptr->if_mask), v4_netmask, INET_ADDRSTRLEN))
    {
    ...
    prefix_len = (unsigned int)ds_get_num_bits_set_count(addr_ptr->if_mask);
    if (addr_ptr->ssc3_pdu_lifetime_valid)
    {
      lft = addr_ptr->ssc3_pdu_lifetime;
    }

    if (NETMGR_FAILURE == netmgr_rtnetlink_addr_cfg(ifname, &addr_ptr->if_addr, prefix_len, lft_chng, lft))
    {
      netmgr_log_err("%s(): handling IPv4 address assignment failed\n", __func__);
      return NETMGR_FAILURE;
    }
  ...
  else if (NETMGR_IPV6_ADDR == addr_type)
  {
...
#else
    sockfd = netmgr_socket(AF_INET6, SOCK_DGRAM | SOCK_CLOEXEC, IPPROTO_IP);
    if (sockfd  < 0)
    {
      netmgr_log_err("%s(): socket failed\n", __func__);
      return NETMGR_FAILURE;
    ...
    memcpy((char *)&ifr6.ifr6_addr,
           (char *)&addr_ptr->if_addr.addr.v6_addr8,
    ...
    ifr6.ifr6_prefixlen = addr_ptr->if_mask;
    ...
    NETMGR_LOG_IPV6_ADDR(med,
                         "netmgr_kif_cfg_req(): ",
                         addr_ptr->if_addr.addr.v6_addr64);
...
{
  int ret = NETMGR_SUCCESS;
  const struct ds_sockaddr_storage *sa_ptr = NULL;
  ...
  if (AF_INET6 == sa_ptr->ss_family) {
    ...
    v6_addr = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV6_ADDR,
                                      NETMGR_ADDR_TYPE_DEFAULT);
    NETMGR_LOG_IPV6_ADDR( med, "IID match: Link local address ",
                          v6_addr->ll_addr.addr.v6_addr64 );
    ...
    ll_iid_start = v6_addr->ll_addr.addr.v6_addr8+NETMGR_KIF_IPV6_PREFIX_LEN;
...
netmgr_kif_save_ll_addr( int link, const netmgr_nl_msg_t * nlmsg_info_ptr )
{
  ...
  if (AF_INET6 == sa_ptr->ss_family) {
    ...
    v6_addr = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV6_ADDR,
                                      NETMGR_ADDR_TYPE_DEFAULT);
    memcpy( &v6_addr->ll_addr.addr.v6_addr8,
            SASTORAGE_DATA(*sa_ptr),
            sizeof(v6_addr->ll_addr.addr.v6_addr8) );
...
/*===========================================================================
  FUNCTION  netmgr_kif_pdu_swap
===========================================================================*/
/*!
@brief
  Once Secondary has been promoted clear secondary PDU structs and move valid
  information to primary PDU structures.
...
{
  if (NETMGR_FAILURE == netmgr_kif_verify_link(link) ||
      NETMGR_KIF_IS_REV_RMNET_LINK(link))
  {
    netmgr_log_med("Invalid link swap won't take place. Should not here. link :%d",link);
    return;
  }

  netmgr_sscm3_secondary_promote(link, ip_family);
}
...
netmgr_kif_cfg_cnf( int link, const netmgr_nl_msg_t * nlmsg_info_ptr )
{
  ...
  /* Ignore Expiring Addresses if SSC not supported. Otherwise include them in cnf */
  if (!netmgr_main_cfg.sscm3_config &&
      nlmsg_info_ptr->addr_info.attr_info.param_mask & NETMGR_NLA_PARAM_CACHEINFO)
  {
...
/* Current enabled feature. Will Enter this. */
        ...
        netmgr_kif_info[link].dns_v6_queried == FALSE)
  {
    if (netmgr_kif_info[link].dns_v6_queried == FALSE)
    {
      ...
      netmgr_qmi_get_modem_link_info((uint8)link, WDS_IP_FAMILY_IPV6_V01, 0);
  ...
  /* Update address cache for later use */
  if (NETMGR_SUCCESS != netmgr_sscm3_addr_store(link, nlmsg_info_ptr, &addr_ptr))
  {
    NETMGR_NL_REPORT_ADDR(err, "Error during processing new addr",
                          (*sa_ptr));
    return;
  }
  ...
  switch( state = netmgr_kif_get_state( link ) ) {
    ...
    case NETMGR_KIF_RECONFIGURING:
      ...
      /*reconfig_ip_type must match the new address */
  ...
  if (NETMGR_INVALID_EV != event ) {
    ...
    cmd_buf->data.type = event; /* Can be NETMGR_KIF_(RE/)CONFIGURED_EV*/
    cmd_buf->data.link = link;
    cmd_buf->data.info.connect_msg.nlmsg_info = *nlmsg_info_ptr;
    cmd_buf->data.info.connect_msg.addr_info_ptr = addr_ptr;
    cmd_buf->data.info.connect_msg.reconfig_required = FALSE;

    netmgr_log_high("%s(): posting configure event for PDU ID %u\n", __func__, addr_ptr->pdu_id);
    /* Post command for processing in the command thread context */
    if( NETMGR_SUCCESS != netmgr_exec_put_cmd( cmd_buf ) ) {
      netmgr_exec_release_cmd(cmd_buf);
      NETMGR_ABORT("failed to put commmand");
...
netmgr_kif_cfg_fail (int link, const netmgr_nl_msg_t * nlmsg_info_ptr)
{
  ...
  const struct ds_sockaddr_storage *sa_ptr = NULL;
  netmgr_address_set_t *addr_ptr = NULL;
  netmgr_addr_type addr_type = NETMGR_ADDR_TYPE_DEFAULT;
  int pdu_id = 0;
  boolean purge_pdu = FALSE;
  ...
  sa_ptr = &nlmsg_info_ptr->addr_info.attr_info.prefix_addr;
  if (NETMGR_SUCCESS != netmgr_sscm3_addr_del(link, nlmsg_info_ptr, &addr_ptr,
                                              &purge_pdu))
  {
    NETMGR_NL_REPORT_ADDR(err, "Error during processing del addr",
                          (*sa_ptr));
    return;
  }

  /* Update the PDU if a valid address was found and removed */
  if (addr_ptr)
  {
    pdu_id = addr_ptr->pdu_id;
    addr_type = addr_ptr->addr_type;
  }
  ...
  switch (state = netmgr_kif_get_state(link)) {
    ...
    case NETMGR_KIF_RECONFIGURING:
      ...
      if (NETMGR_ADDR_TYPE_DEFAULT == addr_type)
      {
        netmgr_sscm3_candidate_promote(link, (AF_INET == sa_ptr->ss_family) ?
                                             NETMGR_IPV4_ADDR : NETMGR_IPV6_ADDR);
      }

      if (purge_pdu)
      {
        netmgr_qmi_reset_link_wds_data_by_family(link, (pdu_id) ? SECONDARY_PDU : PRIMARY_PDU,
                                                 (AF_INET == sa_ptr->ss_family) ?
                                                 NETMGR_IPV4_ADDR : NETMGR_IPV6_ADDR);
      }
  ...
  switch (state = netmgr_kif_get_state(link)) {
    ...
    case NETMGR_KIF_CLOSING:
      ...
      */
      ...
      netmgr_kif_delete_all_route_timers(link);
      clntcb = netmgr_kif_get_clntcb(link);
      if( clntcb ) {
        clnt_hdl = netmgr_kif_get_clnt_hdl(link);
...
netmgr_kif_is_ifindex_wlan(uint32 ifindex)
{
  ...
  boolean ret = FALSE;
  int idx;
  char *wlan_ifname[NETMGR_KIF_MAX_WLAN_INTF] = {NETMGR_KIF_INTF_WLAN0, NULL};

  if (NETMGR_MAIN_CFG_IS_IWLAN_CONCURRENCY_SET())
  {
    wlan_ifname[1] = NETMGR_KIF_INTF_WLAN1;
  }

  for (idx = 0; idx < NETMGR_KIF_MAX_WLAN_INTF && wlan_ifname[idx]; idx++)
  {
    /* Call ioctl on device to get the if index */
    memset( &ifr, 0, sizeof(struct ifreq) );
    if (( NETMGR_SUCCESS ==
        netmgr_kif_call_ioctl_on_dev( wlan_ifname[idx], SIOCGIFINDEX, &ifr ) ))
    {
      ret = (ifindex == (uint32) ifr.ifr_ifindex) ? TRUE : FALSE;
      netmgr_log_high("%s(): wlan%d ifidx:%d\n",__func__, idx, ifr.ifr_ifindex);
      if ( ret ) {
       netmgr_log_high("%s(): Found wlan%d matching ifindex=[%d]\n",
                  __func__, idx, ifindex);
       return ret;
      }
    }
    else
    {
      netmgr_log_err("%s(): Cannot get ifindex for dev %s!\n", __func__,
                      wlan_ifname[idx]);
    }
  }

  netmgr_log_high("%s(): Could not find matching wlan for ifindex=[%d]\n",
                  __func__, ifindex);
  return ret;
}
...
/*=========================================================================*/
LOCAL void
netmgr_kif_process_wlan_up_event(char *wlan_ifname)
{
  netmgr_exec_cmd_t * cmd_buf = NULL;
  ...
  for (i = 0; i < netmgr_main_cfg.max_links_in_total; ++i) {
    ...
    if (WDS_TECHNOLOGY_NAME_WLAN_LOCAL_BRKOUT_V01 == tech ||
       ...
       netmgr_log_med("%s(): posting WLAN opened event on link=%d tech: %d\n",
                     __func__, i, tech);
      ...
      cmd_buf->data.link = i;
      memcpy(cmd_buf->data.info.wlan_ifname, wlan_ifname, NETMGR_IF_NAME_MAX_LEN);
...
/*=========================================================================*/
LOCAL void
netmgr_kif_process_wlan_down_event(char* wlan_ifname)
{
  netmgr_exec_cmd_t * cmd_buf = NULL;
  wds_technology_name_enum_v01 tech;
  int i;
...
LOCAL void netmgr_kif_iface_notify(int link, int enable)
{
  ...
  ifname = netmgr_kif_get_name(link);
  if (!ifname && strncmp(ifname, "rmnet_data", 10)) {
  ...
  sock = netmgr_socket(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0);
  if (sock < 0) {
...
netmgr_kif_nl_recv_link_msg (const netmgr_nl_msg_t  * nlmsg_info_ptr)
{
    ...
    ** care about. */
    ...
    if( ( iface_change != clat_iface_st ) &&
      ...
      if( iface_change && netmgr_main_cfg.clat_rps_mask > 0 ) {
        ...
        if ( NETMGR_SUCCESS !=
             netmgr_kif_set_rps_mask( link_name, 0, netmgr_main_cfg.clat_rps_mask) ) {
  ...
  if( ifinfo->ifi_change & IFF_UP ) {
    if( ifinfo->ifi_flags & IFF_UP ) {
      ...
      if (is_ifindex_wlan) {
        netmgr_log_high("wlan interface up %s\n", link_name);
        netmgr_kif_process_wlan_up_event(link_name);
      }
      else {
      ...
      if (is_ifindex_wlan) {
        netmgr_log_high("wlan interface down %s\n", link_name);
        netmgr_kif_process_wlan_down_event(link_name);
      }
      else {
...
#ifdef FEATURE_DATA_IWLAN
/*===========================================================================
  FUNCTION  netmgr_kif_get_wlan_by_ipaddr
===========================================================================*/
/*!
@brief
  Get the wlan interface information based on ipaddress

@return
  return the wlan ifname corresponding to the ipaddress
...
*/
/*=========================================================================*/
static char *netmgr_kif_get_wlan_by_ipaddr(char *ipaddr, int family)
{
  int idx, rc;
  struct ifaddrs *addr, *addriter;
  char *dev_name;
  char host[NI_MAXHOST];

  if (getifaddrs(&addr) != 0)
  {
    netmgr_log_err("%s() Could not invoke getifaddrs", __func__);
    goto err0;
  }

  dev_name = calloc(IFNAMSIZ, sizeof(char));
  if (dev_name == NULL)
  {
    netmgr_log_err("%s() Could not allocate dev_name", __func__);
    goto err1;
  }

  for (addriter = addr; addriter; addriter = addriter->ifa_next) {
    if (addriter->ifa_addr == NULL)
    {
      continue;
    }

    if (addriter->ifa_addr->sa_family == family)
    {
      rc = getnameinfo(addriter->ifa_addr,
                       family == AF_INET ? sizeof(struct sockaddr_in) : sizeof(struct sockaddr_in6),
                       host, NI_MAXHOST, NULL, 0, NI_NUMERICHOST);
      if (rc != 0)
      {
        netmgr_log_err("%s() getnameinfo failed - %s", __func__, strerror(errno));
        continue;
      }

      if (strstr(host, ipaddr))
      {
        rc = strlcpy(dev_name, addriter->ifa_name, IFNAMSIZ);
        if (rc > IFNAMSIZ)
        {
          netmgr_log_err("%s() String truncation occurred", __func__);
          goto err2;
...
err1:
  freeifaddrs(addr);
err0:
  netmgr_log_err("%s() Could not find matching wlan for IP %s family:%s",
                 __func__, ipaddr, family == AF_INET ? "AF_INET" : "AF_INET6");
  return NULL;

success:
  freeifaddrs(addr);
  netmgr_log_low("%s() Found matching wlan interface %s for IP %s family:%s",
                 __func__, dev_name, ipaddr, family == AF_INET ? "AF_INET" : "AF_INET6");

  return dev_name;
}

/*===========================================================================
  FUNCTION  netmgr_kif_save_wlan_ifname_to_link
===========================================================================*/
/*!
@brief
  save the wlan_ifname to the link info
...
void
netmgr_kif_save_wlan_ifname_to_link(int link, char* ipaddr, int family)
{
  netmgr_address_info_t *addr_info_ptr = NULL;
  const char *ifname = NULL;

  addr_info_ptr = netmgr_qmi_get_addr_info( link, PRIMARY_PDU);
  if (addr_info_ptr && NETMGR_MAIN_CFG_IS_IWLAN_CONCURRENCY_SET())
  {
    if ((ifname = netmgr_kif_get_wlan_by_ipaddr(ipaddr, family)) != NULL) {
        netmgr_log_low("%s() updated Wlan ifname: %s for link:%d family:%d ",
                        __func__, ifname, link, family);
        addr_info_ptr->wlan_info.is_info_valid = TRUE;
        ds_memscpy(addr_info_ptr->wlan_info.wlan_ifname, NETMGR_IF_NAME_MAX_LEN, ifname, strlen(ifname));
    }
    else
    {
      netmgr_log_err("%s() Could not find Wlan iface for ipaddr:%s"
                      "update failed for link %d  ", __func__, ipaddr, link);
    }
   }
   else if (addr_info_ptr)
   {
     addr_info_ptr->wlan_info.is_info_valid = false;
   }

}

/*===========================================================================
  FUNCTION  netmgr_kif_save_wwan_ifname_to_link
===========================================================================*/
/*!
@brief
  save the wwan_ifname to the link info
...
void
netmgr_kif_save_wwan_ifname_to_link(int link, char* ipaddr, int family)
{
  netmgr_address_info_t *addr_info_ptr = NULL;
  const char *ifname = NULL;
  ...
  {
    if ((ifname = netmgr_kif_get_wlan_by_ipaddr(ipaddr, family)) != NULL) {
        netmgr_log_low("%s() updated wwan ifname: %s for link:%d family:%d ",
                        __func__, ifname, link, family);
        addr_info_ptr->wwan_info.is_info_valid = TRUE;
        ds_memscpy(addr_info_ptr->wwan_info.wlan_ifname, NETMGR_IF_NAME_MAX_LEN, ifname, strlen(ifname));
    }
    else
    {
      netmgr_log_err("%s() Could not find wwan iface for ipaddr:%s"
                      "update failed for link %d  ", __func__, ipaddr, link);
    }
   }
   else if (addr_info_ptr)
   {
     addr_info_ptr->wwan_info.is_info_valid = false;
   }

}

/*===========================================================================
  FUNCTION  netmgr_kif_set_wlan_ifname_addrmask()
===========================================================================*/
/*!
@brief
  mark the wlan_ifname valid per family by setting valid mask.
...
/*=========================================================================*/
void
netmgr_kif_set_wlan_ifname_addrmask(int link, int family)
{
  netmgr_address_info_t *addr_info_ptr = NULL;
  ...
  {
    (AF_INET == family) ?
    (addr_info_ptr->wlan_info.valid_mask |= NETMGR_ADDRSET_MASK_IPV4):
    (addr_info_ptr->wlan_info.valid_mask |= NETMGR_ADDRSET_MASK_IPV6);
    netmgr_log_low("%s(): Setting valid_mask for link %d family: %d, mask:%d",
                   __func__, link, family, addr_info_ptr->wlan_info.valid_mask);
  }
}

/*===========================================================================
  FUNCTION  netmgr_kif_clear_wlan_ifname_on_link()
===========================================================================*/
/*!
@brief
  mark the wlan_ifname invalid only when config complete is done with cleanup
  for both IP families. valid_mask will be used to identify this.
...
/*=========================================================================*/
void
netmgr_kif_clear_wlan_ifname_on_link(int link, int family)
{
  netmgr_address_info_t *addr_info_ptr = NULL;
  ...
  if (addr_info_ptr && NETMGR_MAIN_CFG_IS_IWLAN_CONCURRENCY_SET())
  {
    ...
    (addr_info_ptr->wlan_info.valid_mask &= ~NETMGR_ADDRSET_MASK_IPV4):
    (addr_info_ptr->wlan_info.valid_mask &= ~NETMGR_ADDRSET_MASK_IPV6);
     netmgr_log_low("%s(): Clearing valid_mask for link %d family: %d, mask:%d",
                   __func__, link, family, addr_info_ptr->wlan_info.valid_mask);
   if (!addr_info_ptr->wlan_info.valid_mask)
    {
      addr_info_ptr->wlan_info.is_info_valid = false;
      netmgr_log_low("%s(): Invalidated_wlan info for link: %d",__func__, link);
    }
  }
}

/*===========================================================================
  FUNCTION  netmgr_kif_get_wlan_ifname_by_link()
===========================================================================*/
/*!
@brief
  return the wlan ifname from the link
...
/*=========================================================================*/
const char*
netmgr_kif_get_wlan_ifname_by_link(int link)
{
  netmgr_address_info_t *addr_info_ptr = NULL;

  if (!NETMGR_MAIN_CFG_IS_IWLAN_CONCURRENCY_SET())
  {
    netmgr_log_low("%s() Concurrency not supported, return wlan0", __func__);
    return NETMGR_KIF_INTF_WLAN0;
  }
  /* IWLAN uses primary PDU only */
  addr_info_ptr = netmgr_qmi_get_addr_info( link, PRIMARY_PDU);
  if (addr_info_ptr)
  {
    if (addr_info_ptr->wlan_info.is_info_valid) {
      netmgr_log_low("%s() Found wlan ifname[%s] by link[%d]",
                      __func__, addr_info_ptr->wlan_info.wlan_ifname,
                      link);
      return addr_info_ptr->wlan_info.wlan_ifname;
    }
  }
  netmgr_log_err("%s() Could not find a wlan ifname[%s] by link[%d]",
                      __func__, addr_info_ptr->wlan_info.wlan_ifname,
                      link);
  return NULL;
}
...
/*===========================================================================
  FUNCTION  netmgr_kif_get_wwan_ifname_by_link()
===========================================================================*/
/*!
@brief
  return the wlan ifname from the link
...
/*=========================================================================*/
const char*
netmgr_kif_get_wwan_ifname_by_link(int link)
{
  netmgr_address_info_t *addr_info_ptr = NULL;

  if (!NETMGR_MAIN_CFG_IS_IWLAN_CONCURRENCY_SET())
  {
    netmgr_log_low("%s() Concurrency not supported, return %s",
                    __func__, NETMGR_KIF_INTF_RMNET_DATA);
    return NETMGR_KIF_INTF_RMNET_DATA;
  }
  /* IWLAN uses primary PDU only */
  addr_info_ptr = netmgr_qmi_get_addr_info( link, PRIMARY_PDU);
  if (addr_info_ptr)
  {
    if (addr_info_ptr->wwan_info.is_info_valid) {
      netmgr_log_low("%s() Found wlan ifname[%s] by link[%d]",
                      __func__, addr_info_ptr->wwan_info.wlan_ifname,
                      link);
      return addr_info_ptr->wwan_info.wlan_ifname;
    }
  }
  netmgr_log_err("%s() Could not find a wlan ifname[%s] by link[%d]",
                      __func__, addr_info_ptr->wwan_info.wlan_ifname,
...
netmgr_kif_nl_recv_xfrm_newae_msg (const netmgr_nl_msg_t  * nlmsg_info_ptr)
{
  ...
  netmgr_address_info_t  *addr_info_ptr = NULL;
  netmgr_address_set_t *v4_addr, *v6_addr;
      ...
      for (i = 0; i < netmgr_main_cfg.max_links_in_total; i++)
      {
        if (NETMGR_KIF_IS_REV_RMNET_LINK(i))
        {
          /* Check the SPI with the one obtained from the netlink message */
            /* IWLAN uses primary PDU only */
          addr_info_ptr = netmgr_qmi_get_addr_info( i, PRIMARY_PDU);
          ...
          v6_addr = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV6_ADDR,
                                            NETMGR_ADDR_TYPE_DEFAULT);
          if (spi != v4_addr->sa.esp_spi_v4
              && spi != v6_addr->sa.esp_spi_v6)
          {
            /* We need to continue searching all rev_rmnet ports */
            continue;
          }
          else if (v4_addr->sa.esp_spi_v4 == spi)
          {
            netmgr_log_low("ESP SPI v4: 0x%08x link [%d]",
                           (unsigned int)v4_addr->sa.esp_spi_v4, i);
            reti = netmgr_qmi_initiate_esp_rekey(i, NETMGR_QMI_CLIENT_IPV4);
            break;
          }
          else if (v6_addr->sa.esp_spi_v6 == spi)
          {
            netmgr_log_low("ESP SPI v6: 0x%08x link [%d]",
                           (unsigned int)v6_addr->sa.esp_spi_v6, i);
...
/*===========================================================================
  FUNCTION  netmgr_kif_is_default_route_event
===========================================================================*/
/*!
@brief
  Function to return TRUE when the route event parameters match default route
  FALSE other wise

@return
  int - TRUE on default route match , FALSE otherwise
...
/*=========================================================================*/
LOCAL int
netmgr_kif_is_default_route_event (netmgr_nl_route_info_t     *route_info)
{
  /* Check if the route getting added or deleted is a default route*/
  /* Parameters to check for default route for forward rmnet interfaces are
   * 1. Scope -- RT_SCOPE_UNIVERSE
   * 2. Type -- RTN_UNICAST
   * 3. Protocol -- RTPROTO_RA
   * 4. Gateway address without the destination address --"default via <GW_IPv6>
   * 5. Output interface oif -- "dev rmnet_data<x>"
   * Route which matches above params will look like one below:
   * "default via <IPv6_address> dev rmnet_data<x> proto RA expires <time_in_secs> mtu <mtu_val>"
   */

  if ((route_info->metainfo.rtm_family == AF_INET6) &&
      (route_info->metainfo.rtm_type == RTN_UNICAST) &&
      (route_info->metainfo.rtm_scope == RT_SCOPE_UNIVERSE) &&
      (route_info->metainfo.rtm_protocol == RTPROT_RA) &&
      ((route_info->attr_info.param_mask & NETMGR_DEFAULT_ROUTE_ATTR_MASK) ==
       NETMGR_DEFAULT_ROUTE_ATTR_MASK) &&
      !(route_info->attr_info.param_mask & (NETMGR_ROUTE_PARAM_DST|NETMGR_ROUTE_PARAM_SRC))) {
    return TRUE;
  }
  return FALSE;
}
...
/*===========================================================================
  FUNCTION  netmgr_kif_delete_all_route_timers
===========================================================================*/
/*!
@brief
  Function to delete all routetimers
...
netmgr_kif_delete_all_route_timers(int link)
{
  ...
  for (index = 0; index < 2; index++) {
     if (netmgr_kif_info[link].route_timers[index].is_active) {
       netmgr_log_med("timer deleted timerid:0x%p %d",
                       netmgr_kif_info[link].route_timers[index].timerid,
                       link);
       timer_delete(netmgr_kif_info[link].route_timers[index].timerid);
       netmgr_kif_info[link].route_timers[index].is_active = FALSE;
       netmgr_kif_info[link].route_timers[index].link = -1;
     }
  }
}

/*===========================================================================
  FUNCTION  netmgr_kif_route_timer_expiry_hdlr
===========================================================================*/
/*!
@brief
  Function which gets called on timer expiry.

@return
  int - NETMGR_SUCCESS if timer expiry is handled , NETMGR_FAILURE otherwise
...
/*=========================================================================*/
LOCAL int
netmgr_kif_route_timer_expiry_hdlr(netmgr_timer_info_t *timer_info)
{
   int link = timer_info->link;
   netmgr_log_med("%s(), link: %d\n", __func__, link);

  if (netmgr_kif_verify_link(link) == NETMGR_FAILURE) {
   netmgr_log_err("Route timer expired for unknown/disabled link"
                  "Did you Forget to delete timer on link_down? \n",
                   link);
   return NETMGR_FAILURE;
  }

  if (NETMGR_SUCCESS == netmgr_kif_send_icmpv6_router_solicitation(link)) {
    netmgr_log_med("RS sent on link:%d successfully\n", link);
  }
...
/*===========================================================================
  FUNCTION  netmgr_kif_route_timer_create
===========================================================================*/
/*!
@brief
  Function which creates a route timer.

@return
  int - NETMGR_SUCCESS if timer is created, NETMGR_FAILURE otherwise
...
/*=========================================================================*/
LOCAL int
netmgr_kif_route_timer_create(int link, int timeout_in_sec, int is_global_addr)
{
  timer_t timerid;
  struct sigevent sev;
  ...
  sev.sigev_notify_function = &netmgr_kif_route_timer_expiry_hdlr;
  sev.sigev_value.sival_ptr = &netmgr_kif_info[link].route_timers[is_global_addr];

  if (timer_create(CLOCK_REALTIME, &sev, &timerid) != -1) {

    trigger.it_value.tv_sec = timeout_in_sec;
    timer_settime(timerid, 0, &trigger, NULL);

    if (netmgr_kif_info[link].route_timers[is_global_addr].is_active) {
      netmgr_log_med("timer is already active, deleting timerid: 0x%x, link: %d ",
                      netmgr_kif_info[link].route_timers[is_global_addr].timerid,
                      link);
      timer_delete(netmgr_kif_info[link].route_timers[is_global_addr].timerid);
      netmgr_kif_info[link].route_timers[is_global_addr].is_active = FALSE;
      netmgr_kif_info[link].route_timers[is_global_addr].link = -1;
    }

    netmgr_log_med("timer created successfully, timerid: 0x%x link: %d",
                    timerid, link);
    netmgr_kif_info[link].route_timers[is_global_addr].timerid = timerid;
    netmgr_kif_info[link].route_timers[is_global_addr].is_active = TRUE;
    netmgr_kif_info[link].route_timers[is_global_addr].link = link;
  }
  else {
    netmgr_log_med("timer create failed, link:%d, errno:%d", link, errno);
  }
...
/*===========================================================================
  FUNCTION  netmgr_kif_is_ipv6_addr_global
===========================================================================*/
/*!
@brief
  Function which check if ipv6address is global

@return
  int - TRUE if global ipv6 address, FALSE otherwise
...
/*=========================================================================*/
LOCAL int
netmgr_kif_is_ipv6_addr_global(struct ds_sockaddr_storage *addr)
{
  char linklocal[2] = {0xfe, 0x80};
  char address[INET6_ADDRSTRLEN];

  if (!inet_ntop(AF_INET6, addr->__data, address, INET6_ADDRSTRLEN))
  {
    netmgr_log_err("%s(): Invalid IPv6 address - %d %s", __func__, errno, strerror(errno));
    return FALSE;
  }

  if ((addr->ss_family == AF_INET6)  && !memcmp(addr->__data, linklocal, 2))
  {
    netmgr_log_low("%s(): not a global IPv6 address - %s", __func__, address);
    return FALSE;
  }

  netmgr_log_low("%s(): global IPv6 address - %s", __func__, address);
  return TRUE;
}

/*===========================================================================
  FUNCTION  netmgr_kif_nl_route_recv_msg
===========================================================================*/
/*!
@brief
  Function to process a received NETLINK ROUTE message
...
/*=========================================================================*/
LOCAL int
netmgr_kif_nl_route_recv_msg(netmgr_nl_msg_t  *nlmsg_info_ptr)
{
  int result = NETMGR_FAILURE;
  int link;
  netmgr_nl_route_info_t     *route_info;
  char linkname[IF_NAMESIZE];
  int  timeout, is_global_addr;

  NETMGR_ASSERT( nlmsg_info_ptr );
  NETMGR_LOG_FUNC_ENTRY;

  route_info = &nlmsg_info_ptr->route_info;

#ifdef FEATURE_DATA_IWLAN
  if (netmgr_kif_is_default_route_event(route_info)) {

    if ( (!route_info->attr_info.oif) ||
         ( NULL == if_indextoname(route_info->attr_info.oif, linkname)) ) {
      netmgr_log_err( "Not able interface name for ifidx:%d\n",
                      route_info->attr_info.oif );
      return result;
    }

    if (!strncmp(linkname, NETMGR_KIF_INTF_RMNET_DATA,
                  sizeof(NETMGR_KIF_INTF_RMNET_DATA)-1)) {
      if ((link = netmgr_kif_get_link_for_ifindex(route_info->attr_info.oif)) < 0) {
        netmgr_log_err( "Not able to get link for rmnet_data interface with ifindex: %d\n",
                        route_info->attr_info.oif);
        return result;
      }

      if (NETMGR_KIF_IS_REV_RMNET_LINK(link)) {
        netmgr_log_med("Link %d does not have a rev link associated, ignore");
        return result;
      }

      if (RTM_DELROUTE == nlmsg_info_ptr->type) {
        netmgr_log_med("Sending router solicitation.... on link %d\n", link);
        netmgr_kif_send_icmpv6_router_solicitation (link);
      }
      else if (RTM_NEWROUTE == nlmsg_info_ptr->type) {
        is_global_addr = netmgr_kif_is_ipv6_addr_global(&route_info->attr_info.gateway_addr);
        timeout = NETMGR_ROUTE_EXPIRY_TIMEOUT_PERCENT * NETMGR_MODEM_ROUTER_LIFETIME;
        netmgr_log_med("Create route timer for %s timeout:%d\n",
                         linkname, timeout);

         if (timeout)
            netmgr_kif_route_timer_create(link, timeout, is_global_addr);
      }
    }
    else {
      netmgr_log_med("Not a forward rmnet interface, ignoring link:%s oif:%d\n",
                      linkname, route_info->attr_info.oif);
    }
  }
  else {
    netmgr_log_med("Route event is not for a default route, ignoring");
  }
#endif /* FEATURE_DATA_IWLAN */
  result = NETMGR_SUCCESS;
  return result;
...
netmgr_kif_nl_msg_recv_cmd_exec (ds_cmd_t * cmd, void * data)
{
  ...
  switch( nlmsg_info.type ) {
    ...
    case RTM_NEWROUTE:
    case RTM_DELROUTE:
      netmgr_log_high("Received ROUTE event msg, processing\n");
      netmgr_kif_nl_route_recv_msg(&nlmsg_info);
      break;
...
#endif /* FEATURE_DATA_IWLAN */
/*===========================================================================
  FUNCTION  netmgr_kif_tipc_send_event
===========================================================================*/
/*!
@brief
  Generates an asynchronous event indication messages using TIPC socket.
...
)
{
  ...
  if( NETMGR_SUCCESS == result ) {
    ...
    result = netmgr_tipc_send_nl_msg(netmgr_kif_sk_grp.ev_sk.sk_fd,
                                     buffer,
                                     (uint16)buflen );

    netmgr_log_err("netmgr_tipc_send msg %d\n", result);

    if(result > 0)
      result = NETMGR_SUCCESS;

  } else {
    netmgr_log_err("failed on netmgr_tipc_encode_netmgr_event\n");
  }
...
/*===========================================================================
  FUNCTION  netmgr_kif_netlink_send_event
===========================================================================*/
/*!
@brief
...
)
{
  ...
  int user_sock = 0;
  int oemprxy_iface = NETMGR_INVALID_IFACE;
  size_t bytes = 0;
  int error = 0;
  NETMGR_LOG_FUNC_ENTRY;
  ...
  switch (cmd_data->cmd_id)
  {
    ...
    case NETMGR_USER_CMD_SCREEN_ON:
      ...
      (void) netmgr_ds_exec(cmd, NULL);
      break;
    case NETMGR_USER_CMD_QUERY_LINK_IFACE_MTU_ONLY:
    {
      unsigned int *iface_mtu;
      netmgr_wds_qmi_drv_info_t *wds_info = NULL;

      netmgr_log_med("%s(): Received QUERY_LINK_IFACE_MTU_ONLY cmd", __func__);
      event_info = (netmgr_nl_event_info_t*) malloc(
                   sizeof(netmgr_nl_event_info_t));
      if(NULL == event_info)
      {
        netmgr_log_err("%s(): Out of memory error ", __func__);
        return;
      }
      ...
      if( cmd_data->link < 0 || cmd_data->link >= NETMGR_MAX_LINK )
      {
        netmgr_log_err("%s(): Invalid Link Received ", __func__);
      }
      else
      {
          wds_info = netmgr_qmi_get_link_wds_info(cmd_data->link);
          iface_mtu = &event_info->cmd_data.data.iface_mtu;
          /*
           * We can bypass checking the validity of v4_pkt_conn/v6_pkt_conn since on
           * the wds disconnect event mtu's get knocked back down to 0
           */
          *iface_mtu = (unsigned int) wds_info->iface_mtu;

          netmgr_log_med("%s(): iface MTU = %u", __func__, *iface_mtu);
    ...
    case NETMGR_USER_CMD_QUERY_LINK_MTU:
    case NETMGR_USER_CMD_QUERY_LINK_MTU_PDU_ID:

    {
      netmgr_v4_v6_mtu_t *v4_v6_mtu = NULL;
      netmgr_wds_qmi_drv_info_t *wds_info = NULL;
      int validpdu = PRIMARY_PDU;

      netmgr_log_med("netmgr_kif_process_user_cmd: Received QUERY_LINK_MTU "
                     "cmd");
      ...
      /* Determine PDU index based on cmd and pdu_id */
      if (cmd_data->cmd_id == NETMGR_USER_CMD_QUERY_LINK_MTU_PDU_ID){
        if (cmd_data->pdu_id != PRIMARY_PDU  && cmd_data->pdu_id != SECONDARY_PDU)
        {
          netmgr_log_err("%s: Invalid pdu_id ",__func__);
          error = 1;
        }
        else
        {
          validpdu =(!cmd_data->pdu_id)? PRIMARY_PDU : SECONDARY_PDU;
        }
      }
      if(error == 0)
      {
          wds_info = netmgr_qmi_get_link_wds_info_pdu(cmd_data->link, validpdu);
          v4_v6_mtu = &event_info->cmd_data.data.v4_v6_mtu;
          /*
           * We can bypass checking the validity of v4_pkt_conn/v6_pkt_conn since on
           * the wds disconnect event mtu's get knocked back down to 0
           */
          v4_v6_mtu->v4_mtu = (unsigned int) wds_info->mtu_ipv4;
          v4_v6_mtu->v6_mtu = (unsigned int) wds_info->mtu_ipv6;

          netmgr_log_med("%s(): v4 MTU = %u, v6 MTU = %u",
                         __func__, v4_v6_mtu->v4_mtu, v4_v6_mtu->v6_mtu);
    ...
    case NETMGR_USER_CMD_QUERY_OEMPROXY_DATAPATH_IFACE:
    {
      netmgr_log_med("netmgr_kif_process_user_cmd: Received "
                     "NETMGR_USER_CMD_QUERY_OEMPROXY_DATAPATH_IFACE cmd");
      ...
      /* Check if a reverse rmnet link is available to service this request */
      oemprxy_iface = netmgr_qmi_find_available_rev_iface();

      if (NETMGR_INVALID_IFACE != oemprxy_iface)
      {
        /* Cache WLAN iface name sent by DSI into KIF link structure to be used
           later for datapath setup */
        bytes = strlcpy(netmgr_kif_info[oemprxy_iface].oemprxy_info.wlan_iface_name,
                        cmd_data->netmgr_oemprxy_req_params.wlan_iface_name,
                        NETMGR_IF_NAME_MAX_LEN);
        if (NETMGR_IF_NAME_MAX_LEN <= bytes)
        {
          netmgr_log_err("%s(): truncation occurred!", __func__);
          netmgr_kif_info[oemprxy_iface].oemprxy_info.wlan_iface_name_valid = FALSE;
          event_info->cmd_data.txn.txn_status = NETMGR_USER_CMD_STATUS_FAILED;
        }
        else
        {
          netmgr_kif_info[oemprxy_iface].oemprxy_info.wlan_iface_name_valid = TRUE;
          event_info->cmd_data.data.netmgr_oemprxy_resp_params.oemprxy_iface_id = oemprxy_iface;
          event_info->cmd_data.txn.txn_status = NETMGR_USER_CMD_STATUS_SUCCESS;
        }
      }
      else
...
)
{
  for(;;)
  {
    ...
    else if (event_info->event == NETMGR_USER_CMD)
    {
     ...
     else if (user_cmd_data->cmd_id ==  NETMGR_USER_CMD_QUERY_MATCHING_CALL_IFACE)
     {
       netmgr_nl_event_info_t *event_info_resp = NULL;

       netmgr_log_high("%s(): received request for checking matching call", __func__);
       ...
       event_info_resp->event = NETMGR_USER_CMD;
       event_info_resp->user_cmd = NETMGR_USER_CMD_QUERY_MATCHING_CALL_IFACE;
       event_info_resp->param_mask |= NETMGR_EVT_PARAM_USER_CMD;
       event_info_resp->param_mask |= NETMGR_EVT_PARAM_CMD_DATA;

       netmgr_log_err("%s DBG  r_lup %d  pmask %d pi %d "
                       "tpref %d xt_t_pref %d  ippref %d apn %s datagin %d",
                       __func__,
                       user_cmd_data->rl_params.route_lookup,
                       user_cmd_data->rl_params.params_mask,
       ...
       if (NETMGR_SUCCESS != netmgr_qmi_check_matching_call(&(user_cmd_data->rl_params),
                                                            &(event_info_resp->cmd_data.matching_call_check_resp),
                                                            user_cmd_data->modem_subs_id))
       {
         event_info_resp->cmd_data.txn.txn_status = NETMGR_USER_CMD_STATUS_FAILED;
         event_info_resp->cmd_data.matching_call_check_resp.device_name_valid = 0;
         netmgr_log_err("%s(): matching call check failed!", __func__);
       }
       else
       {
         event_info_resp->cmd_data.txn.txn_status = NETMGR_USER_CMD_STATUS_SUCCESS;
...
/*=========================================================================*/
LOCAL int
netmgr_kif_configure( int link, netmgr_ip_addr_t addr_type, int pdu_status )
{
  int ret = NETMGR_FAILURE;
  netmgr_kif_state_t state;
  ...
  do
  {
    ...
    switch( state = netmgr_kif_get_state(link) ) {
      ...
      /* In SSM3 there are two cases where we can be here, WDS_CONNECTED with no secondary PDU
       * and WDS_CONNECTED where Secondary PDU is immediately available from the start.
       */
        netmgr_log_med("Starting address type %d configuration on link %d\n",
                       addr_type, link);


        if (pdu_status == WDS_SEC_PDU_AVAILABLE_V01)
        {
          /* If we get AVAILABLE and 1st WDS connected event at same time we install an expiring primary PDU firstt then we install
           * forever address Secondary PDU
           */
          if( NETMGR_SUCCESS == (ret = netmgr_kif_cfg_req(link, addr_type, FALSE, PRIMARY_PDU, NETMGR_KIF_LFT_ADD_W_LFT)) ) {
            ret = NETMGR_SUCCESS;
          } else {
            netmgr_log_err("netmgr_kif_cfg_req failed for link %d\n", link);
            /* Primary PDU configuration has failed and there is no sense in proceeding with
               the secondary PDU. We will bail out here and return failure to clients */
            break;
          }
          if( NETMGR_SUCCESS == (ret = netmgr_kif_cfg_req(link, addr_type, FALSE, SECONDARY_PDU, NETMGR_KIF_LFT_DEFAULT)) ) {
            ret = NETMGR_SUCCESS;
          } else {
            netmgr_log_err("netmgr_kif_cfg_req failed for second PDU link %d\n", link);
          }
        }
        else {
            if( NETMGR_SUCCESS == (ret = netmgr_kif_cfg_req(link, addr_type, FALSE, PRIMARY_PDU, NETMGR_KIF_LFT_DEFAULT)) ) {
              ret = NETMGR_SUCCESS;
            } else {
              netmgr_log_err("netmgr_kif_cfg_req failed for link %d\n", link);
            }
...
LOCAL int
netmgr_kif_purge_and_maybe_reconfig
(
  int link,
  netmgr_address_set_t *addr_info_ptr,
  int pdu,
  boolean reconfig,
  netmgr_kif_lft_change_t lft
)
{
  int ret;
  ...
  if (NETMGR_FAILURE == netmgr_kif_verify_link(link))
  {
    netmgr_log_err("%s(): called with invalid link\n", __func__);
    return NETMGR_FAILURE;
  }

  if (!addr_info_ptr)
  {
    netmgr_log_err("%s(): No address information provided, ignoring input\n",
                   __func__);
    return NETMGR_FAILURE;
  }

  netmgr_kif_purge_address(link, &addr_info_ptr->if_addr,
                           addr_info_ptr->if_mask);
  if (!reconfig)
  {
    /* User only wanted purge */
    return NETMGR_SUCCESS;
  }

  netmgr_log_med("Starting address reconfiguration on link=%d, family=%d\n",
                 link, addr_info_ptr->if_addr.type);
  ret = netmgr_kif_cfg_req(link, addr_info_ptr->if_addr.type,
                           addr_info_ptr->is_reconfig, pdu, lft);
  ...
  {
    netmgr_log_err("netmgr_kif_cfg_req failed ofr link %d\n", link);
  }
                        ...
                        netmgr_address_set_t * addr_info_ptr )
{
  int ret = NETMGR_FAILURE;
  netmgr_kif_state_t state;
  netmgr_address_set_t * sec_pdu_ptr = NULL;
  int need_lfts = 0;
  ...
  do
  {
    ...
    switch( state = netmgr_kif_get_state(link) ) {
      ...
      case NETMGR_KIF_RECONFIGURING:
        ...
        /* Handle the legacy events same as before */
        sec_pdu_ptr = addr_info_ptr->sec_addr_info_ptr;
        if (!sec_pdu_ptr)
        {
          ret = netmgr_kif_purge_and_maybe_reconfig(link, addr_info_ptr,
                                                    PRIMARY_PDU, TRUE,
                                                    NETMGR_KIF_LFT_DEFAULT);
          break;
        }

        /* Reconfig is occuring for Primary PDU so purge and re-install. */
        if (addr_info_ptr->is_reconfig &&
          /*Ignore if we get an invalid PROMOTED + primary reconfig as are going to flush primary anyways*/
          !(sec_pdu_ptr && ((sec_pdu_ptr->pdu_status == WDS_SEC_PDU_PROMOTED_V01) ||
                           /*Available will do reconfig in its own if statment*/
                            (sec_pdu_ptr->pdu_status == WDS_SEC_PDU_AVAILABLE_V01))))
        {

          netmgr_kif_purge_address( link, &addr_info_ptr->if_addr, addr_info_ptr->if_mask );
          netmgr_log_med("Starting Primary PDU address reconfiguration on link=%d, family=%d\n",
                         link, addr_info_ptr->if_addr.type);
          if( NETMGR_SUCCESS == (ret = netmgr_kif_cfg_req(link, addr_info_ptr->if_addr.type,
                                                          addr_info_ptr->is_reconfig,
                                                          PRIMARY_PDU,
                                                          /* If secondary is active LFT is needed otherwise install as default. */
                                                          (need_lfts)? NETMGR_KIF_LFT_ADD_W_LFT :
                                                                       NETMGR_KIF_LFT_DEFAULT)) )
          {
            /* Don't move the local breakout interface to reconfig state (as there's no
               address assignment) */
        ...
        /*Weird IND from modem where Primary PDU is being reconfiged but also flushed. */
        else if (addr_info_ptr->is_reconfig &&
                 sec_pdu_ptr && (sec_pdu_ptr->pdu_status == WDS_SEC_PDU_PROMOTED_V01))
        {
          netmgr_log_err("netmgr_kif_cfg_req weird reconfig on prmotion pdu ind for link %d\n", link);
        }

        /* Order of reconfig and pdu status's need to be kept, change this order carfullly.*/
        if (sec_pdu_ptr->pdu_status == WDS_SEC_PDU_AVAILABLE_V01)
        {
          /* This will install new Secondary PDU address */
          if( NETMGR_SUCCESS == (ret = netmgr_kif_cfg_req(link, sec_pdu_ptr->if_addr.type,
                                                          /* IPv6 needs reconfig unconditionally to trigger RS */
                                                          (NETMGR_IPV6_ADDR == sec_pdu_ptr->if_addr.type) ?
                                                          TRUE : sec_pdu_ptr->is_reconfig,
                                                          SECONDARY_PDU, NETMGR_KIF_LFT_DEFAULT)) )
          {
             netmgr_log_med("Debug: Waiting for adding of new Secondary IP %d on link %d\n",
                       sec_pdu_ptr->if_addr.type, link);
              /* Don't move the local breakout interface to reconfig state (as there's no
               address assignment) */

            netmgr_kif_set_state(link, NETMGR_KIF_RECONFIGURING);
            netmgr_kif_info[link].reconfig_ip_type = sec_pdu_ptr->if_addr.type;
            netmgr_log_med("Debug: Waiting for IP %d Newaddr on link %d\n",
                       sec_pdu_ptr->if_addr.type, link);
            /*Needed if primary is undergoing a reconfig of it's own. */
            if (addr_info_ptr->is_reconfig)
            {
              netmgr_kif_purge_address( link, &addr_info_ptr->if_addr, addr_info_ptr->if_mask );
            }
            /* This will change Primary PDU address to new lft, modem should have been queried as
             * part of exec cmd formation
             */
            if( NETMGR_SUCCESS == (ret = netmgr_kif_cfg_req(link, addr_info_ptr->if_addr.type,
                                                            addr_info_ptr->is_reconfig,
                                                            PRIMARY_PDU,
                                                            /*If reconfiged then addr was purged, ip add needed instead */
                                                            (addr_info_ptr->is_reconfig)? NETMGR_KIF_LFT_ADD_W_LFT : NETMGR_KIF_LFT_VALID_CHANGE)) )
            {
              netmgr_log_med("Debug: Waiting for changing of primary IP %d lft on link %d\n",
                             sec_pdu_ptr->if_addr.type, link);
            }
            ret = NETMGR_SUCCESS;
          }
        }
        /* Release secondary address, if primary reconfig is happening, then primary should have
         * been purged/re-added by previous reconfig code otherwise we have duty to make old address permenant.
         */
        else if (sec_pdu_ptr->pdu_status == WDS_SEC_PDU_RELEASED_V01)
        {
          /* Change Primary PDU address, To be permanent and then Flush Secondary PDU*/
          if( !addr_info_ptr->is_reconfig &&
              NETMGR_SUCCESS == (ret = netmgr_kif_cfg_req(link, addr_info_ptr->if_addr.type,
                                                          addr_info_ptr->is_reconfig,
                                                          PRIMARY_PDU, NETMGR_KIF_LFT_MAKE_PERM)) )
          {
             netmgr_log_med("Debug: Making primary PDU permanent %d lft on link %d\n",
                       addr_info_ptr->if_addr.type, link);
            /* Don't move the local breakout interface to reconfig state (as there's no
               address assignment) */
          }
          ...
          netmgr_log_med("Debug: Waiting for IP %d Newaddr on link %d\n",
                        addr_info_ptr->if_addr.type, link);
          ret = NETMGR_SUCCESS;
          /* Purge Secondary PDU address */
          netmgr_kif_purge_address( link, &sec_pdu_ptr->if_addr, sec_pdu_ptr->if_mask );
        }
        /* Secondary PDU reconfig */
        else if (sec_pdu_ptr->is_reconfig )
        {
          /* Purge Secondary PDU address */
          netmgr_kif_purge_address( link, &sec_pdu_ptr->if_addr, sec_pdu_ptr->if_mask );

          /* This will install new Secondary PDU address with a ip change command but make it a permenant addr */
          if ( NETMGR_SUCCESS == (ret = netmgr_kif_cfg_req(link, sec_pdu_ptr->if_addr.type,
                                                          sec_pdu_ptr->is_reconfig,
                                                          SECONDARY_PDU,
                                                          NETMGR_KIF_LFT_MAKE_PERM)) )
          {
            ...
            netmgr_kif_info[link].reconfig_ip_type = sec_pdu_ptr->if_addr.type;
            ret = NETMGR_SUCCESS;
            netmgr_log_med("Debug: Waiting for re-config of new Secondary IP %d on link %d\n",
                       sec_pdu_ptr->if_addr.type, link);
          }
        }

        /* No else if here because there is a chance for Promoted + sec reconfig, Currently we reconfig sec addr 1st and then we purge
         * primary addr.
         */
        if (sec_pdu_ptr->pdu_status == WDS_SEC_PDU_PROMOTED_V01)
        {
          /* Purge Primary PDU address*/
          netmgr_kif_purge_address( link, &addr_info_ptr->if_addr, addr_info_ptr->if_mask );
          /* Swap will be done Sychronous */
          netmgr_kif_pdu_swap(link, sec_pdu_ptr->if_addr.type);
                  ...
                  boolean   force_cleanup )
{
  int rval = NETMGR_FAILURE;
  netmgr_kif_state_t state;
  netmgr_address_info_t * address_info = netmgr_qmi_get_addr_info(link, PRIMARY_PDU);
  netmgr_address_set_t *tmp_addr;
  int ip_family = AF_INET;
  struct netmgr_kif_cb_ifinfo ifinfo;
  const char *local_addr = NULL, *dest_addr = NULL;
  int tunnel_family;
  ...
  /* Verify the link id first */
  if (netmgr_kif_verify_link(link) == NETMGR_FAILURE) {
    netmgr_log_err("%s(): netmgr_kif_close called with invalid link %d\n", __func__, link);
    goto error;
  }

  /* We call kif_close for error scenarios as well. In those cases we should blindly cleanup
     and bring down the link to ensure KIF state is clean */
  if (force_cleanup)
  {
    goto opening_skip;
  }
  ...
  {
    switch(addr_info_ptr->if_addr.type)
    {
      case NETMGR_IPV4_ADDR:
        ip_family = AF_INET;
        break;
      case NETMGR_IPV6_ADDR:
        ...
        break;
      default:
        if (netmgr_qmi_wds_get_tech_name(link) == WDS_TECHNOLOGY_NAME_EMBMS_V01)
        {
          netmgr_log_err("%s(): ip_family [%d] received for EMBMS call\n",
                       __func__, addr_info_ptr->if_addr.type);
          goto embms_skip;
        }

        if (teardown_iface &&
            (netmgr_kif_get_state(link) == NETMGR_KIF_OPENING))
        {
          goto opening_skip;
        }

        /*Dont know what can be done here to send a correct rev_ip_config_complete*/
        netmgr_log_err("%s(): Wrong ip_family received, close should be called with one IP family, rcvd [%d]\n",
                       __func__, addr_info_ptr->if_addr.type);
        goto error;
    }
  }
  ...
  {
    netmgr_log_err("%s(): netmgr_kif_close called with NULL add_info_ptr on link %d\n", __func__, link);
    goto error;
  }
...
embms_skip:
  ...
  {
  case AF_INET:
    tmp_addr = netmgr_qmi_get_addr_set(address_info, NETMGR_IPV4_ADDR,
                                       NETMGR_ADDR_TYPE_DEFAULT);
    ifinfo.addr.type = NETMGR_KIF_CB_ADDR_IPV4;
    ifinfo.addr._in4.addr = tmp_addr->if_addr.addr.v4;
    ifinfo.addr._in4.mask = tmp_addr->if_mask;
    break;
  case AF_INET6:
    tmp_addr = netmgr_qmi_get_addr_set(address_info, NETMGR_IPV6_ADDR,
                                       NETMGR_ADDR_TYPE_DEFAULT);
    ifinfo.addr.type = NETMGR_KIF_CB_ADDR_IPV6;
    memcpy(&ifinfo.addr._in6.addr,
           &tmp_addr->if_addr.addr,
           sizeof(ifinfo.addr._in6.addr));
    ifinfo.addr._in6.mask = tmp_addr->if_mask;
    break;
  default:
  ...
  if (NETMGR_KIF_IS_REV_RMNET_LINK(link))
  {
    ...
    /*Now since we have decided that we are processing disconnect, we need to update
     *first_conn_clnt because for rev links below operations are costly.
    */
    wds_info_ptr = netmgr_qmi_get_link_wds_info_pdu(link, PRIMARY_PDU);
    if((wds_info_ptr != NULL) && (NULL != addr_info_ptr))
    {
      if(wds_info_ptr->first_conn_clnt == netmgr_qmi_get_first_conn_clnt_from_ip_family_type(addr_info_ptr->if_addr.type))
      {
        netmgr_log_high("%s(): Resetting first_conn_clnt prev_val [%d] disconn rcvd for ip_family [%d]\n",
                         __func__,
                         wds_info_ptr->first_conn_clnt,
                         addr_info_ptr->if_addr.type);
        wds_info_ptr->first_conn_clnt = NETMGR_QMI_CLIENT_MAX;
      }
    }
  }
#endif /* FEATURE_DATA_IWLAN */
  ...
  if (NETMGR_KIF_IS_REV_RMNET_LINK(link) && NULL != addr_info_ptr)
  {
    ...
    if (NETMGR_KIF_CB_IF_TECH_TYPE_EPDG_PROXY == ifinfo.tech_type
          && netmgr_kif_info[link].oemprxy_info.wlan_iface_name_valid == TRUE)
    {
      (void) strlcpy(ifinfo.wlan_ifname,
                     netmgr_kif_info[link].oemprxy_info.wlan_iface_name,
    ...
    {
      netmgr_log_med("netmgr_kif_close: link=%d is not being torn down, sending rev_ip_config_complete\n",
                     link);
      netmgr_qmi_send_rev_ip_config_complete(NETMGR_QMI_IWLAN_CALL_CLEANUP,
             ...
             NETMGR_LINK_MAX != (assoc_fwd_link = netmgr_qmi_iwlan_get_link_assoc(link)))
    {
      /*Since an association between forward and reverse link exist, clear that now*/
      if(NETMGR_SUCCESS != netmgr_qmi_iwlan_clear_link_assoc(assoc_fwd_link, link))
      {
        netmgr_log_err("%s(): Unable to clear assoc between fwd link [%d] rev link [%d]\n",
                        __func__, assoc_fwd_link, link);
      }
  ...
  switch (state = netmgr_kif_get_state(link)) {
    ...
    case NETMGR_KIF_OPENING:
      ...
      else
      {
        ...
        if( addr_info_ptr && addr_info_ptr->sec_addr_info_ptr )
        {
          netmgr_kif_purge_address( link,
                                    &(((netmgr_address_set_t*)(addr_info_ptr->sec_addr_info_ptr))->if_addr),
                                    ((netmgr_address_set_t*)(addr_info_ptr->sec_addr_info_ptr))->if_mask );
          addr_info_ptr->is_addr_purge_pend = FALSE;
          netmgr_kif_set_mtu(link,TRUE);
        }
      }
...
netmgr_kif_netlink_init( void )
{
  ...
  /* Open a netlink/tipc socket for NETLINK_GENERIC protocol. This socket
  ** is used to receive/generate netlink messaging related to NetMgr
  ** event indications for client multicast group
  */
  if( netmgr_main_cfg.netmgr_listen_ev_proto ) {
    if( NETMGR_SUCCESS !=
      netmgr_tipc_listener_init(&netmgr_kif_sk_grp.sk_thrd_info,
                                  netmgr_kif_sk_grp.sk_thrd_fdmap,
                                  ds_arrsize(netmgr_kif_sk_grp.sk_thrd_fdmap),
                                  &netmgr_kif_sk_grp.ev_sk,
                                  NULL))
    {
      NETMGR_ABORT("netmgr_kif_netlink_init: Error on netmgr_tipc_listener_init\n");
      return;
    }
  }
...
#ifdef RMNET_IOCTL_EXTENDED_V2
  netmgr_kif_set_dev_hw_feature_flags(link_name);
#endif /* RMNET_IOCTL_EXTENDED_V2 */
...
netmgr_kif_init_iface( int link )
{
...
#endif
  {
    ...
    netmgr_log_high("%s(): setting router_solicitation_delay on interface [%s] : status [%s]",
                    __func__, link_name, cmd_rc == NETMGR_SUCCESS ? "SUCCESS" : "FAILURE");
    if (netmgr_main_cfg.sscm3)
    {
      cmd_rc = netmgr_kif_set_secondary_addr_promote(link_name, NETMGR_DEV_SECONDARY_ADDR_PROMOTE_ENABLED);
      netmgr_log_high("%s() setting promote_secondaries on interface [%s] : status[%s]\n",
                      __func__, link_name, cmd_rc == NETMGR_SUCCESS ? "SUCCESS" : "FAILURE");
    }
  }
...
)
{
  ...
  {
    /* IWLAN uses primary PDU only */
    netmgr_address_info_t *addr_info = netmgr_qmi_get_addr_info( link, PRIMARY_PDU);
    ...
    else
    {
      ...
      netmgr_kif_remove_sa_and_routing_rules(link, AF_INET,
                                             netmgr_qmi_get_addr_set(addr_info, NETMGR_IPV4_ADDR,
                                                                     NETMGR_ADDR_TYPE_DEFAULT));
      netmgr_kif_remove_sa_and_routing_rules(link, AF_INET6,
                                             netmgr_qmi_get_addr_set(addr_info, NETMGR_IPV6_ADDR,
                                                                     NETMGR_ADDR_TYPE_DEFAULT));

      /* Remove XFRM policies which handle specific use-cases
       * Ex. skip encryption of NS and NA */
      NETMGR_KIF_UTIL_REMOVE_IPSEC_XFRM_POLICY_EX(XFRM_POLICY_OUT);
    }

#ifdef NETMGR_USE_IWLAN_CLIENT
    if (((NSWO_ONLY == netmgr_main_get_iwlan_enabled() ||
          NSWO_V2_ONLY == netmgr_main_get_iwlan_enabled()) &&
...
/*===========================================================================
  FUNCTION netmgr_notify_clients_init_start
===========================================================================*/
/*!
@brief
  Notifies clients that intialization is started. Used in case of restart.

@return
  None
*/
/*=========================================================================*/
static void netmgr_notify_clients_init_start(void)
{
  netmgr_nl_event_info_t *event_info = NULL;

  /*After restart, notify clients that we are not ready to accept calls now*/
  event_info = netmgr_malloc(sizeof(netmgr_nl_event_info_t));
  if(NULL == event_info)
  {
    netmgr_log_err("%s(): failed to allocate event buffer\n", __func__);
    return;
  }
  else
  {
    memset(event_info, 0x0, sizeof(netmgr_nl_event_info_t));
    event_info->event = NETMGR_USER_CMD;
    event_info->cmd_data.netmgr_restart_state = NETMGR_POST_RESTART_STATE_INIT_START;
    event_info->param_mask |= NETMGR_EVT_PARAM_CMD_DATA;

    if (NETMGR_SUCCESS != netmgr_kif_send_event_msg(event_info))
    {
       netmgr_log_err("%s(): failed on notifying clients about netmgr state post restart\n",
                       __func__);
    }
    netmgr_log_high("%s(): Notified NETMGR_POST_RESTART_STATE_INIT_START to clients\n", __func__);
   }
...
int netmgr_kif_vnd_cleanup_then_notify_clients(void)
{
  ...
  netmgr_notify_clients_init_start();
  for (link = 0; link < netmgr_main_cfg.num_data_ports; link++)
  {
    /* close the interface if open; this ensures we are in DOWN state
     * initially. */
    flags = 0;
    ...
    /* Post event PLATFORM_DOWN_EV indication to clients */
    event_info = netmgr_malloc( sizeof(netmgr_nl_event_info_t) );
    if(NULL == event_info)
    {
  ...
  /* Post NETMGR_POST_RESTART_STATE_INIT_ONGOING indication to clients */
  event_info = netmgr_malloc( sizeof(netmgr_nl_event_info_t) );
  if(NULL == event_info)
  {
    netmgr_log_err("%s(): failed to allocate event buffer for link %u!\n",
  ...
  else
  {
    ...
    event_info->cmd_data.netmgr_restart_state = NETMGR_POST_RESTART_STATE_INIT_ONGOING;
    event_info->param_mask |= NETMGR_EVT_PARAM_CMD_DATA;
    ...
    netmgr_log_high("%s(): Notified client that netmgr has not completed initialization\n",
                   __func__);
  }
  return rc;
}
...
LOCAL void netmgr_kif_cleanup( void )
{
  ...
  (void) netmgr_kif_vnd_cleanup_then_notify_clients();
...
static void netmgr_kif_update_rmnet_proc_entries(int i)
{
  ...
  if (NETMGR_KIF_IS_REV_RMNET_LINK(i))
  {
    ...
    /* Disable accept_ra on reverse Rmnets to stop sending unnecessary RSs */
    memset(proc_file, 0, NETMGR_MAX_COMMAND_LENGTH);
    ...
    if (NETMGR_SUCCESS != netmgr_kif_update_system_config(proc_file, NETMGR_CMDQ_TYPE_INT, val))
    {
      netmgr_log_err("%s(): failed to disable accept_ra on link [%s]", __func__, link_name);
    }
  }
  else
#endif /* FEATURE_DATA_IWLAN */
...
/*===========================================================================
  FUNCTION  netmgr_kif_install_default_xfrm_policies
===========================================================================*/
/*!
@brief
  Installs the IPSec policy selectors in the specified direction
...
/*=========================================================================*/
LOCAL int
netmgr_kif_install_default_xfrm_policies
(
  netmgr_xfrm_state_id_t *sa_state_id,
...
)
{
  ...
  netmgr_address_set_t *ip_addr = NULL;
  unsigned int prefix_len;
  ...
  {
    ip_addr = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV4_ADDR,
                                      NETMGR_ADDR_TYPE_DEFAULT);
    prefix_len = (unsigned int)ds_get_num_bits_set_count(ip_addr->if_mask);
  }
  else if (NETMGR_IPV6_ADDR == ip_type)
  {
    ip_addr = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV6_ADDR,
                                      NETMGR_ADDR_TYPE_DEFAULT);
    prefix_len = ip_addr->if_mask;
  }
  else
  {
    netmgr_log_err("%s(): unknwon addr type=%d\n", __func__, ip_type);
    goto bail;
  }
  ...
  {
    rc = netmgr_xfrm_add_policies(sa_state_id, iface_addr_buf, NULL, NULL, TRUE);
  }
  else
  {
    rc = netmgr_xfrm_add_policies(sa_state_id, NULL, iface_addr_buf, NULL, FALSE);
  }
...
)
{
  ...
  if (!netmgr_xfrm_utils_get_crypto_algo_valid(sa_config, sa_config_ex_frag, frag_count))
  {
    netmgr_log_err("%s(): crypto mask not set\n", __func__);
    goto bail;
  }

  if ((NETMGR_SA_DIR_TX == dir && !(netmgr_xfrm_utils_get_crypto_key_tx_valid(sa_config, sa_config_ex_frag, frag_count))) ||
      (NETMGR_SA_DIR_RX == dir && !(netmgr_xfrm_utils_get_crypto_key_rx_valid(sa_config, sa_config_ex_frag, frag_count))))
  {
    netmgr_log_err("%s(): crypto key not present in dir=%d", __func__, dir);
    goto bail;
  }

  if (!(ealgo = netmgr_util_get_ipsec_algo_str(NETMGR_IPSEC_ALGO_CRYPTO,
                                               sa_config, sa_config_ex_frag, frag_count)))
  {
    netmgr_log_err("%s(): failed to convert crypto algo=%d to string",
                   __func__, netmgr_xfrm_utils_get_crypto_algo(sa_config, sa_config_ex_frag, frag_count));
    goto bail;
  }

  if (NETMGR_SA_DIR_TX == dir)
  {
    ekey = netmgr_xfrm_utils_get_crypto_key_tx(sa_config, sa_config_ex_frag, frag_count);
    ekey_len = netmgr_xfrm_utils_get_crypto_key_tx_len(sa_config, sa_config_ex_frag, frag_count);
  }
  else
  {
    ekey = netmgr_xfrm_utils_get_crypto_key_rx(sa_config, sa_config_ex_frag, frag_count);
    ekey_len = netmgr_xfrm_utils_get_crypto_key_rx_len(sa_config, sa_config_ex_frag, frag_count);
  }
  ...
  if (NETMGR_SUCCESS != netmgr_util_convert_qmi_ipsec_key_to_str(ekey,
                                                                 ...
                                                                 sizeof(ekey_str)))
  {
    netmgr_log_err("%s(): failed to convert crypto key to string", __func__);
    goto bail;
  }
...
)
{
  ...
  if (!netmgr_xfrm_utils_get_hash_algo_valid(sa_config, sa_config_ex_frag, frag_count))
  {
  ...
  if (( NETMGR_SA_DIR_TX == dir
          && !((netmgr_xfrm_utils_get_hash_key_tx_valid(sa_config, sa_config_ex_frag, frag_count))
               || (sa_config->ext_hash_key_tx_valid)
               || (sa_config->ext2_hash_key_tx_valid)) )
      ||
      ( NETMGR_SA_DIR_RX == dir
          && !((netmgr_xfrm_utils_get_hash_key_rx_valid(sa_config, sa_config_ex_frag, frag_count))
  ...
  if (!(halgo = netmgr_util_get_ipsec_algo_str(NETMGR_IPSEC_ALGO_HASH, sa_config, sa_config_ex_frag, frag_count)))
  {
    netmgr_log_err("%s(): failed to convert hash algo=%d to string",
                   __func__, netmgr_xfrm_utils_get_hash_algo(sa_config, sa_config_ex_frag, frag_count));
    goto bail;
  }
  ...
  if (NETMGR_SA_DIR_TX == dir)
  {
    ...
    {
      hkey = netmgr_xfrm_utils_get_hash_key_tx(sa_config, sa_config_ex_frag, frag_count);
      hkey_len = netmgr_xfrm_utils_get_hash_key_tx_len(sa_config, sa_config_ex_frag, frag_count);
    }
  }
  else
  {
    ...
    {
      hkey = netmgr_xfrm_utils_get_hash_key_rx(sa_config, sa_config_ex_frag, frag_count);
      hkey_len = netmgr_xfrm_utils_get_hash_key_rx_len(sa_config, sa_config_ex_frag, frag_count);
    }
  }
  ...
  switch (netmgr_xfrm_utils_get_hash_algo(sa_config, sa_config_ex_frag, frag_count))
  {
...
)
{
  ...
  if (NETMGR_SUCCESS !=
      netmgr_util_convert_qmi_ip_addr_to_str(netmgr_xfrm_utils_get_local_addr(sa_config, sa_config_ex_frag, frag_count),
  ...
  if (NETMGR_SUCCESS !=
      netmgr_util_convert_qmi_ip_addr_to_str(netmgr_xfrm_utils_get_dest_addr(sa_config, sa_config_ex_frag, frag_count),
                                             dest_addr, sizeof(dest_addr)))
  {
    netmgr_log_err("%s(): failed to convert SA dest address to string!", __func__);
    goto bail;
  }
  if (netmgr_xfrm_utils_get_udp_encap_src_port_valid(sa_config, sa_config_ex_frag, frag_count))
  {
    /* Update ikeport with TLV's port network to host order*/
    modem_port = netmgr_main_cfg_update_ike_port(ntohs(netmgr_xfrm_utils_get_udp_encap_src_port(sa_config, sa_config_ex_frag, frag_count)),
  ...
  if (netmgr_xfrm_utils_get_is_udp_encaps(sa_config, sa_config_ex_frag, frag_count))
  {
  ...
  for (i = 0; i < num_ts; i++)
  {
    ...
    else
    {
         ...
         )
      {
        ...
        if ( NETMGR_SUCCESS !=
               netmgr_util_convert_ip_addr_to_str( &ip_addr->if_addr,
                                                   prefix_len,
                                                   iface_addr_buf,
                                                   sizeof(iface_addr_buf) ) )
        {
...
/*===========================================================================
  FUNCTION  netmgr_kif_install_nl_traffic_selector_policies
===========================================================================*/
/*!
@brief
  Installs the XFRM policies based on traffic selector criteria (Netlink
  flavor)
...
/*=========================================================================*/
LOCAL int
netmgr_kif_install_nl_traffic_selector_policies
(
  netmgr_xfrm_state_id_t *sa_state_id,
  netmgr_sa_ts_t         *netmgr_tsi,
  netmgr_sa_ts_t         *netmgr_tsr,
      ...
      if (NETMGR_SA_DIR_TX == sa_dir)
      {
        ...
        for (i = 0; i < netmgr_tsi->num_ts; i++)
        {
          for (j = 0; j < netmgr_tsr->num_ts; j++)
          {
            ...
            /* Install the xfrm policy */
            if (NETMGR_SUCCESS
                 != netmgr_xfrm_add_policies(sa_state_id,
                                             netmgr_tsi->ts_list[i].addr_range,
                                             netmgr_tsr->ts_list[j].addr_range,
                                             netmgr_tsi->ts_list[i].proto,
                                             TRUE))
            {
              netmgr_log_err("%s(): Failed to add traffic selector policy sel!\n", __func__);
              reti = NETMGR_FAILURE;
              break;
            }
          } /* 'j' loop*/
...
/*===========================================================================
  FUNCTION  netmgr_kif_remove_nl_traffic_selector_policies
===========================================================================*/
/*!
@brief
  Removes the XFRM policies for traffic selectors (Netlink flavor)
...
/*=========================================================================*/
LOCAL int
netmgr_kif_remove_nl_traffic_selector_policies
(
  netmgr_sa_ts_t         *netmgr_tsi,
  netmgr_sa_ts_t         *netmgr_tsr,
  netmgr_ts_mode_t       ts_mode,
      ...
      if (NETMGR_SA_DIR_TX == sa_dir)
      {
        ...
        for (i = 0; i < netmgr_tsi->num_ts; i++)
        {
          for (j = 0; j < netmgr_tsr->num_ts; j++)
          {
            ...
            reti = netmgr_xfrm_remove_policies(netmgr_tsi->ts_list[i].addr_range,
                                               netmgr_tsr->ts_list[j].addr_range,
                                               netmgr_tsi->ts_list[i].proto,
                                               TRUE);
            if (NETMGR_FAILURE == reti)
            {
              netmgr_log_err("%s(): Failed to remove TX traffic selector policy\n", __func__);
              break;
            }
          } /* 'j' loop*/
        } /* 'i' loop */
      ...
      else
      {
        ...
        for (i = 0; i < netmgr_tsi->num_ts; i++)
        {
          for (j = 0; j < netmgr_tsr->num_ts; j++)
          {
            ...
            reti = netmgr_xfrm_remove_policies(netmgr_tsr->ts_list[j].addr_range,
                                               netmgr_tsi->ts_list[i].addr_range,
                                               netmgr_tsr->ts_list[j].proto,
                                               FALSE);
            if (NETMGR_FAILURE == reti)
            {
              netmgr_log_err("%s(): Failed to remove RX traffic selector policy\n", __func__);
              break;
            }
          } /* 'j' loop */
        } /* 'i' loop */
  ...
  case NETMGR_TS_MODE_TSI_ONLY:
    {
      ...
      else
      {
        ...
        for (i = 0; i < netmgr_tsi->num_ts; i++)
        {
          ...
          reti = netmgr_xfrm_remove_policies(NULL,
                                             netmgr_tsi->ts_list[i].addr_range,
                                             netmgr_tsi->ts_list[i].proto,
                                             FALSE);
          if (NETMGR_FAILURE == reti)
  ...
  case NETMGR_TS_MODE_TSR_ONLY:
    {
      ...
      else
      {
        ...
        for (i = 0; i < netmgr_tsr->num_ts; i++)
        {
          ...
          reti = netmgr_xfrm_remove_policies(netmgr_tsr->ts_list[i].addr_range,
                                             NULL,
                                             netmgr_tsr->ts_list[i].proto,
                                             FALSE);
          if (NETMGR_FAILURE == reti)
...
LOCAL void netmgr_kif_qmi_ip_to_netmgr_ip
(
  netmgr_ip_address_t *naddr,
  wds_ip_addr_type_v01 *waddr
)
{
  if (WDS_DATA_CALL_ADDR_FAMILY_IPV4_V01 == waddr->addr_family)
  {
    uint32_t *v4_addr = (uint32_t *)waddr->ip_addr;

    naddr->type = NETMGR_IPV4_ADDR;
    naddr->addr.v4 = htonl(*v4_addr);
  }
  else if (WDS_DATA_CALL_ADDR_FAMILY_IPV6_V01 == waddr->addr_family)
  {
    naddr->type = NETMGR_IPV6_ADDR;
    memcpy(naddr->addr.v6_addr8, waddr->ip_addr, waddr->ip_addr_len);
  }
}
...
)
{
  ...
  netmgr_ip_addr_t   ip_type;
  /* Primary is used for IWLAN. */
  netmgr_address_info_t  *addr_info_ptr = netmgr_qmi_get_addr_info(link, PRIMARY_PDU);
  netmgr_address_set_t *addr_set;
  int rc = NETMGR_FAILURE;
  unsigned int prefix_len = 0;
  wds_get_ipsec_static_sa_config_resp_msg_v01  sa_config;
  wds_get_ipsec_static_sa_config_ex_resp_msg_v01 sa_config_ex_resp;
  wds_ipsec_static_sa_config_ex_info_ind_msg_v01 **sa_config_ex_frag = NULL;
  file_info_t file_info;
  int file_index = 0, i = 0;
  ...
  memset(&netmgr_qmi_wds_ipsec_config_ex_frags.first_frag, 0,
         sizeof(wds_ipsec_static_sa_config_ex_info_ind_msg_v01));

  memset(&sa_config, 0, sizeof(wds_get_ipsec_static_sa_config_ex_resp_msg_v01));

  /* Block here to synchronize the transaction id.
   * Reset reassembly count here for this transaction.
   */
  netmgr_qmi_wds_ipsec_config_ex_frags.num_reassembled = 0;
  netmgr_qmi_wds_ipsec_config_ex_frags.other_frags = NULL;
  netmgr_qmi_wds_block_ipsec_static_sa_config_ind();
  if (NETMGR_SUCCESS != netmgr_qmi_query_ipsec_sa_config_ex(ip_family, link, &sa_config_ex_resp))
  {
    netmgr_qmi_wds_unblock_ipsec_static_sa_config_ind();
    if (NETMGR_SUCCESS != netmgr_qmi_query_ipsec_sa_config(ip_family, link, &sa_config))
    {
      netmgr_log_err("%s(): failed to query SAs for ip_family=%d, link=%d\n",
                     __func__, ip_family, link);
      goto bail;
    }
  }
  else
  {
    netmgr_log_low("%s(): transaction id [%d:%d]\n", __func__, sa_config_ex_resp.transaction_id_valid, sa_config_ex_resp.transaction_id);
    if (sa_config_ex_resp.transaction_id_valid)
    {
      netmgr_qmi_wds_ipsec_config_ex_frags.transaction_id = sa_config_ex_resp.transaction_id;
    }
    else
    {
      netmgr_log_err("%s(): transaction id not set\n", __func__);
      netmgr_qmi_wds_unblock_ipsec_static_sa_config_ind();
      goto bail;
    }

    /* still holding the lock here to ensure that we dont miss the signal from netmgr_qmi_wds */
    if (netmgr_qmi_wds_get_ipsec_static_sa_config_ex_sync())
    {
      netmgr_log_err("%s(): netmgr_qmi_wds_get_ipsec_static_sa_config_ex_sync timed out", __func__);
      goto bail;
    }
    else
    {
      netmgr_log_low("%s(): reassembled sa ex ind\n", __func__);
      frag_count = netmgr_qmi_wds_ipsec_config_ex_frags.num_reassembled;
      sa_config_ex_frag = calloc(frag_count, sizeof(wds_ipsec_static_sa_config_ex_info_ind_msg_v01 *));
      if (!sa_config_ex_frag)
      {
        netmgr_log_err("%s(): fragment memory allocation failure link [%d]", __func__, link);
        goto bail;
      }
      sa_config_ex_frag[0] = &netmgr_qmi_wds_ipsec_config_ex_frags.first_frag;

      for (i = 0; i < (frag_count - 1); i++)
      {
        sa_config_ex_frag[i + 1] = &netmgr_qmi_wds_ipsec_config_ex_frags.other_frags[i];
      }
    }
  }

  /* Make sure that the basic parameters are present */
  if (!netmgr_xfrm_utils_get_ipsec_sa_protocol_valid(&sa_config, sa_config_ex_frag, frag_count) ||
      !netmgr_xfrm_utils_get_encapsulation_mode_valid(&sa_config, sa_config_ex_frag, frag_count) ||
      !netmgr_xfrm_utils_get_dest_addr_valid(&sa_config, sa_config_ex_frag, frag_count) ||
      !netmgr_xfrm_utils_get_local_addr_valid(&sa_config, sa_config_ex_frag, frag_count) ||
      !netmgr_xfrm_utils_get_spi_rx_valid(&sa_config, sa_config_ex_frag, frag_count) ||
      !netmgr_xfrm_utils_get_spi_tx_valid(&sa_config, sa_config_ex_frag, frag_count) ||
      !netmgr_xfrm_utils_get_hash_algo_valid(&sa_config, sa_config_ex_frag, frag_count) ||
      !netmgr_xfrm_utils_get_crypto_algo_valid(&sa_config, sa_config_ex_frag, frag_count))
  {
  ...
  if (AF_INET == ip_family)
  {
    ...
    addr_set = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV4_ADDR,
                                       NETMGR_ADDR_TYPE_DEFAULT);
    sa = &addr_set->sa;
  }
  else
  {
    ...
    addr_set = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV6_ADDR,
                                       NETMGR_ADDR_TYPE_DEFAULT);
    sa = &addr_set->sa;
  }
  ...
  if (NETMGR_SUCCESS !=
      netmgr_util_convert_qmi_ip_addr_to_str(netmgr_xfrm_utils_get_dest_addr(&sa_config, sa_config_ex_frag, frag_count),
                                             sa->tunnel_ep.dest_addr, sizeof(sa->tunnel_ep.dest_addr)))
  {
  ...
  if (NETMGR_SUCCESS !=
      netmgr_util_convert_qmi_ip_addr_to_str(netmgr_xfrm_utils_get_local_addr(&sa_config, sa_config_ex_frag, frag_count),
                                             sa->tunnel_ep.local_addr, sizeof(sa->tunnel_ep.local_addr)))
  {
  ...
  sa->tunnel_ep.ip_family = (WDS_DATA_CALL_ADDR_FAMILY_IPV4_V01 ==
                             ((wds_ip_addr_type_v01 *)netmgr_xfrm_utils_get_local_addr(&sa_config, sa_config_ex_frag, frag_count))->addr_family) ?
                                                                                       AF_INET : AF_INET6;
  sa->tunnel_ep.is_valid = TRUE;

  /* Get the protocol string */
  if (!(proto = netmgr_util_get_ipsec_proto_str(netmgr_xfrm_utils_get_ipsec_sa_protocol(&sa_config, sa_config_ex_frag, frag_count))))
  {
  ...
  if (!(mode = netmgr_util_get_ipsec_mode_str(netmgr_xfrm_utils_get_encapsulation_mode(&sa_config, sa_config_ex_frag, frag_count))))
  {
  ...
  if (netmgr_xfrm_utils_get_traffic_selector_list_valid(&sa_config, sa_config_ex_frag, frag_count))
  {
    /* Get the number of TSi selectors */
    sa->sa_tsi.num_ts = netmgr_xfrm_utils_get_traffic_selector_list_len(&sa_config, sa_config_ex_frag, frag_count);
  }

  if (netmgr_xfrm_utils_get_traffic_selector_responder_list_valid(&sa_config, sa_config_ex_frag, frag_count))
  {
    /* Get the number of TSr selectors */
    sa->sa_tsr.num_ts = netmgr_xfrm_utils_get_traffic_selector_responder_list_len(&sa_config, sa_config_ex_frag, frag_count);
  }
      ...
      NETMGR_TS_MODE_BOTH_ENABLED == sa->ts_mode)
  {
    ...
    if (NETMGR_SUCCESS != netmgr_kif_store_ts_params(addr_info_ptr,
                                                     netmgr_xfrm_utils_get_traffic_selector_list(&sa_config, sa_config_ex_frag, frag_count),
                                                     sa->sa_tsi.ts_list,
                                                     sa->sa_tsi.num_ts,
                                                     TRUE))
    {
      ...
      NETMGR_TS_MODE_BOTH_ENABLED == sa->ts_mode)
  {
    ...
    if (NETMGR_SUCCESS != netmgr_kif_store_ts_params(addr_info_ptr,
                                                     netmgr_xfrm_utils_get_traffic_selector_responder_list(&sa_config, sa_config_ex_frag, frag_count),
                                                     sa->sa_tsr.ts_list,
                                                     sa->sa_tsr.num_ts,
                                                     FALSE))
    {
...
if (netmgr_main_cfg.netmgr_recovery_enabled)
{
  ...
  if (AF_INET == ip_family)
  {
    inet_ntop(AF_INET, &addr_set->if_addr.addr.v4,
              file_info.recovery_info.link_info.link_addr,
              NETMGR_MAX_STR_LENGTH);
  }
  else
  {
    inet_ntop(AF_INET6, &addr_set->if_addr.addr,
              file_info.recovery_info.link_info.link_addr,
              NETMGR_MAX_STR_LENGTH);

    snprintf(prefix, 8, "/%u", addr_set->if_mask);
  ...
  if (netmgr_xfrm_utils_get_spi_tx_valid(&sa_config, sa_config_ex_frag, frag_count))
  {
    if (netmgr_main_cfg.netmgr_recovery_enabled)
    {
      file_info.recovery_info.link_info.tunnel_info.spi_tx = netmgr_xfrm_utils_get_spi_tx(&sa_config, sa_config_ex_frag, frag_count);
    }
    ...
    snprintf(sa->sa_state_id[sa_dir],
             ...
             proto,
             netmgr_xfrm_utils_get_spi_tx(&sa_config, sa_config_ex_frag, frag_count));
    ...
    if (AF_INET == ip_family)
    {
      addr_set->sa.esp_spi_v4 = netmgr_xfrm_utils_get_spi_tx(&sa_config, sa_config_ex_frag, frag_count);
    }
    else
    {
      addr_set->sa.esp_spi_v6 = netmgr_xfrm_utils_get_spi_tx(&sa_config, sa_config_ex_frag, frag_count);
    }

    if (NETMGR_SUCCESS !=
       netmgr_kif_get_xfrm_state_encryption_str(&sa_config, sa_config_ex_frag, frag_count,
                                                sa_dir, encryption, sizeof(encryption)))
    {
    ...
    if (NETMGR_SUCCESS !=
        netmgr_kif_get_xfrm_state_authentication_str(&sa_config, sa_config_ex_frag, frag_count,
                                                     sa_dir, authentication, sizeof(authentication)))
    {
      netmgr_log_err("%s(): failed to get auth string", __func__);
      goto bail;
    }
  ...
  if (netmgr_xfrm_utils_get_spi_rx_valid(&sa_config, sa_config_ex_frag, frag_count))
  {
    if (netmgr_main_cfg.netmgr_recovery_enabled)
    {
      file_info.recovery_info.link_info.tunnel_info.spi_rx = netmgr_xfrm_utils_get_spi_rx(&sa_config, sa_config_ex_frag, frag_count);
    }
  ...
  if (netmgr_main_cfg.iwlan_enabled == IWLAN_V2_ENABLE)
  {
    if (!netmgr_xfrm_utils_get_udp_encap_src_port_valid(&sa_config, sa_config_ex_frag, frag_count) ||
        !netmgr_xfrm_utils_get_is_udp_encaps_valid(&sa_config, sa_config_ex_frag, frag_count) ||
        !netmgr_xfrm_utils_get_is_udp_encaps(&sa_config, sa_config_ex_frag, frag_count))
    {
      netmgr_log_low("%s(): not invoking netmgr_modem_proxy_add_connected_udp_encap", __func__);
    }
    else
    {
      netmgr_log_low("%s(): netmgr_modem_proxy_add_connected_udp_encap link %d ifname %s", __func__, link, ifname);
      source_port = netmgr_xfrm_utils_get_udp_encap_src_port(&sa_config, sa_config_ex_frag,
                                                             frag_count);

      if (netmgr_modem_proxy_add_connected_udp_encap(netmgr_kif_translate_wds_tech(netmgr_qmi_wds_get_tech_name(link)),
                                                     sa->tunnel_ep.ip_family, source_port,
                                                     sa->tunnel_ep.local_addr,
                                                     sa->tunnel_ep.dest_addr, ifname))
      {
        netmgr_log_err("%s(): netmgr_modem_proxy_add_connected_udp_encap failed", __func__);
        goto bail;
      }
    }
  }
...
bail:
  if (sa_config_ex_frag)
  {
    free(sa_config_ex_frag);
  }

  if (netmgr_qmi_wds_ipsec_config_ex_frags.other_frags)
  {
    free(netmgr_qmi_wds_ipsec_config_ex_frags.other_frags);
  }
...
/*===========================================================================
  FUNCTION  netmgr_kif_store_crypto_algo
===========================================================================*/
/*!
@brief
  Store crypto algorithm key, key length and algorithm name
  for a given link and family.
...
)
{
  ...
  netmgr_ipsec_sa_t  *sa = NULL;
  netmgr_address_set_t *addr_set;
  int aes_mode_valid = netmgr_xfrm_utils_get_aes_mode_valid(&sa_config, sa_config_ex_frag, frag_count);
  int aes_mode = netmgr_xfrm_utils_get_aes_mode(&sa_config, sa_config_ex_frag, frag_count);
  uint8_t *key = NULL;
  if(!addr_info_ptr)
  {
    netmgr_log_err("%s () Invalid Address Pointer", __func__);
    goto bail;
  }
  if (AF_INET == ip_family)
  {
  ...
  switch (netmgr_xfrm_utils_get_crypto_algo(&sa_config, sa_config_ex_frag, frag_count))
  {
    case WDS_IPSEC_CRYPTO_ALGO_AES128_V01:
    case WDS_IPSEC_CRYPTO_ALGO_AES256_V01:
      if (! aes_mode_valid ||
          WDS_IPSEC_AES_MODE_CBC_V01 == aes_mode)
      {
        algo_name = NETMGR_XFRM_CRYPTO_CBS_AES;
      }
      else if (WDS_IPSEC_AES_MODE_CTR_V01 == aes_mode)
      {
        algo_name = NETMGR_XFRM_CRYPTO_RFC_3686;
      }
      else
      {
        netmgr_log_err("%s(): invalid AES mode %d\n", __func__, aes_mode);
        goto bail;
      }

      break;
    case WDS_IPSEC_CRYPTO_ALGO_DES_V01:
      algo_name = NETMGR_XFRM_CRYPTO_CBS_DES;
      break;
    case WDS_IPSEC_CRYPTO_ALGO_3DES_V01:
      algo_name = NETMGR_XFRM_CRYPTO_CBS_3_DES;
      break;
    default:
      netmgr_log_err("%s(): invalid crypto algo %d\n", __func__,
                     netmgr_xfrm_utils_get_crypto_algo(&sa_config, sa_config_ex_frag, frag_count));
      goto bail;
  }
  memset(sa->nl_sa_state_id.crypto_algo, 0, sizeof(sa->nl_sa_state_id.crypto_algo));
  strlcpy(sa->nl_sa_state_id.crypto_algo, algo_name, strlen(algo_name) + 1);
  memset(sa->nl_sa_state_id.tx_crypto_key, 0, sizeof(sa->nl_sa_state_id.tx_crypto_key));
  sa->nl_sa_state_id.tx_crypto_key_len =
    netmgr_xfrm_utils_get_crypto_key_tx_len(&sa_config, sa_config_ex_frag, frag_count);
  key = netmgr_xfrm_utils_get_crypto_key_tx(&sa_config, sa_config_ex_frag, frag_count);
  if(!key)
  {
    netmgr_log_err("%s() Invalid TX Crypto Key", __func__);
    goto bail;
  }

  ds_memscpy(sa->nl_sa_state_id.tx_crypto_key, sizeof(sa->nl_sa_state_id.tx_crypto_key), key, sa->nl_sa_state_id.tx_crypto_key_len * sizeof(uint8_t));
  memset(sa->nl_sa_state_id.rx_crypto_key, 0, sizeof(sa->nl_sa_state_id.rx_crypto_key));
  key = netmgr_xfrm_utils_get_crypto_key_rx(&sa_config, sa_config_ex_frag, frag_count);
  if(!key)
  {
    netmgr_log_err("%s() Invalid RX Crypto Key", __func__);
    goto bail;
  }
  sa->nl_sa_state_id.rx_crypto_key_len =
    netmgr_xfrm_utils_get_crypto_key_rx_len(&sa_config, sa_config_ex_frag, frag_count);
  ds_memscpy(sa->nl_sa_state_id.rx_crypto_key, sizeof(sa->nl_sa_state_id.rx_crypto_key), key, sa->nl_sa_state_id.rx_crypto_key_len * sizeof(uint8_t));
  rc = NETMGR_SUCCESS;
bail:
  return rc;
}
/*===========================================================================
  FUNCTION  netmgr_kif_store_auth_algo
===========================================================================*/
/*!
@brief
  Store auth algorithm key, key length and algorithm name
  for a given link and family.
...
*/
/*=========================================================================*/
static int netmgr_kif_store_auth_algo
(
  int link,
  int ip_family,
  wds_get_ipsec_static_sa_config_resp_msg_v01 sa_config,
...
)
{
  ...
  if(!addr_info_ptr)
  {
    netmgr_log_err("%s(): Invalid addr info ptr", __func__);
    goto bail;
  }
  ...
  {
    case WDS_IPSEC_CRYPTO_ALGO_AES128_V01:
      algo_name = NETMGR_XFRM_AUTH_CBS_AES;
      break;
    case WDS_IPSEC_CRYPTO_ALGO_SHA_V01:
      algo_name = NETMGR_XFRM_AUTH_SHA1;
      break;
    case WDS_IPSEC_CRYPTO_ALGO_MD5_V01:
      algo_name = NETMGR_XFRM_AUTH_MD5;
      break;
    case WDS_IPSEC_CRYPTO_ALGO_SHA256_V01:
      algo_name = NETMGR_XFRM_AUTH_SHA256;
      trunc_len = 128;
      break;
    case WDS_IPSEC_CRYPTO_ALGO_SHA512_V01:
      algo_name = NETMGR_XFRM_AUTH_SHA512;
      trunc_len = 256;
      break;
    case WDS_IPSEC_CRYPTO_ALGO_SHA384_V01:
      algo_name = NETMGR_XFRM_AUTH_SHA384;
      trunc_len = 192;
      break;
    default:
      netmgr_log_err("%s(): invalid auth algo %d\n", __func__,
                     netmgr_xfrm_utils_get_hash_algo(&sa_config, sa_config_ex_frag, frag_count));
      goto bail;
  }

  memset(sa->nl_sa_state_id.tx_auth_key, 0, sizeof(sa->nl_sa_state_id.tx_auth_key));
  if (sa_config.ext2_hash_key_tx_valid)
  {
    key = sa_config.ext2_hash_key_tx;
    key_len = sa_config.ext2_hash_key_tx_len;
  }
   else if (sa_config.ext_hash_key_tx_valid)
  {
    key = sa_config.ext_hash_key_tx;
    key_len = sa_config.ext_hash_key_tx_len;
  }
  else
  {
    key = netmgr_xfrm_utils_get_hash_key_tx(&sa_config, sa_config_ex_frag, frag_count);
    key_len = netmgr_xfrm_utils_get_hash_key_tx_len(&sa_config, sa_config_ex_frag, frag_count);
  }
  if(!key)
  {
    netmgr_log_err("%s() Invalid TX Auth Key", __func__);
    goto bail;
  }
  sa->nl_sa_state_id.auth_trunc = trunc_len;
  sa->nl_sa_state_id.tx_auth_key_len = key_len;
  ds_memscpy(sa->nl_sa_state_id.tx_auth_key, sizeof(sa->nl_sa_state_id.tx_auth_key), key, key_len * sizeof(uint8_t));
  memset(sa->nl_sa_state_id.auth_algo, 0, sizeof(sa->nl_sa_state_id.auth_algo));
  strlcpy(sa->nl_sa_state_id.auth_algo, algo_name, strlen(algo_name) + 1);
  key = NULL;
  if (sa_config.ext2_hash_key_rx_valid)
  {
    key = sa_config.ext2_hash_key_rx;
    key_len = sa_config.ext2_hash_key_rx_len;
  }
  else if (sa_config.ext_hash_key_rx_valid)
  {
    key = sa_config.ext_hash_key_rx;
    key_len = sa_config.ext_hash_key_rx_len;
  }
  else
  {
    key = netmgr_xfrm_utils_get_hash_key_rx(&sa_config, sa_config_ex_frag, frag_count);
    key_len = netmgr_xfrm_utils_get_hash_key_rx_len(&sa_config, sa_config_ex_frag, frag_count);
  }
  if(!key)
  {
    netmgr_log_err("%s() Invalid RX Auth Key", __func__);
    goto bail;
  }
  sa->nl_sa_state_id.rx_auth_key_len = key_len;
  ds_memscpy(sa->nl_sa_state_id.rx_auth_key, sizeof(sa->nl_sa_state_id.rx_auth_key), key, key_len * sizeof(uint8_t));
  rc = NETMGR_SUCCESS;
bail:
  return rc;
}
/*===========================================================================
  FUNCTION  netmgr_kif_install_nl_sa_rules
===========================================================================*/
/*!
@brief
  Installs the SAs to the SAD (Security Association Database) and
  SPD (Security Policy Database) for the given family and link (Netlink
  flavor)
...
/*=========================================================================*/
LOCAL int
netmgr_kif_install_nl_sa_rules
(
  int link,
  int ip_family
)
{
  ...
  if (NETMGR_SUCCESS != netmgr_util_convert_qmi_ip_addr_to_str(netmgr_xfrm_utils_get_dest_addr(&sa_config, sa_config_ex_frag, frag_count),
                                                               sa->tunnel_ep.dest_addr,
  ...
  if (NETMGR_SUCCESS != netmgr_util_convert_qmi_ip_addr_to_str(netmgr_xfrm_utils_get_local_addr(&sa_config, sa_config_ex_frag, frag_count),
                                                               sa->tunnel_ep.local_addr,
  ...
  sa->tunnel_ep.ip_family = (WDS_DATA_CALL_ADDR_FAMILY_IPV4_V01 ==
                             ((wds_ip_addr_type_v01 *)netmgr_xfrm_utils_get_local_addr(&sa_config, sa_config_ex_frag, frag_count))->addr_family) ? AF_INET :
                                                                 AF_INET6;
  sa->tunnel_ep.is_valid = TRUE;
  ...
  /* Save XFRM ID state */
  netmgr_kif_qmi_ip_to_netmgr_ip(&sa->nl_sa_state_id.local_addr,
                                 netmgr_xfrm_utils_get_local_addr(&sa_config, sa_config_ex_frag, frag_count));
  netmgr_kif_qmi_ip_to_netmgr_ip(&sa->nl_sa_state_id.dest_addr,
                                 netmgr_xfrm_utils_get_dest_addr(&sa_config, sa_config_ex_frag, frag_count));
  switch (netmgr_xfrm_utils_get_ipsec_sa_protocol(&sa_config, sa_config_ex_frag, frag_count))
  {
    case WDS_IPSEC_SA_PROTOCOL_IPSEC_AH_V01:
      sa->nl_sa_state_id.proto = IPPROTO_AH;
      break;
    case WDS_IPSEC_SA_PROTOCOL_IPSEC_ESP_V01:
      sa->nl_sa_state_id.proto = IPPROTO_ESP;
      break;
    default:
      break;
  }

  sa->nl_sa_state_id.spi_tx = netmgr_xfrm_utils_get_spi_tx(&sa_config, sa_config_ex_frag, frag_count);
  sa->nl_sa_state_id.spi_rx = netmgr_xfrm_utils_get_spi_rx(&sa_config, sa_config_ex_frag, frag_count);
  switch (netmgr_xfrm_utils_get_encapsulation_mode(&sa_config, sa_config_ex_frag, frag_count))
  {
    case WDS_IPSEC_SA_ENCAPSULATE_TUNNEL_V01:
      sa->nl_sa_state_id.mode = XFRM_MODE_TUNNEL;
      break;
    default:
      sa->nl_sa_state_id.mode = XFRM_MODE_TRANSPORT;
      break;
  }

  sa->nl_sa_state_id.valid = TRUE;
  if(NETMGR_SUCCESS != netmgr_kif_store_crypto_algo(link, ip_family, sa_config, sa_config_ex_frag, frag_count) )
  {
    netmgr_log_err("%s(): failured to store crypto algo\n", __func__);
    goto bail;
  }
  if(NETMGR_SUCCESS != netmgr_kif_store_auth_algo(link, ip_family, sa_config, sa_config_ex_frag, frag_count) )
  {
    netmgr_log_err("%s(): failured to store auth algo\n", __func__);
    goto bail;
  }
if (netmgr_main_cfg.netmgr_recovery_enabled)
{
  ...
  memcpy(&file_info.recovery_info.link_info.tunnel_info.sa_state_id,
         &sa->nl_sa_state_id, sizeof(sa->nl_sa_state_id));
  (void) strlcpy(file_info.recovery_info.link_info.tunnel_info.mode,
                 mode, NETMGR_MAX_MODE_LENGTH);
  file_info.recovery_info.link_info.tunnel_info.ike_port
    = netmgr_main_cfg_get_ike_port(sa->tunnel_ep.local_addr, sa->tunnel_ep.dest_addr);
  ...
  /* Install XFRM sttes for 'out' and 'in' directions */
  if (NETMGR_FAILURE == netmgr_xfrm_add_states(&sa_config, sa_config_ex_frag, frag_count,
                                               netmgr_qmi_wds_get_tech_name(link)))
  {
    netmgr_log_err("%s(): failed to add XFRM states\n", __func__);
    goto bail;
  }

  if (netmgr_main_cfg.netmgr_recovery_enabled)
  {
    file_info.recovery_info.link_info.tunnel_info.sa_state |= SA_XFRM_STATE_OUT | SA_XFRM_STATE_IN;
    if (NETMGR_SUCCESS != netmgr_recovery_file_write_entry(&file_info))
    {
      netmgr_log_err("%s(): failed to update XFRM state in recovery file", __func__);
    }
  ...
  /* Install XFRM policies */
  if (NETMGR_TS_MODE_DISABLED == sa->ts_mode)
  {
    /* Install the default policies if traffic selector is disabled */
    if (NETMGR_SUCCESS
           != netmgr_kif_install_default_xfrm_policies(&sa->nl_sa_state_id,
                                                       addr_info_ptr,
                                                       ip_type,
                                                       NETMGR_SA_DIR_TX))
    {
      netmgr_log_err("%s(): failed to add xfrm policy sel string", __func__);
      goto bail;
    }
    ...
    if (netmgr_main_cfg.netmgr_recovery_enabled)
    {
      file_info.recovery_info.link_info.tunnel_info.sa_state |= SA_XFRM_POLICY_IN | SA_XFRM_POLICY_FWD;
      if (NETMGR_SUCCESS != netmgr_recovery_file_write_entry(&file_info))
      {
        netmgr_log_err("%s(): failed to update XFRM state in recovery file", __func__);
      }
  ...
  {
    /* Install traffic selector specific policies in 'out' direction */
    if (NETMGR_SUCCESS != netmgr_kif_install_nl_traffic_selector_policies(&sa->nl_sa_state_id,
                                                                          &sa->sa_tsi,
                                                                          &sa->sa_tsr,
                                                                          sa->ts_mode,
                                                                          NETMGR_SA_DIR_TX,
    ...
    /* Install traffic selector specific policies in 'in' and 'fwd' directions */
    if (NETMGR_SUCCESS != netmgr_kif_install_nl_traffic_selector_policies(&sa->nl_sa_state_id,
                                                                          &sa->sa_tsi,
                                                                          &sa->sa_tsr,
                                                                          sa->ts_mode,
  ...
  if (netmgr_main_cfg.iwlan_enabled == IWLAN_V2_ENABLE)
  {
    ...
    {
      netmgr_log_low("%s(): not invoking netmgr_modem_proxy_add_connected_udp_encap", __func__);
      sa->nl_sa_state_id.encap_valid = 0;
    }
    else
    {
      netmgr_log_low("%s(): netmgr_modem_proxy_add_connected_udp_encap link %d ifname %s", __func__, link, ifname);
      source_port = netmgr_xfrm_utils_get_udp_encap_src_port(&sa_config, sa_config_ex_frag,
                                                             frag_count);
      sa->nl_sa_state_id.modem_port = source_port;
      sa->nl_sa_state_id.encap_valid = 1;
      if (netmgr_modem_proxy_add_connected_udp_encap(netmgr_kif_translate_wds_tech(netmgr_qmi_wds_get_tech_name(link)),
                                                     sa->tunnel_ep.ip_family, source_port,
                                                     sa->tunnel_ep.local_addr,
                                                     sa->tunnel_ep.dest_addr, ifname))
...
/*===========================================================================
  FUNCTION  netmgr_kif_cleanup_nl_sa_rules
===========================================================================*/
/*!
@brief
...
*/
/*=========================================================================*/
int netmgr_kif_cleanup_nl_sa_rules(file_info_t *recovery_data)
{
  netmgr_xfrm_state_id_t *sa_state_id;
  ...
  sa_state_id = &recovery_data->recovery_info.link_info.tunnel_info.sa_state_id;

  /* If XFRM states are installed */
  if (sa_state & (SA_XFRM_STATE_OUT| SA_XFRM_STATE_IN))
  {
    netmgr_log_med("%s(): removing SA states for link=%d, family=%d",
                   __func__, link, ip_family);

    reti = netmgr_xfrm_remove_states(sa_state_id);
    if (NETMGR_FAILURE == reti)
    {
      netmgr_log_err("%s(): removing SA states failed\n", __func__);
    }

    /* Remove XFRM state bits and update the file */
    sa_state &= ~(SA_XFRM_STATE_OUT | SA_XFRM_STATE_IN);
    recovery_data->recovery_info.link_info.tunnel_info.sa_state &=
      ~(SA_XFRM_STATE_OUT | SA_XFRM_STATE_IN);
  ...
  if (sa_state & SA_XFRM_POLICY_OUT)
  {
    netmgr_log_med("%s(): removing TX SA policy for link=%d, family=%d",
                   __func__, link, ip_family);
    reti = netmgr_xfrm_remove_policies(recovery_data->recovery_info.link_info.link_addr,
                                       NULL, NULL, TRUE);
    if (NETMGR_FAILURE == reti)
    {
      netmgr_log_err("%s(): removing default TX SA policy failed\n", __func__);
    }
  ...
  /* If XFRM 'in' or 'fwd' policy is installed */
  if (sa_state & (SA_XFRM_POLICY_IN | SA_XFRM_POLICY_FWD))
  {
    ...
    if (NETMGR_FAILURE == reti)
    {
      netmgr_log_err("%s(): failed to remove default RX SA policy\n", __func__);
    }

    /* Remove XFRM policy 'in'/'fwd' bits and update the file */
    sa_state &= ~(SA_XFRM_POLICY_IN | SA_XFRM_POLICY_FWD);
    recovery_data->recovery_info.link_info.tunnel_info.sa_state &=
      ~(SA_XFRM_POLICY_IN | SA_XFRM_POLICY_FWD);
    if (NETMGR_SUCCESS != netmgr_recovery_file_write_entry(recovery_data))
    {
      netmgr_log_err("%s(): failed to update XFRM policy in recovery file", __func__);
      reti = NETMGR_FAILURE;
...
int netmgr_kif_cleanup_sa_rules(file_info_t *recovery_data)
{
  ...
  /* Check for updated handling */
  if (netmgr_main_cfg.nl_xfrm)
  {
    return netmgr_kif_cleanup_nl_sa_rules(recovery_data);
  }
...
/*===========================================================================
  FUNCTION  netmgr_kif_remove_nl_sa_rules
===========================================================================*/
/*!
@brief
  Removes the SAs from the SAD (Security Association Database) and
  SPD (Security Policy Database) for the given link and family (Netlink
  flavor)
...
)
{
  ...
  {
    sa = v4_sa;
    ip_addr = &v4_addr->if_addr;
    prefix_len = (unsigned int)ds_get_num_bits_set_count(v4_addr->if_mask);
    sa_state_id   = &sa->nl_sa_state_id;
  ...
  {
    sa = v6_sa;
    ip_addr = &v6_addr->if_addr;
    prefix_len = v6_addr->if_mask;
    sa_state_id   = &sa->nl_sa_state_id;
  ...
  if (TRUE == remove_sa && sa_state_id->valid)
  {
    netmgr_log_med("%s(): removing SA states for link=%d, family=%d",
                   __func__, link, ip_family);

    if (NETMGR_FAILURE == netmgr_xfrm_remove_states(sa_state_id))
    {
      netmgr_log_err("%s(): removing XFRM states failed\n", __func__);
    }

    sa_state_id->valid = FALSE;
  }
        ...
        && NETMGR_TS_MODE_DISABLED == sa->ts_mode)
  {
    ...
    if (NETMGR_SUCCESS != netmgr_util_convert_ip_addr_to_str(ip_addr, prefix_len,
                                                             iface_addr_buf,
                                                             sizeof(iface_addr_buf)))
    {
      netmgr_log_err("%s(): failed to convert iface addr to string\n", __func__);
    ...
    netmgr_log_med("%s(): removing TX SA policy for link=%d, family=%d",
                   __func__, link, ip_family);
    if (NETMGR_FAILURE == netmgr_xfrm_remove_policies(iface_addr_buf, NULL,
                                                      NULL, TRUE))
    {
      netmgr_log_err("%s(): removing TX SA policy failed\n", __func__);
    }
    ...
    netmgr_log_med("%s(): removing RX SA policy for link=%d, family=%d",
                   __func__, link, ip_family);
    if (NETMGR_FAILURE == netmgr_xfrm_remove_policies(NULL, iface_addr_buf,
                                                      NULL, FALSE))
    {
      netmgr_log_err("%s(): removing RX SA policy failed\n", __func__);
    }
  ...
  else
  {
    ...
    if (NETMGR_SUCCESS
         != netmgr_kif_remove_nl_traffic_selector_policies(&sa->sa_tsi,
                                                           &sa->sa_tsr,
                                                           sa->ts_mode,
                                                           NETMGR_SA_DIR_TX,
                                                           ip_family))
...
/*===========================================================================
  FUNCTION  netmgr_kif_get_link_ifindex
===========================================================================*/
/*!
@brief
  Accessor for getting device ifindex for a given link.

@return
  unsigned int - system-assigned ifindex
...
*/
/*=========================================================================*/
unsigned int netmgr_kif_get_link_ifindex(int link)
{
  if (NETMGR_SUCCESS == netmgr_kif_verify_link(link))
  {
    return (unsigned int)netmgr_kif_info[link].ifi_index;
  }
...
netmgr_kif_send_icmpv6_router_solicitation (int link)
{
  ...
  if ((sock_fd = netmgr_socket(AF_INET6, SOCK_RAW | SOCK_CLOEXEC, IPPROTO_ICMPV6)) < 0)
  {
    netmgr_log_sys_err("router solicitation netmgr_socket() failed:");
    goto bail;
  }
...
)
{
  ...
  for (i = 0; i < netmgr_main_cfg.max_links_in_total; i++)
  {
    ...
    {
      /* IWLAN uses PRIMARY pdu */
      netmgr_address_info_t  *addr_info = netmgr_qmi_get_addr_info(i, PRIMARY_PDU);
...
LOCAL int netmgr_kif_copy_sa_rules
(
  netmgr_address_info_t *addr_info_ptr,
  file_info_t *recovery_data,
  netmgr_ipsec_sa_t *dest_sa,
  netmgr_ipsec_sa_t *src_sa,
  int ip_family
)
{
  recovery_info_t *link_info = &recovery_data->recovery_info.link_info;
  netmgr_ip_addr_t ip_type = (AF_INET == ip_family) ? NETMGR_IPV4_ADDR :
                                                      NETMGR_IPV6_ADDR;
  netmgr_address_set_t *addr_set;
  int rc;
  ...
  if (netmgr_main_cfg.netmgr_recovery_enabled)
  {
    /* Fill in link info recovery information */
    link_info->tech_type = TECH_TYPE_EPDG;
    link_info->link_state = LINK_STATE_UP;
    link_info->ip_family = ip_family;
    link_info->tunnel_info.sa_state = SA_XFRM_CLEAN;
    if (AF_INET == ip_family)
    {
      addr_set = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV4_ADDR,
                                         NETMGR_ADDR_TYPE_DEFAULT);
      inet_ntop(AF_INET, &addr_set->if_addr.addr.v4,
                link_info->link_addr, NETMGR_MAX_STR_LENGTH);
    }
    else if (AF_INET6 == ip_family)
    {
      ...
      addr_set = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV6_ADDR,
                                         NETMGR_ADDR_TYPE_DEFAULT);
      inet_ntop(AF_INET6, &addr_set->if_addr.addr.v6_addr8,
                link_info->link_addr, NETMGR_MAX_STR_LENGTH);
      snprintf(prefix, sizeof(prefix), "/%u", addr_set->if_mask);
      strlcat(link_info->link_addr, prefix, NETMGR_MAX_STR_LENGTH);
    }
  }

  /* Copy over SA state information to the new family's SA */
  dest_sa->mode = src_sa->mode;
  memcpy(&dest_sa->nl_sa_state_id, &src_sa->nl_sa_state_id,
         sizeof(dest_sa->nl_sa_state_id));

  if (TRUE == netmgr_main_get_iwlan_traffic_sel_enabled() &&
      src_sa->ts_mode != NETMGR_TS_MODE_DISABLED)
  {
    dest_sa->ts_mode = src_sa->ts_mode;
    memcpy(&dest_sa->sa_tsi, &src_sa->sa_tsi, sizeof(src_sa->sa_tsi));
    memcpy(&dest_sa->sa_tsr, &src_sa->sa_tsr, sizeof(dest_sa->sa_tsr));

    /* Install the traffic selector_policies for the new family */
    rc = netmgr_kif_install_nl_traffic_selector_policies(&dest_sa->nl_sa_state_id,
                                                         &dest_sa->sa_tsi,
                                                         &dest_sa->sa_tsr,
                                                         dest_sa->ts_mode,
                                                         NETMGR_SA_DIR_TX,
    ...
    if (NETMGR_FAILURE == rc)
    {
      netmgr_log_err("%s(): Failed to install TX traffic selector policies\n",
                     __func__);
      goto bail;
    }
    ...
    if (NETMGR_FAILURE == rc)
    {
      netmgr_log_err("%s(): Failed to install RX traffic selector policies\n",
                     __func__);
      goto bail;
    }
  }
  ...
  {
    /* Install default policies for the new family */
    rc = netmgr_kif_install_default_xfrm_policies(&dest_sa->nl_sa_state_id,
                                                  addr_info_ptr, ip_type,
                                                  NETMGR_SA_DIR_TX);
    if (NETMGR_FAILURE == rc)
    {
      netmgr_log_err("%s(): Failed to install TX XFRM policy\n", __func__);
      goto bail;
    }

    if (netmgr_main_cfg.netmgr_recovery_enabled)
    {
      link_info->tunnel_info.sa_state |= SA_XFRM_POLICY_OUT;
      rc = netmgr_recovery_file_write_entry(recovery_data);
      if (NETMGR_SUCCESS != rc)
      {
        netmgr_log_err("%s(): Failed to write TX XFRM policy to recovery file\n",
                       __func__);
        goto bail;
      }
    }
    ...
    if (NETMGR_FAILURE == rc)
    {
      netmgr_log_err("%s(): Failed to install RX XFRM policy\n", __func__);
      goto bail;
    }

    if (netmgr_main_cfg.netmgr_recovery_enabled)
    {
      link_info->tunnel_info.sa_state |= SA_XFRM_POLICY_IN | SA_XFRM_POLICY_FWD;
      rc = netmgr_recovery_file_write_entry(recovery_data);
      if (NETMGR_SUCCESS != rc)
      {
        netmgr_log_err("%s(): Failed to write RX XFRM policy to recovery file\n",
                       __func__);
        goto bail;
      }
    }
  ...
  memcpy(&dest_sa->tunnel_ep, &src_sa->tunnel_ep, sizeof(dest_sa->tunnel_ep));
  dest_sa->is_sa_valid = TRUE;
...
/*===========================================================================
  FUNCTION  netmgr_kif_install_nl_sa_and_routing_rules
===========================================================================*/
/*!
@brief
  Installs forwarding rules and security associations (SA) (if any)
  for given reverse rmnet link (Netlink flavor)
...
/*=========================================================================*/
int
netmgr_kif_install_nl_sa_and_routing_rules
(
  int  link,
  int  ip_family
)
{
  ...
  /* IWLAN uses PRIMARY pdu */
  if (!(addr_info_ptr = netmgr_qmi_get_addr_info(link, PRIMARY_PDU)))
  {
    ...
    if (netmgr_main_cfg.netmgr_recovery_enabled)
    {
      ...
      if (NETMGR_SUCCESS != ret)
      {
        ...
        return NETMGR_FAILURE;
           ...
           WDS_TECHNOLOGY_NAME_CIWLAN_V01 == netmgr_qmi_wds_get_tech_name(link))
  {
    ...
    if (AF_INET6 == ip_family)
    {
      NETMGR_KIF_UTIL_INSTALL_IPSEC_XFRM_POLICY_EX(XFRM_POLICY_OUT);
    }
    ...
    if (AF_INET == ip_family && v4_sa->is_sa_shared && v6_sa && v6_sa->is_sa_valid)
    {
      ...
      ret = netmgr_kif_copy_sa_rules(addr_info_ptr, &recovery_data, v4_sa,
                                     v6_sa, ip_family);
      if (NETMGR_SUCCESS != ret)
      {
        netmgr_log_err("%s(): installing V4 policy failed\n", __func__);
      }
    ...
    else if (AF_INET6 == ip_family && v6_sa->is_sa_shared && v4_sa && v4_sa->is_sa_valid)
    {
      ...
      ret = netmgr_kif_copy_sa_rules(addr_info_ptr, &recovery_data, v6_sa,
                                     v4_sa, ip_family);
      if (NETMGR_SUCCESS != ret)
      {
        netmgr_log_err("%s(): installing V6 policy failed\n", __func__);
      }

      goto bail;
    }

    if (NETMGR_SUCCESS != netmgr_kif_install_nl_sa_rules(link, ip_family))
    {
...
)
{
  ...
  if (netmgr_main_cfg.nl_xfrm)
  {
    return netmgr_kif_install_nl_sa_and_routing_rules(link, ip_family);
  }
  ...
  if (WDS_TECHNOLOGY_NAME_WLAN_LOCAL_BRKOUT_V01 == netmgr_qmi_wds_get_tech_name(link))
  {
    if (netmgr_main_cfg.netmgr_recovery_enabled)
    {
      ...
      {
        inet_ntop(AF_INET6, &v6_addr->if_addr.addr,
                  recovery_data.recovery_info.link_info.link_addr,
                  NETMGR_MAX_STR_LENGTH);

        snprintf(prefix, 8, "/%u", v6_addr->if_mask);
        strlcat(recovery_data.recovery_info.link_info.link_addr, prefix, NETMGR_MAX_STR_LENGTH);
      }
...
)
{
  ...
  /* Remove the SAs only for an ePDG call or CIWLAN */
  if (WDS_TECHNOLOGY_NAME_IWLAN_S2B_V01 == netmgr_qmi_wds_get_tech_name(link) ||
      WDS_TECHNOLOGY_NAME_CIWLAN_V01 == netmgr_qmi_wds_get_tech_name(link))  {
    /* Check for updated handling */
    if (netmgr_main_cfg.nl_xfrm)
    {
      if (NETMGR_SUCCESS != netmgr_kif_remove_nl_sa_rules(link, ip_family))
      {
        netmgr_log_err("%s(): failed to remove SA rules\n", __func__);
        goto bail;
      }
    }
    else
...
/*=========================================================================*/
static int
netmgr_kif_install_wlan_rules(int link, char* wlan_ifname)
{
  struct netmgr_kif_cb_ifinfo ifinfo;
  ...
  /* IWLAN uses primary pdu */
  address_info = netmgr_qmi_get_addr_info(link, PRIMARY_PDU);
  ...
  v4_addr = netmgr_qmi_get_addr_set(address_info, NETMGR_IPV4_ADDR,
                                    NETMGR_ADDR_TYPE_DEFAULT);
  v6_addr = netmgr_qmi_get_addr_set(address_info, NETMGR_IPV6_ADDR,
                                    NETMGR_ADDR_TYPE_DEFAULT);
       ...
       address_info->valid_mask & NETMGR_ADDRSET_MASK_IPV6)
  {
    ...
    ifinfo.addr._in4.addr = v4_addr->if_addr.addr.v4;
    ifinfo.addr._in4.mask = v4_addr->if_mask;

    memcpy(&ifinfo.addr._in6.addr,
           &v6_addr->if_addr.addr,
           sizeof(ifinfo.addr._in6.addr));
    ifinfo.addr._in6.mask = v6_addr->if_mask;
  ...
  ifinfo.tunnel_info.dest_addr     = dest_addr;
  ifinfo.tunnel_info.tunnel_family = tunnel_family;
  memcpy(ifinfo.wlan_ifname, wlan_ifname, NETMGR_IF_NAME_MAX_LEN);
...
/*=========================================================================*/
static int
netmgr_kif_remove_wlan_rules(int link, char* wlan_ifname)
{
  struct netmgr_kif_cb_ifinfo ifinfo;
  netmgr_address_info_t * address_info = NULL;
  netmgr_address_set_t *v4_addr, *v6_addr;
...
/*===========================================================================
  FUNCTION  netmgr_kif_exec_cb_cmd
 ===========================================================================*/
/*!
@brief
  Execute commands raised by external modules using callback framework
*/
/*=========================================================================*/
static void* netmgr_kif_exec_cb_cmd(void *arg)
{
  int i = 0;
  int rc = DS_EXEC_OK;
  char result[NETMGR_KIF_MAXCMDSTR_LEN] = "";
  netmgr_kif_cb_exec_cmd_info_t *exec_cmd = (netmgr_kif_cb_exec_cmd_info_t*) arg;
  netmgr_cmdq_cmd_t * cmd_buf = NULL;

  if (!exec_cmd)
  {
    netmgr_log_err("%s(): invalid command received!", __func__);
    return 0;
  }

  DS_SYSCALL_DEFAULTS(opts);

  for (i = 0; i < exec_cmd->req.cmd_list_len; i++)
  {
    opts.stderr = result;
    opts.stderr_length = sizeof(result);
    opts.log_options |= DS_EXEC_LOG_EXECTIME;

    netmgr_log_low("%s(): CMD: %s", __func__, exec_cmd->req.cmd_list[i]);
    rc = netmgr_ds_exec(exec_cmd->req.cmd_list[i], &opts);
    if (DS_EXEC_OK != rc)
    {
      netmgr_log_err("%s(): failed to execute command! Err [%d][%s]", __func__, rc, result);
      exec_cmd->resp->idx = i;
      exec_cmd->resp->ret_code = rc;
      break;
    }

    memset(result, 0, NETMGR_KIF_MAXCMDSTR_LEN);
  }

  if (i == exec_cmd->req.cmd_list_len)
  {
    exec_cmd->resp->idx = i;
    exec_cmd->resp->ret_code = DS_EXEC_OK;
  }

  return 0;
}

static void* netmgr_kif_get_wlan_ifname_by_link_exec_cb_cmd(void *arg)
{
  netmgr_kif_cb_get_wlan_ifname_by_link_exec_info *exec_cmd = NULL;
  const char *iface = NETMGR_KIF_INTF_WLAN0;
  size_t rc = 0;

  exec_cmd = (netmgr_kif_cb_get_wlan_ifname_by_link_exec_info *)arg;
  ...
  exec_cmd->iface_valid = 0;

#ifdef FEATURE_DATA_IWLAN
  iface = netmgr_kif_get_wlan_ifname_by_link(exec_cmd->link);
  if (iface)
  {
    rc = strlcpy(exec_cmd->iface, iface, NETMGR_KIF_CB_IFNAMELEN);
    if (rc >= NETMGR_KIF_CB_IFNAMELEN)
    {
      netmgr_log_err("%s(): string truncation occurred", __func__);
      return 0;
    }
    ...
    {
      exec_cmd->iface_valid = 1;
      netmgr_log_low("%s() link %d iface %s",
                     __func__, exec_cmd->link, exec_cmd->iface);
    }
  }
#endif /* FEATURE_DATA_IWLAN */
...
static void* netmgr_kif_get_wwan_ifname_by_link_exec_cb_cmd(void *arg)
{
  netmgr_kif_cb_get_wlan_ifname_by_link_exec_info *exec_cmd = NULL;

  const char *iface = NETMGR_KIF_INTF_RMNET_DATA;
  size_t rc = 0;
...
#ifdef FEATURE_DATA_IWLAN
  iface = netmgr_kif_get_wwan_ifname_by_link(exec_cmd->link);
  if (iface)
  {
    rc = strlcpy(exec_cmd->iface, iface, NETMGR_KIF_CB_IFNAMELEN);
    if (rc >= NETMGR_KIF_CB_IFNAMELEN)
...
)
{
  netmgr_ipsec_sa_t  *sa = NULL;
  netmgr_ip_addr_t        ip_type;
  ip_type = (family == AF_INET)?NETMGR_IPV4_ADDR:NETMGR_IPV6_ADDR;
  netmgr_xfrm_state_id_t *sa_state_id;
  netmgr_address_set_t *addr_set_v4 ,*addr_set_v6;
  int rc = NETMGR_FAILURE;
  int ipv4_sa_valid = 0, ipv6_sa_valid = 0, ipv4_sa_shared = 0, ipv6_sa_shared = 0;
  addr_set_v4 = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV4_ADDR,
                                       NETMGR_ADDR_TYPE_DEFAULT);
  addr_set_v6 = netmgr_qmi_get_addr_set(addr_info_ptr, NETMGR_IPV6_ADDR,
                                       NETMGR_ADDR_TYPE_DEFAULT);
  ipv4_sa_valid = addr_set_v4->sa.tunnel_ep.is_valid;
  ipv6_sa_valid = addr_set_v6->sa.tunnel_ep.is_valid;
  ipv4_sa_shared = addr_set_v4->sa.is_sa_shared;
  ipv6_sa_shared = addr_set_v6->sa.tunnel_ep.is_valid;
  if(ipv4_sa_valid)
  {
    sa_state_id = &addr_set_v4->sa.nl_sa_state_id;
    if(NETMGR_TS_MODE_DISABLED == addr_set_v4->sa.ts_mode)
    {
        if (NETMGR_SUCCESS != netmgr_kif_install_default_xfrm_policies(sa_state_id,
                                                                       addr_info_ptr,
                                                                       ip_type,
                                                                       NETMGR_SA_DIR_TX))
        {
            netmgr_log_err("%s(): failed to add default XFRM policies in 'in' direction", __func__);
        }
        if (NETMGR_SUCCESS != netmgr_kif_install_default_xfrm_policies(sa_state_id,
                                                                       addr_info_ptr,
                                                                       ip_type,
                                                                       NETMGR_SA_DIR_RX))
        {
            netmgr_log_err("%s(): failed to add default XFRM policies in 'out' direction", __func__);
        }
    }
    else
    {
        if (NETMGR_SUCCESS != netmgr_kif_install_nl_traffic_selector_policies(sa_state_id,
                                                                              &addr_set_v4->sa.sa_tsi,
                                                                              &addr_set_v4->sa.sa_tsr,
                                                                              addr_set_v4->sa.ts_mode,
                                                                              NETMGR_SA_DIR_TX,
                                                                              family))
        {
            netmgr_log_err("%s(): Failed to install traffic selector policies in 'in' direction!\n", __func__);
        }
        if (NETMGR_SUCCESS != netmgr_kif_install_nl_traffic_selector_policies(sa_state_id,
                                                                              &addr_set_v4->sa.sa_tsi,
                                                                              &addr_set_v4->sa.sa_tsr,
  ...
  if(ipv6_sa_valid)
  {
    if((ipv4_sa_valid && (!ipv4_sa_shared && !ipv6_sa_shared )) || !(ipv4_sa_valid))
    {
        sa_state_id = &addr_set_v6->sa.nl_sa_state_id;
        if(NETMGR_TS_MODE_DISABLED == addr_set_v6->sa.ts_mode)
        {
            if (NETMGR_SUCCESS != netmgr_kif_install_default_xfrm_policies(sa_state_id,
                                                                           addr_info_ptr,
                                                                           ip_type,
                                                                           NETMGR_SA_DIR_TX))
            {
                netmgr_log_err("%s(): failed to add xfrm default policy in 'in' and 'fwd' direction", __func__);
            }
            if (NETMGR_SUCCESS != netmgr_kif_install_default_xfrm_policies(sa_state_id,
                                                                           addr_info_ptr,
                                                                           ip_type,
                                                                           NETMGR_SA_DIR_RX))
            {
                netmgr_log_err("%s(): failed to add xfrm default policy in 'out' direction", __func__);
            }
        }
        else
        {
            if (NETMGR_SUCCESS != netmgr_kif_install_nl_traffic_selector_policies(sa_state_id,
                                                                                  &addr_set_v6->sa.sa_tsi,
                                                                                  &addr_set_v6->sa.sa_tsr,
                                                                                  addr_set_v6->sa.ts_mode,
                                                                                  NETMGR_SA_DIR_TX,
                                                                                  family))
            {
                netmgr_log_err("%s(): Failed to install traffic selector policies in 'in' and 'fwd' direction!\n", __func__);
            }
            if (NETMGR_SUCCESS != netmgr_kif_install_nl_traffic_selector_policies(sa_state_id,
                                                                                  &addr_set_v6->sa.sa_tsi,
                                                                                  &addr_set_v6->sa.sa_tsr,
...
#ifdef FEATURE_DATA_IWLAN
      /* We need to remove SA rules in case of netd restart for Reverse IP links and reinstall
         since netd is flushing the XFRM rules during a netd restart.*/
      if (NETMGR_KIF_CB_IF_TECH_TYPE_EPDG == ifinfo.tech_type)
      {
        netmgr_address_info_t  *addr_info_ptr = netmgr_qmi_get_addr_info(link, PRIMARY_PDU);
        if(!addr_info_ptr)
        {
        ...
        ipv4_sa_shared = addr_set_v4->sa.is_sa_shared;
        ipv6_sa_shared = addr_set_v6->sa.tunnel_ep.is_valid;
        /* Remove SA rules if ipv4 tunnel is valid */
        if(ipv4_sa_valid)
        {
            if(NETMGR_SUCCESS != netmgr_kif_remove_nl_sa_rules(link, AF_INET))
            {
                netmgr_log_err("%s(): failed to remove SA rules\n", __func__);
            }
        }
        if(ipv6_sa_valid)
        {
            if((ipv4_sa_valid && (!ipv4_sa_shared && !ipv6_sa_shared )) || !(ipv4_sa_valid))
            {
                if(NETMGR_SUCCESS != netmgr_kif_remove_nl_sa_rules(link, AF_INET6))
                {
                    netmgr_log_err("%s(): failed to remove SA rules\n", __func__);
                }
            }
        }
        netmgr_log_high("%s() Reinstalling XFRM rules\n", __func__);
        if(ipv4_sa_valid)
        {
            if(NETMGR_FAILURE == netmgr_xfrm_add_state_from_addr_set(addr_set_v4, TRUE,
                                                                      netmgr_qmi_wds_get_tech_name(link)))
            {
                netmgr_log_err("%s(): failed to add incoming XFRM states\n", __func__);
                goto dfs_query;
            }
            if(NETMGR_FAILURE == netmgr_xfrm_add_state_from_addr_set(addr_set_v4, FALSE,
                                                                      netmgr_qmi_wds_get_tech_name(link)))
            {
                netmgr_log_err("%s(): failed to add outgoing XFRM states\n", __func__);
                goto dfs_query;
            }
        }
        if(ipv6_sa_valid)
        {
            if((ipv4_sa_valid && (!ipv4_sa_shared && !ipv6_sa_shared )) || !(ipv4_sa_valid))
            {
                if(NETMGR_FAILURE == netmgr_xfrm_add_state_from_addr_set(addr_set_v6, TRUE,
                                                                          netmgr_qmi_wds_get_tech_name(link)))
                {
                    netmgr_log_err("%s(): failed to add incoming XFRM states\n", __func__);
                    goto dfs_query;
                }
                if(NETMGR_FAILURE == netmgr_xfrm_add_state_from_addr_set(addr_set_v6, FALSE,
                                                                          netmgr_qmi_wds_get_tech_name(link)))
                {
                    netmgr_log_err("%s(): failed to add outgoing XFRM states\n", __func__);
                    goto dfs_query;
        ...
        if(ipv4_sa_valid)
        {
            if(NETMGR_FAILURE == netmgr_xfrm_add_policies_from_addr_info(addr_info_ptr, AF_INET))
            {
                netmgr_log_err("%s(): failed to add XFRM policies\n", __func__);
                goto dfs_query;
            }
        }
        if(ipv6_sa_valid)
        {
            if(NETMGR_SUCCESS != netmgr_xfrm_add_policies_from_addr_info(addr_info_ptr, AF_INET6))
            {
                netmgr_log_err("%s(): failed to add SA policies\n", __func__);
            }
        }

dfs_query:
        link_info = netmgr_qmi_get_link_info(link);
        if (link_info) {
          /* Restore SPI filters for non LBO interfaces */
          netmgr_log_low("%s(): restore IPv4 filters", __func__);
          netmgr_qmi_dfs_query_and_process_rev_ip_filters(link, link_info->dfs_info.clnt_hdl[NETMGR_QMI_CLIENT_IPV4]);
          netmgr_log_low("%s(): restore IPv6 filters", __func__);
          netmgr_qmi_dfs_query_and_process_rev_ip_filters(link, link_info->dfs_info.clnt_hdl[NETMGR_QMI_CLIENT_IPV6]);
        }
      }
#endif
    }
...
*/
/*=========================================================================*/
void* netmgr_kif_handle_netd_event(__attribute__((unused)) void *arg)
{
  int                          i = 0;
...
/*===========================================================================
  FUNCTION netmgr_notify_clients_init_complete
===========================================================================*/
/*!
@brief
  Notifies clients that intialization is complete. Used in case of restart.

@return
  None
*/
/*=========================================================================*/
static void netmgr_notify_clients_init_complete(void)
{
  netmgr_nl_event_info_t *event_info = NULL;

  /*After restart, notify clients that we are ready to accept calls now*/
  event_info = netmgr_malloc(sizeof(netmgr_nl_event_info_t));
  if(NULL == event_info)
  {
    netmgr_log_err("%s(): failed to allocate event buffer\n", __func__);
  ...
  else
  {
    memset(event_info, 0x0, sizeof(netmgr_nl_event_info_t));
    event_info->event = NETMGR_USER_CMD;
    event_info->cmd_data.netmgr_restart_state = NETMGR_POST_RESTART_STATE_INIT_DONE;
    event_info->param_mask |= NETMGR_EVT_PARAM_CMD_DATA;
    ...
    netmgr_log_high("%s(): Notified NETMGR_POST_RESTART_STATE_INIT_DONE to clients\n", __func__);
   }
...
)
{
  ...
  if ((server_sock_fd = netmgr_socket(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0)) == NETMGR_UNIX_API_ERROR)
  {
  ...
  if (netmgr_main_cfg.netmgr_recovery_enabled)
  {
    if (netmgr_restart)
    {
      netmgr_notify_clients_init_complete();
    }
  }
...
)
{
  ...
  */
  ...
  if (netmgr_main_cfg.netmgr_listen_ev_proto)
    vtable.send_event = netmgr_kif_tipc_send_event;
  else
    vtable.send_event = netmgr_kif_netlink_send_event;
  ...
  netmgr_cb_create_chain(NETMGR_KIF_CB_TABLE, NETMGR_KIF_CB_EXEC_CMD);
  netmgr_cb_create_chain(NETMGR_KIF_CB_TABLE, NETMGR_KIF_CB_GET_WLAN_IFNAME_BY_LINK);
  netmgr_cb_create_chain(NETMGR_KIF_CB_TABLE, NETMGR_KIF_CB_GET_WWAN_IFNAME_BY_LINK);
  ...
  netmgr_cb_insert_cb(NETMGR_NETD_CB_TABLE,
                      NETMGR_NETD_CB_RESTART_EV,
                      NETMGR_KIF_CB_MODNAME,
                      &netmgr_kif_handle_netd_event);
  netmgr_cb_insert_cb(NETMGR_KIF_CB_TABLE,
                      NETMGR_KIF_CB_EXEC_CMD,
                      NETMGR_KIF_CB_MODNAME,
                      &netmgr_kif_exec_cb_cmd);
  netmgr_cb_insert_cb(NETMGR_KIF_CB_TABLE,
                      NETMGR_KIF_CB_GET_WLAN_IFNAME_BY_LINK,
                      NETMGR_KIF_CB_MODNAME,
                      &netmgr_kif_get_wlan_ifname_by_link_exec_cb_cmd);
  netmgr_cb_insert_cb(NETMGR_KIF_CB_TABLE,
                      NETMGR_KIF_CB_GET_WWAN_IFNAME_BY_LINK,
                      NETMGR_KIF_CB_MODNAME,
                      &netmgr_kif_get_wwan_ifname_by_link_exec_cb_cmd);
...
/*===========================================================================
  FUNCTION  netmgr_kif_translate_wds_tech
===========================================================================*/
/*!
@brief
  Convert WDS tech type to the internal data-binary tech type.

@return
  int - Internal data-binary tech-type
*/
/*=========================================================================*/
int netmgr_kif_translate_wds_tech(wds_technology_name_enum_v01 tech)
{

  int translated = NETMGR_KIF_CB_IF_TECH_TYPE_DONT_CARE;

  switch(tech)
  {
    case WDS_TECHNOLOGY_NAME_WLAN_LOCAL_BRKOUT_V01:
      translated = NETMGR_KIF_CB_IF_TECH_TYPE_LBO;
      break;
    case WDS_TECHNOLOGY_NAME_IWLAN_S2B_V01:
      translated = NETMGR_KIF_CB_IF_TECH_TYPE_EPDG;
      break;
    case WDS_TECHNOLOGY_NAME_IWLAN_S2B_PROXY_V01:
      translated = NETMGR_KIF_CB_IF_TECH_TYPE_EPDG_PROXY;
      break;
    case WDS_TECHNOLOGY_NAME_CIWLAN_V01:
      translated = NETMGR_KIF_CB_IF_TECH_TYPE_C_WLAN;
      break;
    default:
      break;
  }
