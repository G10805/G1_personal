...
/*===========================================================================
  ...
  Copyright (c) 2010-2019, 2021-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
#define __netmgr_util_fd_open(FNAME, syscall, type, ...) \
  ({ \
    type __fd = syscall(__VA_ARGS__); \
    netmgr_log_low("%s(): opened file descriptor %p\n", FNAME, \
                   (void *)(uintptr_t)__fd); \
    __fd; \
  })

#define __netmgr_util_fd_close(FNAME, syscall, fd) \
  ({ \
    netmgr_log_low("%s(): closing file descriptor %p\n", FNAME, \
                   (void *)(uintptr_t)fd); \
    syscall(fd); \
  })

/* Define logging versions of functions that create file descriptors */
#define netmgr_socket(domain, type, proto) \
  __netmgr_util_fd_open(__func__, socket, int, domain, type, proto)
#define netmgr_open(path, flag, ...) \
  __netmgr_util_fd_open(__func__, open, int, path, flag, ##__VA_ARGS__);
#define netmgr_fopen(path, mode) \
  __netmgr_util_fd_open(__func__, fopen, FILE *, path, mode)

/* Define logging versions of functions that close file descriptors */
#define netmgr_close(fd) \
  __netmgr_util_fd_close(__func__, close, fd)
#define netmgr_fclose(fd) \
  __netmgr_util_fd_close(__func__, fclose, fd)

/* Define logging macros for qmi client handles. These also internally have
 * file descriptors (3 per, currently)
 */
#define __netmgr_util_log_qmi_client_create(FNAME, client_ptr, func, ...) \
  ({ \
    qmi_client_type *__hdl_ptr = (client_ptr); \
    qmi_client_error_type __rc = func(__VA_ARGS__, __hdl_ptr); \
    if (__rc == QMI_NO_ERR) \
    { \
      netmgr_log_low("%s(): opened qmi client %p\n", FNAME, \
                 (void *)(uintptr_t)(*__hdl_ptr)); \
    } \
    __rc; \
  })
#define __netmgr_util_log_qmi_client_close(FNAME, client, func) \
  ({ \
    netmgr_log_low("%s(): closing qmi client %p\n", FNAME, \
                   (void *)(uintptr_t)(client)); \
    func(client); \
  })

#define netmgr_qmi_client_init_instance(obj, id, cb, data, params, time, hndl) \
  __netmgr_util_log_qmi_client_create(__func__, hndl, \
                                      qmi_client_init_instance, obj, id, cb, \
                                      data, params, time)
#define netmgr_qmi_client_wds_init_instance(dev, obj, id, cb, data, params, \
                                            time, hndl) \
  __netmgr_util_log_qmi_client_create(__func__, hndl, \
                                      qmi_client_wds_init_instance, dev, obj, \
                                      id, cb, data, params, time)
#define netmgr_qmi_client_notifier_init(obj, params, hndl) \
  __netmgr_util_log_qmi_client_create(__func__, hndl, \
                                      qmi_client_notifier_init, obj, params)
#define netmgr_qmi_client_release(hndl) \
  __netmgr_util_log_qmi_client_close(__func__, hndl, qmi_client_release)
