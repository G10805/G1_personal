...
extern int nm_link_local_routing_main_register_plugin(void);
extern void nm_link_local_routing_main_init(int restart);

extern int nm_mdmprxy_register_plugin(void);
...
#define NETMGR_MAIN_OFFLOAD_KO_PROPERTY "persist.vendor.data.offload_ko_load"
#define NETMGR_MAIN_PERF_KO_PROPERTY_MAX 3
#define NETMGR_MAIN_PERF_KO_PROPERTY_DEFAULT 1
...
struct netmgr_main_cfg_s netmgr_main_cfg = {
  ...
  0,                                         /* Endpoint configuration via extended ioctl v2 */
  {0,},                                      /* Egress format V2 */
  {0,},                                      /* Ingress format V2 */
  ...
  0,                                          /* netmgr-client event proto */
  0,                                          /* netmgr recovery enabled */
  0,                                          /* debug netdev backlog */
  0,                                          /* debug netdev budget */
  FALSE,                                      /* iwlan_concurrency */
  FALSE,                                      /* nl_xfrm */
  0,                                          /* phy_gso_max_size */
  {0,},                                       /* LLC configuration */
  FALSE,                                      /* oemprxy_enabled */
  FALSE,                                      /* DFC powersave ext */
  0,                                          /* sscm3 config*/
  0,                                          /* sscm3 */
  0,                                          /* ipv6 multihoming config */
  0,                                          /* ipv6 multihoming */
...
#ifdef FEATURE_DATA_IWLAN
  boolean is_oemprxy_plugin_initialized = FALSE;
#endif /* FEATURE_DATA_IWLAN */

static struct netmgr_sscm3_ops netmgr_main_sscm3_ops;
  ...
  do
  {
      ...
      else if(netmgr_main_cfg.iwlan_enabled != IWLAN_DISABLE)
      {
          ...
          if( !strncasecmp( NETMGR_TRUE, args, sizeof(NETMGR_TRUE) ) )
          {
              ...
              /* If property is not set, use value from config.xml */
              netmgr_log_low("%s(): netmgr_main_cfg.iwlan_enabled [%d]\n",
                             __func__, netmgr_main_cfg.iwlan_enabled);
          }
...
LOCAL void netmgr_main_process_target()
{
  ...
  if ((DS_TARGET_FUSION_SDA855_SDX55 == target) ||
      ...
      (DS_TARGET_FUSION_WAIPIO_SDXLEMUR == target))
...
netmgr_main_init_cb()
{
  ...
  /* OEM Proxy */
  netmgr_cb_create_table(NETMGR_NETD_CB_TABLE);
  netmgr_cb_create_chain(NETMGR_NETD_CB_TABLE, NETMGR_NETD_CB_RESTART_EV);
#endif /* defined(FEATURE_DS_LINUX_ANDROID) */
}

/*===========================================================================
  FUNCTION netmgr_main_init_link_local_plugin
===========================================================================*/
/*!
@brief
...
*/
/*=========================================================================*/
static void netmgr_main_init_link_local_plugin(int restart)
{
#if (!defined(NETMGR_OFFTARGET) && defined(FEATURE_DS_LINUX_ANDROID))
  if(NETMGR_FAILURE != nm_link_local_routing_main_register_plugin())
  {
    nm_link_local_routing_main_init(restart);
  }
  else
  {
    netmgr_log_err("%s(): Error in registering link-local plugin\n", __func__);
  }
#endif /* !defined(NETMGR_OFFTARGET) && defined(FEATURE_DS_LINUX_ANDROID) */
}

/*===========================================================================
  FUNCTION netmgr_main_init_mdmprxy_plugin
===========================================================================*/
/*!
@brief
  Registers the plugins with netmgr callback framework
...
*/
/*=========================================================================*/
static void netmgr_main_init_mdmprxy_plugin(int restart)
{
#ifdef FEATURE_DATA_IWLAN
...
#endif /* FEATURE_DATA_IWLAN */
  {
    ...
    if(NETMGR_FAILURE != nm_mdmprxy_register_plugin())
    {
      nm_mdmprxy_init(restart);
    }
    else
    {
      netmgr_log_err("%s(): Error in initing mdmprxy, iwlan calls wont work\n",
                      __func__);
    }
...
static void netmgr_main_init_modem_proxy(int restart)
{
#ifdef FEATURE_DATA_IWLAN
  if (IWLAN_DISABLE != netmgr_main_get_iwlan_enabled())
#endif /* FEATURE_DATA_IWLAN */
  {
#if (defined (FEATURE_DS_LINUX_ANDROID) || defined (NETMGR_OFFTARGET))
    if(NETMGR_FAILURE != netmgr_modem_proxy_register())
    {
      netmgr_log_low("%s(): Initing modem_proxy\n", __func__);
      netmgr_modem_proxy_init(restart);
    }
    else
    {
      netmgr_log_err("%s(): Error in initing modem_proxy, iwlan calls wont work\n",
                      __func__);
    }
#endif /* (defined (FEATURE_DS_LINUX_ANDROID) || defined (NETMGR_OFFTARGET)) */
  }
...
/*===========================================================================
  FUNCTION netmgr_main_init_oemprxy_plugin
===========================================================================*/
/*!
@brief
  Registers plugins with netmgr callback framework and set iptable

@return
  None
*/
/*=========================================================================*/
#define NETMGR_OEM_PRXY_LIB "libnetmgr_oemprxy.so"
#define NETMGR_OEM_PRXY_INIT_FUNC "nm_oemprxy_init"
typedef int netmgr_oem_prxy_init_f(int flags);

static void netmgr_main_init_oemprxy_plugin(int restart)
{
#if (defined(FEATURE_DATA_IWLAN) && (defined(FEATURE_DS_LINUX_ANDROID) || defined(NETMGR_OFFTARGET)))

  void *dl_handle = NULL;
  char *err = NULL;
  netmgr_oem_prxy_init_f *oem_prxy_init = NULL;

  dl_handle = dlopen(NETMGR_OEM_PRXY_LIB, RTLD_NOW);
  if (NULL == dl_handle)
  {
    netmgr_log_err("%s(): Can't open %s", __func__, NETMGR_OEM_PRXY_INIT_FUNC);
    return;
  }

  oem_prxy_init = (netmgr_oem_prxy_init_f *) dlsym(dl_handle, NETMGR_OEM_PRXY_INIT_FUNC);

  if(NULL == oem_prxy_init)
  {
    err = (char *) dlerror();
    if (err != NULL)
    {
      netmgr_log_err("%s(): %s not found in %s: %s",
                     __func__, NETMGR_OEM_PRXY_INIT_FUNC, NETMGR_OEM_PRXY_LIB, err);
    }
    return;
  }

  if(NETMGR_SUCCESS != oem_prxy_init(restart))
  {
    netmgr_log_err("%s(): Error in initing oemprxy, iwlan calls wont work\n", __func__);
    is_oemprxy_plugin_initialized = FALSE;
  }
  else
  {
    is_oemprxy_plugin_initialized = TRUE;
  }
#endif /* (defined(FEATURE_DATA_IWLAN)
            && (defined(FEATURE_DS_LINUX_ANDROID) || defined(NETMGR_OFFTARGET))) */
}
...
/*===========================================================================
  FUNCTION  netmgr_main_unset_shsusrd_load_properties
===========================================================================*/
/*!
@brief
  Unsets android properties to stop shsusrd
...
*/
/*=========================================================================*/
int netmgr_main_unset_shsusrd_load_properties(void)
{
  ...
  if (NETMGR_SUCCESS != netmgr_rmnet_set_shsusr_loading_property(NETMGR_MAIN_SHSUSR_LOAD_PROPERTY,
							       NETMGR_MAIN_SHSUSR_PROPERTY_DISABLE))
  {
      netmgr_log_high("%s(): failed to set propery to unload shsusrd!", __func__);
      reti = NETMGR_FAILURE;
  }
  else
  {
      netmgr_log_high("%s(): disable property for shsusrd unload", __func__);
  }
...
*/
/*=========================================================================*/
static int netmgr_main_set_ko_loading_properties(void)
{
  ds_target_t target = ds_get_target();
  int reti = NETMGR_SUCCESS;
  ...
  /* Set offload KO property */
  if (!netmgr_main_cfg.rmnet_offload)
  {
    netmgr_rmnet_set_ko_loading_property(NETMGR_MAIN_OFFLOAD_KO_PROPERTY,
  ...
  if (netmgr_main_cfg.rmnet_perf)
  {
    do {
      ...
      if (netmgr_main_cfg.rmnet_perf != NETMGR_MAIN_PERF_KO_PROPERTY_DEFAULT)
      {
  ...
  {
    netmgr_log_high("%s(): stop shsusrd before unloading kernel modules", __func__);
    netmgr_main_unset_shsusrd_load_properties();

    netmgr_log_high("%s(): unload ko modules", __func__);
...
#define NETMGR_INTERFACE_SERVER_LIB "libnetmgrext_interface_server.so"
#define NETMGR_INTERFACE_SERVER_LIB_INIT "netmgr_interface_server_init"

typedef int netmgr_interface_server_init_func(void);

void netmgr_main_interface_server_init(void)
{
  void *dl_handle = NULL;
  char *err = NULL;
  netmgr_interface_server_init_func *netmgr_interface_server_init_fn = NULL;

  netmgr_log_low("%(): Opening %s", __func__, NETMGR_INTERFACE_SERVER_LIB);
  dl_handle = dlopen(NETMGR_INTERFACE_SERVER_LIB, RTLD_NOW);
  if (!dl_handle)
  {
    netmgr_log_err("%s(): Unable to open %s", __func__, NETMGR_INTERFACE_SERVER_LIB);
    return;
  }

  netmgr_interface_server_init_fn = (netmgr_interface_server_init_func *)
                                    dlsym(dl_handle, NETMGR_INTERFACE_SERVER_LIB_INIT);

  if (!netmgr_interface_server_init_fn)
  {
    err = (char *)dlerror();

    netmgr_log_err("%s(): function %s() not found in %s - error %s", __func__,
                   NETMGR_INTERFACE_SERVER_LIB_INIT, NETMGR_INTERFACE_SERVER_LIB,
                   err ?  err : "NULL dlerror()");
    return;
  }

  netmgr_log_low("%s(): Loaded function %s() from %s", __func__,
                 NETMGR_INTERFACE_SERVER_LIB_INIT, NETMGR_INTERFACE_SERVER_LIB);

  netmgr_interface_server_init_fn();
}

/*===========================================================================
  FUNCTION  netmgr_main_is_oemprxy_plugin_enabled
===========================================================================*/
/*!
@brief
  Returns true if OEMPRXY plugin is enabled
*/
/*=========================================================================*/
boolean netmgr_main_is_oemprxy_plugin_enabled(void)
{
  boolean ret = FALSE;

#ifdef FEATURE_DATA_IWLAN
  ret = is_oemprxy_plugin_initialized;
#endif /* FEATURE_DATA_IWLAN */
...
/*===========================================================================
  FUNCTION  netmgr_main_sscm3_register
===========================================================================*/
/*!
@brief
  Conditionally register necessary SSC mode 3 functions
...
LOCAL int netmgr_main_sscm3_register(void)
{
  netmgr_main_sscm3_ops.cfg = &netmgr_main_cfg;
  netmgr_main_sscm3_ops.exec_get_cmd = netmgr_exec_get_cmd;
  netmgr_main_sscm3_ops.exec_release_cmd = netmgr_exec_release_cmd;
  netmgr_main_sscm3_ops.exec_put_cmd = netmgr_exec_put_cmd;
  netmgr_main_sscm3_ops.submit_flow_info = netmgr_qmi_submit_flow_info;
  netmgr_main_sscm3_ops.get_modem_link_info = netmgr_qmi_get_modem_link_info;
  netmgr_main_sscm3_ops.get_link_wds_info = netmgr_qmi_get_link_wds_info_pdu;
  netmgr_main_sscm3_ops.get_addr_info = netmgr_qmi_get_addr_info;
  netmgr_main_sscm3_ops.get_addr_set = netmgr_qmi_get_addr_set;

  netmgr_sscm3_register(&netmgr_main_sscm3_ops);
  return NETMGR_SUCCESS;
}
...
netmgr_main (int argc, char ** argv)
{
  ...
  if ( TRUE == log_to_file )
  {
    ...
    logging_fd = netmgr_fopen(NETMGR_LOG_FILE_LOCATION, "w+e");
    if (!logging_fd)
    {
      log_to_file = FALSE;
      perror("failed");
  ...
  if (netmgr_main_get_rmnet_data_enabled())
  {
    ...
    if (netmgr_main_cfg.rtm_rmnet_enabled)
    {
      ...
      if (NETMGR_SUCCESS != netmgr_main_set_ko_loading_properties())
      {
  ...
  netmgr_main_init_link_local_plugin(netmgr_restart);

  if (netmgr_main_cfg.iwlan_enabled == IWLAN_ENABLE ||
      netmgr_main_cfg.iwlan_enabled == NSWO_ONLY)
  {
    netmgr_main_init_mdmprxy_plugin(netmgr_restart);
  }
  else if (netmgr_main_cfg.iwlan_enabled == IWLAN_V2_ENABLE ||
           netmgr_main_cfg.iwlan_enabled == NSWO_V2_ONLY)
  {
    netmgr_main_init_modem_proxy(netmgr_restart);
  }
  else if ((netmgr_main_cfg.oemprxy_enabled)
            && (netmgr_main_cfg.iwlan_enabled == IWLAN_OEMPRXY))
  {
    netmgr_main_init_oemprxy_plugin(netmgr_restart);
  }
  ...
  netmgr_main_sscm3_register();

  netmgr_main_interface_server_init();

  netmgr_unix_listener_init(netmgr_restart);
