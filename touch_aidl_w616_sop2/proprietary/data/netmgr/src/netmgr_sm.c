...
)
{
    ...
    )
  {
    ...
    /* Reset the QMI link WDS data for all links.
    */

    netmgr_qmi_reset_link_wds_data(cmd_buf->data.link, PRIMARY_PDU);
    netmgr_qmi_reset_link_wds_data(cmd_buf->data.link, SECONDARY_PDU);
    ...
    {
      /* If we moved to INITED state as a result of some internal failure,
         send that failure result to clients so that they can recover properly */
      if (cmd_buf->data.op_status.op_code != NETMGR_OP_STATUS_CODE_SUCCESS)
      {
        netmgr_exec_process_op_status_msg(cmd_buf->data.link,
                                          netmgr_qmi_cfg.links[cmd_buf->data.link].modem_subs_id,
                                          &cmd_buf->data.op_status);
      }
      else
      {
        ...
        if( NULL == event_info ) {
          ...
          /* Handle OEM Proxy Reverse Link */
          if(netmgr_main_cfg.oemprxy_enabled && NETMGR_KIF_IS_REV_RMNET_LINK(cmd_buf->data.link))
          {
            event_info->event = NET_PLATFORM_OEMPRXY_IFACE_DOWN;
          }
          else
          {
...
)
{
  ...
  netmgr_ip_addr_t ip_addr_type = NETMGR_IP_ANY_ADDR;
  ...
  ip_addr_type = netmgr_qmi_get_ip_addr_type_first_conn_clnt(cmd_buf->data.link);
  ...
  if( NETMGR_SUCCESS !=
    ...
    /* Update op code status to failed */
    memset(&cmd_buf->data.op_status, 0, sizeof(netmgr_op_status_info_t));

    switch (ip_addr_type)
    {
      case NETMGR_IPV4_ADDR:
        cmd_buf->data.op_status.op_ip_family = NETMGR_OP_STATUS_IP_FAMILY_V4;
        break; /* NETMGR_IPV4_ADDR */

      case NETMGR_IPV6_ADDR:
        cmd_buf->data.op_status.op_ip_family = NETMGR_OP_STATUS_IP_FAMILY_V6;
        break; /* NETMGR_IPV6_ADDR */

      default:
        break;
    }

    cmd_buf->data.op_status.op_code = NETMGR_OP_STATUS_CODE_FAIL;
...
bail:
  if (NULL != cmd_buf  && NULL != cmd_buf->data.info.connect_msg.addr_info_ptr &&
      NULL != cmd_buf->data.info.connect_msg.addr_info_ptr->sec_addr_info_ptr)
  {
    ds_free( cmd_buf->data.info.connect_msg.addr_info_ptr->sec_addr_info_ptr );
  }
...
)
{
  ...
  if (NETMGR_KIF_IS_REV_RMNET_LINK(cmd_buf->data.link) )
  {
     netmgr_qmi_send_rev_ip_config_complete(NETMGR_QMI_IWLAN_CALL_CLEANUP,
                                            cmd_buf->data.link,
                                            ((cmd_buf->data.info.disconnect_msg.addr_info_ptr->if_addr.type == NETMGR_IPV4_ADDR) ? AF_INET : AF_INET6),
                                            NETMGR_SUCCESS,
                                            NETMGR_INVALID_TXN_ID);
  }
#endif /* FEATURE_DATA_IWLAN */
...
)
{
  ...
  if (NETMGR_KIF_IS_REV_RMNET_LINK(cmd_buf->data.link) )
  {
    ...
    if (NETMGR_LINK_MAX != netmgr_qmi_iwlan_get_link_assoc(cmd_buf->data.link))
    {
      ...
      if( NETMGR_SUCCESS !=
          netmgr_kif_iface_configure( (uint8)cmd_buf->data.link,
                                      ...
                                      /* Should always be 0 since secondary PDU doesn't come for rev_ip links*/
                                      cmd_buf->data.info.connect_msg.pdu_status ) )
      {
        netmgr_log_err("failed on kif_iface_configure\n");

        /* Force platform interface to close */
        (void) netmgr_kif_iface_close( (uint8)cmd_buf->data.link, NULL, TRUE, TRUE );
        next_state = NETMGR_STATE_INITED;
      }
      else
      {
    ...
    {
     int ip_family = cmd_buf->data.info.connect_msg.addr_info_ptr->if_addr.type;
      /*We cannot handle this right now, drop this and modem should retry*/
      /*Staying in same state, waiting for KIF_CLOSED*/
      netmgr_qmi_send_rev_ip_config_complete(NETMGR_QMI_IWLAN_CALL_CLEANUP,
                                             cmd_buf->data.link,
                                             ((ip_family == NETMGR_IPV4_ADDR) ? AF_INET : AF_INET6),
                                             NETMGR_FAILURE,
                                             cmd_buf->data.info.connect_msg.txn_id);
      goto out;
    }
  }
  else
...
)
{
  stm_state_t next_state = STM_SAME_STATE; /* Default 'next' state */
  netmgr_exec_cmd_t * cmd_buf = NULL;
  netmgr_ctl_port_config_type *link_info;
  int link_num;
  ...
  /* TC cleanup, in case netmgr_sm_kif_closed is not called */
#ifdef NETMGR_QOS_ENABLED
  if( netmgr_main_get_qos_enabled() &&
      !NETMGR_KIF_IS_REV_RMNET_LINK(link_num) )
  {
    ...
    if (NETMGR_SUCCESS !=
        netmgr_tc_flow_delete(link_num, &qos_flow))
    {
      netmgr_log_err("failed on tc_flow_delete\n");
    }

    /*Delete the cached qos flows and filters*/
    netmgr_qmi_clear_flow_cache(link_num, MAX_PDU);
  }
  ...
  link_info = netmgr_qmi_get_port_config(link_num);
  if (link_info && link_info->on_demand) {
    link_info->on_demand = FALSE;
    link_info->enabled = FALSE;
...
#ifdef NETMGR_QOS_ENABLED
  netmgr_log_med("%s():  configure link %d  qos %d", __func__, link_num,
                  netmgr_main_get_qos_enabled());
  if ( netmgr_main_get_qos_enabled())
  {
    netmgr_ctl_port_config_type *port_cfg;

    port_cfg = netmgr_qmi_get_port_config(link_num);
    if (!port_cfg)
    {
      netmgr_log_err("%s(): link %d in invalid state", __func__, link_num);
      goto bail;
    }

    netmgr_log_med("%s():  configure link %d dynamic %d ondemand %d", __func__, link_num,
                  netmgr_main_cfg.dynamic_link_creation,
                  port_cfg->on_demand );
    if ( netmgr_main_cfg.dynamic_link_creation && port_cfg->on_demand )
    {
      (void) netmgr_tc_reset_link( link_num );
      netmgr_log_med("%s(): netmgr_tc_reset_link  link %d", __func__, link_num);
    }
  }
#endif
...
)
{
      ...
      !NETMGR_KIF_IS_REV_RMNET_LINK(cmd_buf->data.link) )
  {
    ...
    /*Delete the cached qos flows and filters*/
    netmgr_qmi_clear_flow_cache(cmd_buf->data.link, MAX_PDU);
  }
#endif /* NETMGR_QOS_ENABLED */
  ...
  if (NETMGR_KIF_IS_REV_RMNET_LINK(cmd_buf->data.link))
  {
      ...
      if (NETMGR_SUCCESS != netmgr_qmi_retrieve_rev_ip_config_status(cmd_buf->data.link, &ip_family, &config_status))
      {
         netmgr_log_err("netmgr_sm_kif_closed: failed to get rev IP config status for link=%d, ignoring\n",
                        cmd_buf->data.link);
      }
      else
      {
...
)
{
  ...
  cmd_buf = (netmgr_exec_cmd_t *)payload;
  /* Interface never went fully up so primary should be used SSCM3*/
  addr_info = netmgr_qmi_get_addr_info(cmd_buf->data.link, PRIMARY_PDU);
...
)
{
  ...
  wds_info = netmgr_qmi_get_link_wds_info(cmd_buf->data.link);
  if (!wds_info)
  {
    netmgr_log_err("Invalid link %d received\n", cmd_buf->data.link);
    return next_state;
  }

  if (wds_info->v4_pkt_conn)
  {
    if (NETMGR_SUCCESS != netmgr_qmi_get_modem_link_info_mtu((uint8)cmd_buf->data.link,
  ...
  netmgr_kif_set_mtu(cmd_buf->data.link, FALSE);
  ...
  {
    {
      netmgr_log_med("Submitting cached flows");
      netmgr_qmi_try_submit_cached_flows(cmd_buf->data.link);
    }
  }
#endif /* NETMGR_QOS_ENABLED */
...
)
{
  ...
  if( NULL == event_info ) {
    ...
    /* Handle OEM Proxy Reverse Link */
    if(netmgr_main_cfg.oemprxy_enabled && NETMGR_KIF_IS_REV_RMNET_LINK(event_info->link))
    {
      event_info->event = NET_PLATFORM_OEMPRXY_IFACE_UP;
    }
    ...
    if( modem_addr_ptr )
    {
      ...
      if( NETMGR_NLA_PARAM_PREFIXADDR & nl_addr_ptr->attr_info.param_mask ) {
        ...
        if (NETMGR_IPV4_ADDR == modem_addr_ptr->if_addr.type)
        {
          event_info->addr_info.addr.mask = (unsigned int)ds_get_num_bits_set_count(modem_addr_ptr->if_mask);
        }
        else
        {
          event_info->addr_info.addr.mask = modem_addr_ptr->if_mask;
        }
      ...
      event_info->mtu = netmgr_kif_get_mtu(cmd_buf->data.link, PRIMARY_PDU);
      ...
      event_info->pdu_id = modem_addr_ptr->pdu_id;
      event_info->param_mask |= NETMGR_EVT_PARAM_PDU_ID;

      link_info = netmgr_qmi_get_link_info(event_info->link);
      if (link_info)
      {
        event_info->cmd_data.modem_subs_id = link_info->modem_subs_id;
      }
...
)
{
  ...
  cmd_buf = (netmgr_exec_cmd_t *)payload;
  if (cmd_buf == NULL || cmd_buf->data.info.connect_msg.addr_info_ptr == NULL) {
    NETMGR_LOG_FUNC_EXIT;
    return NETMGR_STATE_INITED;
  }

  ip_addr_type = cmd_buf->data.info.connect_msg.addr_info_ptr->if_addr.type;
...
#endif
  else if ( NETMGR_KIF_RECONFIGURING == netmgr_kif_get_state(cmd_buf->data.link) ) {
  ...
  if (NULL != cmd_buf->data.info.connect_msg.addr_info_ptr->sec_addr_info_ptr)
  {
    ds_free( cmd_buf->data.info.connect_msg.addr_info_ptr->sec_addr_info_ptr );
  }
  /* Release heap memory allocated in QMI module */
...
)
{
  ...
  {
    netmgr_qmi_hold_flow_info(cmd_buf->data.link,
                              &cmd_buf->data.info.qos_flow);
    return next_state;
  }

  if (netmgr_qmi_maybe_hold_flow_info(cmd_buf->data.link,
                                      &cmd_buf->data.info.qos_flow))
  {
    return next_state;
  }
     ...
     !NETMGR_KIF_IS_REV_RMNET_LINK(cmd_buf->data.link) )
  {
    ...
    else if (!NETMGR_IS_SEC_PDU_FLOW(cmd_buf->data.info.qos_flow.flow_id)) {
      /* Post event indication to clients */
      event_info = netmgr_malloc( sizeof(netmgr_nl_event_info_t) );
      if( NULL == event_info ) {
        netmgr_log_err("failed to allocate event buffer!\n");
...
)
{
  ...
  if (!NETMGR_KIF_IS_REV_RMNET_LINK(cmd_buf->data.link))
  {
    ...
    if (NETMGR_LINK_MAX != (rev_link = netmgr_qmi_iwlan_get_link_assoc(cmd_buf->data.link)))
    {
      ...
      {
        netmgr_kif_iface_close((uint8)rev_link, NULL, TRUE, FALSE);
      }
    }
  }
#else
...
)
{
  ...
  /* Initiate KIF interface configuration process */
  if( NETMGR_SUCCESS !=
      netmgr_kif_wlan_close( (uint8)cmd_buf->data.link, cmd_buf->data.info.wlan_ifname))
  {
