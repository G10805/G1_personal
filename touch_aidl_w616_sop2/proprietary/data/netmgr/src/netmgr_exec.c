...
/*===========================================================================
  ...
  Copyright (c) 2010-2017, 2019, 2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
*/
/*=========================================================================*/
void netmgr_exec_process_is_complete_msg(void)
{
  int i = 0;
...
/*!
@brief
  Process OOS complete event

@return
  None
*/
/*=========================================================================*/
void netmgr_exec_process_oos_complete_msg(void)
{
...
/*===========================================================================
  FUNCTION  netmgr_exec_process_op_status_msg
===========================================================================*/
/*!
@brief
  Report back to clients in case some internal netmgr processing of events
  fails
...
*/
/*=========================================================================*/
void netmgr_exec_process_op_status_msg(int link,
                                       int subs_id,
                                       netmgr_op_status_info_t *op_info)
{
  int rc = NETMGR_SUCCESS;
  netmgr_nl_event_info_t *event_info = NULL;

  if ((NETMGR_INVALID_IFACE == link)
        || !op_info
        || (NETMGR_OP_STATUS_CODE_INVALID >= op_info->op_code)
        || (NETMGR_OP_STATUS_CODE_MAX <= op_info->op_code))
  {
    /* Parameter is invalid. As a last resort, abort
       and restart netmgr to force recovery */
    NETMGR_ABORT("process_op_status: invalid parameter!");
    return;
  }
  ...
  if (NULL == event_info)
  {
    /* Failed to allocate memory to post event to clients
       As a last resort, abort and restart netmgr to force
       recovery */
    NETMGR_ABORT("process_op_status: failed to alloc memory!");
    return;
  }

  memset(event_info, 0, sizeof(netmgr_nl_event_info_t));

  event_info->event = NET_PLATFORM_OP_FAILED_EV;
  event_info->link = link;
  event_info->cmd_data.modem_subs_id = subs_id;
  memcpy(&event_info->op_status, op_info, sizeof(netmgr_op_status_info_t));
  ...
  rc = netmgr_kif_send_event_msg(event_info);
  if (NETMGR_SUCCESS != rc)
  {
    /* Failed to send message to clients
       As a last resort, abort and restart netmgr to force
       recovery */
    NETMGR_ABORT("process_op_status: failed to send event to client!");
    return;
  }
...
netmgr_exec_process_msg (ds_cmd_t * cmd, void * data)
{
  ...
  switch (exec_cmd->data.type)
  {
    ...
    case NETMGR_OOS_COMPLETE_MSG:
      ...
      break;
    case NETMGR_OP_FAILED_EV:
      netmgr_exec_process_op_status_msg(exec_cmd->data.link,
                                        exec_cmd->data.info.usr_cmd.modem_subs_id,
                                        &exec_cmd->data.op_status);
      break;
    default:
