...
/*===========================================================================
  ...
  Copyright (c) 2010-2019, 2021 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
)
{
  ...
  switch (type)
  {
    ...
    {
      algo = netmgr_xfrm_utils_get_hash_algo(sa_config, sa_config_ex_frag, frag_count);
    ...
    {
      algo = netmgr_xfrm_utils_get_crypto_algo(sa_config, sa_config_ex_frag, frag_count);

      if (WDS_IPSEC_CRYPTO_ALGO_AES128_V01 == algo ||
          WDS_IPSEC_CRYPTO_ALGO_AES256_V01 == algo)
      {
        if (!(netmgr_xfrm_utils_get_aes_mode_valid(sa_config, sa_config_ex_frag, frag_count)) ||
             (WDS_IPSEC_AES_MODE_CBC_V01 == netmgr_xfrm_utils_get_aes_mode(sa_config, sa_config_ex_frag, frag_count)))
        {
          algo_str = "cbc(aes)";
        }
        else if (WDS_IPSEC_AES_MODE_CTR_V01 == netmgr_xfrm_utils_get_aes_mode(sa_config, sa_config_ex_frag, frag_count))
        {
        ...
        else
        {
          netmgr_log_err("%s(): invalid AES mode=%d received",
                         __func__, netmgr_xfrm_utils_get_aes_mode(sa_config, sa_config_ex_frag, frag_count));
          break;
        }
      }
