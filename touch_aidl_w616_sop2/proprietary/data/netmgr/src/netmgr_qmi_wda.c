...
/*===========================================================================
  ...
  Copyright (c) 2013-2015, 2018-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
)
{
  ...
  rc = netmgr_qmi_client_init_instance(wda_get_service_object_v01(),
                                       qcci_conn_id,
                                       NULL,
                                       NULL,
                                       &os_params,
...
)
{
  ...
  {
    rc = netmgr_qmi_client_release(*qmi_clnt_hndl);
...
/*===========================================================================
  FUNCTION  netmgr_wda_set_ll_data_format
===========================================================================*/
/*!
@brief
  Negociate LL data format with modem.
*/
/*=========================================================================*/
int netmgr_wda_set_ll_data_format
(
  qmi_client_type  qmi_clnt_hndl
)
{
  ...
  request.ep_id_valid = 1;
  request.ep_id.iface_id = netmgr_main_cfg.llc.epid;
  request.ep_id.ep_type = netmgr_main_cfg.ep_type;

  request.link_prot_valid = 1;
  request.link_prot = WDA_LINK_LAYER_IP_MODE_V01;

  request.ul_data_aggregation_protocol_valid = 1;
  request.ul_data_aggregation_protocol =
      netmgr_main_cfg.data_format.ul_data_aggregation_protocol;

  request.dl_data_aggregation_protocol_valid = 1;
  request.dl_data_aggregation_protocol =
      netmgr_main_cfg.data_format.dl_data_aggregation_protocol;

  request.ul_data_aggregation_max_datagrams_valid = 1;
  request.ul_data_aggregation_max_datagrams = netmgr_main_cfg.llc.ul_agg_cnt;

  request.ul_data_aggregation_max_size_valid = 1;
  request.ul_data_aggregation_max_size = netmgr_main_cfg.llc.ul_agg_size;

  request.dl_data_aggregation_max_datagrams_valid = 1;
  request.dl_data_aggregation_max_datagrams = netmgr_main_cfg.llc.dl_agg_cnt;

  request.dl_data_aggregation_max_size_valid = 1;
  request.dl_data_aggregation_max_size = netmgr_main_cfg.llc.dl_agg_size;

  request.flow_control_valid = 1;
  request.flow_control = 1;
  ...
  if (QMI_NO_ERR != rc || QMI_RESULT_SUCCESS_V01 != response.resp.result)
  {
    netmgr_log_err("Error sending LL data format message: %d", rc);
    return NETMGR_WDA_QMI_FAILURE;
  }

  if (response.ul_data_aggregation_protocol_valid &&
      response.ul_data_aggregation_protocol !=
        netmgr_main_cfg.data_format.ul_data_aggregation_protocol)
  {
    netmgr_log_err("UL protocol not supported");
    return NETMGR_WDA_QMI_FAILURE;
  }

  if (response.dl_data_aggregation_protocol_valid &&
      response.dl_data_aggregation_protocol !=
        netmgr_main_cfg.data_format.dl_data_aggregation_protocol)
  {
    netmgr_log_err("DL protocol not supported");
    return NETMGR_WDA_QMI_FAILURE;
  }

  if (response.ul_data_aggregation_max_datagrams_valid &&
      response.ul_data_aggregation_max_datagrams < netmgr_main_cfg.llc.ul_agg_cnt)
    netmgr_main_cfg.llc.ul_agg_cnt = response.ul_data_aggregation_max_datagrams;

  if (response.ul_data_aggregation_max_size_valid &&
      response.ul_data_aggregation_max_size < netmgr_main_cfg.llc.ul_agg_size)
    netmgr_main_cfg.llc.ul_agg_size = response.ul_data_aggregation_max_size;

  if (response.dl_data_aggregation_max_datagrams_valid &&
      response.dl_data_aggregation_max_datagrams < netmgr_main_cfg.llc.dl_agg_cnt)
    netmgr_main_cfg.llc.dl_agg_cnt = response.dl_data_aggregation_max_datagrams;

  if (response.dl_data_aggregation_max_size_valid &&
      response.dl_data_aggregation_max_size < netmgr_main_cfg.llc.dl_agg_size)
    netmgr_main_cfg.llc.dl_agg_size = response.dl_data_aggregation_max_size;

  netmgr_log_high("LLC UL agg cnt  [%u]", netmgr_main_cfg.llc.ul_agg_cnt);
  netmgr_log_high("LLC UL agg size [%u]", netmgr_main_cfg.llc.ul_agg_size);
  netmgr_log_high("LLC DL agg_cnt  [%u]", netmgr_main_cfg.llc.dl_agg_cnt);
  netmgr_log_high("LLC DL agg_size [%u]", netmgr_main_cfg.llc.dl_agg_size);
...
)
{
  ...
  /* Configures SSCM3*/
  request.ssc_mode3_capability_valid = netmgr_main_cfg.sscm3_config;
  request.ssc_mode3_capability = netmgr_main_cfg.sscm3_config;
  netmgr_log_high("%s(): %s requesting sscm3\n", __func__, (netmgr_main_cfg.sscm3_config) ? "" : "not");

  /* Configures IPv6 multihoming */
  request.ipv6_multihoming_capability_valid = TRUE;
  request.ipv6_multihoming_capability = (netmgr_main_cfg.ipv6_multihoming_config) ?
                                        TRUE : FALSE;
  netmgr_log_high("%s(): %s requesting multohoming\n", __func__,
                  (netmgr_main_cfg.ipv6_multihoming_config) ? "" : "not");

  request.default_flow_rebind_ver_valid = TRUE;
  request.default_flow_rebind_ver = netmgr_main_cfg.kfc_mode ?
    WDA_DEFAULT_FLOW_REBIND_VERSION_1_V01 : WDA_DEFAULT_FLOW_REBIND_VERSION_0_V01;
  ...
  /* Verify that modem supports SSCM3 before configuring it internally */
  netmgr_main_cfg.sscm3 = response.ssc_mode3_capability_valid && response.ssc_mode3_capability;
  netmgr_log_high("%s(): modem sscm3 support: %d\n", __func__, netmgr_main_cfg.sscm3);

  /* Similarly for IPv6 multohoming */
  netmgr_main_cfg.ipv6_multihoming = response.ipv6_multihoming_capability_valid &&
                                     response.ipv6_multihoming_capability;
  netmgr_log_high("%s(): modem multihoming support: %d\n", __func__,
                  netmgr_main_cfg.ipv6_multihoming);
  ...
  if (netmgr_main_cfg.llc.enabled)
  {
    ...
    request.low_latency_capability_valid = TRUE;
    request.low_latency_capability = 1;
    ...
    if (QMI_NO_ERR != rc || QMI_RESULT_SUCCESS_V01 != response.resp.result)
    {
      netmgr_log_err("%s(): Error sending LL cap message: %d", __func__, rc);
      netmgr_main_cfg.llc.enabled = FALSE;
      rc = NETMGR_WDA_QMI_FAILURE;
      goto out;
    }

    if (response.low_latency_capability_valid &&
        response.low_latency_capability)
    {
        netmgr_wda_set_ll_data_format(qmi_clnt_hndl);
    }
    else
    {
        netmgr_log_err("No Low latency capability");
        netmgr_main_cfg.llc.enabled = FALSE;
    }
  }
