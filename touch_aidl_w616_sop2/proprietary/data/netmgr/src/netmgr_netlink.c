...
/*===========================================================================
  ...
  Copyright (c) 2010, 2013-2016, 2019-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
#define NETMGR_NL_COPY_ROUTE_ADDR( name, mask, element )                                  \
        route_info->attr_info.element.ss_family = route_info->metainfo.rtm_family;        \
        memcpy( &route_info->attr_info.element.__data,                                    \
                RTA_DATA(rtah),                                                           \
                sizeof(route_info->attr_info.element.__data) );                           \
        route_info->attr_info.param_mask |= mask;                                         \
        NETMGR_NL_REPORT_ADDR( med, "Attribute: " name, route_info->attr_info.element );
  ...
  */
  ...
  while( info->running )
  {
    ...
    if ((ret = select(info->maxfd+1, &info->fdset, NULL, NULL, NULL)) < 0)
    {
      ...
      /* Ignore select errors due to EINTR  or EAGAIN */
      if (-1 == ret && (EINTR == errno || EAGAIN == errno))
      {
        netmgr_log_err("%s(): Ignoring select errors with errno=%s",
                              __func__, strerror(errno));
        continue;
      }

      NETMGR_ABORT("netmgr_nl_socklthrd_main: select failed with errno=[%d, %s]!",
...
init:
  ...
  if ((*p_sk_fd = netmgr_socket(AF_NETLINK, SOCK_RAW | SOCK_CLOEXEC, proto)) <= 0)
  {
    netmgr_log_err("%s(): Allocated fd: %d \n", __func__, *p_sk_fd );
    if(*p_sk_fd == 0)
    {
      goto init;
    }
...
)
{
  ...
  size+= (NETMGR_EVT_PARAM_PDU_ID    & event_info->param_mask)? (int)NETMGR_RTASIZE_PDU_ID : 0;
  size+= (NETMGR_EVT_PARAM_ADDR_TYPE & event_info->param_mask)? (int)NETMGR_RTASIZE_ADDR_TYPE : 0;
  ...
  size+= (NETMGR_EVT_PARAM_OP_STATUS & event_info->param_mask)? (int)NETMGR_RTASIZE_OP_STATUS : 0;
    ...
    case NET_PLATFORM_OP_FAILED_EV:
      ...
      *buflen = size;
      ...
      if(NETMGR_EVT_PARAM_MTU & event_info->param_mask) {
        ...
        memcpy( rtad, &event_info->mtu, sizeof(event_info->mtu)*NUM_MTU_VALS );
      }

      if (NETMGR_EVT_PARAM_PDU_ID & event_info->param_mask) {
        rtah = RTA_NEXT(rtah, size);
        NETMGR_NL_RTA_NEXT( NETMGR_RTATYPE_PDU_ID, NETMGR_RTASIZE_PDU_ID );
        memcpy( rtad, &event_info->pdu_id, sizeof(event_info->pdu_id) );
      }

      if (NETMGR_EVT_PARAM_ADDR_TYPE & event_info->param_mask) {
        rtah = RTA_NEXT(rtah, size);
        NETMGR_NL_RTA_NEXT( NETMGR_RTATYPE_ADDR_TYPE, NETMGR_RTASIZE_ADDR_TYPE );
        memcpy (rtad, &event_info->addr_info.address_type, sizeof(event_info->addr_info.address_type) );
      }
      ...
      if (NETMGR_EVT_PARAM_OP_STATUS & event_info->param_mask) {
        rtah = RTA_NEXT(rtah, size);
        NETMGR_NL_RTA_NEXT(NETMGR_RTATYPE_OP_STATUS, NETMGR_RTASIZE_OP_STATUS);
        memcpy( rtad, &event_info->op_status, sizeof(event_info->op_status) );
      }
  ...
  while( RTA_OK(rtah, buflen ) ) {
    ...
    switch( rtah->rta_type ) {
      ...
      case NETMGR_RTATYPE_MTU:
        ds_memscpy( &event_info->mtu, sizeof(event_info->mtu)*NUM_MTU_VALS, RTA_DATA(rtah), RTA_ALIGN((rtah)->rta_len) );
        event_info->param_mask |= NETMGR_EVT_PARAM_MTU;
        netmgr_log_med( "Attribute: MTU=%d, MTU_v4 = %d, MTU_v6 = %d",
                        event_info->mtu, event_info->mtu4, event_info->mtu6 );
        break;

      case NETMGR_RTATYPE_PDU_ID:
        ds_memscpy( &event_info->pdu_id, sizeof(event_info->pdu_id), RTA_DATA(rtah), RTA_ALIGN((rtah)->rta_len) );
        event_info->param_mask |= NETMGR_EVT_PARAM_PDU_ID;
        netmgr_log_med( "Attribute: PDU_ID = %u", event_info->pdu_id);
        break;

      case NETMGR_RTATYPE_ADDR_TYPE:
        ds_memscpy (&event_info->addr_info.address_type, sizeof(event_info->addr_info.address_type), RTA_DATA(rtah), RTA_ALIGN((rtah->rta_len) ));
        event_info->param_mask |= NETMGR_EVT_PARAM_ADDR_TYPE;
        netmgr_log_med( "Attribute: ADDR_TYPE = %u", event_info->addr_info.address_type );
        break;
      ...
      case NETMGR_RTATYPE_OP_STATUS:
        ds_memscpy( &event_info->op_status, sizeof(event_info->op_status), RTA_DATA(rtah), RTA_ALIGN((rtah)->rta_len) );
        event_info->param_mask |= NETMGR_EVT_PARAM_OP_STATUS;
        netmgr_log_med( "Attribute: op_failed err_code [%d]", event_info->op_status);
        break;
...
#define RTN_RTA(r)  ((struct rtattr*)(((char*)(r)) + NLMSG_ALIGN(sizeof(struct rtmsg))))

/*===========================================================================
  FUNCTION  netmgr_nl_decode_route_msg
===========================================================================*/
/*!
@brief
  Decode Route event parameters from Netlink attribute TLVs.
...
*/
/*=========================================================================*/
int netmgr_nl_decode_route_msg
(
  const char                 *buffer,
  unsigned int               buflen,
...
)
{
  ...
  unsigned int             rta_nested_len = 0;
  char                     *devname = NULL;

  NETMGR_ASSERT( buffer );
  NETMGR_ASSERT( route_info );
  ...
  netmgr_log_med( "Metainfo: msg_type:%s Family=%d Table=%d %s%s%s%s  Protocol=%d %s%s%s%s%s "
                  "Scope=0x%x type=%d Flags=[0x%x]",
                  (msg_type == RTM_NEWROUTE) ? "RTM_NEWROUTE" : "RTM_DELROUTE",
                  route_info->metainfo.rtm_family,
                  route_info->metainfo.rtm_table,
                  (RT_TABLE_UNSPEC == route_info->metainfo.rtm_table) ? "RT_TABLE_UNSPEC":"",
                  (RT_TABLE_MAIN == route_info->metainfo.rtm_table) ? "RT_TABLE_MAIN":"",
                  (RT_TABLE_LOCAL == route_info->metainfo.rtm_table) ? "RT_TABLE_LOCAL":"",
                  (RT_TABLE_DEFAULT == route_info->metainfo.rtm_table) ? "RT_TABLE_DEFAULT":"",
                  route_info->metainfo.rtm_protocol,
                  (RTPROT_UNSPEC == route_info->metainfo.rtm_protocol) ? "RTPROT_UNSPEC": "",
                  (RTPROT_REDIRECT == route_info->metainfo.rtm_protocol) ? "RTPROT_REDIRECT": "",
                  (RTPROT_KERNEL == route_info->metainfo.rtm_protocol) ? "RTPROT_KERNEL": "",
                  (RTPROT_STATIC == route_info->metainfo.rtm_protocol) ? "RTPROT_STATIC": "",
                  (RTPROT_BOOT == route_info->metainfo.rtm_protocol) ? "RTPROT_SBOOT": "",
                  route_info->metainfo.rtm_scope,
                  route_info->metainfo.rtm_type,
                  route_info->metainfo.rtm_flags);

    /* Extract the available attributes */
    route_info->attr_info.param_mask = NETMGR_ROUTE_PARAM_NONE;
    rtah = RTN_RTA( NLMSG_DATA(nlh) );

    while( RTA_OK(rtah, buflen ) ) {
      switch( rtah->rta_type ) {
        case RTA_DST:
             NETMGR_NL_COPY_ROUTE_ADDR( "Destination", NETMGR_ROUTE_PARAM_DST, dst_addr );
             break;
        case RTA_SRC:
             NETMGR_NL_COPY_ROUTE_ADDR( "Source", NETMGR_ROUTE_PARAM_SRC, src_addr );
             break;
        case RTA_GATEWAY:
             NETMGR_NL_COPY_ROUTE_ADDR( "Gateway", NETMGR_ROUTE_PARAM_GATEWAY, gateway_addr );
             break;
        case RTA_OIF:
             route_info->attr_info.oif = *(int*)RTA_DATA(rtah);
             route_info->attr_info.param_mask |= NETMGR_ROUTE_PARAM_OIF;

             devname = if_indextoname(route_info->attr_info.oif, linkname);
             if (!devname)
             {
               netmgr_log_err("%s(): failed to convert oif ifindex to name!", __func__);
               break;
             }
             netmgr_log_med("RTA_OIF idx:%d link_name:%s\n", route_info->attr_info.oif, devname);
             break;
        case RTA_IIF:
             route_info->attr_info.iif = *(int*)RTA_DATA(rtah);
             route_info->attr_info.param_mask |= NETMGR_ROUTE_PARAM_IIF;

             devname = if_indextoname(route_info->attr_info.iif, linkname);
             if (!devname)
             {
               netmgr_log_err("%s(): failed to convert iif ifindex to name!", __func__);
               break;
             }
             netmgr_log_med("RTA_IIF idx:%d link_name:%s\n", route_info->attr_info.iif, devname);
             break;
        case RTA_CACHEINFO:
             route_info->attr_info.param_mask |= NETMGR_ROUTE_PARAM_CACHEINFO;
             memcpy(&route_info->attr_info.cacheinfo, (uint8*)RTA_DATA(rtah), sizeof(struct rta_cacheinfo));
             if (route_info->attr_info.cacheinfo.rta_expires)
               netmgr_log_med("RTA_EXPIRES:%dsec\n", route_info->attr_info.cacheinfo.rta_expires/sysconf(_SC_CLK_TCK));
             break;
        case RTA_METRICS:
             route_info->attr_info.param_mask |= NETMGR_ROUTE_PARAM_METRICS;
             rta_nested_len = rtah->rta_len;
             rtah = (struct rtattr*) RTA_DATA(rtah);
             while ( RTA_OK(rtah, rta_nested_len) ) {
                if ( RTAX_MTU == rtah->rta_type ) {
                  route_info->attr_info.rtax_mtu = *(int*)RTA_DATA(rtah);
             	}
             netmgr_log_med("RTA_METRICS type: %d len: %d\n", rtah->rta_type, *(int*)RTA_DATA(rtah));
             if ( rta_nested_len >= RTA_ALIGN((rtah)->rta_len ))
                 rtah = RTA_NEXT(rtah, rta_nested_len);
             else
                 return NETMGR_FAILURE;
             }
             break;
        case RTA_TABLE:
             route_info->attr_info.param_mask |= NETMGR_ROUTE_PARAM_TABLE;
             route_info->attr_info.tableid  = *(int*)RTA_DATA(rtah);
             netmgr_log_med("RTA_TABLE %d\n", route_info->attr_info.tableid);
             break;
        default:
             break;
    }
...
)
{
  ...
  while( NLMSG_OK(nlh, buflen) ) {
    switch( nlh->nlmsg_type ) {
      ...
      case RTM_DELROUTE:
        msg_ptr->type = nlh->nlmsg_type;
        netmgr_log_err( "Processing %s\n", (RTM_NEWROUTE == nlh->nlmsg_type) ?
                                            "RTM_NEWROUTE": "RTM_DELROUTE" );
        if ( NETMGR_SUCCESS !=
            netmgr_nl_decode_route_msg(buffer, buflen, &msg_ptr->route_info, msg_ptr->type) ) {
            return NETMGR_FAILURE;
        }
        break;
    ...
    if( buflen >= NLMSG_ALIGN((nlh)->nlmsg_len) )
    {
...
)
{
  ...
  netmgr_log_low("Starting nl listener thread\n");
