...
#define NETMGR_CONFIG_USE_NEW_RMNET_DRIVER 1
#define NETMGR_CONFIG_MIN_PHY_GSO_MAX_SIZE (4096)
#define NETMGR_CONFIG_MAX_PHY_GSO_MAX_SIZE (64000)
...
)
{
  ...
  switch(prop_val_int)
  {
    ...
    case IWLAN_V2_ENABLE:
     netmgr_main_cfg.iwlan_enabled = IWLAN_V2_ENABLE;
     netmgr_log_med("setting netmgr_main_cfg.iwlan_enabled as IWLAN_V2_ENABLE");
     break;

    case NSWO_V2_ONLY:
     netmgr_main_cfg.iwlan_enabled = NSWO_V2_ONLY;
     netmgr_log_med("setting netmgr_main_cfg.iwlan_enabled as NSWO_V2_ONLY");
     break;

    case IWLAN_OEMPRXY:
      netmgr_main_cfg.iwlan_enabled = IWLAN_OEMPRXY;
      netmgr_log_med("setting netmgr_main_cfg.iwlan_enabled as IWLAN_OEMPRXY");
      break;
...
)
{
  ...
  netmgr_main_cfg.rmnet_offload = (int)prop_val_int;
  ...
  if (ret != CFGDB_SUCCESS)
  {
    netmgr_log_err("Error reading property [%s] Err[%d]", prop_name, ret);
    prop_val_int = 0;
    /*Based on kernel version enable new rmnet_driver.
     *For targets like Starlord 2 versions exists - one with kernel upgraded to
     *4.19 and others having old kernel - since we dont have older rmnet driver
     *on 4.19+ kernels we need to rely on newer rmnet driver. Enable that here*/
    if(NETMGR_CONFIG_USE_NEW_RMNET_DRIVER == netmgr_config_check_new_rmnet_driver_availability())
    {
      netmgr_log_high("%s(): setting netmgr_main_cfg.rtm_rmnet_enabled to TRUE\n", __func__);
      prop_val_int = 1;
    }
  }
  ...
  netmgr_main_cfg.netmgr_listen_ev_proto = (int)prop_val_int;
...
/* netdev_budget */
  ...
  /* DFC powersave ext */
  snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".dfc_ps_ext");
  ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
  if (ret != CFGDB_SUCCESS)
  {
    prop_val_int = 0;
  }
  netmgr_main_cfg.dfc_ps_ext = (boolean)prop_val_int;
...
/* netdev_max_backlog */
  ...
  /* sscm3 */
  snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".sscm3");
  ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
  if (ret != CFGDB_SUCCESS)
  {
    prop_val_int = 0;
  ...
  netmgr_main_cfg.sscm3_config = (int)prop_val_int;

  /* ipv6 multihoming*/
  snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".ipv6_multihoming");
  ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
  if (ret != CFGDB_SUCCESS)
  {
    prop_val_int = 0;
  ...
  netmgr_main_cfg.ipv6_multihoming_config = (int)prop_val_int;
  ...
  /* netmgr_recovery_enabled */
  snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".netmgr_recovery_enabled");
  ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
  if (ret != CFGDB_SUCCESS)
  {
    netmgr_log_err("Error reading property [%s] Err[%d]; Setting to 0", prop_name, ret);
  ...
  netmgr_main_cfg.netmgr_recovery_enabled = (int)prop_val_int;

/* iwlan_concurrency  */
  snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".iwlan_concurrency");
  ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
  if (ret != CFGDB_SUCCESS)
  {
    netmgr_log_err("Error reading property [%s] Err[%d]; Setting to 0", prop_name, ret);
  ...
  netmgr_main_cfg.iwlan_concurrency = (boolean)prop_val_int;

  /* nl_xfrm */
  snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".nl_xfrm");
  ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
  if (ret != CFGDB_SUCCESS)
  {
    netmgr_log_err("Error reading property [%s] Err[%d]; Setting to 0", prop_name, ret);
  ...
  netmgr_main_cfg.nl_xfrm = (boolean)prop_val_int;

  /* phy_gso_max_size */
  snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".phy_gso_max_size");
  ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
  if (ret != CFGDB_SUCCESS)
  {
    netmgr_log_err("Error reading property [%s] Err[%d]; Setting to 0", prop_name, ret);
  ...
  if (prop_val_int)
  {
    if (prop_val_int < NETMGR_CONFIG_MIN_PHY_GSO_MAX_SIZE)
    {
      prop_val_int = NETMGR_CONFIG_MIN_PHY_GSO_MAX_SIZE;
    }
    if (prop_val_int > NETMGR_CONFIG_MAX_PHY_GSO_MAX_SIZE)
    {
      prop_val_int = NETMGR_CONFIG_MAX_PHY_GSO_MAX_SIZE;
    }
  }
  netmgr_main_cfg.phy_gso_max_size = (int)prop_val_int;

  /* LL channel */
  snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".llc");
  ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
  netmgr_main_cfg.llc.enabled = (ret == CFGDB_SUCCESS ? (boolean)prop_val_int : FALSE);

  if (netmgr_main_cfg.llc.enabled)
  {
    /* LL epid */
    snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".llc_epid");
    ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
    netmgr_main_cfg.llc.epid = (ret == CFGDB_SUCCESS ? prop_val_int : 0);

    snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".llc_ul_agg_cnt");
    ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
    if (ret == CFGDB_SUCCESS)
      netmgr_main_cfg.llc.ul_agg_cnt = prop_val_int;
    else
      netmgr_main_cfg.llc.ul_agg_cnt = 1;

    snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".llc_ul_agg_size");
    ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
    if (ret == CFGDB_SUCCESS)
      netmgr_main_cfg.llc.ul_agg_size = prop_val_int;
    else
      netmgr_main_cfg.llc.ul_agg_size = 0;

    snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".llc_ul_agg_time");
    ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
    if (ret == CFGDB_SUCCESS)
      netmgr_main_cfg.llc.ul_agg_time = prop_val_int;
    else
      netmgr_main_cfg.llc.ul_agg_time = 0;
    if (netmgr_main_cfg.llc.ul_agg_time < 1000000)
      netmgr_main_cfg.llc.ul_agg_time = 1000000;

    snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".llc_ul_agg_features");
    ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
    if (ret == CFGDB_SUCCESS)
      netmgr_main_cfg.llc.ul_agg_features = prop_val_int;
    else
      netmgr_main_cfg.llc.ul_agg_features = 0;

    snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".llc_dl_agg_cnt");
    ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
    if (ret == CFGDB_SUCCESS)
      netmgr_main_cfg.llc.dl_agg_cnt = prop_val_int;
    else
      netmgr_main_cfg.llc.dl_agg_cnt = 1;

    snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".llc_dl_agg_size");
    ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
    if (ret == CFGDB_SUCCESS)
      netmgr_main_cfg.llc.dl_agg_size = prop_val_int;
    else
      netmgr_main_cfg.llc.dl_agg_size = 0;

    snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".llc_pnd_rps_mask");
    ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
    if (ret == CFGDB_SUCCESS)
      netmgr_main_cfg.llc.pnd_rps_mask = prop_val_int;
    else
      netmgr_main_cfg.llc.pnd_rps_mask = 0;
  }

  /* oemprxy_enabled */
  snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.",
           target, ".oemprxy_enabled");
  ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
  if (ret != CFGDB_SUCCESS)
  {
    netmgr_log_err("Error reading property [%s] Err[%d]; Setting to 0", prop_name, ret);
  ...
  netmgr_main_cfg.oemprxy_enabled = (boolean)prop_val_int;
  ...
  {
    int j;
    int egress_param_len, egress_param_len_w_padding;
    int ingress_param_len, ingress_param_len_w_padding;
    int32 ep_config_ext_ioctl_v2 = 0;
    int32 egress_ep_configs_len = 0;
    int32 ingress_ep_configs_len = 0;
    struct egress_format_v2 *eg_fmt;
    struct ingress_format_v2 *ing_fmt;
    __u32 *eg_ptr;
    __u32 *ing_ptr;

    /* ep_config_ext_ioctl_v2 */
    snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".ep_config_ext_ioctl_v2");
    ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &ep_config_ext_ioctl_v2, sizeof(ep_config_ext_ioctl_v2));
    if (ret != CFGDB_SUCCESS)
    {
      netmgr_log_err("Error reading property [%s] Err[%d]", prop_name, ret);
      /* It's ok to not use ioctvl2  */
      netmgr_main_cfg.ep_config_ext_ioctl_v2 = 0;
      goto post_ioctl_extended_v2;
    }
    netmgr_main_cfg.ep_config_ext_ioctl_v2 = ep_config_ext_ioctl_v2;

    /* egress_ep_configs_len and egress_ep_configs */
    snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".egress_ep_configs_len");
    ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &egress_ep_configs_len, sizeof(egress_ep_configs_len));
    if (ret != CFGDB_SUCCESS)
    {
      netmgr_log_err("Error reading property [%s] Err[%d]", prop_name, ret);
      result = -1;
    ...
    eg_fmt = malloc(sizeof(struct egress_format_v2));
    if (eg_fmt)
    {
      struct rmnet_egress_param *eg_ptr_temp;

      eg_fmt->number_of_eps = egress_ep_configs_len;
      eg_fmt->egress_param_size = sizeof(struct rmnet_egress_param);
      eg_fmt->egress_param_ptr = (__u64)(uintptr_t) malloc(sizeof(struct rmnet_egress_param) * egress_ep_configs_len);
      if (!eg_fmt->egress_param_ptr)
      {
        free(eg_fmt);
        netmgr_log_err("Failed to allocate resources for egress ep configuration");
        result = -1;
        goto bail;
      }
      memcpy(&netmgr_main_cfg.eg_fmt, eg_fmt, sizeof(struct egress_format_v2));
      free(eg_fmt);

      /* Since rmnet_egress_param is mandated to only contain __u32 with the potential
      * for padding if there are an odd number of values within the struct, we can
      * count the number of values like below
      */
      egress_param_len_w_padding = sizeof(struct rmnet_egress_param) / sizeof(__u32);
      egress_param_len = egress_param_len_w_padding;
      for (i = 0; i < egress_ep_configs_len; i++)
      {
        /* The order with which EPs are read in is the order which they will be
        * sent to HW. HW expects the EPs in a specific order so please be careful
        * if modifying ordering with the xml file.
        */
        for (j = 0; j < egress_param_len; j++) {
          snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s%d%s%d", "netmgr_config.", target, ".egress_ep_configs.", i, ".", j);
          ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
          if (ret != CFGDB_SUCCESS)
          {
            prop_val_int = 0;
          ...
          eg_ptr = (__u32 *) (((__u32 *)netmgr_main_cfg.eg_fmt.egress_param_ptr) + ((i*egress_param_len_w_padding) + j));
          *eg_ptr = (__u32) prop_val_int;
        }
        eg_ptr_temp = ((struct rmnet_egress_param *) netmgr_main_cfg.eg_fmt.egress_param_ptr) + i;
        netmgr_log_med("Summary of writes for ep config #%d: %u, %u, %u, %u, %u, %u, %u, %u", i, eg_ptr_temp->egress_ep_type, eg_ptr_temp->pipe_setup_status,
    ...
    /* ingress_ep_configs_len and ingress_ep_configs */
    snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s", "netmgr_config.", target, ".ingress_ep_configs_len");
    ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &ingress_ep_configs_len, sizeof(ingress_ep_configs_len));
    if (ret != CFGDB_SUCCESS)
    {
      netmgr_log_err("Error reading property [%s] Err[%d]", prop_name, ret);
      result = -1;
    ...
    ing_fmt = malloc(sizeof(struct ingress_format_v2));
    if (ing_fmt)
    {
      struct rmnet_ingress_param *ing_ptr_temp;

      ing_fmt->number_of_eps = ingress_ep_configs_len;
      ing_fmt->ingress_param_size = sizeof(struct rmnet_ingress_param);
      ing_fmt->ingress_param_ptr = (__u64)(uintptr_t) malloc(sizeof(struct rmnet_ingress_param) * ingress_ep_configs_len);
      if (!ing_fmt->ingress_param_ptr)
      {
        free(ing_fmt);
        netmgr_log_err("Failed to allocate resources for ingress ep configuration");
        result = -1;
        goto bail;
      }
      memcpy(&netmgr_main_cfg.ing_fmt, ing_fmt, sizeof(struct ingress_format_v2));
      free(ing_fmt);

      /* Since rmnet_ingress_param is mandated to only contain __u32 with the potential
      * for padding if there are an odd number of values within the struct, we can
      * count the number of values like below
      */
      ingress_param_len_w_padding = sizeof(struct rmnet_ingress_param) / sizeof(__u32);
      ingress_param_len = ingress_param_len_w_padding - 1;
      for (i = 0; i < ingress_ep_configs_len; i++)
      {
        /* The order with which EPs are read in is the order which they will be
        * sent to HW. HW expects the EPs in a specific order so please be careful
        * if modifying ordering with the xml file.
        */
        for (j = 0; j < ingress_param_len; j++) {
          snprintf(prop_name, NETMGR_CFGDB_STRVAL_MAX, "%s%s%s%d%s%d", "netmgr_config.", target, ".ingress_ep_configs.", i, ".", j);
          ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &prop_val_int, sizeof(prop_val_int));
          if (ret != CFGDB_SUCCESS)
          {
            prop_val_int = 0;
          ...
          ing_ptr = (__u32 *) (((__u32 *)netmgr_main_cfg.ing_fmt.ingress_param_ptr) + ((i*ingress_param_len_w_padding) + j));
          *ing_ptr = (__u32) prop_val_int;
        }
        ing_ptr_temp = ((struct rmnet_ingress_param *) netmgr_main_cfg.ing_fmt.ingress_param_ptr) + i;
        netmgr_log_med("Summary of writes for ep config #%d: %u, %u, %u, %u, %u, %u, %u, %u, %u", i, ing_ptr_temp->ingress_ep_type, ing_ptr_temp->pipe_setup_status,
...
bail:
  /* Update config based on SOC id for some targets */
  netmgr_config_overwrite();
  configdb_release();
  return result;
}

void netmgr_update_ingress_ep_configs()
{
  struct rmnet_ingress_param *ing_ptr_temp;
  int i = 0;

  if(!netmgr_main_cfg.ing_fmt.ingress_param_ptr)
  {
    netmgr_log_err("%s(): ingress ep configs ptr invalid\n", __func__);
    return;
  }

  for(i = 0; i < netmgr_main_cfg.ing_fmt.number_of_eps/2; i++)
  {
    ing_ptr_temp = ((struct rmnet_ingress_param *) netmgr_main_cfg.ing_fmt.ingress_param_ptr) + i;
    ing_ptr_temp->buff_size = 16384;
    netmgr_log_high("%s(): Updated ingress ep config #%d: buff_size: %u\n",
                     __func__, i, ing_ptr_temp->buff_size);
  }
...
void netmgr_config_overwrite()
{
  ...
  else if (DS_TARGET_SOC_ID_CROW == ds_get_soc_id() || DS_TARGET_SOC_ID_SHAW == ds_get_soc_id())
  {
    /* Update UL agg features based on
     * SOC ID to reduce memory impact.
     */
    netmgr_main_cfg.data_format.ul_agg_features = 0;

    /*Update ingress_format buff_size to 16k for mem optimizations*/
    netmgr_update_ingress_ep_configs();
  }
#ifdef NETMGR_AUTO
...
void netmgr_config_print()
{
  ...
  netmgr_log_med("..netmgr_listen_ev_proto                    : [%d]",     netmgr_main_cfg.netmgr_listen_ev_proto);
  ...
  netmgr_log_med("..iwlan_concurrency      : [%u]",     netmgr_main_cfg.iwlan_concurrency);
  netmgr_log_med("..phy_gso_max_size       : [%u]",     netmgr_main_cfg.phy_gso_max_size);
...
/*===========================================================================
  FUNCTION:  netmgr_config_check_new_rmnet_driver_availability
===========================================================================*/
/*!
    @brief
    Check whether new rmnet is present or not. Decision is based
    on kernel version as from kernel 4.19 it is present in kernels.
*/
/*=========================================================================*/
int
netmgr_config_check_new_rmnet_driver_availability()
{
  char *kernel_ver;
  ...
  if(ret != NETMGR_FAILURE)
  {
    ...
    if(kernel_ver[0] == '5')
    {
      netmgr_log_high("enabling new rmnet driver\n");
      return NETMGR_CONFIG_USE_NEW_RMNET_DRIVER;
    }
