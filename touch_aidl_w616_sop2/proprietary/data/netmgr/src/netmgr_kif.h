...
/*---------------------------------------------------------------------------
   Constant representing max wlan interfaces
---------------------------------------------------------------------------*/
#define NETMGR_KIF_MAX_WLAN_INTF 2
#define NETMGR_KIF_INTF_WLAN0 "wlan0"
#define NETMGR_KIF_INTF_WLAN1 "wlan1"

/*--------------------------------------------------------------------------
  Constant representing forward rmnet interfaces
---------------------------------------------------------------------------*/
#define NETMGR_KIF_INTF_RMNET_DATA "rmnet_data"

/*--------------------------------------------------------------------------
  Constant representing fraction of route expiry before an RS will be attempted
---------------------------------------------------------------------------*/
#define NETMGR_ROUTE_EXPIRY_TIMEOUT_PERCENT (0.8)
#define NETMGR_MODEM_ROUTER_LIFETIME (65535)
...
typedef enum
{
  /*ip addr add lfts = indefinite */
  NETMGR_KIF_LFT_DEFAULT,
  /*ip addr change lfts custom from modem */
  NETMGR_KIF_LFT_VALID_CHANGE,
  /*ip addr change lfts = indefinite */
  NETMGR_KIF_LFT_MAKE_PERM,
  /*ip addr add lfts custom from modem */
  NETMGR_KIF_LFT_ADD_W_LFT,
  NETMGR_KIF_LFT_MAX
} netmgr_kif_lft_change_t;
...
typedef struct netmgr_timer_info_s {
  timer_t                         timerid;
  bool                            is_active;
  int                             link;
} netmgr_timer_info_t;
...
typedef struct
{
  int wlan_iface_name_valid;
  char wlan_iface_name[NETMGR_IF_NAME_MAX_LEN];
} netmgr_kif_oemprxy_info_t;
#endif /* FEATURE_DATA_IWLAN */
...
/* Timers associated with routes of this link only local default route
 * is installed for iwlan, the timer structure is expanded to global address
 * for futue scope */
  netmgr_timer_info_t             route_timers[2];
#ifdef FEATURE_DATA_IWLAN
  netmgr_kif_oemprxy_info_t oemprxy_info;   /* Parameters required for supporting
                                               oemprxy data calls */
#endif /* FEATURE_DATA_IWLAN */
} netmgr_kif_info_t;
...
void netmgr_kif_set_wlan_ifname_addrmask(int link, int family);
...
*/
/*=========================================================================*/
void netmgr_kif_clear_wlan_ifname_on_link(int link, int family);
...
/*!
@brief
  Get the wlan interface information based on link

@return
  return the wlan ifname corresponding to the link
...
*/
/*=========================================================================*/
const char* netmgr_kif_get_wlan_ifname_by_link(int link);
...
/*!
@brief
  update the wlan ifname to linkinfo, if the ipaddr matches one of the wlan iface
...
*/
/*=========================================================================*/
void netmgr_kif_save_wlan_ifname_to_link(int link, char* ipaddr, int family);
...
*/
/*=========================================================================*/
void netmgr_kif_save_wwan_ifname_to_link(int link, char* ipaddr, int family);
...
*/
/*=========================================================================*/
void* netmgr_kif_handle_netd_event(void *arg);
...
/*=========================================================================*/
void
netmgr_unix_listener_init(int netmgr_restart);
