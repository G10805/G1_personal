...
)
{
  ...
  if ((unix_sock_fd = netmgr_socket(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0)) == NETMGR_UNIX_API_ERROR)
  {
...
poll_socket:
  ...
  *resp_bufflen = recv(unix_sock_fd, *resp_buffer, NETMGR_UNIX_MAX_MESSAGE_SIZE,
                       MSG_DONTWAIT);
