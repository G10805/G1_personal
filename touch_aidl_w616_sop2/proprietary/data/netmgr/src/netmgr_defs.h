...
#define NETMGR_XFRM_CRYPTO_CBS_AES   "cbc(aes)"
#define NETMGR_XFRM_CRYPTO_RFC_3686  "rfc3686(ctr(aes))"
#define NETMGR_XFRM_CRYPTO_CBS_DES   "cbc(des)"
#define NETMGR_XFRM_CRYPTO_CBS_3_DES "cbc(des3_ede)"
#define NETMGR_XFRM_AUTH_CBS_AES     "xcbc(aes)"
#define NETMGR_XFRM_AUTH_SHA1        "hmac(sha1)"
#define NETMGR_XFRM_AUTH_MD5         "hmac(md5)"
#define NETMGR_XFRM_AUTH_SHA256      "hmac(sha256)"
#define NETMGR_XFRM_AUTH_SHA512      "hmac(sha512)"
#define NETMGR_XFRM_AUTH_SHA384      "hmac(sha384)"
...
#define NETMGR_MAX_TRAFFIC_SELS   (4)
  ...
  typedef struct
  {
    netmgr_ip_address_t local_addr;
    netmgr_ip_address_t dest_addr;
    unsigned int spi_tx;
    unsigned int spi_rx;
    unsigned char proto;
    unsigned char mode;
    char crypto_algo[64];
    int tx_crypto_key_len;
    int rx_crypto_key_len;
    uint8_t tx_crypto_key[32];
    uint8_t rx_crypto_key[32];
    char auth_algo[64];
    int tx_auth_key_len;
    int rx_auth_key_len;
    int auth_trunc;
    uint8_t tx_auth_key[1024];
    uint8_t rx_auth_key[1024];
    int encap_valid;
    uint16_t modem_port;
    unsigned char valid;
  } netmgr_xfrm_state_id_t;
  ...
  typedef struct
  {
    ...
    netmgr_xfrm_state_id_t nl_sa_state_id;
...
typedef struct netmgr_address_set_s {
  ...
  netmgr_ip_address_t modem_addr;          /* Modem interface address      */
  netmgr_ip_address_t route_prefix;        /* Routing prefix associated with this addr */
  unsigned int        route_mask;          /* V6 prefix len */
  boolean             is_reconfig;         /* reconfig event from modem */
  int                 pdu_id;
  int                 pdu_status;
  unsigned int        ssc3_pdu_lifetime_valid;
  unsigned int        ssc3_pdu_lifetime;
  void               *sec_addr_info_ptr;
  unsigned int        addr_label;
...
#define NETMGR_ADDRSET_MASK_IPV6_CANDIDATE (0x04)
#define NETMGR_ADDRSET_MASK_IPV6_MULTIHOME (0x08)
...
#ifdef FEATURE_DATA_IWLAN
typedef struct netmgr_wlanaddr_info_s {
  boolean      is_info_valid;   /* Validity of wlan_if info */
  unsigned int valid_mask;      /* Equivalent to address mask for IP families*/
  char         wlan_ifname[NETMGR_IF_NAME_MAX_LEN]; /* Interface name */
} netmgr_wlanaddr_info_t;
#endif /* FEATURE_DATA_IWLAN */

typedef struct netmgr_address_info_s {
  unsigned int           valid_mask;
  netmgr_address_set_t   ipv4;
  netmgr_address_set_t   ipv6[NETMGR_ADDR_TYPE_MAX];
#ifdef FEATURE_DATA_IWLAN
  netmgr_wlanaddr_info_t wlan_info;
  netmgr_wlanaddr_info_t wwan_info;
#endif /* FEATURE_DATA_IWLAN */
