...
)
{
  ...
  else
  {
    ...
    netmgr_cb_runall(NETMGR_NETD_CB_TABLE, NETMGR_NETD_CB_RESTART_EV, NULL);
  }
...
)
{
    ...
    NETMGR_LOG_LOW("%s() %s %s: Attempting setForwardingBetweenInterfaces",
                   __func__, inputIfName.c_str(), outputIfName.c_str());
