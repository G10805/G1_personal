...
#----------------------------------------------------------------------------
   ...
   # HPS address is not aligned to a page. We need to ensure the offset and the address are aligned w.r.t Page.
   align_offset = hps_address%IMAGE_ALLIGMNET;
   hps_header = Elf32_Phdr(hps_address, gsi_fw_mem_size + st2.st_size, st3.st_size, st3.st_size, init_val = int('0x0',16), region_offset=current_offset + align_offset, make_reloc=1)
   ...
   add_padding_to_offset(output_file_name, align_offset)
...
def add_padding_to_page_align(output_file_name, written_size):
   ...
   return
#----------------------------------------------------------------------------
# Add padding to required offset
#----------------------------------------------------------------------------
def add_padding_to_offset(output_file_name, written_size):
