...
/*===========================================================================
  Copyright (c) 2013-2014, 2017, 2019-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
)
{
  ...
  snprintf(prop_name, DSI_CFGDB_STRVAL_MAX, "%s%s%s", "dsi_config.", target, ".oemprxy_enabled");
  ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &oemprxy_enabled, sizeof(oemprxy_enabled));
  if (ret != CFGDB_SUCCESS)
  {
    DSI_LOG_ERROR("Error reading property [%s] Err[%d]", prop_name, ret);
    oemprxy_enabled = DSI_FALSE;
  }
  dsi_config.oemprxy_enabled = oemprxy_enabled;
...
bail:
  if (0 == result)
  {
    DSI_LOG_INFO("dsi_config_load using configdb successful");
  }
  else
  {
    DSI_LOG_ERROR("dsi_config_load using configdb failed!!!");
  }
  configdb_release();
  return result;
}
...
void dsi_config_print(void)
{
  ...
  DSI_LOG_DEBUG("DSI configuration:");
