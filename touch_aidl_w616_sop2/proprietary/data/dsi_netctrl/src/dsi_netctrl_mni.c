...
/*===========================================================================
  ...
  Copyright (c) 2008-2017, 2019-2023 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
int dsi_mni_dsi_release_invoked = 0;

int dsi_mni_get_dsi_release_invoked(void)
{
  DSI_LOG_DEBUG("%s(): dsi_mni_dsi_release_invoked - %d",
                __func__, dsi_mni_dsi_release_invoked);
  return dsi_mni_dsi_release_invoked;
}

void dsi_mni_set_dsi_release_invoked(int val)
{
  dsi_mni_dsi_release_invoked = val ? DSI_MNI_SET_RELEASE_INVOKED :
                                      DSI_MNI_SET_RELEASE_CLEARED;
  DSI_LOG_DEBUG("%s(): dsi_mni_dsi_release_invoked - %d",
                __func__, dsi_mni_dsi_release_invoked);
}
...
)
{
  ...
  DSI_LOG_DEBUG( "dsi_mni_start: ENTRY" );
  ...
  do
  {
    ...
    hash = dsi_get_hash_from_str_ptr(st_hndl);

    if (!DSI_IS_ID_VALID(i) ||
        !DSI_IS_HNDL_VALID(st_hndl) || !hash)
    {
      DSI_LOG_ERROR("**programming err* invalid params received");
      break;
    }
    ...
    */
    ...
    {
      st_hndl->priv.call_tech = (wds_technology_name_enum_v01)st_hndl->priv.start_nw_params.ext_technology_preference;
    }
    else
    {
    ...
    if(qmi_ret < 0)
    {
      ...
      /* Call handle will be freed in dsi_mni_release_client() function
         as part of failure cleanup sequence */
    ...
    */
    ...
    if(st_hndl->priv.ip_version == DSI_IP_VERSION_4_6)
    {
      ...
      {
        DSI_SET_WDS_V4_TXN(st_hndl, qmi_ret);
        DSI_LOG_DEBUG("wds-txn-v4");
      }
      else if(ip_family == QDI_MODE_V6)
      {
        DSI_SET_WDS_V6_TXN(st_hndl, qmi_ret);
        DSI_LOG_DEBUG("wds-txn-v6");
      }
    }
  ...
  if (ret == DSI_SUCCESS)
  {
    DSI_LOG_DEBUG( "dsi_mni_start: EXIT with suc" );
  }
  else
  {
    ...
    {
      DSI_LOG_ERROR("dsi_link_release failed: iface=[%d] ",i);
    }
    else
    {
      DSI_LOG_DEBUG("dsi_link_release success: iface=[%d] ",i);
    }
    DSI_LOG_DEBUG( "dsi_mni_start: EXIT with err" );
  }
...
int dsi_mni_stop(int i, dsi_store_t * st_hndl)
{
  ...
  DSI_LOG_DEBUG( "dsi_mni_stop: ENTRY" );
  ...
  do
  {
    ...
    if (NULL == st_hndl || !DSI_IS_ID_VALID(i) || !hash)
    {
  ...
  if (ret == DSI_ERROR)
  {
    DSI_LOG_DEBUG( "dsi_mni_stop: EXIT with err");
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_mni_stop: EXIT with suc");
  }
...
int dsi_mni_abort_start(int i, dsi_store_t * st_hndl)
{
  ...
  DSI_LOG_DEBUG( "dsi_mni_abort_start: ENTRY" );
  ...
  {
    ret = DSI_ERROR;
    hash_value = dsi_get_hash_from_str_ptr(st_hndl);
    if (NULL == st_hndl || !DSI_IS_ID_VALID(i) || hash_value == 0)
    {
      DSI_LOG_ERROR("invalid params received");
      break;
    }
    ...
    if(QMI_NO_ERR == qmi_ret)
    {
      ...
      if (DSI_IP_VERSION_4_6 == st_hndl->priv.ip_version &&
          ...
          (dsi_is_iface_addr_valid(st_hndl->priv.dsi_iface_id, DSI_IP_FAMILY_V4) ||
           dsi_is_iface_addr_valid(st_hndl->priv.dsi_iface_id, DSI_IP_FAMILY_V6)))
      {
        DSI_LOG_DEBUG("successfully aborted START (Dual-IP), waiting for NET_PLATFORM_DOWN");
      }
      else
      {
        DSI_LOG_DEBUG("successfully aborted START, sending NET_NO_NET");
        ...
        {
          st_hndl->net_ev_cb((dsi_store_t *)hash_value,
                             st_hndl->user_data,
                             DSI_EVT_NET_NO_NET,
                             NULL);
        }
    ...
    DSI_LOG_DEBUG("ip_version [%d] interface [%d] v4_count %d v6_count %d",
                  st_hndl->priv.ip_version, i, DSI_GET_V4_COUNT(i), DSI_GET_V6_COUNT(i));
  ...
  if (ret == DSI_SUCCESS)
  {
    DSI_LOG_DEBUG( "dsi_mni_abort_start: EXIT with suc" );
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_mni_abort_start: EXIT with err" );
  }
...
int dsi_mni_look_up(dsi_store_t * st_hndl, int * pi)
{
  ...
  DSI_LOG_DEBUG( "dsi_mni_look_up: ENTRY" );

  if (NULL == pi || NULL == st_hndl)
  {
    DSI_LOG_ERROR("invalid parameters");
    return DSI_ERROR;
  }
  /* this do..while loop decides the overall return value
     set ret to ERROR at the beginning. set ret to SUCCESS
  ...
  do
  {
    ...
    DSI_LOG_VERBOSE("QCRIL is using QDP. No need to create IPv6 profiles");
    ...
    if (DSI_ERROR ==
        dsi_mm_iface_look_up(st_hndl,
                             ...
                             &dsd_technology))
    {
      DSI_LOG_ERROR("dsi_mm_iface_look_up failed");
      break;
    }
    ...
    case DSI_RADIO_TECH_UMTS:
      ...
      if (!(dsd_technology == DSI_PS_UMTS_IFACE ||
            ...
            dsd_technology == DSI_PS_CLAT_IFACE ))
      {
  ...
  {
    *pi = i;
    DSI_LOG_DEBUG( "dsi_mni_look_up: EXIT with suc" );
  }
  else
  {
    *pi = DSI_INVALID_IFACE;
    ...
    DSI_LOG_DEBUG( "dsi_mni_look_up: EXIT with err" );
  }
...
dsi_mni_release_client(int conn_id)
{
  ...
  DSI_LOG_DEBUG("dsi_mni_release_client: ENTRY");
  ...
  if( DSI_IS_WDS_HNDL_VALID(conn_id) )
  {
    ...
    {
      DSI_LOG_ERROR("QDI srvc release failed");
    }
  ...
  if( DSI_IS_QOS_HNDL_VALID(conn_id) )
  {
    ...
    {
      DSI_LOG_ERROR("qos srvc release failed");
    }

    DSI_SET_QOS_HNDL(conn_id, DSI_INVALID_QOS_HNDL);
  }

  DSI_LOG_DEBUG("dsi_mni_release_client: EXIT");
}
/*===========================================================================
...
dsi_mni_cleanup(boolean from_ssr)
{
  ...
  int i = 0;
  DSI_LOG_DEBUG("dsi_mni_cleanup: ENTRY");
  ...
  if(!from_ssr)
  {
    ...
    {
      DSI_LOG_ERROR("QDI release failed");
    }
  }
  else
  {
    DSI_LOG_DEBUG("qdi cmdq flush");
    if ( QDI_SUCCESS != qdi_flush_cmdq())
    {
      DSI_LOG_ERROR("QDI flush failed");
    }
  }
  ...
  DSI_LOG_DEBUG("dsi_mni_cleanup: EXIT");
}
...
int dsi_mni_init_client(int conn_id)
{
  ...
  int subs_id;
  DSI_LOG_DEBUG( "dsi_mni_init_client: ENTRY" );
  ...
  do
  {
    ...
    {
      DSI_LOG_ERROR("dsi_mni_init_client: invalid iface");
      break;
    }
    ...
    {
      DSI_LOG_ERROR(
        "dsi_mni_init_clients: qdi_wds_srvc_init_client failed");
    ...
    if( DSI_TRUE == dsi_config.qos_enable )
    {
      ...
      if(DSI_SUCCESS != dsi_qmi_qos_init_qmi_client(DSI_GET_DEV_STR(conn_id),
                                                    ...
                                                    &qos_qmi_hndl))
      {
        DSI_LOG_ERROR("dsi_qos_init: qmi_qos_srvc_init_client failed");
...
*/
/*=========================================================================*/
int dsi_mni_init(boolean from_ssr, int retry_qmi)
{
  int ret = DSI_ERROR;
  ...
  DSI_LOG_DEBUG( "dsi_mni_init: ENTRY" );
  ...
  do
  {
    ...
    for (modem=0; modem < DSI_MAX_MODEMS; modem++)
    {
      if(is_default_handle_inited[modem] == DSI_FALSE)
      {
        ...
        DSI_LOG_DEBUG("%s(): Initing qmi wds client with iface %s", __func__, DSI_GET_WDS_STR(default_iface));

        /* why does rmnet_sdio0 even come up here and fail everytime here?
         * why even install rmnet_sdio devices into the dsi tables
         * note that dsi_qmi_map_dev_conn_instance is done within dsi_qmi_wds_init_qmi_client as well
         * this check is to remove rmnet_sdio0 from the loop
         * at some point, rmnet_sdio0 and other similar deprecated devices need to be removed
         */
        if (DSI_SUCCESS !=(dsi_qmi_map_dev_conn_instance(DSI_GET_WDS_STR(default_iface), &qcci_conn_id)))
        {
          DSI_LOG_ERROR("%s(): bad device ID %s received", __func__, DSI_GET_WDS_STR(default_iface));
          continue;
        }

        /* now that rmnet_sdiox is eliminated above, qmi can only fail if modem is out of service
         * retry as required here
         */
qmi_wds_init:
        if (DSI_SUCCESS != dsi_qmi_wds_init_qmi_client(DSI_GET_WDS_STR(default_iface), &wds_hndl))
        {
          DSI_LOG_ERROR("%s(): dsi_qmi_wds_init_qmi_client failed", __func__);
          /* retry if dsi_init_ex is invoked
           * do not have to retry is dsi_init is invoked
           * do not have to retry if dsi_init_ex is invoked followed by dsi_release
           */
          if (retry_qmi && !dsi_mni_get_dsi_release_invoked())
          {
            DSI_LOG_DEBUG("%s(): Retry initing qmi wds client with iface %s",
                          __func__, DSI_GET_WDS_STR(default_iface));
            goto qmi_wds_init;
          }
          continue;
        }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_DEBUG( "dsi_mni_init: EXIT with success" );
  ...
  else
  {
    ...
    DSI_LOG_DEBUG( "dsi_mni_init: EXIT with error" );
  }
...
/*===========================================================================
  FUNCTION:  dsi_qmi_oemproxy_wds_ind_cb
===========================================================================*/
/*!
    @brief
    callback function registered for oemproxy wds indications

*/
/*=========================================================================*/
void dsi_qmi_oemproxy_wds_ind_cb
(
...
)
{
  ...
  wds_iwlan_s2b_proxy_srvc_status_ind_msg_v01 s2b_proxy_buf;
  ...
  qmi_err = qmi_client_message_decode(user_handle,
                                      ...
                                      ind_buf_len,
                                      &s2b_proxy_buf,
                                      sizeof(wds_iwlan_s2b_proxy_srvc_status_ind_msg_v01));
  if (QMI_NO_ERR != qmi_err)
  {
    DSI_LOG_ERROR("%s(): failed to decode message!", __func__);
    return;
  }
  ...
  do
  {
    ...
    memset(cmd_buf, 0x0, sizeof(dsi_netctrl_cb_cmd_t));

    /* From QMI decode set all the fields to pass to DSI cmdq */
    cmd_buf->cmd_data.data_union.wds_ext_ind.wds_hndl = user_handle;
    cmd_buf->cmd_data.data_union.wds_ext_ind.ind_data.session_hndl = s2b_proxy_buf.sess_handle;
    cmd_buf->cmd_data.data_union.wds_ext_ind.ind_data.status = s2b_proxy_buf.conn_status;
    cmd_buf->cmd_data.data_union.wds_ext_ind.ind_data.ip_family = s2b_proxy_buf.ip_family;

    if(s2b_proxy_buf.verbose_call_end_reason_valid  == TRUE)
      cmd_buf->cmd_data.data_union.wds_ext_ind.ind_data.verbose_call_end_reason = s2b_proxy_buf.verbose_call_end_reason;

    DSI_LOG_DEBUG("%s(): sess_handle: %d", __func__, s2b_proxy_buf.sess_handle);
    DSI_LOG_DEBUG("%s(): conn_status: %d", __func__, s2b_proxy_buf.conn_status);
    DSI_LOG_DEBUG("%s(): ip_family: %d", __func__, s2b_proxy_buf.ip_family);
    DSI_LOG_DEBUG("%s(): verbose_call_end_reason: %d", __func__, s2b_proxy_buf.verbose_call_end_reason);

    /* DSI cmdq type */
    cmd_buf->cmd_data.type = DSI_NETCTRL_CB_CMD_QMI_WDS_EXT_IND;
    ...
    {
      DSI_LOG_ERROR("%s(): Failed to post on DSI cmdq!", __func__);
      break;
    }
  ...
  if(ret != DSI_SUCCESS)
  {
    if(cmd_buf != NULL) ds_free(cmd_buf);
  }
}

/*===========================================================================
  FUNCTION:  dsi_oemproxy_common_ind
===========================================================================*/
/*!
    @brief
    Common function to process both netmgr iface up and wds indication
...
{
  static int wds_enabled = -1;
  static int netmgr_enabled = -1;
  int count = 0;
  uint64_t hash_value = 0;
  dsi_net_evt_t event = DSI_EVT_INVALID;
  dsi_store_t *st = NULL;

  DSI_LOG_DEBUG("Proccessing Indication %d for iface %d", ind, iface_id);

  /* Check for a valid handle with a filled wlan_iface_name */
  for(count = 0; count < DSI_MAX_DATA_CALLS; count++)
  {
    st = (dsi_store_t *)dsi_store_table[count].dsi_store_ptr;
    if (st != NULL && st->priv.oemprxy_data.oemprxy_iface_id == iface_id) break;
  }

  if(st == NULL || st->priv.oemprxy_data.oemprxy_iface_id != iface_id)
  {
    DSI_LOG_ERROR("%s(): No valid OEM proxy handle found!", __func__);
    return;
  }
  ...
  switch (ind)
  {
  case DSI_EVT_WDS_EXT_IS_CONN:
    DSI_LOG_DEBUG("%s(): wds enable", __func__);
    if(netmgr_enabled == FALSE) netmgr_enabled = -1;
    wds_enabled = TRUE;
    break;
  case DSI_EVT_WDS_EXT_NO_NET:
    DSI_LOG_DEBUG("%s(): wds disable", __func__);
    if(netmgr_enabled == TRUE) netmgr_enabled = -1;
    wds_enabled = FALSE;
    break;
  case DSI_EVT_OEMPRXY_IS_CONN:
    DSI_LOG_DEBUG("%s(): oemprxy enable", __func__);
    if(wds_enabled == FALSE) wds_enabled = -1;
    netmgr_enabled = TRUE;
    break;
  case DSI_EVT_OEMPRXY_NO_NET:
    DSI_LOG_DEBUG("%s(): oemprxy disable", __func__);
    if(wds_enabled == TRUE) wds_enabled = -1;
    netmgr_enabled = FALSE;
    break;

  default:
    DSI_LOG_ERROR("%s(): Unknown indication: %d! ", __func__, ind);
    break;
  }

  /* Both need to be in a valid state in order to send client an indication */
  if(wds_enabled == -1 || netmgr_enabled == -1)
  {
    DSI_UNLOCK_MUTEX(&(st->priv.mutex));
    return;
  }

  if(wds_enabled == TRUE && netmgr_enabled == TRUE)
  {
    event = DSI_EVT_NET_IS_CONN;
  }
  else if(wds_enabled == FALSE && netmgr_enabled == FALSE)
  {
    event = DSI_EVT_NET_NO_NET;
  }
  else
  {
    DSI_LOG_ERROR("%s(): Invalid state!", __func__);
    wds_enabled = -1;
    netmgr_enabled = -1;
  }
  ...
  if (NULL != st->net_ev_cb && hash_value > 0)
  {
    DSI_LOG_DEBUG("notifying event [%d] on handle [%p] hash_value [%p]",
                   event, st, hash_value);
    st->net_ev_cb((dsi_hndl_t *)hash_value,
                  st->user_data,
                  event,
                  NULL);
  }

  /* Reset state to avoid calling callback twice */
  wds_enabled = -1;
  netmgr_enabled = -1;
}

/*===========================================================================
  FUNCTION:  dsi_mni_init_extended
===========================================================================*/
/*!
    @brief
    This function initializes the QMI WDS service and corresponding
...
)
{
  ...
  wds_ip_family_enum_v01 ip_family = -1;

  /* Check and Set IP Family type */
  if(st_hndl->priv.ip_version == DSI_IP_VERSION_4)
    ip_family = WDS_IP_FAMILY_IPV4_V01;
  else if(st_hndl->priv.ip_version == DSI_IP_VERSION_6)
    ip_family = WDS_IP_FAMILY_IPV6_V01;
  else
  {
    DSI_LOG_ERROR("%s(): Invalid IP type", __func__);
    return DSI_ERROR;
  }

  /* If a valid wds handle exists exit early */
  if(st_hndl->priv.oemprxy_data.qmi_wds_hndl != DSI_INVALID_WDS_HNDL)
  {
    DSI_LOG_DEBUG("%s(): Using existing WDS handle!", __func__);
    return DSI_SUCCESS;
  }

  /* Create DSI client instance */
  rc = qmi_client_init_instance(wds_get_service_object_v01(),
                                QMI_CLIENT_INSTANCE_ANY,
                                dsi_qmi_oemproxy_wds_ind_cb,
                                NULL,
                                &os_params,
                                DSI_QMI_TIMEOUT,
                                &st_hndl->priv.oemprxy_data.qmi_wds_hndl);
  if(rc != QMI_NO_ERR)
  {
    DSI_LOG_ERROR("%s(): Failed to create dsi client", __func__);
    return DSI_ERROR;
  }

  /* Bind subs */
  if (DSI_SUCCESS != dsi_qmi_wds_bind_subscription(st_hndl->priv.oemprxy_data.qmi_wds_hndl,
                                                   st_hndl->priv.oemprxy_data.subs_id))
  {
    DSI_LOG_ERROR("%s(): subs binding failed",__func__);
    return DSI_ERROR;
  }

  /* Set IP preference */
  if(DSI_SUCCESS != dsi_qmi_wds_set_client_ip_pref(st_hndl->priv.oemprxy_data.qmi_wds_hndl, ip_family))
  {
    DSI_LOG_ERROR("%s(): dsi_qmi_wds_set_client_ip_pref failed",__func__);
    return DSI_ERROR;
  }

  /* Mark the indications to register for */
  memset(&ind_type, 0, sizeof(ind_type));
  ind_type.report_iwlan_s2b_proxy_srvc_status_valid = TRUE;
  ind_type.report_iwlan_s2b_proxy_srvc_status = TRUE;

  /* V4 Indication */
  if(DSI_SUCCESS != dsi_qmi_wds_indication_register(st_hndl->priv.oemprxy_data.qmi_wds_hndl,
                                                    &ind_type))
  {
    DSI_LOG_ERROR("%s(): wds indication register failed!",__func__);
    return DSI_ERROR;
  }
...
)
{
  ...
  DSI_LOG_DEBUG("dsi_mni_reg_unreg_handoff_ind: ENTRY");
  ...
  do
  {
    ...
    default:
      {
        DSI_LOG_ERROR("dsi_qmi_wds_indication_register: Unknown IP type!");
        reti = DSI_ERROR;
      }
      break;
    }
  ...
  if (ret == DSI_ERROR)
  {
    DSI_LOG_DEBUG( "dsi_mni_reg_unreg_handoff_ind: EXIT with err");
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_mni_reg_unreg_handoff_ind: EXIT with suc");
  }
...
int dsi_mni_register_embms_ind(int i, dsi_store_t * st_hndl)
{
  ...
  DSI_LOG_DEBUG( "dsi_mni_register_embms_ind: ENTRY" );
  ...
  if (ret == DSI_ERROR)
  {
    DSI_LOG_DEBUG( "dsi_mni_register_embms_ind: EXIT with err");
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_mni_register_embms_ind: EXIT with suc");
  }
...
int dsi_mni_embms_tmgi_activate(int i, dsi_store_t * st_hndl)
{
  ...
  DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_activate: ENTRY" );
  ...
  if (ret == DSI_ERROR)
  {
    DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_activate: EXIT with err");
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_activate: EXIT with suc");
  }
...
int dsi_mni_embms_tmgi_deactivate(int i, dsi_store_t * st_hndl)
{
  ...
  DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_deactivate: ENTRY" );
  ...
  if (ret == DSI_ERROR)
  {
    DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_deactivate: EXIT with err");
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_deactivate: EXIT with suc");
  }
...
int dsi_mni_embms_tmgi_activate_deactivate(int i, dsi_store_t * st_hndl)
{
  ...
  DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_activate_deactivate: ENTRY" );
  ...
  if (ret == DSI_ERROR)
  {
    DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_activate_deactivate: EXIT with err");
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_activate_deactivate: EXIT with suc");
  }
...
int dsi_mni_embms_tmgi_list_query(int i, dsi_store_t * st_hndl)
{
  ...
  DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_list_query: ENTRY" );
  ...
  if (ret == DSI_ERROR)
  {
    DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_list_query: EXIT with err");
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_list_query: EXIT with suc");
  }
...
int dsi_mni_embms_tmgi_content_desc_update(int i, dsi_store_t * st_hndl)
{
  ...
  DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_content_desc_update: ENTRY" );
  ...
  if (ret == DSI_ERROR)
  {
    DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_content_desc_update: EXIT with err");
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_content_desc_update: EXIT with suc");
  }
...
int dsi_mni_embms_tmgi_svc_interest_list(int i, dsi_store_t * st_hndl)
{
  ...
  DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_svc_interest_list: ENTRY" );
  ...
  if (ret == DSI_ERROR)
  {
    DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_svc_interest_list: EXIT with err");
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_mni_embms_tmgi_svc_interest_list: EXIT with suc");
  }
...
/*===========================================================================
  FUNCTION:  dsi_mni_check_matching_call
===========================================================================*/
/*!
    @brief
    Check for matching call on the modem

    @return
    DSI_ERROR if no matching call exists
    DSI_SUCCESS if there is a matching call
*/
/*=========================================================================*/
int dsi_mni_check_matching_call
(
  dsi_store_t               *st,
...
)
{
  ...
  if (!st || !device_name || device_name_len == 0 || !call_params)
  {
    DSI_LOG_ERROR("%s(): invalid param!", __func__);
    return ret;
  }

  do
  {
    rc = dsi_mm_check_matching_call(st, call_params, device_name, device_name_len);
    if (DSI_SUCCESS != rc)
    {
