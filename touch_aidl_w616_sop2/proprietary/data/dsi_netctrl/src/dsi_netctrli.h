...
/*===========================================================================
  ...
  Copyright (c) 2008-2017, 2019-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
static char dsi_evt_str[DSI_EVT_MAX + 1][DSI_EVT_STR_MAX_LENGTH] =
  {
    ...
    "TMGI_LIST_CHANGED",
    "SAI_LIST_CHANGED",
    "CONTENT_DESC_CONTROL",
    "TMGI_SERVICE_INTERESTED",

    "NET_HANDOFF",
    "WDS_CONNECTED",
    "NET_NEWMTU",                 /**< MTU update. */

    "PDU_AVAILABLE",              /**< Secondary PDU Available */
    "PDU_PROMOTION",              /**< Secondary PDU becomes Primary */
    "PDU_RELEASE",                /**< Secondary PDU Released */
    "PDU_RECONFIGURED",           /**< PDU Address reconfigured */
    "EXTENDED_IP_CHANGE",         /**< Change in EXTEND_IP_IND */
...
#define DSI_ASSERT( cond, msg ) \
  if ( !( cond ) ) \
  { \
    DSI_LOG_FATAL( "*************ASSERTION FAILED (soft)***************" ); \
...
#define DSI_ASSERT_H( cond, msg ) \
  if ( !( cond ) ) \
  { \
    DSI_LOG_FATAL( "*************ASSERTION FAILED (hard)***************" ); \
    DSI_LOG_FATAL( "File: %s, Line: %d, [%s]", __FILE__, __LINE__, msg );         \
    DSI_LOG_FATAL( "***************************************************" ); \
    assert(0); \
  }
...
#define DSI_PS_CLAT_IFACE 0x8881
...
/* wrapper struct around the addresses owned by a PDU.
 * Helps prevent continuually adding dimensions to the array in the main
 * dsi_iface_t struct. Also prevents wasting as much space since some
 * combinations of family + pdu + address type are not possible.
 */
typedef struct {
  dsi_addr_info_ex_t ipv4;
  dsi_addr_info_ex_t ipv6[DSI_NUM_ADDR_TYPES];
} dsi_pdu_addr_info_t;
...
typedef struct dsi_iface_s
{
  ...
  */
  ...
  int ipv6_profile_index; /* temporary: save ipv6 profile
                          ...
                          * is brought down */
  dsi_pdu_addr_info_t pdu_addr_info[DSI_NUM_PDUS]; /* address info structure */

  unsigned int     mtu; /* MTU of the interface */
  unsigned int     mtu_v4; /* MTU of the interface's v4 call */
  unsigned int     mtu_v6; /* MTU of the interface's v6 call */
...
#define DSI_GET_MTU_V4(i) \
  ( dsi_iface_tbl[i].mtu_v4 )

#define DSI_SET_MTU_V4(i,x) \
  ( dsi_iface_tbl[i].mtu_v4 = x )

#define DSI_GET_MTU_V6(i) \
  ( dsi_iface_tbl[i].mtu_v6 )

#define DSI_SET_MTU_V6(i,x) \
  ( dsi_iface_tbl[i].mtu_v6 = x )

#define DSI_SET_SUBS_ID(i,x) \
  ( dsi_iface_tbl[i].dsi_modem_subs_id = x ); \
  DSI_LOG_DEBUG("Set subs_id [%d] on iface index [%d]", x, i)
...
typedef enum
{
  ...
  SUSPENDED = 0x03,
  AUTHENTICATING = 0x04
} wds_iwlan_s2b_proxy_ind_conn_status_t;

typedef struct
{
  uint32_t session_hndl;
  wds_iwlan_s2b_proxy_ind_conn_status_t status;
  int ip_family;
  wds_call_end_reason_enum_v01 call_end_reason;
  wds_verbose_call_end_reason_type_v01 verbose_call_end_reason;
} wds_iwlan_s2b_proxy_ind_t;

typedef struct
{
  qmi_client_type wds_hndl;
  int ind_id;
  wds_iwlan_s2b_proxy_ind_t ind_data;
} dsi_netctrl_cb_wds_extended_ind_data_t;

typedef struct
{
  qmi_client_type  qmi_wds_hndl; /* WDS handle */
  int oemprxy_iface_id; /* Cached id given by netmgr */
  int subs_id; /* Subscription ID */
  uint32_t sess_handle; /* Session ID given by QMI*/
  char wlan_iface_name[NETMGR_IF_NAME_MAX_LEN];
  wds_enable_iwlan_s2b_proxy_data_path_req_msg_v01 wds_req;
}dsi_oemprxy_internal_t;
...
typedef struct
{
  ...
  /* oemprxy parameters */
  dsi_oemprxy_internal_t oemprxy_data;
} dsi_priv_t;
...
typedef struct bearer_tech_string_s {
  dsi_data_bearer_tech_t value;
  const char* str;
} bearer_tech_string_t;
...
/* Constant representing call end reasons generated internally */
#define DSI_CE_REASON_INTERNAL (-2)
...
/* Version of DSI_IS_HNDL_VALID usable through netmgrd restarts */
#define DSI_IS_HNDL_VALID_R( hndl ) \
...
/* Version of DSI_IS_ID_VALID usable through netmgrd restarts */
#define DSI_IS_ID_VALID_R(id) ((id < DSI_MAX_IFACES) && (id >= 0))
...
typedef int (*dsi_mni_init_fptr)(boolean from_ssr, int retry_qmi);
...
typedef int (*dsi_mni_matching_call_check_fptr)(dsi_store_t *st,
...
typedef struct dsi_mni_vtable_s
{
  ...
  dsi_mni_matching_call_check_fptr             mni_matching_call_check_f;
} dsi_mni_vtable_t;

extern dsi_mni_vtable_t dsi_mni_vtbl;

/* dsi_inited is no longer a boolean, but C doesn't have a real boolean type
 * boolean is generally defined as uint8_t anyway
 * This new state is to prevent any concurrent execution of dsi_init_ex which
 * is now an asynchronous API
 */
#define DSI_IN_PROGRESS (2)

extern uint8_t dsi_inited;
...
extern netmgr_restart_state_t netmgr_state;
extern dsi_ssc_mode_t dsi_ssc_mode;
...
/*===========================================================================
  FUNCTION:  dsi_get_str_ptr_from_hash
===========================================================================*/
/*!
    @brief
    Perform translation of hash_value to dsi_store_ptr

    @param
    hash - unsigned int
    st_hndl - double ptr to store dsi_store_ptr

    @return
    dsi_store_ptr - if SUCCESS
    NULL - if ERROR
*/
/*=========================================================================*/
...
extern int dsi_prepare_call_match_lookup_params
(
  dsi_matching_call_check_t             *call_params,
  dsi_qmi_wds_route_look_up_params_type *pparams
);
