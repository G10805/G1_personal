...
/*Since we expect netmgr to be inited already, if there is any change netmgr
will notify us*/
netmgr_restart_state_t netmgr_state = NETMGR_POST_RESTART_STATE_INIT_DONE;
...
)
{
  ...
  if (dsi_inited != DSI_TRUE)
  {
    ...
    if (DSI_IS_HNDL_VALID(st)) {
        ...
        st->priv.ce_reason[DSI_IP_FAMILY_V6].reason_code = DSI_CE_REASON_CODE_NETMGR_RESTART_ONGOING;
    }
  }
...
)
{
  ...
  if ( (netmgr_client_send_user_cmd(NETMGR_USER_CMD_ENABLE_PORT_FORWARDING,
                                  ...
                                  resp_info,
                                  NETMGR_UNIX_MESSAGE_RECV_TIMEOUT)
    != DSI_SUCCESS) || (resp_info->cmd_data.txn.txn_status == NETMGR_USER_CMD_STATUS_FAILED))
  {
...
)
{
  ...
  *forwarding_status = DSI_PORT_FORWARDING_INVALID;
  ...
  /* store command data in dsi store handle for validation later */
  memcpy(&st_hndl->priv.user_cmd_data, cmd_data, sizeof(netmgr_user_cmd_data_t));
...
)
{
  ...
  if (!pdu_id){
    cmd_data->cmd_id = NETMGR_USER_CMD_QUERY_LINK_MTU;
    cmd_data->pdu_id = PRIMARY_PDU;
  }
  else
  {
    cmd_data->cmd_id = NETMGR_USER_CMD_QUERY_LINK_MTU_PDU_ID;
  ...
  DSI_LOG_DEBUG("%s(): Sending user cmd NETMGR_USER_CMD_QUERY_LINK_MTU_VX"
                " for pid[%d] using txn_id[%d]",
                __func__, cmd_data->txn.pid, cmd_data->txn.txn_id);
  ...
  if ( (netmgr_client_send_user_cmd(cmd_data->cmd_id,
                                  cmd_data,
                                  resp_info,
                                  NETMGR_UNIX_MESSAGE_RECV_TIMEOUT)
    != DSI_SUCCESS) || (resp_info->cmd_data.txn.txn_status == NETMGR_USER_CMD_STATUS_FAILED))
...
)
{
  ...
  /* update txn info in the cmd_data */
  DSI_NETMGR_GET_NEXT_TXN_ID(cmd_data->txn);
  cmd_data->cmd_id = NETMGR_USER_CMD_QUERY_LINK_IFACE_MTU_ONLY;
  cmd_data->link = st->priv.dsi_iface_id;
  ...
  /* Clear mtu for query as we need to get this information from netmgr */
  memset(&st->priv.user_cmd_data.data.iface_mtu, 0x0, sizeof(unsigned int));

  DSI_LOG_DEBUG("%s(): Sending user cmd "
                "NETMGR_USER_CMD_QUERY_LINK_IFACE_MTU_ONLY for pid[%d] "
                "using txn_id[%d]",
                __func__, cmd_data->txn.pid, cmd_data->txn.txn_id);
  ...
  *iface_mtu = resp_info->cmd_data.data.iface_mtu;
  st->priv.user_cmd_data.txn.txn_status = DSI_SUCCESS;

  DSI_LOG_DEBUG("%s(): link = %u, iface MTU = %u",
                     __func__, cmd_data->link, *iface_mtu);
...
int dsi_oemproxy_query_netmgr(dsi_store_t * st_hndl)
{
  ...
  /* Set wlan iface and netmgr CMD ID*/
  memcpy(&usr_cmd.netmgr_oemprxy_req_params.wlan_iface_name,
         st_hndl->priv.oemprxy_data.wlan_iface_name, IFACE_NAME_LEN);
  usr_cmd.cmd_id = NETMGR_USER_CMD_QUERY_OEMPROXY_DATAPATH_IFACE;

  result = netmgr_client_send_user_cmd(NETMGR_USER_CMD_QUERY_OEMPROXY_DATAPATH_IFACE,
                                        &usr_cmd,
                                        &resp_info,
                                        NETMGR_UNIX_MESSAGE_RECV_TIMEOUT);

  /* Cache iface ID from netmgr */
  if(result != DSI_ERROR && resp_info.cmd_data.txn.txn_status == NETMGR_USER_CMD_STATUS_SUCCESS)
  {
    st_hndl->priv.oemprxy_data.oemprxy_iface_id =
      resp_info.cmd_data.data.netmgr_oemprxy_resp_params.oemprxy_iface_id;
