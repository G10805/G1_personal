...
dsi_mni_vtable_t dsi_mni_vtbl =
{
  ...
  .mni_matching_call_check_f = dsi_mni_check_matching_call,
};
...
bearer_tech_string_t bearer_tech_string_tbl[] =
{
  {DSI_DATA_BEARER_TECH_UNKNOWN       , "DATA BEARER TECH UNKNOWN"},

  /* 3GPP2 related data bearer technologies */
  {DSI_DATA_BEARER_TECH_CDMA_1X       , "DATA BEARER TECH CDMA 1X"},
  {DSI_DATA_BEARER_TECH_EVDO_REV0     , "DATA BEARER TECH CDMA REV0"},
  {DSI_DATA_BEARER_TECH_EVDO_REVA     , "DATA BEARER TECH CDMA Rev A"},
  {DSI_DATA_BEARER_TECH_EVDO_REVB     , "DATA BEARER TECH CDMA Rev B"},
  {DSI_DATA_BEARER_TECH_EHRPD         , "DATA BEARER TECH EHRPD"},
  {DSI_DATA_BEARER_TECH_FMC           , "DATA BEARER TECH FIXED MOBILE CONVERGENCE"},
  {DSI_DATA_BEARER_TECH_HRPD          , "DATA BEARER TECH HRPD"},
  {DSI_DATA_BEARER_TECH_3GPP2_WLAN    , "DATA BEARER TECH 3GPP2 IWLAN"},

   /* 3GPP related data bearer technologies */
  {DSI_DATA_BEARER_TECH_WCDMA         , "DATA BEARER TECH WCDMA"},
  {DSI_DATA_BEARER_TECH_GPRS          , "DATA BEARER TECH GPRS"},
  {DSI_DATA_BEARER_TECH_HSDPA         , "DATA BEARER TECH HSDPA"},
  {DSI_DATA_BEARER_TECH_HSUPA         , "DATA BEARER TECH HSUPA"},
  {DSI_DATA_BEARER_TECH_EDGE          , "DATA BEARER TECH EDGE"},
  {DSI_DATA_BEARER_TECH_LTE           , "DATA BEARER TECH LTE"},
  {DSI_DATA_BEARER_TECH_HSDPA_PLUS    , "DATA BEARER TECH HSDPA+"},
  {DSI_DATA_BEARER_TECH_DC_HSDPA_PLUS , "DATA BEARER TECH DC HSDPA+"},
  {DSI_DATA_BEARER_TECH_HSPA          , "DATA BEARER TECH HSPA"},
  {DSI_DATA_BEARER_TECH_64_QAM        , "DATA BEARER TECH 64 QAM"},
  {DSI_DATA_BEARER_TECH_TDSCDMA       , "DATA BEARER TECH TD-SCDMA"},
  {DSI_DATA_BEARER_TECH_GSM           , "DATA BEARER TECH GSM"},
  {DSI_DATA_BEARER_TECH_3GPP_WLAN     , "DATA BEARER TECH 3GPP IWLAN"},
  {DSI_DATA_BEARER_TECH_5G            , "DATA BEARER TECH 5G"},
  {DSI_DATA_BEARER_TECH_MAX           , "DATA BEARER TECH MAX"}
};
...
/*===========================================================================
  FUNCTION:  dsi_is_iface_addr_valid_pdu
===========================================================================*/
/*!
    @brief
    Check if the interface address on a fiven interface, IP family, and PDU
    index is valid

    @param
    iface_id - the DSI interface id
    ipf - the IP family
    pdu_cnf - the PDU index

    @return
    TRUE if the address is marked as valid, FALSE otherwise
*/
/*=========================================================================*/
boolean dsi_is_iface_addr_valid_pdu
(
  int iface_id,
  dsi_ip_family_t ipf,
  unsigned int pdu_cnf,
  dsi_addr_type addr_type
)
{
  dsi_addr_info_ex_t *addr_info = dsi_get_addrinfo_ex(iface_id, ipf, pdu_cnf,
                                                      addr_type);

  return DSI_TRUE == addr_info->iface_addr_s.valid_addr;
}

/*===========================================================================
  FUNCTION:  dsi_is_iface_addr_valid
===========================================================================*/
/*!
    @brief
    Check if the interface address on a given interface and IP family is
    valid
...
*/
/*=========================================================================*/
boolean dsi_is_iface_addr_valid
(
  int iface_id,
  dsi_ip_family_t ipf
)
{
  return dsi_is_iface_addr_valid_pdu(iface_id, ipf, PRIMARY_PDU,
                                     DSI_ADDR_TYPE_DEFAULT);
}

/*===========================================================================
  FUNCTION:  dsi_get_addrinfo_ex
===========================================================================*/
/*!
    @brief
    Retrieve the dsi_addr_info_ex_t for an interface of a specific family on
    a specifc PDU of the given type

    @param
    iface_id - the DSI interface id
    ipf - the IP family
    pdu_cnf - the PDU index
    addr_type - the ADDRESS_TYPE

    @return
    A pointer to the dsi_addr_info_ex_t for the given interface, IP family,
    PDU, and address type
*/
/*=========================================================================*/
dsi_addr_info_ex_t *dsi_get_addrinfo_ex
(
  int iface_id,
  dsi_ip_family_t ipf,
  unsigned int pdu_cnf,
...
{
  dsi_pdu_addr_info_t *pdu_addr;

  pdu_addr = &dsi_iface_tbl[iface_id].pdu_addr_info[pdu_cnf];
  if (DSI_IP_FAMILY_V4 == ipf)
  {
    return &pdu_addr->ipv4;
  }

  return &pdu_addr->ipv6[addr_type];
}

/*===========================================================================
  FUNCTION:  dsi_get_addrinfo
===========================================================================*/
/*!
    @brief
    Retrieve the dsi_addr_info_t for an interface of a specific family
    ...
    @return
    A pointer to the dsi_addr_info_t for the given interface and IP family
*/
/*=========================================================================*/
dsi_addr_info_t *dsi_get_addrinfo
(
  int iface_id,
  dsi_ip_family_t ipf
)
{
  return (dsi_addr_info_t *)dsi_get_addrinfo_ex(iface_id, ipf, PRIMARY_PDU,
                                                DSI_ADDR_TYPE_DEFAULT);
}

/*===========================================================================
  FUNCTION:  dsi_convert_netmgr_addr_type
===========================================================================*/
/*!
    @brief
    Converts the netmgr_addr_type enum into the equivalent dsi_addr_type

    @param
    addr_type - the netmgr_addr_type to convert

    @return
    An equivalent dsi_addr_type value
*/
/*=========================================================================*/
dsi_addr_type dsi_convert_netmgr_addr_type
(
  netmgr_addr_type addr_type
)
{
  dsi_addr_type dsi_type = DSI_ADDR_TYPE_INVALID;

  switch (addr_type)
  {
    case NETMGR_ADDR_TYPE_DEFAULT:
      dsi_type = DSI_ADDR_TYPE_DEFAULT;
      break;
    case NETMGR_ADDR_TYPE_CANDIDATE:
      dsi_type = DSI_ADDR_TYPE_CANDIDATE;
      break;
    case NETMGR_ADDR_TYPE_MULTIHOME:
      dsi_type = DSI_ADDR_TYPE_MULTIHOME;
      break;
    default:
      /* sudo gcc --silence-dumb-warning --i-mean-it */
      break;
  }
...
/*===========================================================================
  FUNCTION:  dsi_get_random_hash
===========================================================================*/
/*!
    @brief
    Wrapper to rand(). Generates a random value in range [1 to RAND_MAX]

    @param
    None

    @return
    random unsigned int in range [1 to RAND_MAX]
*/
/*=========================================================================*/
unsigned int dsi_get_random_hash
(
  void
)
{
  unsigned int temp_hash=0;
  temp_hash = (unsigned int)( rand() % RAND_MAX + 1);
  DSI_LOG_DEBUG(" Final Handle: temp_hash=%d ", temp_hash);
  return temp_hash;
}

/*===========================================================================
  FUNCTION:  dsi_get_hash_from_str_ptr
===========================================================================*/
/*!
    @brief
    Returns hash_value corresponding to dsi_store_ptr entry

    @param
    st_hndl - dsi_store_ptr for matching entry

    @return
    hash_value - if SUCCESS
    0 - if ERROR
*/
/*=========================================================================*/
void  dsi_get_str_ptr_from_hash
(
  uint64_t hash,
  dsi_store_t ** st_hndl
)
  ...
  if ( hash > 0 )
  {
    for (i = 0; i < DSI_MAX_DATA_CALLS; i++)
    {
      if (dsi_store_table[i].is_valid &&
          dsi_store_table[i].hash_value == hash &&
          dsi_store_table[i].dsi_store_ptr != NULL)
      {
        *st_hndl = dsi_store_table[i].dsi_store_ptr;
        DSI_LOG_DEBUG("dsi_st_hndl=%p hash=%p",
                      *st_hndl, dsi_store_table[i].hash_value);
        break;
      }
    }
    if (DSI_MAX_DATA_CALLS == i)
    {
      DSI_LOG_ERROR( "hash_value = %p not found", hash);
      *st_hndl = NULL;
    }
  }
  else
  {
    DSI_LOG_ERROR( "Invalid hash_value = %p", hash);
    *st_hndl = NULL;
  }
}
...
*/
/*=========================================================================*/
uint64_t dsi_get_hash_from_str_ptr
(
  dsi_store_t * st_hndl
)
{
  ...
  if ((DSI_TRUE == dsi_inited) && (st_hndl != NULL))
  {
    for (i = 0; i < DSI_MAX_DATA_CALLS; i++)
    {
      if (dsi_store_table[i].is_valid &&
          dsi_store_table[i].dsi_store_ptr == st_hndl)
      {
        DSI_LOG_DEBUG("dsi_st_hndl=%p hash=%p",
                      st_hndl, dsi_store_table[i].hash_value);
        return dsi_store_table[i].hash_value;
      }
    }
    if(DSI_MAX_DATA_CALLS == i)
    {
      DSI_LOG_ERROR( "st_hndl = %p not found", st_hndl);
      return 0;
    }
  }
  else
  {
    DSI_LOG_ERROR( "Invalid st_hndl = %p", st_hndl);
    return 0;
  }
...
)
{
  ...
  DSI_LOG_DEBUG( "dsi_translate_qmi_to_dsi_bearer_tech: ENTER" );

  if (!qmi_bearer_tech)
  {
    DSI_LOG_ERROR( "dsi_translate_qmi_to_dsi_bearer_tech: bad param" );
    goto bail;
  }
  else if (WDS_CURRENT_NETWORK_UNKNOWN_V01 == qmi_bearer_tech->current_nw)
  {
    DSI_LOG_ERROR( "dsi_translate_qmi_to_dsi_bearer_tech: unknown network" );
    goto bail;
  }
...
bail:
  DSI_LOG_DEBUG( "dsi_translate_qmi_to_dsi_bearer_tech: EXIT" );
  return bearer_tech;
}
...
)
{
  ...
  DSI_LOG_DEBUG( "dsi_translate_qmi_to_dsi_bearer_tech_ex: ENTER" );

  if (!qmi_bearer_tech_ex)
  {
    DSI_LOG_ERROR( "dsi_translate_qmi_to_dsi_bearer_tech_ex: bad param" );
    goto bail;
  }
  ...
  if (WDS_BEARER_TECH_NETWORK_3GPP_V01 == qmi_bearer_tech_ex->technology)
  {
    switch (qmi_bearer_tech_ex->rat_value)
    {
      ...
      case WDS_BEARER_TECH_RAT_EX_3GPP_5G_V01:
        bearer_tech = DSI_DATA_BEARER_TECH_5G;
        break;

      case WDS_BEARER_TECH_RAT_EX_3GPP_CIWLAN_V01:
        bearer_tech = DSI_DATA_BEARER_TECH_3GPP_CIWLAN;
        break;
...
bail:
  DSI_LOG_DEBUG( "dsi_translate_qmi_to_dsi_bearer_tech_ex: EXIT" );
  return bearer_tech;
}
...
void dsi_cleanup_store_tbl(short int index)
{
  ...
  DSI_LOG_DEBUG( "dsi_cleanup_store_tbl: ENTRY" );
  if (index >= 0 && index < DSI_MAX_DATA_CALLS)
  {
    if (dsi_store_table[index].dsi_store_ptr != NULL)
    {
      ...
      DSI_LOG_DEBUG( "dsi_cleanup_store_tbl: Freeing up store pointer[%p] and \
              setting hash_value [%p] to [0]", st,
              dsi_store_table[index].hash_value);
    ...
    dsi_store_table[index].hash_value = 0;
  }
  else
  {
    DSI_LOG_ERROR( "dsi_cleanup_store_tbl: Invalid index sent" );
  }
  DSI_LOG_DEBUG( "dsi_cleanup_store_tbl: EXIT" );
  return;
}
...
)
{
  ...
  for(i=0; i<DSI_MAX_DATA_CALLS; i++)
  {
    if (dsi_store_table[i].is_valid)
    {
      ...
      if (NULL == st  || dsi_store_table[i].hash_value <= 0)
      {
        DSI_LOG_ERROR("fix programming err: NULL handle pointer found" \
               "or INVALID hash_value found on a store entry" \
               "that is valid st_hndl=%p, hash=%p",st, dsi_store_table[i].hash_value);
      ...
      if(st->priv.dsi_iface_id == dsi_id)
      {
       ...
       {
          if (NULL != st->net_ev_cb && dsi_store_table[i].hash_value > 0)
          {
            DSI_LOG_DEBUG("notifying event [%d] on handle [%p]",
                          event, st);
            DSI_LOG_DEBUG("dsi_st_hndl=%p hash=%p",
                          st, dsi_store_table[i].hash_value);
            st->net_ev_cb( (dsi_hndl_t *)dsi_store_table[i].hash_value,
                           st->user_data,
                           event,
                           st->ev_payload );
          ...
          else
          {
            DSI_LOG_ERROR("NULL callback found on store pointer" \
                          "[%p] or INVALID hash_value=%p", (void *) st,dsi_store_table[i].hash_value);
          }
        }
      }
    }
...
)
{
  ...
  DSI_LOG_DEBUG("dsi_fill_addr: ENTRY");
  ...
  do
  {
    ...
    {
      DSI_LOG_ERROR("NULL pointer specified");
      break;
    }
  ...
  DSI_LOG_DEBUG("dsi_fill_addr: EXIT");
  return ret;
}
...
)
{
  ...
  if (!info)
  {
    DSI_LOG_ERROR("invalid nl info rcvd");
    DSI_LOG_DEBUG("dsi_fill_addr_info: EXIT");
    return;
  }

  if ((info->param_mask & NETMGR_EVT_PARAM_PDU_ID) && info->pdu_id)
  {
    pdu_cnf = SECONDARY_PDU;
  }

  if (info->param_mask & NETMGR_EVT_PARAM_ADDR_TYPE)
  {
    addr_type = dsi_convert_netmgr_addr_type(info->addr_info.address_type);
  }
  ...
  {
    DSI_LOG_VERBOSE("dsi_fill_addr_info: valid client not found");
    return;
  }

  addr_info = dsi_get_addrinfo_ex(dsi_id, ipf, pdu_cnf, addr_type);

  if (!info->addr_info.ext_ip)
  {
    DSI_LOG_DEBUG("storing address for pdu[%d]", pdu_cnf);
    DSI_INET_NTOP("new address", &info->addr_info.addr.ip_addr);

    /* Copy in new address information from netmgr */
    ds_memscpy(&addr_info->iface_addr_s.addr, sizeof(addr_info->iface_addr_s.addr),
               &info->addr_info.addr.ip_addr, sizeof(info->addr_info.addr.ip_addr));
    addr_info->iface_mask = info->addr_info.addr.mask;
    addr_info->iface_addr_s.valid_addr = DSI_TRUE;
  }

  /* Query Modem for auxiliary information. Netmgr does not always send it */
  if (QDI_SUCCESS != qdi_get_addr_info_ex(DSI_GET_QDI_HNDL(dsi_id),
                                          DSI_GET_DEV_STR(dsi_id),
                                          ((DSI_IP_FAMILY_V4 == ipf) ? AF_INET : AF_INET6),
                                          (dsi_addr_info_t *)addr_info,
                                          ((SECONDARY_PDU == pdu_cnf) ? info->pdu_id : 0),
                                          tech_name))
  {
    DSI_LOG_ERROR("failed to get addr info from QDI");
  }

  if (PRIMARY_PDU == pdu_cnf)
  {
    addr_info->pdu_id = 0;
    addr_info->primary = TRUE;
  }
  else
  {
    addr_info->pdu_id = info->pdu_id;
    addr_info->primary = FALSE;
  }

  if (info->param_mask & NETMGR_EVT_PARAM_MTU)
  {
    DSI_SET_MTU(dsi_id, info->mtu);
    DSI_SET_MTU_V4(dsi_id, info->mtu4);
    DSI_SET_MTU_V6(dsi_id, info->mtu6);
  }

  if ((info->param_mask & NETMGR_EVT_PARAM_CACHE) == NETMGR_EVT_PARAM_CACHE)
  {
    addr_info->deprecateTime = info->addr_info.cache_info.prefered;
    addr_info->expiryTime = info->addr_info.cache_info.valid;
  }
  else
  {
    /* Address is permanent */
    addr_info->deprecateTime = 0xFFFFFFFF;
    addr_info->expiryTime = 0xFFFFFFFF;
  }

  /* IPv4 doesn't have any address type fanciness */
  addr_info->address_type = (DSI_IP_FAMILY_V4 == ipf) ? DSI_ADDR_TYPE_INVALID :
                                                        addr_type;
...
)
{
  ...
  DSI_LOG_DEBUG("dsi_fill_qos_info: ENTRY");

  if (!DSI_IS_ID_VALID(dsi_id))
  {
    DSI_LOG_ERROR("invalid dsi id [%d] rcvd", dsi_id);
    DSI_LOG_DEBUG("dsi_fill_qos_info: EXIT");
    return;
  }
...
)
{
  ...
  if (NULL == ce_reason ||
      !DSI_IS_ID_VALID_R(i))
  {
    DSI_LOG_ERROR("**programming err** bad param rcvd");
    return;
  }

    DSI_LOG_ERROR("dsi_release_dsi_iface");
  ...
  DSI_SET_MTU_V4(i, DSI_INVALID_MTU);
  DSI_SET_MTU_V6(i, DSI_INVALID_MTU);

  for (pdu = PRIMARY_PDU; pdu < DSI_NUM_PDUS; pdu++)
  {
    dsi_pdu_addr_info_t *pdu_addr;

    pdu_addr = &dsi_iface_tbl[i].pdu_addr_info[pdu];
    if(ipf == DSI_NUM_IP_FAMILIES)
    {
      /* Clear both the family addresses in case we are unable to determine IP
       * family.
       */
      memset(pdu_addr, 0, sizeof(*pdu_addr));
    }
    else if (ipf == DSI_IP_FAMILY_V4)
    {
      memset(&pdu_addr->ipv4, 0, sizeof(pdu_addr->ipv4));
    }
    else
    {
      memset(pdu_addr->ipv6, 0, sizeof(pdu_addr->ipv6));
    }
  }
}
...
)
{
  ...
  DSI_LOG_DEBUG( "dsi_is_qmiport_incall: entry" );
  ...
  {
    DSI_LOG_ERROR("Invalid iface passed");
    return ret;
  }
  *inuse_ptr = FALSE;
  ...
  *link_state_v6 = WDS_CONNECTION_STATUS_DISCONNECTED_V01;
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_DEBUG( "dsi_is_qmiport_incall: " \
                   "exit with success" );
  }
  else
  {
...
DSI_INLINE void dsi_attach_dsi_iface(int i, dsi_store_t * st)
{
  if (st == NULL)
  {
    DSI_LOG_ERROR("NULL st received");
    return;
  }
...
DSI_INLINE void dsi_detach_dsi_iface(dsi_store_t * st)
{
  ...
  {
    DSI_LOG_ERROR("invalid dsi iface");
    return;
  }

  DSI_LOG_ERROR("dsi_detach_dsi_iface");
  ...
  if (QDI_INVALID_CALL_HANDLE != st->priv.qdi_call_hndl)
  {
    DSI_LOG_ERROR("Releasing qdi_release_call_handle");
    qdi_release_call_handle(st->priv.qdi_call_hndl);
    st->priv.qdi_call_hndl = QDI_INVALID_CALL_HANDLE;
  }
...
/*===========================================================================
  FUNCTION:  dsi_prepare_call_match_lookup_params
===========================================================================*/
/*!
    @brief
    Given a store handle, this function prepares the params to check
    for matching call. The params are returned in the pparams place holder
...
*/
/*=========================================================================*/
int dsi_prepare_call_match_lookup_params
(
...
)
{
  ...
  do
  {
    if (!pparams || !call_params)
    {
      DSI_LOG_ERROR("%s(): invalid params received", __func__);
      break;
    }

    memset(pparams, 0, sizeof(*pparams));

    if (call_params->param_mask & DSI_MATCHING_CALL_PARAMS_APN_NAME)
    {
      pparams->params_mask |= DSI_QMI_WDS_ROUTE_LOOKUP_APN_NAME_PARAM;
      bytes = strlcpy(pparams->apn_name,
                      call_params->apn_name,
                      sizeof(pparams->apn_name));
      if (bytes >= (QMI_WDS_APN_NAME_MAX_V01 + 1))
      {
        DSI_LOG_ERROR("%s(): truncation occurred!", __func__);
        break;
      }

      pparams->bringup_by_apn_name = 1;
      pparams->params_mask |= DSI_QMI_WDS_ROUTE_LOOKUP_BRINGUP_BY_APN_NAME_PARAM;

      DSI_LOG_DEBUG("%s(): setting APN name [%s]", __func__, pparams->apn_name);
    }

    /* We are assuming only 3GPP profile ID will be passed here */
    if (call_params->param_mask & DSI_MATCHING_CALL_PARAMS_PROFILE_ID)
    {
      pparams->params_mask |= DSI_QMI_WDS_ROUTE_LOOKUP_PROFILE_IDX_PARAM;
      pparams->profile_index = call_params->profile_id;
    }

    if (call_params->param_mask & DSI_MATCHING_CALL_PARAMS_APN_TYPE_MASK)
    {
      pparams->params_mask |= DSI_QMI_WDS_ROUTE_LOOKUP_APN_TYPE_MASK_PARAM;
      pparams->bringup_by_apn_type = 1;
      pparams->apn_type_mask = call_params->apn_type_mask;
    }

    if (call_params->param_mask & DSI_MATCHING_CALL_PARAMS_IP_TYPE)
    {
      pparams->params_mask |= DSI_QMI_WDS_ROUTE_LOOKUP_IP_FAMILY_PREF_PARAM;

      if (DSI_IP_VERSION_4 == call_params->ip_type)
      {
        pparams->ip_family_pref = WDS_IP_FAMILY_PREF_IPV4_V01;
      }
      else if (DSI_IP_VERSION_6 == call_params->ip_type)
      {
        pparams->ip_family_pref = WDS_IP_FAMILY_PREF_IPV6_V01;
      }
      else if (DSI_IP_VERSION_4_6 == call_params->ip_type)
      {
        pparams->ip_family_pref = WDS_IP_FAMILY_PREF_UNSPEC_V01;
      }
    }
...
)
{
  ...
  DSI_LOG_DEBUG("dsi_prepare_route_lookup_params: entry");
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_DEBUG("dsi_prepare_route_lookup_params: " \
                  "exit with success");
  }
  else
  {
    DSI_LOG_ERROR("dsi_prepare_route_lookup_params: " \
                  "exit with error");
  }
...
)
{
  ...
  {
    DSI_LOG_ERROR("** Programming Err ** invalid dsi iface");
    return;
  }

  if (!DSI_IS_IP_FAMILY_VALID(ipf))
  {
    DSI_LOG_ERROR("dsi_update_store_table: invalid ip family");
    return;
  }
...
static DSI_INLINE void dsi_invalidate_iface(int dsi_iface)
{
  ...
  DSI_SET_MTU_V4(dsi_iface, DSI_INVALID_MTU);
  DSI_SET_MTU_V6(dsi_iface, DSI_INVALID_MTU);
  ...
  for (pdu = PRIMARY_PDU; pdu < DSI_NUM_PDUS; pdu++)
  {
    ...
    pdu_addr = &dsi_iface_tbl[dsi_iface].pdu_addr_info[pdu];
    memset(pdu_addr, 0, sizeof(*pdu_addr));
  }
}
...
static DSI_INLINE void dsi_call_cleanup(int conn_id)
{
  ...
  for (i = 0; i < DSI_MAX_DATA_CALLS; i++)
  {
    ...
    {
      if (NULL != st_ptr->net_ev_cb && dsi_store_table[i].hash_value > 0 )
      {
       DSI_LOG_DEBUG("dsi_st_hndl=%p hash=%p",
                      st_ptr, dsi_store_table[i].hash_value);
        st_ptr->net_ev_cb((dsi_hndl_t *)dsi_store_table[i].hash_value,
                          st_ptr->user_data,
                          DSI_EVT_NET_NO_NET,
                          NULL);
      ...
      {
        DSI_LOG_ERROR("NULL callback found on store pointer" \
                      "[%p] or INVALID hash_value=%p",
                      (void *) st_ptr, dsi_store_table[i].hash_value);
      }
...
void dsi_modem_out_of_service_hdlr(int conn_id)
{
  ...
  DSI_LOG_DEBUG("dsi_modem_out_of_service_hdlr ENTRY");
  ...
  DSI_LOG_DEBUG("dsi_modem_out_of_service_hdlr EXIT");
}
...
void dsi_modem_in_service_hdlr(int conn_id)
{
  ...
  qmi_client_type wds_hndl;
  int modem;
  DSI_LOG_DEBUG("dsi_modem_in_service_hdlr ENTRY");
  ...
  DSI_LOG_DEBUG("let go of global lock");
  DSI_GLOBAL_UNLOCK;

  DSI_LOG_DEBUG("dsi_modem_in_service_hdlr EXIT");
}
...
static DSI_INLINE int dsi_init_ifaces()
{
  ...
  {
    DSI_LOG_ERROR("error on device name installation (local modem)");
    return DSI_ERROR;
  }
  ...
  {
    DSI_LOG_ERROR("error on device name installation (remote modem)");
    return DSI_ERROR;
  }
  ...
  {
    DSI_LOG_ERROR("error on target configuration");
    return DSI_ERROR;
  }
                      ...
                      int retry_qmi)
{
  ...
  DSI_LOG_DEBUG( "dsi_init_internal: ENTRY" );
  ...
  do
  {
    ...
    {
      DSI_LOG_ERROR("dsi_init_internal: dsi already inited");
      break;
    }
    ...
    {
      DSI_LOG_ERROR("couldn't init diag");
    }
#endif
#endif
    ...
    /* =======================================================
      ...
      ======================================================= */
    DSI_LOG_DEBUG("dsi_init init modem DSI_RADIO_4GMODEM" \
    ...
    DSI_LOG_DEBUG("dsi_init init modem DSI_RADIO_3GMODEM" \
    ...
    if(!from_ssr)
    {
      ...
      {
        DSI_LOG_ERROR("couldn't init dsi_netctrl_cb");
        break;
      }
    }

    /* now init the real modem network interfaces */
    ret = dsi_mni_vtbl.mni_init_f(from_ssr, retry_qmi);
    if (DSI_ERROR == ret)
    {
      break;
    }
  ...
  if (DSI_SUCCESS == ret)
  {
    ...
    DSI_LOG_DEBUG( "dsi_init_internal: EXIT with success" );
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_init_internal: EXIT with error" );
  }
...
void dsi_release_internal(void)
{
  ...
  DSI_LOG_DEBUG( "dsi_release_internal: ENTRY" );

  if (0 != ds_cmdq_flush(&dsi_netctrl_cb_cmdq))
  {
    DSI_LOG_ERROR("*** could not flush dsi_netctrl_cb_cmdq ***");
  }
  ...
  DSI_LOG_DEBUG( "dsi_release_internal: EXIT" );
}/* dsi_release_internal() */

/*===========================================================================
  FUNCTION:  dsi_get_current_data_bearer_tech_string
===========================================================================*/
/** @ingroup dsi_get_current_data_bearer_tech_string

   Returns the current data bearer technology string on which a call was brought up.

   @param[in] bearer tech value returned from dsi_get_current_data_bearer_tech()

   @return
   The data bearer technology in string format.

*/
/*=========================================================================*/
char * dsi_get_current_data_bearer_tech_string
(
  int bearer_tech
)
{
  if( bearer_tech > 0 && bearer_tech < DSI_DATA_BEARER_TECH_MAX )
  {
    return bearer_tech_string_tbl[bearer_tech].str;
  }
  else
  {
    return "DATA BEARER TECH UNKNOWN";
  }
}/* dsi_get_current_data_bearer_tech_string() */
