...
/*===========================================================================
  ...
  Copyright (c) 2010-2015, 2019, 2021-2022, 2024 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
static int dsi_init_netmgr_general(void)
{
  ...
  {
    if (NETMGR_SUCCESS !=
        netmgr_client_register(dsi_netmgr_cb, NULL, &netmgr_hndl, dsi_config.netmgr_msg_proto))
    {
      return DSI_ERROR;
    }
...
static void dsi_init_query_netmgr()
{
        ...
        i < DSI_INIT_MAX_RETRY_COUNT )/* stop retrying after reaching max trial number */
  {
    ...
    {
      DSI_LOG_DEBUG("update dsi_inited to TRUE\n");
...
static int dsi_init_ping_thread()
{
  ...
  {
    DSI_LOG_ERROR("failed to create dsi_ping_thread\n");
    goto bail;
  }
...
static int __dsi_init_ex(void *mode_ptr)
{
  ...
  case DSI_MODE_GENERAL:
    ...
    {
      ret = DSI_ERROR;
      DSI_LOG_ERROR("%s(): dsi_init_netmgr_general failed", __func__);
    }
    break;
  case DSI_MODE_SSR:
...
int dsi_init(int mode)
{
  if(dsi_inited == DSI_TRUE)
  {
    DSI_LOG_ERROR("dsi_init: dsi already inited");
    return DSI_EINITED;
  }
  ...
  do
  {
    ...
    case DSI_MODE_TEST:
      ...
      break;
    case DSI_MODE_GENERAL:
      DSI_LOG_DEBUG("initializing dsi_netctrl in general mode");
      /* if no vtbl is provided, it's inited in general mode
         by default */
      if (DSI_SUCCESS != dsi_init_internal(NULL, FALSE, 0))
      {
        reti = DSI_ERROR;
        DSI_LOG_ERROR("dsi_init_internal failed");
      }
      if (DSI_SUCCESS != dsi_init_netmgr_general())
      {
        reti = DSI_ERROR;
        DSI_LOG_ERROR("dsi_init_netmgr_general failed");
      }
      break;
    case DSI_MODE_SSR:
      DSI_LOG_DEBUG("initializing dsi_netctrl in SSR mode");
  ...
  if (ret == DSI_SUCCESS)
  {
    DSI_LOG_DEBUG( "dsi_init: EXIT with suc" );
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_init: EXIT with err" );
  }
