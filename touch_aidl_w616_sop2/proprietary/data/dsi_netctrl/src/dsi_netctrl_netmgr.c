...
/*===========================================================================
  ...
  Copyright (c) 2010-2015, 2017, 2019-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
dsi_netmgr_link_map_type dsi_netmgr_oemprxy_link_map_tbl[DSI_MAX_IFACES] =
{
  { NETMGR_LINK_REV_RMNET_0,  34 },
  { NETMGR_LINK_REV_RMNET_1,  35 },
  { NETMGR_LINK_REV_RMNET_2,  36 },
  { NETMGR_LINK_REV_RMNET_3,  37 },
  { NETMGR_LINK_REV_RMNET_4,  38 },
  { NETMGR_LINK_REV_RMNET_5,  39 },
  { NETMGR_LINK_REV_RMNET_6,  40 },
  { NETMGR_LINK_REV_RMNET_7,  41 },
  { NETMGR_LINK_REV_RMNET_8,  42 },
  { NETMGR_LINK_REV_RMNET_9,  43 },
  { NETMGR_LINK_REV_RMNET_10, 44 },
  { NETMGR_LINK_REV_RMNET_11, 45 },
  { NETMGR_LINK_REV_RMNET_12, 46 },
  { NETMGR_LINK_REV_RMNET_13, 47 },
  { NETMGR_LINK_REV_RMNET_14, 48 },
  { NETMGR_LINK_REV_RMNET_15, 49 },
};

#define dsi_link_map_tbl_len(X) (int) (sizeof(X) / sizeof(dsi_netmgr_link_map_type));
#endif /* FEATURE_DATA_IWLAN */

/*===========================================================================
  FUNCTION:  dsi_netmgr_find_ip_family
===========================================================================*/
/*!
    @brief
...
)
{
  ...
  DSI_LOG_DEBUG("dsi_netmgr_find_ip_family: ENTRY");

  if (NULL == info || NULL == ipf)
  {
    DSI_LOG_ERROR("dsi_netmgr_find_ip_family: invalid param(s)");
    goto bail;
  }

  /* If there's an IP addresses included in the message */
  if ((info->param_mask & NETMGR_EVT_PARAM_IPADDR) == NETMGR_EVT_PARAM_IPADDR)
  {
        ...
        DSI_STATE_CALL_CONNECTING == st->priv.call_state)
    {
      ...
      switch(st->priv.ip_version)
      {
        case DSI_IP_VERSION_4:
          ...
          {
            DSI_LOG_DEBUG("dsi_netmgr_abort_pending_start: stopping call");
...
static void dsi_netmgr_candidate_promote
(
...
static boolean dsi_netmgr_store_ip_match
(
...
static int dsi_netmgr_store_net_reconfig
(
  int dsi_iface_id,
  int dsi_event,
  const netmgr_nl_event_info_t *info,
...
)
{
  ...
  for (i = 0; i < DSI_MAX_DATA_CALLS; i++)
  {
    ...
    if (!st || dsi_iface_id != st->priv.dsi_iface_id ||
        DSI_TRUE != dsi_netmgr_store_ip_match(st, ipf))
    {
      continue;
    }
...
static int dsi_netmgr_store_net_newaddr
(
  int dsi_iface_id,
  int dsi_event,
  const netmgr_nl_event_info_t *info,
...
static int dsi_netmgr_store_net_deladdr
(
  int dsi_iface_id,
  int dsi_event,
  const netmgr_nl_event_info_t *info,
...
)
{
      ...
      DSI_ADDR_TYPE_DEFAULT == addr_type)
  {
    ...
    dsi_netmgr_candidate_promote(addrinfo, candidate, ipf);
  }
  ...
  for (i = 0; i < DSI_MAX_DATA_CALLS; i++)
  {
    ...
    if ((DSI_STATE_CALL_DISCONNECTING == st->priv.call_state) ||
        ((DSI_IP_VERSION_4 == st->priv.ip_version) && (0 == DSI_GET_V4_COUNT(st->priv.dsi_iface_id))) ||
...
/*===========================================================================
  FUNCTION:  dsi_netmgr_update_internal_call_end_reason
===========================================================================*/
/*!
    @brief
...
*/
/*=========================================================================*/
static void dsi_netmgr_update_internal_call_end_reason
(
  dsi_store_t *st,
  dsi_ip_family_t ipf,
  netmgr_op_status_info_t op_status
)
{
  ...
  case DSI_IP_FAMILY_V4:
    st->priv.ce_reason[DSI_IP_FAMILY_V4].reason_code = DSI_CE_REASON_CODE_NETMGR_ERROR;
  ...
  case DSI_IP_FAMILY_V6:
    st->priv.ce_reason[DSI_IP_FAMILY_V6].reason_code = DSI_CE_REASON_CODE_NETMGR_ERROR;
...
static int dsi_netmgr_store_net_no_net
(
  int dsi_iface_id,
  int dsi_event,
  const netmgr_nl_event_info_t *info,
...
)
{
  ...
  for (i = 0; i < DSI_MAX_DATA_CALLS; i++)
  {
    ...
    if (NETMGR_OP_STATUS_CODE_SUCCESS != info->op_status.op_code)
    {
      /* If we reached here due to a netmgr error, we should update the call end reason
      ...
      {
        dsi_netmgr_update_internal_call_end_reason(st, DSI_IP_FAMILY_V4, info->op_status);
        dsi_netmgr_update_internal_call_end_reason(st, DSI_IP_FAMILY_V6, info->op_status);
      }
      else if ((st->priv.ip_version == DSI_IP_VERSION_4)
                && (NETMGR_OP_STATUS_IP_FAMILY_V4 == info->op_status.op_ip_family))
      {
        dsi_netmgr_update_internal_call_end_reason(st, DSI_IP_FAMILY_V4, info->op_status);
      }
      else if ((st->priv.ip_version == DSI_IP_VERSION_6)
                && (NETMGR_OP_STATUS_IP_FAMILY_V6 == info->op_status.op_ip_family))
      {
        dsi_netmgr_update_internal_call_end_reason(st, DSI_IP_FAMILY_V6, info->op_status);
      }
    }
...
static int dsi_netmgr_store_qos_status
(
  int dsi_iface_id,
  int dsi_event,
  const netmgr_nl_event_info_t *info,
...
static int dsi_netmgr_store_net_newmtu
(
  int dsi_iface_id,
  int dsi_event,
  const netmgr_nl_event_info_t *info,
...
static int dsi_netmgr_store_pdu_available
(
  int dsi_iface_id,
  int dsi_event,
  const netmgr_nl_event_info_t *info,
...
)
{
  ...
  for (i = 0; i < DSI_MAX_DATA_CALLS; i++)
  {
    ...
    if (NETMGR_EVT_PARAM_PDU_ID & info->param_mask)
    {
...
static int dsi_netmgr_store_pdu_promotion
(
  int dsi_iface_id,
  int dsi_event,
  const netmgr_nl_event_info_t *info,
...
)
{
  ...
  sec_addr_info = dsi_get_addrinfo_ex(dsi_iface_id, ipf, SECONDARY_PDU,
                                      DSI_ADDR_TYPE_CANDIDATE);
  dsi_netmgr_candidate_promote(addr_info, sec_addr_info, ipf);
...
static int dsi_netmgr_store_pdu_release
(
  int dsi_iface_id,
  int dsi_event,
  const netmgr_nl_event_info_t *info,
...
)
{
  ...
  {
    DSI_LOG_ERROR("dsi_netmgr_store_event_data: bad param");
    return DSI_EVT_INVALID;
  }

  DSI_LOG_DEBUG("dsi_netmgr_store_event_data: ENTRY");
  ...
  do
  {
    ...
    switch(dsi_event)
    {
      ...
      case DSI_EVT_NET_RECONFIGURED:
        reti = dsi_netmgr_store_net_reconfig(dsi_iface_id, dsi_event, info, &evt_ret);
        break;
      case DSI_EVT_NET_NEWADDR:
        reti = dsi_netmgr_store_net_newaddr(dsi_iface_id, dsi_event, info, &evt_ret);
        break;
      case DSI_EVT_NET_DELADDR:
        reti = dsi_netmgr_store_net_deladdr(dsi_iface_id, dsi_event, info, &evt_ret);
        break;
      case DSI_EVT_NET_NO_NET:
        reti = dsi_netmgr_store_net_no_net(dsi_iface_id, dsi_event, info, &evt_ret);
        break;
      case DSI_EVT_QOS_STATUS_IND:
        reti = dsi_netmgr_store_qos_status(dsi_iface_id, dsi_event, info, &evt_ret);
        break;
      case DSI_EVT_NET_NEWMTU:
        reti = dsi_netmgr_store_net_newmtu(dsi_iface_id, dsi_event, info, &evt_ret);
        break;
      case DSI_EVT_PDU_AVAILABLE:
        reti = dsi_netmgr_store_pdu_available(dsi_iface_id, dsi_event, info, &evt_ret);
        break;
      case DSI_EVT_PDU_PROMOTION:
        reti = dsi_netmgr_store_pdu_promotion(dsi_iface_id, dsi_event, info, &evt_ret);
        break;
      case DSI_EVT_PDU_RELEASE:
        reti = dsi_netmgr_store_pdu_release(dsi_iface_id, dsi_event, info, &evt_ret);
        break;
  ...
  {
    DSI_LOG_ERROR("dsi_netmgr_store_event_data: EXIT error, evt_ret %d dsi_event %d", (int)evt_ret, dsi_event);
  }
...
)
{
  ...
  DSI_LOG_DEBUG("dsi_netmgr_map_link: ENTRY");
  ...
  DSI_LOG_DEBUG("dsi_netmgr_map_link: EXIT");
...
#ifdef FEATURE_DATA_IWLAN
/*===========================================================================
  FUNCTION:  dsi_netmgr_map_reverse_link
===========================================================================*/
/*!
    @brief
    maps netmgr reverse link to dsi iface id. if a match is found, dsi iface id
    is returned in dsi_iface_id placeholder.
...
*/
/*=========================================================================*/
static int dsi_netmgr_map_reverse_link
(
  netmgr_link_id_t netmgr_link,
  int * dsi_iface_id
)
{
  ...
  DSI_LOG_DEBUG("dsi_netmgr_map_reverse_link: ENTRY");
  ...
  do
  {
    ...
    {
      if (dsi_netmgr_oemprxy_link_map_tbl[i].netmgr_link == netmgr_link)
      {
        break;
      }
    }
    ...
    {
      DSI_LOG_ERROR("we don't recognize netlink link [%d]. Check " \
                    "dsi_netmgr_oemprxy_link_map_tbl", netmgr_link);
      break;
    }
  ...
  DSI_LOG_DEBUG("dsi_netmgr_map_reverse_link: EXIT");
...
)
{
  ...
  int rc = DSI_ERROR;
  int oemprxy_event = FALSE;

  DSI_LOG_VERBOSE("dsi_process_netmgr_ev ENTRY");
  (void)data;
  ...
  do
  {
    ...
    if (NULL == info)
    {
      DSI_LOG_ERROR("NULL info ptr received");
      break;
    }

#ifdef FEATURE_DATA_IWLAN
    if (dsi_config.oemprxy_enabled
          && (event == NET_PLATFORM_OEMPRXY_IFACE_UP
                || event == NET_PLATFORM_OEMPRXY_IFACE_DOWN))
    {
      /* If one of these event is sent and iwlan is not defined rc will remain as an error */
      rc = dsi_netmgr_map_reverse_link(info->link, &dsi_iface_id);
      if (DSI_SUCCESS != rc)
      {
        DSI_LOG_DEBUG("dsi_netmgr_map_link failed with err [%d]", rc);
        break;
      }
    }
    else
    ...
    if (dsi_inited != DSI_TRUE &&
        (NETMGR_READY_RESP != event && NETMGR_USER_CMD != event &&
         NET_PLATFORM_DOWN_EV != event))
    {
    ...
    switch(event)
    {
      case NET_PLATFORM_UP_EV:
        DSI_LOG_DEBUG("NET_PLATFORM_UP_EV received");
        DSI_L2S_MSG_SEQ(dsi_iface_id,"NET_PLATFORM_UP_EV");
        dsi_event = DSI_EVT_NET_IS_CONN;
        break;
      case NET_PLATFORM_DOWN_EV:
        DSI_LOG_DEBUG("NET_PLATFORM_DOWN_EV received");
        DSI_L2S_MSG_SEQ(dsi_iface_id,"NET_PLATFORM_DOWN_EV");
        dsi_event = DSI_EVT_NET_NO_NET;
        break;
      case NET_PLATFORM_RECONFIGURED_EV:
        DSI_LOG_DEBUG("NET_PLATFORM_RECONFIGURED_EV received");
        DSI_L2S_MSG_SEQ(dsi_iface_id,"NET_PLATFORM_RECONFIGURED_EV");
        dsi_event = DSI_EVT_NET_RECONFIGURED;
        break;
      case NET_PLATFORM_NEWADDR_EV:
        DSI_LOG_DEBUG("NET_PLATFORM_NEWADDR_EV received");
        dsi_event = DSI_EVT_NET_NEWADDR;
        break;
      case NET_PLATFORM_DELADDR_EV:
        DSI_LOG_DEBUG("NET_PLATFORM_DELADDR_EV received");
        dsi_event = DSI_EVT_NET_DELADDR;
        break;
      case NET_PLATFORM_FLOW_ACTIVATED_EV:
        /* Used for mobile & network-initiated flows */
        DSI_LOG_DEBUG("NET_PLATFORM_FLOW_ACTIVATED_EV received");
        dsi_event = DSI_EVT_QOS_STATUS_IND;
        break;
      case NET_PLATFORM_FLOW_SUSPENDED_EV:
        /* Used for mobile & network-initiated flows */
        DSI_LOG_DEBUG("NET_PLATFORM_FLOW_SUSPENDED_EV received");
        dsi_event = DSI_EVT_QOS_STATUS_IND;
        break;
      case NET_PLATFORM_FLOW_DELETED_EV:
        /* Used for mobile & network-initiated flows */
        DSI_LOG_DEBUG("NET_PLATFORM_FLOW_DELETED_EV received");
        dsi_event = DSI_EVT_QOS_STATUS_IND;
        break;
      case NET_PLATFORM_FLOW_MODIFIED_EV:
        /* Used for mobile & network-initiated flows */
        DSI_LOG_DEBUG("NET_PLATFORM_FLOW_MODIFIED_EV received");
        dsi_event = DSI_EVT_QOS_STATUS_IND;
        break;
      case NET_PLATFORM_MTU_UPDATE_EV:
        DSI_LOG_DEBUG("NET_PLATFORM_MTU_UPDATE_EV received");
        dsi_event = DSI_EVT_NET_NEWMTU;
        break;
      case NETMGR_USER_CMD:
        DSI_LOG_ERROR("NETMGR_USER_CMD received, netmgr_state [%d]\n", info->cmd_data.netmgr_restart_state);
        netmgr_state = info->cmd_data.netmgr_restart_state;
        if (netmgr_state == NETMGR_POST_RESTART_STATE_INIT_DONE)
        {
          set_dsi_init_state(DSI_TRUE);
          DSI_LOG_DEBUG("NETMGR_POST_RESTART_STATE_INIT_DONE received, dsi inited");
        }
        else
        if (netmgr_state == NETMGR_POST_RESTART_STATE_INIT_START)
        {
          dsi_inited = DSI_FALSE;
          DSI_LOG_DEBUG("NETMGR_POST_RESTART_STATE_INIT_START received, dsi not inited");
        }
        break;
      case NET_PLATFORM_OEMPRXY_IFACE_UP:
        DSI_LOG_DEBUG("NET_PLATFORM_OEMPRXY_IFACE_UP received");
        dsi_oemproxy_common_ind(DSI_EVT_OEMPRXY_IS_CONN, info->link);
        oemprxy_event = TRUE;
        break;
      case NET_PLATFORM_OEMPRXY_IFACE_DOWN:
        DSI_LOG_DEBUG("NET_PLATFORM_OEMPRXY_IFACE_DOWN received");
        dsi_oemproxy_common_ind(DSI_EVT_OEMPRXY_NO_NET, info->link);
        oemprxy_event = TRUE;
        break;

      case NET_PLATFORM_PDU_AVAILABLE_EV:
        DSI_LOG_DEBUG("NET_PLATFORM_PDU_AVILABLE_EV received");
        DSI_L2S_MSG_SEQ(dsi_iface_id,"NET_PLATFORM_PDU_AVILABLE_EV");
        dsi_event = DSI_EVT_PDU_AVAILABLE;
        break;
      case NET_PLATFORM_PDU_PROMOTION_EV:
        DSI_LOG_DEBUG("NET_PLATFORM_PDU_PROMOTION_EV received");
        DSI_L2S_MSG_SEQ(dsi_iface_id,"NET_PLATFORM_PROMOTION_EV");
        dsi_event = DSI_EVT_PDU_PROMOTION;
        break;
      case NET_PLATFORM_PDU_RELEASE_EV:
        DSI_LOG_DEBUG("NET_PLATFORM_PDU_RELEASE_EV received");
        DSI_L2S_MSG_SEQ(dsi_iface_id,"NET_PLATFORM_PDU_RECONFIG_EV");
        dsi_event = DSI_EVT_PDU_RELEASE;
        break;
      case NET_PLATFORM_PDU_RECONFIG_EV:
        DSI_LOG_DEBUG("NET_PLATFORM_PDU_RELEASE_EV received");
        DSI_L2S_MSG_SEQ(dsi_iface_id,"NET_PLATFORM_PDU_RECONFIG_EV");
        dsi_event = DSI_EVT_PDU_RECONFIGED;
      case NET_PLATFORM_OP_FAILED_EV:
        DSI_LOG_DEBUG("NET_PLATFORM_OP_FAILED_EV received");
        DSI_L2S_MSG_SEQ(dsi_iface_id,"NET_PLATFORM_OP_FAILED_EV");
        dsi_event = DSI_EVT_NET_NO_NET;
        break;
    ...
    /* OEM Proxy events don't need to post, skip and exit */
    if(oemprxy_event == TRUE)
    {
      ret = DSI_SUCCESS;
      break;
    }
  ...
  if (ret == DSI_ERROR)
  {
    DSI_LOG_VERBOSE("dsi_process_netmgr_ev EXIT with err");
  }
  else
  {
    DSI_LOG_VERBOSE("dsi_process_netmgr_ev EXIT with suc");
  }
