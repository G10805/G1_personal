...
#include <stdint.h> /* user mode only */
#endif
#if defined(__QNXNTO__) || defined(__INTEGRITY)
#include "vmm_api.h"

/* host only wrapper functions */
enum hab_vmm_event {
	HAB_GVM_WDOG_BITE = (1 << 0),
	HAB_GVM_CONTAINER_CRASH = (1 << 1),
	HAB_GVM_RESTARTING = (1 << 2),
	HAB_GVM_STOPPED = (1 << 4),
};

typedef int (*hab_vmm_client_handler)(uint32_t vm_id,  uint32_t event, void *priv_data);
 ...
 *
 * This flag is not supported for HAB clients in user-space.
 * And just reserve this bitmask here, and don't use it for any other purpose.
 */
...
/*
 * this flag is used to import to fixed linear address where platform supports
 */
#define HABMM_IMP_FLAGS_FIXED  0x00020000

/*
 * this flag is used when clients import memory to fd, and close the fd
 * on client side before calling hab unimport. Client can close fd once
 ...
 * unimport.
 */
#define HABMM_UNIMP_FLAGS_FD_ALREADY_CLOSED  0x00040000
