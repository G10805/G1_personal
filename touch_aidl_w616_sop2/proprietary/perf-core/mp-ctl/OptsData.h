...
public:
    bool Add(int tid, thread_group tids_of_interest) {
        if (findByKey(tid) != mSchedGroupHashSet.end())
            return false;
        sched_group_key_value* newEntry = new(std::nothrow) sched_group_key_value(tid, tids_of_interest);
        if (newEntry == NULL) {
            return false;
        }
...
public:
    bool Add(int tid, int instanceRefCount) {
        sched_freq_aggr_group_key_value* newEntry = new(std::nothrow) sched_freq_aggr_group_key_value(tid, instanceRefCount);
        if (newEntry == NULL) {
            return false;
        }
    ...
    void IncrementInsRefCntVal(int tid) {
        ...
        if (find_result == mSchedFreqAggrGroupHashSet.end())
            return;
        sched_freq_aggr_group_key_value *entry = *find_result;
        QLOGV(LOG_TAG, "%s, pid=%d instanceRefCount=%d",__FUNCTION__, entry->key, entry->value);
        entry->value++;
    }
    ...
    void checkSchedFreqAggrPresence() {
        ...
        memset(sched_freq_aggregate_node_val, 0, sizeof(sched_freq_aggregate_node_val));
        ...
        if (rc > 0 && atoi(sched_freq_aggregate_node_val)) {
            ...
            QLOGV(LOG_TAG, "Sched Frequency Aggregation support present");
        } else {
            sched_freq_aggr_present = 0;
            QLOGE(LOG_TAG, "Error: Sched Frequency Aggregation support not present");
        }
    }
};
