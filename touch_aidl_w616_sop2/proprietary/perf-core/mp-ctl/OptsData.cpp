...
#define SCALING_MINFREQ_DEFAULT_FILE "/data/vendor/perfd/default_scaling_min_freq"
...
OptsData::OptsData() {
    ...
    for (uint16_t i=0; i<MAX_MINOR_RESOURCES; i++) {
        sysfsnode_storage_length[i] = -1;
        node_type[i] = SINGLE_NODE;
        is_supported[i] = true;
    }

    for (uint16_t i=0; i<MAX_INTERACTIVE_MINOR_OPCODE; i++) {
        cluster0_interactive_node_storage_length[i] = -1;
        cluster1_interactive_node_storage_length[i] = -1;
        cluster2_interactive_node_storage_length[i] = -1;
        cluster3_interactive_node_storage_length[i] = -1;
    }
...
int32_t OptsData::Init() {
    ...
    if (core_ctl_present == 1 && kpm_hotplug_support == 1) {
        QLOGE(LOG_TAG, "Error: KPM hotplug support and core control both are present at the same time");
    }


    if(FAILED == setup_cpu_freq_values()) {
      QLOGE(LOG_TAG, "Frequency initialization failed. Exit perflock server");
      return -1;
    }
...
void OptsData::init_node_paths() {
    ...
    static bool l3_init_complete = false;
    ...
    for (idx = 0; idx < MAX_MINOR_RESOURCES; idx++) {
        ...
        if(rc == FAILED) {
            QLOGE(LOG_TAG, "Couldn't find path for idx=%" PRIu16, idx);
            continue;
        }
        QLOGV(LOG_TAG, "Found path for index=%" PRIu16 " as %s and supported=%" PRIu8, idx, sysfsnode_path[idx], is_supported[idx]);
        if (!is_supported[idx]) {
            continue;
        }
        ...
        case SCHED_START_INDEX + SCHED_COLOC_BUZY_HYST_CPU_PCT:
        case SCHED_START_INDEX + SCHED_COLOC_BUZY_HYST_CPU_NS:
        case SCHED_START_INDEX + SCHED_UTIL_BUSY_HYST_CPU_NS:
        /* These nodes contain space separated numbers. Each number is the value for a core/
        So, these are cluster based nodes where we need to update values for each core in a
        cluster.*/
            node_type[idx] = UPDATE_CORES_PER_CLUSTER;
            break;
        ...
        case CORE_HOTPLUG_START_INDEX + CORE_CTL_ENABLE_BIG_OPCODE:
        case CORE_HOTPLUG_START_INDEX + CORE_CTL_CPU_NOT_PREFERRED_BIG:
        case CORE_HOTPLUG_START_INDEX + CORE_CTL_OFFLINE_DELAY_MS_BIG:
        case CORE_HOTPLUG_START_INDEX + CORE_CTL_MIN_PARTIAL_CPUS_BIG:
        case CPUFREQ_START_INDEX + WALT_ADAPTIVE_LOW_FREQ_OPCODE:
        case CPUFREQ_START_INDEX + WALT_ADAPTIVE_HIGH_FREQ_OPCODE:
        case CPUFREQ_START_INDEX + SCALING_MIN_FREQ_OPCODE:
            node_type[idx] = SELECT_CORE_TO_UPDATE;
            break;

        case MEMLAT_START_INDEX + L3_MEMLAT_MINFREQ_OPCODE:
        case MEMLAT_START_INDEX + L3_MEMLAT_MAXFREQ_OPCODE:
            ...
            for (int8_t j = 0; j < tc.getNumCluster(); j++) {
                ...
                for (uint8_t i = 0; i < NODE_MAX && node_path[i] != '\0'; i++, node_i++) {
                    if (node_path[i] == '%' && (i + 1) < NODE_MAX && node_path[i + 1] == 'd') {
                        l3_minfreq_path[j][node_i] = ('0' + t.getFirstCoreIndex(j));
                        i++;
                    } else {
                        l3_minfreq_path[j][node_i] = node_path[i];
                    }
                }
                l3_minfreq_path[j][node_i] = '\0';
                get_nodes_folder_path(l3_minfreq_path[j], l3_path[j]);
            }
            init_l3_path();
            l3_init_complete = true;
            break;
        case MEMLAT_START_INDEX + CPU_CPU_DDR_LATFLOOR_MINFREQ_OPCODE:
        case MEMLAT_START_INDEX + CPU_CPU_DDR_LATFLOOR_MAXFREQ_OPCODE:
        case MEMLAT_START_INDEX + LLCC_DDR_LAT_MIN_FREQ_OPCODE:
        case MEMLAT_START_INDEX + LLCC_DDR_LAT_MAX_FREQ_OPCODE:
        case MEMLAT_START_INDEX + LLCC_MEMLAT_MIN_FREQ_OPCODE:
        case MEMLAT_START_INDEX + LLCC_MEMLAT_MAX_FREQ_OPCODE:
        node_type[idx] = MEM_LAT_NODE;
        break;

        case SCHED_START_INDEX + SCHED_UPMIGRATE_OPCODE:
        case SCHED_START_INDEX + SCHED_DOWNMIGRATE_OPCODE:
        case SCHED_START_INDEX + SCHED_UP_DOWN_MIGRATE:
        ...
        case SCHED_EXT_START_INDEX + SCHED_IDLE_ENOUGH_CLUST:
        case SCHED_EXT_START_INDEX + SCHED_UTIL_THRES_PCT_CLUST:
        case SCHED_EXT_START_INDEX + SCHED_FMAX_CAP:
            node_type[idx] = UPDATE_CORES_PER_CLUSTER;
            break;
...
void OptsData::get_nodes_folder_path(const char *node_path, char *folder_path) {
    ...
    if(pch != NULL) {
        *pch = '\0';
        ...
        QLOGE(LOG_TAG, "Unable to find the folders path for node=%s", node_path);
    }
}

int8_t OptsData::init_available_cpubw_freq() {
   int32_t rc = -1;
   ...
   if (rc > 0) {
      listf[rc - 1] = '\0';
      QLOGL(LOG_TAG, QLOG_L2, "Initializing available cpubw freq as %s", listf);
      ...
      if (cfL) {
         ...
         while ((cfL = strtok_r(NULL, " ", &pcfL)) != NULL) {
             ...
             if(cpubw_avail_freqs_n >= MAX_FREQ) {
                QLOGE(LOG_TAG, "Number of cpubw frequency is more than the size of the array. Exiting");
                return FAILED;
             }
         }
     }
   } else {
     QLOGL(LOG_TAG, QLOG_WARNING, "Initialization of available cpubw freq failed, as %s not present", avlnode);
   }
   return SUCCESS;
}

int8_t OptsData::init_devfreq_freqlist(char* parent_path) {
   int32_t rc = -1;
   char listf[FREQLIST_STR];
   char *cfL = NULL, *pcfL = NULL;
   char avlnode[NODE_MAX];
   ...
   if (rc > 0) {
      listf[rc - 1] = '\0';
      QLOGL(LOG_TAG, QLOG_L2, "Initializing available freq as %s", listf);
      cfL = strtok_r(listf, " ", &pcfL);
      if (cfL) {
         ...
         while ((cfL = strtok_r(NULL, " ", &pcfL)) != NULL) {
             ...
             if(avail_freqs_n >= MAX_FREQ) {
                QLOGE(LOG_TAG, "Number of available frequency is more than the size of the array. Exiting");
                return FAILED;
             }
         }
      }
   }  else {
         QLOGL(LOG_TAG, QLOG_WARNING, "Initialization of available freq failed, as %s not present", avlnode);
         return FAILED;
   }
...
// npu_llcc_ddr
int8_t OptsData::init_npubw_hwmon_path() {
    int8_t ret = FAILED;
...
int8_t OptsData::init_npu_llcbw_hwmon_path() {
    char d_name[NODE_MAX] = "";
    int8_t ret = FAILED;
    char folder_path[NODE_MAX] = "";
    char *parent = NULL;
...
int8_t OptsData::init_cpubw_hwmon_path() {
    int8_t ret = FAILED;
    char d_name[NODE_MAX] = "";
    char folder_path[NODE_MAX] = "";
    char *parent = NULL;
...
int8_t OptsData::init_devfreq_child_path(const char *parent, char *d_name) {
    DIR *devfreq_path;
    struct dirent *ep;
    int8_t ret = FAILED;
    ...
    if (!devfreq_path) {
        QLOGE(LOG_TAG, "Could not find devfreq path for %s", parent);
        return ret;
    }
...
int8_t OptsData::init_llcbw_hwmon_path() {
    char d_name[NODE_MAX] = "";
    int8_t ret = FAILED;
    char folder_path[NODE_MAX] = "";
    char *parent = NULL;
    ...
    if (ret == SUCCESS) {
        ...
        for (uint16_t i = 0; i < llcbw_avail_freqs_n; i++) {
            llcbw_avail_freqs[i] = avail_freqs[i];
            QLOGL(LOG_TAG, QLOG_L2, "LLCBW, avail freq is %" PRIu32, llcbw_avail_freqs[i]);
        }
        return SUCCESS;
    }
    else
...
/*New init function supporting LLCC_DDR_BW_MIN_FREQ*/
 int8_t OptsData::init_llcbw_hwmon_path_newOpcode() {
     char d_name[NODE_MAX] = "";
     int8_t ret = FAILED;
     char folder_path[NODE_MAX] = "";
     char *parent = NULL;
...
/*New Init function supporting CPU_LLCC_BW_MIN_FREQ*/
 int8_t OptsData::init_cpubw_hwmon_path_newOpcode() {
     int8_t ret = FAILED;
     char d_name[NODE_MAX] = "";
     char folder_path[NODE_MAX] = "";
     char *parent = NULL;
...
int8_t OptsData::init_l3_path() {
    int8_t ret = FAILED;
    Target &t = Target::getCurTarget();
    TargetConfig &tc =  TargetConfig::getTargetConfig();

    for (int8_t j = 0; j < tc.getNumCluster(); j++) {
        snprintf(l3_maxfreq_path[j], NODE_MAX, "%s/max_freq", l3_path[j]);
        ret = init_devfreq_freqlist(l3_path[j]);
        if (ret == SUCCESS) {
            l3_avail_freqs_n[j] = avail_freqs_n;
            for (uint32_t i = 0; i < l3_avail_freqs_n[j]; i++) {
                l3_avail_freqs[j][i] = avail_freqs[i];
                QLOGL(LOG_TAG, QLOG_L2, "L3 CPU%" PRId8 ", avail freq is %" PRIu32,t.getFirstCoreIndex(j), l3_avail_freqs[j][i]);
            }
        }
    }
    return ret;
...
/* toggle KSM
   0 -- KSM won't run
   1 -- KSM will run */
int8_t OptsData::toggle_ksm_run(uint8_t run) {
    int8_t rc=0;
    if(is_ksm_supported == 0)
    {
        if(run == 0)
...
/*
* Parse post boot settings for scaling_min_freq values
*/
void OptsData::initPostbootScalingMinFreq() {

    char buf[NODE_MAX] = "";
    int8_t rc = -1;
    char *cpu_freq = NULL, *cpuid = NULL, *freq = NULL;
    char *saveptr = NULL, *saveptr1 = NULL;
    char sysfspath[NODE_MAX] = "";
    int8_t cpuid_idx = -1;

    FREAD_STR(SCALING_MINFREQ_DEFAULT_FILE, buf, NODE_MAX,rc );
    if (rc < 0) {
        QLOGE(LOG_TAG, "Could not read scaling_minfreq from data file");
        return;
    }
    strlcpy(sysfspath, sysfsnode_path[CPUFREQ_START_INDEX + SCALING_MIN_FREQ_OPCODE], NODE_MAX);
    cpuid_idx = static_cast<string>(sysfspath).find_first_of("0123456789");

    //buf content- "0:595200 3:633600 7:633600"
    cpu_freq = strtok_r(buf, " ", &saveptr);
    if (cpu_freq) {
        do {
            cpuid = strtok_r(cpu_freq, ":", &saveptr1);
            freq = strtok_r(NULL, ":", &saveptr1);
            if (cpuid && freq) {
                sysfspath[cpuid_idx] = atoi(cpuid) + '0';
                if (!access(sysfspath, F_OK)) {
                    postboot_scaling_minfreq[atoi(cpuid)] = atoi(freq);
                } else {
                    QLOGE(LOG_TAG, "Invalid node path:%s", sysfspath);
                }
            }
        } while ((cpu_freq = strtok_r(NULL, " ", &saveptr)) != NULL);
    }
}

uint8_t OptsData::kpm_support_for_hotplug() {
    int8_t rc = 0;
    ...
    //check for KPM hot plug support
    FREAD_STR(kpmSysNode, kpm_max_cpus, NODE_MAX, rc);
    if (rc > 0) {
        kpm_hotplug_support = 1;
    } else
...
uint8_t OptsData::check_core_ctl_presence() {
    int8_t rc = -1;
    Target &t = Target::getCurTarget();
...
int8_t OptsData::core_ctl_init() {
    int8_t rc = 0;
    Target &t = Target::getCurTarget();
    char val[NODE_MAX];

    if(core_ctl_present > 0) {
       for (int8_t i =0; i < MAX_CLUSTER; i++) {
...
int32_t OptsData::setup_cpu_freq_values()
{
  int8_t i = 0;

  for (i = 0; i < TargetConfig::getTargetConfig().getTotalNumCores(); i++ )
...
int8_t OptsData::get_reset_cpu_freq(int8_t cpu, uint8_t ftype)
{
   if (cpu_freq_val[cpu].valid != 1) {
      QLOGE(LOG_TAG, "Error: Perf-lock release for an un-initialized cpu %" PRId8, cpu);
      return FAILED;
   }
   else {
...
int32_t OptsData::find_next_cpubw_available_freq(uint32_t freq) {
    int32_t setval = -1;
    uint32_t i = 0;
...
int32_t OptsData::find_next_llccbw_available_freq(uint32_t freq) {
     int32_t setval = -1;
     uint32_t i = 0;
...
uint32_t OptsData::find_next_avail_freq(uint32_t freq) {
    uint32_t setval = freq;
    uint32_t i = 0;
...
int32_t OptsData::find_next_cpu_frequency(int8_t cpureq, uint32_t freq) {
   int32_t i, clust_id, clust_cpu_num;
   int8_t cluster = -1;
   ...
   if ((cluster < 0) || (startcpu < 0) || (endcpu < 0)){
       QLOGE(LOG_TAG, "Invalid first and last cpu in the cluster for cpu %" PRId8, cpureq);
       return FAILED;
   }
   ...
   for (i = cpu_freq_val[cpu].min_freq_pos; i < cpu_freq_val[cpu].count ; i++)
       if ((uint)cpu_freq_val[cpu].avl_freq[i] >= freq)
...
int32_t OptsData::init_available_freq(int8_t cpu) {
   int32_t rc = -1;
   char listf[FREQLIST_STR];
   char *cfL = NULL, *pcfL = NULL;
   char avlnode[NODE_MAX];
   ...
   if (cpu < 0 || cpu >= TargetConfig::getTargetConfig().getTotalNumCores()) {
      QLOGE(LOG_TAG, "Incorrect cpu%" PRId8,cpu);
      return 0;
   }
   ...
   if (rc > 0) {
      listf[rc - 1] = '\0';
      QLOGL(LOG_TAG, QLOG_L2, "Initializing available freq for core %" PRId8 " as %s", cpu, listf);
      cfL = strtok_r(listf, " ", &pcfL);
      if (cfL) {
     ...
     QLOGL(LOG_TAG, QLOG_WARNING, "Initialization of available freq for core %" PRId8 " failed, as %s not present", cpu, avlnode);
   }
   return SUCCESS;
}

void OptsData::check_min_freq_prop_set(int8_t cpu) {
    Target &t = Target::getCurTarget();
    ...
    if ((cpu <= t.getLastCoreIndex(0)) && (min_freq_prop_0_set))
    {
        ...
        if (cpu_freq_val[cpu].min_freq_pos == -1) {
            QLOGV(LOG_TAG, "Warning: min prop frequency not found setting it to default" );
            cpu_freq_val[cpu].min_freq_pos = 0;
        }
    }
 ...
 */
int32_t OptsData::find_frequency_pos(int8_t cpu, uint32_t freq) {
  int32_t i;
  for (i = 0; i < cpu_freq_val[cpu].count; i++) {
      if (cpu_freq_val[cpu].avl_freq[i] == (int32_t)freq)
             return i;
  }
  return -1;
}
...
float OptsData::get_fps_hyst_time() {
    QLOGL(LOG_TAG, QLOG_L1, "OptsData::get_fps_hyst_time %f", fps_hyst_time_sec);
    return fps_hyst_time_sec;
}

void OptsData::set_fps_hyst_time(float fht) {
    QLOGL(LOG_TAG, QLOG_L1, "OptsData::set_fps_hyst_time new_value: %f", fht);
    fps_hyst_time_sec = fht;
}

int8_t OptsData::get_node_type(int32_t qIndex) {
    return node_type[qIndex];
}
