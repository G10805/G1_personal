...
#if USES_RPROC
#define SSR_BUS_ROOT "/sys/class/remoteproc"
#define SUBSYS_NAME_FORMAT "remoteproc-%s"
#define RECOVERY_PATH "%s/%s/recovery"
#define ENABLE_RECOVERY "enabled"
#define DISABLE_RECOVERY "disabled"
#else
#define SSR_BUS_ROOT "/sys/bus/msm_subsys/devices"
#define SUBSYS_NAME_FORMAT "%s"
#define RECOVERY_PATH "%s/%s/restart_level"
#define ENABLE_RECOVERY "related"
#define DISABLE_RECOVERY "system"
#endif
...
int populate_subsys_map()
{
	...
	while((de = readdir(dir_ssr_bus))) {
		...
		if ((esoc_name = strstr(subsys_name, "esoc"))) {
			soc_name = get_soc_name(esoc_name);
			if (!soc_name) {
...
int setup_ssr(int num_subsys)
{
	...
	for (i = 0; i < num_subsys; i++)
	{
		for (j = 0; j < ssr_info.count; j++) {
			if (!strstr(ssr_info.subsys_map[j].subsys_name, subsys_list[i]) &&
			    !enable_all)
				continue;

			strlcpy(enable_list[enable_count], ssr_info.subsys_map[j].ssr_node,
				sizeof(enable_list[enable_count]));
			enable_count++;
			match_found = 1;
			if (!enable_all)
				break;
	...
	while((de = readdir(dir_ssr_bus))) {
		...
		if (enable_ssr) {
			...
			if (write(fd, ENABLE_RECOVERY, strlen(ENABLE_RECOVERY)) < 0) {
			...
			if (write(fd, DISABLE_RECOVERY, strlen(DISABLE_RECOVERY)) < 0) {
				ALOGE("Failed to write to ssr node: %s",
						strerror(errno));
				goto error;
			}
...
int main(int argc, char *argv[])
{
	...
	if (!use_prop) {
		for (i = 1; i < argc; i++)
		{
			snprintf(subsys_list[i-1], SUBSYS_NAME_LEN * sizeof(char),
				 SUBSYS_NAME_FORMAT,
				 argv[i]);
			num_subsys++;
		}
		...
		if (!strncmp(subsys_buf, "N/A", sizeof(subsys_buf)) ||
		    !strncmp(subsys_buf, "ALL_DISABLE", sizeof(subsys_buf))) {
			ALOGI("Disabling SSR for all modules");
		...
		if (!strncmp(subsys_buf, "ALL_ENABLE", sizeof(subsys_buf))) {
			ALOGI("Enabling SSR for all modules");
			strlcpy(subsys_list[0], "ALL_ENABLE", SUBSYS_NAME_LEN *
					sizeof(char));
			num_subsys++;
			goto setup;
		}
		...
		while (subsys_name && num_subsys < MAX_NUM_SUBSYS) {
			snprintf(subsys_list[num_subsys], SUBSYS_NAME_LEN * sizeof(char),
				 SUBSYS_NAME_FORMAT,
				 subsys_name);
			num_subsys++;
