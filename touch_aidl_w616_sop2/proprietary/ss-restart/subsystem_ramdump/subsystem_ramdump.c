...
#define UEVENT_SUBSYSTEM_RAMDUMP "subsystem_ramdump:"
#define DEVCOREDUMP_PATH "/sys/class/devcoredump"
#define RPROC_SUFFIX "/failing_device/device/of_node/name"
#define NON_RPROC_SUFFIX "/failing_device/of_node/name"
...
#define LEGACY_DUMP_SETTING "/sys/module/subsystem_restart/parameters/enable_ramdumps"
#define DUMP_SETTING "/sys/module/qcom_ramdump/parameters/enable_dump_collection"
...
/*==========================================================================*/
int generate_ramdump(int index, ramdump_s *dump, const char *tm);
...
int check_folder(char *f_name)
{
	...
	if ((ret = stat(f_name, &st)) != 0)
		ALOGE( "Directory %s does not exist, ret:%d, error:%d", f_name, ret, errno);
...
int create_folder(char *f_name)
{
	...
	if ((ret = mkdir(f_name, S_IRWXU | S_IRWXG | S_IRWXO)) < 0)
		ALOGE("Unable to create %s, ret:%d, error:%d", f_name, ret, errno);
...
int copy_file(char *out_f, char *in_f)
{
	...
	if (out_f == NULL || in_f == NULL) {
		ret = -EINVAL;
		ALOGE("Invalid arguments for copy_file");
		goto skip_copy;
	}
	...
	if (access(in_f, F_OK) != 0) {
		ret = -EINVAL;
		ALOGE("File %s does not exist, error:%d", in_f, errno);
		goto skip_copy;
	}
	...
	if (in < 0) {
		ALOGE("Unable to open %s, ret: %d, error:%d", in_f, in, errno);
		ret = -EIO;
		goto skip_copy;
	}
	...
	if (out < 0) {
		ALOGE("Unable to open %s, ret:%d, error:%d", out_f, out, errno);
		ret = -EIO;
		goto open_error;
	}
	...
	while ((rbytes = read(in, buf, BUFFER_SIZE)) > 0) {
		...
		if (ret < 0) {
			ALOGE("Unable to write to file %s, ret:%d, error:%d", out_f, ret, errno);
			break;
		}
	}
	close(out);
...
static int ssr_copy_dump_based_on_type(char *ramdump_path)
{
	...
	if (dumpFlag == SSR_EVENT_MDM_RAMDUMP) {
		...
		if (ret != 0){
			ALOGE("MDM ramdump failed, error:%d", ret);
			return RET_FAILED;
		}
		else
		...
		if (ret != 0){
			ALOGE("QSC ramdump failed, error:%d", ret);
			return RET_FAILED;
		}
		else
...
static void *read_devcore_dump(void *devcd)
{
	...
	snprintf(file_path2, FILENAME_SIZE, "%s/%s%s", DEVCOREDUMP_PATH, devcd, NON_RPROC_SUFFIX);
	snprintf(file_path3, FILENAME_SIZE, "%s/%s%s", DEVCOREDUMP_PATH, devcd, NON_DT_RPROC_SUFFIX);
	...
	for (size_t i = 0; i < RETRY_COUNT; i++) {
		if (access(file_path1, R_OK) != -1) {
			...
			dev_file = file_path2;
			break;
		} else if (access(file_path3, R_OK) != -1) {
			non_dt_path = 1;
			break;
		}
	...
	if (non_dt_path) {
		ALOGE("found device name in %s\n", file_path3);
		len = readlink(file_path3, link_buf, sizeof(link_buf) - 1);
		if (len >= 0) {
			link_buf[len] = '\0';
		} else {
	...
	snprintf(file_path2, FILENAME_SIZE, "%s/%s%s", DUMP_EMMC_DIR, dev_name, tm);

	/* Read and copy dumps to output folder */
	ret = create_ramdump(fd, file_path2, &rd_fd_closed);
	ALOGE("create_ramdump return value:%d", ret);
close_ramdump:
	if (!rd_fd_closed)
		if (write(fd, "1", 1) < 0)
...
static void *mon_devcore_uevents()
{
	...
	ufd = open_uevent();
	if (ufd < 0) {
		ALOGE("Failed to initialize uevent, ufd:%d", ufd);
		return NULL;
	}
	...
	while (1) {
		...
		if (uevent) {
			...
			ALOGE("devcore_dump: devcoredump device added %s\n", dev);
			ret = pthread_create(&read_dump_hdl, NULL, read_devcore_dump, dev);
			if (ret) {
				ALOGE("devcore_dump: Failed in creating dump thread for device %s, error:%d\n", dev, ret);
				free(dev);
			}
		}
	}
...
int generate_log(char *buf, int len)
{
	...
	if (fd < 0) {
		ALOGE("Unable to open log file %s, error:%d", name_buf, errno);
		ret = -EIO;
		goto open_error;
	}
	...
	if (ret < 0) {
		ALOGE("Unable to write to log file %s, error:%d", name_buf, errno);
		goto write_error;
	}
...
char *ssr_ramdump_filename(int index, ramdump_s *dump, char *name, const char *tm)
{
...
int open_ramdump_fd(ramdump_s *dump)
{
	...
	for (i = 0; i < pfd_num; i++) {
		...
		if ((fd = open(buf, O_RDONLY)) < 0) {
			ALOGE("Unable to open %s, error:%d", buf, errno);
			ret = -EINVAL;
			break;
		}
...
static int ssr_check_ramdump_setting(void)
{
	...
	if ((fd = open(LEGACY_DUMP_SETTING, O_RDONLY)) < 0) {
		ALOGE("Unable to open %s, error:%d", LEGACY_DUMP_SETTING, errno);
		if (errno != ENOENT) {
			ret = fd;
			goto open_error;
		}
		ALOGE("Trying %s", DUMP_SETTING);
		if ((fd = open(DUMP_SETTING, O_RDONLY)) < 0) {
			ALOGE("Unable to open %s, error:%d", DUMP_SETTING, errno);
			ret = fd;
			goto open_error;
		}
	}
	...
	if (read(fd, buf, 2) <= 0) {
		ALOGE("Unable to read %s, error:%d", LEGACY_DUMP_SETTING, errno);
		ret = -EIO;
		goto read_error;
	}
...
static void enable_wlan_ssr_for_le(void)
{
	...
	if ((fd = open(LEGACY_DUMP_SETTING, O_WRONLY)) < 0) {
		ALOGE("Cannot open %s", LEGACY_DUMP_SETTING);
		if (errno != ENOENT)
			return;
		ALOGE("Trying %s", DUMP_SETTING);
...
int main(int argc, char *argv[])
{
	...
	ret = ssr_stop_thread(dev_coredump_hdl, SIGUSR2);
...
int generate_ramdump(int index, ramdump_s *dump, const char *tm)
{
	...
	ssr_ramdump_filename(index, dump, f_name, tm);

	/* create ramdump */
	ret = create_ramdump(pfd[index].fd, f_name, NULL);
