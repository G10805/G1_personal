                                                        ...
                                                        jobject map)
{
    ...
    custom_offset = AIDL_flag? (int32_t)ConfigTagAidl::SECURE_PROCESSOR_CUSTOM_CONFIG_START
    ...
    while (hasNext) {
        ...
        AIDL_flag ? MediaSecureProcessorConfigMapAidl.insert({strKey, Keyvalue}):
                                                      ...
                                                      jstring jtag)
{
    ...
    if (AIDL_flag) {
        ptr = MediaSecureProcessorConfigMapAidl.find(tag);
        SecureProcessorConfigAidl *configBufAidl =
            GetNativeHandleAidl(env, thiz);
        if (configBufAidl == NULL) {
        ...
        if (ptr != MediaSecureProcessorConfigMapAidl.end()) {
    ...
    JNIEnv *env, jint jsessionId, jobject jSessionConfig)
{
    ...
    if (configBuf == nullptr) {
       ALOGE("No configuration backing buffer present to set config");
       return -1;
    }
