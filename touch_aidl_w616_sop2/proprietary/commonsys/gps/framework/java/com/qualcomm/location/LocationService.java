...
/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  Copyright (c) 2013, 2018, 2020-2022 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
public class LocationService extends Service implements Handler.Callback {
    ...
    public void onCreate() {
        ...
        if (LocIDLClientBase.getIDLServiceVersion().compareTo(IDLServiceVersion.V_AIDL) >= 0) {
            ...
            mLocationServiceIDLClient = new LocationServiceHidlClient(this);
        }
        enable();
        ...
        if (agpsConnInfo != null) {
            if (agpsConnInfo.mState == AGpsConnectionInfo.STATE_OPENING) {
                if(state == NETWORK_UNAVAILABLE) {
                    ...
                    mIZatServiceCtx.executeRunnable(getApnInfo);
                }
            }
            else if(DEBUG) {
    ...
    private class AGpsConnectionInfo {
        ...
        private void connect(int connType, int apnTypeMask, int subId) {
            ...
            int subscriptionIds[] = subManager.getSubscriptionIds(subId);

            boolean isSubIdActive;
            int mySid = -1;

            if (null != subscriptionIds) {
                for (int sId : subscriptionIds) {
                    isSubIdActive = subManager.isActiveSubscriptionId(sId);
                    if (DEBUG) Log.d(TAG, "sId: " + sId + " isSubIdActive " + isSubIdActive);
                    if (isSubIdActive) {
                        mySid = sId;
                        break;
                    }
     ...
     *   HIDL Client
     * =================================================*/
    public class LocationServiceHidlClient extends LocationServiceIDLClient
        ...
        private class AGnssCallback extends IAGnssCallback.Stub {
            public void agnssStatusIpV4Cb(
                    ...
                    mLocationServiceProvider.reportAGpsStatus(status.type, APN_TYPE_MASK_SUPL,
                            status.status, LocAidlAGnssSubId.LOC_DEFAULT_SUB);
                }
            ...
            @Override
                public void agnssStatusIpV4Cb(
                    ...
                    mLocationServiceProvider.reportAGpsStatus(status.type,
                            APN_TYPE_MASK_SUPL,status.status, LocAidlAGnssSubId.LOC_DEFAULT_SUB);
                    IDLClientUtils.fromIDLService(TAG);
                }
