...
void ais_aidl_client::copyPayloadData(QCarCamEventPayload_t *pqPayload, const QCarCamEventPayload *pPayload)
{
    int i = 0;
    if (pqPayload) {
        switch (pPayload->getTag())
        {
            ...
            case QCarCamEventPayload::Tag::array:
                ...
                break;
            default:
                AIS_AIDL_ERRORMSG("Unsupported QCarCamEventPayload Tag");
                break;
        }
    }
}
...
::ndk::ScopedAStatus ais_aidl_client::qcarcam_event_callback(int32_t EventType,
        const QCarCamEventPayload& Payload)
{
    ...
    event_id = EventType;
    ais_aidl_client::copyPayloadData(&qPayload, &Payload);
    lock.unlock();
    if (app_cb)
...
// Public methods
QCarCamRet_e ais_aidl_client::ais_client_open_stream(const QCarCamOpen_t* pOpenParams)
{
    QCarCamError Error;
    ...
    if (mpService) {
        ...
        if (ret.isOk()) {
            AIS_AIDL_INFOMSG("openStream success for stream id %u", mStreamId);
            mpStream = Stream;
            Error = QCarCamError::QCARCAM_RET_OK;
        } else {
            Error = QCarCamError::QCARCAM_RET_FAILED;
            AIS_AIDL_ERRORMSG("openStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
            mpService.reset();
        }
    } else {
        AIS_AIDL_ERRORMSG("Service obj already destroyed for stream id %u", mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
    }
    return static_cast<QCarCamRet_e>(Error);
}

QCarCamRet_e ais_aidl_client::ais_client_close_stream()
{
    QCarCamError Error;
    if (true == mStreaming)
    ...
    if (mpStream && mpService) {
        ...
        if (!ret.isOk()) {
            AIS_AIDL_ERRORMSG("Aidl transport layer error, closeStream() failed for stream id %u", mStreamId);
            Error = QCarCamError::QCARCAM_RET_FAILED;
        } else {
            if (QCarCamError::QCARCAM_RET_OK == Error) {
                AIS_AIDL_INFOMSG("closeStream success for stream id %u", mStreamId);
                mpStream.reset();
            }
            else
                AIS_AIDL_ERRORMSG("closeStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
        }
    } else {
        AIS_AIDL_ERRORMSG("Stream obj already destroyed for stream id %u", mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
    }
    return static_cast<QCarCamRet_e>(Error);
}

QCarCamRet_e ais_aidl_client::ais_client_set_cb(QCarCamEventCallback_t cb_ptr, void *pPrivateData)
{
...
QCarCamRet_e ais_aidl_client::ais_client_unset_cb()
{
    QCarCamRet_e ret = QCARCAM_RET_OK;
    app_cb = NULL;
    mpPrivateData = NULL;
...
QCarCamRet_e ais_aidl_client::ais_client_s_param(QCarCamParamType_e param, const void* p_value, uint32_t size)
{
    QCarCamError Error;
    bool setParam = true;
    (void)size;
...
QCarCamRet_e ais_aidl_client::ais_client_g_param(QCarCamParamType_e param, const void* p_value, uint32_t size)
{
    QCarCamError Error;
    bool setParam = false;
    (void)size;
...
QCarCamRet_e ais_aidl_client::ais_client_s_buffer(const QCarCamBufferList_t* p_buffers)
{
    ...
    if (mpStream) {
        ...
        if (!ret.isOk()) {
            AIS_AIDL_ERRORMSG("Aidl transport layer error, setStreamBuffers() failed for stream id %u", mStreamId);
        } else {
            if (QCarCamError::QCARCAM_RET_OK == Error)
                AIS_AIDL_INFOMSG("setStreamBuffers success for stream id %u", mStreamId);
            else
                AIS_AIDL_ERRORMSG("setStreamBuffers Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
        }
    } else {
        AIS_AIDL_ERRORMSG("Stream obj already destroyed for stream id %u", mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
...
QCarCamRet_e ais_aidl_client::ais_client_start_stream(QCarCamHndl_t q_hndl)
{
    QCarCamError Error;
    //Save qcarcam hndl for callback
    mQcarcamHndl = q_hndl;
    AIS_AIDL_DBGMSG("mQcarcamHndl = %p",mQcarcamHndl);

    // Call start stream with current class obj for callback
    if (mpStream) {
        auto ret = mpStream->startStream(ref<ais_aidl_client>(),  &Error);
        if (!ret.isOk()) {
            AIS_AIDL_ERRORMSG("Aidl transport layer error, startStream() failed for stream id %u", mStreamId);
            mStreaming = false;
        } else {
            if (QCarCamError::QCARCAM_RET_OK == Error) {
                AIS_AIDL_INFOMSG("startStream success for stream id %u", mStreamId);
                mStreaming = true;
            } else {
                AIS_AIDL_ERRORMSG("startStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
                mStreaming = false;
            }
        }
    } else {
...
QCarCamRet_e ais_aidl_client::ais_client_stop_stream()
{
    QCarCamError Error;
    std::unique_lock<std::mutex> lock(mLock);
    if (mpStream) {
        ...
        if (!ret.isOk()) {
            AIS_AIDL_ERRORMSG("Aidl transport layer error, stopStream() failed for stream id %u", mStreamId);
        } else {
            if (QCarCamError::QCARCAM_RET_OK == Error) {
                AIS_AIDL_INFOMSG("stopStream success for stream id %u", mStreamId);
                mStreaming = false;
            } else
                AIS_AIDL_ERRORMSG("stopStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
        }
    } else {
        AIS_AIDL_ERRORMSG("Stream obj already destroyed for stream id %u", mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
...
QCarCamRet_e ais_aidl_client::ais_client_get_frame(QCarCamFrameInfo_t* pqFrameInfo,
        unsigned long long int timeout, unsigned int flags)
{
    ...
    if (mpStream) {
        ...
        if (ret.isOk()) {
            AIS_AIDL_INFOMSG("getFrame success for stream id %u id %u", mStreamId, Info.id);
            ...
            AIS_AIDL_ERRORMSG("getFrame Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
        }
    } else {
        AIS_AIDL_ERRORMSG("Stream obj already destroyed for stream id %u", mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
...
QCarCamRet_e ais_aidl_client::ais_client_release_frame(uint32_t id, uint32_t bufferIdx)
{
    QCarCamError Error;
    if (mpStream) {
        auto ret = mpStream->releaseFrame(id, bufferIdx, &Error);
        if (!ret.isOk()) {
            AIS_AIDL_ERRORMSG("Aidl transport layer error, releaseFrame() failed for stream id %u", mStreamId);
        }
        else {
            if (QCarCamError::QCARCAM_RET_OK == Error)
                AIS_AIDL_INFOMSG("releaseFrame success for stream id %u idx %u", mStreamId, bufferIdx);
            else
                AIS_AIDL_ERRORMSG("releaseFrame Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
        }
    } else {
        AIS_AIDL_ERRORMSG("Stream obj already destroyed for stream id %u",mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
...
QCarCamRet_e ais_aidl_client::ais_client_pause_stream()
{
    QCarCamError Error;
    if (mpStream) {
        auto ret = mpStream->pauseStream(&Error);
        if (!ret.isOk()) {
            AIS_AIDL_ERRORMSG("Aidl transport layer error, pauseStream() failed for stream id %u", mStreamId);
        }
        else {
            if (QCarCamError::QCARCAM_RET_OK == Error) {
                AIS_AIDL_INFOMSG("pauseStream success for stream id %u", mStreamId);
                mStreaming = false;
            } else
                AIS_AIDL_ERRORMSG("pauseStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
        }
    } else {
        AIS_AIDL_ERRORMSG("Stream obj already destroyed for stream id %u",mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
...
QCarCamRet_e ais_aidl_client::ais_client_resume_stream()
{
    QCarCamError Error;
    if (mpStream) {
        auto ret = mpStream->resumeStream(&Error);
        if (!ret.isOk()) {
            AIS_AIDL_ERRORMSG("Aidl transport layer error, resumeStream() failed for stream id %u", mStreamId);
            Error = QCarCamError::QCARCAM_RET_FAILED;
        }
        else {
            if (QCarCamError::QCARCAM_RET_OK == Error) {
                AIS_AIDL_INFOMSG("resumeStream success for stream id %u", mStreamId);
                mStreaming = true;
            } else
                AIS_AIDL_ERRORMSG("resumeStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
        }
    } else {
        AIS_AIDL_ERRORMSG("Stream obj already destroyed for stream id %u",mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
...
// Utility functions
QCarCamError ais_aidl_client::config_aidl_params(QCarCamParamType_e qParam,
        ...
        case QCARCAM_STREAM_CONFIG_PARAM_LATENCY_CONTROL:
            {
                ...
                tmp_latencyParam.latencyMax = platencyParam->latencyMax;
                tmp_latencyParam.latencyReduceRate = platencyParam->latencyReduceRate;
                streamConfigData.set<QCarCamStreamConfigData::latencyParam>(tmp_latencyParam);
            }
            break;
        case QCARCAM_STREAM_CONFIG_PARAM_FRAME_DROP_CONTROL:
            {
                ...
                tmp_frameDropConfig.frameDropPeriod  = pframeDropConfig->frameDropPeriod;
                tmp_frameDropConfig.frameDropPattern = pframeDropConfig->frameDropPattern;
                streamConfigData.set<QCarCamStreamConfigData::frameDropConfig>(tmp_frameDropConfig);
            }
            break;
        ...
        case QCARCAM_STREAM_CONFIG_PARAM_BATCH_MODE:
            {
                ...
                tmp_batchConfig.mode = static_cast<QCarCamBatchMode>(pbatchConfig->mode);
                tmp_batchConfig.numBatchFrames = pbatchConfig->numBatchFrames;
                tmp_batchConfig.frameIncrement = pbatchConfig->frameIncrement;
                tmp_batchConfig.detectFirstPhaseTimer = pbatchConfig->detectFirstPhaseTimer;
                streamConfigData.set<QCarCamStreamConfigData::batchConfig>(tmp_batchConfig);
            }
            break;
        case QCARCAM_STREAM_CONFIG_PARAM_ISP_USECASE:
            {
                ...
                tmp_ispConfig.id = pispConfig->id;
                tmp_ispConfig.cameraId = pispConfig->cameraId;
                tmp_ispConfig.usecaseId = static_cast<QCarCamIspUsecase>(pispConfig->usecaseId);
                tmp_ispConfig.tuningMode = pispConfig->tuningMode;
                streamConfigData.set<QCarCamStreamConfigData::ispConfig>(tmp_ispConfig);
            }
            break;
        ...
        case QCARCAM_SENSOR_PARAM_EXPOSURE:
            {
                ...
                tmp_exposureConfig.mode = static_cast<QCarCamExposureMode>(pexposureConfig->mode);
                tmp_exposureConfig.hdrMode = pexposureConfig->hdrMode;
                tmp_exposureConfig.numExposures = pexposureConfig->numExposures;
                for (int i = 0; i < 4; i++) {
                    tmp_exposureConfig.exposureTime[i] = pexposureConfig->exposureTime[i];
                    tmp_exposureConfig.exposureRatio[i] = pexposureConfig->exposureRatio[i];
                    tmp_exposureConfig.gain[i] = pexposureConfig->gain[i];
                }
                streamConfigData.set<QCarCamStreamConfigData::exposureConfig>(tmp_exposureConfig);
            }
            break;
        case QCARCAM_SENSOR_PARAM_GAMMA:
            {
                ...
                tmp_gammaConfig.type = static_cast<QCarCamGammaType>(pgammaConfig->type);
                tmp_gammaConfig.fpValue = pgammaConfig->fpValue;
                tmp_gammaConfig.tableSize = pgammaConfig->tableSize;
                for (int i = 0; i < tmp_gammaConfig.tableSize; i++)
                    tmp_gammaConfig.table[i] = pgammaConfig->table[i];
                streamConfigData.set<QCarCamStreamConfigData::gammaConfig>(tmp_gammaConfig);
            }
            break;
        ...
        case QCARCAM_VENDOR_PARAM:
            {
                ...
                for (int i = 0; i < QCARCAM_MAX_VENDOR_PAYLOAD_SIZE; i++) {
                    tmp_vendorParam.data[i] = pvendorParam->data[i];
                }
                streamConfigData.set<QCarCamStreamConfigData::vendorParam>(tmp_vendorParam);
            }
            break;
        default:
            AIS_AIDL_ERRORMSG("Invalid config param %d", static_cast<int>(qParam));
            Error = QCarCamError::QCARCAM_RET_FAILED;
            break;
    }
    ...
    if (true == setParam) {
        if (QCarCamError::QCARCAM_RET_OK == Error) {
            ...
            if (mpStream) {
                ...
                if (!ret.isOk()) {
                    AIS_AIDL_ERRORMSG("Aidl transport layer error, configureStream() failed for stream id %u", mStreamId);
                } else {
                    if (QCarCamError::QCARCAM_RET_OK == Error)
                        AIS_AIDL_INFOMSG("configureStream success for stream id %u", mStreamId);
                    else
                        AIS_AIDL_ERRORMSG("configureStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
                }
            } else {
                AIS_AIDL_ERRORMSG("Stream obj already destroyed for stream id %u",mStreamId);
                Error = QCarCamError::QCARCAM_RET_FAILED;
        ...
        if (QCarCamError::QCARCAM_RET_OK == Error) {
            ...
            if (mpStream) {
                ...
                if (QCarCamError::QCARCAM_RET_OK == Error) {
                    AIS_AIDL_INFOMSG("getStreamConfig success for stream id %u", mStreamId);
                    // Copy the result from tmpData
                    std::memcpy(&streamConfigData, &tmpData, sizeof(tmpData));
                } else {
                    AIS_AIDL_ERRORMSG("getStreamConfig Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
                }
                if (!ret.isOk()) {
                    AIS_AIDL_ERRORMSG("Aidl transport error; getStreamConfig() failed for stream id %u",mStreamId);
                }
            } else {
                AIS_AIDL_ERRORMSG("Stream obj already destroyed for stream id %u",mStreamId);
                Error = QCarCamError::QCARCAM_RET_FAILED;
