...
static void process_cmds(uint32 option)
{
    ...
    {
        QCARCAM_INFOMSG("Set Gamma Mode");
        stream_id = get_input_stream_id();
        input_ctxt = get_input_ctxt(stream_id);
        if (input_ctxt && input_ctxt->qcarcam_hndl)
        {
            ...
            case 0:
            {
                QCARCAM_INFOMSG("Common Mode Selected");
                printf("Enter Positive Gamma Value: \n");
                if (fgets(buf, sizeof(buf), stdin) != NULL)
                {
                    param_value = strtof(buf, NULL);
                }
                ...
                if (param_value < 0) {
                    QCARCAM_ERRORMSG("Failed: Gamma Value Can't be Negative");
                    printf("Gamma Value Can't be Negative\n");
                    break;
                }
                QCARCAM_PERFMSG(gCtxt.enableStats, "Entered Gamma Value: %f ", param_value);
                param.type = QCARCAM_GAMMA_EXPONENT;
                param.fpValue = param_value;

                ret = QCarCamSetParam(input_ctxt->qcarcam_hndl, QCARCAM_SENSOR_PARAM_GAMMA, &param, sizeof(param));
                if (ret != QCARCAM_RET_OK)
                {
                    QCARCAM_ERRORMSG("Failed: qcarcam_s_param for Stream ID: %d! ret = %d",
                    input_ctxt->qcarcam_input_id, ret);
                }
            ...
            case 1:
            {
                QCARCAM_INFOMSG("User Defiened Mode Selected");
                printf("Enter Gamma Table Length\n");
                if (fgets(buf, sizeof(buf), stdin) != NULL)
                {
                    /* 10 indicates decimal number*/
                    len = strtol(buf, &p, 10);
                ...
                {
                    QCARCAM_ERRORMSG("Failed: Invalid Length  %u (max %u)", len, QCARCAM_MAX_GAMMA_TABLE);
                    break;
                }
                QCARCAM_PERFMSG(gCtxt.enableStats, "Entered Gamma Table Length: %lu ", len);
                printf("Enter Gamma Table File Name:\n");
                ...
                if (NULL == fp)
                {
                    QCARCAM_ERRORMSG("Failed: Open File %s", file_name);
                    break;
                }

                p_value = (uint32_t*)calloc(len, sizeof(*p_value));
                if (NULL == p_value)
                {
                    QCARCAM_ERRORMSG("Failed: Allocate Gamma Table Size: %d", len);
                    fclose(fp);
                    break;
                }
                ...
                if (index != len)
                {
                    QCARCAM_ERRORMSG("Failed: Gamma Table Length Does Not Match");
                    free(p_value);
                    break;
                }

                param.type = QCARCAM_GAMMA_KNEEPOINTS;
                for (int i = 0; i < len; i++)
                    param.table[i] = p_value[i];
                param.tableSize = len;
                ret = QCarCamSetParam(input_ctxt->qcarcam_hndl, QCARCAM_SENSOR_PARAM_GAMMA, &param, sizeof(param));
                if (ret != QCARCAM_RET_OK)
                {
        ...
        if (input_ctxt && input_ctxt->qcarcam_hndl)
        {
            ...
            switch (param_option)
            {
                case 0:
                    ...
                    param.type = QCARCAM_GAMMA_EXPONENT;
                    param.fpValue = 0;
                    ret = QCarCamGetParam(input_ctxt->qcarcam_hndl, QCARCAM_SENSOR_PARAM_GAMMA, &param, sizeof(param));
                    if (ret != QCARCAM_RET_OK)
                    {
                        QCARCAM_ERRORMSG("Failed: qcarcam_g_param for Stream ID %d! ret = %d",
                        input_ctxt->qcarcam_input_id, ret);
                    }
                    else
                    {
                        QCARCAM_PERFMSG(gCtxt.enableStats, "Returned Gamma Value: %f ", param.fpValue);
                ...
                case 1:
                    ...
                    if (fgets(file_name, sizeof(file_name), stdin) == NULL)
                    {
                        QCARCAM_ERRORMSG("Failed: Invalid Gamma Table File Name");
                        printf("Failed: Invalid Gamma Table File Name");
                        break;
                    }
                    ...
                    param.type = QCARCAM_GAMMA_KNEEPOINTS;
                    memset(&param.table[0], 0, sizeof(param.table[0]) * len);
                    param.tableSize = len;
                    ret = QCarCamGetParam(input_ctxt->qcarcam_hndl, QCARCAM_SENSOR_PARAM_GAMMA, &param, sizeof(param));
                    if (ret != QCARCAM_RET_OK)
                    {
                    ...
                    else
                    {
                        ...
                        if (NULL == fp)
                        {
                            QCARCAM_ERRORMSG("Failed: Open File %s :errno=%d", file_name, errno);
                            break;
                        }
...
int main(int argc, char **argv)
{
    ...
    for (int input_idx = 0; input_idx < gCtxt.numInputs; input_idx++)
    {
...
#endif
        {
            ...
            {
                input_ctxt->p_buffers_disp.pBuffers[i].numPlanes = 1;
                input_ctxt->p_buffers_disp.pBuffers[i].planes[0].width = input_ctxt->buffers_param[QCARCAM_TEST_BUFFERS_DISPLAY].width;
                input_ctxt->p_buffers_disp.pBuffers[i].planes[0].height = input_ctxt->buffers_param[QCARCAM_TEST_BUFFERS_DISPLAY].height;
            }
