...
int main(int argc, char** argv)
{
    ...
    sp<EvsVehicleListener> pEvsListener = nullptr;
    if (useVehicleHal) {
    ...
    if (!config.initialize("/system/etc/automotive/evs/config_qcarcam_qcc6.json")) {
    ...
    // Create number of parallel threads equal to number of cameras
    configureRpcThreadpool(config.getCameras().size(), false /* callerWillJoin */);
    ...
    // If displayId is not enumerated, use the default one
    pEvs->getDisplayIdList([&displayId](auto idList) {
      bool match = false;
      for (auto id : idList) {
          if (id == displayId) {
              match = true;
      ...
      if (!match) {
          displayId = idList[0];
          LOG(INFO) << "switch to default display " << displayId;
      }
    });
