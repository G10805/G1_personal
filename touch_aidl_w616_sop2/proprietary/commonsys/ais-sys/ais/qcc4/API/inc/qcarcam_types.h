...
#define QCARCAM_MAX_NUM_MODES  10
...
typedef enum
{
    ...
    QCARCAM_YUV_Y_U_V,
    QCARCAM_YUV_Y_V_U,
    QCARCAM_YUV_NV12 = QCARCAM_YUV_Y_UV,
    QCARCAM_YUV_NV21 = QCARCAM_YUV_Y_VU,
    QCARCAM_YUV_YU12 = QCARCAM_YUV_Y_U_V,
    QCARCAM_YUV_YV12 = QCARCAM_YUV_Y_V_U,
...
typedef enum
{
    ...
    QCARCAM_BITDEPTH_24 = 24,
    QCARCAM_BITDEPTH_32 = 32
...
typedef enum
{
    ...
    ///< YUV Planar Formats
    QCARCAM_FMT_YU12 = QCARCAM_COLOR_FMT(QCARCAM_YUV_YU12, QCARCAM_BITDEPTH_8, QCARCAM_PACK_FOURCC),
    QCARCAM_FMT_YV12 = QCARCAM_COLOR_FMT(QCARCAM_YUV_YV12, QCARCAM_BITDEPTH_8, QCARCAM_PACK_FOURCC),
    ...
    QCARCAM_FMT_P010 = QCARCAM_COLOR_FMT(QCARCAM_YUV_Y_UV, QCARCAM_BITDEPTH_10, QCARCAM_PACK_PLAIN16),

    ///< RGB Formats
    QCARCAM_FMT_RGB_888 = QCARCAM_COLOR_FMT(QCARCAM_RGB_RGB888, QCARCAM_BITDEPTH_24, QCARCAM_PACK_FOURCC),
    QCARCAM_FMT_BGR_888 = QCARCAM_COLOR_FMT(QCARCAM_RGB_BGR888, QCARCAM_BITDEPTH_24, QCARCAM_PACK_FOURCC),
    QCARCAM_FMT_RGB_565 = QCARCAM_COLOR_FMT(QCARCAM_RGB_RGB565, QCARCAM_BITDEPTH_16, QCARCAM_PACK_FOURCC),
    QCARCAM_FMT_RGBX_8888 = QCARCAM_COLOR_FMT(QCARCAM_RGB_RGBX8888, QCARCAM_BITDEPTH_32, QCARCAM_PACK_FOURCC),
    QCARCAM_FMT_BGRX_8888 = QCARCAM_COLOR_FMT(QCARCAM_RGB_BGRX8888, QCARCAM_BITDEPTH_32, QCARCAM_PACK_FOURCC),
    QCARCAM_FMT_RGBX_1010102 = QCARCAM_COLOR_FMT(QCARCAM_RGB_RGBX1010102, QCARCAM_BITDEPTH_32, QCARCAM_PACK_FOURCC),
    QCARCAM_FMT_BGRX_1010102 = QCARCAM_COLOR_FMT(QCARCAM_RGB_BGRX1010102, QCARCAM_BITDEPTH_32, QCARCAM_PACK_FOURCC),
...
/// QCARCAM_PARAM_BRIGHTNESS              |  float_value             |
/// QCARCAM_PARAM_CONTRAST                |  float_value             |
...
/// QCARCAM_PARAM_MASTER                  |  uint_value              | 1 to set and 0 to release
...
/// QCARCAM_PARAM_RECOVERY                |  uint_value              | 1 to enable recovery
/// QCARCAM_PARAM_BATCH_MODE,             |  batch_config            |
/// QCARCAM_PARAM_ISP_USECASE             |  isp_config              |
///
...
typedef enum
{
    ...
    QCARCAM_PARAM_BATCH_MODE,           ///< Configures batch mode through qcarcam_batch_mode_config_t
    QCARCAM_PARAM_ISP_USECASE,          ///< Configures ISP usecase type
    QCARCAM_PARAM_INPUT_COLOR_SPACE,    ///< Sensor supported color space
...
typedef enum
{
    QCARCAM_COLOR_SPACE_UNCORRECTED = 0x0,
    QCARCAM_COLOR_SPACE_SRGB = 0x1,
    QCARCAM_COLOR_SPACE_LRGB = 0x2,
    QCARCAM_COLOR_SPACE_BT601 = 0x3,
    QCARCAM_COLOR_SPACE_BT601_FULL = 0x4,
    QCARCAM_COLOR_SPACE_BT709 = 0x5,
    QCARCAM_COLOR_SPACE_BT709_FULL = 0x6,
}qcarcam_color_space_t;
...
typedef struct
{
    ...
    qcarcam_mode_t modes[QCARCAM_MAX_NUM_MODES]; ///< Array of supported modes
    unsigned int num_modes;                      ///< Number of supported modes
...
/// @brief set of buffers
/// @note  used with qcarcam_s_buffers
typedef struct
{
...
typedef struct
{
    ...
    unsigned int flags;
    unsigned int seq_no;                 ///< sequence number (i.e. frame ID)
...
typedef struct{
    ...
    unsigned int idx; ///< index into the qcarcam_buffers_t buffers table
    unsigned int flags;
    unsigned int seq_no[QCARCAM_MAX_BATCH_FRAMES]; ///< sequence number (i.e. frame ID)
    unsigned long long timestamp;        ///< monotonic timestamp
    unsigned long long timestamp_system; ///< system timestamp
    unsigned long long sof_qtimestamp[QCARCAM_MAX_BATCH_FRAMES]; ///< sof qtimer timestamp
    qcarcam_field_t field_type;
...
typedef struct
{
    unsigned int state;    ///< frame freeze state (e.g. detected, cleared, etc...)
    unsigned int seq_no;   ///< frame id impacted
    unsigned int data[16]; ///< additional data as needed (e.g. cookies, etc...)
...
typedef struct
{
    unsigned long long timestamp_system; ///system SW timestamp
    unsigned long long qtimestamp;       ///sof qtimer HW timestamp
} qcarcam_timestamp_t;
...
typedef struct
{
    ...
    unsigned int frame_increment;   ///< offset in bytes frame N first pixel to frame N+1
    unsigned int detect_first_phase_timer;
}qcarcam_batch_mode_config_t;

/// @brief ISP Control parameters
typedef enum
{
...
///  -------------------------------------|----------------------------------------------
///   QCARCAM_CONTROL_MODE_OFF            | All control by the device's metering
///                                       | and 3A routines is disabled.
///                                       |
///   QCARCAM_CONTROL_MODE_ON             | Use settings for each individual 3A routine.
///                                       |
///   QCARCAM_CONTROL_MODE_USE_SCENE_MODE | Use a specific scene mode.
///                                       | Enabling this disables,
///                                       | (except for FACE_PRIORITY scene mode).
///                                       |
///   QCARCAM_CONTROL_MODE_OFF_KEEP_STATE | Same as OFF mode, except that this
///                                       | capture will not be used by camera device
...
typedef struct
{
    ...
    int weight;       ///< Weight of the region, The weight must be within [0, 1000], and represents a weight for every pixel in the area
...
typedef union
{
    ...
    qcarcam_isp_usecase_config_t isp_config;         ///< isp instance config
    int arr_padding[QCARCAM_MAX_PAYLOAD_SIZE];       ///< Used to ensure union size won't change
    qcarcam_color_space_t color_space;               ///< color space format
