...
static int qcarcam_test_get_frame(qcarcam_test_input_t *input_ctxt)
{
    ...
    else
    {
        if (gCtxt.enable_deinterlace && (input_ctxt->field_type_previous != frame_info.field_type))
        {
            ...
            QCARCAM_ERRORMSG("Field type changed: %d -> %d @frame_%d", input_ctxt->field_type_previous, frame_info.field_type, frame_info.seq_no[0]);
            if (frame_info.field_type == QCARCAM_FIELD_UNKNOWN)
    ...
    QCARCAM_DBGMSG("[%d] frameId:%d (seq_no %u) bufId:%d qtime:%llu", input_ctxt->idx, input_ctxt->frameCnt, frame_info.seq_no[0],
        frame_info.idx, frame_info.sof_qtimestamp[0]);
