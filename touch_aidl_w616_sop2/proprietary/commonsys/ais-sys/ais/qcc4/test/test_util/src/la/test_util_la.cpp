 ...
 *
 * @return qcarcam_ret_t
 */
#if defined(ANDROID_U_AOSP)||defined(ANDROID_V_AOSP)
static qcarcam_ret_t get_display_info(test_util_ctxt_t *ctxt)
{
    int err = 0;
    const auto ids = SurfaceComposerClient::getPhysicalDisplayIds();
    sp<IBinder> displayToken = SurfaceComposerClient::getPhysicalDisplayToken(ids.front());
 ...
 * @return void
 */
static void test_util_fill_planes(qcarcam_buffer_v2_t* pBuffer, qcarcam_color_fmt_t fmt)
{
    switch (fmt)
    {
 ...
 * @return void
 */
static void test_util_fill_native_planes(test_util_window_t *p_window, test_util_buffer_t* pCtxtBuffer, qcarcam_buffer_v2_t* pBuffer)
{
 ...
 * @return qcarcam_ret_t
 */
static qcarcam_ret_t alloc_graphic_buffers(test_util_window_t *p_window, qcarcam_bufferlist_t *buffers)
{
    ...
    for (int i = 0; i < p_window->n_buffers; i++)
    {
        ...
        QCARCAM_DBGMSG("%dx%d 0x%x",
            p_window->width, p_window->height, buffers->color_fmt);
        ...
        if (is_secure)
        {
            allocSize = TESTUTIL_ALIGN(buffers->buffers[i].planes[0].size + buffers->buffers[i].planes[1].size, SIZE_2M);
        }
        else
        {
            allocSize = TESTUTIL_ALIGN(buffers->buffers[i].planes[0].size + buffers->buffers[i].planes[1].size, SIZE_4KB);
        }
        ...
        if (!p_window->gfx_bufs[i])
        {
            QCARCAM_ERRORMSG("Failed to allocate buffer %dx%d (fmt=0x%x) %d",
                p_window->width, p_window->height, buffers->color_fmt, allocSize);
        ...
        buffers->buffers[i].planes[0].hndl = (unsigned int)(p_window->buffers[i].private_hndl->fd);
        ...
        buffers->buffers[i].planes[0].hndl = (unsigned int)get_window_buffer_fd(p_window->buffers[i].window_buf);
        p_window->buffers[i].is_dequeud = 1;
 ...
 * @return qcarcam_ret_t
 */
static qcarcam_ret_t alloc_native_window_buffers(test_util_window_t *p_window, qcarcam_bufferlist_t *buffers)
{
    qcarcam_ret_t rc = QCARCAM_RET_OK;
#if defined(ANDROID_Q_AOSP) || !defined(TESTUTIL_VENDOR_LIB)
    unsigned int is_secure = 0;
...
/// @return QCARCAM_RET_OK if successful
///////////////////////////////////////////////////////////////////////////////
qcarcam_ret_t test_util_init_window_buffers(test_util_ctxt_t *ctxt, test_util_window_t *p_window, qcarcam_bufferlist_t *buffers)
{
    qcarcam_ret_t rc;
