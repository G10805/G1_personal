...
static int PowerEventCb(pm_event_t power_event_id, void* pUsrCtxt) {
    ...
    switch (power_event_id)
    {
        case AIS_PM_SUSPEND:
        {
            LOG(DEBUG) << "App EVS PowerEventCb QCARCAM_PM_SUSPEND ";

            LOG(ERROR) << "Joining the UpdateLoop Thread for Exit and Closing the Camera";
            if (pStateController != nullptr) {
                pStateController->postCommand({EvsStateControl::Op::EXIT, 0, 0}, true);
                pStateController->terminateUpdateLoop();
            }

            LOG(ERROR) << "Closing the Display";
            if (pEvsService != nullptr) {
                pEvsService->closeDisplay(pDisplay);
            }

            LOG(DEBUG) << "Exiting the App for Suspend ";
            std::unique_lock<std::mutex> lk(gAppExitLock);
            gCV.notify_all();
        ...
        case AIS_PM_RESUME:
        {
            LOG(DEBUG) << "App EVS PowerEventCb QCARCAM_PM_RESUME";
            break;
        }
    }
    lock.unlock();

    LOG(DEBUG) << "PowerEventCb " << power_event_id;
    return 0;
}

void PowerEventThread (void *args) {
    LOG(DEBUG) << "PowerEventThread PowerEventInit";
    gPowerService = PowerEventInit(PowerEventCb, args);
}
...
int main(int argc, char** argv) {
    ...
    std::shared_ptr<EvsVehicleListener> pEvsListener = nullptr;
    if (useVehicleHal) {
        // Construct our async helper object
        pEvsListener = std::make_shared<EvsVehicleListener>();
        // when vechicleHal enabled, its expected to run as service
    } else {
    ...
    if (auto status = pEvsService->getDisplayIdList(&idList); !status.isOk())
    {
        LOG(ERROR) << "EVS read available display failed";
    }
    ...
    if (!match) {
        displayId = idList[0];
        LOG(INFO) << "EVS switch to default display " << displayId;
    }
...
#ifdef CAMERA_CPMS_SUPPORT
    gPowerEventThread = std::thread(PowerEventThread, nullptr);
#endif
    ...
    if (useVehicleHal) {
        ...
        LOG(INFO) << "Waiting till signal handler exit";
        std::unique_lock<std::mutex> lk(gAppExitLock);
        gCV.wait(lk);
    }
...
#ifdef CAMERA_CPMS_SUPPORT
    gPowerService.reset();
    if (gPowerEventThread.joinable())
        gPowerEventThread.join();
#endif
