...
#define MAX_CHANNEL_ISP_INSTANCES QCARCAM_MAX_ISP_INSTANCES
#define AIS_ISP_CAMERA_ID_DEFAULT           0xFFFFFFFF
...
typedef enum {
    ...
    CAMERA_HW_BOARD_QDRIVE2P5_PM134,
    CAMERA_HW_BOARD_QDRIVE2P5_PM135,
    CAMERA_HW_BOARD_QDRIVE3P0_PM134,
    CAMERA_HW_BOARD_QDRIVE3P0_PM135,
    CAMERA_HW_BOARD_QDRIVE2P5_PM136,
    CAMERA_HW_BOARD_ADPSTAR_GEN4_PW171,
    CAMERA_HW_BOARD_ADPAIR_GEN4_V1_PW227,
 ...
 *
 * CAMERA_POWER_POST_HIBERNATION - this enum is propagated to Userspace from restore
 * callback in kernel. POWER_SUSPEND event is not received prior to this as userspace will
 * be frozen during hibernation. Upon receiving this, we POWER_RESUME the Soc along with
 * bridge chip and enable I2C forwarding to re-detect and initialize the sensors.
 *
 * CAMERA_POWER_POST_LPM - this enum is propagated to Userspace from resume callback in
 * kernel. Upon receiving this, we POWER_UP the Soc along with bridge chip and enable
 * I2C forwarding to re-detect and initialize the sensors.
 *
 * +----------------------------------+----------+--------+
 * |         PM EVENT                 | Soc/Clks | Bridge |
 * +----------------------------------+----------+--------+
 * | CAMERA_POWER_DOWN                | OFF      | OFF    |
 * | CAMERA_POWER_UP                  | ON       | ON     |
 * | CAMERA_POWER_SUSPEND             | -        | OFF    |
 * | CAMERA_POWER_RESUME              | -        | ON     |
 * | CAMERA_POWER_SOC_CLK_ON          | ON       | -      |
 * | CAMERA_POWER_SOC_CLK_OFF         | OFF      | -      |
 * | CAMERA_POWER_POST_HIBERNATION    | ON       | RESET  |
 * | CAMERA_POWER_POST_LPM            | ON       | RESET  |
 * +----------------------------------+----------+--------+
 */
typedef enum
{
    ...
    CAMERA_POWER_RESUME,
    CAMERA_POWER_SOC_CLK_ON,
    CAMERA_POWER_SOC_CLK_OFF,
    CAMERA_POWER_POST_LPM,
    CAMERA_POWER_POST_HIBERNATION,
 ...
 * 
 * +---------------------------------------+-------------------+-------------------+
 * |              PM Policy                |  SOC Pwr/Clks     |    Bridge Chip    |
 * +---------------------------------------+-------------------+-------------------+
...
/**
 * camera latency measurement modes
 *
 * +--------------------------+-----------------------------------------+
 * |         LM Mode          |       Enabled Latency Measurement       |
 * +--------------------------+-----------------------------------------+
 * | CAMERA_LM_MODE_DISABLE   |                None                     |
 * | CAMERA_LM_MODE_END2END   | Frame done IST to V4L2 application      |
 * | CAMERA_LM_MODE_ALL_STEPS | All Steps of Frame Delivery:            |
 * |                          |   1. Frame done IST to AIS Server       |
 * |                          |   2. AIS Server to AIS BE Server        |
 * |                          |   3. AIS BE Server to AIS V4L2 Proxy    |
 * |                          |   4. AIS V4L2 Proxy to V4L2 Application |
 * +--------------------------+-----------------------------------------+
 */
typedef enum
{
    CAMERA_LM_MODE_DISABLE = 0,
    CAMERA_LM_MODE_END2END,
    CAMERA_LM_MODE_ALL_STEPS,
    CAMERA_LM_MODE_TYPE_MAX
} CameraLatencyMeasurementModeType;
...
typedef struct
{
    ...
    uint32 numIspInstances;
    struct
    {
        uint32 id;
        uint32 cameraId;
        qcarcam_isp_usecase_t useCase;
    }ispInstance[MAX_CHANNEL_ISP_INSTANCES];
...
typedef struct
{
    ...
    qcarcam_buffer_v2_t bufferInfo;
    ...
    int32 mapCount; /**< reference count for mapping */
}AisBuffer;

/**
 * AIS Buffer handle Definition
 */
typedef struct
{
    AisBuffer *base;
