...
void QCarCamSetupUBWCPlanes(QCarCamPlane_t* pPlane, const QCarCamUBWCTileInfo* pTileInfo)
{
    ...
    float local1              = static_cast<float>((static_cast<float>(localStride) / pTileInfo->widthBytes)) / 64;
    uint32_t metadataStride   = static_cast<uint32_t>(ceil(local1)) * 1024;
    float local2              = static_cast<float>((static_cast<float>(pPlane->height) / pTileInfo->height)) / 16;
    uint  localMetaSize       = static_cast<uint32_t>(ceil(local2)) * metadataStride;
 ...
 * @return QCarCamRet_e
 */
#if defined(ANDROID_U_AOSP_ABOVE)
...
static void test_util_fill_planes(QCarCamBuffer_t* pBuffer, QCarCamColorFmt_e fmt)
{
    ...
    case QCARCAM_FMT_NV21:
    {
        ...
        uint32_t width_align = 64;
        uint32_t height_align = 64;
#else
        uint32_t width_align = 128;
        ...
        pBuffer->planes[1].size = pBuffer->planes[1].stride * TESTUTIL_ALIGN(pBuffer->planes[1].height, height_align);
    ...
    {
        pTileInfo = &(QCarCamSupportedUBWCTileInfo[2]);
        pBuffer->numPlanes = 2;

        //plane 2
        pBuffer->planes[1].width = pBuffer->planes[0].width;
        pBuffer->planes[1].height = pBuffer->planes[0].height / 2;
        QCarCamSetupUBWCPlanes(&(pBuffer->planes[0]),pTileInfo);
        QCARCAM_DBGMSG("qcarcam buffer : plane 0 size: %d", pBuffer->planes[0].size);
        QCarCamSetupUBWCPlanes(&(pBuffer->planes[1]),pTileInfo);
        QCARCAM_DBGMSG("qcarcam buffer : plane 1 size: %d", pBuffer->planes[1].size);
        break;
    }
...
static void test_util_fill_native_planes(test_util_window_t *p_window, test_util_buffer_t* pCtxtBuffer, QCarCamBuffer_t* pBuffer)
{
    ...
    case TESTUTIL_FMT_P010:
    {
        ...
        //
        uint32_t width_align = 256;
        uint32_t height0_align = 32;
        uint32_t height1_align = 16;
...
static QCarCamRet_e alloc_graphic_buffers(test_util_window_t *p_window, QCarCamBufferList_t *buffers)
{
    ...
    for (int i = 0; i < p_window->n_buffers; i++)
    {
        ...
        buffers->pBuffers[i].planes[0].bufHndl.bufFd = (uint64_t)(uintptr_t)get_window_buffer_fd(p_window->buffers[i].window_buf);
        p_window->buffers[i].is_dequeud = 1;
...
static QCarCamRet_e alloc_native_window_buffers(test_util_window_t *p_window, QCarCamBufferList_t *buffers)
{
    ...
    if(p_window->format == TESTUTIL_FMT_NV12){
        p_window->width = TESTUTIL_ALIGN(p_window->width, 128);
    }
    ...
    for (int i = 0; i < p_window->n_buffers; i++)
    {
        ...
        QCARCAM_ERRORMSG("%dx%d %d [%d] %d %d fd=%d",
            window_buf->width, window_buf->height,
...
QCarCamRet_e test_util_post_window_buffer(test_util_ctxt_t *ctxt, test_util_window_t *p_window, unsigned int idx,
        std::list<uint32_t>* p_rel_buf_idx, QCarCamInterlaceField_e field_type)
{
...
QCarCamRet_e test_util_dump_window_buffer(test_util_ctxt_t *ctxt, test_util_window_t *p_window, unsigned int idx, const char *filename)
{
    ...
    if (0 != fp)
    {
        ...
        if ((p_window->format == TESTUTIL_FMT_NV12) || (p_window->format == TESTUTIL_FMT_P010)
            || (p_window->format == TESTUTIL_FMT_UBWC_NV12) || (p_window->format == TESTUTIL_FMT_UBWC_TP10))
        {
