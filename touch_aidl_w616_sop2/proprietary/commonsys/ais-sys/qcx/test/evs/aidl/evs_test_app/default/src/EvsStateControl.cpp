...
static unsigned long gMaxTimeToDisplay = 33;
...
bool EvsStateControl::selectStateForCurrentConditions() {
    ...
    if (strncmp(value, "reverse", 7) == 0) {
        ...
        desiredState = LEFT;
    } else if ((strncmp(value, "park", 4) == 0) || sMockGear == int32_t(VehicleGear::GEAR_PARK)) {
        desiredState = PARKING;
    } else {
        //Default state set to reverse if user doesnt input pos or not in parking mode
        desiredState = REVERSE;
    }
#else
