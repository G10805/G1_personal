...
static bool isValidInteger(const std::string& str) {
    ...
    if(ret_val != false) {
        ...
        if (str[0] == '+' || str[0] == '-') {
            if (str.size() == 1) {
                ret_val = false; // A single sign is not valid
            }
            startIndex = 1;
        }
...
bool ConfigManager::initialize(const char* configFileName)
{
    ...
    //
    {
        ...
        for (auto&& node: cameraArray) {
            ...
            int32_t streamId = node.get("streamId", -1).asInt();
            if(streamId == -1)
            {
                if(NULL != cameraId)
                {
                    if((std::strcmp(cameraId, "MISSING") == 0)){
                        printf("cameraId and streamId are not present \n");
                        streamId = 0;
                    }
                    else if(isValidInteger(cameraId)){
                        streamId = std::stoi(cameraId);
                    }
                    else{
                        printf("cameraId is not a valid integer and streamId is not present : %s \n",cameraId);
                        streamId = 0;
                    }
                }
                else{
                    printf("cameraId is NULL \n");
                    streamId = 0;
                }
            }
            ...
            if (function == NULL) {
                printf("CameraInfo function is NULL");
                break;
            }
