...
bool RenderTopView::activate() {
    ...
    mTexAssets.checkerBoard.reset(
            createTextureFromPng("/system/etc/automotive/evs/LabeledChecker_qcarcam_qcx.png"));
    ...
    // Load the car image
    mTexAssets.carTopView.reset(createTextureFromPng("/system/etc/automotive/evs/CarFromTop_qcarcam_qcx.png"));
    ...
    for (auto&& cam : mActiveCameras) {
        cam.tex.reset(
                createVideoTexture(mEnumerator, cam.info.cameraId.c_str(), nullptr, sDisplay,cam.info.streamId));
        if (!cam.tex) {
