...
bool VideoTex::refresh() {
    ...
    if (auto handle = getNativeHandle(mImageBuffer); handle != nullptr) {
        // Drop our device texture image
        if (mKHRimage != EGL_NO_IMAGE_KHR) {
            eglDestroyImageKHR(mDisplay, mKHRimage);
            mKHRimage = EGL_NO_IMAGE_KHR;
    ...
    frame_count++;
    // we caluclate fps for every 30 frames
    if (0 == (frame_count % 30)) {
        gettimeofday(&current_time, NULL);
        ...
        std::string cameraId;
        aidl::android::hardware::automotive::evs::CameraDesc desc;
        if (auto status = mCamera->getCameraInfo(&desc); !status.isOk()) {
            LOG(ERROR) << "Failed to getCameraInfo for fps print";
        }
        else
        {
            cameraId = desc.id;
            double fps = (30.0 * 1000)/diff_msec;
...
VideoTex* createVideoTexture(const std::shared_ptr<IEvsEnumerator>& pEnum, const char* evsCameraId,
                             std::unique_ptr<Stream> streamCfg, EGLDisplay glDisplay, const uint32_t uiStreamId,
