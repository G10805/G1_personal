...
typedef enum
{
    ...
    CAMERA_ALLOCATE_ID_OS_THREAD,
    CAMERA_ALLOCATE_ID_OS_TIMER,
...
/**
 * Enable MPM Timer, can get 1/32 ms resolution if use this timer.
 * 
 * @return CAMERA_SUCCESS -if successful
 */
CAM_API CameraResult CameraEnableMPMTimer(void);

/**
 * Disable MPM Timer.
 */
CAM_API void CameraDisableMPMTimer(void);
 ...
 * @return CAMERA_SUCCESS - if successful
 */
CAM_API CameraResult CameraCreateSignal(void** ppSignal);
 ...
 */
CAM_API CameraResult CameraResetSignal(void* pSignal);
 ...
 *         CAMERA_EFAILED - if failed
 */
CAM_API CameraResult CameraWaitOnSignal(void* pSignal,
 ...
 *         CAMERA_EFAILED - if failed
 */
CAM_API CameraResult CameraSetSignal(void* pSignal);
 ...
 */
CAM_API void CameraDestroySignal(void* pSignal);
...
/*
 * Creates and Starts timer. When the timer expires the registered timer callback handler
 * is invoked
 *
 *
 ...
 * @param[out] ppTimer - timer handle filled on success
 *
 * @return CameraResult
 */
...
/*
 * Starts a timer created using CameraCreateTimer
 *
 * @param[in] pTimer - handle of timer
 * @param[in] nTimeOut - timeout in millseconds
 *
 * @return CameraResult
 */
...
/*
 * Release and free the Timer handle
 *
 * @param[in] pTimer - reference to the timer handle
 *
 * @return CameraResult
 */
