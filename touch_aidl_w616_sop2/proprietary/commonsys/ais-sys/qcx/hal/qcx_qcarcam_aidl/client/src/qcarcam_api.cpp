...
static std::map<unsigned long, std::shared_ptr<qcx_aidl_client>> gMap;
...
{
    (void)p_init_params;
    QCX_AIDL_DBGMSG("Acquiring qcx-aidl Enumerator");
    ...
    if (!pService) {
        QCX_AIDL_ERRORMSG("getService returned NULL");
        return QCARCAM_RET_FAILED;
    }
    bool success = ABinderProcess_setThreadPoolMaxThreadCount(QCARCAM_MAX_STREAMS);
    ABinderProcess_startThreadPool();
    QCX_AIDL_INFOMSG("ABinderProcess_startThreadPool started");
    if (success == false)
    {
        QCX_AIDL_ERRORMSG("ABinderProcess_setThreadPoolMaxThreadCount returned false");
    }
    QCX_AIDL_DBGMSG("Initilaze success");
        ...
        if (result.isOk()) {
            ...
            if (pInputLists) {
                for (auto&& cam: InputLists) {
                    QCX_AIDL_DBGMSG("Found camera %d input streams %d", static_cast<uint32_t>(cam.inputId), InputLists.size());
            ...
            QCX_AIDL_ERRORMSG("getInputStreamList() Failed with Error %d",static_cast<int32_t>(Error));
        }
        if (!result.isOk()) {
            QCX_AIDL_ERRORMSG("Hidl transport error; getInputStreamList() failed");
...
PUBLIC_API QCarCamRet_e QCarCamQueryInputModes(const uint32_t inputId, QCarCamInputModes_t* pInputModes)
{
    ...
    if (QCarCamError::QCARCAM_RET_OK == Error) {
        ...
        QCX_AIDL_ERRORMSG("getInputStreamMode() Failed with Error %d",static_cast<int32_t>(Error));
    }
    if (!result.isOk()) {
        QCX_AIDL_ERRORMSG("Aidl transport error; getInputStreamMode() failed");
        Error = QCarCamError::QCARCAM_RET_FAILED;
    }
    return static_cast<QCarCamRet_e>(Error);
}
...
PUBLIC_API QCarCamRet_e QCarCamOpen(const QCarCamOpen_t* pOpenParams, QCarCamHndl_t* pHndl)
{
    ...
    if(pHndl == NULL)
    {
        QCX_AIDL_ERRORMSG("pHndl is null");
    }
    std::shared_ptr<qcx_aidl_client> clientObj = ndk::SharedRefBase::make<qcx_aidl_client>(pService);
    ret = clientObj->qcx_client_open_stream(pOpenParams);
    if (QCARCAM_RET_OK != ret) {
        QCX_AIDL_ERRORMSG("qcx_client_open_stream failed with ret = %d", ret);
        clientObj.reset();
    } else {
    ...
    *pHndl = (QCarCamHndl_t)clientObj.get();
    QCX_AIDL_ERRORMSG("qcx_client_open_stream handle %x", *pHndl);
...
{
    QCarCamRet_e ret = QCARCAM_RET_OK;
    QCX_AIDL_ERRORMSG("qcx_client_close_stream handle %x", hndl);
    if (gMap[hndl] && gMap[hndl]->mInitialized) {
        ret = gMap[hndl]->qcx_client_close_stream();
        auto client = gMap[hndl];
        gMap.erase(hndl);
        client.reset();
    } else
        ret = QCARCAM_RET_BADPARAM;
    QCX_AIDL_ERRORMSG(" Exit qcx_client_close_stream handle %x", hndl);
    return ret;
}

PUBLIC_API QCarCamRet_e QCarCamGetParam(QCarCamHndl_t hndl, QCarCamParamType_e param, void* pValue, uint32_t size)
{
    QCX_AIDL_ERRORMSG(" Entry qcx_client_getparam handle %x", hndl);
    QCarCamRet_e ret = QCARCAM_RET_OK;
    if (gMap[hndl] && gMap[hndl]->mInitialized)
        ret = gMap[hndl]->qcx_client_g_param(param, pValue, size);
    else
        ret = QCARCAM_RET_BADPARAM;
    QCX_AIDL_ERRORMSG(" Exit qcx_client_getparam handle %x", hndl);
    return ret;
}
...
{
    QCX_AIDL_ERRORMSG(" Entry qcx_client_register_call_back handle %x", hndl);
    QCarCamRet_e ret = QCARCAM_RET_OK;
    if (gMap[hndl] && gMap[hndl]->mInitialized) { 
        ret = gMap[hndl]->qcx_client_set_cb(callbackFunc, pPrivateData);
    }
    else
        ret = QCARCAM_RET_BADPARAM;
    QCX_AIDL_ERRORMSG(" Exit qcx_client_register_call_back handle %x", hndl);
    return ret;
}

PUBLIC_API QCarCamRet_e QCarCamSetParam(QCarCamHndl_t hndl, QCarCamParamType_e param, const void* pValue, uint32_t size)
{
    QCX_AIDL_ERRORMSG(" Entry qcx_client_setparam handle %x", hndl);
    QCarCamRet_e ret = QCARCAM_RET_OK; 
    if (gMap[hndl] && gMap[hndl]->mInitialized) {
        ret = gMap[hndl]->qcx_client_s_param(param, pValue, size);
    }
    else
        ret = QCARCAM_RET_BADPARAM;
    QCX_AIDL_ERRORMSG(" Exit qcx_client_setparam handle %x", hndl);
    return ret;
}

PUBLIC_API QCarCamRet_e QCarCamSetBuffers(QCarCamHndl_t hndl, const QCarCamBufferList_t* pBuffers)
{
    QCX_AIDL_ERRORMSG(" Entry qcx_client_setbuffers handle %x", hndl);
    QCarCamRet_e ret = QCARCAM_RET_OK;   
    if (gMap[hndl] && gMap[hndl]->mInitialized)
        ret = gMap[hndl]->qcx_client_s_buffer(pBuffers);
    else
        ret = QCARCAM_RET_BADPARAM;
    QCX_AIDL_ERRORMSG(" Exit qcx_client_setbuffers handle %x", hndl);
    return ret;
}

PUBLIC_API QCarCamRet_e QCarCamStart(QCarCamHndl_t hndl)
{
    QCX_AIDL_ERRORMSG(" Entry qcx_client_startstream handle %x", hndl);
    QCarCamRet_e ret = QCARCAM_RET_OK;
    if (gMap[hndl] && gMap[hndl]->mInitialized)
        ret = gMap[hndl]->qcx_client_start_stream(hndl);
    else
        ret = QCARCAM_RET_BADPARAM;
    QCX_AIDL_ERRORMSG(" Exit qcx_client_startstream handle %x", hndl);
    return ret;
}

PUBLIC_API QCarCamRet_e QCarCamStop(QCarCamHndl_t hndl)
{
    QCX_AIDL_ERRORMSG(" Entry qcx_client_stopstream handle %x", hndl);
    QCarCamRet_e ret = QCARCAM_RET_OK;  
    if (gMap[hndl] && gMap[hndl]->mInitialized)
        ret = gMap[hndl]->qcx_client_stop_stream();
    else
        ret = QCARCAM_RET_BADPARAM;
     QCX_AIDL_ERRORMSG(" Exit qcx_client_resumestream handle %x", hndl);
    return ret;
}

PUBLIC_API QCarCamRet_e QCarCamPause(QCarCamHndl_t hndl)
{
    QCX_AIDL_ERRORMSG(" Entry qcx_client_pausestream handle %x", hndl);
    QCarCamRet_e ret = QCARCAM_RET_OK;   
    if (gMap[hndl] && gMap[hndl]->mInitialized)
        ret = gMap[hndl]->qcx_client_pause_stream();
    else
        ret = QCARCAM_RET_BADPARAM;
     QCX_AIDL_ERRORMSG(" Exit qcx_client_resumestream handle %x", hndl);
    return ret;
...
{
    QCX_AIDL_ERRORMSG(" Entry qcx_client_resumestream handle %x", hndl);
    QCarCamRet_e ret = QCARCAM_RET_OK;   
    if (gMap[hndl] && gMap[hndl]->mInitialized)
        ret = gMap[hndl]->qcx_client_resume_stream();
    else
        ret = QCARCAM_RET_BADPARAM;
     QCX_AIDL_ERRORMSG(" Exit qcx_client_resumestream handle %x", hndl);
    return ret;
...
{
    QCX_AIDL_ERRORMSG(" Entry qcx_client_reservestream handle %x", hndl);
    QCarCamRet_e ret = QCARCAM_RET_OK;
    if (gMap[hndl] && gMap[hndl]->mInitialized)
        ret = gMap[hndl]->qcx_client_reserve_stream();
    else
        ret = QCARCAM_RET_BADPARAM;
    QCX_AIDL_ERRORMSG(" Exit qcx_client_reservestream handle %x", hndl);
    return ret;
}

PUBLIC_API QCarCamRet_e QCarCamRelease(QCarCamHndl_t hndl)
{
    QCX_AIDL_ERRORMSG(" Entry qcx_client_releasestream handle %x", hndl);
    QCarCamRet_e ret = QCARCAM_RET_OK;  
    if (gMap[hndl] && gMap[hndl]->mInitialized)
        ret = gMap[hndl]->qcx_client_release_stream();
    else
        ret = QCARCAM_RET_BADPARAM;
    QCX_AIDL_ERRORMSG(" Exit qcx_client_releasestream handle %x", hndl);
    return ret;
}
...
{
    QCX_AIDL_ERRORMSG(" Entry qcx_client_getframe handle %x", hndl);
    QCarCamRet_e ret = QCARCAM_RET_OK;  
    if (gMap[hndl] && gMap[hndl]->mInitialized)
        ret = gMap[hndl]->qcx_client_get_frame(pFrameInfo, timeout, flags);
    else
        ret = QCARCAM_RET_BADPARAM;
    QCX_AIDL_ERRORMSG(" Exit qcx_client_getframe handle %x", hndl);
    return ret;
}
...
PUBLIC_API QCarCamRet_e QCarCamReleaseFrame(QCarCamHndl_t hndl, uint32_t id, uint32_t bufferIdx)
{
    ...
    QCarCamRet_e ret = QCARCAM_RET_OK;    
    if (gMap[hndl] && gMap[hndl]->mInitialized)
        ret = gMap[hndl]->qcx_client_release_frame(id, bufferIdx);
    else
        ret = QCARCAM_RET_BADPARAM;
    QCX_AIDL_ERRORMSG(" Exit qcx_client_releasestream handle %x", hndl);
    return ret;
...
{
    QCX_AIDL_ERRORMSG(" Entry qcx_client_unregister handle %x", hndl);
    QCarCamRet_e ret = QCARCAM_RET_OK;  
    if (gMap[hndl] && gMap[hndl]->mInitialized) {
        ret = gMap[hndl]->qcx_client_unset_cb();
    }
    else
        ret = QCARCAM_RET_BADPARAM;
    QCX_AIDL_ERRORMSG(" Exit qcx_client_unregister handle %x", hndl);
    return ret;
}

PUBLIC_API QCarCamRet_e QCarCamSubmitRequest(const QCarCamHndl_t hndl, const QCarCamRequest_t* pqRequest)
{
    QCX_AIDL_DBGMSG(" Entry qcx_client_submit requeststream handle %x", hndl);
    QCarCamRet_e ret = QCARCAM_RET_OK;
    if (gMap[hndl] && gMap[hndl]->mInitialized)
        ret = gMap[hndl]->qcx_client_submitRequest(pqRequest);
    else
        ret = QCARCAM_RET_BADPARAM;
    QCX_AIDL_DBGMSG(" Exit qcx_client_submit requeststream handle %x", hndl);
    return ret;
}
