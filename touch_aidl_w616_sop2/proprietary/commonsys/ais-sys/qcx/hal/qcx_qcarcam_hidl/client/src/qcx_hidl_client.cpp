...
qcx_hidl_client::qcx_hidl_client(android::sp<IQcarCamera> service): mpService(service)
{
    mInitialized = false;
    mStreaming = false;
}
qcx_hidl_client::~qcx_hidl_client()
{
    QCX_HIDL_DBGMSG("Destructor called for qcx_hidl_client hndl %p",mQcarcamHndl);
    mInitialized = false;
    mStreaming = false;
}

void qcx_hidl_client::copyPlayloadData(QCarCamEventPayload_t *pqPayload, const QCarCamEventPayload *pPayload)
{
    int i=0;
...
Return<void> qcx_hidl_client::qcarcam_event_callback(uint32_t EventType,
        const QCarCamEventPayload& Payload)
{
    std::unique_lock<std::mutex> lock(mLock);
    QCX_HIDL_DBGMSG("Received qcarcam_event_callback");
    uint32_t event_id;
    QCarCamEventPayload_t qPayload;
    event_id = EventType;
    qcx_hidl_client::copyPlayloadData(&qPayload, &Payload);
    lock.unlock();
    if (app_cb)
...
// Public methods
QCarCamRet_e qcx_hidl_client::qcx_client_open_stream(const QCarCamOpen_t* pOpenParams)
{
    QCarCamError Error;
    ...
    if (mpService) {
        auto ret = mpService->openStream(openParams, [&](const auto& Stream, const auto& tmpError) {
                ...
                if (QCarCamError::QCARCAM_RET_OK == tmpError) {
                    QCX_HIDL_INFOMSG("openStream success for stream id %u", mStreamId);
                    mpStream = Stream;
                } else {
                    QCX_HIDL_ERRORMSG("openStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(tmpError));
                }
                } );
        if (!ret.isOk()) {
            QCX_HIDL_ERRORMSG("Hidl transport error, openStream() failed for stream id %u", mStreamId);
            Error = QCarCamError::QCARCAM_RET_FAILED;
            mpService = nullptr;
        }
    } else {
        QCX_HIDL_ERRORMSG("Service obj already destroyed for stream id %u", mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
    }
...
QCarCamRet_e qcx_hidl_client::qcx_client_abort_due_to_death()
{
    QCX_HIDL_ERRORMSG("Sending BROKEN_CONN");
    uint32_t event_id;
    QCarCamEventPayload_t qPayload;
    event_id = QCARCAM_EVENT_ERROR;
    qPayload.errInfo.errorId = QCARCAM_ERROR_BROKEN_CONN;
    if (app_cb)
        app_cb(mQcarcamHndl, event_id, &qPayload, mpPrivateData);
    return QCARCAM_RET_OK;
}

QCarCamRet_e qcx_hidl_client::qcx_client_close_stream()
{
    QCarCamError Error;
    ...
    if (mpStream && mpService) {
        ...
        if (!ret.isOk()) {
            QCX_HIDL_ERRORMSG("Hidl transport layer error, closeStream() failed for stream id %u", mStreamId);
            Error = QCarCamError::QCARCAM_RET_FAILED;
        } else {
            Error = ret;
            if (QCarCamError::QCARCAM_RET_OK == Error) {
                QCX_HIDL_INFOMSG("closeStream success for stream id %u", mStreamId);
                mpStream = nullptr;
            }
            else
                QCX_HIDL_ERRORMSG("closeStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
        }
    } else {
        QCX_HIDL_ERRORMSG("Stream obj already destroyed for stream id %u", mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
    }
...
QCarCamRet_e qcx_hidl_client::qcx_client_set_cb(QCarCamEventCallback_t cb_ptr, void *pPrivateData)
{
...
QCarCamRet_e qcx_hidl_client::qcx_client_unset_cb()
{
    QCarCamRet_e ret = QCARCAM_RET_OK;
    app_cb = NULL;
    mpPrivateData = NULL;
...
QCarCamRet_e qcx_hidl_client::qcx_client_s_param(QCarCamParamType_e param, const void* p_value, uint32_t size)
{
    QCarCamError Error;
    bool setParam = true;
    (void)size;
...
QCarCamRet_e qcx_hidl_client::qcx_client_g_param(QCarCamParamType_e param, const void* p_value, uint32_t size)
{
    QCarCamError Error;
    bool setParam = false;
    (void)size;
...
QCarCamRet_e qcx_hidl_client::qcx_client_s_buffer(const QCarCamBufferList_t* p_buffers)
{
    ...
    if (mNativeHndl == NULL) {
        QCX_HIDL_ERRORMSG("Native handle is null!! sending error status");
        Error = QCarCamError::QCARCAM_RET_FAILED;
        return static_cast<QCarCamRet_e>(Error);
    }
    ...
    if (mpStream) {
        ...
        if (!ret.isOk()) {
            QCX_HIDL_ERRORMSG("Hidl transport layer error, setStreamBuffers() failed for stream id %u", mStreamId);
            Error = QCarCamError::QCARCAM_RET_FAILED;
        } else {
            Error = ret;
            if (QCarCamError::QCARCAM_RET_OK == Error)
                QCX_HIDL_INFOMSG("setStreamBuffers success for stream id %u", mStreamId);
            else
                QCX_HIDL_ERRORMSG("setStreamBuffers Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
        }
    } else {
        QCX_HIDL_ERRORMSG("Stream obj already destroyed for stream id %u", mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
...
QCarCamRet_e qcx_hidl_client::qcx_client_start_stream(QCarCamHndl_t q_hndl)
{
    QCarCamError Error;
    ...
    if (mpStream) {
        ...
        if (!ret.isOk()) {
            QCX_HIDL_ERRORMSG("Hidl transport layer error, startStream() failed for stream id %u", mStreamId);
            Error = QCarCamError::QCARCAM_RET_FAILED;
            mStreaming = false;
        } else {
            Error = ret;
            if (QCarCamError::QCARCAM_RET_OK == Error) {
                QCX_HIDL_INFOMSG("startStream success for stream id %u", mStreamId);
                mStreaming = true;
            } else {
                QCX_HIDL_ERRORMSG("startStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
                mStreaming = false;
            }
        }
    } else {
...
QCarCamRet_e qcx_hidl_client::qcx_client_stop_stream()
{
    QCarCamError Error;
    std::unique_lock<std::mutex> lock(mLock);
    ...
    if (mpStream) {
        ...
        if (!ret.isOk()) {
            QCX_HIDL_ERRORMSG("Hidl transport layer error, stopStream() failed for stream id %u", mStreamId);
            Error = QCarCamError::QCARCAM_RET_FAILED;
        } else {
            Error = ret;
            if (QCarCamError::QCARCAM_RET_OK == Error) {
                QCX_HIDL_INFOMSG("stopStream success for stream id %u", mStreamId);
                mStreaming = false;
            } else
                QCX_HIDL_ERRORMSG("stopStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
        }
    } else {
        QCX_HIDL_ERRORMSG("Stream obj already destroyed for stream id %u", mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
...
QCarCamRet_e qcx_hidl_client::qcx_client_get_frame(QCarCamFrameInfo_t* pqFrameInfo,
        unsigned long long int timeout, unsigned int flags)
{
    ...
    if (mpStream) {
        auto ret = mpStream->getFrame(timeout, flags, [&](const auto& tmpError, const auto& Info) {
                Error = tmpError;
                if (QCarCamError::QCARCAM_RET_OK == tmpError) {
                QCX_HIDL_INFOMSG("getFrame success for stream id %u id %u", mStreamId, Info.id);
                ...
                for (int i=0; i<QCARCAM_MAX_BATCH_FRAMES; i++) {
                    ...
                    QCX_HIDL_ERRORMSG("getFrame Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(tmpError));
                }
        } );
        if (!ret.isOk()) {
            QCX_HIDL_ERRORMSG("Hidl transport layer error, getFrame() failed for stream id %u", mStreamId);
            Error = QCarCamError::QCARCAM_RET_FAILED;
        }
    } else {
        QCX_HIDL_ERRORMSG("Stream obj already destroyed for stream id %u", mStreamId);
...
QCarCamRet_e qcx_hidl_client::qcx_client_release_frame(uint32_t id, uint32_t bufferIdx)
{
    QCarCamError Error;
    ...
    if (mpStream) {
        ...
        if (!ret.isOk()) {
            QCX_HIDL_ERRORMSG("Hidl transport layer error, releaseFrame() failed for stream id %u", mStreamId);
            Error = QCarCamError::QCARCAM_RET_FAILED;
        }
        else {
            Error = ret;
            if (QCarCamError::QCARCAM_RET_OK == Error)
                QCX_HIDL_INFOMSG("releaseFrame success for stream id %u idx %u", mStreamId, bufferIdx);
            else
                QCX_HIDL_ERRORMSG("releaseFrame Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
        }
    } else {
        QCX_HIDL_ERRORMSG("Stream obj already destroyed for stream id %u",mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
...
QCarCamRet_e qcx_hidl_client::qcx_client_pause_stream()
{
    QCarCamError Error;
    ...
    if (mpStream) {
        ...
        if (!ret.isOk()) {
            QCX_HIDL_ERRORMSG("Hidl transport layer error, pauseStream() failed for stream id %u", mStreamId);
            Error = QCarCamError::QCARCAM_RET_FAILED;
        }
        else {
            Error = ret;
            if (QCarCamError::QCARCAM_RET_OK == Error) {
                QCX_HIDL_INFOMSG("pauseStream success for stream id %u", mStreamId);
                mStreaming = false;
            } else
                QCX_HIDL_ERRORMSG("pauseStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
        }
    } else {
        QCX_HIDL_ERRORMSG("Stream obj already destroyed for stream id %u",mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
...
QCarCamRet_e qcx_hidl_client::qcx_client_resume_stream()
{
    QCarCamError Error;
    ...
    if (mpStream) {
        ...
        if (!ret.isOk()) {
            QCX_HIDL_ERRORMSG("Hidl transport layer error, resumeStream() failed for stream id %u", mStreamId);
            Error = QCarCamError::QCARCAM_RET_FAILED;
        }
        else {
            Error = ret;
            if (QCarCamError::QCARCAM_RET_OK == Error) {
                QCX_HIDL_INFOMSG("resumeStream success for stream id %u", mStreamId);
                mStreaming = true;
            } else
                QCX_HIDL_ERRORMSG("resumeStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
        }
    } else {
        QCX_HIDL_ERRORMSG("Stream obj already destroyed for stream id %u",mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
...
QCarCamRet_e qcx_hidl_client::qcx_client_reserve_stream()
{
    QCarCamError Error;

    if (mpStream) {
        auto ret = mpStream->reserveStream();
        if (!ret.isOk()) {
            QCX_HIDL_ERRORMSG("Hidl transport layer error, reserveStream() failed for stream id %u", mStreamId);
            Error = QCarCamError::QCARCAM_RET_FAILED;
        }
        else {
            Error = ret;
            if (QCarCamError::QCARCAM_RET_OK == Error) {
                QCX_HIDL_INFOMSG("reserveStream success for stream id %u", mStreamId);
                mStreaming = true;
            } else
                QCX_HIDL_ERRORMSG("reserveStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
        }
    } else {
        QCX_HIDL_ERRORMSG("Stream obj already destroyed for stream id %u",mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
...
QCarCamRet_e qcx_hidl_client::qcx_client_release_stream()
{
    QCarCamError Error;

    if (mpStream) {
        auto ret = mpStream->releaseStream();
        if (!ret.isOk()) {
            QCX_HIDL_ERRORMSG("Hidl transport layer error, releaseStream() failed for stream id %u", mStreamId);
            Error = QCarCamError::QCARCAM_RET_FAILED;
        }
        else {
            Error = ret;
            if (QCarCamError::QCARCAM_RET_OK == Error) {
                QCX_HIDL_INFOMSG("releaseStream success for stream id %u", mStreamId);
                mStreaming = false;
            } else
                QCX_HIDL_ERRORMSG("releaseStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
        }
    } else {
        QCX_HIDL_ERRORMSG("Stream obj already destroyed for stream id %u",mStreamId);
        Error = QCarCamError::QCARCAM_RET_FAILED;
...
// Utility functions
QCarCamError qcx_hidl_client::config_hidl_params(QCarCamParamType_e qParam,
        const void *pqConfigData, bool setParam)
{
    ...
    switch (qParam)
    {
        ...
        case QCARCAM_VENDOR_PARAM:
            ...
            break;
        default:
            QCX_HIDL_ERRORMSG("Invalid config param %d", static_cast<int>(qParam));
            Error = QCarCamError::QCARCAM_RET_FAILED;
            break;
    }
    ...
    if (true == setParam) {
        if (QCarCamError::QCARCAM_RET_OK == Error) {
            ...
            if (mpStream) {
                ...
                if (!ret.isOk()) {
                    QCX_HIDL_ERRORMSG("Hidl transport layer error, configureStream() failed for stream id %u", mStreamId);
                    Error = QCarCamError::QCARCAM_RET_FAILED;
                } else {
                    Error = ret;
                    if (QCarCamError::QCARCAM_RET_OK == Error)
                        QCX_HIDL_INFOMSG("configureStream success for stream id %u", mStreamId);
                    else
                        QCX_HIDL_ERRORMSG("configureStream Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(Error));
                }
            } else {
                QCX_HIDL_ERRORMSG("Stream obj already destroyed for stream id %u",mStreamId);
                Error = QCarCamError::QCARCAM_RET_FAILED;
        ...
        if (QCarCamError::QCARCAM_RET_OK == Error) {
            ...
            if (mpStream) {
                auto ret = mpStream->getStreamConfig(hidlParamType, streamConfigData, [&](const auto& tmpError, const auto& tmpData) {
                        Error = tmpError;
                        if (QCarCamError::QCARCAM_RET_OK == tmpError) {
                        QCX_HIDL_INFOMSG("getStreamConfig success for stream id %u", mStreamId);
                        // Copy the result from tmpData
                        std::memcpy(&streamConfigData, &tmpData, sizeof(tmpData));
                        } else {
                        QCX_HIDL_ERRORMSG("getStreamConfig Failed for stream id %u with Error %d", mStreamId, static_cast<int32_t>(tmpError));
                        }
                        } );
                if (!ret.isOk()) {
                    QCX_HIDL_ERRORMSG("Hidl transport error; getStreamConfig() failed for stream id %u",mStreamId);
                    Error = QCarCamError::QCARCAM_RET_FAILED;
                }
            } else {
                QCX_HIDL_ERRORMSG("Stream obj already destroyed for stream id %u",mStreamId);
