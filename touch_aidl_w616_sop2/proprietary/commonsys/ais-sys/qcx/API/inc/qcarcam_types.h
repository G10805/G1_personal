...
#define QCARCAM_OPEN_FLAGS_REQUEST_MODE 1U << 2 /**< Request based mode of operation */
#define QCARCAM_OPEN_FLAGS_AVG_FPS_MONITOR_MODE 1U << 3 /**< Enables average frame count based
...
/** @brief Buffer list ID range identifiers
 *  The bufferlist ID is comprised of 2 parts:
 *  1. the type of bufferlist which is in the TYPE bits
...
#define QCARCAM_BUFFERLIST_ID_TYPE_MASK                 (QCARCAM_BUFFERLIST_TYPE_MAX    << QCARCAM_BUFFERLIST_ID_TYPE_SHIFT)
#define QCARCAM_BUFFERLIST_ID_OUTPUT_RANGE              (QCARCAM_BUFFERLIST_TYPE_OUTPUT << QCARCAM_BUFFERLIST_ID_TYPE_SHIFT)
#define QCARCAM_BUFFERLIST_ID_INPUT_RANGE               (QCARCAM_BUFFERLIST_TYPE_INPUT  << QCARCAM_BUFFERLIST_ID_TYPE_SHIFT)
#define QCARCAM_BUFFERLIST_ID_INPUT_METADATA_RANGE      (QCARCAM_BUFFERLIST_TYPE_INPUT_METADATA  << QCARCAM_BUFFERLIST_ID_TYPE_SHIFT)
#define QCARCAM_BUFFERLIST_ID_OUTPUT_METADATA_RANGE     (QCARCAM_BUFFERLIST_TYPE_OUTPUT_METADATA << QCARCAM_BUFFERLIST_ID_TYPE_SHIFT)
#define QCARCAM_BUFFERLIST_ID_INPUT_OUTPUT_RANGE        (QCARCAM_BUFFERLIST_TYPE_INPUT_OUTPUT    < QCARCAM_BUFFERLIST_ID_TYPE_SHIFT)

/** macros to retrieve bufferlist type from bufferlist id */
#define QCARCAM_GET_BUFFERLIST_TYPE(_id_) (((_id_) & QCARCAM_BUFFERLIST_ID_TYPE_MASK) >> QCARCAM_BUFFERLIST_ID_TYPE_SHIFT)

/* macro to retrieve bufferlist index within type from bufferlist id */
...
#define QCARCAM_BUFFER_FLAG_UBWC_LOSSY  (1U << 3)


/** @brief Invalid QCarCam handle type. */
#define QCARCAM_HNDL_INVALID ((QCarCamHndl_t)0)

/** @brief Buffer status type definition. */
#define QCARCAM_BUFFER_STATUS_INVALID   (1U << 0)
#define QCARCAM_BUFFER_STATUS_VALID     (1U << 1)


/** @brief QCarCamRequest_t flag to indicate injection request */
#define QCARCAM_REQUEST_FLAG_INJECTION  (1U << 0)

/** QCarCamGetParamEx_t and QCarCamSetParamEx_t flags */
#define QCARCAM_PARAM_EX_FLAG_INPUT   (0U)          /** @brief indicates parameter is for specific input */
#define QCARCAM_PARAM_EX_FLAG_OUTPUT  (1U << 0)     /** @brief indicates parameter is for specific output */
...
typedef enum
{
    ...
    QCARCAM_RET_BIST_FAILURE,      /**< API return type bist failur. */
...
typedef enum
{
    ...
    QCARCAM_RGB_R8_G8_B8,       /**< Type for Planar RGB with 8bit per channel */
...
typedef enum
{
    ...
    QCARCAM_PACK_PLAIN16,           /**< Type for Plain 16 color pack into the lowest significant bits */
    QCARCAM_PACK_PLAIN32,           /**< Type for Plain 32 color pack. */
    QCARCAM_PACK_UBWC,              /**< Type for UBWC color pack.  */
    QCARCAM_PACK_PLAIN16_MSB,       /**< Type for Plain 16 color pack into the most significant bits */
...
typedef enum
{
    ...
    QCARCAM_FMT_Y8 = QCARCAM_COLOR_FMT(QCARCAM_YUV_Y,
                                       QCARCAM_BITDEPTH_8,
                                       QCARCAM_PACK_QTI),       /**< QTI packed YUV_Y 8bit */

    QCARCAM_FMT_Y10 = QCARCAM_COLOR_FMT(QCARCAM_YUV_Y,
                                     QCARCAM_BITDEPTH_10,
                                     QCARCAM_PACK_PLAIN16_MSB),  /**< QTI packed YUV_Y 10bit */

    QCARCAM_FMT_Y10LSB = QCARCAM_COLOR_FMT(QCARCAM_YUV_Y,
                                     QCARCAM_BITDEPTH_10,
                                     QCARCAM_PACK_PLAIN16),  /**< QTI packed YUV_Y 10bit LSB*/
    ...
    QCARCAM_FMT_P010 = QCARCAM_COLOR_FMT(QCARCAM_YUV_Y_UV,
                                         QCARCAM_BITDEPTH_10,
                                         QCARCAM_PACK_PLAIN16_MSB),     /**< Plain16 packed Y_UV 10bit */

    QCARCAM_FMT_P010LSB = QCARCAM_COLOR_FMT(QCARCAM_YUV_Y_UV,
                                         QCARCAM_BITDEPTH_10,
                                         QCARCAM_PACK_PLAIN16),     /**< Plain16 packed Y_UV 10bit LSB*/
   ...
   QCARCAM_FMT_P01208 = QCARCAM_COLOR_FMT(QCARCAM_YUV_Y12_UV8,
    ...
    QCARCAM_FMT_UBWC_NV12 = QCARCAM_COLOR_FMT(QCARCAM_YUV_NV12,
                                              QCARCAM_BITDEPTH_8,
                                              QCARCAM_PACK_UBWC),      /**< UBWC packed YUV_NV12 8bit */

    QCARCAM_FMT_UBWC_TP10 = QCARCAM_COLOR_FMT(QCARCAM_YUV_Y_UV,
                                              QCARCAM_BITDEPTH_10,
                                              QCARCAM_PACK_UBWC),      /**< UBWC packed YUV_TP10 10bit */
 ...
 * @verbatim
  ...
  QCARCAM_STREAM_CONFIG_PARAM_FRAME_RATE_MONITOR_CONTROL    |  QCarCamFrameRateMonitoringConfig_t |
  ----------------------------------------------------------|-------------------------------------|
...
typedef enum
{
    ...
    QCARCAM_STREAM_CONFIG_PARAM_FRAME_RATE_MONITOR_CONTROL,      /**< frame monitoring control. */
...
typedef enum
{
    QCARCAM_BUFFERLIST_ID_OUTPUT_START          = QCARCAM_BUFFERLIST_ID_OUTPUT_RANGE,  /**< output buffer lists start at this index */
    QCARCAM_BUFFERLIST_ID_OUTPUT_0              = QCARCAM_BUFFERLIST_ID_OUTPUT_START,
    QCARCAM_BUFFERLIST_ID_INPUT_START           = QCARCAM_BUFFERLIST_ID_INPUT_RANGE,  /**< input buffer lists for injection start at this index */
    QCARCAM_BUFFERLIST_ID_INPUT_0               = QCARCAM_BUFFERLIST_ID_INPUT_START,
    QCARCAM_BUFFERLIST_ID_INPUT_METADATA_START  = QCARCAM_BUFFERLIST_ID_INPUT_METADATA_RANGE,  /**< input metadata bufferlists start at this index */
    QCARCAM_BUFFERLIST_ID_INPUT_METADATA        = QCARCAM_BUFFERLIST_ID_INPUT_METADATA_START,
    QCARCAM_BUFFERLIST_ID_OUTPUT_METADATA_START = QCARCAM_BUFFERLIST_ID_OUTPUT_METADATA_RANGE, /**< output metadata bufferlists start at this index */
    QCARCAM_BUFFERLIST_ID_OUTPUT_METADATA       = QCARCAM_BUFFERLIST_ID_OUTPUT_METADATA_START,
    QCARCAM_BUFFERLIST_ID_INPUT_OUTPUT_START    = QCARCAM_BUFFERLIST_ID_INPUT_OUTPUT_RANGE,   /**< input/output buffer lists start at this index */
...
typedef struct
{
    ...
    uint32_t bufferIdx;    /**< buffer index within the buffer list. QCARCAM_BUFFERLIST_ID_OUTPUT or
                                QCARCAM_BUFFERLIST_ID_INPUT_OUTPUT_0 ranges */

    uint32_t metaBufferlistId; /**< metadata buffer list. Must be in QCARCAM_BUFFERLIST_ID_INPUT_METADATA range*/
    uint32_t metaBufferId;
...
typedef struct
{
    ...
    uint32_t flags; /**< bitmask of QCARCAM_REQUEST_FLAG_ .  For injection */
...
#if defined(LINUX_LRH) || defined(__ANDROID__)
/** @brief DMA Buffer handle, same as PLMBufHndl_t */
typedef struct
{
   int32_t    bufFd;     /**< Buffer/Memory handle - fd */
   uint32_t   size;      /**< Buffer size */
   pid_t      pid;       /**< Buffer allocator's process ID */
   void*      pVA;
} BufHndl_t;
#endif
