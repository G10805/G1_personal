...
Copyright (c)2007-2012, 2014-2024 Qualcomm Technologies, Inc.
All Rights Reserved.
...
static void parse_args(int argc, char **argv)
{
	...
	while ((command = getopt_long(argc, argv, "f:m:l:o:p:s:w:n:t:x:y:g:q:j:cdkebrahiu", longopts, NULL))
			!= -1) {
		...
		switch (command) {
			...
			case 'e':
				...
				break;
			case 'f':
				strlcpy(mask_file_proc[MSM], optarg, FILE_NAME_LEN);
				mask_file_entered = 1;
				break;
			case 'g':
			...
			case 'x':
				...
				break;
			case 'y':
				etr_buffer_size = to_integer(optarg);
...
static void stop_mdlog(int pid, int kill_count)
{
	...
	if (kill_count > 1) {
		if (pid == 0 || tgkill(pid, pid, SIGKILL) < 0) {
			DIAG_LOGE("\ndiag_mdlog: Unable to SIGKILL diag_mdlog instance pid: %d, errno: %d\n", pid, errno);
		} else {
...
#ifndef FEATURE_LE_DIAG
static void adjust_permissions(void)
{
	int status;
	int size;
 ...
 */
static int diag_mdlog_pid_init(void)
{
	int fd;
	int pid;
	int ret;
...
static void diag_mdlog_configure_peripheral_buffering_tx_mode(void)
{
	int ret = 0;
