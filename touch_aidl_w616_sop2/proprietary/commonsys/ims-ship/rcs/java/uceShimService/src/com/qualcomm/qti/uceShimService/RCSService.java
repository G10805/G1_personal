...
public class RCSService extends Service {
   ...
   private HashSet<String> iccIdActivatedOnModem = new HashSet<>();
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(INTENT_SIM_STATE_CHANGED_STR)||action.equals(INTENT_SIM_APPLICATION_STATE_CHANGED_STR)) {
                ...
                if (isSimInserted) {
                    ...
                    if(subMgr != null)
                        subInfo = subMgr.getActiveSubscriptionInfoForSimSlotIndex(slotIdx);

                    if(subInfo != null)
                        iccId = subInfo.getIccId();

                    if(iccIdActivatedOnModem.contains(iccId) && !isSimRcsEnabled[slotIdx]) {
                        isSimRcsEnabled[slotIdx] = true;
                        sendServiceUpIndication();
                    } else {
                        Log.d(logTAG, " not posting service up intent as either instance not up on modem or previous intent was same");
                    }
                }
                else {
    ...
    void handleHidlStatus(boolean isUp, String iccId) {
        ...
        if(isUp) {
            if(slotId != SubscriptionManager.INVALID_SIM_SLOT_INDEX) {
              ...
              if(mTelephonyManager == null) {
                  Log.e(logTAG, "handleHidlStatus: mTelephonyManager is null");
                  return;
              }

              if(mTelephonyManager.getSimState(slotId) == TelephonyManager.SIM_STATE_READY &&
            ...
            iccIdActivatedOnModem.add(iccId);
        } else {
            iccIdActivatedOnModem.remove(iccId);
