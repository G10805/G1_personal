...
static qcarcam_ret_t send_ioctrl_cmd(int fd, uint8 op_code, uint32 param_type, unsigned int size, uint8* payload)
{
    int rc = 0;
...
int InputStream::set_batch_mode(unsigned int batch_frames)
{
    ...
    param.batch_config.num_batch_frames = batch_frames;

    rc = send_ioctrl_cmd(fd, AIS_V4L2_CAPTURE_PRIV_SET_PARAM,
        AIS_V4L2_PARAM_BATCH_MODE, MAX_AIS_V4L2_PAYLOAD_SIZE, (uint8*)&param.batch_config);
    if (rc)
    {
    ...
    {
        CAM_MSG(ALWAYS, "batch_frames 0x%x", param.batch_config.num_batch_frames);
    }
...
int InputStream::set_isp_usecase(qcarcam_isp_usecase_config_t isp_config)
{
    int rc = 0;
    int fd = m_wrapper->getfd();
    ...
    qcarcam_param_value_t param;
    memset(&param, 0, sizeof(param));
    param.isp_config = isp_config;

    rc = send_ioctrl_cmd(fd, AIS_V4L2_CAPTURE_PRIV_SET_PARAM,
        AIS_V4L2_PARAM_ISP_USECASE, MAX_AIS_V4L2_PAYLOAD_SIZE, (uint8*)&param.isp_config);
    if (rc)
    {
        CAM_MSG(ERROR,  "VIDIOC_S_PARAM failed %d", rc);
        return rc;
    ...
    else
    {
        CAM_MSG(ALWAYS, "Set ISP instance(%d) isp camera id %d, use case %d",
            param.isp_config.id, param.isp_config.camera_id, param.isp_config.use_case);
    }
...
static int test_vendor_extension(int fd)
{
    ...
    else
    {
        CAM_MSG(ALWAYS, "s_param data[0] %x data[1] %x",
            payload.vendor_param.data[0], payload.vendor_param.data[1]);
    }
    ...
    rc = send_ioctrl_cmd(fd, AIS_V4L2_CAPTURE_PRIV_GET_PARAM,
        AIS_V4L2_PARAM_VENDOR, MAX_AIS_V4L2_PARAM_EVNET_SIZE, (uint8*)&payload.vendor_param);
    if (rc)
    {
    ...
    {
        uint8* p = (uint8*)payload.vendor_param.data;
    ...
    // for get, also need the size, set it MAX_AIS_V4L2_PARAM_EVNET_SIZE here
    rc = send_ioctrl_cmd(fd, AIS_V4L2_CAPTURE_PRIV_GET_PARAM,
        AIS_V4L2_PARAM_VENDOR, MAX_AIS_V4L2_PAYLOAD_SIZE-1, (uint8*)&payload.vendor_param);
    if (rc)
    {
        CAM_MSG(ERROR,  "G_PARAM failed %d", rc);
        return rc;
...
static int test_vendor_extension1(int fd)
{
    ...
    qcarcam_param_value_t payload;
    memset(payload.vendor_param.data, 0x45, sizeof(payload));

    rc = send_ioctrl_cmd(fd, AIS_V4L2_CAPTURE_PRIV_SET_PARAM,
        AIS_V4L2_PARAM_VENDOR, MAX_AIS_V4L2_PAYLOAD_SIZE, (uint8*)&payload.vendor_param);
    if (rc)
    {
        CAM_MSG(ERROR,  "VIDIOC_S_PARAM failed %d", rc);
        return rc;
...
static int test_latency_1(int fd)
{
    ...
    qcarcam_param_value_t payload;
    payload.uint_value = 5; // the latency_max is unsigned int uint_value; in qcarcam_types.h

    rc = send_ioctrl_cmd(fd, AIS_V4L2_CAPTURE_PRIV_SET_PARAM,
        AIS_V4L2_PARAM_LATENCY_MAX, sizeof(payload.uint_value), (uint8*)&payload.uint_value);
    if (rc)
    {
    ...
    {
        CAM_MSG(HIGH, "s_param latency max %u", payload.uint_value);
    }

    payload.uint_value = 0;
    rc = send_ioctrl_cmd(fd, AIS_V4L2_CAPTURE_PRIV_SET_PARAM,
        AIS_V4L2_PARAM_LATENCY_REDUCE_RATE, sizeof(payload.uint_value), (uint8*)&payload.uint_value);
    if (rc)
    {
    ...
    {
        CAM_MSG(HIGH, "s_param LATENCY_REDUCE_RATE %u", payload.uint_value);
    }

    return 0;
}

int InputStream::handle_input_signal(qcarcam_input_signal_t signal_type)
{
...
int InputStream::init_window_buffer(test_util_ctxt_t* test_util_ctxt, test_util_xml_input_t *xml_inputs)
{
   ...
   // input_ctxt->state = QCARCAMTEST_STATE_INIT;
    ...
    for (int i = 0; i < input_ctxt->m_num_isp_instances; i++)
    {
        ...
        input_ctxt->m_isp_config[i].camera_id = p_xml_input->isp_params.isp_config[i].camera_id;
        input_ctxt->m_isp_config[i].use_case = p_xml_input->isp_params.isp_config[i].use_case;
    ...
    input_ctxt->m_p_buffers_output.n_buffers = output_n_buffers;
    input_ctxt->m_p_buffers_output.color_fmt = input_ctxt->m_buffers_param[QCARCAM_TEST_BUFFERS_OUTPUT].format;
    input_ctxt->m_p_buffers_output.buffers = (qcarcam_buffer_t *)calloc(input_ctxt->m_p_buffers_output.n_buffers, sizeof(*input_ctxt->m_p_buffers_output.buffers));

    if (input_ctxt->m_p_buffers_output.buffers == 0)
    {
    ...
    {
        input_ctxt->m_p_buffers_output.buffers[i].n_planes = 1;
        input_ctxt->m_p_buffers_output.buffers[i].planes[0].width = input_ctxt->m_buffers_param[QCARCAM_TEST_BUFFERS_OUTPUT].width;
        input_ctxt->m_p_buffers_output.buffers[i].planes[0].height = input_ctxt->m_buffers_param[QCARCAM_TEST_BUFFERS_OUTPUT].height;
    }
    ...
    input_ctxt->m_p_buffers_output.n_buffers = input_ctxt->m_buffers_param[QCARCAM_TEST_BUFFERS_OUTPUT].n_buffers;
...
int InputStream::get_frame()
{
    ...
    V4L2Wrapper* wrapper = input_ctxt->m_wrapper;
    int rc = 0;
    struct ais_v4l2_buffer_ext_t ext = {0};
    ...
    QCARCAM_INFOMSG("get_frame %u size %u",
        input_ctxt->m_buf_idx_qcarcam, input_ctxt->m_p_buffers_output.buffers[input_ctxt->m_buf_idx_qcarcam].planes[0].size);
...
int InputStream::release_buffers()
{
    ...
    while(!input_ctxt->m_release_buf_idx_list.empty())
    {
        ...
        if (rel_idx >= 0 &&
            (int)rel_idx < input_ctxt->m_buffers_param[QCARCAM_TEST_BUFFERS_OUTPUT].n_buffers)
        {
...
int InputStream::processV4L2Event(int fd)
{
    ...
    if (rc >= 0)
    {
        ...
        case AIS_V4L2_EVENT_INPUT_SIGNAL:
        {
            ...
            handle_input_signal((qcarcam_input_signal_t)(*value));
            break;
        }
