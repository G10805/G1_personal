...
public:
    ...
    int set_isp_usecase(qcarcam_isp_usecase_config_t isp_config);
...
private:
    ...
    int handle_input_signal(qcarcam_input_signal_t signal_type);
...
private:
    ...
    qcarcam_buffers_t m_p_buffers_disp;
    qcarcam_buffers_t m_p_buffers_input;
    uint32 m_nbufs;

    qcarcam_res_t m_resolution;

    qcarcam_opmode_type m_op_mode;

    unsigned int m_num_isp_instances;
    qcarcam_isp_usecase_config_t m_isp_config[QCARCAM_MAX_ISP_INSTANCES];
