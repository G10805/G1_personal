...
qcarcam_ret_t qcarcam_test_power_event_callback(test_util_pm_event_t power_event_id, void* p_usr_ctxt)
{
...
int main(int argc, char **argv)
{
    ...
    for (input_idx = 0; input_idx < gCtxt.numInputs; input_idx++)
    {
        ...
        for (i = 0; i < output_n_buffers; ++i)
        {
            ...
            input_ctxt->p_buffers_output.buffers[i].planes[0].width =
                input_ctxt->buffers_param[QCARCAM_TEST_BUFFERS_OUTPUT].width;
            input_ctxt->p_buffers_output.buffers[i].planes[0].height =
