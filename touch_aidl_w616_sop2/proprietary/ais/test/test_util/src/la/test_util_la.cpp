 ...
 * @return void
 */
static void test_util_fill_native_planes(test_util_window_t *p_window, test_util_buffer_t* pCtxtBuffer, qcarcam_buffer_t* pBuffer)
{
 ...
 * @return qcarcam_ret_t
 */
static qcarcam_ret_t alloc_graphic_buffers(test_util_window_t *p_window, qcarcam_buffers_t *buffers)
{
    ...
    for (int i = 0; i < p_window->n_buffers; i++)
    {
        ...
        buffers->buffers[i].planes[0].p_buf = (void*)(uintptr_t)(p_window->buffers[i].private_hndl->fd);
        ...
        buffers->buffers[i].planes[0].p_buf = (void*)(uintptr_t)get_window_buffer_fd(p_window->buffers[i].window_buf);
        p_window->buffers[i].is_dequeud = 1;
 ...
 * @return qcarcam_ret_t
 */
static qcarcam_ret_t alloc_native_window_buffers(test_util_window_t *p_window, qcarcam_buffers_t *buffers)
{
    qcarcam_ret_t rc = QCARCAM_RET_OK;
#if defined(ANDROID_Q_AOSP) || !defined(TESTUTIL_VENDOR_LIB)
    unsigned int is_secure = 0;
