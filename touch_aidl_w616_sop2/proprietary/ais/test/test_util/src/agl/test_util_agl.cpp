...
static struct display_wl *create_wl_display(void)
{
    ...
    /*poll until wayland-socket connect success*/
    int poll_ms = 0;
...
static int set_parent_surface(test_util_ctxt_t *ctxt)
{
    ...
    {
        ctxt->parent_shell_surface = wl_shell_get_shell_surface(ctxt->wl_display->shell, ctxt->parent_surface);
        if (ctxt->parent_shell_surface == NULL)
        {
            QCARCAM_ERRORMSG("wl_shell_get_shell_surface failed.");
            return QCARCAM_RET_FAILED;
        }
        wl_shell_surface_set_toplevel(ctxt->parent_shell_surface);
        wl_shell_surface_set_fullscreen(ctxt->parent_shell_surface, WL_SHELL_SURFACE_FULLSCREEN_METHOD_SCALE, 0, NULL);
        wl_shell_surface_add_listener(ctxt->parent_shell_surface, &shell_surface_listener, NULL);
...
static int create_parent_buffer(test_util_ctxt_t *ctxt, qcarcam_color_fmt_t format, uint32 window_flags)
{
    ...
    uint32 flags = GBM_BO_USE_SCANOUT;
...
static boolean isYUVformat(qcarcam_color_fmt_t fmt)
{
...
static boolean isSupportedFormat(qcarcam_color_fmt_t fmt)
{
    switch (fmt)
    {
    case QCARCAM_FMT_UYVY_8:
...
qcarcam_ret_t test_util_deinit(test_util_ctxt_t *ctxt)
{
    if (ctxt)
    {
        ...
        if (ctxt->parent_shell_surface)
        {
            wl_shell_surface_destroy(ctxt->parent_shell_surface);
            ctxt->parent_shell_surface = NULL;
        }
...
qcarcam_ret_t test_util_deinit_window(test_util_ctxt_t *ctxt, test_util_window_t *p_window)
{
    ...
    if (!ctxt->params.disable_display && p_window)
    {
        ...
        if (p_window->wl_window)
        {
            if (p_window->wl_window->shell_surface)
            {
                wl_shell_surface_destroy(p_window->wl_window->shell_surface);
                p_window->wl_window->shell_surface = NULL;
            }
...
qcarcam_ret_t test_util_init_window_buffers(test_util_ctxt_t *ctxt, test_util_window_t *p_window, qcarcam_buffers_t *buffers)
{
    ...
    p_window->buffer_size[0] = buffers->buffers[0].planes[0].width;
    p_window->buffer_size[1] = buffers->buffers[0].planes[0].height;

    p_window->format = buffers->color_fmt;
    gbmfmt = get_gbm_format(buffers->color_fmt);
    ...
    if (isSupportedFormat(buffers->color_fmt))
    {
        uint32 flags = GBM_BO_USE_SCANOUT;
        ...
        {
            if (p_window->is_imported_buffer && buffers->buffers[i].planes[0].p_buf != NULL)
            {
                //Request GBM to import buffer
                fd = (int)(uintptr_t)buffers->buffers[i].planes[0].p_buf;

                struct gbm_import_fd_data buf_data;
                buf_data.fd = fd;
                buf_data.width = buffers->buffers[i].planes[0].width;
                buf_data.height = buffers->buffers[i].planes[0].height;
            ...
            else
            {
                ...
                bo = gbm_bo_create(ctxt->gbm, buffers->buffers[i].planes[0].width, buffers->buffers[i].planes[0].height, gbmfmt, flags);
                if (bo == NULL)
                {
            ...
            p_window->stride = qcarcam_get_stride(buffers->color_fmt, align_w);

            buffers->buffers[i].n_planes = 1;
            buffers->buffers[i].planes[0].stride = qcarcam_get_stride((qcarcam_color_fmt_t)p_window->format, align_w);
            ...
            if (isYUVformat(buffers->color_fmt))
            { // get the YUV format information by gbm
                generic_buf_layout_t layout = {0};
                ...
                {
                     buffers->buffers[i].n_planes = layout.num_planes;
                }

                switch (layout.num_planes)
                {
                case 3:
                    buffers->buffers[i].planes[2].stride = layout.planes[2].v_increment;
                    buffers->buffers[i].planes[1].stride = layout.planes[1].v_increment;
                    buffers->buffers[i].planes[0].stride = layout.planes[0].v_increment;

                    buffers->buffers[i].planes[2].size = length - layout.planes[2].offset;
                    buffers->buffers[i].planes[1].size = layout.planes[2].offset - layout.planes[1].offset;
                    buffers->buffers[i].planes[0].size = layout.planes[1].offset - layout.planes[0].offset;
                    QCARCAM_INFOMSG("plane2 size2 %u stride2 %u size1 %u stride1 %u size0 %u stride0 %u",
                        buffers->buffers[i].planes[2].size, buffers->buffers[i].planes[2].stride,
                        buffers->buffers[i].planes[1].size, buffers->buffers[i].planes[1].stride,
                        buffers->buffers[i].planes[0].size, buffers->buffers[i].planes[0].stride);
                    break;
                case 2:
                    buffers->buffers[i].planes[1].stride = layout.planes[1].v_increment;
                    buffers->buffers[i].planes[0].stride = layout.planes[0].v_increment;

                    buffers->buffers[i].planes[1].size = length - layout.planes[1].offset;
                    buffers->buffers[i].planes[0].size = layout.planes[1].offset - layout.planes[0].offset;
                ...
                case 1:
                    buffers->buffers[i].planes[0].stride = layout.planes[0].v_increment;
                    buffers->buffers[i].planes[0].size = length;
            ...
            QCARCAM_INFOMSG("plane_num %u w %u h %u aign_w %u align_h %u stride %u length %u size %u",
                    buffers->buffers[i].n_planes,
                    buffers->buffers[i].planes[0].width,
                    buffers->buffers[i].planes[0].height,
                    align_w, align_h,
                    buffers->buffers[i].planes[0].stride, length, buffers->buffers[i].planes[0].size);
            ...
            p_window->buffers[i].ptr = buffers->buffers[i].planes[0].p_buf;
    ...
    else
    {
        if (!ctxt->params.disable_display)
        {
            ...
            for (int i = 0; i < buffers->n_buffers; i++)
            {
                ...
                {
                    //Need GBM to allocate buffer
                    bo = gbm_bo_create(ctxt->gbm, buffers->buffers[i].planes[0].width, buffers->buffers[i].planes[0].height, GBM_FORMAT_UYVY, flags);
                    if (bo == NULL)
                    {
                        QCARCAM_ERRORMSG("Failed to gbm_bo_create %u", gbmfmt);
                        return QCARCAM_RET_FAILED;
...
/// @return QCARCAM_RET_OK if successful
///////////////////////////////////////////////////////////////////////////////
qcarcam_ret_t test_util_post_window_buffer(test_util_ctxt_t *ctxt, test_util_window_t *p_window,
    unsigned int idx, std::list<uint32>* p_rel_buf_idx, qcarcam_field_t field_type)
{
    ...
    if (!ctxt->params.disable_display)
    {
        ...
        else
        {
            ...
            if (ctxt->params.enable_di == TESTUTIL_DEINTERLACE_SW_BOB)
            {
                if (field_type == QCARCAM_FIELD_ODD)
                {
                    // offset to odd field
                ...
                else if (field_type == QCARCAM_FIELD_EVEN)
                {
                ...
                else if (field_type == QCARCAM_FIELD_ODD_EVEN)
                {
                    // offset to odd field
                    wp_viewport_set_source(p_viewport,
                            wl_fixed_from_int(0),
...
/// @return QCARCAM_RET_OK if successful
///////////////////////////////////////////////////////////////////////////////
qcarcam_ret_t test_util_get_c2d_surface_id(test_util_ctxt_t *ctxt, test_util_window_t *p_window, unsigned int idx, uint32 *surface_id)
{
    if (!surface_id)
    {
        return QCARCAM_RET_BADPARAM;
...
///////////////////////////////////////////////////////////////////////////////
/// test_util_window_clear_state
///
/// @brief clear commit_frames
///
/// @param ctxt             Pointer to test_util context
/// @param p_window         Pointer to window
///
/// @return QCARCAM_RET_OK if successful
///////////////////////////////////////////////////////////////////////////////
qcarcam_ret_t test_util_window_clear_state(test_util_ctxt_t *ctxt, test_util_window_t *p_window)
{
    if (!ctxt->params.disable_display)
    {
        p_window->commit_frames = 0;
