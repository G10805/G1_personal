...
LOCAL_SHARED_LIBRARIES := liblog libais_log_proprietary \
    libxml2 \
    libdl libcutils libEGL libGLESv2 libui \
    libhidlbase \
    libutils libdmabufheap
...
LOCAL_MODULE := libais_test_util_proprietary
...
LOCAL_CFLAGS += -DRVC_DEFAULT_ENABLE
endif

ifneq (($(AIS_BUILD_FOR_EARLYSERVICE),true) || (,$(filter Tiramisu 13 U UpsideDownCake 14 V VanillaIceCream 15, $(PLATFORM_VERSION))))
LOCAL_MODULE_OWNER := qti
LOCAL_PROPRIETARY_MODULE := true
LOCAL_MODULE := libais_test_util_edrm_proprietary
LOCAL_SHARED_LIBRARIES += libais_log_proprietary libgsl libadreno_utils libEGL_adreno libGLESv2_adreno libC2D2 libdmabufheap
...
LOCAL_MODULE := libais_test_util_edrm
LOCAL_CFLAGS += -DAIS_EARLYSERVICE
LOCAL_HEADER_LIBRARIES := libmmosal_headers
LOCAL_SHARED_LIBRARIES += libmmosal libais_log libgsl_early libadreno_utils_early libEGL_adreno_early libGLESv2_adreno_early libC2D2_early libdmabufheap
endif
...
include $(BUILD_PREBUILT)
include $(CLEAR_VARS)
LOCAL_MODULE:= 1cam_pproc_nv12.xml
LOCAL_MODULE_CLASS := EXECUTABLES
