 ...
 *
 */
static void qcarcam_test_query_diagnostics(void *diagnosticInfo, uint32 diag_size)
{
    int ret = 0;

    ret = qcarcam_query_diagnostics(diagnosticInfo, diag_size);
    if (ret != QCARCAM_RET_OK)
    {
        QCARCAMDIAG_ERRORMSG("qcarcam_query_diagnostics failed with ret:%d", ret);
    }
    else
    {
        parse_diagnostics(diagnosticInfo);
        QCARCAMDIAG_INFOMSG("qcarcam_query_diagnostics is success and parsed");
    }
}
...
int main(int argc, char **argv)
{
    ...
    int ret = qcarcam_initialize(&qcarcam_init);
    if (ret != QCARCAM_RET_OK)
    {
        QCARCAMDIAG_ERRORMSG("qcarcam_initialize failed %d", ret);
        exit(-1);
    }
    ...
    while (1)
    {
        ...
        qcarcam_test_query_diagnostics((void*)diagnosticInfo, sizeof(QCarCamDiagInfo));
    }
