...
static int ais_client_event_recv_thread(void *p_arg)
{
    ...
    while (!p->event_thread_abort[AIS_EVENT_THREAD_IDX_RECV])
    {
        ...
        if (event.event_id == QCARCAM_EVENT_FRAME_READY && event.payload.frame_info.flags != 0)
        {
            uint64 ptimestamp = 0;
            ...
            AIS_LOG_CLI_HIGH("LATENCY| qcarcam_Hndl %lu frame_cnt %d cur_timestamp %llu latency_from_ais_server %llu us",
                ...
                ptimestamp,
                (ptimestamp - event.payload.frame_info.sof_qtimestamp[0]) / 1000);
...
static CameraResult ais_client_process_cmd(s_ais_client *p, void *p_param, unsigned int param_size, unsigned int size, unsigned int recv_timeout)
{
    ...
    if (cmd_id == AIS_CMD_S_BUFFERS)
    {
        ...
        AIS_LOG_CLI_MED("export %d buffers", p_buffers->buffers.n_buffers);
        for (i = 0; i < p_buffers->buffers.n_buffers; i++)
        {
            size_arr[i] = 0;
            if (p_buffers->buffers.buffers[i].n_planes <= QCARCAM_MAX_NUM_PLANES)
            {
                for (j = 0; j < p_buffers->buffers.buffers[i].n_planes; ++j)
                {
                    size_arr[i] += p_buffers->buffers.buffers[i].planes[j].size;
                }
            }
            p_buf_arr[i] = (long long)p_buffers->buffers.buffers[i].planes[0].p_buf;
        }

        ret = AIS_CONN_API(ais_conn_export)(&p->cmd_conn[idx], p_buf_arr, size_arr, p_buffers->buffers.n_buffers);
        if (ret != 0)
        {
            rc = CAMERA_EFAILED;
            goto EXIT_FLAG;
    ...
    else if (cmd_id == AIS_CMD_S_BUFFERS_V2)
    {
        unsigned int i, j;
        unsigned int temp_size = sizeof(int);
        s_ais_cmd_s_buffers_v2 *p_buffers = (s_ais_cmd_s_buffers_v2 *)p_param;
        long long p_buf_arr[QCARCAM_MAX_NUM_BUFFERS];
        unsigned int size_arr[QCARCAM_MAX_NUM_BUFFERS];
        ...
        //use this send/recv pair to synchronize on both sides
        ret = AIS_CONN_API(ais_conn_recv)(&p->cmd_conn[idx], &i, &temp_size, recv_timeout);
        if (ret != 0 || temp_size != sizeof(int) || i != p_buffers->bufferlist.n_buffers)
        {
            rc = CAMERA_EFAILED;
            goto EXIT_FLAG;
        }

        AIS_LOG_CLI_MED("export %d buffers", p_buffers->bufferlist.n_buffers);
        for (i = 0; i < p_buffers->bufferlist.n_buffers; i++)
        {
            size_arr[i] = 0;
            if (p_buffers->bufferlist.buffers[i].n_planes <= QCARCAM_MAX_NUM_PLANES)
            {
                for (j = 0; j < p_buffers->bufferlist.buffers[i].n_planes; ++j)
                {
                    size_arr[i] += p_buffers->bufferlist.buffers[i].planes[j].size;
                }
            }
            p_buf_arr[i] = (long long)p_buffers->bufferlist.buffers[i].planes[0].hndl;
        }

        ret = AIS_CONN_API(ais_conn_export)(&p->cmd_conn[idx], p_buf_arr, size_arr, p_buffers->bufferlist.n_buffers);
        if (ret != 0)
        {
            rc = CAMERA_EFAILED;
            goto EXIT_FLAG;
    ...
    else if (cmd_id == AIS_CMD_S_PARAM)
    {
        s_ais_cmd_s_param *cmd_param = (s_ais_cmd_s_param *)p_param;

        if ((cmd_param->param == QCARCAM_PARAM_GAMMA) &&
            (cmd_param->value.gamma_config.config_type  == QCARCAM_GAMMA_KNEEPOINTS))
        {
            unsigned int length;
            unsigned int size = sizeof(unsigned int);

            ret = AIS_CONN_API(ais_conn_recv)(&p->cmd_conn[idx], &length, &size, recv_timeout);
            if (ret != 0 ||length != cmd_param->value.gamma_config.gamma.table.length)
            {
                rc = CAMERA_EFAILED;
                goto EXIT_FLAG;
            }

            unsigned int *p_value = cmd_param->value.gamma_config.gamma.table.p_value;
            ret = AIS_CONN_API(ais_conn_send)(&p->cmd_conn[idx], p_value, length * sizeof(unsigned int));
            if (ret != 0)
            {
                rc = CAMERA_EFAILED;
                goto EXIT_FLAG;
    ...
    else if (cmd_id == AIS_CMD_G_PARAM)
    {
        s_ais_cmd_g_param *cmd_param = (s_ais_cmd_g_param *)p_param;
            ...
            (cmd_param->value.gamma_config.config_type  == QCARCAM_GAMMA_KNEEPOINTS))
        {
            ...
            if (ret != 0 ||length != cmd_param->value.gamma_config.gamma.table.length)
            {
                AIS_LOG_CLI_ERR("ret %d size %d, param %d", ret, size, cmd_param->param);
                rc = CAMERA_EFAILED;
                goto EXIT_FLAG;
            }
            ...
            if (ret != 0 || size != sizeof(unsigned int) * length)
            {
                rc = CAMERA_EFAILED;
                goto EXIT_FLAG;
            }
...
EXIT_FLAG2:
    ...
    AIS_LOG_CLI(rc == 0 ? AIS_LOG_LVL_LOW : AIS_LOG_LVL_ERROR,
            "X %p %p %d %d %d", p, p_param, param_size, size, rc);
...
/// @return CAMERA_SUCCESS if successful
///////////////////////////////////////////////////////////////////////////////
PUBLIC_API CameraResult ais_query_inputs_v2(qcarcam_input_v2_t* p_inputs, unsigned int size, unsigned int* ret_size)
{
    CameraResult rc = CAMERA_SUCCESS;
    s_ais_cmd_query_inputs_v2 cmd_param;
    s_ais_client *p;
    ...
    memset(&cmd_param, 0, sizeof(cmd_param));
    cmd_param.cmd_id = AIS_CMD_QUERY_INPUTS_V2;
    cmd_param.result = CAMERA_SUCCESS;
    cmd_param.is_p_inputs_set = (p_inputs == NULL) ? 0 : 1;
    cmd_param.size = size;
...
/// @return CAMERA_SUCCESS if successful
///////////////////////////////////////////////////////////////////////////////
PUBLIC_API CameraResult ais_query_diagnostics(void *p_diag_info, unsigned int diag_size)
{
    CameraResult rc = CAMERA_SUCCESS;
    s_ais_cmd_query_diagnostics cmd_param;
    s_ais_client *p;

    AIS_LOG_CLI_API("E %p %d", p_diag_info, diag_size);
    ...
    cmd_param.result = CAMERA_SUCCESS;
    cmd_param.diag_size = diag_size;
    cmd_param.ais_diag_info = p_diag_info;

    rc = ais_client_process_cmd(p, &cmd_param, sizeof(cmd_param), sizeof(cmd_param), AIS_CONN_RECV_TIMEOUT);

EXIT_FLAG:
    AIS_LOG_CLI(rc == CAMERA_SUCCESS ? AIS_LOG_CLI_API_LEVEL : AIS_LOG_LVL_ERROR,
            "X %p %d | %d", p_diag_info, diag_size, rc);
...
EXIT_FLAG:
    ...
    AIS_LOG_CLI(rc == CAMERA_SUCCESS ? AIS_LOG_CLI_API_LEVEL : AIS_LOG_LVL_ERROR,
                "X %d %p active_clients %d", desc, cmd_param.handle, active_clients);
...
PUBLIC_API CameraResult ais_g_param(qcarcam_hndl_t hndl, qcarcam_param_t param, qcarcam_param_value_t* p_value)
{
    ...
    AIS_LOG_CLI_API("E %p %d %p", hndl, param, p_value);
...
EXIT_FLAG:
    ...
    AIS_LOG_CLI(rc == CAMERA_SUCCESS ? AIS_LOG_CLI_API_LEVEL : AIS_LOG_LVL_ERROR,
                "X %p %d %p %d", hndl, param, p_value, rc);
...
/// @return CAMERA_SUCCESS if successful
///////////////////////////////////////////////////////////////////////////////
PUBLIC_API CameraResult ais_s_buffers_v2(qcarcam_hndl_t hndl, const qcarcam_bufferlist_t* p_bufferlist)
{
    CameraResult rc = CAMERA_SUCCESS;
    s_ais_cmd_s_buffers_v2 cmd_param;
    int client_idx = -1;
    s_ais_client *p = NULL;
    unsigned int param_size = 0;

    AIS_LOG_CLI_API("E %p %p", hndl, p_bufferlist);

    if (p_bufferlist == NULL || p_bufferlist->n_buffers > QCARCAM_MAX_NUM_BUFFERS)
    {
        rc = CAMERA_EBADPARM;
        goto EXIT_FLAG;
    }
    ...
    memset(&cmd_param, 0, sizeof(cmd_param));
    cmd_param.cmd_id = AIS_CMD_S_BUFFERS_V2;
    cmd_param.result = CAMERA_SUCCESS;
    cmd_param.handle = hndl;
    cmd_param.bufferlist = *p_bufferlist;
    cmd_param.bufferlist.buffers = cmd_param.buffer;

    memcpy(cmd_param.bufferlist.buffers, p_bufferlist->buffers, sizeof(qcarcam_buffer_v2_t) * p_bufferlist->n_buffers);

    param_size = sizeof(cmd_param) - (QCARCAM_MAX_NUM_BUFFERS - p_bufferlist->n_buffers) * sizeof(qcarcam_buffer_v2_t);
    rc = ais_client_process_cmd(p, &cmd_param, param_size, param_size, AIS_CONN_RECV_TIMEOUT);

EXIT_FLAG:

    AIS_LOG_CLI(rc == CAMERA_SUCCESS ? AIS_LOG_CLI_API_LEVEL : AIS_LOG_LVL_ERROR,
                "X %p %p %d", hndl, p_bufferlist, rc);
...
PUBLIC_API CameraResult ais_get_frame(qcarcam_hndl_t hndl, qcarcam_frame_info_t* p_frame_info, unsigned long long int timeout, unsigned int flags)
{
    ...
    AIS_LOG_CLI_API("E %p %p %lld 0x%x", hndl, p_frame_info, timeout, flags);
...
EXIT_FLAG:
    ...
    AIS_LOG_CLI(rc == CAMERA_SUCCESS ? AIS_LOG_CLI_API_LEVEL : AIS_LOG_LVL_ERROR,
                "X %p %p %lld 0x%x %d", hndl, p_frame_info, timeout, flags, rc);
...
/// @return CAMERA_SUCCESS if successful; CAMERA_EEXPIRED if timeout
///////////////////////////////////////////////////////////////////////////////
PUBLIC_API CameraResult ais_get_frame_v2(qcarcam_hndl_t hndl, qcarcam_frame_info_v2_t* p_frame_info, unsigned long long int timeout, unsigned int flags)
{
    CameraResult rc = CAMERA_SUCCESS;
    s_ais_cmd_get_frame_v2 cmd_param;
    int client_idx = -1;
    s_ais_client *p = NULL;
    AIS_LOG_CLI_API("E %p %p %lld 0x%x", hndl, p_frame_info, timeout, flags);
    if (p_frame_info == NULL)
    ...
    p = &sgs_ais_client[client_idx];
    memset(&cmd_param, 0, sizeof(cmd_param));
    cmd_param.cmd_id = AIS_CMD_GET_FRAME_V2;
    cmd_param.result = CAMERA_SUCCESS;
    cmd_param.handle = hndl;
    cmd_param.timeout = timeout;
    cmd_param.flags = flags;
    ...
    *p_frame_info = cmd_param.frame_info_v2;
EXIT_FLAG:
    AIS_LOG_CLI(rc == CAMERA_SUCCESS ? AIS_LOG_CLI_API_LEVEL : AIS_LOG_LVL_ERROR,
                "X %p %p %lld 0x%x %d", hndl, p_frame_info, timeout, flags, rc);
    return rc;
...
/// @return CAMERA_SUCCESS if successful
///////////////////////////////////////////////////////////////////////////////
PUBLIC_API CameraResult ais_release_frame_v2(qcarcam_hndl_t hndl, unsigned int id, unsigned int idx)
{
    CameraResult rc = CAMERA_SUCCESS;
