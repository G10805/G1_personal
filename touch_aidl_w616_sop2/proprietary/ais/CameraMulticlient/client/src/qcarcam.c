...
{
    return camera_to_qcarcam_result(ais_query_inputs_v2(p_inputs, size, ret_size));
}

PUBLIC_API qcarcam_ret_t qcarcam_query_diagnostics(void *p_diag_info, unsigned int diag_size)
{
    return camera_to_qcarcam_result(ais_query_diagnostics(p_diag_info, diag_size));
}
...
PUBLIC_API qcarcam_ret_t qcarcam_s_buffers_v2(qcarcam_hndl_t hndl, const qcarcam_bufferlist_t* p_bufferlist)
{
    return camera_to_qcarcam_result(ais_s_buffers_v2(hndl, p_bufferlist));
}
...
PUBLIC_API qcarcam_ret_t qcarcam_get_frame_v2(qcarcam_hndl_t hndl, qcarcam_frame_info_v2_t* p_frame_info,
        unsigned long long int timeout, unsigned int flags)
{
    return camera_to_qcarcam_result(ais_get_frame_v2(hndl, p_frame_info, timeout, flags));
}
...
{
    return camera_to_qcarcam_result(ais_release_frame_v2(hndl, id, idx));
}
