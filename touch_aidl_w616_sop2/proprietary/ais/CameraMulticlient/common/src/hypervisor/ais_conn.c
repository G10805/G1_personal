...
#define AIS_LOG_CONN(level, fmt...) AIS_LOG2(CONN_HYPERVISOR, level, fmt)

#define AIS_LOG_CONN_ERR(fmt...) AIS_LOG(CONN_HYPERVISOR, ERROR, fmt)
#define AIS_LOG_CONN_WARN(fmt...) AIS_LOG(CONN_HYPERVISOR, WARN, fmt)
#define AIS_LOG_CONN_HIGH(fmt...) AIS_LOG(CONN_HYPERVISOR, HIGH, fmt)
#define AIS_LOG_CONN_MED(fmt...) AIS_LOG(CONN_HYPERVISOR, MED, fmt)
#define AIS_LOG_CONN_LOW(fmt...) AIS_LOG(CONN_HYPERVISOR, LOW, fmt)
#define AIS_LOG_CONN_DBG(fmt...) AIS_LOG(CONN_HYPERVISOR, DBG, fmt)
...
int AIS_CONN_API(ais_conn_open)(s_ais_conn *p, unsigned int id, unsigned int max_cnt)
{
    ...
    AIS_LOG_CONN_HIGH("E %p %d %d", p, id, max_cnt);
...
EXIT_FLAG:
    ...
    AIS_LOG_CONN(rc == 0 ? AIS_LOG_LVL_HIGH : AIS_LOG_LVL_ERROR,
...
int AIS_CONN_API(ais_conn_accept)(s_ais_conn *p, s_ais_conn *p_new, e_ais_conn_mode mode)
{
    ...
    while (!p->abort)
    {
        ...
        {
            AIS_LOG_CONN_ERR("%p %d %d %d", p, rc, errno, p->abort);
            rc = -4;
            goto EXIT_FLAG;
        }
        else
    ...
    {
        rc = -5;
        AIS_LOG_CONN_ERR("handle NULL, abort: %d", p->abort);
    }
...
int AIS_CONN_API(ais_conn_connect)(s_ais_conn *p)
{
    ...
    if (rc != 0)
    {
        AIS_LOG_CONN_ERR("%p %d %d", p, rc, errno);
        rc = (rc == -ETIMEDOUT) ? 1 : -4;
        goto EXIT_FLAG;
    }
...
int AIS_CONN_API(ais_conn_recv)(s_ais_conn *p, void *p_data, unsigned int *p_size, unsigned int timeout)
{
    ...
    AIS_LOG_CONN_DBG("E %p %p %p | %d %d", p,
 ...
 *
 */
int AIS_CONN_API(ais_conn_export)(s_ais_conn *p, long long *p_data_arr, unsigned int* size_arr, int n_buffer)
{
    int rc = 0;
    ...
    for (int i = 0; i < n_buffer; i++)
    {
        uint32_t id = 0;
        uintptr_t exp_id = 0;
        int temp_size = CAM_ALIGN_SIZE(size_arr[i], 4096);

        AIS_LOG_CONN_LOW("%p %d",p_data_arr[i], temp_size);

        rc = habmm_export(p->handle, (void*)(uintptr_t)p_data_arr[i], temp_size, &id, HABMM_EXPIMP_FLAGS_FD);
        if (rc != 0)
        {
            AIS_LOG_CONN_ERR("%p %d %d", p, rc, errno);
            rc = -4;
        ...
        {
            p->map_num[p->map_idx]--;
            AIS_LOG_CONN_ERR("too many export");
        }

        AIS_LOG_CONN_LOW("%d", id);
 ...
 *
 */
int AIS_CONN_API(ais_conn_import)(s_ais_conn *p, long long *p_data_arr, unsigned int* p_flags, unsigned int* size_arr, int n_buffer, unsigned int timeout)
{
    int rc = 0;

    AIS_LOG_CONN_MED("E %p %p %d %p %d", p,
                p_data_arr, n_buffer, size_arr, timeout);

    if (p == NULL || p_data_arr == NULL || n_buffer == 0 || size_arr == NULL)
    {
        rc = -1;
        goto EXIT_FLAG2;
    }
    ...
    for (int i = 0; i < n_buffer; i++)
    {
        ...
        AIS_LOG_CONN_LOW("%d %d", (int)exp_id, temp_size);

        temp_size = CAM_ALIGN_SIZE(size_arr[i], 4096);

#ifdef USE_PMEM_V2
        rc = habmm_import(p->handle, &p_temp, temp_size, (uint32_t)exp_id, HABMM_EXPIMP_FLAGS_FD);
        //set hndl flag for v2
        *p_flags |= QCARCAM_BUFFER_FLAG_OS_HNDL;
#else
        rc = habmm_import(p->handle, &p_temp, temp_size, (uint32_t)exp_id, 0);
        //unset flag for v1
        *p_flags = (*p_flags & (~QCARCAM_BUFFER_FLAG_OS_HNDL));
#endif
        if (rc != 0)
        {
            AIS_LOG_CONN_ERR("%p %d %d", p, rc, errno);
        ...
        {
            p->map_num[p->map_idx]--;
            AIS_LOG_CONN_ERR("too many import");
        }

        AIS_LOG_CONN_LOW("%p %d", p_temp, temp_size);

        p_data_arr[i] = (long long)p_temp;
    }
...
EXIT_FLAG2:
    ...
    AIS_LOG_CONN(rc == 0 ? AIS_LOG_LVL_MED : AIS_LOG_LVL_ERROR,
                "X %p %p %p %d %d", p,
                p_data_arr, size_arr, timeout, rc);
    return rc;
}
    ...
    for (i = 0; i < p->map_num[p->map_idx]; i++)
    {
        ...
        if (p->map[p->map_idx][i].fd > 0)
        {
            ...
            if (p->map[p->map_idx][i].p_buf)
            {
                ...
                if (rc != 0)
                {
                    AIS_LOG_CONN_ERR("habmm_unimport failed, %p %d", p, rc);
                }
                else
                {
                    AIS_LOG_CONN_HIGH("Backend habmm_unimport ok %p %d %d", p, p->map_idx, i);
                }
            }
            else
            {
                ...
                if (rc != 0)
                {
                    AIS_LOG_CONN_ERR("habmm_unexport failed, %p %d", p, rc);
                }
                else
                {
                    AIS_LOG_CONN_HIGH("Frontend habmm_unexport ok %p %d %d", p, p->map_idx, i);
                }
            }
        }
    }
