/**
 * @file ais_conn.c
 *
 * @brief defines all functions of connection
 *        connection is an abstract layer for socket/Connection/hab on Qnx/Linux/Android/Integrity.
 *        this is an abstraction layer for Linux socket
 *
 * Copyright (c) 2018-2020, 2023 Qualcomm Technologies, Inc.
 * All Rights Reserved.
 * Confidential and Proprietary - Qualcomm Technologies, Inc.
 */

#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <fcntl.h>
#include <unistd.h>
#include <errno.h>

#include <sys/types.h>
#include <sys/stat.h>
#include <sys/socket.h>
#include <sys/un.h>


#include "ais_log.h"
#include "ais_conn.h"
#include "ais_comm.h"

#ifdef CAMERA_UNITTEST
static int sg_socket_path_initialized = 0;
static char* AIS_CONN_SOCKET_PATH = AIS_SOCKET_PATH;
#else
#define AIS_CONN_SOCKET_PATH AIS_SOCKET_PATH
#endif

#define AIS_LOG_CONN(level, fmt...) AIS_LOG2(CONN_LINUX, level, fmt)

#define AIS_LOG_CONN_ERR(fmt...) AIS_LOG(CONN_LINUX, ERROR, fmt)
#define AIS_LOG_CONN_WARN(fmt...) AIS_LOG(CONN_LINUX, WARN, fmt)
#define AIS_LOG_CONN_HIGH(fmt...) AIS_LOG(CONN_LINUX, HIGH, fmt)
#define AIS_LOG_CONN_MED(fmt...) AIS_LOG(CONN_LINUX, MED, fmt)
#define AIS_LOG_CONN_LOW(fmt...) AIS_LOG(CONN_LINUX, LOW, fmt)
#define AIS_LOG_CONN_DBG(fmt...) AIS_LOG(CONN_LINUX, DBG, fmt)

#define AIS_CONNECT_RETRY_TIMEOUT  10000
#define AIS_CONNECT_RETRY_COUNT 200

#if defined(__AGL__)
#define SYSTEM_UID 1000
#define CAMERA_GID 1006
#endif

/**
 * initializes a connection structure
 * @param p points to a connection structure
 * @return 0: success, others: failed
 */
int ais_conn_init(s_ais_conn *p)
{
    int rc = 0;

    AIS_LOG_CONN_MED("E %p", p);

    if (p == NULL)
    {
        rc = -1;
        goto EXIT_FLAG;
    }
    memset(p, 0, sizeof(s_ais_conn));

    p->fd = -1;

    p->status = AIS_CONN_STAT_NONE;

EXIT_FLAG:

    AIS_LOG_CONN(rc == 0 ? AIS_LOG_LVL_MED : AIS_LOG_LVL_ERROR,
                "X %p %d", p, rc);

    return rc;
}

/**
 * opens a new connection
 * @param p points to a connection structure
 * @param id connection id which is used to identify by client and server
 * @param max_cnt maximum number of sub connections with the specific id
 * @return 0: success, others: failed
 */
int ais_conn_open(s_ais_conn *p, unsigned id, unsigned int max_cnt)
{
    int rc = 0;

    AIS_LOG_CONN_MED("E %p %d %d", p, id, max_cnt);

#ifdef CAMERA_UNITTEST
    if (!sg_socket_path_initialized)
    {
        char *p = getenv("TDD_SOCKET_PATH");
        if (p != NULL)
        {
            AIS_CONN_SOCKET_PATH = p;
        }
        AIS_LOG_CONN_HIGH("%s", AIS_CONN_SOCKET_PATH);
        sg_socket_path_initialized = 1;
    }
#endif

    if ((p == NULL) || (sizeof(*p) > sizeof(s_ais_conn)))
    {
        rc = -1;
        goto EXIT_FLAG;
    }
    else
    {
       memset(p, 0, sizeof(s_ais_conn));
    }

    p->id = id;
    p->max_cnt = max_cnt;

    p->fd = -1;

    pthread_mutex_init(&p->mutex, NULL);

    p->status = AIS_CONN_STAT_INIT;

EXIT_FLAG:

    AIS_LOG_CONN(rc == 0 ? AIS_LOG_LVL_MED : AIS_LOG_LVL_ERROR,
                "X %p %d %d %d", p, id, max_cnt, rc);

    return rc;
}

/**
 * closes a connection
 * @param p points to a connection structure
 * @return 0: success, others: failed
 */
int ais_conn_close(s_ais_conn *p)
{
    int rc = 0;

    AIS_LOG_CONN_MED("E %p", p);

    if (p == NULL)
    {
        rc = -1;
        goto EXIT_FLAG;
    }

    if (p->abort != 1)
    {
        //set exit flag for other APIs
        p->abort = 1;

        if (p->fd >= 0)
        {
            //shutdown and close socket, so other APIs would break and exit.
            rc = shutdown(p->fd, SHUT_RDWR);
            rc = close(p->fd);
            p->fd = -1;
        }

        if (p->status != AIS_CONN_STAT_NONE)
        {
            struct timespec ts;

            //waiting for other APIs exit
            AIS_CONN_CALC_ABSTIME(ts, CLOCK_REALTIME, AIS_CONN_CLOSE_TIMEOUT);

            pthread_mutex_timedlock(&p->mutex, &ts);
            p->status = AIS_CONN_STAT_NONE;
            pthread_mutex_unlock(&p->mutex);

            //close all fds
            ais_conn_flush(p, 0);
            ais_conn_flush(p, 0);

            pthread_mutex_destroy(&p->mutex);
        }
    }
EXIT_FLAG:

    AIS_LOG_CONN(rc == 0 ? AIS_LOG_LVL_MED : AIS_LOG_LVL_ERROR,
                "X %p %d", p, rc);

    return rc;
}

/**
 * copys a listening connection to spawn a new connection
 * @param p points to a connection structure
 * @param p points to a spawned connection structure
 * @return none
 */
void ais_conn_copy(s_ais_conn *p, s_ais_conn *p_new)
{
    p_new->status = p->status;
    p_new->id = p->id;
    p_new->max_cnt = p->max_cnt;

    pthread_mutex_init(&p_new->mutex, NULL);

    p_new->map_idx = 0;
    p_new->map_num[0] = 0;
    p_new->map_num[1] = 0;

    p_new->abort = 0;

    return ;
}

/**
 * listens a connection from clients and spawns a new connection once a client requests.
 * @param p points to a connection structure
 * @param p_new points to a spawned connection structure
 * @param mode indicates the mode of the listening connection
 * @return 0: success, others: failed
 */
int ais_conn_accept(s_ais_conn *p, s_ais_conn *p_new, e_ais_conn_mode mode)
{
    int rc = 0;
    int fd;
    struct sockaddr_un addr;
    socklen_t len;

    AIS_LOG_CONN_MED("E %p %p %d", p, p_new, mode);

    if (p == NULL)
    {
        rc = -1;
        goto EXIT_FLAG2;
    }

    rc = pthread_mutex_lock(&p->mutex);
    if (rc != 0)
    {
        rc = -2;
        goto EXIT_FLAG;
    }

    if (p->status != AIS_CONN_STAT_INIT)
    {
        rc = -3;
        goto EXIT_FLAG;
    }

    memset(&addr, 0, sizeof(addr));
    addr.sun_family = AF_UNIX;
    snprintf(addr.sun_path, sizeof(addr.sun_path), "%s/ais_socket_%d", AIS_CONN_SOCKET_PATH, p->id);

    if (p->fd < 0)
    {
        unlink(addr.sun_path);

        fd = socket(AF_UNIX, SOCK_STREAM, 0);
        if (fd < 0)
        {
            AIS_LOG_CONN_ERR("%p %s", p, strerror(errno));
            rc = -4;
            goto EXIT_FLAG;
        }

        rc = bind(fd, (struct sockaddr*)&addr, sizeof(addr));
        if (rc < 0)
        {
            AIS_LOG_CONN_ERR("%p %d %s", p, rc, strerror(errno));
            rc = -5;
            goto EXIT_FLAG;
        }

#if defined(__AGL__)
        /* Change UID and GID of the socket file to system:camera to support LXC use case*/
        chown(addr.sun_path, SYSTEM_UID, CAMERA_GID);
        /* Add 'write' permission for 'camera' group of ais_server socket, same as 'chmod(addr.sun_path, 0770)'*/
        chmod(addr.sun_path, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP);
#endif
#if defined(AIS_EARLYSERVICE) && defined(ANDROID_T_ABOVE)
        chmod(addr.sun_path, 0777);
#endif
        listen(fd, p->max_cnt);

        p->fd = fd;
    }

    len = sizeof(addr);
    fd = accept(p->fd, (struct sockaddr*)&addr, &len);
    if (fd == -1 || p->abort)
    {
        AIS_LOG_CONN_ERR("%p %s %d", p, strerror(errno), p->abort);
        rc = -6;
        goto EXIT_FLAG;
    }
    rc = 0;

    ais_conn_copy(p, p_new);
    p_new->fd = fd;
    p_new->status = AIS_CONN_STAT_READY;

EXIT_FLAG:

    pthread_mutex_unlock(&p->mutex);

EXIT_FLAG2:

    AIS_LOG_CONN(rc == 0 ? AIS_LOG_LVL_MED : AIS_LOG_LVL_ERROR,
                "X %p %p %d %d", p, p_new, mode, rc);

    return rc;
}

/**
 * connects to a listening connection
 * @param p points to a connection structure
 * @return 0: success, others: failed
 */
int ais_conn_connect(s_ais_conn *p)
{
    int rc = 0;
    int cnt = 0;
    int fd;
    struct sockaddr_un addr;

    AIS_LOG_CONN_MED("E %p", p);

    if (p == NULL)
    {
        rc = -1;
        goto EXIT_FLAG2;
    }

    rc = pthread_mutex_lock(&p->mutex);
    if (rc != 0)
    {
        rc = -2;
        goto EXIT_FLAG;
    }

    if (p->status != AIS_CONN_STAT_INIT)
    {
        rc = -3;
        goto EXIT_FLAG;
    }

    fd = socket(AF_UNIX, SOCK_STREAM, 0);
    if (fd < 0)
    {
        AIS_LOG_CONN_ERR("%p %s", p, strerror(errno));
        rc = -4;
        goto EXIT_FLAG;
    }

    memset(&addr, 0, sizeof(addr));
    addr.sun_family = AF_UNIX;
    snprintf(addr.sun_path, sizeof(addr.sun_path), "%s/ais_socket_%d", AIS_CONN_SOCKET_PATH, p->id);

    while (!p->abort)
    {
        rc = connect(fd, (struct sockaddr*)&addr, sizeof(addr));
        if (rc == 0)
        {
            break;
        }
        else if (errno == ENOENT || errno == ECONNREFUSED || errno == EACCES)
        {
            //2 seconds timeout, handle the case of server is not listening
            if (cnt++ >= AIS_CONNECT_RETRY_COUNT)
            {
                rc = -5;
                break;
            }
            usleep(AIS_CONNECT_RETRY_TIMEOUT);
        }
        else
        {
            rc = -6;
            break;
        }
    }
    if (rc != 0)
    {
        AIS_LOG_CONN_ERR("%p %d %s", p, rc, strerror(errno));
        goto EXIT_FLAG;
    }

    p->fd = fd;

    p->status = AIS_CONN_STAT_READY;

EXIT_FLAG:

    pthread_mutex_unlock(&p->mutex);

EXIT_FLAG2:

    AIS_LOG_CONN(rc == 0 ? AIS_LOG_LVL_MED : AIS_LOG_LVL_ERROR,
                "X %p %d", p, rc);

    return rc;
}

/**
 * sends data to the peer via connection.
 * SOCKET WOULD MERGE MULTIPLE MESSAGES INTO ONE FIRST AND THEN SEND OUT, WHICH CAUSE ISSUES
 * @param p points to connection structure
 * @param p_data points to a buffer to be sent
 * @param size data size in the specified buffer
 * @return 0: success, others: failed
 */
int ais_conn_send(s_ais_conn *p, void *p_data, unsigned int size)
{
    int rc = 0;

    struct msghdr msg;
    struct iovec iov;

    AIS_LOG_CONN_DBG("E %p %p %d", p, p_data, size);

    if (p == NULL || p_data == NULL || size == 0)
    {
        rc = -1;
        goto EXIT_FLAG2;
    }

    rc = pthread_mutex_lock(&p->mutex);
    if (rc != 0)
    {
        rc = -2;
        goto EXIT_FLAG;
    }

    if (p->status != AIS_CONN_STAT_READY)
    {
        rc = -3;
        goto EXIT_FLAG;
    }

    memset(&msg, 0, sizeof(msg));

    iov.iov_base = p_data;
    iov.iov_len = size;
    msg.msg_iov = &iov;
    msg.msg_iovlen = 1;

    msg.msg_control = 0;
    msg.msg_controllen = 0;

    rc = sendmsg(p->fd, &msg, 0);
    if (rc < 0 || (size_t)rc != size)
    {
        AIS_LOG_CONN_ERR("%p %d %s", p, rc, strerror(errno));
        rc = -4;
        goto EXIT_FLAG;
    }
    rc = 0;

EXIT_FLAG:

    pthread_mutex_unlock(&p->mutex);

EXIT_FLAG2:

    AIS_LOG_CONN(rc == 0 ? AIS_LOG_LVL_DBG : AIS_LOG_LVL_ERROR,
                "X %p %p %d %d", p, p_data, size, rc);

    return rc;
}

/**
 * receives data from the peer via connection with timeout
 * @param p points to connection structure
 * @param p_data points to a buffer to be sent
 * @param p_size points to the size of the buffer specified by p_data
 * @param timeout timed-out time in millisecond, not supported currently
 * @return 0: success, others: failed
 */
int ais_conn_recv(s_ais_conn *p, void *p_data, unsigned int *p_size, unsigned int timeout)
{
    int rc = 0;

    struct msghdr msg;
    struct iovec iov;

    AIS_LOG_CONN_DBG("E %p %p %p | %d", p,
                p_data, p_size, (p_size != NULL) ? *p_size : 0);

    if (p == NULL || p_data == NULL || p_size == NULL || *p_size == 0)
    {
        rc = -1;
        goto EXIT_FLAG2;
    }

    rc = pthread_mutex_lock(&p->mutex);
    if (rc != 0)
    {
        rc = -2;
        goto EXIT_FLAG;
    }

    if (p->status != AIS_CONN_STAT_READY)
    {
        rc = -3;
        goto EXIT_FLAG;
    }

    memset(&msg, 0, sizeof(msg));

    iov.iov_base = p_data;
    iov.iov_len = *p_size;
    msg.msg_iov = &iov;
    msg.msg_iovlen = 1;

    msg.msg_control = 0;
    msg.msg_controllen = 0;

    rc = recvmsg(p->fd, &msg, 0);
    if (rc <= 0 || p->abort)
    {
        AIS_LOG_CONN_ERR("%p %d %s %d", p, rc, strerror(errno), p->abort);
        rc = -4;
        goto EXIT_FLAG;
    }

    *p_size = rc;
    rc = 0;

EXIT_FLAG:

    pthread_mutex_unlock(&p->mutex);

EXIT_FLAG2:

    AIS_LOG_CONN(rc == 0 ? AIS_LOG_LVL_DBG : AIS_LOG_LVL_ERROR,
                "X %p %p %p | %d %d", p,
                p_data, p_size, (p_size != NULL) ? *p_size : 0, rc);

    return rc;
}

/**
 * exports the buffers from one process/OS to another
 * shares buffer by file description which is handled by socket itself
 * @param p points to a connection structure
 * @param p_data_arr points to the array of buffers to be exported
 * @param size_arr array of buffer sizes
 * @param n_buffer the number of the exported buffer
 * @return 0: success, others: failure
 *
 */
int ais_conn_export(s_ais_conn *p, long long *p_data_arr, unsigned int* size_arr, int n_buffer)
{
    int rc = 0;
    struct msghdr msg;
    struct iovec iov;
    char ctrl[CMSG_SPACE(sizeof(int) * n_buffer)];
    struct cmsghdr *p_cmsg;
    int *fdptr;
    int p_buff_fd[QCARCAM_MAX_NUM_BUFFERS] = {};

    AIS_LOG_CONN_MED("E %p %p %d", p, p_data_arr, n_buffer);

    if (p == NULL || p_data_arr == NULL || n_buffer == 0)
    {
        rc = -1;
        goto EXIT_FLAG2;
    }

    /* convert long long* to int*, since linux kernel fd type is int, */
    /* kernel scm_fp_copy() calculate fd count base on length/sizeof(int) */
    for (uint32 idx = 0; idx < n_buffer; idx++)
    {
        p_buff_fd[idx] = (int)p_data_arr[idx];
    }

    rc = pthread_mutex_lock(&p->mutex);
    if (rc != 0)
    {
        rc = -2;
        goto EXIT_FLAG;
    }

    if (p->status != AIS_CONN_STAT_READY)
    {
        rc = -3;
        goto EXIT_FLAG;
    }

    memset(&msg, 0, sizeof(msg));

    iov.iov_base = p_buff_fd;
    iov.iov_len = sizeof(&p_buff_fd[0]);
    msg.msg_iov = &iov;
    msg.msg_iovlen = 1;

    msg.msg_control = ctrl;
    msg.msg_controllen = sizeof(ctrl);

    p_cmsg = CMSG_FIRSTHDR(&msg);
    if (p_cmsg == NULL)
    {
        rc = -5;
        goto EXIT_FLAG;
    }

    p_cmsg->cmsg_level = SOL_SOCKET;
    p_cmsg->cmsg_type = SCM_RIGHTS;
    p_cmsg->cmsg_len = CMSG_LEN(sizeof(int) * n_buffer);

    fdptr = (int *)CMSG_DATA(p_cmsg);
    for (uint32 idx = 0; idx < n_buffer; idx++)
    {
        fdptr[idx] = p_buff_fd[idx];
    }

    rc = sendmsg(p->fd, &msg, 0);
    if (rc != sizeof(&p_buff_fd[0]))
    {
        AIS_LOG_CONN_ERR("%p %d %s", p, rc, strerror(errno));
        rc = -4;
        goto EXIT_FLAG;
    }
    rc = 0;

EXIT_FLAG:

    pthread_mutex_unlock(&p->mutex);

EXIT_FLAG2:

    AIS_LOG_CONN(rc == 0 ? AIS_LOG_LVL_MED : AIS_LOG_LVL_ERROR,
                 "X %p %p %d %d", p, p_data_arr, n_buffer, rc);

    return rc;
}

/**
 * imports the buffers from another process/OS
 * shares buffer by file description which is handled by socket itself
 * @param p points to a connection structure
 * @param p_data_arr points to the array of buffers to be imported
 * @param p_flags points to buffer flags
 * @param size_arr array of buffer sizes
 * @param n_buffer the number of the exported buffer
 * @param timeout timed-out time in millisecond, not supported currently
 * @return 0: success, others: failure
 *
 */
int ais_conn_import(s_ais_conn *p, long long *p_data_arr, unsigned int* p_flags,unsigned int* size_arr,
         int n_buffer, unsigned int timeout)
{
    int rc = 0;
    int *p_data = NULL;
    int *fdptr;

    struct msghdr msg;
    struct iovec iov;
    char ctrl[CMSG_SPACE(sizeof(int) * n_buffer)];
    struct cmsghdr *p_cmsg;

    AIS_LOG_CONN_MED("E %p %p %d", p, p_data_arr, n_buffer);

    if (p == NULL || p_data_arr == NULL || n_buffer == 0)
    {
        rc = -1;
        goto EXIT_FLAG2;
    }

    rc = pthread_mutex_lock(&p->mutex);
    if (rc != 0)
    {
        rc = -2;
        goto EXIT_FLAG;
    }

    if (p->status != AIS_CONN_STAT_READY)
    {
        rc = -3;
        goto EXIT_FLAG;
    }

    memset(&msg, 0x0, sizeof(msg));

    iov.iov_base = &p_data;
    iov.iov_len = sizeof(p_data);
    msg.msg_iov = &iov;
    msg.msg_iovlen = 1;
    msg.msg_control = ctrl;
    msg.msg_controllen = sizeof(ctrl);

    rc = recvmsg(p->fd, &msg, 0);
    if (rc != sizeof(p_data) || p->abort)
    {
        AIS_LOG_CONN_ERR("%p %d %s %d", p, rc, strerror(errno), p->abort);
        rc = -4;
        goto EXIT_FLAG;
    }
    rc = 0;

    p_cmsg = CMSG_FIRSTHDR(&msg);
    if (p_cmsg == NULL
        || p_cmsg->cmsg_level != SOL_SOCKET
        || p_cmsg->cmsg_type != SCM_RIGHTS
        || p_cmsg->cmsg_len != CMSG_LEN(sizeof(int) * n_buffer))
    {
        rc = -5;
        goto EXIT_FLAG;
    }

    fdptr = (int *) CMSG_DATA(p_cmsg);

    for (uint32 idx = 0; idx < n_buffer; idx++)
    {
        p_data_arr[idx] = fdptr[idx];
    }

    if (rc == 0)
    {
        for(p->map_num[p->map_idx] = 0; p->map_num[p->map_idx] < n_buffer; p->map_num[p->map_idx]++)
        {
            p->map[p->map_idx][p->map_num[p->map_idx]].fd = (int)(uintptr_t)p_data_arr[p->map_num[p->map_idx]];
            p->map[p->map_idx][p->map_num[p->map_idx]].size = size_arr[p->map_num[p->map_idx]];
        }
    }

EXIT_FLAG:

    pthread_mutex_unlock(&p->mutex);

EXIT_FLAG2:

    AIS_LOG_CONN(rc == 0 ? AIS_LOG_LVL_MED : AIS_LOG_LVL_ERROR,
                 "X %p %p %d %d", p, p_data_arr, n_buffer, rc);

    return rc;
}

/**
 * frees buffer mapping for different operations results
 * @param p points to a connection structure
 * @param op indicates operation result
 * @return 0: success, others: failure
 *
 */
int ais_conn_flush(s_ais_conn *p, int op)
{
    int rc = 0;
    int i;

    AIS_LOG_CONN_MED("E %p %d", p, op);

    if (p == NULL)
    {
        rc = -1;
        goto EXIT_FLAG2;
    }

    rc = pthread_mutex_lock(&p->mutex);
    if (rc != 0)
    {
        rc = -2;
        goto EXIT_FLAG;
    }

    if (op == 0)
    {
        p->map_idx = !p->map_idx;
    }

    for (i = 0; i < p->map_num[p->map_idx]; i++)
    {
        close(p->map[p->map_idx][i].fd);
    }
    p->map_num[p->map_idx] = 0;

EXIT_FLAG:

    pthread_mutex_unlock(&p->mutex);

EXIT_FLAG2:

    AIS_LOG_CONN(rc == 0 ? AIS_LOG_LVL_MED : AIS_LOG_LVL_ERROR,
                "X %p %d %d", p, op, rc);

    return rc;
}

