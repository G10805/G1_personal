...
#define AIS_EVENT_QUEUE_MAX_NUM 2

/** maximum queue capacity */
#define AIS_EVENT_QUEUE_MAX_CAPACITY 64
...
typedef struct
{
    ...
    CameraQueue      queue[AIS_EVENT_QUEUE_MAX_NUM]; /**< priority queues */
    unsigned int     numQueues; /**< number of camera queues which are created */

    unsigned int     cnt; /**< event counter, starting from 0 */

    unsigned int (*fcn_event_2_idx)(qcarcam_event_t event); /**< conversion function from event type to camera queue index */
 ...
 *
 */
unsigned int ais_event_queue_event_2_idx(qcarcam_event_t event);

/**
 * initializes an event queue, and creates internal priority queues with specified capacity
 * The caller must have the event Q structure zeroed.
 *
...
int ais_event_queue_init(s_ais_event_queue *p, unsigned int numPriorityQueues, unsigned int queueCapacity,
        unsigned int (*fcn_event_2_idx)(qcarcam_event_t event));
