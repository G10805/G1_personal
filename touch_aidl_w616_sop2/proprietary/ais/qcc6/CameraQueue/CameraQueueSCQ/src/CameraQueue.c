 ...
 * @file CameraQueue.c
 *
 * @brief This module contains SCQ implementation of CameraQueue.h
 *
 * Copyright (c) 2009-2012,2014,2017,2019 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
#ifdef CAMERAQUEUE_ENABLE_LOGEVENT
#define CameraQueueLogEvent CameraQueueLogEvent
#else
#define CameraQueueLogEvent(...)
#endif
                        ...
                        const CameraQueueLockType eLockType)
{
    ...
    switch (eLockType)
    {
        ...
        case CAMERAQUEUE_LOCK_THREAD:
            ...
            {
                AIS_LOG(CAMERAQ, ERROR, "pthread_mutex_init failed : %s", strerror(err));
                err = 1;
            }
            break;
#ifndef DISABLE_QUEUE_LOCK_INTERRUPTS
        case CAMERAQUEUE_LOCK_IST:
            ...
            {
                AIS_LOG(CAMERAQ, ERROR, "ThreadCtl failed: %s", strerror(errno));
                err = 1;
            }
            else
            {
        ...
        default:
            ...
            AIS_LOG(CAMERAQ, ERROR, "Unimplemented locking mechanism : using no locks");
...
static int cameraQueueLockDeInit(cameraQueueCtx_t *pCameraQueue)
{
    ...
    if (CAMERAQUEUE_LOCK_THREAD == pCameraQueue->eLockType)
    {
        ...
        if (EOK != err)
        {
            AIS_LOG(CAMERAQ, ERROR, "pthread_mutex_destroy failed : %s", strerror(err));
            ret = 1;
        }
    }
    return ret;
                                                   ...
                                                   uint32                     *pValue)
{
    ...
    {
        AIS_LOG(CAMERAQ, ERROR, "invalid or destroyed queue handle");
        result = CAMERA_EBADHANDLE;
    }
    else if (0 != pCameraQueue->pFcnLock(&pCameraQueue->sLock))
    {
        AIS_LOG(CAMERAQ, ERROR, "failed to lock hQueue=0x%p", hQueue);
    ...
    else
    {
        ...
        {
            AIS_LOG(CAMERAQ, ERROR, "failed to unlock queue=0x%p", hQueue);
...
{
    CameraResult result = CAMERA_SUCCESS;
    CameraQueueLogEvent(CAMERA_CORE_EVENT_CAMERAQUEUECREATE, 0, 0);
         ...
         (pCreateParams->eLockType < CAMERAQUEUE_LOCK_NUM) ) /* valid locking type      */
    {
        ...
        {
            AIS_LOG(CAMERAQ, ERROR, "failed to allocate memory for queue");
        ...
        else
        {
            ...
            if (0 == (pCameraQueue->pArray = CameraAllocate(CAMERA_ALLOCATE_ID_CAMERA_QUEUE_ARRAY,
                                                            nArraySize)))
            {
                AIS_LOG(CAMERAQ, ERROR, "failed to allocate memory for queue data array");
            ...
            {
                AIS_LOG(CAMERAQ, ERROR, "failed to init lock type for queue");
...
CAM_API CameraResult CameraQueueDestroy(CameraQueue hQueue)
{
    ...
    CameraQueueLogEvent(CAMERA_CORE_EVENT_CAMERAQUEUEDESTROY, 0, 0);
    ...
    {
        AIS_LOG(CAMERAQ, ERROR, "possible concurrent destory or memory corruption");
        result = CAMERA_EHEAP;
    }
    else
    {
        ...
        if (0 != pCameraQueue->pFcnUnLock(&pCameraQueue->sLock))
        {
            AIS_LOG(CAMERAQ, ERROR, "pCameraQueue->pFcnUnLock failed ignored on 0x%p", pCameraQueue);
        }
        if (0 != cameraQueueLockDeInit(pCameraQueue))
        {
            AIS_LOG(CAMERAQ, ERROR, "cameraQueueOSDeInit failed ignored on 0x%p", pCameraQueue);
        }
...
CAM_API CameraResult CameraQueueClear(CameraQueue hQueue)
{
    ...
    CameraQueueLogEvent(CAMERA_CORE_EVENT_CAMERAQUEUECLEAR, 0, 0);
                                ...
                                const CameraQueueDataType dataIn)
{
    ...
    CameraQueueLogEvent(CAMERA_CORE_EVENT_CAMERAQUEUEENQUEUE, 0, 0);
    ...
    else
    {
        if (cameraQueueIsFull(pCameraQueue))
        {
            ...
            AIS_LOG(CAMERAQ, HIGH, "caller needs to allocate more elements than created");
        ...
        else
        {
            ...
            else
            {
                ...
                AIS_LOG(CAMERAQ, ERROR, "possible memory corruption detected");
                result = CAMERA_EHEAP;
            }
        }
                                ...
                                CameraQueueDataType dataOut)
{
    ...
    CameraQueueLogEvent(CAMERA_CORE_EVENT_CAMERAQUEUEDEQUEUE, 0, 0);
    ...
    else
    {
        ...
        else
        {
            ...
            {
                /* impossible to happen */
                AIS_LOG(CAMERAQ, ERROR, "possible memory corruption");
                result = CAMERA_EHEAP;
            }
        }
...
CAM_API CameraResult CameraQueueDropHead(CameraQueue hQueue)
{
    ...
    CameraQueueLogEvent(CAMERA_CORE_EVENT_CAMERAQUEUEDROPHEAD, 0, 0);
