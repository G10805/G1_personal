...
CAisV4l2ProxyStream* CAisV4l2ProxyStream::create_v4l2_stream(QCarCamHndl_t qcarcam_context, int fd,
...
CAisV4l2ProxyStream::CAisV4l2ProxyStream(QCarCamHndl_t qcarcam_context, int fd)
...
*/
    ...
    rc = QCarCamRegisterEventCallback(m_qcarcam_context, &qcarcam_event_cb, NULL);
    if (rc)
    {
    ...
    param = QCARCAM_EVENT_FRAME_READY | QCARCAM_EVENT_INPUT_SIGNAL |
        QCARCAM_EVENT_ERROR | QCARCAM_EVENT_VENDOR;

    rc = QCarCamSetParam(m_qcarcam_context, QCARCAM_STREAM_CONFIG_PARAM_EVENT_MASK, &param, sizeof(param));
    if (rc)
    {
    ...
    {
        rc = QCarCamSetParam(m_qcarcam_context,
                QCARCAM_STREAM_CONFIG_PARAM_ISP_USECASE,
                &pInput->m_isp_config[idx],
                sizeof(pInput->m_isp_config[idx]));
        if (rc != QCARCAM_RET_OK)
        {
        ...
        VPROXY_DBG1("setting ISP usecase instance %u isp_camera_id %u use_case %d - result = %d",
            pInput->m_isp_config[idx].id, pInput->m_isp_config[idx].cameraId, pInput->m_isp_config[idx].usecaseId, rc);
    }
...
void CAisV4l2ProxyStream::uninit()
{
    ...
    m_qcarcam_context == QCARCAM_HNDL_INVALID;
    m_batch_frames = 1;
    m_stream_fd = -1;
...
int CAisV4l2ProxyStream::processCloseEvent()
{
    ...
    rc = QCarCamClose(m_qcarcam_context);
    if (rc != QCARCAM_RET_OK)
    {
...
int CAisV4l2ProxyStream::processStartEvent()
{
    ...
    m_is_running = 1;
    rc = QCarCamStart(m_qcarcam_context);
    if (rc != QCARCAM_RET_OK)
    {
...
int CAisV4l2ProxyStream::processStopEvent()
{
    ...
    m_is_running = 0;
    rc = QCarCamStop(m_qcarcam_context);
    if (rc != QCARCAM_RET_OK)
    {
...
int CAisV4l2ProxyStream::processAllocBufs(struct v4l2_event & v4l2Event)
{
    ...
    if (nbufs > 0)
    {
        ...
        m_p_buffers_output.colorFmt = m_buffers_param[AIS_BUFFERS_DIRECT_OUTPUT].format;

        m_p_buffers_output.pBuffers = m_qcarcam_buffers;
        m_p_buffers_output.nBuffers = nbufs;

        for (int i = 0; i < nbufs; ++i)
        {
            m_p_buffers_output.pBuffers[i].numPlanes = 1;
            m_p_buffers_output.pBuffers[i].planes[0].width = m_buffers_param[AIS_BUFFERS_DIRECT_OUTPUT].width;
            m_p_buffers_output.pBuffers[i].planes[0].height =
        ...
        else
        {
            ...
            {
                bufs.fds[i] = (int)(uintptr_t)m_p_buffers_output.pBuffers[i].planes[0].memHndl;
            }
            ...
            ret = QCarCamSetBuffers(m_qcarcam_context, &m_p_buffers_output);
            if (ret != QCARCAM_RET_OK)
            {
    ...
    { //when allocbuf num is 0, free the buffers
        free_gfx_buffers(m_frame_buf_hndl);
        m_p_buffers_output.pBuffers = NULL;
    }
...
static QCarCamParamType_e mapV4l2ParamToQcarCamParam(uint8 v4l2_param, uint32 *size)
{
    switch (v4l2_param)
    {
        case AIS_V4L2_PARAM_EVENT_MASK:
            *size = sizeof(uint32_t);
            return QCARCAM_STREAM_CONFIG_PARAM_EVENT_MASK;
        case AIS_V4L2_PARAM_BRIGHTNESS:
            *size = sizeof(float);
            return QCARCAM_SENSOR_PARAM_BRIGHTNESS;
        case AIS_V4L2_PARAM_CONTRAST:
            *size = sizeof(float);
            return QCARCAM_SENSOR_PARAM_CONTRAST;
        case AIS_V4L2_PARAM_MIRROR_H:
            *size = sizeof(uint32_t);
            return QCARCAM_SENSOR_PARAM_MIRROR_H;
        case AIS_V4L2_PARAM_MIRROR_V:
            *size = sizeof(uint32_t);
            return QCARCAM_SENSOR_PARAM_MIRROR_V;
        case AIS_V4L2_PARAM_VID_STD:
            *size = sizeof(uint32_t);
            return QCARCAM_SENSOR_PARAM_VID_STD;
        case AIS_V4L2_PARAM_CURRENT_VID_STD:
            *size = sizeof(uint32_t);
            return QCARCAM_SENSOR_PARAM_CURRENT_VID_STD;
        case AIS_V4L2_PARAM_EXPOSURE:
            *size = sizeof(QCarCamExposureConfig_t);
            return QCARCAM_SENSOR_PARAM_EXPOSURE;
        case AIS_V4L2_PARAM_HUE:
            *size = sizeof(float);
            return QCARCAM_SENSOR_PARAM_HUE;
        case AIS_V4L2_PARAM_SATURATION:
            *size = sizeof(float);
            return QCARCAM_SENSOR_PARAM_SATURATION;
        case AIS_V4L2_PARAM_GAMMA:
            *size = sizeof(QCarCamGammaConfig_t);
            return QCARCAM_SENSOR_PARAM_GAMMA;
        case AIS_V4L2_PARAM_VENDOR:
            *size = sizeof(QCarCamVendorParam_t);
            return QCARCAM_VENDOR_PARAM;
        case AIS_V4L2_PARAM_INPUT_MODE:
            *size = sizeof(uint32_t);
            return QCARCAM_STREAM_CONFIG_PARAM_INPUT_MODE;
        case AIS_V4L2_PARAM_MASTER:
            *size = sizeof(uint32_t);
            return QCARCAM_STREAM_CONFIG_PARAM_MASTER;
        case AIS_V4L2_PARAM_EVENT_CHANGE_SUBSCRIBE:
            *size = sizeof(uint64_t);
            return QCARCAM_STREAM_CONFIG_PARAM_EVENT_CHANGE_SUBSCRIBE;
        case AIS_V4L2_PARAM_EVENT_CHANGE_UNSUBSCRIBE:
            *size = sizeof(uint64_t);
            return QCARCAM_STREAM_CONFIG_PARAM_EVENT_CHANGE_UNSUBSCRIBE;
        case AIS_V4L2_PARAM_BATCH_MODE:
            *size = sizeof(QCarCamBatchConfig_t);
            return QCARCAM_STREAM_CONFIG_PARAM_BATCH_MODE;
        case AIS_V4L2_PARAM_ISP_USECASE:
            *size = sizeof(QCarCamIspUsecaseConfig_t);
            return QCARCAM_STREAM_CONFIG_PARAM_ISP_USECASE;
        case AIS_V4L2_PARAM_EVENT_CB:
        case AIS_V4L2_PARAM_COLOR_FMT:
        case AIS_V4L2_PARAM_RESOLUTION:
        case AIS_V4L2_PARAM_STATUS:
        case AIS_V4L2_PARAM_LATENCY_MAX:
        case AIS_V4L2_PARAM_LATENCY_REDUCE_RATE:
        case AIS_V4L2_PARAM_PRIVATE_DATA:
        case AIS_V4L2_PARAM_INJECTION_START:
        case AIS_V4L2_PARAM_FRAME_RATE:
        case AIS_V4L2_PARAM_HDR_EXPOSURE:
        case AIS_V4L2_PARAM_OPMODE:
        case AIS_V4L2_PARAM_ISP_CTRLS:
        case AIS_V4L2_PARAM_RECOVERY:
        case AIS_V4L2_PARAM_NUM:
        default:
            *size = 0;
            return QCARCAM_PARAM_MAX;
    }
}
...
int CAisV4l2ProxyStream::processGetParamEvent(struct v4l2_event & v4l2Event)
{
    ...
    uint32 QcarSize = 0;
    QCarCamParamType_e QcarType = mapV4l2ParamToQcarCamParam(param_type, &QcarSize);
    rc = QCarCamGetParam(m_qcarcam_context, QcarType, param, QcarSize);
...
int CAisV4l2ProxyStream::processSetParamEvent(struct v4l2_event & v4l2Event)
{
    ...
    else
    {
        uint32 QcarSize = 0;
        QCarCamParamType_e QcarType = mapV4l2ParamToQcarCamParam(param_type, &QcarSize);
        rc = QCarCamSetParam(m_qcarcam_context, QcarType, param, QcarSize);
        if (rc != QCARCAM_RET_OK)
        {
...
#ifdef __AGL__
        if (param_type == QCARCAM_STREAM_CONFIG_PARAM_BATCH_MODE)
        {
            QCarCamBatchConfig_t* batch_cfg = (QCarCamBatchConfig_t*)param;
            m_batch_frames = batch_cfg->numBatchFrames;
            VPROXY_INFO("s_batch_mode %d", batch_cfg->numBatchFrames);
            if (m_batch_frames > 1)
            {
...
int CAisV4l2ProxyStream::processOutputBufReady()
{
    ...
    if (!dqbuf(m_stream_fd, idx))
    {
        ...
        if (m_is_buf_dequeued[idx])
        {
            ...
            rc = QCarCamReleaseFrame(m_qcarcam_context, m_frame_info.id, idx);
            if (rc)
            {
...
int CAisV4l2ProxyStream::poll_thread(void *arg)
{
    ...
    {
        QCarCamClose(stream_ctxt->m_qcarcam_context);
        stream_ctxt->m_qcarcam_context = QCARCAM_HNDL_INVALID;
    }
...
int CAisV4l2ProxyStream::qcarcam_get_new_frame()
{
    ...
    //if (m_batch_frames > 1)
    {
        ret = QCarCamGetFrame(m_qcarcam_context, &frame_info, DEFAULT_FRAME_TIMEOUT, 0);
    ...
    if (frame_info.bufferIndex >= m_buffers_param[AIS_BUFFERS_DIRECT_OUTPUT].n_buffers)
    {
        VPROXY_ERROR("Get frame context 0x%p ret invalid idx %d\n", m_qcarcam_context, frame_info.bufferIndex);
        return -1;
    }
    ...
    m_buf_idx_qcarcam = frame_info.bufferIndex;
    m_frame_info = frame_info;

    CameraLockMutex(m_bufstate_mutex);
    if (m_is_buf_dequeued[frame_info.bufferIndex])
    {
        VPROXY_ERROR("[input %d] has get frame %d", m_qcarcam_input_id, frame_info.bufferIndex);
    }
    m_is_buf_dequeued[frame_info.bufferIndex] = 1;
    CameraUnlockMutex(m_bufstate_mutex);

    VPROXY_DBG1("[input %d] get frame %d", m_qcarcam_input_id, frame_info.bufferIndex);
...
int CAisV4l2ProxyStream::qcarcam_post_frame_to_v4l2node()
{
    ...
    if (ais_v4l2_xml_cfg.latency_measurement_mode == CAMERA_LM_MODE_ALL_STEPS)
    {
        ...
        VPROXY_INFO("LATENCY| input_id %d Framecnt %d Framedone_timestamp: %llu, Cur_timestamp: %llu, latency from framedone: %llu us",
            ...
            ptimestamp,
            (ptimestamp - m_frame_info.timestamp) / 1000);
    }
    ...
    if (ret)
    {
        ...
        ret = QCarCamReleaseFrame(m_qcarcam_context, m_frame_info.id, m_buf_idx_qcarcam);
        if (ret)
        {
...
QCarCamRet_e CAisV4l2ProxyStream::qcarcam_event_cb(QCarCamHndl_t hndl, uint32_t event_id, const QCarCamEventPayload_t *p_payload, void  *pPrivateData)
{
    int i = 0;
    int rc = 0;
    ...
    switch (event_id)
    {
               ...
               if (ais_v4l2_xml_cfg.latency_measurement_mode == CAMERA_LM_MODE_ALL_STEPS)
                {
                    ...
                    VPROXY_INFO("LATENCY| qcarcam_Hndl %lu input_id %d Framecnt (%d, %d) aisbeserver_timestamp: %llu Cur_timestamp: %llu Latency_from_aisbeserver: %llu us",
                        ...
                        p_payload->frameInfo.seqNo,
                        p_payload->frameInfo.sofTimestamp.timestamp,
                        ptimestamp,
                        (ptimestamp - p_payload->frameInfo.sofTimestamp.timestamp) / 1000);
                }
                else
                {
                    VPROXY_DBG1("%d received QCARCAM_EVENT_FRAME_READY", stream_ctxt->m_qcarcam_input_id);
        ...
        case QCARCAM_EVENT_INPUT_SIGNAL:
        {
            ...
            rc = send_ioctrl_cmd(stream_ctxt->m_stream_fd,
                ...
                sizeof(p_payload->u32Data), (__u8*)&p_payload->u32Data);
            if (rc < 0)
            {
        ...
        case QCARCAM_EVENT_ERROR:
        {
            ...
            if (p_payload->errInfo.errorId == QCARCAM_ERROR_FATAL)
            {
...
CAisV4l2ProxyStream* CAisV4l2ProxyStream::find_stream(QCarCamHndl_t qcarcam_context)
{
