...
typedef struct
{
    QCarCamInput_t* camInputs;
    QCarCamMode_t* camModes[NUM_MAX_CAMERAS];
} qcarcam_input_t;
///////////////////////////////
/// STATICS
///////////////////////////////
...
static qcarcam_input_t g_pQcarcam_Inputs = { .camInputs = NULL, .camModes = {} };
...
static int query_inputs(qcarcam_input_t *pInputs, uint32 &queryFilled)
{
    int rc = 0;
    /*query qcarcam*/
    uint32 queryNumInputs = 0;
    QCarCamInput_t *pCamInputs;
    uint32 max_query_num = 100;
    uint32 i = 0;

    while (i++ < max_query_num)
    {
        rc = QCarCamQueryInputs(NULL, 0, &queryNumInputs);
    ...
    if (QCARCAM_RET_OK != rc || queryNumInputs == 0)
    {
        ...
        pInputs->camInputs = NULL;
        queryFilled = 0;
        return rc;
    }

    pCamInputs = (QCarCamInput_t *)calloc(queryNumInputs, sizeof(*pCamInputs));
    if (!pCamInputs)
    {
        VPROXY_ERROR("Failed to allocate pCamInputs");
        return 1;
    }

    rc = QCarCamQueryInputs(pCamInputs, queryNumInputs, &queryFilled);
    ...
    {
        if (pCamInputs[i].numModes == 0)
            continue;
        QCarCamInputModes_t inputModes = {};
        QCarCamMode_t *pCamModes = (QCarCamMode_t *)calloc(pCamInputs[i].numModes, sizeof(QCarCamMode_t));
        if (!pCamModes)
        {
            VPROXY_ERROR("Failed to allocate pCamModes");
            return 1;
        }
        inputModes.numModes = pCamInputs[i].numModes;
        inputModes.pModes = pCamModes;
        rc = QCarCamQueryInputModes(pCamInputs[i].inputId, &inputModes);
        if (QCARCAM_RET_OK != rc)
        {
            VPROXY_ERROR("Failed QCarCamQueryInputModes InputId %d with ret %d", i, rc);
            free(pCamModes);
            continue;
        }
        if (pInputs)
        {
            pInputs->camModes[i] = pCamModes;
        }
        VPROXY_INFO("input_id=%d, res=%dx%d fmt=0x%08x fps=%.2f flags=0x%x",
            pCamInputs[i].inputId,
            pCamModes->sources[0].width, pCamModes->sources[0].height,
            pCamModes->sources[0].colorFmt, pCamModes->sources[0].fps, pCamInputs[i].flags);
    }
...
static int init_v4l2_devices(ais_proxy_xml_input_t *xml_inputs, uint32 queryFilled)
{
    ...
    for (uint32 input_idx = 0; input_idx < numInputs; input_idx++)
    {
        ...
        QCarCamInput_t* p_qcarcam_input = NULL;
        QCarCamMode_t *p_qcarcam_mode = NULL;
        ...
        {
           if (g_pQcarcam_Inputs.camInputs[i].inputId == p_xml_input->qcarcam_input_id)
           {
               p_qcarcam_input = &g_pQcarcam_Inputs.camInputs[i];
               p_qcarcam_mode = g_pQcarcam_Inputs.camModes[i];
        ...
        g_ais_proxy_inputs[input_idx]->init(p_xml_input, p_qcarcam_input, p_qcarcam_mode);
...
int main(int argc, char **argv)
{
    ...
    if (QCC_FLAG_ENABLED) {
        if ((value[0] - '0') != QCARCAM_VERSION_MAJOR) {
            VPROXY_ERROR("Blocking ais_v4l2_proxy service for QCC.6.0");
    ...
    VPROXY_ERROR("ais v4l2loopback source (QCC.6.0) app started");
...
EXIT_FLAG:
    ...
    if (g_pQcarcam_Inputs.camInputs)
    {
        free(g_pQcarcam_Inputs.camInputs);
        g_pQcarcam_Inputs.camInputs = NULL;
    }

    for (uint32 i = 0; i < queryFilled; i++)
    {
        if (g_pQcarcam_Inputs.camModes[i])
        {
            free(g_pQcarcam_Inputs.camModes[i]);
            g_pQcarcam_Inputs.camModes[i] = NULL;
        }
    }
    ais_log_uninit();
