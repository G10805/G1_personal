...
#define MODULE_NAME "FormatMetaData"
...
static int GetHalFormats(const std::shared_ptr<V4L2Wrapper>& device,
  ...
  for (auto v4l2_format : v4l2_formats) {
    ...
    if (hal_format < 0) {
      HAL_LOGE("Unrecognized/unused format. Skip it.");
      continue;
    }
    result_formats->insert(hal_format);
  }
  // Fake HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED and HAL_PIXEL_FORMAT_BLOB 
  result_formats->insert(HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED);
  result_formats->insert(HAL_PIXEL_FORMAT_YCbCr_420_888);
  result_formats->insert(HAL_PIXEL_FORMAT_RGBA_8888);
  result_formats->insert(HAL_PIXEL_FORMAT_YCRCB_420_SP);
  result_formats->insert(HAL_PIXEL_FORMAT_BLOB);
...
int AddFormatComponents(
  ...
  if ((hal_formats.find(HAL_PIXEL_FORMAT_YCrCb_420_SP) == hal_formats.end())&&(hal_formats.find(HAL_PIXEL_FORMAT_YCbCr_420_888) == hal_formats.end())) {
  ...
  for (auto hal_format : hal_formats) {
    ...
    for (const auto& frame_size : frame_sizes) {
      ...
      HAL_LOGE(
            "Frame duration range for format %d, "
            "size %u x %u is %d %d",
            v4l2_format,
            frame_size[0],
            frame_size[1],duration_range[0],duration_range[1]);
#endif
      int64_t size_min_frame_duration = 33333333; //The maximum output frame rate of YUV420888 is 30fps
      /* this value is only experience value for the cts debug */
      int64_t size_max_frame_duration = 66666666;
