...
static uint32_t mClientCnt =0;
  ...
  std::condition_variable eventHandlerSignal;
...
V4L2Wrapper* V4L2Wrapper::NewV4L2Wrapper(int id, const V4L2CameraHAL::StreamConfigs_t& qcarcam_info) {
  return new V4L2Wrapper(id, qcarcam_info);
}

V4L2Wrapper::V4L2Wrapper(int id_, const V4L2CameraHAL::StreamConfigs_t qcarcam_info_)
    ...
    mAisFrameHeight(qcarcam_info_.height), mAisFrameFormat(qcarcam_info_.colorFmt),
...
QCarCamRet_e V4L2Wrapper::v4l2_ais_event_cb(const QCarCamHndl_t hndl, const uint32_t event_id, const QCarCamEventPayload_t *p_payload, void *pPrivateData)
{
    qcarcam_event_msg_t event_msg;
    // if(!this->qcarcam_context)
    // {
    //   HAL_LOGE("Event_cb called with invalid handle %p", hndl);
    //   return;
    // }

    event_msg.event_id = event_id;
    memcpy(&event_msg.payload, p_payload, sizeof(QCarCamEventPayload_t));

    std::unique_lock<std::mutex> lock(queue_mutex);
    eventqueue.push(event_msg);
    lock.unlock();
    eventHandlerSignal.notify_one();
    switch (event_id)
    {
...
int V4L2Wrapper::StreamOn() {
  ...
  std::unique_lock<std::mutex> lock(streamStatusLock);
    ...
    ret = QCarCamRegisterEventCallback(qcarcam_context, &v4l2_ais_event_cb, NULL);
    if (ret != QCARCAM_RET_OK) {
        HAL_LOGE("QCarCamRegisterEventCallback() failed with ret %d", ret);
        goto EXIT_FLAG;
    }
    ...
    ret = QCarCamSetParam(qcarcam_context, QCARCAM_STREAM_CONFIG_PARAM_EVENT_MASK, &param, sizeof(param));
    if (ret != QCARCAM_RET_OK) {
        HAL_LOGE("QCarCamSetParam() failed with ret %d",ret);
        goto EXIT_FLAG;
    }
    else
    {
        HAL_LOGV("QCarCamSetParam success");
    }

    ret = QCarCamStart(qcarcam_context);
    if (ret != QCARCAM_RET_OK) {
        HAL_LOGE("QCarCamStart() failed");
        goto EXIT_FLAG;

    } else
        HAL_LOGD("QCarCamStart() success for cam_id %d ",cam_id);
...
int V4L2Wrapper::StreamOff() {
  ...
  ret = QCarCamStop(qcarcam_context);
  if (QCARCAM_RET_OK != ret) {
      lock.unlock();
      HAL_LOGE("QCarCamStop() failed , returns %d ", ret);
      return 0;
  }
  else
    HAL_LOGV("QCarCamStop() success , returns %d ", ret);
  ...
  ret = QCarCamClose(qcarcam_context);
  if (QCARCAM_RET_OK != ret) {
      HAL_LOGE("QCarCamClose() failed , returns %d ", ret);
      return 0;
  }
    else {
        mClientCntStatus();
        HAL_LOGD("QCarCamClose() success , returns %d ", ret);
    }
 ...
 //Added to resolve OpenCamera freeze issue when trying to reopen the app
  ...
  for (int i = 0; i < (int)mQcarcamOutBufs.nBuffers; ++i) {
      ...
      if (mpGfxBufs && mpGfxBufs[i])
        ...
        HAL_LOGV("destroy & munmap success for buffer no. %d of %d ", i,(int)mQcarcamOutBufs.nBuffers);
  }
  if (mpGfxBufs)
      free(mpGfxBufs);
...
int V4L2Wrapper::v4l2QcarcamInit(void)
{
    ...
    char *processName = (char *)"ais_v4l2_hal_client_qcc6";
...
int V4L2Wrapper::openCamera()
{
    ...
    openParams.inputs[0].inputId = (uint32_t)cam_id;
    openParams.flags = QCARCAM_OPEN_FLAGS_RECOVERY;

    ret = QCarCamOpen(&openParams ,&qcarcam_context);
    if (ret != QCARCAM_RET_OK || qcarcam_context == QCARCAM_HNDL_INVALID) {
        HAL_LOGE("QCarCamOpen() failed");
        qcarcam_context = NULL;
        if (0 == mClientCnt)
            v4l2QcarcamDeInit();
        lock.unlock();
      ...
      mClientCnt++;
      HAL_LOGE("mClient count after QCarCamOpen is %d",mClientCnt);
      HAL_LOGE("QCarCamOpen() successful for camera id %d", cam_id);
    }
...
int V4L2Wrapper::allocInputBuffers()
{
    ...
    if (0 == mQcarcamOutBufs.nBuffers) {
        //allocate qcarcam internal pBuffers
        ...
        HAL_LOGE("Number of buffers QCarCamSetBuffers %d", mQcarcamOutBufs.nBuffers);
        QCarCamRet_e ret = QCARCAM_RET_OK;
        ret = QCarCamSetBuffers(qcarcam_context, &mQcarcamOutBufs);
        if (ret != QCARCAM_RET_OK) {
...
/*        std::cout <<"w="<< mQcarcamOutBufs.pBuffers[i].planes[0].width
                <<" h="<< mQcarcamOutBufs.pBuffers[i].planes[0].height
...
int V4L2Wrapper::RequestBuffers(uint32_t num_requested) {
  ...
  // V4L2 will set req_buffers.count to a number of pBuffers it can handle.
  if (num_requested > 0 && req_buffers.count < 1) {
    HAL_LOGE("REQBUFS claims it can't handle any pBuffers.");
    return -ENODEV;
  }
...
uint32_t V4L2Wrapper::getQcarcamToHalFormat(QCarCamColorFmt_e qcarcamFormat)
{
    switch(qcarcamFormat)
    {
        ...
        case QCARCAM_FMT_NV12:
            return HAL_PIXEL_FORMAT_YCbCr_420_SP;
        default: std::cout << "Unsupported HAL format 0x"<<std::hex<<qcarcamFormat;
            return -1;
    }
}
...
int32_t V4L2Wrapper::DoC2dScale(buffer_info_t * dst_buff_info) {
  HAL_LOG_ENTER();
  ...
  c2dObject.surface_id = mpQcarcamMmapBufs[mQcarcamFrameInfo.bufferIndex].srcC2dSurfaceId;
...
bool V4L2Wrapper::DequeueRequest() {
  ...
  eventHandlerSignal.wait(queue_lock);
  if (!eventqueue.empty())
  {
  ...
  ret = QCarCamGetFrame(qcarcam_context, &mQcarcamFrameInfo, QCARCAM_DEFAULT_GET_FRAME_TIMEOUT, 0);
  if (QCARCAM_RET_OK != ret) {
      HAL_LOGE("QCarCamGetFrame failed %p ret %d", qcarcam_context, ret);
      if (mCamera != NULL) {
  ...
  if (!mpQcarcamMmapBufs[mQcarcamFrameInfo.bufferIndex].ptr) {
      HAL_LOGE("buffer is not mapped");
            goto EXIT_FLAG;
  }

  pData = (void *)mpQcarcamMmapBufs[mQcarcamFrameInfo.bufferIndex].ptr;

  buffer_info_t * dst_buff_info = NULL;

   //DumpFrameBuffer((char *)pData, mpQcarcamMmapBufs[mQcarcamFrameInfo.bufferIndex].size, true);
  ...
  ret = QCarCamReleaseFrame(qcarcam_context, 0, mQcarcamFrameInfo.bufferIndex);
  if (QCARCAM_RET_OK != ret)
      HAL_LOGE("QCarCamReleaseFrame() failed %d", mQcarcamFrameInfo.bufferIndex);
