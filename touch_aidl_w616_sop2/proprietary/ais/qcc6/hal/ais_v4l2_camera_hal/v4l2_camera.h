...
class V4L2Camera : public default_camera_hal::Camera {
 public:
  ...
  static V4L2Camera* NewV4L2Camera(const config_port_metadata& config, const V4L2CameraHAL::StreamConfigs_t& qcarcam_info);
  ~V4L2Camera();

void enqueueCompleteRequest(std::shared_ptr<default_camera_hal::CaptureRequest> request);
std::shared_ptr<default_camera_hal::CaptureRequest> dequeueCompleteRequest();

 private:
  // Constructor private to allow failing on bad input.
  // Use NewV4L2Camera instead.
  V4L2Camera(const config_port_metadata& config,
  ...
  int initStaticInfo(android::hardware::camera::common::V1_0::helper::CameraMetadata* out) override;
  // Initialize a template of the given type.
  int initTemplate(int type, android::hardware::camera::common::V1_0::helper::CameraMetadata* out) override;
  ...
  bool isValidRequestSettings(const android::hardware::camera::common::V1_0::helper::CameraMetadata& settings) override;
  ...
  std::mutex mCompleteRequestQueueLock_;
  std::queue<std::shared_ptr<default_camera_hal::CaptureRequest>>
      mCompleteRequestQueue;
  std::condition_variable mCompleteRequestsAvailable_;
