...
int V4L2CameraHAL::v4l2QcarcamQueryInputs(void)
{
    ...
    {
        HAL_LOGE("Failed QCarCamQueryInputs number of inputs= %d  ret= %d", queryNumInputs, ret);
    }
    else
    {
        ...
        if (pAisInputList && mpStreamConfigs) {
            ...
            {
                HAL_LOGE("Failed QCarCamQueryInputs with ret = %d", ret);
            }
            else
            {
                ...
                for (i = 0; i < (int)mNumAisInputs; i++)
                {
                    ...
                    if (QCARCAM_RET_OK == ret) {
                        ...
                        config.device_idx = i;
                        config.port_idx = pAisInputList[i].inputId;
                        HAL_LOGD("%d, input_id = %d, inputName = %s, flags = %d, numModes = %d, width = %d, height = %d, colorFmt =%d, fps=%d", i, pAisInputList[i].inputId,
                        pAisInputList[i].inputName,
                        pAisInputList[i].flags,
                        pAisInputList[i].numModes,
                        mpStreamConfigs[i].width,
                        mpStreamConfigs[i].height,
                        mpStreamConfigs[i].colorFmt,
                        mpStreamConfigs[i].fps);
                    }
                    qcarcamInfo = mpStreamConfigs[i];
                    std::unique_ptr<V4L2Camera> cam(V4L2Camera::NewV4L2Camera(config, qcarcamInfo));
                    if (cam) {
                        mCameras.push_back(std::move(cam));
                    } else {
                        HAL_LOGE("Failed to initialize camera at %s.", pAisInputList[i].inputId);
                    }
