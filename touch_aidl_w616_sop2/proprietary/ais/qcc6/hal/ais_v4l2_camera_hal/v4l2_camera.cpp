...
V4L2Camera* V4L2Camera::NewV4L2Camera(const config_port_metadata& config, const V4L2CameraHAL::StreamConfigs_t& qcarcam_info)
{
  HAL_LOG_ENTER();

  std::shared_ptr<V4L2Wrapper> v4l2_wrapper(V4L2Wrapper::NewV4L2Wrapper(config.port_idx, qcarcam_info));
  if (!v4l2_wrapper) {
    HAL_LOGE("Failed to initialize V4L2 wrapper.");
    return nullptr;
  }
  std::unique_ptr<Metadata> metadata;
  int res = GetV4L2Metadata(config, v4l2_wrapper, &metadata);
  if (res) {
    HAL_LOGE("Failed to initialize V4L2 metadata: %d", res);
    return nullptr;
  }

  return new V4L2Camera(config, std::move(v4l2_wrapper), std::move(metadata));
}

V4L2Camera::V4L2Camera(const config_port_metadata& config,
                       std::shared_ptr<V4L2Wrapper> v4l2_wrapper,
                       std::unique_ptr<Metadata> metadata)
    : default_camera_hal::Camera(config),
...
int V4L2Camera::connect() {
  ...
  // connection_.reset(new V4L2Wrapper::Connection(device_)); // fd set as -1
  // if (connection_->status()) {
  //   HAL_LOGE("Failed to connect to device.");
  //   return connection_->status();
  // }

  res = device_->openCamera();
  if (res) {
      HAL_LOGE("Device Failed to open");
      return -ENODEV;
  }
  res = device_->allocInputBuffers();
  if (res) {
    HAL_LOGE("Failed to get Input Information or Buffer Request.");
    return -EIO;
  }
  res = device_->Init();
  if (res) {
...
int V4L2Camera::initStaticInfo(android::hardware::camera::common::V1_0::helper::CameraMetadata* out) {
  HAL_LOG_ENTER();
...
int V4L2Camera::initTemplate(int type, android::hardware::camera::common::V1_0::helper::CameraMetadata* out) {
  HAL_LOG_ENTER();
...
void V4L2Camera::initDeviceInfo(camera_info_t* info) {
   ...
   * combination of camera devices exposed by the HAL where the
   * sum of the resource costs of these cameras is <= 100.*/
  info->resource_cost = 10;
...
int V4L2Camera::initDevice() {
  ...
  if (!buffer_enqueuer_->isRunning()) {
    std::string threadName = "EQ_Request:" + std::to_string(mId);
    android::status_t res = buffer_enqueuer_->run(threadName.c_str());
    if (res != android::OK) {
  ...
  if (!buffer_dequeuer_->isRunning()) {
    std::string threadName = "DQ_CompReq:" + std::to_string(mId);
    android::status_t res = buffer_dequeuer_->run(threadName.c_str());
    if (res != android::OK) {
...
void V4L2Camera::enqueueCompleteRequest(
    std::shared_ptr<default_camera_hal::CaptureRequest> request) {
  HAL_LOG_ENTER();
  std::lock_guard<std::mutex> guard(mCompleteRequestQueueLock_);
  mCompleteRequestQueue.push(request);
  mCompleteRequestsAvailable_.notify_one();
  HAL_LOGV("---X size: %zu", mCompleteRequestQueue.size());
}

std::shared_ptr<default_camera_hal::CaptureRequest>
V4L2Camera::dequeueCompleteRequest() {
  std::unique_lock<std::mutex> lock(mCompleteRequestQueueLock_);
  while (mCompleteRequestQueue.empty()) {
    mCompleteRequestsAvailable_.wait(lock);
  }

  std::shared_ptr<default_camera_hal::CaptureRequest> request =
      mCompleteRequestQueue.front();
  mCompleteRequestQueue.pop();
  HAL_LOGV("--X left size: %zu", mCompleteRequestQueue.size());
  return request;
}

bool V4L2Camera::dequeueRequestBuffers() {
  HAL_LOGV("in flight count:%d", in_flight_buffer_count_);
  // Dequeue a completed request from v4l2 wrapper.
  std::shared_ptr<default_camera_hal::CaptureRequest> request =
                              dequeueCompleteRequest();

  if (request) {
    completeRequest(request, request->output_buffers[0].status);

    std::unique_lock<std::mutex> lock(in_flight_lock_);
    if (in_flight_buffer_count_ > 0) {
      in_flight_buffer_count_--;
    }
  }
...
int V4L2Camera::setupStreams(camera3_stream_configuration_t* stream_config) {
  ...
  HAL_LOGV("v4l2api::V4L2Camera::setupStreams");
  ...
  uint32_t height = stream->height;
    ...
    // It is now possible to have 3 different functional streams working simultaneously.
    // Note that video function NOT supported yet.
  for (uint32_t i = 0; i < stream_config->num_streams; ++i) {
      stream = stream_config->streams[i];
      HAL_LOGD("stream %d is format %d, width %u, height %u). type = %d  usage = %x ",
              i,
              stream->format,
              stream->width,
              stream->height,
  ...
  for (uint32_t i = 0; i < stream_config->num_streams; ++i) {
    ...
    if (stream->format == HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED) {
     if (stream->usage != 3) stream->format = HAL_PIXEL_FORMAT_RGBA_8888;
      // For video recording Sending default format.
    }
    ...
    switch (stream->stream_type) {
      case CAMERA3_STREAM_INPUT:
        ...
        HAL_LOGV("Stream is GRALLOC_USAGE_SW_READ_OFTEN");
        break;
      case CAMERA3_STREAM_OUTPUT:
        stream->usage = GRALLOC_USAGE_SW_WRITE_OFTEN;
        HAL_LOGV("Stream is GRALLOC_USAGE_SW_WRITE_OFTEN");
        break;
      case CAMERA3_STREAM_BIDIRECTIONAL:
        stream->usage =
            GRALLOC_USAGE_SW_READ_OFTEN | GRALLOC_USAGE_SW_WRITE_OFTEN;
        HAL_LOGV("Stream is GRALLOC_USAGE_SW_READ_OFTEN | GRALLOC_USAGE_SW_WRITE_OFTEN");
        break;
      default:
        // nothing to do.
        break;
...
bool V4L2Camera::isValidRequestSettings(
    const android::hardware::camera::common::V1_0::helper::CameraMetadata& settings) {
