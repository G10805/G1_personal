...
bool StaticProperties::SanityCheckStreamConfiguration(
  ...
  for (size_t i = 0; i < stream_config->num_streams; ++i) {
    ...
    if (stream == nullptr) {
      ALOGE("%s: Stream %lu is null", __func__, i);
      return false;
    } else if (!IsInputType(stream->stream_type) &&
               !IsOutputType(stream->stream_type)) {
      ALOGE("%s: Stream %lu type %lu is neither an input nor an output type",
            __func__,
            i,
            stream->stream_type);
      return false;
...
bool StaticProperties::InputStreamsSupported(
  ...
  if (num_input_streams > max_input_streams_) {
    ALOGE(
        "%s: Requested number of input streams %lu is greater than "
...
bool StaticProperties::OutputStreamsSupported(
  ...
  for (unsigned int i = 0; i < stream_config->num_streams; ++i) {
    ...
    if (IsOutputType(stream->stream_type)) {
      ...
      if (stream->format!= HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED
          ...
          && (capabilities_iterator == stream_capabilities_.end()
  ...
  if (num_raw > max_raw_output_streams_) {
    ALOGE(
        ...
        "raw output streams %d (requested %lu).",
        __func__,
    ...
    ALOGE(
        "%s: Requested stream configuration exceeds maximum supported "
        "stalling output streams %d (requested %lu).",
        __func__,
    ...
    ALOGE(
        "%s: Requested stream configuration exceeds maximum supported "
        "non-stalling output streams %d (requested %lu).",
        __func__,
