...
V4L2Wrapper::V4L2Wrapper(const std::string device_path)
  ...
  if (!mBufferDequeueThread->isRunning()) {
    std::string threadName = "DQBUF:" + device_path_.substr(device_path_.find("video"));
    std::string videoName = device_path_.substr(device_path_.find("video"));
...
int V4L2Wrapper::Connect() {
  ...
  device_fd_.reset(fd);
  HAL_LOGV("Device %s opened with fd:%d", device_path_.c_str(), device_fd_.get());
  if (mInputWidth == 0) {
...
void V4L2Wrapper::Disconnect() {
  ...
  HAL_LOGV("fd after closing the device is fd:%d", device_fd_.get());
  HAL_LOG_EXIT();
}
...
int V4L2Wrapper::StreamOff() {
  ...
  if (res < 0) {
      HAL_LOGE(" VIDIOC_STREAMOFF fails: %s", strerror(errno));
      lock.unlock();
      return -ENODEV;
  }
...
int V4L2Wrapper::GetInputInformation() {
  ...
  memset(&k_format, 0, sizeof(k_format));
  k_format.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
  ret = IoctlLocked(VIDIOC_G_FMT, &k_format);
  if(ret < 0) {
      HAL_LOGE("ERROR: VIDIOC_G_FMT: %s", strerror(errno));
      return -1;
  }

  mInputWidth = k_format.fmt.pix.width;
  mInputHeight = k_format.fmt.pix.height;

  // The V4L2 loopback driver crop setting is initialised to the maximum supported resolution
  // So if the crop is not set by the user space, take the image resolution as crop instead
  struct v4l2_crop k_crop;
  ret = IoctlLocked(VIDIOC_G_CROP, &k_crop);
  if (ret < 0) {
      HAL_LOGE("ERROR: VIDIOC_G_CROP failed, ignoring CROP: %s", strerror(errno));
      mInputCropLeft = 0;
      mInputCropTop = 0;
      mInputCropWidth = mInputWidth;
      mInputCropHeight = mInputHeight;
  } else {
      mInputCropLeft = MIN(k_crop.c.left, mInputWidth - 1);
      mInputCropTop = MIN(k_crop.c.top, mInputHeight - 1);
      mInputCropWidth = MIN(k_crop.c.width, mInputWidth - mInputCropLeft);
      mInputCropHeight = MIN(k_crop.c.height, mInputHeight - mInputCropTop);
  }

  mInputHalFormat = ConvertToHalFormat(k_format.fmt.pix.pixelformat);
    ...
    for(auto it = request->output_buffers.begin();
...
#else
      uint64_t buf_id = get_native_hnd_buffer_id(*(it->buffer));
      ...
      if (buff_info != nullptr) {
        if (buf_id != buff_info->id || buf_size != buff_info->length ||
          ...
          HAL_LOGE("ERROR: request buffer parameter is not match with created c2d surface");
          HAL_LOGE("ERROR: surface fd:%d, offset:%d, size:%d, format:%s, id:%" PRIu64 ", "
                   "output fd:%d, size:%d, format:%s, id:%lu",
                  buff_info->fd, buff_info->offset,
                  buff_info->length, printHalFormat(buff_info->halFormat), buff_info->id,
...
int32_t V4L2Wrapper::DoC2dScale(buffer_info_t * src_buff_info, buffer_info_t * dst_buff_info) {
  HAL_LOG_ENTER();
  if (src_buff_info == NULL || dst_buff_info == NULL) {
    HAL_LOGE("ERROR: bad parameter");
    return -1;
  }
  std::lock_guard<std::mutex> lock_src(src_buff_info->lock);
  std::lock_guard<std::mutex> lock_dst(dst_buff_info->lock);

  if (!src_buff_info->isUsed || !dst_buff_info->isUsed) {
    HAL_LOGE("ERROR: check buffer still valid: src:%d, dst:%d",
                src_buff_info->isUsed, dst_buff_info->isUsed);
    return -1;
  }
  ...
  c2dObject.surface_id = src_buff_info->surfaceId;

  c2dObject.source_rect.height = src_buff_info->cropHeight << 16;
  c2dObject.source_rect.width  = src_buff_info->cropWidth << 16;

  c2dObject.source_rect.x      = src_buff_info->cropLeft << 16;
  c2dObject.source_rect.y      = src_buff_info->cropTop << 16;
...
bool V4L2Wrapper::DequeueRequest() {
  ...
  lock.unlock();
  struct pollfd pfd;
  pfd.fd = device_fd_.get();
  pfd.events = POLLIN|POLLRDNORM;
  HAL_LOGV("polling on fd %d",pfd.fd);
  int rc = 0;
  rc = poll(&pfd, 1, MAX_POLL_TIME); //timout of 33ms
  if (0 == rc) {
      HAL_LOGV("poll timeout, exiting");
      return true;
  }
  if ((rc > 0) && (pfd.revents & POLLIN))
  {
      ...
      if (res) {
          if (errno == EAGAIN) {
              HAL_LOGE("DQBUF fails: %s (Expected failure)", strerror(errno));
              memset(&buffer, 0, sizeof(buffer));
              buffer.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
              buffer.memory = V4L2_MEMORY_MMAP;
              res = IoctlLocked(VIDIOC_DQBUF, &buffer);
              if (res) {
                  HAL_LOGE("DQBUF ERROR: %s still failed, so it is a fatal error!", strerror(errno));
                  if (mCamera != NULL) {
              ...
              // Unexpected failure : When node/resourse is not present,
              // it will return with no device error
              HAL_LOGE("DQBUF ERROR: %s (Unexpected failure)", strerror(errno));
              if (mCamera != NULL) {
                  request->output_buffers[0].status = CAMERA3_BUFFER_STATUS_ERROR;
                  mCamera->enqueueCompleteRequest(request);
              }
      ...
      if (buffer.index >= MAX_INPUT_BUFFER_NUM || buffer.index < 0) {
          HAL_LOGE("ERROR: invalid kernel buffer index:%d", buffer.index);
          if (mCamera != NULL) {
              request->output_buffers[0].status = CAMERA3_BUFFER_STATUS_ERROR;
              mCamera->enqueueCompleteRequest(request);
          }
          IoctlLocked(VIDIOC_QBUF, &buffer);
          return true;
      }

      buffer_info_t * input_buff_info = &input_buffer_infos[buffer.index];
      buffer_info_t * dst_buff_info = NULL;

      if (!input_buff_info->isUsed) {
          HAL_LOGE("ERROR: invalid input buff info: %d", buffer.index);
          if (mCamera != NULL) {
              request->output_buffers[0].status = CAMERA3_BUFFER_STATUS_ERROR;
              mCamera->enqueueCompleteRequest(request);
          }
      ...
      HAL_LOGV("dequeue buffer index: %d, virtual add:%p, mFrameNum:%d",
              buffer.index, input_buff_info->virtPtr, mFrameNum);
      // DumpFrameBuffer((char *)input_buff_info->virtPtr, input_buff_info->length, true);

      for(auto it = request->output_buffers.begin();
              it != request->output_buffers.end(); it++){
#if defined(ANDROID_Q_AOSP)
          const private_handle_t *hnd_output = PRIV_HANDLE_CONST(*(it->buffer));
          dst_buff_info = GetOutputBufferInfo(hnd_output->id);
          if (dst_buff_info == nullptr) {
              HAL_LOGE("ERROR: can't find output c2d surface, format:0x%x, w:%d,h:%d, id:%" PRIu64 "",
                      it->stream->format, it->stream->width, it->stream->height, hnd_output->id);
              if (mCamera != NULL) {
                  request->output_buffers[0].status = CAMERA3_BUFFER_STATUS_ERROR;
                  mCamera->enqueueCompleteRequest(request);
              }
          ...
          if (dst_buff_info == nullptr) {
              HAL_LOGE("ERROR: can't find output c2d surface, format:0x%x, w:%d,h:%d, id:%lu",
          ...
          HAL_LOGV(" output c2d surface, format:0x%x, w:%d,h:%d, id:%d",
                  it->stream->format, it->stream->width, it->stream->height, buf_id);
          res = DoC2dScale(input_buff_info, dst_buff_info);
          if (res != 0) {
      ...
      if (mCamera != NULL) {
          request->output_buffers[0].status = (V4L2_BUF_FLAG_ERROR & buffer.flags) ? CAMERA3_BUFFER_STATUS_ERROR : 0;
          if (V4L2_BUF_FLAG_LAST & buffer.flags) {
              // V4L2_BUF_FLAG_LAST is set when AIS self recovery has aborted
              request->output_buffers[0].status = CAMERA3_BUFFER_STATUS_DEVICE_ERROR | CAMERA3_BUFFER_STATUS_ERROR;
          }
          mCamera->enqueueCompleteRequest(request);
      }

      // Queue back the buffer to v4l2loopback for next frame
      IoctlLocked(VIDIOC_QBUF, &buffer);
  }
  return true;
}
