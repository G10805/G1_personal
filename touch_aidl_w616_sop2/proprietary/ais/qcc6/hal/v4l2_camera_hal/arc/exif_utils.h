...
class ExifUtils {
 public:
  ...
  // Used when setting the following TAG values:
  //   EXIF_TAG_DATE_TIME_DIGITIZED
  bool SetTime(ExifIfd ifd, ExifTag TimeTAG  ,const struct tm& t);

  // Used when setting the following TAG values:
  //   EXIF_TAG_SUB_SEC_TIME
  //	 EXIF_TAG_SUB_SEC_TIME_ORIGINAL
  //   EXIF_TAG_SUB_SEC_TIME_DIGITIZED
  bool SetAscii(ExifIfd ifd, ExifTag tag ,const std::string& Ascii );

  // Used when setting the following TAG values:
  //   EXIF_TAG_APERTURE_VALUE
  //   EXIF_TAG_FNUMBER
  //   EXIF_TAG_EXPOSURE_TIME
  bool SetRational(ExifIfd ifd, ExifTag tag , uint16_t numerator , uint16_t denominator);

  // Used when setting the following TAG values:
  //   EXIF_TAG_FLASH
  //   EXIF_TAG_WHITE_BALANCE
  //   EXIF_TAG_ISO_SPEED_RATINGS
  bool SetShort(ExifIfd ifd, ExifTag tag , uint16_t value);

  // Set some  default TAG for CTS.
  bool SetDefaultTag();
