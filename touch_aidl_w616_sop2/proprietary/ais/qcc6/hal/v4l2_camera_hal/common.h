...
#define HAL_LOGE(fmt, args...) do { \
    ALOGE("%s:%s:%d: " fmt, MODULE_NAME, __FUNCTION__, __LINE__, ##args);   \
  } while(0)

#define HAL_LOGE_IF(cond, fmt, args...) do { \
    ALOGE_IF(cond, "%s:%s/%s:%d: " fmt, MODULE_NAME, __FUNCTION__, __LINE__, ##args);  \
  } while(0)

#define HAL_LOGW(fmt, args...) do { \
    ALOGW("%s:%s:%d: " fmt, MODULE_NAME, __FUNCTION__, __LINE__, ##args);   \
  } while(0)

#define HAL_LOGW_IF(cond, fmt, args...) do { \
    ALOGW_IF(cond, "%s:%s:%d: " fmt, MODULE_NAME, __FUNCTION__, __LINE__, ##args);  \
  } while(0)

#define HAL_LOGI(fmt, args...) do { \
    ALOGI("%s:%s:%d: " fmt, MODULE_NAME, __FUNCTION__, __LINE__, ##args);   \
  } while(0)

#define HAL_LOGI_IF(cond, fmt, args...) do { \
    ALOGI_IF(cond, "%s:%s:%d: " fmt, MODULE_NAME, __FUNCTION__, __LINE__, ##args);  \
  } while(0)

#define HAL_LOGD(fmt, args...) do { \
    ALOGD("%s:%s:%d: " fmt, MODULE_NAME, __FUNCTION__, __LINE__, ##args);   \
  } while(0)

#define HAL_LOGV(fmt, args...) do { \
    ALOGV("%s:%s:%d: " fmt, MODULE_NAME, __FUNCTION__, __LINE__, ##args);   \
  } while(0)
