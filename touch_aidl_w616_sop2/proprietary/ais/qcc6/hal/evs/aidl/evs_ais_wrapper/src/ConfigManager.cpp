...
const char * getQcarCamPos(uint32_t inputId) {
    std::string camPos[]={"rear","front","left","right",
        "driver","lane","digital","analog",
        "gesture","fingerprint","iris","tuner"};
    return camPos[inputId].c_str();
}
...
std::unique_ptr<ConfigManager::CameraInfo>& ConfigManager::setCameraInfo(const std::string cameraId,
        StreamConfiguration *pStreamConfigs) {
    std::unique_lock<std::mutex> lock(mConfigLock);

    const char *id = cameraId.c_str();
    const char *pos = getQcarCamPos(pStreamConfigs->inputId);
    LOG(INFO) << "Storing camera info "<<pStreamConfigs->inputId;
    ...
    int32_t height = (int32_t)pStreamConfigs->height;
    int32_t width = (int32_t)pStreamConfigs->width;
    int32_t framerate = (int32_t)pStreamConfigs->fps;
