...
#if defined(_LINUX) || defined(__AGL__)
#include <sys/mman.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
...
#ifdef __QNXNTO__
static uintptr_t s_MPMVaddr = 0;
#define MPM_ADDR 0xC221000
#define MPM_MAPSIZE 0x1000           // 1 page size for MPM timer

#elif defined(__AGL__) || defined(ANDROID)
static int s_MPMFd = 0;
static pthread_mutex_t mpm_mutex;
#endif

#define MPM_FREQ 32768
static int s_useMPM = 0;
...
boolean CameraGetTime(uint64* pnCurrentTime)
{
    ...
    if (s_useMPM)
    {
        ...
        if (s_MPMVaddr)
        {
            int64 tm = 0;
            tm = in32(s_MPMVaddr);
            *pnCurrentTime = tm * 1000000000 / MPM_FREQ;
            bResult = TRUE;
        }
#elif defined(__AGL__) || defined(ANDROID)
        pthread_mutex_lock(&mpm_mutex);
        double mpm_time;
        size_t rc_read;
        ...
        {
            int counter;
            rc_read = read(s_MPMFd, &counter, 4);
            mpm_time = (double)counter / MPM_FREQ;
        }
        else
        {
            pthread_mutex_unlock(&mpm_mutex);
            return bResult;
        }

        *pnCurrentTime = (int64)(mpm_time * 1000000000);
        bResult = TRUE;
        pthread_mutex_unlock(&mpm_mutex);
#endif
    }
...
void CameraBusyWait(uint32 nNanosecondsToWait)
{
    ...
    {
        if (EOK != (status = clockcycles_nanospin_ns(nNanosecondsToWait)))
        {
            CAM_MSG(ERROR, "clockcycles_nanospin_ns: %s", strerror(status));
        }
    }
#endif
    else
 ...
 */
void CameraMicroSleep(uint32 nMicroSec)
{
...
#ifdef __QNXNTO__
    else if (nMicroSec < 2000U) /* less than 2ms */
    {
        if (EOK != (status = clockcycles_nanospin_ns(nMicroSec * 1000U)))
        {
            CAM_MSG(ERROR, "clockcycles_nanospin_ns: %s", strerror(status));
        }
    }
#endif
    else if (nMicroSec < 1000000U) /* less than 1 sec */
    {
        /* timing profile for usleep is better than clock_nanosleep;
         * so, whenever possible, we want to use usleep */
        if (-1 == usleep(nMicroSec))
        {
            CAM_MSG(ERROR, "usleep : %s", strerror(errno));
        }
    }
    ...
    {
        struct timespec remain, req;
        /* based on profiling, there is no advantage to eliminate
         * the following division and multiplications in release
         * build */
        remain.tv_sec    = nMicroSec/1000000U;
        nMicroSec -= remain.tv_sec*1000000U;
        remain.tv_nsec   = nMicroSec*1000U;
...
#else /* !defined(__QNXNTO__) && !defined(__INTEGRITY) */
    usleep(nMicroSec);
#endif
}
...
CameraResult CameraEnableMPMTimer(void)
{
    int rc = CAMERA_SUCCESS;
#ifdef __QNXNTO__
    if (!s_MPMVaddr)
    {
        s_MPMVaddr = mmap_device_io(MPM_MAPSIZE, MPM_ADDR);
        if (s_MPMVaddr == MAP_DEVICE_FAILED)
        {
            CAM_MSG(ERROR, "EnableMPMTimer fail\n");
            rc = CAMERA_EFAILED;
        }
        else
        {
            CAM_MSG(ALWAYS, "EnableMPMTimer succeed %u\n", s_MPMVaddr);
            s_useMPM = 1;
        }
    }
#elif defined(__AGL__)
    s_MPMFd = open("/dev/mpm", O_RDONLY);
    if (s_MPMFd == -1)
    {
        CAM_MSG(ERROR, "Open /dev/mpm failed %s", strerror(errno));
        rc = CAMERA_EFAILED;
    }
    else
    {
        CAM_MSG(ALWAYS, "Open /dev/mpm succeed %d", s_MPMFd);
        s_useMPM = 1;
    }

    pthread_mutex_init(&mpm_mutex, 0);
#endif

    return rc;
}

void CameraDisableMPMTimer(void)
{
#ifdef __QNXNTO__
    if (s_MPMVaddr)
    {
        s_useMPM = 0;
        munmap_device_io(s_MPMVaddr, MPM_MAPSIZE);
        s_MPMVaddr = 0;
    }
#elif defined(__AGL__)
    if (s_MPMFd != -1)
    ...
    pthread_mutex_destroy(&mpm_mutex);
#endif
}
    ...
    CameraThread* ppHandle)
{
    ...
    return MM_Thread_CreateEx(sPriorityMap[nPriority], nSuspend, pfnStart, pvStartArg, nStackSize, (char*)pstrName, ppHandle) ?
 ...
 */
CameraResult CameraReleaseThread(CameraThread pHandle)
{
    return MM_Thread_Release(pHandle) ? CAMERA_EFAILED : CAMERA_SUCCESS;
}

/**
 * This function terminates a thread
 */
CameraResult CameraExitThread(CameraThread pHandle, int nExitCode)
{
    return MM_Thread_Exit(pHandle, nExitCode) ? CAMERA_EFAILED : CAMERA_SUCCESS;
}

/**
 * This function joins a thread
 */
CameraResult CameraJoinThread(CameraThread pHandle, void* pExitCode)
{
    return MM_Thread_Join(pHandle, pExitCode) ? CAMERA_EFAILED : CAMERA_SUCCESS;
}

/**
 * This function detaches a thread.
 */
CameraResult CameraDetachThread(CameraThread pHandle)
{
#if defined(__QNXNTO__) || defined(__INTEGRITY)
    return MM_Thread_Detach(pHandle) ? CAMERA_EFAILED : CAMERA_SUCCESS;
#else
 ...
 */
CameraResult CameraSetThreadPriority(CameraThread pHandle, CameraThreadPriorityType nPriority)
{
    if (nPriority >= CAMERA_THREAD_PRIO_MAX)
    {
        nPriority = CAMERA_THREAD_PRIO_MAX - 1;
    ...
    return MM_Thread_SetPriority(pHandle, sPriorityMap[nPriority]) ? CAMERA_EFAILED : CAMERA_SUCCESS;
}
...
CameraResult CameraOpenFile(CameraOSFileHandle_t *handle, const char *pFileName, const char *mode)
{
    ...
    if (handle && pFileName)
    {
        handle->pFileHandle = fopen(pFileName, mode);
        ...
        {
            CAM_MSG(ERROR, "%s, %s", pFileName, strerror(errno));
...
CameraResult CameraReadFile(char * pFileName, void ** ppBuff, unsigned int nMaxSize)
{
    CameraResult         result;
    ...
    /* open file handle */
    result = CameraOpenFile(&handle, pFileName, "rb");
...
CameraResult CameraWriteFile(char * pFileName, void * pBuff, unsigned int nMaxSize)
{
    CameraResult         result;
    CameraOSFileHandle_t handle;

    CAM_MSG(ALWAYS, "Dumping %s", pFileName);

    /* open file handle */
    result = CameraOpenFile(&handle, pFileName, "wb");
    if (CAMERA_SUCCESS == result)
    {
        size_t written = fwrite((void*)pBuff,
                                1,
                                nMaxSize,
                                handle.pFileHandle);
        ...
        if (written != nMaxSize)
        {
            CAM_MSG(ERROR, "%s, %d != %d %s", pFileName, written, nMaxSize, strerror(errno));
            result = CAMERA_EFAILED;
        }
...
CameraResult CameraCreateTimer(int nTimeOut, int nPeriodic, void (*pfnTimerHandler)(void* ), void *pvUserArg, CameraTimer *ppTimer)
{
    return MM_Timer_Create(nTimeOut, nPeriodic, pfnTimerHandler, pvUserArg, ppTimer)? CAMERA_EFAILED : CAMERA_SUCCESS;
}

CameraResult CameraUpdateTimer(CameraTimer pTimer, int nTimeOut)
{
    return MM_Timer_Start(pTimer, nTimeOut)? CAMERA_EFAILED : CAMERA_SUCCESS;
}

CameraResult CameraStopTimer(CameraTimer pTimer)
{
    return MM_Timer_Stop(pTimer)? CAMERA_EFAILED : CAMERA_SUCCESS;
}

CameraResult CameraReleaseTimer(CameraTimer pTimer)
{
    return MM_Timer_Release(pTimer)? CAMERA_EFAILED : CAMERA_SUCCESS;
}
