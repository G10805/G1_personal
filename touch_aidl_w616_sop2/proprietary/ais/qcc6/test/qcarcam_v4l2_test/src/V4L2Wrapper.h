...
public:
    ...
    int streamon();
    int streamoff();
    int requestbuf(uint32 num, QCarCamBufferList_t *buffers, bool use_buf_width=false);
    int releasebufs();
    ...
    static QCarCamColorFmt_e toQcarcamFormat(unsigned int fmt);

private:
    int io_ctrl(uint32 type, void* pData);
    int    request_mmapbuf_bottomhalf(uint32 num);
    int    request_dmabuf_bottomhalf(uint32 num, QCarCamBufferList_t *buffers);
