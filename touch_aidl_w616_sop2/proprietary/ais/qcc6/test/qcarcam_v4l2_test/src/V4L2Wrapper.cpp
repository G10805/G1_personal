...
int V4L2Wrapper::request_dmabuf_bottomhalf(uint32 num, QCarCamBufferList_t *buffers)
{
    v4l2_buffer device_buffer;
    int rc = 0;
    ...
    for (uint32 idx =0;idx < num; ++idx)
    {
        ...
        if (buffers != NULL)
        {
            buffers->pBuffers[idx].planes[0].memHndl = (uint64_t)(uintptr_t)expbuf.fd;
        }
        else
        {
...
int V4L2Wrapper::requestbuf(uint32 num, QCarCamBufferList_t *buffers, bool use_buf_width)
{
...
QCarCamColorFmt_e V4L2Wrapper::toQcarcamFormat(unsigned int fmt)
{
    QCarCamColorFmt_e q_fmt = QCARCAM_FMT_UYVY_8;
    switch(fmt)
    {
