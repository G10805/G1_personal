...
QCarCamRet_e test_util_standalone_init(test_util_ctxt_t *ctxt)
{
    ais_initialize(NULL);
...
QCarCamRet_e test_util_standalone_deinit(test_util_ctxt_t *ctxt)
{
    ais_uninitialize();
    return QCARCAM_RET_OK;
}
...
PUBLIC_API QCarCamRet_e QCarCamQueryInputs(qcarcam_input_t* p_inputs, unsigned int size, unsigned int* ret_size)
{
    return camera_to_qcarcam_result(ais_query_inputs(p_inputs, size, ret_size));
}

PUBLIC_API qcarcam_hndl_t QCarCamOpen(uint32_t desc)
{
    return ais_open(desc);
}

PUBLIC_API QCarCamRet_e QCarCamClose(qcarcam_hndl_t hndl)
{
    return camera_to_qcarcam_result(ais_close(hndl));
}

PUBLIC_API QCarCamRet_e QCarCamGetParam(qcarcam_hndl_t hndl, qcarcam_param_t param, qcarcam_param_value_t* p_value)
{
    return camera_to_qcarcam_result(ais_g_param(hndl, param, p_value));
}

PUBLIC_API QCarCamRet_e QCarCamSetParam(qcarcam_hndl_t hndl, qcarcam_param_t param, const qcarcam_param_value_t* p_value)
{
    return camera_to_qcarcam_result(ais_s_param(hndl, param, p_value));
}

PUBLIC_API QCarCamRet_e QCarCamSetBuffers(qcarcam_hndl_t hndl, QCarCamBufferList_t* p_buffers)
{
    return camera_to_qcarcam_result(ais_s_buffers(hndl, p_buffers));
}

PUBLIC_API QCarCamRet_e QCarCamStart(qcarcam_hndl_t hndl)
{
    return camera_to_qcarcam_result(ais_start(hndl));
}

PUBLIC_API QCarCamRet_e QCarCamStop(qcarcam_hndl_t hndl)
{
    return camera_to_qcarcam_result(ais_stop(hndl));
}

PUBLIC_API QCarCamRet_e QCarCamPause(qcarcam_hndl_t hndl)
{
    return camera_to_qcarcam_result(ais_pause(hndl));
}

PUBLIC_API QCarCamRet_e QCarCamResume(qcarcam_hndl_t hndl)
{
    return camera_to_qcarcam_result(ais_resume(hndl));
}

PUBLIC_API QCarCamRet_e QCarCamGetFrame(qcarcam_hndl_t hndl, QCarCamFrameInfo_t* p_frame_info,
...
PUBLIC_API QCarCamRet_e QCarCamReleaseFrame(qcarcam_hndl_t hndl, unsigned int idx)
{
