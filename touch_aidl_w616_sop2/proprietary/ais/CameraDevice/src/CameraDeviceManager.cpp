/**
 * @file CameraDeviceManager.cpp
 *
 * @brief Implementation of CameraDeviceManager and CameraDevice interface.
 *
 * Copyright (c) 2009, 2011, 2016-2021 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
=========================================================================== */
CameraDeviceManager* CameraDeviceManager::CreateInstance(void)
{
    if(m_pDeviceManager == nullptr)
    {
        ...
        if (m_pDeviceManager)
        {
            CameraResult rc = m_pDeviceManager->Initialize();
            if (rc)
            {
                DestroyInstance();
            }
...
CameraResult CameraDeviceManager::RegisterDeviceFromLib(CameraDeviceIDType deviceId, void const * pLibInfo)
{
    ...
    if (CAMERA_SUCCESS == result)
    {
        ...
        cameraDeviceInfo.deviceID = deviceId;
    ...
    if (CAMERA_SUCCESS == result)
    {
        ...
        if (NULL != hSymTable)
        {
            ...
            if (NULL != pfnGetCameraConfigInterface)
            {
                ...
                {
                    CameraBoardType const * pBoardInfo;

                    pBoardInfo = pCameraConfigIf->GetCameraBoardInfo();
                    // Retrieve list of dynamic devices
                    if (pBoardInfo)
                    {
                        // Load and register each device with the device manager.
                        for (device = 0; device < MAX_NUM_CAMERA_INPUT_DEVS; ++device)
                        {
                            if (!pBoardInfo->camera[device].devId)
                            {
                                //null terminated table
                                break;
                            }

                            result = RegisterDeviceFromLib(pBoardInfo->camera[device].devId, &pBoardInfo->camera[device].driverInfo);
                            if (result != CAMERA_SUCCESS)
                            {
                                ...
                                CAM_MSG(ERROR, "Register dynamic device %s:%s failed!",
                                        pBoardInfo->camera[device].driverInfo.strDeviceLibraryName,
                                        pBoardInfo->camera[device].driverInfo.strCameraDeviceOpenFn);
        ...
        for (nDevice = 0; nDevice < nFreeRegisteredDevicesIndex; ++nDevice)
        {
                ...
                (deviceCategory == registeredDevices[nDevice].info.deviceCategory))
            {
                if ((pCameraDeviceInfo != NULL) &&
                    (nDeviceInCategory < nCameraDeviceInfoLen))
                {
