...
/* contains basename of program being executed */
...
#if defined(__INTEGRITY) || defined(_LINUX)
...
#define AIS_LOG_CONF_FILE_DIR "/usr/share/camera"
#elif !defined(AIS_EARLYSERVICE)
#define AIS_LOG_CONF_FILE_DIR "/data/misc/camera"
#else
#define AIS_LOG_CONF_FILE_DIR "/usr/bin"
#endif
#elif defined(__ANDROID__)
#define AIS_LOG_CONF_FILE_DIR "/vendor/etc/camera"
#elif defined(__INTEGRITY)
#define AIS_LOG_CONF_FILE_DIR "/bin"
#else
...
static volatile int sg_ais_log_initialized = 0;
...
static int ais_log_lock(void);
static int ais_log_unlock(void);
 ...
 */
static int ais_log_mem_init(s_ais_log_mgr *p)
{
    int rc = 0;
    if (p->mem_size <= 0)
    {
 ...
 * @return 0: success, others: failed
 */
static int ais_log_mem_uninit(s_ais_log_mgr *p)
{
   if (p->p_mem_buf != NULL)
   {
        free(p->p_mem_buf);
 ...
 * @return 0: success, others: failed
 */
static int ais_log_mem_printf(s_ais_log_mgr *p, int level, const char *fmt, va_list va)
{
    int rc;
    struct timespec ts;
    long long curr;
 ...
 * @return 0: success, others: failed
 */
static int ais_log_os_init(s_ais_log_mgr *p)
{
    int rc;
 ...
 * @return 0: success, others: failed
 */
static int ais_log_os_uninit(s_ais_log_mgr *p)
{
    slog2_reset();
 ...
 * @return 0: success, others: failed
 */
static int ais_log_os_printf(s_ais_log_mgr *p, int level, const char *fmt, va_list va)
{
 ...
 * @return 0: success, others: failed
 */
static int ais_log_file_init(s_ais_log_mgr *p)
{
    int rc = 0;
 ...
 * @return 0: success, others: failed
 */
static int ais_log_file_uninit(s_ais_log_mgr *p)
{
    if (p->p_file_fd != NULL)
    {
        fclose(p->p_file_fd);
 ...
 * @return 0: success, others: failed
 */
static int ais_log_fd_printf(s_ais_log_mgr *p, FILE *fp, int level, const char *fmt, va_list va)
{
    int rc;
    struct timespec ts;
    long long curr;
 ...
 * @return 0: success, others: failed
 */
static int ais_log_va(s_ais_log_mgr *p, int id, int level, const char *fmt, va_list va)
{
    int rc = -1;
    int f_size, file_len;
    char file_path_back_up[AIS_LOG_PATH_SIZE] = {0};
    ...
    {
        fseek(p->p_file_fd, 0L, SEEK_END);
        f_size = ftell(p->p_file_fd);
        //If file size exceeds the specified file size then create backup file
        if (f_size >= p->file_size)
        {
            for (int i = 0; *(p->file_path + i) != '\0'; i++)
            {
                if (*(p->file_path+i) == '.')
                {
                    break;
                }
                *(file_path_back_up + i) = *(p->file_path + i);
            }
            file_len = strlen(file_path_back_up);
            snprintf(file_path_back_up+file_len, sizeof(file_path_back_up) - file_len, "_0.txt");
            rename(p->file_path, file_path_back_up);
            p->p_file_fd = freopen(p->file_path, "a", p->p_file_fd);
            if (p->p_file_fd == NULL)
            {
                rc = -1;
                goto EXIT_FLAG;
 ...
 * @return 0: success, others: failed
 */
static int ais_log_parse(s_ais_log_mgr *p, char *p_path, char *p_name)
{
    int rc = 0;
    FILE *fp = NULL;
...
#if defined(__QNXNTO__)
    int status = bmetrics_log_component_launch((const char*)__progname);
    if (status)
        fprintf(stderr, "Failed to init bmetrics module");
#endif
...
PUBLIC_API int ais_log_uninit(void)
{
    ...
    sg_ais_log_initialized--;

    if (sg_ais_log_initialized > 0)
    {
        goto EXIT_FLAG;
    }
 ...
 * @return >0: success, others: failed
 */
PUBLIC_API void ais_log(int id, int level, const char *fmt, ...)
{
    int rc;

    //check level first to avoid locking everywhere.
    //because log level won't change after initialization,
    //even if it changes, it won't affect the program behavior,
...
#if defined(__QNXNTO__)
static void map_bmetrics_api(ais_event_kpi_t event)
{
    int status = 0;
    switch(event)
    {
        case AIS_EVENT_KPI_SERVER_INIT:
        case AIS_EVENT_KPI_BE_SERVER_INIT:
        case AIS_EVENT_KPI_CLIENT_START:
            status = bmetrics_log_component_start();
            break;
        case AIS_EVENT_KPI_SERVER_READY:
        case AIS_EVENT_KPI_BE_SERVER_READY:
            status = bmetrics_log_component_ready();
            break;
        case AIS_EVENT_KPI_CLIENT_FIRST_FRAME:
            status = bmetrics_log_subcomponent_done("Camera First Frame");
            break;
        case AIS_EVENT_KPI_SENSOR_PROG_START:
            status = bmetrics_log_subcomponent_start("Camera Sensor Prog");
            break;
        case AIS_EVENT_KPI_SENSOR_PROG_END:
            status = bmetrics_log_subcomponent_stop("Camera Sensor Prog");
            break;
        case AIS_EVENT_KPI_DETECT_SENSORS_START:
            status = bmetrics_log_subcomponent_start("Camera Sensor start");
            break;
        case AIS_EVENT_KPI_DETECT_SENSORS_END:
            status = bmetrics_log_subcomponent_stop("Camera Sensor stop");
            break;
        case AIS_EVENT_KPI_CLIENT_FIRST_FRAME_POST_START:
            status = bmetrics_log_subcomponent_start("Camera frame post");
            break;
        case AIS_EVENT_KPI_CLIENT_FIRST_FRAME_POST_END:
            status = bmetrics_log_subcomponent_stop("Camera frame post");
            break;
        case AIS_EVENT_KPI_CLIENT_FRAME_FREEZE:
        case AIS_EVENT_KPI_DETECT_FRAME_FREEZE:
            status = bmetrics_log_subcomponent_done("Camera frame freeze");
        default: printf("No matching KPI");
    }
    if (status)
        fprintf(stderr, "Failed to bmetrics module API failed");
}
#elif defined(__INTEGRITY)
static char sg_kpi_msg[AIS_EVENT_KPI_MAX][64] =
{
    ...
    "M - Camera Detect sensors start",
    "M - Camera Detect sensors end",
    "M - Camera First Frame post start",
    "M - Camera First Frame post end",
    "M - Camera Frame Freeze Received",
    "M - Camera Sensor Frame Freeze Detect"
};
#else
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0))
static char sg_kpi_msg[AIS_EVENT_KPI_MAX][64] =
{
    "M - DRIVER Camera Init ",
    "M - DRIVER Camera Ready ",
    "M - DRIVER BE Camera Init ",
    "M - DRIVER BE Camera Ready ",
    "M - Camera Start ",
    "M - Camera First Frame ",
    "M - DRIVER Camera Sensor prog start ",
    "M - DRIVER Camera Sensor prog end ",
    "M - DRIVER Camera Detect sensors start ",
    "M - DRIVER Camera Detect sensors end ",
    "M - Camera First Frame post start ",
    "M - Camera First Frame post end ",
    "M - Camera Frame Freeze Received ",
    "M - DRIVER Camera Sensor Frame Freeze Detect ",
    "M - Early RVC Camera First Frame",
    "M - Early RVC Camera Self Exit",
    "M - Qultivate Camera Not Supported "
};
#else // 6.1 kernel "boot_kpi string" is not printed for Profiling
static char sg_kpi_msg[AIS_EVENT_KPI_MAX][64] =
{
    "boot_kpi: M - DRIVER Camera Init ",
    "boot_kpi: M - DRIVER Camera Ready ",
    "boot_kpi: M - DRIVER BE Camera Init ",
    "boot_kpi: M - DRIVER BE Camera Ready ",
    "boot_kpi: M - Camera Start ",
    "boot_kpi: M - Camera First Frame ",
    "boot_kpi: M - DRIVER Camera Sensor prog start ",
    "boot_kpi: M - DRIVER Camera Sensor prog end ",
    "boot_kpi: M - DRIVER Camera Detect sensors start ",
    "boot_kpi: M - DRIVER Camera Detect sensors end ",
    "boot_kpi: M - Camera First Frame post start ",
    "boot_kpi: M - Camera First Frame post end ",
    "boot_kpi: M - Camera Frame Freeze Received ",
    "boot_kpi: M - DRIVER Camera Sensor Frame Freeze Detect ",
    "boot_kpi: M - Early RVC Camera First Frame",
    "boot_kpi: M - Early RVC Camera Self Exit",
    "boot_kpi: M - Qultivate Camera Not Supported "
};
#endif
#endif
...
PUBLIC_API void ais_log_kpi(ais_event_kpi_t event)
{
    ...
    map_bmetrics_api(event);
#else
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0))
#if defined(__ANDROID__)|| defined(__AGL__)
    sg_fd_kpi = open("/sys/kernel/boot_kpi/kpi_values", O_WRONLY);
#endif
...
#if defined(__ANDROID__)|| defined(__AGL__)
    char* msg = sg_kpi_msg[event];
    sg_fd_kpi =  open("/dev/kmsg", O_WRONLY);
    if (sg_fd_kpi && event < AIS_EVENT_KPI_MAX)
    {
        char* msg = sg_kpi_msg[event];
        (void)write(sg_fd_kpi, msg, strlen(msg)+1);
    }
    ALOGE("boot_kpi:  %s", msg);
    if (sg_fd_kpi)
    {
        close(sg_fd_kpi);
        sg_fd_kpi = 0;
