 ...
 */
#define AIS_LOG_RAW(module, level, fmt, ...) ais_log((module), (level),  "R0x%x|%d %s " fmt "\n", \
    AIS_RELEASE_VERSION, AIS_RELEASE_CHANGEID, AIS_LOG_LVL_STR[AIS_LOG_LVL_CORR(level)], ##__VA_ARGS__)

/**
 * prints out log level string, file name and file line
 */
#define AIS_LOG(module, level, fmt, ...) ais_log((AIS_MOD_ID_ ## module), (AIS_LOG_LVL_ ## level),  "R0x%x|%d %s:%d %s " fmt "\n", \
    AIS_RELEASE_VERSION, AIS_RELEASE_CHANGEID, __FUNCTION__, __LINE__, AIS_LOG_LVL_STR[AIS_LOG_LVL_CORR((AIS_LOG_LVL_ ## level))], ##__VA_ARGS__)

#define AIS_LOG2(module, level, fmt, ...) ais_log((AIS_MOD_ID_ ## module), (level),  "R0x%x|%d %s:%d %s " fmt "\n", \
    AIS_RELEASE_VERSION, AIS_RELEASE_CHANGEID, __FUNCTION__, __LINE__, AIS_LOG_LVL_STR[AIS_LOG_LVL_CORR((level))], ##__VA_ARGS__)
 ...
 */
#define AIS_LOG_ENTER(module, level, fmt, ...) ais_log((AIS_MOD_ID_ ## module), (AIS_LOG_LVL_ ## level),  "R0x%x|%d %s E %s " fmt "\n", \
    AIS_RELEASE_VERSION, AIS_RELEASE_CHANGEID, AIS_LOG_LVL_STR[AIS_LOG_LVL_CORR((AIS_LOG_LVL_ ## level))], __FUNCTION__, ##__VA_ARGS__)
#define AIS_LOG_EXIT(module, level, fmt, ...) ais_log((AIS_MOD_ID_ ## module), (AIS_LOG_LVL_ ## level),  "R0x%x|%d %s X %s " fmt "\n", \
    AIS_RELEASE_VERSION, AIS_RELEASE_CHANGEID, AIS_LOG_LVL_STR[AIS_LOG_LVL_CORR((AIS_LOG_LVL_ ## level))], __FUNCTION__, ##__VA_ARGS__)
 ...
 */
#define AIS_LOG_ASSERT(x) ((void)((x) || ais_log(AIS_MOD_ID_RSRV, AIS_LOG_LVL_ALWAYS, "R0x%x|%d %s:%s:%d ASSERT %s\n\n", \
    AIS_RELEASE_VERSION, AIS_RELEASE_CHANGEID, __FILE__, __FUNCTION__, __LINE__, #x)))
 ...
 * @return >0: success, others: failed
 */
void ais_log(int id, int level, const char *fmt, ...);
...
typedef enum
{
    ...
    AIS_EVENT_KPI_CLIENT_FRAME_FREEZE,
    AIS_EVENT_KPI_DETECT_FRAME_FREEZE,
    AIS_EVENT_KPI_EARLY_RVC,
    AIS_EVENT_KPI_EARLY_RVC_EXIT,
    AIS_EVENT_KPI_QULTIVATE_CAMERA_NOT_ALLOWED,
    AIS_EVENT_KPI_MAX
} ais_event_kpi_t;
