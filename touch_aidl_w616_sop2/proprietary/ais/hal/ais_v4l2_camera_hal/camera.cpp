...
void PowerEventThread (void *args) {
    HAL_LOGD("PowerEventThread PowerEventInit");
    gPowerService = PowerEventInit(Camera::PowerEventCb, args);
}

int Camera::PowerEventCb(pm_event_t eventId, void* pUsrCtxt) {
    HAL_LOGD("Entry PowerEventCb");

    Camera *pCamera = (Camera *)pUsrCtxt;
    ...
    HAL_LOGD("sleep Value : %d", sleepTimeUs);

    switch (eventId)
    {
        case AIS_PM_SUSPEND:
        {
            HAL_LOGE("PowerEventCb AIS_PM_SUSPEND ");
            if (gPMStatus == AIS_PM_SUSPEND) {
                HAL_LOGE("Status is already in the Suspend");
                break;
            }

            HAL_LOGE("PowerEventCb Starting sleep for 3 Sec to allow App to close sessions");
            usleep(sleepTimeUs);
            HAL_LOGE("PowerEventCb Checking for any resource still alive and closing them");

            if(pCamera->mBusy == true)
                pCamera->close();
            else
                HAL_LOGE("PowerEventCb Camera is Already Closed by Someone");
            gPMStatus = AIS_PM_SUSPEND;
            break;
        }
        case AIS_PM_RESUME:
        {
            HAL_LOGE("PowerEventCb AIS_PM_RESUME");
            if (gPMStatus == AIS_PM_RESUME) {
                HAL_LOGE("Status is already in the Resume");
                break;
            }
...
int Camera::loadStaticInfo() {
  ...
  if (res) {
    ...
    return res;
      ...
      HAL_LOGD("----Init staticInfo complete");
...
void Camera::completeRequest(std::shared_ptr<CaptureRequest> request, int err)
{
    ...
    int64_t readoutTimestamp = 0;
    res = v4l2_camera_hal::SingleTagValue(
        request->settings, ANDROID_SENSOR_EXPOSURE_TIME, &readoutTimestamp);
    if (res) {
        HAL_LOGE("Failed reading ANDROID_SENSOR_EXPOSURE_TIME");
    }
      else {
        HAL_LOGD("timestamp :%llu ## readoutTimestamp :%llu",timestamp, readoutTimestamp);
    }

    notifyShutter(request->frame_number, timestamp, readoutTimestamp);

    for (int i = 0; i < request->output_buffers.size(); i++) {
        request->output_buffers[i].status = CAMERA3_BUFFER_STATUS_OK;
    }

    //listing the following calibration metadata entries in CaptureResult
    for (auto&& info: mconfig.getCameras()) {
        if(mId == atoi(info.input_id.c_str())) {
            if(info.lens_pose_translation.size() == 3)
                v4l2_camera_hal::UpdateMetadata(&(request->settings), ANDROID_LENS_POSE_TRANSLATION, info.lens_pose_translation);
            if(info.lens_pose_rotation.size() == 4)
                v4l2_camera_hal::UpdateMetadata(&(request->settings), ANDROID_LENS_POSE_ROTATION, info.lens_pose_rotation);
            if(info.lens_intrinsic_calibration.size() == 5)
                v4l2_camera_hal::UpdateMetadata(&(request->settings), ANDROID_LENS_INTRINSIC_CALIBRATION, info.lens_intrinsic_calibration);
        }
    }
...
void Camera::notifyShutter(uint32_t frame_number, uint64_t timestamp, uint64_t readoutTimestamp)
{
    ...
    message.message.shutter.readoutTimestamp = readoutTimestamp + timestamp;
