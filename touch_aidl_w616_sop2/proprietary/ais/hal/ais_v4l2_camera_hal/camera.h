...
#define CAMERA3_BUFFER_STATUS_DEVICE_ERROR 0x2
...
class Camera {
    public:
        ...
        Camera(const config_port_metadata& config);
        virtual ~Camera();
        ...
        void dump(int fd);
        int flush();
        static const camera3_device_ops_t* getsOps() {
            return &sOps;
        }
#ifdef HAL_CAMERA_CPMS_SUPPORT
    ...
    private:
        ...
        void notifyShutter(uint32_t frame_number, uint64_t timestamp, uint64_t readoutTimestamp);
