...
#define MAX_OUTPUT_BUFFER_NUM 128  // > 2 * MAX_BUFFER_NUM //Changing the maximum buffers no, address the CTS issue
...
typedef struct {
    ...
    float mAisFrameFPS;
    qcarcam_color_fmt_t mAisFrameFormat;
    uint32_t mHalFrameFormat;
    int cam_id;
}qcarcam_buffer_info_t;
...
class V4L2Wrapper {
 public:
  ...
  static V4L2Wrapper* NewV4L2Wrapper(int id, const V4L2CameraHAL::QCarCamInfo& qcarcam_info);
  ...
  class Connection {
   public:
    Connection(std::shared_ptr<V4L2Wrapper> device)
        : device_(std::move(device)), connect_result_(0) {}
    ~Connection() {
...
//        device_->Disconnect();
  ...
  int Init();
  int openCamera();
  int allocInputBuffers();
  static void v4l2_ais_event_cb(const qcarcam_hndl_t hndl, const qcarcam_event_t event_id, const qcarcam_event_payload_t *p_payload);
  ...
  int GetSensorHeight();
 private:
  qcarcam_test_input_t input_ctxt;
  ...
  qcarcam_color_fmt_t mAisFrameFormat;
  uint32_t mHalFrameFormat;
  sp<GraphicBuffer> *mpGfxBufs;
  qcarcam_buffers_t mQcarcamOutBufs;
  qcarcam_mapped_buffer_t *mpQcarcamMmapBufs;
  qcarcam_frame_info_t mQcarcamFrameInfo;
  std::mutex qcarcamInitLock;
...
//  uint32_t mClientCnt;
  ...
  void mClientCntStatus(void);
  // Constructor is private to allow failing on bad input.
  // Use NewV4L2Wrapper instead.
  V4L2Wrapper(int id_, const V4L2CameraHAL::QCarCamInfo qcarcam_info);
  ...
  int32_t DoC2dScale(buffer_info_t * dst_buff_info, bool useTestPattern);
  ...
  int get_handle_fd(buffer_handle_t hndl);
  ...
  uint64_t get_native_hnd_buffer_id(buffer_handle_t hndl);
    bool isTestPatternSet(std::shared_ptr<default_camera_hal::CaptureRequest> request);
  ...
  nsecs_t mAeMinExposureTime;
  int     mAeCurrentSensitivity;
  uint32_t mExposureSeed;
  uint64_t mTimesBetweentAndroidQnx;
