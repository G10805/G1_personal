    ...
    switch (out_frame->GetFourcc()) {
      case V4L2_PIX_FMT_YVU420:  // YV12
      {
        ...
        int res = YU12ToYV12(in_frame.GetData(), out_frame->GetData(),
                             ...
                             uvstride);
        if (res)
            LOGF(ERROR) << "YU12ToYV12() returns " << res;
        return res ? -EINVAL : 0;
      }
      case V4L2_PIX_FMT_YUV420:  // YU12
      {
      ...
      case V4L2_PIX_FMT_NV21:  // NV21
      {
        ...
        int res = YU12ToNV21(in_frame.GetData(), out_frame->GetData(),
                             in_frame.GetWidth(), in_frame.GetHeight());
        if (res)
            LOGF(ERROR) << "YU12ToNV21() returns " << res;
        return res ? -EINVAL : 0;
      }
      case V4L2_PIX_FMT_BGR32: {
        int res = libyuv::I420ToABGR(
            ...
            in_frame.GetWidth(), in_frame.GetHeight());
        if (res)
            LOGF(ERROR) << "I420ToABGR() returns " << res;
        return res ? -EINVAL : 0;
      }
      case V4L2_PIX_FMT_RGB32: {
        int res = libyuv::I420ToARGB(
            ...
            in_frame.GetWidth(), in_frame.GetHeight());
        if (res)
            LOGF(ERROR) << "I420ToARGB() returns " << res;
        return res ? -EINVAL : 0;
      }
      case V4L2_PIX_FMT_JPEG: {
        bool res = ConvertToJpeg(metadata, in_frame, out_frame);
        if (res)
            LOGF(ERROR) << "ConvertToJpeg() returns " << res;
        return res ? -EINVAL : 0;
      }
      default:
        LOGF(ERROR) << "Destination pixel format "
    ...
    switch (out_frame->GetFourcc()) {
      case V4L2_PIX_FMT_YUV420:  // YU12
      {
        int res = libyuv::MJPGToI420(
            ...
            out_frame->GetHeight());
        if (res)
            LOGF(ERROR) << "MJPEGToI420() returns " << res;
        return res ? -EINVAL : 0;
      }
      default:
        LOGF(ERROR) << "Destination pixel format "
...
int ImageProcessor::Scale(const FrameBuffer& in_frame, FrameBuffer* out_frame) {
  ...
  LOGF(DEBUG) << "Scale image from " << in_frame.GetWidth() << "x"
  ...
  if (ret)
      LOGF(ERROR) << "I420Scale failed: " << ret;
  return ret;
}
...
//Use this function to set some EXIF parameters based on metadata.
bool SetExifTags(const CameraMetadata& metadata, ExifUtils* utils) {
  time_t raw_time = 0;
  struct tm time_info;
