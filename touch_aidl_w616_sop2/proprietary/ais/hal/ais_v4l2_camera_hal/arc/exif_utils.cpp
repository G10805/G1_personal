...
#define PROPERTY_PRODUCT_MANUFACTURER "ro.product.manufacturer"
...
//Set Some TAG for CTS.
bool ExifUtils::SetDefaultTag()
{
  std::unique_ptr<ExifEntry> entry;
  time_t raw_time=time(NULL);
  tm*  tm_info = localtime(&raw_time);

  // The following functions mainly set some EXIF parameters required by CTS.
  // The parameters are the default data.
#define TOSTR(DATA) #DATA
#define TOSTR1(DATA) TOSTR(DATA)
char prop_name[PROPERTY_VALUE_MAX];
  std::string maker;
  std::string model;
  if (!((property_get(PROPERTY_PRODUCT_MANUFACTURER, prop_name, "unknown")) > 0)){
    std::string maker(TOSTR1(PRODUCT_MANUFACTURER));
    LOGF(ERROR) << "get Property failed, adding default value for EXIF_TAG_MAKE";
  }
  else{
    maker.assign(prop_name);
    LOGF(ERROR) << "Adding tag EXIF_TAG_MAKE success";
  }

  if (!((property_get(PROPERTY_PRODUCT_MODEL, prop_name, "unknown")) > 0)){
    std::string model(TOSTR1(PRODUCT_MODEL));
    LOGF(ERROR) << "get Property failed, adding default value for EXIF_TAG_MODEL";
  }
  else{
    model.assign(prop_name);
    LOGF(ERROR) << "Adding tag EXIF_TAG_MODEL success";
  }
  std::string subsec("010");

  //TAG Maker. CTS requires that the value of this TAG must be "QTI".
  if (!SetMaker(maker)){
    LOGF(ERROR) << "Adding tag EXIF_TAG_MAKE failed";
	 return false;
  }

  //TAG Model. CTS requires that the value of this TAG must be "msmnile_gvmq for arm64".
  if (!SetModel(model)){
    LOGF(ERROR) << "Adding tag EXIF_TAG_model failed";
	 return false;
  }

  //TAG EXIF_TAG_DATE_TIME_DIGITIZED. The value of this TAG uses the current time.
  if (!SetTime(EXIF_IFD_EXIF,EXIF_TAG_DATE_TIME_DIGITIZED,*tm_info)){
	 LOGF(ERROR) << "Adding tag EXIF_TAG_DATE_TIME_DIGITIZED failed";
	 return false;
  }

  //TAG EXIF_TAG_SUB_SEC_TIME.
  if (!SetAscii(EXIF_IFD_EXIF,EXIF_TAG_SUB_SEC_TIME,subsec)){
	 LOGF(ERROR) << "Adding tag EXIF_TAG_SUB_SEC_TIME failed";
	 return false;
  }

  //TAG EXIF_TAG_SUB_SEC_TIME_ORIGINAL.
  if (!SetAscii(EXIF_IFD_EXIF,EXIF_TAG_SUB_SEC_TIME_ORIGINAL,subsec)){
	 LOGF(ERROR) << "Adding tag EXIF_TAG_SUB_SEC_TIME_ORIGINAL failed";
	 return false;
  }

  //TAG SUB_SEC_TIME.
  if (!SetAscii(EXIF_IFD_EXIF,EXIF_TAG_SUB_SEC_TIME_DIGITIZED,subsec)){
	 LOGF(ERROR) << "Adding tag EXIF_TAG_SUB_SEC_TIME_DIGITIZED failed";
	 return false;
  }

  //TAG EXIF_TAG_FLASH. The value of this TAG is 0, this mean "No flash".
  if (!SetShort(EXIF_IFD_EXIF,EXIF_TAG_FLASH,0)){
	 LOGF(ERROR) << "Adding tag EXIF_TAG_FLASH failed";
	 return false;
  }

  //TAG EXIF_TAG_WHITE_BALANCE. The value of this TAG is 0, this mean "Auto white balance".
  if (!SetShort(EXIF_IFD_EXIF,EXIF_TAG_WHITE_BALANCE,0)){
	 LOGF(ERROR) << "Adding tag EXIF_TAG_WHITE_BALANCE failed";
	 return false;
  }

  //TAG EXIF_TAG_ISO_SPEED_RATINGS. The value of this TAG is 400.
  if (!SetShort(EXIF_IFD_EXIF,EXIF_TAG_ISO_SPEED_RATINGS,400)){
	 LOGF(ERROR) << "Adding tag EXIF_TAG_ISO_SPEED_RATINGS failed";
	 return false;
  }

  //TAG EXIF_TAG_APERTURE_VALUE. The value of this TAG is 2 this is mean "f/2.0"
  if (!SetRational(EXIF_IFD_0,EXIF_TAG_APERTURE_VALUE,10,5)){
	 LOGF(ERROR) << "Adding tag EXIF_TAG_APERTURE_VALUE failed";
	 return false;
  }

  //TAG EXIF_TAG_FNUMBER. The value of this TAG is 2 this is mean "f/2.0"
  if (!SetRational(EXIF_IFD_EXIF,EXIF_TAG_FNUMBER,10,5)){
	 LOGF(ERROR) << "Adding tag EXIF_TAG_FNUMBER failed";
	 return false;
  }

  //TAG EXIF_TAG_APERTURE_VALUE. The value of this TAG is 2 this is mean "0.01s"
  if (!SetRational(EXIF_IFD_EXIF,EXIF_TAG_EXPOSURE_TIME,1,100)){
	 LOGF(ERROR) << "Adding tag EXIF_TAG_EXPOSURE_TIME failed";
	 return false;
  }
  return true;
}

bool ExifUtils::SetRational(ExifIfd ifd, ExifTag tag , uint16_t numerator , uint16_t denominator)
{
  std::unique_ptr<ExifEntry> entry = AddEntry(ifd, tag);
  if (!entry) {
    LOGF(ERROR) << "Adding ExifIfd ="<<ifd<<" and ExifTag = "<<tag<<"  failed";
    return false;
  }
...
bool ExifUtils::SetShort(ExifIfd ifd, ExifTag tag , uint16_t value)
{
  std::unique_ptr<ExifEntry> entry = AddEntry(ifd, tag);
  if (!entry) {
    LOGF(ERROR) << "Adding ExifIfd ="<<ifd<<" and ExifTag = "<<tag<<"  failed";
...
bool ExifUtils::SetAscii(ExifIfd ifd, ExifTag tag ,const std::string& ascii  )
{
  size_t entrySize = ascii.length() + 1;
  std::unique_ptr<ExifEntry> entry = AddVariableLengthEntry(
      ifd, tag, EXIF_FORMAT_ASCII, entrySize, entrySize);
  if (!entry) {
    LOGF(ERROR) << "Adding ASCII  exif entry failed";
    return false;
  }
  memcpy(entry->data, ascii.c_str(), entrySize);
  return true;

}

bool ExifUtils::SetTime(ExifIfd ifd, ExifTag TimeTAG  ,const struct tm& t) {
  ...
  std::unique_ptr<ExifEntry> entry =
      AddVariableLengthEntry(ifd, TimeTAG, EXIF_FORMAT_ASCII,
...
bool ExifUtils::GenerateYuvThumbnail(std::vector<uint8_t>* scaled_buffer) {
  ...
  uint8_t* scaled_y_plane = (uint8_t *)scaled_buffer->data();
