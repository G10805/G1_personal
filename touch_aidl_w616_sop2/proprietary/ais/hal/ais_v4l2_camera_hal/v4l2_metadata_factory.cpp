...
#define MODULE_NAME "V4L2MetaData"
#include "common.h"
...
int GetV4L2Metadata(const config_port_metadata& config, std::shared_ptr<V4L2Wrapper> device,
  ...
  // Prepare our automotive metadata information
    if (config.cameraConfigFound) {
        HAL_LOGD("config information for hardware camera present");
        uint32_t value = 0;
        for (auto&& info: config.getCameras()) {
            if (config.port_idx == atoi(info.input_id.c_str())) {
                // We found a match!
                const char* cam_location = info.automotive_location.c_str();
                const char* cam_facing   = info.automotive_lens_facing.c_str();

                int ret = camera_metadata_enum_value(ANDROID_AUTOMOTIVE_LOCATION,
                        cam_location,CAMERA_METADATA_ENUM_STRING_MAX_SIZE,&value);
                if(ret == android::OK) {
                    HAL_LOGD("Inserting location value");
                    components.insert(
                    std::unique_ptr<PartialMetadataInterface>(new Property<uint8_t>(
                    ANDROID_AUTOMOTIVE_LOCATION, value)));
                }

                ret = camera_metadata_enum_value(ANDROID_AUTOMOTIVE_LENS_FACING,cam_facing,
                    CAMERA_METADATA_ENUM_STRING_MAX_SIZE,&value);
                if(ret == android::OK) {
                    HAL_LOGD("Inserting facing value");
                    components.insert(
                    std::unique_ptr<PartialMetadataInterface>(new Property<uint8_t>(
                    ANDROID_AUTOMOTIVE_LENS_FACING, value)));
                }
            }
        }
    }
    ...
    {
      //Insert Default values for static metadata fields android.automotive.lens.facing and android.automotive.location.
      //Assuming all cameras as EXTERIOR_REAR
      HAL_LOGD("Inserting default location value");
      uint32_t value = ANDROID_AUTOMOTIVE_LOCATION_EXTERIOR_REAR;
      components.insert(
          std::unique_ptr<PartialMetadataInterface>(new Property<uint8_t>(
              ANDROID_AUTOMOTIVE_LOCATION, value)));

      HAL_LOGD("Inserting default facing value");
  ...
  HAL_LOGD("Inserting AVAILABLE_STREAM_USE_CASES");
  components.insert(std::unique_ptr<PartialMetadataInterface>(
      new Property<std::array<int64_t, 7>>(
  ...
  components.insert(
      NoEffectMenuControl<uint8_t>(ANDROID_CONTROL_AF_MODE,
                                   ...
                                   {{CAMERA3_TEMPLATE_PREVIEW, ANDROID_CONTROL_AF_MODE_CONTINUOUS_PICTURE},
                                   {CAMERA3_TEMPLATE_STILL_CAPTURE, ANDROID_CONTROL_AF_MODE_CONTINUOUS_PICTURE},
                                   {CAMERA3_TEMPLATE_ZERO_SHUTTER_LAG, ANDROID_CONTROL_AF_MODE_CONTINUOUS_PICTURE},
                                   {CAMERA3_TEMPLATE_VIDEO_RECORD, ANDROID_CONTROL_AF_MODE_CONTINUOUS_VIDEO},
                                   {CAMERA3_TEMPLATE_VIDEO_SNAPSHOT, ANDROID_CONTROL_AF_MODE_CONTINUOUS_VIDEO},
...
if (config.cameraConfigFound) {
    HAL_LOGD("config information for Android LENS facing present");
    uint32_t value = 0;
    for (auto&& info: config.getCameras()) {
        if (config.port_idx == atoi(info.input_id.c_str())) {
            // We found a match!
            const char* cam_lens_facing   = info.lens_facing.c_str();
            const char *cam_lens_pose_reference = info.lens_pose_reference.c_str();

            int ret = camera_metadata_enum_value(ANDROID_LENS_FACING,cam_lens_facing,
                    CAMERA_METADATA_ENUM_STRING_MAX_SIZE,&value);
            if(ret == android::OK) {
                HAL_LOGD("Inserting lens facing value");
                components.insert(
                    std::unique_ptr<PartialMetadataInterface>(new Property<uint8_t>(
                        ANDROID_LENS_FACING, value)));
            }

            ret = camera_metadata_enum_value(ANDROID_LENS_POSE_REFERENCE,cam_lens_pose_reference,
                CAMERA_METADATA_ENUM_STRING_MAX_SIZE,&value);
            if(ret == android::OK) {
                HAL_LOGD("Inserting lens pose reference value");
                components.insert(
                    std::unique_ptr<PartialMetadataInterface>(new Property<uint8_t>(
                        ANDROID_LENS_POSE_REFERENCE, value)));
            }

            if(info.lens_pose_translation.size() == 3) {
                HAL_LOGD("Inserting lens pose translation values");
                components.insert(
                    std::unique_ptr<PartialMetadataInterface>(new Property<std::vector<float>>(
                        ANDROID_LENS_POSE_TRANSLATION, info.lens_pose_translation)));
            }

            if(info.lens_intrinsic_calibration.size() == 5) {
                HAL_LOGD("Inserting lens intrinsic calibration values");
                components.insert(
                    std::unique_ptr<PartialMetadataInterface>(new Property<std::vector<float>>(
                        ANDROID_LENS_INTRINSIC_CALIBRATION, info.lens_intrinsic_calibration)));
            }

            if(info.lens_pose_rotation.size() == 4) {
                HAL_LOGD("Inserting lens pose rotation values");
                components.insert(
                    std::unique_ptr<PartialMetadataInterface>(new Property<std::vector<float>>(
                        ANDROID_LENS_POSE_ROTATION, info.lens_pose_rotation)));
            }
        }
    }
}
else {
    if(config.device_idx == QCARCAM_INPUT_TYPE_EXT_FRONT)
    {
        components.insert(
        std::unique_ptr<PartialMetadataInterface>(new Property<uint8_t>(
            ANDROID_LENS_FACING, ANDROID_LENS_FACING_FRONT)));
    }
    else if(config.device_idx == QCARCAM_INPUT_TYPE_EXT_REAR)
    {
        components.insert(
        std::unique_ptr<PartialMetadataInterface>(new Property<uint8_t>(
  ...
  // focus distance: 0.0f, for infinity
  //                 20.0f: 1.0/0.05: 5 cm min focus distance
  components.insert(
      NoEffectMenuControl<float>(ANDROID_LENS_FOCUS_DISTANCE,
                                 ANDROID_LENS_INFO_MINIMUM_FOCUS_DISTANCE,
                                 {20.0f,  0.0f},
                                 {{CAMERA3_TEMPLATE_STILL_CAPTURE, 0.0f},
                                  {OTHER_TEMPLATES, 0.0f}}));
...
*/
  ...
  //TODO(add jpeg orientation).
  components.insert(NoEffectSliderControl<int32_t>(
  ...
  //TODO(add jpeg quality).
  components.insert(NoEffectSliderControl<uint8_t>(
  ...
  //TODO(add jpeg thumbnail quality).
  components.insert(NoEffectSliderControl<uint8_t>(
  ...
  /* size must smaller than 3840*2160   */
  /* To support 3840x2160 resolution needed by CTS */
  std::array<int32_t, 2> pixel_array_size = {{3840, 2160}};
  //std::array<int32_t, 2> pixel_array_size = {device->GetSensorWidth(),
    //                                        device->GetSensorHeight()};
  components.insert(std::unique_ptr<PartialMetadataInterface>(
  ...
  components.insert(
      NoEffectMenuControl<int32_t>(ANDROID_SENSOR_TEST_PATTERN_MODE,
                                   ...
                                   {ANDROID_SENSOR_TEST_PATTERN_MODE_OFF, ANDROID_SENSOR_TEST_PATTERN_MODE_SOLID_COLOR}));
...
if (config.cameraConfigFound) {
    HAL_LOGD("config information for ANDROID_REQUEST_AVAILABLE_CAPABILITIES present");
    uint32_t value = 0;
    int ret = -1;
    for (auto&& info: config.getCameras()) {
        if (config.port_idx == atoi(info.input_id.c_str())) {
            // We found a match!
            std::vector<uint8_t> static_tags;
            for (unsigned int index = 0; index <  info.req_avl_cap.size(); ++index)
            {
                const char* cam_req_avl_cap   = info.req_avl_cap[index].c_str();

                ret = camera_metadata_enum_value(ANDROID_REQUEST_AVAILABLE_CAPABILITIES,
                        cam_req_avl_cap,CAMERA_METADATA_ENUM_STRING_MAX_SIZE,&value);
                if(ret == android::OK)
                    static_tags.push_back(value);
            }
            if(!static_tags.empty()) {
                components.insert(
                    std::unique_ptr<PartialMetadataInterface>(new Property<std::vector<uint8_t>>(
                        ANDROID_REQUEST_AVAILABLE_CAPABILITIES, static_tags)));
                static_tags.clear();
            }
        }
    }
...
#if defined(AIDL_ANDROID_API) // Android-T or better
          {ANDROID_REQUEST_AVAILABLE_CAPABILITIES_BACKWARD_COMPATIBLE,ANDROID_REQUEST_AVAILABLE_CAPABILITIES_STREAM_USE_CASE})));
#else
