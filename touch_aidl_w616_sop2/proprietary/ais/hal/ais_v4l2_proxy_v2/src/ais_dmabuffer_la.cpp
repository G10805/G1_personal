...
static void fill_qcarcam_buffer_params(qcarcam_buffer_t* pBuffer, qcarcam_color_fmt_t fmt)
{
    switch (fmt)
    {
...
int init_gfx_buffers(frame_buffer_handle_t *phndl, uint32 w, uint32 h, uint32 format, bool is_align)
{
    ...
    *phndl = framebufs_ctxt;
    ...
    qcarcam_buffer_t* buffer = (qcarcam_buffer_t*)malloc(sizeof(qcarcam_buffer_t));
    if (!buffer)
    {
        QCARCAM_ERRORMSG("buffer is null");
        return -1;
    }
    memset(buffer, 0, sizeof(qcarcam_buffer_t));
    buffer->planes[0].width = w;
    buffer->planes[0].height = h;
    fill_qcarcam_buffer_params(buffer, (qcarcam_color_fmt_t)format);
...
///
/// @param hndl             Pointer to structure containing buffer parameters
/// @param qcarcam_buffers  Pointer to qcarcam_buffers_t
///
...
int alloc_gfx_buffers(frame_buffer_handle_t hndl, qcarcam_buffers_t *qcarcam_buffers)
{
    ...
    if (qcarcam_buffers)
    {
        ...
        const int nativeBufferFormat = qcarcam_to_native_color_format((qcarcam_color_fmt_t)qcarcam_buffers->color_fmt);
        ...
        {
            fill_qcarcam_buffer_params(&qcarcam_buffers->buffers[i], qcarcam_buffers->color_fmt);
            framebufs_ctxt->buffers[i].size = qcarcam_buffers->buffers[i].planes[0].size;
            ...
            if (!framebufs_ctxt->gfx_bufs[i])
            {
                QCARCAM_ERRORMSG("Failed to allocate buffer %dx%d (fmt=0x%x) %d",
                    framebufs_ctxt->uiw, framebufs_ctxt->uih, qcarcam_buffers->color_fmt, allocSize);
            ...
            qcarcam_buffers->buffers[i].planes[0].p_buf = (void*)(uintptr_t)fd;
