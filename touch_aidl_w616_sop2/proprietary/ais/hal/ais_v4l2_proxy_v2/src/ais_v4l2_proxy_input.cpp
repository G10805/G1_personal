...
qcarcam_ret_t QcarcamInitialize()
{
...
int CAisV4l2ProxyInput::init(ais_proxy_xml_input_t *p_xml_input, qcarcam_input_t* p_qcarcam_input)
{
    int rc = 0;
    ...
    else
    {
        m_v4l2_params.pixformat = (p_qcarcam_input == NULL) ?
            QCARCAM_FMT_UYVY_8: p_qcarcam_input->color_fmt[0];
    }
    ...
    else
    {
        m_v4l2_params.v4l2_format = get_v4l2_format_from_qcarcam(
            (qcarcam_color_fmt_t)m_v4l2_params.pixformat);
    }

    if (p_xml_input->width == -1)
    {
        m_v4l2_params.width = (p_qcarcam_input == NULL) ?
            1920 : p_qcarcam_input->res[0].width;
    }
    else
    {
    ...
    if (p_xml_input->height == -1)
    {
        m_v4l2_params.height = (p_qcarcam_input == NULL) ?
            1024: p_qcarcam_input->res[0].height;
    }
    else
    {
    ...
    for (int i = 0; i < m_num_isp_instances; i++)
    {
        ...
        m_isp_config[i].camera_id = p_xml_input->isp_params.isp_config[i].camera_id;
        m_isp_config[i].use_case = p_xml_input->isp_params.isp_config[i].use_case;
    }
...
int CAisV4l2ProxyInput::processOpenEvent()
{
    ...
    qcarcam_hndl_t qcarcam_context = qcarcam_open(m_qcarcam_input_id);
