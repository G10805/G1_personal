...
int qbuf(int fd, const qcarcam_frame_info_v2_t& frame_info, uint32 batch_frames)
{
    ...
    device_buffer.index = frame_info.idx;
    device_buffer.sequence = frame_info.seq_no[0];
    device_buffer.timestamp.tv_sec = frame_info.timestamp_system/1000000000;
    device_buffer.timestamp.tv_usec = (unsigned long long)(frame_info.timestamp_system * 0.001)%1000000;

    ext.batch_num = batch_frames;
    for (uint32 i = 0; i < ext.batch_num ; ++i)
    {
        ext.timestamp[i] = frame_info.sof_qtimestamp[i];
    ...
    VPROXY_DBG1("QBUF idx %u seq %u timestamp %u %u, batch_frame %u", device_buffer.index,frame_info.seq_no[0],
