...
buffer_handle_t HandleImporter::importBufferInternal(const sp<M> mapper, buffer_handle_t& handle) {
    E error;
...
buffer_handle_t HandleImporter::importBuffer(buffer_handle_t& handle) {
    if (nullptr != handle && !handle->numFds && !handle->numInts) {
        ALOGE("%s: handle Fd and Int are not all null!", __FUNCTION__);
        return nullptr;
    }
