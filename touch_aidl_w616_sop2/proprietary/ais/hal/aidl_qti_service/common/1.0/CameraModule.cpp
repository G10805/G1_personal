...
#define LOG_TAG "AidlComm1.0-CamModule"
...
int CameraModule::getCameraInfo(int cameraId, struct camera_info *info) {
    ...
    if (apiVersion < CAMERA_MODULE_API_VERSION_2_0) {
        int ret;
        ALOGV("camera_module->get_camera_info");
...
int CameraModule::getPhysicalCameraInfo(int physicalCameraId, camera_metadata_t **physicalInfo) {
    ...
    if (index == NAME_NOT_FOUND) {
        ...
        ALOGV("camera_module->get_physical_camera_info");
...
int CameraModule::openLegacy(
    ...
    int res;
    ALOGV("camera_module->open_legacy");
...
int CameraModule::getNumberOfCameras() {
    int numCameras;
    ALOGV("camera_module->get_number_of_cameras");
...
int CameraModule::setCallbacks(const camera_module_callbacks_t *callbacks) {
    int res = OK;
    ALOGV("camera_module->set_callbacks");
...
void CameraModule::getVendorTagOps(vendor_tag_ops_t* ops) {
    if (mModule->get_vendor_tag_ops) {
        ALOGV("camera_module->get_vendor_tag_ops");
...
bool CameraModule::isSetTorchModeSupported(int cameraId) const {
    // AIDL use tag value to determine if support setTorchMode, not use APIVersion
    camera_info rawInfo;
    mModule->get_camera_info(cameraId, &rawInfo);
    CameraMetadata m;
    m.append(rawInfo.static_camera_characteristics);

    camera_metadata_entry entry = m.find(ANDROID_FLASH_INFO_AVAILABLE);
    if (entry.count > 0 ) {
        if (ANDROID_FLASH_INFO_AVAILABLE_TRUE == entry.data.u8[0]){
            return true;
        }
        else
        {
            ALOGE("%s: OPERATION_NOT_SUPPORTED, cameraId:%d!", __FUNCTION__, cameraId);
            return false;
        }
    }
    else
    {
        ALOGE("%s: Failed to find entry for ANDROID_FLASH_INFO_AVAILABLE, cameraId:%d!", __FUNCTION__, cameraId);
        return false;
    }
...
int CameraModule::setTorchMode(const char* camera_id, bool enable) {
    ...
    if (mModule->set_torch_mode != NULL) {
        ALOGV("camera_module->set_torch_mode");
...
int CameraModule::turnOnTorchWithStrengthLevel(const char* camera_id, int32_t torchStrength) {
    int res = INVALID_OPERATION;
    if (mModule->turn_on_torch_with_strength_level != NULL) {
        ALOGV("camera_module->turn_on_torch_with_strength_level");
        res = mModule->turn_on_torch_with_strength_level(camera_id, torchStrength);
        ATRACE_END();
    }
    return res;
}

int CameraModule::getTorchStrengthLevel(const char* camera_id, int32_t* strengthLevel) {
    int res = INVALID_OPERATION;
    if (mModule->get_torch_strength_level != NULL) {
        ALOGV("camera_module->get_torch_strength_level");
        res = mModule->get_torch_strength_level(camera_id, strengthLevel);
        ATRACE_END();
    }
    return res;
}
...
int CameraModule::isStreamCombinationSupported(int cameraId, camera_stream_combination_t *streams) {
    ...
    if (mModule->is_stream_combination_supported != NULL) {
        ALOGV("camera_module->is_stream_combination_supported");
...
void CameraModule::notifyDeviceStateChange(uint64_t deviceState) {
   if (getModuleApiVersion() >= CAMERA_MODULE_API_VERSION_2_5 &&
       ...
       ALOGV("camera_module->notify_device_state_change");
       ALOGV("%s: calling notify_device_state_change with state %" PRId64, __FUNCTION__,
               deviceState);
...
int CameraModule::getConcurrentStreamingCameraIds(
        uint32_t*                          pConcCamArrayLength,
        concurrent_camera_combination_t**  ppConcCamArray) {

    int res = INVALID_OPERATION;
    ALOGV("%s CAM module Calling getConcurrentStreamingCameraIds", __FUNCTION__);
#if defined(AIDL_ANDROID_API) // Android-T or better
    if (getModuleApiVersion() >= CAMERA_MODULE_API_VERSION_2_5 &&
        (mModule->get_concurrent_streaming_camera_ids != NULL))
    {
        ALOGV("camera_module->get_concurrent_streaming_camera_ids");
...
int CameraModule::isConcurrentStreamCombinationSupported(
        const std::vector<cameraid_stream_combination_t>& rCameraIdStreamComboVec)
{
    int res = INVALID_OPERATION;
    ...
    {
        res = mModule->is_concurrent_stream_combination_supported(rCameraIdStreamComboVec.size(),
                                                                  rCameraIdStreamComboVec.data());
    }
#endif
...
uint16_t CameraModule::getModuleApiVersion() const {
    ALOGV("Module api version %d",mModule->common.module_api_version);
...
uint16_t CameraModule::getHalApiVersion() const {
    ALOGV("HAL api version %d",mModule->common.hal_api_version);
