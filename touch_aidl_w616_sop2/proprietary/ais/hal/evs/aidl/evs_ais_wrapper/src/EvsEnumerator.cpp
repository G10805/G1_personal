        ...
        case AIS_PM_SUSPEND:
        {
            ...
            while(itr != sCameraList.end()) {
                ...
                if (pActiveCamera != nullptr) {
                     LOG(ERROR) << "found the record go for the Stoping of the Camera";
                     ...
                     LOG(ERROR) << "It seems the App has been Crashed or killed improperly";
...
EvsEnumerator::EvsEnumerator(sp<IAutomotiveDisplayProxyService> proxyService) {
    ...
    int ready_status = 1;
    std::string ready_property_name = "vendor.evs.qcc4.hal.ready";
    int rc = 0;

    rc = property_set(ready_property_name.c_str(),std::to_string(ready_status).c_str());
    if (rc != 0)
    {
        LOG(ERROR) << "Not able to set vendor.evs.qcc4.hal.ready, evsmanagerd will not be started";
    }
    else
    {
        LOG(INFO) << "set vendor.evs.ais.qcc4.ready to 1, evsmanagerd will be launched";
    }
