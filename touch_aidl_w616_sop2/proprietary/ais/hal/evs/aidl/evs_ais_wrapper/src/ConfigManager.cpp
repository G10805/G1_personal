...
std::unique_ptr<ConfigManager> ConfigManager::Create(const char *path) {
    std::unique_ptr<ConfigManager> cfgMgr(new ConfigManager(path));
    ...
    if (!cfgMgr->readConfigDataFromXML()) {
        LOG(ERROR) << "xml config not found";
    } else 
        cfgMgr->mExternalMetadata = true;
    return cfgMgr;
}
...
const char * getQcarCamPos(qcarcam_input_desc_t desc) {
    switch(desc)
    {
        case QCARCAM_INPUT_TYPE_EXT_REAR:
            return "rear";
        case QCARCAM_INPUT_TYPE_EXT_FRONT:
            return "front";
        case QCARCAM_INPUT_TYPE_EXT_LEFT:
            return "left";
        case QCARCAM_INPUT_TYPE_EXT_RIGHT:
            return "right";
        case QCARCAM_INPUT_TYPE_DRIVER:
            return "driver";
        case QCARCAM_INPUT_TYPE_LANE_WATCH:
            return "lane";
        case QCARCAM_INPUT_TYPE_DIGITAL_MEDIA:
            return "digital";
        case QCARCAM_INPUT_TYPE_ANALOG_MEDIA:
            return "analog";
        case QCARCAM_INPUT_TYPE_GESTURE:
            return "gesture";
        case QCARCAM_INPUT_TYPE_FINGERPRINT:
            return "fingerprint";
        case QCARCAM_INPUT_TYPE_IRIS:
            return "iris";
        case QCARCAM_INPUT_TYPE_TUNER:
            return "tuner";
 ...
 */
std::unique_ptr<ConfigManager::CameraInfo>& ConfigManager::setCameraInfo(const std::string cameraId,
        qcarcam_input_t *qcarcamCamInfo) {
    std::unique_lock<std::mutex> lock(mConfigLock);

    const char *id = cameraId.c_str();
    const char *pos = getQcarCamPos(qcarcamCamInfo->desc);
    LOG(INFO) << "Storing camera info "<<qcarcamCamInfo->desc;

    //TODO::See how to add group & characteristics

    /* create a camera device to be filled */
    CameraInfo *aCamera = new CameraInfo();

    int32_t streamId = std::stoi(id);
    //TODO::Consider all available resolutions
    int32_t height = qcarcamCamInfo->res[0].height;
    int32_t width = qcarcamCamInfo->res[0].width;
    int32_t framerate = qcarcamCamInfo->res[0].fps;
    //TODO::Currently EVS only handles RGB format
    ...
    aCamera->streamConfigurations.insert_or_assign(streamId, cfg);

    int32_t dataSize = calculate_camera_metadata_entry_data_size(
    ...
    /* construct camera_metadata_t */
    if (!constructCameraMetadata(aCamera, 1, dataSize)) {
        LOG(WARNING) << "Either failed to allocate memory or "
            << "allocated memory was not large enough";
    }
