...
void sigHandler(int sig) {
    ALOGE("evs_app is being terminated on receiving a signal %x", sig);
    if (pEvs != nullptr) {
        // Attempt to clean up the resources
        pStateController->postCommand({EvsStateControl::Op::EXIT, 0, 0});
...
int main(int argc, char** argv)
{
    ...
    //Temparary disable vehicleHal
    useVehicleHal = false;
    // Load qcarcam configuration information
    ConfigManager config;
    config.initialize("/system/etc/automotive/evs/config_qcarcam.json");
    ...
    if (useVehicleHal) {
        // Run forever, reacting to events as necessary
        ALOGD("Entering running state");
        pEvsListener->run(pStateController);
    } else {
        ALOGD("Waiting till signal handler exit");
        std::unique_lock<std::mutex> lk(gAppExitLock);
        gCV.wait(lk);
    }
