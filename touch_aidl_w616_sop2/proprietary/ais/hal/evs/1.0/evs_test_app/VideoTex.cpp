...
#ifdef ENABLE_FRAME_DUMP
    // Dump buffer content
    struct private_handle_t * private_hndl = (struct private_handle_t *)(pGfxBuffer->getNativeBuffer()->handle);
    if (private_hndl) {
        int fd = private_hndl->fd;
        int size = mImageBuffer.height * mImageBuffer.width * 4; //RGBA8888
        ...
        static int f_cnt = 0;
        if (f_cnt < 5) {
            snprintf(filename, sizeof(filename), "/data/vendor/camera/evs_app_frame_%d",f_cnt);
            if (!maped_buf) {
                ALOGE("buffer is not mapped");
            }
            fp = fopen(filename, "w+");
            ALOGE("dumping video frame %s numbytes %d", filename, size);
            if (0 != fp) {
