...
#ifndef ANDROID_HARDWARE_AUTOMOTIVE_EVS_V1_1_EVSAISCAMERA_H
#define ANDROID_HARDWARE_AUTOMOTIVE_EVS_V1_1_EVSAISCAMERA_H
...
typedef struct QcarcamCamera {
    qcarcam_hndl_t gQcarcamContext;
...
public:
    ...
    static sp<EvsAISCamera> Create(const char *deviceName,
...
private:
    ...
    void forwardFrame(qcarcam_frame_info_t *frame_info);
    bool convertToqcarcamParamID(CameraParam id,
        qcarcam_param_t& qcarcamParamId,
        hidl_vec<int32_t>& values,
        qcarcam_param_value_t& param);
    void convertFormqcarcamParamID(qcarcam_param_t& qcarcamParamId,
        hidl_vec<int32_t>& values,
        qcarcam_param_value_t& param);
    ...
    EvsEnumerator *mEvs = nullptr;    // pointer to hold the Enumerator service
    ...
    //qcarcam datatypes
    qcarcam_hndl_t mQcarcamContext;
    ...
    static qcarcam_color_fmt_t getHalToQcarcamFormat(uint32_t halFormat);
    static uint32_t getQcarcamStride(uint32_t width, uint32_t mFormat);
    static uint32_t getQcarcamToHalFormat(qcarcam_color_fmt_t qcarcamFormat);
