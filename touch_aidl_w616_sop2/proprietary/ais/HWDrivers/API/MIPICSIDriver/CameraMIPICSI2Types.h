...
typedef enum
{
    ...
    CSIPHY_CMD_ID_START,
    CSIPHY_CMD_ID_STOP,
    CSIPHY_CMD_ID_DIAG_INFO,
    CSIPHY_CMD_ID_POWERON,
    CSIPHY_CMD_ID_POWEROFF,
    CSIPHY_CMD_ID_MAX,
...
{
    uint32 idx;
    MIPICsiPhyMessage_t type;
    uint32 statusMsg[CSIPHY_DIAG_STATUS_MAX];
}CsiEventMsgType;

typedef struct
{
    uint32 status[CSIPHY_DIAG_STATUS_MAX];
}CsiDiagInfo;
