...
CameraResult CSIPHYDevice::Start()
{
    ...
    if (CAMERA_SUCCESS == result)
    {
        ...
        csi_info.lane_mask = (uint16_t)m_sMipiCsiCfg.nLaneMask;
        csi_info.lane_assign = (uint16_t)m_sMipiCsiCfg.nLaneAssign;
...
CameraResult CSIPHYDevice::CSIphyDiag(CsiDiagInfo* pCSIphyDiagInfo)
{
    CameraResult result = CAMERA_SUCCESS;
    int rc = 0;
    struct cam_control cam_cmd;
    ...
    memset(pCSIphyDiagInfo, 0x0, sizeof(pCSIphyDiagInfo));

    cam_cmd.op_code     = CAM_DIAG_INFO;
    cam_cmd.size        = sizeof(CsiDiagInfo);
    cam_cmd.handle_type = CAM_HANDLE_USER_POINTER;
    cam_cmd.reserved    = 0;
    cam_cmd.handle      = (uint64_t)pCSIphyDiagInfo;

    rc = ioctl(m_fd, VIDIOC_CAM_CONTROL, &cam_cmd);
    if (rc < 0)
    {
        CAM_MSG(ERROR, "CSIPHY%d query diagnostic info failed %d", m_csiphyId, rc);
        result = CAMERA_EFAILED;
    }
    return result;
}
        ...
        void* pOut, int nOutLen, int* pnOutLenReq)
{
    ...
    switch (uidControl)
    {
        ...
        case CSIPHY_CMD_ID_DIAG_INFO:
        {
            if (NULL != pnOutLenReq)
            {
                *pnOutLenReq = sizeof(CsiDiagInfo);
            }
            if (pOut)
            {
                CsiDiagInfo* pCSIphyDiagInfo = (CsiDiagInfo*)pOut;

                if (nOutLen != sizeof(CsiDiagInfo))
                {
                    return CAMERA_EBADPARM;
                }
                result = CSIphyDiag(pCSIphyDiagInfo);
            }
            break;
        }
        default:
