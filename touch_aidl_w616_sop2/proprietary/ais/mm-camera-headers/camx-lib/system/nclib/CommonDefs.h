...
#if defined(ANDROID) || defined(__KLOCWORK__) || defined(__QNXNTO__) || defined(CAMERA_UNITTEST)
...
#define NCLIB_VERIFY_POINTER(c, doOnFail) { if (NULL == (c)) { LOGE("NULL pointer: %s", #c); doOnFail; } }
...
///////////////////////////////////////////////////
// Set params
// Use the macros to enable debugging by replacing them with, e.g. "paramName = 0xAD"
...
inline int32_t NcLibValidatePackedField(unsigned param, uint32_t bitwidth, int32_t minVal, int32_t maxVal, const char* fieldName)
{
    int32_t sparam = ((int32_t)param) << (32 - bitwidth) >> (32 - bitwidth);
    if ((sparam > maxVal) || (sparam < minVal))
    {
        LOGE("VALIDATE-SIGNED-PARAM failed param: %s, minVal:%d, maxVal:%d actualVal:%d", fieldName, minVal, maxVal, sparam);
        return 1;
    }
    return 0;
}
...
{\
    ...
    {\
        {\
            int32_t isValid = 0; \
            for (iter = 0; iter < NCLIB_ARRAYSIZE(list); iter++)\
            {\
