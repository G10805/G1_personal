...
#if defined(__QNXNTO__)
// Hardcoding the evendId to 16
#define CAMERA_TRACE_EVENT_ID _NTO_TRACE_USERFIRST+16

#define CAMX_ATRACE_BEGIN(group, traceNameRandomString) trace_logf(CAMERA_TRACE_EVENT_ID, traceNameRandomString)
#define CAMX_ATRACE_END(group) trace_logf(CAMERA_TRACE_EVENT_ID, "Event End");
#define CAMX_ATRACE_ASYNC_BEGIN(group, traceNameRandomString, id) trace_logf(CAMERA_TRACE_EVENT_ID, traceNameRandomString)
#define CAMX_ATRACE_ASYNC_END(group, traceNameRandomString, id) trace_logf(CAMERA_TRACE_EVENT_ID, "Event End");
#define CAMX_ATRACE_INT(...)

#else
#define CAMX_ATRACE_BEGIN(group, traceNameRandomString) ATRACE_BEGIN(traceNameRandomString)
#define CAMX_ATRACE_END(group) ATRACE_END()
#define CAMX_ATRACE_ASYNC_BEGIN(group, traceNameRandomString, id) ATRACE_ASYNC_BEGIN(traceNameRandomString, id)
#define CAMX_ATRACE_ASYNC_END(group, traceNameRandomString, id) ATRACE_ASYNC_END(traceNameRandomString, id)
#define CAMX_ATRACE_INT(traceNameRandomString, value) ATRACE_INT(traceNameRandomString, value)
...
#define CAMX_TRACE_SYNC_BEGIN_F(group, ...)                                                                                   \
    ...
    {                                                                                                                         \
        ...
        CAMX_ATRACE_BEGIN(group, traceNameRandomString);                                                                                  \
    }
...
#define CAMX_TRACE_ASYNC_BEGIN_F(group, id, ...)                                                                              \
    ...
    {                                                                                                                         \
        CHAR traceNameRandomString[CamX::MaxTraceStringLength];                                                               \
        CamX::OsUtils::SNPrintF(traceNameRandomString, sizeof(traceNameRandomString), ##__VA_ARGS__);                         \
        CAMX_ATRACE_ASYNC_BEGIN(group, traceNameRandomString, static_cast<UINT32>(id));                                                   \
    }
...
#define CAMX_TRACE_ASYNC_END_F(group, id, ...)                                                                                \
    ...
    {                                                                                                                         \
        CHAR traceNameRandomString[CamX::MaxTraceStringLength];                                                               \
        CamX::OsUtils::SNPrintF(traceNameRandomString, sizeof(traceNameRandomString), ##__VA_ARGS__);                         \
        CAMX_ATRACE_ASYNC_END(group, traceNameRandomString, static_cast<UINT32>(id));                                                     \
    }
...
#define CAMX_TRACE_INT32_F(group, value, ...)                                                                                 \
    ...
    {                                                                                                                         \
        CHAR traceNameRandomString[CamX::MaxTraceStringLength];                                                               \
        CamX::OsUtils::SNPrintF(traceNameRandomString, sizeof(traceNameRandomString), ##__VA_ARGS__);                         \
        CAMX_ATRACE_INT(traceNameRandomString, value);                                                                             \
    }
...
#define CAMX_TRACE_MESSAGE_F(group, ...)                                                                                      \
    ...
    {                                                                                                                         \
        ...
        CamX::OsUtils::SNPrintF(traceNameRandomString, sizeof(traceNameRandomString), ##__VA_ARGS__);                         \
        CAMX_ATRACE_BEGIN(group, traceNameRandomString);                                                                      \
        CAMX_ATRACE_END(group);                                                                                               \
    }

#define CAMX_TRACE_SYNC_BEGIN(group, pName)                                                                                   \
    if (CamX::g_traceInfo.groupsEnable & group)                                                                               \
    {                                                                                                                         \
        CAMX_ATRACE_BEGIN(group, pName);                                                                                      \
    }
...
#define CAMX_TRACE_MESSAGE(group, pString)                                                                                    \
    if (CamX::g_traceInfo.groupsEnable & group)                                                                               \
    {                                                                                                                         \
        CAMX_ATRACE_BEGIN(group, pString);                                                                                    \
        CAMX_ATRACE_END(group);                                                                                               \
    }
#else
