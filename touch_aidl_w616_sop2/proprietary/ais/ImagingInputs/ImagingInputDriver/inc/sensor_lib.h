 ...
 */
#define SENSOR_DRIVER_VERSION "2.5.0"
...
struct camera_i2c_bulk_reg_setting {
...
struct sensor_csi_params {
  ...
  unsigned char  vcx_mode;
  unsigned long long  mipi_rate;
};
...
typedef struct {
    ...
    int (*i2c_slave_write_array)(void* ctxt,
            unsigned short slave_addr,
            struct camera_i2c_reg_setting *reg_setting);
    /* i2c write bulk from slave_addr */
    int (*i2c_slave_write_bulk)(void* ctxt,
            unsigned short slave_addr,
            struct camera_i2c_bulk_reg_setting *reg_setting,
            boolean exec_pending_i2ccmds);
    /* i2c write array sync to slave_addr */
    int (*i2c_slave_write_array_sync)(void* ctxt, struct camera_i2c_sync_cfg sync_cfg,
            struct camera_i2c_reg_setting_array_sync* reg_setting_array_sync,
            int size);
    /* i2c bulk write then bulk read from slave_addr */
    ...
    int (*execute_power_setting)(void *ctxt,
            ...
            unsigned short size,
            CameraPowerEventType mode);
    /* setup gpio interrupt */
    int (*setup_gpio_interrupt)(void *ctrl,
            enum camera_gpio_type gpio_id,
            sensor_intr_callback_t cb, void *data);
    /* setup cci frame sync */
    int (*setup_cci_frame_sync)(void *ctrl,
            unsigned int fsync_freq);
    /* trigger cci frame sync */
    int (*trigger_cci_frame_sync)(void *ctrl);
    /* event callback from driver */
    int (*event_cb)(void *ctrl, CameraInputEventType type,
            CameraInputEventPayloadType *p_payload);

}sensor_platform_func_table_t;

/** Custom Function table
 *
 * Custom sensor driver functions instead of default behavior
 *
 * return value
 *   0 for success
 *   negative value for failure **/
typedef struct {
    /** Function to set platform function table pointers */
    ...
    int (*sensor_power_suspend)(void* ctxt, CameraPowerEventType powerEventId);
    int (*sensor_power_resume)(void* ctxt, CameraPowerEventType powerEventId);
    ...
    int (*sensor_set_channel_mode)(void* ctxt, unsigned int src_id, unsigned int mode);

    int (*sensor_set_cci_sync_param)(void* ctxt, void* param);
    ...
    int (*sensor_query_field)(void* ctxt, boolean *even_field, uint64 *field_ts);

    int (*sensor_process_frame_data)(void* ctxt, CameraInputProcessFrameDataType* p_frame);

    /** Sensor specific recovery function */
    int (*sensor_error_recovery)(void* ctxt, unsigned int severity);
...
typedef struct {
  ...
  int (*sensor_hdr_exposure_config)(void* ctxt, unsigned int src_id, qcarcam_hdr_exposure_config_t *);
...
typedef struct {
    ...
    unsigned int csi_idx;/* csi_params index */
...
typedef enum {
    ...
    SENSOR_CAPABILITY_GAMMA_CONFIG,
    SENSOR_CAPABILITY_VENDOR_PARAM,
    SENSOR_CAPABILITY_BRIGHTNESS,
    SENSOR_CAPABILITY_CONTRAST,
    SENSOR_CAPABILITY_MIRROR,
...
typedef struct
{
    ...
    struct sensor_csi_params             csi_params[CSIPHY_CORE_MAX];
...
typedef struct
{
    ...
    const CameraDeviceConfigType *config;
    enum sensor_camera_id cameraId;
} sensor_open_lib_t;
