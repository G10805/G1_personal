...
typedef enum
{
    ...
    /// This is the control ID for error recovery of the sensor.
    /// @par Type:
    ///   Input
    /// @par Input Expected:
    ///   Error severity
    ...
    /// This is the control ID for get vendor param
    /// @par Sensor Type Supporting This Configuration:
    ///   YCbCr only
    /// @par Type:
    ///   Input/Output
    /// @par Input Expected:
    ///   Pointer to a Camera_Sensor_ParamConfigType structure
    /// @par Output Expected:
    ///   Pointer to a Camera_Sensor_ParamConfigType structure with the
    ///   current values.
    Camera_Sensor_AEEUID_CTL_GET_VENDOR_PARAM,

    /// This is the control ID for getting sensor parameters
    /// @par Sensor Type Supporting This Configuration:
    ///   YCbCr only
    /// @par Type:
    ///   Input/Output
    ...
    /// Process or parse frame data.
    /// @par Type:
    ///   Input/Output
    /// @par Input Expected:
    ///   Pointer to CameraInputProcessFrameDataType
    /// @par Output Expected:
    ///   None
    Camera_Sensor_AEEUID_CTL_PROCESS_FRAME_DATA,

    /// Set CCI SYNC parameters
    /// @par Type:
    ///   Input/Output
    /// @par Input Expected:
    ///   Pointer to Camera_Sensor_ParamConfigType
    /// @par Output Expected:
    ///   None
...
typedef struct
{
    qcarcam_param_t paramId;
    uint32 srcId;

    union
    {
        qcarcam_exposure_config_t*     pExposureConfig;     /**< exposure */
        qcarcam_hdr_exposure_config_t* pHdrExposureConfig;  /**< HDR exposure */
        qcarcam_gamma_config_t*        pGammaConfig;        /**< gamma */
        qcarcam_vendor_param_t*        pVendorParam;        /**< vendor */
        camera_i2c_sync_cfg            CCISyncParam;

        float fVal;  /*Values can range from -1.0f to 1.0f.*/

        uint32 uVal;

        /// Ensure struct size will not change
        uint32 arr_padding[32];
    }param;
...
typedef struct {
    ...
    uint8  vcx_mode;
    uint64 mipi_rate;
} Camera_Sensor_MipiCsiInfo;

typedef struct {
    Camera_Sensor_MipiCsiInfo MipiCsiInfo[CSIPHY_CORE_MAX];
    uint32 sensor_num_frame_skip;
