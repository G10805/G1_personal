...
{ \
    { 0x0007, 0xC7, _ar0231_ext_isp_delay_ }, \
...
static int ap0200_poll_doorbell_bit(max9296_context_t* pCtxt, max9296_sensor_info_t* pSensor, unsigned int* reg_val);
static int ap0200_refresh_cmd(max9296_context_t* pCtxt, max9296_sensor_info_t* pSensor);
static int ap0200_req_exp_ctl(max9296_context_t* pCtxt, max9296_sensor_info_t* pSensor);
static int ap0200_apply_exposure(max9296_context_t* ctxt, uint32 link, sensor_exposure_info_t* exposure_info);
static int ap0200_apply_sensormode(max9296_context_t* ctxt, uint32 link, uint32 sensormode);
static int ap0200_apply_gamma(max9296_context_t* ctxt, uint32 link, qcarcam_gamma_config_t* gamma);
static int ar0231_ext_set_trigger_mode(max9296_context_t* ctxt, uint32 link);
...
static max9296_sensor_t ar0231_ext_isp_info = {
    ...
    .apply_sensormode = ap0200_apply_sensormode,
...
static maxim_pipeline_t ar0231_ext_isp_pipelines[AR0231_EXT_ISP_MODE_MAX] =
{
...
static int ar0231_ext_isp_detect(max9296_context_t* ctxt, uint32 link)
{
    ...
    int rc = 0;
    int i = 0;
    uint16 supported_ser_addr[] = {0xC4, 0x88, 0x80, 0};// List of serializer addresses we support
    ...
    if (FALSE == isSkipRemap)
    {
        ...
        if (MAX9296_FSYNC_MODE_DISABLED != pCtxt->max9296_config.sensors[link].fsync_mode)
        {
...
static int ar0231_ext_isp_get_link_cfg(max9296_context_t* ctxt, uint32 link, max9296_link_cfg_t* p_cfg)
{
    ...
    if(mode < AR0231_EXT_ISP_MODE_MAX)
    {
        ...
        p_cfg->pipelines[0] = ar0231_ext_isp_pipelines[mode];
    }
    else
    {
...
static int ar0231_ext_isp_init_link(max9296_context_t* ctxt, uint32 link)
{
    ...
    if (SENSOR_STATE_DETECTED == pSensor->state)
    {
        ...
        mode = ctxt->max9296_sensors[link].mode;
        switch (ar0231_ext_isp_pipelines[mode].mode.channel_info.dt)
        {
        case CSI_DT_RAW8:
            ar0231_ctxt->cam_ser_reg_setting.reg_array = link ? max9295_init_8bit_regs_1 : max9295_init_8bit_regs_0;
            ar0231_ctxt->cam_ser_reg_setting.size = link ? STD_ARRAY_SIZE(max9295_init_8bit_regs_1) : STD_ARRAY_SIZE(max9295_init_8bit_regs_0);
            break;
        case CSI_DT_RAW10:
            ar0231_ctxt->cam_ser_reg_setting.reg_array = link ? max9295_init_10bit_regs_1 : max9295_init_10bit_regs_0;
            ar0231_ctxt->cam_ser_reg_setting.size = link ? STD_ARRAY_SIZE(max9295_init_10bit_regs_1) : STD_ARRAY_SIZE(max9295_init_10bit_regs_0);
            break;
        default:
            SENSOR_WARN("link %d unknown DT: 0x%x", link,
                ar0231_ext_isp_pipelines[mode].mode.channel_info.dt);
            rc = CAMERA_EBADPARM;
        }
...
static int ar0231_ext_set_trigger_mode(max9296_context_t* pCtxt, uint32 link)
{
    max9296_sensor_info_t* pSensor = &pCtxt->max9296_sensors[link];
    ar0231_ext_isp_contxt_t* ar0231_ctxt = pSensor->pPrivCtxt;
    unsigned int state;
...
{
    /*
     * This camera's FPS can only be changed by updating external ISP firmware,
     * cannot be changed by setting register.
     * So we cannot support dynamically changing frame rate,
     * and left a blank function here.
     */
    max9296_context_t* pCtxt = ctxt;
    (void)link;
    (void)sensormode;
    ...
    SERR("ap0200 cannot support dynamically change mode");
    return 0;
}

static int ap0200_apply_gamma(max9296_context_t* ctxt, uint32 link, qcarcam_gamma_config_t* gamma_info)
{
    max9296_context_t* pCtxt = ctxt;
    max9296_sensor_info_t* pSensor = &pCtxt->max9296_sensors[link];
    ar0231_ext_isp_contxt_t* ar0231_ctxt = pSensor->pPrivCtxt;
