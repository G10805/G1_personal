  ...
  {
    {
      .output_mode =
      {
        ...
        .interlaced = INTERLACED_QUERY_FIELD_TYPE,
      },
      .num_subchannels = 1,
      .subchan_layout =
      {
...
static void* ba_sensor_open_lib(void* ctrl, void* arg)
{
    ...
    if (pCtxt)
    {
        ...
        if (pCtxt->input_type == HDMI)
        {
            ...
            pCtxt->sensor_lib.channels[0].output_mode.interlaced = INTERLACED_DISABLED;
            ...
            pCtxt->sensor_lib.subchannels[0].modes[0].interlaced = INTERLACED_DISABLED;
...
static int ba_sensor_config_resolution (void* ctxt, int32 width, int32 height)
{
    ...
    if (pCtxt->input_type == HDMI) {
        ...
        pCtxt->sensor_lib.csi_params[0].settle_cnt = csi_ctrl.settle_count;
        pCtxt->sensor_lib.csi_params[0].lane_cnt = csi_ctrl.lane_count;
    }
    SHIGH("%s:resolution %dx%d settle_cnt %d lane_cnt %d", __func__, width, height,
                pCtxt->sensor_lib.csi_params[0].settle_cnt, pCtxt->sensor_lib.csi_params[0].lane_cnt);
    return rc;
}
 ...
 **/
static int ba_sensor_field_info_query(void* ctxt, boolean *even_field, uint64 *field_ts)
{
    int rc = 0;
    ba_context_t* pCtxt =  (ba_context_t* )ctxt;
