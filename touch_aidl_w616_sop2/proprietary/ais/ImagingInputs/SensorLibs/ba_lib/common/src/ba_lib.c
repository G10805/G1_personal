...
)
{
   ...
   if ( AVIN_EVT_PORT_CONNECTED == cb_msg->event )
   {
      ...
      pCtxt->sensor_platform_fcn_tbl.event_cb(pCtxt->ctrl, INPUT_EVENT_LOCK_STATUS, &payload);
...
static void* ba_sensor_open_lib(void* ctrl, void* arg)
{
   ...
   if (pCtxt)
   {
       ...
       pCtxt->is_cvbs = !device_info->config->subdevId;
       if (!pCtxt->is_cvbs)
       {
           ...
           pCtxt->sensor_lib.csi_params[0].lane_cnt = 4;
           pCtxt->ba_port_id = BA_DEVICE_HDMI_PORT_ID;
       }
       else
       {
           pCtxt->sensor_lib.subchannels[0].modes[0].interlaced = INTERLACED_QUERY_FIELD_TYPE;
