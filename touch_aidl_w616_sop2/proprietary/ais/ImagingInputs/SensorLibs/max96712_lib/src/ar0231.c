...
typedef enum
{
    ...
    AR0231_MODE_16BIT_HDR_3EXP_30FPS = 3, /*not verified*/
    AR0231_MODE_20BIT_HDR_3EXP_30FPS = 4, /*not verified*/
    AR0231_MODE_MAX = 5
}ar0231_mode_id_t;
...
static int ar0231_detect(max96712_context_t* ctxt, uint32 link);
static int ar0231_get_link_cfg(max96712_context_t* ctxt, uint32 link, max96712_link_cfg_t* p_cfg);
static int ar0231_init_link(max96712_context_t* ctxt, uint32 link);
static int ar0231_start_link(max96712_context_t* ctxt, uint32 link);
static int ar0231_stop_link(max96712_context_t* ctxt, uint32 link);
static int ar0231_calculate_exposure(max96712_context_t* ctxt, uint32 link, sensor_exposure_info_t* exposure_info);
static int ar0231_apply_exposure(max96712_context_t* ctxt, uint32 link, sensor_exposure_info_t* exposure_info);
static int ar0231_apply_hdr_exposure(max96712_context_t* ctxt, uint32 link, qcarcam_hdr_exposure_config_t* hdr_exposure);

static max96712_sensor_t ar0231_info = {
...
static struct camera_i2c_reg_array max9295_init[] = CAM_SER_INIT;
...
static int ar0231_create_ctxt(max96712_sensor_info_t* pSensor)
{
    ...
    SERR("sensor mode %d", pSensor->mode);
...
static int ar0231_init_link(max96712_context_t* ctxt, uint32 link)
{
    ...
    if (SENSOR_STATE_DETECTED == pSensor->state)
    {
        ...
        ar0231_ctxt->cam_ser_reg_setting.reg_array = max9295_init;
        ar0231_ctxt->cam_ser_reg_setting.size = STD_ARRAY_SIZE(max9295_init);
        ...
        if (pSensor->mode == AR0231_MODE_12BIT_LINEAR_60FPS)
        {
...
static int ar0231_calculate_exposure(max96712_context_t* ctxt, uint32 link, sensor_exposure_info_t* exposure_info)
{
    ar0231_contxt_t* ar0231_ctxt = ctxt->max96712_sensors[link].pPrivCtxt;
    ...
    SENSOR_WARN("AEC - AR0231 : exp_time=%f => line_cnt=0x%x", exposure_info->exposure_time, exposure_info->line_count);
    ...
    SENSOR_WARN("AEC - AR0231 :idx = %d, gain=%0.2f, a_gain=%0.2f + d_gain=%0.2f",
...
static int ar0231_apply_exposure(max96712_context_t* ctxt, uint32 link, sensor_exposure_info_t* exposure_info)
{
       ...
       exposure_info->exposure_mode_type == QCARCAM_EXPOSURE_AUTO)
    {
        ...
        SENSOR_WARN("AEC - AR0231 : coarse=0x%x, fine=0x%x, real_gain=%0.2f",
...
static int ar0231_apply_hdr_exposure(max96712_context_t* ctxt, uint32 link, qcarcam_hdr_exposure_config_t* hdr_exposure)
{
    ...
    case QCARCAM_EXPOSURE_LUX_IDX:
    {
        ...
        SENSOR_WARN("AEC - AR0231 : exp_time=%f => line_cnt=0x%x", exposure_time, line_count);
