...
/* differentiate stream id between link A and B
*  by default both link A and B use pipeX
*  replicate linkB's stream to pipeZ(0x53->0x12)
*  enable pipeZ streaming(0x5B->0x10)
*/
...
{ \
    ...
    { 0x01B6, 0x08, _ar0231_ext_isp_delay_ }, \
    { 0x01B7, 0x09, _ar0231_ext_isp_delay_ }, \
    { 0x0053, 0x12, _ar0231_ext_isp_delay_ }, \
    { 0x005B, 0x10, _ar0231_ext_isp_delay_ }, \
}
...
static int ap0200_poll_doorbell_bit(max9296_context_t* pCtxt, unsigned int ext_isp_alias, unsigned int* reg_val);
static int ap0200_refresh_cmd(max9296_context_t* pCtxt, unsigned int ext_isp_alias);
static int ap0200_req_exp_ctl(max9296_context_t* pCtxt, unsigned int ext_isp_alias);
static int ap0200_apply_exposure(max9296_context_t* ctxt, uint32 link, sensor_exposure_info_t* exposure_info);
...
static struct camera_i2c_reg_setting ap0200_reg_setting_w =
{
    .reg_array = NULL,
    .size = 0,
    .addr_type = CAMERA_I2C_WORD_ADDR,
...
static struct camera_i2c_reg_setting ap0200_reg_setting_b =
{
    .reg_array = NULL,
    .size = 0,
    .addr_type = CAMERA_I2C_WORD_ADDR,
...
static int ap0200_poll_doorbell_bit(max9296_context_t* pCtxt, unsigned int ext_isp_alias, unsigned int* reg_val)
{
    ...
    ap0200_reg_setting_w.reg_array = ap0200_sys_cmd_reg;
    ap0200_reg_setting_w.size = STD_ARRAY_SIZE(ap0200_sys_cmd_reg);
    ...
    while (timeout)
    {
        if ((rc = pCtxt->platform_fcn_tbl.i2c_slave_read(
            pCtxt->ctrl,
            ext_isp_alias,
            &ap0200_reg_setting_w)))
        {
            SENSOR_ERROR("Failed to read AP0200 0x%x command status", ext_isp_alias);
            return rc;
        }

        if (!(ap0200_reg_setting_w.reg_array[0].reg_data & AP0200_CMD_DOORBELL_BIT))
        {
            //doorbell bit has been cleared, means command finished, returns result
            *reg_val = ap0200_reg_setting_w.reg_array[0].reg_data;
            rc = 0;
            break;
        }
        else
    ...
    if (0 == timeout)
    {
        SENSOR_ERROR("AP0200 0x%x poll door bell bit failed", ext_isp_alias);
        rc = -1;
    }
...
static int ap0200_refresh_cmd(max9296_context_t* pCtxt, unsigned int ext_isp_alias)
{
    ...
    ap0200_reg_setting_w.reg_array = ap0200_sys_cmd_reg;
    ap0200_reg_setting_w.size = STD_ARRAY_SIZE(ap0200_sys_cmd_reg);
    ap0200_reg_setting_w.reg_array[0].reg_data = AP0200_REFRESH_CMD;
    ...
    if (rc || ap0200_poll_doorbell_bit(pCtxt, ext_isp_alias, &state)
        || state != AP0200_STATE_ENOERR)
    {
        SENSOR_ERROR("AP0200 0x%x failed to issue refresh command", ext_isp_alias);
        return -1;
    }
    ...
    while(timeout)
    {
        ap0200_reg_setting_w.reg_array = ap0200_sys_cmd_reg;
        ap0200_reg_setting_w.size = STD_ARRAY_SIZE(ap0200_sys_cmd_reg);
        ap0200_reg_setting_w.reg_array[0].reg_data = AP0200_GET_STATE_CMD;
        ...
        if (rc || ap0200_poll_doorbell_bit(pCtxt, ext_isp_alias, &state))
        {
            SENSOR_ERROR("AP0200 0x%x failed to get latest refresh status", ext_isp_alias);
            return -1;
        }
        ...
        {
            SENSOR_ERROR("AP0200 0x%x unknown refresh status", ext_isp_alias);
            rc = -1;
            break;
        }
    }
    ...
    if (0 == timeout)
    {
        SENSOR_ERROR("AP0200 0x%x refresh command time out", ext_isp_alias);
        rc = -1;
    }
...
static int ap0200_req_exp_ctl(max9296_context_t* pCtxt, unsigned int ext_isp_alias)
{
    ...
    ap0200_reg_setting_b.reg_array = ap0200_exp_ctl_req;
    ap0200_reg_setting_b.size = STD_ARRAY_SIZE(ap0200_exp_ctl_req);
    ap0200_reg_setting_b.reg_array[0].reg_data = AP0200_EXP_CTL_BIT;

    rc = pCtxt->platform_fcn_tbl.i2c_slave_write_array(
        pCtxt->ctrl,
        ext_isp_alias,
        &ap0200_reg_setting_b);
    if (rc)
    {
        SENSOR_ERROR("ISP(0x%x) sensor manager commit new exposure failed", ext_isp_alias);
        return rc;
    }
    ...
    while (0 != timeout)
    {
        if ((rc = pCtxt->platform_fcn_tbl.i2c_slave_read(
            ...
            &ap0200_reg_setting_b)))
        {
            SENSOR_ERROR("Failed to read ISP sensor ctrl register = 0x%x", ext_isp_alias);
            return rc;
        }

        if (!(ap0200_reg_setting_b.reg_array[0].reg_data & AP0200_EXP_CTL_BIT))
        {
    ...
    {
        rc = -1;
        SENSOR_ERROR("ISP(0x%x) submit exposure change time out", ext_isp_alias);
    }
...
static int ap0200_apply_exposure(max9296_context_t* ctxt, uint32 link, sensor_exposure_info_t* exposure_info)
{
    ...
    {
        //switch to auto mode
        ap0200_reg_setting_b.reg_array = ap0200_ae_mode_auto;
        ap0200_reg_setting_b.size = STD_ARRAY_SIZE(ap0200_ae_mode_auto);
        ...
        rc = ap0200_refresh_cmd(pCtxt, pSensor->sensor_alias);
        if (rc)
        {
    ...
    else if (exposure_info->exposure_mode_type == QCARCAM_EXPOSURE_MANUAL)
    {
        ...
        ap0200_reg_setting_b.reg_array = ap0200_ae_mode_manual;
        ap0200_reg_setting_b.size = STD_ARRAY_SIZE(ap0200_ae_mode_manual);
        ...
        ap0200_reg_setting_w.reg_array = ap0200_exp_config;
        ap0200_reg_setting_w.size = STD_ARRAY_SIZE(ap0200_exp_config);
        ap0200_reg_setting_w.reg_array[0].reg_data = STD_MIN(FLOAT_TO_FIXEDPOINT(7, exposure_info->exposure_time), 0xffff);
        ap0200_reg_setting_w.reg_array[1].reg_data = STD_MIN(FLOAT_TO_FIXEDPOINT(7, exposure_info->real_gain), 0xffff);
        ...
        rc = ap0200_req_exp_ctl(pCtxt, pSensor->sensor_alias);
        if (rc)
        {
