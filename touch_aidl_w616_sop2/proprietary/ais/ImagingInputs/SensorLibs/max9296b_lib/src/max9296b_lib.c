                   ...
                   const char* param_value)
{
    ...
    {
        SERR("Error: Unknown section/name");
        return 0;  /* unknown section/name, error */
    }

    SHIGH("Identified section = %s, param_name = %s", section, param_name);
    return 1;
}
#endif
...
int max9296_read_ini_file(max9296_context_t * pCtxt)
{
    ...
    if(error)
    {
        SERR("Couldn't parse %s", filename);
        goto def_set;
    }

    SHIGH("topology section params: %d, %d",
    ...
    {
        SERR("Unsupported value of opMode param value = %d", pCtxt->max9296_config.opMode);
        error = -1;
        goto def_set;
    }
    ...
    {
        SERR("Unsupported value of num_of_cameras param value = %d", pCtxt->max9296_config.num_of_cameras);
        error = -1;
        goto def_set;
    }
...
def_set:
    if(error)
    {
        ...
        SERR("using default topology params: %d, %d",
                pCtxt->max9296_config.opMode,
                pCtxt->max9296_config.num_of_cameras
                );
    }
...
{
    max9296_context_t* pCtxt = CameraAllocate(CAMERA_ALLOCATE_ID_SENSOR_DRIVER_CTXT, sizeof(max9296_context_t));
    ...
    if (pCtxt)
    {
        ...
        /*default to dev id 0*/
        switch (device_info->config->subdevId)
        {
        case 1:
        case 3:
...
static int max9296_set_init_sequence(max9296_context_t* pCtxt)
{
    ...
    {
        //Set them to intialize as data is coming from another alcor and not actual cameras here
...
static int max9296_sensor_hdr_exposure_config(void* ctxt, unsigned int src_id, qcarcam_hdr_exposure_config_t* hdr_exposure)
{
    ...
    if (src_id < MAXIM_LINK_MAX)
    {
        if (pCtxt->max9296_sensors[src_id].sensor->apply_hdr_exposure)
        {
