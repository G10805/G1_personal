...
void PreferredDataState::sendResponse(std::shared_ptr<Message> msg, SetPreferredDataModemResult_t error) {
    ...
    if (m != nullptr) {
        ...
        mStateInfo->isTelephonySwitchedDDS = true;
    } else {
        SM_LOGE("Could not send response. Message is null");
        mStateInfo->isTelephonySwitchedDDS = false;
    }
    ...
    switch (e.event) {
        case InitializeSM:
        {
            ...
            if (status == Message::Callback::Status::SUCCESS) {
                ...
                status = ModemEndPointFactory<DSDModemEndPoint>::getInstance().buildEndPoint()->registerForDDSInd();
                if (status == Message::Callback::Status::SUCCESS) {
                    mStateInfo->dds = ddsInfo.dds_sub_id;
                    mStateInfo->currentDDSSwitchType = static_cast<dsd_dds_switch_type_enum_v01>(ddsInfo.switch_type);
                    ddsTransition();
                } else {
        ...
        case IAStarted:
        {
            //This can be received by both the RIL when IA is requested
            DDSSwitchInfo_t *switchInfo = static_cast<DDSSwitchInfo_t *>(e.data);
        ...
        case IAStartRequest:
        {
            //Slave RIL sends this event when IA is received on slave RIL
            //This is handled by Master RIL
...
int DefaultDataState::handleEvent(const PreferredDataEventType& e) {
    ...
    switch (e.event) {
        case DDSSwitchInternal:
        {
            Log::getInstance().d("[DefaultDataState]: DDSSwitchInternal switch type="+std::to_string((int)switchInfo->switch_type)+
...
int NonDefaultDataState::handleEvent(const PreferredDataEventType& e) {
    ...
    switch (e.event) {
        case DDSSwitchInternal:
        {
            Log::getInstance().d("[NonDefaultDataState]: DDSSwitchInternal switch type="+std::to_string((int)switchInfo->switch_type)+
                                 " subId="+std::to_string((int)switchInfo->subId)+" dds="+std::to_string((int)mStateInfo->dds));
            sendRecommendationToFramework(switchInfo);
            break;
        }
...
void ApTriggeredState::enter() {
    if (mStateInfo->isRilIpcNotifier) {
        ...
        auto ipcStarted = make_shared<DDSSwitchIPCMessage>(DDSSwitchIPCMessageState::ApStarted,
                    mStateInfo->dds, mStateInfo->currentDDSSwitchType, mStateInfo->tempDdsSwitched, mStateInfo->permanentDDSSub);
        ...
        if(msg && msg->event == PendingEvents::ddsRequest)
        {
            if(msg->msg != nullptr)
            {
                ...
                if (ddsRequestMessage != nullptr) {
                    status = ModemEndPointFactory<DSDModemEndPoint>::getInstance()
                        .buildEndPoint()->triggerDDSSwitchSync(ddsRequestMessage->getModemId(), error, switchType);
                } else {
            ...
            else
            {
                ...
                //With thsi recommendation, We will make permanent switch request for the current dds sub
                SM_LOGE("[ApTriggeredState]: pending SetPreferredDataModem message is null");
                if (mStateInfo->tempDdsSwitched &&
                    ...
                    mStateInfo->tempDdsSwitchRecommendInfo.value().switch_type == DSD_DDS_SWITCH_PERMANENT_V01) {
                        ModemEndPointFactory<DSDModemEndPoint>::getInstance()
                            .buildEndPoint()->triggerDDSSwitchSync(mStateInfo->tempDdsSwitchRecommendInfo.value().subId,
    ...
    switch (e.event) {
        case CurrentDDSInd:
        {
              ...
              if (mStateInfo->isRilIpcNotifier && switchInfo != nullptr) {
                ...
                if(mStateInfo->mPreferredDataMsgList->getNextMessageFromList()) {
                  auto msg =  std::static_pointer_cast<SetPreferredDataModemRequestMessage>(mStateInfo->mPreferredDataMsgList->getNextMessageFromList()->msg);
                  sendResponse(msg, SetPreferredDataModemResult_t::NO_ERROR);
                }

                if(mStateInfo->dds != mStateInfo->sub) {
        ...
        case DDSSwitchInd:
        {
            ...
            if (switchInfo != nullptr && switchInfo->switchAllowed == DDSSwitchRes::FAIL) {
                ...
                if(mStateInfo->mPreferredDataMsgList->getNextMessageFromList()) {
                    sendResponse(mStateInfo->mPreferredDataMsgList->getNextMessageFromList()->msg, SetPreferredDataModemResult_t::DDS_SWITCH_NOT_ALLOWED);
                }
            }
        ...
        case DDSSwitchTimerExpired:
        {
            ddsTransition();
            if(mStateInfo->mPreferredDataMsgList->getNextMessageFromList()) {
                sendResponse(mStateInfo->mPreferredDataMsgList->getNextMessageFromList()->msg, SetPreferredDataModemResult_t::DDS_SWITCH_TIMEOUT);
            }
            return HANDLED;
        }
        case SetPreferredDataModem:
        ...
        case ServiceDown:
        {
            ...
            if (mStateInfo->isRilIpcNotifier)
            {
                ...
                if(mStateInfo->mPreferredDataMsgList->getNextMessageFromList()) {
                    sendResponse(mStateInfo->mPreferredDataMsgList->getNextMessageFromList()->msg, SetPreferredDataModemResult_t::DDS_SWITCH_FAILED);
                }
