...
#include "sync/DdsSwitchRecommendInternalMessage.h"
...
DataModule::DataModule() : mMessageList("DataModule")  {
  ...
  if(preferred_data_state_info != nullptr)
  {
    ...
    preferred_data_state_info->isTelephonySwitchedDDS = true;
  }
  else
  {
  ...
  mMessageHandler = {
#ifndef RIL_FOR_LOW_RAM
                      HANDLER(RilRequestEmbmsActivateDeactivateTmgiMessage, DataModule::handleEmbmsActivateDeactivateTmgiMessage),
                      HANDLER(RilRequestEmbmsActivateTmgiMessage, DataModule::handleEmbmsActivateTmgiMessage),
                      HANDLER(RilRequestEmbmsContentDescUpdateMessage,DataModule::handleEmbmsContentDescUpdateMessage),
                      HANDLER(RilRequestEmbmsDeactivateTmgiMessage, DataModule::handleEmbmsDeactivateTmgiMessage),
                      HANDLER(EmbmsEnableDataReqMessage, DataModule::handleEmbmsEnableDataReqMessage),
                      HANDLER(RilRequestEmbmsGetActiveTmgiMessage, DataModule::handleEmbmsGetActiveTmgiMessage),
                      HANDLER(RilRequestEmbmsGetAvailTmgiMessage, DataModule::handleEmbmsGetAvailTmgiMessage),
                      HANDLER(RilRequestEmbmsSendIntTmgiListMessage, DataModule::handleEmbmsSendIntTmgiListMessage),
                      HANDLER(RilDataEmbmsActiveMessage, DataModule::handleDataEmbmsActiveMessage),
                      ...
                      HANDLER(SetRadioClientCapUnSolMessage, DataModule::handleSetRadioClientCapUnSolMessage),
...
void DataModule::handleProcessStackSwitchMessage(std::shared_ptr<Message> msg) {
  ...
  if( m != NULL ) {
    ...
    if (info.instanceId != dataCtlInstanceId) {
      ...
      if (mInitCompleted) {
        dcSymbols.datactlInit(dataCtlInstanceId, logFn);
      }
    }
...
void DataModule::handleDdsSwitchRecommendInternalMessage(std::shared_ptr<Message> msg) {
  ...
  if (m != nullptr && preferred_data_state_info != nullptr) {
    ...
    m->sendResponse(m, Message::Callback::Status::SUCCESS, nullptr);
  } else {
...
void DataModule::handleEmbmsActivateDeactivateTmgiMessage(std::shared_ptr<Message> msg) {
  ...
  if( m != NULL ) {
    embms_activate_deactivate_tmgi_req_msg_v01 req = m->get_params();
    call_manager->handleEmbmsActivateDeactivateTmgi(req, msg);
  } else {
    Log::getInstance().d("[" + mName + "]: Improper message received = " + msg->dump());
  }
}
...
void DataModule::handleEmbmsActivateTmgiMessage(std::shared_ptr<Message> msg) {
  ...
  if( m != NULL ) {
    embms_activate_tmgi_req_msg_v01 req = m->get_params();
    call_manager->handleEmbmsActivateTmgi(req, msg);
  } else {
    Log::getInstance().d("[" + mName + "]: Improper message received = " + msg->dump());
  }
}
...
void DataModule::handleEmbmsContentDescUpdateMessage(std::shared_ptr<Message> msg) {
  ...
  if( m != NULL ) {
    embms_update_content_desc_req_msg_v01 req = m->get_params();
    call_manager->handleEmbmsContentDescUpdate(req, msg);
  } else {
    Log::getInstance().d("[" + mName + "]: Improper message received = " + msg->dump());
  }
}
...
void DataModule::handleEmbmsDeactivateTmgiMessage(std::shared_ptr<Message> msg) {
  ...
  if( m != NULL ) {
    embms_deactivate_tmgi_req_msg_v01 req = m->get_params();
    call_manager->handleEmbmsDeactivateTmgi(req, msg);
  } else {
    Log::getInstance().d("[" + mName + "]: Improper message received = " + msg->dump());
  }
}
...
void DataModule::handleEmbmsEnableDataReqMessage(std::shared_ptr<Message> msg) {
  ...
  if( m != NULL ) {
    call_manager->handleEmbmsEnableDataRequestMessage();
  } else {
    Log::getInstance().d("[" + mName + "]: Improper message received = " + msg->dump());
  }
}
...
void DataModule::handleEmbmsGetActiveTmgiMessage(std::shared_ptr<Message> msg) {
  ...
  if( m != NULL ) {
    embms_get_active_tmgi_req_msg_v01 req = m->get_params();
    call_manager->handleEmbmsGetActiveTmgi(req, msg);
  } else {
    Log::getInstance().d("[" + mName + "]: Improper message received = " + msg->dump());
  }
}
...
void DataModule::handleEmbmsGetAvailTmgiMessage(std::shared_ptr<Message> msg) {
  ...
  if( m != NULL ) {
    embms_get_available_tmgi_req_msg_v01 req = m->get_params();
    call_manager->handleEmbmsGetAvailableTmgi(req, msg);
  } else {
    Log::getInstance().d("[" + mName + "]: Improper message received = " + msg->dump());
  }
}
...
void DataModule::handleEmbmsSendIntTmgiListMessage(std::shared_ptr<Message> msg) {
  ...
  if( m != NULL ) {
    embms_get_interested_tmgi_list_resp_msg_v01 req = m->get_params();
    call_manager->handleEmbmsSendInterestedList(req, msg);
  } else {
    Log::getInstance().d("[" + mName + "]: Improper message received = " + msg->dump());
  }
}
...
void DataModule::handleGetDdsSubIdMessage(std::shared_ptr<Message> msg) {
  ...
  if( m != NULL ) {
    ...
    if (dsd_endpoint) {
        dsd_endpoint->getCurrentDDSSync(ddsInfo);
    }
...
void DataModule::handleDataRequestDDSSwitchMessage(std::shared_ptr<Message> msg) {
  ...
  if( m != NULL ) {
    ...
    if (dsd_endpoint) {
        int error = 0;
        if (Message::Callback::Status::SUCCESS == dsd_endpoint->triggerDDSSwitchSync(m->dds_sub_info.dds_sub_id,
...
void DataModule::handleQmiDsdEndpointStatusIndMessage(std::shared_ptr<Message> msg) {
  ...
  if (shared_indMsg->getState() == ModemEndPoint::State::OPERATIONAL) {
    if( !mInitTracker.getDsdServiceReady() ) {
       ...
       if ( mInitCompleted ) {
         //Post SSR, getting current DDS at modem & registring for DDS indication
         if(preferred_data_state_info->isRilIpcNotifier) {
          ...
          dsd_endpoint->registerForDDSInd();
        }
...
void DataModule::performDataModuleInitialization() {
  if( mInitTracker.allServicesReady() && mInitTracker.isModeDetermined()) {
    if( !mInitCompleted ) {
      ...
      auto cb = std::make_shared<std::function<void(std::shared_ptr<Message>, RIL_Errno)>>(std::bind(&DataModule::sendIAResponse, this, std::placeholders::_1, std::placeholders::_2));
      ...
      if(wds_endpoint != nullptr) {
        ...
        if (status != Message::Callback::Status::SUCCESS) {
          Log::getInstance().d("[DataModule] failed to get call bringup capability, result = "
      ...
      wds_endpoint->registerforTdInfoInd();
      propValue = readProperty("persist.vendor.radio.nswo_enabled", "0");
      wds_endpoint->setV2Capabilities(propValue == "1");
      wds_endpoint->registerForPdnThrottle();
      ...
      if( mInitTracker.isAPAssistMode() ) {
        ...
        if( mInitTracker.isIWLANEnabled() ) {
          ...
          status = dsd_endpoint->registerForAPAsstIWlanInd(true);
...
std::shared_ptr<IPCMessage> DataModule::constructDDSSwitchIPCMessage(IPCIStream& is) {
  ...
  std::shared_ptr<DDSSwitchIPCMessage> ipcMessage = std::make_shared<DDSSwitchIPCMessage>(
     DDSSwitchIPCMessageState::ApStarted, -1, DSD_DDS_SWITCH_PERMANENT_V01, false, -1);
