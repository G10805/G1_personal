...
void RadioDataServiceImpl::sendRadioClientCapabilities() {
  Log::getInstance().d("[ " + mName + " ] SendRadioClientCapabilities");
  bool isAPAssistCap = true;
  std::function<void(const std::vector<rildata::DataCallResult_t>&)> dcListChangedCallback =
              [](const vector<rildata::DataCallResult_t>& dcList) -> void {
  ...
  if (mIRadioDataResponse == nullptr || mIRadioDataIndication == nullptr) {
    Log::getInstance().d("[ " + mName + " ] RespCb or IndCb is nullptr");
    isAPAssistCap = false;
  }
...
)
{
  ...
  memset(&slicingConfig, 0, sizeof(slicingConfig));
  this->sendResponseForGetSlicingConfig(responseInfo, slicingConfig);
#endif
  return ndk::ScopedAStatus::ok();
}
