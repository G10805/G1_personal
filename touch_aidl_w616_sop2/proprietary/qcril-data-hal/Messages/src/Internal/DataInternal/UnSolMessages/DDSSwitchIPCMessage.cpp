...
DDSSwitchIPCMessage::DDSSwitchIPCMessage(DDSSwitchIPCMessageState setState,
                      int setSubId, dsd_dds_switch_type_enum_v01 switch_type, bool tempdds, int permanentSub):
  IPCMessage(get_class_message_id()), state(setState), subId(setSubId),
  isTempDdsSwitched(tempdds), permanentDDSSub(permanentSub), switchType(switch_type)
{}
...
{
  os << static_cast<int>(state) << "," << subId << "," << static_cast<int>(switchType) << "," << isTempDdsSwitched << "," << permanentDDSSub;
}
...
{
  auto copy = std::make_shared<DDSSwitchIPCMessage>(state, subId, switchType, isTempDdsSwitched, permanentDDSSub);
...
string DDSSwitchIPCMessage::dump()
{
  ...
  return std::string(MESSAGE_NAME) + " isRemote: " +
         ...
         " isTempDDS: " + tempDDS + " permanentDDSSub: " + std::to_string(permanentDDSSub);
}
