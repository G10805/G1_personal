...
#define MAX_LINX_ADDRESS_TIMER_VAL 0x7FFFFFFFFFFFFFFF
#define QMI_CLIENT_INSTANCE_MODEM 0x0
...
struct AddrInfo {
  ...
  void dump(std::string padding, std::ostream& os) {
    os << padding << " address: " << ipAddress;
    os << padding << " gateway address: " << gatewayAddress;
    os << padding << " dns addresses: " << dnsAddress;
    os << padding << " dep time: " << deprecationTime;
    os << padding << " exp time: " << expirationTime;
    os << padding << " deprecated: " << (isDeprecated ? "true" : "false");
  }
};

struct PduInfoByIpFamily {
  string pcscfAddress;
  int32_t mtu;
  std::map<AddrType,AddrInfo> addrMap;

  void dump(std::string padding, std::ostream& os) {
    for (auto addr: addrMap) {
      os << padding << " addr type: " << (int)addr.first;
      addr.second.dump(padding, os);
    }
    os << padding << " pcscf address: " << pcscfAddress;
    os << padding << " mtu: " << mtu;
  }

  bool operator !=(const PduInfoByIpFamily& other) {
    if (addrMap.size() != other.addrMap.size()) {
      Log::getInstance().d("addrMap size is different");
      return true;
    }
    std::map<rildata::AddrType, rildata::AddrInfo>::iterator addr1 = addrMap.begin();
    std::map<rildata::AddrType, rildata::AddrInfo>::const_iterator addr2 = other.addrMap.begin();
    for (addr1 = addrMap.begin(), addr2 = other.addrMap.begin();
          addr1 != addrMap.end(); addr1++, addr2++) {
      if ((addr1->first != addr2->first) ||
          (addr1->second.ipAddress.compare(addr2->second.ipAddress)) ||
          (addr1->second.gatewayAddress.compare(addr2->second.gatewayAddress)) ||
          (addr1->second.dnsAddress.compare(addr2->second.dnsAddress)) ||
          (addr1->second.deprecationTime != addr2->second.deprecationTime) ||
          (addr1->second.expirationTime != addr2->second.expirationTime)){
        Log::getInstance().d("Addr contents are different for " + std::to_string((int)(addr1->first)));
        return true;
      }
    }

    if (pcscfAddress.compare(other.pcscfAddress)) {
      Log::getInstance().d("PCSCF address is different");
      return true;
    }
    if (mtu != other.mtu) {
      Log::getInstance().d("MTU is different");
      return true;
    }
    return false;
  }
...
struct PduInfo {
  ...
  std::optional<PduInfoByIpFamily> v4Info;
  std::optional<PduInfoByIpFamily> v6Info;

  PduInfo(): pduId(0), active(LinkActiveState::_eInactive) {}
  void dump(std::string padding, std::ostream& os) {
    os << padding << "pdu id: " << pduId << std::endl;
    os << padding << " active: " << active << std::endl;
    if (v4Info) {
      os << padding << " v4 info: ";
      v4Info->dump(padding, os);
      os << padding << std::endl;
    }
    if (v6Info) {
      os << padding << " v6 info: ";
      v6Info->dump(padding, os);
      os << padding << std::endl;
    }
  }

  bool operator !=(const PduInfo& other) {
    if (v4Info) {
      if (!other.v4Info) {
        Log::getInstance().d("V4 info not present");
        return true;
      }
      if (*v4Info != *other.v4Info) {
        Log::getInstance().d("V4 info changed");
        return true;
      }
    }
    else if (other.v4Info) {
      Log::getInstance().d("V4 info added");
      return true;
    }
    if (v6Info) {
      if (!other.v6Info) {
        Log::getInstance().d("V6 info not present");
        return true;
      }
      if (*v6Info != *other.v6Info) {
        Log::getInstance().d("V6 info changed");
        return true;
      }
    }
    else if (other.v6Info) {
      Log::getInstance().d("V6 info added");
      return true;
    }
    return false;
  }
  ...
  string concatGatewayAddress() const {
    string tmpString = "";
    if (v4Info) {
      std::map<AddrType,AddrInfo> tmpAddrMap = v4Info->addrMap;
      //There should only be 1 V4 address
      if ((tmpAddrMap.size() > 0) &&
          (tmpAddrMap[AddrType::_eInvalid].gatewayAddress.length() > 0)) {
        tmpString = tmpAddrMap[AddrType::_eInvalid].gatewayAddress;
        tmpString += " ";
      }
    }
    if (v6Info){
      //Only pick gateway address of primary/default v6 addr
      for (auto const& addr: v6Info->addrMap) {
        if (addr.first == AddrType::_eDefault) {
          tmpString += addr.second.gatewayAddress;
          break;
        }
      }
    }
  ...
  string concatDnsAddress() const {
    string tmpString = "";
    if (v4Info) {
      std::map<AddrType,AddrInfo> tmpAddrMap = v4Info->addrMap;
      //There should only be 1 V4 address
      if ((tmpAddrMap.size() > 0) &&
          (tmpAddrMap[AddrType::_eInvalid].dnsAddress.length() > 0)) {
        tmpString = tmpAddrMap[AddrType::_eInvalid].dnsAddress;
        tmpString += " ";
      }
    }
    if (v6Info){
      //Only pick dns address of primary/default or candidate v6 addr
      for (auto const& addr: v6Info->addrMap) {
        if ((addr.first == AddrType::_eDefault) || (addr.first == AddrType::_eCandidate)) {
          tmpString += addr.second.dnsAddress;
          tmpString += " ";
        }
      }
    }
  ...
  string concatPcscfAddress() const {
    string tmpString = "";
    if (v4Info && (v4Info->pcscfAddress.length() > 0)) {
      tmpString = v4Info->pcscfAddress;
      tmpString += " ";
    }
    if (v6Info){
      tmpString += v6Info->pcscfAddress;
    }
    return tmpString;
  }
};
...
enum class LqeIndicationSwitch: uint16_t {
  LQE_TURN_OFF_INDICATIONS    = 0x00,
  LQE_TURN_ON_INDICATIONS     = 0x01
};

enum class LqeDataDirection : uint16_t {
...
inline std::ostream& operator<<(std::ostream& os, DataProfileInfoType_t type);
...
enum class PdpProtocolType_t : int32_t {
  UNKNOWN = -1,
  IP = 0,
  IPV6 = 1,
  IPV4V6 = 2,
...
enum class HandoverFailureMode_t : int32_t {
  Legacy                      = 0,
  DoFallback                  = 1,
  NoFallbackRetryHandover     = 2,
  NoFallbackRetrySetupNormal  = 3,
};

enum class QosProtocol_t : int8_t {
  UNSPECIFIED = -1,
  TCP         = 6,
  UDP         = 17,
  ESP         = 50,
...
enum class QosFilterDirection_t : int8_t {
  DOWNLINK      = 0,
  UPLINK        = 1,
  BIDIRECTIONAL = 2,
};
...
struct TrafficDescriptor_t {
  ...
  bool operator==(TrafficDescriptor_t const& other) const {
    if (((!dnn && !other.dnn) ||
         (dnn && other.dnn && !dnn.value().compare(other.dnn.value()))) &&
        ((!osAppId && !other.osAppId ) ||
         (osAppId && other.osAppId && osAppId.value() == other.osAppId.value())))
    {
      return true;
    }
    else {
  ...
  void dump(std::string padding, std::ostream& os) const {
    ...
    ss << ",appid:";
    if(osAppId.has_value() && osAppId.value().size() > 17) {
      ss << std::to_string(osAppId.value()[16]) << "/";
      for (auto it=osAppId.value().begin()+17 ;
          it!=osAppId.value().begin()+17+osAppId.value()[16] ; ++it) {
        ss << *it;
      }
    }
    os << ss.str();
  ...
  std::string dump() const {
    ...
    if(osAppId.has_value() && osAppId.value().size() > 16) {
      ss << ",appid:";
      ss << std::to_string(osAppId.value()[16]) << "/";
      for (auto it=osAppId.value().begin()+17 ;
          it!=osAppId.value().begin()+17+osAppId.value()[16] ; ++it) {
    ...
    } else if(!osAppId.has_value()) {
      ss << ",appid:no value";
    }
    return ss.str();
  }

  string getDnn() {return dnn.value_or("");}
  std::vector<uint8_t> getOsAppId() {
    if(osAppId.has_value()) {
      return osAppId.value();
    } else return {};
  }
  std::vector<uint8_t> getOsId() {
    if(osAppId.has_value()) {
      std::vector<uint8_t> osid;
      for (auto it=osAppId.value().begin() ; it<osAppId.value().begin()+16 ; ++it) {
        osid.push_back(*it);
      }
      return osid;
    } else return {};
  }
  std::vector<uint8_t> getAppId() {
    if(osAppId.has_value()) {
      std::vector<uint8_t> appid;
      for (auto it=osAppId.value().begin()+17 ; it<osAppId.value().begin()+17+osAppId.value().begin()[16] ; ++it) {
        appid.push_back(*it);
      }
      return appid;
    } else return {};
...
struct DataProfileInfo_t {
  ...
  string apn;
  /*protocol represents with which iptype the call is attempted*/
  string protocol;
  /*homeProtocol represents original home protocol received with setupdatacall*/
  string homeProtocol;
  /*roamingProtocol represents original roaming protocol received with setupdatacall*/
  ...
  bool preferred;
  bool persistent;
  std::optional<TrafficDescriptor_t> trafficDescriptor;
  bool matchAllRuleAllowed;
  bool operator==(DataProfileInfo_t const& other) const {
    //Compare the parameters we use
    if ((profileId == other.profileId) &&
        (!apn.compare(other.apn)) &&
        (!protocol.compare(other.protocol)) &&
        (!roamingProtocol.compare(other.roamingProtocol)) &&
        (dataProfileInfoType == other.dataProfileInfoType) &&
        (supportedApnTypesBitmap == other.supportedApnTypesBitmap) &&
        (preferred == other.preferred) &&
        (persistent == other.persistent) &&
        ((!trafficDescriptor && !other.trafficDescriptor) ||
         //If both have TDs then compare them
         (trafficDescriptor && other.trafficDescriptor &&
          trafficDescriptor == other.trafficDescriptor)))
    {
      return true;
  ...
  void dump(std::string padding, std::ostream& os) const {
    os << std::boolalpha << padding << "{"
        ...
        << persistent << ","
        << ",matchAllRule:"
        << std::boolalpha << matchAllRuleAllowed << "}";
    if (trafficDescriptor)
    {
      trafficDescriptor.value().dump(padding, os);
    }
  }
};
...
struct appIdList_t {
  ...
  void dump(std::string padding, std::ostream& os) const {
    std::stringstream ss;
    ss << "{osAppId:";
    for (auto it=osAppId.begin() ; it!=osAppId.end() ; ++it) {
      ss << std::to_string(*it);
    }
    ss << "}";
    os << padding << ss.str();
...
struct TrafficDescriptorInfo_t {
  vector<string> dnnList;
  vector<appIdList_t> appIdList;
  void dump(std::string padding, std::ostream& os) const {
    std::ignore = padding;
    if(dnnList.size() > 0) {
      os << ",dnn={";
    ...
    if(appIdList.size() > 0) {
      os << "},appIdList={";
      for(auto a : appIdList) {
        a.dump("", os);
        os << ",";
      }
    } else {
      os << "},appIdList={No value ";
    }
    os << "}";
    os << "}";
  }
...
struct TrafficDescriptorFragment_t {
  uint16_t tdIdx;
  std::optional<TrafficDescriptorInfo_t> tdInfo;
  void dump(std::string padding, std::ostream& os) const {
    os << padding << "{tdIdx=";
    os << tdIdx << ",tdInfo=";
    if(tdInfo.has_value()) {
      tdInfo.value().dump("", os);
    }
    os << "}";
  }
};
...
struct DataCallResult_t {
  ...
  HandoverFailureMode_t handoverFailureMode;
  vector<TrafficDescriptor_t> trafficDescriptors;
  void dump(std::string padding, std::ostream& os) const {
    os << padding << "cid=" << cid << "{"
        << (int)cause << ","
        << suggestedRetryTime << ","
    ...
    os << "},"
       << (int)handoverFailureMode << "}" << std::endl << "td={";
    for (TrafficDescriptor_t td : trafficDescriptors) {
      td.dump("", os);
      os << ",";
    }
    os << "}";
...
struct SetupDataCallResponse_t {
  ...
  void dump(std::string padding, std::ostream& os) const {
    os << padding << "respErr=" << (int)respErr << std::endl;
    call.dump("", os);
  }
};
...
struct RadioDataService_t {
  ...
  void dump(std::string padding, std::ostream& os) const {
    if(radioFamily == RadioAccessFamily_t::NR) {
      int nsa = (isNSA)?1:0;
      os << padding << "<Tech: " << techType << " RadioFamily: " << radioFamily << "isNSA" << nsa <<" RadioFreq: ";
    }
    else {
      os << padding << "<Tech: " << techType << " RadioFamily: " << radioFamily << " RadioFreq: ";
    }
...
enum class SliceInfoServiceType_t : uint8_t {
  SERVICE_TYPE_NONE = 0,
  SERVICE_TYPE_EMBB = 1,
  SERVICE_TYPE_URLLC = 2,
  SERVICE_TYPE_MIOT = 3,
};

enum class SliceInfoStatusType_t : uint8_t {
  STATUS_UNKNOWN = 0,
  STATUS_CONFIGURED = 1,
  STATUS_ALLOWED = 2,
  STATUS_REJECTED_NOT_AVAILABLE_IN_PLMN = 3,
  STATUS_REJECTED_NOT_AVAILABLE_IN_REG_AREA = 4,
  STATUS_DEFAULT_CONFIGURED = 5,
};

// S-NSSAI as defined in 3GPP TS 24.501 section 9.11.2.8
struct SliceInfo_t {
...
struct RouteSelectionDescriptor_t {
  uint8_t precedence;
...
struct SlicingConfig_t {
  std::vector<UrspRule_t> urspRules;
  std::vector<SliceInfo_t> sliceInfos;
};

class WdsThrottleInfo {
private:
  std::string apn;
  std::optional<uint32_t> v4ThrottleTime;
  std::optional<uint32_t> v6ThrottleTime;
public:
  WdsThrottleInfo(std::string apn): apn(apn) {}
  std::string getApn() { return apn; }

  bool hasV4ThrottleTime() { return v4ThrottleTime ? true : false; }
  uint32_t getV4ThrottleTime() { return *v4ThrottleTime; }
  void setV4ThrottleTime(uint32_t val) { v4ThrottleTime = val; }

  bool hasV6ThrottleTime() { return v6ThrottleTime ? true : false; }
  uint32_t getV6ThrottleTime() { return *v6ThrottleTime; }
  void setV6ThrottleTime(uint32_t val) { v6ThrottleTime = val; }

  std::string dump() {
    std::stringstream ss;
    ss << "{" << apn << " ";
    if (hasV4ThrottleTime()) {
      ss << "v4: " << (int)getV4ThrottleTime() << ",";
    }
    if (hasV6ThrottleTime()) {
      ss << "v6: " << (int)getV6ThrottleTime() << ",";
    }
    ss << "}";
    return ss.str();
  }
...
constexpr uint32_t QMI_INFINITE_THROTTLING = 0xffffffff;
constexpr int64_t RIL_INFINITE_THROTTLING = 0x7fffffffffffffff;

inline int64_t convertToRilThrottleTime(uint32_t ms) {
  // infinite throttling conversion
  if (ms == QMI_INFINITE_THROTTLING) {
    return RIL_INFINITE_THROTTLING;
  }
  // safe conversion from uint32 to int64
  return (int64_t)ms;
}

struct ThrottleApn_t {
  DataProfileInfo_t profileInfo;
  std::unordered_map<int, TimeKeeper::timer_id> timer;
};

struct RadioServiceStatus_t {
  std::unordered_map<std::string, RadioDataService_t> apnPreferredRadioDataService;
  std::vector<rildata::RadioDataService_t> availableRadioDataService;
  std::unordered_map<std::string, ThrottleApn_t> throttledApns;
  RadioDataService_t lastAvailableRadioDataService;
};
...
{
  os << std::to_string((int)type) << ":";
  if (ApnTypes_t::NONE == type) {
      os << "NONE";
      return os;
  }
  if (ApnTypes_t::DEFAULT == (ApnTypes_t)((int)type & (int)ApnTypes_t::DEFAULT)) {
      os << "DEFAULT|";
  }
  if (ApnTypes_t::MMS == (ApnTypes_t)((int)type & (int)ApnTypes_t::MMS)) {
      os << "MMS|";
  }
  if (ApnTypes_t::SUPL == (ApnTypes_t)((int)type & (int)ApnTypes_t::SUPL)) {
      os << "SUPL|";
  }
  if (ApnTypes_t::DUN == (ApnTypes_t)((int)type & (int)ApnTypes_t::DUN)) {
      os << "DUN|";
  }
  if (ApnTypes_t::HIPRI == (ApnTypes_t)((int)type & (int)ApnTypes_t::HIPRI)) {
      os << "HIPRI|";
  }
  if (ApnTypes_t::FOTA == (ApnTypes_t)((int)type & (int)ApnTypes_t::FOTA)) {
      os << "FOTA|";
  }
  if (ApnTypes_t::IMS == (ApnTypes_t)((int)type & (int)ApnTypes_t::IMS)) {
      os << "IMS|";
  }
  if (ApnTypes_t::CBS == (ApnTypes_t)((int)type & (int)ApnTypes_t::CBS)) {
      os << "CBS|";
  }
  if (ApnTypes_t::IA == (ApnTypes_t)((int)type & (int)ApnTypes_t::IA)) {
      os << "IA|";
  }
  if (ApnTypes_t::EMERGENCY == (ApnTypes_t)((int)type & (int)ApnTypes_t::EMERGENCY)) {
      os << "EMERGENCY|";
  }
  if (ApnTypes_t::MCX == (ApnTypes_t)((int)type & (int)ApnTypes_t::MCX)) {
      os << "MCX|";
  }
  if (ApnTypes_t::XCAP == (ApnTypes_t)((int)type & (int)ApnTypes_t::XCAP)) {
      os << "XCAP|";
  }
  return os;
}
...
enum class SubscriptionId_t : int8_t {
  INVALID_SUBSCRIPTION_ID   = -1,
  PRIMARY_SUBSCRIPTION_ID   = 0,
  SECONDARY_SUBSCRIPTION_ID = 1,
  TERTIARY_SUBSCRIPTION_ID  = 2,
};

enum class TcpKeepaliveType_t {
  TCP_KEEP_ALIVE = 3, //KeepaliveType_t::NATT_IPV6 +1,
  TCP_HEART_BEAT = 4, //KeepaliveType_t::NATT_IPV6 +2,
};
...
struct tcp_socket_status_t {
  uint32_t uplink_seqnum;
  uint32_t uplink_window;
  uint32_t downlink_seqnum;
  uint32_t downlink_window;
};

struct TcpKeepaliveRequest_t {
  uint32_t client_id;
  TcpKeepaliveType_t type;
  IpTuple tuple;
  int32_t maxKeepaliveIntervalMillis;
  uint32_t config_handle;
  tcp_socket_status_t tcp_status;
  std::vector<uint8_t> ka_payload;
  std::vector<uint8_t> ack_payload;
};

enum class TcpKaResponseError_t {
  ERROR,
  SUCCESS,
  NETWORK_DOWN,
  IPTUPLE_DEST_IP_INVALID,
  IPTUPLE_SRC_IP_INVALID,
  IPTUPLE_DEST_PORT_INVALID,
  IPTUPLE_SRC_PORT_INVALID,
  INVALID_CONFIG_HANDLE,
  INVALID_KA_HANDLE,
  KA_PAYLOAD_EMPTY,
  ACK_PAYLOAD_EMPTY,
  INVALID_KEEP_ALIVE_PARAMS,
  INVALID_TUPLE_INFO,
  UPLINK_DATA_INVALID,
  DOWNLINK_DATA_INVALID,
  TCP_STATUS,
};

enum class Tcp_keepAlive_Type_t {
  KA_HANDLE_STATUS,
  CONFIG_HANDLE_STATUS,
  TCP_STATUS,
};

struct TcpKeepAliveIndInfo_t {
  TcpKaResponseError_t status;
  uint32_t ka_handle;
  Tcp_keepAlive_Type_t ind_type;
  TcpKeepaliveType_t type;
  uint32_t config_handle;
  tcp_socket_status_t tcp_status;
};
