      ...
      if (pendingMsg) {
        ...
        if (!matchedCalls.empty()) {
          ...
          if (cid >= 0) {
            ...
            if (status == PduSessionParamLookupResultStatus_t::SUCCESS) {
              callInfo.callBringUpCapability = BRING_UP_APN_NAME;
    ...
    for(auto it = tdList.begin(); it != tdList.end(); it++) {
      for (auto& psp : callInfo.pduSessionParams) {
        for (auto rp = psp.requestProfiles.begin(); rp != psp.requestProfiles.end(); ) {
          ...
          if (rp->trafficDescriptor.has_value() && rp->trafficDescriptor.value() == *it)
          {
...
void CallManager::handleEmbmsEnableDataRequestMessage()
{
  int cid = getAvailableCid();
  list<reference_wrapper<CallStateMachine>> matchedCalls;
  if (cid < 0) {
  ...
  PduSessionParam_t defaultPduSessionParam = {};
  DataProfileInfo_t defaultRequestProfile;
  callInfo.pduSessionParams.push_back(defaultPduSessionParam);
  callInfo.pduSessionParams[0].requestProfiles.push_back(defaultRequestProfile);

  callInfo.pduSessionParams[0].requestProfiles[0].apn = "";
  callInfo.pduSessionParams[0].requestProfiles[0].protocol = "IP";
  callInfo.pduSessionParams[0].requestProfiles[0].homeProtocol = "IP";
  callInfo.roamingAllowed = true;
...
void CallManager::handleEmbmsActivateTmgi(embms_activate_tmgi_req_msg_v01 req, std::shared_ptr<Message> msg)
{
    ...
    if (!matchedCalls.empty()) {
      ...
      if (callInfo.embmsInfo != nullptr) {
        ...
        callInfo.embmsInfo->activateTmgi(req, msg);
      }
    }
}

void CallManager::handleEmbmsDeactivateTmgi(embms_deactivate_tmgi_req_msg_v01 req, std::shared_ptr<Message> msg)
{
    Log::getInstance().d("[CallManager]: handleEmbmsDeactivateTmgi");
    auto matchedCalls = mFilter
      .matchCid(req.call_id)
      .getResults();
    if (!matchedCalls.empty()) {
      CallStateMachine& callInstance = matchedCalls.front();
      CallInfo& callInfo = callInstance.getCallInfo();
      if (callInfo.embmsInfo != nullptr) {
        callInfo.embmsInfo->setDsiHandle(callInfo.dsiHandle);
        callInfo.embmsInfo->deactivateTmgi(req, msg);
      }
    }
}

void CallManager::handleEmbmsActivateDeactivateTmgi(embms_activate_deactivate_tmgi_req_msg_v01 req, std::shared_ptr<Message> msg)
{
    ...
    if (!matchedCalls.empty()) {
      ...
      if (callInfo.embmsInfo != nullptr) {
        callInfo.embmsInfo->setDsiHandle(callInfo.dsiHandle);
        callInfo.embmsInfo->activateDeactivateTmgi(req, msg);
      }
    }
}

void CallManager::handleEmbmsContentDescUpdate(embms_update_content_desc_req_msg_v01 req, std::shared_ptr<Message> msg)
{
    ...
    if (!matchedCalls.empty()) {
      ...
      if (callInfo.embmsInfo != nullptr) {
        callInfo.embmsInfo->setDsiHandle(callInfo.dsiHandle);
        callInfo.embmsInfo->contentDescUpdate(req, msg);
      }
    }
}

void CallManager::handleEmbmsGetActiveTmgi(embms_get_active_tmgi_req_msg_v01 req, std::shared_ptr<Message> msg)
{
    ...
    if (!matchedCalls.empty()) {
      ...
      if (callInfo.embmsInfo != nullptr) {
        callInfo.embmsInfo->setDsiHandle(callInfo.dsiHandle);
        callInfo.embmsInfo->getActiveTmgi(req, msg);
      }
    }
}

void CallManager::handleEmbmsGetAvailableTmgi(embms_get_available_tmgi_req_msg_v01 req, std::shared_ptr<Message> msg)
{
    ...
    if (!matchedCalls.empty()) {
      ...
      if (callInfo.embmsInfo != nullptr) {
        callInfo.embmsInfo->setDsiHandle(callInfo.dsiHandle);
        callInfo.embmsInfo->getAvailableTmgi(req, msg);
      }
    }
}

void CallManager::handleEmbmsSendInterestedList(embms_get_interested_tmgi_list_resp_msg_v01 req, std::shared_ptr<Message> msg)
{
    ...
    if (!matchedCalls.empty()) {
      ...
      if (callInfo.embmsInfo != nullptr) {
        callInfo.embmsInfo->setDsiHandle(callInfo.dsiHandle);
        callInfo.embmsInfo->sendInterestedList(req, msg);
      }
    }
}
