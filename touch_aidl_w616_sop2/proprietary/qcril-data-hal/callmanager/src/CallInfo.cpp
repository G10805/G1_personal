...
unordered_map<int, DataCallFailCause_t> CallEndReason::internalReasonMap = {
  ...
  {WDS_VCER_INTERNAL_URSP_UPDATE_V01, DataCallFailCause_t::SLICE_REJECTED},
};
...
unordered_map<dsi_ce_reason_type_t, unordered_map<int, DataCallFailCause_t>> CallEndReason::callEndReasonTypeMap = {
  ...
  {DSI_CE_TYPE_HANDOFF, handoffReasonMap},
#endif
...
void CallInfo::dump(string padding, ostream& os) const {
  ...
  os << padding << "consolidated active: " << (int)consolidatedActive << endl;
  ...
  for (auto pduInfo: pduInfoList) {
    pduInfo.dump(padding, os);
  }
  os << padding << "ipType: " << ipType << endl;
  os << padding << "mtu: " << mtu << endl;
  os << padding << "consolidatedMtuV4: " << consolidatedMtuV4 << endl;
  os << padding << "consolidatedMtuV6: " << consolidatedMtuV6 << endl;
...
string CallInfo::convertToLegacyLinkAddresses() const {
  string tmpString = "";
  if (pduInfoList.size() > 0) {
    if (pduInfoList.front().v4Info && (pduInfoList.front().v4Info->addrMap.size() > 0)) {
      Log::getInstance().d("v4 addrmap size " + std::to_string(pduInfoList.front().v4Info->addrMap.size()));
      for (auto const& addr: pduInfoList.front().v4Info->addrMap) {
        Log::getInstance().d("v4 addrtype " + std::to_string((int)addr.first));
        Log::getInstance().d("v4 addr value " + addr.second.ipAddress);
      }
      std::map<AddrType,AddrInfo> tmpAddrMap = pduInfoList.front().v4Info->addrMap;
      tmpString = tmpAddrMap[AddrType::_eInvalid].ipAddress;
      tmpString += " ";
    }
    if (pduInfoList.front().v6Info) {
      for (auto const& addr: pduInfoList.front().v6Info->addrMap) {
        Log::getInstance().d("v6 addrtype " + std::to_string((int)addr.first));
        if (addr.first == AddrType::_eDefault) {
          tmpString += addr.second.ipAddress;
          break;
        }
      }
    }
...
vector<LinkAddress_t> CallInfo::convertAddrStringToLinkAddresses() const {
  vector<LinkAddress_t> linkAddressesVector;
  for(auto pduInfo:pduInfoList) {
    if (pduInfo.v4Info && (pduInfo.v4Info->addrMap.size() > 0)) {
      //There should only be 1 V4 address
      LinkAddress_t v4LinkAddress;
      v4LinkAddress.address = pduInfo.v4Info->addrMap[AddrType::_eInvalid].ipAddress;
      v4LinkAddress.deprecationTime = pduInfo.v4Info->addrMap[AddrType::_eInvalid].deprecationTime;
      v4LinkAddress.expirationTime = pduInfo.v4Info->addrMap[AddrType::_eInvalid].expirationTime;
      if (pduInfo.v4Info->addrMap[AddrType::_eInvalid].isDeprecated) {
        v4LinkAddress.properties = AddressProperty_t::DEPRECATED;
      }
      linkAddressesVector.push_back(v4LinkAddress);
    }
    if (pduInfo.v6Info) {
      for (auto const& addr: pduInfo.v6Info->addrMap) {
        if ((addr.first == AddrType::_eDefault) || (addr.first == AddrType::_eCandidate)) {
          LinkAddress_t v6LinkAddress;
          v6LinkAddress.address = addr.second.ipAddress;
          v6LinkAddress.deprecationTime = addr.second.deprecationTime;
          v6LinkAddress.expirationTime = addr.second.expirationTime;
          if (addr.second.isDeprecated) {
            v6LinkAddress.properties = AddressProperty_t::DEPRECATED;
          }
          linkAddressesVector.push_back(v6LinkAddress);
        }
      }
    }
  }
  Log::getInstance().d("linkaddress vector size " + std::to_string(linkAddressesVector.size()));
  return linkAddressesVector;
}

string CallInfo::convertToResultDnsAddresses() const {
  string tmpString = "";
  if (pduInfoList.size() > 0) {
    tmpString = pduInfoList.front().concatDnsAddress();
    if (pduInfoList.size() > 1) {
      tmpString += pduInfoList.back().concatDnsAddress();
    }
  }
  return tmpString;
}

void CallInfo::calculateConsolidatedMtu() {
  int32_t calcMtuV4 = 0;
  int32_t calcMtuV6 = 0;
  if (pduInfoList.size() == 1) {
    calcMtuV4 = pduInfoList.front().v4Info ? pduInfoList.front().v4Info->mtu : 0;
    calcMtuV6 = pduInfoList.front().v6Info ? pduInfoList.front().v6Info->mtu : 0;
  }
  else if (pduInfoList.size() == 2){
    int32_t primMtuV4 = 0, secMtuV4 = 0, primMtuV6 = 0, secMtuV6 = 0;
    primMtuV4 = pduInfoList.front().v4Info ? pduInfoList.front().v4Info->mtu : 0;
    secMtuV4 = pduInfoList.back().v4Info ? pduInfoList.back().v4Info->mtu : 0;
    primMtuV6 = pduInfoList.front().v6Info ? pduInfoList.front().v6Info->mtu : 0;
    secMtuV6 = pduInfoList.back().v6Info ? pduInfoList.back().v6Info->mtu : 0;
    if (primMtuV4 == 0) {
      calcMtuV4 = secMtuV4;
    }
    else if (secMtuV4 == 0) {
      ...
      calcMtuV4 = std::min(primMtuV4, secMtuV4);
    }
    if (primMtuV6 == 0) {
      calcMtuV6 = secMtuV6;
    }
      ...
      calcMtuV6 = std::min(primMtuV6, secMtuV6);
    }
  }
  else {
    Log::getInstance().d("Invalid PDU list length " + std::to_string(pduInfoList.size()));
    return;
  }
  // Interface or v6 MTU can never be below MIN_V6_MTU.
  if ((calcMtuV4 < MIN_V6_MTU && calcMtuV6 == 0) ||
      (calcMtuV6 < MIN_V6_MTU && calcMtuV6 != 0)) {
    calcMtuV6 = MIN_V6_MTU;
  }

  if ((consolidatedMtuV4 != calcMtuV4) || (consolidatedMtuV6 != calcMtuV6)) {
    callParamsChanged = true;
    consolidatedMtuV4 = calcMtuV4;
    // Only set the V6 MTU if it has not been set before.
    if (consolidatedMtuV6 == 0) {
      consolidatedMtuV6 = calcMtuV6;
    }
  }
  Log::getInstance().d("Consolidated v4 mtu:" + std::to_string(consolidatedMtuV4) +
                       " Consolidated v6 mtu:" + std::to_string(consolidatedMtuV6));
}

void CallInfo::calculateConsolidatedActive() {
  if (pduInfoList.size() == 1) {
    consolidatedActive = pduInfoList.front().active;
  }
  else if (pduInfoList.size() == 2) {
    if ((pduInfoList.front().active == LinkActiveState::_eActivePhysicalLinkUp) ||
        (pduInfoList.back().active == LinkActiveState::_eActivePhysicalLinkUp)) {
      consolidatedActive = LinkActiveState::_eActivePhysicalLinkUp;
    }
    else {
      consolidatedActive = LinkActiveState::_eActivePhysicalLinkDown;
    }
  }
  else {
    Log::getInstance().d("Invalid PDU list length " + std::to_string(pduInfoList.size()));
   ...
   Log::getInstance().d("Consolidated active:" + std::to_string(consolidatedActive));
}
