...
unsigned int dsiGetAddresses(dsi_hndl_t dsiHandle, bool &v4Connected, bool &v6Connected,
  ...
  if (addr_info != nullptr) {
    ...
    for (unsigned int i=0 ; i<count ; i++) {
      ...
      IpFamilyType_t currIpType = IpFamilyType_t::IPv4;
 ...
 * @return [description]
 */
int dsiGetPcscfAddresses(dsi_hndl_t dsiHandle, std::string &pcscfAddrV4,
...
int dsiInit(bool fromSSR, void (* dsi_init_cb_func)( void * ))
{
  ...
  rilInstanceId = global_instance_id;
  subsId = global_subs_id;
  Log::getInstance().d("[DsiWrapper]:: instanceId =" + std::to_string(rilInstanceId) + " subs_id =" + std::to_string(subsId));
...
int dsiSetSSCCapability() {
  Log::getInstance().d("SSC Mode 3 unsupported for LE");
  return DSI_SUCCESS;
}

int dsiGetCurrentDataBearerTech(dsi_hndl_t dsiHandle, HandoffNetworkType_t &network)
{
