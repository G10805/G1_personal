...
unsigned int dsiGetAddresses(dsi_hndl_t DsiHandle, bool &v4Connected, bool &v6Connected,
    rildata::PduInfo &primPduInfo, rildata::PduInfo &secPduInfo);
 ...
 * @return [description]
 */
void dsiGetMtuByFamily(dsi_hndl_t dsiHandle, dsi_v4_v6_mtu_t &v4_v6_mtu, uint32_t pduId);
 ...
 * @param pduId [description]
 * @param ipType [description]
 *
 * @return [description]
 */
int dsiGetPcscfAddresses(dsi_hndl_t DsiHandle, std::string &pcscfAddressesV4,
...
int dsiCheckIfCallExists
(
  dsi_hndl_t DsiHandle, std::string apnName,
  wds_apn_type_mask_v01 apnType, int ipType,
  std::string &deviceName
);

/**
 * @brief Routine to set SSC Mode 3 capability
 *
 * @return DSI_SUCCESS if capability set successfully
 * @return DSI_ERROR if error while setting capability
 */

int dsiSetSSCCapability();

/**
 * @brief Getting current network with data bearer
 *
 * @return DSI_SUCCESS if network is determined successfully
 * @return DSI_ERROR if error while getting data bearer
 */

int dsiGetCurrentDataBearerTech(dsi_hndl_t dsiHandle, rildata::HandoffNetworkType_t &network);
#endif
