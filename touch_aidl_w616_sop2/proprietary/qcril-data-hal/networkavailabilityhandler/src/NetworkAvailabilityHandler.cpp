...
void NetworkAvailabilityHandler::processQmiDsdSystemStatusInd(dsd_apn_avail_sys_info_type_v01 * apn_sys, uint32_t len)
{
...
void NetworkAvailabilityHandler::processQmiDsdPreferredApnUpdated(std::shared_ptr<PreferredApnUpdatedMessage> msg) {
  Log::getInstance().d("[NetworkAvailabilityHandler]: process preferred APN updated");
  logBuffer.addLogWithTimestamp("process preferred APN updated");

  dsd_preferred_apn_updated_ind_msg_v01 ind = msg->getParams();
  //Validate apnType first
  auto radioApnType = convertToRadioApnType(ind.apn_type_mask);
  if (mLastReportedNetworkAvailability.find(radioApnType) == mLastReportedNetworkAvailability.end()) {
    Log::getInstance().d("[NetworkAvailabilityHandler] Cannot find entry in cache for APN type " +
                         convertToString(static_cast<ApnTypes_t>(radioApnType)));
    return;
  }

  //If an error has been returned for a previous request, there isnt much to do
  if (ind.err_code_valid == true && ind.err_code != DSD_PREF_APN_UPDATE_NO_ERR_V01) {
    Log::getInstance().d("[NetworkAvailabilityHandler] Error = " +
       std::to_string(ind.err_code) + " returned for APN type " +
       convertToString(static_cast<ApnTypes_t>(radioApnType)) +
       " and tx_id " + std::to_string(ind.transaction_id));
    if (mLastReportedNetworkAvailability[radioApnType].tx_id == ind.transaction_id) {
      mLastReportedNetworkAvailability[radioApnType].tx_id = 0xFFFF;
    }
    return;
  }

  //Validate apn name returned
  std::string apnLower(" ");
  if (ind.apn_name_valid) {
    // convert apn to lower case for case insensitive matching
    apnLower = ind.apn_name;
  ...
  if (mNetworkAvailabilityCache.find(apnLower) == mNetworkAvailabilityCache.end()) {
    Log::getInstance().d("[NetworkAvailabilityHandler] Cannot find apn name = " +
                         apnLower);
    if (mLastReportedNetworkAvailability[radioApnType].tx_id == ind.transaction_id) {
      mLastReportedNetworkAvailability[radioApnType].tx_id = 0xFFFF;
    }
  ...
  if (ind.transaction_id != mLastReportedNetworkAvailability[radioApnType].tx_id) {
    Log::getInstance().d("[NetworkAvailabilityHandler] Modem tx id = " +
          std::to_string(ind.transaction_id) + " does not match cache " +
          std::to_string(mLastReportedNetworkAvailability[radioApnType].tx_id));
    return;
  }
  std::vector<QualifiedNetwork_t> qualifiedNetworks;
  AccessNetwork_t newran = mNetworkAvailabilityCache[apnLower].networks.size() > 0 ?
      static_cast<AccessNetwork_t>(mNetworkAvailabilityCache[apnLower].networks.front())
      : AccessNetwork_t::UNKNOWN;
  vector<int32_t> oldNetworks = mLastReportedNetworkAvailability[radioApnType].networks;
  ...
  if (newran != oldran) {
    ...
    qualifiedNetworks.push_back({radioApnType, mNetworkAvailabilityCache[apnLower].networks});
  }
  else {
    Log::getInstance().d("[NetworkAvailabilityHandler] No change in pref RAT for APN type " +
              convertToString(static_cast<ApnTypes_t>(radioApnType)));
  }
  //Update cache
  mLastReportedNetworkAvailability[radioApnType].networks = mNetworkAvailabilityCache[apnLower].networks;
  mLastReportedNetworkAvailability[radioApnType].tx_id = 0xFFFF;
...
void NetworkAvailabilityHandler::getQualifiedNetworks(vector<QualifiedNetwork_t> &qualifiedNetworks)
{
  ...
  for (auto it = mLastReportedNetworkAvailability.begin(); it != mLastReportedNetworkAvailability.end(); it++) {
    qualifiedNetworks.push_back({it->first, it->second.networks});
  }
...
void NetworkAvailabilityHandler::dump(string padding, std::ostream& os)
{
  ...
  for (auto it = mLastReportedNetworkAvailability.begin(); it != mLastReportedNetworkAvailability.end(); it++) {
    string prefNw;
    if (!it->second.networks.empty()) {
      prefNw = convertToString(static_cast<AccessNetwork_t>(it->second.networks.front()));
    }
    else {
      prefNw = "NULL";
    }
    os << padding << "apnType=" << convertToString(static_cast<ApnTypes_t>(it->first))
    << " prefNw=" << prefNw << " tx id=" << std::to_string(it->second.tx_id) << endl;
  }

}

vector<int32_t> NetworkAvailabilityHandler::convertResultList(vector<QualifiedNetwork_t> &resultList,
                                                   vector<QualifiedNetwork_t> &qualifiedNetworks)
{
  vector<int32_t> conflictingApnTypes;
  for (QualifiedNetwork_t &qualNetwork : resultList) {
    ...
    if (qualNetwork.network.empty() ||
      ...
      // Find a matching QualifiedNetwork by apn type and check if the RAT is conflicting
      ...
      if (priorEntry != qualifiedNetworks.end()) {
        if ((((QualifiedNetwork_t)*priorEntry).network.size() > 0) &&
            (((QualifiedNetwork_t)*priorEntry).network.front() != qualNetwork.network.front()) &&
            (!isSameRadioFamily(((QualifiedNetwork_t)*priorEntry).network.front(), qualNetwork.network.front()))) {
           Log::getInstance().d("[NetworkAvailabilityHandler] existing entry with APN type " +
           convertToString(qualApnType) + " has preferred RAT which is conflicting " );
          //Ignore current qualNetwork since the RAT is conflicting with RAT of another
          //entry with the same APN type. We need to send a query to the modem to resolve
          //the conflict
          conflictingApnTypes.push_back(qualNetwork.apnType);
        }
        else {
          Log::getInstance().d("[NetworkAvailabilityHandler] existing entry with APN type " +
           convertToString(qualApnType) + " and same RAT, do nothing");
        }
      }
      else {
        //No prior entry, just add the qualified network
        qualifiedNetworks.push_back(qualNetwork);
      }
    }
  }
  //Remove any extra apnType entry for which there was a conflict. This might happen if
  //there are > 2 APNs with the same type?
  for (auto cap : conflictingApnTypes) {
    auto priorEntry = find_if(qualifiedNetworks.begin(), qualifiedNetworks.end(),
          [cap](const QualifiedNetwork_t &existingNetwork) -> bool {
            return existingNetwork.apnType == cap;
          });
      if (priorEntry != qualifiedNetworks.end()) {
  ...
  while (qualIt != qualifiedNetworks.end()) {
    ...
    if (lastRepIt != mLastReportedNetworkAvailability.end()) {
      vector<int32_t> oldNetworks = mLastReportedNetworkAvailability[qualNetwork.apnType].networks;
      AccessNetwork_t oldran = oldNetworks.size() > 0 ?
        static_cast<AccessNetwork_t>(oldNetworks.front()) : AccessNetwork_t::UNKNOWN;
      ...
      if (newran != oldran) {
          Log::getInstance().d("[NetworkAvailabilityHandler] Pref RAT for APN type " +
           ...
           " changed to " + convertToString(static_cast<AccessNetwork_t>(newran)));
          //Update cache
          mLastReportedNetworkAvailability[qualNetwork.apnType].networks = qualNetwork.network;
          qualIt++;
      }
      else {
      ...
      //Add new entry to last reported cache with tx_id 0xFFFF
      ApnTypeInfo_t apnInfo = {qualNetwork.network, 0xFFFF};
      mLastReportedNetworkAvailability.insert(
         std::pair<int32_t,ApnTypeInfo_t>(qualNetwork.apnType, apnInfo));
      qualIt++;
    }

  }

  return conflictingApnTypes;

}

void NetworkAvailabilityHandler::processConflictingApnTypes(vector<int32_t> conflictingApnTypes) {
  //reset any pending transaction ids since we are about to send out new requests
  for (auto apnTypeEntry: mLastReportedNetworkAvailability) {
    apnTypeEntry.second.tx_id = 0xFFFF;
  }
  for (auto cat : conflictingApnTypes) {
    dsd_get_preferred_apn_req_msg_v01 req;
    dsd_get_preferred_apn_resp_msg_v01 resp;
    uint32_t apnNameCnt = 0;

    memset(&req, 0, sizeof(req));
    memset(&resp, 0, sizeof(resp));

    for (auto networkAvailabilityEntry : mNetworkAvailabilityCache) {
      auto it = find(networkAvailabilityEntry.second.apnTypes.begin(),
                     networkAvailabilityEntry.second.apnTypes.end(),
                     cat);
      if (it != networkAvailabilityEntry.second.apnTypes.end()) {
        if(networkAvailabilityEntry.second.networks.empty())
        {
          Log::getInstance().d("[NetworkAvailabilityHandler] : networkList is empty for" + networkAvailabilityEntry.first);
          continue;  //Skipping this iteration
        }
        else
        {
          int isEntryFound = false;
          int currentNetwork = networkAvailabilityEntry.second.networks.front();
          if(currentNetwork == (int)(AccessNetwork_t::UNKNOWN))
          {
            Log::getInstance().d("[NetworkAvailabilityHandler] : Skip adding UNKNOWN networkList ");
            continue;
          }

          if(apnNameCnt > 0)
          {
            for(int count= 0; count < apnNameCnt; count++)
            {
              //checking prior entries inserted into reqMsg
              string priorApn = req.apn_name_list[count].apn_name;
              if(mNetworkAvailabilityCache.find(priorApn) != mNetworkAvailabilityCache.end())
              {//There should always be an entry with priorApn in mNetworkAvailabilityCache

                //This vector cant be empty as empty networkList entries are discarded from the earlier check
                if(isSameRadioFamily(mNetworkAvailabilityCache[priorApn].networks.front(), currentNetwork)) {
                  Log::getInstance().d("[NetworkAvailabilityHandler]: Entry is Already present with same RAT and apnName:  "+
                                          priorApn);
                  isEntryFound = true;
                  break;
                }
          ...
          if(isEntryFound)
            continue;  //break statement is executed here. So skip copying this entry
        }
        strlcpy(req.apn_name_list[apnNameCnt].apn_name, networkAvailabilityEntry.first.c_str(),
                sizeof(req.apn_name_list[apnNameCnt].apn_name));
        apnNameCnt++;
      }
    }

    req.apn_name_list_len = apnNameCnt;
    req.apn_type_mask = convertToDsdApnType(cat);

    if (dsd_endpoint != nullptr) {
      int rc = dsd_endpoint->sendRawSync(
         QMI_DSD_GET_PREFERRED_APN_REQ_V01,
         &req, sizeof(req),
         &resp, sizeof(resp));
      if ((rc == QMI_NO_ERR) &&
          (resp.resp.result == QMI_RESULT_SUCCESS_V01)){
        mLastReportedNetworkAvailability[cat].tx_id = resp.transaction_id;
      }
      else {
         Log::getInstance().d("Failed to get preferred APN rc=" + std::to_string(rc)
...
void NetworkAvailabilityHandler::clearNetworkAvailabilityCache()
{
...
void NetworkAvailabilityHandler::updateNetworkAvailabilityCache(std::shared_ptr<Message> msg) {
  ...
  if (m != nullptr) {
    ...
    std::unordered_map<std::string, std::vector<int32_t>> setDataprofileInfo;

    //create a map with apnName and apnType received with setdataprofile
    for (auto profile : profiles) {
      ...
      if(setDataprofileInfo.find(apnLower) != setDataprofileInfo.end())
      {
        Log::getInstance().d("Duplicate entry in setdataProfile request for apn " + apnLower);
        std::vector<int32_t> consolidatedApnTypes = setDataprofileInfo.at(apnLower);
        consolidatedApnTypes.insert(consolidatedApnTypes.end(), apnTypes.begin(), apnTypes.end());
        std::unordered_set<int32_t> set(consolidatedApnTypes.begin(), consolidatedApnTypes.end());
        std::vector<int32_t> type(set.begin(), set.end());
        setDataprofileInfo[apnLower] = type;
      }
      else
      {
        setDataprofileInfo[apnLower] = apnTypes;
      }
    }

    //Insert new apnName entry received with setDataProfile to this cache
    //update mNetworkAvailabilityCache if there is any change in apnType of existing apnname
    for(auto profile: setDataprofileInfo) {
      std::vector<int32_t> apnTypes((profile.second).begin(), (profile.second).end());
      ConsolidatedNetwork_t cacheEntry =  {false, apnTypes, vector<int32_t>()};
      if(mNetworkAvailabilityCache.find(profile.first) != mNetworkAvailabilityCache.end())
      {
        Log::getInstance().d("Entry already exist in cache for apn " + profile.first);
        ConsolidatedNetwork_t consolidateNet = mNetworkAvailabilityCache.at(profile.first);
        cacheEntry.networks = consolidateNet.networks;
      }
      mNetworkAvailabilityCache[profile.first] = cacheEntry;
      ...
      logBuffer.addLogWithTimestamp("Added new apn=" + profile.first + " types=" + typeSs.str());
      Log::getInstance().d("Added new apn=" + profile.first + "types=" +typeSs.str());
    }

    //removing extra apn which is not received with setDataProfile but present in cache
    for(auto it = mNetworkAvailabilityCache.begin(); it != mNetworkAvailabilityCache.end();) {
      if(setDataprofileInfo.find(it->first) == setDataprofileInfo.end())
      {
        Log::getInstance().d("Removing entry with apn " + it->first);
        it = mNetworkAvailabilityCache.erase(it);
      }
      else
      {
        it++;
...
void NetworkAvailabilityHandler::updateNetworkAvailabilityCache(dsd_apn_avail_sys_info_type_v01 * apn_sys, uint32_t len)
{
  ...
  for (uint32_t i = 0; i < len; i++) {
    ...
    if (mNetworkAvailabilityCache.find(apnLower) != mNetworkAvailabilityCache.end()) {
      ...
      std::vector<int32_t> networksList = convertToRadioAccessNetworkList(apn_sys[i]);
      std::vector<int32_t> radioApnTypes = convertToRadioApnTypeList(getApnTypesForName(apnLower));
  ...
  std::vector<QualifiedNetwork_t> qualifiedNetworks;
  vector<int32_t> conflictingApnTypes = convertResultList(resultList, qualifiedNetworks);
  processConflictingApnTypes(conflictingApnTypes);
  for (auto qn : qualifiedNetworks) {
...
void NetworkAvailabilityHandler::updateNetworkAvailabilityCache(dsd_apn_pref_sys_type_ex_v01 * apn_pref_sys, uint32_t len)
{
  vector<QualifiedNetwork_t> resultList;
...
void NetworkAvailabilityHandler::processHandoverfailureForApnType(ApnTypes_t  type, string apn)
{
  Log::getInstance().d("[NetworkAvailabilityHandler]: process handoverFailure for apnType " + std::to_string((int)type));
  std::vector<int32_t> apnTypes = convertToRadioApnTypeList(type);
  for(auto it = apnTypes.begin();it!= apnTypes.end(); it++)
  {
    //Assigning UNKNOWN to all the apnType for which Handover is failed
    //This ensures that qualifiedNetwork is sent with next dsd_system_status.
    vector<int32_t> networks;
    networks.push_back((int32_t)AccessNetwork_t::UNKNOWN);
    auto lastRepIt = mLastReportedNetworkAvailability.find(*it);
    if (lastRepIt != mLastReportedNetworkAvailability.end()) {
      mLastReportedNetworkAvailability[*it].networks = networks;
    }
  }

  //Update the mNetworkAvailabilityCache
  mNetworkAvailabilityCache[apn].hasPendingIntentToChange = false;
...
bool NetworkAvailabilityHandler::isRadioRAT(int network) {
  if(network != (int)AccessNetwork_t::UNKNOWN  && network != (int)AccessNetwork_t::IWLAN)
      return true;
  else
      return false;
}

bool NetworkAvailabilityHandler::isWlanRAT(int network) {
  if(network != (int)AccessNetwork_t::UNKNOWN && network == (int)AccessNetwork_t::IWLAN)
      return true;
  else
      return false;
...
bool NetworkAvailabilityHandler::isSameRadioFamily(int priorNetwork, int newNetwork) {
    if((isWlanRAT(priorNetwork) && isWlanRAT(newNetwork)) ||
       (isRadioRAT(priorNetwork) && isRadioRAT(newNetwork)))
       return true;
    else
      return false;
}
...
vector<int32_t> NetworkAvailabilityHandler::convertToRadioAccessNetworkList(dsd_apn_avail_sys_info_type_v01 apn_sys) {
  vector<int32_t> networks;
  for (uint32_t j=0; j < apn_sys.apn_avail_sys_len; j++)
  {
...
int32_t NetworkAvailabilityHandler::convertToRadioApnType(uint64_t qmiApnType)
{
  ...
  switch (qmiApnType)
  {
     ...
     case QMI_DSD_APN_TYPE_MASK_HIPRI_V01:
       //There is a bug in telephony code.They consider default apnType = default|Hipri
      //So RIL also maps default apnType with value 17(default|Hipri)
       result = (int32_t)ApnTypes_t::DEFAULT | (int32_t)ApnTypes_t::HIPRI;
       break;
     ...
     case QMI_DSD_APN_TYPE_MASK_EMERGENCY_V01:
       ...
       break;
     case QMI_DSD_APN_TYPE_MASK_UT_V01:
       result = (int32_t)ApnTypes_t::XCAP;
       break;
     case QMI_DSD_APN_TYPE_MASK_MCX_V01:
       result = (int32_t)ApnTypes_t::MCX;
       break;
     default:
...
uint64_t NetworkAvailabilityHandler::convertToDsdApnType(int32_t radioApnType)
{
  uint64_t result;
  switch (radioApnType)
  {
     case (int32_t)ApnTypes_t::DEFAULT:
       result = (uint64_t)QMI_DSD_APN_TYPE_MASK_DEFAULT_V01;
       break;
     case (int32_t)ApnTypes_t::IMS:
       result = (uint64_t)QMI_DSD_APN_TYPE_MASK_IMS_V01;
       break;
     case (int32_t)ApnTypes_t::MMS:
       result = (uint64_t)QMI_DSD_APN_TYPE_MASK_MMS_V01;
       break;
     case (int32_t)ApnTypes_t::DUN:
       result = (uint64_t)QMI_DSD_APN_TYPE_MASK_DUN_V01;
       break;
     case (int32_t)ApnTypes_t::SUPL:
       result = (uint64_t)QMI_DSD_APN_TYPE_MASK_SUPL_V01;
       break;
     case (int32_t)ApnTypes_t::HIPRI:
       result = (uint64_t)QMI_DSD_APN_TYPE_MASK_HIPRI_V01;
       break;
     case (int32_t)ApnTypes_t::FOTA:
       result = (uint64_t)QMI_DSD_APN_TYPE_MASK_FOTA_V01;
       break;
     case (int32_t)ApnTypes_t::CBS:
       result = (uint64_t)QMI_DSD_APN_TYPE_MASK_CBS_V01;
       break;
     case (int32_t)ApnTypes_t::IA:
       result = (uint64_t)QMI_DSD_APN_TYPE_MASK_IA_V01;
       break;
     case (int32_t)ApnTypes_t::EMERGENCY:
       result = (uint64_t)QMI_DSD_APN_TYPE_MASK_EMERGENCY_V01;
       break;
     case (int32_t)ApnTypes_t::XCAP:
       result = (uint64_t)QMI_DSD_APN_TYPE_MASK_UT_V01;
       break;
     case (int32_t)ApnTypes_t::MCX:
       result = (uint64_t)QMI_DSD_APN_TYPE_MASK_MCX_V01;
       break;
     default:
       result = (int32_t)ApnTypes_t::NONE;
  }
...
vector<int32_t> NetworkAvailabilityHandler::convertToRadioApnTypeList(const ApnTypes_t &apnTypes) {
  ...
  for (int32_t mask = 1;
               mask <= static_cast<int32_t>(ApnTypes_t::XCAP); mask <<= 1) {
