            ...
            REG_MSG(WDSModemEndPoint_ENDPOINT_STATUS_IND))
{
  ...
  mMessageHandler = {
      ...
      HANDLER(SetCapabilitiesMessage,
              WDSModemEndPointModule::handleSetCapabilities),
...
void WDSModemEndPointModule::handleSetCapabilities(std::shared_ptr<Message> m)
{
  Log::getInstance().d("[WDSModemEndPointModule]: Handling msg = " + m->dump());
  ...
  if (msg != nullptr && mQmiSvcClient != nullptr)
  {
    ...
    if (!cap_req.pdn_throttle_cap_valid) {
      Log::getInstance().d("[WDSModemEndPointModule] No WDS capabilities");
      return;
    }

    int rc = qmi_client_send_msg_sync(mQmiSvcClient,
                        QMI_WDS_SET_CAPABILITIES_REQ_V01,
                        (void *)&cap_req, sizeof(cap_req),
                        (void *)&cap_resp, sizeof(cap_resp),
                        QCRIL_DATA_QMI_TIMEOUT);

    if ((rc != QMI_NO_ERR) || (cap_resp.resp.result == QMI_RESULT_FAILURE_V01 )) {
      Log::getInstance().d("[WDSModemEndPointModule] Failed to send QMI_WDS_SET_CAPABILITIES_REQ_V01");
      return;
    }
    if (cap_resp.pdn_throttle_cap_valid &&
        cap_resp.pdn_throttle_cap == WDS_CAPABILITY_PDN_THROTTLE_V2_V01) {
      usingPdnThrottleV2 = true;
    } else {
      usingPdnThrottleV2 = false;
    }
    stringstream ss;
    ss << "[WDSModemEndPointModule] QMI_WDS_SET_CAPABILITIES_REQ_V01::Successfully sent. " << std::boolalpha <<
      "pdnThrottleV2=" << usingPdnThrottleV2;
  ...
  if (msg != nullptr) {
    ...
    if (usingPdnThrottleV2) {
      if (resp_msg.tx_id_valid) {
        ...
        Log::getInstance().d("[WDSModemEndPointModule::handleGetPdnThrottleTime] invalid transaction id");
        msg->sendResponse(msg, Message::Callback::Status::FAILURE, nullptr);
      }
    } else {
      ...
      if (resp_msg.throttle_info_valid) {
        ...
        Log::getInstance().d("[WDSModemEndPointModule::handleGetPdnThrottleTime] invalid TLV");
        msg->sendResponse(msg, Message::Callback::Status::FAILURE, nullptr);
      }
    }
  }
    ...
    case SegmentationStatus_t::Success:
    {
      ...
      if (pendingSegmentRequests.find(key) != pendingSegmentRequests.end()) {
        ...
        if (reqMsg != nullptr) {
          ...
          Log::getInstance().d("[WDSModemEndPointModule]: processGetPdnThrottleInfoResultComplete reqMsg is null");
        }
      } else {
  ...
  if (msg != NULL && mQmiSvcClient != nullptr)
  {
    ...
    *capabilityMask = BRING_UP_LEGACY;
    ...
    if ( QMI_NO_ERR != rc ||
      ...
      if (TRUE == ps_resp.is_clat_supported_valid && TRUE == ps_resp.is_clat_supported) {
        global_qcril_clat_supported = true;
      } else {
        global_qcril_clat_supported = false;
      }
    }
    Log::getInstance().d("[WDSModemEndPointModule]: Global Qcril Clat Supported : " + std::to_string(global_qcril_clat_supported));
...
{
  Log::getInstance().d("[WDSModemEndPointModule]: handleRegisterForKeepAliveMessageSync : Handling msg = " + m->dump());
...
void WDSModemEndPointModule::handleGetSlicingConfigInternalMessage(std::shared_ptr<Message> m)
{
  ...
  if(msg != nullptr)
  {
    ...
    {
      Log::getInstance().d("[WDSModemEndPointModule]: GetSlicingConfigRequestMessage is nullptr");
    }
  }
  else
  {
    Log::getInstance().d("[WDSModemEndPointModule]: message is nullptr");
  }
}
