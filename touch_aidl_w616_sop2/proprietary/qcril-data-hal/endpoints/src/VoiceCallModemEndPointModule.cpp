...
DEFINE_MSG_ID_INFO(VoiceSub1EndPoint_QMI_IND)
DEFINE_MSG_ID_INFO(VoiceSub0EndPoint_QMI_IND)
DEFINE_MSG_ID_INFO(VoiceSub0EndPoint_ENDPOINT_STATUS_IND)
DEFINE_MSG_ID_INFO(VoiceSub1EndPoint_ENDPOINT_STATUS_IND)
...
VoiceCallModemEndPointModule::VoiceCallModemEndPointModule(string name,
      ModemEndPoint &owner, SubId subId) : ModemEndPointModule(name, owner,
          subId == SubId::SECONDARY? REG_MSG(VoiceSub1EndPoint_QMI_IND):
              REG_MSG(VoiceSub0EndPoint_QMI_IND), subId == SubId::SECONDARY?
                  REG_MSG(VoiceSub1EndPoint_ENDPOINT_STATUS_IND):
                      REG_MSG(VoiceSub0EndPoint_ENDPOINT_STATUS_IND))
{
  ...
  {
    mMessageHandler = {
        HANDLER_MULTI(QmiIndMessage, VoiceSub1EndPoint_QMI_IND, VoiceCallModemEndPointModule::handleVoiceIndMessage),
    };
  }
  else
  {
    mMessageHandler = {
        HANDLER_MULTI(QmiIndMessage, VoiceSub0EndPoint_QMI_IND, VoiceCallModemEndPointModule::handleVoiceIndMessage),
    };
  }
}
  ...
  switch(msg_id)
  {
    case QMI_VOICE_ALL_CALL_STATUS_IND_V02:
      if(decoded_payload != NULL)
      {
        ...
        {
          uint32_t i;
          std::vector<VoiceIndResp> voiceInfo;
          for(i = 0; i < resp->call_info_len; i++)
          {
            ...
            static_cast<VoiceCallModemEndPoint *>(&mOwner)->setCallState(voiceResp.voiceCallState);

            voiceInfo.push_back(voiceResp);
          }
          if(!voiceInfo.empty())
          {
            auto msg = std::make_shared<rildata::VoiceIndMessage>(voiceInfo);
            msg->broadcast();
          }
        }
      }
