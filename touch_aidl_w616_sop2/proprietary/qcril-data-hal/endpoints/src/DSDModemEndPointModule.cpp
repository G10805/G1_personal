...
#if (!defined(RIL_FOR_LOW_RAM) || defined(RIL_FOR_MDM_LE))
      HANDLER_MULTI(EndpointStatusIndMessage, DSDModemEndPoint_ENDPOINT_STATUS_IND, DSDModemEndPointModule::handleEndPointStatusIndMessage),
...
void DSDModemEndPointModule::handleChangeStackIdDsdSyncMessage(std::shared_ptr<Message> msg) {
    ...
    if (successBit) {
        ...
        Log::getInstance().d("[DSDModemEndPointModule]: Stack Id Changed Successfully");
        return;
    }
...
void DSDModemEndPointModule::processDdsRecommendationInd(dsd_dds_recommendation_ind_v01 *ind_data)
{
  ...
  if(subId == SubscriptionId_t::INVALID_SUBSCRIPTION_ID) {
    return;
  }

  DdsSwitchType_t switchType = DdsSwitchType_t::Permanent;
  if (ind_data->switch_type_valid) {
    if (ind_data->switch_type == DSD_DDS_SWITCH_PERMANENT_V01 &&
        ind_data->source != DSD_DDS_RECOMMEND_SOURCE_UNSPECIFIED_V01) {
    ...
    auto msg = std::make_shared<DdsSwitchRecommendInternalMessage>(nullptr);
    msg->setParams(subId, switchType, level);
    ...
    ss << "[NetworkServiceHandler::processIWlanRegStatusInd] status = " << (int) apiStatus;
...
{
  Log::getInstance().d("[DSDModemEndPointModule]: processDsdCIWlanCapabilityInd="+std::to_string(ind_data->ciwlan_capability));
  ...
  if (msg != nullptr) {
    ...
    do {
      ...
      if ((rc != QMI_NO_ERR) || (resp_msg.resp.result == QMI_RESULT_FAILURE_V01 )) {
        ...
        Log::getInstance().d("[DSDModemEndPointModule] QMI_DSD_SET_UI_ICON_CONFIG_REQ_V01::Successfully sent");
      }
    } while(0);
    std::shared_ptr<RIL_Errno> result = std::make_shared<RIL_Errno>(retVal);
    msg->sendResponse(msg, Message::Callback::Status::SUCCESS, result);
