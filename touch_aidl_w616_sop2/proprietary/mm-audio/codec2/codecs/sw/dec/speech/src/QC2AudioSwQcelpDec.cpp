...
#undef LOG_TAG
#define LOG_TAG "QC2AudioSwQcelpDec"
#define QC2_TRACE_DOMAIN QC2_TRACE_DOMAIN_CODEC

namespace qc2audio {

static char const * QCELP_DECODER_LIB = "libQcelp13SwCodec.so";

///////////////////////////////////////////////////////////////////////////////////////////////
//                              QC2AudioSwQcelpDec                                                 //
///////////////////////////////////////////////////////////////////////////////////////////////

QC2AudioSwQcelpDec::QC2AudioSwQcelpDec(
        ...
        memset(&ttyStruct, 0, sizeof(TTYDecStruct));
}

QC2Status QC2AudioSwQcelpDec::onStop() {
    return QC2AudioSwSpeechCommonDec::onStop();
}

QC2Status QC2AudioSwQcelpDec::onInit() {
    QLOGD_INST("%s", __func__);

    // Initialize QCELP specific CSIM structure with defaults
    ...
    mLibHandle = dlopen(QCELP_DECODER_LIB, RTLD_LAZY);
    if (!mLibHandle) {
        QLOGE("%s: CSIM decoder library open failed!", __func__);
        return QC2_ERROR;
    }
    mDecoderInit = (DecoderInit) dlsym (mLibHandle, "v13k_init_dec13");
    mGetDecoderConfigSize =
        (DecoderConfigSize) dlsym (mLibHandle, "v13k_get_dec_struct_size");
    mProcessData = (DecoderLib_Process) dlsym (mLibHandle, "v13k_decoder13");
...
QC2Status QC2AudioSwQcelpDec::onStart() {
    ttyStruct.tty_option = 0;
...
QC2Status QC2AudioSwQcelpDec::process(const std::shared_ptr<QC2Buffer>& input,
            ...
            if (bytesTranscoded == QC2SpeechCodecConfig::getMaxOutputBufferSize()) {
                for (int transIndex = 0; transIndex < transcodeFrameCount; transIndex++) {
                    ...
                    (*mProcessData)(static_cast<void *>(mDecStructInst->data()),
                        ...
                        (int16_t*)(outputBasePtr + transIndex * 160), &ttyStruct);
                }
