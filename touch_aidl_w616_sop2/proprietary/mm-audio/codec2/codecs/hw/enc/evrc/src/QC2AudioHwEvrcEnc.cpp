...
#undef LOG_TAG
#define LOG_TAG "QC2AudioHwEvrcEnc"
...
QC2AudioHwEvrcEnc::QC2AudioHwEvrcEnc(
    ...
    mEncoderConfig = std::make_shared<QC2EvrcEncConfig>();
}

QC2Status QC2AudioHwEvrcEnc::configure(const C2Param& param) {
        ...
        case kParamIndexAudioMaxVocoderRate: {
            ...
            if (param.index() == in.index()) {
                ...
                if (!mEncoderConfig->isVocoderRateValid(maxRate)) {
                    QLOGE("Invalid min rate %d", maxRate);
                    retVal = QC2_BAD_VALUE;
                    break;
                }
...
QC2Status QC2AudioHwEvrcEnc::updateConfigParams() {
...
QC2Status QC2AudioHwEvrcEnc::onStart() {
    ...
    QC2EvrcEncConfig::EvrcEncConfig encConfig = {
    ...
    mOutputMediaFmt = {.sample_rate = mEncoderConfig->getSampleRate(),
                        .bits_per_sample = mEncoderConfig->getBitDepth(),
                        .channel_count = mEncoderConfig->getChannels(),
                        .codec_type = Type_EVRC,
                        .codec_info_size = sizeof(QC2EvrcEncConfig::EvrcEncConfig),
                        .codec_info = &encConfig };

    QLOGV("%s: EVRC enc config: sample_rate %u, channel_count %u, minRate %u, "
...
QC2Status QC2AudioHwEvrcEnc::onInit() {
    QC2Status retVal = QC2_OK;
    QLOGD_INST("%s", __func__);

    //set output buffer size
    mOutBufSize = QC2EvrcEncConfig::getMaxOutputBufferSize();
    ...
    mEncoderConfig->setMinRate(EvrcDefaults::EvrcDefaultMinRate);
    mEncoderConfig->setMaxRate(EvrcDefaults::EvrcDefaultMaxRate);
    mEncoderConfig->setRateModulation(EvrcDefaults::EvrcDefaultRateModCmd);
