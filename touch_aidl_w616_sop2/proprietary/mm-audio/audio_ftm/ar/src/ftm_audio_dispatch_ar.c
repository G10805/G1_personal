...
====================================================================================================
Copyright (c) 2011-2013, 2015-2018, 2020-2021, Qualcomm Technologies, Inc.
All Rights Reserved.
                ...
                if (temp) {
                    ...
                    if (val < 0) {
                        ...
                        if (strstr(ctlname, "Volume")) {
                            ...
                            val = (val * volume_range) / MAX_VOLUME;
                        }
                        printf("\n Set %s %d", ctlname, val);
...
static DALBOOL ftm_audio_start_adie_loopback(void);
static DALBOOL ftm_audio_stop_adie_loopback(void);

static DALBOOL ftm_audio_start_tone(struct test_params *params, uint16  lo_freq, uint16  hi_freq);
static DALBOOL ftm_audio_stop_tone(void);

static DALBOOL ftm_audio_start_dsp_loopback(void);
static DALBOOL ftm_audio_stop_dsp_loopback(void);

static DALBOOL ftm_audio_start_ext_loopback(uint16 low_freq, char *filename,
                                                    int16 tone_analysis, int16 freq_tolerance);
static DALBOOL ftm_audio_stop_ext_loopback(void);

static DALBOOL aud_ftm_common_start(void);
static DALBOOL aud_ftm_common_stop(void);

static DALBOOL ftm_audio_start_pcm_play(AUDIO_FTM_PCM_RATE_T  rate, AUDIO_FTM_CHN_T  channel);
static DALBOOL ftm_audio_stop_pcm_play(void);

static DALBOOL ftm_audio_stop_pcm_capture(void);
static DALBOOL ftm_audio_start_file_play(struct test_params *params);
static DALBOOL ftm_audio_stop_file_play(void);

static DALBOOL ftm_audio_checkif_active_session(void)
{
   DALBOOL ret;
     int   instances;
