 ...
 * None.
 *
 * Copyright (c) 2010-2021, 2024 by Qualcomm Technologies, Inc.
 * All Rights Reserved.
 ...
 *
 * PARAMS:
 *   diag_init_status - Status of acph_diag_init.
 *
 * RETURN VALUE : ACPH_SUCCESS when success, ACPH_FAILURE otherwise
 *
 * SIDE EFFECTS : None
 */
int32_t acph_init(int32_t* diag_init_status)
{
...
*/
#ifndef _DIAG_DISABLE
    *diag_init_status = actp_diag_init(acph_execute_command);
#else
    ACDB_DEBUG_LOG("[ACPH]->ACPH actp_diag_init skipped due to diag disable!\n");
    *diag_init_status = DIAG_INIT_SUCCESS;
#endif
    acph_init_count ++;
end:
    ...
    if (result != ACPH_SUCCESS)
    {
       ACDB_DEBUG_LOG("[ACPH]->ACPH online init failed!\n");
    }
    else
    {
       ACDB_DEBUG_LOG("[ACPH]->ACPH online init success\n");
    }

    if (*diag_init_status == DIAG_INIT_SUCCESS)
    {
       ACDB_DEBUG_LOG("[ACPH]->ACPH diag init success\n");
    }
    else
    {
      /* On automotive, diag may fail for the first client (audio chime) since
      it may not be ready yet. It is expected that the next client
      (audio hal) will initialize diag. */
       ACDB_DEBUG_LOG("[ACPH]->ACPH diag init failed! Next client will retry.\n");
    }
                          ...
                          )
{
   ...
   *resp_buf_ptr = acph_main_buffer;
   ...
   {
      ACDB_DEBUG_LOG("[ACPH ERROR]->acph_execute_command->The command id provided do not belong to any service category [%u]\n",(uint32_t)nCommandId);
   ...
   if (g_pCmdTbl != NULL)
   {
      ...
      {
         ACDB_DEBUG_LOG("[ACPH ERROR]->acph_execute_command->Received Invalid commandId[%08X], result[%d]\n",
                        nCommandId, result);
      ...
      {
         ACDB_DEBUG_LOG("[ACPH ERROR]->acph_execute_command->Error received while executing commandId[%08X] with result[%d]\n",
                        nCommandId, result);
         acph_create_error_resp(result, req_buf_ptr, *resp_buf_ptr, resp_buf_length);
      }
      else
