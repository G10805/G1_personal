...
Copyright (c) 2010-2018, 2020, 2022, 2024 Qualcomm Technologies, Inc.
All Rights Reserved.
	...
	uint32_t *pDstBytesUsed)
{
	...
	if(pDstBuff == NULL || pDstBytesUsed == NULL)
	{
		ACDB_DEBUG_LOG("Recieved pDstBuff or pDstBytesUsed buffer pointer with NULL value for tblid = %u\n",tblId);
		return ACDB_BADPARM;
	}
	...
	uint8_t *pDstBuff, uint32_t nDstBuffLen,uint32_t *pDstBytesUsed)
{
	...
	if(pDstBuff == NULL || pDstBytesUsed == NULL)
	{
		ACDB_DEBUG_LOG("Recieved pDstBuff or pDstBytesUsed pointer with NULL value for tblid = %u\n",tblId);
		return ACDB_BADPARM;
	}
	...
	uint32_t *pSize)
{
	...
	if(pSize == NULL)
	{
		ACDB_DEBUG_LOG("Recieved pSize pointer with NULL value for tblid = %u\n",tblId);
		return ACDB_BADPARM;
	}
...
int32_t IsOffloadedParameter(uint32_t paramID)
{
    ...
    if (status != ACDB_SUCCESS)
    {
        ...
        return ACDB_OFFLOAD_PARAM_DATA_NOT_FOUND;
    }
...
uint32_t GetOffloadedParamData(uint8_t  **offloadParamData)
{
    int32_t status = ACDB_SUCCESS;
    uint32_t num_entries;
    AcdbGlbalPropInfo prop_info = { 0,{ 0, NULL } };
    ...
    num_entries = READ_UInt32(prop_info.dataInfo.pData);
    *offloadParamData = prop_info.dataInfo.pData;
    return num_entries;
}
    ...
    uint8_t *pDstBuff, uint32_t nDstBuffLen, uint32_t *pDstBuffOffset)
{
    ...
    if ((header.alignment > 0) && (*pDstBuffOffset + offset) % header.alignment)
    {
	...
	*pDstBuffOffset += header.data_offset;
	offset += header.data_offset;
    if (data_size > 0 && ACDB_UTILITY_INIT_SUCCESS ==
	AcdbReadFile(path, pDstBuff + offset , nDstBuffLen, &data_size))
    {
        *pDstBuffOffset += data_size;
        offset += data_size;
