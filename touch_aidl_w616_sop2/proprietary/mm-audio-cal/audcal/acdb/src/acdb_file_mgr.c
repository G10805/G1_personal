...
Copyright (c) 2010-2018, 2024 Qualcomm Technologies, Inc.
All Rights Reserved.
...
int32_t AcdbDataGetGlobalPropData(AcdbGlbalPropInfo *glbInfo)
{
	...
	if(result != ACDB_SUCCESS && result != ACDB_OFFLOAD_PARAM_DATA_NOT_FOUND)
	{
...
int32_t AcdbDataGetDevicesList(AcdbDevices *pDevs)
{
	...
	for(i=0;i<gDataFileInfo.noOfFiles;i++)
	{
		...
		for(j=0;j<devPropLut.devPropLen;j++)
		{
			...
			{
				ACDB_DEBUG_LOG("ACDBFILE_MGR:Max devices limit of %d reached\n",ACDB_MAX_DEVICES);
				return ACDB_SUCCESS;
			}
