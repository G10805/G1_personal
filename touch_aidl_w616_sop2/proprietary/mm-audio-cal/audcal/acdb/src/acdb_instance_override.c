                     ...
                     while (pCurTop != NULL)
                     {
                        if (pInput->nTblEntriesOffset <= curNoOfEntriesOffset)
                        {
                           if (((pInput->nBuffSize - offset) >= noOfBytesReqPerEntry))
                           {
                              ...
                              ACDB_MEM_CPY(pInput->pBuff + offset, ((uint32_t)(noOfTableIndices - noOfCmdIndices)*sizeof(uint32_t)), pLut, (uint32_t)(noOfTableIndices - noOfCmdIndices)*sizeof(uint32_t));
