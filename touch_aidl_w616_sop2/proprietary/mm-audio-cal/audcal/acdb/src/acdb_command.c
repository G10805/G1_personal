...
Copyright (c) 2010-2020,2024 Qualcomm Technologies, Inc.
All Rights Reserved.
...
int32_t AcdbCmdGetAudProcInfo(uint32_t queryType, uint8_t *pIn, uint8_t *pOut)
{
   ...
   else
   {
	  ...
	  AcdbTableInfo tblInfo = { { 0, NULL }, { 0, NULL }, { 0, NULL }, { 0, NULL } };
...
int32_t AcdbCmdSetAudProcInfo(AcdbAudProcTableCmdType *pInput)
{
   ...
   while (remaining_bufLength > sizeof(AudProcTableEntryHeader))
   {
      ...
      if (remaining_bufLength >= oneEntryHeader.nParamSize)
	  {
		  ...
		  if (result != ACDB_SUCCESS)
		  {
			  ACDB_DEBUG_LOG("[ACDB Command]->[AcdbCmdSetAudProcInfo]->Err:%d Failed to set AudProcData for...\n", result);
...
int32_t CheckOffloadedParameter(uint32_t paramID, uint8_t *offloadParamData, uint32_t num_entries)
{
    uint32_t i = 0;
    if (SEARCH_SUCCESS ==
        AcdbDataBinarySearch(offloadParamData + sizeof(uint32_t),
...
int32_t AcdbCmdGetVocProcStatWithInstanceInfo(uint32_t queryType, uint8_t *pIn, uint8_t *pOut)
{
   ...
   else
   {
      ...
      int temp_pointer = 0, extraheap_cdot = 0, isheapentryfound = 0;
      int count = 0;
      uint8_t  *offloadParamData = NULL;

      count = GetOffloadedParamData(&offloadParamData);
      switch (queryType)
      {
      case TABLE_CMD:
      {
      ...
      for (lut_index = 0; lut_index < lutLength; lut_index++)
      {
         ...
         for (m = 0; m < (int)cdotTbl.nLen; m++)
         {
            int32_t isPersistPID = ACDB_ERROR;
            int unusedIndex = 0;
            ACDB_MEM_CPY(&dataofst, sizeof(dataofst), (void*)(&cdotTbl.pDataOffsets[m]), sizeof(dataofst));
			//mid = cdefTbl.pCntDef[m].nMid;
			...
			pid = cdefTbl.pCntDef[m].nPid;
				...
				if(count > 0)
				{
					isPersistPID = CheckOffloadedParameter(pid, offloadParamData, count);
				}
        ...
        for (pdi = 0; pdi < persistDataIndex; pdi++)
        {
            uint32_t pid_size = 0;
            int dataofst = 0;
            uint32_t temp_cvd_blob_offset = cvd_blob_offset + size_data;
            temp_cvd_blob_offset += sizeof(uint32_t); // <data_size, data[data_size]>
         ...
         if (heapsize > 0)
         {
            for (lut_index = 0; lut_index < lutLength; lut_index++)
            {
               ...
               for (m = 0; m < (int)cdotTbl.nLen; m++)
               {
                  ...
                  {
                            cdotTbl.pDataOffsets[m] = datapool_offset - datapool_start;
                            if ( count > 0 && CheckOffloadedParameter(pid, offloadParamData, count) == ACDB_SUCCESS)
                            {
...
{
   AcdbQueryCmdType *acdbQueryCmdType = (AcdbQueryCmdType *)calloc(1, sizeof(AcdbQueryCmdType));
   AcdbQueryResponseType *acdbQueryRspnseType = (AcdbQueryResponseType *)calloc(1, sizeof(AcdbQueryResponseType));
   ...
   if (acdbQueryCmdType != NULL) {
      if (acdbQueryCmdType->pBufferPointer != NULL) {
         ACDB_MEM_FREE(acdbQueryCmdType->pBufferPointer);
         acdbQueryCmdType->pBufferPointer = NULL;
      }
      ACDB_MEM_FREE(acdbQueryCmdType);
   }
   ...
   else
   {
      ...
      if (queryType == TABLE_CMD)// write lut size for the first time
      {
         ...
         for (l = 0; l < lut_data_entries; l++)//fill data pool entries table in buffer
         {
            ...
            datapool_offset += (int32_t)sizeof(cData.nLen);
            memset((pInput->pBufferPointer + datapool_offset), 0, Valid_length);
...
int32_t AcdbCmdGetDeviceProperty(uint32_t queryType, uint8_t *pIn, uint8_t *pOut)
{
   ...
   else
   {
      ...
      default:
      {
         ACDB_DEBUG_LOG("[ACDB Command]->[AcdbCmdGetDeviceProperty]->Invalid device property ID %u\n",
            devPropID);
         return ACDB_ERROR;
      }
      }
...
int32_t AcdbCmdSetAudStreamInstanceInfo(AcdbAudStrmTableV2CmdType *pInput)
{
   ...
   while (remaining_bufLength > sizeof(AudStrmTableEntryHeader))
   {
      ...
      if (remaining_bufLength >= oneEntryHeader.nParamSize)
	  {
		  ...
		  if (result != ACDB_SUCCESS)
		  {
			  ACDB_DEBUG_LOG("[ACDB Command]->[AcdbCmdSetAudStreamInstanceInfo]->Err:%d Failed to set AudProcData for...\n", result);
			  ACDB_DEBUG_LOG("ModID:%0x PID:%0x ParamSize:%0x \n", oneEntryHeader.nModuleId, oneEntryHeader.nParamId, oneEntryHeader.nParamSize);
			  return result;
		  }
                  ACDB_MEM_FREE(buffer);
...
int32_t AcdbCmdGetCodecTopId(int deviceid, AcdbGetTopologyIdRspType *pOutput)
{
   ...
   else
   {
      ...
      if (result != ACDB_SUCCESS)
      {
         ACDB_DEBUG_LOG("ACDB_COMMAND: Failed to fetch the codec topo info for devid %d \n", deviceid);
         return ACDB_PARMNOTFOUND;
      }
