 ...
 *
 * Copyright (c) 2012-2022, 2024 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
#ifdef ENABLE_AUDIO_LEGACY_DMABUFF
...
#include "acdb-rtac.h"
#include "adie-rtac.h"
#if (defined _ANDROID_) || (defined ENABLE_MCS_FTS_DEFINED)
#include "acdb-fts.h"
#include "acdb-mcs.h"
#endif
...
#else /* _ANDROID_ */
#define LOG_LEVEL_VERB ( 0 )
#define LOG_LEVEL_DEBUG ( 1 )
#define LOG_LEVEL_INFO  ( 2 )
#define LOG_LEVEL_WARNING  ( 3 )
#define LOG_LEVEL_ERROR ( 4 )
#define ACDB_LOG_LEVEL LOG_LEVEL_DEBUG

#if (ACDB_LOG_LEVEL <= LOG_LEVEL_VERB)
#define  LOGV(...)  fprintf(stdout,__VA_ARGS__)
#else
#define  LOGV(...)  ((void)0)
#endif
#if (ACDB_LOG_LEVEL <= LOG_LEVEL_DEBUG)
#define  LOGD(...)  fprintf(stdout,__VA_ARGS__)
#else
#define  LOGD(...)  ((void)0)
#endif
#if (ACDB_LOG_LEVEL <= LOG_LEVEL_INFO)
#define  LOGI(...)  fprintf(stdout,__VA_ARGS__)
#else
#define  LOGI(...) ((void)0)
#endif

#if (ACDB_LOG_LEVEL <= LOG_LEVEL_WARNING)
#define  LOGW(...)  fprintf(stdout,__VA_ARGS__)
#else
#define  LOGW(...)  ((void)0)
#endif
#if (ACDB_LOG_LEVEL <= LOG_LEVEL_ERROR)
...
#if defined(AUDIO_USE_SYSTEM_HEAP_ID) && !defined(AUDIO_AUTO)
...
#ifdef AUDIO_CMA_HEAP_ENABLE
#define HEAP_CMA_MASK	ION_HEAP(ION_AUDIO_ML_HEAP_ID)
#endif

#ifndef ASM_CAL_SEND_VERSION_OFFSET
...
#define VOICE_TX_FNN_COPP_TOPOLOGY 0x10028008
#define VOICE_TX_TRI_MIC_FNN_COPP_TOPOLOGY 0x10000077
#define VOICE_TX_DMIC_FNN_COPP_TOPOLOGY 0x10000079
#define INVALID_FD -1
...
enum {
	FNN_USECASE_STATE_NONE = -1,
	FNN_USECASE_STATE_STOPPED,
	FNN_USECASE_STATE_STARTED,
};
...
#endif
#ifdef ADM_AUDPROC_PERSISTENT_CAL_TYPE
	{0, 1},			/* ADM_AUDPROC_PERSISTENT_CAL_TYPE */
#endif
};
...
#ifdef AUDIO_MEM_OPT_ENABLED
#ifdef AUDIO_CMA_HEAP_ENABLE
	{12582912, false, 0, -1, 0, VERSION_0_0},	/* CVP_VOCPROC_STATIC_CAL_TYPE */
#else
	{3670016, false, 0, -1, 0, VERSION_0_0},          /* CVP_VOCPROC_STATIC_CAL_TYPE */
#endif
#else
#ifdef AUDIO_CMA_HEAP_ENABLE
        {12582912, true, 0, -1, 0, VERSION_0_0},        /* CVP_VOCPROC_STATIC_CAL_TYPE */
#else
	{3670016, true, 0, -1, 0, VERSION_0_0},          /* CVP_VOCPROC_STATIC_CAL_TYPE */
#endif
#endif
...
#ifdef AUDIO_MEM_OPT_ENABLED
	{4096, false, 0, -1, 0, VERSION_0_0},		/* ULP_AFE_CAL_TYPE */
	{4096, false, 0, -1, 0, VERSION_0_0},		/* ULP_LSM_CAL_TYPE */
#else
...
#ifdef AUDIO_MEM_OPT_ENABLED
	{4096, false, 0, -1, 0,VERSION_0_0},		/* AUDIO_CORE_METAINFO_CAL_TYPE */
#else
...
#ifdef AUDIO_MEM_OPT_ENABLED
	{8192, false, 0, -1, 0, VERSION_0_0},		/* ADM_RTAC_AUDVOL_CAL_TYPE */
#else
...
#ifdef AUDIO_MEM_OPT_ENABLED
	{4096, false, 0, -1, 0, VERSION_0_0},		/* AFE_LSM_TX_CAL_TYPE */
#else
	{4096, true, 0, -1, 0, VERSION_0_0},		/* AFE_LSM_TX_CAL_TYPE */
#endif
	{0, false, 0, -1, 0, VERSION_0_0},		/* ADM_LSM_TOPOLOGY_CAL_TYPE */
#ifdef AUDIO_MEM_OPT_ENABLED
	{16384, false, 0, -1, 0, VERSION_0_0},		/* ADM_LSM_AUDPROC_CAL_TYPE */
	{16384, false, 0, -1, 0, VERSION_0_0},		/* ADM_LSM_AUDPROC_PERSISTENT_CAL_TYPE */
#else
...
static int		cal_driver_handle = INVALID_FD;
static int		ion_handle = INVALID_FD;
...
static uint32_t 	current_adm_copp_id;
static bool		fnn_usecase_state_enabled = false;
static bool		is_fnn_usecase_started = false;
static int		fnn_usecase_state = FNN_USECASE_STATE_NONE;
static int		fnn_app_type = 0;
static int32_t	diag_init_status = DIAG_INIT_UNDEFINED;
...
static int get_vocstrmtable_size(void);
...
static int get_custom_topology_size(void);
...
static void check_fnn_usecase_status(int app_type)
{
	if (!fnn_usecase_state_enabled) {
	/* check app type and copp in case of missing set FNN state in HAL */
		if (app_type == ACDB_APPTYPE_VOIP_AUDIO &&
		    (current_adm_copp_id == VOICE_TX_FNN_COPP_TOPOLOGY ||
			 current_adm_copp_id == VOICE_TX_TRI_MIC_FNN_COPP_TOPOLOGY ||
			 current_adm_copp_id == VOICE_TX_DMIC_FNN_COPP_TOPOLOGY)) {
			 is_fnn_usecase_started = false;
			 fnn_usecase_state = FNN_USECASE_STATE_STARTED;
		} else {
			is_fnn_usecase_started = false;
			fnn_usecase_state = FNN_USECASE_STATE_STOPPED;
		}
	}

	LOGD("%s: fnn usecase active %d", __func__, fnn_usecase_state);
}
...
static void delete_cal_node(int cal_type, int buffer_number)
{
	struct cal_node		*cal_node = NULL;
	struct cal_node		*cal_head = cal_data[cal_type].next;
	struct cal_node		*cal_node_prev = cal_head;

	while (cal_head != NULL) {
		if (cal_head->buffer_number == buffer_number) {
			cal_node = cal_head;
			cal_head = cal_head->next;
			cal_node_prev->next = cal_head;
			break;
		}
		cal_node_prev = cal_head;
		cal_head = cal_head->next;
	}
	if (cal_node == NULL)
		goto done;
...
#else
static int get_files_from_properties(AcdbInitCmdType *acdb_init_cmd __unused) {return -ENODEV;}
#endif
...
static int get_files_from_device_tree(AcdbInitCmdType *acdb_init_cmd, char *snd_card_name)
{
	...
	if (!strcmp(board_type, "SLVTE_SURF"))
                strlcpy(board_type, "ADP", 4);
...
#if !defined(TARGET_ION_ABI_VERSION) && !defined(ENABLE_AUDIO_LEGACY_DMABUFF)
...
#ifndef ENABLE_AUDIO_LEGACY_DMABUFF
	if (ion_handle != INVALID_FD) {
#if TARGET_ION_ABI_VERSION >= 2
		ion_close(ion_handle);
#else
		close(ion_handle);
#endif
		ion_handle = INVALID_FD;
	}
#endif
	enable_gcov();
}
...
static void deallocate_cal_type_by_buf_number(int cal_type_idx, int buffer_number)
{
        struct cal_node                 *cal_node = NULL;
        struct audio_cal_dealloc        audio_dealloc;
        ...
        if (cal_node != NULL) {
            deallocate_cal_block(cal_type_idx, cal_node->buffer_number,
                                  &audio_dealloc, cal_node->cal_block);
            delete_cal_node(cal_type_idx, buffer_number);
        }
}
				...
				struct cal_block *cal_block)
{
...
#if defined(MSM_CMA_MEM_ALLOC)
	audio_alloc->cal_type.cal_data.cma_mem = 0;
#endif

#ifndef ENABLE_AUDIO_LEGACY_DMABUFF
#if TARGET_ION_ABI_VERSION >= 2
#if defined(MSM_CMA_MEM_ALLOC) && defined(AUDIO_CMA_HEAP_ENABLE)
	if (cal_type == CVP_VOCPROC_STATIC_CAL_TYPE ||
	    (cal_type == ADM_AUDPROC_PERSISTENT_CAL_TYPE &&
	     fnn_usecase_state == FNN_USECASE_STATE_STARTED)) {
		ret = ion_alloc_fd(ion_handle, cal_block->size,
				   ION_ALLOC_ALIGN, HEAP_CMA_MASK,
				   0, &cal_block->map_handle);
		audio_alloc->cal_type.cal_data.cma_mem = 1;
	} else {
		ret = ion_alloc_fd(ion_handle, cal_block->size,
	...
	if (ret) {
		...
		audio_alloc->cal_type.cal_data.cma_mem = 0;
#else
		LOGD("ACDB -> %s: ION alloc failed with heap mask 0x%x, cal type %d, "
			 "errno: %d, trying system heap...\n", __func__, HEAP_MASK, cal_type, ret);
		ret = ion_alloc_fd(ion_handle, cal_block->size,
				   ION_ALLOC_ALIGN, ION_HEAP(ION_SYSTEM_HEAP_ID),
				   0, &cal_block->map_handle);
#endif
		if (ret) {
...
#endif
#else
        ret=DmabufHeapAlloc(bufferAllocator, "qcom,system-uncached", cal_block->size, 0, 0);
        cal_block->map_handle = ret;
        alloc_data.len = cal_block->size;
        alloc_data.align = ION_ALLOC_ALIGN;
        alloc_data.heap_id_mask = HEAP_MASK;
        alloc_data.flags = 0;
...
static int allocate_memory(void)
{
	...
	struct cal_node			*cal_node = NULL;
	struct audio_cal_alloc		audio_alloc;
#if TARGET_ION_ABI_VERSION && !defined(ENABLE_AUDIO_LEGACY_DMABUFF)
#if TARGET_ION_ABI_VERSION >= 2
	ion_handle = ion_open();
#else
...
{
	AcdbModuleVersionTypeV3		response = {0};
	int				ret = 0;
	...
	ret = response.major;
	LOGD("%s: ACDB SW Major version: %d\n", __func__, ret);
done:
	return ret;
}
...
static bool get_instance_id_support(void)
{
	...
	AcdbQueryResponseType	response = {0};
	...
	LOGD("%s: ACDB file instance ID support info:%u\n",
...
int acdb_loader_init_v4(void* acdb_init_data, int acdb_loader_init_version)
{
	...
	if ((acdb_init_ref_cnt != 0) &&
		(diag_init_status == DIAG_INIT_SUCCESS)) {
		/* If acdb has been initialized already and
		   acph_init was previously successful, exit. */
		//TODO: add a dev note about why tracking diag_init_status is necessary
		LOGD("ACDB -> already initialized successfully, exit");
	...
	if (acdb_init_ref_cnt == 0) {
		/* Only initialize if this is the
		   first time acdb_init is being called. */
	...
	LOGD("%s: snd_card_name:%s, cvd_version:%s, instance_id_supported:%d",
	      __func__, init_data->snd_card_name, init_data->cvd_version,
	      instance_id_supported);

	LOGD("ACDB -> ACPH INIT\n");
	ret = acph_init(&diag_init_status);
	...
	if (diag_init_status == DIAG_INIT_FAILURE)
		LOGE("ACDB -> ACPH diag init failed.\n");
...
static int get_afe_topology(struct audio_cal_afe_top *afe_top)
{
	...
	LOGD("ACDB -> GET_AFE_TOPOLOGY_ID for adcd_id %u, Topology Id %x\n",
				...
				struct audio_cal_basic *afe_top)
{
	...
	LOGD("ACDB -> ACDB_CMD_GET_AFE_TOPOLOGIES: size:%d ret=%d \n",
...
static int get_hw_delay(struct cal_block *block __unused,
...
static int get_meta_info_size(int metaInfoKey)
{
	...
	LOGD("ACDB -> Metainfo size:%u bytes for key:%d\n",
...
int send_meta_info_list(struct listnode *key_list)
{
	...
	list_for_each(node, key_list) {
		...
		LOGD("ACDB -> send_meta_info_list key : %u",
			     ...
			     int cal_type)
{
	...
	current_adm_copp_id = adm_top.cal_type.cal_info.topology;
	if (result < 0)
		goto done;
...
static int get_audproc_persist_table_size(int app_id, int acdb_id, int sample_rate)
{
	...
	if (response.nSize == 0) {
		LOGD("Error: ACDB_CMD_GET_PERSISTENT_AUDPROC_INSTANCE_COMMON_TABLE_SIZE returned %u bytes",
			response.nSize);
		goto done;
	}
done:
...
static int send_audproc_persist_table(int acdb_id, int path, int app_id, int sample_rate)
{
	...
	block = get_cal_block(ADM_AUDPROC_PERSISTENT_CAL_TYPE, 0);
	if (block == NULL) {
		LOGD("%s: Error: Could not get cal block!\n", __func__);
		goto done;
	}

	if (is_fnn_usecase_started) {
		LOGD("%s: Error: Cannot use persit cal concurrent!\n", __func__);
		goto done;
	}

	if (audproc_persist_size > block->size || (fnn_usecase_state == FNN_USECASE_STATE_STARTED)) {
		...
		deallocate_cal_type(ADM_AUDPROC_PERSISTENT_CAL_TYPE, 0);
		allocate_cal_type(ADM_AUDPROC_PERSISTENT_CAL_TYPE, reallocate_size, 0);
	}
	...
	audproc_cal.cal_type.cal_hdr.buffer_number = 0;
	...
	if (fnn_usecase_state == FNN_USECASE_STATE_STARTED) {
		is_fnn_usecase_started = true;
		fnn_app_type = app_id;
	}
...
{
	acdb_loader_deallocate_cal_v2(capability, use_case, CAL_OFFSET_NONE);
}

void acdb_loader_deallocate_cal_v2(int capability, int use_case, int offset_index)
{
	int				path;
	int				cal_type;
	int				buffer_idx = use_case;
	int				buffer_idx_w_path;
	int				buffer_idx_w_offset;
	...
	if ((offset_index >= CAL_OFFSET_INDEX_MAX) || (offset_index < 0)) {
		LOGE("ACDB -> AUDIO cal offset index is not avalid!"
			"offset_index = %d\n", offset_index);
		goto done;
	}
	...
	LOGD("ACDB -> deallocate_cal, path = %d\n", path);

	if (buffer_idx_w_path >= MAX_PATH_TYPE) {
		deallocate_cal_type_by_buf_number(ASM_AUDSTRM_CAL_TYPE, buffer_idx_w_path);
		deallocate_cal_type_by_buf_number(ADM_AUDVOL_CAL_TYPE, buffer_idx_w_path);
		deallocate_cal_type_by_buf_number(ADM_AUDPROC_CAL_TYPE, buffer_idx_w_path);
		deallocate_cal_type_by_buf_number(ADM_TOPOLOGY_CAL_TYPE, buffer_idx_w_path);
	}
	if ((offset_index & CAL_OFFSET_ASM_TOP) != 0) {
		buffer_idx_w_offset = buffer_idx_w_path + ASM_CAL_SEND_VERSION_OFFSET;
	} else {
		buffer_idx_w_offset = buffer_idx;
	}
	if (buffer_idx_w_offset >= MAX_PATH_TYPE) {
		deallocate_cal_type_by_buf_number(ASM_TOPOLOGY_CAL_TYPE, buffer_idx_w_offset);
	}
...
void acdb_loader_send_audio_cal_v6(int acdb_id, int capability, int app_id, int sample_rate,
					int use_case, int afe_sample_rate, int cal_mode, int offset_index)
{
	int				path;
	int				aud_vol_idx = 0;
	...
	LOGD("ACDB -> send_audio_cal, acdb_id = %d, path = %d, app id = %d, sample rate = %d, use_case = %d,"
			"buffer_idx_w_path =%d, afe_sample_rate = %d, cal_mode = %d, offset_index = %d\n",
			acdb_id, path, app_id, sample_rate, use_case, buffer_idx_w_path, afe_sample_rate, cal_mode, offset_index);
...
{
	acdb_loader_send_audio_cal_v6(acdb_id, capability, app_id, sample_rate, use_case,
					afe_sample_rate, cal_mode, CAL_OFFSET_NONE);
	return;
}
...
void acdb_set_fluence_nn_state(bool state)
{
	if (!fnn_usecase_state_enabled)
		fnn_usecase_state_enabled = true;

	fnn_usecase_state = state;
	if (!state) {
		fnn_app_type = 0;
		is_fnn_usecase_started = false;
	}

	LOGD("%s: fnn usecase state %d\n", __func__, fnn_usecase_state);
}

int acdb_get_fluence_nn_state()
{
	if (!fnn_usecase_state_enabled)
		fnn_usecase_state_enabled = true;

	return is_fnn_usecase_started;
}
...
void send_mbhc_data(struct param_data *params)
{
	...
	if (result)
	{
		LOGD("Error reading MBHC general config returned = %d\n", result);
		goto acdb_error;
	}
	...
	if (result)
	{
		LOGD("Error reading MBHC removal config returned = %d\n", result);
		goto acdb_error;
	}
	...
	if (result)
	{
		LOGD("Error reading MBHC plug type config returned = %d\n", result);
		goto acdb_error;
	}
	...
	if (result)
	{
		LOGD("Error reading MBHC button press config returned = %d\n", result);
		goto acdb_error;
	}
	...
	if (result)
	{
		LOGD("Error reading MBHC impedance config returned = %d\n", result);
		goto acdb_error;
	}
...
void send_vad_data(struct param_data *params)
{
	...
	if (result)
	{
		LOGD("Error reading VAD core config returned = %d\n", result);
		goto acdb_error;
	}
...
static void send_vbat_data(struct param_data *params)
{
	...
	if (result)
	{
		LOGD("Error reading VBAT ADC CAL returned = %d\n", result);
		goto acdb_error;
	}
	...
	if (result)
	{
		LOGD("Error reading VBAT GAIN PROC returned = %d\n", result);
		goto acdb_error;
	}
	...
	if (result)
	{
		LOGD("Error converting vbat data to wcd9xxx data = %d\n", result);
	}
...
static int get_sidetone_iir_config(struct audio_cal_sidetone_iir *sidetone_iir_cal)
{
	...
	*/
	...
	memcpy(sidetone_iir_cal->cal_type.cal_info.iir_config,
		...
		sizeof(psidetone_iir_config->iir_config)?response.nBytesUsedInBuffer - 2 * 
		sizeof(psidetone_iir_config->num_biquad_stages):sizeof(psidetone_iir_config->iir_config)));
...
static int get_col_cal_type(uint32_t table_id)
{
	...
	case ACDB_VOC_PROC_STREAM_INSTANCE_TABLE:
		...
		break;
	default:
		LOGE("%s: Error: %u not a valid table ID!\n",
			__func__, table_id);
	}
	return table;
}
	   ...
	   uint32_t txacdb_id, uint32_t table_id)
{
	...
	LOGD("ACDB -> send_voice_columns, rxacdb_id %u, txacdb_id %u, table %u\n",
...
static int validate_voc_cal_dev_pair(int rxacdb_id, int txacdb_id)
{
	...
	AcdbDevicePairingResponseType response = {0};
...
int acdb_loader_reload_vocvoltable(int feature_set)
{
	...
	LOGD("ACDB -> acdb_loader_reload_vocvoltable, acdb_rx = %u, acdb_tx = %u, feature_set = %d\n",
		rxacdb_id, txacdb_id, feature_set);
...
void acdb_loader_deallocate_ACDB(void)
{
	...
	is_fnn_usecase_started = false;
	fnn_usecase_state = FNN_USECASE_STATE_NONE;
	fnn_app_type = 0;
	...
	if (cal_driver_handle != INVALID_FD) {
		close(cal_driver_handle);
		cal_driver_handle = INVALID_FD;
        }
...
int acdb_loader_get_remote_acdb_id(unsigned int native_acdb_id)
{
	...
	AcdbGetRmtCompDevIdRspType	response = {0};

	LOGD("ACDB -> acdb_loader_get_remote_acdb_id, acdb_id = %u\n",
		native_acdb_id);
...
int acdb_loader_get_calibration(char *attr, int size, void *data)
{
	...
	if (attr == NULL) {
		...
		LOGD("%s: Error: Invalid size %d, expected %lu\n",
...
int acdb_loader_store_set_audio_cal(acdb_audio_cal_cfg_t *cal, void *data, uint32_t datalen)
{
    ...
    if(cal->cal_type == AUDIO_DEVICE_CAL_TYPE) {
        ...
        LOGE("[%s] unsupported caltype %u", __func__, cal->cal_type);
        result = -EINVAL;
    }
...
int acdb_loader_adsp_set_audio_cal(acdb_audio_cal_cfg_t *cal, void *data, uint32_t datalen)
{
    ...
    if (instance_id_supported) {
        ...
        if(NULL == req) {
            LOGE("[%s] failed to allocate memory for length %u", __func__, req_len);
            result = -ENOMEM;
            goto done;
        }
...
int acdb_loader_set_audio_cal_v2(void *cfg, void *data, uint32_t datalen)
{
        ...
        if (!acdb_loader_is_initialized()) {
                LOGE("[%s] ACDB -> Not correctly initialized!", __func__);
                return -EINVAL;
        }
        ...
        LOGV("[%s] Exiting %d", __func__, result);
        return result;
}
...
int acdb_loader_store_get_audio_cal(acdb_audio_cal_cfg_t *cal, void *data, uint32_t *datalen)
{
    ...
    if(ACDB_SUCCESS == result) {
        ...
        *datalen = 0;
        LOGE("[%s] Error: ACDB get returned = %d length %u\n",
                                           ...
                                           uint32_t *cal_size)
{
    ...
    if (cal_size == NULL) {
        LOGE("%s: received null pointer for cal_size", __func__);
        return -EINVAL;
    }
    ...
    LOGD("ACDB -> ACDB_WDSP_CMD_GET_SUBGRAPH_LIST_SIZE\n");
    ret = acdb_ioctl(ACDB_WDSP_CMD_GET_SUBGRAPH_LIST_SIZE,
                     ...
                     sizeof(response));
    if (ret < 0) {
        LOGE("%s: Failed to get size for Subgraph list, ret = %d\n",
             __func__, ret);
        ret = -EINVAL;
        *cal_size = 0;
        goto done;
                                      ...
                                      uint8_t *cal_buff, size_t cal_size)
{
    ...
    if (cal_buff == NULL) {
        LOGE("%s: received null pointer for cal_buff", __func__);
        return -EINVAL;
    }
    ...
    if (response.nBytesUsedInBuffer != cal_size) {
        LOGE("%s: Subgraph list data size mismatch. Exiting\n", __func__);
        ret = -EINVAL;
        goto done;
    }
                                                    ...
                                                    uint32_t *cal_size)
{
    ...
    LOGD("ACDB -> ACDB_WDSP_CMD_GET_SUBGRAPH_MODULE_INFO_LIST_SIZE\n");
    ret = acdb_ioctl(ACDB_WDSP_CMD_GET_SUBGRAPH_MODULE_INFO_LIST_SIZE,
                     ...
                     sizeof(response));
    if (ret < 0) {
        LOGE("%s: Failed to get size for Subgraph Module list, ret = %d\n",
             __func__, ret);
        ret = -EINVAL;
        *cal_size = 0;
        goto done;
                                               ...
                                               size_t cal_size)
{
    ...
    /* Compare the size of the data copied to the buffer */
    if (response.nBytesUsedInBuffer != cal_size) {
        LOGE("%s: Subgraph Module list data size mismatch. Exiting\n", __func__);
        ret = -EINVAL;
        goto done;
    }
                                               ...
                                               uint32_t *cal_size)
{
    ...
    LOGD("ACDB -> ACDB_WDSP_CMD_GET_SUBGRAPH_MODULE_CONNECTION_SIZE\n");
    ret = acdb_ioctl(ACDB_WDSP_CMD_GET_SUBGRAPH_MODULE_CONNECTION_SIZE,
                     ...
                     sizeof(response));
    if (ret < 0) {
        LOGE("%s: Failed to get size for Subgraph Module connections, ret = %d\n",
             __func__, ret);
        ret = -EINVAL;
        *cal_size = 0;
        goto done;
                                          ...
                                          uint8_t *cal_buff, size_t cal_size)
{
    ...
    /* Compare the size of the data copied to the buffer */
    if (response.nBytesUsedInBuffer != cal_size) {
        LOGE("%s: Subgraph Module connections data size mismatch. Exiting\n",
             __func__);
        ret = -EINVAL;
        goto done;
    }
                                           ...
                                           uint32_t *cal_size)
{
    ...
    LOGD("ACDB -> ACDB_WDSP_CMD_GET_SUBGRAPH_CONNECTIONS_SIZE\n");
    ret = acdb_ioctl(ACDB_WDSP_CMD_GET_SUBGRAPH_CONNECTIONS_SIZE,
                     ...
                     sizeof(response));
    if (ret < 0) {
        LOGE("%s: Failed to get size for Subgraph connections, ret = %d\n",
             __func__, ret);
        ret = -EINVAL;
        *cal_size = 0;
        goto done;
                                      ...
                                      uint8_t *cal_buff, size_t cal_size)
{
    ...
    /* Compare the size of the data copied to the buffer */
    if (response.nBytesUsedInBuffer != cal_size) {
        LOGE("%s: Subgraph Pair connections data size mismatch. Exiting\n",
             __func__);
        ret = -EINVAL;
        goto done;
    }
                                              ...
                                              uint32_t *cal_size)
{
    ...
    LOGD("ACDB -> ACDB_WDSP_CMD_GET_SUBGRAPH_MODULE_CALIBRATION_SIZE\n");
    ret = acdb_ioctl(ACDB_WDSP_CMD_GET_SUBGRAPH_MODULE_CALIBRATION_SIZE,
                     ...
                     sizeof(response));
    if (ret < 0) {
        LOGE("%s: Failed to get size for Subgraph Module calibration, ret = %d\n",
             __func__, ret);
        ret = -EINVAL;
        *cal_size = 0;
        goto done;
                                         ...
                                         uint8_t *cal_buff, size_t cal_size)
{
    ...
    /* Compare the size of the data copied to the buffer */
    if (response.nBytesUsedInBuffer != cal_size) {
        LOGE("%s: Subgraph Module calibration data size mismatch. Exiting\n",
             __func__);
        ret = -EINVAL;
        goto done;
    }
...
int32_t acdb_loader_get_boot_time_mod_list_size(uint32_t *cal_size)
{
    ...
    LOGD("ACDB -> ACDB_WDSP_CMD_GET_BOOT_TIME_MODULE_LIST_SIZE\n");
    ret = acdb_ioctl(ACDB_WDSP_CMD_GET_BOOT_TIME_MODULE_LIST_SIZE,
                     NULL, 0, (uint8_t *)&response, sizeof(response));
    if (ret < 0) {
        LOGE("%s: Failed to get size of Boottime Load Module list, ret = %d\n",
             __func__, ret);
        ret = -EINVAL;
        *cal_size = 0;
        goto done;
...
int32_t acdb_loader_get_boot_time_mod_list(uint8_t *cal_buff, size_t cal_size)
{
    ...
    /* Compare the size of the data copied to the buffer */
    if (response.nBytesUsedInBuffer != cal_size) {
        LOGE("%s: Boottime Module list data size mismatch. Exiting\n",
             __func__);
        ret = -EINVAL;
        goto done;
    }
                                                   ...
                                                   uint32_t *cal_size)
{
    ...
    if (mod_list == NULL) {
        LOGE("%s: received null pointer for mod_list", __func__);
        return -EINVAL;
    }
    ...
    LOGD("ACDB -> ACDB_WDSP_CMD_GET_MODULE_LOADING_META_INFO_SIZE\n");
    ret = acdb_ioctl(ACDB_WDSP_CMD_GET_MODULE_LOADING_META_INFO_SIZE,
                     ...
                     sizeof(response));
    if (ret < 0) {
        LOGE("%s: Failed to get size for Dynamic Load Module Meta Info, ret = %d\n",
             __func__, ret);
        ret = -EINVAL;
        *cal_size = 0;
        goto done;
                                              ...
                                              size_t cal_size)
{
    ...
    /* Compare the size of the data copied to the buffer */
    if (response.nBytesUsedInBuffer != cal_size) {
        LOGE("%s: Dynamic Load Module Meta Info data size mismatch. Exiting\n",
             __func__);
        ret = -EINVAL;
        goto done;
    }
