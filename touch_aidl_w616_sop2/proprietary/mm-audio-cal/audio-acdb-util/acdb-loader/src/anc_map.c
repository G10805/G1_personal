   ...
   case TAIKO:
      {
            ...
            for(ancCh = 0; ancCh < input_size; ancCh++) {
                LOGD("%s:enter, ancch  %u, input device   %u", __func__,
...
//   uint32_t temp_ctl_reg_val;
   ...
   case TASHA:
   {
         ...
         if (anc_acdb_size < sizeof(anc_tasha_acdb_cfg_t)) {
            LOGE("%s: input buffer %u is not big enough for TASHA", __func__, anc_acdb_size);
            res = NOT_ENOUGH_MEMORY;
            break;
         }
   ...
   case(TAIKO):
      {
         ...
         for(index = 0; index < anc_reg_config_ptr->size; index++) {
             ...
             if (anc_acdb_size < 2 * sizeof(anc_taiko_acdb_cfg_t)) {
                 LOGE("%s: input buffer %u is not big enough for TAIKO", __func__, anc_acdb_size);
                 res = NOT_ENOUGH_MEMORY;
                 break;
             }
