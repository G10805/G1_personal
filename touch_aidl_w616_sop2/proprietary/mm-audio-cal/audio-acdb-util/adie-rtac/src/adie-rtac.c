 ...
 *
 * Copyright (c) 2013-2018, 2020, 2024 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
int find_codec_index(uint32_t handle)
{
	...
	for (i=0; i < (int)number_of_codecs; i++)
		if (codec_info[i].handle == handle)
			goto done;
	LOGE("%s->could not find codec with handle %u\n", __func__, handle);
	i = -EINVAL;
done:
	return i;
}
...
static int find_codecs(void)
{
	...
	for (i=0; i < number_of_codecs; i++)
		LOGD("%s: codec %s: handle %u, chipset id %u, major %u, minor %u, reg path %s\n",
			  ...
			  uint32_t *resp_buf_bytes_filled)
{
	...
	if((resp_buf_ptr != NULL) && (resp_buf_len >= sizeof(ACPH_CMD_GET_ADIE_REGISTER_rsp)))
	{
		...
		else
		{
			...
			LOGD("%s: valid register length is %u\n", __func__, register_length);
			while(numBytes>offset)
			{
				...
				offset += 3;
				LOGE("[ACPH_CMD_GET_ADIE_REGISTER]->reg[%08X],val[%d]\n",
			...
			{
				LOGE("ACPH_CMD_GET_ADIE_REGISTER command success. Found the value for register = 0x%X, value = %d\n",regAddr,lRegValue);
			}
	...
	if(NULL != resp_buf_bytes_filled)
	{
		...
		else
		{
			...
			if ((adie_rtac_major_version == 1) && (adie_rtac_minor_version == 0)) {
				...
				memcpy(&handle, pCurInputBuf, ACPH_CAL_DATA_UNIT_LENGTH);
				pCurInputBuf += ACPH_CAL_DATA_UNIT_LENGTH;
				LOGE("[ACPH_CMD_GET_MULTIPLE_ADIE_REGISTERS]->handle %u",
					handle);
			}
			...
			codec_idx = find_codec_index(handle);
			if (codec_idx < 0) {
				LOGE("[ACPH_CMD_GET_MULTIPLE_ADIE_REGISTERS]->could not find codec index for handle %u",
					handle);
				goto done;
			} else if (strlen(codec_info[codec_idx].reg_path) == 0) {
				LOGE("[ACPH_CMD_GET_MULTIPLE_ADIE_REGISTERS]->codec path is empty %u",
					handle);
				goto done;
			}
			...
			for(i=0; i<nTotalRegisters && found; i++)
			{
				...
				while(numBytes>offset)
				{
					...
					if(ultempRegAddr == regAddr)
					{
						...
						usleep(30);
						LOGE("[ACPH_CMD_GET_ADIE_REGISTER]->reg[%08X],val[%d], count[%u]\n",
				...
				uint32_t *resp_buf_bytes_filled)
{
	...
	if(NULL != resp_buf_bytes_filled)
	{
		...
		else
		{
			...
			codec_idx = find_codec_index(handle);
			if (codec_idx < 0) {
				LOGE("[ACPH_CMD_SET_MULTIPLE_ADIE_REGISTERS]->could not find codec index for handle %u",
					handle);
				goto done;
			} else if (strlen(codec_info[codec_idx].reg_path) == 0) {
				LOGE("[ACPH_CMD_SET_MULTIPLE_ADIE_REGISTERS]->codec path is empty %u",
					handle);
				goto done;
			}
