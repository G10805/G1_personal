...
API_MANAGEMENT_EXCEPTIONED_TARGET = ["pitti"]
...
def main():
    ...
    if arg_type == "all" and arg_tp_build:
        for plugin in pl_list:
            if plugin != None:
                ...
                if plugin_name == "qiifa_api_management_plugin":
                    if Constants.target_board_platform in API_MANAGEMENT_EXCEPTIONED_TARGET:
                        Logger.logInternal.warning("Skipping API Management for exceptioned target "+str(Constants.target_board_platform))
                    else:
    ...
    if arg_qssi is not None and arg_target is not None:
        ...
        for plugin in pl_list:
            if plugin != None:
                if "hidl" in plugin.MyPlugin().register() or "aidl" in plugin.MyPlugin().register() :
