...
Copyright (c) 2012, 2019 - 2020 Qualcomm Technologies, Inc.
All Rights Reserved.
    ...
    else
    {
      ...
      else
      {
        ...
        if (nPriority != 0)
        {
            int prio_max = 0;
            int prio_min = 0;
            if ((status = pthread_attr_setinheritsched(&attr, PTHREAD_EXPLICIT_SCHED)) != 0)
            {
                printf("pthread_attr_setinheritsched status %d\n",status);
                return status;
            }
            if ((status = pthread_attr_setschedpolicy(&attr,SCHED_RR)) != 0)
            {
                printf("pthread_attr_setschedpolicy status %d\n",status);
                return status;
            }
            prio_max = sched_get_priority_max(SCHED_RR);
            prio_min = sched_get_priority_min(SCHED_RR);
            if (nPriority < prio_min)
            {
                nPriority = prio_min;
            }
            if (nPriority > prio_max)
            {
                nPriority = prio_max;
            }
        }
        ...
        /*memcpy(attr.name, pstrName, PTHREAD_NAME_MAX_LEN);*/
          ...
          {
            MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH, "MM_Thread_CreateEx::thread 0x%lu created with stack %d bytes successfully!",
                         (unsigned long)*pHandle, stacksize);
#if defined(_LINUX_) || defined(_ANDROID_)
            // Try to set thread name, but do not fail if unsuccessful
           if(pstrName != NULL)
            {
              char pthread_setname[PTHREAD_NAME_MAX_LEN] = { 0 };
              strlcpy(pthread_setname, pstrName, PTHREAD_NAME_MAX_LEN-1);
              int status_opt = pthread_setname_np(*((pthread_t*)pHandle), pthread_setname);
              if (status_opt != 0) {
                 MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_HIGH, "MM_Thread_CreateEx::pthread_setname_np \"%s\" to thread 0x%lu failed %d",
                           pthread_setname, (unsigned long)*pHandle, status_opt);
	      }
	    }
#endif
          }
