...
static PyObject* simSetFacilityLockReq(PyObject* self, PyObject* args){
    ...
    set_fci_lock[2] = const_cast<char*>(service_class);
    set_fci_lock[2] = const_cast<char*>(aid);
    ...
    Py_XINCREF(callback);
    Status s = rilSession.simSetFacilityLockReq(set_fci_lock,[callback] (RIL_Errno e, int32_t * retries) -> void {
            PyObject* cbArgs = Py_BuildValue("(i)", e);
            if (cbArgs == NULL) {
                return;
            }
            ...
            if (retries != NULL)
            {
                std::cout << "Response for set facility req: "<< std::to_string(*retries)<<std::endl;
                ofs << "Response for set facility req: "<< std::to_string(*retries)<<std::endl;
            }
            ofs.close();
        }
    );
...
static PyObject* simQueryFacilityLockReq(PyObject* self, PyObject* args){
    ...
    quert_fci_lock[2] = const_cast<char*>(password);
    quert_fci_lock[2] = const_cast<char*>(service_class);
    quert_fci_lock[2] = const_cast<char*>(aid);
    userInput.push_back(facility);
    userInput.push_back(password);
    userInput.push_back(service_class);
    userInput.push_back(aid);
    Py_XINCREF(callback);
    Status s = rilSession.simQueryFacilityLockReq(quert_fci_lock,[callback] (RIL_Errno e, int32_t * retries) -> void {
            PyObject* cbArgs = Py_BuildValue("(i)", e);
            if (cbArgs == NULL) {
                return;
            }
            ...
            if (retries != NULL)
            {
                std::cout << "Response for queryFdn lock: "<< std::to_string(*retries)<<std::endl;
                ofs << "Response for queryFdn lock: "<< std::to_string(*retries)<<std::endl;
            }
            ofs.close();
        }
    );
...
static PyObject* setupDataCall_1_6(PyObject* self, PyObject* args) {
    ...
    datacallparams.trafficDescriptorValid = TrafficDescriptorvalid;
    if (datacallparams.trafficDescriptorValid) {
        datacallparams.trafficDescriptor.dnnValid = dnnValid;
        if (datacallparams.trafficDescriptor.dnnValid) {
            datacallparams.trafficDescriptor.dnn = const_cast<char*>(dnn);
            std::cout << "dnn :" << std::string(datacallparams.trafficDescriptor.dnn) << std::endl;
            ofs << "dnn :" << std::string(datacallparams.trafficDescriptor.dnn) << std::endl;
        }
        datacallparams.trafficDescriptor.osAppIdValid = osAppIdValid;
        if (datacallparams.trafficDescriptor.osAppIdValid) {
            datacallparams.trafficDescriptor.osAppIdLength = std::min(static_cast<int>(osAppId.size()), MAX_OS_APP_ID_LENGTH);
            memcpy(datacallparams.trafficDescriptor.osAppId, osAppId.data(), datacallparams.trafficDescriptor.osAppIdLength);
            for (int i = 0; i < datacallparams.trafficDescriptor.osAppIdLength; i++) {
                std::cout << "osAppID[" << i << "]" << datacallparams.trafficDescriptor.osAppId[i] << " ,";
                ofs << "osAppID[" << i << "]" << datacallparams.trafficDescriptor.osAppId[i] << " ,";
            }
        }
    }
...
static PyObject* dial(PyObject* self, PyObject* args) {
    ...
    std::cout << "Number to dial" << const_cast<char*>(address) << std::endl;
    if (!PyCallable_Check(callback)) {
        PyErr_SetString(PyExc_TypeError, "Second parameter must be callable.");
        return NULL;
    }
    ...
    Status s = rilSession.getDataCallList([callback] (const RIL_Errno e, const RIL_Data_Call_Response_v11* response, const unsigned long sz)-> void {
        ...
        for (int i = 0; (i < sz); i++) {
            ...
            if (response[i].linkAddressesLength > 0) {
                for (int j = 0; j < response[i].linkAddressesLength; j++) {
                    std::stringstream ss;
...
/*----------------------------< RIL request for GSM get broadcast SMS config >---------------------------*/
static PyObject* GsmGetBroadcastSmsConfig(PyObject* self, PyObject* args){
    ...
    Py_XINCREF(callback);
        ...
        Status s = rilSession.GsmGetBroadcastSmsConfig([callback] (RIL_Errno e, const RIL_GSM_BroadcastSmsConfigInfo * response, size_t respLen) -> void {
            PyObject* cbArgs = Py_BuildValue("(i)", e);
            if (cbArgs == NULL) {
                return;
            }
            ...
            if(response){
                std::cout << "Length of response is: " << std::to_string(respLen) << std::endl;
                std::cout << "fromServiceId: " << response->fromServiceId << std::endl;
                std::cout << "toServiceId: " << response->toServiceId << std::endl;
                std::cout << "fromCodeScheme: " << response->fromCodeScheme << std::endl;
                std::cout << "toCodeScheme: " << response->toCodeScheme << std::endl;
                std::cout << "selected: " << response->selected << std::endl;

            }
            ofs << "Callback error value: " << e << std::endl;
            if(response){
                ofs << "Length of response is: " << std::to_string(respLen) << std::endl;
                ofs << "fromServiceId: " << response->fromServiceId << std::endl;
                ofs << "toServiceId: " << response->toServiceId << std::endl;
                ofs << "fromCodeScheme: " << response->fromCodeScheme << std::endl;
                ofs << "toCodeScheme: " << response->toCodeScheme << std::endl;
                ofs << "selected: " << response->selected << std::endl;
            }
            ofs.close();
        }
    );
    ...
    {
        std::cout << "------------------------------------------------" << std::endl;
        std::cout << "Request GSM get broadcast SMS config sent success" << std::endl;
        std::cout << "------------------------------------------------" << std::endl;
    }
    return PyLong_FromLong(static_cast<long>(s));
}
...
static PyObject* simTransmitApduBasicReq(PyObject* self, PyObject* args){
    ...
    if (!PyArg_ParseTuple(args, "iiiiiisO", &sessionid, &cla, &instruction, &p1, &p2, &p3, &data, &callback)) {
        return NULL;
    }
    if (!PyCallable_Check(callback)) {
        PyErr_SetString(PyExc_TypeError, " Parameter must be callable.");
...
static PyObject* setDataProfile(PyObject* self, PyObject* args){
    ...
    dataProfilePtr = new RIL_DataProfileInfo_IRadio_1_4[1];
...
static PyObject* GsmSetBroadcastSMSConfig(PyObject* self, PyObject* args){
    ...
    int len;
    PyObject* callback;
    if (!PyArg_ParseTuple(args, "iiiisO", &fromServiceId, &toServiceId, &fromCodeScheme, &toCodeScheme, &selected, &len, &callback)) {
    ...
    Py_XINCREF(callback);
    Status s = rilSession.GsmSetBroadcastSMSConfig(config, len,[callback] (RIL_Errno e) -> void {
            PyObject* cbArgs = Py_BuildValue("(i)", e);
            if (cbArgs == NULL) {
                return;
            }
...
static PyObject* simChangePinReq(PyObject* self, PyObject* args){
    ...
    if (!PyArg_ParseTuple(args, "sssO",&newPin, &oldPin, &aid, &callback)) {
        return NULL;
    }
    if (!PyCallable_Check(callback)) {
        PyErr_SetString(PyExc_TypeError, " Parameter must be callable.");
    ...
    reqParams[0] = const_cast<char*>(newPin);
    reqParams[1] = const_cast<char*>(oldPin);
...
static PyObject* sendSms(PyObject* self, PyObject* args){
    ...
    if (!PyArg_ParseTuple(args, "zO", &Address, &callback)) {
        return NULL;
    }
    std::string destinationAddress (Address);
    if (!PyCallable_Check(callback)) {
        PyErr_SetString(PyExc_TypeError, " Parameter must be callable.");
        return NULL;
    }
    ...
    Py_XINCREF(callback);
    Status s = rilSession.sendSms(pdu,"",false, [callback] (RIL_Errno e, const RIL_SMS_Response& resp) -> void {
            PyObject* cbArgs = Py_BuildValue("(i)", e);
            if (cbArgs == NULL) {
                return;
            }
...
static PyObject* writeCdmaSmsToRuim(PyObject* self, PyObject* args){
    ...
    if (!PyArg_ParseTuple(args, "ibiiiiiiiiO", &uTeleserviceID, &bIsServicePresent,&uServicecategory, &len, &digit_mode, &number_mode, &number_type, &number_plan, &number_of_digits, &status, &callback)) {
        return NULL;
    }
...
static PyObject* setCallWaiting(PyObject* self, PyObject* args){
    ...
    Status s = rilSession.setCallWaiting(serviceClass, enabled, [callback] (RIL_Errno e) -> void {
            PyObject* cbArgs = Py_BuildValue("(i)", e);
            if (cbArgs == NULL) {
                return;
            }
...
static PyObject* translate(const RIL_IMS_SendSmsResponse& imsSendSmsResponse) {
    ...
    PyDict_SetItemString(dImsSendSmsResponse, "messageRef", PyLong_FromLong
        (imsSendSmsResponse.messageRef));
...
static PyObject* imsSendSms(PyObject* self, PyObject* args) {
    ...
    if (!PyArg_ParseTuple(args, "sO",&destinationAddress, &callback)) {
        return NULL;
    }
    ...
    std::string pdu("01000B91");
    pdu.append(destinationAddress);
    pdu.append("00000BC8329BFD065DDF723619");

    Py_XINCREF(callback);
    RIL_IMS_SmsMessage imsSmsMessage = {};
    imsSmsMessage.pdu = new uint8_t[pdu.size()];
    if (imsSmsMessage.pdu != nullptr) {
        imsSmsMessage.pduLength = pdu.size();
        std::copy(pdu.begin(), pdu.end(), imsSmsMessage.pdu);
        } else {
    ...
    if (imsSmsMessage.pdu == nullptr) {
        return PyLong_FromLong(static_cast<long>(Status::FAILURE));
    }
...
static PyObject* registerImsCallStateChangedIndicationHandler(PyObject* self, PyObject* args) {
    ...
    Status s = rilSession.registerImsUnsolCallStateChangedIndicationHandler(
        [callback] (RIL_IMS_CallInfo* calls, size_t numCalls) -> void {
            ...
            PyObject* cbArgs = Py_BuildValue("(N)", lCalls);
            if (cbArgs == nullptr) {
                return;
            }
...
static PyObject* registerCellInfoIndicationHandler(PyObject* self, PyObject* args) {
    ...
    Status s = rilSession.registerCellInfoIndicationHandler(
        [] (const RIL_CellInfo_v12 cellInfoList[], size_t cellInfoListLen)->void {
...
static PyMethodDef RilApiMethods[] = {
    ...
    {"registerSuppSvcNotificationIndicationHandler", registerSuppSvcNotificationIndicationHandler, METH_VARARGS, "Register supp svc notification indication handler"},
    {"registerOnUssdIndicationHandler", registerOnUssdIndicationHandler, METH_VARARGS, "Register on ussd indication handler"},
    {"registerEmergencyCbModeIndHandler", registerEmergencyCbModeIndHandler, METH_VARARGS, "Register Emergency Cb mode Ind handler"},
    {"registerLCEDataChangeHandler", registerLCEDataChangeHandler, METH_VARARGS, "Register LCEData Change Handler"},
    {"registerCdmaCallWaitingIndicationHandler", registerCdmaCallWaitingIndicationHandler, METH_VARARGS, "Register CDMA Call Waiting Indication handler"},
    {"registerCdmaNewSmsIndicationHandler", registerCdmaNewSmsIndicationHandler, METH_VARARGS, "Register CDMA New SMS Indication handler"},
    {"registerRestrictedStateChangedIndicationHandler", registerRestrictedStateChangedIndicationHandler, METH_VARARGS, "Register Restricted State Changed Indication handler"},
    {"registerRadioStateIndicationHandler", registerRadioStateIndicationHandler, METH_VARARGS, "Register Radio State Indication handler"},
    {"registerNitzTimeIndicationHandler", registerNitzTimeIndicationHandler, METH_VARARGS, "Register Nitz Time Indication Handler"},
    {"registerVoiceNetworkStateIndicationHandler", registerVoiceNetworkStateIndicationHandler, METH_VARARGS, "Register for voice network state indication handler"},
    {"registerImsNetworkStateChangeIndicationHandler", registerImsNetworkStateChangeIndicationHandler, METH_VARARGS, "Register IMS Network Change indication handler"},
    {"registerImsCallStateChangedIndicationHandler", registerImsCallStateChangedIndicationHandler, METH_VARARGS, "Register IMS call state change indication handler"},
    {"registerRadioCapabilityChangedIndHandler", registerRadioCapabilityChangedIndHandler, METH_VARARGS, "Register for radio Capability Changed Ind Handler"},
    ...
    {"registerUiccSubStatusIndHandler", registerUiccSubStatusIndHandler, METH_VARARGS, "Register for UICC Sub status ind handler"},
    {"registerSignalStrengthIndicationHandler", registerSignalStrengthIndicationHandler, METH_VARARGS, "Register for signal strength indication handler"},
    {"registerCellInfoIndicationHandler", registerCellInfoIndicationHandler, METH_VARARGS, "Register for Cell info indication handler"},
    {"registerRingBackToneIndicationHandler", registerRingBackToneIndicationHandler, METH_VARARGS, "Register for ring back tone indication handler"},
    {"registerUnsolCallStateChangeIndicationHandler", registerUnsolCallStateChangeIndicationHandler, METH_VARARGS, "Register for unsol call state change handler"},
    {"registerCallRingIndicationHandler", registerCallRingIndicationHandler, METH_VARARGS, "Request for register call ring indication handler"},
    {"registerSimStatusChanged", registerSimStatusChanged, METH_VARARGS, "Register for SIM status changed"},
    {"registerDataCallListChangedIndicationHandler", registerDataCallListChangedIndicationHandler, METH_VARARGS, "Register for Data Call List Changed Indication handler"},
    {"registerUnthrottleApnIndicationHandler", registerUnthrottleApnIndicationHandler, METH_VARARGS, "Register for Untrottle Apn Indication Handler"},
    {"registerBroadcastSmsIndicationHandler", registerBroadcastSmsIndicationHandler, METH_VARARGS, "Request for register broadcast SMS indication handler"},
    {"registerIncoming3GppSmsIndicationHandler", registerIncoming3GppSmsIndicationHandler, METH_VARARGS, "Request for register incoming 3GPP SMS indication"},
    {"registerVoiceRadioTechIndicationHandler", registerVoiceRadioTechIndicationHandler, METH_VARARGS, "Request for register Voice Radio tech"},
    {"registerNewSmsStatusReportIndicationHandler", registerNewSmsStatusReportIndicationHandler, METH_VARARGS, "Request fot register new SMS status"},
    {"registerIncoming3GppSmsIndicationHandler", registerIncoming3GppSmsIndicationHandler, METH_VARARGS, "Request for register Incoming 3Gpp"},
    {"registerPhysicalConfigStructHandler", registerPhysicalConfigStructHandler, METH_VARARGS, "Request for register Physical Config Indication"},
    {"registerDataNrIconChangeHandler", registerDataNrIconChangeHandler, METH_VARARGS, "Request for register Nr Icon Change Indication"},
    {"registerRadioKeepAliveHandler", registerRadioKeepAliveHandler, METH_VARARGS, "Request for register Radio Keep Alive Indication"},
    {"ackSms", ackSms, METH_VARARGS, "Request for acknowlwdge SMS"},
    {"sendSms", sendSms, METH_VARARGS, "Request to send SMS"},
    {"imsQueryVirtualLineInfo", imsQueryVirtualLineInfo, METH_VARARGS, "Request for ims Query Virtual Line info"},
    {"imsSendRttMessage", imsSendRttMessage, METH_VARARGS, "Request for ims Send Rtt Message"},
    {"imsSendGeolocationInfo", imsSendGeolocationInfo, METH_VARARGS, "Request for ims Send Geo Location Info"},
    ...
    {"imsSetSuppSvcNotification", imsSetSuppSvcNotification, METH_VARARGS, "Request for ims SuppSvc status"},
    ...
    {"imsRequestResumeCall", imsRequestResumeCall, METH_VARARGS, "Request for ims Requesr resume call"},
    ...
    {"imsModifyCallConfirm", imsModifyCallConfirm, METH_VARARGS, "Request for ims Modify Call Confirm"},
    {"imsModifyCallInitiate", imsModifyCallInitiate, METH_VARARGS, "Request ims Modify Call Initiate"},
    ...
    {"imsExplicitCallTransfer", imsExplicitCallTransfer, METH_VARARGS, "Request for ims Explicit Call Transfer"},
    {"imsRequestExitEmergencyCallbackMode", imsRequestExitEmergencyCallbackMode, METH_VARARGS, "Request for ims Request Exit Emergency callback"},
    {"imsRequestConference", imsRequestConference, METH_VARARGS, "Request for ims Request Conference"},
    {"CarrierInfoForImsiEncryption", CarrierInfoForImsiEncryption, METH_VARARGS, "Request for carrier info imsi encryption"},
    ...
    {"imsQueryServiceStatus", imsQueryServiceStatus, METH_VARARGS, "Request for ims Query Service status"},
    {"imsRegistrationChange", imsRegistrationChange, METH_VARARGS, "Request for ims Registration change"},
    ...
    {"imsGetRegistrationState", imsGetRegistrationState, METH_VARARGS, "Request for ims Get Registration State"},
    {"SetPreferredDataModem", SetPreferredDataModem, METH_VARARGS, "Request for set preferred data modem"},
    {"SetLinkCapRptCriteria", SetLinkCapRptCriteria, METH_VARARGS, "Request for set link cap rpt criteria"},
    ...
    {"ReportStkServiceIsRunning", ReportStkServiceIsRunning, METH_VARARGS, "Request for Stk service is running"},
    {"StkHandleCallSetupRequestedFromSim", StkHandleCallSetupRequestedFromSim, METH_VARARGS, "Request for Stk Handle Call Set up request from sim"},
    {"StkSendTerminalResponse", StkSendTerminalResponse, METH_VARARGS, "Request for stk send terminal response"},
    {"StkSendEnvelopeCommand", StkSendEnvelopeCommand, METH_VARARGS, "Request for Stk Send Envolope command"},
    {"EnterNetworkDepersonalization", EnterNetworkDepersonalization, METH_VARARGS, "Request for network depersonalisation"},
    {"shutDown", shutDown, METH_VARARGS, "Shut down request"},
    {"sendCdmaSms", sendCdmaSms, METH_VARARGS, "Send Cdma SMS"},
    {"getCdmaRoamingPreference", getCdmaRoamingPreference, METH_VARARGS, "Request for get Cdma Roaming Preference"},
    {"getCdmaSubscriptionSource", getCdmaSubscriptionSource , METH_VARARGS, "Request for get Cdma Subscription Service"},
    {"queryAvailableNetworks", queryAvailableNetworks, METH_VARARGS, "Request for query available networks"},
    {"setNetworkSelectionManual", setNetworkSelectionManual, METH_VARARGS, "Request for set Network Selection Mannual"},
    ...
    {"getCdmaVoicePrefMode", getCdmaVoicePrefMode, METH_VARARGS, "Request for get Cdms Voice Pref mode"},
    {"setCdmaVoicePrefMode", setCdmaVoicePrefMode, METH_VARARGS, "Request for set Cdma Voice Pref mode"},
    {"getTtyMode", getTtyMode, METH_VARARGS, "Request fot get Tty mode"},
    {"setTtyMode", setTtyMode, METH_VARARGS, "Request fot set Tty mode"},
    {"setSuppSvcNotification", setSuppSvcNotification , METH_VARARGS, "Request for set Supp Svc Notification"},
    {"queryClip", queryClip, METH_VARARGS, "Request for query Clip"},
    {"changeBarringPassword", changeBarringPassword, METH_VARARGS, "Request for change barring password"},
    ...
    {"setCallForward", setCallForward, METH_VARARGS, "Request for set Call Forward"},
    {"queryCallForwardStatus", queryCallForwardStatus, METH_VARARGS, "Request for query Call Forward Status"},
    ...
    {"reportSmsMemoryStatus", reportSmsMemoryStatus, METH_VARARGS, "Request for report SMS Memory Status"},
    {"deleteCdmaSmsOnRuim", deleteCdmaSmsOnRuim, METH_VARARGS, "Request for delete CDMA SMS on RUIM"},
    {"getCdmaSubscription", getCdmaSubscription, METH_VARARGS ,"Request for get CDMA subscription"},
    {"setCdmaSmsBroadcastConfig", setCdmaSmsBroadcastConfig,METH_VARARGS , "RIL request for set CDMA SMS broadcast config"},
    ...
    {"GsmSetBroadcastSMSConfig", GsmSetBroadcastSMSConfig, METH_VARARGS, "Request for GSM set broadcast sms config"},
    {"simTransmitApduChannelReq", simTransmitApduChannelReq, METH_VARARGS, "Request for sim transmit apdu channel"},
    ...
    {"setCdmaSmsBroadcastActivation", setCdmaSmsBroadcastActivation, METH_VARARGS, "Request for CDMA SMS broadcast activation"},
    {"simCloseChannelReq", simCloseChannelReq, METH_VARARGS, "Request for sim close channel"},
    {"simIsimAuthenticationReq", simIsimAuthenticationReq, METH_VARARGS, "Request for sim isim authentication"},
    ...
    {"simTransmitApduBasicReq", simTransmitApduBasicReq, METH_VARARGS,"Request to sim transmit apdu basic req"},
    {"setInitialAttachApn", setInitialAttachApn, METH_VARARGS, "Request to set initial apn"},
    {"setUnsolCellInfoListRate", setUnsolCellInfoListRate, METH_VARARGS, "Request for set unsol cell info"},
    {"VoiceRadioTech", VoiceRadioTech, METH_VARARGS, "Request for voice radio tech"},
    {"simStkSendEnvelopeWithStatusReq", simStkSendEnvelopeWithStatusReq, METH_VARARGS, "Request for sim stk send envolope with status"},
    ...
    {"GsmSmsBroadcastActivation", GsmSmsBroadcastActivation, METH_VARARGS, "Request for GSM SMS broadcast activation"},
    {"GsmGetBroadcastSmsConfig",GsmGetBroadcastSmsConfig, METH_VARARGS, "Request for GSM get broadcast SMS config"},
    {"explicitCallTransfer", explicitCallTransfer, METH_VARARGS, "Request for explicit call transfer"},
    {"queryAvailableBandMode", queryAvailableBandMode, METH_VARARGS, "Request for availble band mode"},
    ...
    {"QueryNetworkSelectionMode", QueryNetworkSelectionMode, METH_VARARGS, "Request for query Network Selection Mode"},
    {"setNetworkSelectionAutomatic", setNetworkSelectionAutomatic, METH_VARARGS, "Request for Set Network Selection Automatic"},
    {"getPreferredNetworkType", getPreferredNetworkType, METH_VARARGS, "Get Set Preferred network type"},
    ...
    {"hangupWaitingOrBackground", hangupWaitingOrBackground, METH_VARARGS, "hang up waiting or background"},
    {"hangupForegroundResumeBackground",hangupForegroundResumeBackground, METH_VARARGS, "hang up foreground and resume background"},
    {"switchWaitingOrHoldingAndActive",switchWaitingOrHoldingAndActive, METH_VARARGS,"switch waiting or holding active"},
    ...
    {"simQueryFacilityLockReq", simQueryFacilityLockReq, METH_VARARGS, "RIL request for sim Query Facility Lock request"},
    {"simSetFacilityLockReq", simSetFacilityLockReq, METH_VARARGS, "RIL request for sim set facility lock req"},
    {"setPreferredNetworkType", setPreferredNetworkType, METH_VARARGS, "RIL request for set Preferred network type"},
    {"getCdmaSmsBroadcastConfig", getCdmaSmsBroadcastConfig, METH_VARARGS, "RIL request for get Cdma SMS broadcast config"},
