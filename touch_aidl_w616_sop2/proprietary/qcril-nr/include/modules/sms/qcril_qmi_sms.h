...
typedef struct
{
  ...
  TimeKeeper::timer_id mt_sms_ack_needed_expry_tmr;
...
typedef struct
{
  ...
  uint32_t gw_transaction_id;                     /* Transaction id for the GW SMS message to be acked */
  ...
  boolean cdma_ack_needed;                      /* Indicates whether QCRIL will drop the Android ack
                                                   (QCRIL will drop the ack if the modem indicates
                                                   that no ack is needed) */
  uint32_t cdma_transaction_id;                   /* Transaction id for the CDMA SMS message to be acked */
...
typedef struct
{
  ...
  TimeKeeper::timer_id        mt_pending_ack_expry_tmr;
...
{
  uint16_t                    data_len;
  uint8_t                     data[ RIL_CDMA_SMS_BEARER_DATA_MAX ];
...
void qcril_sms_byte_to_hex(uint8_t * byte_pdu,   // INPUT
                           char * hex_pdu,    // OUTPUT
                           uint32_t num_bytes);
...
qmi_ril_gen_operational_status_type qcril_qmi_sms_get_operational_status();
