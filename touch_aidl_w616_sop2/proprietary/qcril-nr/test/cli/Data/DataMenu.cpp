...
void DataMenu::startDataCall_1_6(std::vector<std::string> inputCommand) {
    ...
    callparams.trafficDescriptorValid = inputIntValidator();
    if (callparams.trafficDescriptorValid == 1) {
        std::cout << "Enter dnn? 1 yes, 0 no: " << std::endl;
        callparams.trafficDescriptor.dnnValid = inputIntValidator();
        if (callparams.trafficDescriptor.dnnValid == 1) {
            std::string dnn;
            std::cout << "Enter dnn: " << std::endl;
            std::cin >> dnn;
            callparams.trafficDescriptor.dnn = const_cast<char*>(dnn.c_str());
        }
        std::cout << "Enter osAppId? 1 yes, 0 no: " << std::endl;
        callparams.trafficDescriptor.osAppIdValid = inputIntValidator();
        if (callparams.trafficDescriptor.osAppIdValid == 1) {
            ...
            callparams.trafficDescriptor.osAppIdLength = std::min(static_cast<int>(osAppId.size()), MAX_OS_APP_ID_LENGTH);
            memcpy(callparams.trafficDescriptor.osAppId, osAppId.data(), callparams.trafficDescriptor.osAppIdLength);
        }
    }
