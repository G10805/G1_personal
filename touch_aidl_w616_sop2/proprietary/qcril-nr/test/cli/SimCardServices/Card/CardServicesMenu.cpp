...
static bool base64_from_bytes(bytestring bytes, char **base64_ptr)
{
    ...
    if ((*base64_ptr = (char *)malloc(buffer_size)) == NULL) {
        return false;
    }

    for (size_t i = 0; i < num_triplets; i++) {
        uint32_t byte_a = bytes.data[3*i + 0];
        uint32_t byte_b = 3*i + 1 < bytes.size ? bytes.data[3*i + 1] : 0;
        uint32_t byte_c = 3*i + 2 < bytes.size ? bytes.data[3*i + 2] : 0;
        uint32_t value = byte_a << 16 | byte_b << 8 | byte_c;
...
void CardServicesMenu::registerForIndications(std::vector<std::string> userInput) {
  ...
  rilSession.registerLpaHttpsTxnIndicationHandler(
          ...
          if(ind != nullptr){
              ...
              std::cout<<"[CardServicesMenu]: Https indication payload"<<std::string(payload_data)<<std::endl;
...
void CardServicesMenu::lpauserreq(std::vector<std::string> userInput) {
    ...
    std::cout << ((s == Status::SUCCESS) ? "Received lpa response  succesfully"
                                         : "Error in receiving lpa response")
              << std::endl;
    for (auto& arg : userInput) std::cout << __func__ << ":" << arg << std::endl;
}
