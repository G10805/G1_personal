...
void PhoneMenu::init() {
  ...
  std::vector<std::shared_ptr<ConsoleAppCommand>> commandsListPhoneSubMenu = {
    ...
    CREATE_COMMAND(PhoneMenu::getRadioCapability, "Get Radio capability"),
    CREATE_COMMAND(PhoneMenu::setRadioCapability, "Set Radio capability"),
  };
...
void printSignalInfo(RIL_TD_SCDMA_SignalStrength info) {
  std::cout << "TDSCDMA Signal Strength Information: " << std::endl;
  std::cout << "signalStrength = " << info.signalStrength << std::endl;
  std::cout << "bitErrorRate = " << info.bitErrorRate << std::endl;
  std::cout << "rscp = " << info.rscp << std::endl;
  std::cout << std::endl;
...
std::ostream& operator <<(std::ostream& os, const RIL_RadioCapability& arg) {

  os << "Radio Capability Info: " << std::endl;
  os << "session: " << arg.session << std::endl;
  os << "phase: " << arg.phase << std::endl;
  os << "rat: " << std::endl;
  printRadioTechnologyBitmap(arg.rat);
  os << "logicalModemUuid: " << arg.logicalModemUuid << std::endl;
...
void printRadioTechnologyBitmap(int rat) {
  if(rat & RAF_UNKNOWN){
    std::cout << "RADIO_TECH_UNKNOWN" << std::endl;
  }
  if(rat & RAF_GPRS){
    std::cout << "RADIO_TECH_GPRS" << std::endl;
  }
  if(rat & RAF_EDGE){
    std::cout << "RADIO_TECH_EDGE" << std::endl;
  }
  if(rat & RAF_UMTS){
    std::cout << "RADIO_TECH_UMTS" << std::endl;
  }
  if(rat & RAF_IS95A){
    std::cout << "RADIO_TECH_IS95A" << std::endl;
  }
  if(rat & RAF_IS95B){
    std::cout << "RADIO_TECH_IS95B" << std::endl;
  }
  if(rat & RAF_1xRTT){
    std::cout << "RADIO_TECH_1xRTT" << std::endl;
  }
  if(rat & RAF_EVDO_0){
    std::cout << "RADIO_TECH_EVDO_0" << std::endl;
  }
  if(rat & RAF_EVDO_A){
    std::cout << "RADIO_TECH_EVDO_A" << std::endl;
  }
  if(rat & RAF_HSDPA){
    std::cout << "RADIO_TECH_HSDPA" << std::endl;
  }
  if(rat & RAF_HSUPA){
    std::cout << "RADIO_TECH_HSUPA" << std::endl;
  }
  if(rat & RAF_HSPA){
    std::cout << " RADIO_TECH_HSPA" << std::endl;
  }
  if(rat & RAF_EVDO_B){
    std::cout << "RADIO_TECH_EVDO_B" << std::endl;
  }
  if(rat & RAF_EHRPD){
    std::cout << "RADIO_TECH_EHRPD" << std::endl;
  }
  if(rat & RAF_LTE){
    std::cout << "RADIO_TECH_LTE" << std::endl;
  }
  if(rat & RAF_HSPAP){
    std::cout << "RADIO_TECH_HSPAP" << std::endl;
  }
  if(rat & RAF_GSM){
    std::cout << "RADIO_TECH_GSM" << std::endl;
  }
  if(rat & RAF_LTE_CA){
    std::cout << "RADIO_TECH_TD_SCDMA" << std::endl;
  }
  if(rat & RAF_5G){
    std::cout << "RADIO_TECH_5G" << std::endl;
  }
}
...
void printCellIdentity(RIL_CellIdentityNr &cell) {
...
void printCellInfo(RIL_CellIdentity_v16 cell) {
  ...
  switch (cell.cellInfoType) {
    ...
    case RIL_CELL_INFO_TYPE_NR:
      printCellIdentity(cell.cellIdentityNr);
      break;
  }
}
...
void PhoneMenu::setRadioPower(std::vector<std::string> userInput) {
  ...
  std::cout << "Radio Power: ";
  std::cout << "state : (1 - On, 0 - Off): ";
  std::cin >> req.state;
  std::cout << "forEmergencyCall (1 - true, 0 - false): ";
  std::cin >> req.forEmergencyCall;
  std::cout << "preferredForEmergencyCall (1 - true, 0 - false): ";
  std::cin >> req.preferredForEmergencyCall;
  req.state = !!req.state;
  req.forEmergencyCall = !!req.forEmergencyCall;
  req.preferredForEmergencyCall = !!req.preferredForEmergencyCall;
  Status s = rilSession.radioPower(req, [](RIL_Errno err) -> void {
...
void PhoneMenu::registerForUnsols(std::vector<std::string> userInput) {
    ...
    rilSession.registerRadioCapabilityChangedIndHandler(
        [] (const RIL_RadioCapability& radCap) {
            std::cout << "Received Radio Capability Changed Indication" << std::endl;
            std::cout << radCap << std::endl;
        }
    );
}
...
void PhoneMenu::getRadioCapability(std::vector<std::string> userInput) {
  Status s = rilSession.getRadioCapability(
    [] (RIL_Errno e, const RIL_RadioCapability& radCap) {
      std::cout << std::endl
        << (e == RIL_Errno::RIL_E_SUCCESS ? "Successfully received" : "Failed to receive")
        << " radio capability info." << std::endl;
  ...
  if (s != Status::SUCCESS) {
    std::cout << "Failed to issue request to get Radio Capability Info." << std::endl;
  }
}

void PhoneMenu::setRadioCapability(std::vector<std::string> userInput) {
  RIL_RadioCapability radCap = {};
  std::cin.clear();

  radCap.version = RIL_RADIO_CAPABILITY_VERSION;

  std::cout << "Input Radio Capability Info: " << std::endl;
  std::cout << "Input session value:" << std::endl;
  std::cin >> radCap.session;

  std::cout << "Input phase: " << std::endl;
  std::cin >> radCap.phase;

  std::cout << "Input rat: " << std::endl;
  std::cin >> radCap.rat;

  std::string logModUuid;
  std::cout << "Input logical Modem Uuid: " << std::endl;
  std::cin >> logModUuid;
  auto len = logModUuid.length();
  if (len > 0) {
    strlcpy(radCap.logicalModemUuid, logModUuid.c_str(), len+1);
  }

  std::cout << "Input return status: " << std::endl;
  std::cin >> radCap.status;

  Status s = rilSession.setRadioCapability(radCap,
    [] (RIL_Errno e) {
      std::cout << "Got response for Set Radio Capability Info: " << e << std::endl;
    }
  );

  if (s != Status::SUCCESS) {
    std::cout << "Failed to issue request to set Radio Capability Info." << std::endl;
  }
}
