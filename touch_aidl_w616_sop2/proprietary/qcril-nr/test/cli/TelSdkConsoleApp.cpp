...
#include "Oemhook/OemhookMenu.hpp"
...
//            "6", "Data", {}, std::bind(&TelSdkConsoleApp::dataMenu, this, std::placeholders::_1)));
    ...
    std::vector<std::shared_ptr<ConsoleAppCommand>> mainMenuCommands = {
       ...
       CREATE_COMMAND(TelSdkConsoleApp::oemhookMenu, "OEMHOOK"),
    };
...
void TelSdkConsoleApp::oemhookMenu(std::vector<std::string> userInput) {
  OemhookMenu oemhookMenu("OEMHOOK Menu", "OEMHOOK> ", rilSession);
  oemhookMenu.init();
  oemhookMenu.mainLoop();
  TelSdkConsoleApp::displayMenu();
}
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (argv[i] != nullptr && !strncmp(argv[i], "-v", 2)) {
      verboseLogging = true;
    }
  }
  ...
  if (verboseLogging) {
    qti::ril::logger::Logger::setDefault("stdout");
  } else {
    qti::ril::logger::Logger::setDefault("diag");
  }
