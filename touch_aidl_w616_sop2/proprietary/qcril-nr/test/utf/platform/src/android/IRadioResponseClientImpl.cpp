...
Return<void> IRadioResponseClientImpl::getCurrentCallsResponse_1_6(const V1_6::RadioResponseInfo& info, const hidl_vec<V1_6::Call>& calls) {
    ...
    if (num_calls > 0) {
      ...
      for (int i = 0; i < num_calls; ++i) {
        payload[i] = (RIL_Call*)malloc(sizeof(RIL_Call));
        memset(payload[i], 0, sizeof(RIL_Call));
        payload[i]->state = (RIL_CallState)(calls[i].base.base.state);
        payload[i]->index = calls[i].base.base.index;
        payload[i]->toa = calls[i].base.base.toa;
        payload[i]->isMpty = calls[i].base.base.isMpty;
        payload[i]->isMT = calls[i].base.base.isMT;
        payload[i]->als = calls[i].base.base.als;
        payload[i]->isVoice = calls[i].base.base.isVoice;
        payload[i]->isVoicePrivacy = calls[i].base.base.isVoicePrivacy;
        payload[i]->number = strndup(calls[i].base.base.number.c_str(), calls[i].base.base.number.size()+1);
        payload[i]->numberPresentation = (int)calls[i].base.base.numberPresentation;
        payload[i]->name = strndup(calls[i].base.base.name.c_str(), calls[i].base.base.name.size()+1);
        payload[i]->namePresentation = (int)calls[i].base.base.namePresentation;
        // TODO UusInfo
        payload[i]->audioQuality = (RIL_AudioQuality)calls[i].base.audioQuality;
        payload[i]->redirNum = strndup(calls[i].forwardedNumber.c_str(), calls[i].forwardedNumber.size() + 1);
      }
    }
    int slot = acquire_expectation_table();
    enqueue_ril_expect(
