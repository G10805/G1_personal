...
typedef class ril_utf_test_t {
  ...
  bool is_same(const RIL_CellIdentityGsm_v12& expect, const RIL_CellIdentityGsm_v12& recv);
  bool is_same(const RIL_CellIdentityWcdma_v12& expect, const RIL_CellIdentityWcdma_v12& recv);
  bool is_same(const RIL_CellIdentityLte_v12& expect, const RIL_CellIdentityLte_v12& recv);
  bool is_same(const RIL_CellIdentityTdscdma& expect, const RIL_CellIdentityTdscdma& recv);
  bool is_same(const RIL_CellIdentityNr& expect, const RIL_CellIdentityNr& recv);
  bool is_same(const RIL_CellIdentity_v16& expect, const RIL_CellIdentity_v16& recv);
  ...
  bool compare_pre_alerting_call_info(const ims_PreAlertingCallInfo& expected,
                                      const ims_PreAlertingCallInfo& received);
  bool compare_ecnam_info(const ims_EcnamInfo& expected, const ims_EcnamInfo& received);
  void validate_ims_unsol_pre_alerting_call_info_ind(ril_utf_node_base_t* exp_node,
                                                     struct ril_utf_msg_t* data);
  void validate_ims_unsol_retrieving_geolocation_data_status(ril_utf_node_base_t *exp_node,
      struct ril_utf_msg_t *data);
  void validate_ims_unsol_sms_callback_mode_changed(
      ril_utf_node_base_t *exp_node, struct ril_utf_msg_t *data);
  void validate_ims_unsol_incoming_dtmf(ril_utf_node_base_t* exp_node, struct ril_utf_msg_t* data);
  void validate_ims_multi_sim_voice_capability(ril_utf_node_base_t* exp_node,
                                               struct ril_utf_msg_t* data);
  void validate_ims_unsol_auto_call_rejection_ind2(ril_utf_node_base_t* exp_node,
                                                   struct ril_utf_msg_t* data);

  void validate_lte_direct_disc_message(ril_utf_node_base_t *exp_node,
                     struct ril_utf_msg_t *data);
  void validate_ims_vos_support_status(ril_utf_node_base_t* exp_node, struct ril_utf_msg_t* data);
  void validate_ims_vos_action_info(ril_utf_node_base_t* exp_node, struct ril_utf_msg_t* data);
  bool compare_vos_move_info(const ims_VosMoveInfo& expected, const ims_VosMoveInfo& received);
  bool compare_vos_touch_info(const ims_VosTouchInfo& expected, const ims_VosTouchInfo& received);
  bool compare_coordinate2d(const ims_Coordinate2D& expected, const ims_Coordinate2D& received);
  ...
  void execute_data (void(*update_globals)(void));
  void set_start(void);
  void set_end(void);
  ...
  void verify_ril_data_registration_state(ril_utf_node_base_t *exp_node,
                                 struct ril_utf_msg_t *data);
  void verify_ril_get_cell_info_list(ril_utf_node_base_t *exp_node,
                                 struct ril_utf_msg_t *data);
  void validate_dsi_request_response( ril_utf_node_base_t *exp_node, struct ril_utf_msg_t *data);
  void validate_qdp_request( ril_utf_node_base_t *exp_node, struct ril_utf_msg_t *data);
  void validate_qdp_response( ril_utf_node_base_t *exp_node, struct ril_utf_msg_t *data);
