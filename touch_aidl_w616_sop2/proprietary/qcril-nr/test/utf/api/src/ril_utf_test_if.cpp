...
void ril_utf_test_t::print_summary(void)
{
  ...
  if ( points_failed == 0 )
  {
    RIL_UTF_LOG_RESULT("     Test PASSED\n");
  }
  else
  {
    RIL_UTF_LOG_RESULT("     Test FAILED\n");
  }
  RIL_UTF_LOG_RESULT("--------\n");
}
...
void ril_utf_test_t::execute(void(*update_globals)(void))
{
  ...
  RIL_UTF_LOG_RESULT("--------\n");
  RIL_UTF_LOG_RESULT("     Test Name: %s\n", result.test_id.c_str());
 ...
 if (boot_case != NULL)
 {
    ...
    if (!(update_globals && is_boot_up))
    {
      ...
      if (clear_db)
      {
        ...
        int rc = sqlite3_open_v2("qcrilNr.db", &qcril_db_handle, SQLITE_OPEN_READWRITE, NULL);
        if (rc == SQLITE_OK)
        {
            char *zErrMsg = NULL;
...
/*=======================================================
  ...
  This is the test_case execution funciton for data ril utf

=========================================================*/
void ril_utf_test_t::execute_data(void)
{
  // Just pass an empty pointer to the execute function with argument
  execute_data(NULL);
}

void ril_utf_test_t::execute_data(void(*update_globals)(void))
{
  Testnode *test_step =  head;
  char outfile_name[200] = "";
  int expect_ret = QCRIL_UNITTTEST_NO_ERR;
...
int ril_utf_test_t::handle_inject_step( struct ril_utf_node_base_t* &test_step )
{
  ...
  case ril_utf_qdp_response:
      err_val = send_ril_utf_qdp_response( inject_ptr );
      break;
                                       ...
                                       struct ril_utf_msg_t *data )
{
  ...
  case ril_utf_dsi_response:
      ...
      break;
  case ril_utf_qdp_request:
      validate_qdp_request(exp_node, data);
      break;
  case ril_utf_qdp_response:
      validate_qdp_response(exp_node, data);
      break;
    default:
                                   ...
                                   struct ril_utf_msg_t *data)
{
  ...
  } else if (exp_node->get_msg_id() == ims_MsgId_UNSOL_PRE_ALERTING_CALL_INFO_AVAILABLE) {
    validate_ims_unsol_pre_alerting_call_info_ind(exp_node, data);
  } else if (exp_node->get_msg_id() == ims_MsgId_UNSOL_INCOMING_CALL_AUTO_REJECTED) {
    validate_ims_unsol_auto_call_rejection_ind2(exp_node, data);
  } else if (exp_node->get_msg_id() == ims_MsgId_UNSOL_CONFERENCE_CALL_STATE_COMPLETED) {
    exp_node->set_expect_status(met);
  } else if (exp_node->get_msg_id() == ims_MsgId_UNSOL_SMS_CALLBACK_MODE_CHANGED) {
    validate_ims_unsol_sms_callback_mode_changed(exp_node, data);
  } else if (exp_node->get_msg_id() == ims_MsgId_UNSOL_INCOMING_DTMF_START ||
             exp_node->get_msg_id() == ims_MsgId_UNSOL_INCOMING_DTMF_STOP) {
    validate_ims_unsol_incoming_dtmf(exp_node, data);
  } else if (exp_node->get_msg_id() == ims_MsgId_UNSOL_MULTI_SIM_VOICE_CAPABILITY_CHANGED ||
             exp_node->get_msg_id() == ims_MsgId_REQUEST_QUERY_MULTI_SIM_VOICE_CAPABILITY) {
    validate_ims_multi_sim_voice_capability(exp_node, data);
  } else if (exp_node->get_msg_id() == ims_MsgId_REQUEST_SEND_VOS_SUPPORT_STATUS) {
    validate_ims_vos_support_status(exp_node, data);
  } else if (exp_node->get_msg_id() == ims_MsgId_REQUEST_SEND_VOS_ACTION_INFO) {
    validate_ims_vos_action_info(exp_node, data);
  } else {
    ...
    struct ril_utf_msg_t *data)
{
  ...
  do
  {
    ...
    if (exp->has_callDetails)
    {
      ...
      if (exp->callDetails.isVosSupported != recv->callDetails.isVosSupported)
      {
        RIL_UTF_DEBUG("\n QMI_RIL_UTL: Error, isVosSupported doesnot match\n");
        validation_success = FALSE;
        break;
      }
    }
                                                     ...
                                                     struct ril_utf_msg_t *data)
{
  ...
  for (i = 0; recv[i] != NULL; ++i)
  {
    ...
    // callProgInfo
    if (recv[i]->callProgInfo.type != expect[i]->callProgInfo.type)
    {
      RIL_UTF_ESSENTIAL("QMI_RIL_UTF: For call %d: failed callProgInfo.type check: exp %d, recv %d",
                    i, expect[i]->callProgInfo.type, recv[i]->callProgInfo.type);
      validation_status = false;
      break;
    }
    if (recv[i]->callProgInfo.reasonCode != expect[i]->callProgInfo.reasonCode)
    {
      RIL_UTF_ESSENTIAL("QMI_RIL_UTF: For call %d: failed callProgInfo.reasonCode check: exp %d, recv %d",
                    i, expect[i]->callProgInfo.reasonCode, recv[i]->callProgInfo.reasonCode);
      validation_status = false;
      break;
    }
    if (strcmp(recv[i]->callProgInfo.reasonText, expect[i]->callProgInfo.reasonText) != 0)
    {
      RIL_UTF_ESSENTIAL("QMI_RIL_UTF: For call %d: failed callProgInfo.reasonText check: exp %s, recv %s",
                    i, expect[i]->callProgInfo.reasonText, recv[i]->callProgInfo.reasonText);
      validation_status = false;
      break;
    }

    //CRS
    if (recv[i]->isPreparatory != expect[i]->isPreparatory) {
        RIL_UTF_ESSENTIAL("QMI_RIL_UTF: For call %d: failed CRS isPreparatory check: exp %d, recv %d",
                i, expect[i]->isPreparatory, recv[i]->isPreparatory);
        validation_status = false;
        break;
    }
    if (recv[i]->has_crsData && expect[i]->has_crsData) {
        if (recv[i]->crsData.originalCallType != expect[i]->crsData.originalCallType) {
            RIL_UTF_ESSENTIAL("QMI_RIL_UTF: For call %d: failed CRS originalCallType check: exp %d, recv %d",
                    i, expect[i]->crsData.originalCallType, recv[i]->crsData.originalCallType);
            validation_status = false;
            break;
        }

        if (!(recv[i]->crsData.type & expect[i]->crsData.type)) {
            RIL_UTF_ESSENTIAL("QMI_RIL_UTF: For call %d: failed CRS crsType check: exp %d, recv %d",
                    i, expect[i]->crsData.type, recv[i]->crsData.type);
            validation_status = false;
            break;
        }
    }
    ...
    if (recv[i]->additionalCallInfo.additionalCode !=
        expect[i]->additionalCallInfo.additionalCode) {
      RIL_UTF_ESSENTIAL(
          "QMI_RIL_UTF: For call %d: failed msim additional call info code check: exp %d, recv "
          "%d",
          i, expect[i]->additionalCallInfo.additionalCode,
          recv[i]->additionalCallInfo.additionalCode);
      validation_status = false;
      break;
    }

    // Audio quality codec
    if (recv[i]->audioQuality.codec != expect[i]->audioQuality.codec) {
      RIL_UTF_ESSENTIAL(
          "QMI_RIL_UTF: For call %d: failed audio quality codec check: exp %d, recv "
          "%d",
          i, expect[i]->audioQuality.codec, recv[i]->audioQuality.codec);
      validation_status = false;
      break;
    }

    // Computed Audio quality
    if (recv[i]->audioQuality.computedAudioQuality != expect[i]->audioQuality.computedAudioQuality) {
      RIL_UTF_ESSENTIAL(
          "QMI_RIL_UTF: For call %d: failed computed audio quality check: exp %d, recv "
          "%d",
          i, expect[i]->audioQuality.computedAudioQuality,
          recv[i]->audioQuality.computedAudioQuality);
      validation_status = false;
      break;
    }
    ...
    if (recv[i]->has_callDetails && expect[i]->has_callDetails)
    {
      do {
        ...
        // Vos support
        if (expect[i]->callDetails.isVosSupported != recv[i]->callDetails.isVosSupported)
        {
          RIL_UTF_ESSENTIAL("\nQMI_RIL_UTF: For call %d: failed isVosSupported check", i);
          validation_status = false;
          break;
        }
      } while (FALSE);
                                   ...
                                   struct ril_utf_msg_t *data)
{
       ...
       (exp_node->get_payload() != NULL) )
  {
    ...
    if (exp_node->get_msg_id() == RIL_UNSOL_RESPONSE_CDMA_NEW_SMS)
    {
        // TODO validate
        exp_node->set_expect_status(met);
    }
    ...
    else if (exp_node->get_msg_id() == RIL_UNSOL_EMERGENCY_NUMBERS_LIST)
    {
        exp_node->set_expect_status(verify_ril_unsol_emergency_numbers_list(exp_node->get_payload(), data->payload));
    }
    else if ( exp_node->get_msg_id() == RIL_UNSOL_NITZ_TIME_RECEIVED )
    {
        exp_node->set_expect_status(verify_ril_unsol_NITZ(exp_node->get_payload(), data->payload));
    }
    else
    {
      if ( memcmp(exp_node->get_payload(), data->payload, data->payload_len) != 0 )
                                        ...
                                        struct ril_utf_msg_t *data)
{
  ...
  else if ( (exp_node->get_msg_id() == RIL_REQUEST_SEND_SMS) ||
            ( exp_node->get_msg_id() == RIL_REQUEST_IMS_SEND_SMS ) ||
            (exp_node->get_msg_id() == RIL_REQUEST_CDMA_SEND_SMS) ||
             (exp_node->get_msg_id() == RIL_REQUEST_CDMA_SEND_SMS_EXPECT_MORE))
  {
  ...
  else if (exp_node->get_msg_id() == RIL_REQUEST_GET_CELL_INFO_LIST)
  {
    verify_ril_get_cell_info_list(exp_node, data);
  }
...
void ril_utf_test_t::validate_qdp_request(ril_utf_node_base_t *exp_node, struct ril_utf_msg_t *data)
{
  RIL_UTF_DEBUG("\n QMI_RIL_UTF: Verifying QDP request");
  if ( (data->msg_id != exp_node->get_msg_id()) || ( (exp_node->is_verify_payload() == true && data->payload_len != exp_node->get_payload_len())) )
  {
    RIL_UTF_DEBUG("\n QMI_RIL_UTF: msg_id/payload_len not matching\n");
    exp_node->set_expect_status(not_met);
...
void ril_utf_test_t::validate_qdp_response(ril_utf_node_base_t *exp_node, struct ril_utf_msg_t *data)
{
  RIL_UTF_DEBUG("\n QMI_RIL_UTF: Verifying QDP response");
  if ( (data->msg_id != exp_node->get_msg_id()) || ( (exp_node->is_verify_payload() == true && data->payload_len != exp_node->get_payload_len())) )
  {
    RIL_UTF_DEBUG("\n QMI_RIL_UTF: msg_id/payload_len not matching\n");
    exp_node->set_expect_status(not_met);
...
bool ril_utf_test_t::is_same(const RIL_CellIdentityGsm_v12& expect,
        const RIL_CellIdentityGsm_v12& recv)
{
    auto ret = (!strncmp(expect.mcc, recv.mcc, MAX_RIL_MCC_MNC_SIZE)
            && !strncmp(expect.mnc, recv.mnc, MAX_RIL_MCC_MNC_SIZE));
    if (!ret)
    {
        RIL_UTF_DEBUG("\n QMI_RIL_UTF: Expected: mcc = %s, mnc = %s, Received: mcc = %s, mnc = %s",
                expect.mcc, expect.mnc, recv.mcc, recv.mnc);
    }
    return ret;
}

bool ril_utf_test_t::is_same(const RIL_CellIdentityWcdma_v12& expect,
        const RIL_CellIdentityWcdma_v12& recv)
{
    auto ret = (!strncmp(expect.mcc, recv.mcc, MAX_RIL_MCC_MNC_SIZE)
            && !strncmp(expect.mnc, recv.mnc, MAX_RIL_MCC_MNC_SIZE));
    if (!ret)
...
bool ril_utf_test_t::is_same(const RIL_CellIdentityLte_v12& expect,
        const RIL_CellIdentityLte_v12& recv)
{
    auto ret = (!strncmp(expect.mcc, recv.mcc, MAX_RIL_MCC_MNC_SIZE)
            && !strncmp(expect.mnc, recv.mnc, MAX_RIL_MCC_MNC_SIZE));
    if (!ret)
...
bool ril_utf_test_t::is_same(const RIL_CellIdentityTdscdma& expect,
        const RIL_CellIdentityTdscdma& recv)
{
    auto ret = (!strncmp(expect.mcc, recv.mcc, MAX_RIL_MCC_MNC_SIZE)
            && !strncmp(expect.mnc, recv.mnc, MAX_RIL_MCC_MNC_SIZE));
    if (!ret)
...
bool ril_utf_test_t::is_same(const RIL_CellIdentityNr& expect,
        const RIL_CellIdentityNr& recv)
{
    auto ret = (!strncmp(expect.mcc, recv.mcc, MAX_RIL_MCC_MNC_SIZE)
            && !strncmp(expect.mnc, recv.mnc, MAX_RIL_MCC_MNC_SIZE));
    if (!ret)
...
bool ril_utf_test_t::is_same(const RIL_CellIdentity_v16& expect,
        const RIL_CellIdentity_v16& recv)
{
    if (expect.cellInfoType != recv.cellInfoType)
    {
        RIL_UTF_DEBUG("\n QMI_RIL_UTF: Expected cell type %d, received cell type: %d",
                expect.cellInfoType, recv.cellInfoType);
        return false;
    }
    switch (expect.cellInfoType) {
    case RIL_CELL_INFO_TYPE_GSM:
        return is_same(expect.cellIdentityGsm, recv.cellIdentityGsm);
    case RIL_CELL_INFO_TYPE_LTE:
        return is_same(expect.cellIdentityLte, recv.cellIdentityLte);
    case RIL_CELL_INFO_TYPE_WCDMA:
        return is_same(expect.cellIdentityWcdma, recv.cellIdentityWcdma);
    case RIL_CELL_INFO_TYPE_TD_SCDMA:
        return is_same(expect.cellIdentityTdscdma, recv.cellIdentityTdscdma);
    case RIL_CELL_INFO_TYPE_NR:
        return is_same(expect.cellIdentityNr, recv.cellIdentityNr);
    default:
        return true;
    }
}
                                  ...
                                  struct ril_utf_msg_t *data)
{
  ...
  if (!is_same(expect->cellIdentity, recv->cellIdentity)) {
    exp_node->set_expect_status(not_met);
  }
}
...
void ril_utf_test_t::verify_ril_get_cell_info_list(ril_utf_node_base_t *exp_node, struct ril_utf_msg_t *data)
{
  enum ril_utf_expectation_t result = met;

  RIL_CellInfo_v12 *expect = (RIL_CellInfo_v12 *) exp_node->get_payload();
  RIL_CellInfo_v12 *recv = (RIL_CellInfo_v12 *) data->payload;

  do {
    if (data->error_val == RIL_E_SUCCESS) {
      if (exp_node->get_payload_len() != data->payload_len) {
        RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: expected len = %d, received len = %d\n",
                          exp_node->get_payload_len(), data->payload_len);
        result = not_met;
        break;
      }
      ...
      // For each cell info
      size_t num_cellinfo = (exp_node->get_payload_len() / sizeof(RIL_CellInfo_v12 *));
      for (size_t i = 0; i < num_cellinfo; ++i) {
        if (expect[0].cellInfoType != recv[0].cellInfoType) {
          RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: expected cellInfoType = %d, received cellInfoType = %d\n",
              expect[0].cellInfoType, recv[0].cellInfoType);
          result = not_met;
          break;
        }
      }
  ...
  exp_node->set_expect_status(result);
}
...
void ril_utf_test_t::print_received_message(ril_utf_msg_t* msg_ptr)
{
  switch ( msg_ptr->type )
  {
   ...
   case ril_utf_dsi_request:
   case ril_utf_dsi_response:
      print_dsi_message_name(msg_ptr->msg_id);
      RIL_UTF_HTML_LOG("     QMI_RIL_UTL: Message ID: %d, Type: %s", msg_ptr->msg_id, ril_utf_q_element_t_name[msg_ptr->type].name);
      print_msg_in_hex(msg_ptr->payload, msg_ptr->payload_len);
      break;
   case ril_utf_qdp_request:
   case ril_utf_qdp_response:
      print_qdp_message_name(msg_ptr->msg_id);
      RIL_UTF_HTML_LOG("     QMI_RIL_UTL: Message ID: %d, Type: %s", msg_ptr->msg_id, ril_utf_q_element_t_name[msg_ptr->type].name);
      print_msg_in_hex(msg_ptr->payload, msg_ptr->payload_len);
      break;
   default:
...
bool ril_utf_test_t::compare_pre_alerting_call_info(const ims_PreAlertingCallInfo& expected,
                                                    const ims_PreAlertingCallInfo& received) {
  if (expected.call_id != received.call_id) {
    RIL_UTF_DEBUG("\n QMI_RIL_UTF: call_id, expected = %d, received = %d", expected.call_id,
                  received.call_id);
    return false;
  }
  return compare_call_composer_info(expected.callComposerInfo, received.callComposerInfo) &&
         compare_ecnam_info(expected.ecnamInfo, received.ecnamInfo);
}

bool ril_utf_test_t::compare_ecnam_info(const ims_EcnamInfo& expected,
                                        const ims_EcnamInfo& received) {
  if (strncmp(expected.name, received.name, IMS_CALL_ECNAM_NAME_MAX_LEN)) {
    RIL_UTF_DEBUG("\n QMI_RIL_UTF: ecnam name, expected = %s, received = %s", expected.name,
                  received.name);
    return false;
  }
  if (strncmp(expected.icon_url, received.icon_url, IMS_CALL_ECNAM_ICON_URL_MAX_LEN)) {
    RIL_UTF_DEBUG("\n QMI_RIL_UTF: ecnam icon_url, expected = %s, received = %s", expected.icon_url,
                  received.icon_url);
    return false;
  }
  if (strncmp(expected.info_url, received.info_url, IMS_CALL_ECNAM_INFO_URL_MAX_LEN)) {
    RIL_UTF_DEBUG("\n QMI_RIL_UTF: ecnam info_url, expected = %s, received = %s", expected.info_url,
                  received.info_url);
    return false;
  }
  if (strncmp(expected.card_url, received.card_url, IMS_CALL_ECNAM_CARD_URL_MAX_LEN)) {
    RIL_UTF_DEBUG("\n QMI_RIL_UTF: ecnam card_url, expected = %s, received = %s", expected.card_url,
                  received.card_url);
    return false;
  }
  return true;
...
bool ril_utf_test_t::compare_call_composer_info(const ims_CallComposer& expected,
    ...
    if (expected.organization_len != received.organization_len) {
        RIL_UTF_DEBUG("\n QMI_RIL_UTF: organization_len, expected = %d, received = %d",
                expected.organization_len, received.organization_len);
        return false;
    }
    if (memcmp(expected.organization, received.organization,
            sizeof(uint16_t)*expected.organization_len)) {
        RIL_UTF_DEBUG("\n QMI_RIL_UTF: organization content does not match");
        print_msg_in_hex(expected.organization, sizeof(uint16_t)*expected.organization_len);
        print_msg_in_hex(received.organization, sizeof(uint16_t)*received.organization_len);
        return false;
    }
    return true;
}

void ril_utf_test_t::validate_ims_unsol_pre_alerting_call_info_ind(ril_utf_node_base_t* exp_node,
                                                                   struct ril_utf_msg_t* data) {
  auto exp = static_cast<ims_PreAlertingCallInfo*>(exp_node->get_payload());
  auto recv = static_cast<ims_PreAlertingCallInfo*>(data->payload);

  if (compare_pre_alerting_call_info(*exp, *recv)) {
    exp_node->set_expect_status(met);
  } else {
    exp_node->set_expect_status(not_met);
  }
...
void ril_utf_test_t::validate_ims_unsol_auto_call_rejection_ind2(ril_utf_node_base_t* exp_node,
                                                                 struct ril_utf_msg_t* data) {
  auto exp = static_cast<ims_AutoCallRejectionInfo2*>(exp_node->get_payload());
  auto recv = static_cast<ims_AutoCallRejectionInfo2*>(data->payload);

  if (compare_call_rejection_info(exp->rej, recv->rej) &&
      compare_call_composer_info(exp->composer, recv->composer) &&
      compare_ecnam_info(exp->ecnamInfo, recv->ecnamInfo)) {
    exp_node->set_expect_status(met);
  } else {
    exp_node->set_expect_status(not_met);
  }
...
void ril_utf_test_t::validate_ims_unsol_sms_callback_mode_changed(
    ril_utf_node_base_t* exp_node, struct ril_utf_msg_t* data) {
  int validation_success = TRUE;
  ims_SmsCallbackModeStatusInfo* exp = (ims_SmsCallbackModeStatusInfo*)exp_node->get_payload();
  ims_SmsCallbackModeStatusInfo* recv = (ims_SmsCallbackModeStatusInfo*)data->payload;

  do {
    if (exp->smsCallbackModeStatus != recv->smsCallbackModeStatus) {
      RIL_UTF_DEBUG("\n QMI_RIL_UTF: smsCallbackModeStatus exp = %d and recv = %d\n",
                    exp->smsCallbackModeStatus, recv->smsCallbackModeStatus);
      validation_success = FALSE;
      break;
    }
  } while (0);
...
void ril_utf_test_t::validate_ims_unsol_incoming_dtmf(ril_utf_node_base_t* exp_node,
                                                      struct ril_utf_msg_t* data) {
  int validation_success = TRUE;
  ims_Dtmf* exp = (ims_Dtmf*)exp_node->get_payload();
  ims_Dtmf* recv = (ims_Dtmf*)data->payload;

  char* e_str = (char*)exp->dtmf.arg;
  char* r_str = (char*)recv->dtmf.arg;

  do {
    if (e_str != NULL && r_str != NULL && (strcmp(e_str, r_str) != 0)) {
      RIL_UTF_DEBUG(
          "\n QMI_RIL_UTL: unsol_incoming_dtmf ; expected = \"%s\" but received = \"%s\"\n", e_str,
          r_str);
      validation_success = FALSE;
      break;
    }
    if (exp->callId != recv->callId) {
      RIL_UTF_DEBUG("\n QMI_RIL_UTF: unsol_incoming_dtmf exp = %d and recv = %d\n", exp->callId,
                    recv->callId);
      validation_success = FALSE;
      break;
    }
...
void ril_utf_test_t::validate_ims_multi_sim_voice_capability(ril_utf_node_base_t* exp_node,
                                                             struct ril_utf_msg_t* data) {
  int validation_success = TRUE;
  ims_MultiSimVoiceCapability* exp = (ims_MultiSimVoiceCapability*)exp_node->get_payload();
  ims_MultiSimVoiceCapability* recv = (ims_MultiSimVoiceCapability*)data->payload;

  do {
    if (exp && recv && exp->voiceCapability != recv->voiceCapability) {
      RIL_UTF_DEBUG(
          "\n QMI_RIL_UTF: validate_ims_unsol_multi_sim_voice_capability_changed exp = %d and recv "
          "= %d\n",
          exp->voiceCapability, recv->voiceCapability);
      validation_success = FALSE;
      break;
    }
  } while (0);
...
void ril_utf_test_t::validate_ims_vos_support_status(ril_utf_node_base_t* exp_node,
                                                     struct ril_utf_msg_t* data) {
  int validation_success = TRUE;
  ims_VosSupportStatus* exp = (ims_VosSupportStatus*)exp_node->get_payload();
  ims_VosSupportStatus* recv = (ims_VosSupportStatus*)data->payload;
  RIL_UTF_DEBUG("\n QMI_RIL_UTL: validate_ims_vos_support_status");

  do {
    if (exp && recv && exp->isVosSupported != recv->isVosSupported) {
      RIL_UTF_DEBUG(
          "\n QMI_RIL_UTF: validate_ims_vos_support_status exp = %d and recv "
          "= %d\n",
          exp->isVosSupported, recv->isVosSupported);
      validation_success = FALSE;
      break;
    }
  } while (0);
...
void ril_utf_test_t::validate_ims_vos_action_info(ril_utf_node_base_t* exp_node,
                                                  struct ril_utf_msg_t* data) {
  int validation_success = TRUE;
  ims_VosActionInfo* exp = (ims_VosActionInfo*)exp_node->get_payload();
  ims_VosActionInfo* recv = (ims_VosActionInfo*)data->payload;

  if (exp != NULL)
  {
    if (exp->has_vosMoveInfo != recv->has_vosMoveInfo ||
        !compare_vos_move_info(exp->vosMoveInfo, recv->vosMoveInfo)) {
      RIL_UTF_DEBUG(
          "\n QMI_RIL_UTF: Expected has_vosMoveInfo: %d Received has_vosMoveInfo: %d",
          exp->has_vosMoveInfo, recv->vosMoveInfo);
      RIL_UTF_DEBUG("\n QMI_RIL_UTL: Error, messages do not match(vosMoveInfo)");
      validation_success = FALSE;
    }
    if (exp->has_vosTouchInfo != recv->has_vosTouchInfo ||
        !compare_vos_touch_info(exp->vosTouchInfo, recv->vosTouchInfo)) {
      RIL_UTF_DEBUG(
          "\n QMI_RIL_UTF: Expected vosTouchInfo: %d Received vosTouchInfo: %d",
          exp->has_vosTouchInfo, recv->vosTouchInfo);
      RIL_UTF_DEBUG("\n QMI_RIL_UTL: Error, messages do not match(vosTouchInfo)");
      validation_success = FALSE;
    }
  }
...
bool ril_utf_test_t::compare_vos_move_info(const ims_VosMoveInfo& expected,
                                           const ims_VosMoveInfo& received) {
  if (expected.has_start != received.has_start ||
      !compare_coordinate2d(expected.start, received.start) ||
      expected.has_end != received.has_end ||
      !compare_coordinate2d(expected.end, received.end)) {
    RIL_UTF_DEBUG(
        "\n QMI_RIL_UTF: Expected has_start: %d has_end: %d "
        "Received has_start: %d has_end: %d",
        expected.has_start, expected.has_end,
        received.has_start, received.has_end);
    return false;
  }
  return true;
}

bool ril_utf_test_t::compare_vos_touch_info(const ims_VosTouchInfo& expected,
                                            const ims_VosTouchInfo& received) {
  if (expected.has_touch != received.has_touch ||
      !compare_coordinate2d(expected.touch, received.touch) ||
      expected.touchDuration != received.touchDuration) {
    RIL_UTF_DEBUG(
        "\n QMI_RIL_UTF: Expected has_touch: %d touchDuration: %d "
        "Received has_end: %d touchDuration: %d",
        expected.has_touch, expected.touchDuration,
        received.has_touch, received.touchDuration);
    return false;
  }
  return true;
}

bool ril_utf_test_t::compare_coordinate2d(const ims_Coordinate2D& expected,
                                           const ims_Coordinate2D& received) {
  if (expected.x != received.x || expected.y != received.y) {
    RIL_UTF_DEBUG(
        "\n QMI_RIL_UTF: Expected x: %d y: %d Received x: %d y: %d",
