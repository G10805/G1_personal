/******************************************************************************
#  Copyright (c) 2013, 2019, 2022, 2023 Qualcomm Technologies, Inc.
#  All Rights Reserved.
...
void ril_utf_ims_if::set_ims_request_answer_rtt_mode(ims_RttMode rttMode)
{
  ims_Answer *ptr = (ims_Answer*) payload;
  ptr->has_rttMode = TRUE;
  ptr->rttMode = rttMode;
}
...
void ril_utf_ims_if::set_ims_request_modify_call_confirm_rtt_mode(ims_RttMode rttMode)
{
  ims_CallModify *ptr = (ims_CallModify*) payload;
...
void ril_utf_ims_if::set_ims_unsol_modify_call_call_details_rtt_mode(ims_RttMode rttMode)
{
  ims_CallModify *ptr = (ims_CallModify*) payload;
...
void ril_utf_ims_if::set_ims_unsol_modify_call_call_details_vos_support(bool isVosSupported)
{
  ims_CallModify *ptr = (ims_CallModify*) payload;

  ptr->has_callDetails = TRUE;
  ptr->callDetails.isVosSupported = isVosSupported;
}
...
/*
    ims_MsgId_UNSOL_INCOMING_CALL_AUTO_REJECTED
*/
ims_AutoCallRejectionInfo2* ril_utf_ims_if::update_ims_unsol_call_auto_rejection_ind2(
    ims_CallType callType, ims_CallFailCause autoRejectionCause, uint16_t sipErrorCode,
    ims_VerstatVerificationStatus verificationStatus, const char* number) {
  payload_type = ril_utf_ims_indication;
  msg_id = ims_MsgId_UNSOL_INCOMING_CALL_AUTO_REJECTED;

  payload = malloc(sizeof(ims_AutoCallRejectionInfo2));
  assert(payload != nullptr);
  payload_len = sizeof(ims_AutoCallRejectionInfo2);
  memset(payload, 0x00, payload_len);

  ims_AutoCallRejectionInfo2* ptr = (ims_AutoCallRejectionInfo2*)payload;
  ptr->rej.callType = callType;
  ptr->rej.autoRejectionCause = autoRejectionCause;
  ptr->rej.sipErrorCode = sipErrorCode;
  ptr->rej.verificationStatus = verificationStatus;

  ptr->rej.hasNumber = TRUE;
  ptr->rej.number.arg = qcril_malloc(strlen(number) + 1);
  assert(ptr->rej.number.arg);
  strlcpy((char*)ptr->rej.number.arg, number, strlen(number) + 1);

  return static_cast<ims_AutoCallRejectionInfo2*>(payload);
}
...
/*========================================================
  ims_MsgId_UNSOL_CONFERENCE_CALL_STATE_COMPLETED
========================================================*/
void ril_utf_ims_if::update_default_ims_unsol_conference_call_state_completed()
{
  payload = nullptr;
  payload_len = 0;
  payload_type = ril_utf_ims_indication;
  msg_id = ims_MsgId_UNSOL_CONFERENCE_CALL_STATE_COMPLETED;
}

/*========================================================
  ims_MsgId_UNSOL_SMS_CALLBACK_MODE_CHANGED
========================================================*/
void ril_utf_ims_if::update_default_ims_unsol_sms_callback_mode_changed(
    enum ims_SmsCallbackModeStatus smsCallbackModeStatus)
{
  payload = malloc(sizeof(ims_SmsCallbackModeStatus));
  ims_SmsCallbackModeStatusInfo *info = (ims_SmsCallbackModeStatusInfo *)payload;
  payload_len = sizeof(ims_SmsCallbackModeStatusInfo);
  memset(payload, 0x00, payload_len);
  payload_type = ril_utf_ims_indication;
  msg_id = ims_MsgId_UNSOL_SMS_CALLBACK_MODE_CHANGED;
  info->smsCallbackModeStatus = smsCallbackModeStatus;
}

/*========================================================
  ims_MsgId_UNSOL_C_IWLAN_EXIT_NOTIFICATION
========================================================*/
void ril_utf_ims_if::update_default_ims_unsol_c_iwlan_notification(
    ims_CIWlan_Notification show)
{
  payload = malloc(sizeof(ims_CiWlanNotificationInfo));
  ims_CiWlanNotificationInfo *info = (ims_CiWlanNotificationInfo*)payload;
  payload_len = sizeof(ims_CIWlan_Notification);
  memset(payload, 0x00, payload_len);
  payload_type = ril_utf_ims_indication;
  msg_id = ims_MsgId_UNSOL_C_IWLAN_EXIT_NOTIFICATION;
  info->notificationInfo = show;
}
...
ims_SmsSendResponse* ril_utf_ims_if::update_default_ims_send_sms_response()
{
    payload = malloc(sizeof(ims_SmsSendResponse));
    payload_len = sizeof(ims_SmsSendResponse);
    memset(payload, 0x00, payload_len);
    payload_type = ril_utf_ims_response;
    msg_id = ims_MsgId_REQUEST_SEND_SMS;
    return static_cast<ims_SmsSendResponse*>(payload);
}
...
/*
   ims_MsgId_UNSOL_PRE_ALERTING_CALL_INFO_AVAILABLE
*/
ims_PreAlertingCallInfo* ril_utf_ims_if::update_default_ims_unsol_pre_alerting_call_info() {
  payload_type = ril_utf_ims_indication;
  msg_id = ims_MsgId_UNSOL_PRE_ALERTING_CALL_INFO_AVAILABLE;

  payload = malloc(sizeof(ims_PreAlertingCallInfo));
  assert(payload != nullptr);
  payload_len = sizeof(ims_PreAlertingCallInfo);
  memset(payload, 0x00, payload_len);

  return static_cast<ims_PreAlertingCallInfo*>(payload);
}

/*
    ims_MsgId_UNSOL_INCOMING_DTMF_START
*/
ims_Dtmf* ril_utf_ims_if::update_default_ims_unsol_incoming_dtmf_start() {
  payload_type = ril_utf_ims_indication;
  msg_id = ims_MsgId_UNSOL_INCOMING_DTMF_START;

  payload = malloc(sizeof(ims_Dtmf));
  assert(payload != nullptr);
  payload_len = sizeof(ims_Dtmf);
  memset(payload, 0x00, payload_len);

  return static_cast<ims_Dtmf*>(payload);
}

/*
    ims_MsgId_UNSOL_INCOMING_DTMF_STOP
*/
ims_Dtmf* ril_utf_ims_if::update_default_ims_unsol_incoming_dtmf_stop() {
  payload_type = ril_utf_ims_indication;
  msg_id = ims_MsgId_UNSOL_INCOMING_DTMF_STOP;
...
/*
    ims_MsgId_UNSOL_MULTI_SIM_VOICE_CAPABILITY_CHANGED
*/
void ril_utf_ims_if::update_default_ims_unsol_multi_sim_voice_capability_changed(
    ims_Voice_Capability voiceCapability) {
  payload_type = ril_utf_ims_indication;
  msg_id = ims_MsgId_UNSOL_MULTI_SIM_VOICE_CAPABILITY_CHANGED;

  payload = malloc(sizeof(ims_MultiSimVoiceCapability));
  assert(payload != nullptr);
  payload_len = sizeof(ims_MultiSimVoiceCapability);
  memset(payload, 0x00, payload_len);

  ims_MultiSimVoiceCapability* ptr = (ims_MultiSimVoiceCapability*)payload;
  ptr->voiceCapability = voiceCapability;
}

/*
    ims_MsgId_REQUEST_QUERY_MULTI_SIM_VOICE_CAPABILITY
*/
void ril_utf_ims_if::update_default_ims_query_multi_sim_voice_capability() {
  payload = NULL;
  payload_len = 0;
  payload_type = ril_utf_ims_request;
  msg_id = ims_MsgId_REQUEST_QUERY_MULTI_SIM_VOICE_CAPABILITY;
}

ims_MultiSimVoiceCapability*
ril_utf_ims_if::update_default_ims_query_multi_sim_voice_capability_resp() {
  payload = (ims_MultiSimVoiceCapability*)malloc(sizeof(ims_MultiSimVoiceCapability));
  assert(payload != nullptr);
  payload_len = sizeof(ims_MultiSimVoiceCapability);
  memset(payload, 0x00, payload_len);
  payload_type = ril_utf_ims_response;
...
void ril_utf_ims_if::set_ims_request_ims_query_multi_sim_voice_capability_resp_voice_capability(
    ims_Voice_Capability voiceCapability) {
  ims_MultiSimVoiceCapability* ptr = (ims_MultiSimVoiceCapability*)payload;
  ptr->voiceCapability = voiceCapability;
}
...
/*
    ims_MsgId_REQUEST_SEND_VOS_SUPPORT_STATUS
*/
void ril_utf_ims_if::update_default_ims_send_vos_support_status(void) {
  payload = (void*)malloc(sizeof(ims_VosSupportStatus));
  payload_len = sizeof(ims_VosSupportStatus);
  memset(payload, 0x00, payload_len);
  payload_type = ril_utf_ims_request;
  msg_id = ims_MsgId_REQUEST_SEND_VOS_SUPPORT_STATUS;
}

void ril_utf_ims_if::set_ims_request_send_vos_support_status_support_status(
    bool isVosSupported) {
  ims_VosSupportStatus* ptr = (ims_VosSupportStatus*)payload;
  ptr->isVosSupported = isVosSupported;
}

void ril_utf_ims_if::update_default_ims_send_vos_support_status_resp(void) {
  payload = NULL;
  payload_len = 0;
  payload_type = ril_utf_ims_response;
  msg_id = ims_MsgId_REQUEST_SEND_VOS_SUPPORT_STATUS;
...
/*
    ims_MsgId_REQUEST_SEND_VOS_ACTION_INFO
*/
void ril_utf_ims_if::update_default_ims_send_vos_action_info(void) {
  payload = (void*)malloc(sizeof(ims_VosActionInfo));
  payload_len = sizeof(ims_VosActionInfo);
  memset(payload, 0x00, payload_len);
  payload_type = ril_utf_ims_request;
  msg_id = ims_MsgId_REQUEST_SEND_VOS_ACTION_INFO;
}

void ril_utf_ims_if::set_ims_request_send_vos_move_info_move_info(int16_t startx, int16_t starty,
                                                                  int16_t endx, int16_t endy) {
  ims_VosActionInfo* ptr = (ims_VosActionInfo*)payload;
  ptr->has_vosMoveInfo = TRUE;
  ptr->vosMoveInfo.has_start = TRUE;
  ptr->vosMoveInfo.start.x = startx;
  ptr->vosMoveInfo.start.y = starty;
  ptr->vosMoveInfo.has_end = TRUE;
  ptr->vosMoveInfo.end.x = endx;
  ptr->vosMoveInfo.end.y = endy;
}

void ril_utf_ims_if::set_ims_request_send_vos_touch_info_touch_info(int16_t touchx, int16_t touchy,
                                                                    uint32_t touchDuration) {
  ims_VosActionInfo* ptr = (ims_VosActionInfo*)payload;
  ptr->has_vosTouchInfo = TRUE;
  ptr->vosTouchInfo.has_touch = TRUE;
  ptr->vosTouchInfo.touch.x = touchx;
  ptr->vosTouchInfo.touch.y = touchy;
  ptr->vosTouchInfo.touchDuration = touchDuration;
}

void ril_utf_ims_if::update_default_ims_send_vos_action_info_resp(void) {
  payload = NULL;
  payload_len = 0;
  payload_type = ril_utf_ims_response;
  msg_id = ims_MsgId_REQUEST_SEND_VOS_ACTION_INFO;
