...
========================================================*/
void ril_utf_androidTel_if::update_default_ril_unsol_sib9_new_msg(char *ascii_hex_string)
{
  payload = malloc(strlen(ascii_hex_string) +1);
  payload_len = strlen(ascii_hex_string)+1;
  memset(payload, 0, payload_len);
  strlcpy((char*)payload, ascii_hex_string, strlen(ascii_hex_string)+1);
  payload_type = ril_utf_ril_unsol_response;
  msg_id = RIL_UNSOL_NITZ_TIME_RECEIVED ;
}

/*========================================================
  RIL_UNSOL_RESPONSE_CDMA_NEW_SMS
========================================================*/
RIL_CDMA_SMS_Message *ril_utf_androidTel_if::update_default_ril_unsol_response_cdma_new_sms()
{
  payload_len = sizeof(RIL_CDMA_SMS_Message);
  payload = malloc(payload_len);
  memset(payload, 0, payload_len);
  payload_type = ril_utf_ril_unsol_response;
  msg_id = RIL_UNSOL_RESPONSE_CDMA_NEW_SMS;

  RIL_CDMA_SMS_Message *temp = (RIL_CDMA_SMS_Message *)payload;
  return temp;
}
...
========================================================*/
void ril_utf_androidTel_if::update_default_ril_request_radio_power(int on, int emergency, int preferredSub)
{
  ...
  tmp->power = on;
  tmp->reasonEmergency = emergency;
  tmp->isPreferredSub = preferredSub;
...
========================================================*/
RIL_CDMA_SMS_Message* ril_utf_androidTel_if::update_default_ril_request_cdma_send_sms_expect_more()
{
  payload = malloc_ril_request_cdma_send_sms();
  payload_len = sizeof(RIL_CDMA_SMS_Message) ;
  memset(payload, 0x0, payload_len);
  payload_type = ril_utf_ril_request;
  msg_id = RIL_REQUEST_CDMA_SEND_SMS_EXPECT_MORE;

  RIL_CDMA_SMS_Message* ptr = (RIL_CDMA_SMS_Message*) payload;
  return ptr;
}
RIL_SMS_Response* ril_utf_androidTel_if::update_default_ril_request_cdma_send_sms_expect_more_resp()
{
  payload = malloc_ril_request_cdma_send_sms_resp();
  payload_len = sizeof(RIL_SMS_Response) ;
  memset(payload, 0x0, payload_len);
...
void ril_utf_androidTel_if::set_ril_request_get_current_calls_resp_redirected_number(
        int call_slot, char *number)
{
  RIL_Call** ptr = (RIL_Call**) payload;
  ptr[call_slot]->redirNum = (char *) malloc(strlen(number)+1);
  strlcpy(ptr[call_slot]->redirNum, number, strlen(number)+1);
}
...
RIL_VoiceRegistrationStateResponse* ril_utf_androidTel_if::update_default_ril_request_registration_state_resp()
{
  ...
  memset(payload, 0, sizeof(RIL_VoiceRegistrationStateResponse));
...
*/
void ril_utf_androidTel_if::update_default_ril_request_get_system_selection_channels(void)
{
  payload = NULL;
  payload_len = 0;
  payload_type = ril_utf_ril_request;
  msg_id = RIL_REQUEST_GET_SYSTEM_SELECTION_CHANNELS;
}

RIL_SysSelChannels* ril_utf_androidTel_if::update_default_ril_request_get_system_selection_channels_resp()
{
  payload = malloc(sizeof(RIL_SysSelChannels));
  payload_len = sizeof(RIL_SysSelChannels);
  memset(payload, 0, payload_len);
  payload_type = ril_utf_ril_response;
  msg_id = RIL_REQUEST_GET_SYSTEM_SELECTION_CHANNELS;
  RIL_SysSelChannels *ptr = (RIL_SysSelChannels*)payload;
  return ptr;
}
...
*/
ril_request_set_sys_channels_t *ril_utf_androidTel_if::update_default_ril_request_set_system_selection_channels(ril_request_set_sys_channels_t *req)
{
  payload = malloc(sizeof(ril_request_set_sys_channels_t));
  payload_len = sizeof(ril_request_set_sys_channels_t);
  memset(payload, 0x0, payload_len);
  payload_type = ril_utf_ril_request;
  msg_id = RIL_REQUEST_SET_SYSTEM_SELECTION_CHANNELS;
  if (req) {
    memcpy(payload, req, payload_len);
  }
  ril_request_set_sys_channels_t *ptr = (ril_request_set_sys_channels_t*) payload;
  return ptr;
}
void ril_utf_androidTel_if::update_default_ril_request_set_system_selection_channels_resp()
{
  payload = NULL;
  payload_len = 0;
  payload_type = ril_utf_ril_response;
