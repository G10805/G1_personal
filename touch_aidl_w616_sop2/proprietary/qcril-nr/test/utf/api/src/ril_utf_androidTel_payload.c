...
void free_ril_request_get_current_calls_resp(RIL_Call** call_list, int len)
{
  ...
  for (x = 0; x < num_calls; ++x)
  {
    ...
    if (call_list[x]->redirNum != NULL)
    {
      free(call_list[x]->redirNum);
    }
...
enum ril_utf_expectation_t verify_ril_get_current_calls(void *exp_node, int exp_len, void *data, int data_len)
{
  ...
  for (i = 0; i < num_calls; ++i)
  {
    if ( expect[i]->state != recv[i]->state ||
         ...
         expect[i]->redirNumPresentation != recv[i]->redirNumPresentation ||
    ...
    if ( (expect[i]->redirNum != NULL) &&
         (recv[i]->redirNum == NULL || strcmp(expect[i]->redirNum, recv[i]->redirNum) != 0) )
    {
      RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: For Call id %d", expect[i]->index);
      RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: For Received Call id %d", recv[i]->index);
      RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: Expected number: %s", expect[i]->redirNum);
      print_msg_in_hex( expect[i]->redirNum, strlen(expect[i]->redirNum));
      if (recv[i]->redirNum != NULL) {
          RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: Recvd number: %s", recv[i]->redirNum);
          print_msg_in_hex( recv[i]->redirNum, strlen(recv[i]->redirNum));
      } else {
          RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: Recvd number is null!");
      }
      result = not_met;
      break;
    }
...
enum ril_utf_expectation_t verify_ril_unsol_NITZ(void *exp_node, void *data)
{
    enum ril_utf_expectation_t result = met;
    ...
    if (expect && recv && (strcmp(expect, recv) != 0))
    {

       RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: expect = %s\n", expect);
       RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: recv = %s\n", recv);
       result = not_met;
...
enum ril_utf_expectation_t verify_ril_unsol_emergency_numbers_list(void *exp_node, void *data)
{
  enum ril_utf_expectation_t result = met;

  ril_unsol_emergency_list_t * expect = (ril_unsol_emergency_list_t*) exp_node;
  ril_unsol_emergency_list_t * recv = (ril_unsol_emergency_list_t*) data;

  if(expect->ecc_list.emergency_list_length != recv->ecc_list.emergency_list_length)
  {
    RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch emergency_list_length: exp: %d, recv: %d",
        expect->ecc_list.emergency_list_length, recv->ecc_list.emergency_list_length);
    result = not_met;
    return result;
  }

  for(int i=0; i<expect->ecc_list.emergency_list_length; i++)
  {
    int j = 0;
    while( (j<expect->ecc_list.emergency_list_length) &&
        (strcmp(expect->ecc_list.emergency_numbers[i].number,
                recv->ecc_list.emergency_numbers[j].number) != 0) )
    {
      j++;
    }
    if(j == expect->ecc_list.emergency_list_length)
    {
      RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: ecc number %s not found in received payload",
          expect->ecc_list.emergency_numbers[i].number);
    ...
    {
      if(strcmp(expect->ecc_list.emergency_numbers[i].mcc, recv->ecc_list.emergency_numbers[j].mcc) != 0)
      {
         RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch mcc for emergency number %s: "
             "exp mcc: %s, recv mcc: %s", expect->ecc_list.emergency_numbers[i].number,
             expect->ecc_list.emergency_numbers[i].mcc, recv->ecc_list.emergency_numbers[j].mcc);
         result = not_met;
         return result;
      }
      else if(strcmp(expect->ecc_list.emergency_numbers[i].mnc, recv->ecc_list.emergency_numbers[j].mnc) != 0)
      {
         RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch mnc for emergency number %s: "
             "exp mnc: %s, recv mnc: %s", expect->ecc_list.emergency_numbers[i].number,
             expect->ecc_list.emergency_numbers[i].mnc, recv->ecc_list.emergency_numbers[j].mnc);
         result = not_met;
         return result;
      }
      else if(expect->ecc_list.emergency_numbers[i].category != recv->ecc_list.emergency_numbers[j].category)
      {
         RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch category for emergency number %s: "
             "exp category: %d, recv category: %d", expect->ecc_list.emergency_numbers[i].number,
             expect->ecc_list.emergency_numbers[i].category, recv->ecc_list.emergency_numbers[j].category);
         result = not_met;
         return result;
      }
      else if(expect->ecc_list.emergency_numbers[i].sources != recv->ecc_list.emergency_numbers[j].sources)
      {
         RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch sources for emergency number %s: "
             "exp sources: %d, recv sources: %d", expect->ecc_list.emergency_numbers[i].number,
             expect->ecc_list.emergency_numbers[i].sources, recv->ecc_list.emergency_numbers[j].sources);
         result = not_met;
         return result;
      }
      else if(expect->ecc_list.emergency_numbers[i].no_of_urns != recv->ecc_list.emergency_numbers[j].no_of_urns)
      {
         RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch no_of_urns for emergency number %s: "
             "exp no_of_urns: %d, recv no_of_urns: %d", expect->ecc_list.emergency_numbers[i].number,
             expect->ecc_list.emergency_numbers[i].no_of_urns, recv->ecc_list.emergency_numbers[j].no_of_urns);
         result = not_met;
         return result;
      }
      else
      {
        for(int k=0; k<expect->ecc_list.emergency_numbers[i].no_of_urns; k++)
        {
          if(strcmp(expect->ecc_list.emergency_numbers[i].urn[k], recv->ecc_list.emergency_numbers[j].urn[k]) != 0)
          {
            RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch urn for emergency number %s: "
                "exp urn: %s, recv urn: %s", expect->ecc_list.emergency_numbers[i].number,
                expect->ecc_list.emergency_numbers[i].urn[k], recv->ecc_list.emergency_numbers[j].urn[k]);
            result = not_met;
            return result;
          }
        }
