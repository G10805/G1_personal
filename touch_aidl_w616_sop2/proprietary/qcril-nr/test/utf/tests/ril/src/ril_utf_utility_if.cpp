...
void util_ril_utf_fill_qmi_mcc_mnc(char (&qmi_arr)[NAS_MCC_MNC_MAX_V01],
        const char* str)
{
    /* as per QMI spec, unused byte is set to 0xFF */
    memset(qmi_arr, 0xFF, NAS_MCC_MNC_MAX_V01);
    for (size_t idx = 0; idx < NAS_MCC_MNC_MAX_V01 && *str != '\0'; str++)
        qmi_arr[idx++] = *str;
}
...
void util_voice_registration_state_for_wcdma_success(Testcase &t, QMIRILTestnode &QMI_node, AndroidTelTestnode &RIL_node, OEMTestnode &OEM_node, int rat, int reg_state, nas_service_status_enum_type_v01 srv_status, nas_true_service_status_enum_type_v01 true_srv_status, nas_service_domain_enum_type_v01 domain, nas_roam_status_enum_type_v01 roam_status, uint8_t reg_reject_info_valid, nas_service_domain_enum_type_v01 reject_srv_domain, uint8_t rej_cause, uint8_t net_id_valid, const char *mcc, const char *mnc, uint8_t csg_info_valid, uint32_t cid, uint32_t cid_name_len, uint16_t *cid_name, bool is_data_rat_change)
{
    ...
    {
        util_ril_utf_fill_qmi_mcc_mnc(
                wcdma_sys_info_resp->threegpp_specific_sys_info.network_id.mcc, mcc);
        util_ril_utf_fill_qmi_mcc_mnc(
                wcdma_sys_info_resp->threegpp_specific_sys_info.network_id.mnc, mnc);
    }
    ...
    if (reg_state == RIL_VAL_REG_REGISTERED_HOME_NET
            || reg_state == RIL_VAL_REG_REGISTERED_ROAMING
            || reg_state == RIL_VAL_REG_REGISTRATION_DENIED_LTD_SRV
            || reg_state == RIL_VAL_REG_NOT_REGISTERED_SEARCHING_LTD_SRV
            || srv_status == NAS_SYS_SRV_STATUS_SRV_V01)
    {
        if (rat == RADIO_TECH_UMTS)
        {
            reg_state_resp->cellIdentity.cellInfoType = RIL_CELL_INFO_TYPE_WCDMA;
            if (mcc != nullptr && mnc != nullptr)
            {
                strlcpy(reg_state_resp->cellIdentity.cellIdentityWcdma.mcc, mcc, MAX_RIL_MCC_MNC_SIZE);
                strlcpy(reg_state_resp->cellIdentity.cellIdentityWcdma.mnc, mnc, MAX_RIL_MCC_MNC_SIZE);
            }
        }
    }
    memset(tags,'\0', MAX_TAG_SIZE);
...
uint8_t net_id_valid, const char *mcc, const char *mnc, bool is_data_rat_change)
{
    ...
    {
        util_ril_utf_fill_qmi_mcc_mnc(
            gsm_sys_info_resp->threegpp_specific_sys_info.network_id.mcc, mcc);
        util_ril_utf_fill_qmi_mcc_mnc(
            gsm_sys_info_resp->threegpp_specific_sys_info.network_id.mnc, mnc);
    }
    memset(tags,'\0', MAX_TAG_SIZE);
    ...
    if (reg_state == RIL_VAL_REG_REGISTERED_HOME_NET
            ...
            || srv_status == NAS_SYS_SRV_STATUS_SRV_V01)
    {
        if (rat == RADIO_TECH_GSM)
        {
            reg_state_resp->cellIdentity.cellInfoType = RIL_CELL_INFO_TYPE_GSM;
            if (mcc != nullptr && mnc != nullptr)
            {
                strlcpy(reg_state_resp->cellIdentity.cellIdentityGsm.mcc, mcc, MAX_RIL_MCC_MNC_SIZE);
                strlcpy(reg_state_resp->cellIdentity.cellIdentityGsm.mnc, mnc, MAX_RIL_MCC_MNC_SIZE);
            }
        }
    }
    memset(tags,'\0', MAX_TAG_SIZE);
...
void util_voice_registration_state_for_tdscdma_success(Testcase &t, QMIRILTestnode &QMI_node, AndroidTelTestnode &RIL_node, int rat, int reg_state, nas_service_status_enum_type_v01 srv_status, nas_true_service_status_enum_type_v01 true_srv_status, nas_service_domain_enum_type_v01 domain, nas_roam_status_enum_type_v01 roam_status, uint8_t reg_reject_info_valid, nas_service_domain_enum_type_v01 reject_srv_domain, uint8_t rej_cause, bool is_data_rat_change)
{
    ...
    if (reg_state == RIL_VAL_REG_REGISTERED_HOME_NET
            ...
            || srv_status == NAS_SYS_SRV_STATUS_SRV_V01)
    {
        if (rat == RADIO_TECH_TD_SCDMA)
        {
            reg_state_resp->cellIdentity.cellInfoType = RIL_CELL_INFO_TYPE_TD_SCDMA;
        }
    }
    memset(tags,'\0', MAX_TAG_SIZE);
    snprintf(tags, MAX_TAG_SIZE, "RAT = %d, REG_STATE= %d", rat, reg_state);
...
void util_voice_registration_state_for_lte(Testcase &t, QMIRILTestnode &QMI_node, AndroidTelTestnode &RIL_node, OEMTestnode &OEM_node, int rat, int reg_state, nas_service_status_enum_type_v01 srv_status, nas_true_service_status_enum_type_v01 true_srv_status, nas_service_domain_enum_type_v01 domain, nas_roam_status_enum_type_v01 roam_status, uint8_t voice_support_on_lte, uint8_t reg_reject_info_valid, nas_service_domain_enum_type_v01 reject_srv_domain, uint8_t rej_cause, uint8_t net_id_valid, const char *mcc, const char *mnc, const char *primary_mcc, const char *primary_mnc, uint8_t csg_info_valid, uint32_t cid, uint32_t cid_name_len, uint16_t *cid_name, uint8_t lte_is_eb_supported_valid, nas_tri_state_boolean_type_v01 lte_is_eb_supported, bool is_data_rat_change)
{
    char tags[MAX_TAG_SIZE];
    t.set_start();
    ...
    if (nullptr != mcc && nullptr != mnc)
    {
        util_ril_utf_fill_qmi_mcc_mnc(
            lte_sys_info_resp->threegpp_specific_sys_info.network_id.mcc, mcc);
        util_ril_utf_fill_qmi_mcc_mnc(
            lte_sys_info_resp->threegpp_specific_sys_info.network_id.mnc, mnc);
    }

    ptr->lte_is_eb_supported_valid = lte_is_eb_supported_valid;
    ptr->lte_is_eb_supported = lte_is_eb_supported;

    if (nullptr != primary_mcc && nullptr != primary_mnc)
    {
        ptr->primary_plmn_info_valid = TRUE;
        ptr->primary_plmn_info.radio_if = NAS_RADIO_IF_LTE_V01;
        util_ril_utf_fill_qmi_mcc_mnc(
            ptr->primary_plmn_info.plmn_id.mcc, primary_mcc);
        util_ril_utf_fill_qmi_mcc_mnc(
            ptr->primary_plmn_info.plmn_id.mnc, primary_mnc);
    }
    ...
    if (reg_state == RIL_VAL_REG_REGISTERED_HOME_NET
            ...
            || srv_status == NAS_SYS_SRV_STATUS_SRV_V01)
    {
        reg_state_resp->cellIdentity.cellInfoType = RIL_CELL_INFO_TYPE_LTE;
        if (nullptr != mcc && nullptr != mnc)
        {
            strlcpy(reg_state_resp->cellIdentity.cellIdentityLte.mcc, mcc,
                    MAX_RIL_MCC_MNC_SIZE);
            strlcpy(reg_state_resp->cellIdentity.cellIdentityLte.mnc, mnc,
                    MAX_RIL_MCC_MNC_SIZE);
        }
        if (nullptr != primary_mcc && nullptr != primary_mnc)
        {
            strlcpy(reg_state_resp->cellIdentity.cellIdentityLte.mcc,
                    primary_mcc, MAX_RIL_MCC_MNC_SIZE);
            strlcpy(reg_state_resp->cellIdentity.cellIdentityLte.mnc,
                    primary_mnc, MAX_RIL_MCC_MNC_SIZE);
        }
    }
...
void util_voice_registration_state_in_sglte_case(Testcase &t, QMIRILTestnode &QMI_node, AndroidTelTestnode &RIL_node, int rat, int reg_state, nas_service_status_enum_type_v01 gsm_srv_status, nas_true_service_status_enum_type_v01 gsm_true_srv_status, nas_service_domain_enum_type_v01 gsm_domain, nas_roam_status_enum_type_v01 gsm_roam_status, uint8_t gsm_reg_reject_info_valid, nas_service_domain_enum_type_v01 gsm_reject_srv_domain, uint8_t gsm_rej_cause, nas_service_status_enum_type_v01 lte_srv_status, nas_true_service_status_enum_type_v01 lte_true_srv_status, nas_service_domain_enum_type_v01 lte_domain, nas_roam_status_enum_type_v01 lte_roam_status, uint8_t voice_support_on_lte, uint8_t lte_reg_reject_info_valid, nas_service_domain_enum_type_v01 lte_reject_srv_domain, uint8_t lte_rej_cause, uint8_t gsm_net_id_valid, const char *gsm_mcc, const char *gsm_mnc, uint8_t lte_net_id_valid, const char *lte_mcc, const char *lte_mnc, bool is_data_rat_change)
{
    ...
    if (NULL != gsm_mcc && NULL != gsm_mnc)
    {
        util_ril_utf_fill_qmi_mcc_mnc(
            gsm_sys_info_resp->threegpp_specific_sys_info.network_id.mcc, gsm_mcc);
        util_ril_utf_fill_qmi_mcc_mnc(
            gsm_sys_info_resp->threegpp_specific_sys_info.network_id.mnc, gsm_mnc);
    }
    nas_3gpp_srv_status_info_type_v01* wcdma_srv_status_resp = QMI_node.set_qmi_nas_get_sys_info_resp_wcdma_srv_status_info();
    wcdma_srv_status_resp->srv_status = NAS_SYS_SRV_STATUS_NO_SRV_V01;
    wcdma_srv_status_resp->true_srv_status = SYS_SRV_STATUS_NO_SRV_V01;
    ...
    if (NULL != lte_mcc && NULL != lte_mnc)
    {
        util_ril_utf_fill_qmi_mcc_mnc(
            lte_sys_info_resp->threegpp_specific_sys_info.network_id.mcc, lte_mcc);
        util_ril_utf_fill_qmi_mcc_mnc(
            lte_sys_info_resp->threegpp_specific_sys_info.network_id.mnc, lte_mnc);
    }
    memset(tags,'\0', MAX_TAG_SIZE);
            ...
            || reg_state == RIL_VAL_REG_NOT_REGISTERED_SEARCHING_LTD_SRV)
    {
        ...
        {
            reg_state_resp->cellIdentity.cellInfoType = RIL_CELL_INFO_TYPE_LTE;
            if (lte_mcc != nullptr && lte_mnc != nullptr)
            {
                strlcpy(reg_state_resp->cellIdentity.cellIdentityLte.mcc, lte_mcc,
                        MAX_RIL_MCC_MNC_SIZE);
                strlcpy(reg_state_resp->cellIdentity.cellIdentityLte.mnc, lte_mnc,
                        MAX_RIL_MCC_MNC_SIZE);
            }
        }
        else
        {
            reg_state_resp->cellIdentity.cellInfoType = RIL_CELL_INFO_TYPE_GSM;
            if (gsm_mcc != nullptr && gsm_mnc != nullptr)
            {
                strlcpy(reg_state_resp->cellIdentity.cellIdentityGsm.mcc, gsm_mcc,
                        MAX_RIL_MCC_MNC_SIZE);
                strlcpy(reg_state_resp->cellIdentity.cellIdentityGsm.mnc, gsm_mnc,
                        MAX_RIL_MCC_MNC_SIZE);
            }
        }
...
void util_voice_registration_state_in_svlte_case(Testcase &t, QMIRILTestnode &QMI_node, AndroidTelTestnode &RIL_node, int rat, int reg_state, nas_service_status_enum_type_v01 cdma_srv_status, nas_service_domain_enum_type_v01 cdma_domain, nas_roam_status_enum_type_v01 cdma_roam_status, nas_service_status_enum_type_v01 lte_srv_status, nas_true_service_status_enum_type_v01 lte_true_srv_status, nas_service_domain_enum_type_v01 lte_domain, nas_roam_status_enum_type_v01 lte_roam_status, uint8_t voice_support_on_lte, uint8_t lte_reg_reject_info_valid, nas_service_domain_enum_type_v01 lte_reject_srv_domain, uint8_t lte_rej_cause, bool is_data_rat_change)
{
    ...
    if (reg_state == RIL_VAL_REG_REGISTERED_HOME_NET
            ...
            || reg_state == RIL_VAL_REG_NOT_REGISTERED_SEARCHING_LTD_SRV)
    {
        if (reg_state_resp->rat == RADIO_TECH_LTE)
        {
            reg_state_resp->cellIdentity.cellInfoType = RIL_CELL_INFO_TYPE_LTE;
        }
        else
        {
            reg_state_resp->cellIdentity.cellInfoType = RIL_CELL_INFO_TYPE_CDMA;
        }
    }
    memset(tags,'\0', MAX_TAG_SIZE);
    snprintf(tags, MAX_TAG_SIZE, "RAT = %d, REG_STATE= %d", rat, reg_state);
