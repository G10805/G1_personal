...
void reset_i_ims_aidl_radio() {
  RIL_UTF_DEBUG("\n QMI_RIL_UTL: reset_i_ims_radio");
    ::android::sp<android::IBinder> binder = getServiceManager()->getService(android::String16(
        "vendor.qti.hardware.radio.ims.IImsRadio/imsradio0"));
    ::android::sp<AIBinder> ret = static_cast<ABBinder*>(binder.get());
    AIBinder_incStrong(ret.get());
    auto imsBinder = ndk::SpAIBinder(ret.get());
...
void connect_to_i_ims_aidl_radio() {
  ...
  while (utfImsAidlRadio == nullptr) {
    ...
    if (utfImsAidlRadio != nullptr) {
      ...
      mRespClient = std::make_shared<IImsAidlRadioResponseClient>();
      mIndClient = std::make_shared<IImsAidlRadioIndicationClient>();
