...
const char* lookup_qmi_qos_message_name(int msg_id)
{
  int i;
  for (i = 0; i < QMI_QOS_MESSAGE_SIZE; ++i)
  {
    if (qmi_qos_service_msg_string_table[i].number == msg_id)
    {
      return qmi_qos_service_msg_string_table[i].name;
    }
  }
  RIL_UTF_ESSENTIAL("\n@@@@@@     ERROR! NO MATCHING MESSAGE!!!\n");
  return NULL;
...
const char* lookup_rfrpe_message_name(int msg_id)
{
  int i;
  for (i = 0; i < RFRPE_MESSAGE_SIZE; ++i)
  {
    if (rfrpe_service_msg_string_table[i].number == msg_id)
    {
      return rfrpe_service_msg_string_table[i].name;
    }
  }
  RIL_UTF_ESSENTIAL("\n@@@@@@     ERROR! NO MATCHING MESSAGE!!!\n");
  return NULL;
...
const char* lookup_ott_message_name(int msg_id)
{
  int i;
  for (i = 0; i < OTT_MESSAGE_SIZE; ++i)
  {
    if (ott_service_msg_string_table[i].number == msg_id)
    {
      return ott_service_msg_string_table[i].name;
    }
  }
  RIL_UTF_ESSENTIAL("\n@@@@@@     ERROR! NO MATCHING MESSAGE!!!\n");
  return NULL;
...
const char* lookup_tmd_message_name(int msg_id)
{
  int i;
  for (i = 0; i < TMD_MESSAGE_SIZE; ++i)
  {
    if (tmd_service_msg_string_table[i].number == msg_id)
    {
      return tmd_service_msg_string_table[i].name;
    }
  }
  RIL_UTF_ESSENTIAL("\n@@@@@@     ERROR! NO MATCHING MESSAGE!!!\n");
  return NULL;
...
const char* lookup_qtuner_message_name(int msg_id)
{
  int i;
  for (i = 0; i < QTUNER_MESSAGE_SIZE; ++i)
  {
    if (qtuner_service_msg_string_table[i].number == msg_id)
    {
      return qtuner_service_msg_string_table[i].name;
    }
  }
  RIL_UTF_ESSENTIAL("\n@@@@@@     ERROR! NO MATCHING MESSAGE!!!\n");
  return NULL;
...
const char* lookup_dsi_message_name(int msg_id)
{
  int i;
  for (i = 0; i < DSI_MESSAGE_SIZE; ++i)
  {
    if (dsi_service_msg_string_table[i].number == msg_id)
    {
      return dsi_service_msg_string_table[i].name;
    }
  }
  RIL_UTF_ESSENTIAL("\n@@@@@@     ERROR! NO MATCHING MESSAGE!!!\n");
  return NULL;
...
const char* lookup_qdp_message_name(int msg_id)
{
  int i;
  for (i = 0; i < QDP_MESSAGE_SIZE; ++i)
  {
    if (qdp_service_msg_string_table[i].number == msg_id)
    {
      return qdp_service_msg_string_table[i].name;
    }
  }
  RIL_UTF_ESSENTIAL("\n@@@@@@     ERROR! NO MATCHING MESSAGE!!!\n");
  return NULL;
...
void print_oem_message_name(int service_id, int msg_id, int oem_command_id, enum ril_utf_q_element_t type)
{
...
#endif
  {
    ...
    case QMI_RIL_OEM_HOOK_QMI_TUNNELING_SERVICE_QTUNER:
      ...
      tmp = lookup_qtuner_message_name(msg_id);
      RIL_UTF_ESSENTIAL("\n@@@@@@     Message %s", tmp);
      break;
...
void print_qmi_message_name(int qmi_srv_id, int msg_id, enum ril_utf_q_element_t type)
{
  ...
  case QMI_RFRPE:
    RIL_UTF_ESSENTIAL("\n@@@@@@     Service QMI_RFRPE_SERVICE");
    tmp = lookup_rfrpe_message_name(msg_id);
    RIL_UTF_ESSENTIAL("\n@@@@@@     Message %s%s", tmp, suffix);
    break;
  ...
  case QMI_AUTH:
    ...
    break;
  case QMI_OTT:
    RIL_UTF_ESSENTIAL("\n@@@@@@     Service QMI_OTT_SERVICE");
    tmp = lookup_ott_message_name(msg_id);
    RIL_UTF_ESSENTIAL("\n@@@@@@     Message %s%s", tmp, suffix);
    break;
  case QMI_QOS:
    RIL_UTF_ESSENTIAL("\n@@@@@@     Service QMI_QOS_SERVICE");
    tmp = lookup_qmi_qos_message_name(msg_id);
    RIL_UTF_ESSENTIAL("\n@@@@@@     Message %s%s", tmp, suffix);
    break;
  case QMI_TMD:
    RIL_UTF_ESSENTIAL("\n@@@@@@     Service QMI_TMD_SERVICE");
    tmp = lookup_tmd_message_name(msg_id);
    RIL_UTF_ESSENTIAL("\n@@@@@@     Message %s%s", tmp, suffix);
    break;
  default:
...
void print_dsi_message_name(int msg_id)
{
#ifndef RIL_UTF_IOE
  RIL_UTF_ESSENTIAL("\n@@@@@@     Service DSI");
  const char* str = lookup_dsi_message_name(msg_id);
  RIL_UTF_ESSENTIAL("\n@@@@@@     Message %s", str);
#endif
}

void print_qdp_message_name(int msg_id)
{
#ifndef RIL_UTF_IOE
  RIL_UTF_ESSENTIAL("\n@@@@@@     Service QDP");
  const char* str = lookup_qdp_message_name(msg_id);
  RIL_UTF_ESSENTIAL("\n@@@@@@     Message %s", str);
#endif
}
