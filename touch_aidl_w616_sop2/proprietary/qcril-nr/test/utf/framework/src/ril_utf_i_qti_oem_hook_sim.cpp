...
class IQtiOemHookResponseClient : public IQtiOemHookResponse {
  public:
    ::android::hardware::Return<void> oemHookRawResponse(int32_t serial, RadioError error, const ::android::hardware::hidl_vec<uint8_t>& data) {
      ...
      send_i_oemhook_enqueue_oem_expect(data.size(), (void *)payload, 0, serial, (uint32_t)error);
      return Void();
    }
};
...
static android::sp<IQtiOemHookResponseClient> mRespClient;
static android::sp<IQtiOemHookIndicationClient> mIndClient;
...
extern "C" void reset_i_qti_oem_hook() {
  ...
  if (utfOemhook != nullptr) {
    android::hardware::hidl_death_recipient *deathRecipient =
        dynamic_cast<android::hardware::hidl_death_recipient *>(utfOemhook.get());
