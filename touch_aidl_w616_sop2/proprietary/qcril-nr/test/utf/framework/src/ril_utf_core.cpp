...
extern struct sig_expect_pool_t wakeup_qdp_sync_signals[MAX_SIGNALS];
...
int send_ril_utf_qdp_response(struct ril_utf_msg_t *test_step)
{
  int err_val = QCRIL_UNITTTEST_NO_ERR;
  int sig_err = QCRIL_UNITTTEST_NO_ERR;
  int i;
  ...
  if ( is_sync_response == 1 )
  {
    for ( i = 0; i < MAX_SIGNALS; ++i )
    {
      if ( (wakeup_qdp_sync_signals[i].in_use == 1) &&
           (wakeup_qdp_sync_signals[i].msg_id == test_step->msg_id) )
      {
        break;
      }
    }
    RIL_UTF_DEBUG("\n QMI_RIL_UTL: Waking up sync request with signal slot %d\n", i);
    sig_err = pthread_cond_signal(&(wakeup_qdp_sync_signals[i].signal_exptectation));
  }
...
int send_ril_utf_db_request (struct ril_utf_msg_t *test_step)
{
  ...
  switch (ptr->action)
  {
    ...
    case UTF_DB_INSERT_PROPERTIES_TABLE:
    {
      ...
      ret = qcril_db_properties_table_user_set(ptr->property_name, ptr->value);
