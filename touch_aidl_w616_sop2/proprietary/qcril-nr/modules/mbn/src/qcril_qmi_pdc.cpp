...
void qcril_qmi_pdc_clear_busy_flag()
{
...
//===========================================================================
// qcril_qmi_pdc_map_refresh_event
//===========================================================================
qcril::interfaces::PdcRefreshEvent qcril_qmi_pdc_map_refresh_event(
    pdc_refresh_event_enum_v01 in)
{
  switch (in) {
    case PDC_EVENT_REFRESH_START_V01:
      return qcril::interfaces::PdcRefreshEvent::REFRESH_START;
    case PDC_EVENT_REFRESH_COMPLETE_V01:
      return qcril::interfaces::PdcRefreshEvent::REFRESH_COMPLETE;
    case PDC_EVENT_CLIENT_REFRESH_V01:
      return qcril::interfaces::PdcRefreshEvent::CLIENT_REFRESH;
    default:
      return qcril::interfaces::PdcRefreshEvent::UNKNOWN;
  }
  return qcril::interfaces::PdcRefreshEvent::UNKNOWN;
} // qcril_qmi_pdc_map_refresh_event
...
/*=========================================================================
  FUNCTION:  qcril_qmi_pdc_refresh_ind_hdlr

===========================================================================*/
/*!
    @brief
    Handle QMI_PDC_REFRESH_IND_V01

    @return
    None.
*/
/*=========================================================================*/
void qcril_qmi_pdc_refresh_ind_hdlr
(
  void *ind_data_ptr,
  uint32_t /*ind_data_len */
)
{
  pdc_refresh_ind_msg_v01 *ind = nullptr;

  QCRIL_LOG_FUNC_ENTRY();

  if ( ind_data_ptr != nullptr )
  {
    ind = (pdc_refresh_ind_msg_v01*)ind_data_ptr;
    if (ind->subscription_id_valid)
    {
      qcril::interfaces::PdcRefreshEvent event =
          qcril_qmi_pdc_map_refresh_event(ind->refresh_event);
      if (event != qcril::interfaces::PdcRefreshEvent::UNKNOWN) {
        auto msg = std::make_shared<QcRilUnsolPdcRefreshMessage>();
        msg->setEvent(event);
        msg->setSubscriptionId(ind->subscription_id);
...
RIL_Errno qcril_qmi_pdc_get_error_from_ind(qmi_error_type_v01 error) {
  RIL_Errno result = RIL_E_SUCCESS;
  switch(error) {
    ...
    case QMI_ERR_NOT_PROVISIONED_V01:
      result = RIL_E_NOT_PROVISIONED;
    break;
    case QMI_ERR_INTERNAL_V01:
      result = RIL_E_MODEM_ERR;
    break;
    default:
      result = RIL_E_GENERIC_FAILURE;
    break;
  ...
  do
  {
    ...
    if ( ind_data_ptr != NULL )
    {
      ...
      else // QMI ERROR
      {
        ...
        result = qcril_qmi_pdc_get_error_from_ind(valid_ind->error);
        break;
      }
    }
    else // ind_data_ptr = NULL
...
)
{
  ...
  if ( ind_data_ptr != NULL )
  {
    ...
    else // QMI ERROR
    {
      ...
      result = qcril_qmi_pdc_get_error_from_ind(deact_ind->error);
    }
  }
  else // ind_data_ptr = NULL
  {
...
)
{
  ...
  if ( ind_data_ptr != NULL )
  {
    ...
    {
      QCRIL_LOG_ERROR("no selected config id, QMI error code %d", sel_ind->error);
      result =  qcril_qmi_pdc_get_error_from_ind(sel_ind->error);
    }
  }
  else // ind_data_ptr = NULL
  {
...
)
{
  ...
  struct stat f_stat;
  size_t str_size;
  char mbn_file_path[QCRIL_MBN_FILE_PATH_LEN] = { 0 };
...
)
{
  ...
  do
  {
    ...
    if (msg->getConfigId().size() > sizeof(qmi_request.new_config_info.config_id))
    {
      QCRIL_LOG_ERROR("Invalid length received");
      result = RIL_E_INVALID_ARGUMENTS;
      break;
    }
  ...
  do {
      switch(msg_id)
      {
        ...
        case QMI_PDC_REFRESH_IND_V01:
            qcril_qmi_pdc_refresh_ind_hdlr(decoded_payload, decoded_payload_len);
        break;
...
)
{
  ...
  string path;
  if (qcril_config_get(PERSIST_VENDOR_RADIO_MBN_PATH, path) == E_SUCCESS)
  {
    strlcpy(g_pdc_info.mbn_file_dir, path.c_str(), sizeof(g_pdc_info.mbn_file_dir));
  }
  else
  {
    strlcpy(g_pdc_info.mbn_file_dir, QCRIL_MODEM_MBN_FILE_PATH,
            sizeof(g_pdc_info.mbn_file_dir));
  }
  ...
  qcril_qmi_register_pdc_indication();
...
void   qcril_qmi_register_pdc_indication
(
void
)
{
  QCRIL_LOG_FUNC_ENTRY();
  pdc_indication_register_req_msg_v01  qmi_request;
  pdc_indication_register_resp_msg_v01 qmi_response;
  qmi_client_error_type                qmi_error;
  ...
  qmi_error = qmi_client_pdc_send_raw_sync(QMI_PDC_INDICATION_REGISTER_REQ_V01,
                                          &qmi_request, sizeof( qmi_request ),
                                          &qmi_response, sizeof( qmi_response ));

  if (qmi_error != QMI_NO_ERR || qmi_response.resp.result != QMI_RESULT_SUCCESS_V01) {
    QCRIL_LOG_ERROR("qcril_qmi_register_pdc_indication(): failed ");
  }
  QCRIL_LOG_FUNC_RETURN();
}
...
)
{
  ...
  static const property_id_type id_mapping[] = {PERSIST_VENDOR_RADIO_STACK_ID_0,
        PERSIST_VENDOR_RADIO_STACK_ID_1};
  static constexpr auto id_mapping_size = sizeof(id_mapping)/sizeof(id_mapping[0]);
  if (instance_id < id_mapping_size)
  {
    int int_config;
    if ((qcril_config_get(id_mapping[instance_id], int_config) == E_SUCCESS)
            && (int_config < 2))
    {
        sub_id = int_config;
    }
    ...
    {
        QCRIL_LOG_ERROR("failed to retrieve the sub id from database");
    }
  }
