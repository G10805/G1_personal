...
struct qcril_mbn_hw_state_machine_type {
    qcril_mbn_hw_state_machine_type() {
        qcril_mbn_hw_cur_state = QCRIL_MBN_HW_STATE_NONE;
        qcril_mbn_hw_complete_next_action = nullptr;
    }
    qcril_mbn_hw_state_type qcril_mbn_hw_cur_state;
    qcril_mbn_hw_complete_callback qcril_mbn_hw_complete_next_action;
    qtimutex::QtiRecursiveMutex qcril_mbn_hw_lock;
};
static struct qcril_mbn_hw_state_machine_type qcril_mbn_hw_state_machine;
...
#define MBN_HW_CONFIG_LOCK()    qcril_mbn_hw_state_machine.qcril_mbn_hw_lock.lock()
#define MBN_HW_CONFIG_UNLOCK()  qcril_mbn_hw_state_machine.qcril_mbn_hw_lock.unlock()
...
void qcril_mbn_hw_set_complete_callback(qcril_mbn_hw_complete_callback cb) {
    MBN_HW_CONFIG_LOCK();
    qcril_mbn_hw_state_machine.qcril_mbn_hw_complete_next_action = cb;
    MBN_HW_CONFIG_UNLOCK();
}

qcril_mbn_hw_complete_callback qcril_mbn_hw_get_complete_callback() {
    MBN_HW_CONFIG_LOCK();
    auto cb = qcril_mbn_hw_state_machine.qcril_mbn_hw_complete_next_action;
    MBN_HW_CONFIG_UNLOCK();
    return cb;
}
...
{
    MBN_HW_CONFIG_LOCK();
    auto res = qcril_mbn_hw_state_machine.qcril_mbn_hw_cur_state;
    MBN_HW_CONFIG_UNLOCK();
    return res;
}
...
)
{
    ...
    MBN_HW_CONFIG_LOCK();
    qcril_mbn_hw_state_machine.qcril_mbn_hw_cur_state = state;
    MBN_HW_CONFIG_UNLOCK();
}

static void qcril_mbn_hw_mbn_update_done(qcril_mbn_hw_state_type state) {
    // set cb and state together under lock
    MBN_HW_CONFIG_LOCK();
    qcril_mbn_hw_set_complete_callback(nullptr);
    qcril_mbn_hw_set_cur_state(state);
    MBN_HW_CONFIG_UNLOCK();
...
)
{
    ...
    */
    bool bool_config, is_hw_enabled = false;
    if (qcril_config_get(PERSIST_VENDOR_RADIO_HW_MBN_UPDATE, bool_config) == E_SUCCESS)
    {
        is_hw_enabled = bool_config;
    }
    if(!is_hw_enabled) {
        return TRUE;
    }

    switch (qcril_mbn_hw_get_cur_state())
    {
        ...
        case QCRIL_MBN_HW_STATE_MBN_LOADING_TO_DB_FAILED:
    ...
    do
    {
        ...
        else
        {
                                        ...
                                        config_id_len))
            {
                ...
                if (action) action(QCRIL_MBN_HW_STATE_ACTIVATION_NOT_NEEDED);
            }
            else
            {
...
)
{
    ...
    {
        auto action = qcril_mbn_hw_get_complete_callback();
        if (action) action(QCRIL_MBN_HW_STATE_LOAD_FAILED);
    }
    else
    {
...
)
{
    ...
    {
        auto action = qcril_mbn_hw_get_complete_callback();
        if (action) action(QCRIL_MBN_HW_STATE_SELECTION_FAILED);
    }
    else
    {
...
)
{
    ...
    {
        auto action = qcril_mbn_hw_get_complete_callback();
        if (action) action(QCRIL_MBN_HW_STATE_ACTIVATION_FAILED);
    }
    else
    {
        auto msg = getMbnModule().getPendingMessageList().extract(
...
)
{
    ...
    do
    {
        ...
        if (!qcril_reqlist_new(QCRIL_DEFAULT_INSTANCE_ID, &qcril_req_info))
        {
            auto msg = getMbnModule().getPendingMessageList().find(
                QcRilRequestSetModemsConfigMessage::get_class_message_id());
            auto setConfigMsg = std::static_pointer_cast<QcRilRequestSetModemsConfigMessage>(msg);
            if (setConfigMsg) {
                setConfigMsg->setState(QcRilRequestSetModemsConfigMessage::Status::ACTIVATING);
            }
            result = qcril_qmi_pdc_activate_configuration_hdlr(req->sub_id,
                                                            req->config_type);
            if (result != RIL_E_SUCCESS && setConfigMsg) {
                setConfigMsg->setState(QcRilRequestSetModemsConfigMessage::Status::NOT_ACTIVATED);
            }
        }
        else
        {
...
)
{
    ...
    bool is_sw_ota_enabled = false;
    if (qcril_config_get(PERSIST_VENDOR_RADIO_START_OTA_DAEMON, bool_config) == E_SUCCESS)
    {
        is_sw_ota_enabled = bool_config;
    }
    //store the sub count
    ...
    {
        qcril_mbn_hw_set_complete_callback(qcril_mbn_hw_mbn_update_done);
...
)
{
    ...
    do {
        ...
        if (hw_init_req->apss_max_sub != hw_init_req->modem_max_sub)
        {
            ...
            {
                auto action = qcril_mbn_hw_get_complete_callback();
                if (action) action(QCRIL_MBN_HW_STATE_MBN_LOADING_TO_DB_FAILED);
                break;
            }
...
)
{
    ...
    {
        mbn_loaded = 1;
        if (qcril_config_set(HW_MBN_LOADED, mbn_loaded) != E_SUCCESS)
        {
            QCRIL_LOG_ERROR("failed to save hw_mbn_loaded");
        }
...
{
  qcril_mbn_hw_set_cur_state(QCRIL_MBN_HW_STATE_NONE);
...
RIL_Errno qcril_mbn_hw_check_readiness_for_setting_modems_config()
{
     if (!qcril_mbn_is_copy_completed()) return RIL_E_INTERNAL_ERR;
     // make sure HW and SW update is not in progress
     if (!qcril_mbn_hw_is_completed() || !qcril_mbn_sw_is_completed())
        return RIL_E_DEVICE_IN_USE;
     return RIL_E_SUCCESS;
}

// This can be called
// 1) when there is failure in any of the steps - by global pointer (existed mechanism)
// 2) in activation indication handler (failure or success if ind does come) - by global pointer
// 3) in SSR handling - by invoking directly in MBN module (if State is ACTIVATING, which is
// set before sending ACTIVATE QMI req)
static void qcril_mbn_hw_set_modems_config_done(
        qcril_mbn_hw_state_type state) {
    ...
    auto msg = getMbnModule().getPendingMessageList().extract(
            QcRilRequestSetModemsConfigMessage::get_class_message_id());
    if (msg == nullptr) {
        QCRIL_LOG_ERROR("Unexpected: no QcRilRequestSetModemsConfigMessage is pending!");
        return;
    }
    ...
    if (res == RIL_E_SUCCESS) {
        setConfigMsg->setState(QcRilRequestSetModemsConfigMessage::Status::ACTIVATED);
    } else {
        setConfigMsg->setState(QcRilRequestSetModemsConfigMessage::Status::NOT_ACTIVATED);
    }
    ...
    auto respPayload = std::make_shared<QcRilRequestMessageCallbackPayload>(
            res, nullptr);
    setConfigMsg->sendResponse(setConfigMsg, Message::Callback::Status::SUCCESS, respPayload);
}

void qcril_mbn_hw_set_modems_config(std::shared_ptr<QcRilRequestSetModemsConfigMessage> msg) {
    int modem_subs = 0;
    auto res = qcril_mbn_hw_check_readiness_for_setting_modems_config();
    if (res == RIL_E_SUCCESS) {
        std::shared_ptr<int> shared_modem_subs;
        auto queryMsg = std::make_shared<QcRilGetMaxModemSubscriptionsSyncMessage>();
        if (queryMsg) {
            auto result = queryMsg->dispatchSync(shared_modem_subs);
            if (result == Message::Callback::Status::SUCCESS && shared_modem_subs != nullptr) {
                modem_subs = *shared_modem_subs;
            } else {
                res = RIL_E_GENERIC_FAILURE;
            }
        } else {
    ...
    qcril_mbn_max_apss_instance_id = msg->getNumOfModems();
    qcril_mbn_hw_init_req hw_init_req = {
        .modem_max_sub = modem_subs,
        .apss_max_sub = qcril_mbn_max_apss_instance_id,
    };

    MBN_HW_CONFIG_LOCK();
    qcril_mbn_hw_set_complete_callback(qcril_mbn_hw_set_modems_config_done);
    qcril_mbn_hw_set_cur_state(QCRIL_MBN_HW_STATE_INITIALIZING);
    MBN_HW_CONFIG_UNLOCK();
