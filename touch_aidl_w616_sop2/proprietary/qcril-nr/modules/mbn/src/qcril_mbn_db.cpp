...
)
{
    ...
    // the latest HW MBNs are using leading "SS-LA", "DSDS-LA"
    if (strstr(hw_name, "LA_SS") || strstr(hw_name, "LA-SS")
            || !strncasecmp(hw_name, "SS-LA", 5))
    {
        ret = qcril_db_update_hw_mbn_file_with_str(mbn_file_name, "SHORT_NAME", "LA_SS");
    }
    else if (strstr(hw_name, "LA_DS") || strstr(hw_name, "LA-DS")
            || !strncasecmp(hw_name, "DSDS-LA", 7))
    {
