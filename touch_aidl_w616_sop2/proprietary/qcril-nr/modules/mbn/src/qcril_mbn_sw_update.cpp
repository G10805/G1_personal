...
)
{
    ...
    */
    bool bool_config, is_enabled = false;
    if (qcril_config_get(PERSIST_VENDOR_RADIO_SW_MBN_UPDATE, bool_config) == E_SUCCESS)
    {
        is_enabled = bool_config;
    }
    if(!is_enabled) {
        return TRUE;
    }
...
)
{
    ...
    {
        mbn_loaded = 1;
        if (qcril_config_get(SW_MBN_LOADED, mbn_loaded) != E_SUCCESS)
        {
            QCRIL_LOG_ERROR("failed ot save sw_mbn_loaded");
        }
...
)
{
    ...
    bool bool_config, is_volte = true;
    if (qcril_config_get(PERSIST_VENDOR_RADIO_SW_MBN_VOLTE, bool_config) == E_SUCCESS)
    {
        is_volte = bool_config;
    }
    bool is_openmkt = true;
    if (qcril_config_get(PERSIST_VENDOR_RADIO_SW_MBN_OPENMKT, bool_config) == E_SUCCESS)
    {
        is_openmkt = bool_config;
    }
    bool is_long_iin_mbn_needed = false;
    if (qcril_config_get(PERSIST_VENDOR_RADIO_LONG_IIN_MBN, bool_config) == E_SUCCESS)
    {
        is_long_iin_mbn_needed = bool_config;
    }
