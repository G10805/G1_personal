...
bool convertToAidlCallInfoList(std::vector<aidlimports::CallInfo>& out,
  ...
  for (uint32_t i = 0; i < count; ++i) {
    out[i] = {};
    convertToAidlCallInfo(out[i], in[i]);
  }
  return true;
...
std::shared_ptr<QcRilRequestImsDialMessage> makeQcRilRequestImsDialMessage(
  ...
  return msg;
}  // makeQcRilRequestImsDialMessage
...
bool changeVonrFromAppSupported() {
  bool changeVonrFromAppSupported = false;
  bool bool_config;
  if (qcril_config_get(PERSIST_VENDOR_RADIO_SUPPORT_VONR_UPDATE, bool_config) == E_SUCCESS) {
    changeVonrFromAppSupported = bool_config;
  }
  return changeVonrFromAppSupported;
} // changeVonrFromAppSupported
...
bool convertToAidlSuppServiceStatus(
