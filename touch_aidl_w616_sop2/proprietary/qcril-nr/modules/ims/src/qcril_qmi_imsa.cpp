...
static void qcril_qmi_imsa_store_service_status_cl(
    ...
    uint8_t ussd_service_status_valid,
    imsa_service_status_enum_v01 ussd_service_status,
    uint8_t ussd_service_rat_valid,
...
void qcril_qmi_imsa_init()
{
   ...
   ind_reg_req.scbm_mode_update_config_valid = TRUE;
   ind_reg_req.scbm_mode_update_config = 1; // enable
   // Register for voice_capability_update ind only if its primary RIL instance
   if (qmi_ril_get_process_instance_id() == QCRIL_DEFAULT_INSTANCE_ID) {
       ind_reg_req.voice_capability_update_config_valid = TRUE;
       ind_reg_req.voice_capability_update_config = 1; // enable
   }
   // Register for C_IWLAN Only feature exit indication
   ind_reg_req.c_iwlan_status_config_valid = TRUE;
   ind_reg_req.c_iwlan_status_config = 1;
...
void qcril_qmi_imsa_cleanup()
{
   ...
   qcril_qmi_imsa_store_service_status_cl(
       ...
       TRUE, IMSA_NO_SERVICE_V01,  // USSD SERVICE_STATUS
       FALSE, IMSA_WWAN_V01,       // USSD SERVICE_RAT
   ...
   qcril_qmi_imsa_info.ims_srv_status.ussd_service_status_valid = FALSE;
   ...
   qcril_qmi_imsa_info.ims_srv_status.ussd_service_rat_valid = FALSE;
...
// qcril_qmi_imsa_pdp_status_ind_hdlr
//===========================================================================
void qcril_qmi_imsa_active_sub_status_ind_hdlr(void *ind_data_ptr) {
  QCRIL_LOG_FUNC_ENTRY();
  auto active_sub_status_ind_msg = static_cast<imsa_active_subscription_ind_msg_v01*>(ind_data_ptr);
  if (active_sub_status_ind_msg)
  {
    auto msg = std::make_shared<ImsActiveSubStatusInd>(active_sub_status_ind_msg->isActive);
    Dispatcher::getInstance().broadcast(msg);
  }
  else
  {
...
//===========================================================================
// qcril_qmi_imsa_c_iwlan_status_ind_hdlr
//===========================================================================
void qcril_qmi_imsa_c_iwlan_status_ind_hdlr(
    const imsa_c_iwlan_status_ind_msg_v01& qmi_ind) {
  QCRIL_LOG_FUNC_ENTRY();
  if (qmi_ind.c_iwlan_feature_exit_needed_valid) {
    qcril::interfaces::CiWlanNotificationInfo info {};
    info = qmi_ind.c_iwlan_feature_exit_needed ?
      qcril::interfaces::CiWlanNotificationInfo::DISABLE_CIWLAN :
      qcril::interfaces::CiWlanNotificationInfo::NONE;
    auto msg = std::make_shared<QcRilUnsolImsCIwlanNotification>(info);
    Dispatcher::getInstance().broadcast(msg);
  }
  QCRIL_LOG_FUNC_RETURN();
} // qcril_qmi_imsa_c_iwlan_status_ind_hdlr
...
)
{
    ...
    if (ussd_service_status_valid)
    {
      qcril_qmi_imsa_info.ims_srv_status_valid = TRUE;

      qcril_qmi_imsa_info.ims_srv_status.ussd_service_status_valid = ussd_service_status_valid;
      qcril_qmi_imsa_info.ims_srv_status.ussd_service_status = ussd_service_status;
    }

    if (ussd_service_rat_valid)
    {
      qcril_qmi_imsa_info.ims_srv_status.ussd_service_rat_valid = ussd_service_rat_valid;
      qcril_qmi_imsa_info.ims_srv_status.ussd_service_rat = ussd_service_rat;
    }
...
void qcril_qmi_imsa_service_status_ind_hdlr(void *ind_data_ptr)
{
    ...
    if (NULL != service_ind_msg_ptr)
    {
        ...
        QCRIL_LOG_ESSENTIAL("USSD: service_status(%s): %d, rat(%s): %d",
            service_ind_msg_ptr->ussd_service_status_valid ? "valid": "not valid",
            service_ind_msg_ptr->ussd_service_status,
            service_ind_msg_ptr->ussd_service_rat_valid ? "valid": "not valid",
            service_ind_msg_ptr->ussd_service_rat);
        ...
        qcril_qmi_imsa_store_service_status_cl(
            ...
            service_ind_msg_ptr->ut_service_rat,
            service_ind_msg_ptr->ussd_service_status_valid,
            service_ind_msg_ptr->ussd_service_status,
            service_ind_msg_ptr->ussd_service_rat_valid,
            service_ind_msg_ptr->ussd_service_rat,
...
RIL_RadioTechnology convert_imsa_rat_type(imsa_service_rat_enum_v01 in)
{
  ...
  switch (in) {
    ...
    case IMSA_WWAN_V01:
      ...
      break;
    case IMSA_C_IWLAN_V01:
      result = RADIO_TECH_C_IWLAN;
      break;
    default:
...
static void qcril_qmi_imsa_rat_handover_status_ind_hdlr(void *ind_data_ptr)
{
  ...
  if (NULL != rat_ho_ind_msg_ptr)
  {
    if (rat_ho_ind_msg_ptr->rat_ho_status_info_valid)
    {
      ...
      {
        auto amMsg = std::make_shared<AudioManagementEventMessage>(
            AudioManagementEventMessage::Event::IMS_HANDOVER);
        if (amMsg != nullptr)
        {
          amMsg->setTargetRat(rat_ho_ind_msg_ptr->rat_ho_status_info.target_rat);
          Dispatcher::getInstance().dispatchSync(amMsg);
        }
      }
    }
  }
...
//===========================================================================
// qcril_qmi_ims_map_scbm_status
//===========================================================================
qcril::interfaces::SmsCallbackModeStatus qcril_qmi_ims_map_scbm_status(
    imsa_scbm_mode_update_type_v01 in)
{
  switch (in) {
    case IMSA_SCBM_MODE_ENTER_V01:
      return qcril::interfaces::SmsCallbackModeStatus::ENTER;
    case IMSA_SCBM_MODE_EXIT_V01:
      return qcril::interfaces::SmsCallbackModeStatus::EXIT;
    default:
      QCRIL_LOG_INFO("Unknown QMI IMSA Sms Callback Mode %d", in);
      return qcril::interfaces::SmsCallbackModeStatus::UNKNOWN;
  }
} // qcril_qmi_ims_map_scbm_status

//===========================================================================
// qcril_qmi_imsa_scbm_update_ind_hdlr
//===========================================================================
void qcril_qmi_imsa_scbm_update_ind_hdlr(const imsa_scbm_mode_update_ind_msg_v01 &qmi_ind) {
  QCRIL_LOG_FUNC_ENTRY();
  QCRIL_LOG_INFO("Sms callback mode status: %d", qmi_ind.scbm_mode_update_type);
  qcril::interfaces::SmsCallbackModeStatus scbmStatus =
      qcril_qmi_ims_map_scbm_status(qmi_ind.scbm_mode_update_type);
  if (scbmStatus != qcril::interfaces::SmsCallbackModeStatus::UNKNOWN)
  {
    auto msg = std::make_shared<QcRilUnsolImsSmsCallbackModeMessage>();
    if (msg)
    {
      msg->setSmsCallbackModeStatus(scbmStatus);
      Dispatcher::getInstance().dispatchSync(msg);
    }
  }
  QCRIL_LOG_FUNC_RETURN();
...
//===========================================================================
// qcril_qmi_ims_map_multi_sim_voice_capability
//===========================================================================
qcril::interfaces::MultiSimVoiceCapability qcril_qmi_ims_map_multi_sim_voice_capability(
    imsa_dual_sim_voice_capability_type_v01 in) {
  switch (in) {
    case IMSA_VOICE_CAPABILITY_NONE_MODE_V01:
      return qcril::interfaces::MultiSimVoiceCapability::NONE;
    case IMSA_VOICE_CAPABILITY_DSSS_MODE_V01:
      return qcril::interfaces::MultiSimVoiceCapability::DSSS;
    case IMSA_VOICE_CAPABILITY_DSDS_MODE_V01:
      return qcril::interfaces::MultiSimVoiceCapability::DSDS;
    case IMSA_VOICE_CAPABILITY_DSDA_MODE_V01:
      return qcril::interfaces::MultiSimVoiceCapability::DSDA;
    case IMSA_VOICE_CAPABILITY_PSEUDO_DSDA_MODE_V01:
...
//===========================================================================
// qcril_qmi_imsa_dual_sims_voice_capability_ind_hdlr
//===========================================================================
void qcril_qmi_imsa_dual_sims_voice_capability_ind_hdlr(
    const imsa_dual_sim_voice_capability_ind_msg_v01& qmi_ind) {
  QCRIL_LOG_FUNC_ENTRY("Multi sim voice capability: %d", qmi_ind.voice_capability_type);

  qcril::interfaces::MultiSimVoiceCapability voiceCapability =
      qcril_qmi_ims_map_multi_sim_voice_capability(qmi_ind.voice_capability_type);

  if (voiceCapability != qcril::interfaces::MultiSimVoiceCapability::UNKNOWN) {
    auto msg = std::make_shared<QcRilUnsolImsMultiSimVoiceCapabilityChanged>(voiceCapability);
    if (msg) {
      Dispatcher::getInstance().dispatchSync(msg);
    }
  }
...
)
{
  ...
  if (decoded_payload && decoded_payload_len)
  {
    switch(msg_id)
    {
      ...
      case QMI_IMSA_ACTIVE_SUBSCRIPTION_IND_V01:
        qcril_qmi_imsa_active_sub_status_ind_hdlr(decoded_payload);
        break;
      ...
      case QMI_IMSA_SCBM_MODE_UPDATE_IND_V01: {
        imsa_scbm_mode_update_ind_msg_v01 *qmi_ind = (imsa_scbm_mode_update_ind_msg_v01 *)
            (decoded_payload);
        qcril_qmi_imsa_scbm_update_ind_hdlr(*qmi_ind);
      } break;

      case QMI_IMSA_DUAL_SIMS_VOICE_CAPABILITY_IND_V01: {
        imsa_dual_sim_voice_capability_ind_msg_v01* qmi_ind =
            (imsa_dual_sim_voice_capability_ind_msg_v01*)(decoded_payload);
        qcril_qmi_imsa_dual_sims_voice_capability_ind_hdlr(*qmi_ind);
      } break;

      case QMI_IMSA_C_IWLAN_STATUS_IND_V01: {
        imsa_c_iwlan_status_ind_msg_v01* qmi_ind =
            (imsa_c_iwlan_status_ind_msg_v01*)(decoded_payload);
        qcril_qmi_imsa_c_iwlan_status_ind_hdlr(*qmi_ind);
        } break;
...
)
{
  ...
  do {
    ...
    if (resp_msg_ptr->resp.result == QMI_RESULT_SUCCESS_V01)
    {
      ...
      QCRIL_LOG_ESSENTIAL("USSD: service_status(%s): %d, rat(%s): %d",
          resp_msg_ptr->ussd_service_status_valid ? "valid": "not valid",
          resp_msg_ptr->ussd_service_status,
          resp_msg_ptr->ussd_service_rat_valid ? "valid": "not valid",
          resp_msg_ptr->ussd_service_rat);
      QCRIL_LOG_ESSENTIAL("CallComposer: service_status(%s): %d, rat(%s): %d",
      ...
      qcril_qmi_imsa_store_service_status_cl(
          ...
          resp_msg_ptr->ut_service_rat,
          resp_msg_ptr->ussd_service_status_valid,
          resp_msg_ptr->ussd_service_status,
          resp_msg_ptr->ussd_service_rat_valid,
          resp_msg_ptr->ussd_service_rat,
...
static ImsServiceStatusInd::ServiceRat qcril_qmi_imsa_convert_qmi_rat_to_imsa_rat (
  ...
  switch (qmi_rat) {
    ...
    case IMSA_IWLAN_V01:
      ...
      break;
    case IMSA_C_IWLAN_V01:
      return ImsServiceStatusInd::ServiceRat::C_IWLAN;
      break;
    default:
...
(
 const DeviceImsCapability &deviceImsCap
)
{
  QCRIL_LOG_FUNC_ENTRY();

  qcril_qmi_imsa_store_ims_capability(deviceImsCap);
  auto subConfig = qcril_qmi_imsa_get_ims_sub_configs();
  if (subConfig)
  {
...
)
{
  ...
  QCRIL_LOG_FUNC_ENTRY();
  if (msg) {
    std::shared_ptr<DeviceImsCapability> deviceImsCap{};
    Message::Callback::Status retVal = Message::Callback::Status::FAILURE;
    auto getDevImsCap = std::make_shared<GetDeviceImsCapabilityMessage>();
    if (getDevImsCap) {
      retVal = getDevImsCap->dispatchSync(deviceImsCap);
    }
    QCRIL_LOG_INFO("retVal=%d, deviceImsCap is %svalid", retVal, !deviceImsCap ? "not " : "");
    if ((retVal == Message::Callback::Status::SUCCESS) && deviceImsCap) {
      // Store the Device IMS capability
      qcril_qmi_imsa_store_ims_capability(*deviceImsCap);
    ...
    auto respPayload = std::make_shared<QcRilRequestMessageCallbackPayload>(errorCode, subConfig);
    msg->sendResponse(msg, Message::Callback::Status::SUCCESS, respPayload);
  }
  QCRIL_LOG_FUNC_RETURN();
}
...
)
{
    ...
    if (deviceImsCap.max_ims_instances)
    {
      qcril_qmi_imsa_info.max_ims_instances_valid =TRUE;
      qcril_qmi_imsa_info.max_ims_instances = *(deviceImsCap.max_ims_instances);
    }
    if (!qmi_ril_is_multi_sim_feature_supported())
    {
      qcril_qmi_imsa_info.max_ims_instances = 1;
    }
    qcril_qmi_imsa_info.ims_capability = deviceImsCap.ims_capability;
...
{
  QCRIL_LOG_FUNC_ENTRY();
  auto subConfig = std::make_shared<qcril::interfaces::ImsSubConfigInfo>();
  if (subConfig) {
