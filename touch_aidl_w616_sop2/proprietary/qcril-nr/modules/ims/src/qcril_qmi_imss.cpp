...
// Modem supports SET_CONCURRENCY with call_mode_pref TLV (0x11)
#define BIT_POS_SET_CONCURRENCY_ALLOW_ALERTING_MODE 1
//===========================================================================
//                     GLOBALS
//===========================================================================
...
static boolean qcril_qmi_imss_query_modem_supported_features
(
 uint16_t msgId,
 int bitPosition
);
...
static boolean modem_supports_set_concurrency_allow_alerting_mode = FALSE;
...
void qcril_qmi_imss_pre_init(void)
{
  ...
  feature_voice_dom_pref_on_toggle_ims_cap = false;
  bool bool_config;
  if (qcril_config_get(PERSIST_VENDOR_RADIO_VDP_ON_IMS_CAP,  bool_config) == E_SUCCESS)
    feature_voice_dom_pref_on_toggle_ims_cap = bool_config;
  QCRIL_LOG_INFO("voice domain pref on toggle ims cap is %d",
        feature_voice_dom_pref_on_toggle_ims_cap);

  feature_disabled_modem_req = false;
  if (qcril_config_get(PERSIST_VENDOR_RADIO_DISABLE_MODEM_CFG, bool_config) == E_SUCCESS)
    feature_disabled_modem_req = bool_config;
  QCRIL_LOG_INFO("disable modem ims config req status is %d",
        feature_disabled_modem_req);
...
void qcril_qmi_imss_init(void)
{
   ...
   ims_settings_config_ind_reg_rsp_msg_v01 ind_reg_resp = {};

   ret = qmi_client_imss_send_sync(QMI_IMS_SETTINGS_CONFIG_IND_REG_REQ_V01,
       &ind_reg_req,
...
)
{
  ...
  if (msg != nullptr) {
    do {
      ...
      // Make sure n_lines doesn't exceed the number of elements in line_info_list.
      if (n_lines < 0 || n_lines > (sizeof(qmi_req.line_info_list) / sizeof(ims_settings_digits_line_info_v01))) {
        QCRIL_LOG_ERROR("Number of lines in request %s",  n_lines < 0 ? "negative" : "exceed the line_info_list buffer size.");
        sendResponse = true;
        doCleanup = false;
        break;
      }
...
uint8_t qcril_qmi_imss_is_modem_supports_set_concurrency_allow_alerting(void)
{
  return modem_supports_set_concurrency_allow_alerting_mode;
}
...
void qcril_qmi_imss_update_modem_features(void)
{
    ...
    modem_supports_wfc_roaming_config = qcril_qmi_imss_query_modem_supported_features(
      QMI_IMS_SETTINGS_SET_IMS_SERVICE_ENABLE_CONFIG_REQ_V01,
      BIT_POS_WFC_ROAMING_MODE_PREFERENCE);
    modem_supports_wfc_preferred_ims = qcril_qmi_imss_query_modem_supported_features(
      QMI_IMS_SETTINGS_SET_IMS_SERVICE_ENABLE_CONFIG_REQ_V01,
      BIT_POS_WFC_PREFERENCE_IMS);
    modem_supports_set_concurrency_allow_alerting_mode =
      qcril_qmi_imss_query_modem_supported_features(
      QMI_IMS_SETTINGS_SET_CONCURRENCY_CONFIG_REQ_V01,
      BIT_POS_SET_CONCURRENCY_ALLOW_ALERTING_MODE);
    ...
    QCRIL_LOG_INFO("modem_supports_set_concurrency_allow_alerting_mode = %d",
                   modem_supports_set_concurrency_allow_alerting_mode);

    qcril_qmi_ims_send_unsol_wfc_roaming_config();
}

boolean qcril_qmi_imss_query_modem_supported_features(uint16_t msgId, int bitPosition)
{
    qmi_client_error_type qmi_client_error = QMI_NO_ERR;
    ...
    qmi_client_error = qmi_client_imss_send_sync(
            ...
            &qmi_resp,
            sizeof(qmi_resp));
    QCRIL_LOG_INFO(".. qmi send sync res %d, qmi resp result %d", (int) qmi_client_error,
      (int) qmi_resp.resp.result);

    if (qmi_client_error == QMI_NO_ERR && qmi_resp.resp.result == QMI_RESULT_SUCCESS)
    {
        if(qmi_resp.request_fields_valid)
        {
          ...
          return qcril_qmi_ims_is_field_supported(
            bitPosition, qmi_resp.request_fields,
