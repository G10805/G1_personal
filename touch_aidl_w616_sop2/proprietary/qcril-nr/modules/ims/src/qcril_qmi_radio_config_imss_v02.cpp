/******************************************************************************
#  Copyright (c) 2015-2017,2021-2022 Qualcomm Technologies, Inc.
#  All Rights Reserved.
...
bool
qcril_qmi_radio_config_map_qcril_auto_reject_mode_to_ims_call_pref
(
  uint32_t auto_reject_mode,
  ims_settings_call_pref_during_high_priority_v01 &ims_call_pref
)
{
  bool valid = FALSE;
  switch(auto_reject_mode)
  {
    case QCRIL_QMI_RADIO_CONFIG_IMSS_CALL_MODE_NORMAL:
      ims_call_pref = IMS_SETTINGS_CALL_MODE_NORMAL_V01;
      valid = TRUE;
      break;
    case QCRIL_QMI_RADIO_CONFIG_IMSS_CALL_MODE_AUTO_REJECT:
      ims_call_pref = IMS_SETTINGS_CALL_MODE_AUTO_REJECT_V01;
      valid = TRUE;
      break;
    case QCRIL_QMI_RADIO_CONFIG_IMSS_CALL_MODE_ALLOW_ALERTING:
      ims_call_pref = IMS_SETTINGS_CALL_MODE_ALLOW_ALERTING_V01;
      valid = TRUE;
      break;
  }
  QCRIL_LOG_INFO("Mapped radio config auto reject mode: %d, to ims call pref: %d",
                 auto_reject_mode, ims_call_pref);
  return valid;
}
//===========================================================================
...
)
{
  ...
  do
  {
    ...
    switch(config_ptr->config_item)
    {
      ...
      case QCRIL_QMI_RADIO_CONFIG_VOWIFI_ENTITLEMENT_ID:
        qmi_req.wifi_provisioning_id_valid = TRUE;
        strlcpy(qmi_req.wifi_provisioning_id,
                (const char *)config_ptr->config_item_value,
                IMS_SETTINGS_WIFI_PROVISION_ID_LEN_V01);
        QCRIL_LOG_INFO("Set config voice_over_wifi_entitlement_id to: %s",
                qmi_req.wifi_provisioning_id);
        break;

      case QCRIL_QMI_RADIO_CONFIG_B2C_ENRICHED_CALLING_PREFERENCE:
        qmi_req.b2c_enriched_calling_config_enabled_valid = TRUE;
        qmi_req.b2c_enriched_calling_config_enabled = *((bool *) config_ptr->config_item_value);
        QCRIL_LOG_INFO("Set config b2c_enriched_calling_config_enabled to: %d",
                        qmi_req.b2c_enriched_calling_config_enabled);
        break;
...
)
{
  ...
  // placeholder for validating integer value
  uint32_t int_value = 0;
  do
  {
    /* Validate the config pointer */
    ...
    QCRIL_LOG_INFO("Config item to set: %s, type: %d",
                    qcril_qmi_radio_config_get_item_log_str(config_ptr->config_item),
                    item_value_type);
    int_value = *static_cast<int32_t *>(config_ptr->config_item_value);
    ims_settings_call_pref_during_high_priority_v01 call_pref =
      IMS_SETTINGS_CALL_MODE_NORMAL_V01;
    if(qcril_qmi_radio_config_map_qcril_auto_reject_mode_to_ims_call_pref(int_value, call_pref))
    {
      if(qcril_qmi_imss_is_modem_supports_set_concurrency_allow_alerting())
      {
        qmi_req.call_pref_mode_valid = TRUE;
        qmi_req.call_pref_mode = call_pref;
        QCRIL_LOG_INFO("Set config SET_CONCURRENCY call_pref_mode to: %d",
                qmi_req.call_pref_mode);
      }
      else
      {
        QCRIL_LOG_INFO("Set config SET_CONCURRENCY int_value: %d", int_value);
        if(int_value == IMS_SETTINGS_CALL_MODE_ALLOW_ALERTING_V01)
        {
            radio_config_error = QCRIL_QMI_RADIO_CONFIG_ERROR_REQUEST_NOT_SUPPORTED;
            QCRIL_LOG_INFO("Set config SET_CONCURRENCY allow alerting is not supported");
            break;
        }
        qmi_req.auto_reject_call_enabled_valid = TRUE;
        if(int_value != IMS_SETTINGS_CALL_MODE_NORMAL_V01)
        {
          qmi_req.auto_reject_call_enabled = TRUE;
        }
        else
        {
          qmi_req.auto_reject_call_enabled = FALSE;
        }
  ...
  do
  {
    ...
    if(qmi_resp->resp.result == QMI_RESULT_SUCCESS_V01)
    {
      ...
      switch(config_ptr->config_item)
      {
        ...
        case QCRIL_QMI_RADIO_CONFIG_VOWIFI_ENTITLEMENT_ID:
          if (qmi_resp->wifi_provisioning_id_valid)
          {
            config_params.config_item_value_len = strlen(qmi_resp->wifi_provisioning_id);
            config_params.config_item_value = (void*)&qmi_resp->wifi_provisioning_id;
            QCRIL_LOG_INFO("get voice_over_wifi_entitlement_id value: %s",
                    qmi_resp->wifi_provisioning_id);
          }
          else
          {
            QCRIL_LOG_INFO("Did not get the value requested in qmi message");
        ...
        case QCRIL_QMI_RADIO_CONFIG_B2C_ENRICHED_CALLING_PREFERENCE:
          if (qmi_resp->b2c_enriched_calling_config_enabled_valid)
          {
            int_value = qmi_resp->b2c_enriched_calling_config_enabled;
            config_params.config_item_value_len = sizeof(int_value);
            config_params.config_item_value = (void*) &int_value;
            QCRIL_LOG_INFO("get b2c_enriched_calling_config_enabledvalue: %d",
                            int_value);
          }
          else
          {
