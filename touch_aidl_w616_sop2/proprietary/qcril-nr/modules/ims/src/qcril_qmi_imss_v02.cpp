...
void qcril_qmi_imss_request_set_ims_service_enable_config
(
...
void qcril_qmi_imss_request_set_ims_srv_status_v02(
  ...
  if (msg != nullptr) {
    do {
      ...
      QCRIL_LOG_INFO("message is: %s", msg->dump().c_str());
      if (msg->hasVonrEnabled()) {
        // Set vonr through nas module
        auto setVonrCapMsg = std::make_shared<NasSetVonrCapRequest>(msg->getVonrEnabled(),
            [msg](std::shared_ptr<Message> /*solicitedMsg*/,
              ...
              QCRIL_LOG_DEBUG("%s: result = %d", msg->dump().c_str(), res);
              if (res == RIL_E_SUCCESS  &&
                (msg->hasVolteEnabled() || msg->hasVideoTelephonyEnabled() ||
                msg->hasWifiCallingEnabled() || msg->hasUTEnabled())) {
                  qcril_qmi_imss_request_set_ims_service_enable_config(msg);
              } else {
                  msg->sendResponse(
...
//===========================================================================
// qcril_qmi_imss_request_set_ims_service_enable_config
//===========================================================================
void qcril_qmi_imss_request_set_ims_service_enable_config
(
  std::shared_ptr<QcRilRequestImsSetServiceStatusMessage> msg
