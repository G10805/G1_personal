...
qcril_qmi_ims_create_ServiceStatusInfoList(const imsa_service_status_ind_msg_v01 *qmi_data) {
  ...
  if (srvStatInfoList != nullptr) {
    ...
    if (qmi_data->ussd_service_status_valid) {
      srvStatInfoList->addToServiceStatusInfo(qcril_qmi_ims_create_ServiceStatusInfo(
          qcril::interfaces::CallType::USSD, qmi_data->ussd_service_status,
          qmi_data->ussd_service_rat_valid, qmi_data->ussd_service_rat));
    }
...
)
{
  ...
  switch(ims_config_item)
  {
    ...
    case qcril::interfaces::ConfigItem::VOWIFI_ENTITLEMENT_ID:
      ret =  QCRIL_QMI_RADIO_CONFIG_VOWIFI_ENTITLEMENT_ID;
      break;

    case qcril::interfaces::ConfigItem::B2C_ENRICHED_CALLING_SETTING:
      ret = QCRIL_QMI_RADIO_CONFIG_B2C_ENRICHED_CALLING_PREFERENCE;
      break;
...
)
{
  ...
  switch(radio_config_item)
  {
    ...
    case QCRIL_QMI_RADIO_CONFIG_VOWIFI_ENTITLEMENT_ID:
      ret = qcril::interfaces::ConfigItem::VOWIFI_ENTITLEMENT_ID;
      break;

    case QCRIL_QMI_RADIO_CONFIG_B2C_ENRICHED_CALLING_PREFERENCE:
      ret = qcril::interfaces::ConfigItem::B2C_ENRICHED_CALLING_SETTING;
      break;
