...
#define QCRIL_UIM_SW1_INCORRECT_PARAMS              0x6A
#define QCRIL_UIM_SW2_INCORRECT_PARAMS_P1_P2        0x86

#define QCRIL_UIM_SW1_WRONG_PARAMS                  0x6B
#define QCRIL_UIM_SW2_WRONG_PARAMS_P1_P2            0x00
...
*/
/*=========================================================================*/
static uint8_t qcril_uim_get_file_status_byte
(
  uint16_t                       data_len,
...
)
{
  ...
  uint8_t  file_status_byte = QCRIL_UIM_GET_RESPONSE_FILE_STATUS;
...
)
{
  ...
  {
    apdu_data_ptr->data_ptr[0] = (uint8_t)(cla & 0xFF);
    apdu_data_ptr->data_ptr[1] = (uint8_t)(ins & 0xFF);
    apdu_data_ptr->data_ptr[2] = (uint8_t)(p1 & 0xFF);
    apdu_data_ptr->data_ptr[3] = (uint8_t)(p2 & 0xFF);
  ...
  {
    apdu_data_ptr->data_ptr[4] = (uint8_t)(p3 & 0xFF);
    apdu_data_ptr->data_len = QCRIL_UIM_APDU_MIN_SIZE_PLUS_P3;
  }
  else if (p3 >= QCRIL_UIM_APDU_MAX_SHORT_APDU_SIZE)
  {
    apdu_data_ptr->data_ptr[4] = (uint8_t)(0x00);
    apdu_data_ptr->data_ptr[5] = (uint8_t)((p3 >> 8) & 0xFF);
    apdu_data_ptr->data_ptr[6] = (uint8_t)(p3 & 0xFF);
...
{
  uint8_t  select_resp_index = QCRIL_UIM_MAX_SELECT_RESP_COUNT;
  ...
  for (uint8_t i = 0; i < QCRIL_UIM_MAX_SELECT_RESP_COUNT; i++)
  {
      ...
      (logical_ch_rsp_ptr->select_response.data_ptr != NULL))
  {
    qcril_uim.select_response_info[select_resp_index].select_resp_ptr =
      (uint8_t *)qcril_malloc(logical_ch_rsp_ptr->select_response.data_len);
...
)
{
  ...
  uint8_t                               aid[QCRIL_UIM_MAX_AID_SIZE]   = {};
  uint8_t                               path[QCRIL_UIM_MAX_PATH_SIZE] = {};
  ...
  if (!io_ptr->pin2.empty())
  {
    ...
    verify_pin_params.pin_data.data_ptr = (uint8_t*)io_ptr->pin2.c_str();
...
)
{
  ...
  if (!io_ptr->pin2.empty())
  {
    ...
    /* PIN value */
    verify_pin_params.pin_data.data_len = (uint16_t)(io_ptr->pin2.length());
    verify_pin_params.pin_data.data_ptr = (uint8_t*)io_ptr->pin2.c_str();
...
)
{
  ...
  if (req_ptr->get_password().length() != 0)
  {
    ...
    /* PIN value */
    verify_pin_params.pin_data.data_len = (uint16_t)(req_ptr->get_password().length());
...
)
{
  ...
  if (rsp_ptr != NULL)
  {
    ...
    {
      char         *simResponse                                       = NULL;
      uint8_t       icc_attributes[QCRIL_UIM_GET_RESPONSE_MIN_SIZE+1] = {};
          ...
          (params_ptr->qmi_rsp_data.rsp_data.get_file_attributes_rsp.raw_value.data_len >= QCRIL_UIM_GET_RESPONSE_MIN_SIZE))
      {
        ...
        uint16_t data_len = (params_ptr->qmi_rsp_data.rsp_data.get_file_attributes_rsp.raw_value.data_len <= sizeof(icc_attributes)) ?
      ...
      {
        /* File size */
        icc_attributes[2] = (uint8_t)(params_ptr->qmi_rsp_data.rsp_data.get_file_attributes_rsp.file_size >> 8);
        icc_attributes[3] = (uint8_t)(params_ptr->qmi_rsp_data.rsp_data.get_file_attributes_rsp.file_size & 0xFF);
        /* File id */
        icc_attributes[4] = (uint8_t)(params_ptr->qmi_rsp_data.rsp_data.get_file_attributes_rsp.file_id >> 8);
        icc_attributes[5] = (uint8_t)(params_ptr->qmi_rsp_data.rsp_data.get_file_attributes_rsp.file_id & 0xFF);
        /* File type: EF */
        ...
        switch (params_ptr->qmi_rsp_data.rsp_data.get_file_attributes_rsp.file_type)
        {
          case QMI_UIM_FILE_TYPE_LINEAR_FIXED:
            ...
            /* Record size */
            icc_attributes[14] =
              (uint8_t)(params_ptr->qmi_rsp_data.rsp_data.get_file_attributes_rsp.record_size);
            break;
...
)
{
  ...
  if(
    ((params_ptr->qmi_rsp_data.rsp_data.logical_channel_rsp.sw1 == QCRIL_UIM_SW1_INCORRECT_PARAMS &&
      params_ptr->qmi_rsp_data.rsp_data.logical_channel_rsp.sw2 == QCRIL_UIM_SW2_INCORRECT_PARAMS_P1_P2)
  || (params_ptr->qmi_rsp_data.rsp_data.logical_channel_rsp.sw1 == QCRIL_UIM_SW1_WRONG_PARAMS &&
      params_ptr->qmi_rsp_data.rsp_data.logical_channel_rsp.sw2 == QCRIL_UIM_SW2_WRONG_PARAMS_P1_P2))
  &&  qcril_fci_value == QCRIL_UIM_FCI_VALUE_FCI_FALLBACK_FCP)
  {
      qmi_uim_open_logical_channel_params_type open_logical_ch_params             = {};
      uint8_t                                  aid_buffer[QMI_UIM_MAX_AID_LEN]    = {};
      uint16_t                                 aid_size                           = 0;
      ...
      open_logical_ch_params.slot         = (qmi_uim_slot_type)((uint8_t)QMI_UIM_SLOT_1 + qmi_ril_get_process_instance_id());
...
)
{
  ...
  if (qcril_uim.long_apdu_info.apdu_ptr == NULL)
  {
    ...
    qcril_uim.long_apdu_info.apdu_ptr = (uint8_t *)qcril_malloc(apdu_ind_ptr->total_len);
    if (qcril_uim.long_apdu_info.apdu_ptr == NULL)
    {
...
{
  uint16_t    mnc_3digit_mcc_list[] = {302, 310, 311, 312, 313, 314,
  ...
  if (file_id == QCRIL_UIM_FILEID_EF_IMSI)
  {
    ...
    /* Extract MCC */
    imsi_mcc  = (uint16_t)((rsp_data_ptr->data_ptr[1] >> 4) & 0x0F) * 100;
    imsi_mcc += (uint16_t)((rsp_data_ptr->data_ptr[2]) & 0x0F) * 10;
    imsi_mcc += (uint16_t)((rsp_data_ptr->data_ptr[2] >> 4) & 0x0F);
  ...
  {
    uint16_t        mcc_in_bcd        = 0;
    uint8_t         mnc_in_bcd        = 0;
    ...
    mcc_in_bcd = (uint16_t)(((rsp_data_ptr->data_ptr[9] & 0x03) << 8) +
...
)
{
  ...
  read_params_ptr->file_id.path.data_ptr = (unsigned char*)qcril_malloc(sizeof(uint8_t) * 4);
...
)
{
  uint8_t                               slot                 = qmi_ril_get_process_instance_id();
  qmi_uim_authenticate_params_type      auth_params          = {};
  uint8_t                               computed_aid[QMI_UIM_MAX_AID_LEN] = {};
  RIL_UIM_Errno                         ret_err              = RIL_UIM_E_INTERNAL_ERR;
  std::shared_ptr<RIL_UIM_SIM_IO_Response> rsp_ptr           = NULL;
...
)
{
  ...
  uint8_t                                  aid_buffer[QMI_UIM_MAX_AID_LEN]    = {};
  qcril_uim_fci_value_type                 qcril_fci_value                    = QCRIL_UIM_FCI_VALUE_FCP;
  uint8_t                                  ara_m_aid[]                        = {0xA0, 0x00, 0x00, 0x01, 0x51,
                                                                                 0x41, 0x43, 0x4C, 0x00};
...
)
{
  ...
  /* Check if P3 and data_len of the payload is not more than MAX APDU length*/
  else if ((p3 < QCRIL_UIM_APDU_MAX_SHORT_APDU_SIZE ) && ((strlen(data_ptr)/2) <= QCRIL_UIM_APDU_MAX_LONG_APDU_SIZE - QCRIL_UIM_APDU_MIN_SIZE_PLUS_P3))
  {
    in_apdu_length = QCRIL_UIM_APDU_MIN_SIZE_PLUS_P3 + strlen(data_ptr)/2;
  }
  /* Check if Payload is less than QCRIL_UIM_APDU_MAX_LONG_APDU_SIZE-8 bytes bcoz data_length
     in apdu_params is 16 bytes which needs to hold APDU(7 bytes)+ payload */
  else if ((p3 < QCRIL_UIM_APDU_MAX_LONG_APDU_SIZE) && ((strlen(data_ptr)/2) <= QCRIL_UIM_APDU_MAX_LONG_APDU_SIZE- QCRIL_UIM_LONG_APDU_MIN_SIZE_PLUS_P3))
  {
    in_apdu_length = QCRIL_UIM_LONG_APDU_MIN_SIZE_PLUS_P3 + strlen(data_ptr)/2;
  }
  else
  {
      QCRIL_LOG_ERROR( "Invalid Length in P3 or Data_Ptr P3:%x Data_length:%x",
                        p3, strlen(data_ptr)/2);
      ril_err = RIL_UIM_E_INVALID_ARGUMENTS;
      goto send_apdu_response;
  }
  /* Allocate memory and compose the raw APDU data */
  send_apdu_params.apdu.data_ptr = (uint8_t*) qcril_malloc(in_apdu_length);
