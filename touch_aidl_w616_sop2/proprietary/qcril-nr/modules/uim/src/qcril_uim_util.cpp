...
*/
/*=========================================================================*/
uint8_t qcril_uim_hexchar_to_bin
(
  char ch
)
{
...
*/
/*=========================================================================*/
uint8_t* qcril_uim_alloc_hexstring_to_bin
(
  const char * string_ptr,
  uint16_t   * buffer_size_ptr
)
{
  ...
  out_ptr = (uint8_t *)qcril_malloc(buffer_size);
...
*/
/*=========================================================================*/
uint16_t qcril_uim_hexstring_to_bin
(
  const char * string_ptr,
  uint8_t    * buffer_ptr,
...
{
  const uint8_t isim_aid[] = {0xA0, 0x00, 0x00, 0x00, 0x87, 0x10, 0x04};
  const uint8_t usim_aid[] = {0xA0, 0x00, 0x00, 0x00, 0x87, 0x10, 0x02};
  const uint8_t csim_aid[] = {0xA0, 0x00, 0x00, 0x03, 0x43, 0x10, 0x02};

  const uint8_t *app_type_aid = NULL;
  uint8_t        app_type_aid_len = 0;
...
(
  ...
  uint16_t                    first_level_df_path,
  qmi_uim_session_info_type * session_info_ptr,
  uint8_t                   * aid_ptr,
  uint16_t                    max_aid_len
)
{
  uint16_t      aid_size     = 0;
                  ...
                  i < QMI_UIM_MAX_APP_PER_CARD_COUNT; i++)
      {
            ...
            aid_size <= sizeof(aid_buffer))
        {
                     ...
                     aid_size) == 0)
          {
            ...
            uint16_t index = ((slot & 0xFF) << 8) | (i & 0xFF);
...
(
  uint8_t                                    slot,
  uint8_t                                    *iccid_data_ptr,
  uint8_t                                    *iccid_len_ptr
)
{
  qmi_uim_rsp_data_type                  read_rsp_data;
  qmi_uim_read_transparent_params_type   read_params;
  uint8_t                                iccid_path[QCRIL_UIM_ICCID_PATH_LEN] =
...
*/
/*=========================================================================*/
static uint8_t qcril_uim_find_base64_values
(
  boolean  find_char,
  char     input_char,
  uint8_t  input_index
...
*/
/*=========================================================================*/
uint8_t* qcril_uim_alloc_base64string_to_bin
(
...
)
{
  ...
  output_ptr = (uint8_t *)qcril_malloc(output_len);
  if (output_ptr == NULL)
  {
  ...
  while (i <= (input_len - 4))
  {
    ...
    uint32_t first  = qcril_uim_find_base64_values(TRUE, input_ptr[i++], 0);
    uint32_t second = qcril_uim_find_base64_values(TRUE, input_ptr[i++], 0);
    uint32_t third  = qcril_uim_find_base64_values(TRUE, input_ptr[i++], 0);
    uint32_t fourth = qcril_uim_find_base64_values(TRUE, input_ptr[i++], 0);

    uint64_t all_three = (first  << (3 * QCRIL_UIM_NUM_BITS_BASE64_CHAR)) +
...
)
{
  ...
  while (i < input_len)
  {
    ...
    uint32_t all_three = (first  << (2 * QCRIL_UIM_NUM_BITS_CHAR)) +
...
)
{
  ...
  slot_app_index = (uint16_t)(((uint16_t)slot << 8) | (uint16_t)i);
...
)
{
  ...
  switch (qcril_err_code)
  {
    ...
    case QMI_UIM_SERVICE_ERR_NO_EFFECT:
...
)
{
  ...
  {
    QCRIL_LOG_INFO("iccid is NULL or iccid_length is 0");
    return iccid_str;
  }
  ...
  QCRIL_LOG_INFO("iccid:  %s ", iccid_str);
  return iccid_str;
...
)
{
 ...
 QCRIL_LOG_INFO("%s, request: %d", __FUNCTION__, request_type);
  ...
  switch(request_type)
  {
    ...
    case QCRIL_UIM_REQUEST_SET_HLOS_PROV_PREFERENCE:
...
/*!
    @brief
    Function determines from QCRIL Database if GBA procedures are forced by
    OEM/MNO over ISIM application in the UICC.

    @return
    TRUE if forced, FALSE otherwise.
*/
/*=========================================================================*/
boolean qcril_uim_is_gba_forced_on_isim_app(void)
{
  bool gbaForcedOverImsApp = false;
  bool boolConfig;

  if (qcril_config_get(PERSIST_VENDOR_RADIO_FORCE_GBA_OVER_ISIM_APP, boolConfig) == E_SUCCESS)
  {
    gbaForcedOverImsApp = boolConfig;
  }
  QCRIL_LOG_INFO("%s, force_gba_over_isim_app: %d", __FUNCTION__, gbaForcedOverImsApp);
  return gbaForcedOverImsApp;
}
/* qcril_uim_is_gba_forced_on_isim_app */
