...
)
{
  ...
  token = req_ptr->get_token();
  QCRIL_LOG_INFO("Token value at gba_impi resp  function %x",token);
  ...
  if (rsp_ptr != NULL)
  {
    ...
    rsp_ptr->err = ril_err;
    rsp_ptr->token = token;
  QCRIL_LOG_INFO("Token value at gba_impi resp  function %x",rsp_ptr->token);
  }
...
)
{
  ...
  QCRIL_LOG_INFO("Token value at gba_init resp  function %x",token);
  ...
  if (rsp_ptr != NULL)
  {
        ...
        (params_ptr->qmi_rsp_data.rsp_data.gba_rsp.ks_naf.data_len > 0))
    {
      ...
      /* If plaintext, convert to base64 format when GBA is Not forced on ISIM app */
      if ((rsp_ptr->naf_type == RIL_UIM_GBA_KS_NAF_TYPE_PLAIN) &&
          (qcril_uim_is_gba_forced_on_isim_app() != TRUE))
      {
    ...
    rsp_ptr->err = ril_err;
    rsp_ptr->token = token;
  QCRIL_LOG_INFO("Token value at gba_init resp  function %x",rsp_ptr->token);
  }
  req_ptr->sendResponse(req_ptr, Message::Callback::Status::SUCCESS, rsp_ptr);
} /* qcril_uim_gba_resp */
...
)
{
  ...
  if(qcril_uim_is_gba_forced_on_isim_app() == TRUE)
  {
    /*Few MNOs supports GBA boostrap on ISIM app only, force app type as ISIM*/
    in_ptr->app_type = RIL_UIM_APPTYPE_ISIM;
    QCRIL_LOG_INFO("GBA forced on ISIM TRUE - updated app type: 0x%x", in_ptr->app_type);
  }
...
report_error:
  ...
  {
    rsp_ptr->err = err;
    rsp_ptr->token  = req_ptr->get_token();
  }
  req_ptr->sendResponse(req_ptr, Message::Callback::Status::SUCCESS, rsp_ptr);
...
)
{
  ...
  uint32_t                             token;
  uint16_t                             subStrLength = 0;
  uint16_t                             foundAtIndex = 0;
  ...
  QCRIL_LOG_INFO("slotId:0x%x, applicationType: 0x%x  token:%x",
                 in_ptr->slot_id,
                 in_ptr->app_type,token);

  if(qcril_uim_is_gba_forced_on_isim_app())
  {
    /*Few MNOs supports GBA boostrap on ISIM app only, force app type as ISIM*/
    in_ptr->app_type = RIL_UIM_APPTYPE_ISIM;
    QCRIL_LOG_INFO("GBA forced on ISIM - updated app type: 0x%x", in_ptr->app_type);
  }
  ...
  /* Domain name can sometimes be passed in username@domain format by AP clients.
     below code finds the index at which @ is found in the domain name, if any
     otherwise returns 0 */

  foundAtIndex = in_ptr->domain_name.find("@");

  if((qcril_uim_is_gba_forced_on_isim_app()) && (foundAtIndex != string::npos))
  {
    /* Get the length of substring after the 1st occurance of '@' */
    subStrLength = (in_ptr->domain_name.length() - foundAtIndex);

    /* Reassign the substring after '@' symbol in itself, to be sent as domain name to Modem */
    in_ptr->domain_name = in_ptr->domain_name.substr(foundAtIndex+1, subStrLength);

    QCRIL_LOG_INFO("After extracting domain name - FQDN length: %d, FQDN: %s",
                 in_ptr->domain_name.length(),
                 in_ptr->domain_name.c_str());
  }
  /* Add FQDN */
