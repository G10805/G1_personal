...
extern uint32_t qcril_uim_refresh_progress[QMI_UIM_MAX_CARD_COUNT];
/*===========================================================================
...
===========================================================================*/
static RIL_UIM_Errno qcril_uim_find_app_indices_for_session
(
  qcril_uim_subs_mode_pref   RIL_UIM_Subs_mode_pref,
  uint8_t                    slot,
  int                      * app_indices_ptr
)
{
  uint8_t usim_app_index = QMI_UIM_MAX_APP_PER_CARD_COUNT;
  uint8_t csim_app_index = QMI_UIM_MAX_APP_PER_CARD_COUNT;

  if(slot >= QMI_UIM_MAX_CARD_COUNT || slot < 0)
  {
    return RIL_UIM_E_INVALID_ARGUMENTS;
  }

  for (uint8_t index = 0; index < qcril_uim.card_status.card[slot].num_app &&
       index < QMI_UIM_MAX_APP_PER_CARD_COUNT; index++)
  {
    if(usim_app_index == QMI_UIM_MAX_APP_PER_CARD_COUNT && (qcril_uim.card_status.card[slot].application[index].app_type == QMI_UIM_APP_USIM ||
       qcril_uim.card_status.card[slot].application[index].app_type == QMI_UIM_APP_SIM))
    {
      usim_app_index = index;
    }
    if(csim_app_index == QMI_UIM_MAX_APP_PER_CARD_COUNT && (qcril_uim.card_status.card[slot].application[index].app_type == QMI_UIM_APP_CSIM ||
       qcril_uim.card_status.card[slot].application[index].app_type == QMI_UIM_APP_RUIM))
    {
      csim_app_index = index;
    }
  }

  switch(RIL_UIM_Subs_mode_pref) {
    case QCRIL_SUBS_MODE_1X:
        app_indices_ptr[0] = csim_app_index;
        break;
    case QCRIL_SUBS_MODE_GW:
        app_indices_ptr[0] = usim_app_index;
        break;
    case QCRIL_SUBS_MODE_GW_1X:
        app_indices_ptr[0] = usim_app_index;
        app_indices_ptr[1] = csim_app_index;
        break;

    default:
        QCRIL_LOG_INFO("Invalid subs mode pref");
        return RIL_UIM_E_INVALID_ARGUMENTS;
  }
...
(
  qcril_uim_subs_mode_pref                   RIL_subs_mode_pref,
  const RIL_UIM_SelectUiccSub              * RIL_uicc_subs_info_ptr,
  qmi_uim_change_prov_session_params_type  * session_params_ptr,
  int                                        RIL_app_index
)
{
  int RIL_slot          = 0;
  uint16_t prov_app_index = QCRIL_UIM_INVALID_SLOT_APP_INDEX_VALUE;
  ...
  RIL_slot = RIL_uicc_subs_info_ptr->slot;
  ...
  if ((RIL_subs_mode_pref == QCRIL_SUBS_MODE_GW) &&
      (RIL_uicc_subs_info_ptr->sub_type == UIM_SUBSCRIPTION_1))
  {
    if (((RIL_uicc_subs_info_ptr->act_status == UIM_UICC_SUBSCRIPTION_ACTIVATE) &&
        ...
        ((RIL_uicc_subs_info_ptr->act_status == UIM_UICC_SUBSCRIPTION_DEACTIVATE) &&
  ...
  else if ((RIL_subs_mode_pref == QCRIL_SUBS_MODE_1X) &&
           (RIL_uicc_subs_info_ptr->sub_type == UIM_SUBSCRIPTION_1))
  {
    if (((RIL_uicc_subs_info_ptr->act_status == UIM_UICC_SUBSCRIPTION_ACTIVATE) &&
  ...
  else if ((RIL_subs_mode_pref == QCRIL_SUBS_MODE_GW) &&
           (RIL_uicc_subs_info_ptr->sub_type == UIM_SUBSCRIPTION_2))
  {
    if (((RIL_uicc_subs_info_ptr->act_status == UIM_UICC_SUBSCRIPTION_ACTIVATE) &&
           ...
           (RIL_uicc_subs_info_ptr->sub_type == UIM_SUBSCRIPTION_2))
  {
        ...
        (qcril_uim.prov_session_info.session_state_1x_indexes[1] == QCRIL_UIM_PROV_SESSION_NOT_ACTIVATED)))
    {
      ...
      return RIL_UIM_E_SUCCESS;
           ...
           (RIL_uicc_subs_info_ptr->sub_type == UIM_SUBSCRIPTION_3))
  {
    if (((RIL_uicc_subs_info_ptr->act_status == UIM_UICC_SUBSCRIPTION_ACTIVATE) &&
...
)
{
      ...
      RIL_uicc_subs_info_ptr->act_status != UIM_UICC_SUBSCRIPTION_DEACTIVATE)
  {
    QCRIL_LOG_ERROR("Invalid Activation status request : 0x%x",
                                RIL_uicc_subs_info_ptr->act_status);
    return RIL_UIM_E_INTERNAL_ERR;
  }
  ...
  if (RIL_uicc_subs_info_ptr->act_status == UIM_UICC_SUBSCRIPTION_ACTIVATE)
  {
    uint8_t aid_len = qcril_uim.card_status.card[RIL_slot].application[RIL_app_index].aid_len;
  ...
  return qcril_uim_find_prov_session_type(session_params_ptr, app_type,
                                          RIL_uicc_subs_info_ptr->sub_type,
                                          RIL_subs_mode_pref,
                                          RIL_uicc_subs_info_ptr->act_status);
...
{
  uint8_t prov_app_index = QCRIL_UIM_INVALID_APP_INDEX_VALUE;
  uint8_t prov_app_slot  = QCRIL_UIM_INVALID_SLOT_INDEX_VALUE;
...
/*===========================================================================
  ...
  FUNCTION:  qcril_uim_convert_ril_perso_substate_to_hal

===========================================================================*/
/*!
    @brief
    Converts RIL_UIM_PersoSubstate to int32_t to be sent to Telephony over
    HAL interface.

    With latest Google IRadio.1.5 changes, Telephony will not handle the
    RIL_UIM_PersoSubstate values and will commonly handle HAL perso substate
    for both IRadio.1.4 and IRadio.1.5 PLs, also OEMHook interface specific
    enum values in RIL_UIM_PersoSubstate, needs to be retained.
    Thus coverting the values to integer for now to have common codebase.

    @return
    int32_t.
*/
/*=========================================================================*/
static int32_t qcril_uim_convert_ril_perso_substate_to_hal
(
  RIL_UIM_PersoSubstate   rilPersoSubstate
)
{
  int32_t halPersoSubstate = RIL_UIM_PERSOSUBSTATE_UNKNOWN;

  if(rilPersoSubstate < RIL_UIM_PERSOSUBSTATE_SIM_SPN || rilPersoSubstate > RIL_UIM_PERSOSUBSTATE_SIM_NS_SP_PUK)
  {
    return rilPersoSubstate;
  }

  switch(rilPersoSubstate)
  {
    case RIL_UIM_PERSOSUBSTATE_SIM_SPN:
      halPersoSubstate = 25; //SIM_SPN
      break;
    case RIL_UIM_PERSOSUBSTATE_SIM_SPN_PUK:
      halPersoSubstate = 26; //SIM_SPN_PUK
      break;
    case RIL_UIM_PERSOSUBSTATE_SIM_SP_EHPLMN:
      halPersoSubstate = 27; //SIM_SP_EHPLMN
      break;
    case RIL_UIM_PERSOSUBSTATE_SIM_SP_EHPLMN_PUK:
      halPersoSubstate = 28; //SIM_SP_EHPLMN_PUK
      break;
    case RIL_UIM_PERSOSUBSTATE_SIM_ICCID:
      halPersoSubstate = 29; //SIM_ICCID
      break;
    case RIL_UIM_PERSOSUBSTATE_SIM_ICCID_PUK:
      halPersoSubstate = 30; //SIM_ICCID_PUK
      break;
    case RIL_UIM_PERSOSUBSTATE_SIM_IMPI:
      halPersoSubstate = 31; //SIM_IMPI
      break;
    case RIL_UIM_PERSOSUBSTATE_SIM_IMPI_PUK:
      halPersoSubstate = 32; //SIM_IMPI_PUK
      break;
    case RIL_UIM_PERSOSUBSTATE_SIM_NS_SP:
      halPersoSubstate = 33; //SIM_NS_SP
      break;
    case RIL_UIM_PERSOSUBSTATE_SIM_NS_SP_PUK:
      halPersoSubstate = 34; //SIM_NS_SP_PUK
      break;
    default:
      QCRIL_LOG_ERROR("Invalid persoSubstate value %d ", rilPersoSubstate);
      break;
  }

  return halPersoSubstate;
} /* qcril_uim_convert_ril_perso_substate_to_hal */
...
)
{
  ...
  *ril_aid_ptr = qcril_uim_alloc_bin_to_hexstring((const uint8_t*)qmi_aid_ptr,
                                                  qmi_aid_len);
...
)
{
  ...
  /* Convert the Label in hex to a null terminated ASCII string
     The function qcril_uim_alloc_bin_to_hexstring will perform the allocation & data conversion */
  ril_label_ptr = qcril_uim_alloc_bin_to_hexstring((const uint8_t*)qmi_label_rsp_ptr->label_value,
...
)
{
  int                   app_index     = 0;
  int                   max_apps      = 0;
  uint8_t               isim_index    = QCRIL_UIM_INVALID_APP_INDEX_VALUE;
  ...
  for (app_index = 0; app_index < max_apps; app_index++)
  {
    ...
    RIL_UIM_PersoSubstate rilPersoSubstate = ril_card_status_ptr->applications[app_index].perso_substate;
    int32_t halPersoSubstate = qcril_uim_convert_ril_perso_substate_to_hal(rilPersoSubstate);
    ril_card_status_ptr->applications[app_index].perso_substate = (RIL_UIM_PersoSubstate)halPersoSubstate;
    QCRIL_LOG_INFO("Converted rilPersoSubstate: %d to halPersoSubstate: %d ",(int)rilPersoSubstate, halPersoSubstate);
...
)
{
  ...
  uint8_t                         instance_slot     = qmi_ril_get_sim_slot();
  ...
  if (new_status_ptr->card[slot].card_state == QMI_UIM_CARD_STATE_PRESENT)
  {
    ...
    uint8_t iccid_data[QCRIL_UIM_ICCID_LEN];
    ...
    if( qcril_uim_refresh_progress[slot] != QCRIL_CARD_STATUS_REFRESH )
    {
        QCRIL_LOG_INFO( "Refresh slot values is %d ",qcril_uim_refresh_progress[slot]);
      ...
      new_status_ptr->card[slot].card_state == QMI_UIM_CARD_STATE_ABSENT)
  {
          ...
          (new_status_ptr->card[slot].card_error == QMI_UIM_CARD_ERROR_POWER_DOWN)) ))
    {
      ...
      {
        qcril_uim_clear_encrypted_pin_after_card_reset((uint8_t)slot);
      }
  ...
  if (ril_err == RIL_UIM_E_SUCCESS)
  {
    /* Convert the raw ATR bytes to a null terminated ASCII string
     ...
     atr_ptr = qcril_uim_alloc_bin_to_hexstring(
                 (const uint8_t*)params_ptr->qmi_rsp_data.rsp_data.get_atr_rsp.atr_response.data_ptr,
                 (uint16_t)params_ptr->qmi_rsp_data.rsp_data.get_atr_rsp.atr_response.data_len);
...
{
  qcril_provisioning_response_info_type               provision_info               = {};
  qcril_uim_prov_session_state_type                   session_state                = {};
  const qcril_uim_uicc_subs_info_type               * sub_info_ptr                 = NULL;
  uim_session_type                                    session_type                 = UIM_SESSION_TYPE_PRI_GW_PROV;
  qcril_provisioning_response_info                    provisioning_response_info   = {};
  std::shared_ptr<qcril_provisioning_response_info>   rsp_ptr                      = NULL;

  /* Indicates expected number of responses which actually depends on
     qcril_uim_subs_mode_pref which is part of UimChangeSubscriptionReqMsg request */
  uint8_t                                             number_of_responses_expected = 0;
  ...
  provisioning_response_info = req_ptr->get_provisioning_response_info();
      ...
      (sub_info_ptr->uicc_subs_info.sub_type == UIM_SUBSCRIPTION_1))
  {
    ...
    number_of_responses_expected =1;
  }
  ...
  else if ((sub_info_ptr->subs_mode_pref == QCRIL_SUBS_MODE_GW_1X) &&
           (sub_info_ptr->uicc_subs_info.sub_type == UIM_SUBSCRIPTION_1))
  {
    /* Indicates that it is the response for first change prov request we attempted for
       QCRIL_SUBS_MODE_GW_1X mode */
    if (provisioning_response_info.response_info_len == 0)
    {
      session_type = UIM_SESSION_TYPE_PRI_GW_PROV;
    }
    else
    ...
    number_of_responses_expected = 2;
  }
  else if ((sub_info_ptr->subs_mode_pref == QCRIL_SUBS_MODE_GW_1X) &&
           (sub_info_ptr->uicc_subs_info.sub_type == UIM_SUBSCRIPTION_2))
  {
    /* Indicates that it is the response for first change prov request we attempted for
       QCRIL_SUBS_MODE_GW_1X mode. So, that indicates it is for GW */
    if (provisioning_response_info.response_info_len == 0)
    {
      session_type = UIM_SESSION_TYPE_SEC_GW_PROV;
    }
  ...
  req_ptr->set_provisioning_response_info(provision_info);

  provisioning_response_info = req_ptr->get_provisioning_response_info();

  QCRIL_LOG_DEBUG("Number of responses expected %d ,response_info_length %d\n",
                     number_of_responses_expected,provisioning_response_info.response_info_len);

  if(number_of_responses_expected == provisioning_response_info.response_info_len)
  {
    rsp_ptr = std::make_shared<qcril_provisioning_response_info>();

    if (rsp_ptr != NULL)
    {
      rsp_ptr->response_info_len = provisioning_response_info.response_info_len;
      for(uint8_t index = 0 ;index < rsp_ptr->response_info_len; index++)
      {
        rsp_ptr->response_info[index] = provisioning_response_info.response_info[index];
      }
    }
    else
    {
      QCRIL_LOG_ERROR("rsp_ptr is NULL ");
    }

    QCRIL_LOG_INFO("Sending Provisioning reponse ");
    req_ptr->sendResponse(req_ptr, Message::Callback::Status::SUCCESS, rsp_ptr);
  }
...
)
{
  ...
  RIL_UIM_Errno                               readIccidStatus       = RIL_UIM_E_INTERNAL_ERR;
      ...
      if (status_change_ind_ptr->card[i].card_state != qcril_uim.card_status.card[i].card_state)
      {
        if(status_change_ind_ptr->card[i].atr_len == 0)
        {
          ...
          if (qmi_err_code == 0 && rsp_data.qmi_err_code == 0)
          {
            ...
            rsp_data.rsp_data.get_atr_rsp.atr_response.data_ptr = nullptr;
          }
        }
        ...
        if (status_change_ind_ptr->card[i].iccid_len == 0)
        {
          ...
          readIccidStatus = qcril_uim_read_iccid(i,
                                     &status_change_ind_ptr->card[i].iccid[0],
                                     (uint8_t *)&status_change_ind_ptr->card[i].iccid_len);

          if(readIccidStatus != RIL_UIM_E_SUCCESS)
          {
            /*eUICC without any Operational or Provisioning profile may not have ICCID file
              on the card, thus explicitely mark iccid as invalid */

            memset(&status_change_ind_ptr->card[i].iccid[0],
                   0x00,
                   status_change_ind_ptr->card[i].iccid_len);

            status_change_ind_ptr->card[i].iccid_len = 0;

            QCRIL_LOG_INFO("ICCID Invalid, read from cache/card failed");
  ...
  if ((status_change_ind_ptr->card[slot].card_state != QMI_UIM_CARD_STATE_PRESENT) && (qcril_uim_refresh_progress[slot] == QCRIL_CARD_STATUS_REFRESH))
  {
      /**Resetting Refresh related global since ongoing Refresh was ended at modem*/
      qcril_uim_refresh_progress[slot] = 0;
      QCRIL_LOG_INFO("Card_state is %d",status_change_ind_ptr->card[slot].card_state);
      QCRIL_LOG_INFO("Resetting Internal Refresh cb Since Refresh has been ended ");
  }

  /* Determine change in card status only if it is not invalid */
  if ((card_status_invalid == NULL || !card_status_invalid[slot]) &&(qcril_uim_refresh_progress[slot] != QCRIL_CARD_STATUS_REFRESH))
  {
  ...
  else
  {
    if(qcril_uim_refresh_progress[slot] == QCRIL_CARD_STATUS_REFRESH)
    {
      ...
      QCRIL_LOG_ERROR("Skipping Card_Status indication Since Refresh is going on ");
      qcril_uim_copy_card_status(&qcril_uim.card_status,
                                 status_change_ind_ptr,
                                 sizeof(qmi_uim_card_status_type));
    }
    ...
    {
        /*When card_status_invalid pointer was not null*/
  ...
  if (status_changed)
  {
       ...
       qcril_uim.card_status.card[slot].card_state == QMI_UIM_CARD_STATE_PRESENT)
    {
      ...
      if (status_change_ind_ptr->num_slots > 0)
      {
...
/*===========================================================================
  ...
  FUNCTION:  qcril_uim_get_number_of_sessions

===========================================================================*/
static uint8_t qcril_uim_get_number_of_sessions
(
  qcril_uim_subs_mode_pref subs_mode
)
{
  ...
  switch(subs_mode)
  {
    ...
    default:
      QCRIL_LOG_INFO("Invalid subs_mode 0x%x", subs_mode);
  }
...
)
{
  ...
  qcril_provisioning_response_info          provision_info         = {};
  qcril_uim_prov_session_info_type          orig_session_info      = {};
  int                                       app_indices[2]         = {};
  uint8_t                                   provisining_response_info_len = 0;
  std::shared_ptr<qcril_provisioning_response_info>   rsp_ptr             = NULL;
  ...
  QCRIL_LOG_INFO("qcril_uim_process_change_subscription: subs_mode_pref: 0x%x, slot: 0x%x, sub_type: 0x%x, act_status: 0x%x",
  ...
  provision_info.response_info_len =
    qcril_uim_get_number_of_sessions(ril_uicc_subs_info_ptr->subs_mode_pref);

  for(uint8_t index = 0; index < provision_info.response_info_len; index++)
  {
    provision_info.response_info[index].status = QCRIL_PROVISION_STATUS_FAILURE;
  }

  memcpy(&orig_session_info, &qcril_uim.prov_session_info,
         sizeof(qcril_uim_prov_session_info_type));

  qcril_uim_find_app_indices_for_session(ril_uicc_subs_info_ptr->subs_mode_pref,
                                         ril_uicc_subs_info_ptr->uicc_subs_info.slot,
                                         &app_indices[0]);

  for(uint8_t index = 0; index < provision_info.response_info_len; index++)
  {
    uint8_t appOffset = 1; //appOffset is 1 for subs_mode_pref - GW & GW_1X
    if(ril_uicc_subs_info_ptr->subs_mode_pref == QCRIL_SUBS_MODE_1X)
    {
      appOffset = 2;
    }

    /* Ignore requests if already activated/deactivated and send SUCCESS */
    if (qcril_uim_ignore_change_subscription((qcril_uim_subs_mode_pref)(ril_uicc_subs_info_ptr->subs_mode_pref & (index + appOffset)),
                                             &ril_uicc_subs_info_ptr->uicc_subs_info,
                                             &change_session_params,
                                             app_indices[index]) == RIL_UIM_E_SUCCESS)
    {
      provision_info.response_info[index].err_code      = RIL_UIM_E_SUCCESS;
      provision_info.response_info[index].status        = QCRIL_PROVISION_STATUS_SUCCESS;
      provision_info.response_info[index].session_type  = (uim_session_type)change_session_params.session_type;

      msg_ptr->set_provisioning_response_info(provision_info.response_info[index]);
      continue;
    //goto send_resp;
    }

    /* Fill the aid, session type, activation type */
    provision_info.response_info[index].err_code = (uint32_t)qcril_uim_fill_change_session_params(
                                                   (qcril_uim_subs_mode_pref)(ril_uicc_subs_info_ptr->subs_mode_pref & (index + appOffset)),
                                                   &ril_uicc_subs_info_ptr->uicc_subs_info,
                                                   &change_session_params,
                                                   app_indices[index]);

    if (provision_info.response_info[index].err_code != RIL_UIM_E_SUCCESS)
    {
      //goto send_resp;
      msg_ptr->set_provisioning_response_info(provision_info.response_info[index]);
      continue;
    ...
    else if (ril_uicc_subs_info_ptr->uicc_subs_info.act_status == UIM_UICC_SUBSCRIPTION_DEACTIVATE)
    {
    ...
    {
      msg_ptr->set_provisioning_response_info(provision_info.response_info[index]);
      QCRIL_LOG_ERROR("Error in sending QCRIL_UIM_REQUEST_CHANGE_PROV_SESSION request");
    }
  }

  provisining_response_info_len = msg_ptr->get_provisioning_response_info().response_info_len;

  if ((provisining_response_info_len == 1 &&
     (ril_uicc_subs_info_ptr->subs_mode_pref == QCRIL_SUBS_MODE_GW || ril_uicc_subs_info_ptr->subs_mode_pref == QCRIL_SUBS_MODE_1X)) ||
     (provisining_response_info_len == 2 && ril_uicc_subs_info_ptr->subs_mode_pref == QCRIL_SUBS_MODE_GW_1X))
  {
    rsp_ptr = std::make_shared<qcril_provisioning_response_info>();

    if (rsp_ptr != NULL)
    {
      rsp_ptr->response_info_len = provision_info.response_info_len;
      for(uint8_t index = 0 ;index < rsp_ptr->response_info_len; index++)
      {
        rsp_ptr->response_info[index] = provision_info.response_info[index];
      }
...
)
{
  ...
  slot = (qmi_uim_slot_type)((uint8_t)QMI_UIM_SLOT_1 + qmi_ril_get_process_instance_id());
...
/*=========================================================================
  ...
  FUNCTION:  qcril_uim_handle_deep_sleep_notification

===========================================================================*/
void qcril_uim_handle_deep_sleep_notification
(
  const std::shared_ptr<UimDeepSleepNotifyMsg> msg_ptr
)
{
  qmi_uim_slot_type                 slot_id         = QMI_UIM_SLOT_1;
  RIL_UIM_DeepSleepState            deepSleepState  = RIL_UIM_DEEP_SLEEP_MODE_EXIT;
  std::shared_ptr<RIL_UIM_Errno>    err             = std::make_shared<RIL_UIM_Errno>(RIL_UIM_E_INTERNAL_ERR);
  qcril_uim_card_status_e_type      new_card_status = QCRIL_CARD_STATUS_POWERDOWN_FAILED;
  qmi_uim_rsp_data_type             rsp_data;
  int                               qmi_err_code = 0;
  ...
  deepSleepState = msg_ptr->get_deep_sleep_state();
  ...
  switch((uint8_t)qmi_ril_get_process_instance_id())
  {
    ...
    case 2:
      ...
      break;
    default:
      QCRIL_LOG_ERROR( " Invalid slot_id: 0x%x\n", (uint8_t)qmi_ril_get_process_instance_id());
      *err = RIL_UIM_E_INVALID_ARGUMENTS;
      goto send_deep_sleep_req_error;
  }

  if (deepSleepState == RIL_UIM_DEEP_SLEEP_MODE_ENTER)
  {
    qmi_uim_power_down_params_type power_down_params = {};

    power_down_params.slot = slot_id;

    qmi_err_code = qcril_uim_send_qmi_sync_msg(QCRIL_UIM_REQUEST_POWER_DOWN,
                                     &power_down_params,
                                     &rsp_data);

    QCRIL_LOG_ERROR( " qmi_err_code:0x%x  rsp_data.qmi_err_code:0x%x\n", qmi_err_code,
                       rsp_data.qmi_err_code);

    *err = qcril_uim_convert_err_value(rsp_data.qmi_err_code);

    msg_ptr->sendResponse(msg_ptr, Message::Callback::Status::SUCCESS, err);

  }
  else if (deepSleepState == RIL_UIM_DEEP_SLEEP_MODE_EXIT)
  {
    qmi_uim_power_up_params_type power_up_params;
    ...
    qmi_err_code = qcril_uim_send_qmi_sync_msg(QCRIL_UIM_REQUEST_POWER_UP,
                                     &power_up_params,
                                     &rsp_data);
...
send_deep_sleep_req_error:
  /* Send indication to NAS about power UP request failure */
  if (deepSleepState == RIL_UIM_DEEP_SLEEP_MODE_ENTER || deepSleepState == RIL_UIM_DEEP_SLEEP_MODE_EXIT)
  {
    if(deepSleepState == RIL_UIM_DEEP_SLEEP_MODE_EXIT)
    {
      new_card_status = QCRIL_CARD_STATUS_POWERUP_FAILED;
    }

    qcril_uim_update_cm_card_status((uint8_t)qmi_ril_get_process_instance_id(), new_card_status);
  }
  msg_ptr->sendResponse(msg_ptr, Message::Callback::Status::SUCCESS, err);
} /* qcril_uim_handle_deep_sleep_notification */
