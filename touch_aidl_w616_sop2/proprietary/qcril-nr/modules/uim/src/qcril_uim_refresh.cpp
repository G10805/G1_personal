...
uint32_t qcril_uim_refresh_progress[QMI_UIM_MAX_CARD_COUNT];
/*===========================================================================
...
*/
/*=========================================================================*/
static uint16_t qcril_uim_ef_pbr_resp_calculate_files
(
...
(
  qmi_uim_session_type                    qmi_session_type,
  uint32_t                              * telecom_mask_ptr
)
{
  uint8_t                   prov_app_index    = 0;
  uint8_t                   slot              = 0;
...
)
{
  ...
  if (refresh_pbr.record_index == refresh_pbr.num_records)
  {
    ...
    if(qcril_uim_convert_session_type_to_mask(req_ptr->get_session_type(), &reg_mask))
    {
      ...
      uint8_t app_index = QCRIL_UIM_INVALID_SLOT_INDEX_VALUE;
...
)
{
  ...
  if(slot >= QMI_UIM_MAX_CARD_COUNT)
  {
     QCRIL_LOG_INFO( "Invalid Slot index %d\n",slot);
     return;
  }
  ...
  switch (refresh_ind_ptr->refresh_event.refresh_stage)
  {
    ...
    case QMI_UIM_REFRESH_STAGE_START:
      switch (refresh_ind_ptr->refresh_event.refresh_mode)
      {
        ...
        case QMI_UIM_REFRESH_MODE_RESET:
          qcril_uim_refresh_progress[slot] = QCRIL_CARD_STATUS_REFRESH;
          QCRIL_LOG_INFO( "slot %d Refresh values is %d ",slot,qcril_uim_refresh_progress[slot]);
          qcril_uim_refresh_handle_mode_change(refresh_ind_ptr);
    ...
    case QMI_UIM_REFRESH_STAGE_END_SUCCESS:
      ...
      if(refresh_ind_ptr->refresh_event.refresh_mode == QMI_UIM_REFRESH_MODE_RESET)
      {
          qcril_uim_refresh_progress[slot] = qcril_uim.card_status.card[slot].card_state;

          QCRIL_LOG_INFO( "slot %d Refresh values is %d ",slot,qcril_uim_refresh_progress[slot]);
          if(qcril_uim.card_status.card[slot].card_state ==  QMI_UIM_CARD_STATE_PRESENT)
          {

             QCRIL_LOG_INFO( "Triggering card_status update to clients");
             qcril_uim_update_cm_card_status(slot,QCRIL_CARD_STATUS_UP);
          }
        ...
        (prov_app_slot < QMI_UIM_MAX_CARD_COUNT))
    {
      ...
      if (qcril_uim.card_status.card[prov_app_slot].application[prov_app_index].aid_len != 0)
      {
        char *aid = qcril_uim_alloc_bin_to_hexstring(
          (const uint8_t*)qcril_uim.card_status.card[prov_app_slot].application[prov_app_index].aid_value,
