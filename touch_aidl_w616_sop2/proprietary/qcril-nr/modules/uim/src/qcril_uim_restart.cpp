...
)
{
  ...
  int                                       res       = 0;
  uint16_t                                  slot_app  = 0;
  uint8_t                                   pin1_data[QMI_UIM_MAX_ENCRYPTED_PIN_LEN];
...
*/
/*=========================================================================*/
static uint8_t qcril_uim_match_aid_in_encrypted_pin_cache
(
  uint8_t         aid_len,
  const char    * aid_ptr
)
...
{
  uint8_t app_index = 0;
  uint8_t pin_index = QCRIL_UIM_MAX_ENCRYPTED_PIN_INFO;
...
*/
/*=========================================================================*/
static uint8_t qcril_uim_get_empty_encrypted_pin_index
(
  void
)
{
...
/*===========================================================================
  ...
  FUNCTION:  qcril_uim_set_hlos_provisioning_preference

===========================================================================*/
/*!
    @brief
    Issues the QMI command for sending hlos provisioning preference.

    @return
    result_code of the QMI call.
*/
/*=========================================================================*/
int qcril_uim_set_hlos_provisioning_preference
(
  uint8_t         hlosProvPreference
)
{
  int result_code                           = -1;
  qmi_uim_hlos_prov_preference_req_type     hlosProvParams;
  qmi_uim_rsp_data_type                     rsp_data;

  memset(&hlosProvParams, 0, sizeof(qmi_uim_hlos_prov_preference_req_type));
  memset(&rsp_data, 0x00, sizeof(rsp_data));

  QCRIL_LOG_DEBUG( "qcril_uim_set_hlos_provisioning_preference  hlosProvPreference: %d \n", (int)hlosProvPreference);

  if (hlosProvPreference != 0)
  {
    QCRIL_LOG_DEBUG( "Android R: HLOS trying to set Automatic UIM provisioning: %d \n", (int)hlosProvPreference);
  }

  hlosProvParams.hlosProvPreference = hlosProvPreference;

  result_code = qcril_uim_send_qmi_sync_msg(QCRIL_UIM_REQUEST_SET_HLOS_PROV_PREFERENCE,
                                            &hlosProvParams,
                                            &rsp_data);
  if(result_code < 0)
  {
    QCRIL_LOG_ERROR( "Error for hlos_prov_pref, result_code: %d, qmi_err_code: 0x%X\n",
                     result_code, rsp_data.qmi_err_code);
  }

  QCRIL_LOG_INFO( "=DBG= Sync Response recvd for hlos_prov_pref, result_code: %d, qmi_err_code: 0x%X\n",
                     result_code, rsp_data.qmi_err_code);
  return result_code;
