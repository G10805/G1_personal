/******************************************************************************
#  Copyright (c) 2010, 2013, 2017, 2020, 2023 Qualcomm Technologies, Inc.
#  All Rights Reserved.
...
#include <interfaces/sms/QcRilEventSmsRawReadMessage.h>
#include <interfaces/sms/QcRilEventSmsResetAckNeededForPowerOptMessage.h>
...
static qmi_ril_gen_operational_status_type operational_status
...
)
{
...

  {
     /* Initialize internal data */
     {
       ...
       i_ptr->mt_pending_ack_expry_tmr = TimeKeeper::no_timer;
     }
     ...
     int int_config;
     if (qcril_config_get(PERSIST_VENDOR_RADIO_MT_SMS_ACK, int_config) == E_SUCCESS)
     {
       qmi_ril_sms_mt_expiry_window_len = int_config;
     ...
     if (qcril_config_get(PERSIST_VENDOR_RADIO_SMS_LINK_TIMER, int_config) == E_SUCCESS)
     {
       qmi_ril_sms_link_timer = (int_config > 0 && int_config <= 5) ? int_config : 5;
     }
     ...
     std::string config;
     if (qcril_config_get(ALL_BC_MSG, config) == E_SUCCESS)
     {
       ...
       else if (!config.compare("gsm"))
       {
         feature_all_bc_msg_3gpp = 1;
         feature_all_bc_msg_3gpp2 = 0;
       }
       else if (!config.compare("cdma"))
       {
     ...
     is_force_on_dc = 0;
     bool bool_config;
     if (qcril_config_get(PERSIST_VENDOR_RADIO_FORCE_ON_DC, bool_config) == E_SUCCESS)
     {
        is_force_on_dc = bool_config;
     }
...
void qcril_sms_perform_transport_layer_info_initialization()
{
  ...
  if( i_ptr )
  {
    ...
    i_ptr->transport_layer_info.block_sms_on_1x = FALSE;
    bool bool_config;
    if (qcril_config_get(PERSIST_VENDOR_RADIO_BLOCK_SMS_ON_1X, bool_config) == E_SUCCESS)
    {
        i_ptr->transport_layer_info.block_sms_on_1x = bool_config;
    }
...
/*===========================================================================
  ...
  FUNCTION:  qcril_mo_smsc_error_check

===========================================================================*/
/*!
    @brief
    Does error checking on SMSC address.  This checking should be
    done before sending an SMS, or writing an SMS to SIM.

    @return
    TRUE if SMSC address passes error-checking; FALSE otherwise.
*/
/*=========================================================================*/
boolean qcril_mo_smsc_error_check
(
  const char *smsc
)
{
  ...
  /*-----------------------------------------------------------------------*/
  QCRIL_LOG_FUNC_ENTRY();
  /* Check if the SMSC address is null */
  if ( smsc == NULL )
  {
    QCRIL_LOG_ERROR("SMSC address is NULL!");
    err_check_passed = FALSE;
  }

  /* Check if the length exceeds the maximum */
  else if ( ( strlen( smsc ) ) > WMS_SMSC_ADDRESS_LENGTH_MAX_V01 )
  {
    QCRIL_LOG_ERROR("SMSC address exceeds maximum allowable length!");
    err_check_passed = FALSE;
  }
...
)
{
  ...
  do
  {
    auto lte_sms_status = qcril_qmi_sms_external_get_lte_sms_status();
    // service status check
    auto is_voice_over_lte =
            qcril_qmi_sms_external_query_is_voice_over_lte_registered();

    if (!wms_ready_atleast_limited_3gpp() &&
        !(is_voice_over_lte && (NasSmsStatus::STATUS_3GPP == lte_sms_status))
       ) {
    ...
    if ( !qcril_mo_sms_error_check(msg->getPdu().c_str()) &&
         !qcril_mo_smsc_error_check(msg->getSmscPdu().c_str()))
    {
    ...
    auto module = getSmsModule();
    assert(module != nullptr);
    auto res = module->getPendingMessageList().insert(msg);
    if (!res.second)
    {
    ...
    if (!qcril_sms_fill_wms_payload(
                                   ...
                                   (uint32_t *)&request_msg.raw_message_data.raw_message_len,
    ...
    if (msg->getLinkCtlEnable())
    {
      ...
      request_msg.link_control_mode_valid = TRUE;
      request_msg.link_control_mode = WMS_LINK_CONTROL_ENABLED_ONE_V01;
    } else {
      request_msg.link_control_mode_valid = TRUE;
      request_msg.link_control_mode = WMS_LINK_CONTROL_DISABLED_V01;
    }
...
//===========================================================================
// qcril_qmi_wms_ims_rat_info_to_ril_radio_tech
//===========================================================================
static RIL_RadioTechnology qcril_qmi_wms_ims_rat_info_to_ril_radio_tech(
    wms_ims_rat_info_enum_v01 wms_ims_rat) {
  RIL_RadioTechnology ret = RIL_RadioTechnology::RADIO_TECH_UNKNOWN;
  switch (wms_ims_rat) {
    case WMS_IMS_RAT_WCDMA_V01:
      ret = RIL_RadioTechnology::RADIO_TECH_UMTS;
      break;
    case WMS_IMS_RAT_LTE_V01:
      ret = RIL_RadioTechnology::RADIO_TECH_LTE;
      break;
    case WMS_IMS_RAT_NR_V01:
      ret = RIL_RadioTechnology::RADIO_TECH_5G;
      break;
    case WMS_IMS_RAT_WLAN_V01:
      ret = RIL_RadioTechnology::RADIO_TECH_WIFI;
      break;
    case WMS_IMS_RAT_CIWLAN_V01:
      ret = RIL_RadioTechnology::RADIO_TECH_C_IWLAN;
      break;
    case WMS_IMS_RAT_NONE_V01:
    default:
      ret = RIL_RadioTechnology::RADIO_TECH_UNKNOWN;
      break;
  }
...
)
{
  ...
  int message_ref = -1;
  int error_code = -1;
  int network_error_rp_cause = -1;
  int cause_code_3gpp2 = -1;
  ...
  else
  {
    ...
    {
      QCRIL_LOG_INFO("QMI_WMS_RAW_SEND_RESP received with error %d",
                      raw_send_resp->resp.error);
      ...
      if (raw_send_resp->resp.error == QMI_ERR_CAUSE_CODE_V01)
      {
        if (raw_send_resp->gw_cause_info_valid)
        {
          ...
          network_error_rp_cause = raw_send_resp->gw_cause_info.rp_cause;
        ...
        else if (raw_send_resp->cause_code_valid)
        {
          ...
          cause_code_3gpp2 = raw_send_resp->cause_code;
          QCRIL_LOG_INFO("QMI_WMS_RAW_SEND_RESP cause code %s",
      ...
      if (raw_send_resp->resp.error == QMI_ERR_CAUSE_CODE_V01)
      {
        if ((raw_send_resp->gw_cause_info_valid) &&
            ...
            (raw_send_resp->gw_cause_info.rp_cause == 38) ||
            (raw_send_resp->gw_cause_info.rp_cause == 42) ||
            (raw_send_resp->gw_cause_info.rp_cause == 69)))
        {
          retry_status = TRUE;
        }
      ...
      {
        bool is_device_registered_on_cs =
            qcril_qmi_sms_external_query_is_device_registered_for_cs_domain();
      ...
      {
        if (raw_send_resp->message_delivery_failure_cause_valid)
        {
          switch (raw_send_resp->message_delivery_failure_cause)
          {
            case WMS_MESSAGE_BLOCKED_DUE_TO_CALL_CONTROL_V01:
              ril_err = RIL_E_SMS_BLOCKED_DUE_TO_CALL;
              break;
            case WMS_MESSAGE_BLOCKED_DUE_TO_VOICE_OR_DATA_CALL_V01:
              ril_err = RIL_E_SIMULTANEOUS_SMS_AND_CALL_NOT_ALLOWED;
              break;
            case WMS_MESSAGE_BLOCKED_DUE_TO_ACCESS_BARRING_V01:
              ril_err = RIL_E_SMS_ACCESS_BARRED;
              break;
            default:
              break;
          }
  ...
  else if (msg->get_message_id() == RilRequestImsSendSmsMessage::get_class_message_id())
  {
    ...
    RIL_RadioTechnology rat = RIL_RadioTechnology::RADIO_TECH_UNKNOWN;
    if (raw_send_resp->ims_rat_info_valid) {
      rat = qcril_qmi_wms_ims_rat_info_to_ril_radio_tech(raw_send_resp->ims_rat_info);
    }

    auto respData = std::make_shared<RilSendSmsResult_t>(
             message_ref, qcril_qmi_sms_external_query_is_lte_only_registered(),
             network_error_rp_cause, cause_code_3gpp2, rat);
    auto resp = std::make_shared<QcRilRequestMessageCallbackPayload> (ril_err, respData);
...
void qcril_qmi_sms_transfer_sim_ucs2_alpha_to_std_ucs2_alpha(const wms_call_control_modified_info_type_v01 *sim_alpha, wms_call_control_modified_info_type_v01 *std_alpha)
{
  ...
  do
  {
    ...
    {
      case 0x80:
        while ( (uint32_t)(idx+1) < sim_alpha->alpha_id_len )
        {
        ...
        if ( (uint32_t)(idx+1) == sim_alpha->alpha_id_len && 0 != sim_alpha->alpha_id[idx] )
        {
      ...
      case 0x81:
        ...
        else
        {
          ...
          for ( i = 0; i< num_of_char; i++, idx++ )
          {
            ...
            std_alpha->alpha_id[2*i]   = (uint8_t) (ucs2_val);
            std_alpha->alpha_id[2*i+1] = (uint8_t) (ucs2_val >> 8);
          }
...
)
{
  ...
  {
      if (TimeKeeper::no_timer != qcril_sms.mt_pending_ack_expry_tmr)
      {
         QCRIL_LOG_INFO( "resetting MT SMS ack tmr per ack reception" );
         TimeKeeper::getInstance ().clear_timer(qcril_sms.mt_pending_ack_expry_tmr);
         qcril_sms.mt_pending_ack_expry_tmr = TimeKeeper::no_timer;
      }
  }
...
)
{
  ...
  do
  {
    ...
    if (!qcril_sms_fill_wms_payload( FALSE,
                                     ...
                                     (uint32_t *)&write_request_msg.raw_message_write_data.raw_message_len,
...
)
{
  ...
  do
  {
    ...
    if (msg->getIndex() > 0)
    {
        ...
        request_msg.index = msg->getIndex() - 1;
    }
    else
    {
        QCRIL_LOG_ERROR("index (%d) is not 1-based index", msg->getIndex());
        ril_err = RIL_E_INVALID_ARGUMENTS;
        break;
    }
...
boolean qcril_sms_force_on_dc()
{
  ...
  if (res == Message::Callback::Status::SUCCESS && shared_result != nullptr)
  {
    if (shared_result->success) {
        QCRIL_LOG_INFO("qcril_sms_force_on_dc: mcc %s, mnc %s",
                shared_result->mcc.c_str(), shared_result->mnc.c_str());
        return qcril_db_query_force_on_dc(shared_result->mcc.c_str(),
...
)
{
  ...
  do
  {
    ...
    if (!wms_ready_atleast_limited_3gpp2() &&
        !(is_voice_over_lte && (NasSmsStatus::STATUS_1X == lte_sms_status))
       ) {
       ril_err = RIL_E_INVALID_STATE;
       break;
    }
...
)
{
  ...
  do
  {
    ...
    if (!qcril_sms_fill_wms_payload( TRUE,
                                     ...
                                     (uint32_t *)&request_msg.raw_message_write_data.raw_message_len,
   ...
   do
   {
      ...
      else
      {
         ...
         if ( !qcril_mo_sms_error_check( gw_pdu ) &&
              !qcril_mo_smsc_error_check(smsc_address))
         {
...
)
{
  ...
  instance_id = (qcril_instance_id_e_type) QCRIL_EXTRACT_INSTANCE_ID_FROM_USER_DATA( (uint32_t)(uintptr_t) resp_cb_data );
  modem_id = (qcril_modem_id_e_type) QCRIL_EXTRACT_MODEM_ID_FROM_USER_DATA( (uint32_t)(uintptr_t) resp_cb_data );
  req_id = QCRIL_EXTRACT_USER_ID_FROM_USER_DATA( (uint32_t)(uintptr_t) resp_cb_data );
  /*-----------------------------------------------------------------------*/

  QCRIL_LOG_INFO("msg_id (0x%x) %s", msg_id, qcril_sms_lookup_cmd_name(msg_id));
  auto module = getSmsModule();
  assert(module != nullptr);
  auto msg = module->getPendingMessageList().extract(req_id);
  if (msg == nullptr)
  {
      ...
      if ( qmi_ril_sms_mt_expiry_window_len_set )
      {
        qcril_sms.mt_pending_ack_expry_tmr =
                TimeKeeper::getInstance().set_timer(
                        ...
                        nullptr,
                        qmi_ril_sms_mt_expiry_window_len * 1000);
      }
      ...
      else
      {
        if (event_report_ind->sms_on_ims_valid && event_report_ind->sms_on_ims)
        {
          ...
          if (mt_ims_msg)
          {
              ...
              Dispatcher::getInstance().dispatchSync(mt_ims_msg);
          }
        }
        else
        {
          auto mt_sms_msg = std::make_shared<RilUnsolIncoming3Gpp2SMSMessage>(cdma_msg);
          Dispatcher::getInstance().dispatchSync(mt_sms_msg);
        }
      }
    }
    qcril_free( buf_ptr );
...
(
  std::shared_ptr<QcRilEventSmsRawReadMessage> msg
)
{
  wms_raw_read_req_msg_v01 wms_raw_read_req_msg;
  ...
  auto message_mode = msg->getMessageMode();
  if ((message_mode != QcRilEventSmsRawReadMessage::MessageMode::GW) &&
      (message_mode != QcRilEventSmsRawReadMessage::MessageMode::CDMA))
  {
  ...
  memset(&wms_raw_read_req_msg, 0, sizeof(wms_raw_read_req_msg));
  memset(&wms_raw_read_resp_msg, 0 ,sizeof(wms_raw_read_resp_msg));
  if (message_mode == QcRilEventSmsRawReadMessage::MessageMode::CDMA)
  {
    wms_raw_read_req_msg.message_mode_valid = TRUE;
    wms_raw_read_req_msg.message_mode = WMS_MESSAGE_MODE_CDMA_V01;
  }
  else if (message_mode == QcRilEventSmsRawReadMessage::MessageMode::GW)
  {
    wms_raw_read_req_msg.message_mode_valid = TRUE;
    wms_raw_read_req_msg.message_mode = WMS_MESSAGE_MODE_GW_V01;
  }
  auto storage_type = msg->getStorageType();
  if (storage_type == QcRilEventSmsRawReadMessage::StorageType::UIM)
  {
    wms_raw_read_req_msg.message_memory_storage_identification.storage_type
        = WMS_STORAGE_TYPE_UIM_V01;
  }
  else if (storage_type == QcRilEventSmsRawReadMessage::StorageType::NV)
  {
    wms_raw_read_req_msg.message_memory_storage_identification.storage_type
  ...
  {
    wms_raw_read_req_msg.message_memory_storage_identification.storage_type
        = WMS_STORAGE_TYPE_NONE_V01;
  }
  wms_raw_read_req_msg.message_memory_storage_identification.storage_index
        = msg->getStorageIndex();

  const auto& sms_on_ims = msg->getSmsOnIms();
  if (sms_on_ims)
  {
    wms_raw_read_req_msg.sms_on_ims_valid = TRUE;
    wms_raw_read_req_msg.sms_on_ims = *sms_on_ims;
  }

  QCRIL_LOG_INFO("MSG PARAMS %d %d %d",
                 wms_raw_read_req_msg.message_memory_storage_identification.storage_index,
                 wms_raw_read_req_msg.message_memory_storage_identification.storage_type,
                 wms_raw_read_req_msg.message_mode);

  qmi_client_error = qmi_client_sms_send_sync(QMI_WMS_RAW_READ_REQ_V01,
                                              &wms_raw_read_req_msg,
                                              sizeof(wms_raw_read_req_msg),
...
static std::shared_ptr<QcRilEventSmsRawReadMessage>
qcril_sms_create_raw_read_message(const wms_event_report_ind_msg_v01* event_report_ind)
{
    uint32_t index = event_report_ind->mt_message.storage_index;

    auto mode = QcRilEventSmsRawReadMessage::MessageMode::INVALID;
    if (event_report_ind->message_mode == WMS_MESSAGE_MODE_CDMA_V01)
        mode = QcRilEventSmsRawReadMessage::MessageMode::CDMA;
    else if (event_report_ind->message_mode == WMS_MESSAGE_MODE_GW_V01)
        mode = QcRilEventSmsRawReadMessage::MessageMode::GW;

    auto type = QcRilEventSmsRawReadMessage::StorageType::NONE;
    if (event_report_ind->mt_message.storage_type == WMS_STORAGE_TYPE_UIM_V01)
        type = QcRilEventSmsRawReadMessage::StorageType::UIM;
    else if (event_report_ind->mt_message.storage_type ==WMS_STORAGE_TYPE_NV_V01)
        type = QcRilEventSmsRawReadMessage::StorageType::NV;

    auto msg = std::make_shared<QcRilEventSmsRawReadMessage>(mode, type, index);
    if (msg && event_report_ind->sms_on_ims_valid)
    {
        msg->setSmsOnIms(event_report_ind->sms_on_ims_valid);
    }
    return msg;
}
           ...
           (event_report_ind->message_mode_valid))
  {
    if (event_report_ind->mt_message.storage_type == WMS_STORAGE_TYPE_UIM_V01)
    {
       ...
       else if (event_report_ind->message_mode == WMS_MESSAGE_MODE_CDMA_V01)
       {
          ...
          auto msg = qcril_sms_create_raw_read_message(event_report_ind);
...
void qcril_sms_mt_sms_ind_hdlr
(
  wms_event_report_ind_msg_v01* decoded_payload,
  qcril_instance_id_e_type instance_id
)
{
  bool power_opt_enabled = qcril_qmi_sms_external_query_is_power_opt_enabled();
  if (power_opt_enabled)
  {
      uint8_t atel_ui_status = qcril_qmi_sms_external_get_ui_status();
      if(atel_ui_status)
      {
...
)
{
  ...
  auto nofify_msg = std::make_shared<QcRilSmsNotifyRegisteredOnImsMessage>(
        WMS_TRANSPORT_NW_REG_STATUS_FULL_SERVICE_V01 ==
        transport_nw_reg_info_ind->transport_nw_reg_status);
  Dispatcher::getInstance().dispatch(nofify_msg);
...
)
{
  ...
  auto msg = std::make_shared<QcRilUnsolWmsReadyMessage>(
    static_cast<uint8_t>(service_msg->ready_status));
  Dispatcher::getInstance().dispatchSync(msg);
...
*/
/*=========================================================================*/
uint8_t qcril_sms_hex_char_to_byte
(
  char hex_char
)
{
  ...
  {
    byte_value = (uint8_t)(hex_char - 'a' + 10);
  }
...
(
  ...
  uint8_t * byte_pdu,        // OUTPUT
  uint32_t num_hex_chars
)
{
  uint16_t buf_pos = 0;
  uint32_t i;
  ...
  {
    b_packb(qcril_sms_hex_char_to_byte(hex_pdu[i]), byte_pdu, buf_pos, (uint16_t)4);
    buf_pos += 4;
  }
...
*/
/*=========================================================================*/
char qcril_sms_byte_to_hex_char (uint8_t val)
{
  char hex_char;
...
)
{
  ...
  {
  /* Digit Mode */
  b_packb( (uint8_t) address_ptr->digit_mode,
           data,
           (uint16_t) (bit_pos),
           1 );
  bit_pos++;
  ...
  /* Number mode */
  b_packb( (uint8_t) address_ptr->number_mode,
           data,
           (uint16_t) (bit_pos),
           1 );
  bit_pos++;
  ...
  if ( address_ptr->digit_mode == RIL_CDMA_SMS_DIGIT_MODE_8_BIT )
  {
    ...
    b_packb( (uint8_t) address_ptr->number_type,
             data,
             (uint16_t) (bit_pos),
             3 );
    bit_pos += 3;
    ...
    {
      b_packb( (uint8_t) address_ptr->number_plan,
               data,
               (uint16_t) (bit_pos),
               4 );
      bit_pos += 4;
  ...
  if ( bit_pos % 8 != 0 )
  {
    ...
    */
    digit_size = (uint8_t)(8 - bit_pos % 8);
    b_packb( 0, data, (uint16_t) (bit_pos),  digit_size );
    bit_pos += digit_size;
  }

  *parm_len_ptr = (uint8_t) (bit_pos/8);
  }
  else
  {
    QCRIL_LOG_INFO("Invalid Arguments");
...
)
{
  ...
  {
  /* subaddress type */
  b_packb( (uint8_t) address_ptr->subaddressType,
           data,
           (uint16_t) (bit_pos),
           3 );
  bit_pos += 3;
...
(
  ...
  uint32_t                           raw_bd_len,   /* IN */
  uint8_t                          * raw_bd_ptr,   /* IN */
...
)
{
  ...
  */
  if ( tl_msg_ptr->mask & QCRIL_SMS_MASK_TL_TELESERVICE_ID )
  {
    ...
    {
      OTA_msg_ptr->data_len = (uint16_t) MIN (raw_bd_len, RIL_CDMA_SMS_BEARER_DATA_MAX);
      memcpy( (uint8_t *)OTA_msg_ptr->data, raw_bd_ptr, OTA_msg_ptr->data_len );
          status = TRUE;
          break;
    }
  }
  ...
  /* msg type
  */
  data[0] = (uint8_t) tl_msg_ptr->tl_message_type;
  pos ++;
  ...
  */
  ...
  if ( tl_msg_ptr->mask & QCRIL_SMS_MASK_TL_TELESERVICE_ID )
  {
    data[pos] = (uint8_t) QCRIL_SMS_TL_TELESERVICE_ID;
    pos++; /* skip parm id */

    data[pos] = parm_len = 2;
    pos++; /* skip len */

    b_packw( (uint16_t) tl_msg_ptr->teleservice,
             data,
             (uint16_t) (pos * 8),
             parm_len * 8 );
  ...
  {
    data[pos] = (uint8_t) QCRIL_SMS_TL_BC_SRV_CATEGORY;
    pos++; /* skip parm id */

    data[pos] = parm_len = 2;
    pos++; /* skip len */

    b_packw( (uint16_t) (tl_msg_ptr->service),
             data,
             (uint16_t) (pos * 8),
             parm_len * 8 );
  ...
  {
    data[pos] = (uint8_t) (tl_msg_ptr->is_mo ? QCRIL_SMS_TL_DEST_ADDRESS : QCRIL_SMS_TL_ORIG_ADDRESS);
    pos++; /* skip parm id */
  ...
  {
    data[pos] = (uint8_t) (tl_msg_ptr->is_mo ? QCRIL_SMS_TL_DEST_SUBADDRESS : QCRIL_SMS_TL_ORIG_SUBADDRESS);
    pos++; /* skip parm id */
  ...
  {
    data[pos] = (uint8_t) QCRIL_SMS_TL_BEARER_REPLY_OPTION;
    pos++; /* skip parm id */
  ...
  if ( tl_msg_ptr->mask & QCRIL_SMS_MASK_TL_BEARER_DATA )
  {
    ...
    {
      data[pos] = (uint8_t) QCRIL_SMS_TL_BEARER_DATA;
      pos++; /* skip parm id */

      data[pos] = (uint8_t) MIN (raw_bd_len, RIL_CDMA_SMS_BEARER_DATA_MAX);
      pos++; /* skip parm len */

      memcpy( data+pos, raw_bd_ptr, MIN((uint8_t)raw_bd_len, (uint8_t)RIL_CDMA_SMS_BEARER_DATA_MAX-pos) );
      pos += (uint16_t) raw_bd_len; /* skip parm data */
    }
  }
...
)
{
  ...
  {
  (void) memset((uint8_t *)ota_data.data, 0, sizeof(ota_data.data));
  ...
  if (status)
  {
    ...
    {
      (void) memcpy((uint8_t *)data_buf, (uint8_t *)ota_data.data, MIN(data_buf_len, ota_data.data_len));
    }
  }
  }
...
)
{
  ...
  address_ptr->digit_mode = (RIL_CDMA_SMS_DigitMode)
                            b_unpackb( (uint8_t*) data, (uint16_t) (bit_pos), 1 );
  bit_pos ++;
  ...
  if ( address_ptr->digit_mode == RIL_CDMA_SMS_DIGIT_MODE_8_BIT )
  {
    address_ptr->number_type = (RIL_CDMA_SMS_NumberType)
                               b_unpackb( (uint8_t*) data, (uint16_t) (bit_pos), 3 );
    bit_pos += 3;
       ...
       address_ptr->number_mode == RIL_CDMA_SMS_NUMBER_MODE_NOT_DATA_NETWORK )
  {
    address_ptr->number_plan = (RIL_CDMA_SMS_NumberPlan)
                               b_unpackb( (uint8_t*) data, (uint16_t) (bit_pos), 4);
    bit_pos += 4;
  }
  else
  {
  ...
  b_unpackb( (uint8_t*) data, (uint16_t) (bit_pos), 8 );
  bit_pos += 8;
  ...
  */
  ...
  else
  {
    for ( i=0; i < address_ptr->number_of_digits; i++ )
    {
      address_ptr->digits[i] = b_unpackb( (uint8_t*) data,
                                          (uint16_t) (bit_pos),
                                          digit_size );
      bit_pos += digit_size;
    }
  ...
  /* Parm len checking */
  if ( bit_pos > (uint32_t)(parm_len*8) )
  {
...
)
{
  ...
  /* Odd flag */
  address_ptr->odd = b_unpackb( (uint8_t*) data, (uint16_t) (bit_pos), 1 );
  bit_pos += 1;

  /* Address size */
  address_ptr->number_of_digits = b_unpackb( (uint8_t*) data, (uint16_t) (bit_pos), 8 );
  bit_pos += 8;
...
(
  ...
  uint32_t                       * raw_bd_len,    /* OUT */
  uint8_t                        * raw_bd_ptr     /* OUT */
)
{
  uint16_t                  pos;      /* running position in bytes */
  qcril_sms_tl_parm_id_e_type parm_id = QCRIL_SMS_TL_DUMMY;
  uint8_t                   parm_len;
  uint16_t                  tl_len;  /* len of TL data */
  ...
  else
  {
    ...
    if (OTA_msg_ptr->data[pos] > (uint8_t) QCRIL_SMS_TL_TYPE_MAX )
    {
    ...
    while ( status == TRUE  )
    {
      ...
      switch ( parm_id )
      {
        case QCRIL_SMS_TL_TELESERVICE_ID:
          ...
          else
          {
            tl_ptr->teleservice = (qcril_sms_teleservice_e_type)
                                  b_unpackw( (uint8_t*) OTA_msg_ptr->data,
                                             (uint16_t) (pos*8),
                                             parm_len*8 );
            pos += parm_len; /* skip parm data */
          }
        ...
        case QCRIL_SMS_TL_BC_SRV_CATEGORY:
          ...
          {
            tl_ptr->service = b_unpackw( (uint8_t*) OTA_msg_ptr->data,
                                         (uint16_t) (pos*8),
                                         parm_len*8 );
            pos += parm_len; /* skip parm data */
          }
        ...
        case QCRIL_SMS_TL_DEST_ADDRESS:
          ...
          status = qcril_sms_decode_address( (uint8_t*) OTA_msg_ptr->data+pos,
                                             parm_len,
                                             & tl_ptr->address );
          pos += parm_len;
          break;
        ...
        case QCRIL_SMS_TL_DEST_SUBADDRESS:
          ...
          status = qcril_sms_decode_subaddress( (uint8_t*) OTA_msg_ptr->data+pos,
                                                parm_len,
                                                & tl_ptr->subaddress );
        ...
        case QCRIL_SMS_TL_BEARER_DATA:
          ...
          else
          {
            ...
            *raw_bd_len = MIN (parm_len, RIL_CDMA_SMS_BEARER_DATA_MAX);
            ...
            {
                memcpy( raw_bd_ptr, (uint8_t*)OTA_msg_ptr->data + pos, *raw_bd_len );
            }
          }
...
)
{
  ...
  (void) memcpy((uint8_t *)ota_data.data, (uint8_t *)data, ota_data.data_len);

  status = qcril_sms_convert_qmi_to_tl(&ota_data,
                                       &tl_msg,
                                       (uint32_t *) &cdma_sms_msg->uBearerDataLen,
                                       cdma_sms_msg->aBearerData);
  if (status == TRUE)
  {
...
int qmi_ril_block_mo_sms_on_1x()
{
  ...
  {
    if( !(QMI_DMS_MASK_SVLTE_CAPABLE_V01 & qcril_qmi_sms_external_query_simul_voice_data_capability()) &&
        (FALSE == i_ptr->transport_layer_info.registered_ind_valid ||
        FALSE == i_ptr->transport_layer_info.registered_ind ) &&
        (RADIO_TECH_LTE == qcril_qmi_sms_external_query_reported_data_technology() ||
         RADIO_TECH_LTE_CA == qcril_qmi_sms_external_query_reported_data_technology() )
      )
    {
      ret = TRUE;
    }
  ...
  if( FALSE == ret )
  {
    ret = qcril_qmi_sms_external_query_is_1x_sms_blocked() ? TRUE : FALSE;
  }
...
void set_wms_service_state(wms_service_ready_status_enum_v01 state) {
   ...
   SMS_CACHE_UNLOCK;
    ...
    {
        qcril_qmi_sms_set_primary_client(nullptr);
    }
   ...
   {
       qcril_sms_mt_transaction_expired(nullptr);
   }
...
int wms_ready_supported(void) {
   ...
   if ( qmi_err != QMI_NO_ERR)
   {
      QCRIL_LOG_ERROR("Failed wms get indication register request w/%d",
                                                          qmi_err);
      result = 0;
   }
   else {
...
void query_wms_ready_status(void)
{
   ...
   if ( qmi_err != QMI_NO_ERR )
   {
      QCRIL_LOG_ERROR("Failed wms service ready status request w/%d",
                                                      qmi_err);
   }
   else
   {
...
)
{
   ...
   if (wms_resp->resp.result != QMI_RESULT_SUCCESS_V01) {
      QCRIL_LOG_ERROR("wms_get_service_ready_status returned w/%d",
                                          wms_resp->resp.error);
      QCRIL_LOG_FUNC_RETURN();
      return;
   }
...
int qcril_qmi_sms_block_retry_3pgg_sms_on_lte_only(RIL_RadioTechnologyFamily tech, unsigned char retry)
{
    ...
    if (RADIO_TECH_3GPP == tech &&
        TRUE == retry &&
        qcril_qmi_sms_external_query_is_lte_only_registered())
    {
        ret = TRUE;
    }
    QCRIL_LOG_FUNC_RETURN_WITH_RET(ret);
...
void qcril_qmi_sms_reset_mt_sms_ack_needed_power_opt_buffer()
{
    ...
    if ( TimeKeeper::no_timer != qcril_mt_sms_buffer_power_opt.mt_sms_ack_needed_expry_tmr )
    {
...
void qcril_sms_update_mt_sms_power_opt_buffer(wms_event_report_ind_msg_v01 *event_report_ind)
{
    ...
    TimeKeeper::millisec ack_expry_tmr_window = 0;
    ...
    do
    {
        ...
        if (event_report_ind->transfer_route_mt_message_valid)
        {
                ...
                (event_report_ind->transfer_route_mt_message.format == WMS_MESSAGE_FORMAT_MWI_V01))
            {
                if(WMS_ACK_INDICATOR_SEND_ACK_V01 == event_report_ind->transfer_route_mt_message.ack_indicator)
                {
                    ...
                    ack_expry_tmr_window = 2000; // 2sec for CDMA
                }
                else
                {
            ...
            else if (event_report_ind->transfer_route_mt_message.format == WMS_MESSAGE_FORMAT_GW_PP_V01)
            {
                ...
                {
                    update_mt_sms_ack_needed_buffer = TRUE;
                    ack_expry_tmr_window = 20000; // 20sec for GSM
                }
                else
                {
                    update_mt_sms_ack_not_needed_buffer = TRUE;
        ...
        else if(update_mt_sms_ack_needed_buffer)
        {
            ...
            qcril_mt_sms_buffer_power_opt.mt_sms_ack_needed_expry_tmr =
                    TimeKeeper::getInstance ().set_timer(
                        [](void* /*userdata*/) {
                            auto msg = std::make_shared<QcRilEventSmsResetAckNeededForPowerOptMessage>();
                            Dispatcher::getInstance().dispatch(msg);
                        },
                        nullptr,
...
void qcril_qmi_sms_set_operational_status(
...
qcril_qmi_sms_get_operational_status()
{
  SMS_CACHE_LOCK;
  auto res = operational_status;
  SMS_CACHE_UNLOCK;
