/******************************************************************************
#  Copyright (c) 2009-2010, 2017, 2020 Qualcomm Technologies, Inc.
#  All Rights Reserved.
...
#include "modules/pbm/qcril_pbm.h"
...
#include "modules/pbm/qcril_pbm_legacy.h"
...
struct qcril_pbm_nw_detected_ecc_info {
    std::string number;
...
struct pbm_cache_info_type {
    ...
    std::unique_ptr<qcril_pbm_nw_detected_ecc_info> nw_detected_ecc_info;
...
typedef struct {
  uint8_t network_emer_nums_valid;
  uint32_t network_emer_nums_len;
  pbm_network_emer_num_type_v01 network_emer_nums[QMI_PBM_MAX_NUM_NETWORK_EMER_NUM_V01];

  uint8_t nv_emer_nums_extended_valid;
  uint32_t nv_emer_nums_extended_len;
  pbm_emer_num_type_v01 nv_emer_nums_extended[QMI_PBM_MAX_NUM_NV_EMER_NUM_EXTENDED_V01];

  uint8_t nv_emer_nums_extended_cat_valid;
  uint32_t nv_emer_nums_extended_cat_len;
  uint8_t nv_emer_nums_extended_cat[QMI_PBM_MAX_NUM_NV_EMER_NUM_EXTENDED_V01];

  uint8_t emer_nums_extended_valid;
  uint32_t emer_nums_extended_len;
  pbm_emer_num_type_v01 emer_nums_extended[QMI_PBM_MAX_NUM_EMER_NUM_EXTENDED_V01];

  uint8_t emer_nums_extended_cat_valid;
  uint32_t emer_nums_extended_cat_len;
  uint8_t emer_nums_extended_cat[QMI_PBM_MAX_NUM_EMER_NUM_EXTENDED_V01];

  uint8_t card_emer_nums_valid;
  uint32_t card_emer_nums_len;
  pbm_card_emer_num_type_v01 card_emer_nums[QMI_PBM_MAX_NUM_CARD_EMER_NUM_V01];

  uint8_t mcc_emer_nums_valid;
  uint32_t mcc_emer_nums_len;
  pbm_mcc_emer_num_type_v01 mcc_emer_nums[QMI_PBM_MAX_NUM_SLOT_V01];

  uint8_t elenl_emer_nums_valid;
  uint32_t elenl_emer_nums_len;
  pbm_elenl_emer_num_type_v01 elenl_emer_nums[QMI_PBM_MAX_NUM_NETWORK_EMER_NUM_V01];

  uint8_t serving_mcc_emer_nums_valid;
  uint32_t serving_mcc_emer_nums_len;
  pbm_mcc_emer_num_type_v01 serving_mcc_emer_nums[QMI_PBM_MAX_NUM_SLOT_V01];
...
void qcril_qmi_pbm_record_update_ext_ind_hdlr(void *ind_data_ptr,
                                            uint32 ind_data_len,
                                            qcril_instance_id_e_type instance_id);
...
)
{
  ...
  pbm_cache_info.nw_detected_ecc_info = nullptr;
  memset(&pbm_cache_info, 0, sizeof(pbm_cache_info));
  PBM_CACHE_UNLOCK();
  QCRIL_LOG_FUNC_RETURN();
}
...
)
{
  ...
  auto msg = std::make_shared<QcRilUnsolPbmInitializedMessage>();
...
)
{
  ...
  if (  emerg_info != NULL )
  {
    ...
    if(emerg_info->elenl_emer_nums_valid) {
      for (k=0; k<emerg_info->elenl_emer_nums_len; k++) {
        for(i=0; i < emerg_info->elenl_emer_nums[k].emer_nums_len; i++) {
          auto ecc = std::make_shared<RIL_EmergencyNumber_t>();
          if (ecc != nullptr) {
              auto ecc_num = emerg_info->elenl_emer_nums[k].emer_nums[i].emer_num;
              auto ecc_num_len = emerg_info->elenl_emer_nums[k].emer_nums[i].emer_num_len;
              ecc->number = std::string(ecc_num, ecc_num_len);
              QCRIL_LOG_INFO("elenl emergency number - %s", ecc->number.c_str());
              auto sub_service = emerg_info->elenl_emer_nums[k].emer_nums[i].sub_service;
              auto sub_service_len = emerg_info->elenl_emer_nums[k].emer_nums[i].sub_service_len;
              ecc->urns.push_back(std::string((char*)sub_service, sub_service_len));
              ecc->sources = RIL_EccNumberSourceMask::NETWORK_SIGNALING;
              auto msg = std::make_shared<QcRilGetCurrentMccMncSyncMessage>(false, true);
              assert(msg != nullptr);
              std::shared_ptr<QcRilGetMccMncType> shared_result;
              auto res = msg->dispatchSync(shared_result);
              if (res == Message::Callback::Status::SUCCESS &&
                    shared_result != nullptr && shared_result->success) {
                  ecc->mcc = shared_result->mcc;
                  ecc->mnc = shared_result->mnc;
              }
    ...
    if(emerg_info->network_emer_nums_valid) {
      for (k=0; k<emerg_info->network_emer_nums_len; k++) {
        for(i=0; i < emerg_info->network_emer_nums[k].emer_nums_len; i++) {
          auto ecc = std::make_shared<RIL_EmergencyNumber_t>();
          if (ecc != nullptr) {
              auto ecc_num = emerg_info->network_emer_nums[k].emer_nums[i].emer_num;
              auto ecc_num_len = emerg_info->network_emer_nums[k].emer_nums[i].emer_num_len;
              ecc->number = std::string(ecc_num, ecc_num_len);
              QCRIL_LOG_INFO("network emergency number - %s", ecc->number.c_str());
              ecc->categories = convertToRilCategories(emerg_info->network_emer_nums[k].emer_nums[i].cat);
    ...
    if ( emerg_info->card_emer_nums_valid ) {
      for (k=0; k<emerg_info->card_emer_nums_len; k++) {
        for ( i = 0; i < emerg_info->card_emer_nums[k].emer_nums_len; i++ ) {
            auto ecc = std::make_shared<RIL_EmergencyNumber_t>();
            if (ecc != nullptr) {
                auto ecc_num = emerg_info->card_emer_nums[k].emer_nums[i].emer_num;
                auto ecc_num_len = emerg_info->card_emer_nums[k].emer_nums[i].emer_num_len;
                ecc->number = std::string(ecc_num, ecc_num_len);
                QCRIL_LOG_INFO("card emergency number - %s", ecc->number.c_str());
                ecc->categories = convertToRilCategories(emerg_info->card_emer_nums[k].emer_nums[i].cat);
    ...
    if ( emerg_info->emer_nums_extended_valid ) {
      for(i = 0; i < emerg_info->emer_nums_extended_len; i++) {
          auto ecc = std::make_shared<RIL_EmergencyNumber_t>();
          if (ecc != nullptr) {
              auto ecc_num = emerg_info->emer_nums_extended[i].emer_num;
              auto ecc_num_len = emerg_info->emer_nums_extended[i].emer_num_len;
              ecc->number = std::string(ecc_num, ecc_num_len);
              QCRIL_LOG_INFO("extended emergency number - %s", ecc->number.c_str());
              if (emerg_info->emer_nums_extended_cat_valid &&
                  i < emerg_info->emer_nums_extended_cat_len) {
                ecc->categories = convertToRilCategories(
                    emerg_info->emer_nums_extended_cat[i]);
    ...
    if ( emerg_info->nv_emer_nums_extended_valid ) {
      for(i = 0; i < emerg_info->nv_emer_nums_extended_len; i++) {
          auto ecc = std::make_shared<RIL_EmergencyNumber_t>();
          if (ecc != nullptr) {
              auto ecc_num = emerg_info->nv_emer_nums_extended[i].emer_num;
              auto ecc_num_len = emerg_info->nv_emer_nums_extended[i].emer_num_len;
              ecc->number = std::string(ecc_num, ecc_num_len);
              QCRIL_LOG_INFO("nv emergency number - %s", ecc->number.c_str());
              if (emerg_info->nv_emer_nums_extended_cat_valid &&
                  i < emerg_info->nv_emer_nums_extended_cat_len) {
                ecc->categories = (uint64_t)convertToRilCategories(
                    emerg_info->nv_emer_nums_extended_cat[i]);
    ...
    if ( emerg_info->mcc_emer_nums_valid ) {
      ...
      if (slot < QMI_PBM_MAX_NUM_SLOT_V01) {
          for ( i = 0; i < emerg_info->mcc_emer_nums[slot].emer_nums_len; i++ ) {
              auto ecc = std::make_shared<RIL_EmergencyNumber_t>();
              if (ecc != nullptr) {
                  auto ecc_num = emerg_info->mcc_emer_nums[slot].emer_nums[i].emer_num;
                  auto ecc_num_len = emerg_info->mcc_emer_nums[slot].emer_nums[i].emer_num_len;
                  ecc->number = std::string(ecc_num, ecc_num_len);
    ...
    /* Emergency numbers based on Serving mcc */
    if ( emerg_info->serving_mcc_emer_nums_valid ) {
      /* For mcc numbers, pick numbers based on slot as these are based on specific card.
         For all other emergency numbers, keep legacy behaviour intact */
      uint32_t slot = qmi_ril_get_sim_slot();
      if (slot < QMI_PBM_MAX_NUM_SLOT_V01) {
          for ( i = 0; i < emerg_info->serving_mcc_emer_nums[slot].emer_nums_len; i++ ) {
              auto ecc = std::make_shared<RIL_EmergencyNumber_t>();
              if (ecc != nullptr) {
                  auto ecc_num = emerg_info->serving_mcc_emer_nums[slot].emer_nums[i].emer_num;
                  auto ecc_num_len = emerg_info->serving_mcc_emer_nums[slot].emer_nums[i].emer_num_len;
                  ecc->number = std::string(ecc_num, ecc_num_len);
                  QCRIL_LOG_INFO("serving_mcc emergency number - %s", ecc->number.c_str());
                  ecc->categories = (uint64_t)RIL_EccCategoryMask::UNKNOWN;
                  QCRIL_LOG_INFO("serving_mcc emergency number categories - %d",
                          ecc->categories);
                  ecc->sources = RIL_EccNumberSourceMask::MODEM_CONFIG;
                  ecc_map.insert({ecc->number, ecc});
              }
    ...
    auto msg = std::make_shared<QcRilUnsolAssessEccNumberListMessage>(true);
    Dispatcher::getInstance().dispatch(msg);
  }
...
/*=========================================================================
  FUNCTION: qcril_qmi_pbm_copy_pbm_list_resp_data

===========================================================================*/
/*!
    @brief
    helper function for copying indication data to common pbm_list structure

    @return
    None.
*/
/*=========================================================================*/
void qcril_qmi_pbm_copy_pbm_list_resp_data
(
    const pbm_get_emergency_list_resp_msg_v01& emerg_resp,
    pbm_ecc_list_info& list_info
)
{
    QCRIL_LOG_DEBUG("Copy response data");

    list_info.network_emer_nums_valid = emerg_resp.network_emer_nums_valid;
    list_info.network_emer_nums_len = emerg_resp.network_emer_nums_len;
    memcpy(list_info.network_emer_nums,
        emerg_resp.network_emer_nums, sizeof(emerg_resp.network_emer_nums));

    list_info.nv_emer_nums_extended_valid = emerg_resp.nv_emer_nums_extended_valid;
    list_info.nv_emer_nums_extended_len = emerg_resp.nv_emer_nums_extended_len;
    memcpy(list_info.nv_emer_nums_extended,
        emerg_resp.nv_emer_nums_extended, sizeof(emerg_resp.nv_emer_nums_extended));

    list_info.nv_emer_nums_extended_cat_valid =
        emerg_resp.nv_emer_nums_extended_cat_valid;
    list_info.nv_emer_nums_extended_cat_len = emerg_resp.nv_emer_nums_extended_cat_len;
    memcpy(list_info.nv_emer_nums_extended_cat,
        emerg_resp.nv_emer_nums_extended_cat,
        sizeof(emerg_resp.nv_emer_nums_extended_cat));

    list_info.emer_nums_extended_valid = emerg_resp.emer_nums_extended_valid;
    list_info.emer_nums_extended_len = emerg_resp.emer_nums_extended_len;
    memcpy(list_info.emer_nums_extended,
        emerg_resp.emer_nums_extended, sizeof(emerg_resp.emer_nums_extended));

    list_info.emer_nums_extended_cat_valid = emerg_resp.emer_nums_extended_cat_valid;
    list_info.emer_nums_extended_cat_len = emerg_resp.emer_nums_extended_cat_len;
    memcpy(list_info.emer_nums_extended_cat,
        emerg_resp.emer_nums_extended_cat, sizeof(emerg_resp.emer_nums_extended_cat));

    list_info.mcc_emer_nums_valid = emerg_resp.mcc_emer_nums_valid;
    list_info.mcc_emer_nums_len = emerg_resp.mcc_emer_nums_len;
    memcpy(list_info.mcc_emer_nums,
        emerg_resp.mcc_emer_nums, sizeof(emerg_resp.mcc_emer_nums));

    list_info.elenl_emer_nums_valid = emerg_resp.elenl_emer_nums_valid;
    list_info.elenl_emer_nums_len = emerg_resp.elenl_emer_nums_len;
    memcpy(list_info.elenl_emer_nums,
        emerg_resp.elenl_emer_nums, sizeof(emerg_resp.elenl_emer_nums));

    list_info.card_emer_nums_valid = emerg_resp.card_emer_nums_valid;
    list_info.card_emer_nums_len = emerg_resp.card_emer_nums_len;
    memcpy(list_info.card_emer_nums,
        emerg_resp.card_emer_nums, sizeof(emerg_resp.card_emer_nums));

    list_info.serving_mcc_emer_nums_valid = emerg_resp.serving_mcc_emer_nums_valid;
    list_info.serving_mcc_emer_nums_len = emerg_resp.serving_mcc_emer_nums_len;
    memcpy(list_info.serving_mcc_emer_nums,
        emerg_resp.serving_mcc_emer_nums, sizeof(emerg_resp.serving_mcc_emer_nums));

    QCRIL_LOG_FUNC_RETURN();
}

/*=========================================================================
  FUNCTION: qcril_qmi_pbm_copy_pbm_list_ind_data
...
*/
/*=========================================================================*/
void qcril_qmi_pbm_copy_pbm_list_ind_data
(
    const pbm_emergency_list_ind_msg_v01& emerg_ind,
    pbm_ecc_list_info& list_info
)
{
    QCRIL_LOG_DEBUG("Copy Indication Data");
    list_info.network_emer_nums_valid = emerg_ind.network_emer_nums_valid;
    list_info.network_emer_nums_len = emerg_ind.network_emer_nums_len;
    memcpy(list_info.network_emer_nums,
        emerg_ind.network_emer_nums, sizeof(emerg_ind.network_emer_nums));

    list_info.nv_emer_nums_extended_valid = emerg_ind.nv_emer_nums_extended_valid;
    list_info.nv_emer_nums_extended_len = emerg_ind.nv_emer_nums_extended_len;
    memcpy(list_info.nv_emer_nums_extended,
        emerg_ind.nv_emer_nums_extended, sizeof(emerg_ind.nv_emer_nums_extended));

    list_info.emer_nums_extended_valid = emerg_ind.emer_nums_extended_valid;
    list_info.emer_nums_extended_len = emerg_ind.emer_nums_extended_len;
    memcpy(list_info.emer_nums_extended,
        emerg_ind.emer_nums_extended, sizeof(emerg_ind.emer_nums_extended));

    list_info.mcc_emer_nums_valid = emerg_ind.mcc_emer_nums_valid;
    list_info.mcc_emer_nums_len = emerg_ind.mcc_emer_nums_len;
    memcpy(list_info.mcc_emer_nums,
        emerg_ind.mcc_emer_nums, sizeof(emerg_ind.mcc_emer_nums));

    list_info.elenl_emer_nums_valid = emerg_ind.elenl_emer_nums_valid;
    list_info.elenl_emer_nums_len = emerg_ind.elenl_emer_nums_len;
    memcpy(list_info.elenl_emer_nums,
        emerg_ind.elenl_emer_nums, sizeof(emerg_ind.elenl_emer_nums));

    list_info.card_emer_nums_valid = emerg_ind.card_emer_nums_valid;
    list_info.card_emer_nums_len = emerg_ind.card_emer_nums_len;
    memcpy(list_info.card_emer_nums,
        emerg_ind.card_emer_nums, sizeof(emerg_ind.card_emer_nums));

    list_info.serving_mcc_emer_nums_valid = emerg_ind.serving_mcc_emer_nums_valid;
    list_info.serving_mcc_emer_nums_len = emerg_ind.serving_mcc_emer_nums_len;
    memcpy(list_info.serving_mcc_emer_nums,
        emerg_ind.serving_mcc_emer_nums, sizeof(emerg_ind.serving_mcc_emer_nums));

    QCRIL_LOG_FUNC_RETURN();
}

/*=========================================================================
  FUNCTION:  qcril_qmi_pbm_fetch_ecc_list_hndlr

===========================================================================*/
/*!
    @brief
    Handle QcRilRequestFetchEccListMessage

    @return
    None.
*/
/*=========================================================================*/
void qcril_qmi_pbm_fetch_ecc_list_hndlr
(
    std::shared_ptr<PbmFetchEccListMessage> msg
)
{
    RIL_Errno ril_req_res = RIL_E_GENERIC_FAILURE;
    qmi_client_error_type qmi_transport_error;

    pbm_ecc_list_info ecc_list_info {};
    pbm_get_emergency_list_resp_msg_v01 qmi_resp {};

    qmi_transport_error = qmi_client_pbm_send_sync(QMI_PBM_GET_EMERGENCY_LIST_REQ_V01,
                                        NULL,
                                        0,
                                        &qmi_resp,
                                        sizeof(qmi_resp));
    ...
    if ( RIL_E_SUCCESS == ril_req_res )
    {
        qcril_qmi_pbm_copy_pbm_list_resp_data(qmi_resp, ecc_list_info);
        qcril_qmi_pbm_emergency_list_ind_hdlr(&ecc_list_info);
    }
    else
    {
        QCRIL_LOG_INFO("failed, error %d", ril_req_res);
    }

    msg->sendResponse(msg, Message::Callback::Status::SUCCESS,
        std::shared_ptr<RIL_Errno>(new RIL_Errno{RIL_E_SUCCESS}));
}
...
)
{
    ...
    do {
       ...
       if( decoded_payload == nullptr || !decoded_payload_len )
       {
       ...
       else
       {
          switch ( msg_id )
          {
            case QMI_PBM_EMERGENCY_LIST_IND_V01:
              qcril_qmi_pbm_copy_pbm_list_ind_data(
                *(pbm_emergency_list_ind_msg_v01*)decoded_payload, ecc_list_info);
              qcril_qmi_pbm_emergency_list_ind_hdlr(&ecc_list_info);
              break;
            ...
            case QMI_PBM_RECORD_UPDATE_IND_V01:
              qcril_qmi_pbm_record_update_ext_ind_hdlr(decoded_payload,
                                                decoded_payload_len,
                                                QCRIL_DEFAULT_INSTANCE_ID);
              break;
...
RIL_Errno qmi_ril_send_ecc_list_indication()
{
    ...
    qcril_qmi_pbm_external_get_ril_ecc_map(ril_ecc_map);
    ...
    PBM_CACHE_LOCK();
    // Add alt_emer_num
    if (pbm_cache_info.nw_detected_ecc_info)
    {
        auto alt_ecc = std::make_shared<RIL_EmergencyNumber_t>();
        alt_ecc->number = pbm_cache_info.nw_detected_ecc_info->number;
...
)
{
    ...
    if ( ind_data_ptr != NULL )
    {
        ...
        if( slot_id == qmi_ril_get_sim_slot() + 1 ) {
            ...
            auto msg = std::make_shared<QcRilUnsolPhonebookRecordsUpdatedMessage>();
            if (pbm_cache_info.pbm_status == QCRIL_PBM_STATUS_UNKNOWN) {
                auto module = getPbmModule();
                if(module) {
                  auto ret = module->getPendingMessageList().insert(msg);
                  if (!ret.second)
                  {
                    QCRIL_LOG_ERROR("Failed to insert to PendingList for sending unsol phone book changed message");
                  }
                }
            } else {
                if (msg != nullptr)
...
void qcril_qmi_pbm_ril_connected_hndlr()
{
    QCRIL_LOG_FUNC_ENTRY();

    PBM_CACHE_LOCK();
    QCRIL_LOG_INFO("RIL Connected");
    pbm_cache_info.pbm_status = QCRIL_PBM_STATUS_REGISTER_READY;
    PBM_CACHE_UNLOCK();

    auto module = getPbmModule();
    if(module) {
        auto res = module->getPendingMessageList().find(
                QcRilUnsolPhonebookRecordsUpdatedMessage::get_class_message_id());
        if (res != nullptr)
        {
          module->getPendingMessageList().erase(res);
          auto msg = std::static_pointer_cast<QcRilUnsolPhonebookRecordsUpdatedMessage>(res);
          msg->broadcast();
        } else {
          QCRIL_LOG_INFO("No Pending Unsol Message");
        }
    }
...
/*=========================================================================
  FUNCTION:  qcril_qmi_pbm_get_all_pb_capacity_hndlr

===========================================================================*/
/*!
    @brief
    Handle QCRIL_REQUEST_GET_ADN_CAPACITY.

    @return
    None.
*/
/*=========================================================================*/
void qcril_qmi_pbm_get_all_pb_capacity_hndlr
(
    std::shared_ptr<QcRilRequestGetAdnCapacityMessage> msg
)
{
    ...
    ril_req_res = qcril_qmi_pbm_get_records_count();
    if(ril_req_res == RIL_E_SUCCESS
        && pbm_cache_info.pbm_status == QCRIL_PBM_STATUS_GET_CAPACITY_COMPLETE)
    {
      for (size_t i = 0; i < pbm_cache_info.pbm_capacity.size(); i++ )
      {
...
)
{
    ...
    if(qcril_qmi_pbm_get_records_count() == RIL_E_SUCCESS
        && pbm_cache_info.pbm_status == QCRIL_PBM_STATUS_GET_CAPACITY_COMPLETE)
    {
...
/*=========================================================================
  FUNCTION:  qcril_qmi_pbm_get_all_adn_records_hndlr

===========================================================================*/
/*!
    @brief
    Handle QCRIL_REQUEST_GET_ADN_RECORDS.

    @return
    None.
*/
/*=========================================================================*/
void qcril_qmi_pbm_get_all_adn_records_hndlr
(
    std::shared_ptr<QcRilRequestGetPhonebookRecordsMessage> msg
)
{
    RIL_Errno                             ril_req_res = RIL_E_SUCCESS;
    ...
    if (pbm_cache_info.pbm_status == QCRIL_PBM_STATUS_GET_CAPACITY_COMPLETE)
    {
      for (size_t i = 0; i < pbm_cache_info.pbm_capacity.size(); i++ )
      {
        int adn_count = pbm_cache_info.pbm_capacity[i].max_adn_num;
    ...
    auto resp = std::make_shared<QcRilRequestMessageCallbackPayload> (ril_req_res, nullptr);
    msg->sendResponse(msg, Message::Callback::Status::SUCCESS, resp);
...
)
{
    ...
    if ( ind_data_ptr != NULL )
    {
        ...
        int seq_num = (int) pbm_read_ind->basic_record_data.seq_num;
        ...
        /* Send unsolicited response to framework */
        auto msg = std::make_shared<QcRilUnsolAdnRecordsOnSimMessage>(*adn_records_ptr, seq_num);
        if (msg != nullptr)
        {
          msg->broadcast();
        }
...
/*=========================================================================
  FUNCTION:  qcril_qmi_pbm_record_update_ext_ind_hdlr

===========================================================================*/
/*!
    @brief
    Handle QMI_PBM_RECORD_UPDATE_IND_V01.

    @return
    None.
*/
/*=========================================================================*/
void qcril_qmi_pbm_record_update_ext_ind_hdlr
(
  void *ind_data_ptr,
  uint32 /*ind_data_len*/,
  qcril_instance_id_e_type   /*instance_id*/
...
{
    pbm_record_update_ind_msg_v01 *pbm_update_ind;

    QCRIL_LOG_FUNC_ENTRY();

    if (ind_data_ptr != NULL)
    {
        pbm_update_ind = (pbm_record_update_ind_msg_v01*)ind_data_ptr;
        QCRIL_LOG_INFO("Record_id: %d with session_type:  %d changed since %d",
                pbm_update_ind->record_update_info.record_id,
                pbm_update_ind->record_update_info.session_type,
                pbm_update_ind->record_update_info.operation);
...
/*===========================================================================
  FUNCTION:  qcril_qmi_pbm_uim_card_status_changed_hndlr

===========================================================================*/
void qcril_qmi_pbm_uim_card_status_changed_hndlr
(
    std::shared_ptr<UimCardStatusIndMsg> msg
)
{
  QCRIL_LOG_FUNC_ENTRY();
  if (msg == nullptr)
  ...
  qcril_uim_card_info_type status = msg->get_card_status();

  QCRIL_LOG_DEBUG( "Card_status on slot : %d is : %d\n", status.slot, status.status);
  if (status.slot == qmi_ril_get_process_instance_id() &&
          (status.status == QCRIL_CARD_STATUS_ABSENT ||
          status.status == QCRIL_CARD_STATUS_DOWN))
  {
    auto unsolMsg = std::make_shared<QcRilUnsolPhonebookRecordsUpdatedMessage>();
    if (unsolMsg != nullptr)
    {
      unsolMsg->broadcast();
    }
...
)
{
    ...
    if(RIL_E_SUCCESS != ril_req_res)
    {
      ...
      auto module = getPbmModule();
      assert(module != nullptr);
      module->getPendingMessageList().erase(ret.first);
...
/*=========================================================================
  FUNCTION:  qcril_qmi_pbm_update_phone_book_record_hndlr

===========================================================================*/
/*!
    @brief
    Handle QCRIL_REQUEST_UPDATE_PHONE_BOOK_RECORDS.

    @return
    None.
*/
/*=========================================================================*/
void qcril_qmi_pbm_update_phone_book_record_hndlr
(
    std::shared_ptr<QcRilRequestUpdatePhonebookRecordsMessage> msg
)
{
    RIL_Errno                             ril_req_res = RIL_E_GENERIC_FAILURE;
    const            qcril::interfaces::AdnRecordInfo* in_ptr = &msg->getRecordData();
...
)
{
  ...
  auto module = getPbmModule();
  assert(module != nullptr);
  module->getPendingMessageList().print();
  auto msg = module->getPendingMessageList().extract(token_id);
  module->getPendingMessageList().print();
  if (msg == nullptr)
  {
...
void qmi_ril_set_network_detected_ecc_number_info(const std::string number,
    uint32_t emergecyCategory,  const std::vector<uint8_t> urn)
{
  QCRIL_LOG_FUNC_ENTRY();
  if (!number.empty())
  {
    PBM_CACHE_LOCK();
    pbm_cache_info.nw_detected_ecc_info = std::make_unique<qcril_pbm_nw_detected_ecc_info>();
    pbm_cache_info.nw_detected_ecc_info->number = number;
    pbm_cache_info.nw_detected_ecc_info->emerg_srv_categ = emergecyCategory;
    pbm_cache_info.nw_detected_ecc_info->urn = urn;
    PBM_CACHE_UNLOCK();
...
#ifndef QMI_RIL_UTF
    // Reevaluate the ecc list to include the customized
    // emergency numbers if any.
    auto msg = std::make_shared<QcRilUnsolAssessEccNumberListMessage>();
    Dispatcher::getInstance().dispatch(msg);
#endif
  }
  QCRIL_LOG_FUNC_RETURN();
...
void qmi_ril_reset_network_detected_ecc_number_info()
{
  QCRIL_LOG_FUNC_ENTRY();
  if (pbm_cache_info.nw_detected_ecc_info)
  {
...
void qcril_qmi_pbm_get_network_detected_ecc_number(
    std::shared_ptr<GetNetworkDetectedEccNumberInfoMessage> msg) {
  std::shared_ptr<PbmNetworkDetectedEccNumber> resp{ nullptr };
  Message::Callback::Status status = Message::Callback::Status::FAILURE;
  if (pbm_cache_info.nw_detected_ecc_info) {
    resp = std::make_shared<PbmNetworkDetectedEccNumber>();
    if (resp) {
      resp->number = pbm_cache_info.nw_detected_ecc_info->number;
      resp->emerg_srv_categ = pbm_cache_info.nw_detected_ecc_info->emerg_srv_categ;
      resp->urn = pbm_cache_info.nw_detected_ecc_info->urn;
