...
VoiceModule::VoiceModule(string name) : AddPendingMessageList(name) {
  ...
  */
  ...
  mMessageHandler = {
      ...
      HANDLER(ClientConnectedMessage, VoiceModule::handleRilClientConnectedMessage),
...
void VoiceModule::handleRilClientConnectedMessage(std::shared_ptr<ClientConnectedMessage> msg) {
  Log::getInstance().d("[" + mName + "]: Handling msg = " + msg->dump());
  ...
  if (msg != nullptr) {
    do {
      ...
      dial_call_req.service_type =
          qcril_qmi_voice_external_get_call_service_type(true);
