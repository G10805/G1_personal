/******************************************************************************
#  Copyright (c) 2008-2009, 2012-2013, 2017, 2019-2020, 2023 Qualcomm Technologies, Inc.
#  All Rights Reserved.
...
typedef struct
{
  ...
  qcril::interfaces::AudioCodec codec[CM_CALL_ID_MAX];
  ...
  uint8_t diversion_info_valid [ CM_CALL_ID_MAX ];
  char *diversion_info [ CM_CALL_ID_MAX ];
  ...
  uint8_t silent_ui_valid[CM_CALL_ID_MAX];
  uint8_t silent_ui[CM_CALL_ID_MAX];
  uint8_t original_call_type_valid[CM_CALL_ID_MAX];
  call_type_enum_v02  original_call_type[ CM_CALL_ID_MAX ];
  uint8_t crs_type_valid[CM_CALL_ID_MAX];
  voice_call_crs_type_enum_v02  crs_type[ CM_CALL_ID_MAX ];
  uint8_t prog_info_notif_valid[ CM_CALL_ID_MAX ];
  qcril::interfaces::CallProgressInfo prog_info_notif[ CM_CALL_ID_MAX ];
  uint8_t msim_additional_call_info_valid[CM_CALL_ID_MAX];
  qcril::interfaces::MsimAdditionalCallInfo msim_additional_call_info[CM_CALL_ID_MAX];
  uint8_t computed_audio_quality_valid[CM_CALL_ID_MAX];
  qcril::interfaces::ComputedAudioQuality computed_audio_quality[CM_CALL_ID_MAX];
  uint8_t vos_support_valid[CM_CALL_ID_MAX];
  bool vos_support[CM_CALL_ID_MAX];
...
typedef enum qcril_qmi_voice_ccfc_reason_e
{
  ...
  QCRIL_QMI_VOICE_CCFC_REASON_NOTLOGIN  = 6,
...
typedef struct
{
    ...
    uint8_t                                     call_id_info;
    uint8_t                                     is_alpha_relayed;
...
typedef struct
{
    ...
    boolean               pending_req;
    TimeKeeper::timer_id  pending_request_timeout_timer_id;
...
typedef struct qcril_qmi_voice_pd_info
{
   ...
   qcril_qmi_voice_pd_info() :
     handle(nullptr),
     state(QCRIL_QMI_PD_UNKNOWN),
     pd_init_thread_valid(FALSE) {
   }
} qcril_qmi_voice_pd_info;
...
typedef struct  qcril_qmi_audio_info
{
    ...
    qcril_qmi_audio_info() : state(QCRIL_QMI_AUDIO_STATE_UNKNOWN) {
    }
} qcril_qmi_audio_info;

struct qcril_qmi_voice_info_type
{
  ...
  uint8_t clir;
  uint8_t ussd_user_action_required;
  ...
  qcril::interfaces::UssdDomain ussd_request_domain;

  qcril_qmi_voice_info_type() :
    last_call_is_local_ringback(FALSE),
    last_local_ringback_call_id(VOICE_INVALID_CALL_ID),
    ussd_user_action_required(FALSE),
    send_vice_unsol_on_socket_connect(FALSE),
    ussd_request_domain(qcril::interfaces::UssdDomain::UNKNOWN)
  {
      clir = ( uint8_t ) QCRIL_QMI_VOICE_SS_CLIR_PRESENTATION_INDICATOR;
      pil_state.state = QCRIL_QMI_PIL_STATE_UNKNOWN;
      memset(&qmi_info_ind_xml, 0, sizeof(qmi_info_ind_xml));
  }
};
...
#define QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_NW_DETECTED_EME_CALL       ((((uint64_t) 1) << 2) + 1)
#define QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_IS_SILENT_UI_VALID         ((((uint64_t) 1) << 3) + 1)
#define QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_IS_ORIGINAL_CALL_TYPE_VALID ((((uint64_t) 1) << 4) + 1)
#define QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_IS_CRS_TYPE_VALID          ((((uint64_t) 1) << 5) + 1)
#define QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_PROG_INFO_NOTIF_VALID      ((((uint64_t) 1) << 6) + 1)
#define QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_DIVERSION_INFO_VALID       ((((uint64_t) 1) << 7) + 1)
#define QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_MSIM_ADDITIONAL_CALL_INFO_VALID \
  ((((uint64_t)1) << 8) + 1)
#define QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_COMPUTED_AUDIO_QUALITY_VALID \
  ((((uint64_t)1) << 9) + 1)
#define QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_IS_VOS_SUPPORT_VALID       ((((uint64_t) 1) << 10) + 1)
...
typedef struct qcril_qmi_voice_voip_call_info_entry_type
{
    ...
    TimeKeeper::timer_id                    ringing_time_id;
    ...
    TimeKeeper::timer_id                    call_obj_phase_out_timer_id;
    ...
    voice_ip_diversion_info_type_v02          diversion_info;
    // QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_MSIM_ADDITIONAL_CALL_INFO_VALID
    qcril::interfaces::MsimAdditionalCallInfo msim_additional_call_info;
    // QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_COMPUTED_AUDIO_QUALITY_VALID
    qcril::interfaces::ComputedAudioQuality computed_audio_quality;
    // QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_IS_VOS_SUPPORT_VALID
    bool                                     vos_support;
...
struct qcril_qmi_voice_voip_overview_type
{
    ...
    TimeKeeper::timer_id                        num_1x_wait_timer_id;
    boolean                                     num_1x_wait_timer_wakelock_is_acquired;
    TimeKeeper::timer_id                        auto_answer_timer_id;

    qcril_qmi_voice_voip_overview_type() :
        call_info_root(nullptr),
        call_info_enumeration_current(nullptr),
        command_exec_oversight_root(nullptr),
        command_exec_oversight_current(nullptr),
        num_1x_wait_timer_id(TimeKeeper::no_timer),
        num_1x_wait_timer_wakelock_is_acquired(FALSE),
        auto_answer_timer_id(TimeKeeper::no_timer) {
    }
};
...
boolean qcril_qmi_voice_has_crs_type_call(const qcril_qmi_voice_voip_call_info_entry_type* call_info_entry);

void qcril_qmi_voice_make_incoming_call_ring(
    std::shared_ptr<VoiceMakeIncomingCallRingMessage> msg);

void qcril_qmi_voice_set_operational_status(qmi_ril_gen_operational_status_type status);
qmi_ril_gen_operational_status_type qcril_qmi_voice_get_operational_status();
