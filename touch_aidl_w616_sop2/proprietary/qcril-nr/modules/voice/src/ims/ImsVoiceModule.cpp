...
ImsVoiceModule::ImsVoiceModule() : VoiceModule("ImsVoiceModule") {
  ...
  std::unordered_map<std::reference_wrapper<message_id_info>, std::function<void(std::shared_ptr<Message>)>>
      subMessageHandler = {
          HANDLER(QcRilRequestImsDialMessage, ImsVoiceModule::handleQcRilRequestImsDialMessageWrapper),
          ...
          HANDLER(QcRilRequestImsAnswerMessage, ImsVoiceModule::handleQcRilRequestImsAnswerMessageWrapper),
          ...
          HANDLER(IpcReleaseAudioMessage,
                  ImsVoiceModule::handleIpcReleaseAudioMessage),
          HANDLER(IpcReleaseAudioCnfMessage,
                  ImsVoiceModule::handleIpcReleaseAudioCnfMessage),
          HANDLER(ReleaseAudioRequestMessage,
                  ImsVoiceModule::handleReleaseAudioRequestMessage),
      };
...
void ImsVoiceModule::init() {
  ...
  // ring2: Dial/Resume/Answer.
  // This ring specifically to handle hybrid DSDA related usecases
  std::vector<std::string> ring2 {
        QcRilRequestImsDialMessage::MESSAGE_NAME,
        QcRilRequestImsResumeMessage::MESSAGE_NAME,
        QcRilRequestImsAnswerMessage::MESSAGE_NAME,
  };
...
RIL_Errno ImsVoiceModule::dispatchReleaseAudio(
...
void ImsVoiceModule::handleQcRilRequestImsDialMessageWrapper(
    std::shared_ptr<QcRilRequestImsDialMessage> msg) {
  RIL_Errno rilErr = RIL_E_SUCCESS;
  bool isDsda = isDeviceInDsdaMode();
  bool callOnOtherSub = isCallPresentOnOtherSub();
  QCRIL_LOG_DEBUG("dsda - %d, call on other sub - %d", isDsda, callOnOtherSub);
  // Check if device is in DSDA mode and call present on other SUB
  if(isDsda && callOnOtherSub) {
    ...
    handleQcRilRequestImsDialMessage(msg);
  }
}
...
void ImsVoiceModule::handleQcRilRequestImsAnswerMessageWrapper(
  ...
  if(isDsda && callOnOtherSub) {
    ...
    handleQcRilRequestImsAnswerMessage(msg);
  }
}
...
void ImsVoiceModule::handleQcRilRequestImsResumeMessageWrapper(
  ...
  if(isDsda && callOnOtherSub) {
    ...
    handleQcRilRequestImsResumeMessage(msg);
  }
}
...
void ImsVoiceModule::notifyConferenceCallStateCompleted() {
...
void ImsVoiceModule::processReleaseAudioResponse(CommonVoiceResponseData *data) {
  QCRIL_LOG_FUNC_ENTRY();
  if (data) {
  ...
  if (data) {
    ...
    if (pendingMsg &&
        (pendingMsg->get_message_id() ==
               QcRilRequestImsAnswerMessage::get_class_message_id())) {
      if (data->errNo != RIL_E_SUCCESS) {
...
void ImsVoiceModule::handleIpcReleaseAudioMessage(std::shared_ptr<IpcReleaseAudioMessage> ipcMsg) {
...
void ImsVoiceModule::handleReleaseAudioRequestMessage(std::shared_ptr<ReleaseAudioRequestMessage> msg) {
  RIL_Errno rilErr = RIL_E_SUCCESS;
  QCRIL_LOG_FUNC_ENTRY("msg = %s", (msg ? msg->dump().c_str() : "nullptr"));
  auto pendingMsgStatus = getPendingMessageList().insert(msg);
  do {
    ...
    rilErr = qcril_qmi_voice_process_release_audio_req(
      req_id,
      std::bind(&ImsVoiceModule::processReleaseAudioResponse, this, std::placeholders::_1));
  } while(false);
...
void ImsVoiceModule::handleIpcReleaseAudioCnfMessage(std::shared_ptr<IpcReleaseAudioCnfMessage> ipcMsg) {
  QCRIL_LOG_FUNC_ENTRY("msg = %s", (ipcMsg ? ipcMsg->dump().c_str() : "nullptr"));
  // Handle only the message from the other RILD
  if (ipcMsg && ipcMsg->getIsRemote()) {
    ...
    do {
      ...
      if(pendingMsg) {
        msg = std::static_pointer_cast<QcRilRequestImsDialMessage>(pendingMsg);
        if(msg && msg->getPendingMessageState() == PendingMessageState::AWAITING_PRECONDITIONS) {
          ...
          if(ipcMsg->getStatus() == RIL_E_SUCCESS) {
            handleQcRilRequestImsDialMessage(std::static_pointer_cast<QcRilRequestImsDialMessage>(msg));
          }
        }
      ...
      if(pendingMsg) {
        ...
        if(msg && msg->getPendingMessageState() == PendingMessageState::AWAITING_PRECONDITIONS) {
          ...
          msg->setPendingMessageState(PendingMessageState::FREE);
          if(ipcMsg->getStatus() == RIL_E_SUCCESS) {
            handleQcRilRequestImsResumeMessage(std::static_pointer_cast<QcRilRequestImsResumeMessage>(msg));
          }
        }
        // Assuming only one of the DIAL/RESUME/ANSWER in pending list.
        break;
      ...
      pendingMsg = getPendingMessageList().find(
        QcRilRequestImsAnswerMessage::get_class_message_id());
      if(pendingMsg) {
        msg = std::static_pointer_cast<QcRilRequestImsAnswerMessage>(pendingMsg);
        if(msg && msg->getPendingMessageState() == PendingMessageState::AWAITING_PRECONDITIONS) {
          getPendingMessageList().erase(pendingMsg);
          msg->setPendingMessageState(PendingMessageState::FREE);
          if(ipcMsg->getStatus() == RIL_E_SUCCESS) {
            handleQcRilRequestImsAnswerMessage(std::static_pointer_cast<QcRilRequestImsAnswerMessage>(msg));
          }
        }
        // Assuming only one of the DIAL/RESUME/ANSWER in pending list.
        break;
