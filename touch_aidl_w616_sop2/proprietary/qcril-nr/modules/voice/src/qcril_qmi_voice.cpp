/******************************************************************************
#  Copyright (c) 2012-2013, 2017-2023 Qualcomm Technologies, Inc.
#  All Rights Reserved.
...
{
  auto pendingMsg =
      findVoiceModulePendingMessage(QcRilRequestImsHangupMessage::get_class_message_id());
  if (pendingMsg == nullptr)
  {
    pendingMsg =
        findVoiceModulePendingMessage(QcRilRequestCsHangupMessage::get_class_message_id());
  }
  if (pendingMsg == nullptr)
  {
    ...
    *number_presentation = QCRIL_QMI_VOICE_RIL_PI_UNKNOWN;
    ...
    // ------------------------------------------------------------------------------------------
      ...
      if ((call_info_entry->elaboration & QCRIL_QMI_VOICE_VOIP_CALLINFO_ELA_REDIRECTING_PARTY_NUM_VALID)
            && (call_info_entry->voice_svc_redirecting_party_num.num_len > 0))
      {
        auto len = call_info_entry->voice_svc_redirecting_party_num.num_len;
        *redir_number = (char*)qcril_malloc(len + 1);
        if (*redir_number)
        {
           memcpy(*redir_number, call_info_entry->voice_svc_redirecting_party_num.num, len);
...
)
{
    ...
    do
    {
        ...
        if (qcril_qmi_voice_voip_call_info_is_single_elaboration_set(
                call_info_entry,
                QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_DIVERSION_INFO_VALID))
        {
          payload_ptr->diversion_info_valid[iter] = TRUE;
          ...
          size_t diversion_info_utf8_size = strlen(diversion_info_utf8) + 1;
          payload_ptr->diversion_info[iter] = (char *)qcril_malloc(diversion_info_utf8_size);
          strlcpy(payload_ptr->diversion_info[iter], diversion_info_utf8, diversion_info_utf8_size);
        }
        ...
        err = qcril_qmi_voice_get_current_call_number(call_info_entry,
                ...
                &payload_ptr->info[iter].redirNum,
                &payload_ptr->info[iter].redirNumPresentation);
        ...
        if (is_ims)
        {
            if (call_info_entry->elaboration & QCRIL_QMI_VOICE_VOIP_CALLINFO_ELA_CODEC_VALID)
            {
                ...
                payload_ptr->codec[iter] = qcril_qmi_map_audio_codec(call_info_entry->codec);
            }

            if (qcril_qmi_voice_voip_call_info_is_single_elaboration_set(
                    call_info_entry,
                    QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_COMPUTED_AUDIO_QUALITY_VALID)) {
              payload_ptr->computed_audio_quality_valid[iter] = TRUE;
              payload_ptr->computed_audio_quality[iter] = call_info_entry->computed_audio_quality;
            }
        ...
        {
            uint8_t alternate_sip_uris_len = 0;
        ...
        payload_ptr->silent_ui_valid[iter] = FALSE;
        if (qcril_qmi_voice_voip_call_info_is_single_elaboration_set(
                call_info_entry,
                QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_IS_SILENT_UI_VALID))
        {
            payload_ptr->silent_ui_valid[iter] = TRUE;
            payload_ptr->silent_ui[iter] = call_info_entry->silent_ui;
            QCRIL_LOG_INFO("call_info_entry->silent_ui = %d",
                    call_info_entry->silent_ui);
        }

        // Original call type for CRS calls
        payload_ptr->original_call_type_valid[iter] = FALSE;
        if (qcril_qmi_voice_voip_call_info_is_single_elaboration_set(
                call_info_entry,
                QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_IS_ORIGINAL_CALL_TYPE_VALID))
        {
            payload_ptr->original_call_type_valid[iter] = TRUE;
            payload_ptr->original_call_type[iter] = call_info_entry->original_call_type;
            QCRIL_LOG_INFO("call_info_entry->original_call_type = %d",
                    call_info_entry->original_call_type);
        }

        // CRS type for CRS calls
        payload_ptr->crs_type_valid[iter] = FALSE;
        if (qcril_qmi_voice_voip_call_info_is_single_elaboration_set(
                call_info_entry,
                QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_IS_CRS_TYPE_VALID))
        {
            payload_ptr->crs_type_valid[iter] = TRUE;
            payload_ptr->crs_type[iter] = call_info_entry->crs_type;
            QCRIL_LOG_INFO("call_info_entry->crs_type = %d",
                    call_info_entry->crs_type);
        }

        // Call progress notification info
        payload_ptr->prog_info_notif_valid[iter] = FALSE;
        if (qcril_qmi_voice_voip_call_info_is_single_elaboration_set(
                call_info_entry,
                QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_PROG_INFO_NOTIF_VALID))
        {
            payload_ptr->prog_info_notif_valid[iter] = TRUE;
            payload_ptr->prog_info_notif[iter] = call_info_entry->prog_info_notif;
            QCRIL_LOG_INFO("call_info_entry->prog_info_notif = %s",
                           qcril::interfaces::toString(call_info_entry->prog_info_notif).c_str());
        }

        // Msim additional call information
        payload_ptr->msim_additional_call_info_valid[iter] = FALSE;
        if (qcril_qmi_voice_voip_call_info_is_single_elaboration_set(
                call_info_entry,
                QCRIL_QMI_VOICE_VOIP_CALLINFO_EXT_ELA_MSIM_ADDITIONAL_CALL_INFO_VALID)) {
          payload_ptr->msim_additional_call_info_valid[iter] = TRUE;
          payload_ptr->msim_additional_call_info[iter] = call_info_entry->msim_additional_call_info;
          QCRIL_LOG_INFO(
              "call_info_entry->msim_additional_call_info = %s",
        ...
        {
          payload_ptr->vos_support_valid[iter] = TRUE;
          payload_ptr->vos_support[iter] = call_info_entry->vos_support;
...
)
{
  ...
  if ( NULL != payload_ptr )
  {
    for ( i=0; i < payload_ptr->num_of_calls; i++ )
    {
      ...
      if ( payload_ptr->info[ i ].redirNum && FALSE == is_ril_number_already_freed )
      {
        qcril_free( payload_ptr->info[ i ].redirNum );
      }
      ...
      if( NULL != payload_ptr->diversion_info[i])
      {
        qcril_free(payload_ptr->diversion_info[i]);
      }
...
void qcril_qmi_voice_request_get_current_atel_calls(std::shared_ptr<QcRilRequestGetCurrentCallsMessage> msg)
{
  ...
  if ((ril_req_res == RIL_E_SUCCESS) && payload_ptr)
  {
    ...
    for ( i = 0; i < payload_ptr->num_of_calls; i++ )
    {
      ...
      if (payload_ptr->info_ptr[i]->redirNum) {
        callInfo.setRedirNum(payload_ptr->info_ptr[i]->redirNum);
      }
      callInfo.setRedirNumPresentation(payload_ptr->info_ptr[i]->redirNumPresentation);
      ...
      QCRIL_LOG_INFO( "...num %s, num presentation %d, name %s, name presentation %d"
                      "redirected number: %s, redireced number presentation number: %d",
                       ...
                       payload_ptr->info_ptr[ i ]->redirNum,
                       payload_ptr->info_ptr[ i ]->redirNumPresentation);
    }
  }
   ...
   if ( RIL_E_SUCCESS == ril_req_res )
   {
      ...
      std::vector<qcril::interfaces::CallInfo> calls(payload_ptr->num_of_calls);
        ...
        for ( i = 0; i < payload_ptr->num_of_calls; i++ )
        {
          ...
          if (payload_ptr->diversion_info_valid[ i ] == TRUE)
          {
            calls[i].setDiversionInfo(payload_ptr->diversion_info[i]);
          }
          ...
          if (payload_ptr->computed_audio_quality_valid[i]) {
            calls[i].setComputedAudioQuality(payload_ptr->computed_audio_quality[i]);
          }
          ...
          if (payload_ptr->silent_ui_valid[i]) {
            calls[i].setSilentUi(payload_ptr->silent_ui[i]);
          }
          if (payload_ptr->original_call_type_valid[i]) {
            calls[i].setOriginalCallType(
                    map_qmi_call_type_to_ril_type(payload_ptr->original_call_type[i]));
          }
          if (payload_ptr->crs_type_valid[i]) {
            calls[i].setCrsType(
                    qcril_qmi_map_crs_type_to_ims(payload_ptr->crs_type[i]));
          }
          if (payload_ptr->prog_info_notif_valid[i]) {
            calls[i].setProgressInfo(payload_ptr->prog_info_notif[i]);
          }
          if (payload_ptr->msim_additional_call_info_valid[i]) {
            calls[i].setMsimAdditionalCallInfo(payload_ptr->msim_additional_call_info[i]);
          }
          if (payload_ptr->vos_support_valid[i]) {
            calls[i].setVosSupport(payload_ptr->vos_support[i]);
          }
        }
  ...
  if( temp_ptr )
  {
    ...
    if( temp_ptr->is_valid )
    {
      ...
      if( info_ptr )
      {
        ...
        if (info_ptr->redirNum)
        {
          free(info_ptr->redirNum);
        }
...
)
{
    ...
    {
      pendingMsg = extractVoiceModulePendingMessage(
              QcRilRequestImsHangupMessage::get_class_message_id());
      if (pendingMsg != nullptr)
      {
  ...
  if (entry && call_status_ind)
  {
    ...
    if (call_status_ind->prog_info_notif_valid)
    {
      ...
      for (size_t idxCall = 0; idxCall < max_calls; idxCall++)
      {
        ...
        {
          entry->prog_info_notif.setType(qcril_qmi_map_call_prog_info_notif_type(
                call_status_ind->prog_info_notif[idxCall].notif_type));
          entry->prog_info_notif.setReasonCode(
              call_status_ind->prog_info_notif[idxCall].notif_reason);
          entry->prog_info_notif.setReasonText(
               call_status_ind->prog_info_notif[idxCall].notif_text);
          qcril_qmi_voice_voip_call_info_set_single_elaboration(entry,
...
)
{
    ...
    do
    {
        ...
        if (NULL != current_calls_type_ptr->info[0].redirNum) {
          payload_ptr->setRedirNum(current_calls_type_ptr->info[0].redirNum);
        }
        payload_ptr->setRedirNumPresentation(current_calls_type_ptr->info[0].redirNumPresentation);
