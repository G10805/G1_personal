...
DECLARE_MSG_ID_INFO(LTE_QMI_IND);
DECLARE_MSG_ID_INFO(LTE_ENDPOINT_STATUS_IND);
...
LteDirectModule::LteDirectModule() : AddPendingMessageList("LteDirectModule") {
  ...
  mMessageHandler = {
      ...
      HANDLER_MULTI(QmiIndMessage, LTE_QMI_IND, LteDirectModule::handleQmiIndMessage),
      HANDLER_MULTI(EndpointStatusIndMessage, LTE_ENDPOINT_STATUS_IND, LteDirectModule::handleEndpointStatusIndMessage),
      // QMI Async response
...
void LteDirectModule::handleQcrilInitMessage(std::shared_ptr<QcrilInitMessage> msg) {
  ...
  ModemEndPointFactory<LteModemEndPoint>::getInstance().buildEndPoint()->requestSetup(
      "LteDirectModule-Token", msg->get_instance_id(), &qmiLteSetupCallback);
}
