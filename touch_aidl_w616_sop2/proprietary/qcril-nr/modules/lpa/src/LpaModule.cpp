...
#include "modules/lpa/LpaQmiUimHttpRequestCallback.h"
#include "modules/lpa/LpaUimReqMessageCallback.h"
...
::android::sp<LpaServiceBase> mUimlpaService = nullptr;
...
)
{
  ...
  if (!mUimlpaService)
  {
     for (auto svcImpl : getHalServiceImplFactory<LpaServiceBase>()) {
      bool result = svcImpl->registerService(instanceId);
      if (result) {
        Log::getInstance().d("[" + mName + "]: Registered!");
        mUimlpaService = svcImpl;
        mUimlpaService->mModule = getLpaModule();
        break;
      }
    }
  }
...
)
{
  ...
  if (user_req_ptr == nullptr)
  {
    ...
    if (mUimlpaService != nullptr)
    {
      mUimlpaService->uimLpaUserResponse(token, &resp_data);
    }
    return;
  }
...
)
{
  ...
  if (http_req_ptr == nullptr)
  {
    ...
    if (mUimlpaService != nullptr)
    {
      mUimlpaService->uimLpaHttpTxnCompletedResponse(token, LPA_SERVICE_RESULT_FAILURE);
    }
    return;
  }
...
)
{
  ...
  if (mUimlpaService != nullptr)
  {
    mUimlpaService->uimLpaHttpTxnCompletedResponse(rsp_data_ptr->token, http_result);
  }
    ...
    case QMI_UIM_HTTP_SRVC_TRANSACTION_IND_MSG:
      {
        ...
        if (mUimlpaService != nullptr)
        {
          mUimlpaService->uimLpaHttpTxnIndication(svc_ind_ptr);
        }
    ...
    case UIM_LPA_USER_RESP_ID:
      {
        ...
        if (mUimlpaService != nullptr)
        {
          mUimlpaService->uimLpaUserResponse(msg_ptr->get_token(), &resp_data);
        }
      }
      break;
    default:
  ...
  switch(msg_ptr->get_ind_id())
  {
    case UIM_LPA_PROFILE_PROGRESS_IND_ID:
      if (msg_ptr->get_data_ptr() != nullptr)
      {
        ...
        if (mUimlpaService != nullptr)
        {
          mUimlpaService->uimLpaAddProfileProgressInd(&lpa_ind);
        }
      }
      break;
    default:
