...
)
{
  ...
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    ...
    {
      for (uint8_t index  =0; index < qmi_response->extended_card_info_len && index < rsp_data->rsp_data.get_card_status_rsp.card_status.num_slots && index < QMI_UIM_MAX_SLOT_COUNT; index++)
      {
  ...
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    ...
    if(qmi_response->personalization_status_valid)
    {
      ...
      if(qmi_response->personalization_status_len > 0 &&
         qmi_response->personalization_status_len < QMI_UIM_MAX_PERSO_FEATURES)
      {
        uint8_t i = 0;
        uint8_t j = 0;
    ...
    if(qmi_response->temporary_depersonalization_status_valid)
    {
      ...
      for( i = 0; i < qmi_response->temporary_depersonalization_status_len && i < QMI_UIM_MAX_EXT_CARD_COUNT; i++)
      {
...
/*===========================================================================
  FUNCTION  qmi_uim_get_euicc_info2_resp
===========================================================================*/
/*!
@brief
  translates response data coming from QCCI into response data
...
*/
/*=========================================================================*/
static void qmi_uim_get_euicc_info2_resp
(
  const uim_get_euicc_info2_resp_msg_v01   * qmi_response,
  qmi_uim_rsp_data_type                    * rsp_data
)
{
  if(qmi_response == NULL || rsp_data == NULL)
  ...
  rsp_data->rsp_id = QMI_UIM_SRVC_GET_EUICC_INFO2_RSP_MSG;
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    rsp_data->qmi_err_code = QMI_NO_ERR;
    if (qmi_response->euicc_info2_val_valid &&
        qmi_response->euicc_info2_val_len > 0 &&
        qmi_response->euicc_info2_val_len <= QMI_UIM_EUICC_INFO2_MAX_V01)
    {
      rsp_data->rsp_data.euicc_info2_rsp.euicc_info2_len = qmi_response->euicc_info2_val_len;
      rsp_data->rsp_data. euicc_info2_rsp.euicc_info2_ptr = (uint8_t *)&qmi_response->euicc_info2_val[0];
    }
  }
  else
  {
...
/*===========================================================================
  FUNCTION  qcril_uim_qmi_set_hlos_prov_pref_resp
===========================================================================*/
/*!
@brief
  translates response data coming from QCCI into response data
...
*/
/*=========================================================================*/
static void qcril_uim_qmi_set_hlos_prov_pref_resp
(
  const qmi_uim_provisioning_resp_msg_v01     * qmi_response,
  qmi_uim_rsp_data_type                       * rsp_data
)
{
  if(qmi_response == NULL || rsp_data == NULL)
  ...
  rsp_data->rsp_id = QMI_UIM_SRVC_SET_HLOS_PROV_PREFERENCE_RSP_MSG;
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    rsp_data->qmi_err_code = QMI_NO_ERR;
  }
  else
  {
   ...
   QCRIL_LOG_INFO( "=DBG= qcril_uim_qmi_set_hlos_prov_pref_resp error: 0x%x", qmi_response->resp.error);

} /* qcril_uim_qmi_set_hlos_prov_pref_resp */
...
)
{
  if(ind_data_ptr != NULL)
  {
    ...
    {
      for (uint8_t index  =0; index < ind_data_ptr->extended_card_info_len && index < ind_data->status_change_ind.card_status.num_slots && index < QMI_UIM_MAX_SLOT_COUNT; index++)
      {
...
/**
 * Converts QMI refresh stage enum to QCRIL enum
 */
static int qcril_uim_convert_refresh_stage(uim_refresh_stage_enum_v01 qmi_ref_stage, qmi_uim_refresh_stage* ref_stage)
{
  switch (qmi_ref_stage)
    {
      case UIM_REFRESH_STAGE_WAIT_FOR_OK_V01:
        *ref_stage = QMI_UIM_REFRESH_STAGE_WAIT_FOR_OK_TO_INIT;
        break;
      case UIM_REFRESH_STAGE_START_V01:
        *ref_stage = QMI_UIM_REFRESH_STAGE_START;
        break;
      case UIM_REFRESH_STAGE_END_WITH_SUCCESS_V01:
        *ref_stage = QMI_UIM_REFRESH_STAGE_END_SUCCESS;
        break;
      case UIM_REFRESH_STAGE_END_WITH_FAILURE_V01:
        *ref_stage = QMI_UIM_REFRESH_STAGE_END_FAILURE;
        break;
      default:
...
/**
 * Converts QMI refresh mode enum to QCRIL enum
 */
static int qcril_uim_convert_refresh_mode(uim_refresh_mode_enum_v01 qmi_refresh_mode, qmi_uim_refresh_mode* refresh_mode)
{
  switch (qmi_refresh_mode)
  {
    case UIM_REFRESH_MODE_RESET_V01:
      *refresh_mode = QMI_UIM_REFRESH_MODE_RESET;
      break;
    case UIM_REFRESH_MODE_INIT_V01:
      *refresh_mode = QMI_UIM_REFRESH_MODE_INIT;
      break;
    case UIM_REFRESH_MODE_INIT_FCN_V01:
      *refresh_mode = QMI_UIM_REFRESH_MODE_INIT_FCN;
      break;
    case UIM_REFRESH_MODE_FCN_V01:
      *refresh_mode = QMI_UIM_REFRESH_MODE_FCN;
      break;
    case UIM_REFRESH_MODE_INIT_FULL_FCN_V01:
      *refresh_mode = QMI_UIM_REFRESH_MODE_INIT_FULL_FCN;
      break;
    case UIM_REFRESH_MODE_APP_RESET_V01:
      *refresh_mode = QMI_UIM_REFRESH_MODE_APP_RESET;
      break;
    case UIM_REFRESH_MODE_3G_RESET_V01:
      *refresh_mode = QMI_UIM_REFRESH_MODE_3G_SESSION_RESET;
      break;
    default:
      return QMI_INTERNAL_ERR;
  }
...
/**
 * Converts QMI session type enum to QCRIL enum
 */
static int qcril_uim_convert_session_type(uim_session_type_enum_v01 qmi_session_type, qmi_uim_session_type* session_type){
  ...
  switch (qmi_session_type)
  {
    ...
    case UIM_SESSION_TYPE_CARD_ON_SLOT_2_V01:
      *session_type = QMI_UIM_SESSION_TYPE_CARD_SLOT_2;
      break;
    ...
    case UIM_SESSION_TYPE_CARD_ON_SLOT_3_V01:
      *session_type = QMI_UIM_SESSION_TYPE_CARD_SLOT_3;
      break;
    default:
      return QMI_INTERNAL_ERR;
  }
...
)
{
  ...
  if(ind_data_ptr->refresh_event_valid)
  {
    ...
    // convert the refresh_stage
    if ( qcril_uim_convert_refresh_stage(ind_data_ptr->refresh_event.stage,
        &(ind_data->refresh_ind.refresh_event.refresh_stage)) != QMI_NO_ERR )
    {
      QCRIL_LOG_ERROR("Unknown refresh stage %d",ind_data_ptr->refresh_event.stage);
      return QMI_SERVICE_ERR;
    }

    // convert the refresh_mode
    if ( qcril_uim_convert_refresh_mode(ind_data_ptr->refresh_event.mode,
        &(ind_data->refresh_ind.refresh_event.refresh_mode)) != QMI_NO_ERR )
    {
      QCRIL_LOG_ERROR("Unknown refresh mode %d",ind_data_ptr->refresh_event.mode);
      return QMI_SERVICE_ERR;
    }

    // convert the session type
    if ( qcril_uim_convert_session_type(ind_data_ptr->refresh_event.session_type,
        &(ind_data->refresh_ind.refresh_event.session_type)) != QMI_NO_ERR )
    {
      QCRIL_LOG_ERROR("Unknown or unsupported session type %d",ind_data_ptr->refresh_event.session_type);
      return QMI_SERVICE_ERR;
    }

    ind_data->refresh_ind.refresh_event.aid_len = (unsigned char)ind_data_ptr->refresh_event.aid_len;
    if (ind_data->refresh_ind.refresh_event.aid_len > 0)
    {
...
)
{
  switch (msg_id)
  {
    ...
    case QMI_UIM_PROVISIONING_RESP_V01:
      *rsp_id = QMI_UIM_SRVC_SET_HLOS_PROV_PREFERENCE_RSP_MSG;
      break;
    case QMI_UIM_GET_EUICC_INFO2_RESP_V01:
      *rsp_id = QMI_UIM_SRVC_GET_EUICC_INFO2_RSP_MSG;
      break;
    default:
      return QMI_INTERNAL_ERR;
  }
  ...
  else
  {
    ...
    switch (msg_id)
    {
      ...
      case QMI_UIM_PROFILE_CONFIRMATION_CODE_RESP_V01:
        ...
        break;
      case QMI_UIM_PROVISIONING_RESP_V01:
        qcril_uim_qmi_set_hlos_prov_pref_resp((qmi_uim_provisioning_resp_msg_v01*)qmi_response, &rsp_data);
        break;
      case QMI_UIM_GET_EUICC_INFO2_RESP_V01:
        qmi_uim_get_euicc_info2_resp((uim_get_euicc_info2_resp_msg_v01 *)qmi_response,&rsp_data);
        break;
...
/**
  * Convert QCRIL session_type enum to QMI enum
 */

static int qcril_qmi_convert_session_type(qmi_uim_session_type uim_session_type,
    uim_session_type_enum_v01& qmi_session_type)
{
  switch(uim_session_type)
    {
      case QMI_UIM_SESSION_TYPE_PRI_GW_PROV:
        qmi_session_type = UIM_SESSION_TYPE_PRIMARY_GW_V01;
        break;
      case QMI_UIM_SESSION_TYPE_PRI_1X_PROV:
        qmi_session_type = UIM_SESSION_TYPE_PRIMARY_1X_V01;
        break;
      case QMI_UIM_SESSION_TYPE_SEC_GW_PROV:
        qmi_session_type = UIM_SESSION_TYPE_SECONDARY_GW_V01;
        break;
      case QMI_UIM_SESSION_TYPE_SEC_1X_PROV:
        qmi_session_type = UIM_SESSION_TYPE_SECONDARY_1X_V01;
        break;
      case QMI_UIM_SESSION_TYPE_NON_PROV_SLOT_1:
        qmi_session_type = UIM_SESSION_TYPE_NONPROVISIONING_SLOT_1_V01;
        break;
      case QMI_UIM_SESSION_TYPE_NON_PROV_SLOT_2:
        qmi_session_type = UIM_SESSION_TYPE_NONPROVISIONING_SLOT_2_V01;
        break;
      case QMI_UIM_SESSION_TYPE_CARD_SLOT_1:
        qmi_session_type = UIM_SESSION_TYPE_CARD_ON_SLOT_1_V01;
        break;
      case QMI_UIM_SESSION_TYPE_CARD_SLOT_2:
        qmi_session_type = UIM_SESSION_TYPE_CARD_ON_SLOT_2_V01;
        break;
      case QMI_UIM_SESSION_TYPE_TER_GW_PROV:
        qmi_session_type = UIM_SESSION_TYPE_TERTIARY_GW_V01;
        break;
      case QMI_UIM_SESSION_TYPE_TER_1X_PROV:
        qmi_session_type = UIM_SESSION_TYPE_TERTIARY_1X_V01;
        break;
      case QMI_UIM_SESSION_TYPE_NON_PROV_SLOT_3:
        qmi_session_type = UIM_SESSION_TYPE_NONPROVISIONING_SLOT_3_V01;
        break;
      case QMI_UIM_SESSION_TYPE_CARD_SLOT_3:
        qmi_session_type = UIM_SESSION_TYPE_CARD_ON_SLOT_3_V01;
        break;
      default:
        return QMI_INTERNAL_ERR;
    }
...
)
{
  ...
  if(qcril_qmi_convert_session_type(params->session_info.session_type, qmi_request->session_information.session_type) != QMI_NO_ERR )
  {
    QCRIL_LOG_ERROR("Unknown session type %d", params->session_info.session_type);
    qcril_free(qmi_request);
    qcril_free(qmi_response);
    return QMI_SERVICE_ERR;
  }
...
)
{
  ...
  if(rc == QMI_NO_ERR && qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    rsp_data->qmi_err_code = QMI_NO_ERR;
  }
  else
...
/**
 * Convert QCRIL pin_id enum to QMI enum
 */
static int qcril_qmi_convert_pin_id(qmi_uim_pin_id_type uim_pin_id,
    uim_pin_id_enum_v01* qmi_pin_id )
{
  switch(uim_pin_id)
  {
    case QMI_UIM_PIN_ID_PIN1:
      *qmi_pin_id = UIM_PIN_ID_PIN_1_V01;
      break;
    case QMI_UIM_PIN_ID_PIN2:
      *qmi_pin_id = UIM_PIN_ID_PIN_2_V01;
      break;
    case QMI_UIM_PIN_ID_UPIN:
      *qmi_pin_id = UIM_PIN_ID_UNIVERSAL_V01;
      break;
    default:
      return QMI_INTERNAL_ERR;
  }
...
/**
    * Convert QCRIL pin_operation enum to QMI enum
   */

static int qcril_qmi_convert_pin_operation(qmi_uim_pin_op_type uim_pin_operation ,
    uim_pin_operation_enum_v01* qmi_pin_operation)
{
  switch(uim_pin_operation)
  {
    case QMI_UIM_PIN_OP_DISABLE:
      *qmi_pin_operation = UIM_PIN_OPERATION_DISABLE_V01;
      break;
    case QMI_UIM_PIN_OP_ENABLE:
      *qmi_pin_operation = UIM_PIN_OPERATION_ENABLE_V01;
      break;
    default:
      return QMI_INTERNAL_ERR;
  }
...
)
{
  ...
  if(qcril_qmi_convert_pin_id(params->pin_id, &(qmi_request->set_pin_protection.pin_id)) != QMI_NO_ERR )
  {
    QCRIL_LOG_ERROR("Unknown pin id %d", params->pin_id);
    return QMI_SERVICE_ERR;
  }

  qmi_request->set_pin_protection.pin_operation = (uim_pin_operation_enum_v01)params->pin_operation;
  if(qcril_qmi_convert_pin_operation(params->pin_operation, &(qmi_request->set_pin_protection.pin_operation)) != QMI_NO_ERR )
  {
   QCRIL_LOG_ERROR("Unknown pin operation %d", params->pin_operation);
   qcril_free(qmi_request);
   qcril_free(qmi_response);
   return QMI_SERVICE_ERR;
  }
...
)
{
  ...
  if(params->is_pin1_encrypted)
  {
    ...
    if(qcril_qmi_convert_pin_id(params->pin_id, &(qmi_request->verify_pin.pin_id)) != QMI_NO_ERR )
    {
    QCRIL_LOG_ERROR("Unknown pin id %d", params->pin_id);
    qcril_free(qmi_request);
    qcril_free(qmi_response);
...
)
{
  ...
  if(qcril_qmi_convert_pin_id(params->pin_id, &(qmi_request->unblock_pin.pin_id)) != QMI_NO_ERR )
  {
    QCRIL_LOG_ERROR("Unknown pin id %d", params->pin_id);
    qcril_free(qmi_request);
    qcril_free(qmi_response);
  ...
  memcpy(qmi_request->unblock_pin.new_pin_value,
         params->new_pin_data.data_ptr,
  ...
  qmi_request->session_information.aid_len = (uint32_t)(params->session_info.aid.data_len);
  memcpy(qmi_request->session_information.aid,
         params->session_info.aid.data_ptr,
...
)
{
  ...
  if(qcril_qmi_convert_pin_id(params->pin_id, &(qmi_request->change_pin.pin_id)) != QMI_NO_ERR )
  {
    QCRIL_LOG_ERROR("Unknown pin id %d", params->pin_id);
    qcril_free(qmi_request);
    qcril_free(qmi_response);
...
)
{
  ...
  if(qcril_qmi_convert_session_type(params->session_type, qmi_request->session_change.session_type) != QMI_NO_ERR )
  {
    QCRIL_LOG_ERROR("Unknown session type %d", params->session_type);
    qcril_free(qmi_request);
    qcril_free(qmi_response);
    return QMI_SERVICE_ERR;
  }
...
/*===========================================================================
  FUNCTION  qcril_qmi_uim_get_euicc_info2
===========================================================================*/
/*!
@brief
  LPA get euicc_info2
...
*/
/*=========================================================================*/
EXTERN int qcril_qmi_uim_get_euicc_info2
(
  qmi_client_type                                         client_handle,
  const qmi_uim_get_euicc_info2_req_type                  * params_ptr,
  qmi_uim_user_async_cb_type                              user_cb,
  void                                                    * user_data,
  qmi_uim_rsp_data_type                                   * rsp_data
)
{
  qmi_txn_handle                                txn_handle;
  int                                           rc               = 0;
  uim_get_euicc_info2_req_msg_v01               euicc_info2_req;
  uim_get_euicc_info2_resp_msg_v01            * euicc_info2_resp_ptr     = NULL;
  qmi_uim_cb_struct_type                      * cb_data          = NULL;

  memset(&euicc_info2_req, 0x00, sizeof(euicc_info2_req));
  ...
  euicc_info2_req.slot = (uim_slot_enum_v01)params_ptr->slot;

  /* Allocate response pointer since it is an async command */
  euicc_info2_resp_ptr = (uim_get_euicc_info2_resp_msg_v01 *)
                     qcril_malloc(sizeof(uim_get_euicc_info2_resp_msg_v01));
  if (euicc_info2_resp_ptr == NULL)
  {
    QCRIL_LOG_ERROR("error allocating memory for euicc_info2_resp_ptr");
    return QMI_SERVICE_ERR;
  }
  ...
  if (user_cb)
  {
    ...
    {
      QCRIL_LOG_ERROR("failed to allocate cb_data");
      qcril_free(euicc_info2_resp_ptr);
      return QMI_SERVICE_ERR;
    }
    QMI_UIM_MAKE_CB_STRUCT(cb_data, user_cb, user_data);

    rc = qmi_client_send_msg_async( client_handle,
                                    QMI_UIM_GET_EUICC_INFO2_REQ_V01,
                                    &euicc_info2_req,
                                    sizeof(uim_get_euicc_info2_req_msg_v01),
                                    (void *) euicc_info2_resp_ptr,
                                    sizeof(uim_get_euicc_info2_resp_msg_v01),
                                    qmi_uim_client_async_cb,
                                    cb_data,
                                    &txn_handle);
    if( rc != QMI_NO_ERR )
  ...
  else
  {
    ...
    if(!rc)
    {
      qmi_uim_get_euicc_info2_resp(euicc_info2_resp_ptr, rsp_data);
    }
    qcril_free(euicc_info2_resp_ptr);
  }
...
/*===========================================================================
  FUNCTION  qcril_qmi_uim_set_hlos_prov_preference
===========================================================================*/
/*!
@brief
  Issues the request to set provisioning preference of HLOS at Modem.
  If the user_cb function pointer is set to NULL, this function will
  be invoked synchronously. Otherwise it will be invoked asynchronously.
...
*/
/*=========================================================================*/
int qcril_qmi_uim_set_hlos_prov_preference
(
  qmi_client_type                                   client_handle,
  const qmi_uim_hlos_prov_preference_req_type     * params,
  qmi_uim_user_async_cb_type                        user_cb,
  void                                            * user_data,
  qmi_uim_rsp_data_type                           * rsp_data
)
{
  int                                 rc;
  qmi_txn_handle                      txn_handle;
  qmi_uim_provisioning_req_msg_v01   *qmi_request   = NULL;
  qmi_uim_provisioning_resp_msg_v01  *qmi_response  = NULL;
  qmi_uim_cb_struct_type          * cb_data       = NULL;
  ...
  qmi_request = (qmi_uim_provisioning_req_msg_v01*)qcril_malloc(sizeof(qmi_uim_provisioning_req_msg_v01));
  if(qmi_request == NULL)
  {
    return QMI_SERVICE_ERR;
  }

  qmi_response = (qmi_uim_provisioning_resp_msg_v01*)qcril_malloc(sizeof(qmi_uim_provisioning_resp_msg_v01));
  if(qmi_response == NULL)
  {
    qcril_free(qmi_request);
    return QMI_SERVICE_ERR;
  ...
  memset(qmi_request, 0, sizeof(qmi_uim_provisioning_req_msg_v01));
  memset(qmi_response, 0, sizeof(qmi_uim_provisioning_resp_msg_v01));

  /*set HLOS provisioning type */
  qmi_request->provisioning_type = params->hlosProvPreference;

  QCRIL_LOG_INFO("provisioning_type - %d", qmi_request->provisioning_type);
  ...
  else
  {
    ...
    if(!rc)
    {
      qcril_uim_qmi_set_hlos_prov_pref_resp(qmi_response, rsp_data);
    }
    qcril_free(qmi_response);
  }
  qcril_free(qmi_request);
