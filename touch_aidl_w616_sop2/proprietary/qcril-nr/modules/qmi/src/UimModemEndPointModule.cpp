 ...
 QCRIL_LOG_INFO( "%s, request: %d", __FUNCTION__, request_type);
  ...
  switch(request_type)
  {
    ...
    case QCRIL_UIM_REQUEST_SET_HLOS_PROV_PREFERENCE:
      ret = qcril_qmi_uim_set_hlos_prov_preference( mQmiSvcClient,
                                           (const qmi_uim_hlos_prov_preference_req_type*)param_data_ptr,
                                           callback_function_ptr,
                                           user_data,
                                           rsp_data_ptr);
      break;

    case QCRIL_UIM_REQUEST_GET_EUICC_INFO2:
      ret = qcril_qmi_uim_get_euicc_info2(mQmiSvcClient,
                                          (const qmi_uim_get_euicc_info2_req_type*)param_data_ptr,
                                           callback_function_ptr,
                                           user_data,
                                           rsp_data_ptr);
      break;
