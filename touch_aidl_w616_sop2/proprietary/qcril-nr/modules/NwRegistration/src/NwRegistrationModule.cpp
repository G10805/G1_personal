...
NwRegistrationModule::NwRegistrationModule() : AddPendingMessageList("NwRegistrationModule")
{
    ...
    mMessageHandler = {
         HANDLER_MULTI(EndpointStatusIndMessage, NAS_ENDPOINT_STATUS_IND, NwRegistrationModule::handleNasEndpointStatusIndMessage),
...
void NwRegistrationModule::qcril_qmi_nwreg_process_nas_registration_info(
    ...
    if (nas_reg_info.rilCellIdentity_valid) {
        ...
        rejInfo.choosenPlmn_valid = true;
    ...
    if( reportRejInd && rejInfo.choosenPlmn_valid &&
    ...
    if ((msg != nullptr)) {
        if (msg->isRejectCauseValid()) {
            ...
            snprintf(rejInfo.choosenPlmn,
                sizeof(rejInfo.choosenPlmn),
                "%s%s",
                msg->getPLMNInfo().mMcc.c_str(),
                msg->getPLMNInfo().mMnc.c_str());
