...
#define QCRIL_GSTK_TOKEN_MASK                      (1<<16)
...
)
{
  ...
  /* Updating Userdata Pointer with a Constant Value and original token values
   * Received from the list therby ensuring user_data pointer is always non null
   * irrespective of token values
   */
  user_data.user_data = (void *)(uintptr_t)(QCRIL_GSTK_TOKEN_MASK | token.first);
