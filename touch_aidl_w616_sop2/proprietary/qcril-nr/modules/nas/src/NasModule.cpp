...
NasModule::NasModule() : AddPendingMessageList("NasModule") {
  ...
  mMessageHandler = {
    ...
    HANDLER(RilRequestSetNrConfigMessage, NasModule::handleSetNrConfig),
    HANDLER(RilRequestQueryNrConfigMessage, NasModule::handleQueryNrConfig),
    ...
    HANDLER(NasSetVonrCapRequest, NasModule::handleSetVonrCapRequest),
...
void NasModule::init()
{
  ...
  std::vector<std::string> ring5{RilRequestSetNrConfigMessage::MESSAGE_NAME,
    RilRequestQueryNrConfigMessage::MESSAGE_NAME};
...
void NasModule::handleSetVonrCapRequest(std::shared_ptr<NasSetVonrCapRequest> msg) {
  Log::getInstance().d("[" + mName + "]: Handling msg = " + msg->dump());
...
void NasModule::handleSetNrConfig(std::shared_ptr<RilRequestSetNrConfigMessage> msg) {
    Log::getInstance().d("[NasModule]: Handling msg = " + msg->dump());
    qcril_qmi_nas_request_set_nr_config(msg);
}

void NasModule::handleQueryNrConfig(std::shared_ptr<RilRequestQueryNrConfigMessage> msg) {
    Log::getInstance().d("[NasModule]: Handling msg = " + msg->dump());
    qcril_qmi_nas_request_query_nr_config(msg);
}
