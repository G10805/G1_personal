...
)
{
  ...
  do
  {
      ...
      if (qcril_qmi_nas_get_last_android_mode_pref(&last_android_mode_pref))
      {

          if (mode_pref_param == last_android_mode_pref)

          {
              QCRIL_LOG_INFO("Requested mode is same as previously set; return success directly");
              break;
          }
      }
      ...
      if (RIL_E_SUCCESS == ril_req_res)
      {
          ...
          if (qcril_qmi_nas_is_feature_power_opt_enable())
          {
...
)
{
    ...
    nas_nr5g_disable_mode_enum_type_v01  qmi_nr_disable_mode;
    ...
    switch (msg->getConfig())
    {
      case NR_DISABLE_MODE_NONE:
          qmi_nr_disable_mode = NAS_NR5G_DISABLE_MODE_NONE_V01;
          break;
      case NR_DISABLE_MODE_SA:
          qmi_nr_disable_mode = NAS_NR5G_DISABLE_MODE_SA_V01;
          break;
      case NR_DISABLE_MODE_NSA:
          qmi_nr_disable_mode = NAS_NR5G_DISABLE_MODE_NSA_V01;
          break;
      default:
          qmi_nr_disable_mode = NAS_NR5G_DISABLE_MODE_NONE_V01;
          break;
    ...
    qmi_request.nr5g_disable_mode_valid = TRUE;
    qmi_request.nr5g_disable_mode     = qmi_nr_disable_mode;

    QCRIL_LOG_INFO("disable mode: %d", qmi_request.nr5g_disable_mode);
...
{
    RIL_Errno ril_req_res = RIL_E_SUCCESS;
    RIL_NR_DISABLE_MODE nr_disable_mode;
    uint8_t nr_disable_mode_valid = FALSE;

    QCRIL_LOG_FUNC_ENTRY();

    nr_disable_mode_valid = qcril_qmi_nas_fetch_nr_disable_mode(&nr_disable_mode);
    if (!nr_disable_mode_valid) {
        qcril_qmi_fetch_system_selection_preference();
        nr_disable_mode_valid = qcril_qmi_nas_fetch_nr_disable_mode(&nr_disable_mode);
    }

    if (!nr_disable_mode_valid) {
        QCRIL_LOG_ERROR("Cannot obtain current nr disable mode.");
        ril_req_res = RIL_E_INVALID_MODEM_STATE;
    }
    QCRIL_LOG_INFO("disable mode = %d", nr_disable_mode);
    auto payload = std::make_shared<qcril::interfaces::RilQueryNrConfigResult_t>(nr_disable_mode);
    if (payload == nullptr) {
        ril_req_res = RIL_E_NO_MEMORY;
    }
...
)
{
  ...
  qcril::interfaces::RilBandPrefType deferred_band_pref_map = qcril::interfaces::RilBandPrefType::NONE;
  qcril::interfaces::RilRatBandType band_type = qcril::interfaces::RilRatBandType::NONE;
  uint8_t                        band_pref_valid = FALSE;
  uint8_t                        band_pref_map = 0;
  ...
  do {
    ...
    band_type = *((qcril::interfaces::RilRatBandType*)params_ptr->data);
...
{
  qcril_request_resp_params_type resp;
  qcril::interfaces::RilBandPrefType band_pref_map = qcril::interfaces::RilBandPrefType::NONE;
  RIL_Errno                      ril_req_res = RIL_E_GENERIC_FAILURE;
  int                            res = FALSE;
  ...
  do
  {
      ...
      band_pref_map = *((qcril::interfaces::RilBandPrefType*)params_ptr->data);

      switch ( band_pref_map )
      {
          case qcril::interfaces::RilBandPrefType::LTE_FULL:
          case qcril::interfaces::RilBandPrefType::TDD_LTE:
          case qcril::interfaces::RilBandPrefType::FDD_LTE:
                res = qmi_ril_nas_cache_deferred_band_pref(
                    qcril::interfaces::RilRatBandType::LTE_BAND, band_pref_map);
            break;
...
)
{
  ...
  if (msg) {
    qcril::interfaces::RilBandPrefType band_pref_map = qcril::interfaces::RilBandPrefType::NONE;
    qcril::interfaces::RilRatBandType band_type = msg->getRatBandType();
    uint8_t band_pref_valid = qcril_qmi_nas_get_band_pref_map(band_type, &band_pref_map);
    QCRIL_LOG_DEBUG("band_pref_valid: %d band_pref_map:%d band_type: %d",
        band_pref_valid, band_pref_map, band_type);

    auto payload = std::make_shared<qcril::interfaces::GetPreferredNeworkBandPrefResult_t>();
    if (payload) {
      payload->bandPrefMap = band_pref_map;
    }
...
)
{
  ...
  if (msg) {
    RIL_Errno ril_req_res = RIL_E_GENERIC_FAILURE;
    qcril::interfaces::RilBandPrefType band_pref_map = msg->getBandPrefMap();
    ...
    switch (band_pref_map)
    {
      ...
      case qcril::interfaces::RilBandPrefType::TDD_LTE:
      case qcril::interfaces::RilBandPrefType::FDD_LTE:
        if (qmi_ril_nas_cache_deferred_band_pref(qcril::interfaces::RilRatBandType::LTE_BAND,
                                                 band_pref_map)) {
          ril_req_res = RIL_E_SUCCESS;
        }
        break;
    ...
    QCRIL_LOG_DEBUG("band_pref_map:%d, ril_req_res: %d", band_pref_map, ril_req_res);
    auto respData = std::make_shared<QcRilRequestMessageCallbackPayload>(ril_req_res, nullptr);
    msg->sendResponse(msg, Message::Callback::Status::SUCCESS, respData);
  } while ( FALSE );
  QCRIL_LOG_FUNC_RETURN();
...
RIL_Errno qmi_ril_nwreg_request_mode_pref( int android_mode_pref, uint8_t *is_change )
{
    ...
    if ( RIL_E_SUCCESS == res )
    {
          ...
          lte_band_pref_valid = qcril_qmi_nas_get_band_pref(qcril::interfaces::RilRatBandType::LTE_BAND, &lte_band_pref);
...
)
{
  ...
  if (sim_mcc_str && mcc_str && mnc_str && long_ons_ptr && short_ons_ptr)
  {
    ...
    *short_ons_ptr = NULL;
    ...
    {
      string config;
      if (qcril_config_get(PERSIST_VENDOR_RADIO_CN_MCC, config) == E_SUCCESS)
      {
        res = strlcpy(nas_cn_mcc, config.c_str(), sizeof(nas_cn_mcc));
        if (res >= sizeof(nas_cn_mcc))
        {
            QCRIL_LOG_ERROR("Too long value for cn MCC: %s", config.c_str());
            strlcpy( nas_cn_mcc, NAS_CN_MCC, sizeof(nas_cn_mcc));
        }
        else
        {
            QCRIL_LOG_DEBUG("nas_cn_mcc = %s", nas_cn_mcc);
        }
      }

      if (qcril_config_get(PERSIST_VENDOR_RADIO_TW_MCC, config) == E_SUCCESS)
      {
        res = strlcpy(nas_tw_mcc, config.c_str(), sizeof(nas_tw_mcc));
        ...
        {
            QCRIL_LOG_DEBUG("nas_tw_mcc = %s", nas_tw_mcc);
        }
      }
    }
...
#ifndef FEATURE_ENHANCED_OEMHOOK_IMPLN
static RIL_Errno readSetMaxTransmitPowerParams(struct sar_rf_state &out, const uint8_t* in, uint32_t inLen)
{
  if (!in || !inLen) {
    QCRIL_LOG_DEBUG("Invalid input data and length");
    return RIL_E_INVALID_ARGUMENTS;
  }
  if (inLen < sizeof(out.rf_state_index)) {
    QCRIL_LOG_DEBUG("Invalid input data length for rf_state_index");
    return RIL_E_INVALID_ARGUMENTS;
  }
  memcpy(&out.rf_state_index, in, sizeof(out.rf_state_index));
  inLen -= sizeof(out.rf_state_index);
  in += sizeof(out.rf_state_index);

  if (inLen < sizeof(out.compatibility_key)) {
    QCRIL_LOG_DEBUG("Invalid input data length for compatibility_key");
    return RIL_E_INVALID_ARGUMENTS;
  }
  memcpy(&out.compatibility_key, in, sizeof(out.compatibility_key));
  inLen -= sizeof(out.compatibility_key);
  in += sizeof(out.compatibility_key);
  ...
  out.compatibility_key_valid = TRUE;
  if (inLen >= sizeof(out.compatibility_key_valid)) {
    memcpy(&out.compatibility_key_valid, in, sizeof(out.compatibility_key_valid));
  }
  return RIL_E_SUCCESS;
}

static qmi_sar_rf_state_enum_type_v01 map_qmi_sar_rf_state(int rf_state_index)
{
  qmi_sar_rf_state_enum_type_v01 sar_rf_state;
  switch (rf_state_index)
  {
    case 1:
      sar_rf_state = QMI_SAR_RF_STATE_1_V01;
      break;

    case 2:
      sar_rf_state = QMI_SAR_RF_STATE_2_V01;
      break;

    case 3:
      sar_rf_state = QMI_SAR_RF_STATE_3_V01;
      break;

    case 4:
      sar_rf_state = QMI_SAR_RF_STATE_4_V01;
      break;

    case 5:
      sar_rf_state = QMI_SAR_RF_STATE_5_V01;
      break;

    case 6:
      sar_rf_state = QMI_SAR_RF_STATE_6_V01;
      break;

    case 7:
      sar_rf_state = QMI_SAR_RF_STATE_7_V01;
      break;

    case 8:
      sar_rf_state = QMI_SAR_RF_STATE_8_V01;
      break;

    case 9:
      sar_rf_state = QMI_SAR_RF_STATE_9_V01;
      break;

    case 10:
      sar_rf_state = QMI_SAR_RF_STATE_10_V01;
      break;

    case 11:
      sar_rf_state = QMI_SAR_RF_STATE_11_V01;
      break;

    case 12:
      sar_rf_state = QMI_SAR_RF_STATE_12_V01;
      break;

    case 13:
      sar_rf_state = QMI_SAR_RF_STATE_13_V01;
      break;

    case 14:
      sar_rf_state = QMI_SAR_RF_STATE_14_V01;
      break;

    case 15:
      sar_rf_state = QMI_SAR_RF_STATE_15_V01;
      break;

    case 16:
      sar_rf_state = QMI_SAR_RF_STATE_16_V01;
      break;

    case 17:
      sar_rf_state = QMI_SAR_RF_STATE_17_V01;
      break;

    case 18:
      sar_rf_state = QMI_SAR_RF_STATE_18_V01;
      break;

    case 19:
      sar_rf_state = QMI_SAR_RF_STATE_19_V01;
      break;

    case 20:
      sar_rf_state = QMI_SAR_RF_STATE_20_V01;
      break;

    case 0:     // fallthrough
    default:
      sar_rf_state = QMI_SAR_RF_STATE_DEFAULT_V01;
      break;
  }
  return sar_rf_state;
}
...
)
{
  ...
  if (params_ptr)
  {
      ril_req_res = readSetMaxTransmitPowerParams(ril_param, (const uint8_t*)params_ptr->data,
                                                  params_ptr->datalen);
  }
  ...
  if (ril_req_res == RIL_E_SUCCESS)
  {
      ...
      qmi_request.sar_rf_state = map_qmi_sar_rf_state(ril_param.rf_state_index);
      qmi_request.compatibility_key_valid = ril_param.compatibility_key_valid;
      qmi_request.compatibility_key = ril_param.compatibility_key;
      QCRIL_LOG_INFO(".. params rf state index %d / sar rf state %d",
          (int)ril_param.rf_state_index, (int)qmi_request.sar_rf_state);
      QCRIL_LOG_INFO(".. params compatibility_key valid %d / qmi compatibility_key valid %d",
          (int)ril_param.compatibility_key_valid, (int)qmi_request.compatibility_key_valid);
...
)
{
  ...
  if ( NULL != rfpe_client_handle )
  {
      ...
      if( RIL_E_SUCCESS == ril_req_res )
      {
          ...
          resp.resp_pkt = (void *) &ril_response;
          resp.resp_len =  sizeof(Qtuner_get_provisioned_table_revision_resp_v01);
      }
  }
  else
  {
