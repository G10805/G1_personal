...
void qcril_qmi_prov_send_unsol_sub_pref_change()
{
    ...
    qcril::interfaces::RILSubProvStatus_t provStatus(
      resp_payload.user_preference,
      resp_payload.current_sub_preference);
    auto msg = std::make_shared<RilUnsolSubProvisioningStatusMessage>(provStatus);
    Dispatcher::getInstance().dispatchSync(msg);
}
#else
void qcril_qmi_prov_send_unsol_sub_pref_change()
...
)
{
  ...
  do {
    ...
    std::shared_ptr<qcril::interfaces::RilIccidResult_t> payload = nullptr;
    payload = std::make_shared<qcril::interfaces::RilIccidResult_t>(iccid);
...
void qcril_qmi_prov_get_sub_prov_pref_req_handler(
  ...
  do {
    ...
    qcril_qmi_prov_fill_prov_preference_info(&status);
    QCRIL_LOG_INFO( "User pref %d", status.user_preference);
    QCRIL_LOG_INFO( "Current sub pref %d", status.current_sub_preference);

    std::shared_ptr<qcril::interfaces::RILSubProvStatus_t> payload = nullptr;
    payload = std::make_shared<qcril::interfaces::RILSubProvStatus_t>(
    status.user_preference, status.current_sub_preference);
    auto respData = std::make_shared<QcRilRequestMessageCallbackPayload>(
        res, payload);
    msg->sendResponse(msg, Message::Callback::Status::SUCCESS, respData);
  } while(0);
...
void qcril_qmi_prov_set_sub_prov_pref_req_handler(
  ...
  do {
    ...
    qcril::interfaces::RILSetSubProvState_t prov_pref = msg->getState();
...
)
{
    ...
    if ( params_ptr->data != NULL && params_ptr->datalen > 0 )
    {
        ...
        else
        {
            ...
            {
                auto& pendingList = getNasModule().getPendingMessageList();
                auto pendingProvMsg =
                    pendingList.find(RilRequestSetSubProvisioningPrefMessage::get_class_message_id());
                if (pendingProvMsg)
                {
                    auto msg =
                        std::static_pointer_cast<RilRequestSetSubProvisioningPrefMessage>(pendingProvMsg);
                    //Activation triggerd through OEM interface
