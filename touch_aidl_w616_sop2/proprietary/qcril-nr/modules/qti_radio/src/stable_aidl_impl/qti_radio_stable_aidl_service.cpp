...
void QtiRadioStable::notifyConfigureDeactivateDelayTime(int64_t delayTime) {
  auto indCb = getIndicationCallback();
  if (indCb) {
    QCRIL_LOG_DEBUG("notifyConfigureDeactivateDelayTime: delayTime:%l", delayTime);
...
void QtiRadioStable::sendResponseForSetNrConfig(int32_t serial, RIL_Errno errorCode) {
  auto respCb = getResponseCallback();
  if (respCb) {
...
void QtiRadioStable::sendResponseForQueryNrConfig(
    int32_t serial, RIL_Errno errorCode,
...
void QtiRadioStable::sendResponseForGetFacilityLockForApp(int32_t serial, RIL_Errno errorCode,
                                                          int32_t response) {
  auto respCb = getResponseCallback();
  if (respCb) {
    QCRIL_LOG_DEBUG("getFacilityLockForAppResponse: serial=%d, error=%d", serial, errorCode);
    auto ret =
        respCb->getFacilityLockForAppResponse(serial, static_cast<uint32_t>(errorCode), response);
    if (!ret.isOk()) {
      QCRIL_LOG_ERROR("Unable to send response. Exception : %s", ret.getDescription().c_str());
    }
  }
...
void QtiRadioStable::sendResponseForGetImei(
  ...
  if (respCb) {
    QCRIL_LOG_DEBUG("getImeiResponse: serial=%d errorcode=%d imei=%s type %d", serial, errorCode,
                    imeiInfo.imei.c_str(), imeiInfo.type);
...
::ndk::ScopedAStatus QtiRadioStable::setNrConfig(int32_t in_serial,
  ...
  auto msg = std::make_shared<RilRequestSetNrConfigMessage>(this->getContext(in_serial),
  ...
  if (msg) {
    GenericCallback<QcRilRequestMessageCallbackPayload> cb(
        [this, in_serial](std::shared_ptr<Message> /*msg*/, Message::Callback::Status /*status*/,
          ...
          sendResponseForSetNrConfig(in_serial, errorCode);
        });
    msg->setCallback(&cb);
    msg->dispatch();
  } else {
    sendResponseForSetNrConfig(in_serial, RIL_E_NO_MEMORY);
  }
  return ndk::ScopedAStatus::ok();
}
...
::ndk::ScopedAStatus QtiRadioStable::queryNrConfig(int32_t in_serial) {
  ...
  auto msg = std::make_shared<RilRequestQueryNrConfigMessage>(this->getContext(in_serial));
  if (msg != nullptr) {
    GenericCallback<QcRilRequestMessageCallbackPayload> cb(
        [this, in_serial](std::shared_ptr<Message> /*msg*/, Message::Callback::Status /*status*/,
                          std::shared_ptr<QcRilRequestMessageCallbackPayload> resp) -> void {
          ...
          sendResponseForQueryNrConfig(in_serial, errorCode, payload);
        });
    msg->setCallback(&cb);
    msg->dispatch();
  } else {
    sendResponseForQueryNrConfig(in_serial, RIL_E_NO_MEMORY, nullptr);
  }
  return ndk::ScopedAStatus::ok();
}
...
::ndk::ScopedAStatus QtiRadioStable::setNrUltraWidebandIconConfig(
    ...
    const std::vector<aidlimports::NrUwbIconRefreshTime>& in_refreshTime,
  ...
  if (msg) {
    ...
    if(!in_refreshTime.empty()) {
      std::vector<rildata::NrUCIconRefreshTime_t> refreshList;
      refreshList.resize(in_refreshTime.size());
      for(int i=0; i<in_refreshTime.size(); i++) {
        refreshList[i].timerType = utils::convertNrUCIconRefreshTimer((in_refreshTime)[i].timerType);
        refreshList[i].timeValue = (in_refreshTime)[i].timeValue;
      }
