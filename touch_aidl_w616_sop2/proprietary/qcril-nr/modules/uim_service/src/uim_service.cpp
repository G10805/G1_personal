...
)
{
  ...
  if (msg)
  {
    GenericCallback<RIL_UIM_GBA_InitResponse> cb(
        ([this](std::shared_ptr<Message> solicitedMsg, Message::Callback::Status ret_status,
                       std::shared_ptr<RIL_UIM_GBA_InitResponse> rsp_ptr) -> void
    {
      ...
      uimGbainitResponse(rsp_token, gbaStatus, isKsNafEncrypted, ksNaf, bTid, lifeTime);
    }));
    msg->setCallback(&cb);
    msg->dispatch();
  }
...
)
{
  ...
  if (msg)
  {
                       ...
                       std::shared_ptr<RIL_UIM_GBA_IMPI_Response> rsp_ptr) -> void
    {
      ...
      uimGetImpiResponse(rsp_token, gbaStatus, is_impi_encrypted, impi);
    }));
    msg->setCallback(&cb);
    msg->dispatch();
  }
