...
)
{
  ...
  QCRIL_LOG_DEBUG("unsol_event = 0x%X\n", unsol_event);
  // EVENT REFACTOR TEST
  payload = NULL;
  do
  {
...
RIL_RadioState qcril_get_current_radio_state()
{
  ...
  if (qcril_qmi_nas_modem_power_is_mdm_shdn_in_apm() &&
      0 == PeripheralMgr::getInstance().getPrimaryModemVotingState() &&
...
)
{
  ...
  PeripheralMgr::getInstance().deregisterClientWithModem();
...
void qcril_qmi_modem_power_process_bootup(void)
{
    ...
    if (PeripheralMgr::getInstance().getPrimaryModemVotingState() == 0) {
        PeripheralMgr::getInstance().voteUpPrimaryModem();
    }

    if (PeripheralMgr::getInstance().getSecondaryModemVotingState() == 0) {
...
{
    QCRIL_LOG_FUNC_ENTRY();
    QCRIL_LOG_DEBUG("Vote for primary modem");
    PeripheralMgr::getInstance().voteUpPrimaryModem();

    QCRIL_LOG_DEBUG("Vote for secondary modem");
    PeripheralMgr::getInstance().voteUpSecondaryModem();
...
{
    boolean ret = PeripheralMgr::getInstance().pmVotingSupportPrimaryModem() ||
                    PeripheralMgr::getInstance().esocVotingSupportPrimaryModem();

    QCRIL_LOG_FUNC_RETURN_WITH_RET(ret);
    return ret;
} // qcril_qmi_modem_power_is_voting_feature_supported_for_primary_modem

void QCRIL_setProcessInstanceId(int client_id) {
  ...
  QCRIL_LOG_DEBUG("client id = %d, instance id = %d", client_id, instance_id);
  qmi_ril_set_process_instance_id(instance_id);
}

void QCRIL_dbInit() {
  DatabaseModule::createDataBaseModule(qmi_ril_get_process_instance_id());
}
#ifdef RIL_SHLIB
/*===========================================================================
...
)
{
  ...
  PeripheralMgr::getInstance().registerClientWithModem();

  uint32_t qmi_ril_sim_slot = 0; // use 1st slot as default

  if (instance_id == QCRIL_SECOND_INSTANCE_ID)
  ...
  else if (instance_id == QCRIL_THIRD_INSTANCE_ID)
  ...
  QCRIL_LOG_DEBUG( "RIL %d, running RIL_Init()", instance_id );
