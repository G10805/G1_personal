...
#ifdef QMI_RIL_UTF
extern uint32_t qcril_get_time_milliseconds();
#include <unistd.h>
#endif
...
#ifndef RIL_FOR_MDM_LE //TODO: decouple OEMHOOK
...
void sendOemhookResponse(qcril_instance_id_e_type oemSocketInstanceId, int serial,
    RIL_Errno errorCode, uint8_t* buf, size_t bufLen);
...
bool oemhookServiceRegistered = 0;
...
//===========================================================================
RIL_Errno qmi_ril_parse_oem_hook_header ( unsigned char *data, size_t data_len, qmi_ril_oem_hook_request_details_type *outcome )
{
  ...
  do
  {
    ...
    if (data_len < QCRIL_OTHER_OEM_NAME_LENGTH)
    {
       QCRIL_LOG_ERROR("insufficient data");
       break;
    }
    ...
    index += (uint32_t)QCRIL_OTHER_OEM_NAME_LENGTH;
    data_len -= QCRIL_OTHER_OEM_NAME_LENGTH;

    if (data_len < QCRIL_OTHER_OEM_REQUEST_ID_LEN)
    {
       QCRIL_LOG_ERROR("insufficient data for command id");
       break;
    }
    ...
    index += (uint32_t)QCRIL_OTHER_OEM_REQUEST_ID_LEN;
    data_len -= (uint32_t)QCRIL_OTHER_OEM_REQUEST_ID_LEN;

    if (data_len < QCRIL_OTHER_OEM_REQUEST_DATA_LEN)
    {
       QCRIL_LOG_ERROR("insufficient data for data len");
       break;
    }
    ...
    data_len -= (uint32_t)QCRIL_OTHER_OEM_REQUEST_DATA_LEN;
    if (outcome_data.hook_req_len > data_len)
    {
       QCRIL_LOG_ERROR("Invalid data_len received");
       break;
    }
...
)
{
  ...
  *audit_result = RIL_E_INTERNAL_ERR;
  ...
  info_fetch_result = qmi_ril_oem_hook_get_request_id( oem_hook_req_details->ril_idl_service_id,
                                                       ...
                                                       (uint32_t*)&oem_hook_req_details->hook_req,
                                                       &evt_name );
...
)
{
  ...
  if ( ( RIL_E_SUCCESS == qmi_ril_parse_oem_hook_header( data, recv_byte_num, oem_hook_req_details ) ) && oem_hook_req_details->is_oem_hook )
  {
