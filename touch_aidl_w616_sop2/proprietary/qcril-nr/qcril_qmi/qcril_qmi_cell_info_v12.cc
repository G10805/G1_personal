...
void qcril_qmi_nas_initialize_nr5g_ril_cellinfo
(
...
void qcril_qmi_nas_init_cell_info_struct_v12(RIL_CellInfo_v12 *cell_info)
{
    ...
    cell_info->CellInfo.lte.cellIdentityLte.band = INT_MAX;
    ...
    // NR SNPN info
    cell_info->CellInfo.nr.cellIdentityNr.snpn_info_valid = FALSE;

    // NR CAG info
    cell_info->CellInfo.nr.cellIdentityNr.cag_info_valid = FALSE;
    ...
    do
    {
      ...
      if (cellinfobuf)
      {
        ...
        if ( li->lte_intra_valid )
        {
          ...
          srv_cell->CellInfo.lte.cellIdentityLte.band = getNasModule().getLteBand();
          ...
          for (i = 0; i < li->lte_intra.cells_len && ((unsigned)(curr_cel - cellinfobuf) < ncells || !srv_done); i++)
          {
            if (li->lte_intra.cells[i].pci == li->lte_intra.serving_cell_id)
            {
              ...
              QCRIL_LOG_DEBUG("Output rsrp: %d, rsrq: %d, rssnr: %d, cqi: %d, ta: %d"
                      ...
                      srv_cell->CellInfo.lte.cellIdentityLte.band);
              srv_done = TRUE;
            }
            ...
            else
            {
              ...
              curr_cel->CellInfo.lte.cellIdentityLte.band = INT_MAX; // We don't know bandwidth for neighbors
        ...
        if ( li->lte_inter_valid )
        {
          for (i = 0; i < li->lte_inter.freqs_len; i++)
          {
            for (j = 0; j < li->lte_inter.freqs[i].cells_len && (unsigned)(curr_cel - cellinfobuf) < ncells; j++)
            {
              ...
              if((li->lte_inter_earfcn_valid) && (i < li->lte_inter_earfcn_len))
              {
                qcril_qmi_nas_init_lte_cell_identity_v12(
                      &curr_cel->CellInfo.lte.cellIdentityLte,
                      NULL,
              ...
              curr_cel->CellInfo.lte.cellIdentityLte.bandwidth = INT_MAX; // We don't know bandwidth for neighbors
              curr_cel->CellInfo.lte.cellIdentityLte.band = INT_MAX; // We don't know band for neighbors
              qcril_qmi_nas_init_lte_cell_ss_v8(
                      &curr_cel->CellInfo.lte.signalStrengthLte,
                      NAS_RADIO_IF_LTE,
                      TRUE,
        ...
        if (li->nr5g_serving_cell_info_valid)
        {
            int mcc;
            int mnc;
            qcril_qmi_nas_initialize_nr5g_ril_cellinfo(curr_cel,
                                                     TRUE,
                                                     RIL_TIMESTAMP_TYPE_OEM_RIL,
                                                     timestamp);
            mcc = QCRIL_MCC_FROM_BCD_PLMN(li->nr5g_serving_cell_info.plmn);
            mnc = QCRIL_MNC_FROM_BCD_PLMN(li->nr5g_serving_cell_info.plmn);
            snprintf( curr_cel->CellInfo.nr.cellIdentityNr.mcc, NAS_MCC_MNC_MAX_SIZE, "%03d", (int)mcc);
            if ( (mnc > 99) ||  QCRIL_MNC_INCLUDES_PCS(li->nr5g_serving_cell_info.plmn) )
            {
                snprintf (curr_cel->CellInfo.nr.cellIdentityNr.mnc, NAS_MCC_MNC_MAX_SIZE, "%03d", (int)mnc);
            }
            else
            {
                snprintf (curr_cel->CellInfo.nr.cellIdentityNr.mnc, NAS_MCC_MNC_MAX_SIZE, "%02d", (int)mnc);
            }

            qcril_qmi_nas_cell_identity_plmn_name(
              curr_cel->CellInfo.nr.cellIdentityNr.operatorNames.alphaShort,
              MAX_OPERATOR_NAME_LEN,
              curr_cel->CellInfo.nr.cellIdentityNr.operatorNames.alphaLong,
              MAX_OPERATOR_NAME_LEN,
              (char*)li->nr5g_serving_cell_info.plmn,
              NAS_RADIO_IF_NR5G_V01
            );

            //fill tac : uint32_t nr_tac =  00000000 + tac[0]+tac[1]+tac[2] ;
            int32_t tac_code = 0x00000000;
            tac_code = li->nr5g_serving_cell_info.nr5g_tac_info.tac[0] <<16 | li->nr5g_serving_cell_info.nr5g_tac_info.tac[1] << 8 | li->nr5g_serving_cell_info.nr5g_tac_info.tac[2];

            curr_cel->CellInfo.nr.cellIdentityNr.nci = li->nr5g_serving_cell_info.global_cell_id;
            curr_cel->CellInfo.nr.cellIdentityNr.pci = li->nr5g_serving_cell_info.pci;
            curr_cel->CellInfo.nr.cellIdentityNr.tac = tac_code;
            curr_cel->CellInfo.nr.cellIdentityNr.bandwidth = getNasModule().getNgranBw();
            curr_cel->CellInfo.nr.cellIdentityNr.band = getNasModule().getNgranBand();

            if (li->nr5g_arfcn_valid)
            {
                curr_cel->CellInfo.nr.cellIdentityNr.nrarfcn = li->nr5g_arfcn;
            }

            curr_cel->CellInfo.nr.signalStrengthNr.ssRsrp = (li->nr5g_serving_cell_info.rsrp/10) * (-1);
            curr_cel->CellInfo.nr.signalStrengthNr.ssRsrq = (li->nr5g_serving_cell_info.rsrq/10) * (-1);
            curr_cel->CellInfo.nr.signalStrengthNr.ssSinr = li->nr5g_serving_cell_info.snr/10;
            curr_cel ++;
        }
        else if (li->nr5g_arfcn_valid)
        {
          /*handle NR cell-info in NSA mode, where only nr-arfcn is present in the indication
          * from modem */
          qcril_qmi_nas_initialize_nr5g_ril_cellinfo(curr_cel,
                                                   TRUE,
                                                   RIL_TIMESTAMP_TYPE_OEM_RIL,
                                                   timestamp);
          curr_cel->CellInfo.nr.cellIdentityNr.nrarfcn = li->nr5g_arfcn;
          /* in NSA , fill the cell-info details from LTE serving cell as NR cell-info
          will not be present in NSA mode*/
          if (li->lte_intra_valid) {
            curr_cel->CellInfo.nr.cellIdentityNr.tac = li->lte_intra.tac;
            curr_cel->CellInfo.nr.cellIdentityNr.pci = li->lte_intra.serving_cell_id;
          }
          curr_cel++;
        }
      }
...
)
{
    if (cell_info)
    {
        ...
        cell_info->cellInfoType = RIL_CELL_INFO_TYPE_NR;
        cell_info->registered = FALSE;

        if (srv)
        {
            cell_info->registered = qcril_qmi_nas_is_rte_registered(QMI_RIL_RTE_SUB_NR5G);
        }
        if (timestamp)
        {
        ...
        cell_info->CellInfo.nr.cellIdentityNr.tac = INT_MAX;
        cell_info->CellInfo.nr.cellIdentityNr.nrarfcn = -1;
        memset(cell_info->CellInfo.nr.cellIdentityNr.operatorNames.alphaShort, 0, sizeof(cell_info->CellInfo.nr.cellIdentityNr.operatorNames.alphaShort));
        memset(cell_info->CellInfo.nr.cellIdentityNr.operatorNames.alphaLong, 0, sizeof(cell_info->CellInfo.nr.cellIdentityNr.operatorNames.alphaLong));
        // NR SNPN info
        cell_info->CellInfo.nr.cellIdentityNr.snpn_info_valid = FALSE;
...
} // qcril_qmi_nas_initialize_nr5g_ril_cellinfo
