...
// cache handling
#define CLIENT_CACHE_LOCK()                            { client_info_cache_lock_mutex.lock(); }
#define CLIENT_CACHE_UNLOCK()                          { client_info_cache_lock_mutex.unlock(); }
...
static qtimutex::QtiSharedMutex           client_info_cache_lock_mutex;
...
static bool qmi_ril_stay_suspended = false;

static qtimutex::QtiSharedMutex qcril_ims_client_cache_lock_mutex;

WakeLock clientWakeLock(QCRIL_PRE_CLIENT_INIT_WAKE_LOCK_NAME);
...
static void qmi_ril_qmi_client_pre_initialization_get(uint8_t status);
...
RIL_Errno qcril_qmi_init_core_client_handles( void )
{
  ...
  do
  {
        if ( qmi_ril_is_multi_sim_feature_supported() )
        {
            ...
            static const property_id_type id_mapping[] = {PERSIST_VENDOR_RADIO_MSIM_STACKID_0,
                    PERSIST_VENDOR_RADIO_MSIM_STACKID_1};
            static constexpr auto id_mapping_size = sizeof(id_mapping)/sizeof(id_mapping[0]);
            stack_id = QCRIL_MODEM_MAX_STACK_ID;
            int int_config;
            if (cur_proc_instance < id_mapping_size &&
                    qcril_config_get(id_mapping[cur_proc_instance], int_config) == E_SUCCESS)
            {
                QCRIL_LOG_INFO("[MSIM] instance: %d, stack id read from db: %d",
                        (int) cur_proc_instance, int_config);
                stack_id = int_config;
            }
            QCRIL_LOG_INFO( "[MSIM].. stack id mapping  %s - %u",
                    qcril_instance_id_name(cur_proc_instance), (unsigned)stack_id );
        ...
        if (client_err)
        {
          ...
          QCRIL_LOG_INFO("qmi_client_init_instance returned failure(%d) for RFRPE ",
                          client_err);
        ...
        for ( idx = 0; idx < QCRIL_QMI_CLIENT_MAX; idx ++ )
        {
                  ...
                  idx != QCRIL_QMI_CLIENT_VOICE))
            {
                ...
                // QMI FWK will invoke the registered error cb in both QMI_NO_ERR and
                // QMI_SERVICE_ERR cases.
                if( client_err == QMI_NO_ERR || client_err == QMI_SERVICE_ERR )
                {
        ...
        if(client_info.client_state[QCRIL_QMI_CLIENT_DMS] == QCRIL_QMI_SERVICE_CONNECTED)
        {
 ...
 /* check if the client is initialized already */
  ...
  if( svc_type < QCRIL_QMI_CLIENT_LAST )
  {
    if (NULL != client_info.qmi_svc_clients[svc_type])
    {
      ...
      {
        QCRIL_LOG_ERROR("Failed to send async qmi msg 0x%02x w/%d",
                         msg_id, qmi_error);
      }
      else
      {
...
)
{
  ...
  if( svc_type < QCRIL_QMI_CLIENT_LAST )
  {
    if (NULL != client_info.qmi_svc_clients[svc_type])
    {
      ...
      if (qmi_error!= QMI_NO_ERR )
      {
        QCRIL_LOG_ERROR("Failed to send sync qmi msg 0x%02x w/%d",
                         msg_id, qmi_error);
      }
      else
      {
...
)
{
  ...
  else
  {
    if (NULL != client_info.qmi_svc_clients[svc_type])
    {
      ...
      {
        QCRIL_LOG_ERROR("Response not populated: "
            "qmi_client_send_msg_sync_with_shm failed");
        return ril_err;
      }
...
RIL_Errno qcril_qmi_client_dsds_bind_to_subscription( RIL_SubscriptionType sub_num )
{
  ...
  static const property_id_type id_mapping[] = {PERSIST_VENDOR_RADIO_MSIM_STACKID_0,
        PERSIST_VENDOR_RADIO_MSIM_STACKID_1};
  static constexpr auto id_mapping_size = sizeof(id_mapping)/sizeof(id_mapping[0]);
  if (inst_id < id_mapping_size &&
        qcril_config_set(id_mapping[inst_id], sub_num) == E_SUCCESS)
  {
      QCRIL_LOG_INFO(".. [MSIM] instance id: %d - stack id: %d", inst_id, sub_num);
  }
  else
  {
...
//===========================================================================
// qmi_ril_set_stay_suspended
//===========================================================================
void qmi_ril_set_stay_suspended(bool stay_suspended)
{
    QCRIL_LOG_FUNC_ENTRY("stay_suspended = %d", stay_suspended);
    qmi_ril_enter_critical_section();
    qmi_ril_stay_suspended = stay_suspended;
    qmi_ril_leave_critical_section();
    if (!stay_suspended) {
        qmi_ril_gen_operational_status_type cur_state = qmi_ril_get_operational_status();
        QCRIL_LOG_INFO("cur_state = %d", (int) cur_state);
        if (cur_state == QMI_RIL_GEN_OPERATIONAL_STATUS_SUSPENDED)
        {
            // acquire lock and register up
...
void qmi_ril_suspended_main_threaded(void * param)
{
    ...
    qmi_ril_enter_critical_section();
    bool stay_suspended = qmi_ril_stay_suspended;
    qmi_ril_leave_critical_section();

    QCRIL_LOG_DEBUG("stay_suspended = %d", stay_suspended);
    if (stay_suspended)
    {
        // Release the wake lock and stop further processing.
        qmi_ril_qmi_client_pre_initialization_release();
    }
    else
    {
        if (!qcril_qmi_nas_modem_power_is_mdm_shdn_in_apm() ||
              1 == PeripheralMgr::getInstance().getPrimaryModemVotingState())
        {
            qcril_qmi_register_for_up_event();
        }
    }
...
void qmi_ril_wave_modem_status(void)
{
    ...
    auto msg = std::make_shared<VoiceImsWaveRadioStateMessage>();
...
void qmi_ril_send_modem_restart_notification(char* reason) {
    QCRIL_LOG_FUNC_ENTRY();
    std::string reasonStr = reason ? reason : MODEM_RESTART_REASON_DEFAULT;
    auto modemRestartMsg = std::make_shared<RilUnsolModemRestartMessage>(reasonStr);
    Dispatcher::getInstance().dispatchSync(modemRestartMsg);
    QCRIL_LOG_FUNC_RETURN();
}
...
void qmi_ril_pre_resume_main_threaded(void * param)
{
    ...
    {
        if (PeripheralMgr::getInstance().isSecondaryModemPresent())
        {
            PeripheralMgr::getInstance().voteUpSecondaryModem();
        }
    }
...
{
  int srv_index;
  bool is_all_clients_down = false;
  ...
  if(params_ptr)
  {
    ...
    if (client_info.num_of_active_clients)
    {
      ...
      if (!client_info.num_of_active_clients)
      {
          is_all_clients_down = true;
      }
    }
    CLIENT_CACHE_UNLOCK();
    if (is_all_clients_down)
    {
      qmi_ril_qmi_client_pre_initialization_acquire();
      qmi_ril_send_modem_restart_notification(NULL);
      auto ssr_msg = std::make_shared<SsrIndicationMessage>(true);
      Dispatcher::getInstance().dispatchSync(ssr_msg);
      qmi_ril_enter_suspend();
    }
  }
...
void qmi_ril_qmi_client_pre_initialization_init()
{
    ...
    status = qcril_qmi_nas_is_non_modem_build_id();
    qmi_ril_enter_critical_section();
    qmi_ril_qmi_client_pre_initialization_get(status);
    qmi_ril_leave_critical_section();
...
void qmi_ril_qmi_client_pre_initialization_acquire()
{
       ...
       TRUE == qmi_ril_qmi_client_pre_init_lock_acq_decision)
    {
        ...
        clientWakeLock.acquire();
        qmi_ril_qmi_client_pre_initialization_lock_held = TRUE;
        QCRIL_LOG_ESSENTIAL("wake lock acquire");
...
//===========================================================================
void qmi_ril_qmi_client_pre_initialization_get(uint8_t status)
{
    int temp_len;
    ...
    if((!strncmp(baseband, "apq", 3)) || (TRUE == status))
    {
    ...
    QCRIL_LOG_DEBUG( "qmi_ril_qmi_client_pre_init_lock_acq_decision=%d", qmi_ril_qmi_client_pre_init_lock_acq_decision);
    if ( temp_len > 0 )
    {
