...
template <>
Marshal::Result Marshal::write<RIL_IMS_MsimAdditionalCallInfo>(
    const RIL_IMS_MsimAdditionalCallInfo &arg) {
  WRITE_AND_CHECK(arg.additionalCode);
  return Result::SUCCESS;
}

template <>
Marshal::Result Marshal::read<RIL_IMS_MsimAdditionalCallInfo>(
    RIL_IMS_MsimAdditionalCallInfo &arg) const {
  READ_AND_CHECK(arg.additionalCode);
  return Result::SUCCESS;
}

template <>
Marshal::Result Marshal::release<RIL_IMS_MsimAdditionalCallInfo>(RIL_IMS_MsimAdditionalCallInfo &arg) {
  release(arg.additionalCode);
  return Result::SUCCESS;
}

template <>
Marshal::Result Marshal::write<RIL_IMS_AudioQuality>(const RIL_IMS_AudioQuality& arg) {
...
template <>
Marshal::Result Marshal::read<RIL_IMS_AudioQuality>(RIL_IMS_AudioQuality& arg) const {
  READ_AND_CHECK(arg.codec);
...
template <>
Marshal::Result Marshal::release<RIL_IMS_AudioQuality>(RIL_IMS_AudioQuality& arg) {
  release(arg.codec);
...
Marshal::Result Marshal::write<RIL_IMS_CallInfo>(const RIL_IMS_CallInfo& arg) {
  ...
  WRITE_AND_CHECK(arg.imsAudioQuality, 1);
  return Result::SUCCESS;
}
...
Marshal::Result Marshal::read<RIL_IMS_CallInfo>(RIL_IMS_CallInfo &arg) const {
    ...
    sz = 0;
    RUN_AND_CHECK(readAndAlloc(arg.imsAudioQuality, sz));
    if (sz != 1 && sz != 0) {
      return Result::FAILURE;
    }
    return Result::SUCCESS;
