...
#define WRITE_AND_CHECK_OPTIONAL(valid, value) (({ \
                bool _valid = !!valid; \
                auto &_value = value; \
                RUN_AND_CHECK(writeValid(_valid, _value)); \
                Result::SUCCESS; \
            }))

#define READ_AND_CHECK_OPTIONAL(valid, value) (({ \
                bool _valid = false; \
                auto &_value = value; \
                RUN_AND_CHECK(readValid(_valid, _value)); \
                auto &destvalid = (valid); \
                destvalid = (decltype(destvalid))_valid; \
                Result::SUCCESS; \
            }))
...
class Marshal {
    ...
    public:
        ...
        template <typename T>
        Result writeValid(bool valid, const T& arg) {
                WRITE_AND_CHECK(valid);
                if (valid) WRITE_AND_CHECK(arg);
                return Result::SUCCESS;
        }
        template <typename T>
        Result readValid(bool &valid, T&arg) const {
                READ_AND_CHECK(valid);
                if (valid) READ_AND_CHECK(arg);
                return Result::SUCCESS;
        }
    ...
    public:
        ...
        size_t availableBytes() const {
            ...
            if (RIL_unlikely(current_pos > size)) {
                current_pos = size;
                assert(0);
            }
         ...
         */
        template <typename T, std::size_t SZ>
        Result release(T (&arg)[SZ]) {
            return release<T>(arg, SZ);
        }
...
template <>
Marshal::Result Marshal::read<std::string>(std::string &out) const;
...
template <>
inline Marshal::Result Marshal::read<std::string>(std::string &out) const {
    uint32_t length = 0;
...
inline Marshal::Result Marshal::read<char>(char *&out) const{
    ...
    if (inlen == -1) {
        ...
        length = inlen;
        val = new(std::nothrow) char[length + 1]{};
        if (!val) return Result::FAILURE;
...
template <>
inline Marshal::Result Marshal::write<std::string>(const std::string &str) {
