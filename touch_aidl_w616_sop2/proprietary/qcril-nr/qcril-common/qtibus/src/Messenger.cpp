...
class MessengerModule: public Module {
    public:
        ...
        void handleOutgoingMessages( std::shared_ptr<IPCMessage> msg) {
            ...
            if (msg) {
                if (!msg->getIsRemote()) {
                    message_id_ref msgId = msg->get_message_id();
                    DEBUG_LOG( << "msgId: " << &msgId );
                    DEBUG_LOG( << "idx for msgId: " << msgId.get_name() << ": " << msgId.get_idx());
        ...
        void proxyRegisterForMessage(message_id_ref msgId) {
            DEBUG_LOG( << "Registering with Dispatcher. msgId: " << &msgId );
            DEBUG_LOG( << "idx for msgId: " << msgId.get_name() << ": " << msgId.get_idx());
...
void MessengerImpl::localMessageDelivery(std::string msgIdString, std::string payload) {
    message_id_p msgId = REG_MSG_Q(msgIdString); //Query to check if messsage is registered
    if (msgId) {
...
void MessengerImpl::localRegisterMessage(pid_t sender, std::string msgIdString) {
    message_id_p msgid = REG_MSG_Q(msgIdString); //Query to check if msg is registered
    DEBUG_LOG( << "Registering with dispatcher. Sender: " << sender
            << " Message id: " << msgid << std::endl);
    if (nullptr != msgid) { //drop unregistered messages
...
void MessengerImpl::registerForMessage(message_id_ref msgid, Messenger::MsgConstructor_t cnstrct,
        bool reRegister) {
...
void MessengerImpl::deliverMessage(message_id_ref msgid, std::string payload) {
...
void Messenger::registerForMessage(message_id_ref msgid, Messenger::MsgConstructor_t cnstrct)
{
    if (impl) {
...
void Messenger::deliverMessage(message_id_ref msgid, std::string payload) {
    if (impl) {
