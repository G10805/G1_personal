...
static inline const char *qcril_instance_id_name(qcril_instance_id_e_type instance) {
    if (instance > QCRIL_MAX_INSTANCE_ID || instance < 0) {
        instance = QCRIL_MAX_INSTANCE_ID;
    }
    switch (instance) {
...
#define DEFINE_MSG_ID_INFO_L(fname,stringname) \
    message_id_ref fname() { \
        static message_id_info msg_id{stringname}; \
        static message_id_ref local_ref = REG_MSG_N(stringname, msg_id); \
        return local_ref; }

#define DECLARE_MSG_ID_INFO_L(name) message_id_ref name()


#define QCRIL_DECLARE_LEGACY_EVENT(name, cb, group, flags) \
DECLARE_MSG_ID_INFO_L(get_id_##name); \
...
#define QCRIL_DEFINE_LEGACY_EVENT(name, cb, group, flags) \
    DEFINE_MSG_ID_INFO_L(get_id_##name, FULL_EVENT_NAME(#name)) \
    qcril_event_id_t qcril_evt_id_##name {group, cb, REG_MSG_N(FULL_EVENT_NAME(#name), get_id_##name()), flags, FULL_EVENT_NAME(#name)}; \
...
int qcril_setup_timed_callback( qcril_instance_id_e_type instance_id, qcril_modem_id_e_type modem_id, RIL_TimedCallback callback,
                                   const struct timeval *relativeTime, uint32_t *timer_id );
