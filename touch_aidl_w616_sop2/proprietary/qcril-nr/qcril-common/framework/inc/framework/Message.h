...
inline Message::Message(const char *name, message_id_ref h, TimeKeeper::millisec timeout):
        m_h(h), mName(name), mCallback(nullptr), m_next_hop(nullptr), mExpiryTimer(timeout)
  {}

inline Message::Message(message_id_ref h, TimeKeeper::millisec timeout):
