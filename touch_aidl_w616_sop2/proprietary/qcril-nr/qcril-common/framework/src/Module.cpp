...
void Module::init() {
  ...
  for (const auto elem : mMessageHandler) {
    Log::getInstance().d("[" +to_string()+"]: Register msg=" + elem.first.get().get_name()+" handler with dispatcher.");

    Dispatcher::getInstance().registerHandler(elem.first, this);
    Dispatcher::getInstance().dump_message_id(elem.first);
  }
}
