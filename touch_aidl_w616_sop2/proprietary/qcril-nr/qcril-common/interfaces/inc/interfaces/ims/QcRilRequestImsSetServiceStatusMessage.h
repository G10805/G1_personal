...
class QcRilRequestImsSetServiceStatusMessage
 ...
 public:
  ...
  bool hasVonrEnabled() {
    return mVonrEnabled.has_value();
  }
  void setVonrEnabled(bool val) {
    mVonrEnabled = val;
  }
  void resetVonrEnabled() {
    mVonrEnabled.reset();
  }
  bool getVonrEnabled() {
    return *mVonrEnabled;
  }
  string dumpVonrEnabled() {
    return std::string(" mVonrEnabled = ") +
  ...
  virtual string dump() {
    return QcRilRequestMessage::dump() + "{" + dumpVolteEnabled() + dumpVideoTelephonyEnabled() +
           dumpWifiCallingEnabled() + dumpUTEnabled() + dumpVonrEnabled() + "}";
  }
};
