...
inline std::string toString(const ConfigItem &o) {
  ...
  if (o == ConfigItem::VOWIFI_ENTITLEMENT_ID) {
    return "VOWIFI_ENTITLEMENT_ID";
  }
  if (o == ConfigItem::B2C_ENRICHED_CALLING_SETTING) {
    return "B2C_ENRICHED_CALLING_SETTING";
  }
...
public:
  ...
  const std::string& getMsisdn() const { return mMsisdn; }
  const std::vector<std::string>& getVirtualLines() const { return mVirtualLines; };
};
...
inline std::string toString(const Priority& o) {
...
inline std::string toString(const Location& o) {
  std::string os;
  os += "{";
  os += ".Radius = " + std::to_string(o.getRadius());
  os += ".Latitude = " + std::to_string(o.getLatitude());
  os += ".Longitude = " + std::to_string(o.getLongitude());
  os += "}";
  return os;
}
...
class CallComposerInfo : public qcril::interfaces::BasePayload {
  ...
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR_VECTOR(uint16_t, Organization);
public:
  CallComposerInfo() {
    ...
    QCRIL_INTERFACE_RESET_OPTIONAL_MEMBER_VAR_VECTOR(Organization);
  }
  CallComposerInfo(const CallComposerInfo &from) {
    ...
    QCRIL_INTERFACE_COPY_OPTIONAL_MEMBER_VAR_VECTOR(from, Organization);
  }
...
inline std::string toString(const CallComposerInfo& o) {
  std::string os;
  os += "{";
  os += ".Call ID = " + std::to_string(o.getId());
  os += ".Priority = " + toString(o.getPriority());
  os += ".ImageUrl = " + o.getImageUrl();
  os += ".Location = " + toString(o.getLocation());
  os += "}";
  return os;
}

enum class SrvDomain {
  NO_SRV,
  CS_ONLY,
  PS_ONLY,
  CS_PS,
...
enum class SmsCallbackModeStatus {
  UNKNOWN,
  ENTER, /*Modem enters Sms Callback Mode*/
  EXIT  /*Modem exits SMs Callback Mode*/
};

inline std::string toString(const SmsCallbackModeStatus& o) {
  if (o == SmsCallbackModeStatus::ENTER) {
    return "SCBM_ENTER";
  }
  if (o == SmsCallbackModeStatus::EXIT) {
    return "SCBM_EXIT";
  }
  return "<invalid>";
}

enum class MultiSimVoiceCapability { UNKNOWN, NONE, DSSS, DSDS, DSDA, PSEUDO_DSDA };

inline std::string toString(const MultiSimVoiceCapability& o) {
  if (o == MultiSimVoiceCapability::NONE) {
    return "NONE";
  }
  if (o == MultiSimVoiceCapability::DSSS) {
    return "DSSS";
  }
  if (o == MultiSimVoiceCapability::DSDS) {
    return "DSDS";
  }
  if (o == MultiSimVoiceCapability::DSDA) {
    return "DSDA";
  }
  if (o == MultiSimVoiceCapability::PSEUDO_DSDA) {
    return "PSEUDO_DSDA";
  }
  return "<invalid>";
}

class MultiSimVoiceCapabilityRespData : public qcril::interfaces::BasePayload {
 private:
  MultiSimVoiceCapability voiceCapability;

 public:
  MultiSimVoiceCapabilityRespData(MultiSimVoiceCapability capability)
      : voiceCapability(capability) {
  }
  MultiSimVoiceCapability getVoiceCapability() {
    return voiceCapability;
  }
  void setVoiceCapability(MultiSimVoiceCapability capability) {
    voiceCapability = capability;
  }
};

class EcnamInfo : public qcril::interfaces::BasePayload {
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR(std::string, Name);
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR(std::string, IconUrl);
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR(std::string, InfoUrl);
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR(std::string, CardUrl);
};

inline std::string toString(const EcnamInfo& o) {
  std::string os;
  os += "{";
  os += ".Name = " + o.getName();
  os += ".IconUrl = " + o.getIconUrl();
  os += ".InfoUrl = " + o.getInfoUrl();
  os += ".CardUrl = " + o.getCardUrl();
  os += "}";
  return os;
}

class PreAlertingCallInfo : public qcril::interfaces::BasePayload {
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR(uint32_t, CallId);
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR(CallComposerInfo, CallComposerInfo);
  QCRIL_INTERFACE_OPTIONAL_MEMBER_VAR(EcnamInfo, EcnamInfo);
};

inline std::string toString(const PreAlertingCallInfo& o) {
  std::string os;
  os += "{";
  os += ".call Id = " + std::to_string(o.getCallId());
  os += ".CallComposerInfo = " + toString(o.getCallComposerInfo());
  os += ".EcnamInfo = " + toString(o.getEcnamInfo());
  os += "}";
  return os;
}

enum class CiWlanNotificationInfo {
  NONE,
  DISABLE_CIWLAN, /* Disable C_IWLAN Feature */
};

inline std::string toString(const CiWlanNotificationInfo &o) {
  if (o == CiWlanNotificationInfo::NONE) {
    return "NONE";
  }
  if (o == CiWlanNotificationInfo::DISABLE_CIWLAN) {
    return "DISABLE_CIWLAN";
  }
  return "<invalid>";
}
