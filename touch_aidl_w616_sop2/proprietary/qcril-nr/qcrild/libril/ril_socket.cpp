...
static void listenCallback(int fd, short flags, void *param) {
    ...
    android::rilEventAddWakeup(p_info->commands_event);
...
int sendResponse(std::shared_ptr<SocketRequestContext> context, RIL_Errno errorCode, std::shared_ptr<Marshal> p) {
    ...
    if (p != nullptr) {
        ...
        Marshal m;
        m.setData(rh + rd);
        return sendResponseRaw(m.data(), m.dataSize());
    } else {
        return sendResponseRaw(responseHeader.data(), responseHeader.dataSize());
    }
}
...
int sendUnsolResponse(int unsolResponse, std::shared_ptr<Marshal> p) {
    ...
    if (p != nullptr) {
        ...
        return sendResponseRaw(unsolResponseHeader.data(), unsolResponseHeader.dataSize());
    }
}
...
static void processCommandsCallback(int fd, short flags, void *param) {
    ...
    if (isDisconnectionRequest || ret == 0 || !(errno == EAGAIN || errno == EINTR)) {
        ...
        /* start listening for new connections again */
        android::rilEventAddWakeup(&s_listen_event);
    }
}
