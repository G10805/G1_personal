...
class RadioServiceImpl : public V1_3::implementation::RadioServiceImpl<T> {
 ...
 protected:
  ...
  virtual void sendRadioClientCapabilities() {
    QCRIL_LOG_DEBUG("SendRadioClientCapabilities");
    bool isAPAssistCap = true;
    std::function<void(const std::vector<rildata::DataCallResult_t>&)> dcListChangedCallback = nullptr;
    dcListChangedCallback = [](const vector<rildata::DataCallResult_t>& dcList) -> void {
        auto indMsg = std::make_shared<rildata::RadioDataCallListChangeIndMessage>(dcList);
        if (indMsg != nullptr) {
            indMsg->broadcast();
        } else {
            QCRIL_LOG_DEBUG("dcListChangedCallback failed to allocate message");
        }
    };
#ifdef QMI_RIL_UTF
...
#else
    if (mRadioResponse == nullptr || mRadioIndication == nullptr) {
        QCRIL_LOG_DEBUG("RespCb or IndCb is nullptr");
        isAPAssistCap = false;
    }
    else {
        QCRIL_LOG_DEBUG("IsAPAssistCap is true");
    }
#endif
    auto msg = std::make_shared<rildata::SetRadioClientCapUnSolMessage>(isAPAssistCap,
        dcListChangedCallback);
    if(msg) {
        msg->broadcast();
        QCRIL_LOG_DEBUG("SetRadioClientCapUnSolMessage Broadcasted");
    }
    QCRIL_LOG_DEBUG("SendRadioClientCapabilities EXIT");
  }
 ...
 public:
  ...
  ::android::hardware::Return<void> emergencyDial(
    ...
    do {
      auto msg = std::make_shared<QcRilRequestCsDialMessage>(this->getContext(serial));
      if (msg == nullptr) {
