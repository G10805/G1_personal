...
#undef OFF
  ...
  virtual void sendResponseForSendDeviceState(int32_t serial, RIL_Errno errorCode) {
    V1_0::RadioResponseInfo responseInfo{ V1_0::RadioResponseType::SOLICITED, serial,
                                          static_cast<V1_0::RadioError>(errorCode) };
    auto respCb = this->getResponseCallback();
    if (respCb) {
 ...
 public:
  ...
  Return<void> hangup(int32_t serial, int32_t gsmIndex) {
    ...
    bool sendFailure = false;
    do {
      auto msg = std::make_shared<QcRilRequestCsHangupMessage>(this->getContext(serial));
      if (msg == nullptr) {
        QCRIL_LOG_ERROR("msg is nullptr");
        sendFailure = true;
        break;
  ...
  Return<void> acceptCall(int32_t serial) {
    ...
    bool sendFailure = false;
    do {
      auto msg = std::make_shared<QcRilRequestCsAnswerMessage>(this->getContext(serial));
      if (msg == nullptr) {
        QCRIL_LOG_ERROR("msg is nullptr");
        sendFailure = true;
        break;
...
#undef OFF
  ...
  Return<void> sendDeviceState(int32_t serial, DeviceStateType deviceStateType, bool state) {
    ...
    auto msg = std::make_shared<RilRequestSendDeviceStateMessage>(
        this->getContext(serial), static_cast<RIL_DeviceStateType>(deviceStateType), state);
    if (msg != nullptr) {
      GenericCallback<QcRilRequestMessageCallbackPayload> cb(
          [this, serial](std::shared_ptr<Message> /*msg*/, Message::Callback::Status status,
                         std::shared_ptr<QcRilRequestMessageCallbackPayload> resp) -> void {
            ...
            this->sendResponseForSendDeviceState(serial, errorCode);
          });
      msg->setCallback(&cb);
      msg->dispatch();
    } else {
      this->sendResponseForSendDeviceState(serial, RIL_E_NO_MEMORY);
    }
    return Void();
  }
