...
void dispatchSetupDataCall_1_6(std::shared_ptr<SocketRequestContext> context, Marshal& parcel) {
    ...
    std::vector<std::string> radioAddresses;
    std::vector<std::string> radioDnses;
    bool trafficDescriptorValid = callParams.trafficDescriptorValid;
    ...
    if ( msg ) {
        if (trafficDescriptorValid) {
            ...
            if (callParams.trafficDescriptor.dnnValid) {
                td.dnn = std::string(callParams.trafficDescriptor.dnn);
            }
            if (callParams.trafficDescriptor.osAppIdValid) {
                td.osAppId = std::vector<uint8_t>(callParams.trafficDescriptor.osAppId,
                    callParams.trafficDescriptor.osAppId + callParams.trafficDescriptor.osAppIdLength);
            }
