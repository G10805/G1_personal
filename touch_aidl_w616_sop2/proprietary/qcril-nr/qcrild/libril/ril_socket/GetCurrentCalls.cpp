            ...
            if (currentCalls) {
              ...
              if (!callList.empty()) {
                for (uint32_t i = 0; i < callList.size(); i++) {
                  ...
                  call.redirNum = nullptr;
                  if (callList[i].hasRedirNum()) {
                    auto num = callList[i].getRedirNum();
                    if (!num.empty()) {
                      call.redirNum = new char[num.size() + 1];
                      if (call.redirNum) {
                        num.copy(call.redirNum, num.size() + 1);
                      }
                    }
                  }
                  call.redirNumPresentation = callList[i].getRedirNumPresentation();
