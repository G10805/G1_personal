  ...
  if (msg) {
    GenericCallback<QcRilRequestMessageCallbackPayload> cb(
        [ctx]([[maybe_unused]] std::shared_ptr<Message> msg,
          ...
          RIL_Errno err = dispatchStatusToRilErrno(status);
...
void dispatchRilRequestGsmSetBroadcastSmsConfig(std::shared_ptr<SocketRequestContext> ctx,
  ...
  Marshal::Result res = p.readAndAlloc(configInfo, rdSize);
  ...
  std::vector<RIL_GSM_BroadcastSmsConfigInfo> configVec(rdSize);
