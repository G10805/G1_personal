...
static void resendLastNITZTimeData() {
    ...
    if (s_lastNITZTimeData != NULL) {
        auto msg = std::make_shared<RilUnsolNitzTimeReceivedMessage>(
              static_cast<const char *>(s_lastNITZTimeData));
...
void sendUnsolicitedResponseOverSocket(int unsolResponse, const void *buffer, size_t bufferLength)
{
    ...
    switch (unsolResponse) {
        // All unsol indications are using Messages now.
        default:
            RLOGE("Unsupported unsolicited response code %d", unsolResponse);
            return;
    }
...
*/
    switch(request) {
        ...
        case RIL_REQUEST_SETUP_DATA_CALL_URSP: return "SETUP_DATA_CALL_URSP";
        ...
        case RIL_REQUEST_SET_DATA_THROTTLING: return "SET_DATA_THROTTLING";
...
extern void QCRIL_dbInit();
extern "C" void RIL_dbInit() {
  QCRIL_dbInit();
}

using QtiRil::Loadable::PluginManager;
using QtiRil::Loadable::PluginRegistry;
using QtiRil::Loadable::LoadableModule;
PluginManager *pluginManager;
qtimutex::QtiRecursiveMutex pluginMutex;
extern "C" void RIL_loadPlugins() {
    PluginRegistry reg;

#ifndef RIL_FOR_MDM_LE
#define LIB_DEFAULT_PATH "/vendor/lib64"
#else
#define LIB_DEFAULT_PATH "/usr/lib"
#endif

    reg.add("libqcrilNrPbmModule.so", std::make_shared<LoadableModule>(
            "libqcrilNrPbmModule.so", LIB_DEFAULT_PATH));
    reg.add("libqcrilNrCellInfoModule.so", std::make_shared<LoadableModule>(
            "libqcrilNrCellInfoModule.so", LIB_DEFAULT_PATH));
    reg.add("libqcrilNrSmsModule.so", std::make_shared<LoadableModule>(
            "libqcrilNrSmsModule.so", LIB_DEFAULT_PATH));
    reg.add("libqcrilDataModule.so", std::make_shared<LoadableModule>(
            "libqcrilDataModule.so", LIB_DEFAULT_PATH));
    reg.add("libqcrilNrImsModule.so", std::make_shared<LoadableModule>(
            "libqcrilNrImsModule.so", LIB_DEFAULT_PATH));
    reg.add("libqcrilNrVoiceModule.so", std::make_shared<LoadableModule>(
            "libqcrilNrVoiceModule.so", LIB_DEFAULT_PATH));
    reg.add("libqcrilNrSocketModule.so", std::make_shared<LoadableModule>(
            "libqcrilNrSocketModule.so", LIB_DEFAULT_PATH));

    std::lock_guard<qtimutex::QtiRecursiveMutex> lock(pluginMutex);
    if (!pluginManager) {
        pluginManager = new PluginManager(reg);
    }
    if (pluginManager) {
        pluginManager->load();
        pluginManager->init();
