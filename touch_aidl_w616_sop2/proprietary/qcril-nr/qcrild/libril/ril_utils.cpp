...
void convertToSocket(RIL_IMS_CallFailCause& out, const qcril::interfaces::CallFailCause& in) {
  switch (in) {
    ...
    case qcril::interfaces::CallFailCause::DSDA_CONCURRENT_CALL_NOT_POSSIBLE:
      out = RIL_IMS_FAILCAUSE_DSDA_CONCURRENT_CALL_NOT_POSSIBLE;
      break;
...
template <>
void convertToSocket(RIL_IMS_MsimAdditionalInfoCode& out,
  ...
  switch (in) {
    case qcril::interfaces::MsimAdditionalInfoCode::CONCURRENT_CALL_NOT_POSSIBLE:
      out = RIL_IMS_MSIM_ADDITIONAL_INFO_CONCURRENT_CALL_NOT_POSSIBLE;
      break;
    case qcril::interfaces::MsimAdditionalInfoCode::NONE:
    default:
      out = RIL_IMS_MSIM_ADDITIONAL_INFO_NONE;
      break;
  }
}
...
void convertToSocket(RIL_IMS_MsimAdditionalCallInfo& out,
  ...
  out.additionalCode = RIL_IMS_MSIM_ADDITIONAL_INFO_NONE;
  if (in.hasAdditionalCode()) {
    convertToSocket(out.additionalCode, in.getAdditionalCode());
  }
}
...
void convertToSocket(RIL_IMS_ComputedAudioQuality& out,
  ...
  switch (in) {
    case qcril::interfaces::ComputedAudioQuality::NO_HD:
      out = RIL_IMS_COMPUTED_AUDIO_QUALITY_NO_HD;
      break;
    case qcril::interfaces::ComputedAudioQuality::HD:
      out = RIL_IMS_COMPUTED_AUDIO_QUALITY_HD;
      break;
    case qcril::interfaces::ComputedAudioQuality::HD_PLUS:
      out = RIL_IMS_COMPUTED_AUDIO_QUALITY_HD_PLUS;
      break;
    case qcril::interfaces::ComputedAudioQuality::NONE:
    default:
      out = RIL_IMS_COMPUTED_AUDIO_QUALITY_NONE;
      break;
  }
}
...
void convertToSocket(RIL_IMS_CallInfo& out, const qcril::interfaces::CallInfo& in) {
  ...
  convertToSocket(out.Codec, static_cast<uint32_t>(in.getCodec()));
  ...
  if (in.hasMsimAdditionalCallInfo()) {
    convertToSocket(out.msimAdditionalCallInfo, in.getMsimAdditionalCallInfoRef());
  }

  out.imsAudioQuality =
      new RIL_IMS_AudioQuality{ .codec = RIL_IMS_CODEC_NONE,
                                .computedAudioQuality = RIL_IMS_COMPUTED_AUDIO_QUALITY_NONE };
  if (in.hasCodec()) {
    convertToSocket(out.imsAudioQuality->codec, in.getCodec());
  }

  if (in.hasComputedAudioQuality()) {
    convertToSocket(out.imsAudioQuality->computedAudioQuality, in.getComputedAudioQuality());
  }
}
...
void convertToSocket(RIL_IMS_ConfigItem& out, const qcril::interfaces::ConfigItem& in) {
  switch (in) {
    ...
    case qcril::interfaces::ConfigItem::VOWIFI_ENTITLEMENT_ID:
      out = RIL_IMS_CONFIG_ITEM_VOWIFI_ENTITLEMENT_ID;
      break;
    case qcril::interfaces::ConfigItem::B2C_ENRICHED_CALLING_SETTING:
      out = RIL_IMS_CONFIG_ITEM_B2C_ENRICHED_CALLING_SETTING;
      break;
...
void convertToSocket(RIL_Qos& rilQos, const rildata::Qos_t& qos) {
    rilQos = {};
    if (qos.epsQos) {
        rilQos.qosType = QOS_TYPE_EPS;
        rilQos.eps.qci = qos.epsQos->qci;
        rilQos.eps.downlink.maxBitrateKbps = qos.epsQos->downlink.maxBitrateKbps;
        rilQos.eps.downlink.guaranteedBitrateKbps = qos.epsQos->downlink.guaranteedBitrateKbps;
        rilQos.eps.uplink.maxBitrateKbps = qos.epsQos->uplink.maxBitrateKbps;
        rilQos.eps.uplink.guaranteedBitrateKbps = qos.epsQos->uplink.guaranteedBitrateKbps;
    } else if (qos.nrQos) {
        rilQos.qosType = QOS_TYPE_NR;
        rilQos.nr.fiveQi = qos.nrQos->fiveQi;
        rilQos.nr.downlink.maxBitrateKbps = qos.nrQos->downlink.maxBitrateKbps;
        rilQos.nr.downlink.guaranteedBitrateKbps = qos.nrQos->downlink.guaranteedBitrateKbps;
        rilQos.nr.uplink.maxBitrateKbps = qos.nrQos->uplink.maxBitrateKbps;
        rilQos.nr.uplink.guaranteedBitrateKbps = qos.nrQos->uplink.guaranteedBitrateKbps;
        rilQos.nr.qfi = qos.nrQos->qfi;
        rilQos.nr.averagingWindowMs = qos.nrQos->averagingWindowMs;
    } else {
        rilQos.qosType = QOS_TYPE_NOINIT;
    }
}

void convertToSocket(RIL_QosFilter& rilQosFilter, const rildata::QosFilter_t& qosFilter) {
    rilQosFilter = {};
    rilQosFilter.localAddressesLength = std::min(MAX_QOS_FILTER_ADDRESSES,
                                                 (int)qosFilter.localAddresses.size());
    for (int i = 0; i < rilQosFilter.localAddressesLength; i++) {
        convertToSocket(rilQosFilter.localAddresses[i], qosFilter.localAddresses[i]);
    }
    rilQosFilter.remoteAddressesLength = std::min(MAX_QOS_FILTER_ADDRESSES,
                                                 (int)qosFilter.remoteAddresses.size());
    for (int i = 0; i < rilQosFilter.remoteAddressesLength; i++) {
        convertToSocket(rilQosFilter.remoteAddresses[i], qosFilter.remoteAddresses[i]);
    }
    if (qosFilter.localPort) {
        rilQosFilter.localPortValid = 1;
        rilQosFilter.localPort.start = qosFilter.localPort->start;
        rilQosFilter.localPort.end = qosFilter.localPort->end;
    }
    if (qosFilter.remotePort) {
        rilQosFilter.remotePortValid = 1;
        rilQosFilter.remotePort.start = qosFilter.remotePort->start;
        rilQosFilter.remotePort.end = qosFilter.remotePort->end;
    }
    rilQosFilter.protocol = (RIL_QosProtocol)qosFilter.protocol;
    if (qosFilter.tos.value) {
        rilQosFilter.tosValid = 1;
        rilQosFilter.tos = *qosFilter.tos.value;
    }
    if (qosFilter.flowLabel.value) {
        rilQosFilter.flowLabelValid = 1;
        rilQosFilter.flowLabel = *qosFilter.flowLabel.value;
    }
    if (qosFilter.spi.value) {
        rilQosFilter.spiValid = 1;
        rilQosFilter.spi = *qosFilter.spi.value;
    }
    rilQosFilter.direction = (RIL_QosFilterDirection)qosFilter.direction;
    rilQosFilter.precedence = qosFilter.precedence;
}

void convertToSocket(RIL_QosSession& rilQosSession, const rildata::QosSession_t& qosSession) {
    rilQosSession = {};
    rilQosSession.qosSessionId = qosSession.qosSessionId;
    convertToSocket(rilQosSession.qos, qosSession.qos);
    rilQosSession.qosFiltersLength = std::min(MAX_QOS_FILTERS, (int)qosSession.qosFilters.size());
    for (int i = 0; i < rilQosSession.qosFiltersLength; i++) {
        convertToSocket(rilQosSession.qosFilters[i], qosSession.qosFilters[i]);
    }
}

void convertToSocket(RIL_TrafficDescriptor& rilTrafficDescriptor, const rildata::TrafficDescriptor_t& trafficDescriptor) {
    rilTrafficDescriptor = {};
    if (trafficDescriptor.dnn) {
        rilTrafficDescriptor.dnnValid = 1;
        convertToSocket(rilTrafficDescriptor.dnn, *trafficDescriptor.dnn);
    }
    if (trafficDescriptor.osAppId) {
        rilTrafficDescriptor.osAppIdValid = 1;
        rilTrafficDescriptor.osAppIdLength = std::min(trafficDescriptor.osAppId->size(), (size_t)MAX_OS_APP_ID_LENGTH);
        memcpy(rilTrafficDescriptor.osAppId, trafficDescriptor.osAppId->data(),
               rilTrafficDescriptor.osAppIdLength);
    }
}

void convertAddressesToSocket(RIL_Data_Call_Response_v11& dataCall, const rildata::DataCallResult_t& entry) {
  dataCall.linkAddressesLength = entry.linkAddresses.size();
  for (int i = 0; i < dataCall.linkAddressesLength; i++) {
      convertToSocket(dataCall.linkAddresses[i].address, entry.linkAddresses[i].address);
      dataCall.linkAddresses[i].properties = (int)entry.linkAddresses[i].properties;
      dataCall.linkAddresses[i].deprecationTime = entry.linkAddresses[i].deprecationTime;
      dataCall.linkAddresses[i].expirationTime = entry.linkAddresses[i].expirationTime;
  }
  if (dataCall.linkAddressesLength == 0) {
    convertToSocket(dataCall.addresses, entry.addresses);
  }
  convertToSocket(dataCall.dnses, entry.dnses);
  convertToSocket(dataCall.gateways, entry.gateways);
  convertToSocket(dataCall.pcscf, entry.pcscf);
}

void convertToSocket(RIL_Data_Call_Response_v11& dataCall, const rildata::DataCallResult_t& entry) {
  dataCall.status = static_cast<int>(entry.cause);
  dataCall.suggestedRetryTime = entry.suggestedRetryTime;
  dataCall.cid = entry.cid;
  dataCall.active = entry.active;
  convertToSocket(dataCall.type, entry.type);
  convertToSocket(dataCall.ifname, entry.ifname);
  convertAddressesToSocket(dataCall, entry);
  dataCall.mtu = entry.mtu;
  dataCall.mtuV4 = entry.mtuV4;
  dataCall.mtuV6 = entry.mtuV6;
  ril::socket::utils::convertToSocket(dataCall.defaultQos, entry.defaultQos);
  dataCall.qosSessionsLength = std::min(MAX_QOS_SESSIONS, (int)entry.qosSessions.size());
  for (int j = 0; j < dataCall.qosSessionsLength; j++) {
    ril::socket::utils::convertToSocket(dataCall.qosSessions[j], entry.qosSessions[j]);
  }
  dataCall.handoverFailureMode = (RIL_HandoverFailureMode)entry.handoverFailureMode;
  dataCall.trafficDescriptorsLength = std::min(MAX_TRAFFIC_DESCRIPTORS,
                                              (int)entry.trafficDescriptors.size());
  for (int j = 0; j < dataCall.trafficDescriptorsLength; j++) {
    ril::socket::utils::convertToSocket(dataCall.trafficDescriptors[j], entry.trafficDescriptors[j]);
  }
}
...
RIL_IMS_MultiSimVoiceCapability convertMultiSimVoiceCapability(
  ...
  switch (voiceCapability) {
    case qcril::interfaces::MultiSimVoiceCapability::DSSS:
      return RIL_IMS_MultiSimVoiceCapability::RIL_IMS_DSSS;
    case qcril::interfaces::MultiSimVoiceCapability::DSDS:
      return RIL_IMS_MultiSimVoiceCapability::RIL_IMS_DSDS;
    case qcril::interfaces::MultiSimVoiceCapability::DSDA:
      return RIL_IMS_MultiSimVoiceCapability::RIL_IMS_DSDA;
    case qcril::interfaces::MultiSimVoiceCapability::PSEUDO_DSDA:
      return RIL_IMS_MultiSimVoiceCapability::RIL_IMS_PSEUDO_DSDA;
    case qcril::interfaces::MultiSimVoiceCapability::NONE:
    default:
      return RIL_IMS_MultiSimVoiceCapability::RIL_IMS_NONE;
  }
}
...
void convertToSocket(RIL_IMS_VerstatVerificationStatus& verStatus,
  ...
  switch (entry) {
    case qcril::interfaces::VerificationStatus::PASS:
      verStatus = RIL_IMS_VERSTAT_VERIFICATION_STATUS_PASS;
      break;
    case qcril::interfaces::VerificationStatus::FAIL:
      verStatus = RIL_IMS_VERSTAT_VERIFICATION_STATUS_FAIL;
      break;
    default:
      verStatus = RIL_IMS_VERSTAT_VERIFICATION_STATUS_NONE;
      break;
  }
}

void convertToSocket(RIL_IMS_CallComposerPriority& priority,
                     const qcril::interfaces::Priority& entry) {
  switch (entry) {
    case qcril::interfaces::Priority::NORMAL:
      priority = RIL_IMS_CALL_COMPOSER_PRIORITY_NORMAL;
      break;
    case qcril::interfaces::Priority::URGENT:
      priority = RIL_IMS_CALL_COMPOSER_PRIORITY_URGENT;
      break;
    case qcril::interfaces::Priority::UNKNOWN:
    default:
      priority = RIL_IMS_CALL_COMPOSER_PRIORITY_INVALID;
      break;
  }
}

void convertToSocket(RIL_IMS_CallComposerLocation& location,
                     const qcril::interfaces::Location& entry) {
  location.radius = entry.getRadius();
  location.latitude = entry.getLatitude();
  location.longitude = entry.getLongitude();
}
...
void convertToSocket(RIL_IMS_CallComposerInfo& ccInfo,
  ...
  if (entry.hasPriority()) {
    convertToSocket(ccInfo.priority, entry.getPriority());
  }

  // vector to char*
  if (entry.getSubject().size()) {
    ccInfo.subject = new char[entry.getSubject().size() + 1]{};
    if (ccInfo.subject) {
      std::copy(entry.getSubject().begin(), entry.getSubject().end(), ccInfo.subject);
    }
  }

  // vector to char*
  if (entry.getOrganization().size()) {
    ccInfo.organization = new char[entry.getOrganization().size() + 1]{};
    if (ccInfo.organization) {
      std::copy(entry.getOrganization().begin(), entry.getOrganization().end(), ccInfo.organization);
    }
  }

  if (entry.hasImageUrl() && !entry.getImageUrl().empty()) {
    convertToSocket(ccInfo.imageUrl, entry.getImageUrlRef());
  }
  ccInfo.location.radius = -1;
  if (entry.hasLocation()) {
    convertToSocket(ccInfo.location, entry.getLocation());
  }
}

void release(RIL_IMS_CallComposerInfo& ccInfo) {
  delete[] ccInfo.subject;
  ccInfo.subject = nullptr;
  delete[] ccInfo.imageUrl;
  ccInfo.imageUrl = nullptr;
  delete[] ccInfo.organization;
  ccInfo.organization = nullptr;
}

template <>
void convertToSocket(RIL_IMS_EcnamInfo& ecnamInfo, const qcril::interfaces::EcnamInfo& entry) {
  if (entry.hasName() && !entry.getName().empty()) {
    convertToSocket(ecnamInfo.name, entry.getNameRef());
  }
  if (entry.hasIconUrl() && !entry.getIconUrl().empty()) {
    convertToSocket(ecnamInfo.iconUrl, entry.getIconUrlRef());
  }
  if (entry.hasInfoUrl() && !entry.getInfoUrl().empty()) {
    convertToSocket(ecnamInfo.infoUrl, entry.getInfoUrlRef());
  }
  if (entry.hasCardUrl() && !entry.getCardUrl().empty()) {
    convertToSocket(ecnamInfo.cardUrl, entry.getCardUrlRef());
  }
}

void release(RIL_IMS_EcnamInfo& ecnamInfo) {
  delete[] ecnamInfo.name;
  ecnamInfo.name = nullptr;
  delete[] ecnamInfo.iconUrl;
  ecnamInfo.iconUrl = nullptr;
  delete[] ecnamInfo.infoUrl;
  ecnamInfo.infoUrl = nullptr;
  delete[] ecnamInfo.cardUrl;
  ecnamInfo.cardUrl = nullptr;
}

void release(RIL_IMS_AutoCallRejectionInfo& autoRej) {
  delete[] autoRej.number;
  autoRej.number = nullptr;
  if (autoRej.ccInfo) {
    release(*autoRej.ccInfo);
  }
  if (autoRej.ecnamInfo) {
    release(*autoRej.ecnamInfo);
  }
}

void release(RIL_IMS_PreAlertingCallInfo& preAlertInfo) {
  if (preAlertInfo.ccInfo) {
    release(*preAlertInfo.ccInfo);
  }
  if (preAlertInfo.ecnamInfo) {
    release(*preAlertInfo.ecnamInfo);
  }
}

void freeSocketResource(RIL_QosSession& qosSession) {
  for (int j = 0; j < qosSession.qosFiltersLength; j++) {
      RIL_QosFilter& qf = qosSession.qosFilters[j];
      for (int k = 0; k < qf.localAddressesLength; k++) {
          if (qf.localAddresses[k]) {
              delete[] qf.localAddresses[k]; qf.localAddresses[k] = NULL;
          }
      }
      for (int k = 0; k < qf.remoteAddressesLength; k++) {
          if (qf.remoteAddresses[k]) {
              delete[] qf.remoteAddresses[k]; qf.remoteAddresses[k] = NULL;
          }
      }
  }
}
...
void freeSocketResource(RIL_TrafficDescriptor& td) {
  if (td.dnn) {
      delete[] td.dnn; td.dnn = NULL;
  }
}

void freeSocketResource(RIL_Data_Call_Response_v11& dataCall) {
  if (dataCall.type) delete []dataCall.type;
  if (dataCall.ifname) delete []dataCall.ifname;
  for (int i = 0; i < dataCall.linkAddressesLength; i++) {
      RIL_LinkAddress& la = dataCall.linkAddresses[i];
      if (la.address) {
          delete[] la.address; la.address = NULL;
      }
  }
  if (dataCall.addresses) delete []dataCall.addresses;
  if (dataCall.dnses) delete []dataCall.dnses;
  if (dataCall.gateways) delete []dataCall.gateways;
  if (dataCall.pcscf) delete []dataCall.pcscf;
  for (int i = 0; i < dataCall.qosSessionsLength; i++) {
    freeSocketResource(dataCall.qosSessions[i]);
  }
  for (int i = 0; i < dataCall.trafficDescriptorsLength; i++) {
    freeSocketResource(dataCall.trafficDescriptors[i]);
  }
}
...
void release(RIL_IMS_CallInfo &arg) {
  ...
  delete arg.msimAdditionalCallInfo;
  arg.msimAdditionalCallInfo = nullptr;
  delete arg.imsAudioQuality;
  arg.imsAudioQuality = nullptr;
}
...
void release(RIL_IMS_ConfigInfo& arg) {
  delete []arg.stringValue;
  arg.stringValue = nullptr;
}
