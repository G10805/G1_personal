/********************************************************************
 Copyright (c) 2016-2017, 2021-2024 Qualcomm Technologies, Inc.
 All Rights Reserved.
...
static inline
bool isFirstShot(MinkSocket *me)
{
...
static int sendv_all_remote(MinkSocket *me, struct iovec *iov, size_t iovLen, int *fds, int num_fds)
{
  ...
  while (iovLen > 0) {
    memscpy(ptr, max_payload, iov->iov_base, iov->iov_len);
    buf_len += iov->iov_len;
...
void* __process_message(void *data)
{
  ...
  if (Object_ERROR_DEFUNCT == err) {
    if (isFirstShot(cxt->conn)) {
      cxt->conn->dispatchErr = err;
      MinkSocket_close(cxt->conn, err);
      MinkSocket_notifyClose(cxt->conn, EVENT_CRASH);
...
int32_t process_message(MinkSocket *me, int sock, void *data, size_t data_len)
{
  ...
  memscpy(cxt->data, data_len, data, data_len);
  me->sock = sock;
  cxt->conn = me;
