/********************************************************************
 Copyright (c) 2016-2017, 2022-2024 Qualcomm Technologies, Inc.
 All Rights Reserved.
    ...
    if (node) {
      ...
      */
      if (me->sockType == VSOCK) {
        MinkSocket_close(node->conn, Object_ERROR_UNAVAIL);
        MinkSocket_notifyClose(node->conn, EVENT_CLOSE);
  ...
  QLIST_NEXTSAFE_FOR_ALL(&me->servedConnectionsList, pqn, pqn_next) {
    ...
    if (node) {
      if (node->node == c_node) {
        QNode_dequeue(pqn);
        MinkSocket_close(node->conn, Object_ERROR_UNAVAIL);
        MinkSocket_notifyClose(node->conn, EVENT_CRASH);
        MinkSocket_detachForwarderAll(node->conn);
        MinkSocket_release(node->conn);
        free(node);
  ...
  do {
    ...
    if (pkt.type == QRTR_TYPE_DEL_CLIENT) {
      ...
      if (pkt.node == me->sockaddr.qaddr.sq_node &&
         ...
         */
        usleep(5);
        MinkSocket_notifyClose(me->conn, EVENT_CLOSE);
                                     ...
                                     Object endpoint, int32_t endpointType)
{
  ...
  CHECK_CLEAN (!Primordial_setCloseNotifierReg(&me->primordial,
                                              CloseNotifier_subRegister));
