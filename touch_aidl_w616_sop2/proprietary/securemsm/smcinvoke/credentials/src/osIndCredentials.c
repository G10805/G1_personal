 ...
 */
static int32_t convert_uuid_to_string(uuid_t *uuid, char *uuidCharArray,
                                   size_t len)
{
    ...
    switch (mapIndexToCaseType[index]) {
        ...
        case case_memcmp:
            if (index == indAppID || index == indPID || index == indUID ||
                index == indVmUUID) {
                if(convert_uuid_to_string((uuid_t *)pValue, uuid_str,
                                           sizeof(uuid_str))) {
                    LOG_ERR("convert uuid to string failed!");
                } else {
                    LOG_MSG("\teKey: %s, \tvalue: %s", indToEKeyChar[index],
                            uuid_str);
                }
                                          ...
                                          uint8_t *val_2)
{
    ...
    T_CHECK(!convert_uuid_to_string((uuid_t *)val_2, uuid_str, sizeof(uuid_str)));
