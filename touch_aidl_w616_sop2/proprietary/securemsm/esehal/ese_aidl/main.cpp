...
static int initEseTrustedApp(void) {
    ...
    if (ese_vendor_id == ESE_VENDOR_NXP) {
        if (sizeof(size_t) == sizeof(uint32_t)) {
            app_name = TRUSTED_APP_NXP_32;
            app_uuid = TRUSTED_APP_UUID_NXP_32;
        } else {
        ...
        if (sizeof(size_t) == sizeof(uint32_t)) {
            app_uuid = TRUSTED_APP_UUID_STM_32;
            app_name = TRUSTED_APP_STM_32;
        } else {
            app_uuid = TRUSTED_APP_UUID_STM;
            app_name = TRUSTED_APP_STM;
        }
    } else if (ese_vendor_id == ESE_VENDOR_STSAFE320) {
        if (sizeof(size_t) == sizeof(uint32_t)) {
            app_uuid = TRUSTED_APP_UUID_STSAFE320_32;
            app_name = TRUSTED_APP_STSAFE320_32;
        } else {
            app_uuid = TRUSTED_APP_UUID_STSAFE320;
            app_name = TRUSTED_APP_STSAFE320;
        }
    } else {
...
#else
std::shared_ptr<QtiSecureElement> service;

extern "C" int LLVMFuzzerInitialize(int* argc, char*** argv) {
    int ret = initEseTrustedApp();
    if (ret != 0) {
        ALOGE("Failed on initalize eSE TA");
    }

    service = ndk::SharedRefBase::make<QtiSecureElement>();
    return 0;
}
