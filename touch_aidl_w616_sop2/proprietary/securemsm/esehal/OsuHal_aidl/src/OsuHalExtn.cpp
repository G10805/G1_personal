...
OsuHalExtn::OsuApduMode OsuHalExtn::isOsuMode(const std::vector<uint8_t>& evt,
  ...
  if (getAtrVendorID() >=  SN300_OS_VERSION_ID) {
    // Returning because SN300 use GP based APDU hence this filtering is not needed.
    return osuSubState;
  }
  ...
  switch (type) {
    ...
    case TRANSMIT:
      ...
      if (pCmdData != NULL) {
        memscpy(&pCmdData[0], evt.size(), evt.data(), evt.size());
        if (isOsuMode()) {
          /*
           * Process transmit request(unwrap APDU, proprietary actions) in OSU
           * mode
           */
          osuSubState = checkTransmit(pCmdData, len, evt);
        } else {
          *len = evt.size();
          osuSubState = NON_OSU_MODE;
        }
      } else {
        if (pCmdData != NULL) *len = evt.size();
        osuSubState = NON_OSU_MODE;
      }
      break;
  }
  ...
  switch (type) {
    ...
    case CLOSE:
      ...
      if (channel == ISO7816_BASIC_CHANNEL && isOsuMode()) {
        if (updateOSUState(OSU_STATE_FINISHED)) {
          ALOGE("Unable to notify TEE");
        }

        if (do_cold_reset_protection(false)) {
...
void OsuHalExtn::checkAndUpdateOsuMode() {
  isJcopOSUMode = isJcopUpdateMode();
}
...
OsuHalExtn::OsuApduMode OsuHalExtn::checkTransmit(
  ...
  } else if (length == ISO7816_SHORT_APDU_HEADER &&
             !memcmp(input, defaultSelectAid, length)) {
      if (isJcopOSUMode) {
        halMode = OSU_BLOCKED_MODE;
      }
      else {
        halMode = OSU_ACK_SELECT_MODE;
      }
