        ...
        case KM_BIGNUM:
            ...
            if (!param.blob.data)
                ...
                for (size_t i = 0; i < param.blob.data_length; ++i)
                    os << std::hex << std::setw(2) << static_cast<int>(param.blob.data[i])
                       << std::dec;
            break;
        case KM_BYTES:
            os << " Bytes: ";
...
bool VerifyAttestation::verify_attestation_record(
        const string& challenge, AuthorizationSet expected_sw_enforced,
        AuthorizationSet expected_tee_enforced, uint32_t expected_keymaster_version,
        keymaster_security_level_t expected_keymaster_security_level,
        uint32_t expected_attestation_version, const keymaster_blob_t& attestation_cert) {
    ...
    keymaster_error_t ret = parse_attestation_record(
            ...
            &att_attestation_security_level, &att_keymaster_version, &att_keymaster_security_level,
            &att_challenge, &att_sw_enforced, &att_tee_enforced, &att_unique_id);
    if (ret) {
        cout << "parse_attestation_record failed: " << ret << endl;
        return false;
    }

    if (expected_attestation_version != (att_attestation_version * 100)) {
        cout << "att_attestation_version failed" << att_attestation_version <<  "expected_attestation_version" << expected_attestation_version <<endl;
        return false;
    }

    if (expected_keymaster_version != (att_keymaster_version / 100)) {
        cout << "att_keymaster_version failed: " << att_keymaster_version
    ...
    if (!att_challenge.data || challenge.length() != att_challenge.data_length) {
        if (att_challenge.data)
            cout << "att_challenge.data_length failed: " << att_challenge.data_length << "challenge.length(): " << challenge.length() << endl;
	else
	    cout << "att_challenge.data is NULL" << endl;
        return false;
    }
    ...
    if (provision_device_id) {
        expected_tee_enforced.push_back(TAG_ATTESTATION_ID_BRAND, Brand.c_str(), Brand.size());
        expected_tee_enforced.push_back(TAG_ATTESTATION_ID_DEVICE, Device.c_str(), Device.size());
        ...
        expected_tee_enforced.push_back(TAG_ATTESTATION_ID_SERIAL, SerialNum.c_str(),
                                        SerialNum.size());
	for (size_t i = 0; i< Imei.dev_id_count && i < MAX_SUBS; i++) {
             if (i == 0 && Imei.dev_id[i].size())
                 expected_tee_enforced.push_back(TAG_ATTESTATION_ID_IMEI, Imei.dev_id[i].c_str(), Imei.dev_id[i].size());
             else if (i == 1 && Imei.dev_id[i].size())
                 expected_tee_enforced.push_back(TAG_ATTESTATION_ID_SECOND_IMEI, Imei.dev_id[i].c_str(), Imei.dev_id[i].size());
        }
        for (size_t i = 0; i< Meid.dev_id_count && i < MAX_SUBS; i++) {
             if (Meid.dev_id[i].size())
                 expected_tee_enforced.push_back(TAG_ATTESTATION_ID_MEID, Meid.dev_id[i].c_str(), Meid.dev_id[i].size());
        }
        expected_tee_enforced.push_back(TAG_ATTESTATION_ID_MANUFACTURER, Manufacturer.c_str(),
                                        Manufacturer.size());
        expected_tee_enforced.push_back(TAG_ATTESTATION_ID_MODEL, Model.c_str(), Model.size());
    }
...
void VerifyAttestation::FreeKeyBlob() {
    if (blob_.key_material) free(const_cast<uint8_t*>(blob_.key_material));
...
keymaster_error_t VerifyAttestation::AttestKey(const string& attest_challenge,
    ...
    if (provision_device_id) {
        attest_params.push_back(TAG_ATTESTATION_ID_BRAND, Brand.c_str(), Brand.size());
        attest_params.push_back(TAG_ATTESTATION_ID_DEVICE, Device.c_str(), Device.size());
        attest_params.push_back(TAG_ATTESTATION_ID_PRODUCT, Product.c_str(), Product.size());
        attest_params.push_back(TAG_ATTESTATION_ID_SERIAL, SerialNum.c_str(), SerialNum.size());

        for (size_t i = 0; i< Imei.dev_id_count && i < MAX_SUBS; i++) {
             if (i == 0 && Imei.dev_id[i].size())
                 attest_params.push_back(TAG_ATTESTATION_ID_IMEI, Imei.dev_id[i].c_str(), Imei.dev_id[i].size());
             else if (i == 1 && Imei.dev_id[i].size())
                 attest_params.push_back(TAG_ATTESTATION_ID_SECOND_IMEI, Imei.dev_id[i].c_str(), Imei.dev_id[i].size());
        }

        for (size_t i = 0; i< Meid.dev_id_count && i < MAX_SUBS; i++) {
             if (Meid.dev_id[i].size())
                 attest_params.push_back(TAG_ATTESTATION_ID_MEID, Meid.dev_id[i].c_str(), Meid.dev_id[i].size());
        }

        attest_params.push_back(TAG_ATTESTATION_ID_MANUFACTURER, Manufacturer.c_str(),
                                Manufacturer.size());
        attest_params.push_back(TAG_ATTESTATION_ID_MODEL, Model.c_str(), Model.size());
    }
...
keymaster_error_t VerifyAttestation::Attestation() {
    ...
    uint32_t expected_keymaster_aidl_version = device->get_keymaster_api_version();
    uint32_t expected_attestation_version = expected_keymaster_aidl_version * 100;

    if (!verify_attestation_record("challenge", sw_enforced(), hw_enforced(),
                                   expected_keymaster_aidl_version, level, expected_attestation_version,
...
keymaster_error_t VerifyAttestation::EcAttestation() {
    ...
    ret = GenerateKey(AuthorizationSetBuilder()
                              .EcdsaSigningKeyCurve(KM_EC_CURVE_P_256)
