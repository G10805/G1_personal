...
def findFids(der_bytes):
    ...
    if ident != 4 or len(fid_value) % 4 != 0:
        if ident == 1 and len(fid_value) == 1:
            (ident, fid_value, _) = readDERItem(der_bytes[fid_val_idx+3:])
        else:
...
def createTOCEntry(file_name):
    ...
    try:
        ...
        if fid_count > MAX_FIDS_SUPPORTED:
            raise Exception("Bad FID count: %d" % fid_count)
...
def uefi(args):
    ...
    for fname in pfm_files:
        ...
        if (file_size == 0) or (file_size > PFM_FILE_MAXSIZE):
            print(
                '\nSkipping "%s" file, it is either an empty file or it crosses the allowed max file size limit' % fname)
            print('Received PFM file size - %d Bytes, Allowed max file size limit - %d Bytes\n' % (
                file_size, PFM_FILE_MAXSIZE))
            continue

        TOC_entry = createTOCEntry(fname)
        if TOC_entry is None:
            print('Skipping %s file, as it contains bad FID range/count' % fname)
            continue
...
def ota(args):
    ...
    if os.path.isfile(partition) == False:
        print("createEmptyPartitions and add header entries")
        createEmptyPartition(partition, 0, (SECTOR_SIZE_IN_BYTES * TOTAL_NUM_OF_SECTORS))
        updatePartition(partition, 0, getHeader())

    print("createEmptyOTAPartition")
    createEmptyPartition(partition, SECTOR_SIZE_IN_BYTES * PARTITION_OFFSETS[1], SECTOR_SIZE_IN_BYTES * (PARTITION_SIZES[1]))
    PART2_OFFSET = PARTITION_OFFSETS[1] * SECTOR_SIZE_IN_BYTES

    pfm_files = getInputFiles(args)

    file_count = 0
    PART2_LICENSE_OFFSET = PART2_OFFSET + OTA_HEADER_SIZE
    PART2_LICENSE_OFFSET_HEAD = PART2_LICENSE_OFFSET
    for fname in pfm_files:
        file_size = os.path.getsize(fname)
        if (file_size == 0) or (file_size > PFM_FILE_MAXSIZE):
            print(
        ...
        # serial number
        (serial_num, der_bytes) = readLargePositiveInteger(der_bytes)
        serial = "".join(["%02x" % x for x in serial_num])
        serial_len = len(serial)//2
        print("serial: %s, serial_len: %d" % (serial, serial_len))

        data = struct.pack('>I', serial_len) + bytes.fromhex(serial) + struct.pack('>I', file_size) + open(fname, "rb").read()
        if ((PART2_LICENSE_OFFSET + len(data)) >= (SECTOR_SIZE_IN_BYTES * TOTAL_NUM_OF_SECTORS)) or  file_count >= MAX_OTA_LICENSES:
            print('OTA STORE MAX size/MAX file_count reached: packed "%d" licenses' % file_count)
            break

        updatePartition(partition, PART2_LICENSE_OFFSET, data)
        file_count += 1
        PART2_LICENSE_OFFSET += len(data)

    digest_bytes = bytes.fromhex(calcOTAPartitionDigest(partition, PART2_LICENSE_OFFSET_HEAD))
    otaheader = getOTAHeader(file_count, digest_bytes)

    if len(otaheader) > OTA_HEADER_SIZE:
        print('OTA Header is larger than allocated size - exiting: %d'% len(otaheader))
        return

    # Add OTA Header at Partition2 start
    updatePartition(partition, PART2_OFFSET, getOTAHeader(file_count, digest_bytes))

    print('Successfully generated a "qweslicstore.bin" with %d pfmfiles(OTA) in the following path. \n' % file_count +
          '%s' % str(args.output_dir))
...
def getPartitionName(file_count):
    name = 'store_%d' % ((file_count + 1) / SLOTS_PER_STORE)
    return name


def getPartitionOffset(file_count):
    offset = ((file_count + 1) % SLOTS_PER_STORE) * SLOT_SIZE
    return offset


def getInputFiles(args):
    if args.licenses:
        return filterUniqueFiles(args.licenses)

    if not os.path.exists(args.input_dir):
        print('Input directory and pfm files are not present.\n' +
              'Generating an empty license store.')
        return []

    file_names = fnmatch.filter(os.listdir(args.input_dir), '*.pfm')
    file_paths = [os.path.join(args.input_dir, fname) for fname in file_names]
    pfm_files = filterUniqueFiles(file_paths)
    num_pfm_files = len(pfm_files)

    if num_pfm_files > MAX_LICENSES_HLOS:
        raise Exception('Maximum of %d license files are supported.' % (MAX_LICENSES_HLOS))

    if num_pfm_files == 0:
        print('No pfm files found in %s.' % args.input_dir)
...
def parseArgs(argv):
    ...
    qlsparser = argparse.ArgumentParser(description='Generates the UEFI or HLOS based license store binary files.',
