...
#include "CTAccessControl.hpp"
#include "CTrustedCameraMemory.hpp"
                                      ...
                                      size_t havenLicenseLen)
{
    ...
    T_CHECK(acquire_wake_lock() == SC_SUCCESS);
    ...
    const ISecureCamera2App_BufInfo &bufInfoRef, const IMemObject &bufRef)
{
    ...
    ProxyBase *lockObj = nullptr;
    ITAccessControl *tACObject = nullptr;
    ...
    // gTVMEnv should be retained its refs because it will trigger Object_release
    // when tvmEnv is destructed.
    Object_retain(gTVMEnv);
    ITEnv tvmEnvOpener(gTVMEnv);
    tACObject = new ITAccessControl();
    lockObj = new ProxyBase();
    ...
    T_CALL(tvmEnvOpener.open(CTAccessControl_UID, *tACObject));
    T_CALL(tACObject->acquireLock(confRules, bufRef, *lockObj));
