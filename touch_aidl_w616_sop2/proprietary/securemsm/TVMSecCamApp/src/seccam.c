...
static Object g_power_service = {0};
static Object g_wake_lock = {0};
...
/* Description: Function to acquire wake lock
 * In:          void
 * Out:         void
 * Return:      SC_SUCCESS on success.
 *              SC_FAILURE on failure.
 */
int32_t acquire_wake_lock()
{
    int32_t ret = SC_FAILURE;

    T_CALL_ERR(ITEnv_open(gTVMEnv, CTPowerService_UID, &g_power_service),
               SC_FAILURE);

    T_CHECK(Object_isNull(g_wake_lock));
    T_CALL_ERR(ITPowerService_acquireWakeLock(g_power_service, &g_wake_lock),
               SC_FAILURE);

exit:
    Object_ASSIGN_NULL(g_power_service);
    return ret;
}

/* Description: Function to release wake lock
 * In:          void
 * Out:         void
 * Return:      void
 */
...
{
    Object_ASSIGN_NULL(g_wake_lock);
}
