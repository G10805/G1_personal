...
#include "cppcose.h"
#include <log/log.h>
#include <remote_provisioning_utils.h>
...
validateAndExtractEekPubAndId(bool testMode, const KeymasterBlob& endpointEncryptionCertChain)
{
    ...
    if (!item || !item->asArray()) {
        ALOGE("Error parsing EEK chain: %s", errMsg.c_str());
        return kStatusFailed;
    }

    const cppbor::Array* certArr = item->asArray();
    if (!certArr) {
        ALOGE("Error: certArr is null");
        return kStatusFailed;
    }
    ...
    for (size_t i = 0; i < certArr->size(); ++i) {
        ...
        if (!cosePubKey) {
            ALOGE("Failed to validate EEK chain: %s", cosePubKey.moveMessage().c_str());
        ...
        if (!testMode && i == 0 &&
            ...
            ALOGE("Unrecognized root of EEK chain");
            return kStatusInvalidEek;
        }
    }
    ...
    if (!eek) {
        ALOGE("Failed to get EEK: %s", eek.moveMessage().c_str());
        return kStatusInvalidEek;
    }
...
validateAndExtractPubkeys(bool testMode, uint32_t numKeys, KeymasterBlob* keysToSign,
    ...
    for (size_t i = 0; i < numKeys; i++) {
        ...
        if (!macedKeyItem || !macedKeyItem->asArray() ||
            ...
            ALOGE("Invalid COSE_Mac0 structure");
            return kStatusFailed;
        }
