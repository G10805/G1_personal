...
int acdb_loader_init_v4(void* acdb_init_data, int acdb_loader_init_version __attribute__((__unused__))) {
...
void acdb_loader_deallocate_cal(int capability, int use_case) {
    ALOGD("%s: capability(%d), use_case(%d)", __func__, capability, use_case);

    Parcel data, reply;
    data.writeInterfaceToken(Iacdb_loader_server::getInterfaceDescriptor());
    data.writeInt32(capability);
    data.writeInt32(use_case);
    remote()->transact(DEALLOCATE_CAL, data, &reply);
    return;
}

void acdb_loader_deallocate_cal_v2(int capability, int use_case, int offset_index) {
    ALOGD("%s: capability(%d), use_case(%d), offset_index(%d)", __func__, capability, use_case, offset_index);

    Parcel data, reply;
    data.writeInterfaceToken(Iacdb_loader_server::getInterfaceDescriptor());
    data.writeInt32(capability);
    data.writeInt32(use_case);
    data.writeInt32(offset_index);
    remote()->transact(DEALLOCATE_CAL_V2, data, &reply);
    return;
}
...
void acdb_loader_send_audio_cal_v6(int acdb_id, int capability, int app_id,
    ...
    data.writeInt32(use_case);
    data.writeInt32(afe_sample_rate);
    data.writeInt32(cal_mode);
    data.writeInt32(offset_index);
    remote()->transact(SEND_AUDIO_CAL_V6, data, &reply);
    return;
}
