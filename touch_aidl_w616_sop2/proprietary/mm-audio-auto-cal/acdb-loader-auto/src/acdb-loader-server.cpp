...
int32_t acdb_loader_server::onTransact(uint32_t code,
    ...
    switch(code) {
        ...
        case DEALLOCATE_CAL: {
            ...
            acdb_loader_deallocate_cal(capability, use_case);
            return NO_ERROR;
        }
        break;

        case DEALLOCATE_CAL_V2: {
            int capability;
            data.readInt32(&capability);
            int use_case;
            data.readInt32(&use_case);
            int offset_index;
            data.readInt32(&offset_index);
            ALOGD("%s DEALLOCATE_CAL: capability(%d), use_case(%d), offset_index(%d)", __func__, capability, use_case, offset_index);
            acdb_loader_deallocate_cal_v2(capability, use_case, offset_index);
            return NO_ERROR;
        }
        break;
        ...
        case SEND_AUDIO_CAL_V6: {
            ...
            acdb_loader_send_audio_cal_v6(acdb_id, capability, app_id, sample_rate, use_case, afe_sample_rate, cal_mode, offset_index);
            return NO_ERROR;
        }
        break;
...
acdb_loader_server::acdb_loader_server() {
    ...
    if (mydata->acdb_handle == NULL) {
        ...
        if (mydata->acdb_get_default_app_type == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_get_default_app_type", __func__, error);
        }
        ...
        if (mydata->acdb_send_common_custom_topology == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_send_common_custom_topology", __func__, error);
        }
        ...
        if (mydata->acdb_deallocate == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_deallocate_ACDB", __func__, error);
        }
        ...
        if (mydata->acdb_deallocate_cal == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_deallocate_cal", __func__, error);
        }

        mydata->acdb_deallocate_cal_v2 = (acdb_deallocate_cal_v2_t)dlsym(mydata->acdb_handle, "acdb_loader_deallocate_cal_v2");
        if (mydata->acdb_deallocate_cal_v2 == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_deallocate_cal_v2", __func__, error);
        }
        ...
        if (mydata->acdb_send_voice_cal_v2 == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_send_voice_cal_v2", __func__, error);
        }
        ...
        if (mydata->acdb_send_voice_cal == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_send_voice_cal", __func__, error);
        }
        ...
        if (mydata->acdb_reload_vocvoltable == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_reload_vocvoltable", __func__, error);
        }
        ...
        if (mydata->acdb_send_audio_cal == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_send_audio_cal", __func__, error);
        }
        ...
        if (mydata->acdb_send_audio_cal_v2 == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_send_audio_cal_v2", __func__, error);
        }
        ...
        if (mydata->acdb_send_audio_cal_v3 == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_send_audio_cal_v3", __func__, error);
        }
        ...
        if (mydata->acdb_send_audio_cal_v4 == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_send_audio_cal_v4", __func__, error);
        }
        ...
        if (mydata->acdb_send_audio_cal_v6 == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_send_audio_cal_v6", __func__, error);
        }
        ...
        if (mydata->acdb_send_listen_device_cal == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_send_listen_device_cal", __func__, error);
        }
        ...
        if (mydata->acdb_send_listen_lsm_cal == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_send_listen_lsm_cal", __func__, error);
        }
        ...
        if (mydata->acdb_send_listen_lsm_cal_v1 == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_send_listen_lsm_cal_v1", __func__, error);
        }
        ...
        if (mydata->acdb_send_anc_cal == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGW("%s: dlsym error %s for acdb_loader_send_anc_cal", __func__, error);
        }
        ...
        if (mydata->acdb_send_tabla_anc_data == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGW("%s: dlsym error %s for send_tabla_anc_data", __func__, error);
        }
        ...
        if (mydata->acdb_get_aud_volume_steps == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGW("%s: dlsym error %s for acdb_loader_get_aud_volume_steps", __func__, error);
        }
        ...
        if (mydata->acdb_send_gain_dep_cal == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_send_gain_dep_cal", __func__, error);
        }
        ...
        if (mydata->acdb_get_remote_acdb_id == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_get_remote_acdb_id", __func__, error);
        }
        ...
        if (mydata->acdb_get_ecrx_device == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_get_ecrx_device", __func__, error);
        }
        ...
        if (mydata->acdb_get_calibration == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_get_calibration", __func__, error);
        }
        ...
        if (mydata->acdb_set_audio_cal_v2 == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_set_audio_cal_v2", __func__, error);
        }
        ...
        if (mydata->acdb_get_audio_cal_v2 == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_get_audio_cal_v2", __func__, error);
        }
        ...
        if (mydata->acdb_send_meta_info == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for send_meta_info", __func__, error);
        }
        ...
        if (mydata->acdb_send_meta_info_list == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for send_meta_info_list", __func__, error);
        }
        ...
        if (mydata->acdb_set_codec_data == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_set_codec_data", __func__, error);
        }
        ...
        if (mydata->acdb_is_initialized == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_is_initialized", __func__, error);
        }
        ...
        if (mydata->acdb_reload_acdb_files == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_reload_acdb_files", __func__, error);
        }
        ...
        if (mydata->acdb_reload_acdb_files_v2 == NULL) {
            if ((error = dlerror()) != NULL)
                ALOGE("%s: dlsym error %s for acdb_loader_reload_acdb_files_v2", __func__, error);
        }
    }
}
...
void acdb_loader_server::acdb_loader_deallocate_cal(int capability, int use_case) {
    std::lock_guard<std::mutex> guard(acdb_loader_mutex);

    if(mydata->acdb_deallocate_cal)
        return mydata->acdb_deallocate_cal(capability, use_case);
}

void acdb_loader_server::acdb_loader_deallocate_cal_v2(int capability, int use_case, int offset_index) {
    std::lock_guard<std::mutex> guard(acdb_loader_mutex);

    if(mydata->acdb_deallocate_cal_v2)
        return mydata->acdb_deallocate_cal_v2(capability, use_case, offset_index);
}
...
void acdb_loader_server::acdb_loader_send_audio_cal_v6(int acdb_id, int capability, int app_id,
    int sample_rate, int use_case, int afe_sample_rate, int cal_mode, int offset_index) {
    std::lock_guard<std::mutex> guard(acdb_loader_mutex);

    if(mydata->acdb_send_audio_cal_v6)
        return mydata->acdb_send_audio_cal_v6(acdb_id, capability, app_id, sample_rate, use_case, afe_sample_rate, cal_mode, offset_index);
}
