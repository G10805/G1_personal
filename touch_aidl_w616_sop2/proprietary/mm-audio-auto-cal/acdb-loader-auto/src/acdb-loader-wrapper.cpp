...
void death_notifier::binderDied(const wp<IBinder>& who __attribute__((__unused__)))
{
...
void acdb_loader_deallocate_cal(int capability, int use_case) {
    ...
    if (!g_als_died) {
        ...
        als->acdb_loader_deallocate_cal(capability, use_case);
    } else {
        return;
    }
}
...
void acdb_loader_deallocate_cal_v2(int capability, int use_case, int offset_index) {
    ...
    if (!g_als_died) {
        ...
        als->acdb_loader_deallocate_cal_v2(capability, use_case, offset_index);
    } else {
        return;
    }
}
...
void acdb_loader_send_audio_cal_v6(int acdb_id, int capability, int app_id,
    ...
    if (!g_als_died) {
        ...
        als->acdb_loader_send_audio_cal_v6(acdb_id, capability, app_id, sample_rate, use_case, afe_sample_rate, cal_mode, offset_index);
    } else {
        return;
    }
}
