...
static audcalparam_command_set_t * audcalparam_cmd_set_alloc(void){
    ...
    if ((cmd_set->cmd_fnb=audcalparam_cmd_alloc("FNB"))==NULL)
        goto exit;
    if ((cmd_set->cmd_ints=audcalparam_cmd_alloc("INTS"))==NULL)
        goto exit;
    if ((cmd_set->cmd_outts=audcalparam_cmd_alloc("OUTTS"))==NULL)
        goto exit;
...
static audcalparam_err_t audcalparam_cmd_set_free(audcalparam_command_set_t *cmd_set){
    ...
    r|=audcalparam_cmd_free(cmd_set->cmd_ints);
    r|=audcalparam_cmd_free(cmd_set->cmd_outts);
...
static audcalparam_err_t audcalparam_usecase_list_init(audcalparam_session_t *h, json_object* jo){
    ...
    for (i=0; i<len; i++) {
        ...
        ALOGD("...acdb_dev_id=%d,app_type=0x%08x,topo_id=0x%08x",acdb_dev_id,app_type,topo_id);
#endif
    }
    return AUDCALPARAM_OK;
}
...
audcalparam_err_t audcalparam_usecase_list_print(audcalparam_session_t *h){
    ...
    for (i=0;i<p_ses->use_cases->use_case_num;i++){
        ...
        ALOGE("Usecase app_type=0x%08x", use_case->app_type);
        ALOGE("Usecase acdb_dev_id=%d", use_case->acdb_dev_id);
        ALOGE("Usecase topo_id=0x%08x", use_case->topo_id);
...
audcalparam_err_t audcalparam_cmd_set_print(audcalparam_session_t *h){
    ...
    audcalparam_cmd_print(p_ses->cmd_set->cmd_ints);
    audcalparam_cmd_print(p_ses->cmd_set->cmd_outts);
...
static audcalparam_err_t audcalparam_cmd_print(audcalparam_command_t *cmd){
    ...
    for (i=0;i<cmd->inst_num;i++){
        ...
        ALOGE("....module_id=0x%08x",cmd_inst->module_id);
        ALOGE("....param_id=0x%08x",cmd_inst->param_id);
        ALOGE("....topo_id=0x%08x",cmd_inst->topo_id);
        ALOGE("....params:");
...
static audcalparam_command_t * audcalparam_cmd_type_init(audcalparam_session_t *h, json_object* jo) {
    ...
    if (type){
        ...
        } else if (!strcmp(type, "INTS")){
            cmd = p_ses->cmd_set->cmd_ints;
        } else if (!strcmp(type, "OUTTS")){
            cmd = p_ses->cmd_set->cmd_outts;
                                                     ...
                                                     audcalparam_cmd_tunnel_cfg_t *tunnel_cmd)
{
    ...
    if (use_case!=NULL){
        ...
        if (tunnel_cmd->topo_id==0)
            return AUDCALPARAM_CMDINSTERR;
        fprintf(stderr,"cmd_inst->topo_id 0x%08x\n", cmd_inst->topo_id);
    }
...
audcalparam_err_t audcalparam_cmd_asrc_ints(audcalparam_session_t *h,
                                        char* cmd_instance_name,
                                        audcalparam_cmd_dir_t dir,
                                        audcalparam_cmd_asrc_ints_t* value,
                                        audcalparam_cmd_base_cfg_t* cfg)
{
#ifdef AUDCALPARAM_DBG
    ALOGD("%s:",__func__);
    ...
    // uint32_t ref_timer_abs_ts_lsm;
    // uint32_t ref_timer_abs_ts_msm;
#define MINOR_VERSION_BYTE_BUF_POS 0
#define DEVICE_DIRECTION_BYTE_BUF_POS (MINOR_VERSION_BYTE_BUF_POS+4)
#define REFERENCE_TIMER_BYTE_BUF_POS (DEVICE_DIRECTION_BYTE_BUF_POS+2)
#define RESYNC_FLAG_BYTE_BUF_POS (REFERENCE_TIMER_BYTE_BUF_POS+2)
#define ACC_DRIFT_VALUE_BYTE_BUF_POS (RESYNC_FLAG_BYTE_BUF_POS+4)
#define REF_TIMER_ABS_TS_LSM_BYTE_BUF_POS (ACC_DRIFT_VALUE_BYTE_BUF_POS+4)
#define REF_TIMER_ABS_TS_MSM_BYTE_BUF_POS (REF_TIMER_ABS_TS_LSM_BYTE_BUF_POS+4)
#undef BYTE_BUF_BYTE_HEADROOM
#define BYTE_BUF_BYTE_HEADROOM 20
#undef CMD_BYTE_BUF_LEN
#define CMD_BYTE_BUF_LEN (REF_TIMER_ABS_TS_MSM_BYTE_BUF_POS+4+BYTE_BUF_BYTE_HEADROOM)
    uint8_t byte_buf[CMD_BYTE_BUF_LEN]={0};
    uint32_t buf_len=sizeof(byte_buf);
    uint8_t *pbufOut=byte_buf;
    ...
    r=audcalparam_check_and_tunnel_cmd_build(h,p_ses->cmd_set->cmd_ints,cmd_instance_name, dir, cfg, &tunnel_cmd);
    if (r)
        goto exit;
    ...
    if (dir==AUDCALPARAM_GET){
        r=audcalparam_serv_con_param_get(p_ses->con_hdl,&tunnel_cmd,pbufOut,&buf_len, (int8_t)1);//get buf parsing
        // parse value GET
        value->minor_version=*(uint32_t*)&pbufOut[MINOR_VERSION_BYTE_BUF_POS];
        value->device_direction=*(uint16_t*)&pbufOut[DEVICE_DIRECTION_BYTE_BUF_POS];
        value->reference_timer=*(uint16_t*)&pbufOut[REFERENCE_TIMER_BYTE_BUF_POS];
        value->resync_flag=*(uint32_t*)&pbufOut[RESYNC_FLAG_BYTE_BUF_POS];
        value->acc_drift_value=*(int32_t*)&pbufOut[ACC_DRIFT_VALUE_BYTE_BUF_POS];
        value->ref_timer_abs_ts_lsm=*(uint32_t*)&pbufOut[REF_TIMER_ABS_TS_LSM_BYTE_BUF_POS];
        value->ref_timer_abs_ts_msm=*(uint32_t*)&pbufOut[REF_TIMER_ABS_TS_MSM_BYTE_BUF_POS];
    }
    else{
        // parse value SET
        *(uint32_t*)&pbufOut[MINOR_VERSION_BYTE_BUF_POS]=value->minor_version;
        *(uint16_t*)&pbufOut[DEVICE_DIRECTION_BYTE_BUF_POS]=value->device_direction;
        *(uint16_t*)&pbufOut[REFERENCE_TIMER_BYTE_BUF_POS]=value->reference_timer;
        *(uint32_t*)&pbufOut[RESYNC_FLAG_BYTE_BUF_POS]=value->resync_flag;
        *(int32_t*)&pbufOut[ACC_DRIFT_VALUE_BYTE_BUF_POS]=value->acc_drift_value;
        *(uint32_t*)&pbufOut[REF_TIMER_ABS_TS_LSM_BYTE_BUF_POS]=value->ref_timer_abs_ts_lsm;
        *(uint32_t*)&pbufOut[REF_TIMER_ABS_TS_MSM_BYTE_BUF_POS]=value->ref_timer_abs_ts_msm;
        buf_len = REF_TIMER_ABS_TS_MSM_BYTE_BUF_POS+4;
        r=audcalparam_serv_con_param_set(p_ses->con_hdl,&tunnel_cmd,pbufOut,&buf_len);
    }
...
audcalparam_err_t audcalparam_cmd_asrc_outts(audcalparam_session_t *h,
                                        char* cmd_instance_name,
                                        audcalparam_cmd_dir_t dir,
                                        audcalparam_cmd_asrc_outts_t* value,
                                        audcalparam_cmd_base_cfg_t* cfg)
{
#ifdef AUDCALPARAM_DBG
    ALOGD("%s:",__func__);
...
#define CMD_BYTE_BUF_LEN (REF_TIMER_ABS_TS_MSM_BYTE_BUF_POS+4+BYTE_BUF_BYTE_HEADROOM)
    ...
    r=audcalparam_check_and_tunnel_cmd_build(h,p_ses->cmd_set->cmd_outts,cmd_instance_name, dir, cfg, &tunnel_cmd);
    if (r)
        goto exit;
...
audcalparam_err_t  audcalparam_util_parser_get_caldata_for_cmd_inst(audcalparam_session_t *h, char* cmd_name, int idx, audcalparam_util_parser_caldata_t *caldata)
{
    ...
    if (strcmp(cmd_name, "BMT")==0){
        ...
        cmd = p_ses->cmd_set->cmd_fnb;
    } else if (strcmp(cmd_name, "INTS")==0){
        cmd = p_ses->cmd_set->cmd_ints;
    } else if (strcmp(cmd_name, "OUTTS")==0){
        cmd = p_ses->cmd_set->cmd_outts;
