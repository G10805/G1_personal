...
/** @brief ints command (AUTO_ASRC_INPUT_TIMING_STATS) */
typedef struct {
    uint32_t minor_version;       /**< Minor version, the supported version is 1*/
    uint16_t device_direction;    /**< Indicates source(TX) or sink(RX)*/
    uint16_t reference_timer;     /**< Values AFE_REF_TIMER_TYPE_AVTIMER*/
    uint32_t resync_flag;         /**< Values 0 for FALSE, 1 for TRUE indicates
                                   device interface just started*/
    int32_t acc_drift_value;      /**< Positive indicates AV timer is faster than device,
                                   and negative is slower*/
    uint32_t ref_timer_abs_ts_lsm; /**< Lower 32bits of ref timer in ms*/
    uint32_t ref_timer_abs_ts_msm; /**< Upper 32bits of ref timer in ms*/
} audcalparam_cmd_asrc_ints_t;

/** @brief ints command (AUTO_ASRC_OUTPUT_TIMING_STATS) */
typedef struct {
    uint32_t minor_version;       /**< Minor version, the supported version is 1*/
    uint16_t device_direction;    /**< Indicates source(TX) or sink(RX)*/
    uint16_t reference_timer;     /**< Values AFE_REF_TIMER_TYPE_AVTIMER*/
...
/**
* Updating(setting) ASRC input drift value.
*
* @param h                     session handle
* @param cmd_instance_name     command instance name to select
*                              which command instance to run from config file
* @param dir                   direction: always set
* @param value                 drift value to set
* @param cfg                   User defined command config
* @return Error code
*/
...
/**
* Updating(setting) ASRC output drift value.
*
* @param h                     session handle
* @param cmd_instance_name     command instance name to select
*                              which command instance to run from config file
