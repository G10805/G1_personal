...
std::vector<IWifiChip::ChipMode> removeNan(
        const std::vector<IWifiChip::ChipMode> &modes) {
    std::vector<IWifiChip::ChipMode> res = modes;
    typedef std::vector<android::hardware::wifi::IWifiChip::ChipMode>::size_type size_t;
    ...
    for (size_t i = 0; i < modes.size(); ++i) {
        ...
        for (size_t j = 0; j < tmpCombination.size(); ++j) {
            ...
            for (size_t k = 0; k < tmpLimitations.size(); ++k) {
                auto tmpTypes = tmpLimitations[k].types;
                for (auto type : tmpTypes) {
                    if (type == IfaceConcurrencyType::NAN_IFACE) {
                        res[i].availableCombinations[j].limits[k].maxIfaces = 0;
                    }
                }
            }
        }
...
std::vector<IWifiChip::ChipMode> WifiFeatureFlags::getChipModesForPrimary() {
    ...
    if (res <= 0) {
        //if persist.vendor.wlan.hal.rpc is true, that means enable WLAN remote
        //control. Currently WLAN remote control doesn't support NAN interface
        //creation, thus remove NAN interface.
        bool disableNan = property_get_bool("persist.vendor.wlan.hal.rpc", false);
        if (disableNan)
            return removeNan(kChipModesPrimary);
