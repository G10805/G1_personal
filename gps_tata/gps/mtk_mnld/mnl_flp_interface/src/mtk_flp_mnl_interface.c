    ...
    switch(prmsg->type) {
        ...
        case MTK_FLP_MSG_HAL_STOP_CMD:
            ...
            if(flp2mnl_msg != NULL) {
                ...
                memcpy(((char*)flp2mnl_msg)+sizeof(MTK_FLP_MNL_MSG_HEADER_T), ((char*)&flp2cnn_msg), sizeof(MTK_FLP_MSG_T));
                LOGD("Send to OFL FLP,message type:0x%02x, len:%zu", flp2cnn_msg.type, (flp2mnl_msg->length + sizeof(MTK_FLP_MNL_MSG_HEADER_T)));
                flp2mnl_hdlr((MTK_FLP_MNL_MSG_T *)flp2mnl_msg);
                free(flp2mnl_msg);
            }
            break;
        ...
        case MTK_FLP_MSG_CONN_SCREEN_STATUS:
            if(fgMnldStatus == MNL_STATUS_OFL_LINK_OPEN_DONE) {
                ...
                if(flp2mnl_msg != NULL) {
                    ...
                    LOGD("Send to OFL FLP,message type:0x%02x, len:%zu", prmsg->type, (flp2mnl_msg->length + sizeof(MTK_FLP_MSG_T)));
                    flp2mnl_hdlr((MTK_FLP_MNL_MSG_T *)flp2mnl_msg);
                    free(flp2mnl_msg);
                }
            } else {
