...
CURLcode curl_easy_download(char* url, char* filename) {
    ...
    if (curl) {
        ...
        mnld_curl_easy_setopt(curl, CURLOPT_URL, url);
        mnld_curl_easy_setopt(curl, CURLOPT_SSL_VERIFYPEER, 0L);
        mnld_curl_easy_setopt(curl, CURLOPT_SSL_VERIFYHOST, 0L);
        mnld_curl_easy_setopt(curl, CURLOPT_NOSIGNAL, 1);
        mnld_curl_easy_setopt(curl, CURLOPT_TIMEOUT, 60);
        //   mnld_curl_easy_setopt(curl, CURLOPT_VERBOSE, 1L);
        mnld_curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, write_data);
        mnld_curl_easy_setopt(curl, CURLOPT_WRITEDATA, fp);
...
CURLcode curl_easy_download_epo(void) {
    ...
    strcat(gps_epo_data_file_name, "/vendor/etc/gnss/");
