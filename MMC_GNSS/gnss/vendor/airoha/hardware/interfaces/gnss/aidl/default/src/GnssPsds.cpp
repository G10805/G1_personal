...
ndk::ScopedAStatus GnssPsds::setCallback(const std::shared_ptr<IGnssPsdsCallback>& callback) {
    ...
    GnssAidlImpl::getInstance()->sendPsdsSetCallback();
...
void GnssPsds::clearCallback() {
    GnssAidlImpl::getInstance()->lock();
    sCallback = nullptr;
    GnssAidlImpl::getInstance()->closePsds();
    GnssAidlImpl::getInstance()->unlock();
}
void GnssPsds::syncPsds() {
    ALOGD("Sync PSDS State: %p", sCallback.get());
    GnssAidlImpl::getInstance()->lock();
