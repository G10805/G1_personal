...
class ByteArray {
 public:
    ...
    ByteArray(const void *data, size_t length);
    bool setData(const void *data, size_t length);
    ~ByteArray();
    const void *data();
    size_t size();
    void dump();
    static void easyDump(const void *data, size_t length, const char *comment);
 private:
    void initMember();
    bool copyMem(const void *data, size_t length);
    void clearMem();
    void *mData;
    size_t mLength;
};