        ...
        struct raw_texture *data_plane_2, struct raw_texture *data_plane_3)
{
	...
	/*
        if (isLogoPause == 1) {
            plane_status[i].dst_x = 1436;
        }else {
            plane_status[i].dst_x = 0;
        }
	*/
        ret = drmModeAtomicAddProperty(req, disp->plane[i].plane_id,
...
*/
    ...
    if (ret < 0) {
        //LOG_ERR("atomic commit failed %s", strerror(errno));
		LOG_ERR("atomic commit failed %s fd id:%d crtc id:%d src[x:%d y:%d w:%d h:%d] dst[x:%d y:%d w:%d h:%d]",
			strerror(errno), raw_data_ptr[i]->fb_id, disp->crtc_id,
			plane_status[i].src_x, plane_status[i].src_y, plane_status[i].src_w, plane_status[i].src_h,
			plane_status[i].dst_x, plane_status[i].dst_y, plane_status[i].dst_w, plane_status[i].dst_h);
        drmModeAtomicFree(req);
        return -1;
    }
...
int drm_alloc_gem(int fd, int width, int height, int fourcc, struct raw_texture * raw_data)
{
    ...
    if (map == MAP_FAILED) {
            LOG_ERR("error: mmap fail : 0x%p, %s", map, strerror(errno));
            return -1;
    }
            ...
            drmModeRes *resources, drmModeConnector *connector)
{
    ...
    LOG_DBG("connector->count_encoders is %d", connector->count_encoders);
   	for (j = 0; j < connector->count_encoders; j++) {
        ...
        LOG_DBG("possible_crtcs is %d, resources->count_crtcs is %d",possible_crtcs, resources->count_crtcs);
