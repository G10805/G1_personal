...
int v4l2_create_buffer(int fd, v4l2_buffer_info *buf_info)
{
	...
	if(v4l2_ioctl(fd, VIDIOC_QUERYCAP, &cap) == 0) {
		if(strlen(cap.driver) != 0) {
			//reserve a buffer for di enqueue last output buffer
			if(buf_info->buffer_type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE &&
				strcmp(cap.driver, "mtk-di") == 0){
				req.count = buf_info->count + 1;
				is_di_output = 1;
			} else {
		...
		LOG_ERR("VIDIOC_QUERYCAP error: %s, return", strerror(errno));
		return -1;
	}

	LOG_DBG("req.count is %d", req.count);
	...
	if(is_di_output) {
		buf_info->count = req.count - 1;
		buf_info->lastbuf_allocated = 1;
	} else {
		buf_info->count = req.count;
	}
	LOG_DBG("buf_info->count is %d", buf_info->count);
...
int v4l2_ss_crop(int fd, struct v4l2_crop *crop)
{
	...
	v4l2_ioctl(fd, VIDIOC_S_SELECTION, &sel);
	...
	struct v4l2_plane_pix_format *plane_fmt, unsigned int *nplanes, int do_align)
{
	...
	case V4L2_PIX_FMT_MT21:
	{
		...
		*nplanes = 2;
		...
		c_plane_size = ((align_h / 2 + 15) & (~15)) * align_w;
...
int v4l2_device_open(v4l2_device_t *device)
{
	int ret;

	if (strlen(device->name) == 0) {
		/*
		* device is NULL, try the capability
		*/
		int i;
		int fd;
		char dev_path[64];
		...
		for (i = 0; i < 100; i++) {
			...
			if(access(dev_path, F_OK) != 0) {
				LOG_ERR("dev_path is %s, can't find device %s",
					dev_path, device->dev_name);
				return -1;
			}

			if (v4l2_open(dev_path, &fd) != 0) {
				LOG_DBG("v4l2_open skip %s", dev_path);
				continue;
			}
			...
			if(v4l2_ioctl(fd, VIDIOC_QUERYCAP, &cap) == 0) {
				if(strlen(cap.driver) != 0) {
					LOG_DBG("cap.driver is %s, device->dev_name is %s",
						cap.driver, device->dev_name);
					if(strcmp(cap.driver, device->dev_name) == 0) {
						device->fd = fd;
						ret = 0;
						break;
					}
		...
		/*
		* device is found when device_path_init
		*/
		do {
...
int v4l2_device_init(v4l2_device_t *device)
{
	...
	ret = v4l2_device_open(device);
	V4L2_CHECK(ret, device, "device open fail");

	MUTEX_INIT(device->mutex_deinit);
	device->capture_queued_buf_count = 0;
	device->output_queued_buf_count = 0;
	//V4L2_CHECK(ret, device, "v4l2_open error");
	ret = v4l2_device_s_fmt(device); // must before create_buffer, becuase may
	V4L2_CHECK(ret, device, "v4l2_device_s_fmt error: %s", strerror(errno));
	ret = v4l2_create_buffer(device->fd, &device->out_buff);
	V4L2_CHECK(ret, device, "v4l2_create_buffer out_buff error: %s", strerror(errno));
	ret = v4l2_create_buffer(device->fd, &device->cap_buff);
	V4L2_CHECK(ret, device, "v4l2_create_buffer cap_buff error: %s", strerror(errno));
...
struct v4l2_buffer* v4l2_device_deqbuf(v4l2_device_t *device, int type)
{
	...
	if (0 != v4l2_deqbuf(device->fd, &buffer)) {
		...
		if (buffer.timecode.userbits[0] == 'L'
			&& buffer.timecode.userbits[1] == 'A'
			&& buffer.timecode.userbits[2] == 'S'
			&& buffer.timecode.userbits[3] == 'T') {
			LOG_DBG("get last_flag");
			device->last_flag = 1;
		} else {
			device->last_flag = 0;
		}
	...
	if(type == V4L2_BUFF_CAPTURE)
		device->capture_queued_buf_count--;
	else
		device->output_queued_buf_count--;
...
int v4l2_device_queue_usrptr(v4l2_device_t *device, int type, void *ptr)
{
	...
	for(i= 0; i< MAX_RETRY_COUNT ; i++){
		ret  = v4l2_device_qbuf(device, buffer);
		if(ret == 0)
			break;
		if(i >= MAX_RETRY_COUNT -2){
			V4L2_DEV_ERR(device, "Crashing Deliberately");
			CRASH_CODE(1);
			exit(0);
		}
	}
...
void v4l2_driver_config(struct module *mod, char *name, unsigned long value)
{
	...
	if (strcmp(name, "name") == 0) {
		...
		strcpy(dev->dev_name, (char *)value);
	}
...
typedef struct DI_ADDR_T{
	void *		va_alloc[VIDEO_MAX_PLANES];
} DI_ADDR_T;

int alloc_last_buf(DI_ADDR_T * alloc_buf, struct v4l2_device *dev)
{
	int i;
	for (i = 0; i < dev->cap_buff.count; i++){
		alloc_buf[i].va_alloc[0] = malloc(ALIGN(dev->cap_buff.width * dev->cap_buff.height, 4096) + 4096 - 1);
		alloc_buf[i].va_alloc[1] = malloc(ALIGN(dev->cap_buff.width * dev->cap_buff.height, 4096) + 4096 - 1);
		dev->cap_buff.buffer[i].m.planes[0].m.userptr = ALIGN((unsigned long)alloc_buf[i].va_alloc[0], 4096);
		dev->cap_buff.buffer[i].m.planes[1].m.userptr = ALIGN((unsigned long)alloc_buf[i].va_alloc[1], 4096);
		LOG_DBG("alloc buf[%d], wh[%d, %d], p[0][0x%x, 0x%x], p[1][0x%x, 0x%x]",
			i, dev->cap_buff.width, dev->cap_buff.height,
			alloc_buf[i].va_alloc[0], dev->cap_buff.buffer[i].m.planes[0].m.userptr,
			alloc_buf[i].va_alloc[1], dev->cap_buff.buffer[i].m.planes[1].m.userptr);
	}
	return 0;
}
int free_last_buf(DI_ADDR_T * alloc_buf, struct v4l2_device *dev)
{
	int i;
	for (i = 0; i < dev->cap_buff.count; i++){
		free(alloc_buf[i].va_alloc[0]);
		free(alloc_buf[i].va_alloc[1]);
	}
	return 0;
}

int v4l2_driver_pause(struct module *mod)
{
	LOG_INFO("Enter");
	struct v4l2_device *dev;
	struct v4l2_buffer *buffer, *prev_buffer;
	struct v4l2_buffer_info *buf_info;
	...
	if (dev->out_buff.waiting_queue)
		queue_clear(dev->out_buff.waiting_queue, shm_free);
	if (dev->cap_buff.waiting_queue)
		queue_clear(dev->cap_buff.waiting_queue, shm_free);

	if(buf_info->lastbuf_allocated) {
		buffer = &buf_info->buffer[buf_info->count];
		buffer->flags |= V4L2_BUF_FLAG_LAST;
		ALOGV("DI queue last buffer");
		V4L2_UNTILL_SUCCESS(v4l2_device_qbuf(dev, buffer));

		while(dev->output_queued_buf_count>0 && dev->capture_queued_buf_count>0){
			v4l2_device_deqbuf_all(dev,V4L2_BUFF_OUTPUT);
			v4l2_device_deqbuf_all(dev,V4L2_BUFF_CAPTURE);
			usleep(1000);
		}
		if(dev->capture_queued_buf_count == 0 && dev->output_queued_buf_count >0) {
			DI_ADDR_T * di_dq_lastbuf;
			di_dq_lastbuf = (DI_ADDR_T *)malloc(sizeof(DI_ADDR_T) * dev->cap_buff.count);
			alloc_last_buf(di_dq_lastbuf, dev); //alloc capture buffer for dequeue output last buffer
			for(i=0; i<dev->cap_buff.count; i++){
				V4L2_UNTILL_SUCCESS(v4l2_device_qbuf(dev, &(dev->cap_buff.buffer[i])));
			}
			while(dev->output_queued_buf_count>0) { //wait until all output buffer dequeued
				v4l2_device_deqbuf_all(dev,V4L2_BUFF_OUTPUT);
				v4l2_device_deqbuf_all(dev,V4L2_BUFF_CAPTURE);
				usleep(1000);
				if(dev->capture_queued_buf_count == 0 && dev->output_queued_buf_count >0){
					for(i=0; i<dev->cap_buff.count; i++){
						if(dev->cap_buff.buff_stat[i]==V4L2_BUFF_STAT_DEQUEUED)
							V4L2_UNTILL_SUCCESS(v4l2_device_qbuf(dev, &(dev->cap_buff.buffer[i])));
					}
				}
			}
			free_last_buf(di_dq_lastbuf,dev);
			free(di_dq_lastbuf);
		}
		dev->out_buff.count = dev->out_buff.count + 1;
	}else{
		v4l2_device_deqbuf_all(dev,V4L2_BUFF_OUTPUT);
		v4l2_device_deqbuf_all(dev,V4L2_BUFF_CAPTURE);
	}
