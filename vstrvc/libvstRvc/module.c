...
void module_set_deinit(module *mod, void (*module_deinit)(struct module*))
{
	mod->module_deinit = module_deinit;
}
...
{
	if (mod->next) {
		MUTEX_LOCK(mod->next->mutex_status);
		if (mod->next->status == MODULE_STATUS_ACTIVE)
			queue_enq(mod->next->q_media, buff);
		else
			shm_free(buff);
		MUTEX_UNLOCK(mod->next->mutex_status);
...
{
	queue_clear(mod->q_media, shm_free);
}
...
int module_restart(module *mod)
{
	if (!(mod->status & MODULE_STATUS_STOP)) {
		MOD_ERR(mod, "status wrong 0x%X", mod->status);
		return -1;
	}

	THREAD_WAIT(mod->thread_main);
...
void module_destroy(module *mod)
{
	...
	THREAD_WAIT(mod->thread_main);
	queue_destroy(mod->q_media);
	MUTEX_DESTROY(mod->mutex_status);
	COND_DESTROY(mod->cond_status);

	if (mod->allocator)
		shm_destroy_allocator(mod->allocator);

	if (mod->module_deinit)
		mod->module_deinit(mod);
	else if (mod->driver_data)
...
int link_pause_modules(link_path *ln)
{
	...
	/*
	* clear media queue after all modules paused
	*/
	mod = ln->head;
	while (mod) {
...
int link_restart_modules(link_path *ln)
{
	module *mod;
	int status;
	int r;
