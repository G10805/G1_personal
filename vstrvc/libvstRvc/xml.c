...
#include "libxml/parser.h"
display_parameters *mDisplayParameters = NULL;
...
int xml_set_display_parameters(void* display_param) {
    mDisplayParameters = (display_parameters*) display_param;
    return 0;
}
...
link_path* xml_create_link(void *handle, char *link_name)
{
	...
	*/
	...
	while (moduleNode) {
		...
		tmp = xmlGetProp(moduleNode, BAD_CAST "name");
		if (tmp) {
			mod = module_create_by_name((char*)tmp);
			xmlFree(tmp);
		}
		if (!mod) {
		...
		*/
		...
		if (dev_name) {
			module_config(mod, "dev_name", (unsigned long)dev_name);
			dev_path = device_get_path(dev_name);
			if (dev_path) {
				module_config(mod, "name", (unsigned long)dev_path);
			} else {
				LOG_WARN("device cannot found: %s, but will try capability", dev_name);
				//return NULL;
			}
			xmlFree((xmlChar *)dev_name);
		}
		...
		while (configNode) {
			...
			config_name = xmlGetProp(configNode, BAD_CAST "name");
			config_type = xmlGetProp(configNode, BAD_CAST "type");
			config_value = xmlGetProp(configNode, BAD_CAST "value");
			if (config_name && config_type && config_value) {
				module_config(mod, (char*)config_name,
					string_to_value((char*)config_type, (char*)config_value)
				);
				xmlFree(config_name);
				xmlFree(config_type);
				xmlFree(config_value);
			}
			configNode = configNode->next;
		}
		...
		if(mDisplayParameters != NULL && mDisplayParameters->initialized){
			if (strcmp((char*)xmlGetProp(moduleNode, BAD_CAST "name"), "v4l2_mdp") == 0){
				module_config(mod, "cap_buff.width", mDisplayParameters->mDisplayWidth);
				module_config(mod, "cap_buff.height", mDisplayParameters->mDisplayHeight);
			}
			else if (strcmp((char*)xmlGetProp(moduleNode, BAD_CAST "name"), "list2va") == 0){
				module_config(mod, "width", mDisplayParameters->mDisplayWidth);
				module_config(mod, "height", mDisplayParameters->mDisplayHeight);
			}
			else if (strcmp((char*)xmlGetProp(moduleNode, BAD_CAST "name"), "drm") == 0){
				module_config(mod, "src_w", mDisplayParameters->mDisplayWidth);
				module_config(mod, "src_h", mDisplayParameters->mDisplayHeight);

				module_config(mod, "dst_w", mDisplayParameters->mDisplayWidth);
				module_config(mod, "dst_h", mDisplayParameters->mDisplayHeight);
				
				module_config(mod, "dst_x", mDisplayParameters->mXStart);
				module_config(mod, "dst_y", mDisplayParameters->mYStart);
			}
		}
		moduleNode = moduleNode->next;
	}
...
static link_path * xml_auto_camera_create_link(void *handle)
{
	...
	while (cameraNode) {
		...
		dev_name = xmlGetProp(cameraNode, BAD_CAST "dev_name");
		if(device_get_path((char*)dev_name)) {
			xmlFree(dev_name);
			link_name = xmlGetProp(cameraNode, BAD_CAST "link_name");
			if (!link_name)
				return NULL;
			path = xml_create_link(handle, (char *)link_name);
			xmlFree(link_name);
			return path;
		}
		if (dev_name)
...
link_path* xml_create_camera(void *handle)
{
	...
	tmp = xmlNodeGetContent(linkNode);
	if (xmlStrcmp(tmp, BAD_CAST "camera_auto_detection")==0) {
		path = xml_auto_camera_create_link(handle);
		xmlFree(tmp);
		return path;
	}
	xmlFree(tmp);
	tmp = xmlNodeGetContent(linkNode);
	path = xml_create_link(handle, (char*)tmp);
	xmlFree(tmp);
	return path;
}
