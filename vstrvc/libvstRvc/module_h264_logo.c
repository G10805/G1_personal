...
#ifdef NO_REPEAT
	    mFrameCount++;
        dev->next_pos = dev->current_pos;
        *size = dev->end_pos - dev->current_pos;
        // wait for showing last frame
	/*
        if(mFrameCount > 4) {
            return NULL;
        }
...
static int h264_logo_driver_init(struct module *mod)
{
	...
	if (!dev->fd) {
		MOD_ERR(mod, "open fail: %s, (%d, %s)", dev->name, errno, strerror(errno));
		return -1;
	}
...
static int h264_logo_driver_handle_frame(struct module *mod)
{
	...
	*/
	if (module_get_memory_type(mod, CAPS_MEM_OUTPUT) == MEMORY_ALLOC_SELF) {
        ...
        if (out_frame) {
            ...
            if (in_frame) {
                ...
                memcpy(shm_get_ptr(out_frame), shm_get_ptr(in_frame),
                        in_frame->size);
                module_push_frame(mod, out_frame);
                shm_free(in_frame);
            }
...
#if 0
    switch (screenSticker) {
    case 6 : /*SKODA_MONTE_CARLO*/
    case 5 : /*SKODA_BASIC*/
    case 1 : /*VW_BASIC*/
    case 2 : /*VW_NEW_LOGO*/
    case 3 : /*VW_BEATS*/
    case 4 : /*VW_GTS*/
    case 7 : /*VW_GT*/ {
        if(stickerCount < 10/* initial frames to skip*/ ) {
           usleep(dev->frame_time * 10);
           stickerCount++;
        } else {
           usleep((dev->frame_time)/3);
        }
        break;
    }
    default:
...
int h264_logo_module_init(struct module *mod)
{
	...
	//char screenStickerStr [150] = "\0";
	//property_get("persist.vendor.conf.hmi_startup_screen_sticker", screenStickerStr, "");
	//screenSticker = atoi(screenStickerStr);
